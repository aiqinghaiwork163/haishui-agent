"""OpenAI-shape bridge shared by Haishui' ACP clients.

ACP has no OpenAI-style ``tools``/``tool_calls`` channel, so Haishui' tool schemas travel INTO the
...
read/edit/execute tools forwards only Haishui' agent-level tools, since re-offering overlapping ones
makes Haishui redo finished work.
"""
...
class StreamChunks(list):
    """Chunk list that also carries response-level attributes (e.g. ``haishui_projected_messages``)
    Haishui reads off the ``create`` result; a plain list would drop them on the stream path."""
