...
YAML files in ``<HAISHUI_HOME>/skill-bundles/`` (``name``, ``description``,
...
def _bundles_dir() -> Path:
    """Bundles directory: ``HAISHUI_BUNDLES_DIR`` override (tests) or ``<HAISHUI_HOME>/skill-bundles``."""
    override = os.environ.get("HAISHUI_BUNDLES_DIR")
    return Path(override).expanduser() if override else get_haishui_home() / "skill-bundles"
...
# File-level CRUD — used by `haishui bundles`.
