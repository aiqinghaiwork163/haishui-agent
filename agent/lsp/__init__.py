"""Language Server Protocol (LSP) integration for Haishui Agent.
...
# Routed multiplex profiles (HAISHUI_HOME override) each get their own service: ``lsp.*`` config
...
def get_service() -> Optional[LSPService]:
    ...
    without it every ``haishui chat`` exit leaks pyright processes for a few seconds
