...
a cache problem must never block Haishui startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """``home_path`` as resolved by ``load_haishui_dotenv()``, else ``$HAISHUI_HOME``/``~/.haishui``."""
    if home_path is None:
        from haishui_constants import get_haishui_home

        home_path = get_haishui_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend at ``<haishui_home>/cache/<basename>``::
