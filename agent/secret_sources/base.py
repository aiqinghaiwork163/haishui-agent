...
env-var-shaped values at process startup, AFTER ``~/.haishui/.env`` has loaded
and BEFORE the rest of Haishui reads ``os.environ``. The contract is deliberately
narrow: read-only; startup-time and synchronous (one ``fetch()`` per process per
HAISHUI_HOME, under a registry-enforced wall-clock timeout, no background
...
_SOURCE_ENVIRONMENT = ContextVar("haishui_secret_source_environment", default=None)
...
_GENERIC_REMEDIATION = {
    ErrorKind.NOT_CONFIGURED: "Run `haishui secrets {name} setup` to finish configuration.",
    ErrorKind.BINARY_MISSING: "Run `haishui secrets {name} setup` to install the helper CLI.",
    ErrorKind.AUTH_FAILED: "Credentials rejected — run `haishui secrets {name} setup` to re-authenticate.",
    ErrorKind.AUTH_EXPIRED: "Credentials expired — run `haishui secrets {name} setup` to re-authenticate.",
