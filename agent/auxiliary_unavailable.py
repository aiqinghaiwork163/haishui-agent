...
def _quarantined_nous_error(exc: BaseException) -> BaseException:
    ...
    """
    from haishui_cli.auth import AuthError, get_provider_auth_state
    from haishui_cli.auth_nous import _terminal_quarantine_marker
...
def missing_provider_credentials_message(provider_id: str) -> str:
    ...
    if env_vars:
        ...
        problem, remedy = "no credentials were found", f"Run `haishui auth add {provider_id}` to sign in"
    else:
        problem = "no credentials were found"
    switch = "switch to a different provider with `haishui model`."
...
def record_nous_credential_failure(exc: BaseException) -> str:
    ...
    Logged once per distinct message: WARNING when a real credential failed, DEBUG when Haishui was
