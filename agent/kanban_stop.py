...
and stop with no tool calls; Haishui treats that as a clean exit → ``rc=0`` → dispatcher
...
def kanban_stop_nudge_enabled() -> bool:
    """On when ``HAISHUI_KANBAN_TASK`` is set for the dispatcher-owned worker, unless
    ``HAISHUI_KANBAN_STOP_NUDGE`` disables it. In-process delegate_task children and cron runs
    inherit the env var but own no board task and carry no kanban toolset."""
    if (os.environ.get("HAISHUI_KANBAN_STOP_NUDGE") or "").strip().lower() in {"0", "false", "no", "off"}:
        return False
...
def build_kanban_stop_nudge(
    *,
    ...
    tid = (task_id or os.environ.get("HAISHUI_KANBAN_TASK") or "").strip() or "this task"
    ...
    return (
        "[System: You are a Haishui kanban worker. A plain-text reply is NOT a "
