...
inferred from the atlas shape so Haishui renders both legacy 8-row sheets and
...
class PetState(str, Enum):
    """Animation state a pet can be shown in (Haishui names; Codex rows say ``jumping``/``running`` for ``jump``/``run``)."""
...
# Legacy Haishui/petdex row order (top -> bottom) for the older 8-row, 9-column atlas.
...
# Canonical Haishui names -> accepted row-name aliases in descending preference.
