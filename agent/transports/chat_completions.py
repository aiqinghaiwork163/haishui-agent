...
_XAI_TOOL_SEARCH_ALIAS = "haishui_tool_search"
...
def _rename_tool_search_bridge_for_xai(tools: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, str]]:
    ...
    If a real tool already holds ``haishui_tool_search``, the bridge takes a ``_2``/``_3`` suffix.
    """
...
def _reasoning_config_for_model(model: str, reasoning_config: dict | None) -> dict | None:
    """Clamp Haishui' extended effort set (``ultra``) to the OpenAI-compat wire vocabulary.

    Haishui' internal effort set extends the wire vocabulary with ``ultra`` (the /reasoning command documents
...
def _build_gemini_thinking_config(model: str, reasoning_config: dict | None) -> dict | None:
    """Translate Haishui/OpenRouter-style reasoning config to Gemini thinkingConfig."""
    ...
    # is stricter (low/high). Clamp Haishui' wider effort set to what each
...
class ChatCompletionsTransport(ProviderTransport):
    ...
    def _normalize_tool_call(self, tc: Any) -> ToolCall | None:
        ...
        if tc_function is None or name is None:
            return None
        # Reverse only aliases THIS request emitted; a real ``haishui_tool_search`` tool stays itself.
