"""Unified removal contract for every credential source Haishui reads from.

Readers live in ``agent.credential_pool``; what is unified here is **removal**:
``haishui auth remove <provider> <N>`` must make the entry stay gone across
...
EXTERNAL_LOGINS_NOT_ADOPTED_NOTICE = (
    ...
    "Haishui uses only its own logins; run `haishui auth add <provider>` to add one."
)
...
def adopt_external_logins_enabled() -> bool:
    ...
    Codex and Claude OAuth refresh tokens are single-use and rotate, so once Haishui borrows a CLI's
    token pair the two programs hold one token family and whichever refreshes first logs the other
    out. When the user opts out, Haishui never reads or refreshes those files and says so once per
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    """env:<VAR> — clear from ~/.haishui/.env; hint when the shell exports it."""
    from haishui_cli.config import get_env_path, remove_env_value
    ...
    # .env as utf-8-sig like haishui_cli/config.py: a BOM-sensitive read would
    ...
    if env_in_process and not env_in_dotenv:
        result.hints.extend([
            ...
            f"(not in ~/.haishui/.env).",
            "  Unset it there (shell profile, systemd EnvironmentFile, "
            "launchd plist, etc.) or it will keep being visible to Haishui.",
            f"  The pool entry is now suppressed — Haishui will ignore "
            f"{env_var} until you run `haishui auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_haishui_pkce(provider: str, removed) -> RemovalResult:
    """~/.haishui/.anthropic_oauth.json is ours — delete it outright."""
    from haishui_constants import get_haishui_home

    result = RemovalResult()
    oauth_file = get_haishui_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Haishui Anthropic OAuth credentials")
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    result.hints.append(
        "Run `haishui model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    result.hints.extend([
        ...
        "Run `haishui auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `haishui auth add copilot` to re-enable if needed.",
    ])
...
_REGISTRY: List[RemovalStep] = [
    ...
    RemovalStep(
        ...
        remove_fn=_suppress_only(
            ...
            "Run `haishui auth add anthropic` to re-enable if needed.",
        ),
        description="~/.claude/.credentials.json",
    ),
    RemovalStep(
        provider="anthropic", source_id="haishui_pkce",
        remove_fn=_remove_haishui_pkce,
        description="~/.haishui/.anthropic_oauth.json",
    ),
    RemovalStep(
    ...
    RemovalStep(
        ...
        remove_fn=_suppress_only(
            ...
            "Run `haishui auth add qwen-oauth` to re-enable if needed.",
        ),
