...
try:
    import haishui_cli as _haishui_cli

    _HAISHUI_VERSION = str(_haishui_cli.__version__)
except Exception:
    _HAISHUI_VERSION = "0.0.0"
_API_CLIENT = f"haishui-agent/{_HAISHUI_VERSION}"  # client context per Gemini's partner-integration guidance
...
_FREE_TIER_GUIDANCE = (
    ...
    "Haishui typically makes 3-10 API calls per user turn, so the free tier is exhausted in a handful of "
...
_STANDARD_KEY_GUIDANCE = (
    ...
    "generativelanguage.googleapis.com). Then update GEMINI_API_KEY / GOOGLE_API_KEY in ~/.haishui/.env, "
...
_MISSING_KEY_ERROR = (
    ...
    "environment / ~/.haishui/.env (get one at https://aistudio.google.com/app/apikey), or run `haishui setup` to "
...
class GeminiAPIError(Exception):
    """Error shape compatible with Haishui retry/error classification."""
...
class GeminiNativeClient:
    ...
    # (No HAISHUI_SKIP_ASYNC_WRAP — the async path has a real conversion, AsyncGeminiNativeClient.)
