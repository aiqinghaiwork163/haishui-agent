...
_OBSERVABLE_METRIC_NAMES = (
    "haishui.gateway.up", "haishui.gateway.state", "haishui.gateway.active_agents", "haishui.gateway.busy",
    "haishui.gateway.drainable", "haishui.gateway.restart_requested", "haishui.gateway.background_work",
    "haishui.gateway.background_delegations", "haishui.platform.up", "haishui.platform.degraded",
    "haishui.cron.scheduler.heartbeat_age_seconds", "haishui.cron.scheduler.last_success_age_seconds",
    "haishui.cron.scheduler.catch_up_occurrences", "haishui.cron.jobs.enabled", "haishui.cron.jobs.running",
    "haishui.cron.jobs.overdue",
)
...
class GatewayHealthExportRuntime:
    ...
    def shutdown(self) -> None:
        ...
        if closeables:
            worker = threading.Thread(target=_close, name="haishui-gateway-health-export-shutdown", daemon=True)
            worker.start()
...
def _read_runtime_snapshot(config: Dict[str, Any]):
    ...
    try:
        ...
        for name, read in (
            ("haishui.gateway.background_work", _read_background_work_count),
            ("haishui.gateway.background_delegations", _read_background_delegations_count),
        ):
...
def _start_metric_provider(config: Dict[str, Any], sdk: Dict[str, Any]) -> Any:
    ...
    meter = provider.get_meter("haishui.gateway.health")
...
class GatewayDiagnosticLogStreamer(EmitterStreamer):
    ...
    def __init__(self, config: Dict[str, Any], sdk: Dict[str, Any]):
        ...
        self._logger = self._provider.get_logger("haishui.gateway.diagnostics")
        self._sdk = sdk
        self.exported = 0
...
def start_gateway_health_export(config: Dict[str, Any]) -> GatewayHealthExportRuntime:
    ...
    if metrics_on or diagnostics_on:
        try:
            ...
            logger.warning("monitoring.gateway_health_export.enabled but OTLP SDK is unavailable; install 'haishui-agent[otlp]'", exc_info=True)
    ...
    if diagnostics_on:
        try:
            ...
            thread = threading.Thread(target=_run, name="haishui-gateway-health-export", daemon=True)
            thread.start()
