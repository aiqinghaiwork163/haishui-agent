...
def build_gateway_health_snapshot(
    ...
    base = {
        ...
        "haishui.supervision_mode": mode if mode in _SUPERVISION_MODES else "unknown",
    }
    ...
    metrics: list[GatewayMetric] = [
        metric("haishui.gateway.up", int(bool(gateway_running))),
        metric("haishui.gateway.active_agents", active_agents),
        metric("haishui.gateway.busy", int(bool(busy))),
        metric("haishui.gateway.drainable", int(bool(drainable))),
        metric("haishui.gateway.restart_requested", int(bool(runtime.get("restart_requested")))),
        metric("haishui.gateway.state", 1, **{"haishui.gateway.state": gateway_state}),
    ]
    fatal_count = 0
    events: list[GatewayHealthEvent | GatewayDiagnosticEvent] = []
    ...
    for platform, pdata in platforms.items():
        ...
        pattrs = {"haishui.platform": str(platform), "haishui.platform.state": state}
        metrics.append(metric("haishui.platform.up", int(state in _RUNNING_PLATFORM_STATES), **pattrs))
        metrics.append(metric("haishui.platform.degraded", int(is_degraded), **pattrs, **{"haishui.error_code": error_code}))
        if is_degraded:
