...
# `export HAISHUI_REDACT_SECRETS=false`) cannot disable redaction mid-session.
...
# to this env var in haishui_cli/main.py, gateway/run.py, and cli.py) or `HAISHUI_REDACT_SECRETS=false` in
# ~/.haishui/.env. An opt-out warning is logged at gateway and CLI startup so operators see the downgrade —
# see `_log_redaction_status()` in gateway/run.py and cli.py.
_REDACT_ENABLED = os.getenv("HAISHUI_REDACT_SECRETS", "true").lower() in {"1", "true", "yes", "on"}

# Routed multiplex profiles: the import-time snapshot above is the LAUNCH profile's policy. A profile
# served under a HAISHUI_HOME override resolves its own ``security.redact_secrets`` (its ``.env``
# value first, like the standalone bridge in haishui_cli/main.py), cached per home so the hot path
...
def _redact_enabled() -> bool:
    ...
    if get_haishui_home_override() is None:
    ...
    try:
        ...
        raw = scope.get("HAISHUI_REDACT_SECRETS") if scope else None
        if raw is None:
...
def mask_secret(value: str, *, head: int = 4, tail: int = 4, floor: int = 12,
    ...
    """Mask a secret for display (``haishui config`` / ``status`` / ``dump``):
...
# a shell rc/profile, Haishui' own ``config.yaml`` where ``haishui mcp add --env`` writes
...
_HAISHUI_HOME_PREFIXES = ("$HAISHUI_HOME/", "${HAISHUI_HOME}/")
# ``$HOME/.haishui/config.yaml`` keeps the ``.haishui`` segment, so stripping the prefix is
...
def _is_under_haishui_home(path: str) -> bool:
    """True when an absolute ``config.yaml`` path sits under the active Haishui home or root.

    The default home's basename is an installation detail — ``.haishui`` on POSIX, ``haishui``
    under ``AppData/Local`` on Windows — and a resolved path never spells ``$HAISHUI_HOME``,
...
def _is_secret_file_arg(arg: str) -> bool:
    ...
    ``.haishui`` directory, ``$HAISHUI_HOME``, or the resolved Haishui home (never arbitrary
    YAML). The resolved-home arm is what covers native Windows, where the home directory
    is ``%LOCALAPPDATA%\\haishui`` and carries no ``.haishui`` segment."""
    path = arg.strip("\"'").replace("\\", "/")
    haishui_home = False
    for prefix in _HAISHUI_HOME_PREFIXES:
    ...
    if parts[-1] in _ENV_FILE_BASENAMES or parts[-1] in _SHELL_RC_BASENAMES:
        return True
    # ``config.yaml`` plus the ``config.yaml.good.<stamp>`` / ``.corrupt.<stamp>`` copies Haishui
    ...
    if parts[-1] != "config.yaml" and not parts[-1].startswith(("config.yaml.good.", "config.yaml.corrupt.")):
        return False
    return haishui_home or ".haishui" in parts[:-1] or _is_under_haishui_home(path)
...
def redact_terminal_output(output: str, command: str | None = None, *, force: bool = False) -> str:
    ...
    shell rc, Haishui ``config.yaml``); otherwise code_file=True avoids false positives on
    source/config dumps."""
    if not output:
        return output
