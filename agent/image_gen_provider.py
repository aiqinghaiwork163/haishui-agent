...
``<repo>/plugins/image_gen/<name>/`` or ``~/.haishui/plugins/image_gen/<name>/``);
...
def save_b64_image(b64_data: str, *, prefix: str = "image", extension: str = "png") -> Path:
    """Decode base64 image data into ``$HAISHUI_HOME/cache/images/``; return the path."""
...
def save_url_image(
    ...
    """Download an (often ephemeral) image URL into ``$HAISHUI_HOME/cache/images/``. Raises on
