"""``$HAISHUI_HOME/cache/<kind>/`` materialisation helpers for the image/video
...
def cache_dir(kind: str) -> Path:
    """Return ``$HAISHUI_HOME/cache/<kind>/``, creating parents as needed."""
    from haishui_constants import get_haishui_home
    path = get_haishui_home() / "cache" / kind
