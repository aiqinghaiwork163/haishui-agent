...
from haishui_constants import (
    get_haishui_home, get_skills_dir, is_wsl, reset_haishui_home_override, set_haishui_home_override,
)
...
def _scan_context_content(content: str, filename: str, *, user_authored: bool = False) -> str:
    ...
    *user_authored* (SOUL.md in the user's own HAISHUI_HOME): a hit is WARNED and the file still loads.
    ...
    (repo AGENTS.md / .cursorrules / .haishui.md) arrive with the checkout and keep blocking, and so does
    a SOUL.md owned by a profile distribution (``haishui profile install <git-url>`` copies it in unscanned;
    ...
    if user_authored:
        logger.warning("Context file %s matched injection pattern(s) %s; loaded anyway because it is the "
                       "user's own file in HAISHUI_HOME — review it if you did not write that text",
...
def _find_haishui_md(cwd: Path) -> Optional[Path]:
    """Nearest ``.haishui.md`` / ``HAISHUI.md`` from *cwd* up to the git root, else None."""
    ...
    for directory in [current, *current.parents] if stop_at else [current]:
        found = next((directory / n for n in (".haishui.md", "HAISHUI.md") if _is_file_or_denied(directory / n)), None)
...
KANBAN_GUIDANCE = (
    ...
    "You have been assigned ONE task from the shared board at `~/.haishui/kanban.db`. Your task id is in "
    "`$HAISHUI_KANBAN_TASK`; your workspace is `$HAISHUI_KANBAN_WORKSPACE`. The `kanban_*` tools in your schema are your "
    ...
    "2. **Work inside the workspace.** `cd $HAISHUI_KANBAN_WORKSPACE` before any file operations. The workspace is "
    ...
    "- **Workspace.** `cd $HAISHUI_KANBAN_WORKSPACE` first. For a `worktree` kind with no `.git`, `git worktree add "
    "<path> ${HAISHUI_KANBAN_BRANCH:-wt/$HAISHUI_KANBAN_TASK}` from the main repo, then cd there. For a project-linked "
    "task the workspace is a fresh `<repo>/.worktrees/<task-id>` and `$HAISHUI_KANBAN_BRANCH` a deterministic "
    ...
    "sits in `ready` forever). Ground every assignee in a real profile (`haishui profile list`, or ask the user), and "
    "express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n\n"
    "- Do not shell out to `haishui kanban <verb>` for board operations. Use the `kanban_*` tools — they work across "
...
# cost that compounds over a long conversation. The haishui-agent runtime already executes a batch of tool
...
# to haishui-agent's Python prompt-assembly architecture.
...
# Text after the "[" that opens one of Haishui' own control frames (the steer marker above, the compaction
...
# apps/desktop/src/plugins/haishui-bots/group-round-prompt.ts byte-equivalent to this list.
...
STEER_CHANNEL_NOTE = (
    ...
    "Mid-turn, the user can steer you: Haishui delivers their message as a standalone user message right after "
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    gated = (
        (True,
         "[Note: this message came from HUD mode — a small floating Haishui "
         ...
         "rather than anything inside Haishui. read_window_below identifies that app."),
        (True,
...
PLATFORM_HINTS = {
    ...
    "tui": (
        ...
        "You are in the Haishui terminal UI (TUI). Files: there is no attachment channel and MEDIA:/path tags "
    ...
    "desktop": (
        ...
        "You are chatting inside the Haishui desktop app, a graphical chat surface. Markdown renders with full GitHub "
        ...
        "or it measures full-bleed. Widgets talk back: data-haishui-send=\"prompt\" on any clickable element (or "
        "window.haishui.send(\"prompt\")) sends that prompt as a hidden user turn — answer it by updating the widget's "
        "file, not with prose."
    ),
    "sms": (
...
def _run_backend_probe(env_type: str, terminal_tool) -> str:
    ...
    env = _create_environment(
        ...
        # snapshot. A normal SSHEnvironment would upload the whole ~/.haishui tree just to run `uname`,
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    cache_key = (haishui_home_key(), env_type, _tenv_read("TERMINAL_CWD", ""))
...
def _remote_backend_hint(backend: str) -> str:
    ...
    if probe:
        return lead + (
            f"this {backend} environment — NOT on the machine where Haishui itself is running. The host OS, "
            f"home, and cwd of the Haishui process are irrelevant; only the following backend state matters:\n{probe}"
        )
    description = (
    ...
    return lead + (
        f"{description} — NOT on the machine where Haishui itself runs. The backend probe didn't respond at "
...
def _embedder_environment_hint() -> str:
    """Embedder-supplied environment description: HAISHUI_ENVIRONMENT_HINT (container ENV)
    wins over config.yaml ``agent.environment_hint``. Read once at prompt-build time."""
    return (os.getenv("HAISHUI_ENVIRONMENT_HINT") or "").strip() or str(
...
RUNTIME_ENVIRONMENT_HEADING = "# Haishui runtime environment"
RUNTIME_ENVIRONMENT_END = "<!-- End Haishui runtime environment -->"
...
def _skills_prompt_snapshot_path() -> Path:
    return get_haishui_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("HAISHUI_PLATFORM") or os.environ.get("HAISHUI_SESSION_PLATFORM")
    if platform:
        return platform
    get_session_env = getattr(sys.modules.get("gateway.session_context"), "get_session_env", None)
    try:
        return (get_session_env("HAISHUI_SESSION_PLATFORM") if get_session_env else "") or ""
    except Exception:
        return ""
...
def build_skills_system_prompt(
    ...
    ``skills_dir_override`` makes home resolution EXPLICIT: a build thread that never bound the HAISHUI_HOME
    ...
    if skills_dir_override is not None:
        ...
        _home_token = set_haishui_home_override(str(skills_dir.parent))
    else:
    ...
    try:
        ...
        if _home_token is not None:
            reset_haishui_home_override(_home_token)
...
def load_soul_md(context_length: Optional[int] = None, home_override: "Path | None" = None) -> Optional[str]:
    """SOUL.md from HAISHUI_HOME (identity slot #1), or None.

    Callers must pass ``skip_soul=True`` to ``build_context_files_prompt`` so it isn't injected twice.
    ``home_override`` pins the profile home (a thread that lost the HAISHUI_HOME ContextVar reads the wrong one).

    ``home_override`` scopes the read to an explicit profile home (the agent knows its own home from its
    session_db path). Without it, resolution is ambient — which on a thread that lost the HAISHUI_HOME
    ...
    """
    try:
        from haishui_cli.config import ensure_haishui_home
        ensure_haishui_home()
    except Exception as e:
        logger.debug("Could not ensure HAISHUI_HOME before loading SOUL.md: %s", e)
    soul_path = (Path(home_override) if home_override is not None else get_haishui_home()) / "SOUL.md"
    ...
    try:
        ...
        if not content:
            return None
        # `haishui profile install <git-url>` / `profile update` plant a third-party SOUL.md into a
        # distribution profile (haishui_cli/profile_distribution.py, DEFAULT_DIST_OWNED) with no scan and no
        ...
        from haishui_cli.profile_distribution import read_manifest
        try:
...
def _haishui_md_candidates(cwd_path: Path) -> list[tuple[str, Path, str]]:
    """.haishui.md / HAISHUI.md — nearest match walking up to the git root."""
    path = _find_haishui_md(cwd_path)
    if path is None:
        return []
...
_CONTEXT_FILE_CANDIDATES = {
    "haishui_md": _haishui_md_candidates,
...
def _project_context_suppressed(cwd: Optional[str], cwd_path: Path, allow_install_tree_fallback: bool) -> bool:
    """A FALLBACK-picked cwd inside the Haishui install tree must not gain system-prompt authority (the desktop
    default would load this repo's contributor AGENTS.md). An explicitly configured cwd is honored verbatim —
    the Haishui tree is a legitimate workspace when the user deliberately points a session at it — and
    CLI-style surfaces pass allow_install_tree_fallback=True because their launch dir IS the user's shell cwd
    (developing Haishui in-tree). See #64590."""
    from agent.runtime_cwd import _is_install_tree
    return cwd is None and not allow_install_tree_fallback and _is_install_tree(cwd_path)


def _load_haishui_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.haishui.md / HAISHUI.md — nearest match walking up to the git root."""
    for label, path, content in _haishui_md_candidates(cwd_path):
        if content:
            return _context_section(_strip_yaml_frontmatter(content), label, ".haishui.md", path, context_length)
    return ""
...
def build_context_files_prompt(
    ...
    Only ONE project context type loads, first found wins: .haishui.md/HAISHUI.md (walk to git root) →
    AGENTS.md chain (git root → cwd) → CLAUDE.md (cwd) → .cursorrules + .cursor/rules/*.mdc (cwd). SOUL.md
    from HAISHUI_HOME is independent and always included unless *skip_soul* (already the identity slot).
    """
    ...
    if _project_context_suppressed(cwd, cwd_path, allow_install_tree_fallback):
        logger.warning(
            "skipping project-context discovery: working-directory resolution fell back to the Haishui "
            "install tree (%s) — set terminal.cwd to your project directory", cwd_path,
        )
        sections = []
    else:
        sections = [_load_haishui_md(cwd_path, context_length) or _load_agents_md(cwd_path, context_length)
