...
def ctx_bound(fn: Callable[..., Any]) -> Callable[..., Any]:
    ...
    is a ContextVar-scoped HAISHUI_HOME override plus the per-turn secret scope; a worker started
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include ``haishui_home`` (profile-scoped storage; never hardcode
        ``~/.haishui``) and ``platform``; may include ``agent_context`` ("primary" |
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        """Setup fields for ``haishui memory setup`` ([] if none): ``key``, ``description``,
    ...
    def backup_paths(self) -> List[str]:
        """Absolute paths of provider state OUTSIDE HAISHUI_HOME for ``haishui backup``/``import``
