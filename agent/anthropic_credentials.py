...
``ANTHROPIC_API_KEY``, Haishui-owned OAuth grants in the ``auth.json`` credential
pool, then ``~/.claude/.credentials.json`` / macOS Keychain as a borrowed fallback.
``~/.haishui/.anthropic_oauth.json`` (Haishui PKCE) and
...
# Claude Code's macOS Keychain entry (generic password). Haishui reads it
...
_SPENT_ROTATION_SIDECAR_COMMENT = (
    ...
    "consumed server-side but never durably committed. Written by Haishui so sibling "
...
def _spent_rotation_sidecar_path(source_path: Path) -> Path:
    return source_path.with_name(source_path.name + ".haishui-spent-rotations.json")
...
_SINGLETON_SOURCE_PATHS = {
    "claude_code": lambda: claude_code_credentials_path(), "haishui_pkce": lambda: _get_haishui_oauth_file()
}
...
def claude_code_credentials_path() -> Path:
    ...
    like the Claude CLI itself (blank = unset, as in ``haishui_cli.foreign_sessions``). The supported opt-out of
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    try:
        from haishui_cli.auth import AUTH_LOCK_TIMEOUT_SECONDS, _auth_store_lock, env_float
        refresh_timeout_seconds = env_float("HAISHUI_ANTHROPIC_REFRESH_TIMEOUT_SECONDS", 20)
        ...
        with _auth_store_lock(timeout_seconds=lock_timeout_seconds, target_path=cred_path):
            ...
            if is_rotation_consumed_uncommitted(refresh_token, source_path=cred_path):
                logger.debug("Refresh token was already consumed by an uncommitted rotation "
                             "- refusing to replay it; run 'haishui auth add anthropic'")
                return None
            ...
            try:
                ...
                if is_terminal_anthropic_refresh_error(e):
                    ...
                    logger.warning(
                        "Claude Code OAuth refresh token is terminally invalid (%s); Haishui cannot use this "
                        "login. Run 'haishui auth add anthropic' to give Haishui its own login.", e)
                else:
            ...
            try:
                ...
                logger.error(
                    ...
                    "run 'haishui auth add anthropic' to give Haishui its own login",
                    cred_path, e,
                )
...
def _mirror_claude_code_credentials_to_keychain(
    ...
    """After a Haishui refresh, write the rotated pair into the Claude Code Keychain item too (#98334).
...
def _resolve_claude_code_token_from_credentials(creds: Optional[Dict[str, Any]] = None) -> Optional[str]:
    ...
    if not refreshed:
        logger.debug("Token refresh failed — run 'haishui auth add anthropic' to give Haishui its own login")
    return refreshed or None


def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    """Prefer refreshable Claude Code creds over a static env OAuth token: Haishui historically persisted setup tokens
...
def _resolve_anthropic_pool_token(*, skip_borrowed: bool = False) -> Optional[str]:
    ...
    refresh=False`` (never ``select()``) so diagnostic call sites (account_usage, ``haishui models``) never mutate
...
# ── Haishui-native PKCE OAuth flow (~/.haishui/.anthropic_oauth.json); mirrors Claude Code / pi-ai / OpenCode ──


def _get_haishui_oauth_file() -> Path:
    return get_haishui_home() / ".anthropic_oauth.json"
...
def run_haishui_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Haishui-native OAuth PKCE flow and return credential state."""
    import webbrowser
    ...
    print("\n".join([
        "", "Authorize Haishui with your Claude Pro/Max subscription.", "",
    ...
    try:
        from haishui_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_haishui_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Haishui-managed OAuth credentials from ~/.haishui/.anthropic_oauth.json."""
    data = _load_json_if_exists(_get_haishui_oauth_file(), "Haishui OAuth credentials")
    return data if data is not None and data.get("accessToken") else None


def _write_haishui_oauth_credentials(
    access_token: str, refresh_token: Optional[str], expires_at_ms: Optional[int],
) -> None:
    """Commit refreshed haishui_pkce tokens to ``<HAISHUI_HOME>/.anthropic_oauth.json`` (``CredentialPersistError``
    on failure); without it the next ``load_pool()`` re-seeds the stale (consumed) pair from the file over the
    rotated pool entry."""
    _commit_private_json(
        _get_haishui_oauth_file(),
        {"accessToken": access_token, "refreshToken": refresh_token, "expiresAt": expires_at_ms},
        "Haishui OAuth credentials",
    )
