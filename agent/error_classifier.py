...
# Parameters Haishui sends on SOME routes only → hosts where that is deliberate.
...
def _plugin_verdict(c: _Ctx) -> Optional[Verdict]:
    ...
    try:
        from haishui_cli.plugins import get_plugin_error_classification
...
def _nous_welcome_tier(c: _Ctx) -> Optional[Verdict]:
    ...
    """
    from haishui_cli.anon_auth import (
...
def _off_route_host(c: _Ctx) -> str:
    ...
    from haishui_cli.route_identity import provider_owns_route
