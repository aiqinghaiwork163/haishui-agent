...
Every Haishui request to the Nous Portal (main loop, auxiliary client, fallback
paths) must carry the same product-attribution tags, sent in OpenAI-compatible
``extra_body['tags']``: ``["product=haishui-agent", "client=haishui-client-v<__version__>"]``.
The version is read live from ``haishui_cli.__version__`` — do NOT pre-compute it
...
_affinity_scope: ContextVar[Optional[str]] = ContextVar("haishui_affinity_scope", default=None)
...
def haishui_client_tag() -> str:
    """``client=haishui-client-v<MAJOR>.<MINOR>.<PATCH>`` ("unknown" if haishui_cli is unimportable)."""
    try:
        from haishui_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"client=haishui-client-v{__version__}"
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    """
    tags = ["product=haishui-agent", haishui_client_tag()]
