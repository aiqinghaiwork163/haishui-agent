...
# Must mirror _STALE_TOOL_CALL_MARKER_RE in haishui_state.py; kept local so importing
# haishui_state (module-level DEFAULT_DB_PATH) is not forced at load time.
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Haishui tool use: model=%s provider=%s base_url=%s "
    ...
    return (
        f"Ollama loaded `{model}` with only {runtime_ctx:,} tokens of runtime context, but Haishui "
        ...
        "again. A known-good starting point is 65,536 tokens. In Haishui config, set "
    ...
    if (provider or "").strip().lower() == "anthropic":
        ...
        if unverified:
            return "\n".join([
                ...
                "Retry with a fresh credential state: `haishui auth reset anthropic`. Until that "
...
def _decode_inline_moa_turn(user_message, persist_user_message):
    ...
    try:
        from haishui_cli.moa_config import decode_moa_turn
...
def _run_conversation_turn(
    ...
    # Adopt ~/.haishui/.env credential/base-url edits made since the last turn — a
...
def _close_durable_failed_turn(agent, result: Any) -> None:
    """Append a Haishui-authored assistant boundary when a failed turn left ``user`` as the
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'PARTIAL_STREAM_STUB_ID': ('haishui_constants', 'PARTIAL_STREAM_STUB_ID'),
