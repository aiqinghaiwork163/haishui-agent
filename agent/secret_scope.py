...
def serves_routed_profile() -> bool:
    ...
    multiplexing, else when a HAISHUI_HOME override names another home (dashboard/desktop backend,
    ...
    if is_multiplex_active():
        return True
    from haishui_constants import get_haishui_home_override, get_process_haishui_home, haishui_home_key
    override = get_haishui_home_override()
    return override is not None and haishui_home_key(override) != haishui_home_key(get_process_haishui_home())
...
class UnscopedSecretError(RuntimeError):
    ...
    def __init__(self, secret_name: str = "", developer_detail: str = ""):
        ...
        super().__init__(
            f"Haishui could not read {what} (an internal profile-scoping bug on the multiplexed "
            "gateway, not your configuration). Run `haishui gateway restart`; if it keeps happening, "
            "report it with `haishui debug share`."
        )
...
_GLOBAL_ENV_EXACT = frozenset({
    # Haishui runtime / deployment
    "HAISHUI_HOME", "HAISHUI_PROFILE", "HAISHUI_GATEWAY_LOCK_DIR",
    "HAISHUI_MAX_ITERATIONS", "HAISHUI_API_TIMEOUT",
    "HAISHUI_REDACT_SECRETS", "HAISHUI_NOUS_TIMEOUT_SECONDS",
    "_HAISHUI_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "HAISHUI_KANBAN_DB", "HAISHUI_KANBAN_WORKSPACES_ROOT", "HAISHUI_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "HAISHUI_KANBAN_",
    "HAISHUI_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Haishui writes itself (bare, 'single', or "double" with
    ``\\"`` / ``\\\\`` escapes)."""
...
# ``invalidate_env_file_cache()`` is the explicit knob; ``haishui_cli.config.invalidate_env_cache()``
# calls it for Haishui's own .env writers.
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    """THE ``.env`` tokenizer: every reader (profile scope, ``haishui_cli.config.load_env``, the dashboard
...
def build_profile_secret_scope(haishui_home: Path) -> Dict[str, str]:
    ...
    secrets = load_env_file(Path(haishui_home) / ".env")
    try:
        from haishui_cli.env_loader import get_secret_source_values
        external_secrets = get_secret_source_values(Path(haishui_home))
    except Exception:
        external_secrets = {}
    ...
    if bridged is not None and _is_process_home(haishui_home):
        secrets.setdefault("GATEWAY_ALLOW_ALL_USERS", bridged)
    return secrets


def _is_process_home(haishui_home: Path) -> bool:
    from haishui_constants import get_process_haishui_home
    try:
        return Path(haishui_home).resolve() == get_process_haishui_home().resolve()
    except OSError:
        return False
