...
retries x 3 Haishui retries), all counted against RPH.
"""
...
def _state_path(*, anonymous: bool = False) -> str:
    ...
    try:
        ...
        base = get_haishui_home()
    except ImportError:
        base = os.path.join(os.path.expanduser("~"), ".haishui")
...
def is_long_welcome_rate_limit(error_context: Any) -> bool:
    ...
    (``welcome_refusal`` from ``haishui_cli.anon_auth.parse_welcome_refusal``). Capacity refusals
