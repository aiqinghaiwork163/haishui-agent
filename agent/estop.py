...
``haishui pause`` writes a sentinel at ``$HAISHUI_HOME/ESTOP``; ``haishui resume``
...
(fail safe, e.g. ``touch ~/.haishui/ESTOP``). Ported from gastownhall/gastown estop.go (MIT).
"""
...
# Same profile-aware / fleet-root resolvers the file-safety guards use (fail-open to ~/.haishui).
from agent.file_safety import _haishui_home_path as _haishui_home, _haishui_root_path as _canonical_root
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel this process would write on `haishui pause`."""
    return _haishui_home() / SENTINEL_NAME


def _candidate_sentinel_paths() -> list:
    """Profile home first, then the fleet root if it is a different directory: a profile
    gateway (HAISHUI_HOME=~/.haishui/profiles/<n>) must still honor an operator's ~/.haishui/ESTOP."""
...
def paused_reply() -> Optional[str]:
    ...
    return f"⏸️ Haishui is paused{tag}. New work is on hold; run `haishui resume` to pick things back up."
...
def check_paused(component: str, logger: logging.Logger) -> bool:
    ...
    if first:
        ...
        logger.info(
            "%s dispatch paused by global emergency stop%s — remove with `haishui resume` (%s)",
