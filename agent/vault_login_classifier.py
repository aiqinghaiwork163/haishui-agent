...
INSPECTION_STAMP_ATTR = "data-haishui-vault-slot"
...
_LOGIN_CONTROL_INSPECTION_JS_TEMPLATE = """(() => {
  ...
  elements.forEach((element, index) => element.setAttribute("data-haishui-vault-slot", nonce + ":" + index));
...
_FILL_JS_TEMPLATE = """(() => {
  ...
  for (const f of fills) {
    const el = document.querySelector('[data-haishui-vault-slot="' + nonce + ':' + f.index + '"]');
  ...
  document.querySelectorAll("[data-haishui-vault-slot]").forEach((n) => n.removeAttribute("data-haishui-vault-slot"));
