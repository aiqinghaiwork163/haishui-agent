...
thread refreshes) → disk cache (~/.haishui/models_dev_cache.json, any age) →
...
from haishui_constants import openrouter_variant_base
...
# Haishui provider names → models.dev provider IDs
PROVIDER_TO_MODELS_DEV: Dict[str, str] = {
    ...
    # Haishui provider is "meta-ai"; both aliases are needed or muse-spark-*
...
# Reverse mapping: models.dev id → Haishui ids (built lazily; many-to-one).
_MODELS_DEV_TO_PROVIDER: Optional[Dict[str, List[str]]] = None


def _models_dev_to_haishui_ids(mdev_id: str) -> List[str]:
    """Return the Haishui provider ids that map to *mdev_id* (may be [])."""
    global _MODELS_DEV_TO_PROVIDER
    if _MODELS_DEV_TO_PROVIDER is None:
        _MODELS_DEV_TO_PROVIDER = {}
        for haishui_id, mapped in PROVIDER_TO_MODELS_DEV.items():
            _MODELS_DEV_TO_PROVIDER.setdefault(mapped, []).append(haishui_id)
...
def _models_dev_id(provider: str) -> Optional[str]:
    """models.dev provider id for a Haishui provider id, or None. A custom provider reaches the
    ...
    if mdev_id is None:
        ...
        if mdev_id is not None and mdev_id not in PROVIDER_TO_MODELS_DEV.values() \
            ...
            if (key, alias) not in _UNKNOWN_CATALOG_PROVIDER_WARNED:
                ...
                logger.warning("providers.%s: catalog_provider %r is neither a Haishui provider id nor a "
...
def _haishui_path(name: str) -> Path:
    from haishui_constants import get_haishui_home
    return get_haishui_home() / name


def _get_cache_path() -> Path:
    return _haishui_path("models_dev_cache.json")


def _get_etag_path() -> Path:
    return _haishui_path("models_dev_cache.etag")
...
def _save_etag(etag: str) -> None:
    def write() -> None:
        ...
        mkdir_under_haishui_home(etag_path.parent)
...
def _get_provider_models(provider: str, *, allow_network: bool = False) -> Optional[Dict[str, Any]]:
    """Resolve a Haishui provider ID to its models dict, or None if unknown.
...
# models the catalog does not know and never displace catalog data. Provider keys accept the Haishui
...
def _provider_override_section(provider: str) -> Optional[Dict[str, Any]]:
    """Override section for *provider* (keyed by Haishui OR models.dev id), or None."""
    overrides = _load_model_overrides()
    provider_key = (provider or "").strip()
    if not overrides or not provider_key:
        return None
    # Forward (Haishui → models.dev id) and reverse (caller passed a models.dev id, config keyed by Haishui id) aliases.
    candidates = [provider_key, PROVIDER_TO_MODELS_DEV.get(provider_key), *_models_dev_to_haishui_ids(provider_key)]
...
def _builtin_model_metadata(provider: str, model: str) -> Optional[Dict[str, Any]]:
    """Built-in metadata for a provider/model pair, if Haishui has a vendor-specific entry."""
...
def get_provider_info(provider_id: str, *, allow_network: bool = True) -> Optional[ProviderInfo]:
    """Provider metadata by Haishui or models.dev ID, or None if not cataloged. ``allow_network`` defaults to True (interactive setup)."""
...
def get_model_info(provider_id: str, model_id: str, *, allow_network: bool = False) -> Optional[ModelInfo]:
    """Full model metadata by Haishui or models.dev provider ID (exact match, then case-insensitive), or
