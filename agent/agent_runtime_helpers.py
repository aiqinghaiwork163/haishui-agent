...
def dump_api_request_debug(
    ...
    try:
        ...
        # Sanitize the session ID (may come from an untrusted X-Haishui-Session-Id header) so a
        ...
        if env_var_enabled("HAISHUI_DUMP_REQUEST_STDOUT"):
...
def _moa_aggregator_cache_policy(agent, eff_model: str) -> tuple[bool, bool]:
    ...
    try:
        from haishui_cli.config import load_config as _load_moa_cfg
...
def _route_may_be_custom(agent, eff_provider: str, provider_lower: str, eff_base_url: str) -> bool:
    ...
    if custom_providers:
        ...
        from haishui_cli.providers import custom_provider_aliases
...
def anthropic_prompt_cache_policy(
    ...
    if supports_cache_markers and (
        ...
        try:
            from haishui_cli.config import get_custom_provider_model_capability
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    # before the built-in ladder so a profile registered from ~/.haishui/plugins/ or a pip entry
    ...
    # Delegate all rate-limit / 5xx retry to haishui's outer conversation loop, which honors Retry-After and
    ...
    # All primary construction and recovery paths must identify Haishui to the official Codex
...
def _apply_switched_provider_request_overrides(agent, new_provider):
    ...
    if custom_providers is None:
        try:
            from haishui_cli.config import load_config, get_compatible_custom_providers
...
def _resolve_switch_destination(agent, new_model, new_provider, base_url, api_mode, capabilities, old_norm, new_norm):
    ...
    from haishui_cli.providers import determine_api_mode, is_actual_route
...
def switch_model(
    ...
    try:
        ...
        from haishui_cli.config import load_config as _sm_load_config
...
def _pre_tool_block_message(agent, function_name, function_args, effective_task_id, tool_call_id, middleware_trace):
    ...
    try:
        from haishui_cli.plugins import _dispatch_pre_tool_call_hooks
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    try:
        from haishui_cli.middleware import apply_tool_request_middleware
    ...
    from haishui_cli.middleware import run_tool_execution_middleware
...
def _session_id_for_heal_log() -> str:
    try:
        from haishui_logging import _session_context
        ...
        if escalate:
            ...
            if key not in _empty_heal_user_notified:
                ...
                _empty_heal_pending_notice[key] = (
                    ...
                    "session's history — run /debug share or `haishui "
...
def _realign_tool_result_names(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    #   the OpenAI-compat serializer keeps it provider-agnostic: Gemini reaches Haishui under many model
...
def _iter_pool_sockets(client: Any):
    ...
    if not pools:
        return
    from agent.process_bootstrap import HAISHUI_TRANSPORT_OWNER_EXT
    seen: set[int] = set()
    for pool, owner in pools:
        ...
        for pool_req in list(getattr(pool, "_requests", None) or []):
            if owner is not None:
                ...
                if exts.get(HAISHUI_TRANSPORT_OWNER_EXT) != owner:
                    continue
