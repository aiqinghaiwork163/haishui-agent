...
# long"). Codex message ids can run 400+ chars; Haishui ``msg_...`` ids stay under the cap.
...
def _replay_reasoning_items(
    ...
    matching issuer. ``id`` (store=False lookups 404) and the Haishui provenance fields are stripped."""
...
def _chat_messages_to_responses_input(
    ...
    decision was wrong — xAI explicitly relies on Haishui threading encrypted reasoning back across turns for
    ...
    Haishui' local history is never truncated by native compaction, so the full conversation is still on the
    wire.
    """
