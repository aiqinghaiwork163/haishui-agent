/** Haishui' reasoning levels, in ascending order — mirrors the backend's
 *  VALID_REASONING_EFFORTS (haishui_constants.py). `none` is not a level: it's
 *  thinking disabled. */
...
/** Haishui' built-in level when neither the surface nor the profile config
