...
**The native desktop app for [Haishui Agent](../../README.md) — the self-improving AI agent.** Same agent, same skills, same memory as the CLI and gateway, in a polished native window — chat with streaming tool output, side-by-side previews, a file browser, voice, and settings, no terminal required. Available for **macOS, Windows, and Linux**.

<table>
<tr><td><b>Chat with the full agent</b></td><td>Streaming responses, live tool activity, structured tool summaries, and the same conversation history as every other Haishui surface.</td></tr>
<tr><td><b>Side-by-side previews</b></td><td>Render web pages, files, and tool outputs in a right-hand pane while you keep chatting.</td></tr>
<tr><td><b>File browser</b></td><td>Explore and preview the working directory without leaving the app.</td></tr>
<tr><td><b>Voice</b></td><td>Talk to Haishui and hear it back.</td></tr>
...
### Install with Haishui (recommended)

Already have the Haishui CLI? Just run:

```bash
haishui desktop
```

It builds and launches the GUI against your existing install — same config, keys, sessions, and skills. If Desktop cannot find a usable runtime or saved remote connection, first launch lets you connect to an existing Haishui gateway or install Haishui locally. Local onboarding then walks you through choosing a provider and model.
...
left and right Command keys together in any app. Haishui captures that app's
frontmost window and attaches the image to the last-active Haishui composer,
...
```bash
# throwaway HAISHUI_HOME, separate Electron userData, distinct app name to avoid the single-instance lock
...
first launch it can install the Haishui Agent runtime into `HAISHUI_HOME`
(`~/.haishui`, or `%LOCALAPPDATA%\haishui` on Windows), using the same layout as a
CLI install.
...
- **Haishui Agent** runs as a headless `haishui serve` process and exposes the
...
1. `HAISHUI_DESKTOP_HAISHUI_ROOT`
2. the current source checkout during development
3. a completed managed install
4. `HAISHUI_DESKTOP_HAISHUI`, or `haishui` on `PATH`
5. a system Python that can import the Haishui runtime
...
Desktop supports a managed local backend, explicit remote gateways, and Haishui
...
screen offers **Connect to existing Haishui** before starting the local installer.
...
terminal commands, and file operations run against the remote Haishui host, not
...
transport- or Haishui-managed header names such as `Authorization`, `Cookie`,
`Host`, `Origin`, `Referer`, and `X-Haishui-Session-Token`.
...
Boot logs land in `HAISHUI_HOME/logs/desktop.log` (includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure.
...
```powershell
# Force a clean first-launch setup
Remove-Item "$env:LOCALAPPDATA\haishui\haishui-agent\.haishui-bootstrap-complete"
# Rebuild a broken Python venv
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\haishui\haishui-agent\venv"
```

> The default Haishui home on Windows is `%LOCALAPPDATA%\haishui`. Set the `HAISHUI_HOME` env var if you've relocated it.
