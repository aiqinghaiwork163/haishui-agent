...
// created profile's config.yaml under HAISHUI_HOME), not the toast.
...
test.afterEach(async ({}, info) => {
  ...
  const cfg = join(fixture.sandbox.haishuiHome, 'profiles', 'fresh-scout', 'config.yaml')
...
test('a fresh (non-clone) bot gets a runnable model block on disk', async () => {
  ...
  const cfgPath = join(fixture!.sandbox.haishuiHome, 'profiles', 'fresh-scout', 'config.yaml')
