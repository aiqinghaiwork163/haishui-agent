/**
 * Shared E2E fixtures for the Haishui desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → haishui serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `haishui desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `haishui-e2e-${prefix}-${Math.random()}`))
  const haishuiHome = path.join(root, 'haishui-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(haishuiHome, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(haishuiHome, 'config.yaml')
 ...
 */
export function writeEnvFile(haishuiHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(haishuiHome, '.env')
 ...
 */
function writeEmptyConfig(haishuiHome: string): void {
  const configPath = path.join(haishuiHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - HAISHUI_HOME → sandbox haishui-home (isolated config/sessions)
 *  - HAISHUI_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - HAISHUI_DESKTOP_IGNORE_EXISTING=1 → don't pick up `haishui` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - HAISHUI_DESKTOP_HAISHUI_ROOT → REPO_ROOT (dev checkout resolution)
 *  - HAISHUI_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    HAISHUI_HOME: sandbox.haishuiHome,
    HAISHUI_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    HAISHUI_DESKTOP_IGNORE_EXISTING: '1',
    HAISHUI_DESKTOP_HAISHUI_ROOT: REPO_ROOT,
    HAISHUI_DESKTOP_APP_NAME: `HaishuiE2E-${Date.now()}`,
    ...
    HAISHUI_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated HAISHUI_HOME + userData
 * @param env      - the process environment (already has HAISHUI_HOME etc.)
...
export async function setupMockBackend(options: MockBackendOptions = {}): Promise<MockBackendFixture> {
  ...
  writeEnvFile(sandbox.haishuiHome)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.haishuiHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via HAISHUI_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`haishui serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.haishuiHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { HAISHUI_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Haishui backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'Haishui.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Haishui.app', 'Contents', 'MacOS', 'Haishui')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'haishui')
}
 ...
 * progress without spawning a real Haishui backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * HAISHUI_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    HAISHUI_DESKTOP_BOOT_FAKE: '1',
    HAISHUI_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + haishui-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).HAISHUI_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).HAISHUI_DESKTOP_HAISHUI
  delete (env as Record<string, string | undefined>).HAISHUI_DESKTOP_HAISHUI_ROOT
