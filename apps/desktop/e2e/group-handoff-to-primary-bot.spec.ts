...
// #100406: in a Bot Mode group room a teammate's `@haishui` handoff must give
// the primary profile (internal name `default`) its turn, exactly like
// `@haishui → @code-farmer` already does. The live roster stamps the primary
// row's handle as the bare profile id ("default"), and the mention parser let
// that stamped handle shadow the `@haishui` alias — so the room settled with
// Haishui never driven. The mock inference server scripts each member's line
...
/** The plugin's persisted room log (`haishui.plugin.haishui-bots.group-chats`). */
async function roomLog(page: Page, group: string): Promise<RoomLogEntry[]> {
  return page.evaluate(name => {
    const raw = window.localStorage.getItem('haishui.plugin.haishui-bots.group-chats')
...
test.afterEach(async ({}, info) => {
  ...
  await info.attach('room-log', {
    body: JSON.stringify(await roomLog(fixture.page, 'Haishui, Code Farmer'), null, 2),
  ...
  await info.attach('runtime-source', {
    body: JSON.stringify(
      await fixture.app.evaluate(() => ({
        cwd: process.cwd(),
        argv: process.argv,
        root: process.env.HAISHUI_DESKTOP_HAISHUI_ROOT,
        home: process.env.HAISHUI_HOME
      })),
      null,
      2
    ),
  ...
  await info.attach('desktop-log', {
    body: readFileSync(join(fixture.sandbox.haishuiHome, 'logs/desktop.log')),
...
test('a teammate handing off with @haishui drives the primary profile', async () => {
  test.setTimeout(420_000)
  const page = fixture!.page
  const group = 'Haishui, Code Farmer'
  ...
  for (const title of ['Haishui', 'Code Farmer']) {
  ...
  // to @haishui; Haishui' scripted reply is "B". Neither script token carries
  // a literal `@`, so the user send itself never mentions Haishui.
  await composer.fill(
    '@code-farmer Please reply with one line only. ' +
      'E2E_SAY(code-farmer)[{at}haishui Please reply with the letter B.] E2E_SAY(haishui)[B]'
  )
  await composer.press('Enter')

  // Code Farmer's handoff line lands first (the reverse direction is not in
  // question); then the room must NOT settle without Haishui' turn.
  await expect
    .poll(
      async () =>
        (await roomLog(page, group)).some(e => e.from?.name === 'code-farmer' && /@haishui/.test(e.text || '')),
      {
        timeout: 180_000
      }
    )
  ...
  await expect
    .poll(
      ...
      {
        timeout: 180_000,
        message: 'the primary profile (default / @haishui) never took its turn after being @mentioned by a teammate'
      }
    )
    .toBe(true)
  ...
  await composer.fill(
    '@haishui Begin the reverse handoff. E2E_SAY(haishui)[{at}code-farmer Reply with D.] E2E_SAY(code-farmer)[D]'
  )
  await composer.press('Enter')
  await expect
