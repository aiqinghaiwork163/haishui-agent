 ...
 * host load; the desktop still reads them through the real `haishui serve`.
 */
function seedArchivedRows(haishuiHome: string, hiddenId: string, controlId: string): void {
  execFileSync(
    RUNTIME_PYTHON,
    [
      '-c',
    ...
    { cwd: REPO_ROOT, env: { ...process.env, HAISHUI_HOME: haishuiHome }, stdio: 'pipe' },
  )
  expect(fs.existsSync(path.join(haishuiHome, 'state.db'))).toBe(true)
}
...
async function setupSeededMockBackend(): Promise<MockBackendFixture & { controlId: string; hiddenId: string }> {
  ...
  seedArchivedRows(sandbox.haishuiHome, hiddenId, controlId)
