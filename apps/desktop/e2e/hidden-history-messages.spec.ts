...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  fs.appendFileSync(
    path.join(sandbox.haishuiHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.haishuiHome)
  const builder = await RealSessionBuilder.start(sandbox.haishuiHome)
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.haishuiHome, mock.url)
  fs.appendFileSync(path.join(sandbox.haishuiHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
