...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'Haishui E2E'], { cwd: repo })
...
function configureRepoCwd(haishuiHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(haishuiHome, mockUrl)
  fs.appendFileSync(path.join(haishuiHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
  writeEnvFile(haishuiHome)
}
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.haishuiHome, mock.url, repo)
