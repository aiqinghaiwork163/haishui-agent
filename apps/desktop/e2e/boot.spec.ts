 ...
 * packaged binary) with a real `haishui serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → haishui serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Haishui title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('Haishui')
  })
  ...
  test('the preload bridge reaches the renderer', async () => {
    const bridge = await fixture!.page.evaluate(() => {
      const desktop = (window as unknown as { haishuiDesktop?: Record<string, unknown> }).haishuiDesktop
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → haishui serve is spawned → WS connects → config
