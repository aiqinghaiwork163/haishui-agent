 ...
 * only its `default` profile; "Homelab" is a REAL second `haishui serve`
...
/** The worktree's own backend: `python -m haishui_cli.main` from the repo root
 *  (the venv's `haishui` console script resolves the package it was installed
 *  from, which need not be this checkout). */
function haishuiCommand(): { bin: string; args: string[] } {
  const venvPython = path.join(REPO_ROOT, '.venv', 'bin', 'python')

  if (fs.existsSync(venvPython)) {
    return { bin: venvPython, args: ['-m', 'haishui_cli.main'] }
  }

  const result = spawnSync('which', ['haishui'], { encoding: 'utf8' })

  if (result.status === 0 && result.stdout.trim()) {
    return { bin: result.stdout.trim(), args: [] }
  }

  throw new Error('haishui backend not found: create the repo venv (uv sync) or put haishui on PATH')
}
...
async function startRemoteGateway(root: string, mockUrl: string, profiles: string[]): Promise<RemoteGateway> {
  ...
  const haishui = haishuiCommand()

  const child: ChildProcess = spawn(haishui.bin, [...haishui.args, 'serve', '--host', '127.0.0.1', '--port', String(port), '--skip-build'], {
    cwd: REPO_ROOT,
    detached: true,
    env: { ...process.env, HAISHUI_HOME: home, HAISHUI_DASHBOARD_SESSION_TOKEN: REMOTE_TOKEN, PYTHONPATH: REPO_ROOT },
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null) {
      throw new Error(`remote haishui serve exited early (${child.exitCode}):\n${log}`)
    }

    try {
      const response = await fetch(`${url}/api/status`, { headers: { 'X-Haishui-Session-Token': REMOTE_TOKEN } })
  ...
  if (Date.now() >= deadline) {
    throw new Error(`remote haishui serve never became ready:\n${log}`)
  }
...
test.describe('New Group Chat gate — one local bot plus remote-connection bots', () => {
  ...
  test('the menu entry enables and the dialog seats the remote bot', async () => {
    ...
    await dialog.getByText('Haishui', { exact: true }).first().locator('xpath=ancestor::label').getByRole('checkbox').click()
