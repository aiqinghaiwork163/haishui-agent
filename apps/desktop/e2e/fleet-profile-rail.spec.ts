 ...
 * second gateway, "Homelab", is a REAL second `haishui serve` this spec spawns
 * with its own HAISHUI_HOME, profiles and session token, registered in the v2
...
function findHaishuiBinary(): string {
  const venv = path.join(REPO_ROOT, '.venv', 'bin', 'haishui')
  ...
  throw new Error('haishui binary not found: create the repo venv (uv sync) or put haishui on PATH')
}
...
/**
 * Spawn a second, fully real `haishui serve` as the remote gateway. Its
 * session token is pinned through HAISHUI_DASHBOARD_SESSION_TOKEN so the
  ...
  const child: ChildProcess = spawn(
    ...
    ['serve', '--host', '127.0.0.1', '--port', String(port), '--skip-build'],
    {
      ...
      env: {
        ...process.env,
        HAISHUI_HOME: home,
        HAISHUI_DASHBOARD_SESSION_TOKEN: REMOTE_TOKEN,
      },
  ...
  while (Date.now() < deadline) {
    ...
    try {
      const response = await fetch(`${url}/api/status`, {
        headers: { 'X-Haishui-Session-Token': REMOTE_TOKEN },
      })
...
test.describe('fleet profile rail — two registered gateways', () => {
  ...
  test.beforeAll(async () => {
    ...
    seedProfiles(sandbox.haishuiHome, ['research'])
