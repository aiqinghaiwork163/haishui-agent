 ...
 * Requires the built desktop and a repo venv (or HAISHUI_DESKTOP_PYTHON).
 */
...
function pythonBinary(): string {
  if (process.env.HAISHUI_DESKTOP_PYTHON) {
    return process.env.HAISHUI_DESKTOP_PYTHON
  }
  ...
  if (!candidate) {
    throw new Error('Create the repo Python venv or set HAISHUI_DESKTOP_PYTHON before running this spec')
  }
...
function isolatedEnv(sandbox: Sandbox): Record<string, string> {
  ...
  for (const key of ['HAISHUI_DESKTOP_USER_DATA_DIR', 'HAISHUI_DESKTOP_IGNORE_EXISTING', 'HAISHUI_DESKTOP_HAISHUI_ROOT', 'HAISHUI_DESKTOP_APP_NAME', 'HAISHUI_DESKTOP_SKIP_QUIT_CONFIRM']) {
    env[key] = defaults[key]
  }
  ...
  return {
    ...
    HAISHUI_DESKTOP_PYTHON: pythonBinary(),
...
const peerTest = test.extend<{ gateways: { app: ElectronApplication; source: Page } }>({
  ...
  gateways: async ({}, runTest, info) => {
    ...
    try {
      ...
      child = spawn(pythonBinary(), ['-m', 'haishui_cli.main', 'serve', '--host', '127.0.0.1', '--port', String(port), '--skip-build'], {
        cwd: REPO_ROOT,
        detached: process.platform !== 'win32',
        env: { ...isolatedEnv(remote), HAISHUI_DASHBOARD_SESSION_TOKEN: REMOTE_TOKEN },
