...
async function seedBot(haishuiHome: string, mockUrl: string, name: string): Promise<void> {
...
test.beforeAll(async () => {
  ...
  await seedBot(sandbox.haishuiHome, mock.url, 'alpha')
  ...
  for (const stray of ['sessions', 'logs']) {
    fs.mkdirSync(path.join(sandbox.haishuiHome, 'profiles', stray, 'cron'), { recursive: true })
  }

  // A deleted profile: identity file present, tombstone in profiles/.deleted/<name>.
  const ghost = path.join(sandbox.haishuiHome, 'profiles', 'ghost')
  fs.mkdirSync(ghost, { recursive: true })
  fs.writeFileSync(path.join(ghost, 'profile.yaml'), 'ui_meta:\n  haishui-bots: {}\n')
  fs.mkdirSync(path.join(sandbox.haishuiHome, 'profiles', '.deleted'), { recursive: true })
  fs.writeFileSync(path.join(sandbox.haishuiHome, 'profiles', '.deleted', 'ghost'), '')
