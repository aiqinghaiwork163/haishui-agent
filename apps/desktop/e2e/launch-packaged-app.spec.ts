...
/**
 * E2E smoke tests for the packaged Haishui desktop app.
 *
 ...
 * (credential stripping, isolated HAISHUI_HOME + userData, unique app name).
 *
...
test('window opens with the Haishui title', async () => {
...
test('HUD composer remains fully inside the transparent window', async () => {
  ...
  await fixture!.page.evaluate(() =>
    (window as typeof window & {
      haishuiDesktop?: { hud?: { open: (options: { sessionId: null }) => Promise<void> } }
    }).haishuiDesktop?.hud?.open({ sessionId: null })
  )
