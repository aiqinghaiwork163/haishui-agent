...
// Hosted Group Chat rooms live in the gateway, and the Desktop's own `haishui serve` backend runs
...
const PYTHON = process.env.HAISHUI_DESKTOP_PYTHON ?? path.join(REPO_ROOT, 'venv', 'bin', 'python')

let fixture: MockBackendFixture | null = null
let rpc: Rpc | null = null

function seedProfile(haishuiHome: string, mockUrl: string, name: string): void {
  const dir = path.join(haishuiHome, 'profiles', name)
...
function seedLegacyRoom(haishuiHome: string): void {
  const script = `
import sys
...
`
  const out = execFileSync(PYTHON, ['-c', script, haishuiHome], {
    cwd: REPO_ROOT,
...
/** Locate the Electron-spawned `haishui serve` backend for this sandbox: its port and session token. */
function findBackend(haishuiHome: string): { port: number; token: string; pid: number } {
  ...
  for (const pidText of pids) {
    ...
    if (vars.get('HAISHUI_HOME') !== haishuiHome) {
      continue
    }
    const token = vars.get('HAISHUI_DASHBOARD_SESSION_TOKEN') ?? ''
    // The backend inherits Electron's remote-debugging socket fd too; the serve port is the listener
    // that only the haishui process holds.
  ...
  throw new Error(`no haishui serve backend found for ${haishuiHome} (pids: ${pids.join(',')})`)
}
...
test.beforeAll(async () => {
  ...
  writeMockProviderConfig(sandbox.haishuiHome, mock.url)
  writeEnvFile(sandbox.haishuiHome)
  seedProfile(sandbox.haishuiHome, mock.url, 'sentinel')
  seedProfile(sandbox.haishuiHome, mock.url, 'friday')
  seedLegacyRoom(sandbox.haishuiHome)
  expect(fs.existsSync(path.join(sandbox.haishuiHome, 'shared-state.db'))).toBe(false)
  ...
  const backend = findBackend(sandbox.haishuiHome)
...
test('rooms from before the shared-state.db split are still reachable from the Desktop backend (#109775)', async () => {
  ...
  expect(fs.existsSync(path.join(fixture!.sandbox.haishuiHome, 'shared-state.db'))).toBe(true)
  expect(fs.existsSync(path.join(fixture!.sandbox.haishuiHome, 'state.db'))).toBe(true)
})
