 ...
 * AIAgent loop, using the E2E mock provider configured in `haishuiHome`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(haishuiHome: string) {
  ...
  static async start(haishuiHome: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(haishuiHome)
