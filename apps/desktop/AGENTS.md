...
How to build Haishui Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another Haishui surface can also
...
- A **runtime home change** (switching the underlying `HAISHUI_HOME` profile) is
...
previews) and the preview pane's `<webview>` (`persist:haishui-preview`). Neither
...
- `main.ts` installs `electron/preview-guest-preload-entry.ts` via
  `will-attach-webview`, keyed on the `persist:haishui-preview` partition only.
...
- `PreviewPane` admits `http:`/`https:` only (`src/lib/preview-external.ts`)
  and hands the URL to the existing `haishui:openExternal` IPC, which applies
...
its shape. "Plugin" means several unrelated things across Haishui — do not assume
...
`env -u NODE_ENV HAISHUI_GUEST_ONBOARDING=1 HAISHUI_HOME=<tmp>/.haishui HAISHUI_DESKTOP_USER_DATA_DIR=<tmp>/electron-user-data npm run dev`
(replace `<tmp>` with that directory). To use the portal stand-in, add
`HAISHUI_PORTAL_BASE_URL=http://127.0.0.1:8765 HAISHUI_ANON_API_SECRET=test-secret HAISHUI_SHARED_AUTH_DIR=<tmp>/.haishui/shared`
...
The free tier (a Nous identity with no account, `haishui_cli/anon_auth.py`) reaches the renderer
