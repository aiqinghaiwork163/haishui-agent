...
const UPSTREAM_URL = 'https://github.com/aiqinghaiwork163/haishui-agent.git'
...
const LEGACY_STORAGE_KEY = 'haishui.desktop.inflightTurnJournal.v1'
const LEGACY_MIGRATION_KEY = 'haishui.desktop.inflightTurnJournal.v2.migrated'
...
async function verifyInteractiveSurfaces(cdp, timed, measure, label) {
  ...
  const version = await timed(`version.ipc.${label}`, () => cdp.eval('window.haishuiDesktop.getVersion()'))
...
async function withTemporarySandbox(label, body) {
  const sandbox = mkdtempSync(join(tmpdir(), `haishui-short-session-${label}-`))
...
async function executeRunInSandboxAttempt(target, index, warmup, mock, output, sandbox, attempt) {
  ...
  const haishuiHome = join(sandbox, 'haishui-home')
  const userData = join(sandbox, 'electron-user-data')
  const desktopLog = join(haishuiHome, 'logs', 'desktop.log')
  ...
  writeSandboxConfig(haishuiHome, mock.url)
  ...
  const child = spawn(
    ...
    [target.targetDesktop, `--user-data-dir=${userData}`, `--remote-debugging-port=${port}`],
    {
      ...
      env: sanitizedEnv({
        HAISHUI_DESKTOP_APP_NAME: `HaishuiShortSession-${target.label}-${process.pid}-${index}-${warmup ? 'w' : 'm'}-${attempt}`,
        HAISHUI_DESKTOP_HAISHUI_ROOT: target.targetRoot,
        HAISHUI_DESKTOP_IGNORE_EXISTING: '1',
        HAISHUI_DESKTOP_USER_DATA_DIR: userData,
...
async function main() {
  ...
  const ephemeralRoot = mkdtempSync(join(tmpdir(), 'haishui-short-session-ab-'))
  const prepared = []
