#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Haishui icon + version metadata onto the
// built Haishui.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Haishui.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `haishui desktop`, the installer's
...
// path (which rebuilds via `haishui desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.mjs <path-to-Haishui.exe>
//
...
// Stamp the Haishui icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(
  ...
  console.log('[set-exe-identity] done — Haishui icon + identity stamped')
}
