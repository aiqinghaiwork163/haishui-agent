...
// no longer bundles the Haishui Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Haishui.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Haishui'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Haishui.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Haishui'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default HAISHUI_HOME for non-sandboxed runs -- matches main.ts's
// resolveHaishuiHome(). On Windows it's %LOCALAPPDATA%\haishui; elsewhere
// it's ~/.haishui. The fresh-install sandbox launchFresh() sets its own
// HAISHUI_HOME and never touches this.
const DEFAULT_HAISHUI_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'haishui')
  }
  return path.join(os.homedir(), '.haishui')
})()
const VENV_ROOT = path.join(DEFAULT_HAISHUI_HOME, 'haishui-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'haishui-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.HAISHUI_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Haishui-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Haishui-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Haishui-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Haishui-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.HAISHUI_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.HAISHUI_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const haishuiHome = path.join(sandbox, 'haishui-home')
  ...
  env.HAISHUI_DESKTOP_CWD = cwd
  env.HAISHUI_DESKTOP_IGNORE_EXISTING = '1'
  env.HAISHUI_DESKTOP_TEST_MODE = 'fresh-install'
  env.HAISHUI_DESKTOP_USER_DATA_DIR = userDataDir
  env.HAISHUI_HOME = haishuiHome
  delete env.HAISHUI_DESKTOP_HAISHUI
  delete env.HAISHUI_DESKTOP_HAISHUI_ROOT
  ...
  console.log(`  HAISHUI_HOME: ${haishuiHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(haishuiHome, 'haishui-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Haishui Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of haishui_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'haishui-agent', 'haishui_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Haishui
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + HAISHUI_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  HAISHUI_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
