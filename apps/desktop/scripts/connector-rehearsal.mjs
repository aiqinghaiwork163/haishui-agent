...
//   D=$(mktemp -d) && env -u HAISHUI_SHARED_AUTH_DIR HAISHUI_GUEST_ONBOARDING=1 HAISHUI_SKIP_INTRO=1 \
//     HAISHUI_HOME="$D/home" HAISHUI_DESKTOP_USER_DATA_DIR="$D/userdata" haishui desktop
...
const sandbox = process.argv[2] ? path.resolve(process.argv[2]) : fs.mkdtempSync(path.join(os.tmpdir(), 'haishui-connectors-'))
const port = 5194
const url = `http://127.0.0.1:${port}`
const python = process.env.HAISHUI_DESKTOP_PYTHON
if (!python || !fs.existsSync(python)) throw new Error('Set HAISHUI_DESKTOP_PYTHON to your installed Haishui Python executable.')
...
for (const dir of ['.haishui/shared', 'electron', 'work']) fs.mkdirSync(path.join(sandbox, dir), { recursive: true })
const env = Object.fromEntries(['PATH', 'TMPDIR', 'LANG', 'SHELL'].filter(key => process.env[key]).map(key => [key, process.env[key]]))
Object.assign(env, {
  HOME: sandbox, HAISHUI_HOME: path.join(sandbox, '.haishui'), HAISHUI_SHARED_AUTH_DIR: path.join(sandbox, '.haishui/shared'),
  HAISHUI_DESKTOP_USER_DATA_DIR: path.join(sandbox, 'electron'), HAISHUI_DESKTOP_CWD: path.join(sandbox, 'work'),
  HAISHUI_DESKTOP_HAISHUI_ROOT: root, HAISHUI_DESKTOP_PYTHON: python,
  HAISHUI_DESKTOP_IGNORE_EXISTING: '1', HAISHUI_DESKTOP_APP_NAME: 'Haishui Connector Rehearsal',
  HAISHUI_GUEST_ONBOARDING: '1', HAISHUI_SKIP_INTRO: '1', HAISHUI_DESKTOP_DEV_SERVER: url, HAISHUI_DESKTOP_CDP_PORT: '9344'
})
