 ...
 * product name (`Haishui`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Haishui'
 *
 * This is a hard failure with no obvious cause for the user — `haishui desktop`
 ...
 * (the mac-unpacked Haishui.app bundle) and Windows (win-unpacked), so we
 ...
 * Haishui.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `haishui desktop --build-only` (haishui_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Haishui.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Haishui'}.exe`
