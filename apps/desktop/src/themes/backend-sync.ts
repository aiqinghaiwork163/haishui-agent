/**
 * Live skin sync from the Haishui backend.
 *
 * The backend resolves the active skin (built-in or `$HAISHUI_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Haishui
 * authoring/activating a skin from a prompt, or `/skin` elsewhere) repaints.
 */

import type { HaishuiSkin } from '@haishui/shared/skin'
...
const BACKEND_THEMES_KEY = 'haishui-desktop-backend-themes-v1'
 ...
 */
export function ingestBackendSkin(skin: HaishuiSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as HaishuiSkin)
