...
import { ensureContrast, mix, parseColor } from '@haishui/shared/color'
...
const SKIN_KEY = 'haishui-desktop-theme-v2'
const MODE_KEY = 'haishui-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'haishui-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'haishui-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'haishui-desktop-active-profile-v1'
...
// mix / ensureContrast live in @haishui/shared/color (shared with the TUI);
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark', chatFontFamily = $chatFontFamily.get()) {
  ...
  root.dataset.haishuiTheme = skinName
  root.dataset.haishuiMode = rendered
  ...
  window.haishuiDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('haishui-boot-background', chromeBg)
    window.localStorage.setItem('haishui-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.haishuiThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.haishuiDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (Haishui authoring/activating a skin from a
