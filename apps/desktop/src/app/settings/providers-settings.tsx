...
import { disconnectOAuthProvider, listOAuthProviders } from '@/haishui'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/haishui'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.haishuiDesktop?.terminal)
...
//      in haishui_cli/provider_catalog.py) — the SAME provider identity
//      `haishui model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Haishui can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Haishui can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // Haishui never deletes creds another tool owns behind a silent API call.
