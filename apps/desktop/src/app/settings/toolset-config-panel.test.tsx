...
import type { ToolsetConfig } from '@/types/haishui'
...
const getHaishuiConfigSchema = vi.fn()
const saveHaishuiConfig = vi.fn()
const saveHaishuiConfigRecord = vi.fn()
...
vi.mock('@/haishui', () => ({
  ...
  getHaishuiConfigSchema: () => getHaishuiConfigSchema(),
  saveHaishuiConfig: (config: unknown) => saveHaishuiConfig(config),
  saveHaishuiConfigRecord: (config: unknown, profile?: unknown) => saveHaishuiConfigRecord(config, profile),
...
beforeEach(() => {
  ...
  getHaishuiConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
  saveHaishuiConfig.mockResolvedValue({ ok: true })
  saveHaishuiConfigRecord.mockResolvedValue({ ok: true })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(saveHaishuiConfigRecord).toHaveBeenCalled(), { timeout: 3000 })
    const saved = saveHaishuiConfigRecord.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>
    ...
    expect(saveHaishuiConfigRecord.mock.calls.at(-1)?.[1]).toBeUndefined()
  })
  ...
  it('autosaves the inline voice fields into the profile the panel is scoped to', async () => {
    ...
    await waitFor(() => expect(saveHaishuiConfigRecord).toHaveBeenCalled(), { timeout: 3000 })
    const [saved, forwarded] = saveHaishuiConfigRecord.mock.calls.at(-1) as [
