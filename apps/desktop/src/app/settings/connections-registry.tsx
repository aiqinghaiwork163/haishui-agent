...
function editorFromConnection(conn: DesktopRegistryConnection): EditorState {
  return {
    ...
    remoteHaishuiPath: conn.remoteHaishuiPath || '',
 ...
 * agent sources (local runtime + any number of remote gateways / Haishui Cloud
...
export function ConnectionsRegistrySection() {
  ...
  useEffect(() => {
    if (!editorWantsOauth || !editorUrl || !window.haishuiDesktop?.probeConnectionConfig) {
      setAuthProbe(null)
  ...
  const signInOauth = useCallback(async () => {
    ...
    try {
      const result = await window.haishuiDesktop.oauthLoginConnectionConfig(editorUrl)
    ...
    async (allowPlainTextToken = false) => {
      ...
      try {
        ...
        if (editor.kind === 'remote' || editor.kind === 'cloud') {
          ...
          payload.remoteHaishuiPath = editor.remoteHaishuiPath.trim()
        }
  ...
  // Fan out `haishui update` to every eligible source; per-connection results
          ...
          {editor.kind === 'ssh' && (
            <>
              ...
              <ListRow
                action={
                  <Input
                    onChange={e => setEditor({ ...editor, remoteHaishuiPath: e.target.value })}
                    placeholder={t.settings.gateway.sshHaishuiPathPlaceholder}
                    value={editor.remoteHaishuiPath}
                  />
                }
                description={t.settings.gateway.sshHaishuiPathDesc}
                title={t.settings.gateway.sshHaishuiPathTitle}
              />
            </>
          )}
