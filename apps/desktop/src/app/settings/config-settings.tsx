...
import { getElevenLabsVoices, getHaishuiConfigSchema, saveHaishuiConfig } from '@/haishui'
...
import type { ConfigFieldSchema, HaishuiConfigRecord } from '@/types/haishui'

import { haishuiConfigCacheWriter, useHaishuiConfigRecord } from '../hooks/use-config-record'
...
function ConfigSettingsInner({
  ...
  const [config, setConfig] = useState<HaishuiConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useHaishuiConfigRecord(scopeProfile)
  ...
  const writeConfigCache = useMemo(() => haishuiConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const {
    ...
    queryKey:
      scopeProfile == null ? ['haishui-config-schema'] : ['haishui-config-schema', normalizeProfileKey(scopeProfile)],
    queryFn: () => getHaishuiConfigSchema(scopeProfile),
    staleTime: 5 * 60 * 1000
  })
  ...
  // never touched — possibly changed out-of-band by `haishui config set`
  // while this page sat open — is never resent with its stale value.
  const configBaselineRef = useRef<HaishuiConfigRecord | null>(null)
    ...
    const t = window.setTimeout(() => {
      ...
      saveQueueRef.current = saveQueueRef.current.then(async () => {
        try {
          ...
          const result = await saveHaishuiConfig(patch, scopeProfile)
  ...
  const applyConfig = (next: HaishuiConfigRecord) => {
    saveVersionRef.current += 1
    setConfig(next)
    setSaveVersion(saveVersionRef.current)
  }

  const updateConfig = (next: HaishuiConfigRecord) => {
