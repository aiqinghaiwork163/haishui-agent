...
describe('ConnectionsRegistrySection', () => {
  it('refreshes a cached roster immediately after a successful connection test', async () => {
    ...
    Object.assign(window.haishuiDesktop!, { getAgentRoster })
  ...
  it('saves a custom remote Haishui path for SSH connections', async () => {
    render(<ConnectionsRegistrySection />)
    ...
    fireEvent.change(screen.getByPlaceholderText('auto-detect'), {
      target: { value: '/opt/haishui/bin/haishui' }
    })
    ...
    expect(save.mock.calls[0][0]).toMatchObject({
      ...
      kind: 'ssh',
      label: 'Build host',
      remoteHaishuiPath: '/opt/haishui/bin/haishui'
    })
  })

  it('clears a saved remote Haishui path back to auto-detect', async () => {
    ...
    expect(pathInput.value).toBe('/opt/haishui/bin/haishui')
    fireEvent.change(pathInput, { target: { value: '   ' } })
    fireEvent.click(screen.getByText('Save connection').closest('button')!)

    await waitFor(() => expect(save).toHaveBeenCalledTimes(1))
    expect(save.mock.calls[0][0]).toMatchObject({ id: 'build-host', remoteHaishuiPath: '' })
  })
  ...
  it('offers every kind on create and disables Local while the managed entry exists', async () => {
    ...
    expect(screen.getByRole('button', { name: 'Haishui Cloud' })).toBeTruthy()
