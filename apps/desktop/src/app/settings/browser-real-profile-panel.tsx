...
import { type ProfileScope, saveHaishuiConfigRecord } from '@/haishui'
 ...
 * the default browser's profile (cookies/logins) into a Haishui-owned copy;
...
export function BrowserRealProfilePanel({ profile }: BrowserRealProfilePanelProps) {
  ...
  const { data: config } = useHaishuiConfigRecord(profile)
  ...
  const toggle = useCallback(
    async (on: boolean) => {
      ...
      try {
        ...
        await saveHaishuiConfigRecord({ browser: { use_real_profile: on } }, profile)
        notify({
          kind: 'info',
