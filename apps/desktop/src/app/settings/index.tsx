...
import { getHaishuiConfigDefaults, getHaishuiConfigRecord, saveHaishuiConfig } from '@/haishui'
...
export function SettingsView({ onClose, onConfigSaved, onMainModelChanged }: SettingsPageProps) {
  ...
  const exportConfig = async () => {
    try {
      const cfg = await getHaishuiConfigRecord()
      const blob = new Blob([JSON.stringify(cfg, null, 2)], { type: 'application/json' })
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      a.download = 'haishui-config.json'
      a.click()
  ...
  const resetConfig = async () => {
    ...
    try {
      await saveHaishuiConfig(await getHaishuiConfigDefaults())
      triggerHaptic('success')
      onConfigSaved?.()
    } catch (err) {
