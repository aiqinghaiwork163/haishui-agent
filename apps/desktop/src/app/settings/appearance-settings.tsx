...
function ResumeLastSessionSetting() {
  ...
  const configQuery = useHaishuiConfigRecord()
  ...
  const update = (on: boolean) => {
    ...
    void saveHaishuiConfig(setNested({}, 'display.resume_last_session', on))
      .then(result => {
        if (!result.ok) {
...
function MarketplaceThemeResults({
  ...
  const search = useQuery({
    ...
    queryFn: () => window.haishuiDesktop?.themes?.searchMarketplace(debounced) ?? Promise.resolve([]),
