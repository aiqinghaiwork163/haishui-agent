...
import { DEFAULT_REASONING_EFFORT, REASONING_EFFORT_VALUES } from '@haishui/shared'
...
import { haishuiConfigCacheWriter, invalidateHaishuiConfig, useHaishuiConfigRecord } from '../hooks/use-config-record'
...
// backend now surfaces the full `haishui model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in haishui_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged, scopeProfile }: ModelSettingsProps) {
  ...
  const { data: config } = useHaishuiConfigRecord(scopeProfile)
  const setConfig = useMemo(() => haishuiConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const refresh = useCallback(
    async ({ replaceSelection = false }: { replaceSelection?: boolean } = {}) => {
      ...
      try {
        ...
        void invalidateHaishuiConfig(scopeProfile)
      } catch (err) {
        if (profileEpoch.current === epoch) {
  ...
  const writeAgentDefault = useCallback(
    async (key: string, value: string) => {
      ...
      try {
        await saveHaishuiConfig(setNested({}, key, value), scopeProfile)
      } catch (err) {
        setConfig(prev)
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `haishui model` curation). Best-effort — fall through to the refreshed
  ...
  const recycleStaleBackend = useCallback(async () => {
    ...
    try {
      await window.haishuiDesktop?.recycleBackend?.(scopeProfile)
        ...
        {needsSetup && !setupIsApiKey && selectedProviderRow && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Haishui runs the flow for you.`}
          </p>
        )}
