...
import { setHaishuiConfigCache, useHaishuiConfigRecord } from '../hooks/use-config-record'
...
function fontFamilyFromConfig(config: HaishuiConfigRecord): string {
...
export function TerminalFontSetting() {
  ...
  const { data: loadedConfig, dataUpdatedAt } = useHaishuiConfigRecord()
  ...
  useEffect(() => {
    ...
    // holds — successful saves write it back via setHaishuiConfigCache, so
    ...
    const timeout = window.setTimeout(() => {
      ...
      void saveHaishuiConfig(setNested({}, 'terminal.font_family', value))
        .then(result => {
          if (!result.ok) {
        ...
        <div className="mt-3 space-y-2">
          <div className="flex items-center gap-3">
            <Input
              ...
              className="flex-1"
              disabled={draft === null}
              list="haishui-terminal-font-families"
          ...
          <datalist id="haishui-terminal-font-families">
