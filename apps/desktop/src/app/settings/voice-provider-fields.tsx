...
import {
  getElevenLabsVoices,
  getHaishuiConfigSchema,
  type ProfileScope,
  profileScopeKey,
  saveHaishuiConfigRecord
} from '@/haishui'
...
export function VoiceProviderFields({
  ...
  const { data: loadedConfig } = useHaishuiConfigRecord(profile)
  ...
  const writeConfigCache = useMemo(() => haishuiConfigCacheWriter(profile), [scopeKey])

  const { data: schemaResponse } = useQuery({
    queryKey: ['haishui-config-schema'],
    queryFn: () => getHaishuiConfigSchema(),
    staleTime: 5 * 60 * 1000
  })
  ...
  const [baseline, setBaseline] = useState<HaishuiConfigRecord | null>(null)
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      void saveHaishuiConfigRecord(diffConfig(baseline ?? {}, config), profile)
        .then(() => {
          setBaseline(config)
