...
import type { HaishuiConfigRecord, SessionInfo } from '@/types/haishui'
...
function AutoArchiveSetting() {
  ...
  useEffect(() => {
    ...
    if (!window.haishuiDesktop) {
      return
    }

    let alive = true

    void getHaishuiConfigRecord()
      .then(record => {
        if (!alive) {
          return
        }
  ...
  const persist = useCallback(
    async (autoArchive: boolean, archiveDays: number) => {
      ...
      try {
        ...
        await saveHaishuiConfig({ sessions: { auto_archive: autoArchive, auto_archive_days: archiveDays } })
      } catch (err) {
...
// / Program Files), which buried any files Haishui wrote there.
function DefaultProjectDirSetting() {
  ...
  useEffect(() => {
    ...
    // / Storybook / non-Electron context `window.haishuiDesktop` is
    ...
    const settings = window.haishuiDesktop?.settings
