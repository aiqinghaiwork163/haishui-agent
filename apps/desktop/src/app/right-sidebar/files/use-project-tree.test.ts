...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { haishuiDesktop: { readDir: typeof readDir } }).haishuiDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): HaishuiReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { haishuiDesktop: unknown }).haishuiDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: HaishuiReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<HaishuiReadDirResult>(resolve => {
  ...
  it('discards a stale live refresh after the active registered connection changes', async () => {
    let resolveRefreshFromA: ((result: HaishuiReadDirResult) => void) | undefined
  ...
  it('discards a stale child read after the active registered connection changes', async () => {
    let resolveChildFromA: ((result: HaishuiReadDirResult) => void) | undefined
  ...
  it('discards a stale root read after the active registered connection changes', async () => {
    let resolveFirst: ((result: HaishuiReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<HaishuiReadDirResult>(resolve => {
  ...
  it('returns no-bridge gracefully when window.haishuiDesktop is missing', async () => {
    delete (window as unknown as { haishuiDesktop?: unknown }).haishuiDesktop
  ...
  it('keeps an unreadable root on screen while it re-probes', async () => {
    ...
    let releaseProbe: ((value: HaishuiReadDirResult) => void) | undefined
  ...
  it('keeps loaded rows on screen while the root refreshes', async () => {
    ...
    let releaseRefresh: ((value: HaishuiReadDirResult) => void) | undefined
  ...
  it('clears the rows when the same path is re-read from another backend', async () => {
    ...
    let releaseFromB: ((value: HaishuiReadDirResult) => void) | undefined
  ...
  it('drops a child listing that was read before the preference flipped', async () => {
    const gitRoot = vi.fn(async () => '/p')
    const readFileDataUrl = vi.fn(async () => `data:text/plain;base64,${btoa('*.log\n')}`)
    let releaseChild: ((value: HaishuiReadDirResult) => void) | undefined
    ...
    readDir.mockImplementation(async path => {
      ...
      if (path === '/p/src') {
        return new Promise<HaishuiReadDirResult>(resolve => {
