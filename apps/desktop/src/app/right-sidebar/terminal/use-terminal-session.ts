...
const HAISHUI_PATHS_MIME = 'application/x-haishui-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(HAISHUI_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(HAISHUI_PATHS_MIME)
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const terminalApi = window.haishuiDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      // Full-screen TUIs (haishui --tui, vim) grab the mouse, so a plain drag
    ...
    term.attachCustomKeyEventHandler(event => {
      ...
      void (async () => {
        const text = (await window.haishuiDesktop?.readClipboard?.()) ?? ''
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[haishui-terminal] WebGL unavailable; falling back to DOM', err)
      }
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      hasSessionActivityRef.current = true
      void window.haishuiDesktop?.terminal?.write(sessionId, `${command}\r`)
