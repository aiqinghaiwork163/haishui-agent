...
import type { DesktopBootProgress, HaishuiConnection, HaishuiWindowState } from '@/global'
import { HaishuiGateway } from '@/haishui'
...
export function primaryRuntimeConnectionId(connection: Pick<HaishuiConnection, 'connectionId' | 'mode'>): null | string {
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.haishuiDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: HaishuiGateway | null) => void
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    let pendingWindowState: HaishuiWindowState | null = null

    const publish = (next: HaishuiConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Haishui...". Once the
    ...
    // The other way a cold boot concludes. Main keeps startHaishui() available
    ...
    const attemptReconnect = async (manual?: { profile: string; activationEpoch: number }) => {
      ...
      try {
        ...
        // "Starting Haishui…". The probe is a no-op for a healthy or local backend.
        ...
        const conn = await withTimeout(
          ...
          RECONNECT_ATTEMPT_TIMEOUT_MS,
          'Timed out reconnecting to Haishui backend'
        )
        ...
        // as an opaque "Could not connect to Haishui gateway". resolveGatewayWsUrl
        ...
        if (!manual || (isActivePrimary() && gatewayActivationEpoch() === manual.activationEpoch)) {
          reconcileBusyStatesOnReconnect()
          await callbacksRef.current.refreshHaishuiConfig().catch(() => undefined)
    ...
    async function getWindowBackend(startup = false): Promise<HaishuiConnection> {
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshHaishuiConfig(false, ownsSwitch).catch(() => undefined),
    ...
    const onBootProgress = (payload: DesktopBootProgress) => {
      ...
      // Soft switch / post-boot startHaishui re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new HaishuiGateway()
    ...
    configureGatewayRegistry({
      onServerRequest: request => {
        if (!callbacksRef.current.handleServerRequest(request)) {
          request.fail(JSON_RPC_METHOD_NOT_FOUND, `Haishui Desktop cannot answer ${request.method}`)
        }
      },
    ...
    const offGatewayReconnect = registerGatewayReconnect(async () => {
      ...
      if (!isActivePrimary()) {
        ...
        if (!(await ensureActiveGatewayOpen({ explicit: true }))) {
          throw new Error('Haishui gateway is not connected')
        }
    ...
    const offExit = desktop.onBackendExit(() => {
      ...
      notify({
        ...
        durationMs: 0,
        action: {
          label: translateNow('boot.errors.restartHaishui'),
    ...
    async function boot() {
      ...
      try {
        ...
        // round-trip must not hang "Starting Haishui…" forever. Initial boot
        ...
        const conn = await withTimeout(
          ...
          BACKEND_BOOT_WAIT_TIMEOUT_MS,
          'Timed out connecting to Haishui backend'
        )
        ...
        // reaches the recovery affordance instead of hanging "Starting Haishui…".
        const wsUrl = await withTimeout(
          resolveGatewayWsUrl(desktop, conn),
          RECONNECT_ATTEMPT_TIMEOUT_MS,
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshHaishuiConfig(),
          ...
          // retry it), not brick boot behind the "Haishui couldn't start"
