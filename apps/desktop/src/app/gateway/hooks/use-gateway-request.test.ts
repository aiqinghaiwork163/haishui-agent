...
vi.mock('@/haishui', async importOriginal => {
  const actual = await importOriginal<typeof HaishuiModule>()

  class FakeHaishuiGateway {
  ...
  return { ...actual, HaishuiGateway: FakeHaishuiGateway }
})

import type * as HaishuiModule from '@/haishui'
import type { HaishuiGateway } from '@/haishui'
import {
  $gateway,
...
const fakeGateway = { connectionState: 'open' } as unknown as HaishuiGateway
...
async function activateRemoteGateway() {
  ...
  setPrimaryGateway(primary as unknown as HaishuiGateway, 'default')
  $gateway.set(primary as unknown as HaishuiGateway)
...
describe('useGatewayRequest', () => {
  ...
  it('surfaces a missing optional scoped mint bridge without falling back to a stale ticket or local lookup', async () => {
    const { desktop, gateway } = await activateRemoteGateway()
    Reflect.deleteProperty(window.haishuiDesktop, 'getGatewayWsUrlFor')
  ...
  it('rejects instead of hanging forever when the reconnect getConnection() wedges (#93454)', async () => {
    ...
    } as unknown as HaishuiGateway

    const getConnection = vi.fn(() => new Promise(() => undefined))

    ;(window as unknown as { haishuiDesktop: unknown }).haishuiDesktop = { getConnection }
    $gateway.set(dropped)
