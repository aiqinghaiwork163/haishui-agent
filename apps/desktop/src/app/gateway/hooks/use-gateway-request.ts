import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@haishui/shared'
...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as HaishuiGateway | null
  const gatewayRef = useRef<HaishuiGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.haishuiDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<HaishuiGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as HaishuiGateway | null
      }),
    []
  )
