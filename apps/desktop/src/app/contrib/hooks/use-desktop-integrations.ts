...
import { pathFromHaishuiDeepLink, resolveHaishuiOpenPath } from '@/lib/haishui-open-target'
...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    ...
    // "About Haishui" — it is the OS-standard affordance for updating THIS app,
    ...
    const unsubscribe = window.haishuiDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow('client'))
  ...
  useEffect(() => {
    window.haishuiDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.haishuiDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.haishuiDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // haishui://index-network/intent/1 vocabulary with deep links.
  useEffect(() => {
    const unsubscribe = window.haishuiDesktop?.onNotificationActivate?.(payload => {
      if (!payload) {
        return
      }
      ...
      if (payload.activate) {
        ...
        // the pre-IPC validation — any future haishuiDesktop.notify caller gets
        // funneled through the same resolver.
        const path = resolveHaishuiOpenPath(payload.activate)
  ...
  useEffect(() => {
    const unsubscribe = window.haishuiDesktop?.onDeepLink?.(payload => {
      if (!payload?.kind) {
        return
      }
      ...
      // link (haishui://index-network/intent/1, haishui://open/…). The resolver
      // rejects reserved kinds and unsafe paths.
      const path = pathFromHaishuiDeepLink(payload.kind, payload.name || '', payload.params || {})
    ...
    void window.haishuiDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.haishuiDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.haishuiDesktop?.onPreviewNav?.(command => {
  ...
  useEffect(() => {
    const unsubscribe = window.haishuiDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())
