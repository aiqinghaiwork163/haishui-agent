...
import type { SessionInfo } from '@/types/haishui'
...
const initialHaishuiDesktop = desktopWindow.haishuiDesktop
...
describe('useDesktopIntegrations', () => {
  ...
  beforeEach(() => {
    ...
    // plain object to window.haishuiDesktop rather than using vi.spyOn.
    desktopWindow.haishuiDesktop = {
    ...
    } as unknown as Window['haishuiDesktop']
  })

  afterEach(() => {
    if (initialHaishuiDesktop) {
      desktopWindow.haishuiDesktop = initialHaishuiDesktop
    }
  ...
  describe('profile-ready gate', () => {
    it('does NOT restore before profileReady is true', () => {
      ...
      window.localStorage.setItem('haishui.desktop.lastRoute.profile.default', '/remembered-session')
      window.localStorage.setItem('haishui.desktop.lastSessionId.profile.default', 'remembered-session')
    ...
    it('waits for sessions before validating a remembered session route', () => {
      ...
      expect(navigate).not.toHaveBeenCalled()
      expect(window.localStorage.getItem('haishui.desktop.lastRoute.profile.default')).toBe('/remembered-session')
  ...
  describe('display.resume_last_session', () => {
    it('stays on the fresh chat when the setting is off, and keeps remembering the open chat', () => {
      ...
      expect(window.localStorage.getItem('haishui.desktop.lastSessionId.profile.default')).toBe('other-session')
    })
  ...
  describe('ownership validation', () => {
    it('refuses to restore a session route owned by another profile', () => {
      window.localStorage.setItem('haishui.desktop.lastRoute.profile.default', '/ai-session')
    ...
    it('refuses to restore a session id owned by another profile', () => {
      window.localStorage.setItem('haishui.desktop.lastSessionId.profile.default', 'ai-session')
      window.localStorage.setItem('haishui.desktop.lastRoute.profile.default', '/ai-session')
    ...
    it('clears stale remembered route owned by wrong profile', () => {
      window.localStorage.setItem('haishui.desktop.lastRoute.profile.ai-engineer', '/ai-session')
  ...
  describe('two profiles with distinct sessions', () => {
    it('restores profile A session when profile A is active', () => {
      window.localStorage.setItem('haishui.desktop.lastRoute.profile.coder', '/coder-session')
  ...
  describe('HUD window (win=hud)', () => {
    ...
    it('does NOT write remembered navigation while showing a session', () => {
      ...
      expect(window.localStorage.getItem('haishui.desktop.lastSessionId.profile.default')).toBeNull()
      expect(window.localStorage.getItem('haishui.desktop.lastRoute.profile.default')).toBeNull()
    })
  ...
  describe('legacy key behavior', () => {
    it('discards legacy global keys on read and does NOT restore from them', () => {
      ...
      window.localStorage.setItem('haishui.desktop.lastSessionId', 'legacy-session')
      window.localStorage.setItem('haishui.desktop.lastRoute', '/session/legacy-session')
      ...
      expect(window.localStorage.getItem('haishui.desktop.lastSessionId')).toBeNull()
      expect(window.localStorage.getItem('haishui.desktop.lastRoute')).toBeNull()
  ...
  describe('stale-result suppression during profile switch', () => {
    it('remembers route for the new profile after switch, not the old one', () => {
      ...
      expect(window.localStorage.getItem('haishui.desktop.lastSessionId.profile.coder')).toBe('coder-session')
      ...
      expect(window.localStorage.getItem('haishui.desktop.lastSessionId.profile.ops')).toBe('ops-session')
    ...
    it('does NOT overwrite remembered state when session ownership fails validation', () => {
      ...
      expect(window.localStorage.getItem('haishui.desktop.lastSessionId.profile.ops')).toBeNull()
    })
  })

  describe('route-scoped restoration', () => {
    it('restores a non-session route like /skills', () => {
      window.localStorage.setItem('haishui.desktop.lastRoute.profile.default', '/skills')
    ...
    it('does NOT restore overlay routes (settings/command-center)', () => {
      window.localStorage.setItem('haishui.desktop.lastRoute.profile.default', '/settings')
  ...
  describe('exhausted session cleanup', () => {
    it('clears remembered session id when the exhausted session matches', () => {
      window.localStorage.setItem('haishui.desktop.lastSessionId.profile.default', 'exhausted')
    ...
    it('clears remembered route when it carries the exhausted session', () => {
      window.localStorage.setItem('haishui.desktop.lastRoute.profile.default', '/exhausted')
    ...
    it('does NOT clear exhausted when profileReady is false', () => {
      ...
      expect(window.localStorage.getItem('haishui.desktop.lastSessionId.profile.default')).toBe('exhausted')
    })

    it('does NOT clear remembered state when exhausted id does not match', () => {
      window.localStorage.setItem('haishui.desktop.lastSessionId.profile.default', 'other-session')
  ...
  describe('notification activate + plugin deep links', () => {
    ...
    it('navigates haishui://index-network/intent/1 deep links through the same path vocabulary', () => {
    ...
    it('routes haishui://mcp/install to the pending-install dialog, not navigation', () => {
      let deepLink: ((payload: { kind: string; name: string; params: Record<string, string> }) => void) | undefined
      desktopWindow.haishuiDesktop = {
        ...desktopWindow.haishuiDesktop,
        onDeepLink: (cb: (payload: { kind: string; name: string; params: Record<string, string> }) => void) => {
