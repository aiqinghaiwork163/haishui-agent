...
import { getHaishuiConfigRecord, saveMcpServers } from '@/haishui'
...
import { setHaishuiConfigCache } from '../hooks/use-config-record'

/**
 * Explicit-confirm gate for `haishui://mcp/install` deep links. The payload is
 ...
 * confirming lets Haishui spawn that local process. An existing server name is
...
export function McpInstallDeepLinkDialog() {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const current = getServers(await getHaishuiConfigRecord())
      ...
      setHaishuiConfigCache(previous => (previous ? { ...previous, mcp_servers: nextServers } : previous))
