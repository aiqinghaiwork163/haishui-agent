...
import { getLatestSessionMessages } from '@/haishui'
...
import { useHaishuiConfigRecord } from '../hooks/use-config-record'
...
import { useHaishuiConfig } from '../session/hooks/use-haishui-config'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  // "Restart Haishui" from a toast: recycle the local backend the user is
  ...
  useEffect(() => {
    ...
    if (backendRestartRequest > 0) {
      if ($connection.get()?.mode === 'remote') {
        void reconnectGateway().catch(err => notifyError(err, translateNow('notifications.errors.restartHaishuiFailed')))

        return
      }

      void window.haishuiDesktop?.recycleBackend?.(normalizeProfileKey($activeGatewayProfile.get())).catch(err =>
        notifyError(err, translateNow('notifications.errors.restartHaishuiFailed'))
      )
    }
  ...
  // Without this pin, the owner ladder sends session.create to haishui-setup
  ...
  const { refreshHaishuiConfig, sttEnabled, voiceMaxRecordingSeconds } = useHaishuiConfig({ activeSessionIdRef })
  ...
  useEffect(() => {
    ...
    window.addEventListener('haishui:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('haishui:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshHaishuiConfig(true)
    void refreshActiveProfile()
    resetProjectTreeState()
  }, [gatewayScope, refreshCurrentModel, refreshHaishuiConfig])
  ...
  const configRecord = useHaishuiConfigRecord()
  ...
  const customWindowControls = connection?.customWindowControls ?? window.haishuiDesktop?.windowControls?.custom ?? false
