...
import type { SessionInfo } from '@/haishui'
...
// zero Haishui sessions, and those arrive continuously — so once the user dragged
...
export function useRepoWorktreeMap(
  repoPaths: string[],
  enabled: boolean
): [Record<string, HaishuiGitWorktree[]>, boolean] {
  const [map, setMap] = useState<Record<string, HaishuiGitWorktree[]>>({})
