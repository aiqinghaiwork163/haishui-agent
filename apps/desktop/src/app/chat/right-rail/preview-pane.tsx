        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.haishuiDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({ embedded = false, onRestartServer, reloadRequest = 0, tabId, target }: PreviewPaneProps) {
  ...
  const annotateGuest = useCallback((): null | PreviewAnnotateGuest => {
    ...
    return {
      capture: async rect => {
        ...
        const dataUrl = await window.haishuiDesktop.capturePreview?.({ rect, viewport, webContentsId })
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Haishui to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.haishuiDesktop?.watchPreviewFile ||
      !window.haishuiDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.haishuiDesktop.onPreviewFileChanged(payload => {
    ...
    void window.haishuiDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.haishuiDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.haishuiDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:haishui-preview')
    ...
    // `haishui:openExternal` channel, never a popup side effect.
    const onGuestExternal = (event: Event) => {
      ...
      if (admitPreviewExternalUrl(url)) {
        void window.haishuiDesktop?.openExternal?.(url)
      }
    }
    ...
    const onGuestContextMenu = (event: Event) => {
      ...
      const zoom = window.haishuiDesktop?.zoom?.factor?.() || 1
      ...
      openGuestContextMenu(
        ...
        },
        {
          addToDictionary: (word: string) => {
            ...
            if (typeof webContentsId === 'number') {
              void window.haishuiDesktop?.contextMenuGuestAddWord?.({ webContentsId, word })
            }
          },
          copyImage: () => void window.haishuiDesktop?.contextMenuCopyImage?.(),
          ...
          <PreviewBrowserBar
            ...
            onOpenExternal={
              !isBrowserWindow() && !canOpenBrowserWindow()
                ? () => void window.haishuiDesktop?.openExternal(currentUrl)
                : undefined
            }
