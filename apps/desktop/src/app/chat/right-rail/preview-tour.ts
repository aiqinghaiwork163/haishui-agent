...
function buildTourScript(action: TourAction): string {
  return `(function () {
  var w = window;
  if (!w.__haishuiTourEngine) {
    ${driverIife}
    w.__haishuiTourHolder = {};
    w.__haishuiTourCollect = (${collectTourTargets.toString()});
    w.__haishuiTourEngine = (${runTourEngine.toString()});
  }
  if (!document.getElementById('__haishui-tour-style')) {
    var style = document.createElement('style');
    style.id = '__haishui-tour-style';
    style.textContent = ${JSON.stringify(driverCss)};
    (document.head || document.documentElement).appendChild(style);
  }
  return JSON.stringify(w.__haishuiTourEngine(
    w.driver.js.driver,
    w.__haishuiTourHolder,
    ${JSON.stringify(action)},
    w.__haishuiTourCollect,
    document
  ));
})()`
}
