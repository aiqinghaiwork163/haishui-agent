...
const preamble = () => `  var w = window;
  ...
  w.__haishuiActHolder = w.__haishuiActHolder || {};
  w.__haishuiAct = ${actEngineSource()};
  w.__haishuiWatch_fn = (${watchInPage.toString()});
  w.__haishuiWatchTag = ${WATCH_TAG};
  var holder = w.__haishuiActHolder;
  var act = function (a) { return w.__haishuiAct(document, holder, a); };
  var watch = function (stage, label) {
    try { w.__haishuiWatch_fn(document, holder, stage, label); } catch (err) {}
  };
...
${preamble()}
  ...
  document.addEventListener('pointerdown', function (e) {
    w.__haishuiHit = { tag: e.target ? e.target.tagName : '?', trusted: e.isTrusted === true };
...
function hitNote(hit?: { tag: string; trusted: boolean } | null): string | undefined {
  return hit && hit.tag === 'HAISHUI-WATCH' ? 'The action overlay intercepted the click instead of the page.' : undefined
}
