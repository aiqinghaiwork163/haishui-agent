...
export const HAISHUI_PATHS_MIME = 'application/x-haishui-paths'

/**
 * Eagerly resolve files from a drop event into [File?, path, isDirectory?]
 * triples. Internal Haishui sources (e.g. the project tree) ride on a custom
...
export function extractDroppedFiles(transfer: DataTransfer): DroppedFile[] {
  ...
  const getPath = window.haishuiDesktop?.getPathForFile
  ...
  try {
    const internalRaw = transfer.getData(HAISHUI_PATHS_MIME)
...
export function useComposerActions({
  ...
  const attachImageBlob = useCallback(
    async (blob: Blob, isCurrent: () => boolean = () => true) => {
      ...
      try {
        ...
        const savedPath = await window.haishuiDesktop?.saveImageBuffer(data, blobExtension(blob), name)
  ...
  const pasteClipboardImage = useCallback(
    async ({ silent = false }: { silent?: boolean } = {}) => {
      try {
        const path = await window.haishuiDesktop?.saveClipboardImage()
   ...
   * Haishui-managed composer-pastes file via the main process, then attached
  ...
  const attachPastedText = useCallback(
    async (text: string) => {
      const save = window.haishuiDesktop?.savePastedText
  ...
  const attachDroppedItems = useCallback(
    async (candidates: DroppedFile[]) => {
      ...
      for (const candidate of candidates) {
        ...
        const fallbackPath =
          !knownPath && window.haishuiDesktop?.getPathForFile ? window.haishuiDesktop.getPathForFile(file) : ''
