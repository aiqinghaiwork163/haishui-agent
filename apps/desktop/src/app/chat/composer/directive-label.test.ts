...
import { haishuiDirectiveFormatter } from '@/components/assistant-ui/directive-text'
...
function sentLabel(draft: string) {
  return haishuiDirectiveFormatter
    .parse(draft)
...
describe('one label per reference, on every surface', () => {
  ...
  it('an agent mention inserts as plain @name text, never an @simple: chip', () => {
    ...
    expect(haishuiDirectiveFormatter.serialize(item)).toBe('@mr-tester')
  ...
  it('a url still reads host + path on every surface', () => {
    const item = backendRow('@url:https://github.com/aiqinghaiwork163/haishui-agent/pull/74533', '', '')
    const { editor, result } = typed('@gith')

    act(() => result.current.replaceTriggerWithChip(item))

    const expected = 'github.com/aiqinghaiwork163/haishui-agent/pull/74533'
