...
function setWorkspaceTransfer(transferring: boolean): void {
  window.haishuiDesktop?.hud?.setWorkspaceTransfer?.(transferring)
}

function moveHud(state: PressState): void {
  window.haishuiDesktop?.hud?.moveBy?.({
    width: state.originW,
...
function armGrab(state: PressState, workspaceTransfer: boolean): void {
  ...
  window.haishuiDesktop?.hud?.beginMove?.()
}
 ...
 * (see haishui:hud:move-by — a transparent frameless window drifts wider on
...
export function useHudComposerDrag(
  ...
  const reset = useCallback(() => {
    ...
    if (state) {
      if (state.armed) {
        window.haishuiDesktop?.hud?.endMove?.()
      }
