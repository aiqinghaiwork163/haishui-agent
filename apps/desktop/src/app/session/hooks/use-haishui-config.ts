...
import { getHaishuiConfig, getHaishuiConfigDefaults } from '@/haishui'
...
interface HaishuiConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useHaishuiConfig({ activeSessionIdRef }: HaishuiConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshHaishuiConfig = useCallback(
    async (force = false, shouldPublish: () => boolean = () => true) => {
      ...
      try {
        const [config, defaults] = await Promise.all([getHaishuiConfig(), getHaishuiConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying Haishui' built-in medium over the user's config.
        if (!canPublish()) {
          return
        }
  ...
  return { refreshHaishuiConfig, sttEnabled, voiceMaxRecordingSeconds }
}
