...
import { getHaishuiConfig } from '@/haishui'
...
import { useHaishuiConfig } from './use-haishui-config'

vi.mock('@/haishui', () => ({
  getHaishuiConfig: vi.fn(),
  getHaishuiConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'haishui.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getHaishuiConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getHaishuiConfig>>)

describe('useHaishuiConfig refreshHaishuiConfig', () => {
  beforeEach(() => {
  ...
  it('mirrors display.show_reasoning and resets a missing key to the default', async () => {
    ...
    const { result } = renderHook(() => useHaishuiConfig({ activeSessionIdRef: { current: null } }))

    await act(async () => {
      await result.current.refreshHaishuiConfig()
    })
  ...
  // `agent.reasoning_effort: high` to Haishui' built-in medium.
  ...
  it('does not let terminal.cwd replace an active session workspace', async () => {
    ...
    mockConfig({ terminal: { cwd: '/Users/example/new-workspace' } })
    const { result } = renderHook(() => useHaishuiConfig({ activeSessionIdRef: { current: 'session-1' } }))
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getHaishuiConfig>>>()
    vi.mocked(getHaishuiConfig).mockReturnValueOnce(profileConfig.promise)

    const { result } = renderHook(() => useHaishuiConfig({ activeSessionIdRef: { current: null } }))

    let pendingRefresh!: Promise<void>
    act(() => {
      pendingRefresh = result.current.refreshHaishuiConfig(true)
    })
    expect(getHaishuiConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getHaishuiConfig>>)
  ...
  it('does not publish config after its switch loses ownership', async () => {
    const staleConfig = deferred<Awaited<ReturnType<typeof getHaishuiConfig>>>()
    vi.mocked(getHaishuiConfig).mockReturnValueOnce(staleConfig.promise)
    const { result } = renderHook(() => useHaishuiConfig({ activeSessionIdRef: { current: null } }))
    let ownsSwitch = true

    let refresh!: Promise<void>
    act(() => {
      refresh = result.current.refreshHaishuiConfig(false, () => ownsSwitch)
    })
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getHaishuiConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getHaishuiConfig>>>()
    vi.mocked(getHaishuiConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshHaishuiConfig(true)
      refreshC = result.current.refreshHaishuiConfig(true)
    })
