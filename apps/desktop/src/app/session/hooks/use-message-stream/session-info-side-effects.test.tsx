...
let refreshHaishuiConfig: ReturnType<typeof vi.fn<() => Promise<void>>>
...
beforeEach(() => {
  sessionStates = null
  refreshHaishuiConfig = vi.fn<() => Promise<void>>(async () => undefined)
...
describe('session.info config refetch gating', () => {
  it('coalesces active-session bursts into one trailing config fetch', async () => {
    ...
    expect(refreshHaishuiConfig).not.toHaveBeenCalled()
