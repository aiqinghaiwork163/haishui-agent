import type { BillingBlock } from '@haishui/shared'
 ...
 * billing-specific toast — never the generic "Haishui error" — with a smart CTA
  ...
  if (event.type === 'message.complete') {
    ...
    const failure =
      payload?.status === 'error'
        ? {
            error: coerceGatewayText(payload.error).trim() || finalText || 'Haishui reported an error',
