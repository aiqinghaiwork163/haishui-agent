...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  const { activeSessionIdRef, compactedTurnRef, refreshHaishuiConfig, sessionStateByRuntimeIdRef } = deps
  ...
  // refreshHaishuiConfig — two REST calls (config + defaults) per event, per
