...
import { capabilityScoped, haishuiApi, type ProfileScope, STARTUP_REQUEST_TIMEOUT_MS } from './client'

export function getProfiles(scope?: ProfileScope): Promise<ProfilesResponse> {
  return haishuiApi<ProfilesResponse>({
...
export function createProfile(body: ProfileCreatePayload): Promise<{ name: string; ok: boolean; path: string }> {
  return haishuiApi<{ name: string; ok: boolean; path: string }>({
    path: '/api/profiles',
    method: 'POST',
    body
  })
...
export function deleteProfile(name: string, scope?: ProfileScope): Promise<{ ok: boolean; path: string }> {
  ...
  return haishuiApi<{ ok: boolean; path: string }>({
    ...profileOwnerScoped(scope),
...
export function getProfileSetupCommand(name: string): Promise<ProfileSetupCommand> {
  return haishuiApi<ProfileSetupCommand>({
...
export function exportProfileArchive(
  ...
  return haishuiApi<{ archive: string; ok: boolean }>({
...
export function importProfileArchive(
  ...
  return haishuiApi<{ desktop: null | ProfileDesktopOverlay; name: string; ok: boolean; path: string }>({
