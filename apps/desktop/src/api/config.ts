...
export function getStatus(): Promise<StatusResponse> {
  return haishuiApi<StatusResponse>({
    ...profileScoped(),
    path: '/api/status'
  })
}
...
export function getLogs(params: {
  ...
  return haishuiApi<LogsResponse>({
    ...profileScoped(),
    path: suffix ? `/api/logs?${suffix}` : '/api/logs'
  })
}

export function getHaishuiConfig(profile?: string): Promise<HaishuiConfig> {
  return haishuiApi<HaishuiConfig>({
...
export function getHaishuiConfigRecord(
  profile?: ProfileScope,
  { includeDefaults = true }: { includeDefaults?: boolean } = {}
): Promise<HaishuiConfigRecord> {
  return window.haishuiDesktop.api<HaishuiConfigRecord>({
    ...capabilityScoped(profile),
    path: includeDefaults ? '/api/config' : '/api/config?include_defaults=false'
  })
}

export function getHaishuiConfigDefaults(): Promise<HaishuiConfigRecord> {
  return haishuiApi<HaishuiConfigRecord>({
    ...profileScoped(),
...
export function getHaishuiConfigSchema(profile?: null | string): Promise<ConfigSchemaResponse> {
  return haishuiApi<ConfigSchemaResponse>({
    ...profileScoped(profile),
    path: '/api/config/schema'
  })
}

export function saveHaishuiConfig(
  config: HaishuiConfigRecord,
  profile?: null | string,
  { preserveLanguage = false }: { preserveLanguage?: boolean } = {}
): Promise<{ ok: boolean }> {
  return haishuiApi<{ ok: boolean }>({
    ...profileScoped(profile),
...
/** Capability-scoped counterpart of saveHaishuiConfig — writes the config of
 *  the profile/connection the Capabilities scope selector points at (possibly
 *  on another registered gateway), mirroring getHaishuiConfigRecord. */
export function saveHaishuiConfigRecord(config: HaishuiConfigRecord, profile?: ProfileScope): Promise<{ ok: boolean }> {
  return window.haishuiDesktop.api<{ ok: boolean }>({
    ...capabilityScoped(profile),
    path: '/api/config',
    method: 'PUT',
    body: { config }
...
export function getEnvVars(profile?: null | string): Promise<Record<string, EnvVarInfo>> {
  return haishuiApi<Record<string, EnvVarInfo>>({
    ...profileScoped(profile),
    path: '/api/env'
  })
}
...
export function revealEnvVar(key: string, profile?: ProfileScope): Promise<{ key: string; value: string }> {
  return window.haishuiDesktop.api<{ key: string; value: string }>({
    ...capabilityScoped(profile),
    path: '/api/env/reveal',
    method: 'POST',
    body: { key }
...
export function validateProviderCredential(
  ...
  return haishuiApi<{ ok: boolean; reachable: boolean; message: string; models?: string[] }>({
    ...profileScoped(),
...
export function getCustomEndpoints(): Promise<CustomEndpointsResponse> {
  return haishuiApi<CustomEndpointsResponse>({
    ...profileScoped(),
...
export function validateCustomEndpoint(endpoint: CustomEndpointUpdate): Promise<CustomEndpointValidationResponse> {
  return haishuiApi<CustomEndpointValidationResponse>({
...
export function activateCustomEndpoint(id: string): Promise<{ ok: boolean; provider: string; model: string }> {
  return haishuiApi<{ ok: boolean; provider: string; model: string }>({
    ...profileScoped(),
...
export function listOAuthProviders(profile?: null | string): Promise<OAuthProvidersResponse> {
  return haishuiApi<OAuthProvidersResponse>({
    ...profileScoped(profile),
...
export function disconnectOAuthProvider(
  ...
  return haishuiApi<{ ok: boolean; provider: string }>({
    ...profileScoped(profile),
...
export function startOAuthLogin(providerId: string, profile?: ProfileScope): Promise<OAuthStartResponse> {
  return window.haishuiDesktop.api<OAuthStartResponse>({
    ...capabilityScoped(profile),
...
export function submitOAuthCode(
  ...
  return haishuiApi<OAuthSubmitResponse>({
    ...profileScoped(profile),
...
export function pollOAuthSession(
  ...
  return window.haishuiDesktop.api<OAuthPollResponse>({
    ...capabilityScoped(profile),
