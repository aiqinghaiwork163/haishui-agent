import type { McpCatalogResponse, McpServerSummary } from '@/types/haishui'
...
export function testMcpServer(name: string, profile?: ProfileScope): Promise<McpTestResult> {
  return window.haishuiDesktop.api<McpTestResult>({
 ...
 *  `saveHaishuiConfig`, this REPLACES rather than deep-merges, so deletes,
...
// `haishui mcp` and the dashboard MCP page). Raw JSON editing stays in
// config.yaml via saveHaishuiConfig.
// ---------------------------------------------------------------------------

export function listMcpServers(): Promise<{ servers: McpServerSummary[] }> {
  return haishuiApi<{ servers: McpServerSummary[] }>({
    ...profileScoped(),
    path: '/api/mcp/servers'
  })
}
...
export function addMcpServer(
  ...
  return window.haishuiDesktop.api<McpServerSummary>({
    ...capabilityScoped(profile),
    path: '/api/mcp/servers',
    method: 'POST',
    body
...
export function getMcpCatalog(profile?: ProfileScope, detectApps = false): Promise<McpCatalogResponse> {
  return window.haishuiDesktop.api<McpCatalogResponse>({
    ...capabilityScoped(profile),
...
export function installMcpCatalogEntry(
  ...
  return window.haishuiDesktop.api<{ ok: boolean; name?: string; pid?: number; action?: string; background?: boolean }>({
    ...capabilityScoped(profile),
