import { JsonRpcGatewayClient } from '@haishui/shared'

import type { HaishuiApiRequest } from '@/global'
...
// "Timed out connecting to Haishui backend" that hangs the UI (#48504).
//
...
export class HaishuiGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Haishui gateway connection closed',
      connectErrorMessage: 'Could not connect to Haishui gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'Haishui gateway is not connected',
      ...
      onUnhandledRequest: request =>
        console.warn(`[gateway] Haishui Desktop has no server-request registry for ${request.method} (${request.id})`),
...
// process to route them to that host (haishui:api's registry branch). Null
...
export function haishuiApi<T>(request: HaishuiApiRequest): Promise<T> {
  return window.haishuiDesktop.api<T>({ ...connectionScoped(), ...request })
}
