 ...
 *    a `HaishuiPlugin` registers automatically (vite glob — drop a folder in).
 *    `haishui-bots` (Bot Mode) ships in-tree and is ON by default; other
 *    reference/demo plugins live in the companion `haishui-example-plugins`
 ...
 *  - RUNTIME: the on-disk doors (`<haishui home>/desktop-plugins/<name>/plugin.js`
 *    and the unified-package half `<haishui home>/plugins/<name>/desktop/plugin.js`)
...
const modules = import.meta.glob<{ default: HaishuiPlugin }>('../plugins/*/plugin.{js,ts,tsx}', { eager: true })
...
export function discoverBundledPlugins(): void {
  ...
  for (const [path, mod] of Object.entries(modules)) {
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      console.warn(`[plugins] ${path} has no valid default HaishuiPlugin export — skipped`)
