 ...
 * `HaishuiPlugin`; it never touches the registry directly — it receives a
...
import { pluginRest, type PluginRestOptions, pluginSocket } from '@/haishui'
...
export type { PluginRestOptions } from '@/haishui'
 ...
 *  under `haishui.plugin.<id>.` — plugins can't read or clobber each other. */
 ...
 *  in one attributed namespace instead of the raw `window.haishuiDesktop`
...
export interface PluginOs {
   ...
   *  the user is away from Haishui — use `host.notify` for the in-app toast.
   *  Throttled per plugin; reserve it for genuinely notable events.
   *  Supports `icon`, `activate` (e.g. `haishui://index-network/intent/1`),
...
export interface HaishuiPlugin {
...
function createPluginStorage(pluginId: string): PluginStorage {
  const scoped = (key: string) => `haishui.plugin.${pluginId}.${key}`
...
function createPluginOs(pluginId: string): PluginOs {
  const attempt = async (run: (bridge: NonNullable<typeof window.haishuiDesktop>) => Promise<boolean>) => {
    const bridge = typeof window === 'undefined' ? undefined : window.haishuiDesktop
  ...
  const attemptPath = async (run: (bridge: NonNullable<typeof window.haishuiDesktop>) => Promise<null | string>) => {
    const bridge = typeof window === 'undefined' ? undefined : window.haishuiDesktop
