...
import type { HaishuiReadDirResult } from '@/global'
import type * as HaishuiModule from '@/haishui'
...
// getStatus would supply the connected backend's haishui_home — a REMOTE path in
// remote mode. The disk scanner must NOT derive the plugin root from it (#66899).
const getStatus = vi.fn(async () => ({ haishui_home: '/remote/box/.haishui' }))

vi.mock('@/haishui', async importActual => ({
  ...(await importActual<typeof HaishuiModule>()),
  getStatus: () => getStatus()
}))

const desktopPluginsRoot = vi.fn<() => Promise<string>>()
const readDir = vi.fn<(path: string) => Promise<HaishuiReadDirResult>>()
...
beforeEach(() => {
  ...
  getStatus.mockClear()
  ;(window as unknown as { haishuiDesktop: unknown }).haishuiDesktop = {
    desktopPluginsRoot,
...
afterEach(() => {
  delete (window as unknown as { haishuiDesktop?: unknown }).haishuiDesktop
})

describe('scanDiskPlugins (#66899)', () => {
  it('scans the Electron-resolved local roots, never the backend haishui_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.haishui/desktop-plugins')
    readDir.mockResolvedValue({ entries: [] })

    await discoverRuntimePlugins()

    expect(desktopPluginsRoot).toHaveBeenCalled()
    expect(readDir).toHaveBeenCalledWith('/local/.haishui/desktop-plugins')
    ...
    expect(readDir).not.toHaveBeenCalledWith('/local/.haishui/plugins')
    // The remote backend's haishui_home must never feed the local plugin scan.
    expect(getStatus).not.toHaveBeenCalled()
    expect(readDir).not.toHaveBeenCalledWith('/remote/box/.haishui/desktop-plugins')
  })
  ...
  it('treats a folder without plugin.js as metadata, not a throwing file read', async () => {
    ...
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.haishui/desktop-plugins') {
        return {
          entries: [{ isDirectory: true, name: 'my-feature', path: '/local/.haishui/desktop-plugins/my-feature' }]
        }
      }

      if (dir === '/local/.haishui/desktop-plugins/my-feature') {
        return {
          entries: [
            { isDirectory: false, name: 'README.md', path: '/local/.haishui/desktop-plugins/my-feature/README.md' }
          ]
        }
      }
    ...
    expect(readDir).toHaveBeenCalledWith('/local/.haishui/desktop-plugins/my-feature')
  ...
  it('a DIRECTORY named plugin.js is not a plugin entry (metadata walk rejects it)', async () => {
    ...
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.haishui/desktop-plugins') {
        return { entries: [{ isDirectory: true, name: 'odd', path: '/local/.haishui/desktop-plugins/odd' }] }
      }

      if (dir === '/local/.haishui/desktop-plugins/odd') {
        // A folder literally named plugin.js — must resolve to "no entry".
        return {
          entries: [{ isDirectory: true, name: 'plugin.js', path: '/local/.haishui/desktop-plugins/odd/plugin.js' }]
        }
      }
  ...
  it('loads a unified desktop half (app-root copy + package marker) OPT-IN and tags it with its package', async () => {
    ...
    const root = '/local/.haishui/desktop-plugins'
    ...
    readDir.mockImplementation(async dir => {
      ...
      if (dir === `${root}/uni`) {
        return {
          entries: [
            { isDirectory: false, name: '.haishui-package.json', path: `${root}/uni/.haishui-package.json` },
    ...
    readFileText.mockImplementation(async file =>
      file.endsWith('.haishui-package.json')
...
describe('watchRuntimePlugins dir watch (#66899)', () => {
  it('watches the Electron-resolved app root, never the backend haishui_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.haishui/desktop-plugins')
    readDir.mockResolvedValue({ entries: [] })
    ...
    expect(watchDirectory).toHaveBeenCalledWith('/local/.haishui/desktop-plugins')
    expect(watchDirectory).not.toHaveBeenCalledWith('/remote/box/.haishui/desktop-plugins')
    expect(getStatus).not.toHaveBeenCalled()
  })
})
...
describe('plugin source reads (512 KiB preview-cap bug)', () => {
  ...
  const standaloneRootWith = (name: string) => {
    const folder = `/local/.haishui/desktop-plugins/${name}`
  ...
  it('loads the full source via readPluginSource when the shell offers it', async () => {
    ;(window.haishuiDesktop as unknown as { readPluginSource: unknown }).readPluginSource = readPluginSource
    desktopPluginsRoot.mockResolvedValue('/local/.haishui/desktop-plugins')
    ...
    try {
      ...
      expect(readPluginSource).toHaveBeenCalledWith('/local/.haishui/desktop-plugins/big/plugin.js')
      expect(register).toHaveBeenCalledTimes(1)
  ...
  it('older shell without readPluginSource: a truncated preview read fails LOUDLY, never evaluates', async () => {
    ...
    try {
      ...
      expect($pluginRecords.get().huge).toMatchObject({
        kind: 'disk',
        status: 'error',
        file: '/local/.haishui/desktop-plugins/huge/plugin.js'
      })
  ...
  it('disposes runtime host event subscriptions before a hot reload (#112366)', async () => {
    ...
    try {
      const source = `
        import { host } from '@haishui/plugin-sdk'
        export default {
...
describe('bundled-shadowed disk copies', () => {
  it('skips a disk copy of a bundled plugin but publishes a visible inventory row', async () => {
    ...
    publishPlugin({ id: 'haishui-bots', name: 'Bot Mode', kind: 'bundled', status: 'loaded' })
    ...
    try {
      const id = await loadRuntimePlugin(
        'export default { id: "haishui-bots", name: "Bot Mode", register() {} }',
        'haishui-bots',
        { file: '/local/.haishui/desktop-plugins/haishui-bots/plugin.js' }
      )

      // Skipped — the bundled copy stays the only live registration...
      expect(id).toBeNull()
      expect($pluginRecords.get()['haishui-bots']).toMatchObject({ kind: 'bundled', status: 'loaded' })
      ...
      expect($pluginRecords.get()['haishui-bots:disk-shadowed']).toMatchObject({
        kind: 'disk',
        status: 'disabled',
        file: '/local/.haishui/desktop-plugins/haishui-bots/plugin.js'
      })
