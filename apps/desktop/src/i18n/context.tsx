import { applyDocumentLocale, isRecord } from '@haishui/shared/i18n'
import { createContext, type ReactNode, useCallback, useContext, useEffect, useMemo, useRef, useState } from 'react'

import { getHaishuiConfigRecord, type HaishuiConfigRecord, saveHaishuiConfig } from '@/haishui'
...
export interface I18nConfigClient {
  getConfig: () => Promise<HaishuiConfigRecord>
  saveConfig: (config: HaishuiConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.haishuiDesktop?.api) {
    ...
    return getHaishuiConfigRecord(undefined, { includeDefaults: false })
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.haishuiDesktop?.api) {
    ...
    return saveHaishuiConfig(config, undefined, { preserveLanguage: true })
  }
}

export function getConfigDisplayLanguage(config: HaishuiConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: HaishuiConfigRecord, locale: Locale): HaishuiConfigRecord {
    ...
    const loadLocale = () => {
      ...
      return configClient
        ...
        .then(async config => {
          ...
          const machineProfile = await window.haishuiDesktop?.getMachineProfile?.().catch(() => null)
