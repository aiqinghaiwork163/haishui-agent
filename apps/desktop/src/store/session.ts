import type { ConnectionState } from '@haishui/shared'
...
import type { SessionInfo, UsageStats } from '@/types/haishui'
...
const COMPOSER_MODEL_KEY = 'haishui.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'haishui.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'haishui.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'haishui.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'haishui.desktop.composer.fast'
...
function composerScopeForConnection(connection: HaishuiConnection | null): string | null {
  if (!connection) {
    return null
  }
...
const LAST_SESSION_KEY = 'haishui.desktop.lastSessionId'
const LAST_ROUTE_KEY = 'haishui.desktop.lastRoute'
...
function workspaceCwdKey(connection: HaishuiConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(shouldPublish: () => boolean = () => true): Promise<string> {
  const settings = window.haishuiDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(shouldPublish: () => boolean = () => true): Promise<void> {
  const sanitize = window.haishuiDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<HaishuiConnection | null>(null)
...
const SESSION_OWNER_HINTS_KEY = 'haishui.desktop.sessionOwnerHints.v1'
 ...
 * HaishuiConnection lookup is unavailable. */
...
export const setConnection = (next: Updater<HaishuiConnection | null>) => {
...
// until config loads, and re-seeded on every profile switch by useHaishuiConfig.
