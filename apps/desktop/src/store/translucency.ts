 ...
 * `@haishui/shared/translucency`, which owns the mapping both this store and the
 * main process read.
 *
 ...
 * material underneath the web contents to read (see the `[data-haishui-glass]`
 * block in styles.css).
 */
...
} from '@haishui/shared/translucency'
 ...
 * sets `haishuiDesktop.glassSupported` from `os.release()` so Win10 cannot
...
export const GLASS_SUPPORTED =
  typeof window !== 'undefined' && typeof window.haishuiDesktop?.glassSupported === 'boolean'
    ? window.haishuiDesktop.glassSupported
...
export const TRANSLUCENCY_SUPPORTED =
  typeof window !== 'undefined' && typeof window.haishuiDesktop?.translucencySupported === 'boolean'
    ? window.haishuiDesktop.translucencySupported
    : isMacPlatform() || isWindowsPlatform()
...
/* Peek: while the user is actively adjusting translucency from Settings, the
   ...
   [data-haishui-translucency-peek] rules in styles.css). A counter rather
   than a boolean: a held slider drag and a timed pulse from a picker click
   can overlap. */
const PEEK_ATTR = 'data-haishui-translucency-peek'
...
const applyGlassSurfaces = ({ intensity, mode, scope }: TranslucencyState): void => {
  ...
  root.toggleAttribute('data-haishui-glass-on', glassLive)
  root.toggleAttribute('data-haishui-glass', glassOn)
  root.toggleAttribute('data-haishui-clear', clearOn)
  ...
  if (glassOn) {
    root.setAttribute('data-haishui-glass-scope', scope)
  } else {
    root.removeAttribute('data-haishui-glass-scope')
  }
...
if (typeof window !== 'undefined') {
  ...
  $translucency.subscribe(state => {
    ...
    window.haishuiDesktop?.setTranslucency?.(state)
  })
