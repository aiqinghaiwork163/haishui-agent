 ...
 *  Working in `~/www/haishui-agent`, the draft says "haishui-agent" constantly
 *  — a "Use skill: haishui-agent" pill on every mention is noise (the reported
 *  annoyance that prompted this guard). Boundary containment, not exact
 *  segment equality, so worktree dirs (`haishui-agent-suggest`) suppress too. */
 ...
 *  segment of a qualified form (`github/haishui-agent-dev`, `plugin:skill`). */
