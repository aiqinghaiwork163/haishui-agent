 ...
 *
 * Local models ship on main behind `--local` (either `haishui desktop --local`
 * or the flag on Haishui.exe itself). The flag is strict: without it the GUI
...
export const $localModelsEnabled = atom<boolean>(
  typeof window !== 'undefined' && window.haishuiDesktop?.localModelsEnabled === true
)
