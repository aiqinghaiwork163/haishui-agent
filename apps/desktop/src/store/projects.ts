...
import type { HaishuiGitBaseBranch, HaishuiGitBranch } from '@/global'
import { getHaishuiConfig, haishuiApi, type HaishuiGateway } from '@/haishui'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/haishui'
...
const PROJECT_SCOPE_KEY = 'haishui.desktop.projectScope'
...
async function activeProjectsContext(profile = projectProfile()): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== normalizeProfileKey($activeGatewayProfile.get())) {
    throw new Error('Active Haishui profile changed while connecting')
  }
...
async function refreshProjectTreeAcrossProfiles(): Promise<void> {
  ...
  try {
    const res = await haishuiApi<ProjectTreePayload>({
...
const repoScanStates = new WeakMap<HaishuiGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<HaishuiGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  if (isDesktopFsRemoteMode()) {
    ...
    // with `scan: true` — added in #81723) so repos with zero Haishui
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getHaishuiConfig(context.profile))
...
// repo, returning where Haishui should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<HaishuiGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<HaishuiGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.haishuiDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.haishuiDesktop?.writeClipboard?.(path)
  }
}
