...
import { checkHaishuiUpdate, getActionStatus, updateHaishui } from '@/haishui'
...
import type { BackendUpdateCheckResponse } from '@/types/haishui'
...
const UPDATE_TOAST_SNOOZE_KEY = 'haishui:update-toast-snooze-until'
...
const SKEW_TOAST_SNOOZE_KEY = 'haishui:backend-skew-toast-snooze-until'
...
const INSTALL_METHOD_TOAST_SNOOZE_KEY = 'haishui:install-method-toast-snooze-until'
...
export function reportBackendContract(contract: number | undefined): void {
  ...
  notify({
    action: {
      label: translateNow('notifications.updateHaishui'),
      onClick: () => {
        snoozeSkewToast()
...
export async function refreshDesktopVersion(): Promise<DesktopVersionInfo | null> {
  ...
  try {
    const next = await window.haishuiDesktop?.getVersion?.()
...
export async function checkBackendUpdates({
  ...
  try {
    const status = mapBackendCheck(await checkHaishuiUpdate(force))
...
export async function checkUpdates({ force = false }: UpdateCheckOptions = {}): Promise<DesktopUpdateStatus | null> {
  const bridge = window.haishuiDesktop?.updates
...
export async function applyUpdates(opts: DesktopUpdateApplyOptions = {}): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    ...
    // `haishui update` themselves. Land on a dedicated manual state so the
    ...
    if (result?.manual) {
      $updateApply.set({
        ...
        applying: false,
        stage: 'manual',
        message: result.command ?? 'haishui update',
        command: result.command ?? 'haishui update'
      })
...
function completedAfterRestart(
  ...
  return !!actionId && status.lines.some(line => line === `=== haishui-update completed ${actionId} ===`)
}
...
async function runBackendUpdate(): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    ...
    const started = await updateHaishui()
    ...
    if (!started.ok) {
      ...
      const command = (started as { update_command?: string }).update_command || 'haishui update'
    ...
    while (Date.now() < deadline) {
      ...
      if (!started.action_id && last.exit_code === null) {
        try {
          const status = await checkHaishuiUpdate(true)
...
const FOCUS_RECHECK_KEY = 'haishui.updates.last-passive-check'
