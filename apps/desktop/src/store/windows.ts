...
export function canOpenSessionWindow(): boolean {
  return typeof window !== 'undefined' && typeof window.haishuiDesktop?.openSessionWindow === 'function'
}

// True when the shell can open a full peer app window (⌘⇧N / "New Window").
export function canOpenNewWindow(): boolean {
  return typeof window !== 'undefined' && typeof window.haishuiDesktop?.openWindow === 'function'
}

// True when the shell can pop the in-app Browser into its own OS window.
export function canOpenBrowserWindow(): boolean {
  return typeof window !== 'undefined' && typeof window.haishuiDesktop?.openBrowserWindow === 'function'
}
...
export function canOpenSessionInTerminal(): boolean {
  return typeof window !== 'undefined' && typeof window.haishuiDesktop?.openSessionInTerminal === 'function'
}
...
export async function openSessionInNewWindow(sessionId: string, opts?: { watch?: boolean }): Promise<void> {
  ...
  await runWindowOpen(
    () => window.haishuiDesktop.openSessionWindow(sessionId, { ...opts, profile }),
...
export async function openNewWindow(route?: { connectionId: null | string; profile: string }): Promise<void> {
  ...
  await runWindowOpen(() => window.haishuiDesktop.openWindow(route), 'Could not open a new window')
}
...
export async function openBrowserInNewWindow(tabId: string): Promise<boolean> {
  ...
  return runWindowOpen(() => window.haishuiDesktop.openBrowserWindow(tabId), 'Could not pop out browser')
}
...
export async function openSessionInTerminal(
  ...
  await runWindowOpen(
    () => window.haishuiDesktop.openSessionInTerminal(sessionId, opts),
