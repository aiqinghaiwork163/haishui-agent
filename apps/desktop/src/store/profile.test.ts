...
const { getProfiles } = await import('@/haishui')
...
const profile = (name: string, isDefault = false): ProfileInfo => ({
  ...
  model: null,
  name,
  path: `/tmp/haishui/${name}`,
  provider: null,
  skill_count: 0
})

const remoteConn = (over: Partial<HaishuiConnection> = {}): HaishuiConnection =>
  ({ baseUrl: 'https://haishui-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as HaishuiConnection
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { haishuiDesktop: { getConnection } })
