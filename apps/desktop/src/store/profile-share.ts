 ...
 *
 * The archive is the CLI's own `haishui profile export` tar.gz (config, skills,
...
import { exportProfileArchive, importProfileArchive } from '@/haishui'
...
import type { ProfileDesktopOverlay } from '@/types/haishui'
...
const ARCHIVE_FILTERS = [{ extensions: ['tar.gz', 'tgz'], name: 'Haishui profile' }]
...
export async function runExportProfileFlow(profile?: string): Promise<null | string> {
  ...
  const pick = window.haishuiDesktop?.selectSavePath
...
export async function runImportProfileFlow(): Promise<null | string> {
  const paths = await window.haishuiDesktop?.selectPaths?.({
