...
import type { HaishuiConnection } from '@/global'
import { HaishuiGateway, setApiRequestConnection } from '@/haishui'
...
function dialProfile(
  desktop: NonNullable<typeof window.haishuiDesktop>,
  profile: string,
...
const isOpen = (gateway: HaishuiGateway | null): boolean => gateway?.connectionState === 'open'
...
interface RegistryConfig {
  ...
  onActiveConnectionChanged?: (connection: HaishuiConnection) => void
  /**
...
interface Secondary {
  ...
  connection: HaishuiConnection | null
  gateway: HaishuiGateway
  /**
...
interface GatewayRegistryState {
  ...
  primaryGateway: HaishuiGateway | null
   ...
   *  `haishui serve --profile <primary>` child and can never stand in for a
  ...
  $gateway: ReturnType<typeof atom<HaishuiGateway | null>>
  $activeProfile: ReturnType<typeof atom<string>>
}

const STATE_KEY = Symbol.for('haishui.desktop.gatewayRegistryState')
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<HaishuiGateway | null>(null),
...
export function setPrimaryGateway(gateway: HaishuiGateway | null, profile = 'default'): void {
...
export function setPrimaryGatewayConnection(connection: Pick<HaishuiConnection, 'connectionId' | 'mode'> | null): void {
...
async function isAttachedSharedRemote(
  ...
  // A local primary is one `haishui serve --profile <primary>` child; every other
  ...
  try {
    ...
    // (early return above): it is one `haishui serve --profile <primary>` child
...
export function activeGateway(): HaishuiGateway | null {
  if (g.activeKey === g.primaryProfile) {
...
function applyActive(profile: string, activationEpoch: number): boolean {
  ...
  // Push the active scope's registry connection into the haishui module (null
...
function publishActiveConnection(connection: HaishuiConnection): void {
...
function createSecondary(profile: string, connectionId: null | string = null): Secondary {
  const gateway = new HaishuiGateway()
...
async function gatewayForProfile(
  ...
  leaseRequest = false,
  spawnPriority: SpawnPriority = 'background'
): Promise<{ gateway: HaishuiGateway | null; key: string; release: () => void; scopeProfile: boolean }> {
  const key = normKey(profile)
...
export async function requestGatewayForProfile<T>(
  ...
  try {
    if (!route.gateway) {
      throw new Error(`Haishui gateway unavailable for profile "${route.key}"`)
    }
...
export async function requestGatewayForAgent<T>(
  ...
  if (!window.haishuiDesktop?.getConnectionFor) {
    throw new Error('This Desktop build cannot dial registry connections. Update Haishui Desktop.')
  }
...
function publishTurnLease(scope: string, activeTurn: boolean): void {
  void window.haishuiDesktop?.touchBackend?.(scope, { activeTurn }).catch(() => undefined)
}
...
export async function ensureActiveGatewayOpen({ explicit = false }: { explicit?: boolean } = {}): Promise<HaishuiGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }
  ...
  if (!isOpen(entry.gateway)) {
    ...
    // "Haishui gateway is not connected" on the Sessions `+` action (#88880).
