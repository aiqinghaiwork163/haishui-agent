 ...
 * app's bounds and stays visible while Haishui is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'haishui.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'haishui.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.haishuiDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.haishuiDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.haishuiDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.haishuiDesktop?.petOverlay?.close()
}
