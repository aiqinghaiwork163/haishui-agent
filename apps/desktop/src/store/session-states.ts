...
import { type GatewayEvent, LOCAL_CONNECTION_ID, registryBackendScopeKey } from '@haishui/shared'
...
const TILES_KEY = 'haishui.desktop.sessionTiles.v2'
const LEGACY_TILES_KEY = 'haishui.desktop.sessionTiles.v1'
...
const BOT_CHAT_SCOPE_KEY = 'haishui.desktop.botChatSessions.v1'
 ...
 *  (haishui-agent#90102). The tile bucket is a Local Storage cache, and a
 ...
 * restore re-dials the profile's backend, whose ensure_haishui_home() re-creates
 * the profile directory the delete just removed (haishui-agent#94235). Same
...
export function dropTilesForProfile(
  ...
  const ownerMatches = (owner: SessionProfileRoute | undefined): boolean => {
    ...
    // would orphan a live conversation (haishui-agent#94235). Tiles persisted
...
// Dev hook for automation (mirrors __HAISHUI_LAYOUT_TREE__).
if ((import.meta.env.DEV || import.meta.env.VITE_PERF_PROBE === '1') && typeof window !== 'undefined') {
  ;(window as unknown as Record<string, unknown>).__HAISHUI_SESSION_TILES__ = {
