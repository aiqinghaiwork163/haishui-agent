...
/**
 * Feature store for backend (agent) plugins — the native Haishui plugins plus
 ...
 * primitives `haishui plugins` and the dashboard use, so all surfaces agree on
...
export interface AgentPluginRow {
  ...
  /** Agent Plugins v1 package (portable skills/MCP format) vs native Haishui. */
  portable?: boolean
 ...
 *  HAISHUI_HOME. Always refetches (it's a cheap local disk scan on the
...
export async function installAgentPlugin(
  ...
  opts: {
    ...
    ref?: string
    /** Target profile's HAISHUI_HOME (null/undefined = backend launch profile). */
    profile?: string | null
  }
