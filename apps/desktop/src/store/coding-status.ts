...
import type { HaishuiGitWorktree, HaishuiRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: HaishuiGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, HaishuiRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, HaishuiGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<HaishuiRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<HaishuiGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<HaishuiRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<HaishuiGitWorktree[]>>()
const $noRepoStatus = atom<HaishuiRepoStatus | null>(null)
const $noWorktrees = atom<HaishuiGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<HaishuiRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<HaishuiGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: HaishuiRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: HaishuiGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<HaishuiRepoStatus | null>
  seq: number
  target: string
}
