import { LOCAL_CONNECTION_ID, registryBackendScopeKey } from '@haishui/shared'
import { atom, batch, computed } from 'nanostores'

import type { HaishuiConnection } from '@/global'
import { getProfiles, haishuiApi, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/haishui'
...
import type { ProfileInfo } from '@/types/haishui'
...
// /api/profiles/active `current`). "default" is the root ~/.haishui. This is the
...
function profileListSource(connection: HaishuiConnection | null): null | string {
  if (!connection) {
    return null
  }
...
const PROFILE_ORDER_STORAGE_KEY = 'haishui.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'haishui.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  ...
  try {
    const res = await haishuiApi<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new HAISHUI_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.haishuiDesktop.profile.set(name)
}
...
export function prewarmProfileBackend(name: string, connectionId: null | string = null): void {
  ...
  // tunnel and spawns `haishui -p <profile> serve --isolated` on the remote
...
async function resolveConnectionForProfile(profile: string): Promise<HaishuiConnection | null> {
  const getConnection = window.haishuiDesktop?.getConnection
...
export async function ensureGatewayProfile(
  ...
  // #89206 split-brain — observed live as atom 'default' over a haishui-setup
...
// previous descriptor.
async function resolveConnectionForAgent(connectionId: string, profile: string): Promise<HaishuiConnection | null> {
  const getConnectionFor = window.haishuiDesktop?.getConnectionFor
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'haishui.desktop.showAllProfiles'
...
export function selectProfile(name: string): void {
  ...
  void Promise.all([activateOnCurrentSource(target), shouldRememberStartupProfile])
    .then(([, shouldRemember]) => {
      if (shouldRemember) {
        return window.haishuiDesktop?.profile?.remember(target)
      }
...
async function isLocalDesktopProfile(target: string): Promise<boolean> {
  const getConnectionConfig = window.haishuiDesktop?.getConnectionConfig
...
// Switch to the default (root ~/.haishui) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.haishuiDesktop?.touchBackend?.(target).catch(() => undefined)
}
