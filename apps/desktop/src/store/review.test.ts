...
// reaches the gateway); everything else routes through window.haishuiDesktop.git,
...
function file(path: string, over: Partial<HaishuiReviewFile> = {}): HaishuiReviewFile {
  return { path, status: 'modified', staged: false, added: 1, removed: 0, ...over } as HaishuiReviewFile
}

type ReviewStub = Record<string, ReturnType<typeof vi.fn>>

// Install a review bridge on window.haishuiDesktop. Any op not supplied defaults
...
describe('ship flow', () => {
  ...
  it('createOrOpenPr opens the existing PR without creating a new one', async () => {
    const review = stubReview()
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'https://example.com/pr/9' } } as HaishuiReviewShipInfo)

    await createOrOpenPr()

    expect(review.createPr).not.toHaveBeenCalled()
    expect(
      (window.haishuiDesktop as unknown as { openExternal: ReturnType<typeof vi.fn> }).openExternal
...
describe('refreshShipInfo', () => {
  it('populates ship info from the bridge', async () => {
    const info: HaishuiReviewShipInfo = {
      ghReady: true,
      pr: { url: 'https://example.com/pr/3' }
    } as HaishuiReviewShipInfo
  ...
  it('resets ship info when there is no bridge', async () => {
    delete (window as unknown as { haishuiDesktop?: unknown }).haishuiDesktop
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'x' } } as HaishuiReviewShipInfo)
