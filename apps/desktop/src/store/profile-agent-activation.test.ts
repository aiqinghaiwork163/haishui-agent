...
const agentConn = (over: Partial<HaishuiConnection> = {}): HaishuiConnection =>
  ({ baseUrl: 'https://homelab.invalid', mode: 'remote', profile: 'research', ...over }) as HaishuiConnection

const localConn = (over: Partial<HaishuiConnection> = {}): HaishuiConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as HaishuiConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<HaishuiConnection>>()

const getConnectionFor =
  vi.fn<(payload: { connectionId?: null | string; profile?: null | string }) => Promise<HaishuiConnection>>()
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { haishuiDesktop: { getConnection, getConnectionFor }, localStorage })
...
describe('ensureGatewayAgent commit hook (beforeActivate) — the Sessions switcher door (#93937)', () => {
  ...
  it('a descriptor lookup that never answers is bounded, fails open, and releases the mutex', async () => {
    ...
    try {
      getConnectionFor.mockImplementationOnce(() => new Promise<HaishuiConnection>(() => undefined))
