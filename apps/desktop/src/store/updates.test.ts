...
const checkHaishuiUpdateSpy = vi.fn()
const updateHaishuiSpy = vi.fn()
const getActionStatusSpy = vi.fn()

vi.mock('@/haishui', () => ({
  checkHaishuiUpdate: (...args: unknown[]) => checkHaishuiUpdateSpy(...args),
  updateHaishui: (...args: unknown[]) => updateHaishuiSpy(...args),
...
describe('checkBackendUpdates', () => {
  beforeEach(() => {
    storage.clear()
    notifySpy.mockClear()
    checkHaishuiUpdateSpy.mockReset()
  ...
  it('maps the backend /update/check onto the backend status, including commits', async () => {
    setRemote(true)
    checkHaishuiUpdateSpy.mockResolvedValue({
      install_method: 'git',
      current_version: '0.16.0',
      behind: 2,
      update_available: true,
      can_apply: true,
      update_command: 'haishui update',
      message: null,
    ...
    expect(checkHaishuiUpdateSpy).toHaveBeenCalled()
  ...
  it('is a no-op in local mode (backend check only runs when remote)', async () => {
    ...
    expect(checkHaishuiUpdateSpy).not.toHaveBeenCalled()
  })
})

// The ⌘K "Update Haishui" row. It used to call applyBackendUpdate() flat, which
...
describe('requestActiveUpdate', () => {
  ...
  beforeEach(() => {
    ...
    updateHaishuiSpy.mockReset().mockResolvedValue({ ok: true, name: 'update' })
    checkHaishuiUpdateSpy.mockReset().mockResolvedValue({
      install_method: 'git',
    ...
    ;(globalThis as unknown as { window: unknown }).window = {
      haishuiDesktop: { updates: { apply: applyClientMock, check: checkClientMock } }
    }
    vi.useRealTimers()
  })
  ...
  it('applies the CLIENT update in local mode, never the backend', async () => {
    ...
    expect(updateHaishuiSpy).not.toHaveBeenCalled()
  ...
  it('applies the BACKEND update in remote mode', async () => {
    ...
    requestActiveUpdate()
    await vi.waitFor(() => expect(updateHaishuiSpy).toHaveBeenCalled())
...
describe('explicit update targets', () => {
  ...
  // for updating THIS app — routes here via `haishui:open-updates`.
  ...
  it('still defaults to the connected machine when no target is named', async () => {
    ...
    expect($updateOverlayTarget.get()).toBe('backend')
    await vi.waitFor(() => expect(checkHaishuiUpdateSpy).toHaveBeenCalled())
  ...
  it('keeps the everything-flow for the generic, target-less apply', async () => {
    ...
    await vi.waitFor(() => expect(updateHaishuiSpy).toHaveBeenCalled(), { timeout: 5000 })
  })
...
describe('applyEverythingUpdate', () => {
  ...
  it('remote mode: updates the backend, then the still-behind client — the stale-GUI gap', async () => {
    ...
    expect(updateHaishuiSpy).toHaveBeenCalledTimes(1)
  ...
  it('a failed backend leg does not strand the fan-out or the client', async () => {
    setRemote(true)
    $mockConnectionsRegistry.set(registryOf(['local', 'vps']))
    updateHaishuiSpy.mockRejectedValue(new Error('backend gone'))
...
describe('applyUpdates terminal state', () => {
  ...
  beforeEach(() => {
    ...
    ;(globalThis as unknown as { window: unknown }).window = {
      haishuiDesktop: { updates: { apply: applyMock } }
    }
    vi.useRealTimers()
  })
  ...
  it('keeps the manual command state for CLI installs with no staged updater', async () => {
    applyMock.mockResolvedValue({ ok: true, manual: true, command: 'haishui update' })

    await applyUpdates()

    expect($updateApply.get().stage).toBe('manual')
    expect($updateApply.get().command).toBe('haishui update')
    expect($updateOverlayOpen.get()).toBe(true)
    expect(notifySpy).not.toHaveBeenCalled()
  })
  ...
  it('lands on a closeable manual-restart state when the rebuilt sandbox blocks auto-relaunch', async () => {
    ...
    applyMock.mockResolvedValue({
      ...
      manualRestart: true,
      sandboxBlocked: true,
      message: 'Backend updated. Quit and reopen Haishui to finish.'
    })
...
describe('applyBackendUpdate recovery', () => {
  ...
  it('waits for the backend to return after the restart drops the connection, then clears the overlay', async () => {
    ...
    updateHaishuiSpy.mockResolvedValue({ action_id: actionId, ok: true, name: 'update', pid: 1 })
    getActionStatusSpy.mockRejectedValueOnce(new Error('ECONNREFUSED')).mockResolvedValueOnce({
      exit_code: null,
      lines: [`=== haishui-update completed ${actionId} ===`],
      name: 'update',
      pid: null,
      running: false
    })
  ...
  it('keeps waiting past the old 45-second cutoff while the update action is running', async () => {
    ...
    updateHaishuiSpy.mockResolvedValue({ action_id: actionId, ok: true, name: 'haishui-update', pid: 1 })

    for (let attempt = 0; attempt < 31; attempt += 1) {
      getActionStatusSpy.mockResolvedValueOnce({
        exit_code: null,
        lines: ['=== haishui-update started now ===', `step ${attempt}`],
        name: 'haishui-update',
        pid: 1,
        running: true
      })
  ...
  it('treats a successful no-op as complete without waiting for a restart', async () => {
    updateHaishuiSpy.mockResolvedValue({ ok: true, name: 'haishui-update', pid: 1 })
    getActionStatusSpy.mockResolvedValue({
      exit_code: 0,
      lines: ['stale output from another run', '=== haishui-update started now ===', '✓ Already up to date!'],
      name: 'haishui-update',
      pid: 1,
      running: false
    })
  ...
  it('treats a successful dependency repair as complete without waiting for a restart', async () => {
    ...
    getActionStatusSpy.mockResolvedValue({
      exit_code: 0,
      lines: ['=== haishui-update started now ===', '✓ Dependencies repaired!', '✓ Update complete!'],
      name: 'haishui-update',
      pid: 1,
      running: false
    })
  ...
  it('waits for current-action completion proof after the backend restarts', async () => {
    ...
    getActionStatusSpy
      ...
      .mockResolvedValueOnce({
        exit_code: null,
        lines: ['Update complete!', `=== haishui-update completed ${'c'.repeat(32)} ===`],
        name: 'haishui-update',
        pid: null,
        running: false
      })
      .mockResolvedValueOnce({
        exit_code: null,
        lines: ['Update complete!', `=== haishui-update completed ${actionId} ===`],
        name: 'haishui-update',
        pid: null,
        running: false
      })
  ...
  it('accepts its terminal receipt when a verbose update pushes the start marker out of the log tail', async () => {
    ...
    getActionStatusSpy.mockRejectedValueOnce(new Error('ECONNREFUSED')).mockResolvedValueOnce({
      exit_code: null,
      lines: ['final build output', 'Update complete!', `=== haishui-update completed ${actionId} ===`],
      name: 'haishui-update',
      pid: null,
      running: false
    })
    ...
    await expect(promise).resolves.toMatchObject({ ok: true })
    expect(getActionStatusSpy).toHaveBeenCalledWith('haishui-update', 2000)
  })
  ...
  it('proves a pre-action-ID backend reached its requested commit after restart', async () => {
    ...
    await expect(promise).resolves.toMatchObject({ ok: true })
    expect(checkHaishuiUpdateSpy).toHaveBeenCalledTimes(2)
  })
  ...
  it('proves a fast pre-action-ID packaged update by its changed version', async () => {
    ...
    await expect(promise).resolves.toMatchObject({ ok: true })
    expect(checkHaishuiUpdateSpy).toHaveBeenCalledWith(true)
  })
  ...
  it('resumes action polling after a transient status failure', async () => {
    ...
    getActionStatusSpy
      ...
      .mockResolvedValueOnce({
        exit_code: null,
        lines: ['=== haishui-update started now ===', 'still running'],
        name: 'haishui-update',
        pid: 1,
        running: true
      })
      .mockResolvedValueOnce({
        exit_code: 0,
        lines: ['=== haishui-update started now ===', 'Update complete!'],
        name: 'haishui-update',
        pid: 1,
        running: false
      })
  ...
  it('restores the fixed action deadline after reconnecting', async () => {
    updateHaishuiSpy.mockResolvedValue({ action_id: 'a'.repeat(32), ok: true, name: 'haishui-update', pid: 1 })
  ...
  it('shares one in-flight update between concurrent apply requests', async () => {
    ...
    getActionStatusSpy.mockResolvedValue({
      exit_code: 0,
      lines: ['=== haishui-update started now ===', '✓ Already up to date!'],
      name: 'haishui-update',
      pid: 1,
      running: false
    })
  ...
  it('fails immediately when the update action exits nonzero', async () => {
    ...
    getActionStatusSpy.mockResolvedValue({
      exit_code: 1,
      lines: ['=== haishui-update started now ===', 'update failed'],
      name: 'haishui-update',
      pid: 1,
      running: false
    })
  ...
  it('surfaces an error when the backend never comes back after the restart', async () => {
    updateHaishuiSpy.mockResolvedValue({ ok: true, name: 'update', pid: 1 })
    getActionStatusSpy.mockRejectedValue(new Error('ECONNREFUSED'))
    checkHaishuiUpdateSpy.mockRejectedValue(new Error('ECONNREFUSED'))
...
describe('startUpdatePoller', () => {
  ...
  beforeEach(() => {
    ...
    $updateStatus.set(null)
    ;(globalThis as unknown as { window: unknown }).window = {
      haishuiDesktop: { updates: { check: checkMock, onProgress: onProgressMock } },
