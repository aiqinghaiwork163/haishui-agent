...
import type { HaishuiReviewFile, HaishuiReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Haishui' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'haishui.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'haishui.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'haishui.desktop.reviewTreeMode'
const SELECTED_KEY = 'haishui.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<HaishuiReviewFile[]>([])
...
export const $reviewShipInfo = atom<HaishuiReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['haishuiDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: HaishuiReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly HaishuiReviewFile[], path: string): HaishuiReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.haishuiDesktop?.openExternal?.(existing.url)
