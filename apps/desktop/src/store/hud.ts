 ...
 * composer with the reply scrolling above it, so Haishui can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.haishuiDesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.haishuiDesktop?.hud
...
export function resetHudLayout(): void {
  void window.haishuiDesktop?.hud?.resetLayout?.()
}
...
export const reportHudSession = (sessionId: null | string): void => window.haishuiDesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.haishuiDesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)
