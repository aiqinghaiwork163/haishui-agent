...
export function setZoomPercent(percent: number): void {
  window.haishuiDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.haishuiDesktop?.zoom) {
  void window.haishuiDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.haishuiDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
