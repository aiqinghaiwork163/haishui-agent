...
const LAST_PROFILE_STORAGE_KEY = 'haishui.desktop.lastProfileByConnection'
...
async function rememberConnection(connectionId: string): Promise<void> {
  const setLastUsed = window.haishuiDesktop?.connections?.setLastUsed
...
export async function selectConnection(connectionId: string, options: SelectConnectionOptions = {}): Promise<void> {
  ...
  // The primary local descriptor (startHaishui) historically publishes without
