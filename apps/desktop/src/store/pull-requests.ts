...
import type { HaishuiBranchPullRequest } from '@/global'
import { scanSessionPullRequests, type SessionInfo } from '@/haishui'
...
export const $pullRequestsByBranch = atom<Record<string, HaishuiBranchPullRequest>>({})
...
export const $prBranchBySession = persistentAtom<Record<string, string>>(
  'haishui.desktop.prBranchBySession',
  {},
  Codecs.stringRecord
)
...
const $prScannedSessions = persistentAtom<string[]>('haishui.desktop.prScannedSessions', [], Codecs.stringArray)
...
export function pullRequestBucket(pr: HaishuiBranchPullRequest | undefined): PullRequestBucket {
  if (!pr) {
    return 'none'
  }
