...
vi.mock('@/haishui', () => ({
  getHaishuiConfigRecord: mocks.getHaishuiConfigRecord,
...
afterEach(() => {
  ...
  mocks.getHaishuiConfigRecord.mockReset()
...
it('shows the toast with Sign in + Disable, then stays quiet for a day and re-nudges after it', async () => {
  ...
  mocks.getHaishuiConfigRecord.mockResolvedValue(servers)
...
it('honors a persisted snooze in a fresh module session, then re-notifies after it expires', async () => {
  const servers = { mcp_servers: { linear: { url: 'https://mcp.linear.app/mcp', auth: 'oauth' } } }
  const key = 'haishui:mcp-health-snooze-until:default::linear'
  let clock = 1_700_000_000_000
...
it('coalesces reconnects during a sweep into one fresh follow-up sweep', async () => {
  ...
  mocks.getHaishuiConfigRecord.mockReturnValueOnce(first).mockResolvedValue({ mcp_servers: {} })

  startMcpHealthChecker()
  mocks.gatewayState.set('open')
  await flush()
  expect(mocks.getHaishuiConfigRecord).toHaveBeenCalledTimes(1)
  ...
  await flush()
  await flush()
  expect(mocks.getHaishuiConfigRecord).toHaveBeenCalledTimes(2)
})
