import type { ConnectionRequestPayload } from '@haishui/shared'
...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Haishui doesn't know how to remove it. */
...
export interface FreeTierStatus {
   ...
   *  (`haishui_cli/anon_auth.py`), `error` its sentence, `retryable` whether a
...
export interface EnvVarInfo {
  ...
  // in haishui_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `haishui model` uses — instead of
...
export interface HaishuiConfig {
  agent?: {
...
export type HaishuiConfigRecord = Record<string, unknown>
...
export interface ProfileInfo {
  ...
  display_name?: string
  /** Bot Mode title (profile.yaml ui_meta['haishui-bots'].title) — the name the
...
// non-desktop) Haishui simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (haishui.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors haishui_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Haishui, 'hub' = installed. */
  provenance?: 'agent' | 'bundled' | 'hub'
}

/** One entry of the built-in optional-skills catalog (optional-skills/ in the
 *  repo) — official skills that ship with Haishui but install on demand. */
...
export interface ToolProvider {
   ...
   *  CLI `haishui tools` active-provider detection). */
  is_active: boolean
 ...
 *  not Haishui); elsewhere, driver health from `cua-driver doctor`. `null`
...
export interface ActionStatusResponse {
  ...
  pid: number | null
  running: boolean
  /** haishui-update only: durable completion identity recovered from update.log. */
  action_id?: string
  /** haishui-update only: summary of the durable update receipt (#91277 bullet 3) —
...
/** Shape of `GET /api/haishui/update/check` — the backend's own update state.
