 ...
 * `haishui://` deep links and `host.navigate('/path?…')`.
 *
 ...
 *  - `haishui://index-network/intent/1` → `/index-network/intent/1` (plugin-scoped)
 *  - `haishui://open/my-page?item=x` → `/my-page?item=x` (generic open)
 *  - `/my-page?item=x` / `#/my-page?item=x` (hash-router paths)
 */

export type HaishuiOpenTarget = string | { href: string } | { path: string; params?: Record<string, string> }

const HAISHUI_PROTOCOL = 'haishui:'
...
export function normalizeHaishuiOpenString(raw: string): string | null {
  ...
  if (trimmed.startsWith('haishui://') || trimmed.startsWith(`${HAISHUI_PROTOCOL}//`)) {
    try {
      const url = new URL(trimmed)
      const host = url.hostname || ''
      const rest = decodeURIComponent((url.pathname || '').replace(/^\//, ''))

      // haishui://open/<path>?… → /<path>?…
      if (host === 'open') {
        if (!rest) {
          return null
        }
      ...
      // haishui://index-network/intent/1 → /index-network/intent/1
...
export function resolveHaishuiOpenPath(target: HaishuiOpenTarget | null | undefined): string | null {
  if (target == null) {
    return null
  }

  if (typeof target === 'string') {
    return normalizeHaishuiOpenString(target)
  }
  ...
  if ('href' in target && typeof target.href === 'string') {
    return normalizeHaishuiOpenString(target.href)
  }

  if ('path' in target && typeof target.path === 'string') {
    const base = normalizeHaishuiOpenString(target.path)
 ...
 * (`haishui://<kind>/<name>?…` → kind/name/params).
 */
export function pathFromHaishuiDeepLink(kind: string, name: string, params: Record<string, string> = {}): string | null {
  if (!kind || !name) {
    return null
  }

  if (kind === 'open') {
    return resolveHaishuiOpenPath({ path: `/${name.replace(/^\//, '')}`, params })
  }
  ...
  return resolveHaishuiOpenPath({ path: `/${kind}/${name.replace(/^\//, '')}`, params })
}

/** Convenience for `haishui://open/<name>?…` payloads. */
export function pathFromOpenDeepLink(name: string, params: Record<string, string> = {}): string | null {
  return pathFromHaishuiDeepLink('open', name, params)
}
