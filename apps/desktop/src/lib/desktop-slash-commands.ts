...
/** Why a known Haishui command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Haishui profile', surface: action('profile') },
  {
    name: '/skin',
 ...
 * `haishui_cli/commands.py::desktop_surface_registry` by
...
function isKnownHaishuiSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Haishui' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownHaishuiSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with Haishui and were never asked for. It is
