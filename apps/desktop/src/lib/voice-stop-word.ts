...
//
// When someone is in a hands-free "Hey Haishui" voice chat, the natural way to
...
// phrase (optionally addressed to Haishui), so a real turn that merely contains
...
// Optional address prefixes so "haishui stop" / "ok stop" / "hey haishui, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey haishui', 'hey haishui,', 'haishui', 'haishui,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("haishui") is not a stop command on its own.
      continue
    }
 ...
 * to Haishui). Returns false for anything that merely contains "stop" as part of
