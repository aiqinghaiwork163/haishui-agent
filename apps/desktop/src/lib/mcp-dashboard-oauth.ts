...
const UPDATE_BACKEND = 'Update the Haishui backend to support Desktop MCP OAuth callbacks.'
...
export async function completeMcpDesktopOAuth({
  ...
  const bridge = window.haishuiDesktop.mcpOauth

  // A legacy null connection can resolve to a remote registry primary.
  if (!bridge && scope.connectionId !== 'local') {
    throw new Error('Update Haishui Desktop to support MCP OAuth callbacks.')
  }
  ...
  try {
    ...
    await window.haishuiDesktop.openExternal(authUrl)
    let pollFailures = 0
