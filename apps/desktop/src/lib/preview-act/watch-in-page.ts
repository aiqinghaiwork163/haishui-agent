...
export function watchInPage(doc: Document, holder: WatchHolder, stage: WatchStage, label?: string): void {
  const win = doc.defaultView as (Window & { __haishuiWatch?: Record<string, unknown> }) | null
  ...
  const busy = 25
  const state = win.__haishuiWatch || (win.__haishuiWatch = {})
  ...
  const build = () => {
    ...
    const stale = doc.querySelectorAll('haishui-watch')

    for (let i = 0; i < stale.length; i++) {
      stale[i].remove()
    }

    const host = doc.createElement('haishui-watch')
  ...
  // this up" in dev.
  const stamp = (win as unknown as { __haishuiWatchTag?: number }).__haishuiWatchTag
