...
export interface ErrorRecoveryPlan {
  ...
  editMessage: boolean
  /** Reveal the Haishui data folder so the user can free space (disk_full). */
  openHaishuiFolder: boolean
...
const CODE_PLANS: Partial<Record<ErrorCodeKey, Partial<ErrorRecoveryPlan>>> = {
  ...
  disk_full: { openHaishuiFolder: true, retry: true },
