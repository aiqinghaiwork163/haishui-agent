import { haishuiApi } from '@/api/client'
import type {
  HaishuiConnection,
  HaishuiReadDirResult,
  HaishuiReadFileTextResult,
  HaishuiSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: HaishuiConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export function desktopFsCacheKey(connection: HaishuiConnection | null = $connection.get()) {
...
export async function readDesktopDir(path: string): Promise<HaishuiReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<HaishuiReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<HaishuiReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<HaishuiReadFileTextResult>(fsPath('read-text', path))
}
...
export async function readDesktopFileDataUrlLocalFirst(path: string): Promise<string> {
  try {
    const local = await window.haishuiDesktop?.readFileDataUrl?.(path)
...
export async function selectDesktopPaths(options?: HaishuiSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
