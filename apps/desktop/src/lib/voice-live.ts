...
/**
 * GPT-Live voice chat: the full-duplex voice frontend that DELEGATES to Haishui.
 *
 ...
 * that into an ordinary Haishui turn on the open session and streams the reply
 * back with `session.commentary.append`, which the voice paraphrases aloud.
 * Haishui keeps every capability — model choice, tools, memory, approvals.
 *
...
export interface VoiceLiveHandlers {
  /** GPT-Live asked the backend (Haishui) for help. `context` is the recent
...
export async function fetchVoiceLiveStatus(): Promise<null | VoiceLiveStatus> {
  try {
    const response = await haishuiApi<{ ok: boolean } & VoiceLiveStatus>({
      ...profileScoped(),
...
export class VoiceLiveSession {
  ...
  /** The delegation currently being answered by Haishui; late results for an
  ...
  async start(history: LiveHistoryMessage[]): Promise<void> {
    ...
    const response = await haishuiApi<{
      ok: boolean
      session?: { id: string }
  ...
  /** Quiet progress for the live model ("Haishui is running the tests…"). */
