...
describe('reasoning blocks in streamed markdown', () => {
  it('does not fuse the words a stripped block separated', () => {
    expect(preprocessMarkdown('no<thinking>hmm</thinking> Haishui Desktop')).toBe('no Haishui Desktop')
    ...
    expect(preprocessMarkdown('no<thinking>a</thinking><think>b</think>Haishui')).toBe('no Haishui')
  })
