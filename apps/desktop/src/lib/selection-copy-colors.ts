...
// Haishui is rendering, and when the two are opposite schemes it takes over
...
function renderedMode(doc: Document): RenderedMode {
  const attr = doc.documentElement.dataset.haishuiMode
 ...
 * the only carriers of Haishui' palette — are dropped so the paste target's
