...
Bundled Haishui Desktop plugin, **off by default**. Enable **Radio** in
...
`plugin.js` imports only `@haishui/plugin-sdk`, `react`, and
...
`$HAISHUI_HOME/desktop-plugins/radio/plugin.js` for development. Do not install
that duplicate alongside the bundled version. Layout styles use Haishui theme
...
Stream availability, regional restrictions, and metadata vary. Haishui is not
