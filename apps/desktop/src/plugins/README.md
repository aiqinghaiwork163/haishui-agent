...
Drop a `<name>/plugin.{ts,tsx}` here that default-exports a `HaishuiPlugin` and
...
`$HAISHUI_HOME/desktop-plugins/<name>/plugin.js` (the disk door) — see the
`haishui-desktop-plugins` skill.
