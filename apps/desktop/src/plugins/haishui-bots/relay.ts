...
import { host, LruCache } from '@haishui/plugin-sdk'
...
// to (local, remote URL, SSH, Haishui Cloud, docker) must be able to find
...
// haishui_cli/config_defaults.py and tools/bot_relay.py and fails if the
  ...
  try {
    ...
    return profiles
      .map(profile => ({
        ...
        title: String(profile?.ui_meta?.['haishui-bots']?.title || profile?.display_name || ''),
