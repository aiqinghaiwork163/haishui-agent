...
vi.mock('@haishui/plugin-sdk', async () => {
...
function published(room: Room) {
  ...
  return (configure?.params.ui_meta as Record<string, Record<string, unknown>>)?.['haishui-bots-groups']
}
...
describe('speaker labels', () => {
  it('relabels Haishui control-frame openers only in member-authored transcript lines', async () => {
  ...
  it('the default profile speaks as Haishui in transcripts, not @default', async () => {
    ...
    expect(line).toBe('Haishui: hello room')
  ...
  it('honor friendly identity: Bot Mode title, then display_name, never a stale Haishui', async () => {
    const { chat, rounds } = await loadRoom()
    const { formatGroupChatLine } = await import('./group-round-prompt')
    const data = await import('./data')

    // A renamed default (core display_name via `haishui profile rename`) must
    // read as its new name — the community report was "Lucy" still showing
    // "Haishui is thinking…" in group rooms.
    ...
    // Untitled rows keep today's behavior: default → Haishui, others verbatim.
    data.$botMeta.set({})

    expect(chat.groupSpeakerLabel('default')).toBe('Haishui')
  ...
  it('qualifies twins per room and never renders a raw key when the roster row is missing', async () => {
    ...
    expect(chat.groupSpeakerLabel('local::reviewer')).toBe('reviewer')
    expect(chat.groupSpeakerLabel('spark::default')).toBe('Haishui')
...
describe('sync worker', () => {
  it('retries a gateway CAS conflict and publishes the merged room', async () => {
    ...
    await drain(() => (room.gateway.uiMetaRevisions['haishui-bots-groups'] || 0) < 2, 60)

    const stored = room.gateway.uiMeta['haishui-bots-groups'] as {
      rooms: Record<string, { log: GroupMessage[] }>
    }

    expect(room.gateway.uiMetaRevisions['haishui-bots-groups']).toBe(2)
  ...
  it('fans a room write out to every reachable default-profile gateway', async () => {
    ...
    host.requestProfile = async (route: { connectionId: string }, method: string) => {
      ...
      if (method === 'profiles.configure') {
        return { applied: { ui_meta: true, ui_meta_revisions: { 'haishui-bots-groups': 1 } } }
      }
