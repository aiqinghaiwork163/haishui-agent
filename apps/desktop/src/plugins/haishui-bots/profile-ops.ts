...
import { forgetSessionUnread, host, queryClient } from '@haishui/plugin-sdk'
...
export function mergeServerMeta(roster: RosterRow[], fetchedAt = 0) {
  ...
  for (const bot of roster) {
    const server = bot.ui_meta?.['haishui-bots']
...
/** Permanently delete a bot's Haishui profile, then remove plugin-local state
 ...
 * directory (haishui-agent#52279). That is the "can't delete a bot" error. */
