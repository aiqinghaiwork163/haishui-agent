...
export function displayName(bot: Partial<RosterRow>, meta?: BotMeta | null): string {
  ...
  // "Haishui". Annotated active rows carry sourceScoped too, and keying this
  ...
  // Core-profile display name (profile.yaml, set via `haishui profile rename
  ...
  // never generic "Haishui" or a hostname-derived label.
  if (alias) {
  ...
  // that reads like nobody bothered. Present it as Haishui (the agent it is)
...
/** The profile id the backend accepts (`haishui_cli.profiles._PROFILE_ID_RE`
