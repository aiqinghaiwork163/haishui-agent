...
import { atom, host } from '@haishui/plugin-sdk'
...
const GROUP_CHAT_SYNC_META_KEY = 'haishui-bots-groups'
...
/** The v3 envelope stored under the default profile's `haishui-bots-groups`
 ...
 *  clients (Haishui Go, headless backends) see rooms regardless of which
 ...
 *  renamed bot never shows up as its raw profile id or a stale "Haishui"
 *  (community report, Aug 21 2026: renamed default still read "Haishui is
 *  thinking…" in group rooms). The untitled primary profile is literally
 *  named "default" — render it as Haishui (matching displayName and the
 *  @haishui handle) so the main agent never loses its name in rooms.
 *
 ...
 *  failing `default`s are never one anonymous "Haishui" — judged against the
...
export function groupSpeakerLabel(name?: null | string, group?: null | string) {
  ...
  if (boundary !== -1) {
    ...
    const label = title || (profile.toLowerCase() === 'default' ? 'Haishui' : profile)
  ...
  // title, then the local row's display_name, then default → Haishui.
  ...
  return isDefault ? 'Haishui' : trimmed
}
