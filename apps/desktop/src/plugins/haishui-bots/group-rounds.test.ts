...
describe('routing', () => {
  ...
  it('resolves @haishui to the default member', async () => {
    const { rounds } = await loadRoom()
    ...
    const parsed = rounds.parseGroupChatMentions('@haishui take a look', members)
...
describe('turn prompt', () => {
  it('addresses the default profile as @haishui', async () => {
    const { rounds } = await loadRoom()
    ...
    expect(own).toMatch(/You are @haishui,/)
    ...
    expect(peer).toMatch(/group chat with @haishui/)
  })

  // #89720: a renamed primary is @bobby to the roster, autocomplete and the
  // mention resolver; introducing it to itself as @haishui made it treat
