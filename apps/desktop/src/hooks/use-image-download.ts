...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'haishui:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.haishuiDesktop?.saveImageFromUrl) {
        if (await window.haishuiDesktop.saveImageFromUrl(src)) {
