/**
 * @haishui/plugin-sdk — THE plugin language. The vscode-module model: plugin
 ...
 *    object as `window.__HAISHUI_PLUGIN_SDK__` and maps the import to it, so a
...
import { deleteProfile, getLogs, getStatus, haishuiApi, type HaishuiGateway } from '@/haishui'
...
import type { PaginatedSessions, UsageStats } from '@/types/haishui'
...
export interface PluginProfileRoute {
  ...
  profile: string
  /** Backend Haishui profile served by that route. */
...
async function requestPluginProfile<T>(
  ...
  const getAgentRoster = window.haishuiDesktop?.getAgentRoster
...
async function pluginRouteStillRegistered(route: PluginProfileRoute): Promise<boolean> {
  const getProfileRoutes = window.haishuiDesktop?.getProfileRoutes
...
//
// ensureGatewayProfile awaits the store's dial, and HaishuiGateway.connect() has
...
export const host = {
  ...
  deleteProfile: async (profile: string | PluginProfileRoute): Promise<void> => {
    ...
    // restores on relaunch and re-creates the deleted profile (haishui-agent#94235).
    dropTilesForProfile(
  ...
  connections: async () => {
    const bridge = window.haishuiDesktop?.connections

    if (!bridge) {
      throw new Error('This Desktop build has no connection registry. Update Haishui Desktop.')
    }
  ...
  agents: async () => {
    const roster = window.haishuiDesktop?.getAgentRoster

    if (!roster) {
      throw new Error('This Desktop build cannot enumerate multi-source agents. Update Haishui Desktop.')
    }
  ...
  newChat: (profile?: null | string | PluginProfileRoute, options: PluginNewChatOptions = {}): void => {
    if (options.workspaceMode === 'bots') {
      ...
      if (!openTab) {
        notify({ kind: 'error', message: 'Update Haishui Desktop to open another Bot chat.' })
   ...
   *
   *  `isStaleTile` (haishui-agent#90102): the caller's reconciliation probe
  ...
  profileRoutes: async () => {
    const desktop = window.haishuiDesktop
    const getProfileRoutes = desktop?.getProfileRoutes

    if (!getProfileRoutes) {
      throw new Error('Haishui Desktop connection routing unavailable')
    }
  ...
  listPersistedSessions: async (
    ...
    return haishuiApi<PaginatedSessions>({
  ...
  setPersistedSessionHidden: async (
    ...
    return haishuiApi<{ ok: boolean; hidden: boolean }>({
      ...(route ? { connectionId: route.connectionId } : {}),
  ...
  request: async <T>(method: string, params: Record<string, unknown> = {}): Promise<T> => {
    ...
    if (!gateway) {
      throw new Error('Haishui gateway unavailable')
    }
 ...
 *  "HAISHUI AGENT" with it, a `chat.empty` contribution writes its own name. */
...
export type { HaishuiGateway } from '@/haishui'
...
export type { HaishuiOpenTarget } from '@/lib/haishui-open-target'
...
/** Compact labels for the reasoning levels exported from @haishui/shared, so a
...
export type { StatusResponse } from '@/types/haishui'
/** Public SDK name for the shared gateway wire event; kept stable for plugins. */
export type { GatewayEvent as RpcEvent } from '@haishui/shared'
/** THE compact-number formatter — every user-facing count/token figure goes
 *  through here (1230 → "1.2k", 1_500_000 → "1.5M"). Don't hand-roll `/1000`. */
export { compactNumber } from '@haishui/shared'
/** Haishui' reasoning levels, so a plugin surfacing a thinking depth uses the
...
export { contrastRatio } from '@haishui/shared/color'
