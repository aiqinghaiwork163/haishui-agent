...
vi.mock('@/haishui', () => ({ deleteProfile: vi.fn(), getLogs: vi.fn(), getStatus: vi.fn(), haishuiApi: vi.fn() }))
...
const { deleteProfile, haishuiApi } = await import('@/haishui')
...
describe('connection-aware plugin host APIs', () => {
  it('retires a profile gateway before deleting it', async () => {
    ...
    // profile's backend, re-creating its HAISHUI_HOME (#94235).
  ...
  it('refreshes the profile inventory before asking Electron for routes', async () => {
    ...
    ;(window as unknown as { haishuiDesktop: unknown }).haishuiDesktop = { getProfileRoutes }
  ...
  it('reads and hides persisted sessions through the source primary without activating the profile', async () => {
    ...
    expect(haishuiApi).toHaveBeenNthCalledWith(
      1,
    ...
    expect(haishuiApi).toHaveBeenNthCalledWith(2, {
      connectionId: 'source-a',
      path: '/api/sessions/bot-chat',
      method: 'PATCH',
      body: { hidden: true, profile: 'backend-worker' }
    })
    expect(vi.mocked(haishuiApi).mock.calls.every(([request]) => !('profile' in request))).toBe(true)
