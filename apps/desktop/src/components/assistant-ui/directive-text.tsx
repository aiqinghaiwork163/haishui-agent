...
const HAISHUI_REF_TYPES = WIRE_REFERENCE_KINDS
type HaishuiRefType = (typeof HAISHUI_REF_TYPES)[number]
...
const HAISHUI_DIRECTIVE_RE = referenceRe()
...
export const haishuiDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(HAISHUI_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...haishuiDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Haishui directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.haishuiDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.haishuiDesktop?.readFileDataUrl(id)
