...
const ErrorRecoveryActions: FC = () => {
  ...
  // Open Logs reveals the LOCAL Electron profile's HAISHUI_HOME/logs. On a
  ...
  // HAISHUI_HOME/logs, and its parent is the Haishui data folder itself (what
  ...
  const openLocalDir = useCallback(async (resolve: (logsRoot: string) => string, failedMessage: string) => {
    try {
      const root = await window.haishuiDesktop?.logsRoot?.()
      ...
      const result = await window.haishuiDesktop?.openDir?.(resolve(root))
  ...
  const openHaishuiFolder = useCallback(
    () => openLocalDir(root => root.replace(/[\\/]+logs[\\/]*$/, ''), copy.errorOpenHaishuiFolderFailed),
    [copy.errorOpenHaishuiFolderFailed, openLocalDir]
  )
  ...
  const localFolders = Boolean(window.haishuiDesktop?.logsRoot)
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      ...
      {plan.openHaishuiFolder && localFolders && (
        <button className="aui-error-action" onClick={() => void openHaishuiFolder()} type="button">
          {copy.errorOpenHaishuiFolder}
        </button>
      )}
      {plan.retry && (
