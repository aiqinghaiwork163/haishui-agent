 ...
 * own design.
 *
 * WIDGETS TALK BACK OFF-SCREEN. `window.haishui.send(prompt)` (or declarative
 * `data-haishui-send` on any clickable element) routes the prompt through the
...
const SIZE_MESSAGE_TYPE = 'haishui-inline-preview-size'
const INTENT_MESSAGE_TYPE = 'haishui-inline-preview-intent'
...
/** The script that gives the widget its ONE voice: `haishui.send(prompt)`.
 ...
 *  wires `data-haishui-send` so declarative HTML works with zero script:
 *  `<button data-haishui-send="get-price eth">ETH</button>`. */
export function intentScript(token: string): string {
  return (
    ...
    ')},"*");return true}' +
    'window.haishui={send:send};' +
    'addEventListener("click",function(e){var el=e.target&&e.target.closest?' +
    'e.target.closest("[data-haishui-send]"):null;' +
    'if(el)send(el.getAttribute("data-haishui-send")||"")},true)})()</script>'
  )
}
...
export function InlinePreviewDirective({
  ...
  // URL connection — including a same-machine `haishui serve` — renders live.)
