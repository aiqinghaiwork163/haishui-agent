...
describe('buildToolView web-search query', () => {
  it('keeps the query separate from structured search results', () => {
    const view = buildToolView(
      part({
        args: { query: 'Haishui Agent Desktop tool calls' },
        result: { web: [{ snippet: 'Desktop docs', title: 'Haishui docs', url: 'https://example.com/docs' }] },
        toolName: 'web_search'
      }),
      ''
    )

    expect(view.searchQuery).toBe('Haishui Agent Desktop tool calls')
    expect(view.searchHits).toEqual([
      { snippet: 'Desktop docs', title: 'Haishui docs', url: 'https://example.com/docs' }
    ])
  })
})
