...
function installDesktopMock(state: DesktopBootstrapState) {
  ...
  Object.defineProperty(window, 'haishuiDesktop', {
    configurable: true,
    value: desktop
  })
...
afterEach(() => {
  cleanup()
  vi.useRealTimers()
  Reflect.deleteProperty(window, 'haishuiDesktop')
})
...
describe('DesktopInstallOverlay first-run setup', () => {
  it('shows the remote/local choice without installer progress', async () => {
    installDesktopMock(
      bootstrapState({
        setupChoice: { platform: 'win32', activeRoot: 'C:\\Users\\me\\AppData\\Local\\haishui\\haishui-agent' }
      })
    )

    render(<DesktopInstallOverlay />)

    expect(await screen.findByText('Set up Haishui Desktop')).toBeTruthy()
    expect(screen.getByText('Connect to existing Haishui')).toBeTruthy()
    expect(screen.getByText('Install Haishui locally')).toBeTruthy()
    expect(screen.queryByText(/steps complete/i)).toBeNull()
    expect(screen.queryByText(/Fetching installer manifest/i)).toBeNull()
  })

  it('continues local bootstrap only when Install Haishui locally is selected', async () => {
    ...
    fireEvent.click(await screen.findByText('Install Haishui locally'))

    expect(desktop.continueBootstrapLocal).toHaveBeenCalledTimes(1)
    expect(screen.getByText('Set up Haishui Desktop')).toBeTruthy()

    act(() => {
      desktop.emitBootstrapEvent({ type: 'manifest', protocolVersion: 1, stages: [] })
    })

    await waitFor(() => expect(screen.queryByText('Set up Haishui Desktop')).toBeNull())
  ...
  it('surfaces a recoverable error when the local-bootstrap bridge is unavailable', async () => {
    ...
    const install = (await screen.findByText('Install Haishui locally')).closest('button') as HTMLButtonElement
    fireEvent.click(install)

    expect(
      await screen.findByText('Local installation could not start. Restart Haishui Desktop and try again.')
    ).toBeTruthy()
  ...
  it('keeps the local-start error when the first snapshot commits under the click', async () => {
    ...
    const install = (await whenPresent('Install Haishui locally')).closest('button') as HTMLButtonElement
    fireEvent.click(install)
    ...
    expect(screen.queryByText('Local installation could not start. Restart Haishui Desktop and try again.')).toBeTruthy()
  })
  ...
  it('clears a stale local-start error when a repair presents a different root', async () => {
    ...
    fireEvent.click((await screen.findByText('Install Haishui locally')).closest('button') as HTMLButtonElement)
    expect(
      await screen.findByText('Local installation could not start. Restart Haishui Desktop and try again.')
    ).toBeTruthy()
    ...
    act(() => {
      desktop.emitBootstrapEvent({
        ...
        active: false,
        platform: 'win32',
        activeRoot: 'C:\\Users\\me\\AppData\\Local\\haishui\\haishui-agent-repaired'
      })
    })

    expect(screen.queryByText('Local installation could not start. Restart Haishui Desktop and try again.')).toBeNull()
  })

  it('opens the remote connection form from the first-run choice', async () => {
    installDesktopMock(
      bootstrapState({
        setupChoice: { platform: 'linux', activeRoot: '/home/me/.haishui/haishui-agent' }
      })
    )

    render(<DesktopInstallOverlay />)

    fireEvent.click(await screen.findByText('Connect to existing Haishui'))
  ...
  it('requires a successful token connection test before applying remote config', async () => {
    ...
    desktop.probeConnectionConfig.mockResolvedValue({
      authMode: 'token',
      baseUrl: 'https://gateway.example.com/haishui',
      error: null,
      providers: [],
      reachable: true,
      version: '0.17.0'
    ...
    fireEvent.click(await screen.findByText('Connect to existing Haishui'))
    fireEvent.change(await screen.findByPlaceholderText('https://gateway.example.com/haishui'), {
      target: { value: 'https://gateway.example.com/haishui' }
    })
    ...
    await waitFor(() => {
      expect(desktop.testConnectionConfig).toHaveBeenCalledWith({
        ...
        remoteUrl: 'https://gateway.example.com/haishui'
      })
    })

    await screen.findByText('Connected to https://gateway.example.com/haishui (0.17.0).')
  ...
  it('ignores a completed probe after the gateway URL becomes invalid', async () => {
    ...
    fireEvent.click(await screen.findByText('Connect to existing Haishui'))
    const urlInput = await screen.findByPlaceholderText('https://gateway.example.com/haishui')
    fireEvent.change(urlInput, { target: { value: 'https://gateway.example.com/haishui' } })
  ...
  it('does not enable Apply when credentials change during a connection test', async () => {
    ...
    await act(async () => {
      resolveTest?.({ baseUrl: 'https://gateway.example.com/haishui', ok: true, version: '0.17.0' })
      await pendingTest
    })

    expect(screen.queryByText('Connected to https://gateway.example.com/haishui (0.17.0).')).toBeNull()
    expect(apply.disabled).toBe(true)
  })
  ...
  it('signs in, tests, and applies a password-style remote gateway', async () => {
    ...
    await waitFor(() => {
      expect(desktop.oauthLoginConnectionConfig).toHaveBeenCalledWith('https://gateway.example.com/haishui')
    })
    ...
    await screen.findByText('Connected to https://gateway.example.com/haishui.')
    const apply = screen.getByText('Apply and reconnect').closest('button') as HTMLButtonElement
    expect(apply.disabled).toBe(false)
    fireEvent.click(apply)
  ...
  it('offers remote connection from the unsupported packaged install screen', async () => {
    const desktop = installDesktopMock(
      bootstrapState({
        unsupportedPlatform: {
          platform: 'darwin',
          activeRoot: '/Users/me/.haishui/haishui-agent',
    ...
    expect(await screen.findByText('Haishui needs a one-time install')).toBeTruthy()
    ...
    fireEvent.change(screen.getByPlaceholderText('https://gateway.example.com/haishui'), {
      target: { value: 'https://gateway.example.com/haishui' }
    })
    ...
    await waitFor(() => expect(screen.queryByText('Gateway URL')).toBeNull())
    expect(screen.queryByText('Haishui needs a one-time install')).toBeNull()
  })
})
