...
const TREE_KEY = 'haishui.desktop.layoutTree.v2'
const USER_PLACED_KEY = 'haishui.desktop.userPlacedPanes.v1'
const LEGACY_HEAL_KEY = 'haishui.desktop.paneDockHeals.v1'
  ...
  children: [
    {
      ...
      children: [
        ...
        { type: 'group', id: 'g-bots', panes: ['haishui-bots:pane'], active: 'haishui-bots:pane' }
      ]
    },
...
describe('enforced dock (stacked Bots pane → sessions-zone tab, every boot)', () => {
  ...
  it('re-homes a stacked bots pane into the sessions tab strip, keeping sessions active', async () => {
    ...
    const group = model.findGroupOfPane(tree.$layoutTree.get()!, 'haishui-bots:pane')!

    expect(group.panes).toEqual(['sessions', 'haishui-bots:pane'])
    ...
    expect(JSON.stringify(persisted)).toContain('"panes":["sessions","haishui-bots:pane"]')
  })

  it('re-homes even a USER-PLACED pane — the owner invariant beats the drag record', async () => {
    window.localStorage.setItem(USER_PLACED_KEY, JSON.stringify(['haishui-bots:pane']))
  ...
  it('re-homes even when the retired heal token was already burned, and clears the stale ledger', async () => {
    window.localStorage.setItem(LEGACY_HEAL_KEY, JSON.stringify(['haishui-bots:pane:sessions-tab-v1']))
  ...
  it('is idempotent within a boot and does not fight an intra-session drag', async () => {
    ...
    expect(model.findGroupOfPane(tree.$layoutTree.get()!, 'haishui-bots:pane')!.panes).toContain('sessions')
    ...
    expect(group.panes).toEqual(['haishui-bots:pane'])
  })
  ...
  it('re-homes again on the NEXT boot after a drag persisted the stacked shape', async () => {
    ...
    const group = second.model.findGroupOfPane(second.tree.$layoutTree.get()!, 'haishui-bots:pane')!
      ...
      children: [
        {
          type: 'group',
          id: 'g-left',
          panes: ['sessions', 'haishui-bots:pane'],
          active: 'haishui-bots:pane',
          headerHidden: true
        },
        { type: 'group', id: 'g-main', panes: ['workspace'], active: 'workspace' }
      ]
  ...
  it('re-homes an edge-enforced pane stranded in the sessions tab strip', async () => {
      ...
      children: [
        {
          type: 'group',
          id: 'g-sessions',
          panes: ['sessions', 'haishui-bots:pane', 'haishui-bots:routines'],
          active: 'haishui-bots:pane'
        },
        { type: 'group', id: 'g-main', panes: ['workspace'], active: 'workspace' }
      ]
    ...
    const botsGroup = model.findGroupOfPane(tree.$layoutTree.get()!, 'haishui-bots:pane')!
    const routinesGroup = model.findGroupOfPane(tree.$layoutTree.get()!, 'haishui-bots:routines')!

    expect(botsGroup.panes).toEqual(['sessions', 'haishui-bots:pane'])
    expect(botsGroup.active).toBe('haishui-bots:pane')
    expect(routinesGroup.panes).toEqual(['haishui-bots:routines'])
    ...
    const dockedRoutinesTree = {
      ...
      children: [
        ...
        { type: 'group', id: 'g-main', panes: ['workspace'], active: 'workspace' },
        {
          type: 'group',
          id: 'g-routines',
          panes: ['haishui-bots:routines'],
          active: 'haishui-bots:routines'
        }
      ]
    }
