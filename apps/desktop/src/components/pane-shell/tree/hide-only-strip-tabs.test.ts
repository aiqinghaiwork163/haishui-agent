...
function sessionsBotsTree() {
  ...
  registerPane('haishui-bots:pane', { placement: 'left', hideOnly: true })
  registerPane('workspace', { placement: 'main', uncloseable: true })
  $layoutTree.set(
    split('row', [
      group(['sessions', 'haishui-bots:pane'], { active: 'sessions', id: 'g-side' }),
...
describe('hide-only strip tabs', () => {
  it('hides and shows a chrome tab, keeping the pane in the tree', () => {
    ...
    expect(setStripTabHidden('haishui-bots:pane', true)).toBe(true)
    expect($hiddenTreePanes.get()).toContain('haishui-bots:pane')
    expect($hiddenStripTabs.get()).toContain('haishui-bots:pane')
    // Hidden, not dismissed: the pane stays in the layout tree.
    expect(allPaneIds($layoutTree.get()!)).toContain('haishui-bots:pane')

    expect(setStripTabHidden('haishui-bots:pane', false)).toBe(true)
    expect($hiddenTreePanes.get()).not.toContain('haishui-bots:pane')
    expect($hiddenStripTabs.get()).not.toContain('haishui-bots:pane')
  })

  it('refuses to hide the zone last visible tab', () => {
    sessionsBotsTree()
    setStripTabHidden('haishui-bots:pane', true)
  ...
  it('persists hides and clears them on reveal', () => {
    ...
    const persisted = JSON.parse(window.localStorage.getItem('haishui.desktop.hiddenStripTabs.v1') ?? '[]')

    expect(persisted).toContain('haishui-bots:pane')
    ...
    revealTreePane('haishui-bots:pane')
    expect($hiddenTreePanes.get()).not.toContain('haishui-bots:pane')
    expect(window.localStorage.getItem('haishui.desktop.hiddenStripTabs.v1')).toBeNull()
  })
  ...
  it('lists the zone hide-only tabs with live hidden state for the menu', () => {
    ...
    expect(hideOnlyZoneTabs('g-side')).toEqual([
      ...
      { hidden: true, id: 'haishui-bots:pane', title: 'haishui-bots:pane' }
    ])
  ...
  it('excludes hide-only tabs from every close verb', () => {
    ...
    $layoutTree.set(group(['sessions', 'haishui-bots:pane', 'session-tile:x'], { active: 'sessions', id: 'g-mixed' }))
