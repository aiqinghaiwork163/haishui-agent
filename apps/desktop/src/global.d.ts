import type { GatewayWsUrlResult } from '@haishui/shared'
import type { TranslucencyState } from '@haishui/shared/translucency'

import type { ScreenshotApi } from '../electron/command-screenshot-types'
import type { HaishuiNotification } from '../electron/notification-types'
...
declare global {
  interface Window {
    haishuiDesktop: {
      ...
      // Resume this session in the user's own terminal emulator (`haishui --tui
      ...
      // over haishui:window-control; Electron/OS chrome owns them elsewhere.
      windowControls: {
        custom: boolean
        minimize: () => void
        toggleMaximize: () => void
      ...
      // Haishui Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new HAISHUI_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: HaishuiApiRequest) => Promise<T>
      notify: (payload: HaishuiNotification) => Promise<boolean>
      ...
      readFileText: (filePath: string) => Promise<HaishuiReadFileTextResult>
      ...
      readPluginSource?: (filePath: string) => Promise<HaishuiReadFileTextResult>
      selectPaths: (options?: HaishuiSelectPathsOptions) => Promise<string[]>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<HaishuiPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<HaishuiPreviewWatch>
      ...
      watchDirectory?: (dir: string) => Promise<HaishuiPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: HaishuiActiveWork) => void
      setTitleBarTheme?: (payload: HaishuiTitleBarTheme) => void
       ...
       *  (HAISHUI_GUEST_ONBOARDING=1 or --guest-onboarding). Read-only fact the
       *  main process also stamps onto every backend it spawns. */
      guestOnboardingEnabled?: boolean
      /** Launch flag: skip the first-run film (HAISHUI_SKIP_INTRO=1 or
       *  --skip-intro) so a fresh HAISHUI_HOME lands on the guided chat. */
      skipIntro?: boolean
      ...
      readDir: (path: string) => Promise<HaishuiReadDirResult>
      ...
      // Local Desktop runtime-plugin root (<HAISHUI_HOME>/desktop-plugins),
      ...
      /** LOCAL `<HAISHUI_HOME>/logs` (profile-aware) — error card "Open Logs". */
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<HaishuiGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<HaishuiGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<HaishuiGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<HaishuiRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: HaishuiReviewScope, baseRef?: null | string) => Promise<HaishuiReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: HaishuiReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<HaishuiReviewShipInfo>
          ...
          prList: (repoPath: string, branches: string[], numbers?: number[]) => Promise<HaishuiRepoPullRequests>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: HaishuiTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<HaishuiTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: HaishuiWindowState) => void) => () => void
      ...
      onPreviewFileChanged: (callback: (payload: HaishuiPreviewFileChanged) => void) => () => void
...
export interface HaishuiTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface HaishuiTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `haishui update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  haishuiRoot?: string
...
export interface HaishuiConnection {
  baseUrl: string
...
export interface HaishuiTitleBarTheme {
  background: string
  foreground: string
}

/** Turns in flight, so the main process can confirm before a quit kills them. */
export interface HaishuiActiveWork {
  count: number
  titles: string[]
}

export interface HaishuiWindowState {
  customWindowControls?: boolean
  darwinMajor?: number
  isFullscreen: boolean
  isMaximized?: boolean
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Haishui Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Haishui Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteUrl?: string
  // For a 'cloud' connection: the selected Haishui Cloud org (slug or id) to
...
// A discovered Haishui Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface HaishuiApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface HaishuiPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface HaishuiReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface HaishuiPreviewWatch {
  id: string
  path: string
}
...
export interface HaishuiGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface HaishuiGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
  isRemote: boolean
...
// preselect it.
export interface HaishuiGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface HaishuiRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface HaishuiRepoStatus {
  branch: null | string
...
// turn began.
export type HaishuiReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface HaishuiReviewFile {
  path: string
  added: number
  removed: number
...
export interface HaishuiReviewList {
  files: HaishuiReviewFile[]
...
export interface HaishuiReviewPr {
  url: string
  state: string
  number: number
}
...
export interface HaishuiBranchPullRequest {
  branch: string
  draft: boolean
  number: number
...
export interface HaishuiRepoPullRequests {
  ghReady: boolean
  prs: HaishuiBranchPullRequest[]
}
...
export interface HaishuiReviewShipInfo {
  ghReady: boolean
  pr: HaishuiReviewPr | null
}

export interface HaishuiReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface HaishuiReadDirResult {
  entries: HaishuiReadDirEntry[]
  error?: string
}

export interface HaishuiPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface HaishuiSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
