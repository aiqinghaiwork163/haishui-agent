 ...
 *   1. HAISHUI_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\haishui\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect HAISHUI_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.HAISHUI_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'haishui', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'haishui', 'git', 'usr', 'bin', 'bash.exe'))
  }
