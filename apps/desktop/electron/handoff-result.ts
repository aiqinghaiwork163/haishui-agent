 ...
 * console. It writes HAISHUI_HOME/.haishui-update-result.json on every exit
 ...
 * ever surfaces, and the user may not reopen Haishui within 30 minutes.
...
export function handoffResultPath(haishuiHome: string): string {
  return path.join(haishuiHome, '.haishui-update-result.json')
}
...
export function readAndConsumeHandoffResult(
  ...
  const file = handoffResultPath(haishuiHome)
  let raw: string
