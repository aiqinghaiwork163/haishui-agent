 ...
 *
 * The pre-update handoff kills Haishui-OWNED venv daemons (the memory plugin's
 * hindsight daemon) so the updater never races a mapped shim. External
 * holders (a user terminal running `haishui`, unrelated scripts) must NOT be
...
/**
 * True when a process is a Haishui-owned venv daemon: its exe lives under
 ...
 * spawns DETACHED — it outlives Haishui and holds venv shims mapped).
 */
export function isHaishuiOwnedVenvDaemon(
