 ...
 *     `X-Haishui-Session-Token` header; WS uses `?token=`.
...
// haishui_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - haishui_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - haishui_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / haishui #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-haishui_session_at', '__Secure-haishui_session_at', 'haishui_session_at']
const RT_COOKIE_VARIANTS = ['__Host-haishui_session_rt', '__Secure-haishui_session_rt', 'haishui_session_rt']

// Keep this aligned with haishui_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['haishui', 'test', 'tmp', 'root', 'sudo'])
...
function gatewayTicketFailure(error, authMessage, transportMessage) {
  ...
  if (needsOauthLogin) {
    ...
    // change until the user signs in, so tag it the way startHaishui latches
...
/** Which Haishui profile the remote SSH dashboard should actually run as.
...
// Haishui Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteHaishuiPath = String(entry.remoteHaishuiPath || '').trim()

  if (remoteHaishuiPath) {
    out.remoteHaishuiPath = remoteHaishuiPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote Haishui installation. Preserve an explicit mapping
  // when it is a valid Haishui profile identifier; otherwise fall back to the
...
function localPrimaryRequestScope(opts: ProfileRouteOptions): boolean | null {
  ...
  // spawned them: `_spawn_haishui_action` registers the (often dynamic, e.g.
 ...
 *     `--profile`, so its `HAISHUI_HOME` scopes it.
 *
...
function resolveProfileBackendRoute(profile, opts: ProfileRouteOptions = {}): ProfileBackendRoute {
  ...
  if (scopedProfile === primaryProfile) {
    ...
    // the wire: the dashboard's process HAISHUI_HOME can belong to a different
 ...
 * `haishui:api` handler — both routes reach a backend whose namespace is the
...
function translateSelfProfileQuery(path, profile, backendProfile) {
  ...
  try {
    parsed = new URL(rawPath, 'http://haishui.local')
  } catch {
    return path
  }
...
/**
 * Resolve the two decisions made by the `haishui:api` IPC handler from the same
...
/**
 * True if any cookie in `cookies` is a haishui session ACCESS-token cookie
