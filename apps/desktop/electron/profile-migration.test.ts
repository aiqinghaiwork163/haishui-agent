 ...
 *      top of startHaishui() in main.ts, before primaryProfileKey() is read); the
...
function baseDeps(overrides: Record<string, unknown> = {}) {
  ...
  return {
    legacyActivePath: '/home/u/.haishui/active_profile',
    haishuiHome: '/home/u/.haishui',
    profilesRoot: '/home/u/.haishui/profiles',
    existsSync: fs.existsSync,
    readFileSync: fs.readFileSync,
    statSync: fs.statSync,
    readdirSync: fs.readdirSync,
    isHaishuiProcess: () => true,
    now: () => NOW,
    writeJson: () => {},
    isValidProfileName,
    ...overrides
...
test('findRunningGatewayProfiles returns [] when no pid files exist', () => {
  const fs = makeFs({ '/home/u/.haishui/profiles/coder': { dir: true } })
  assert.deepEqual(
    findRunningGatewayProfiles('/home/u/.haishui/profiles', ['coder'], {
      ...fs,
      isHaishuiProcess: () => true
    }),
    []
...
test('findRunningGatewayProfiles drops stale (non-haishui) recycled PIDs', () => {
  // Two profiles have pid files, but only coder's pid is a live haishui process.
  // The recycled PID at 5678 belongs to an unrelated process (e.g. Chrome).
  const fs = makeFs({
    '/home/u/.haishui/profiles/coder/gateway.pid': { content: '{"pid":1234}' },
    '/home/u/.haishui/profiles/writer/gateway.pid': { content: '{"pid":5678}' }
  })

  const deps = {
    ...fs,
    isHaishuiProcess: (pid: number) => pid === 1234
  }

  assert.deepEqual(findRunningGatewayProfiles('/home/u/.haishui/profiles', ['coder', 'writer'], deps), ['coder'])
})

test('findRunningGatewayProfiles tolerates malformed pid files', () => {
  const fs = makeFs({
    '/home/u/.haishui/profiles/coder/gateway.pid': { content: '{not json' }
  })
...
test('findRunningGatewayProfiles drops non-integer and non-positive PIDs', () => {
  const fs = makeFs({
    '/home/u/.haishui/profiles/coder/gateway.pid': { content: '{"pid":-1}' },
    '/home/u/.haishui/profiles/writer/gateway.pid': { content: '{"pid":1.5}' },
    '/home/u/.haishui/profiles/extra/gateway.pid': { content: '{"pid":0}' }
  })

  assert.deepEqual(
    findRunningGatewayProfiles('/home/u/.haishui/profiles', ['coder', 'writer', 'extra'], {
      ...fs,
      isHaishuiProcess: () => true
    }),
    []
...
test('findRunningGatewayProfiles preserves order of allProfiles', () => {
  const fs = makeFs({
    '/home/u/.haishui/profiles/coder/gateway.pid': { content: '{"pid":1}' },
    '/home/u/.haishui/profiles/writer/gateway.pid': { content: '{"pid":2}' }
  })

  const deps = { ...fs, isHaishuiProcess: () => true }
  assert.deepEqual(findRunningGatewayProfiles('/home/u/.haishui/profiles', ['coder', 'writer'], deps), [
    'coder',
    'writer'
  ])
})
...
test('listProfileDirs includes default and any name passing the regex', () => {
  const fs = makeFs({
    '/home/u/.haishui/profiles/default': { dir: true },
    '/home/u/.haishui/profiles/coder': { dir: true },
    '/home/u/.haishui/profiles/writer': { dir: true }
  })
...
test('listProfileDirs skips files (not directories) and invalid names', () => {
  const fs = makeFs({
    '/home/u/.haishui/profiles/default': { dir: true },
    '/home/u/.haishui/profiles/some-file.txt': { dir: false },
    '/home/u/.haishui/profiles/UPPERCASE': { dir: true } // regex rejects
  })
...
test('decideMigration suppresses write when best is default (single-profile fallback)', () => {
  ...
  // default-primary and we leave it alone. Default's DB is $HAISHUI_HOME/state.db,
  // not profiles/default/state.db.
  const deps = baseDeps()

  const d = decideMigration(null, [], ['default', 'coder'], deps, p => (p.endsWith('/.haishui/state.db') ? 99 : 50))
...
test('migrateActiveProfileIfMissing is a no-op when a user-selected preference file exists', () => {
  ...
  const fs = makeFs({
    ...
    '/home/u/.haishui/profiles/coder': { dir: true },
    '/home/u/.haishui/profiles/writer': { dir: true },
    '/home/u/.haishui/profiles/writer/state.db': { size: 400 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing writes legacy choice with no _migrated flag', () => {
  ...
  const fs = makeFs({
    '/home/u/.haishui/active_profile': { content: 'coder' },
    '/home/u/.haishui/profiles/coder': { dir: true },
    '/home/u/.haishui/profiles/writer': { dir: true }
  })
...
test('migrateActiveProfileIfMissing writes heuristic choice with _migrated=true', () => {
  ...
  const fs = makeFs({
    '/home/u/.haishui/profiles/coder/state.db': { size: 50 * 1024 * 1024, mtime: NOW - 86_400_000 },
    '/home/u/.haishui/profiles/writer/state.db': { size: 200 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing is a no-op for single-profile (default-only) installs', () => {
  ...
  // Production default DB is ~/.haishui/state.db, not profiles/default/state.db.
  let written: unknown = null

  const fs = makeFs({
    '/home/u/.haishui/state.db': { size: 10 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing prefers a single running gateway over heuristics', () => {
  ...
  const fs = makeFs({
    '/home/u/.haishui/profiles/coder/gateway.pid': { content: '{"pid":42}' },
    '/home/u/.haishui/profiles/writer/state.db': { size: 500 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })

  const deps = baseDeps({
    ...fs,
    isHaishuiProcess: (pid: number) => pid === 42,
    writeJson: (_p: string, payload: unknown) => {
      written = payload
    }
  })
...
// ---------------------------------------------------------------------------
// Production layout: default is ~/.haishui, not ~/.haishui/profiles/default
// ---------------------------------------------------------------------------

test('profileStateDbPath puts default at haishuiHome, named under profilesRoot', () => {
  assert.equal(profileStateDbPath('default', '/home/u/.haishui', '/home/u/.haishui/profiles'), '/home/u/.haishui/state.db')
  assert.equal(
    profileStateDbPath('conduit', '/home/u/.haishui', '/home/u/.haishui/profiles'),
    '/home/u/.haishui/profiles/conduit/state.db'
  )
})

test('profileGatewayPidPath puts default at haishuiHome', () => {
  assert.equal(
    profileGatewayPidPath('default', '/home/u/.haishui', '/home/u/.haishui/profiles'),
    '/home/u/.haishui/gateway.pid'
  )
  assert.equal(
    profileGatewayPidPath('coder', '/home/u/.haishui', '/home/u/.haishui/profiles'),
    '/home/u/.haishui/profiles/coder/gateway.pid'
  )
})
...
test('findRunningGatewayProfiles sees default gateway.pid at haishuiHome', () => {
  const fs = makeFs({
    '/home/u/.haishui/gateway.pid': { content: '{"pid":99}' },
    '/home/u/.haishui/profiles/coder/gateway.pid': { content: '{"pid":11}' }
  })

  assert.deepEqual(
    findRunningGatewayProfiles('/home/u/.haishui/profiles', ['default', 'coder'], {
      ...fs,
      haishuiHome: '/home/u/.haishui',
      isHaishuiProcess: pid => pid === 99
    }),
    ['default']
  )
})
...
test('migrateActiveProfileIfMissing does not pin a tiny named profile over a large default DB', () => {
  ...
  // ~/.haishui/profiles/<name>, never scored ~/.haishui/state.db, and wrote
  // { profile: named, _migrated: true }.
  let written: unknown = null

  const fs = makeFs({
    '/home/u/.haishui/profiles/conduit': { dir: true },
    '/home/u/.haishui/state.db': { size: 409 * 1024 * 1024, mtime: NOW - 86_400_000 },
    '/home/u/.haishui/profiles/conduit/state.db': { size: 2 * 1024 * 1024, mtime: NOW - 60_000 }
  })
...
test('migrateActiveProfileIfMissing still pins a named profile that actually beats default', () => {
  ...
  const fs = makeFs({
    '/home/u/.haishui/profiles/work': { dir: true },
    '/home/u/.haishui/state.db': { size: 5 * 1024 * 1024, mtime: NOW - 86_400_000 },
    '/home/u/.haishui/profiles/work/state.db': { size: 200 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing does not pin default when only default gateway is running', () => {
  ...
  const fs = makeFs({
    '/home/u/.haishui/gateway.pid': { content: '{"pid":7}' },
    '/home/u/.haishui/state.db': { size: 10 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })

  const deps = baseDeps({
    ...fs,
    isHaishuiProcess: (pid: number) => pid === 7,
    writeJson: (_p: string, payload: unknown) => {
      written = payload
    }
  })
