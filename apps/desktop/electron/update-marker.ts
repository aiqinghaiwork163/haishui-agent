...
export function markerPath(haishuiHome) {
  return path.join(haishuiHome, '.haishui-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(haishuiHome)
  let raw
 ...
 *
 * The Tauri-based haishui-setup.exe takes several seconds to initialise its
...
export function writeUpdateMarker(
  ...
  const file = markerPath(haishuiHome)
  const nowMs = now()
  const owner = readLiveUpdateMarker(haishuiHome, { kill, maxAgeMs, now: () => nowMs })
 ...
 * Haishui to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(haishuiHome, opts)
