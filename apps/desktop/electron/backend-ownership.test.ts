...
function ownershipEntry(overrides: Partial<BackendIdentity> = {}) {
  return { command: 'haishui serve --port 0', ...identity(overrides) }
}
...
test('backend identity check matches only serve and dashboard invocation shapes', () => {
  assert.equal(backendCommandMatches('/venv/bin/haishui serve --port 0'), true)
  assert.equal(backendCommandMatches('python -m haishui_cli.main dashboard --no-open'), true)
  assert.equal(backendCommandMatches('/venv/bin/haishui --profile work serve --port 0'), true)
  assert.equal(backendCommandMatches('"C:\\Haishui Runtime\\haishui.exe" dashboard --no-open'), true)
  assert.equal(backendCommandMatches('haishui chat --query serve'), false)
