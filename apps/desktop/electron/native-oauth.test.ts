...
test('buildNativeAuthorizeUrl omits provider when not given and preserves prefix', () => {
  const url = buildNativeAuthorizeUrl('https://gw.example.com/haishui', {
    challenge: 'C',
  ...
  assert.equal(parsed.pathname, '/haishui/auth/native/authorize')
...
test('nativeTokenUrl / nativeRefreshUrl build the right endpoints', () => {
  ...
  assert.equal(nativeRefreshUrl('https://gw.example.com/haishui'), 'https://gw.example.com/haishui/auth/native/refresh')
})
