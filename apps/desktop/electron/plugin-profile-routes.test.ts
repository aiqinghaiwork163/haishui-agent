...
describe('buildOpaqueProfileRoutes', () => {
  it('groups SSH aliases by their effective route without exposing endpoint data', async () => {
    ...
    const resolveSsh = vi.fn(async () => ({ hostname: 'gateway.example', port: 22, user: 'haishui' }))
    ...
    expect(JSON.stringify(routes)).not.toContain('.haishui')
  })

  it('changes opaque IDs when the effective SSH destination changes', async () => {
    const options = {
      getProfileConfig: () => config({ mode: 'ssh', sshHost: 'lab', sshRemoteHaishuiPath: '~/.haishui' }),
      globalConfig: config(),
      installationId: 'install-a-secret',
      primaryProfile: 'default',
    ...
    const before = await buildOpaqueProfileRoutes({
      ...options,
      resolveSsh: async () => ({ hostname: 'old.example', port: 22, user: 'haishui' })
    })

    const after = await buildOpaqueProfileRoutes({
      ...options,
      resolveSsh: async () => ({ hostname: 'new.example', port: 22, user: 'haishui' })
    })
  ...
  it('isolates an SSH resolution failure to its configured route', async () => {
    const options = {
      getProfileConfig: (profile: string) =>
        profile === 'broken'
          ? config({ mode: 'ssh', sshHost: 'unreachable', sshPort: 2222, sshUser: 'haishui' })
          : config(),
      globalConfig: config(),
      installationId: 'install-a-secret',
      primaryProfile: 'default',
  ...
  it('reports an explicit backend profile inherited from global SSH', async () => {
    const routes = await buildOpaqueProfileRoutes({
      ...
      resolveSsh: vi.fn(async () => ({ hostname: 'gateway.example', port: 22, user: 'haishui' }))
    })
