 ...
 * wire protocol:
 *
 *   gate dials -> drain every captured scope -> detached `haishui update`
...
function buildPosixManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const home = validateRemoteValue(target.haishuiHome, 'Haishui home')
  const haishuiPath = validateRemoteValue(target.haishuiPath, 'launcher path')
  ...
  const launcherWord = expandRemotePath(haishuiPath)

  const updateCommand =
    `env HAISHUI_HOME=${homeWord} ` +
    `HAISHUI_UPDATE_CORRELATION_ID=${shq(correlation)} ` +
    'HAISHUI_UPDATE_ORIGIN_PROFILE=default ' +
    `HAISHUI_UPDATE_ORIGIN_HOME=${homeWord} ` +
    `HAISHUI_UPDATE_OUTPUT_PATH=${outputWord} ` +
...
function buildWindowsManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const wrapper = [
    ...
    `$env:HAISHUI_HOME=${psLiteral(home)}`,
    `$env:HAISHUI_UPDATE_CORRELATION_ID=${psLiteral(correlation)}`,
    '$env:HAISHUI_UPDATE_ORIGIN_PROFILE="default"',
    `$env:HAISHUI_UPDATE_ORIGIN_HOME=${psLiteral(home)}`,
    `$env:HAISHUI_UPDATE_OUTPUT_PATH=${psLiteral(outputPath)}`,
    ...
    `$env:HAISHUI_UPDATE_WINDOWS_DETACHED=${psLiteral(correlation)}`,
    `$env:HAISHUI_UPDATE_TAURI_OUTCOME_PATH=${psLiteral(statusPath)}`,
    `$env:HAISHUI_UPDATE_TAURI_READY_PATH=${psLiteral(readyPath)}`,
    ...
    `& ${psLiteral(haishuiPath)} update --yes *>> ${psLiteral(outputPath)}`,
    '$rc=$LASTEXITCODE',
...
marker_path=install_root/'.haishui-update-in-progress'
...
async function runManagedSshUpdate<TScope extends ManagedSshScope>(
  ...
  return {
    ...
    message:
      outcome === 'updated'
        ? 'Remote Haishui updated and every managed SSH profile is ready.'
        : restoreOk
