// Backend subcommand routing for the desktop-managed Haishui process.
//
// The desktop app launches its own headless backend via `haishui serve` — it
...
// app hasn't updated yet, or an older `haishui` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Haishui profile to pin via `--profile`.
 */
 ...
 * `-m haishui_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `haishui_cli/subcommands/dashboard.py` source registers
