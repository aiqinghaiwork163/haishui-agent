...
export function installCommandScreenshot({ rendererUrl }: { rendererUrl: string }): () => void {
  ...
  const publish = () => {
    for (const win of BrowserWindow.getAllWindows()) {
      if (!win.isDestroyed()) {
        win.webContents.send('haishui:screenshot:status', status())
      }
    }
  }
  ...
  const start = (requestPermission = false) => {
    ...
    monitor.start(window => {
      ...
      // Retain the last Haishui chat window when another application takes focus.
      ...
      if (requestId) {
        recipient.webContents.send('haishui:screenshot:request', requestId)
      }
    }, result => {
  ...
  const handle = (name: string, callback: (event: IpcMainInvokeEvent, value: unknown) => unknown) => {
    const channel = `haishui:screenshot:${name}`
  ...
  ipcMain.on('haishui:screenshot:subscribe', onSubscribe)
  ...
  const dispose = () => {
    ...
    ipcMain.removeListener('haishui:screenshot:subscribe', onSubscribe)
