...
export const ZOOM_STORAGE_KEY = 'haishui:desktop:zoomLevel'
 ...
 * but forgot to emit 'haishui:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('haishui:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
 ...
 * only learns of changes through `haishui:zoom:changed`, which never fired —
