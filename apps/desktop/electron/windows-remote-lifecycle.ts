...
const READY_RE = /^HAISHUI_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitHaishuiPath = '') {
  const explicit = psLiteral(explicitHaishuiPath)
  ...
  const script = [
    ...
    '$haishuiHome=$env:HAISHUI_HOME',
    'if(-not $haishuiHome){$haishuiHome=Join-Path $env:LOCALAPPDATA "haishui"}',
    'Assert-NoReparse $haishuiHome $true',
    '$candidate=[IO.Path]::Combine($haishuiHome, "haishui-agent\\venv\\Scripts\\haishui.exe")',
    ...
    '$profileCandidate=[IO.Path]::Combine($HOME, "haishui-agent\\.venv\\Scripts\\haishui.exe")',
    ...
    '$fallbackHomeCandidate=Join-Path $haishuiHome "haishui-agent\\venv\\Scripts\\haishui.exe"',
    '$fallbackProfileCandidate=Join-Path $HOME "haishui-agent\\.venv\\Scripts\\haishui.exe"',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command haishui.exe -ErrorAction SilentlyContinue',
    ...
    '$haishui=$null',
    'foreach($candidate in $candidates){Assert-NoReparse $candidate $true;$candidatePython=[IO.Path]::Combine([IO.Path]::GetDirectoryName($candidate), "python.exe");Assert-NoReparse $candidatePython $true;try{$item=Get-Item -LiteralPath $candidate -Force -ErrorAction Stop;if(($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -eq 0 -and -not $item.PSIsContainer){$haishui=$item.FullName;break}}catch [Management.Automation.ItemNotFoundException]{continue}}',
    'if(-not $haishui){throw "Haishui is not installed on the remote Windows host."}',
    'Assert-NoReparse $haishui $false',
    'if($explicit -and $haishui -ne $explicit){throw "The configured Haishui path is not an executable file."}',
    '$python=[IO.Path]::Combine([IO.Path]::GetDirectoryName($haishui), "python.exe")',
    'Assert-NoReparse $python $false',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;haishuiHome=$haishuiHome;haishuiPath=$haishui;python=$python}|ConvertTo-Json -Compress'
  ].join(';')

  return JSON.parse((await ssh.exec(powerShellCommand(script))).trim())
}

function windowsUpdateMarkerProbeCommand(haishuiHome) {
  const script = [
    '$ErrorActionPreference="Stop"',
...
public static class HaishuiMarkerNoFollow {
...
`,
    ...
    '}',
    '}',
    `$haishuiHome=${psLiteral(haishuiHome)}`,
    '$installRoot=$haishuiHome',
    '$parent=Split-Path -Parent $haishuiHome',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".haishui-update-in-progress"',
    '$result="UNCERTAIN"',
    '$stream=$null;$memory=$null',
    'try{',
    'Assert-NoReparse $marker $true',
    'if(-not (Test-Path -LiteralPath $marker -PathType Leaf)){$result="CLEAR"}else{$stream=[HaishuiMarkerNoFollow]::OpenRead($marker)',
 ...
 */
async function assertWindowsRemoteInstallUpdateClear(ssh, haishuiHome) {
  let observation = ''

  try {
    observation =
      String(await ssh.exec(windowsUpdateMarkerProbeCommand(haishuiHome)))
        .replace(/^\uFEFF/, '')
        .trim()
        .split(/\r?\n/)
        .pop() || ''
...
async function detectRemotePlatform(ssh, explicitHaishuiPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitHaishuiPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'haishui_cli.windows_ssh_runtime', operation, ...args]
...
function atomicWindowsSpawnCommand(runtime, reservation: any = {}) {
  const argv = [runtime.python, '-m', 'haishui_cli.windows_ssh_runtime', 'spawn']
  const helper = operation => [runtime.python, '-m', 'haishui_cli.windows_ssh_runtime', operation]

  const script = [
    '$ErrorActionPreference="Stop"',
    `$haishuiHome=${psLiteral(runtime.haishuiHome)}`,
    '$installRoot=$haishuiHome',
    '$parent=Split-Path -Parent $haishuiHome',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".haishui-update-in-progress"',
    ...
    reservation.ownershipId
      ...
      : '  if($LASTEXITCODE -ne 0){exit $LASTEXITCODE}',
    reservation.ownershipId
      ? `  $spawned=$spawnLines[-1]|ConvertFrom-Json; $lock=[ordered]@{schemaVersion=2;protocolVersion=1;ownershipId=${psLiteral(reservation.ownershipId)};spawnNonce=${psLiteral(reservation.spawnNonce)};pid=[int]$spawned.pid;creationTimeNs=[string]$spawned.creationTimeNs;port=0;profile=${psLiteral(reservation.profile)};haishuiPath=${psLiteral(reservation.haishuiPath)};haishuiHome=${psLiteral(reservation.haishuiHome)};tokenFingerprint=${psLiteral(reservation.tokenFingerprint)};startedAt=${psLiteral(reservation.startedAt)}}|ConvertTo-Json -Compress; ` +
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.haishuiPath === 'string' &&
    typeof lock.haishuiHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.haishuiPath === runtime.haishuiPath &&
    lock.haishuiHome === runtime.haishuiHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertBootstrapNotSuperseded(signal)
  const runtime = await probeWindowsRemote(ssh, remoteHaishuiPath)
  await assertWindowsRemoteInstallUpdateClear(ssh, runtime.haishuiHome)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.haishuiPath])

  if (!inspection.supported) {
    const error: any = new Error('Update Haishui on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.haishuiPath = inspection.path
  const haishuiVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located haishui at ${runtime.haishuiPath}`)
  ...
  try {
    ...
    spawned = await atomicWindowsSpawn(
      ssh,
      runtime,
      JSON.stringify({ ownershipId, spawnNonce, profile, haishuiPath: runtime.haishuiPath }),
      {
        ownershipId,
        spawnNonce,
        profile,
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForHaishui(baseUrl, token)
    assertBootstrapNotSuperseded(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="Haishui SSH"', 'powershell.exe -NoLogo')
