...
/**
 * Haishui-managed Node.js directories, in preferred lookup order.
 *
 * There are two on-disk layouts. `scripts/install.ps1` unpacks portable Node
 * straight into `%LOCALAPPDATA%\haishui\node` (node.exe at the root, no `bin\`);
 * `scripts/install.sh` and the node-bootstrap helper use the POSIX
 * `$HAISHUI_HOME/node/bin`. Emit BOTH on every platform so mixed and migrated
 ...
 * `iter_haishui_node_dirs()` in haishui_constants.py, which the Electron main
 * process cannot import.
 */
function haishuiManagedNodePathEntries(
  haishuiHome,
  ...
  const root = pathModule.join(haishuiHome, 'node')
...
function buildDesktopBackendPath({
  ...
  const haishuiNodeDirs = haishuiManagedNodePathEntries(haishuiHome, { platform, pathModule })
  const venvBin = venvRoot ? pathModule.join(venvRoot, platform === 'win32' ? 'Scripts' : 'bin') : null
  const saneEntries = platform === 'win32' ? [] : POSIX_SANE_PATH_ENTRIES

  return appendUniquePathEntries([haishuiNodeDirs, venvBin, currentPath, saneEntries], { delimiter })
}

function normalizeHaishuiHomeRoot(
  haishuiHome,
  ...
  // would pin it under cwd and the Python backend inherits that absolute path via HAISHUI_HOME.
  let raw = String(haishuiHome)
...
function buildDesktopBackendEnv({
  ...
  return {
    ...
    // cp1252, ...). haishui_bootstrap sets this inside the child too, but only
