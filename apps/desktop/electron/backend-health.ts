...
export interface HaishuiReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForHaishuiReady`).
   */
...
export const REMOTE_UNSIGNED_OAUTH_MESSAGE =
  'Remote Haishui gateway uses OAuth, but you are not signed in. ' +
 ...
 *  - readiness-probe exhaustion in waitForHaishuiReady().
 *
...
/**
 * True when the backend URL points at a Nous-managed Haishui Cloud instance
 ...
 * `isReauthRequired` so startHaishui latches; a bare `needsOauthLogin` (the
...
export async function waitForHaishuiReady(baseUrl: string, options: HaishuiReadyOptions): Promise<void> {
  ...
  throw new Error(`Haishui backend did not become ready: ${detail}`)
}
