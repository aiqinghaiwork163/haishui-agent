...
test('redactSecrets scrubs the spawn-time session token env var', () => {
  const line = 'setsid env HAISHUI_DASHBOARD_SESSION_TOKEN=abc123deadbeef HAISHUI_DESKTOP=1 haishui dashboard'
  const out = redactSecrets(line)
  assert.ok(!out.includes('abc123deadbeef'))
  assert.match(out, /HAISHUI_DASHBOARD_SESSION_TOKEN=<redacted>/)
  // non-secret env vars are preserved
  assert.match(out, /HAISHUI_DESKTOP=1/)
})
...
test('redactSecrets scrubs Authorization and X-Haishui-Session-Token headers', () => {
  ...
  assert.match(redactSecrets('X-Haishui-Session-Token: hdr_888'), /X-Haishui-Session-Token: ?<redacted>/)
  assert.ok(!redactSecrets('X-Haishui-Session-Token: hdr_888').includes('hdr_888'))
})
...
test('controlSocketPath default base stays under sun_path even with the temp-listener suffix', () => {
  ...
  const p = controlSocketPath('haishui', 'remote-build-server', 22) // no baseDir → default
...
test('buildExecArgs ends with host then the remote command', () => {
  ...
  const args = buildExecArgs(conn, 'command -v haishui', 15000)
  assert.equal(args[args.length - 1], 'command -v haishui')
...
test('close() removes the control socket when -O exit fails', async () => {
  const dir = path.join(os.tmpdir(), `haishui-ssh-close-${process.pid}-${Date.now()}`)
...
test('open() creates the control-socket directory if it does not exist', async () => {
  const dir = path.join(os.tmpdir(), `haishui-ssh-test-${process.pid}-${Date.now()}`)
...
test('withRemoteTimeout kills a hung probe remotely instead of orphaning it (#110478)', async () => {
  ...
  const wrapped = withRemoteTimeout('haishui --version 2>&1', 15)
