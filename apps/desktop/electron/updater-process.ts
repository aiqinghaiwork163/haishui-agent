 ...
 *
 * The staged Tauri `haishui-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `haishui update` refreshes the code that drives the
 ...
 * and QUITS, the script waits it out, runs `haishui update`, swaps/relaunches
 * the app, and writes .haishui-update-result.json. With the app gone before
 * the update starts, the HAISHUI_DESKTOP_CHILD_PID reaper-exclusion dance is
 ...
 * `haishui update` card — old checkouts pull the script on their next update).
 */
 ...
 *
 * The Tauri installer self-copies into HAISHUI_HOME on *every* platform
 * (`haishui-setup.exe` on Windows, `haishui-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_haishui_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `haishui update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(haishuiHome, 'haishui-setup.exe')
 ...
 *
 * `copy_self_to_haishui_home` deliberately no-ops during `--update`
 ...
 * "Another Haishui update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `haishui update`, which
 ...
 * the installer is written to HAISHUI_HOME at install/repair time, making mtime
 ...
 * The lead sentence is plain: nothing changed and Haishui keeps running. The raw
...
export function describeUpdaterHandoffFailure(outcome: Pick<UpdaterHandoffOutcome, 'message'>): string {
  const lead =
    "The updater couldn't start, so nothing was changed and Haishui keeps running as before. " +
