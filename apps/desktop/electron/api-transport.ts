/**
 * Shared HTTP transport policy for the Electron main process's Haishui REST
...
function htmlResponseError(url: string, statusCode: unknown, location?: unknown) {
  ...
  return new Error(
    `Expected JSON from ${url} but got HTML (status ${statusCode}). The endpoint is likely missing on the Haishui backend.`
  )
}
