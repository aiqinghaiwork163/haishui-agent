...
test('output tail interleaves stdout and stderr attached from spawn time', () => {
  ...
  child.stderr.emit('data', Buffer.from("ModuleNotFoundError: No module named 'haishui_cli'\n"))
...
test('exit line carries the buffered tail next to the exit code, preferring the signal', () => {
  ...
  assert.equal(
    formatBackendExitLine('Ignoring stale Haishui backend exit', 1, null, tail),
    'Ignoring stale Haishui backend exit (1)\nRecent backend output:\nTraceback (most recent call last):'
  )
  assert.equal(formatBackendExitLine('Haishui backend exited', null, 'SIGTERM', tail), 'Haishui backend exited (SIGTERM)\nRecent backend output:\nTraceback (most recent call last):')
})

test('exit line stays byte-identical to the legacy shape when the tail is empty or missing', () => {
  assert.equal(formatBackendExitLine('Haishui backend exited', 0, null, createBackendOutputTail(64)), 'Haishui backend exited (0)')
  assert.equal(formatBackendExitLine('Haishui backend exited', 1, null, null), 'Haishui backend exited (1)')
})
