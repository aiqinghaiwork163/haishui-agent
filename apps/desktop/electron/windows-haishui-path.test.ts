// Unit tests for the pure Windows `haishui` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvHaishuiCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `haishui` shim shadows
//      the real haishui.cmd/haishui.exe.
...
//   3. resolveVenvHaishuiCommand() — must probe the venv python via
//      canImportHaishuiCli() before trusting it, or a broken venv gets
...
test('chooseUpdaterArgs: gentle --update when both updater runtime files exist', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvHaishui: true, hasVenvPython: true }, 'main'), [
    '--update',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: marker-only install uses --repair when the venv is gone', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvHaishui: false, hasVenvPython: false }, 'main'),
    ['--repair', '--branch', 'main']
  )
})

test('chooseUpdaterArgs: partial updater runtimes use --repair', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvHaishui: false, hasVenvPython: true }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvHaishui: true, hasVenvPython: false }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: passes the branch through unchanged in both modes', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvHaishui: true, hasVenvPython: true }, 'release/1.2'),
    ['--update', '--branch', 'release/1.2']
  )
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvHaishui: false, hasVenvPython: false }, 'release/1.2'),
    ['--repair', '--branch', 'release/1.2']
  )
})

function makeDeps(overrides: Partial<Parameters<typeof resolveVenvHaishuiCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportHaishuiCli: async () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    haishuiHome: '/fake/haishui-home',
...
test('resolveVenvHaishuiCommand: returns null off Windows', async () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(await resolveVenvHaishuiCommand('/root/venv/Scripts/haishui.exe', [], deps), null)
})

test('resolveVenvHaishuiCommand: returns null for a .cmd/.bat script command', async () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(await resolveVenvHaishuiCommand('/root/venv/Scripts/haishui.cmd', [], deps), null)
})

test('resolveVenvHaishuiCommand: returns null when the basename is not haishui/haishui.exe', async () => {
  const deps = makeDeps()

  assert.equal(await resolveVenvHaishuiCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvHaishuiCommand: returns null when the parent dir is not Scripts', async () => {
  const deps = makeDeps()

  assert.equal(await resolveVenvHaishuiCommand('/root/venv/bin/haishui.exe', [], deps), null)
})

test('resolveVenvHaishuiCommand: returns null when the venv python does not exist on disk', async () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(await resolveVenvHaishuiCommand('/root/venv/Scripts/haishui.exe', [], deps), null)
})

test('resolveVenvHaishuiCommand: probes the venv python before trusting it (returns null on failed probe)', async () => {
  let probed = false

  const deps = makeDeps({
    canImportHaishuiCli: async (python: string) => {
      probed = true
  ...
  const result = await resolveVenvHaishuiCommand('/root/venv/Scripts/haishui.exe', ['serve'], deps)
...
test('resolveVenvHaishuiCommand: returns the resolved python backend descriptor when the probe passes', async () => {
  const deps = makeDeps()
  const result = await resolveVenvHaishuiCommand('/root/venv/Scripts/haishui.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'haishui_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvHaishuiCommand: is case-insensitive on haishui.exe and the Scripts dir name', async () => {
  const deps = makeDeps()

  assert.ok(await resolveVenvHaishuiCommand('/root/venv/Scripts/HAISHUI.EXE', [], deps))
  assert.ok(await resolveVenvHaishuiCommand('/root/venv/SCRIPTS/haishui.exe', [], deps))
})
