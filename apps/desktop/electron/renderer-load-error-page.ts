 ...
 *
 * 1. TORN BUNDLE after an update (#95575): `haishui update` replaces the app
...
export interface RendererLoadErrorDetails {
  ...
  /** Repair command hint, e.g. `haishui desktop --force-build`. */
  repairHint?: string
  /**
...
export function buildRendererLoadErrorPage(details: RendererLoadErrorDetails = {}): string {
  ...
  const title = 'Haishui couldn\u2019t start the desktop UI'
  ...
  const repair = details.repairHint ? `<p>Repair with: <code>haishui desktop --force-build</code></p>` : ''
...
<main>
  ...
  <code>haishui desktop --force-build</code>, then restart the app.</p>
