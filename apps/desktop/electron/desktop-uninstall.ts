 ...
 * the three user-facing uninstall modes to the `haishui uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m haishui_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `haishui_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'haishui_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Haishui.app/Contents/MacOS/Haishui  → …/Haishui.app
 *   Windows: …\Haishui\Haishui.exe                 → …\Haishui  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Haishui.app/Contents/MacOS/Haishui → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Haishui.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Haishui.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Haishui$/i.test(dir) || /[\\/]haishui-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/haishui
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import haishui_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, haishuiHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export HAISHUI_HOME=${q(haishuiHome)}`
  ]
 ...
 * modes + `pythonPath`=agentRoot so `import haishui_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Haishui install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "HAISHUI_HOME=${String(haishuiHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
