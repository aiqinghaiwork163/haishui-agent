...
// registry, and the haishui:terminal:* IPC surface. Extracted from main.ts; the
...
export function registerTerminalIpc({
  ...
  function terminalShellCommand() {
    // HAISHUI_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
    ...
    const override = (process.env.HAISHUI_DESKTOP_SHELL || (isWindows ? '' : process.env.SHELL) || '').trim()
  ...
  function terminalShellEnv() {
    ...
    // /TERM=dumb; some terminals set COLORFGBG which would flip Haishui' TUI into
    ...
    env.TERM_PROGRAM = 'Haishui'
    env.TERM_PROGRAM_VERSION = app.getVersion()

    // Let a haishui/--tui launched in this pane know it's embedded in the desktop
    // GUI (build_environment_hints surfaces this). Distinct from HAISHUI_DESKTOP,
    // which marks the agent *backend* and gates cron/gateway behavior.
    env.HAISHUI_DESKTOP_TERMINAL = '1'

    return env
  }

  function terminalChannel(id, suffix) {
    return `haishui:terminal:${id}:${suffix}`
  }
  ...
  ipcMain.handle('haishui:terminal:start', async (event, payload = {}) => {
  ...
  ipcMain.handle('haishui:terminal:attach', (event, id) => {
  ...
  ipcMain.handle('haishui:terminal:write', (_event, id, data) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('haishui:terminal:resize', (_event, id, size = {}) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('haishui:terminal:cwd', async (_event, id) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('haishui:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))
