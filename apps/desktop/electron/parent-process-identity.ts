export type ParentWatchdogEnv = {
  HAISHUI_PARENT_PID: string
  HAISHUI_PARENT_START_MARKER?: string
  HAISHUI_PARENT_NONCE?: string
  /** Spawn tag consumed by haishui_cli.process_identity (`v1:<install>:<purpose>:<spawner_pid>:<spawner_create_s>`).
...
export function parentWatchdogEnv(pid: number, startMarker: string | null, nonce: string): ParentWatchdogEnv {
  ...
  const env: ParentWatchdogEnv = { HAISHUI_PARENT_PID: String(pid) }
  env.HAISHUI_SPAWN = spawnTag(pid, startMarker)
  ...
  env.HAISHUI_PARENT_START_MARKER = startMarker
  env.HAISHUI_PARENT_NONCE = nonce

  return env
}

/** Build the `HAISHUI_SPAWN` tag mirrored by haishui_cli/process_identity.py.
