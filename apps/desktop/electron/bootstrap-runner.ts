 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Haishui Agent by spawning
...
// Relative, not `@haishui/shared/ansi`: the electron bundle is built by esbuild
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.haishui-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(haishuiHome) {
  return path.join(haishuiHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.haishui/haishui-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(haishuiHome) {
  if (!haishuiHome) {
    return null
  }

  const candidate = path.join(haishuiHome, 'haishui-agent', 'scripts', installScriptName())
...
function cachedScriptPath(haishuiHome, commit) {
  return path.join(bootstrapCacheDir(haishuiHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/aiqinghaiwork163/haishui-agent/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(haishuiHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(haishuiHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, haishuiHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass HAISHUI_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          HAISHUI_HOME: haishuiHome || process.env.HAISHUI_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, haishuiHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, haishuiHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--haishui-home', haishuiHome]
...
async function fetchManifest({
  ...
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, haishuiHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, haishuiHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(haishuiHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, haishuiHome, emit })
    abortSignal?.throwIfAborted()
