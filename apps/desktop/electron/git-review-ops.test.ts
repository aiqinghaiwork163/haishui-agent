...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'haishui-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'haishui-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Haishui Test'], { cwd: dir })
