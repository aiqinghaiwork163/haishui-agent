 ...
 * runtime, remote gateways, Haishui Cloud instances, SSH hosts) that are all
 ...
 * apps/shared/src/backend-scope.ts (`@haishui/shared`) — tsconfig project
...
export async function reuseMatchingPrimarySshBackend({
  ...
  if (
    ...
    String(source.remoteHaishuiPath || '').trim() !== String(activeSsh.remoteHaishuiPath || '').trim() ||
...
/** Turn `ls ~/.haishui/profiles` output into roster names. Always includes
...
/**
 * Whether "Update all instances" may drive this connection. Haishui Cloud
 * instances are platform-managed — we never run `haishui update` against them.
...
export function normalizeConnectionInput(input: ConnectionInput, registry: ConnectionRegistry): RegistryConnection {
  ...
  if (kind === 'ssh') {
    const ssh = normalizeSshConfig({
      ...
      remoteHaishuiPath: input.remoteHaishuiPath,
  ...
  if (kind === 'remote' || kind === 'cloud') {
    ...
    // transport-/Haishui-managed names; an empty result stores nothing.
 ...
 * `remoteHaishuiPath`/`remoteProfile`. Only fields the payload explicitly
...
export function migrateV1ToRegistry(v1: unknown): ConnectionRegistry {
  ...
  const addRemoteLike = (block: Record<string, any>, kind: 'cloud' | 'remote'): null | RegistryConnection => {
    ...
    const label = uniqueLabel(
      hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'Haishui Cloud' : 'Remote gateway'),
...
export function reconcileAppliedGlobalConnection(
  ...
  const hostLabel = hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'Haishui Cloud' : 'Remote gateway')
