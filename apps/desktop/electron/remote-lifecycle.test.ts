...
function fakeSsh(rules: any[] = []) {
  ...
  return {
    ...
    async exec(cmd) {
      ...
      if (cmd.includes('.haishui-update-in-progress') && !cmd.includes('marker_clear()') && !/setsid|nohup/.test(cmd)) {
        return 'CLEAR'
      }
...
test('POSIX relaunch gate refuses live and uncertain install markers without executing Haishui', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = {
      async exec(command) {
        ...
        if (command.includes('HAISHUI_HOME')) {
          return '/home/alice/.haishui\n'
        }

        if (command.includes('.haishui-update-in-progress')) {
          return observation
        }
...
test('POSIX relaunch gate permits absent/dead markers and normalizes named-profile homes install-wide', async () => {
  ...
  await assertRemoteInstallUpdateClear(ssh, '/home/alice/.haishui/profiles/research')
  assert.match(commands[0], /home\.parent\.name/)
  assert.match(commands[0], /profiles/)
  assert.match(commands[0], /\.haishui-update-in-progress/)
})
...
test('listRemoteHaishuiProfiles inventories Mini-style profile dirs without spawning a dashboard', async () => {
  const ssh = fakeSsh([
    [/HAISHUI_HOME/, '/Users/zillajr/.haishui\n'],
    [/ls -1/, 'bob\ndixie\ngoose\nrambo\nbob.rollback-old\n']
  ])

  assert.deepEqual(await listRemoteHaishuiProfiles(ssh), ['default', 'bob', 'dixie', 'goose', 'rambo'])
  assert.equal(
...
test('listRemoteHaishuiProfiles rejects a hostile HAISHUI_HOME', async () => {
  const ssh = fakeSsh([[/HAISHUI_HOME/, '/tmp/x; echo pwned\n']])

  await assert.rejects(
    () => listRemoteHaishuiProfiles(ssh),
    (err: any) => {
...
test('locateHaishui prefers the explicit profile path when executable', async () => {
  const ssh = fakeSsh([[/\[ -x .*\/opt\/haishui/, 'OK']])
  assert.equal(await locateHaishui(ssh, '/opt/haishui'), '/opt/haishui')
})

test('locateHaishui throws (no silent fallback) when an EXPLICIT path is not executable', async () => {
  // command -v WOULD find a different install, but an explicit path must not
  // silently fall back to it — that is the "connected to the wrong haishui" bug.
  const ssh = fakeSsh([
    [/command -v haishui/, '/home/u/.local/bin/haishui\n'],
    [/\[ -x .*\.local\/bin\/haishui/, 'OK']
  ])

  await assert.rejects(
    () => locateHaishui(ssh, '/bad/path/haishui'),
    (err: any) => {
      assert.equal(err.kind, 'haishui-not-found')
      assert.match(err.message, /\/bad\/path\/haishui/)
...
test('locateHaishui falls back to the login-shell command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v haishui/, '/home/u/.local/bin/haishui\n'],
    [/\[ -x .*\.local\/bin\/haishui/, 'OK']
  ])

  assert.equal(await locateHaishui(ssh, ''), '/home/u/.local/bin/haishui')
})

test('locateHaishui preserves an installer wrapper instead of resolving its interpreter', async () => {
  // install.sh venv mode writes: exec "$HAISHUI_BIN" "$HAISHUI_ENTRYPOINT" "$@",
  // where $HAISHUI_BIN is the venv python. The old canonicalization returned
  ...
  const ssh = fakeSsh([
    ...
    [/python3 -c/, '/home/u/.haishui/haishui-agent/venv/bin/python\n']
  ])

  assert.equal(await locateHaishui(ssh, ''), '/home/u/.local/bin/haishui')
  assert.ok(
    !ssh.calls.some(cmd => cmd.includes('python3 -c')),
    'locateHaishui must not shell out to a python3 parser to rewrite the launcher'
  )
})

test('locateHaishui returns an explicit remoteHaishuiPath unchanged', async () => {
  // The override half of #74411: an explicit remoteHaishuiPath pointing at a
  // wrapper was also canonicalized to its interpreter, so overriding to
  // ~/.local/bin/haishui changed nothing for affected users.
  const ssh = fakeSsh([
    [/\[ -x .*\.local\/bin\/haishui/, 'OK'],
    [/python3 -c/, '/home/u/.haishui/haishui-agent/venv/bin/python\n']
  ])

  assert.equal(await locateHaishui(ssh, '~/.local/bin/haishui'), '~/.local/bin/haishui')
  assert.ok(!ssh.calls.some(cmd => cmd.includes('python3 -c')), 'an explicit remoteHaishuiPath must never be rewritten')
})

test('locateHaishui falls back to ~/.local/bin/haishui when the login-shell probe misses', async () => {
  ...
  assert.equal(await locateHaishui(ssh, ''), '~/.local/bin/haishui')
})

test('locateHaishui tries the conventional venv path last', async () => {
  const ssh = fakeSsh([[/\[ -x .*venv\/bin\/haishui/, 'OK']])
  assert.equal(await locateHaishui(ssh, ''), '~/.haishui/haishui-agent/venv/bin/haishui')
})

test('locateHaishui throws a haishui-not-found error with an install hint', async () => {
...
test('locateHaishui uses a login shell for the command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v haishui/, '/x/haishui'],
    [/\[ -x/, 'OK']
  ])

  await locateHaishui(ssh, '')
  assert.ok(
...
test('ownership paths are isolated by ownership ID and spawn nonce', () => {
  assert.equal(ownershipDirectory(OWNERSHIP_ID), `~/.haishui/desktop-ssh/${OWNERSHIP_ID}`)
  assert.equal(lockfilePath(OWNERSHIP_ID), `~/.haishui/desktop-ssh/${OWNERSHIP_ID}/backend.lock.json`)
  assert.equal(spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE), `~/.haishui/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.log`)
})
...
test('metadata and process proof transport failures remain indeterminate', async () => {
  ...
  await assert.rejects(
    () => pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, failure]]), 5, SPAWN_NONCE, '/x/haishui'),
    (error: any) => error.kind === 'transient-transport-error'
  )
})

test('pidIsOurDashboard requires the exact serve ownership nonce', async () => {
  const ours = `/x/haishui serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'OWNED\n']]), 5, SPAWN_NONCE, '/x/haishui'), true)
  assert.equal(
...
test('pidIsOurDashboard accepts the venv entrypoint an installer wrapper execs into', async () => {
  ...
  assert.equal(
    await pidIsOurDashboard(ssh, 5, SPAWN_NONCE, '~/.local/bin/haishui', '/Users/cd9c/.haishui', OWNERSHIP_ID, 'ops'),
    true
  )
  assert.match(ownershipProbe, /haishui-agent.*venv.*bin.*haishui/)
...
test.skipIf(process.platform === 'win32')(
  ...
  async () => {
    const temp = await mkdtemp(path.join(os.tmpdir(), 'haishui wrapper ownership '))
    ...
    const entrypoint = path.join(installDir, 'haishui')
    const launcher = path.join(temp, 'haishui launcher')
    ...
    try {
      ...
      assert.equal(
        await pidIsOurDashboard(ssh, child.pid, SPAWN_NONCE, launcher, '/unrelated/haishui-home', OWNERSHIP_ID, 'ops'),
        true
      )
      assert.equal(
        await pidIsOurDashboard(
...
test('buildSpawnCommand is headless serve, detached, token not in argv', () => {
  const cmd = buildSpawnCommand('/x/haishui', 'work', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  ...
  assert.ok(!cmd.includes('HAISHUI_DASHBOARD_SESSION_TOKEN'), 'token env var must not appear')
})
...
test('buildSpawnCommand atomically reserves the ownership slot through spawn and lock publication', () => {
  const cmd = buildSpawnCommand('/x/haishui', 'work', {
    haishuiHome: '~/.haishui',
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    ownershipId: OWNERSHIP_ID,
  ...
  assert.ok(cmd.includes('.haishui-update-in-progress.mutex'))
  assert.match(cmd, /fcntl\.flock\(fd,fcntl\.LOCK_EX\)/)
  assert.match(cmd, /os\.O_CLOEXEC/)
  assert.match(
    cmd,
    /subprocess\.run\(\["sh","-c",payload,"haishui-update-mutex",str\(fd\)\],pass_fds=\(fd,\),check=False\)/
  )
  assert.doesNotMatch(cmd, /os\.set_inheritable\(fd,True\)/)
  assert.match(cmd, /haishui-update-child "\$1"/)
...
test.skipIf(process.platform === 'win32')('detached backend does not inherit the update mutex descriptor', async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), 'haishui-update-mutex-'))
  const haishuiPath = path.join(directory, 'haishui')
...
for fd in /proc/$$/fd/*; do
  ...
  case "$target" in
    *haishui-update-in-progress.mutex) printf '%s\\n' "$target" >> ${reportPath} ;;
  esac
done
`,
      { mode: 0o700 }
    ...
    const command = buildSpawnCommand(haishuiPath, '', {
      haishuiHome: path.join(directory, 'home'),
      logPath
    })
...
test('spawnRemoteDashboard always spawns serve (legacy dashboard path removed)', async () => {
  ...
  await spawnRemoteDashboard(ssh, { haishuiPath: '/x/haishui', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID })
...
test('READY_RE accepts both serve and dashboard sentinels', () => {
  assert.equal(READY_RE.exec('HAISHUI_BACKEND_READY port=4321')?.[1], '4321')
  assert.equal(READY_RE.exec('HAISHUI_DASHBOARD_READY port=8765')?.[1], '8765')
  // The remote log is `>> log 2>&1`, so a stderr chunk without a newline can be spliced onto the sentinel.
  assert.equal(READY_RE.exec('INFO  Started server process [4711]HAISHUI_BACKEND_READY port=65238')?.[1], '65238')
})
...
test('spawnRemoteDashboard rejects when no pid is returned', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { haishuiPath: '/x/haishui', profile: '', token: 't', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('scrapeReadyPort reads only the named spawn log', async () => {
  ...
  const ssh = fakeSsh([[/cat/, 'some noise\nHAISHUI_DASHBOARD_READY port=51234\n']])
...
test('connect() spawns fresh when there is no lockfile, adopts the served token', async () => {
  const ssh = fakeSsh([
    ...
    [/setsid/, '777\n'],
    [/kill -0 777/, 'ALIVE'],
    [/cat .*\.log/, 'HAISHUI_DASHBOARD_READY port=51999\n']
  ])
...
test('managed SSH maps a local scope to a different non-default remote profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '778\n'],
    [/kill -0 778/, 'ALIVE'],
    [/cat .*\.log/, 'HAISHUI_BACKEND_READY port=52000\n']
  ])
  ...
  assert.match(spawn, /\.haishui\/desktop-ssh\/[0-9a-f]{32}\/[0-9a-f]{16}\.token/)
...
test('connect() respawns when the requested remote profile differs from the lockfile profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/kill 333/, ''],
    [/--version/, 'Haishui Agent v0.18.2\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/setsid/, '890\n'],
    [/kill -0 890/, 'ALIVE'],
    [/cat .*\.log/, 'HAISHUI_DASHBOARD_READY port=52050\n']
  ])
...
test('connect() respawns when the lockfile haishuiPath differs from the resolved path', async () => {
  const reuseToken = 'stored-token'
  const lock = ownedLock({ haishuiPath: '/old/stale/haishui', tokenFingerprint: fingerprintToken(reuseToken) })
  ...
  const result = await connect(
    connectDeps(ssh, { reuseToken, remoteHaishuiPath: '/new/haishui', adoptServedToken: async () => 'fresh' })
  )
...
test('connect() respawns when the lockfile protocolVersion is incompatible', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '901\n'],
    [/kill -0 901/, 'ALIVE'],
    [/cat .*\.log/, 'HAISHUI_DASHBOARD_READY port=44100\n']
  ])
...
test('connect() fresh spawn writes haishuiHome + protocolVersion into the lockfile', async () => {
  const writes: string[] = []
  ...
  const ssh = fakeSsh([
    ...
    [/\[ -x/, 'OK'],
    [/cat .*lock\.json/, ''], // no lockfile
    [/HAISHUI_HOME/, '/home/alice/.haishui\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/printf '%s\\n'/, ''],
    [/setsid/, '700\n'],
    [/kill -0 700/, 'ALIVE'],
    [/cat .*\.log/, 'HAISHUI_DASHBOARD_READY port=45500\n'],
    [
      /printf '%s' '/,
      c => {
        writes.push(c)
  ...
  assert.match(lockWrite, /"haishuiHome":"\/home\/alice\/\.haishui"/)
})
...
test('connect() respawns when the lockfile pid is dead (killed dashboard)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '888\n'],
    [/kill -0 888/, 'ALIVE'],
    [/cat .*\.log/, 'HAISHUI_DASHBOARD_READY port=42000\n']
  ])
...
test('connect() respawns when the dashboard is wedged (alive pid, probe fails)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '999\n'],
    [/kill -0 999/, 'ALIVE'],
    [/cat .*\.log/, 'HAISHUI_DASHBOARD_READY port=43000\n']
  ])
...
test('validateRemotePath accepts absolute POSIX paths', () => {
  assert.doesNotThrow(() => validateRemotePath('/usr/bin/haishui'))
  assert.doesNotThrow(() => validateRemotePath('/home/user/.haishui/haishui-agent/venv/bin/haishui'))
})

test('validateRemotePath accepts ~/ prefix paths', () => {
  assert.doesNotThrow(() => validateRemotePath('~/bin/haishui'))
  assert.doesNotThrow(() => validateRemotePath('~/.haishui/logs/desktop-ssh.log'))
  assert.doesNotThrow(() => validateRemotePath('~'))
})

test('validateRemotePath accepts paths with spaces and quotes', () => {
  assert.doesNotThrow(() => validateRemotePath('/home/user/my project/haishui'))
...
test('validateRemotePath rejects relative paths', () => {
  assert.throws(() => validateRemotePath('haishui'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('./bin/haishui'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('../etc/passwd'), /absolute|relative/i)
})

test('validateRemotePath rejects NUL and newline', () => {
  assert.throws(() => validateRemotePath('/usr/bin/haishui\x00'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/haishui\n'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/haishui\r'), /unsafe/i)
})

test('validateRemotePath preserves shell metacharacters as path data', () => {
  for (const p of ['/usr/$(whoami)/haishui', '/usr/`id`/haishui', '/usr/a;b|c&d<e>f']) {
...
test('expandRemotePath expands ~/ to "$HOME"/', () => {
  const result = expandRemotePath('~/.haishui/logs/desktop-ssh.log')
...
test('expandRemotePath returns quoted absolute paths unchanged', () => {
  const result = expandRemotePath('/usr/local/bin/haishui')
  assert.ok(result.includes('/usr/local/bin/haishui'))
  assert.ok(!result.includes('eval'))
})

test('expandRemotePath preserves spaces as data', () => {
  const result = expandRemotePath('/home/user/my project/haishui')
...
test('buildSpawnCommand does not embed the token in the command string', () => {
  ...
  assert.ok(!cmd.includes('HAISHUI_DASHBOARD_SESSION_TOKEN'), 'env var name must not appear')
})

test('buildSpawnCommand includes --ssh-session-token-file when tokenFilePath is provided', () => {
  const cmd = buildSpawnCommand('/x/haishui', 'work', {
    tokenFilePath: `~/.haishui/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.token`,
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    spawnNonce: SPAWN_NONCE
  })

  assert.match(cmd, /--ssh-session-token-file/)
  assert.match(cmd, /\.haishui\/desktop-ssh\//)
})

test('buildSpawnCommand always uses serve, never dashboard', () => {
  const cmd = buildSpawnCommand('/x/haishui', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /serve --isolated/)
  assert.doesNotMatch(cmd, /\bdashboard\b/)
  assert.doesNotMatch(cmd, /--skip-build/)
...
test('buildSpawnCommand raises the SSH child file limit before execing Haishui', () => {
  const cmd = buildSpawnCommand('/x/haishui', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /ulimit -n 65536 2>\/dev\/null \|\| true; exec env HAISHUI_DESKTOP=1/)
...
test('spawnRemoteDashboard removes a token file when upload reporting fails', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { haishuiPath: '/x/haishui', profile: '', token: 'tok', ownershipId: OWNERSHIP_ID }),
    /channel closed/
  )
...
test('spawnRemoteDashboard fails with update-required when remote lacks --ssh-session-token-file', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { haishuiPath: '/x/haishui', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('readLockfile treats a log path outside the exact ownership and spawn path as skew', async () => {
  const lock = ownedLock({ logPath: '~/.haishui/desktop-ssh/other.log' })
...
test('cleanupStale never deletes a lock-supplied unexpected log path', async () => {
  ...
  await cleanupStale(ssh, OWNERSHIP_ID, ownedLock({ logPath: '~/.haishui/unrelated.log' }))
...
test('pidIsOurDashboard requires an exact nonce option value', async () => {
  const prefix = `/x/haishui serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}ff`
  const suffix = `/x/haishui serve --isolated --ssh-owner-nonce xx${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/haishui'), false)
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/haishui'), false)
})
...
test('remote SSH ownership capability requires both secure bootstrap flags', async () => {
  ...
  assert.equal(await remoteSupportsSshOwnership(supported, '/x/haishui'), true)
  ...
  assert.equal(await remoteSupportsSshOwnership(unsupported, '/x/haishui'), false)
})
...
test.skipIf(process.platform === 'win32')('capability probe survives a zsh login shell on the remote (#111949)', async t => {
  ...
  const dir = await mkdtemp(path.join(os.tmpdir(), 'haishui-zsh-probe-'))

  try {
    const haishui = path.join(dir, 'haishui')
    await writeFile(haishui, '#!/bin/sh\necho "--ssh-session-token-file --ssh-owner-nonce"\n', { mode: 0o700 })

    const ssh = { exec: async (command: string) => (await exec(command, { shell: zsh })).stdout }

    assert.equal(await remoteSupportsSshOwnership(ssh, haishui), true)
  } finally {
...
test('probes run under the remote watchdog so a hung CLI cannot orphan (#110478)', async () => {
  ...
  const versionSsh = fakeSsh([
    [
      ...
      (cmd: string) => {
        ...
        return 'Haishui Agent v0.18.2 (abc123)\n'
      }
    ]
  ])

  assert.equal(await probeHaishuiVersion(versionSsh, '/x/haishui'), 'Haishui Agent v0.18.2 (abc123)')
  ...
  assert.equal(await remoteSupportsSshOwnership(helpSsh, '/x/haishui'), true)
...
test.skipIf(process.platform === 'win32')(
  ...
  async () => {
    ...
    const root = await mkdtemp(path.join(os.tmpdir(), 'haishui-argv-shim-'))
    ...
    try {
      ...
      assert.equal(
        argv[2],
        `${fakeHome}/.haishui/.haishui-update-in-progress.mutex`,
      ...
      const base = `${fakeHome}/.haishui/desktop-ssh/${OWNERSHIP_ID}`
...
test('connect() post-spawn cleanup that cannot prove ownership keeps the original boot error', async () => {
  ...
  await assert.rejects(
    connect(connectDeps(ssh, { platform: { os: 'Linux', arch: 'x86_64' }, waitForHaishui: async () => { throw boot } })),
