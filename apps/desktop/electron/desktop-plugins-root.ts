// The standalone desktop-plugin root (`<HAISHUI_HOME>/desktop-plugins`) and the
...
//      is COPIED out as `<root>/<name>/` with a `.haishui-package.json` marker
...
export const PACKAGE_MARKER = '.haishui-package.json'
...
async function packageOrigin(packageDir: string): Promise<Pick<DesktopHalfMarker, 'catalogName' | 'repo' | 'sha'>> {
  try {
    const sidecar = JSON.parse(await fs.promises.readFile(path.join(packageDir, '.haishui-catalog.json'), 'utf8')) as {
      catalog_name?: string
      repo?: string
      sha?: string
    }
...
/** Every haishui home the app knows about locally: the default plus each profile. */
export async function localHomes(haishuiHome: string): Promise<string[]> {
  const profiles = await listDirs(path.join(haishuiHome, 'profiles'))

  return [haishuiHome, ...profiles.map(name => path.join(haishuiHome, 'profiles', name))]
}
...
export async function migrateProfileScopedDesktopPlugins(haishuiHome: string, appRoot: string): Promise<string[]> {
  const moved: string[] = []

  for (const profile of await listDirs(path.join(haishuiHome, 'profiles'))) {
    const scopedRoot = path.join(haishuiHome, 'profiles', profile, DESKTOP_PLUGINS_DIR)
...
export async function reconcileUnifiedDesktopHalves(haishuiHome: string, appRoot: string): Promise<string[]> {
  const touched: string[] = []
  const seen = new Set<string>()

  for (const home of await localHomes(haishuiHome)) {
