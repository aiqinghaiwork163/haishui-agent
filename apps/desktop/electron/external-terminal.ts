// Launching the Haishui TUI in the user's OWN terminal emulator.
//
...
// `haishui --tui --resume <id>` there. Two problems have to be solved for that
// to work anywhere:
//
//  1. WHAT to run. The desktop's Haishui runtime is often a venv Python invoked
//     as `python -m haishui_cli.main`, not a `haishui` on PATH — so the command
 ...
 * headless child) is the wrong answer for an interactive terminal. The Haishui
