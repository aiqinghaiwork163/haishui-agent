...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'haishui-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/haishui-runner-test',
    sourceRepoRoot: null,
    haishuiHome: '/tmp/haishui-runner-test',
    logRoot: '/tmp/haishui-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'haishui-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'haishui-agent')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/haishui-agent',
      haishuiHome: '/tmp/haishui'
    }),
    ['--dir', '/tmp/haishui-agent', '--haishui-home', '/tmp/haishui', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/haishui-agent', '--haishui-home', '/tmp/haishui', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/haishui',
      haishuiHome: '/tmp/home'
    }),
    ['--dir', '/tmp/haishui', '--haishui-home', '/tmp/home', '--branch', 'main']
  )
})
...
test('installer log lines reach the emitter without escape sequences; \\r redraws keep the last frame', () => {
  ...
  assert.equal(cleanInstallerLogLine('\u001b]0;haishui\u0007Installing Haishui'), 'Installing Haishui')
