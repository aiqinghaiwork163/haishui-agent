// Profile-delete routing logic for the `haishui:api` IPC handler.
//
...
// be rejected: spawning a fresh backend would call ensure_haishui_home() and
...
/** Parse a `haishui:api` request into the profile name a DELETE targets. */
 ...
 * still removing the profile and recreate its HAISHUI_HOME.
 *
...
/**
 * Route the next `haishui:api` request away from the primary/window backend
 ...
 * ensure_haishui_home() recreates the directory the delete just removed.
 */
