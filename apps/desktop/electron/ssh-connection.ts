...
// orphan (ppid=1) — a hung remote CLI (e.g. a wedged `haishui --version`)
...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(HAISHUI_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Haishui-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.haishui/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'haishui-desktop-ssh')
  }

  return path.join(os.homedir(), '.haishui', 'desktop-ssh')
}
