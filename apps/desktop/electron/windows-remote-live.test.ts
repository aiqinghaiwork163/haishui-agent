...
//   HAISHUI_WIN_SSH_HOST   ssh alias/host of the Windows box
//   HAISHUI_WIN_SSH_USER   remote user
//   HAISHUI_WIN_SSH_HAISHUI absolute path to the remote haishui.exe under test
const liveHost = process.env.HAISHUI_WIN_SSH_HOST || ''
const liveUser = process.env.HAISHUI_WIN_SSH_USER || ''
const configuredHaishui = process.env.HAISHUI_WIN_SSH_HAISHUI || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Haishui-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredHaishui)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredHaishui.replace('haishui.exe', 'python.exe')}' -m haishui_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredHaishui.replace('haishui.exe', 'python.exe')
        const terminate = `& '${python}' -m haishui_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.haishuiPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m haishui_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m haishui_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
