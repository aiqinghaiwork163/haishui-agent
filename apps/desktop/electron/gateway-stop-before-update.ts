 ...
 *
 * So we delegate to `haishui gateway stop --all`: the CLI discovers every
 ...
 * force-kills survivors — the same logic `haishui update`'s
...
export function stopGatewayBeforeUpdate(
  haishuiCliPath: string,
  haishuiHome: string,
  deps: StopGatewayBeforeUpdateDeps = {}
): boolean {
  return runGatewayLifecycleCommand(haishuiCliPath, ['gateway', 'stop', '--all'], deps)
}
 ...
 */
export function startGatewaysAfterUpdateAbort(haishuiCliPath: string, deps: StopGatewayBeforeUpdateDeps = {}): boolean {
  return runGatewayLifecycleCommand(haishuiCliPath, ['gateway', 'start', '--all'], deps)
}

function runGatewayLifecycleCommand(haishuiCliPath: string, args: string[], deps: StopGatewayBeforeUpdateDeps): boolean {
  ...
  if (deps.spy) {
    deps.spy(haishuiCliPath, args)
  }

  if (!existsSync(haishuiCliPath)) {
    return false
  }

  try {
    exec(haishuiCliPath, args, {
