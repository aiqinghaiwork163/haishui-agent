...
import { buildDesktopBackendEnv, haishuiManagedNodePathEntries, normalizeHaishuiHomeRoot } from './backend-env'
import { createBackendExitRecoveryLatch } from './backend-exit-recovery'
import { isReauthRequiredError, waitForHaishuiReady } from './backend-health'
import { backendCommandMatches, createBackendOwnership, createBackendShutdownCoordinator } from './backend-ownership'
import { canImportHaishuiCli, PROBE_TIMEOUT_MS, shouldTrustHaishuiOverride, verifyHaishuiCli } from './backend-probes'
...
import { isHaishuiOwnedVenvDaemon } from './venv-holder-select'
...
import {
  ...
  resolveVenvHaishuiCommand
} from './windows-haishui-path'
import {
...
const USER_DATA_OVERRIDE = process.env.HAISHUI_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.HAISHUI_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.HAISHUI_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with HAISHUI_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[haishui] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[haishui] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. HAISHUI_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[haishui] ${why}`)
  }
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[haishui] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// it). The value arrives via HAISHUI_DESKTOP_PASSWORD_STORE, bridged by the
// `haishui desktop` launcher from detection or `desktop.password_store` in
...
if (PASSWORD_STORE.warning) {
  console.warn(`[haishui] ${PASSWORD_STORE.warning}`)
}

if (PASSWORD_STORE.store) {
  app.commandLine.appendSwitch('password-store', PASSWORD_STORE.store)
  console.log(`[haishui] using password-store backend: ${PASSWORD_STORE.store}`)
}
...
// never go through `haishui desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose Haishui sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[haishui] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[haishui] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[haishui] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[haishui] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[haishui] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('haishui:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
...
// always-on timers. A completely idle, minimized Haishui burned ~20% CPU
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[haishui] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[haishui] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[haishui] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[haishui] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// HAISHUI_HOME — the user-facing root for everything Haishui-related. Mirrors
// scripts/install.ps1's $HaishuiHome and scripts/install.sh's $HAISHUI_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\haishui (matches install.ps1)
//   macOS / Linux: ~/.haishui (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.haishui directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\haishui yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// HAISHUI_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// HAISHUI_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.haishui / %LOCALAPPDATA%\haishui.
function resolveHaishuiHome() {
  if (process.env.HAISHUI_HOME) {
    return normalizeHaishuiHomeRoot(process.env.HAISHUI_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'haishui-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a HAISHUI_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\haishui and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('HAISHUI_HOME')

    if (fromRegistry) {
      return normalizeHaishuiHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'haishui')
    const legacy = path.join(app.getPath('home'), '.haishui')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.haishui setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.haishui')
}

const HAISHUI_HOME = resolveHaishuiHome()

function pathWithHaishuiManagedNode(...entries) {
  const managed = haishuiManagedNodePathEntries(HAISHUI_HOME).filter(directoryExists)

  return [...managed, ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_HAISHUI_ROOT — the canonical mutable Haishui install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_HAISHUI_ROOT = path.join(HAISHUI_HOME, 'haishui-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_HAISHUI_ROOT, 'venv')
...
// boots, going straight to `resolveHaishuiBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_HAISHUI_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_HAISHUI_ROOT, '.haishui-bootstrap-complete')
...
// active-profile.json records which Haishui profile the desktop launches its
// local backend as. When set, startHaishui() passes `haishui --profile <name>
// dashboard …`, which deterministically pins HAISHUI_HOME (see
// _apply_profile_override in haishui_cli/main.py) and bypasses the sticky
// ~/.haishui/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors haishui_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = DESKTOP_PROFILE_NAME_RE
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// haishuiDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under HAISHUI_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by haishui_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(HAISHUI_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (haishui_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.HAISHUI_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.HAISHUI_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.HAISHUI_DESKTOP_SKIP_QUIT_CONFIRM === '1'
// Nous free tier gate, decided ONCE here and stamped onto every backend spawn
// (desktopBackendSpawnEnv) and the renderer (haishui:launch-flags).
const GUEST_ONBOARDING = guestOnboardingEnabled()
const SKIP_INTRO = skipIntroEnabled()

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.HAISHUI_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.HAISHUI_DESKTOP_APP_NAME || 'Haishui'
...
// 'haishui:native-theme' ('dark' | 'light' | 'system'); we pin
...
// haishui:window-control IPC channel. See titleBarOverlayOptions.
...
// electron-builder derives from the build `appId` (com.nousresearch.haishui) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nousresearch.haishui')
}

// Seed the native About panel with the live Haishui version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `haishui update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveHaishuiVersion(),
...
// counts startHaishui() calls that have not settled; `primaryRecoverySuppressed`
// is set by every intentional invalidate of the slot and cleared by the next
// startHaishui(), so the dying child's stale exit never respawns behind a
...
// startHaishui(); this pool only holds EXTRA profile
...
// The legacy HAISHUI_DESKTOP_POOL_* env vars remain the initial-value fallback
...
function readPersistedPoolLimits() {
  try {
    ...
    const fromEnv = clampPoolLimits({
      maxBackends: Number(process.env.HAISHUI_DESKTOP_POOL_MAX) || undefined,
      idleMs: Number(process.env.HAISHUI_DESKTOP_POOL_IDLE_MS) || undefined
    })
...
function logPoolSpawnFailure(label: string, error: unknown): void {
  ...
  if (isBackgroundSlotWaitTimeout(error)) {
    ...
    rememberLog(
      `Haishui backend for profile ${label} failed to start: ${error instanceof Error ? error.message : String(error)}`
    )
  }
}
...
//   * WSL2 IPC stall  = the renderer's `haishui:backend:touch` roundtrips
...
const POOL_KEEPALIVE_FRESH_MS = Math.max(
  120_000,
  Number(process.env.HAISHUI_DESKTOP_POOL_KEEPALIVE_FRESH_MS) || 4 * 60_000
)
...
// onto the error so subsequent startHaishui() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning haishui serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Haishui backend',
  phase: 'idle',
  progress: 0,
  retryable: false,
  running: false,
...
function rememberLog(chunk) {
  ...
  haishuiLog.push(...lines)

  if (haishuiLog.length > 300) {
    haishuiLog.splice(0, haishuiLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-haishui-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('haishui:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the haishui:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('haishui:bootstrap:event', ev)
}
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_HAISHUI_ROOT
  })
}
...
// The Tauri updater writes HAISHUI_HOME/.haishui-update-in-progress for the whole
...
// (`force_kill_other_haishui`, taskkill /IM haishui.exe) kills it, the launch
...
function updateGateDeps() {
  return {
    hasLiveMarker: () => Boolean(readLiveUpdateMarker(HAISHUI_HOME)),
...
const BUNDLE_SWAP_RELAUNCH_FLAG = '--haishui-bundle-swap-relaunched'
...
// The detached updater swaps the packaged bundle on disk AFTER `haishui update`
...
async function waitForUpdateToFinish() {
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    ...
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — Haishui will start automatically when it completes…',
        12
      )
    },
  ...
  // "nothing happened").
  try {
    const result = readAndConsumeHandoffResult(HAISHUI_HOME)
    ...
    if (result && result.ok && result.manual) {
      ...
      const handoffLogPath = path.join(HAISHUI_HOME, 'logs', 'desktop-update-handoff.log')
      ...
      // and the same reveal primitive as 'haishui:logs:reveal'.
      void dialog
        .showMessageBox({
          type: 'error',
          title: 'Haishui update',
          message: "Haishui couldn't finish updating",
          detail:
  ...
  if (outcome === 'timeout') {
    ...
    await advanceBootProgress('backend.update-restart', 'Restarting Haishui to load the updated app…', 14)
...
function findOnPath(command) {
  ...
  // shell-script shim named `haishui` — must not shadow `haishui.cmd`/`haishui.exe`.
...
async function unwrapWindowsVenvHaishuiCommand(command, backendArgs) {
  return resolveVenvHaishuiCommand(command, backendArgs, {
    isWindows: IS_WINDOWS,
    isCommandScript,
    fileExists,
    directoryExists,
...
// spawns `haishui serve`; runtimes older than serve only have `dashboard`. We
...
const backendSupportsServe = createBackendServeSupportResolver(HAISHUI_HOME, rememberLog)
...
function isHaishuiSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'haishui_cli', 'main.py'))
}

async function findPythonForRoot(root) {
  const override = process.env.HAISHUI_DESKTOP_PYTHON
...
async function findSystemPython() {
  ...
  // CPython but several Haishui deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Resolves HAISHUI_GIT_BASH_PATH
...
// normal HAISHUI_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\haishui\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'haishui', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'haishui', 'git', 'bin', 'git.exe'))
  }
...
function recentHaishuiLog() {
  return haishuiLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's haishui root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_HAISHUI_ROOT.
// HAISHUI_DESKTOP_HAISHUI_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.HAISHUI_DESKTOP_HAISHUI_ROOT && path.resolve(process.env.HAISHUI_DESKTOP_HAISHUI_ROOT),
    !IS_PACKAGED && isHaishuiSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isHaishuiSourceRoot(ACTIVE_HAISHUI_ROOT) ? ACTIVE_HAISHUI_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_HAISHUI_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('haishui:updates:progress', merged)
  }
}
...
async function checkUpdates({ force = false }: { force?: boolean } = {}) {
  ...
  if (!directoryExists(gitDir)) {
    return {
      ...
      message:
        "This copy of Haishui can't update itself from inside the app. Download the latest version from the Haishui website, " +
        `or reinstall Haishui to enable in-app updates. Details: ${updateRoot} has no version-control metadata.`,
      haishuiRoot: updateRoot,
      branch
    }
  }
        ...
        headers: githubApiHeaders(
          {
            ...
            'User-Agent': 'haishui-desktop-update-check'
          },
          token
        ),
        timeout: 10_000
...
// Windows that binary owns ALL repo mutation — running `haishui update` +
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(HAISHUI_HOME, { fileExists, isWindows: IS_WINDOWS })
}
...
// Path to the venv shim whose lock decides whether `haishui update` can write
// fresh entry points. On Windows this is the file the running backend
// `haishui.exe` holds open; on POSIX it's never mandatory-locked.
function venvHaishuiShimPath(updateRoot) {
  const venvDir = resolveVenvDir(updateRoot)

  return IS_WINDOWS
    ? path.join(venvDir, 'Scripts', 'haishui.exe')
    : path.join(venvDir, 'bin', 'haishui')
}
...
// Kill only Haishui-OWNED venv daemons (the memory plugin's hindsight daemon:
...
// venv files mapped. External holders (a user terminal running `haishui`,
...
function killHaishuiOwnedVenvDaemons(updateRoot) {
  if (!IS_WINDOWS) {
    return
  }
  ...
  try {
    ...
    holders = (Array.isArray(parsed) ? parsed : [parsed]).filter(p =>
      isHaishuiOwnedVenvDaemon(p?.ExecutablePath, p?.CommandLine, scriptsDir)
    )
  } catch {
  ...
  for (const holder of holders) {
    ...
    if (Number.isInteger(pid) && pid > 0) {
      rememberLog(`[updates] stopping Haishui-owned venv daemon (hindsight) PID ${pid} before hand-off`)
...
// only signals the direct child, so on Windows a backend `haishui.exe` that
// spawned its own grandchildren (a `haishui` REPL, a pty terminal session, the
...
async function claimBackendChild(child, command, profile, nonce, outputTail: BackendOutputTail | null = null) {
  ...
  if (decision.action === 'fail') {
    ...
    throw new Error(
      `Haishui backend (PID ${child.pid}) died before its identity could be recorded: ${decision.reason}${outputTail?.describe() ?? ''}`
    )
  }
  ...
  if (decision.action === 'degrade') {
    ...
    rememberLog(
      `WARNING: process start marker probe failed for live Haishui backend PID ${child.pid}; ` +
  ...
  try {
    ...
    child.haishuiBackendIdentity = identity
    ...
    await waitForBackendExit(child)
    throw new Error(
      `Could not persist ownership for the Haishui backend: ${error.message}${outputTail?.describe() ?? ''}`
    )
  }
}

function releaseBackendChild(child) {
  const identity = child?.haishuiBackendIdentity
...
// `haishuiProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `haishui.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a haishui
// REPL, a pty terminal, the gateway) keep `haishui.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const haishuiProcess = backendConnectionState.getProcess()
  ...
  // unlock — the shim probe only covers venv\Scripts\haishui.exe, but the
  // backend is `python.exe -m haishui_cli.main serve`, which need not hold
  // the shim at all (#74805 first-attempt race).
  const initialPids = []

  if (haishuiProcess && Number.isInteger(haishuiProcess.pid)) {
    initialPids.push(haishuiProcess.pid)
  }
  ...
  stopBackendTreesForUpdate(haishuiProcess, {
  ...
  // `haishui gateway stop --all`: the CLI discovers every profile's gateway
  ...
  stopGatewayBeforeUpdate(venvHaishuiShimPath(updateRoot), HAISHUI_HOME)

  // Reap Haishui-OWNED venv daemons the tree-kill above cannot reach: the
  ...
  killHaishuiOwnedVenvDaemons(updateRoot)

  const shim = venvHaishuiShimPath(updateRoot)
  ...
  const gate = await waitForBackendRelease(
    initialPids,
    {
      ...
      collectStragglerPids: () => {
        ...
        const currentHaishuiProcess = backendConnectionState.getProcess()

        if (currentHaishuiProcess && Number.isInteger(currentHaishuiProcess.pid)) {
          stragglers.push(currentHaishuiProcess.pid)
        }
...
// `haishui update`). Instead we spawn the staged Haishui-Setup binary with
// --update and quit, so it can run `haishui update` (which refuses while we
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      ...
      // Windows (quit → detached orchestrator → `haishui update` → relaunch),
      ...
      // in: the HAISHUI_DESKTOP_CHILD_PID reaper-exclusion dance (#37532),
      ...
      // get the manual `haishui update` card once; their next update pulls it.
      return await applyUpdatesPosixHandoff(opts)
    }

    if (!updater) {
      // No staged updater binary — this is a CLI-installed user (they ran
      // `haishui desktop`, never the Tauri installer that self-copies
      // haishui-setup.exe into HAISHUI_HOME). On Windows the repo hand-off
      ...
      if (!resolveUpdateScriptHandoff(updateRoot)) {
        // They DO have a working `haishui` on PATH / in the venv, so the
        ...
        try {
          ...
          if (head.code === 0 && current && current !== 'HEAD') {
            ...
            if (branch !== 'main') {
              command = `haishui update --branch ${branch}`
            }
          }
        } catch {
          // Best-effort: fall back to bare `haishui update` if branch detection fails.
        }

        rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
        emitUpdateProgress({ stage: 'manual', message: command, percent: null })

        return { ok: true, manual: true, command, haishuiRoot: updateRoot }
      }

      rememberLog('[updates] no staged updater; using repo hand-off script for CLI install')
    }

    const handoffConflict = updateHandoffConflict(HAISHUI_HOME)
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Haishui — this window will close and the updater will open. Don’t reopen Haishui yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    preflightStateDb(HAISHUI_HOME, rememberLog)
    ...
    // haishui.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running haishui, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Haishui install open ' +
        '(a second Haishui window or a terminal running haishui?). Close it and retry.'
      ...
      if (IS_WINDOWS) {
        ...
        startGatewaysAfterUpdateAbort(venvHaishuiShimPath(updateRoot))
      }
    ...
    // Haishui processes running from this venv.  The updater normally refuses
    ...
    // `haishui update` will run (the venv shim is locked while we live).
    //
    ...
    // checkout, so each `haishui update` refreshes the code that drives the
    ...
    if (scriptHandoff) {
      ...
      child = spawnUpdaterProcess(wrapped.command, wrapped.args, {
        ...
        env: {
          ...process.env,
          HAISHUI_HOME,
          HAISHUI_UPDATE_STARTED_AT: String(updateStartedAt),
          PATH: pathWithHaishuiManagedNode(venvBin)
        },
        detached: true,
        stdio: 'ignore'
      })
      ...
      // The `haishui update` child adopts the SCRIPT's claim via
      // update_lock.py's process-ancestry rule; no mtime heuristics needed.
      if (Number.isInteger(child.pid)) {
        writeUpdateMarker(HAISHUI_HOME, child.pid, { startedAt: updateStartedAt })
      }
      ...
      // abort with "Another Haishui update is already running (PID <itself>)" —
      ...
      if (Number.isInteger(child.pid) && stagedUpdaterSupportsPrewrittenMarker(updater)) {
        writeUpdateMarker(HAISHUI_HOME, child.pid)
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(resolveVenvDir(updateRoot), IS_WINDOWS ? 'Scripts' : 'bin')
  const venvHaishui = path.join(venvBin, IS_WINDOWS ? 'haishui.exe' : 'haishui')
  const venvPython = path.join(venvBin, IS_WINDOWS ? 'python.exe' : 'python')

  // The updater invokes the venv's Haishui launcher, which in turn requires the
  ...
  // marker-only install through --update dead-ends at "Could not find the haishui
  ...
  const updaterArgs = chooseUpdaterArgs(
    {
      hasBootstrapMarker: fileExists(path.join(updateRoot, '.haishui-bootstrap-complete')),
      hasVenvHaishui: fileExists(venvHaishui),
...
// Python-level pre-update snapshot inside `haishui update`.
function preflightStateDb(haishuiHome, rememberLog) {
  const stateDbPath = path.join(haishuiHome, 'state.db')
  ...
  try {
    ...
    if (stat.size > 100) {
      ...
      const emergencyPath = path.join(haishuiHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(haishuiHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(haishuiHome, old))
            } catch {
              void 0
            }
          }
...
// out, runs `haishui update`, swaps/relaunches the app bundle, and writes
// .haishui-update-result.json for the relaunched Desktop to surface. It shows
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  if (!handoff) {
    emitUpdateProgress({ stage: 'manual', message: 'haishui update', percent: null })

    return { ok: true, manual: true, command: 'haishui update', haishuiRoot: updateRoot }
  }
  ...
  const child = spawnUpdaterProcess(handoff.command, args, {
    ...
    env: {
      ...
      HAISHUI_HOME,
      HAISHUI_UPDATE_STARTED_AT: String(updateStartedAt),
      PATH: pathWithHaishuiManagedNode(path.join(resolveVenvDir(updateRoot), 'bin'))
    },
    detached: true,
    stdio: 'ignore'
  })
  ...
  emitUpdateProgress({
    stage: 'restart',
    message:
      'Updating Haishui — this window will close. Don’t reopen Haishui yourself; it restarts automatically when the update finishes.',
    percent: 100
  })
...
// Marker-independent: is the canonical install at ACTIVE_HAISHUI_ROOT actually
...
async function isActiveRuntimeUsable() {
  ...
  return (
    isHaishuiSourceRoot(ACTIVE_HAISHUI_ROOT) &&
    ...
    (await canImportHaishuiCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_HAISHUI_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    }))
  )
}
...
async function activeRuntimeState() {
  ...
  // pinned commit -- users update via the in-app update path or `haishui
...
function resolveWebDist() {
  const override = process.env.HAISHUI_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set HAISHUI_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndexWithMissing(): { index: string; missing: string[] } {
  ...
  if (present.length > 0) {
    ...
    rememberLog(
      `[renderer] every renderer bundle is incomplete (${present.join(', ')}). ` +
        ...
        `Repair with: haishui desktop --force-build`
    )
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: haishui desktop --force-build`
  )
...
function resolveHaishuiCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Haishui.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.HAISHUI_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveHaishuiCwd(), sanitized: Boolean(trimmed) }
  }
...
async function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'haishui_cli.main', ...backendArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_HAISHUI_ROOT, the
...
async function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Haishui at ${ACTIVE_HAISHUI_ROOT}`,
    command,
    args: ['-m', 'haishui_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      haishuiHome: HAISHUI_HOME,
      pythonPathEntries: [ACTIVE_HAISHUI_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_HAISHUI_ROOT,
    bootstrap: true,
    shell: false
  }
}

async function resolveHaishuiBackend(backendArgs) {
  // 1. Explicit override -- HAISHUI_DESKTOP_HAISHUI_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.HAISHUI_DESKTOP_HAISHUI_ROOT && path.resolve(process.env.HAISHUI_DESKTOP_HAISHUI_ROOT)

  if (overrideRoot && isHaishuiSourceRoot(overrideRoot)) {
    const backend = await createPythonBackend(overrideRoot, `Haishui source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `haishui` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isHaishuiSourceRoot.)
  if (!IS_PACKAGED && isHaishuiSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = await createPythonBackend(SOURCE_REPO_ROOT, `Haishui source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. ACTIVE_HAISHUI_ROOT — the canonical install at
  //    %LOCALAPPDATA%\\haishui\\haishui-agent (Windows) or ~/.haishui/haishui-agent.
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active Haishui runtime at ${ACTIVE_HAISHUI_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  // 4. Existing `haishui` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    HAISHUI_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.HAISHUI_DESKTOP_IGNORE_EXISTING !== '1') {
    let haishuiCommand = null
    const haishuiOverride = process.env.HAISHUI_DESKTOP_HAISHUI

    if (haishuiOverride) {
      const resolvedOverride = findOnPath(haishuiOverride)

      if (resolvedOverride) {
        haishuiCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(haishuiOverride, { isWsl: IS_WSL })) {
        haishuiCommand = haishuiOverride
      } else {
        rememberLog(`Ignoring Windows Haishui override under WSL: ${haishuiOverride}`)
      }
    } else {
      haishuiCommand = findOnPath('haishui')
    }

    if (haishuiCommand) {
      if (looksLikeDesktopAppBinary(haishuiCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Haishui CLI: ${haishuiCommand}`)
        haishuiCommand = null
      }
    }

    if (haishuiCommand) {
      const unwrapped = await unwrapWindowsVenvHaishuiCommand(haishuiCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `haishui` shim
      ...
      const shellForProbe = isCommandScript(haishuiCommand)

      // HAISHUI_DESKTOP_HAISHUI is an explicit deployment override (used by
      ...
      if (
        shouldTrustHaishuiOverride(haishuiOverride) ||
        (await verifyHaishuiCli(haishuiCommand, { shell: shellForProbe }))
      ) {
        ...
        // unwrapWindowsVenvHaishuiCommand here: the second call repeats the
        // same un-memoized import probe, costing up to another full probe
        // timeout on the boot path for an answer we already have.
        return {
          label: `existing Haishui CLI at ${haishuiCommand}`,
          command: haishuiCommand,
          args: backendArgs,
          bootstrap: false,
          env: {},
          kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Haishui CLI at ${haishuiCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed haishui_cli module via system Python.
  ...
  if (python) {
    ...
    // having haishui_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\haishui\haishui-agent\venv.
    if (await canImportHaishuiCli(python)) {
      return {
        kind: 'python',
        label: `installed haishui_cli module via ${python}`,
        command: python,
        args: ['-m', 'haishui_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: haishui_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveHaishuiBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Haishui Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
    ...
    activeRoot: ACTIVE_HAISHUI_ROOT,
...
async function runEnsureRuntime(backend: any, assertStillOwned: () => void): Promise<any> {
  ...
  // backend.kind === 'bootstrap-needed' means resolveHaishuiBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Haishui install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Haishui recovery was handed off to Haishui Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      haishuiHome: HAISHUI_HOME,
      logRoot: path.join(HAISHUI_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Haishui install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      ...
      // Latch the failure so subsequent startHaishui() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // haishui:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(await resolveHaishuiBackend(backend.args), assertStillOwned)
  }
  ...
  if (!isHaishuiSourceRoot(ACTIVE_HAISHUI_ROOT)) {
    throw new Error(missingInstallPartMessage(`Haishui source files are missing or incomplete at ${ACTIVE_HAISHUI_ROOT}`))
  }

  // On Windows, preflight Git Bash. Haishui' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\haishui\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `haishui` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      "Haishui needs a helper called Git for Windows, which isn't installed. " +
        'Choose Repair install to add it automatically, or install it yourself from git-scm.com and reopen Haishui.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isHaishuiSourceRoot()
    // plus an importable haishui_cli before it hands back the active runtime.
  ...
  backend.label = `Haishui at ${ACTIVE_HAISHUI_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Haishui runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----haishui-${crypto.randomBytes(12).toString('hex')}`
    ...
    (requestState: any) =>
      new Promise((resolve, reject) => {
        ...
        if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
          reject(new Error(`Unsupported Haishui backend URL protocol: ${parsed.protocol}`))
        ...
        req.setTimeout(timeoutMs, () => {
          req.destroy(new Error(`Timed out connecting to Haishui backend after ${timeoutMs}ms`))
        })
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = client.request(
      parsed,
      {
        agent,
        method: 'GET',
        headers: options.bearer ? { Authorization: `Bearer ${options.bearer}` } : { 'X-Haishui-Session-Token': token }
      },
      res => {
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Haishui-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('haishui:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveHaishuiCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('haishui:preview-file-changed', payload)
}
...
// waitForHaishuiReady how to read a 401 — rejected session vs gated route.
...
async function waitForHaishui(baseUrl, token, signal?, authMode?, headers = {}) {
  const { probeHealth, probeIsCredentialed } = await buildReadinessHealthProbe(baseUrl, authMode, token)

  return waitForHaishuiReady(baseUrl, {
    token,
    signal,
    fetchPublicJson,
...
function sendBackendExit(payload) {
  ...
  webContents.send('haishui:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('haishui:close-preview-requested')
}
...
function sendPreviewNavCommand(command: 'back' | 'forward' | 'reload') {
  ...
  webContents.send('haishui:preview-nav', command)
}
...
function sendOpenFolderRequested() {
  ...
  webContents.send('haishui:open-folder-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Haishui...".
function sendPowerResume() {
  ...
  webContents.send('haishui:power-resume')
}
...
// 'haishui:power-battery' push below.
ipcMain.handle('haishui:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('haishui:power-battery', next)
    }
  }
}
...
async function showPluginCompatNoticeOnce() {
  ...
  try {
    notice = pendingPluginCompatNotice(HAISHUI_HOME, app.getPath('userData'))
  } catch (err) {
  ...
  try {
    ...
    // 'Open Plugins' rides the existing deep-link channel (haishui://open/…),
    ...
    if (response === 0) {
      handleDeepLink(`${HAISHUI_PROTOCOL}://open/skills?tab=plugins`)
    }
  } finally {
    try {
...
function sendOpenUpdatesRequested() {
  ...
  // before the window is up) queue the request; 'haishui:deep-link-ready' flushes it.
  ...
  webContents.send('haishui:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('haishui:window-state-changed', state)
}
...
function installContextMenuBridge(window: BrowserWindow) {
  window.webContents.on('context-menu', (_event, params) => {
    ...
    if (params.isEditable && params.misspelledWord) {
      window.webContents.send('haishui:context-menu-spellcheck', {
...
//
// Hosted Haishui gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``haishui_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / haishui #37247).
//     Both are set as HttpOnly cookies (``haishui_session_at`` ~15 min,
//     ``haishui_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
// "not signed in", which on the initial boot path (startHaishui → the renderer's
// single-shot boot() with no retry) surfaced as the "Haishui couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Haishui
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Haishui Cloud agent…' : 'Sign in to Haishui gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Haishui backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in haishui_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// --- Haishui Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// portal session cookie living in `persist:haishui-remote-oauth`:
...
// convention (haishui_cli/auth.py DEFAULT_NOUS_PORTAL_URL + the same env names)
// so a single override flips every Haishui surface to the same portal.
const DEFAULT_NOUS_PORTAL_URL = 'https://portal.nousresearch.com'

function resolvePortalBaseUrl() {
  const raw = process.env.HAISHUI_PORTAL_BASE_URL || process.env.NOUS_PORTAL_BASE_URL || DEFAULT_NOUS_PORTAL_URL
...
// Discover the hosted (Haishui Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Haishui Cloud. Open Settings → Gateway, choose Haishui Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (body === undefined) {
      ...
      if (error && error.statusCode === 401) {
        const err = new Error(
          'Your Haishui Cloud session has expired. Open Settings → Gateway and sign in again.'
        ) as any
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Haishui Cloud session has expired. Sign in to Haishui Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Haishui Cloud org tag on cloud-mode entries so Settings can
...
function readDesktopConnectionsRegistry() {
  ...
  if (mtime === null) {
    ...
    try {
      ...
      // hard-failing every haishui:connections:* call.
 ...
 * editor doesn't carry — cloud `org`, ssh `remoteHaishuiPath`/`remoteProfile` —
  ...
  onDefaultChanged: route => {
    for (const win of BrowserWindow.getAllWindows()) {
      if (!win.webContents.isDestroyed()) {
        win.webContents.send('haishui:profile:default:changed', route)
      }
    }
  }
})
...
// contains "haishui", avoiding false positives from stale gateway.pid files
// whose PID was recycled by the OS to an unrelated process.
function isHaishuiProcess(pid) {
  try {
  ...
  try {
    ...
    return cmdline.includes('haishui')
  } catch {
...
//   1. Legacy ~/.haishui/active_profile (explicit CLI choice via haishui profile use)
//   2. Running gateway (gateway.pid with verified liveness + haishui identity)
...
function migrateActiveProfileIfMissing() {
  migrateActiveProfileIfMissingPure(DESKTOP_PROFILE_CONFIG_PATH, {
    legacyActivePath: path.join(HAISHUI_HOME, 'active_profile'),
    haishuiHome: HAISHUI_HOME,
    profilesRoot: path.join(HAISHUI_HOME, 'profiles'),
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.HAISHUI_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.HAISHUI_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    remoteUrl,
    // The persisted Haishui Cloud org (slug/id) for a cloud connection, or '' for
    ...
    sshRemoteHaishuiPath: (ssh || savedSsh)?.remoteHaishuiPath || '',
    sshRemoteProfile: (ssh || savedSsh)?.remoteProfile || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by HAISHUI_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the Haishui Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Haishui Cloud instance, NOT a user-owned remote gateway,
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteHaishuiPath: input.sshRemoteHaishuiPath ?? existingBlock.remoteHaishuiPath,
...
async function buildRemoteConnection(
  ...
  if (!token) {
    throw new Error(
      'Remote Haishui gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget(webContentsId?: number) {
  ...
  const route = resolveDesktopRemoteRoute({
    config,
    env: {
      token: process.env.HAISHUI_DESKTOP_REMOTE_TOKEN,
      url: process.env.HAISHUI_DESKTOP_REMOTE_URL
    },
    profile,
    registry: readDesktopConnectionsRegistry()
  })
...
async function rollbackSshBootstrapResult(ssh, result, profile, sshConfig, boundaryError) {
  ...
  try {
    const expected = {
      ...
      haishuiPath: result.haishuiPath,
      haishuiHome: result.haishuiHome,
    ...
    if (result.platform?.os === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        ssh,
        { haishuiPath: result.haishuiPath, haishuiHome: result.haishuiHome, python: result.pythonPath },
        expected
      )
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, metadata, fingerprint, lease) {
  ...
  try {
    ...
    const platform = await detectRemotePlatform(ssh, sshConfig.remoteHaishuiPath || '')
    const lifecycle = platform.os === 'Windows' ? connectWindowsRemote : remoteLifecycle.connect
    result = await lifecycle({
      ssh,
      platform,
      profile: resolveRemoteSshDashboardProfile(sshConfig.remoteProfile, profile),
      remoteHaishuiPath: sshConfig.remoteHaishuiPath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForHaishui: (baseUrl, token) => waitForHaishui(baseUrl, token, lease.signal, 'token'),
      ...
      // spawn command adds HAISHUI_GUEST_ONBOARDING=1 only when this is on.
  ...
  await fenceManagedSshBootstrapPublication({
    ...
    publish: () => {
      ...
      sshConnections.set(scope, {
        ...
        host: sshConfig.host,
        hostLabel,
        haishuiVersion: result.haishuiVersion || '',
        ...
        // per-profile pools also use bare keys. Only startHaishui' explicit call
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.haishuiVersion || 'haishui (version unknown)'} at ${result.haishuiPath || '?'}`
  )
  ...
  return {
    ...connection,
    remoteHaishuiVersion: result.haishuiVersion || '',
    ssh: {
      effectiveConfigFingerprint: sshConfig.effectiveConfigFingerprint,
      host: sshConfig.host,
...
//   2. env override (HAISHUI_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
function globalRemoteActive() {
  if (process.env.HAISHUI_DESKTOP_REMOTE_URL) {
    return true
  }
...
// env → global) so a startHaishui() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Haishui
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteHaishuiPath: input.sshRemoteHaishuiPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteHaishuiPath || '')
          let haishuiPath
          let haishuiVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            haishuiPath = runtime.haishuiPath
            const inspection = await helper(ssh, runtime, 'inspect', [runtime.haishuiPath])
            haishuiVersion = inspection.version
            supported = inspection.supported
          } else {
            haishuiPath = await remoteLifecycle.locateHaishui(ssh, sshConfig.remoteHaishuiPath || '')
            haishuiVersion = await remoteLifecycle.probeHaishuiVersion(ssh, haishuiPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, haishuiPath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Haishui on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteHaishuiPath: haishuiPath,
            remoteHaishuiVersion: haishuiVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startHaishui())
  ...
  // connect to Haishui gateway". Mirror the renderer's connect here so the test
...
function resetHaishuiConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  // The next startHaishui() re-reads active-profile.json for its launch profile.
  primaryProfilePin.clear()
  const haishuiProcess = invalidatePrimaryConnection()
  stopBackendChild(haishuiProcess)
...
// startHaishui() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetHaishuiConnection() invalidates it.
  const haishuiProcess = backendConnectionState.getProcess()
  const dying = haishuiProcess && !haishuiProcess.killed ? haishuiProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('haishui:connection:applied')
}
...
function broadcastConnectionsChanged(payload: { connectionId: string; reason: 'removed' | 'saved' | 'updated' }) {
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('haishui:connections:changed', payload)
    }
  }
}
...
// startHaishui() and cleared when the primary is torn down; while a primary is
...
async function ensureBackend(profile, opts: { passive?: boolean; spawnPriority?: LocalBackendSpawnPriority } = {}) {
  ...
  if (route.backend === 'primary') {
    const connection = await startHaishui()
  ...
  // spawning, so two children never share one profile's HAISHUI_HOME.
  ...
  const resolveRegistrySshConfig = () => {
    ...
    if (!resolvedRegistrySshConfig) {
      resolvedRegistrySshConfig = normalizeSshConfig({
        ...
        remoteHaishuiPath: source.remoteHaishuiPath,
...
async function connectRegistryBackend(
  ...
  if (source.kind === 'ssh') {
    ...
    return {
      ...
      // is only the routing label. haishui:api uses it to translate explicit
      // self-profile query filters into the backend's namespace.
      remoteProfile: sshConfig.remoteProfile || '',
      logs: haishuiLog.slice(-80),
      ...getWindowState()
    }
  }
  ...
  await waitForHaishui(connection.baseUrl, connection.token, undefined, connection.authMode, connection.headers)
  poolEntry.remoteBaseUrl = connection.baseUrl
...
function remoteUpdateTargetFromState(state): RemoteUpdateTarget {
  if (!state?.ssh || !state?.haishuiPath || !state?.haishuiHome) {
  ...
  return {
    ...
    haishuiPath: state.haishuiPath,
    haishuiHome: state.haishuiHome,
...
async function openManagedSshUpdateTransport(
  ...
  try {
    const platform: any = await detectRemotePlatform(ssh, config.remoteHaishuiPath || '')

    if (platform.os === 'Windows') {
      const runtime = platform.haishuiPath ? platform : await probeWindowsRemote(ssh, config.remoteHaishuiPath || '')
      ...
      return {
        ...
        target: {
          ...
          platform: 'Windows',
          haishuiPath: runtime.haishuiPath,
          haishuiHome: runtime.haishuiHome,
          pythonPath: runtime.python
        }
      }
    }

    const haishuiPath = await remoteLifecycle.locateHaishui(ssh, config.remoteHaishuiPath || '')
    const haishuiHome = await remoteLifecycle.probeRemoteHaishuiHome(ssh)

    return {
      close: () => ssh.close(),
      target: { ssh, platform: platform.os, haishuiPath, haishuiHome }
    }
  } catch (error) {
    await ssh.close()
    throw error
...
async function drainManagedSshScope(scope) {
  ...
  try {
    ...
    if (state.remotePlatform === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        state.ssh,
        { haishuiPath: state.haishuiPath, haishuiHome: state.haishuiHome, python: state.pythonPath },
        expected
      )
    ...
    if (state && state.localPort && state.remotePort && scope.reuseToken) {
      try {
        ...
        if (!forwardClosed) {
          await waitForHaishui(`http://127.0.0.1:${state.localPort}`, scope.reuseToken, undefined, 'token')
        } else {
...
// local-spawn portion of startHaishui() but without the boot-progress UI,
...
async function runPoolBackendStart(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  if (remote) {
    await waitForHaishui(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
  ...
  // --profile wins over the inherited HAISHUI_HOME env (see _apply_profile_override
  // step 3 in haishui_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']

  const backend = await ensureRuntime(await resolveHaishuiBackend(backendArgs), () =>
    assertPoolEntryStillOwned(poolKey, entry)
  )
  ...
  assertPoolEntryStillOwned(poolKey, entry)
  const haishuiCwd = resolveHaishuiCwd()
  ...
  assertLocalProfileCanStart(profile, profileDeletionGate, key =>
    directoryExists(path.join(HAISHUI_HOME, 'profiles', key))
  )
  rememberLog(`Starting Haishui backend for profile "${profile}" via ${backend.label}`)
      ...
      env: desktopBackendSpawnEnv(
        {
          ...
          TERMINAL_CWD: haishuiCwd,
          HAISHUI_DASHBOARD_SESSION_TOKEN: token,
          ...
          ...parentIdentityEnv,
          HAISHUI_WEB_DIST: webDist,
          ...(readyFile ? { HAISHUI_DESKTOP_READY_FILE: readyFile } : {})
        },
        GUEST_ONBOARDING
      ),
      shell: backend.shell,
  ...
  child.once('error', error => {
    rememberLog(`Haishui backend for profile "${profile}" failed to start: ${error.message}`)
    void teardownFailedLocalBackend(poolKey, entry).catch(cleanupError => {
      rememberLog(
        `Haishui backend for profile "${profile}" cleanup failed: ${cleanupError instanceof Error ? cleanupError.message : String(cleanupError)}`
      )
    })
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(formatBackendExitLine(`Haishui backend for profile "${profile}" exited`, code, signal, outputTail))
    releaseBackendChild(child)

    if (!ready) {
      rejectStart?.(
        new Error(
          `Haishui backend for profile "${profile}" exited before it became ready (${signal || code}).${outputTail.describe()}`
        )
      )
    }
  })
  ...
  await Promise.race([waitForHaishui(baseUrl, token), startFailed])
  assertPoolEntryStillOwned(poolKey, entry, { releaseSlot: false })
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Haishui backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `Haishui backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
function broadcastPoolBackendRetiring(poolKey: string) {
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('haishui:pool:retiring', { poolKey })
    }
  }
}
...
// backend whose ensure_haishui_home() recreates the deleted profile directory.
//
...
function startHaishui() {
  primaryRecoverySuppressed = false
  primaryStartsInFlight += 1

  const start = localBackendLifecycle.start(runHaishuiStart)
...
function scheduleUnexpectedPrimaryRecovery({ code = null, signal = null, error = null, ready = false } = {}) {
  ...
  if (!claimed) {
    if (primaryExitRecovery.isCrashLooping()) {
      const message =
        'Haishui backend keeps crashing right after it restarts; not restarting it again. Relaunch Haishui Desktop.'
  ...
  startHaishui().catch(respawnError => rememberLog(`[supervisor] backend respawn failed: ${respawnError.message}`))

  return true
}

async function runHaishuiStart() {
  ...
  if (!isPrimaryInstance) {
    ...
    throw new Error('Haishui Desktop is already running in another window.')
  }
  ...
  // process, every subsequent startHaishui() call re-throws the same error
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving Haishui backend', 8)
  ...
  // later haishui:profile:remember must not retarget requests mid-life.
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      await advanceBootProgress('backend.remote', `Connecting to remote Haishui backend at ${remote.baseUrl}`, 24)
      await waitForHaishui(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Haishui backend is ready',
        progress: 94,
        running: true,
        error: null
      })

      return createPrimaryRemoteConnection(remote, haishuiLog.slice(-80), getWindowState())
    }
    ...
    // deterministic (it wins over the sticky ~/.haishui/active_profile file) and
    // resolves HAISHUI_HOME the same way `haishui -p <name>` does on the CLI. An
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving Haishui runtime', 28)

        return resolveHaishuiBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    await advanceBootProgress('backend.spawn', `Starting Haishui backend via ${backend.label}`, 84)
    rememberLog(`Starting Haishui backend via ${backend.label}`)
    ...
    const haishuiProcess = spawnOwnedBackend(
      backend.command,
      backend.args,
      hiddenWindowsChildOptions({
        cwd: haishuiCwd,
        env: desktopBackendSpawnEnv(
          {
            ...process.env,
            // Explicitly pin HAISHUI_HOME for the child so Python's get_haishui_home()
            // resolves to the SAME location our resolveHaishuiHome() picked. Without
            // this pin, Python falls back to ~/.haishui on every platform — fine on
            // mac/linux (where our default matches), but on Windows our default is
            // %LOCALAPPDATA%\haishui, which differs from C:\Users\<u>\.haishui.
            // Mismatch would split config / sessions / .env / logs across two
            // directories. install.ps1 sets HAISHUI_HOME via setx; the desktop
    ...
    primaryOutputTail.attach(haishuiProcess)
    ...
    const portAnnouncement = waitForDashboardPortAnnouncement(haishuiProcess, {
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, haishuiProcess)

    if (!processOwner) {
      stopBackendChild(haishuiProcess)
      await waitForBackendExit(haishuiProcess)
      releaseBackendChild(haishuiProcess)
      throw new Error('Haishui backend start was superseded by a newer connection attempt.')
    }

    haishuiProcess.stdout.on('data', rememberLog)
    haishuiProcess.stderr.on('data', rememberLog)
    ...
    haishuiProcess.once('error', error => {
      releaseBackendChild(haishuiProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Haishui backend error: ${error.message}`)
        scheduleUnexpectedPrimaryRecovery({ error: error.message, ready: backendReady })
        rejectBackendStart?.(new Error('Haishui backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Haishui backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Haishui backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    haishuiProcess.once('exit', (code, signal) => {
      releaseBackendChild(haishuiProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(formatBackendExitLine('Ignoring stale Haishui backend exit', code, signal, primaryOutputTail))
      ...
      rememberLog(formatBackendExitLine('Haishui backend exited', code, signal, primaryOutputTail))
      ...
      if (!backendReady) {
        const message = `Haishui backend exited before it became ready (${signal || code}).${primaryOutputTail.describe()}`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Haishui backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentHaishuiLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Haishui backend to launch', 86)
    backendConnectionState.assertCurrentAttempt(connectionAttempt)
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Haishui backend to become ready', 90)
    backendConnectionState.assertCurrentAttempt(connectionAttempt)
    await Promise.race([waitForHaishui(baseUrl, token), backendStartFailed])
    backendConnectionState.assertCurrentAttempt(connectionAttempt)
    backendReady = true
    ...
    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => haishuiProcess.exitCode === null && !haishuiProcess.killed,
      rememberLog
    })
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local Haishui backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Haishui backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
    ...
    // The backend's plugin discovery just ran and refreshed HAISHUI_HOME/.plugin-compat-report.json.
    ...
    // getConnection()/startHaishui() caller arriving during the exit wait would
...
function wireCommonWindowHandlers(win, { zoom = true }: { zoom?: boolean } = {}) {
  ...
  // links arrive via `haishui:openExternal`, not here. See window-open-policy.ts.
 ...
 * always carries the `persist:haishui-preview` partition, so the partition is
 ...
 * audited `haishui:openExternal` channel. Popup requests themselves stay
...
function installPreviewGuestPreload() {
  app.on('web-contents-created', (_event, contents) => {
    ...
    contents.on('will-attach-webview', (_attachEvent, webPreferences, params) => {
      if (params.partition !== 'persist:haishui-preview') {
        return
      }
...
function notifyBrowserPopoutClosed(tabId) {
  ...
  for (const other of BrowserWindow.getAllWindows()) {
    if (!other.isDestroyed()) {
      other.webContents.send('haishui:browser-popout:closed', tabId)
    }
  }
}
...
// here so it can leave the app's bounds and stay visible while Haishui is
...
// pushes pet state over IPC (haishui:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via haishui:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Haishui app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Haishui icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see haishui:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Haishui in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('haishui:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
// window showing only the composer and its scrollback, so Haishui can be driven
...
// left the user with NO Haishui window after the second toggle (#88513).
...
function startHudCursorFeed(win: BrowserWindow) {
  ...
  const timer = setInterval(() => {
    ...
    last = key
    win.webContents.send('haishui:hud:cursor', point)
  }, HUD_CURSOR_POLL_MS)
...
function startHudGameOverlayFeed(win: BrowserWindow) {
  ...
  const push = (state: { active: boolean; app: string }) => {
    if (!win.isDestroyed()) {
      win.webContents.send('haishui:hud:game-overlay', state)
    }
  }
  ...
  const enumerate = async () => {
    ...
    if (!reported) {
      reported = true
      console.warn(`[haishui] HUD cannot enumerate windows: ${windows.reason}`)
    }
...
function broadcastHudState(open) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    if (!win.isDestroyed()) {
      win.webContents.send('haishui:hud:changed', payload)
    }
  }
}
...
function spawnHudWindow(sessionId, profile) {
  const win = new BrowserWindow({
    ...
    // `haishui:hud:set-bounds`, which flips resizable on for the call — the
  ...
  // grab, haishui:hud:workspace-transfer temporarily makes the X11 window
...
function openHudWindow(sessionId, profile) {
  ...
  if (hudWindow && !hudWindow.isDestroyed()) {
    ...
    if (sessionId && sessionId !== hudSessionId) {
      ...
      hudWindow.webContents.send('haishui:hud:goto', sessionId)
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('haishui:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('haishui:quick-entry:shown')
}
...
function createWindow() {
    ...
    callbacks: {
      ...
      onFailedLoadBudgetExhausted: details => {
        ...
        void loadRendererLoadErrorPage(mainWindow, {
          ...
          repairHint: 'haishui desktop --force-build',
  ...
  const startup = defaultRoute ? connectDesktopProfileRoute(defaultRoute) : startHaishui()
...
ipcMain.handle('haishui:connection', async (event, profile, extra) => {
...
ipcMain.handle('haishui:connection:for', async (_event, payload) => {
...
ipcMain.on('haishui:connection:active-route', (event, route) => recordWindowConnectionRoute(event.sender, route))
...
// "Starting Haishui…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('haishui:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: () => resetHaishuiConnection({ soft: true }),
...
ipcMain.handle('haishui:backend:touch', async (_event, profile, options) => {
...
ipcMain.handle('haishui:pool-limits:get', async () => ({ ...poolLimits }))
ipcMain.handle('haishui:pool-limits:set', async (_event, raw) => {
...
ipcMain.handle('haishui:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('haishui:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('haishui:window:openInstance', async (event, options) => {
...
ipcMain.handle('haishui:window:openBrowser', async (_event, tabId) => {
...
ipcMain.handle('haishui:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    const backend = await resolveHaishuiBackend(tuiResumeArgs(sessionId.trim(), profile || undefined))

    if (!backend.command) {
      return { ok: false, error: 'Haishui is not installed yet' }
    }
    ...
    const scriptPath = path.join(
      scriptDir,
      `haishui-${crypto.randomBytes(6).toString('hex')}${terminalScriptExtension()}`
    )
    ...
    fs.writeFileSync(
      ...
      buildTerminalScript({
        ...
        cwd,
        env: terminalScriptEnv(backend.env, HAISHUI_HOME)
      }),
      { mode: 0o700 }
    )
...
ipcMain.handle('haishui:wake-indicator:get', () => wakeIndicatorController.getState())
ipcMain.on('haishui:wake-indicator:set', (_event, state) => {
...
ipcMain.handle('haishui:zoom:get', event => {
...
ipcMain.on('haishui:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('haishui:backend:recycle', async (_event, profile) => {
...
ipcMain.handle('haishui:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startHaishui() call restarts the
...
ipcMain.handle('haishui:bootstrap:repair', async () => {
  // Forceful repair: force the next startHaishui() through the full installer
  ...
  // "bypass active runtime" path inside startHaishui() and fall through
  ...
  resetHaishuiConnection()

  return { ok: true }
})
ipcMain.handle('haishui:bootstrap:continue-local', async () => {
  rememberLog('[bootstrap] local install selected by renderer; continuing first-launch bootstrap')
  continueFirstRunLocalBootstrap()

  return { ok: true }
})
ipcMain.handle('haishui:bootstrap:cancel', async () => {
...
ipcMain.handle('haishui:boot-progress:get', async () => bootProgressState)
ipcMain.handle('haishui:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('haishui:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('haishui:plugin-profile-routes', async (_event, rawProfileNames) => {
...
ipcMain.handle('haishui:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('haishui:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('haishui:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
...
ipcMain.handle('haishui:secret-storage:get', async () => ({ on: secretStoragePolicy().on }))
ipcMain.handle('haishui:secret-storage:set', async (_event: any, on: any) => applySecretStorageEncryption(on === true))
...
ipcMain.handle('haishui:connections:list', async () => sanitizeConnectionsRegistry())
ipcMain.handle('haishui:connections:save', async (_event, payload) => {
  const saved = await saveRegistryConnection(payload)

  return { ok: true, connection: saved, registry: sanitizeConnectionsRegistry() }
})
ipcMain.handle('haishui:connections:remove', async (_event, id) => {
  const key = String(id || '')
...
ipcMain.handle('haishui:connections:set-primary', async (_event, id) => {
...
ipcMain.handle('haishui:connections:set-launch-mode', async (_event, mode) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('haishui:connections:set-last-used', async (_event, id) => {
  const registry = setLastUsedConnection(readDesktopConnectionsRegistry(), String(id || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('haishui:connections:test', async (_event, id) => {
  const registry = readDesktopConnectionsRegistry()
  ...
  if (entry.kind === 'ssh') {
    const result = await testDesktopConnectionConfig({
      ...
      sshRemoteHaishuiPath: entry.remoteHaishuiPath
    })
  ...
  if (entry.kind === 'local') {
    const local = await startHaishui()
...
async function probeSshProfileInventory(connection) {
  ...
  const sshConfig = normalizeSshConfig({
    ...
    remoteHaishuiPath: connection.remoteHaishuiPath
  })
  ...
  try {
    await ssh.open()
    const profiles = await remoteLifecycle.listRemoteHaishuiProfiles(ssh)
    ...
    registry.connections.map(async connection => {
      ...
      try {
        ...
        if (connection.kind === 'ssh') {
          ...
          // The root HAISHUI_HOME is an agent too; enumerations that omit it
...
ipcMain.handle('haishui:agents:roster', async () => {
  const registry = readDesktopConnectionsRegistry()
  const enumerations = await enumerateRegistryAgentSources(registry)
...
// haishui:gateway:ws-url. Same single-use-ticket discipline for OAuth sources.
...
ipcMain.handle('haishui:gateway:ws-url-for', async (_event, payload) => {
...
ipcMain.handle('haishui:connections:update-managed', async (_event, rawId) => requestManagedSshUpdate(rawId))

// Fan out `haishui update` to every eligible registered connection at once.
...
ipcMain.handle('haishui:connections:update-all', async (_event, payload) => {
  const registry = readDesktopConnectionsRegistry()
    ...
    registry.connections
      ...
      .map(async connection => {
        ...
        try {
          ...
          const body: any = await postJsonForBackend(descriptor, '/api/haishui/update', {}, { timeoutMs: 15_000 })
...
// analogue of the haishui:api handler's own auth split: OAuth backends prefer a
...
ipcMain.handle('haishui:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('haishui:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startHaishui() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('haishui:connection-config:oauth-logout', async (_event, rawUrl) => {
  const baseUrl = normalizeRemoteBaseUrl(rawUrl)
...
// --- Haishui Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('haishui:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('haishui:connection-config:save', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('haishui:connection-config:apply', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('haishui:profile:default:get', async () => desktopProfilePreferences.getDefault())
ipcMain.handle('haishui:profile:default:set', async (_event, route) => desktopProfilePreferences.setDefault(route))
ipcMain.handle('haishui:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
// Persistence-only sibling of haishui:profile:set: records the profile the
...
ipcMain.handle('haishui:profile:remember', async (_event, name) => ({
  profile: writeActiveDesktopProfile(name)
}))
ipcMain.handle('haishui:profile:set', async (_event, name) => {
  assertCanMutateManagedPrimaryRouting()
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new HAISHUI_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('haishui:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('haishui:requestMicrophoneAccess', async () => {
...
ipcMain.handle('haishui:window:readBelow', async event => {
...
async function handleHaishuiApiRequest(request) {
  ...
  // backend calls ensure_haishui_home() which recreates the profile directory,
...
ipcMain.handle('haishui:api', async (_event, request) => {
  ...
  // respawn the old-name backend and recreate its HAISHUI_HOME (#45474).
  ...
  if (!mutatingProfile) {
    return handleHaishuiApiRequest(request)
  }

  const releaseProfileDeletion = profileDeletionGate.acquire(mutatingProfile)

  return handleHaishuiApiRequest(request).finally(releaseProfileDeletion)
})
...
ipcMain.handle('haishui:ambient:claim', (_event, key) => ownsAmbientCue(String(key ?? '')))
...
ipcMain.handle('haishui:data-url-read-max:get', () => ({
...
ipcMain.handle('haishui:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('haishui:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('haishui:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('haishui:readFileText', async (_event, filePath) => {
...
// `haishui:readFileText` is the *preview* read and silently truncates at
...
ipcMain.handle('haishui:readPluginSource', async (_event: unknown, filePath: unknown) => {
  const { resolvedPath, stat } = await resolveReadableFileForIpc(filePath, {
...
ipcMain.handle('haishui:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('haishui:writeClipboard', (_event, text) => {
...
ipcMain.handle('haishui:selectSavePath', async (_event, options: any = {}) => {
  const result = await dialog.showSaveDialog(mainWindow, {
...
ipcMain.handle('haishui:readClipboard', () => clipboard.readText())

ipcMain.handle('haishui:saveGatewayFile', (_event, payload) => saveGatewayFile(payload))

ipcMain.handle('haishui:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))
...
ipcMain.handle('haishui:context-menu:edit', (event, command) => {
...
ipcMain.handle('haishui:context-menu:copy-image', event => {
...
ipcMain.handle('haishui:context-menu:spellcheck', (event, action) => {
...
ipcMain.handle('haishui:context-menu:guest-add-word', (_event, payload) => {
...
ipcMain.handle('haishui:capturePreview', async (_event, payload) => {
  const guest = electronWebContents.fromId(Number(payload?.webContentsId))

  return capturePreviewContents(guest, payload?.rect, payload?.viewport)
})

ipcMain.handle('haishui:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('haishui:savePastedText', async (_event, payload) => {
...
ipcMain.handle('haishui:saveClipboardImage', async () => {
...
ipcMain.handle('haishui:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('haishui:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('haishui:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))

ipcMain.handle('haishui:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))
...
ipcMain.on('haishui:active-work', (event, payload) => {
...
ipcMain.on('haishui:titlebar-theme', (_event, payload) => {
...
ipcMain.on('haishui:native-theme', (_event, mode) => {
...
ipcMain.on('haishui:translucency:support', event => {
...
// it arrives from `haishui desktop --local` or directly on Haishui.exe (a
// shortcut edit), and survives self-relaunches because collectRelaunchArgs
// only strips internal flags.
ipcMain.on('haishui:launch-flags', event => {
  event.returnValue = {
...
ipcMain.on('haishui:translucency', (_event, payload) => {
...
ipcMain.on('haishui:keep-awake', (_event, on) => {
...
ipcMain.handle('haishui:quick-entry:settings:get', async () => {
...
ipcMain.handle('haishui:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('haishui:quick-entry:submit', (_event, payload) => {
  hideQuickEntryWindow()
  ...
  mainWindow.webContents.send('haishui:quick-entry:submit', {
...
ipcMain.on('haishui:quick-entry:state', (_event, payload) => {
  quickEntryLastState = payload ?? null

  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('haishui:quick-entry:state', payload)
  }
})

ipcMain.on('haishui:quick-entry:dismiss', () => hideQuickEntryWindow())
...
ipcMain.on('haishui:devtools:disable-f12', (_event, on) => {
...
ipcMain.handle('haishui:openExternal', (_event, url) => {
...
// per-session secondary windows spawned via `haishui:window:openSession`).
...
ipcMain.handle('haishui:find-in-page', async (event, query, options) => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('haishui:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('haishui:preview:reach', async (event, url) => reachablePreviewUrl(event.sender.id, String(url || '')))

ipcMain.handle('haishui:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveHaishuiCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('haishui:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveHaishuiCwd()
}))

ipcMain.handle('haishui:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('haishui:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('haishui:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('haishui:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('haishui:resolveFavicon', (_event, url) => resolveFaviconCached(url))

ipcMain.handle('haishui:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('haishui:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: haishuiLog.slice(-200) }))
...
ipcMain.on('haishui:logs:renderer-error', (_event, report) => {
...
// Embedded terminal PTY host (haishui:terminal:*) — see terminal-ipc.ts.
...
ipcMain.handle('haishui:updates:check', async (_event, opts) =>
...
ipcMain.handle('haishui:updates:apply', async (_event, payload) =>
...
ipcMain.handle('haishui:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('haishui:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical Haishui version (the one `release.py` bumps in
// haishui_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Haishui version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveHaishuiVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'haishui_cli', '__init__.py')
...
// Renderer-bundle skew: `haishui update` moves the SOURCE TREE, but the UI
...
// Re-resolve the live Haishui version and push it into the native About panel
// just before showing it, so an in-place `haishui update` is reflected without
  ...
  void detectRendererSkew().then(skew => {
    app.setAboutPanelOptions({
      ...
      applicationVersion: skew.outOfSync
        ? `${resolveHaishuiVersion()} — app build out of date, update the desktop app`
        : resolveHaishuiVersion(),
      copyright: 'Copyright © 2026 Nous Research'
    })
    app.showAboutPanel()
  })
...
ipcMain.handle('haishui:version', async () => {
  const skew = await detectRendererSkew()

  return {
    appVersion: resolveHaishuiVersion(),
    electronVersion: process.versions.electron,
    nodeVersion: process.versions.node,
    platform: process.platform,
    haishuiRoot: resolveUpdateRoot(),
...
// The About page's "Restart Haishui" button (shown when bundleSwapPending):
...
ipcMain.handle('haishui:app:relaunch', async () => {
...
ipcMain.handle('haishui:machine:profile', async () => {
...
// the actual removal via `haishui uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (haishui_cli/uninstall.py + haishui_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_HAISHUI_ROOT
  ...
  const fallback = () => ({
    haishui_home: HAISHUI_HOME,
    agent_installed: isHaishuiSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'haishui_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, HAISHUI_HOME, NO_COLOR: '1' },
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Haishui agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import haishui_cli` resolves from
  ...
  if (modeRemovesAgent(mode)) {
    ...
    if (sysPy) {
      py = sysPy
      pythonPath = ACTIVE_HAISHUI_ROOT
    } else if (IS_WINDOWS) {
      rememberLog(
  ...
  try {
    await releaseBackendLock(ACTIVE_HAISHUI_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `haishui-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `haishui-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('haishui:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('haishui:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('haishui:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('haishui:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// haishui:// deep links (e.g. haishui://blueprint/morning-brief?time=08:00,
// haishui://mcp/install?name=NAME&config=B64 — the vendor "Add to Haishui"
// button, or haishui://plugin/install?repo=owner/repo). Dev
// (`HAISHUI_DESKTOP_DEV_SERVER`) registers haishui-dev:// instead — bare
// Electron or a stale OS handler often owns haishui:// on dev machines.
...
// ---------------------------------------------------------------------------
const HAISHUI_PROTOCOL = DEV_SERVER ? 'haishui-dev' : 'haishui'
/** Schemes accepted when parsing inbound URLs (dev accepts both). */
const DEEPLINK_SCHEMES = DEV_SERVER ? ['haishui-dev', 'haishui'] : ['haishui']
...
function handleDeepLink(url) {
  ...
  // haishui://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('haishui:deep-link', payload)
...
ipcMain.handle('haishui:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${HAISHUI_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(HAISHUI_PROTOCOL, process.execPath, [entry])
    } else {
      app.setAsDefaultProtocolClient(HAISHUI_PROTOCOL)
    }

    rememberLog(`[deeplink] registered ${HAISHUI_PROTOCOL}:// handler`)
  } catch (err) {
...
// second-instance argv. Without the lock a second `haishui://` launch spawns a
...
if (!isPrimaryInstance) {
  ...
  // startup (shortcut registration, createWindow → startHaishui), whose
...
app.whenReady().then(() => {
  ...
  // startHaishui() finishes resolving the configured backend.
  const primaryProfile = primaryProfileKey()
  ...
  // Win/Linux cold start: the launching haishui:// URL is in our own argv.
...
app.on('before-quit', event => {
  ...
  // quitting Haishui never keeps another app's chord hostage.
