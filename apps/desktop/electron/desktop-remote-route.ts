...
export function resolveDesktopRemoteRoute({
  ...
  if (envUrl) {
    ...
    if (!envToken) {
      throw new Error(
        'HAISHUI_DESKTOP_REMOTE_URL is set but HAISHUI_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Haishui backend.'
      )
    }
  ...
  if (!modeIsRemoteLike(config.mode)) {
    ...
    // resolves local and spawns a loopback `haishui serve` the desktop never
