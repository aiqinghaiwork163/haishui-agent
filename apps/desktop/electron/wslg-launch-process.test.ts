...
beforeAll(async () => {
  fixtureDir = await mkdtemp(path.join(tmpdir(), 'haishui-wslg-process-'))
  await build({
    entryPoints: [
  ...
  await writeFile(
    ...
    const fd = Number(process.env.HAISHUI_DESKTOP_READY_FD)
    ...
    console.log(JSON.stringify({ kind: 'launcher', pid: process.pid,
      ...
      readyEnv: process.env.HAISHUI_DESKTOP_READY_FD
    }))
  ...
  const launcher = spawn(
    ...
    [...(inspect ? ['--inspect=127.0.0.1:0'] : []), path.join(fixtureDir, 'launcher.mjs')],
    {
      ...
      env: {
        ...process.env,
        NODE_OPTIONS: '',
        HAISHUI_DESKTOP_READY_FD: readyFd,
