...
export function registerPetOverlayIpc({
  ...
  ipcMain.handle('haishui:pet-overlay:open', async (_event, request) => {
  ...
  ipcMain.handle('haishui:pet-overlay:close', async () => {
    closePetOverlay()
  ...
  ipcMain.on('haishui:pet-overlay:set-bounds', (_event, bounds) => {
  ...
  ipcMain.on('haishui:pet-overlay:ignore-mouse', (_event, ignore) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('haishui:pet-overlay:set-focusable', (_event, focusable) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('haishui:pet-overlay:state', (_event, payload) => {
    const petOverlayWindow = getPetOverlayWindow()

    if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
      petOverlayWindow.webContents.send('haishui:pet-overlay:state', payload)
    }
  })
  // Overlay → main renderer: control messages (pop back in, composer submit).
  ipcMain.on('haishui:pet-overlay:control', (_event, payload) => {
    const mainWindow = getMainWindow()
    ...
    mainWindow.webContents.send('haishui:pet-overlay:control', payload)
  })
}
