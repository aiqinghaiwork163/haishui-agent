 ...
 *   - the browser sees only a minimal "return to Haishui" page.
 */
...
const DONE_HTML =
  ...
  '<p>You can close this window and return to Haishui.</p>' +
...
export function registerMcpOauthCallbackIpc() {
  ...
  ipcMain.handle('haishui:mcp-oauth:listen', async () => {
  ...
  ipcMain.handle('haishui:mcp-oauth:wait', async (_event, id, timeoutMs) => {
  ...
  ipcMain.handle('haishui:mcp-oauth:cancel', (_event, id) => {
    dispose(String(id || ''))
