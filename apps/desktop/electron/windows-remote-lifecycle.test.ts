...
test('Windows spawn holds the update mutex across marker check and helper spawn', () => {
  const command = atomicWindowsSpawnCommand({
    haishuiHome: 'C:\\Users\\andre\\.haishui',
    python: 'C:\\Users\\andre\\.haishui\\python.exe'
  })

  const encoded = command.match(/-EncodedCommand\s+([^\s]+)$/)?.[1]
  const script = encoded ? Buffer.from(encoded, 'base64').toString('utf16le') : ''
  assert.match(script, /\.haishui-update-in-progress/)
...
test('Windows spawn publishes the initial ownership record before releasing the mutex', () => {
  const command = atomicWindowsSpawnCommand(
    ...
    },
    {
      ...
      profile: 'default',
      haishuiPath: 'C:\\Haishui\\haishui.exe',
      haishuiHome: 'C:\\Users\\andre\\.haishui',
...
test('every emitted PowerShell script keeps try blocks attached to their catch/finally handlers', async () => {
  ...
  await assertWindowsRemoteInstallUpdateClear(
    ...
    'C:\\Users\\alice\\.haishui'
  )
  scripts.push(
    decode(atomicWindowsSpawnCommand({ haishuiHome: 'C:\\Users\\alice\\.haishui', python: 'C:\\py\\python.exe' })),
...
test('Windows relaunch gate refuses live and uncertain markers before executing the remote runtime', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = sshWith(async command => {
      ...
      if (script.includes('Get-Command haishui.exe')) {
        return JSON.stringify({
          os: 'Windows',
          arch: 'AMD64',
          haishuiHome: 'C:\\Users\\alice\\.haishui',
          haishuiPath: 'C:\\Haishui\\haishui.exe',
          python: 'C:\\Haishui\\python.exe'
        })
      }

      if (script.includes('.haishui-update-in-progress')) {
        return observation
      }
    ...
    assert.equal(
      scripts.some(script => script.includes('haishui_cli.windows_ssh_runtime')),
      false
    )
  }
})
...
test('Windows relaunch gate uses strict install-wide marker parsing and fail-closed PID probing', async () => {
  ...
  await assertWindowsRemoteInstallUpdateClear(ssh, 'C:\\Users\\alice\\.haishui\\profiles\\research')
  assert.match(script, /\.haishui-update-in-progress/)
...
test('Windows probe validates Haishui and Python topology before selection', async () => {
  let script = ''
  await probeWindowsRemote(
    sshWith(async command => {
      script = Buffer.from(command.split(' ').at(-1) || '', 'base64').toString('utf16le')
  ...
  const fallbackJoin = script.indexOf('Join-Path $haishuiHome')
  ...
  await assert.rejects(
    detectRemotePlatform(
      sshWith(async command => {
        ...
        throw new Error('Haishui is not installed on the remote Windows host.')
      })
    ),
    (err: any) => err.kind === 'unsupported-platform' && /Haishui is not installed/.test(err.message)
  )
})

test('helper command uses the fixed remote Python entry point and quotes path data', () => {
  const command = helperCommand({ python: "C:\\Program Files\\Haishui's\\python.exe" }, 'inspect', [
    'C:\\x y\\haishui.exe'
  ])

  const encoded = command.split(' ').pop()!
  const script = Buffer.from(encoded, 'base64').toString('utf16le')
  assert.match(script, /-m' 'haishui_cli\.windows_ssh_runtime' 'inspect'/)
  assert.match(script, /Haishui''s/)
  assert.match(script, /C:\\x y\\haishui\.exe/)
})
...
test('Windows SSH reuse requires the requested remote profile to match the lock', () => {
  ...
  const runtime = { haishuiPath: lock.haishuiPath, haishuiHome: lock.haishuiHome }
