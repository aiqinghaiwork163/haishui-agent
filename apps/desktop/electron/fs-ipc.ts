...
// hardening, HAISHUI_HOME resolution, and the git binary stay injected.
...
export function registerFsIpc({
  ...
  ipcMain.handle('haishui:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

  ipcMain.handle('haishui:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

  // Reveal a path in the OS file manager (Finder / Explorer / Files).
  ipcMain.handle('haishui:fs:reveal', async (_event, targetPath) => {
  ...
  ipcMain.handle('haishui:fs:openDir', async (_event, dirPath) => {
  ...
  // The LOCAL Desktop runtime-plugin root: `<HAISHUI_HOME>/desktop-plugins`,
  // resolved from the main-process HAISHUI_HOME (see resolveHaishuiHome) — NOT from
  // the connected backend. A remote backend reports its own `haishui_home` over
  ...
  async function localPluginsRoot(dirName: string): Promise<string> {
    ...
    const base = profile && profile !== 'default' ? path.join(haishuiHome, 'profiles', profile) : haishuiHome
  ...
  async function desktopPluginsRoot(): Promise<string> {
    const root = await ensureDir(path.join(haishuiHome, DESKTOP_PLUGINS_DIR))
    await migrateProfileScopedDesktopPlugins(haishuiHome, root)
    await reconcileUnifiedDesktopHalves(haishuiHome, root)

    return root
  }

  ipcMain.handle('haishui:fs:desktopPluginsRoot', async () => desktopPluginsRoot())
  ...
  ipcMain.handle('haishui:fs:reconcileDesktopPlugins', async () => {
    const root = await ensureDir(path.join(haishuiHome, DESKTOP_PLUGINS_DIR))

    return reconcileUnifiedDesktopHalves(haishuiHome, root)
  })

  // The LOCAL logs root (`<HAISHUI_HOME>/logs`, profile-aware) — the error
  // card's "Open Logs" action reveals agent.log/gateway.log without the user
  // knowing where HAISHUI_HOME lives. Same Electron-local resolution as the
  // plugin roots: valid in every connection mode, created on demand.
  ipcMain.handle('haishui:fs:logsRoot', async () => localPluginsRoot('logs'))

  ipcMain.handle('haishui:plugin:probe', async (_event, payload) => {
  ...
  ipcMain.handle('haishui:plugin:installDesktop', async (_event, payload) => {
    const identifier = String(payload?.identifier || payload?.repo || '').trim()
  ...
  ipcMain.handle('haishui:fs:rename', async (_event, targetPath, newName) => {
  ...
  ipcMain.handle('haishui:fs:writeText', async (_event, filePath, content) => {
  ...
  ipcMain.handle('haishui:fs:trash', async (_event, targetPath) => {
    const target = String(targetPath || '').trim()
