 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Haishui dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Haishui install on the remote (login-shell probe),
 ...
 *     `HAISHUI_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForHaishui in.
 *
 * The minted HAISHUI_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const REMOTE_LOCK_DIR = '~/.haishui/desktop-ssh'
...
// Resolve the remote haishui executable. An EXPLICIT path is honored strictly
...
async function locateHaishui(ssh, remoteHaishuiPath) {
  const resolveLauncher = async (candidate: string) => {
    // Return the candidate path directly. The haishui binary or wrapper script
  ...
  if (remoteHaishuiPath) {
    if (await isExecutable(remoteHaishuiPath)) {
      return resolveLauncher(remoteHaishuiPath)
    }

    const err: any = new Error(
      `The Haishui path you set is not an executable on the remote host: "${remoteHaishuiPath}". ` +
        'Check the path (it must be the full path to the `haishui` binary on the remote, e.g. ' +
        '~/haishui-agent/.venv/bin/haishui), or clear it to auto-detect.'
    )

    err.kind = 'haishui-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v haishui')}`)).trim()
  ...
  candidates.push('~/.local/bin/haishui')
  candidates.push('/usr/local/bin/haishui')
  candidates.push('~/.haishui/haishui-agent/venv/bin/haishui')
...
// Probe the resolved binary's version string (first line of `<haishui> --version`,
// e.g. "Haishui Agent v0.18.2 ..."), or '' on failure. Surfaces WHICH haishui a
// connection uses, so a stale/unexpected install is visible.
async function probeHaishuiVersion(ssh, haishuiPath) {
  try {
    // Watchdogged: a hung remote CLI must die remotely instead of orphaning
    // when the local ssh child is SIGKILLed (#110478).
    const out = (await ssh.exec(withRemoteTimeout(`${expandRemotePath(haishuiPath)} --version 2>&1`))).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Haishui Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The HAISHUI_HOME the remote dashboard will use (explicit env wins, else
// ~/.haishui). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteHaishuiHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${HAISHUI_HOME:-$HOME/.haishui}"')).trim().split('\n').pop()

    return out || '~/.haishui'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote Haishui home.')
...
marker=home/'.haishui-update-in-progress'
try:
 ...
 * bytes; it never imports or executes code from the changing Haishui checkout.
 ...
 */
async function assertRemoteInstallUpdateClear(ssh, haishuiHome) {
  const home = assertSafeRemoteHome(haishuiHome)
  let observation = ''
  ...
  try {
    ...
    const error: any = new Error('Could not prove that the remote Haishui install is clear for SSH startup.')
  ...
  const error: any = new Error(
    live
      ? `Remote Haishui update process ${live[1]} is still running; SSH startup is paused.`
      : 'The remote Haishui update marker is unreadable or malformed; refusing SSH startup.'
  )
...
async function listRemoteHaishuiProfiles(ssh) {
  const home = assertSafeRemoteHome(await probeRemoteHaishuiHome(ssh))
  ...
  try {
    ...
    const error: any = new Error('Could not list remote Haishui profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote Haishui home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'haishuiPath', 'haishuiHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(
  ...
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !haishuiPath) {
    return false
  }
  ...
  const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(haishuiPath)})\n` +
      // The installer-facing launcher is intentionally preserved for invocation
      // (#74411), but it may `exec python <install-dir>/haishui`, leaving neither
      // launcher nor HAISHUI_HOME-derived entrypoint in argv. The ownership-scoped
      // token path + random nonce + exact profile below are the alternative proof.
      `haishui_home=os.path.expanduser(${shq(haishuiHome)}) if ${shq(haishuiHome)} else ""\n` +
      'expected_entries={expected}\n' +
      'if haishui_home:\n' +
      ' expected_entries.add(os.path.join(haishui_home,"haishui-agent","venv","bin","haishui"))\n' +
...
function buildOwnedStaleTerminationCommand(lock, ownershipId) {
  ...
  const expectedPath = expandRemotePath(lock.haishuiPath)
  const expectedHome = lock.haishuiHome ? expandRemotePath(lock.haishuiHome) : "''"
  ...
  const executableMatch = lock.haishuiHome
...
function lockMatchesManagedUpdateScope(lock, expected) {
  return Boolean(
    ...
    lock.haishuiPath === expected.haishuiPath &&
    lock.haishuiHome === expected.haishuiHome
  )
}
...
expected_path=os.path.expanduser(${py(lock.haishuiPath)})
haishui_home=os.path.expanduser(${py(lock.haishuiHome)})
expected_entries={expected_path,os.path.join(haishui_home,"haishui-agent","venv","bin","haishui")}
...
// execing Haishui. mutexPath is expandRemotePath() output — a complete shell
...
result=None
try:
 result=subprocess.run(["sh","-c",payload,"haishui-update-mutex",str(fd)],pass_fds=(fd,),check=False)
finally:
 os.close(fd)
...
function buildSpawnCommand(haishuiPath, profile, opts: any = {}) {
  const haishui = expandRemotePath(haishuiPath)
  ...
  const marker = expandRemotePath(`${remoteInstallRoot(opts.haishuiHome || '~/.haishui')}/.haishui-update-in-progress`)

  const updateMutex = expandRemotePath(
    `${remoteInstallRoot(opts.haishuiHome || '~/.haishui')}/.haishui-update-in-progress.mutex`
  )
  ...
  const dashCmd =
    ...
    `exec env HAISHUI_DESKTOP=1${opts.guestOnboarding === true ? ' HAISHUI_GUEST_ONBOARDING=1' : ''} ${haishui} ${profileArgs}${subCmd}`

  const detachedShell = `eval "exec $1>&-"; ${dashCmd} </dev/null >> ${logPath} 2>&1 & echo $!`
  const detachedSpawn = `child=$("$(command -v setsid || echo nohup)" sh -c ${shq(detachedShell)} haishui-update-child "$1" & echo $!)`
...
async function remoteSupportsSshOwnership(ssh, haishuiPath) {
  const haishui = expandRemotePath(haishuiPath)
  ...
  const out = await ssh.exec(
    `help="$( ${withRemoteTimeout(`${haishui} serve --help 2>&1`)} )"; ` +
...
async function spawnRemoteDashboard(
  ...
  if (!(await remoteSupportsSshOwnership(ssh, haishuiPath))) {
    const err: any = new Error(
      'The remote Haishui install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Haishui on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    ...
    out = await ssh.exec(
      buildSpawnCommand(haishuiPath, profile, {
        spawnNonce,
        tokenFilePath,
        logPath,
        haishuiHome,
 ...
 * opened SshConnection, forward/pickLocalPort/waitForHaishui, a token-gated
...
async function connect(deps) {
  ...
  const haishuiHome = await probeRemoteHaishuiHome(ssh)
  await assertRemoteInstallUpdateClear(ssh, haishuiHome)
  const haishuiPath = await locateHaishui(ssh, remoteHaishuiPath)
  log(`located haishui at ${haishuiPath}`)
  const haishuiVersion = await probeHaishuiVersion(ssh, haishuiPath)

  if (haishuiVersion) {
    log(`remote haishui version: ${haishuiVersion}`)
  }
  ...
  if (isLockfileSkew(lock)) {
    ...
    const error: any = new Error(
      `The remote ownership record ${lpath} does not match this Haishui Desktop build (${lock.reason}). ` +
  ...
  if (lock) {
    ...
    const reusable =
      ...
      lock.haishuiPath === haishuiPath &&
      lock.haishuiHome === haishuiHome
  ...
  const spawned = await spawnRemoteDashboard(ssh, {
    ...
    haishuiHome,
    guestOnboarding,
    assertInstallClear: () => assertRemoteInstallUpdateClear(ssh, haishuiHome)
  })
  ...
  try {
    ...
    assertBootstrapNotSuperseded(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForHaishui(baseUrl, spawnToken)
    assertBootstrapNotSuperseded(signal)
