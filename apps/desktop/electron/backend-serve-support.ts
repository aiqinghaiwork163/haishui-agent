...
// spawns `haishui serve`; runtimes older than serve only have `dashboard`, so
...
// (covers a bare `haishui` resolved from PATH with no known source root). Result
...
export function createBackendServeSupportResolver(haishuiHome: string, rememberLog: (message: string) => void) {
    ...
    const pending = (async () => {
      ...
      if (backend.root) {
        try {
          const src = await fs.promises.readFile(
            path.join(backend.root, 'haishui_cli', 'subcommands', 'dashboard.py'),
            'utf8'
          )
      ...
      if (supported === null) {
        try {
          ...
          // `haishui --version` (~10.5s measured cold), and a false negative here
          ...
          await execProbe(backend.command, [...prefix, 'serve', '--help'], {
            ...
            env: { ...process.env, HAISHUI_HOME: haishuiHome, ...(backend.env || {}) },
            timeout: PROBE_TIMEOUT_MS,
            stdio: 'ignore',
            // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
            // (see resolveHaishuiBackend step 4); execFileSync of a .cmd without
