...
test('a live primary keeps answering for its booted profile after the preference moves', () => {
  ...
  // startHaishui() boots the primary as the preference at that moment.
  ...
  // haishui:profile:remember rewrites active-profile.json without re-homing.
  preference = 'claude'
  ...
  // spawned for the same HAISHUI_HOME.
  assert.equal(
    pin.resolve(() => preference),
    'default'
  )
