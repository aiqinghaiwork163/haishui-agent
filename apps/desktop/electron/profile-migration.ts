...
export interface MigrationDeps {
  ...
  /** Default profile home (`~/.haishui`). Default's state.db and gateway.pid live here. */
  haishuiHome: string
  /** Named-profile root (`~/.haishui/profiles`). Does not contain `default`. */
  profilesRoot: string
  ...
  isHaishuiProcess: (pid: number) => boolean
  now: () => number
...
/**
 * Production layout: default IS `haishuiHome`; named profiles are children of
 * `profilesRoot`. There is no `profiles/default` directory on a normal install.
 */
export function profileStateDbPath(name: string, haishuiHome: string, profilesRoot: string): string {
  return name === 'default' ? `${haishuiHome}/state.db` : `${profilesRoot}/${name}/state.db`
}

export function profileGatewayPidPath(name: string, haishuiHome: string, profilesRoot: string): string {
  return name === 'default' ? `${haishuiHome}/gateway.pid` : `${profilesRoot}/${name}/gateway.pid`
}

function resolveHaishuiHome(profilesRoot: string, haishuiHome?: string): string {
  if (haishuiHome) {
    return haishuiHome
  }

  // Tests that predate haishuiHome pass only profilesRoot.
...
/**
 * Return the profile names whose gateway.pid file points to a live haishui process.
 ...
 *
 * `haishuiHome` is optional so existing call sites that only pass `profilesRoot`
 * still work: it is derived as the parent of `…/profiles`.
 */
export function findRunningGatewayProfiles(
  profilesRoot: string,
  allProfiles: string[],
  deps: Pick<MigrationDeps, 'existsSync' | 'readFileSync' | 'isHaishuiProcess'> & { haishuiHome?: string }
): string[] {
  const haishuiHome = resolveHaishuiHome(profilesRoot, deps.haishuiHome)
  const running: string[] = []

  for (const name of allProfiles) {
    const pidFile = profileGatewayPidPath(name, haishuiHome, profilesRoot)
...
export function decideMigration(
  ...
  for (const name of candidates) {
    const s = score(profileStateDbPath(name, deps.haishuiHome, deps.profilesRoot))
...
/** Default is always a candidate; it is `$HAISHUI_HOME`, not `$HAISHUI_HOME/profiles/default`. */
...
export function migrateActiveProfileIfMissing(desktopProfileConfigPath: string, deps: MigrationDeps): boolean {
  ...
  // launches `haishui --profile default` and is worse than writing nothing
