/**
 * windows-haishui-path.ts
 *
 * Pure, dependency-injected pieces of Windows `haishui` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvHaishuiCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `haishui` shim shadowed the real
 *      haishui.cmd/haishui.exe; the shim then failed the --version probe and
 ...
 *      and Haishui launcher. Marker-only or partial runtimes must use --repair;
 *      only a runnable pair can use --update.
 *   3. resolveVenvHaishuiCommand() — unwrapWindowsVenvHaishuiCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `haishui`) must
 * not shadow `haishui.cmd`/`haishui.exe`. The empty entry is kept LAST so
 ...
 * venv Python interpreter and the Haishui launcher that drives `haishui update`.
...
export interface ResolveVenvHaishuiCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportHaishuiCli: (python: string, opts?: { env?: Record<string, string> }) => Promise<boolean>
...
/**
 * If `command` is a Windows venv `haishui`/`haishui.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/haishui(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m haishui_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportHaishuiCli(). A
 * venv whose update died mid-`pip install` still has python.exe + haishui.exe
 ...
 *
 * Returns null when `command` is not a venv haishui shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export async function resolveVenvHaishuiCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvHaishuiCommandDeps
): Promise<{
  label: string
  command: string
  args: string[]
  ...
  if (!/^haishui(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    !(await canImportHaishuiCli(python, {
      env: {
    ...
    rememberLog?.(
      `Ignoring venv Haishui at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Haishui Python at ${python}`,
    command: python,
