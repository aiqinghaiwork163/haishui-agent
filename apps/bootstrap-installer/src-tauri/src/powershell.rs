...
/// read side does not see EOF until the last of them closes it. `haishui update`
...
/// stranded with it.
///
/// Same bound `Invoke-HaishuiStep` grew in `scripts/desktop-update/windows.ps1`
...
///
/// `haishui_home_override` propagates to the child as $HAISHUI_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, haishui_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = haishui_home_override {
        cmd.env("HAISHUI_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, haishui_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = haishui_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_haishui_home() {
    ...
    /// child. `haishui update` is this shape -- the Electron build alone is
    /// megabytes.
    #[cfg(unix)]
    #[tokio::test]
