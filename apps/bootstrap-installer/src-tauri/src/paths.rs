...
//!
//! Mirrors `haishui_constants.get_haishui_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\haishui
//!   macOS:   ~/.haishui
//!   Linux:   ~/.haishui  (override via $HAISHUI_HOME)
//!
//! NOTE (macOS): Python's get_haishui_home(), scripts/install.sh, and the
//! Electron desktop's resolveHaishuiHome() ALL use ~/.haishui on macOS — there
...
/// Returns the canonical Haishui home directory, respecting $HAISHUI_HOME if set.
pub fn haishui_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("HAISHUI_HOME") {
    ...
    {
        // %LOCALAPPDATA%\haishui — matches scripts/install.ps1's $HaishuiHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("haishui");
        }
    }

    // macOS + Linux + fallback: ~/.haishui (matches Python get_haishui_home(),
    // install.sh, and the Electron desktop's resolveHaishuiHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".haishui");
    }

    // Last resort — current dir, almost certainly wrong but at least
    // doesn't panic.
    PathBuf::from(".haishui")
}

pub fn log_dir() -> PathBuf {
    haishui_home().join("logs")
}
...
pub fn bootstrap_cache_dir() -> PathBuf {
    haishui_home().join("bootstrap-cache")
}
...
/// HAISHUI_HOME so it survives repo checkout deletion (unlike anything under
/// haishui-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\haishui\haishui-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_haishui`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under HAISHUI_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves HAISHUI_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    haishui_home().join(".haishui-update-in-progress")
}
...
pub fn copy_self_to_haishui_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to HAISHUI_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_HAISHUI_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".haishui-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under HAISHUI_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[haishui-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("HAISHUI_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_haishui_home() -> String {
    haishui_home().to_string_lossy().into_owned()
}
