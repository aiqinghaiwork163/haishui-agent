...
export const $haishuiHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.haishui/logs/bootstrap-installer.log')
    $haishuiHome.set('~/.haishui')
  ...
  try {
    const [logPath, haishuiHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_haishui_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $haishuiHome.set(haishuiHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Haishui" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Haishui-Setup.exe --update);
...
export async function launchHaishuiDesktop(): Promise<void> {
  ...
  await invoke('launch_haishui_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Haishui repository' },
