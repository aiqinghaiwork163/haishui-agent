# Contributing to Haishui Agent

Thank you for contributing to Haishui Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Haishui to work everywhere.
...
- **Search both open *and* merged PRs and issues** for your topic or error symptom — the duplicate-check in the PR template fires at review time, after you've already done the work:
  ```bash
  gh search issues --repo aiqinghaiwork163/haishui-agent "<your terms>"
  gh search prs --repo aiqinghaiwork163/haishui-agent --state all "<your terms>"
  ```
  Or use the web UI: [issues](https://github.com/aiqinghaiwork163/haishui-agent/issues?q=) · [PRs (all states)](https://github.com/aiqinghaiwork163/haishui-agent/pulls?q=is%3Apr).
...
Related: #38284 covers the agent-side analog — Haishui itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every Haishui install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `haishui skills browse` (labeled "official") and install it with `haishui skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it with the community. Users can install it with `haishui skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.haishui/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(haishui_home, config)` for setup-wizard integration
...
- Integrate with `haishui memory setup` via `post_setup()` — no need to touch core code
...
The installer creates the Haishui venv, wires the `haishui` command, stamps the
install method for `haishui update`, and clones the full git project into
`$HAISHUI_HOME/haishui-agent` (usually `~/.haishui/haishui-agent`). That keeps your
development environment on the same layout the CLI, updater, lazy dependency
installer, gateway, and docs assume.

```bash
curl -fsSL https://aiqinghaiwork163.github.io/haishui-agent/install.sh | bash
cd "${HAISHUI_HOME:-$HOME/.haishui}/haishui-agent"
...
Use this only if you intentionally do not want Haishui' managed install layout
(for example, a throwaway clone inside a container or CI job). If you install
this way, make sure you run the `haishui` entrypoint from this venv; running the
system `python3 -m haishui_cli.main` can pick up unrelated system Python
packages.
...
uv venv ~/.haishui/venvs/haishui-dev --python 3.11
...
```bash
mkdir -p ~/.haishui/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.haishui/config.yaml
touch ~/.haishui/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.haishui/.env
```

### Run

```bash
# The standard installer already put `haishui` on PATH.
haishui doctor
haishui chat -q "Hello"
```

If you used the manual clone fallback, run `./haishui` from the checkout or
symlink this clone's venv explicitly:

```bash
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/haishui" ~/.local/bin/haishui
```
...
├── cli.py                    # HaishuiCLI class — interactive CLI orchestrator (~4.6k LOC + haishui_cli/cli_*_mixin.py)
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (haishui-cli, haishui-telegram, etc.)
├── haishui_state.py           # SessionDB facade (~1.4k LOC); implementation in haishui_state_*.py (21 siblings) — FTS5 search, session titles
...
│
├── haishui_cli/               # CLI command implementations
│   ├── main.py                   # Entry point, argument parsing, command dispatch
│   ├── cli_*_mixin.py            # HaishuiCLI mixins (slash commands, display, session, ...)
...
│
├── skills/                   # Bundled skills (copied to ~/.haishui/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (aiqinghaiwork163.github.io/haishui-agent)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.haishui/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.haishui/`)

| Path | Purpose |
|------|---------|
| `~/.haishui/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.haishui/.env` | API keys and secrets |
| `~/.haishui/auth.json` | OAuth credentials (Nous Portal) |
| `~/.haishui/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.haishui/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.haishui/state.db` | SQLite session database |
| `~/.haishui/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and explicit `/save` exports. Automatic per-session JSON snapshots are no longer written; state.db is canonical. |
| `~/.haishui/cron/` | Scheduled job data |
| `~/.haishui/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`haishui_state.py`) with full-text search and unique session titles. Automatic per-session JSON snapshots have been removed. Existing files are left untouched; use `/save json` or `haishui sessions export` for an explicit export.
...
(for example `_HAISHUI_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.haishui`:
...
The user may skip setup and keep loading the skill. Haishui only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `haishui setup` or update `~/.haishui/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native Haishui tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Haishui Agent" is the secondary collaborator. If the contributor's commit shows "Haishui Agent" as author because they used Haishui to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Haishui tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `haishui --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Haishui uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.haishui/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `haishui_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `haishui_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Haishui runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the haishui wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `haishui_cli/gateway.py::_scan_gateway_pids` for the pattern.
   ```
...
9. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `haishui_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
15. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `haishui_cli/gateway_windows.py::
...
Haishui has terminal access. Security matters.
...
2. **Test manually**: Run `haishui` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/aiqinghaiwork163/haishui-agent/issues)
- Include: OS, Python version, Haishui version (`haishui --version`), full error traceback
