...
from haishui_constants import OPENROUTER_BASE_URL, get_haishui_home
from agent.retry_utils import jittered_backoff
from haishui_cli.env_loader import load_haishui_dotenv

# Load .env from HAISHUI_HOME first, then project root as a dev fallback.
load_haishui_dotenv(haishui_home=get_haishui_home(), project_env=Path(__file__).parent / ".env")
...
class TrajectoryCompressor:
    ...
    def _init_summarizer(self):
        ...
        if provider:
            ...
            if client is None:
                raise RuntimeError(f"Provider '{provider}' is not configured. Check your API key or run: haishui setup")
