...
from haishui_state_errors import classify_persistence_error, is_disk_full_error
...
_DOCTOR = "Run `haishui {profile_arg}doctor --fix` to diagnose and repair."

# cause -> (code, gloss, action). "disk" is split by is_disk_full_error at lookup time.
_STORAGE_FAILURES: dict[str, tuple[str, str, str]] = {
    "locked": (
        "storage_locked",
        "the session database is locked by another Haishui process",
        "Wait a moment and try again; if it persists, stop the other Haishui process "
        "(`haishui {profile_arg}gateway stop`).",
    ),
    "disk_full": (
        "disk_full",
        "the disk is full",
    ...
    "corrupt": (
        ...
        _DOCTOR + " Recovery: `haishui {profile_arg}sessions recover --source <state.db> --inspect-only`.",
    ),
    "fts_index": (
        "storage_index_corrupt",
        "the session search index is damaged (the messages themselves are intact)",
        "Run `haishui {profile_arg}doctor --fix` (or `haishui {profile_arg}sessions repair`) to rebuild it.",
    ),
    "replaced": (
        "storage_replaced",
        "the session database file was replaced while Haishui was running",
        "Stop Haishui (`haishui {profile_arg}gateway stop`), run `haishui {profile_arg}doctor`, then start it again.",
    ),
    "deleted_wal": (
        "storage_replaced",
        "the session database file was changed or replaced while Haishui was running",
        "Stop Haishui (`haishui {profile_arg}gateway stop`), run `haishui {profile_arg}doctor`, then start it again.",
    ),
    "compression": (
        "storage_busy",
    ...
    "turn_lease": (
        "storage_busy",
        "another Haishui process took over this session",
