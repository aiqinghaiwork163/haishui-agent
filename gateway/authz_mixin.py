...
class GatewayAuthorizationMixin:
    ...
    def _adapters_for_profile(self, profile: Optional[str]) -> dict:
        ...
        BEFORE the active profile name: multiplex turns override ``HAISHUI_HOME`` so
        ...
        if profile_name in profile_adapters:
            ...
            if adapters or not self._is_shared_bot_satellite(profile_name):
                return adapters
            return self._primary_adapters()
        # Identity captured at construction, not the per-turn HAISHUI_HOME-derived name.
    ...
    def _authorization_home_for_source(self, source: SessionSource):
        """HAISHUI_HOME whose allowlist admits *source*: the identity's transport home (or the
        ...
        if profile is None:
            ...
            return get_process_haishui_home()  # the primary bot's home, never the per-turn override
