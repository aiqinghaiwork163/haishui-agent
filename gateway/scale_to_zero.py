...
SCALE_TO_ZERO_ENV = "HAISHUI_SCALE_TO_ZERO"  # stamped by NAS when the Labs toggle is on
...
def dashboard_client_heartbeat_path(haishui_home: Optional[os.PathLike | str] = None):
    """Path of the dashboard-client liveness marker under HAISHUI_HOME."""
    if haishui_home is None:
        from haishui_constants import get_haishui_home
        haishui_home = get_haishui_home()
    return Path(haishui_home) / DASHBOARD_CLIENT_HEARTBEAT_REL
