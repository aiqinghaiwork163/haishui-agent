...
(`haishui_cli/AGENTS.md`). Each adapter picks a base toolset (Telegram → `"messaging"`).
...
`if canonical == ...` chains. Registry + adding a command: `haishui_cli/AGENTS.md`.
...
duplicate-final incident (NS-658 canary ledger, haishui#85796 / gateway-gateway#210); violating any
...
(or `HAISHUI_BACKGROUND_NOTIFICATIONS`): `concise` (default; one line, failures append an output
...
`/login` is registered in `haishui_cli/commands.py` with `busy_policy="dispatch"` and
...
`haishui serve` (control plane, desktop-spawned child) dies with the app — by design. The messaging
gateway (`gateway run`) SURVIVES the app: the serve backend's `/api/gateway/*` endpoints spawn it
detached (`_spawn_haishui_action` — `start_new_session` / `DETACHED_PROCESS`), so `before-quit`'s
