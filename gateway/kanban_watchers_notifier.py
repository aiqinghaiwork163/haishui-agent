...
def _kbn():
    from haishui_cli import kanban_db_notify
...
def _warn_anchorless_thread_sub_once(sub: dict, platform: str) -> None:
    ...
    logger.warning(
        ...
        "profile route; it will not be delivered. Re-subscribe with `haishui kanban notify-subscribe ... "
...
class _Collector:
    ...
    def collect(self) -> list[dict]:
        ...
        # HAISHUI_KANBAN_DB pins the board path.
        kb = self.kb
...
def _fmt_gave_up(ev, n) -> tuple:
    ...
    return (
        f"⛔ {n.head} is now blocked: {count}{last}. Fix the cause, then `haishui kanban unblock "
        f"{n.task_id}` (or `haishui kanban reassign {n.task_id}`). Logs: `haishui kanban log {n.task_id}`.",
        None, None,
    )
...
class _KanbanNotification:
    ...
    def _served_wake_profile(self) -> Optional[str]:
        ...
        ``None`` keeps the historical path: a standalone ``haishui -p <name>`` gateway owns its own
    ...
    async def wake(self) -> None:
        ...
        if self.sub_profile and getattr(getattr(self.runner, "config", None), "multiplex_profiles", False):
            from haishui_cli.profiles import profile_exists
