...
``validate_media_delivery_path`` reads ``HAISHUI_MEDIA_DELIVERY_STRICT`` (gateway.strict),
``HAISHUI_MEDIA_ALLOW_DIRS`` (gateway.media_delivery_allow_dirs) and
``HAISHUI_MEDIA_TRUST_RECENT_FILES`` (gateway.trust_recent_files).  Every delivery
entrypoint (gateway startup, ``haishui cron run``, ``haishui send``) calls
...
_FLAG_ENVS = (("strict", "HAISHUI_MEDIA_DELIVERY_STRICT"), ("trust_recent_files", "HAISHUI_MEDIA_TRUST_RECENT_FILES"))
_ALLOW_DIRS_ENV = "HAISHUI_MEDIA_ALLOW_DIRS"
_TRUST_RECENT_SECONDS_ENV = "HAISHUI_MEDIA_TRUST_RECENT_SECONDS"
_TRUTHY = frozenset({"1", "true", "yes", "on"})


def _routed_gateway_cfg() -> Optional[Dict[str, Any]]:
    """``gateway`` section of the ROUTED profile's config when a HAISHUI_HOME override is active
    ...
    if not get_haishui_home_override():
        return None
    try:
