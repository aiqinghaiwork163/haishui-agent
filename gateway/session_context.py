"""Session-scoped context variables for the Haishui gateway.

Replaces the old ``os.environ``-based ``HAISHUI_SESSION_*`` state with task-local ``ContextVar``s
...
_SESSION_VARS = (
    ...
    "HAISHUI_SESSION_PLATFORM", "HAISHUI_SESSION_SOURCE", "HAISHUI_SESSION_CHAT_ID",
    "HAISHUI_SESSION_CHAT_TYPE", "HAISHUI_SESSION_CHAT_NAME", "HAISHUI_SESSION_THREAD_ID",
    "HAISHUI_SESSION_USER_ID", "HAISHUI_SESSION_USER_ID_ALT", "HAISHUI_SESSION_USER_NAME",
    "HAISHUI_SESSION_SCOPE_ID", "HAISHUI_SESSION_KEY", "HAISHUI_SESSION_ID",
    "HAISHUI_UI_SESSION_ID", "HAISHUI_SESSION_MESSAGE_ID", "HAISHUI_SESSION_PROFILE",
    "HAISHUI_BROWSER_CONTROL_PRINCIPAL", "HAISHUI_BROWSER_CONTROL_TRANSPORT_FAMILY",
    "HAISHUI_CRON_SESSION", "HAISHUI_SESSION_PARENT_CHAT_ID",
))
...
_SESSION_ASYNC_DELIVERY = ContextVar("HAISHUI_SESSION_ASYNC_DELIVERY", default=_UNSET)
...
_SESSION_HISTORY_DELIVERY = ContextVar("HAISHUI_SESSION_HISTORY_DELIVERY", default=_UNSET)

# Cron auto-delivery vars, set per-job in run_job() so concurrent jobs don't clobber.
_CRON_AUTO_DELIVER_PLATFORM = ContextVar("HAISHUI_CRON_AUTO_DELIVER_PLATFORM", default=_UNSET)
_CRON_AUTO_DELIVER_CHAT_ID = ContextVar("HAISHUI_CRON_AUTO_DELIVER_CHAT_ID", default=_UNSET)
_CRON_AUTO_DELIVER_THREAD_ID = ContextVar("HAISHUI_CRON_AUTO_DELIVER_THREAD_ID", default=_UNSET)
...
def set_current_session_id(session_id: str) -> None:
    """Synchronize ``HAISHUI_SESSION_ID`` across ContextVar and ``os.environ`` (tools read it
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session var by legacy ``HAISHUI_SESSION_*`` name; drop-in for os.getenv.  The
...
# Surfaces that are not a human chat channel (gateway binds HAISHUI_SESSION_PLATFORM, CLI/TUI/
# desktop bind HAISHUI_SESSION_SOURCE, so both are consulted).  Default-deny: an unrecognized
...
def session_is_messaging_surface() -> bool:
    ...
    ``HAISHUI_PLATFORM``, then the session platform, then the session source)."""
    platform = os.getenv("HAISHUI_PLATFORM") or get_session_env("HAISHUI_SESSION_PLATFORM", "")
    idents = (platform, get_session_env("HAISHUI_SESSION_SOURCE", ""))
...
def async_delivery_supported() -> bool:
    ...
    (``HAISHUI_KANBAN_TASK``: one-shot subprocesses whose parent disappears after the turn)."""
