...
class GatewayStartupMixin:
    ...
    async def _drain_startup_restore_queue(self) -> int:
        ...
        while queue:
            ...
            with suppress(Exception):
                setattr(event, "_haishui_startup_restore_replay", True)
            await adapter.handle_message(event)
            drained += 1
        return drained
    ...
    async def _run_boot_probe_in_launch_scope(self, fn):
        ...
        if getattr(self.config, "multiplex_profiles", False):
            ...
            try:
                async with _async_profile_runtime_scope(get_haishui_home()):
    ...
    async def _recover_unclean_sessions(self) -> tuple[int, int]:
        ...
        with _log_suppressed(logging.WARNING, "Exact active-turn recovery on startup failed: %s"):
            agent_timeout = max(1.0, _float_env("HAISHUI_AGENT_TIMEOUT", 1800))
    ...
    def _start_loop_heartbeat_task(self) -> None:
        ...
        with _log_suppressed(logging.DEBUG, "Failed to start gateway loop heartbeat", exc_info=True):
            ...
            task._haishui_supervised_watcher = True  # type: ignore[attr-defined]
    ...
    def _open_faulthandler_log(self):
        ...
        from gateway.run import get_haishui_home
        log_dir = getattr(self.config, "log_dir", None) or os.path.join(str(get_haishui_home()), "logs")
    ...
    def _start_log_startup_environment(self) -> None:
        ...
        with suppress(Exception):
            logger.info(
                ...
                "or HAISHUI_MAX_ITERATIONS from .env, or default 500)",
                int(os.getenv("HAISHUI_MAX_ITERATIONS", "500")),
            )
        ...
        with suppress(Exception):
            ...
            if _redact_raw.lower() in {"1", "true", "yes", "on"}:
                ...
                logger.warning(
                    "Secret redaction: DISABLED (HAISHUI_REDACT_SECRETS=%s). API keys and tokens may appear "
        ...
        with _log_suppressed(logging.DEBUG, "security advisory check failed at gateway startup", exc_info=True):
            from haishui_cli.security_advisories import detect_compromised, gateway_log_message
            _adv_msg = gateway_log_message(detect_compromised())
            if _adv_msg:
                logger.warning("%s", _adv_msg)
                logger.warning("Run `haishui doctor` on the gateway host for full remediation steps.")
        ...
        with _log_suppressed(logging.DEBUG, "check_systemd_timing_alignment failed: %s"):
            ...
            if _alignment is not None and _alignment.get("mismatch"):
                logger.warning(
                    ...
                    "mid-drain. Run `haishui gateway install --force` to regenerate the unit, or shorten "
    ...
    def _register_launch_profile_config_hooks() -> None:
        ...
        with _profile_runtime_scope(get_process_haishui_home()):
            GatewayStartupMixin._register_config_hooks(
                "shell-hook/webhook registration failed at gateway startup", level=logging.WARNING)
    ...
    def _register_config_hooks(fail_fmt: str, *fail_args, level: int = logging.DEBUG) -> None:
        ...
        Gateway has no TTY, so consent must come from --accept-hooks, HAISHUI_ACCEPT_HOOKS, or
    ...
    def _recover_secondary_process_checkpoints(self, process_registry) -> int:
        ...
        from haishui_constants import get_haishui_home
        launch_home = get_haishui_home().resolve()
        recovered = 0
    ...
    async def _start_recover_previous_run(self) -> None:
        ...
        _clean_marker = _haishui_home / ".clean_shutdown"
        ...
        if connected_count != 0:
            if startup_nonretryable_errors:
                ...
                logger.error(
                    ...
                    "then `haishui gateway restart`): %s. The gateway is DEGRADED — it serves the "
    ...
    async def start(self) -> bool:
        ...
        logger.info("Starting Haishui Gateway...")
        ...
        # The gateway is a boot owner of the Nous free tier, beside `cmd_chat` and `haishui serve`: every
    ...
    async def _handoff_resolve_destination(
        ...
        try:
            new_thread_id = await transport.adapter.create_handoff_thread(
                home_chat_id, f"Haishui — {cli_title}",
            )
        except Exception as exc:
