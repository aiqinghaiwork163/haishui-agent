...
matches the raw ``X-Haishui-Session-Id`` real turns use (invisible parallel session), so we self-POST
...
async def deliver_wake(adapter: Any, *, text: str, session_id: str = "", source: Any = None,
    ...
    (``X-Haishui-Session-Id`` / state.db key) — required for non-push adapters. ``source`` is the
...
async def _self_post_chat_completion(adapter: Any, *, text: str, session_id: str,
    ...
    own bind host/port/key. Session continuation via ``X-Haishui-Session-Id`` is 403-gated on
    ...
    if not api_key:
        raise RuntimeError("wake self-post requires API_SERVER_KEY: session continuation via "
                           "X-Haishui-Session-Id is rejected (403) on an unauthenticated API "
                           "server, so the wake cannot reach the target session")
    if ":" in host and not host.startswith("["):
        host = f"[{host}]"  # bare IPv6 literal
    url = f"http://{host}:{port}/v1/chat/completions"
    headers = {"Authorization": f"Bearer {api_key}", "X-Haishui-Session-Id": session_id}
    payload = {"model": str(getattr(adapter, "_model_name", "") or "haishui-agent"),
               "messages": [{"role": "user", "content": text}], "stream": False}
    if notification_category == "diagnostic":
        payload["haishui_notification_category"] = "diagnostic"
