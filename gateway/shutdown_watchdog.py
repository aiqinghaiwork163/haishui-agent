...
``<HAISHUI_HOME>/state/gateway.heartbeat`` so supervisors can tell "process alive" from "loop
...
def _mark_exited_quietly(exit_code: int, reason: str) -> None:
    ...
    ``gateway_state.json`` (so ``haishui gateway status`` and every other reader of that file stop
...
def _process_haishui_home() -> Path:
    """HAISHUI_HOME for process-level identity files (ignore profile overrides)."""
...
def _home(home: Optional[Path]) -> Path:
    return home if home is not None else _process_haishui_home()
...
def get_loop_tick_socket_path(home: Optional[Path] = None, pid: Optional[int] = None) -> Path:
    """``<HAISHUI_HOME>/state/gateway.loop-tick.<pid>.sock`` — PID-suffixed so a stale node from a
