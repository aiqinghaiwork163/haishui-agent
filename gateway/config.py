...
class PlatformConfig:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "PlatformConfig":
        ...
        # write them where the docs and ``haishui config set platforms.webhook.port`` put them:
...
class GatewayConfig:
    ...
    sessions_dir: Path = field(default_factory=lambda: get_haishui_home() / "sessions")
    ...
    # ``None``: a request the gateway settles at boot, not a verdict. ``haishui_cli.gateway_multiplex_mode
    ...
    # route); HAISHUI_ROOM_LINK_URL overrides.
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        return cls(
            ...
            sessions_dir=Path(data["sessions_dir"]) if "sessions_dir" in data else get_haishui_home() / "sessions",
...
def load_gateway_config() -> GatewayConfig:
    """Load gateway configuration. Priority: env > ~/.haishui/config.yaml > legacy gateway.json > defaults."""
    from gateway import config_loader

    _home = get_haishui_home()
