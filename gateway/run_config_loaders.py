...
from haishui_cli.config import cfg_get, resolve_ephemeral_system_prompt_from_config
...
class GatewayConfigLoadersMixin:
    ...
    def _load_prefill_messages() -> List[Dict[str, Any]]:
        ...
        HAISHUI_PREFILL_MESSAGES_FILE env wins, then top-level prefill_messages_file in config.yaml,
        then legacy agent.prefill_messages_file. Relative paths resolve from ~/.haishui/.
        """
        from gateway.run import _gateway_config_home, _load_gateway_config
        file_path = os.getenv("HAISHUI_PREFILL_MESSAGES_FILE", "")
        if not file_path:
    ...
    def _load_ephemeral_system_prompt() -> str:
        """HAISHUI_EPHEMERAL_SYSTEM_PROMPT env first, then ``display.personality`` / ``agent.system_prompt``."""
        from gateway.run import _load_gateway_config
        prompt = os.getenv("HAISHUI_EPHEMERAL_SYSTEM_PROMPT", "")
        if prompt:
            return prompt
    ...
    def _resolve_model_for_channel(
        ...
        Precedence lives in :func:`haishui_cli.model_switch.resolve_effective_model` (shared with the
    ...
    def _load_reasoning_config(model: str = "") -> dict | None:
        """Reasoning effort from config.yaml via :func:`haishui_constants.resolve_reasoning_config`.
    ...
    def _load_busy_input_mode(cls) -> str:
        ...
        mode = cls._env_or_cfg_str("HAISHUI_GATEWAY_BUSY_INPUT_MODE", "display", "busy_input_mode").lower()
    ...
    def _load_busy_text_mode(cls) -> str:
        ...
        legacy = cls._env_or_cfg_str("HAISHUI_GATEWAY_BUSY_TEXT_MODE", "display", "busy_text_mode").lower()
    ...
    def _load_restart_drain_timeout(cls) -> float:
        ...
        raw = cls._env_or_cfg_str("HAISHUI_RESTART_DRAIN_TIMEOUT", "agent", "restart_drain_timeout")
    ...
    def _load_restart_after_turn_timeout(cls) -> float:
        ...
        return cls._load_env_or_agent_cfg_timeout(
            "HAISHUI_RESTART_AFTER_TURN_TIMEOUT", "restart_after_turn_timeout",
    ...
    def _load_cron_drain_timeout(cls) -> float:
        ...
        """
        return cls._load_env_or_agent_cfg_timeout(
            "HAISHUI_CRON_DRAIN_TIMEOUT", "cron_drain_timeout",
    ...
    def _load_background_notifications_mode() -> str:
        ...
        mode = _platform_gate_env("HAISHUI_BACKGROUND_NOTIFICATIONS")
        if not mode:
    ...
    def _refresh_fallback_model(self) -> list | None:
        ...
        ``self._fallback_model`` at process start, so a chain configured (or changed) after ``haishui
        ...
        home_key = haishui_home_key(home)
        try:
