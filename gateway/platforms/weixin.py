...
def _account_dir(haishui_home: str) -> Path:
    path = Path(haishui_home) / "weixin" / "accounts"
...
def save_weixin_account(haishui_home: str, *, account_id: str, token: str, base_url: str, user_id: str = "") -> None:
    path = _account_dir(haishui_home) / f"{account_id}.json"
...
def load_weixin_account(haishui_home: str, account_id: str) -> Optional[Dict[str, Any]]:
    return _read_json(_account_dir(haishui_home) / f"{account_id}.json")


class ContextTokenStore:
    """Disk-backed ``context_token`` cache keyed by account + peer."""

    def __init__(self, haishui_home: str):
        self._root = _account_dir(haishui_home)
...
def _load_sync_buf(haishui_home: str, account_id: str) -> str:
    data = _read_json(_account_dir(haishui_home) / f"{account_id}.sync.json")
    return data.get("get_updates_buf", "") if isinstance(data, dict) else ""


def _save_sync_buf(haishui_home: str, account_id: str, sync_buf: str) -> None:
    atomic_json_write(_account_dir(haishui_home) / f"{account_id}.sync.json", {"get_updates_buf": sync_buf})
...
async def qr_login(haishui_home: str, *, bot_type: str = "3", timeout_seconds: int = 480) -> Optional[Dict[str, str]]:
    if not AIOHTTP_AVAILABLE:
        ...
        while time.monotonic() < deadline:
            ...
            if status == "wait":
                ...
                if not creds["account_id"] or not creds["token"]:
                    ...
                    return None
                save_weixin_account(haishui_home, **creds)
...
class WeixinAdapter(OwnAccessPolicyMixin, BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._haishui_home = haishui_home = str(get_haishui_home())
        self._token_store = ContextTokenStore(haishui_home)
        ...
        persisted = load_weixin_account(haishui_home, self._account_id) if self._account_id and not self._token else None
        if persisted:
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if self._group_policy != "disabled":
            logger.warning(
                ...
                "ordinary-group events for these accounts, so group messages may never reach Haishui regardless of "
                "this policy. If group delivery doesn't work, the limitation is on the iLink side, not in Haishui.",
    ...
    async def _poll_loop(self) -> None:
        ...
        sync_buf = _load_sync_buf(self._haishui_home, self._account_id)
        ...
        while self._running:
            try:
                ...
                if response.get("get_updates_buf"):
                    ...
                    _save_sync_buf(self._haishui_home, self._account_id, sync_buf)
    ...
    async def send(self, chat_id: str, content: str, reply_to: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None) -> SendResult:
        ...
        try:
            ...
            for idx, chunk in enumerate(chunks):
                client_id = f"haishui-weixin-{uuid.uuid4().hex}"
    ...
    async def _send_file(self, chat_id: str, path: str, caption: str, force_file_attachment: bool = False) -> str:
        ...
        for item_list in item_lists:
            last_message_id = f"haishui-weixin-{uuid.uuid4().hex}"
            while True:
...
async def send_weixin_direct(
    *, extra: Dict[str, Any], token: Optional[str], chat_id: str, message: str, media_files: Optional[List[Tuple[str, bool]]] = None,
    ...
    token_store = ContextTokenStore(str(get_haishui_home()))
