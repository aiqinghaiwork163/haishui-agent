...
class WhatsAppBehaviorMixin(OwnAccessPolicyMixin):
    ...
    DEFAULT_REPLY_PREFIX: str = "☤ *Haishui Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    ...
    /opt/haishui) is mirrored to HAISHUI_HOME so npm install works."""
    import shutil
    from haishui_constants import get_haishui_home
    install_bridge = Path(__file__).resolve().parents[2] / "scripts" / "whatsapp-bridge"
    haishui_home_bridge = get_haishui_home() / "scripts" / "whatsapp-bridge"
    try:
        ...
        pass
    if haishui_home_bridge.exists():
        return haishui_home_bridge
    try:
        haishui_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(install_bridge, haishui_home_bridge, dirs_exist_ok=False)
        return haishui_home_bridge
    except Exception:
        return install_bridge
