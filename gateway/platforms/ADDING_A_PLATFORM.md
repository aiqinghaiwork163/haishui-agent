...
There are two ways to add a platform to the Haishui gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.haishui/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Haishui code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `haishui gateway status` or `get_connected_platforms()` until the SDK
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `haishui_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Haishui core.
...
```python
"haishui-your-platform": {
    "description": "Your Platform bot toolset",
    "tools": _HAISHUI_CORE_TOOLS,
    "includes": []
},
```

And add it to the `haishui-gateway` composite:

```python
"haishui-gateway": {
    "includes": [..., "haishui-your-platform"]
}
```
...
## 12. Status Display (`haishui_cli/status.py`)
...
## 13. Gateway Setup Wizard (`haishui_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ haishui_cli/ toolsets.py \
  --include="*.py" -l | sort -u
