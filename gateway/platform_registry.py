...
class PlatformEntry:
    ...
    required_env: list = field(default_factory=list)  # ``haishui setup`` display
    ...
    plugin_name: str = ""  # owning manifest so ``haishui gateway setup`` can auto-enable it
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved HAISHUI_HOME and overlay the
        ...
        # `haishui` invocation, so the import happens only when a lookup asks for it.
