...
def _gateway_room_grant_secret_for_home(home_value: str) -> bytes:
    ...
    return hmac.new(material, b"haishui-hosted-room-installation-grant-v1", hashlib.sha256).digest()
...
def gateway_room_grant_secret(root: Path | str | None = None) -> bytes:
    ...
    if root is None:
        ...
        # Profile routing uses a context-local HAISHUI_HOME override; the process environment
        # retains the installation root and is the authority here.
        root = os.environ.get("HAISHUI_HOME") or get_haishui_home()
...
def derive_room_grant_secret(api_key: str) -> bytes:
    ...
    return hmac.new(api_key.encode("utf-8"), b"haishui-hosted-room-grant-v1", hashlib.sha256).digest()
...
def catalog_mapping(
    *, installation_id: str, protocol_versions: Iterable[int] = (PROTOCOL_VERSION,),
    ...
    persistent_process = bool(persistent_process and os.getenv("HAISHUI_DESKTOP") != "1")
    profile = str(target_profile or "").strip() or (os.getenv("HAISHUI_PROFILE") or "default").strip() or "default"
...
def _configured_room_link_url() -> str | None:
    ...
    if (override := os.getenv("HAISHUI_ROOM_LINK_URL")) is not None:
        return override
