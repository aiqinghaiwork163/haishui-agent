...
ONLY the HAISHUI_HOME contextvar installed. Every gate fails OPEN: an unavailable store, a failing
...
def profile_has_active_heartbeat(profile_home: Path) -> bool:
    from haishui_cli.heartbeat import store_has_active_heartbeat

    return _gate(profile_home, store_has_active_heartbeat)


def profile_has_active_loop(profile_home: Path) -> bool:
    from haishui_cli.loops import store_has_active_loop
