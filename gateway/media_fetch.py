...
become a bypass of the host denylist. Strict mode (``HAISHUI_MEDIA_DELIVERY_STRICT``) keeps its
...
# Credential dirs under the sandbox home plus the Haishui stores, which live at ``~/.haishui`` there.
_DENIED_HOME_RELATIVE = tuple(PurePosixPath(s) for s in _MEDIA_DELIVERY_DENIED_HOME_SUBPATHS) + tuple(
    PurePosixPath(".haishui", *PurePosixPath(rel.replace(os.sep, "/")).parts) for rel in _ROOT_CREDENTIAL_PATHS)
...
def _active_remote_env():
    ...
    if backend not in _REMOTE_TERMINAL_BACKENDS and not _plugin_backend_is_remote(backend):
        return None
    return get_active_env(get_session_env("HAISHUI_SESSION_ID") or get_session_env("HAISHUI_SESSION_KEY") or "default")
