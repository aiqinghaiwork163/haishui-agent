...
Defenses 1-2 (``_HAISHUI_GATEWAY`` guard on ``haishui gateway stop|restart`` /
...
persisted to ``<HAISHUI_HOME>/gateway/restart_loop.json`` and CHAIN while gaps stay
...
def _state_path():
    return get_haishui_home() / "gateway" / "restart_loop.json"
