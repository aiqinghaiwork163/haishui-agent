...
``<HAISHUI_HOME>/state/gateway.lifecycle.json`` closes the gap:
...
def _process_haishui_home() -> Path:
    """HAISHUI_HOME for process-level identity files (ignore task overrides)."""
    from haishui_constants import get_haishui_home, get_process_haishui_home

    # get_process_haishui_home expands ``~``/``$VAR`` (python -m gateway.run skips the CLI normalizer).
    return get_process_haishui_home() if os.environ.get("HAISHUI_HOME", "").strip() else get_haishui_home()


def _home_path(home: Optional[Path], *relative: str) -> Path:
    return (_process_haishui_home() if home is None else home).joinpath(*relative)


def get_lifecycle_sentinel_path(home: Optional[Path] = None) -> Path:
    """Return ``<HAISHUI_HOME>/state/gateway.lifecycle.json``."""
...
def _report_unclean_exit(evidence: Dict[str, Any], home: Optional[Path]) -> None:
    ...
    if verdict not in ("ok", "absent"):
        logger.error(
            ...
            "missing until it is repaired. Run `haishui doctor`.",
            verdict,
        )
