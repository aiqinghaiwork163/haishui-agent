"""state.db repair, backup and writability preflight (split from haishui_state).

Patchable helpers are looked up as module globals at call time, so tests patch ``haishui_state_repair.<name>``.
"""
...
from haishui_startup_watchdog import report_startup_progress
...
# Log-record parity with the origin module (caplog tests pin "haishui_state").
logger = logging.getLogger("haishui_state")
...
_MANUAL_RECOVER_HINT = ("Free disk space, then retry (or recover manually with "
                        "`haishui sessions recover --source {db_path} --inspect-only` first).")
...
def _read_offline(db_path: Path, what: str, reader) -> Optional[str]:
    """Run *reader()* under ``haishui_cli.sqlite_safe_read.offline_file_access``.
    ...
    without haishui_cli have no tracked connections."""
    try:
...
def _claim_repair_attempt(db_path: Path) -> bool:
    ...
    from haishui_state import _repair_attempt_lock, _repair_attempted_paths
...
def _try_lock_nonblocking(handle) -> None:
    ...
    from haishui_state import _IS_WINDOWS
    if _IS_WINDOWS:
...
def _cross_process_repair_lock(db_path: Path):
    ...
    #36644).
    """
    from haishui_state import _IS_WINDOWS, _REPAIR_LOCK_TIMEOUT_SECONDS
...
def _repair_snapshot_timeout_seconds(source_path: Path) -> float:
    ...
    from haishui_state import _REPAIR_LOCK_TIMEOUT_SECONDS
    source_bytes = 0
...
def _persistent_repair_exhausted_error(db_path: Path) -> str:
    """The stable operator-facing diagnostic for an exhausted repair budget. The ``haishui`` commands
    carry the profile selector: a bare ``haishui`` follows ``active_profile`` (#105887)."""
    ...
    return (f"automatic repair has already failed {_MAX_PERSISTENT_REPAIR_ATTEMPTS} times on this exact file — the "
            ...
            f"required: restore a backup, or salvage with `haishui {profile_arg}sessions recover --source {db_path} "
            f"--inspect-only`, then (if it reports recoverable) `haishui {profile_arg}sessions recover --source {db_path} "
...
def _backup_db_file(db_path: Path) -> "Tuple[Optional[Path], Optional[str]]":
    ...
    ``close()`` a descriptor and cancel that connection's POSIX advisory locks (``haishui_cli.sqlite_safe_read``) —
    ...
    `haishui sessions recover --source <db> --inspect-only`, never at a raw sqlite3 shell on the live file.

    See #69603.
    """
    with contextlib.suppress(ImportError):  # scaffold/embed installs without haishui_cli track no connections
...
def preflight_db_writability(db_path: Path, *, db_label: str = "state.db") -> None:
    ...
    "fix" (deleting the ``-wal``) loses committed transactions. ``chmod u+rw`` repair only inside the Haishui home
    tree (Haishui owns those files; ``chmod`` fails on files the user doesn't own, bounding the repair exactly);
    ...
    with contextlib.suppress(Exception):  # pragma: no cover - defensive
        home = Path(get_haishui_home()).resolve()
    ...
    for p, is_dir in [(db_path.parent, True), *((p, False) for p in (db_path, *sidecars) if p.is_file())]:
        ...
        raise sqlite3.OperationalError(
            f"{db_label} is not writable: {'directory' if is_dir else 'file'} {p} is read-only for this user. Haishui "
...
def _connect_repair_durable(db_path: Path, *, timeout: float = 5.0) -> sqlite3.Connection:
    ...
    Tracked (:func:`haishui_cli.sqlite_safe_read.connect_tracked`) because repair connections hold the
...
def _reapply_durability_barriers(conn: sqlite3.Connection) -> bool:
    ...
    from haishui_state_wal import _apply_macos_checkpoint_barrier, _enforce_macos_synchronous_full
    try:
...
def apply_durability_barriers(conn: sqlite3.Connection) -> bool:
    ...
    from haishui_state_wal import _apply_synchronous_pragma
    ok = _reapply_durability_barriers(conn)
    with contextlib.suppress(Exception):
        from haishui_cli.config import cfg_get, load_config_readonly  # local: avoids an import cycle
...
# Haishui-owned FTS5 objects: the virtual tables and their shadow b-trees. Full-matched, so a
...
def _db_opens_cleanly(db_path: Path) -> Optional[str]:
    ...
    See #50502.
    """
    from haishui_state import SessionDB, load_fts5_cjk_extension
    ...
    try:
        with contextlib.closing(conn):
            ...
            probe_session_id = f"_haishui_fts_health_probe_{time.time_ns()}"
            try:
...
def _live_writer_holds_db(db_path: Path) -> bool:
    ...
    The foreign-holder scan (``haishui_state_holders``) is the authority: any other process with the DB or a
    ...
    with _cross_process_repair_lock(db_path) as holding_lock:
        ...
        # exclusive guard in the locked routine excludes writers through promotion and sees DELETE-mode readers too.
            ...
            _repair_skip(report, "skipped", "a live writer still holds state.db; skipped schema surgery to avoid tearing "
                         "b-tree pages under a concurrent writer. Stop the gateway (haishui gateway stop) and retry.")
        else:
...
def _probe_journal_mode_for_repair(db_path: Path) -> Optional[str]:
    ...
    from haishui_state_wal import _on_disk_journal_mode
    try:
...
def _strategy_rebuild_fts(conn: sqlite3.Connection) -> None:
    ...
    from haishui_state import load_fts5_cjk_extension
