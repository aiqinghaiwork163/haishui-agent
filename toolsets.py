...
_HAISHUI_WEBHOOK_SAFE_TOOLS = ["web_search", "web_extract", "vision_analyze", "clarify"]
...
def _bundle(description, extras=()):
    """A `haishui-*` platform bundle: the shared core tools plus optional platform extras."""
    return _ts(description, _HAISHUI_CORE_TOOLS + list(extras))


def _core_without(*excluded, kanban=True):
    """_HAISHUI_CORE_TOOLS minus *excluded* (and, unless kanban=True, every kanban_* tool); order preserved."""
    return [t for t in _HAISHUI_CORE_TOOLS if t not in excluded and (kanban or not t.startswith("kanban_"))]
...
TOOLSETS = {
    ...
    "x_search": _ts(
        ...
        "(SuperGrok OAuth or XAI_API_KEY). Off by default; enable in `haishui tools` → "
        "X (Twitter) Search.",
        ["x_search"],
    ),
    ...
    "video_gen": _ts(
        ...
        "separate tools. Configure via ``haishui tools`` → Video Generation.",
    ...
    "browser": _ts(
        ...
        "iframes, hold-click)",
        [t for t in _HAISHUI_CORE_TOOLS if t.startswith("browser_")],
    ),
    "cronjob": _ts(
    ...
    "kanban": _ts(
        ...
        "the kanban dispatcher (HAISHUI_KANBAN_TASK env set). The dispatcher runs "
        ...
        [t for t in _HAISHUI_CORE_TOOLS if t.startswith("kanban_")],
    ),
    ...
    # config (see the non-configurable-toolset recovery loop in haishui_cli/tools_config.py).
    "coding": _ts(
    ...
    # Full Haishui toolsets (CLI + messaging platforms). All share the core tools;
    # there is deliberately no agent-callable send_message tool. haishui-acp is the
    ...
    "haishui-cli": _bundle("Full interactive CLI toolset - all default tools plus cronjob management"),

    # Mirrors haishui-cli; `haishui tools` platform config filters it down and
    # _get_platform_tools() drops _DEFAULT_OFF_TOOLSETS unless user-enabled.
    "haishui-cron": _bundle("Default cron toolset - same core tools as haishui-cli; gated by `haishui tools`"),
    "haishui-telegram": _bundle("Telegram bot toolset - full access for personal use (terminal has safety checks)"),
    "haishui-discord": _bundle(
        "Discord bot toolset - full access (terminal has safety checks via dangerous "
        "command approval)",
        ["discord", "discord_admin"],
    ),
    "haishui-whatsapp": _bundle("WhatsApp bot toolset - similar to Telegram (personal messaging, more trusted)"),
    "haishui-slack": _bundle("Slack bot toolset - full access for workspace use (terminal has safety checks)"),
    "haishui-signal": _bundle("Signal bot toolset - encrypted messaging platform (full access)"),
    "haishui-bluebubbles": _bundle("BlueBubbles iMessage bot toolset - Apple iMessage via local BlueBubbles server"),
    "haishui-homeassistant": _bundle("Home Assistant bot toolset - smart home event monitoring and control"),
    "haishui-email": _bundle("Email bot toolset - interact with Haishui via email (IMAP/SMTP)"),
    "haishui-mattermost": _bundle("Mattermost bot toolset - self-hosted team messaging (full access)"),
    "haishui-matrix": _bundle("Matrix bot toolset - decentralized encrypted messaging (full access)"),
    "haishui-dingtalk": _bundle("DingTalk bot toolset - enterprise messaging platform (full access)"),
    "haishui-feishu": _bundle("Feishu/Lark bot toolset - enterprise messaging via Feishu/Lark (full access)", _FEISHU_TOOLS),
    "haishui-weixin": _bundle("Weixin bot toolset - personal WeChat messaging via iLink (full access)"),
    "haishui-qqbot": _bundle("QQBot toolset - QQ messaging via Official Bot API v2 (full access)"),
    "haishui-wecom": _bundle("WeCom bot toolset - enterprise WeChat messaging (full access)"),
    "haishui-wecom-callback": _bundle("WeCom callback toolset - enterprise self-built app messaging (full access)"),
    "haishui-yuanbao": {
        "description": "Yuanbao Bot 元宝消息平台工具集 - 群信息、成员查询、私聊、贴纸表情",
        "tools": _HAISHUI_CORE_TOOLS + _YUANBAO_TOOLS,
        "module": "tools.yuanbao_tools",
        "includes": [],
    },
    "haishui-sms": _bundle("SMS bot toolset - interact with Haishui via SMS (Twilio)"),
    "haishui-webhook": _ts("Webhook toolset - receive and process external webhook events", _HAISHUI_WEBHOOK_SAFE_TOOLS),
    "haishui-gateway": _ts(
        "Gateway toolset - union of all messaging platform tools",
        [],
        includes=[
            "haishui-telegram", "haishui-discord", "haishui-whatsapp", "haishui-slack",
            "haishui-signal", "haishui-bluebubbles", "haishui-homeassistant", "haishui-email",
            "haishui-sms", "haishui-mattermost", "haishui-matrix", "haishui-dingtalk",
            "haishui-feishu", "haishui-wecom", "haishui-wecom-callback", "haishui-weixin",
            "haishui-qqbot", "haishui-webhook", "haishui-yuanbao",
        ],
    ),
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """A bundle's tools minus _HAISHUI_CORE_TOOLS (one level of includes).

    Disabling a `core + extras` bundle must not strip the core tools every other
    toolset shares. One `includes` pass suffices (only haishui-gateway nests
    bundles). Unknown names: full resolution minus core.
    """
    core = set(_HAISHUI_CORE_TOOLS)
...
def _plugin_platform_bundle(name: str) -> List[str]:
    """Implicit `haishui-<platform>` bundle for a registered plugin platform: core
    tools plus whatever the plugin registered under the platform name. [] otherwise."""
    if not name.startswith("haishui-"):
        return []
    platform_name = name[len("haishui-"):]
    try:
