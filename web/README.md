# Haishui Agent — Web UI

Browser-based dashboard for managing Haishui Agent configuration, API keys, and monitoring active sessions.
...
cd ../
python -m haishui_cli.main web --no-open
...
`haishui dashboard` on port 9119 serves the **built** bundle from `haishui_cli/web_dist/`, not the Vite dev server — changes in `web/src/` will not appear there until you run `npm run build` and restart the dashboard (or use `web --no-open` + Vite as above).
...
This outputs to `../haishui_cli/web_dist/`, which the FastAPI server serves as a static SPA. The built assets are included in the Python package via `pyproject.toml` package-data.
...
- **Do not introduce new `uppercase`** (the literal Tailwind class) in `haishui-agent/web/src`. Prefer `text-display` for new brand chrome. Legacy `uppercase` call sites (e.g. `components/ui/label.tsx`, `card.tsx`) remain until migrated.
...
| Wordmark | `Typography` + size/tracking only | Sidebar/mobile “Haishui Agent” — mixed case, no Mondwest, no `text-display` |
