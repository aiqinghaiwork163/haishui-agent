...
const BACKEND = process.env.HAISHUI_DASHBOARD_URL ?? "http://127.0.0.1:9119";

/**
 * In production the Python `haishui dashboard` server injects a one-shot
 * session token into `index.html` (see `haishui_cli/web_server.py`). The
 ...
 * production builds.
 */
function haishuiDevToken(): Plugin {
  const TOKEN_RE = /window\.__HAISHUI_SESSION_TOKEN__\s*=\s*"([^"]+)"/;
  const EMBEDDED_RE =
    /window\.__HAISHUI_DASHBOARD_EMBEDDED_CHAT__\s*=\s*(true|false)/;
  const INITIAL_PROFILE_RE =
    /window\.__HAISHUI_INITIAL_PROFILE__\s*=\s*("(?:\\.|[^"\\])*")/;

  return {
    name: "haishui:dev-session-token",
    apply: "serve",
    async transformIndexHtml() {
      try {
        ...
        if (!match) {
          console.warn(
            `[haishui] Could not find session token in ${BACKEND} — ` +
              `is \`haishui dashboard\` running? /api calls will 401.`,
          );
          return;
        }
        ...
        return [
          {
            ...
            injectTo: "head",
            children:
              `window.__HAISHUI_SESSION_TOKEN__="${match[1]}";` +
              `window.__HAISHUI_DASHBOARD_EMBEDDED_CHAT__=${embeddedJs};` +
              `window.__HAISHUI_INITIAL_PROFILE__=${initialProfileJs};`,
          },
        ];
      } catch (err) {
        console.warn(
          `[haishui] Dashboard at ${BACKEND} unreachable — ` +
            `start it with \`haishui dashboard\` or set HAISHUI_DASHBOARD_URL. ` +
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      "@haishui/shared": path.resolve(__dirname, "../apps/shared/src"),
    },
  ...
  server: {
    proxy: {
      ...
      // Same host as `haishui dashboard` must serve these; Vite has no
