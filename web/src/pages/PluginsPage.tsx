...
const MEMORY_PROVIDER_BUILTIN = "__haishui_memory_builtin__";
  ...
  if (!hasDetails || !setup) {
    return (
      <p className="border border-destructive/50 px-3 py-2 text-xs text-destructive">
        This provider is installed but unavailable. It may need local dependencies or a manual setup step before Haishui can activate it.
      </p>
    );
  }
  ...
  return (
    ...
    >
      <p className={isBlocked ? "text-destructive" : "text-muted-foreground"}>
        {needsDependencySetup
          ? "Finish these setup steps before Haishui can activate this provider."
      ...
      {setup.required_env.length && needsDependencySetup ? (
        <div className="grid gap-2">
          <p className="text-muted-foreground">
            Required environment values. Fill the matching fields below, or set them in the Haishui environment.
          </p>
          <div className="flex flex-wrap gap-2">
                ...
                <div className="flex flex-col gap-4 min-w-0">
                  ...
                  {!selectedMemoryName && (
                    <p className="text-xs text-muted-foreground">
                      Haishui will use the built-in MEMORY.md and USER.md files.
                    </p>
                  )}
          ...
          {catalogLoading ? (
            ...
            <p className="text-xs text-text-tertiary">
              ...
              <a
                ...
                rel="noreferrer"
              >
                {t.pluginsPage.catalogEmptyDocsLink ?? "Learn about Haishui plugins"}
              </a>
            </p>
          ) : (
    ...
    <Card className={cn(busy ? "opacity-70" : undefined)}>
      <CardContent className="flex flex-col gap-3 px-6 py-4">
        ...
        <div className="flex flex-wrap items-center gap-3 text-xs text-text-tertiary">
          ...
          {entry.requires_haishui ? (
            <span>haishui {entry.requires_haishui}</span>
          ) : null}
