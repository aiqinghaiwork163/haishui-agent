/**
 * ChatPage — embeds `haishui --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (haishui_cli/web_server.py)                          .
 *          ▼                                                              .
...
const PTY_ATTACH_TOKEN_KEY = "haishui.pty.token.chat";
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const tokenMissing =
    ...
    !window.__HAISHUI_SESSION_TOKEN__ &&
    !window.__HAISHUI_AUTH_REQUIRED__;
  ...
  const [chatPanelCollapsed, setChatPanelCollapsed] = useState(
    () => localStorage.getItem("haishui-chat-panel-collapsed") === "1",
  );
  const toggleChatPanel = useCallback(() => {
    setChatPanelCollapsed((prev) => {
      const next = !prev;
      localStorage.setItem("haishui-chat-panel-collapsed", next ? "1" : "0");
      return next;
    });
  }, []);
  ...
  useEffect(() => {
    ...
    const gated = !!window.__HAISHUI_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Haishui TUI has enabled xterm mouse-events
    ...
    //      payloads upload to HAISHUI_HOME/images then drive `/image`.
    //
    ...
    // must upload browser bytes to HAISHUI_HOME/images, then drive `/image`
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[haishui-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__HAISHUI_AUTH_REQUIRED__``) this
    ...
    void (async () => {
      ...
      // Profile-scoped chat: the PTY child gets HAISHUI_HOME pointed at the
    ...
    // forwarding them into Haishui. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="haishui-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
