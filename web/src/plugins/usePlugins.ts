...
import { api, HAISHUI_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "haishui:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${HAISHUI_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${HAISHUI_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?haishui_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-haishui-plugin", manifest.name);
