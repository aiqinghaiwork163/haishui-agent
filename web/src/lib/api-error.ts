...
export const API_UNREACHABLE_MESSAGE =
  "Haishui dashboard cannot reach the Haishui service. Is `haishui dashboard` still running?";
...
const STATUS_COPY: Record<number, string> = {
  ...
  500: "The Haishui service hit an internal error.",
  502: "The dashboard proxy could not reach the Haishui service.",
  503: "The Haishui service is not ready yet. Try again in a moment.",
  504: "The Haishui service took too long to respond.",
};

export function humanizeStatus(status: number): string {
  return STATUS_COPY[status] ?? `The Haishui service answered with an unexpected error (${status}).`;
}
