...
// (e.g. https://mission-control.tilos.com/haishui/). The Python backend
// injects ``window.__HAISHUI_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__HAISHUI_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const HAISHUI_BASE_PATH = readBasePath();
const BASE = HAISHUI_BASE_PATH;
...
declare global {
  interface Window {
    __HAISHUI_SESSION_TOKEN__?: string;
    __HAISHUI_BASE_PATH__?: string;
    ...
    __HAISHUI_AUTH_REQUIRED__?: boolean;
  }
}
const SESSION_HEADER = "X-Haishui-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__HAISHUI_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``haishui update``, ``haishui gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__HAISHUI_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__HAISHUI_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__HAISHUI_SESSION_TOKEN__`` is valid, so the next
 ...
 *
 * The dashboard's gated-mode WS auth (``haishui_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__HAISHUI_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Haishui-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``haishui_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__HAISHUI_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export async function buildWsUrl(
  ...
  return buildHaishuiWebSocketUrl({
...
export const api = {
  ...
  updateHaishui: () =>
    fetchJSON<ActionResponse>("/api/haishui/update", { method: "POST" }),
  checkHaishuiUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/haishui/update/check${force ? "?force=true" : ""}`,
    ),
...
/** Preflight for `haishui gateway migrate --multiplex` (mirrors the CLI plan JSON). */
...
export interface SkillHubSource {
  ...
  /** haishui-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``haishui update``. */
  can_update_haishui?: boolean;
  config_path: string;
  ...
  /** NS-656: disk-usage rollup for the HAISHUI_HOME volume. Absent on
   * older gateways. */
 ...
 * sample of the HAISHUI_HOME volume — no staleness dimension, so no
 * sampled_at. */
