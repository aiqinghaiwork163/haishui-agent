...
const SIDEBAR_COLLAPSED_KEY = "haishui-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="haishui-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="haishui-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateHaishui = status?.can_update_haishui === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'haishui update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "haishui update";
    return (
      t.status.updateHaishuiConfirmMessage ??
      `This will run 'haishui update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateHaishuiConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateHaishui) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateHaishui,
      runningLabel: t.status.updatingHaishui,
      spin: false,
    });
  }
  ...
  return (
      ...
      description={
        sharedGateway
          ...
          : (t.status.restartGatewayConfirmMessage ??
            "This restarts the Haishui gateway process. Connected channels and active sessions will reconnect afterward.")
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateHaishuiConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateHaishuiConfirmTitle ?? `${t.status.updateHaishui}?`}
    />
    </>
  );
}
