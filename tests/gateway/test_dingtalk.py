...
class TestSend:
    ...
    async def test_send_posts_to_webhook(self):
        ...
        assert payload["markdown"]["title"] == "Haishui"
...
class TestMentionPatterns:
    ...
    def test_pattern_matches_text(self, monkeypatch):
        adapter = _make_gating_adapter(
            monkeypatch, extra={"mention_patterns": ["^haishui"]}
        )
        assert adapter._message_matches_mention_patterns("haishui please help") is True
        assert adapter._message_matches_mention_patterns("please haishui help") is False
