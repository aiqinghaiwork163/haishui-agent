...
def test_session_store_default_db_uses_runtime_haishui_home(tmp_path, monkeypatch):
    """SessionStore must honor runtime HAISHUI_HOME when opening the default DB.

    Regression for the import-time DEFAULT_DB_PATH freeze: importing
    haishui_state before a fixture redirected HAISHUI_HOME used to pin every
    default SessionDB() at the developer's real ~/.haishui/state.db.
    """
    config = GatewayConfig()
    fake_home = tmp_path / "alt_haishui_home"
    fake_home.mkdir()
    monkeypatch.setenv("HAISHUI_HOME", str(fake_home))
...
class TestReadmeSentinel:
    ...
    def test_save_writes_readme_sentinel_first(self, tmp_path):
        ...
        assert "haishui sessions list" in raw["_README"]
