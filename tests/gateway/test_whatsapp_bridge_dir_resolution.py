...
(/opt/haishui/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable HAISHUI_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_haishui_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable HAISHUI_HOME."""
    ...
    monkeypatch.setattr(
        "haishui_constants.get_haishui_home", lambda: haishui_home
    )
    ...
    expected = haishui_home / "scripts" / "whatsapp-bridge"
