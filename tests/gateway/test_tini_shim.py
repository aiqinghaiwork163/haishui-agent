...
These run without Docker: the shim's HAISHUI_TINI_SHIM_TARGET /
HAISHUI_TINI_SHIM_WRAPPER hooks let us record the argv that would be
handed to /init.
"""
...
def _run_shim(
    ...
    env = os.environ.copy()
    env["HAISHUI_TINI_SHIM_TARGET"] = str(init)
    env["HAISHUI_TINI_SHIM_WRAPPER"] = str(wrapper)
...
def test_shim_script_is_executable_bit_friendly() -> None:
    ...
    assert "HAISHUI_TINI_SHIM_TARGET" in text
