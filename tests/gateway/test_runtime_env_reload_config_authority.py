...
HAISHUI_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env HAISHUI_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``haishui config get terminal.backend`` still says local.
    """
    haishui_home = tmp_path / ".haishui"
    haishui_home.mkdir()
    (haishui_home / "config.yaml").write_text(
    ...
    (haishui_home / ".env").write_text("TERMINAL_ENV=docker\n", encoding="utf-8")
