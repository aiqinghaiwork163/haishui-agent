...
async def test_estop_refuses_fire_before_admission_and_lifts_on_resume(adapter, monkeypatch, tmp_path):
    """`haishui pause` must stop the NAS-driven door too: 503 + Retry-After (NAS redelivers
    after `haishui resume`) and no ``claim_fire`` — a claim followed by a 503 would leave a
