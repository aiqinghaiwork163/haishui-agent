...
# ── scale_to_zero_enabled (the Labs HAISHUI_SCALE_TO_ZERO stamp, D11/Q8=A) ────
...
def test_scale_to_zero_gate_accounts_for_secondary_profile_direct_adapter(monkeypatch):
    ...
    monkeypatch.setenv("HAISHUI_SCALE_TO_ZERO", "1")
...
_FLY_ENV = {FLY_APP_NAME_ENV: "haishui-agent-stg-test", FLY_MACHINE_ID_ENV: "d891234f"}
...
def test_suspend_self_posts_suspend_for_this_machine(short_sock_dir):
    ...
    assert request.startswith(
        "POST /v1/apps/haishui-agent-stg-test/machines/d891234f/suspend HTTP/1.1\r\n"
    )
    assert "Host: flaps\r\n" in request
