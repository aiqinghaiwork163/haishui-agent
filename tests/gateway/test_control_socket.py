...
def test_windows_pipe_name_is_stable_and_home_scoped(tmp_path: Path):
    ...
    assert a.startswith(r"\\.\pipe\haishui-gateway-")
    assert a != b
...
def test_default_identify_payload_shape(home: Path, monkeypatch):
    ...
    for key in ("haishui_home", "supervisor", "kind", "start_time"):
        assert key in ident
...
def test_collect_fleet_versions_prefers_socket(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        "haishui_cli.profiles._get_profiles_root", lambda: tmp_path / "no-profiles"
    )
    # stale state file that would report a WRONG pid — socket must win
    (home / "gateway_state.json").write_text(
        json.dumps({"pid": 1, "code_sha": "stalefile", "kind": "haishui-gateway"})
    )
    monkeypatch.setattr(
...
def test_runtime_inventory_dedupes_same_pid_across_homes(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        "haishui_cli.profiles._get_profiles_root", lambda: profiles_root
    )
    monkeypatch.setattr(
        "haishui_cli.gateway._get_service_pids", lambda all_profiles=False: set()
    )
    monkeypatch.setattr(
        "haishui_cli.gateway.find_profile_gateway_processes", lambda: []
    )
    monkeypatch.setattr(
        "gateway.control_socket.identify_gateway",
