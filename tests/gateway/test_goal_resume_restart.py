...
def haishui_home(tmp_path, monkeypatch):
    ...
    # get_haishui_home() prefers the context-local override over the env
    # var, so a set_haishui_home_override() leaked by ANY earlier test in
...
class TestCliResumeRestartsWork:
    def test_resume_after_budget_exhaustion_queues_continuation(self, haishui_home):
    ...
    def test_resume_without_goal_queues_nothing(self, haishui_home):
...
class TestGatewayResumeRestartsWork:
    ...
    @pytest.mark.asyncio
    async def test_resume_without_goal_enqueues_nothing(self, haishui_home):
        runner, adapter = _make_runner()
