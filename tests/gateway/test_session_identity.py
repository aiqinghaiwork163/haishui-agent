...
(no patched predicates) against a temp ``HAISHUI_HOME`` with two served profiles.
"""
...
def mux(tmp_path, monkeypatch):
    ...
    with patch("haishui_cli.profiles.profiles_to_serve", return_value=served), \
            patch("haishui_cli.profiles.get_profile_dir", side_effect=lambda n: home if n == "default" else home / "profiles" / n), \
            patch("haishui_cli.profiles.profile_exists", return_value=True):
        yield rig
...
def test_unresolved_under_multiplex_raises_and_never_means_default(mux, tmp_path, monkeypatch):
    ...
    solo_home.mkdir()
    monkeypatch.setenv("HAISHUI_HOME", str(solo_home))
