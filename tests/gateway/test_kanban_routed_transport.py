...
from haishui_cli import kanban_db as kb, kanban_db_connect as kbc, kanban_db_notify as kbn
...
def test_route_denials_leave_events_retryable_at_claim_and_send(tmp_path, monkeypatch):
    ...
    from haishui_constants import clear_named_profile_deleted, mark_named_profile_deleted
    yuki_home = tmp_path / ".haishui" / "profiles" / "yuki"
    ...
    monkeypatch.setenv("HAISHUI_KANBAN_DB", str(tmp_path / "tied-routes.db"))
    runner.config.profile_routes = parse_profile_routes([
        dict(platform="discord", guild_id="guild", chat_id="parent", profile="other"),
...
def test_kanban_wakes_install_the_destination_runtime_scope(tmp_path, monkeypatch):
    ...
    class ScopedAdapter(RecordingAdapter):
        async def handle_message(self, event):
            ...
            observed.append((event.source.profile, get_secret("KANBAN_TEST_SECRET"), get_haishui_home()))
...
def test_removed_profile_never_wakes_under_the_primary_runtime(tmp_path, monkeypatch):
    ...
    rows = collect(runner)
    assert len(rows) == 1
    shutil.rmtree(tmp_path / ".haishui" / "profiles" / "yuki")
    asyncio.run(deliver(runner, rows))
