"""Tests for haishui_logging — centralized logging setup."""
import io
import logging
import os
import stat
...
import haishui_logging
# Use whatever RotatingFileHandler class haishui_logging actually resolved so
# the autouse fixture's isinstance checks (which strip rotating handlers
# between tests) match the real handlers on every platform. haishui_logging
...
from haishui_logging import RotatingFileHandler
...
def _reset_logging_state():
    ...
    """
    haishui_logging._logging_initialized = False
    ...
    haishui_logging._reset_queued_handlers()
    ...
    haishui_logging._install_session_record_factory()
    yield
    ...
    haishui_logging._logging_initialized = False
    haishui_logging.clear_session_context()


@pytest.fixture
def haishui_home(tmp_path, monkeypatch):
    """Provide an isolated HAISHUI_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_haishui_home from conftest,
    reading it back from the env var to avoid double-mkdir conflicts.
    """
    home = Path(os.environ["HAISHUI_HOME"])
    return home
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, haishui_home):
        log_dir = haishui_logging.setup_logging(haishui_home=haishui_home)
        assert log_dir == haishui_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, haishui_home):
        haishui_logging.setup_logging(haishui_home=haishui_home)
        root = logging.getLogger()

        agent_handlers = [
            h for h in haishui_logging._queued_file_handlers
    ...
    def test_idempotent_no_duplicate_handlers(self, haishui_home):
        haishui_logging.setup_logging(haishui_home=haishui_home)
        haishui_logging.setup_logging(haishui_home=haishui_home)  # second call — should be no-op
    ...
    def test_writes_to_agent_log(self, haishui_home):
        haishui_logging.setup_logging(haishui_home=haishui_home)

        test_logger = logging.getLogger("test_haishui_logging.write_test")
        test_logger.info("test message for agent.log")

        # Flush handlers
        haishui_logging.flush_log_queue()

        agent_log = haishui_home / "logs" / "agent.log"
        assert agent_log.exists()
        content = agent_log.read_text()
        assert "test message for agent.log" in content

    def test_profile_routing_follows_context_home(self, haishui_home, tmp_path):
        ...
        profile_home.mkdir()
        haishui_logging.setup_logging(haishui_home=haishui_home)
        assert haishui_logging.enable_profile_log_routing(
            [haishui_home, profile_home]
        ) is True
        ...
        assert "profile-routed cron record" not in (
            haishui_home / "logs" / "agent.log"
        ).read_text()

    def test_release_profile_log_handlers_closes_only_deleted_profile(self, haishui_home, tmp_path):
        ...
        haishui_logging.setup_logging(haishui_home=haishui_home)
        assert haishui_logging.enable_profile_log_routing(
            [haishui_home, deleted_home, other_home]
        ) is True

        logger = logging.getLogger("agent.profile-delete-log-release")
        token = set_haishui_home_override(deleted_home)
        try:
            logger.warning("deleted profile log handles")
        finally:
            reset_haishui_home_override(token)
        token = set_haishui_home_override(other_home)
        try:
        ...
        routers = [
            handler for handler in haishui_logging._queued_file_handlers
            if isinstance(handler, haishui_logging._ProfileRoutingFileHandler)
        ]
        ...
        assert haishui_logging.release_profile_log_handlers(deleted_home) == 2
    ...
    def test_a_second_home_routes_instead_of_stacking_an_unfiltered_handler(self, haishui_home, tmp_path):
        ...
        profile_home.mkdir()
        haishui_logging.setup_logging(haishui_home=haishui_home)
        haishui_logging.setup_logging(haishui_home=profile_home)

        assert not [h for h in haishui_logging._queued_file_handlers if isinstance(h, RotatingFileHandler)], (
        ...
        a_log = (haishui_home / "logs" / "agent.log").read_text()
    ...
    def test_setup_for_an_already_routed_home_adds_no_duplicate_writer(self, haishui_home, tmp_path):
        ...
        profile_home.mkdir()
        haishui_logging.setup_logging(haishui_home=haishui_home)
        assert haishui_logging.enable_profile_log_routing([haishui_home, profile_home]) is True
        haishui_logging.setup_logging(haishui_home=profile_home)
        ...
        assert "once please" not in (haishui_home / "logs" / "agent.log").read_text()

    def test_a_component_log_added_after_routing_is_routed_too(self, haishui_home, tmp_path):
        ...
        haishui_logging.setup_logging(haishui_home=haishui_home)
        haishui_logging.setup_logging(haishui_home=profile_home)
        haishui_logging.setup_logging(haishui_home=haishui_home, mode="gateway")
        ...
        a_log = (haishui_home / "logs" / "gateway.log").read_text()
    ...
    def test_explicit_params_override_config(self, haishui_home):
        """Explicit function params take precedence over config.yaml."""
        import yaml
        config = {"logging": {"level": "DEBUG"}}
        (haishui_home / "config.yaml").write_text(yaml.dump(config))

        haishui_logging.setup_logging(haishui_home=haishui_home, log_level="WARNING")
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, haishui_home):
        haishui_logging.setup_logging(haishui_home=haishui_home, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, haishui_home):
        haishui_logging.setup_logging(haishui_home=haishui_home, mode="cli")
        root = logging.getLogger()
    ...
    def test_gateway_log_receives_gateway_records(self, haishui_home):
        ...
        gw_log = haishui_home / "logs" / "gateway.log"
        assert gw_log.exists()
        assert "telegram connected" in gw_log.read_text()

    def test_gateway_log_rejects_non_gateway_records(self, haishui_home):
...
class TestGuiMode:
    ...
    def test_gui_log_created(self, haishui_home):
        haishui_logging.setup_logging(haishui_home=haishui_home, mode="gui")
        root = logging.getLogger()
    ...
    def test_gui_log_receives_only_gui_components(self, haishui_home):
        haishui_logging.setup_logging(haishui_home=haishui_home, mode="gui")

        logging.getLogger("haishui_cli.web_server").info("dashboard online")
        logging.getLogger("tui_gateway.ws").info("ws connected")
        logging.getLogger("gateway.run").info("gateway event")

        haishui_logging.flush_log_queue()

        gui_log = haishui_home / "logs" / "gui.log"
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, haishui_home):
        """When session context is set, log lines include [session_id]."""
        haishui_logging.setup_logging(haishui_home=haishui_home)
        haishui_logging.set_session_context("abc123")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = haishui_logging._ComponentFilter(("gateway",))
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, haishui_home):
        haishui_logging.setup_logging(haishui_home=haishui_home)
        haishui_logging.setup_verbose_logging()
        ...
        verbose_handlers = [
            ...
            and getattr(h, "_haishui_verbose", False)
        ]
...
class TestAddRotatingHandler:
    ...
    def test_no_duplicate_for_same_path(self, tmp_path):
        ...
        haishui_logging._add_rotating_handler(
            log_path,
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        handlers = [h for h in haishui_logging._queued_file_handlers if isinstance(h, RotatingFileHandler)]
        ...
        haishui_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("haishui_cli.config.is_managed", return_value=True):
                haishui_logging._add_rotating_handler(
                    log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
...
class TestWindowsConcurrentLogLockTimeout:
    ...
    def _make_logger_and_handler(self, log_path: Path):
        ...
        handler = haishui_logging._ManagedRotatingFileHandler(
    ...
    def test_helper_only_matches_windows_concurrent_lock_timeout(self):
        ...
        assert haishui_logging._is_windows_concurrent_log_lock_timeout(
            RuntimeError("Cannot acquire lock after 20 attempts")
        )
        assert not haishui_logging._is_windows_concurrent_log_lock_timeout(
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, haishui_home):
        level, max_size, backup = haishui_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, haishui_home):
        import yaml
...
class TestExternalRotationRecovery:
    ...
    def _make_handler(self, log_path: Path) -> haishui_logging._ManagedRotatingFileHandler:
        handler = haishui_logging._ManagedRotatingFileHandler(
    ...
    def _emit(self, handler: logging.Handler, msg: str) -> None:
        ...
        # Match the record factory that haishui_logging installs at import time.
        record.session_tag = ""
        handler.emit(record)
        haishui_logging.flush_log_queue()
    ...
    def test_gateway_log_attached_after_external_rotation_then_re_setup(
        ...
        """
        haishui_logging.setup_logging(haishui_home=haishui_home, mode="gateway")
        gw_path = haishui_home / "logs" / "gateway.log"
        rotated = haishui_home / "logs" / "gateway.log.1"
...
class TestSafeStderr:
    ...
    def test_wraps_non_utf8_stderr(self, monkeypatch):
        ...
        result = haishui_logging._safe_stderr()
...
class TestAsyncQueueLogging:
    ...
    def test_file_handlers_not_on_root(self, haishui_home):
        haishui_logging.setup_logging(haishui_home=haishui_home)
        root = logging.getLogger()
        ...
        queue_handlers = [
            h for h in root.handlers if getattr(h, "_haishui_queue", False)
        ]
        assert len(queue_handlers) == 1
        # The real file handlers are discoverable via the accessor.
        assert any(
            "agent.log" in getattr(h, "baseFilename", "")
            for h in haishui_logging._queued_file_handlers
        )
