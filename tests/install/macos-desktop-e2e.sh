...
# (Haishui-Setup.dmg from the website) can reach HEAD.
#
...
#              haishui-desktop-app-update  capture `haishui desktop`'s spawn,
#                                         launch the spec under Playwright,
#                                         click Update now
#              haishui-update              CLI update from the installed venv
...
#     --update-method open-app-update|haishui-desktop-app-update
...
case "$UPDATE_METHOD" in
  open-app-update|haishui-desktop-app-update|haishui-update|installer-script|installer-script+desktop) ;;
...
EOF
  ...
  export INSTALL_DIR="$HAISHUI_HOME/haishui-agent"
}
...
phase_stage() {
  ...
  touch "$HAISHUI_HOME/.skip_upstream_prompt"
...
find_installed_app() {
  ...
  local cand
  for cand in \
    "$INSTALL_DIR/apps/desktop/release/mac-arm64/Haishui.app" \
    "$INSTALL_DIR/apps/desktop/release/mac/Haishui.app" \
    "/Applications/Haishui.app"; do
...
phase_install() {
  ...
  . "$STATE"
  arm_redirect
  step "installing OLD ($OLD_REF) via the published Haishui-Setup.dmg"

  local dmg="$WORK_ROOT/Haishui-Setup.dmg"
  ...
  # "Install Haishui" button with native input.
  local rc=0
  ...
  log_group "Haishui-Setup (dmg bootstrap) transcript" "$LOG_DIR/bootstrap-install.log"
  ...
  "$haishui" --version 2>&1 | ts_prefix > "$LOG_DIR/version-old.log" || fail "haishui --version failed after install"
  ok "haishui --version works: $(head -c 120 "$LOG_DIR/version-old.log" | tr -d '\n')"
  find_installed_app >/dev/null || fail "no installed Haishui.app after the dmg bootstrap"
...
phase_update() {
  ...
  case "$UPDATE_METHOD" in
    haishui-update)
      ...
      # update subcommand only in later releases; ask the installed haishui.
      local haishui="$INSTALL_DIR/venv/bin/haishui"
      local update_cmd=("$haishui" update)
      ...
      [ "$rc" -eq 0 ] || fail "haishui update exited $rc; transcript above"
      ;;
    installer-script)
    ...
    installer-script+desktop)
      ...
      for cand in \
        "$INSTALL_DIR/apps/desktop/release/mac-arm64/Haishui.app" \
        "$INSTALL_DIR/apps/desktop/release/mac/Haishui.app"; do
        [ -d "$cand" ] && { head_app="$cand"; break; }
      done
      [ -n "$head_app" ] || fail "no built Haishui.app under the checkout after the +desktop update"
    ...
    open-app-update)
      ...
      # which is exactly what the app's children (git, haishui update) need.
      local app app_bin
...
PYEOF
    ...
    haishui-desktop-app-update)
      ...
      [ "$rc" -eq 0 ] || fail "haishui desktop exited $rc during launch capture"
      [ -f "$spec.captured" ] || fail "haishui desktop exited 0 but no launch was captured"
  ...
  mkdir -p "$ildest"
  cp -R "$HOME_SANDBOX/.haishui/logs" "$ildest/haishui-logs" 2>/dev/null || true
  local ud="$HOME_SANDBOX/Library/Application Support/Haishui"
  ...
  "$INSTALL_DIR/venv/bin/haishui" --version 2>&1 | ts_prefix > "$LOG_DIR/version-head.log" \
    || fail "haishui --version failed after update"
  ok "haishui --version works post-update"
