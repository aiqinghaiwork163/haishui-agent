// drive-update.cjs — launch the INSTALLED Haishui.exe (real Electron desktop
...
//
//   node <this file> <path-to-Haishui.exe> <proof-dir>
//
...
if (!exePath || !proofDir) {
  console.error('usage: node drive-update.cjs <Haishui.exe> <proof-dir>')
  process.exit(1)
}
...
async function main() {
  ...
  const app = await _electron.launch({
    ...
    // Inherit the driver's env: HAISHUI_HOME (isolated install) and
  ...
  // The real backend (`haishui serve`) is booting underneath; give it time.
  ...
  // buttonless boot-progress card ("Starting Haishui... 86%"), then the
  ...
  if (!visible) {
    ...
    const status = await page.evaluate(() =>
      window.haishuiDesktop?.updates?.check?.() ?? Promise.resolve('no updates.check bridge')
  ...
  // The "Updating Haishui — this window will close" overlay should appear,
  ...
  // the staged binary), which claims HAISHUI_HOME/.haishui-update-in-progress
  ...
  // proved the desktop had exited and `haishui update` was already running).
  //
  ...
  const haishuiHome = process.env.HAISHUI_HOME
  const markerPath = haishuiHome ? path.join(haishuiHome, '.haishui-update-in-progress') : null
  const resultPath = haishuiHome ? path.join(haishuiHome, '.haishui-update-result.json') : null
