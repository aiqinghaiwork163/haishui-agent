...
; Drive the REAL Haishui-Setup.exe (Tauri bootstrap installer) window:
; click Install, wait for the install to finish, click Launch, and wait for
; the real Haishui.exe (Electron desktop) window to appear.
;
; Adapted from @ethernet8023's e2e/windows/install-haishui-desktop.ahk
...
setupExe := A_Args.Length >= 2 ? A_Args[2] : "Haishui-Setup.exe"
...
Log("Launch clicked; waiting for the Haishui desktop app window")
...
try {
    ...
    throw Error("Haishui.exe real-sized window did not appear within 300s of clicking Launch")
}
