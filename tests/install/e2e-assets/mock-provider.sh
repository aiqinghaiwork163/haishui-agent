...
#   mock_start <workroot>      start, write config into $HAISHUI_HOME
#   mock_stop                  kill the background server
#
# Requires: ASSETS (e2e-assets dir), LOG_DIR, HAISHUI_HOME, ok/fail helpers.
...
mock_start() {
  ...
  cat > "$HAISHUI_HOME/config.yaml" <<EOF
model:
  default: mock-model
  provider: mock
providers:
...
EOF
  ...
  ok "provider 'mock' configured in $HAISHUI_HOME (api $url/v1)"
}
