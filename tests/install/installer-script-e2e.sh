...
#              install landed on OLD with a working `haishui`
#   update     advance served main to HEAD, apply ONE update method, assert
#              the checkout landed on HEAD with a working `haishui`
#
# Usage:
#   tests/install/installer-script-e2e.sh --update-method haishui-update|installer-script|installer-script+desktop
...
case "$UPDATE_METHOD" in
  haishui-update|installer-script|installer-script+desktop|haishui-desktop-app-update) ;;
  *) echo "error: --update-method must be haishui-update, installer-script, installer-script+desktop or haishui-desktop-app-update, got '$UPDATE_METHOD'" >&2; exit 1 ;;
esac
...
WORK_ROOT="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/haishui-installer-script-e2e"
LOG_DIR="${HAISHUI_E2E_LOG_DIR:-$WORK_ROOT/logs}"
...
# Isolated HOME: the runner's real one may carry a preinstalled haishui or a
# developer config, and old installer scripts hardcode $HOME/.haishui (the
# HAISHUI_HOME env override is newer than tags we sample). GIT_CONFIG_GLOBAL
# above keeps working -- an explicit path wins over $HOME/.gitconfig.
export HOME="$WORK_ROOT/home"
mkdir -p "$HOME/.local/bin"
export PATH="$HOME/.local/bin:$PATH"
export HAISHUI_HOME="$HOME/.haishui"
...
assert_desktop_artifact() {
  ...
  local cand
  for cand in \
    "$release_dir/linux-unpacked/Haishui" \
    "$release_dir/linux-unpacked/haishui" \
    "$release_dir/mac-arm64/Haishui.app" \
    "$release_dir/mac/Haishui.app"; do
...
assert_checkout() {
  ...
  ok "checkout is $2 ($1)"
  local haishui="$INSTALL_DIR/venv/bin/haishui"
  [ -x "$haishui" ] || fail "no haishui console script at $haishui"
  "$haishui" --version 2>&1 | ts_prefix > "$LOG_DIR/version-$2.log" \
    || fail "haishui --version failed after $2; log in $LOG_DIR/version-$2.log"
  ok "haishui --version works: $(head -c 120 "$LOG_DIR/version-$2.log" | tr -d '\n')"
}

smoke_desktop() {
  # $1: label (old|head). Prove the installed CLI can produce the desktop
  # app: `haishui desktop --build-only` runs the full desktop pipeline
  # (workspace install, renderer build, stamp write) and stops before the
  # launch -- the same call `haishui update` itself makes. Probe the
  # INSTALLED haishui for the flag rather than assuming this checkout's
  # surface: sampled OLD releases may predate `haishui desktop` or
  # --build-only entirely, and for them the phase skips, loudly.
  local haishui="$INSTALL_DIR/venv/bin/haishui"
  if ! "$haishui" desktop --help 2>/dev/null | grep -qF -- --build-only; then
    ok "haishui desktop --build-only not supported at $1; skipping desktop smoke"
    return 0
  fi
  local rc=0
  (cd "$INSTALL_DIR" && "$haishui" desktop --build-only < /dev/null 2>&1 \
    | ts_prefix > "$LOG_DIR/desktop-smoke-$1.log") || rc=$?
  log_group "haishui desktop --build-only ($1) transcript" "$LOG_DIR/desktop-smoke-$1.log"
  [ "$rc" -eq 0 ] || fail "haishui desktop --build-only ($1) exited $rc; transcript above"
  ok "haishui desktop --build-only works at $1"
...
case "$UPDATE_METHOD" in
  haishui-update)
    ...
    # installed haishui; older ones read the prompt from stdin, so close it.
    HAISHUI="$INSTALL_DIR/venv/bin/haishui"
    if "$HAISHUI" update --help 2>&1 | grep -qF -- --yes; then
      update_cmd=("$HAISHUI" update --yes)
    else
      update_cmd=("$HAISHUI" update)
    fi
    rc=0
    (cd "$INSTALL_DIR" && "${update_cmd[@]}" < /dev/null 2>&1 | ts_prefix > "$LOG_DIR/update.log") || rc=$?
    log_group "haishui update transcript" "$LOG_DIR/update.log"
    [ "$rc" -eq 0 ] || fail "haishui update exited $rc; transcript above, log at $LOG_DIR/update.log"
    ;;
  installer-script)
  ...
  haishui-desktop-app-update)
    # The real user surface: `haishui desktop` launches the app, the user
    ...
    # suite's own) is configured into HAISHUI_HOME exactly like the dev:mock
    ...
    step "capturing the haishui desktop launch spec (build runs for real)"
    rc=0
    (cd "$INSTALL_DIR" && \
      PYTHONPATH="$ASSETS/launch-capture${PYTHONPATH:+:$PYTHONPATH}" \
      HAISHUI_E2E_CAPTURE_LAUNCH="$SPEC" \
      "$HAISHUI" desktop < /dev/null 2>&1 | ts_prefix > "$LOG_DIR/desktop-launch-capture.log") || rc=$?
    log_group "haishui desktop (launch capture) transcript" "$LOG_DIR/desktop-launch-capture.log"
    [ "$rc" -eq 0 ] || fail "haishui desktop exited $rc during launch capture; transcript above"
    # Exit 0 without a capture means a version that never reached its
    # launch - that must fail loudly, not pass as a no-op.
    [ -f "$SPEC.captured" ] || fail "haishui desktop exited 0 but no launch was captured at $SPEC"
...
mkdir -p "$ildest"
cp -R "$HAISHUI_HOME/logs" "$ildest/haishui-logs" 2>/dev/null || true
if [ -n "${XDG_DATA_HOME:-}" ]; then
  cp -R "$XDG_DATA_HOME/haishui/logs" "$ildest/desktop-userdata-logs" 2>/dev/null || true
fi
cp "$HAISHUI_HOME/.haishui-update-result.json" "$ildest" 2>/dev/null || true
ls -la "$HAISHUI_HOME" > "$ildest/haishui-home-ls.txt" 2>/dev/null || true
