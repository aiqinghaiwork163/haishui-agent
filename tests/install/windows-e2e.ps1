...
# ============================================================================
# Proves, on a real Windows machine, that a user who installs Haishui the way
...
# actually touches:
#
#   INSTALL   - downloads the production Haishui-Setup.exe from the website,
#               launches it HEADED, and AutoHotkey clicks Install, waits,
#               then clicks Launch. The real Electron Haishui.exe must appear.
...
#   installer's `git clone` and `haishui update`'s `git fetch origin`
...
param(
    ...
    # open-app-update (from a desktop-installer install) and haishui-update /
    ...
    # they land.
    [ValidateSet("open-app-update", "haishui-desktop-app-update", "haishui-update", "desktop-installer@latest", "installer-script", "installer-script+desktop")]
    [string]$Route = "open-app-update",

    # The OLD version: the ref served as `main` while the installer runs,
    # i.e. what the user starts on. The published Haishui-Setup.exe carries
...
$HaishuiHome  = Join-Path $WorkRoot "haishui-home"
$InstallDir  = Join-Path $HaishuiHome "haishui-agent"
...
function Invoke-Git([string[]]$GitArgs) {
    ...
    # if haishui ever adds a git command that calls something with ^ this will break, lol.
...
function Set-GitRedirect {
    # we redirect to our own repo so we can play around with what commit haishui thinks we're on.
...
function Get-DesktopExe {
    foreach ($c in @(
        (Join-Path $InstallDir "apps\desktop\release\win-unpacked\Haishui.exe"),
        (Join-Path $InstallDir "apps\desktop\release\win-arm64-unpacked\Haishui.exe")
    )) {
...
# Install-side state snapshot, taken BEFORE Test-HaishuiRuns can throw: on
...
function Save-InstallSideState([string]$Label) {
    ...
    $logsDir = Join-Path $HaishuiHome "logs"
    if (Test-Path -LiteralPath $logsDir) {
        Copy-Item $logsDir (Join-Path $dest "haishui-logs") -Recurse -Force -ErrorAction SilentlyContinue
    }
    $resultFile = Join-Path $HaishuiHome ".haishui-update-result.json"
    ...
    Get-ChildItem -LiteralPath $HaishuiHome -ErrorAction SilentlyContinue |
        Select-Object Name, Length, LastWriteTime |
        Format-Table -AutoSize | Out-String |
        Set-Content (Join-Path $dest "haishui-home-ls.txt")
}

function Test-HaishuiRuns([string]$Label) {
    Save-InstallSideState $Label
    $haishuiExe = Join-Path $InstallDir "venv\Scripts\haishui.exe"
    Assert-True (Test-Path -LiteralPath $haishuiExe) "$Label -- venv\Scripts\haishui.exe exists"
    & $haishuiExe --version 2>&1 | ForEach-Object { Write-Host "    haishui --version| $_" }
    Assert-True ($LASTEXITCODE -eq 0) "$Label -- haishui --version exits 0"
}
...
function Invoke-RefInstaller {
    ...
    $flags = @("-SkipSetup", "-HaishuiHome", $HaishuiHome, "-InstallDir", $InstallDir)
...
function Invoke-HaishuiUpdate {
    # The venv updater. --yes reaches the update subcommand only in later
    # releases; ask the installed binary, never parse its source.
    $haishuiExe = Join-Path $InstallDir "venv\Scripts\haishui.exe"
    $updateArgs = @("update")
    $prevEap = $ErrorActionPreference; $ErrorActionPreference = "Continue"
    $helpText = & $haishuiExe update --help 2>&1 | Out-String
    if ($helpText -match '--yes') { $updateArgs += "--yes" }
    New-Item -ItemType Directory -Path (Join-Path $WorkRoot "logs") -Force | Out-Null
    $log = Join-Path $WorkRoot "logs\update.log"
    Push-Location $InstallDir
    try {
        & $haishuiExe @updateArgs 2>&1 | Add-TsPrefix | Out-File -Encoding UTF8 $log
        $updateExit = $LASTEXITCODE
    } finally {
        Pop-Location
        $ErrorActionPreference = $prevEap
    }
    Write-LogGroup "haishui update transcript" $log
    Assert-True ($updateExit -eq 0) "haishui update exited $updateExit (expected 0)"
}

function Invoke-HaishuiDesktopAppUpdate([string]$TargetSha) {
    # The haishui-desktop launch surface: `haishui desktop` runs its whole
    ...
    $prevCap = $env:HAISHUI_E2E_CAPTURE_LAUNCH
    $env:PYTHONPATH = if ($prevPy) { "$capDir;$prevPy" } else { $capDir }
    $env:HAISHUI_E2E_CAPTURE_LAUNCH = $spec
    $prevEap = $ErrorActionPreference; $ErrorActionPreference = "Continue"
    Push-Location $InstallDir
    try {
        & $haishuiExe desktop 2>&1 | Add-TsPrefix | Out-File -Encoding UTF8 $log
        $capExit = $LASTEXITCODE
    } finally {
        Pop-Location
        $ErrorActionPreference = $prevEap
        $env:PYTHONPATH = $prevPy
        $env:HAISHUI_E2E_CAPTURE_LAUNCH = $prevCap
    }
    Write-LogGroup "haishui desktop (launch capture) transcript" $log
    Assert-True ($capExit -eq 0) "haishui desktop exited 0 during launch capture"
    ...
    try {
        & $node "launch-from-spec.mjs" --spec $spec `
            --result (Join-Path $HaishuiHome ".haishui-update-result.json") `
    ...
    Assert-True ($driveExit -eq 0) "app driven via captured haishui desktop spec; update completed"
}
...
function Stop-HaishuiAppProcesses([string]$Label) {
    # Close the desktop app the blunt way between phases (a user quitting).
    # Only Haishui.exe (Electron) -- never haishui.exe (the venv CLI shim).
    $procs = @(Get-Process -Name "Haishui" -ErrorAction SilentlyContinue)
    foreach ($p in $procs) {
        try { Stop-Process -Id $p.Id -Force -ErrorAction SilentlyContinue } catch {}
    }
    if ($procs.Count -gt 0) {
        Write-Host "  [$Label] stopped $($procs.Count) Haishui.exe process(es)"
        Start-Sleep -Seconds 3
    }
}

function Get-ManagedNode {
    # `haishui update`/desktop builds use the Haishui-managed Node; use the same
    # one to run the Playwright driver so no system Node is required.
    $candidates = @(
        (Join-Path $HaishuiHome "node\node.exe"),
        (Join-Path $HaishuiHome "bin\node\node.exe"),
...
# ----------------------------------------------------------------------------
# Phase: install-gui -- website Haishui-Setup.exe, headed, AHK-driven
# ----------------------------------------------------------------------------
function Invoke-PhaseInstallGui {
    ...
    Write-Step "$($Mode.ToUpper()) (GUI): Haishui-Setup.exe from the website, headed, AHK clicks"
    ...
    $setupExe = Join-Path $WorkRoot "Haishui-Setup.exe"
    if (-not (Test-Path -LiteralPath $setupExe)) {
        Write-Host "  downloading $SetupExeUrl"
        Invoke-WebRequest -Uri $SetupExeUrl -OutFile $setupExe
    }
    Assert-True ((Get-Item $setupExe).Length -gt 1MB) "Haishui-Setup.exe downloaded ($([math]::Round((Get-Item $setupExe).Length / 1MB, 1)) MB)"
    ...
    Remove-Item Env:HAISHUI_SETUP_DEV_REPO_ROOT -ErrorAction SilentlyContinue
    New-Item -ItemType Directory -Path $HaishuiHome -Force | Out-Null
    ...
    try {
        ...
        Write-Host "  Haishui-Setup.exe launched (pid $($installer.Id))"

        # Drive it: Install click -> wait -> Launch click -> Haishui.exe window.
        ...
        $ahk = Start-Process -FilePath $ahkExe `
            -ArgumentList (Join-Path $AhkDir "install-and-launch.ahk"), $ahkLog, "Haishui-Setup.exe", (Join-Path $HaishuiHome "logs\bootstrap-installer.log") `
            -PassThru
        ...
        # actually be running.
        Assert-True ($null -ne (Get-Process -Name "Haishui" -ErrorAction SilentlyContinue)) "Haishui.exe process is running (installer Launch hand-off worked)"

        # Installer should have exited after Launch.
        if (-not $installer.HasExited) {
            Start-Sleep -Seconds 10
        }
        Assert-True $installer.HasExited "Haishui-Setup.exe exited after Launch"
    }
    finally {
        Stop-DesktopRecorder $recorder (Join-Path $proof "desktop-frames")
        # Surface the installer's own log win or lose, full and folded.
        $bootLog = Join-Path $HaishuiHome "logs\bootstrap-installer.log"
    ...
    Stop-HaishuiAppProcesses "post-install"
    ...
    Test-HaishuiRuns "post-$Mode-gui"
    Assert-True ($null -ne (Get-DesktopExe)) "packaged Desktop Haishui.exe exists"
...
function Invoke-GuiUpdateDesktopRoute([string]$TargetSha) {
    ...
    Assert-True ($null -ne $desktopExe) "packaged Haishui.exe present before update"
    ...
    try {
        ...
        # `haishui update` + desktop rebuild + relaunch. Which updater depends
        # on the installed checkout, and BOTH are production paths:
        #   * checkouts shipping scripts/desktop-update.ps1 -> that script,
        #     which writes .haishui-update-result.json on every exit;
        #   * older checkouts -> the staged haishui-setup.exe --update flow,
        #     which does NOT write the result JSON.
        # So: poll for COMPLETION = (result JSON) OR (checkout reached the
        # target sha AND the marker is gone). The sha/marker/haishui/relaunch
        ...
        $updateLog = Join-Path $HaishuiHome "logs\update.log"
        $updateLogPos = 0
        ...
        Test-HaishuiRuns "post-update"
        Assert-True ($null -ne (Get-DesktopExe)) "Haishui.exe still present after update"
        ...
        Write-Host "  waiting for the relaunched Haishui.exe ..."
        $rDeadline = (Get-Date).AddMinutes(5)
        $relaunched = $null
        while ((Get-Date) -lt $rDeadline) {
            $relaunched = Get-Process -Name "Haishui" -ErrorAction SilentlyContinue
            if ($relaunched) { break }
        ...
        # Foreground the relaunched Haishui window so the proof screenshot
        ...
        try {
            $mainProc = Get-Process -Name "Haishui" -ErrorAction SilentlyContinue |
...
'@ -ErrorAction SilentlyContinue
        ...
        Stop-DesktopRecorder $recorder (Join-Path $proof "desktop-frames")
        $handoffLog = Join-Path $HaishuiHome "logs\desktop-update-handoff.log"
        ...
        Stop-HaishuiAppProcesses "post-update"
    }
}

function Invoke-PhaseInstall {
    # Dispatch on the install axis. Each arm ends with the same contract:
    # checkout at OLD, haishui runs, and state carries how OLD landed so any
    ...
    switch ($InstallMethod) {
        ...
        "installer-script" {
            ...
            Test-HaishuiRuns "post-install-script"
        }
        "installer-script+desktop" {
            ...
            Assert-True ((Get-InstalledHead) -eq $state.old) "installed checkout is at OLD"
            Test-HaishuiRuns "post-install-script-desktop"
...
function Invoke-PhaseUpdate {
    ...
    New-Item -ItemType File -Path (Join-Path $HaishuiHome ".skip_upstream_prompt") -Force | Out-Null
    ...
    switch ($Route) {
        ...
        "haishui-desktop-app-update" {
            Invoke-HaishuiDesktopAppUpdate $state.current
        }
        "haishui-update" {
            Invoke-HaishuiUpdate
        }
        "installer-script" {
        ...
        "desktop-installer@latest" {
            # A user re-downloading Haishui-Setup.exe and clicking Install over
...
function Invoke-CheckedPhaseUpdate {
    ...
    foreach ($oldLog in @((Join-Path $WorkRoot "logs\update.log"), (Join-Path $HaishuiHome "logs\desktop.log"))) {
