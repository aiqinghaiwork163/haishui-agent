...
def test_root_session_no_compression(db):
    ...
    assert prov["currentHaishuiSessionId"] == "root1"
    assert prov["rootHaishuiSessionId"] == "root1"
    assert prov["parentHaishuiSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_haishui_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentHaishuiSessionId"] == "old"
    assert prov["rootHaishuiSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousHaishuiSessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootHaishuiSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"haishui"}
    assert "sessionProvenance" in meta["haishui"]
    assert meta["haishui"]["sessionProvenance"]["currentHaishuiSessionId"] == "root1"
