...
class TestCreateSession:
    ...
    def test_register_task_cwd_translates_windows_drive_for_wsl_tools(self, monkeypatch):
        ...
        monkeypatch.setattr("haishui_constants._wsl_detected", True)
        monkeypatch.setattr(
    ...
    def test_make_agent_prefers_passed_toolsets_over_config_servers(self, monkeypatch):
        ...
        monkeypatch.setattr("haishui_cli.runtime_provider.resolve_runtime_provider", lambda **_kw: {})
        monkeypatch.setattr("haishui_cli.mcp_startup.ensure_mcp_discovery_before_agent_build", lambda **_kw: None)
        monkeypatch.setattr("acp_adapter.session._register_task_cwd", lambda task_id, cwd: None)
        manager = SessionManager(db=None)

        manager._make_agent(session_id="fresh", cwd=".")
        manager._make_agent(
            session_id="rebuilt", cwd=".", enabled_toolsets=["haishui-acp", "mcp-acp-server"], disabled_toolsets=["browser"],
        )

        assert (seen[0]["enabled_toolsets"], seen[0]["disabled_toolsets"]) == (["haishui-acp", "mcp-cfg-server"], None)
        assert (seen[1]["enabled_toolsets"], seen[1]["disabled_toolsets"]) == (["haishui-acp", "mcp-acp-server"], ["browser"])
...
class TestSymlinkAliasNormalization:
    ...
    def test_missing_path_keeps_lexical_normalization(self):
        ...
        assert acp_session._normalize_cwd_for_compare(
            "/nonexistent-haishui-test/x/../y"
        ) == "/nonexistent-haishui-test/y"
