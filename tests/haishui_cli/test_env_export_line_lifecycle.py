...
``~/.haishui/.env``. ``load_env()`` parses the export prefix (#6659), so every
...
def test_classic_pat_save_via_endpoint_succeeds(haishui_home):
...
def test_remove_export_prefixed_token(haishui_home):
    """DELETE must clear an ``export KEY=...`` line, not 404 on it."""
    _write_env_raw(haishui_home, f"export GITHUB_TOKEN={OLD_PAT}\n")
    ...
    env_text = haishui_home.joinpath(".env").read_text(encoding="utf-8")
...
def test_update_export_prefixed_token_does_not_duplicate(haishui_home):
...
def test_plain_line_save_and_remove_still_work(haishui_home):
    """Sanity: the ordinary KEY= path is unchanged."""
    from haishui_cli.config import load_env, remove_env_value, save_env_value
...
def test_export_line_with_comment_untouched(haishui_home):
...
def test_remove_token_written_with_spaces_around_equals(haishui_home):
    ...
    """
    _write_env_raw(haishui_home, f"GITHUB_TOKEN = {OLD_PAT}\n")
...
def test_spaced_token_rotate_then_delete_does_not_resurrect(haishui_home):
...
def test_writer_matches_exactly_what_load_env_accepts(haishui_home):
    """Parity: the writers must recognise a line iff ``load_env()`` does."""
    from haishui_cli.config import _env_line_defines_key, load_env
    ...
    for line, expected in [
        ...
        _write_env_raw(haishui_home, line + "\n")
