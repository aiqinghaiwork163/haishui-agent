...
(``/model`` picker, ``haishui doctor``, dashboard cards). When that read ran ``pool.select()`` it
...
from haishui_cli.auth import AuthError, DEFAULT_CODEX_BASE_URL, get_codex_auth_status
...
def _pool_only_codex_home(tmp_path, monkeypatch, *, access_tokens: list):
    """HAISHUI_HOME whose only Codex credentials live in ``credential_pool.openai-codex``; the token
    endpoint is a transient failure (the credential itself is still good)."""
    import haishui_cli.auth as auth
    import haishui_cli.codex_models as codex_models
