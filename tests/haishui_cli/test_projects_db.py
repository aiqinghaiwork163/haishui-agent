"""Tests for the per-profile Projects store (haishui_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Haishui Agent", folders=["/tmp/haishui"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "haishui-agent"
    assert proj.name == "Haishui Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/haishui"
    assert [f.path for f in proj.folders] == ["/tmp/haishui"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "haishui-agent").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' HAISHUI_HOME.
