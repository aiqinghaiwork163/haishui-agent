...
``haishui*.exe`` shims aside (``haishui.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``haishui`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("haishui", "haishui-agent", "haishui-acp", "haishui-gateway"):
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "haishui.exe" in names, "haishui.exe must be restored after a no-op install"
    assert "haishui-acp.exe" in names
    assert "haishui-gateway.exe" in names
    assert (scripts / "haishui.exe").read_bytes() == b"MZ-old-haishui"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "haishui.exe").read_bytes() == b"MZ-new-haishui"
...
# ---------------------------------------------------------------------------
# haishui_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "haishui.exe" in names, "haishui.exe must be restored after a no-op recovery install"
    assert (scripts / "haishui.exe").read_bytes() == b"MZ-old-haishui"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "haishui.exe").write_bytes(b"MZ-new-haishui")
