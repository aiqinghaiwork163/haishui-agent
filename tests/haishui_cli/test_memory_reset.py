"""Tests for the `haishui memory reset` CLI command.
...
- Profile-scoped reset (uses HAISHUI_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake HAISHUI_HOME with memory files."""
    haishui_home = tmp_path / ".haishui"
    memories = haishui_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("HAISHUI_HOME", str(haishui_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nHaishui repo is at ~/.haishui/haishui-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `haishui memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        haishui_home = tmp_path / ".haishui"
        (haishui_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("HAISHUI_HOME", str(haishui_home))
