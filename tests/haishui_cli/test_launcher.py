"""Tests for the top-level `./haishui` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./haishui` should use `haishui_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "haishui"
    called = []

    fake_main_module = types.ModuleType("haishui_cli.main")

    def fake_main():
        called.append("haishui_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "haishui_cli.main", fake_main_module)
    ...
    assert called == ["haishui_cli.main"]
