...
def served_root(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HAISHUI_PROFILE", "probe")
    ...
    monkeypatch.setattr("haishui_cli.gateway.find_gateway_pids", lambda: [])
    ...
    monkeypatch.setattr("gateway.status._read_process_cmdline", lambda pid: "haishui gateway run")
...
def test_status_preserves_profile_health_contract(served_root, capsys, monkeypatch, mode):
    ...
    if mode == "local":
        monkeypatch.setattr("haishui_cli.gateway.find_gateway_pids", lambda: [os.getpid()])
    if mode == "external":
    ...
    if mode in {"missing", "stale"}:
        assert "haishui --profile default gateway restart" in output
    if mode == "missing":
    ...
    if mode in {"disabled", "excluded", "unrelated_pid"}:
        ...
        assert "haishui gateway install" in output
        assert "sudo haishui gateway install --system" in output
        assert "haishui gateway run" in output
        assert "haishui --profile default gateway restart" in output
    if mode == "external":
...
def test_satellite_list_and_create_require_own_heartbeat(served_root, capsys, monkeypatch, heartbeat):
    ...
    for output in (created, listed):
        assert ("Check status:  haishui cron status" in output) == (heartbeat != "fresh")
    cron.cron_status()
...
def test_doctor_bounds_persisted_fire_errors(served_root, capsys, detail):
    ...
    if detail:
        ...
        assert f"haishui cron run {records[0]['id']}" in line
...
def test_doctor_reports_persisted_dispatch_health(served_root, capsys, dispatch):
    ...
    assert "Review the findings above, then run `haishui cron doctor` again." in output
