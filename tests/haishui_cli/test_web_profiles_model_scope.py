...
``_write_profile_model`` used to enter only the HAISHUI_HOME override. Once the dashboard
...
def homes(tmp_path, monkeypatch):
    """A throwaway HAISHUI_HOME whose named profile carries its own ``.env`` credential.
...
def probe(monkeypatch):
    ...
    import haishui_cli.models_validate as mv
...
def test_model_pick_resolves_key_env_from_profile_scope(client, homes, probe):
    ...
    from haishui_cli.web_server_profiles import _haishui_home_scope
    with _haishui_home_scope(homes[1]):
...
def test_model_pick_for_default_from_named_profile_launch(homes, probe, monkeypatch):
    ...
    root, demo = homes
    monkeypatch.setenv("HAISHUI_HOME", str(demo))
    (root / ".env").write_text("ACME_RELAY_KEY=root-key\n", encoding="utf-8")
    from haishui_cli.config import invalidate_env_cache, load_config
    from haishui_cli.web_server_profiles import _haishui_home_scope
    invalidate_env_cache()
    from haishui_cli import web_server
    ...
    with _haishui_home_scope(root):
        assert (load_config().get("model") or {}).get("default") == "acme/mini"
    with _haishui_home_scope(demo):
