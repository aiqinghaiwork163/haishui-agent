"""Tests for haishui_cli.managed_uv — one path, no guessing."""
...
from haishui_cli.managed_uv import _CandidateStageError
...
def _runtime_info(
    ...
    from haishui_cli.sqlite_runtime import SQLiteRuntimeInfo
...
def _RRR(status):
    ...
    """
    from haishui_cli.managed_uv import RuntimeRepairResult
...
class TestManagedUvPath:
    ...
    def test_posix(self, tmp_path):
        with patch("haishui_cli.managed_uv.get_haishui_home", return_value=tmp_path):
            from haishui_cli.managed_uv import managed_uv_path
...
class TestMacOSManagedPythonSigning:
    def test_signs_with_stable_identifier_and_verifies(self, tmp_path, monkeypatch):
        import haishui_cli.managed_uv as managed_uv
        ...
        assert calls[0][0] == [
            ...
            "--timestamp=none",
            "--identifier",
            "com.nousresearch.haishui.managed-python",
            "--requirements",
            '=designated => identifier "com.nousresearch.haishui.managed-python"',
            str(python),
        ]
        assert calls[1][0] == [
            "/usr/bin/codesign",
...
class TestPipInstallHint:
    ...
    def test_names_the_managed_uv_when_present(self, tmp_path):
        ...
        _make_executable(uv)
        with patch("haishui_cli.managed_uv.get_haishui_home", return_value=tmp_path):
            from haishui_cli.managed_uv import pip_install_hint
...
class TestEnsureUv:
    ...
    def test_installs_if_missing(self, tmp_path):
        uv = tmp_path / "bin" / _UV_BINARY_NAME
        with patch("haishui_cli.managed_uv.get_haishui_home", return_value=tmp_path), \
             patch("haishui_cli.managed_uv.repair_vulnerable_runtime", return_value=_RRR("not-applicable")), \
             patch("haishui_cli.managed_uv._uv_version", return_value="uv 0.1.2"), \
             patch("haishui_cli.managed_uv._install_uv") as mock_install:
    ...
    def test_install_reports_runtime_repair_to_observer(self, tmp_path):
        from haishui_cli.managed_uv import (
            RuntimeRepairResult,
            ensure_uv,
        )
        ...
        observed = []
        with patch(
            "haishui_cli.managed_uv.get_haishui_home",
            return_value=tmp_path,
        ), patch(
            "haishui_cli.managed_uv._install_uv",
            side_effect=fake_install,
        ), patch(
            "haishui_cli.managed_uv._uv_version",
            return_value="uv 0.1.2",
        ), patch(
            "haishui_cli.managed_uv.repair_vulnerable_runtime",
            return_value=repair,
        ):
...
class TestEnsureUvUpdateBoundary:
    ...
    ``haishui update`` runs the call site from the old, already-imported
    ``haishui_cli.main`` against the freshly pulled ``managed_uv``. A release
    ...
    def test_failure_unpacks_without_raising(self, tmp_path):
        with patch("haishui_cli.managed_uv.get_haishui_home", return_value=tmp_path), \
             patch("haishui_cli.managed_uv.repair_vulnerable_runtime", return_value=_RRR("not-applicable")), \
             patch("haishui_cli.managed_uv._install_uv", side_effect=RuntimeError("network down")):
            from haishui_cli.managed_uv import ensure_uv
...
class TestEnsureUvWindowsSafe:
    ...
    def test_uvresult_would_break_windows_list2cmdline(self):
        ...
        import subprocess
        from haishui_cli.managed_uv import _UvResult
        with pytest.raises(TypeError):
            subprocess.list2cmdline([_UvResult("C:\\haishui\\uv.exe"), "pip"])
    ...
    def test_windows_returns_plain_str_safe_for_subprocess(self, tmp_path):
        ...
        with patch("haishui_cli.managed_uv.get_haishui_home", return_value=tmp_path), \
             patch("haishui_cli.managed_uv.repair_vulnerable_runtime", return_value=_RRR("not-applicable")):
            from haishui_cli.managed_uv import _UvResult, ensure_uv
            uv_bin = ensure_uv()
...
class TestUpdateManagedUv:
    ...
    def test_fresh_stamp_skips_network_self_update_but_not_repair(self, tmp_path):
        ...
        from haishui_cli.managed_uv import RuntimeRepairResult, update_managed_uv

        uv = tmp_path / "bin" / _UV_BINARY_NAME
        _make_executable(uv)
        # The stamp reader imports get_haishui_home separately from the binary
        ...
        with patch("haishui_cli.managed_uv.get_haishui_home", return_value=tmp_path), \
             patch("haishui_cli.managed_uv._uv_self_update_stamp", return_value=stamp), \
             patch("haishui_cli.managed_uv.repair_vulnerable_runtime",
                   return_value=RuntimeRepairResult("skipped")) as mock_repair, \
             patch("haishui_cli.managed_uv.subprocess.run") as mock_run:
    ...
    def test_stale_stamp_runs_self_update_and_refreshes_stamp(self, tmp_path):
        ...
        from haishui_cli.managed_uv import UV_SELF_UPDATE_INTERVAL_SECONDS, update_managed_uv
        ...
        with patch("haishui_cli.managed_uv.get_haishui_home", return_value=tmp_path), \
             patch("haishui_cli.managed_uv._uv_self_update_stamp", return_value=stamp), \
             patch("haishui_cli.managed_uv.repair_vulnerable_runtime", return_value=_RRR("not-applicable")), \
             patch("haishui_cli.managed_uv._uv_version", return_value="uv 0.2.0"), \
             patch("haishui_cli.managed_uv.subprocess.run") as mock_run:
...
class TestManagedPythonStore:
    def test_store_is_checkout_scoped_across_profiles(self, tmp_path, monkeypatch):
        from haishui_cli.managed_uv import managed_python_install_dir

        checkout = tmp_path / "checkout"
        monkeypatch.setenv("HAISHUI_HOME", str(tmp_path / "profiles" / "alpha"))
        alpha = managed_python_install_dir(checkout)
        monkeypatch.setenv("HAISHUI_HOME", str(tmp_path / "profiles" / "beta"))
        beta = managed_python_install_dir(checkout)

        expected = checkout / ".haishui-runtime" / "python"
    ...
    def test_environment_is_private_and_sanitized(self, tmp_path):
        ...
        assert env["UV_PYTHON_INSTALL_DIR"] == str(
            checkout / ".haishui-runtime" / "python"
        )
        for key in (
            "CONDA_DEFAULT_ENV",
            "CONDA_PREFIX",
...
class TestRuntimeRepair:
    def test_safe_runtime_is_a_noop(self, tmp_path):
        from haishui_cli.managed_uv import repair_vulnerable_runtime

        root, live, sentinel = _make_runtime_install(tmp_path)
        current = _runtime_info(live / "bin" / "python", (3, 53, 1))
        with patch(
                 "haishui_cli.managed_uv.probe_sqlite_runtime",
                 return_value=current,
             ), \
             patch(
                 "haishui_cli.managed_uv._install_safe_python_generation"
             ) as mock_install:
        ...
        assert not (root / ".haishui-runtime").exists()
    ...
    def test_failed_candidate_preserves_live_venv(self, tmp_path):
        ...
        generation = root / ".haishui-runtime" / "python" / "generation-test"
        ...
        with patch(
             ...
             patch(
                 "haishui_cli.managed_uv._stage_candidate_venv",
        ...
        reacquired = _acquire_repair_lock(root / ".haishui-runtime")
    ...
    def test_successful_repair_removes_parked_backup(self, tmp_path):
        ...
        candidate_python.write_text("candidate interpreter", encoding="utf-8")
        fixed = _runtime_info(candidate_python, (3, 53, 1))
        candidate_venv = root / ".haishui-runtime" / "venv-candidate"
        ...
        with patch(
             ...
             patch(
                 "haishui_cli.managed_uv._smoke_candidate_venv",
...
def _make_candidate_layout(tmp_path):
    ...
    generation = root / ".haishui-runtime" / "python" / "gen"
...
class TestStageCandidateVenvCrossPlatform:
    ...
    def test_sync_keeps_uv_project_config_and_merges_stderr(self, tmp_path):
        ...
        from haishui_cli.managed_uv import _stage_candidate_venv
        ...
        with patch("haishui_cli.managed_uv.subprocess.run", side_effect=fake_run), \
             patch("haishui_cli.managed_uv.subprocess.Popen", side_effect=fake_popen), \
             patch(
                 "haishui_cli.managed_uv._smoke_candidate_venv",
    ...
    def test_sync_failure_reports_the_child_reason(self, tmp_path, caplog):
        ...
        with caplog.at_level(logging.WARNING), \
             patch("haishui_cli.managed_uv.subprocess.run", return_value=MagicMock(returncode=0)), \
             patch(
                 "haishui_cli.managed_uv.subprocess.Popen",
        ...
        assert not list((root / ".haishui-runtime").glob("venv-candidate-*"))


class TestRuntimeCutover:
    def test_os_lock_blocks_concurrent_repair_and_releases(self, tmp_path):
        from haishui_cli.managed_uv import _acquire_repair_lock, _release_repair_lock

        runtime_root = tmp_path / ".haishui-runtime"
    ...
    def test_post_swap_smoke_failure_rolls_back_live_venv(self, tmp_path):
        from haishui_cli.managed_uv import _cut_over_candidate

        root, live, sentinel = _make_runtime_install(tmp_path)
        runtime_root = root / ".haishui-runtime"
...
class TestInstallUvInternals:
    def test_installer_uses_host_branch_and_managed_directory(self, tmp_path):
        ...
        with patch("haishui_cli.managed_uv._install_uv_posix") as mock_posix, \
             patch("haishui_cli.managed_uv._install_uv_windows") as mock_windows:
...
class TestRuntimeRequestMinorLine:
    ...
    def test_requests_minor_line(self):
        from haishui_cli.managed_uv import _runtime_request
...
class TestMinorLineFallForward:
    ...
    `haishui update` with no path to a fixed runtime.
    """
...
class TestRefreshManagedUvCatalog:
    ...
    def test_version_change_reports_true(self, tmp_path):
        ...
        with patch("haishui_cli.managed_uv.get_haishui_home", return_value=tmp_path), \
             patch("haishui_cli.managed_uv._install_uv"), \
             patch(
                 "haishui_cli.managed_uv._uv_version_string",
...
class TestRepairRetriesAfterUvRefresh:
    def _run_repair(self, tmp_path, *, refresh_result, second_attempt):
        ...
        with patch(
             ...
             patch(
                 "haishui_cli.managed_uv._refresh_managed_uv_catalog",
    ...
    def test_retry_success_proceeds_to_staging(self, tmp_path):
        def second_attempt(root):
            generation = root / ".haishui-runtime" / "python" / "generation-retry"
            candidate_python = generation / "bin" / "python"
            candidate_python.parent.mkdir(parents=True)
...
class TestDefaultLiveVenv:
    ...
    'not-applicable' on every haishui update and stayed on journal_mode=DELETE
    ...
    def test_dot_venv_only_is_targeted(self, tmp_path):
        from haishui_cli.managed_uv import _default_live_venv
...
class TestVenvPythonUpdateBoundary:
    """``_venv_python`` must survive a haishui_constants predating its symbol.

    ``haishui update`` imports haishui_constants from the OLD checkout, ``git
    pull`` replaces that file, and the freshly-pulled managed_uv then runs its
    lazy ``from haishui_constants import venv_python_path`` against the module
    ...
    plainly contains it, which is what made the error so confusing:
        ...
        cannot import name 'venv_python_path' from 'haishui_constants'
        (~/.haishui/haishui-agent/haishui_constants.py)
    ...
    def test_recovers_when_the_cached_module_predates_the_symbol(self, monkeypatch):
        ...
        from haishui_cli.managed_uv import _venv_python
        ...
        monkeypatch.delattr(haishui_constants, "venv_python_path", raising=False)
        ...
        expected = Path("/opt/haishui/venv/Scripts/python.exe") \
            if sys.platform == "win32" else Path("/opt/haishui/venv/bin/python")
        assert _venv_python(Path("/opt/haishui/venv")) == expected

    def test_recovery_uses_the_shared_helper_not_a_second_copy(self, monkeypatch):
        """The reload must resolve through haishui_constants, not open-code it.

        Hand-rolling `Scripts`/`bin` here is what #76105 deduped away and what
        `test_no_open_coded_venv_layout_remains_in_haishui_cli` bans.
        """
        import haishui_constants
        ...
        assert _venv_python(Path("/opt/haishui/venv")) == sentinel
