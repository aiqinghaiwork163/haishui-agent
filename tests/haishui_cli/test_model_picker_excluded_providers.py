...
interactive ``haishui model`` CLI picker.

The CLI picker (``haishui_cli.main.select_provider_and_model``) builds its
...
def test_cli_picker_hides_excluded_provider(config_home):
    ...
    from the ``haishui model`` provider menu."""
...
def test_cli_picker_hides_excluded_provider_by_alias(config_home):
    ...
    haishui_id / alias names."""
    # 'openai' is an alias-style haishui id; ensure excluding it hides the
    # canonical openai provider row if present. Use the canonical slug's
    # alias from _PROVIDER_ALIASES to stay robust to renames.
    from haishui_cli.models import _PROVIDER_ALIASES, CANONICAL_PROVIDERS
