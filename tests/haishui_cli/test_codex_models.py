...
from haishui_cli.codex_models import (
...
def test_curated_codex_fallback_excludes_chatgpt_rejected_pro_slugs(monkeypatch):
    ...
    monkeypatch.setattr(
        "haishui_cli.codex_models._fetch_models_from_api",
...
def test_picker_never_synthesizes_900k_for_pro_or_unknown_slugs():
    ...
    from haishui_cli.codex_models import _finalize_codex_models
...
def test_setup_wizard_codex_import_resolves():
    ...
    # This mirrors the exact import used in haishui_cli/setup.py line 873.
    # A prior bug had 'get_codex_models' (wrong) instead of 'get_codex_model_ids'.
    from haishui_cli.codex_models import get_codex_model_ids as setup_import
...
def test_fetch_from_api_keeps_supported_in_api_false_models(monkeypatch):
    ...
    """
    import sys
    from haishui_cli import codex_models
...
def test_model_command_prompts_to_reuse_or_reauthenticate_codex_session(monkeypatch, capsys):
    from haishui_cli.model_setup_flows import _model_flow_openai_codex
    ...
    monkeypatch.setattr(
        "haishui_cli.auth.get_codex_auth_status",
        lambda: {"logged_in": True, "source": "haishui-auth-store"},
    )
    monkeypatch.setattr(
        "haishui_cli.auth.resolve_codex_runtime_credentials",
    ...
    monkeypatch.setattr("haishui_cli.auth._login_openai_codex", _fake_login)
    monkeypatch.setattr(
...
def _make_cli(model="anthropic/claude-opus-4.6", **kwargs):
    """Create a HaishuiCLI with minimal mocking."""
    ...
    clean_env = {"LLM_MODEL": "", "HAISHUI_MAX_ITERATIONS": ""}
    with (
        patch("cli.get_tool_definitions", return_value=[]),
        patch.dict("os.environ", clean_env, clear=False),
        patch.dict(_cli_mod.__dict__, {"CLI_CONFIG": _clean_config}),
    ):
        cli = HaishuiCLI(model=model, **kwargs)
    return cli
...
class TestNormalizeModelForProvider:
    ...
    def test_default_model_replaced(self):
        ...
        with (
            patch("cli.get_tool_definitions", return_value=[]),
            patch.dict("os.environ", {"LLM_MODEL": "", "HAISHUI_MAX_ITERATIONS": ""}, clear=False),
            patch.dict(_cli_mod.__dict__, {"CLI_CONFIG": _clean_config}),
        ):
            from cli import HaishuiCLI
            cli = HaishuiCLI()
