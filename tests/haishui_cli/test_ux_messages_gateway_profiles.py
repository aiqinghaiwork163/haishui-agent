...
next command, and every ``haishui <sub> <cmd>`` it cites is a registered subcommand.
"""
...
from haishui_cli import gateway as gateway_cli
from haishui_cli import profiles
from haishui_cli.update_lock import UpdateHolder, describe_holder
...
def test_systemctl_failure_is_explained_not_tracebacked(monkeypatch, capsys):
    exc = subprocess.CalledProcessError(1, ["systemctl", "--user", "start", "haishui-gateway"])
    out = _run_gateway_command_raising(monkeypatch, capsys, exc)
    assert not out.lstrip().startswith("Command '")
    assert "haishui gateway status --deep" in out
    assert "haishui gateway install --force" in out
...
def test_missing_systemctl_points_at_foreground_run(monkeypatch, capsys):
    ...
    assert "haishui gateway run" in out
...
def test_existing_gateway_guard_says_bots_are_online_and_names_status(monkeypatch, capsys):
    ...
    for cmd in ("haishui gateway status", "haishui gateway restart", "haishui gateway stop"):
        assert cmd in out
...
def test_unsupported_platform_copy_says_what_is_unsupported(subcommand):
    ...
    assert "haishui gateway run" in text or subcommand == "uninstall"
...
def test_invalid_profile_name_explains_rule_in_words_with_example():
    ...
    assert "haishui profile create my-work" in msg
...
def test_existing_profile_error_offers_use_and_list(tmp_path, monkeypatch):
    ...
    assert "haishui profile use work" in msg
    assert "haishui profile list" in msg
...
def test_describe_holder_avoids_jargon_and_names_next_steps():
    ...
    assert "haishui logs" in message
    assert "haishui update" in message
