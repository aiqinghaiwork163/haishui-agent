...
def haishui_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HAISHUI_ENABLE_PROJECT_PLUGINS", "0")
    monkeypatch.setenv(
        "HAISHUI_BUNDLED_PLUGINS", str(tmp_path / "empty-bundled")
    )
...
class TestV1Regression:
    def test_v1_manifest_parses_with_defaults(self, haishui_home):
        _write_plugin(haishui_home / "plugins", "oldie")
        _enable(haishui_home, ["oldie"])
        mgr = PluginManager()
    ...
    def test_v1_unknown_fields_do_not_warn_loudly(self, haishui_home, caplog):
        _write_plugin(
            haishui_home / "plugins", "oldie",
...
class TestV2Parsing:
    def test_v2_fields_parse(self, haishui_home):
        _write_plugin(
            haishui_home / "plugins", "modern",
            manifest_extra={
                "manifest_version": 2,
                "api_version": 1,
                "license": "MIT",
    ...
    def test_unknown_field_in_v2_warns_but_loads(self, haishui_home, caplog):
        _write_plugin(
            haishui_home / "plugins", "modern",
    ...
    def test_future_manifest_version_warns_but_loads(self, haishui_home, caplog):
        _write_plugin(
            haishui_home / "plugins", "fromfuture",
            manifest_extra={
                "manifest_version": SUPPORTED_MANIFEST_VERSION + 5,
            },
        )
        _enable(haishui_home, ["fromfuture"])
        with caplog.at_level(logging.WARNING):
            mgr = PluginManager()
            mgr.discover_and_load()
        assert mgr._plugins["fromfuture"].enabled
        assert "newer than this Haishui" in caplog.text

    def test_malformed_v2_fields_warn_and_degrade(self, haishui_home, caplog):
        _write_plugin(
            haishui_home / "plugins", "sloppy",
            manifest_extra={
                "manifest_version": 2,
                "api_version": "banana",
...
class TestDependencyOrder:
    def test_dep_registers_before_dependent(self, haishui_home):
        ...
        _write_plugin(
            haishui_home / "plugins", "aaa-consumer",
            manifest_extra={
                "manifest_version": 2,
        ...
        _write_plugin(
            haishui_home / "plugins", "zzz-base",
            register_body="import sys; sys._m2_order.append('zzz-base')",
        )
        _enable(haishui_home, ["aaa-consumer", "zzz-base"])
        import sys
    ...
    def test_missing_dep_warns_but_loads(self, haishui_home, caplog):
        _write_plugin(
            haishui_home / "plugins", "needy",
            manifest_extra={
                "manifest_version": 2,
                "requires_plugins": [{"id": "ghost-plugin"}],
            },
        )
        _enable(haishui_home, ["needy"])
        with caplog.at_level(logging.WARNING):
            mgr = PluginManager()
            mgr.discover_and_load()
    ...
    def test_cycle_warns_and_falls_back_alpha(self, haishui_home, caplog):
        _write_plugin(
            haishui_home / "plugins", "cyc-a",
            manifest_extra={
                "manifest_version": 2,
        ...
        _enable(haishui_home, ["cyc-a", "cyc-b"])
        import sys
...
class TestConfigSchema:
    def test_type_mismatch_warns_but_loads(self, haishui_home, caplog):
        _write_plugin(
            haishui_home / "plugins", "cfgd",
            manifest_extra={
                "manifest_version": 2,
                "config_schema": {
                    "api_url": {"type": "str"},
    ...
    def test_unknown_declared_type_skips_check(self, haishui_home, caplog):
        _write_plugin(
            haishui_home / "plugins", "weird",
            manifest_extra={
                "manifest_version": 2,
...
class TestPythonDependenciesSeam:
    def test_missing_pip_dep_surfaced_with_hint_not_installed(
        self, haishui_home, caplog, monkeypatch
    ):
        calls = []
        import subprocess
        ...
        assert "haishui plugins enable pipful" in caplog.text
        assert calls == []

    def test_satisfied_pip_dep_is_quiet(self, haishui_home, caplog):
        _write_plugin(
            haishui_home / "plugins", "pipok",
            manifest_extra={
                "manifest_version": 2,
                "python_dependencies": ["pyyaml>=5,<7"],
            },
        )
        _enable(haishui_home, ["pipok"])
        with caplog.at_level(logging.WARNING):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestCtxHasPlugin:
    def test_has_plugin_probe(self, haishui_home):
        _write_plugin(haishui_home / "plugins", "probe-target")
        _write_plugin(
            haishui_home / "plugins", "prober",
            manifest_extra={
                "manifest_version": 2,
        ...
        _enable(haishui_home, ["probe-target", "prober"])
        import sys
...
class TestRequiresHaishui:
    def test_unsatisfied_requires_haishui_skips_without_importing(self, haishui_home, monkeypatch):
        """A too-new ``requires_haishui`` records an error and never runs register(); a satisfied one loads."""
        import sys
        from haishui_cli import plugins_manifest
        monkeypatch.setattr(plugins_manifest, "running_haishui_version", lambda: "1.2.3")
        _write_plugin(haishui_home / "plugins", "future", manifest_extra={"requires_haishui": ">=99.0"},
                      register_body="import sys; sys._rh_future = True")
        _write_plugin(haishui_home / "plugins", "current", manifest_extra={"requires_haishui": ">=1.2,<2"},
                      register_body="import sys; sys._rh_current = True")
        _enable(haishui_home, ["future", "current"])
        try:
            mgr = PluginManager()
            mgr.discover_and_load()
            assert not hasattr(sys, "_rh_future")
            assert "requires haishui >=99.0" in (mgr._plugins["future"].error or "")
...
class TestDirectoryPluginKeepsIdentityOverEntryPoint:
    """A pyproject-wrapper plugin depends on a pip package that ships a ``haishui_agent.plugins``
    ...
    def test_loader_and_listing_prefer_the_installed_directory(self, haishui_home, monkeypatch):
        from haishui_cli.plugins_manifest import PluginManifest
        _write_plugin(haishui_home / "plugins", "twin")
        _enable(haishui_home, ["twin"])
        ...
        monkeypatch.setattr("haishui_cli.plugins_cmd.discover_entrypoint_manifests", lambda: [twin_ep], raising=False)
        monkeypatch.setattr("haishui_cli.plugins.discover_entrypoint_manifests", lambda: [twin_ep])

        mgr = PluginManager()
        mgr.discover_and_load()
        assert mgr._plugins["twin"].manifest.source == "user"

        from haishui_cli.plugins_cmd import _discover_all_plugins
