...
the SAME provider universe as ``haishui model`` (the CLI/TUI picker).
...
def test_every_haishui_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `haishui model` universe."""
    ...
    assert not missing, (
        "providers shown in `haishui model` but not configurable in the desktop "
