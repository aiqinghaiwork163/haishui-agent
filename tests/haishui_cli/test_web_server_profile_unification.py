...
profile switcher can target any profile's HAISHUI_HOME. These tests pin:
reads/writes land in the REQUESTED profile, the dashboard's own profile
stays untouched, and the chat PTY env is scoped via HAISHUI_HOME.
"""
import json
...
import haishui_cli.web_server_profiles as _web_server_profiles


@pytest.fixture
def isolated_profiles(tmp_path, monkeypatch, _isolate_haishui_home):
    ...
    monkeypatch.setattr(profiles, "_get_default_haishui_home", lambda: default_home)
...
class TestProfileScopedMcp:
    ...
    def test_mcp_test_oauth_server_without_token_is_not_ok(
        ...
        import haishui_cli.mcp_config as mcp_config
    ...
    def test_mcp_test_resolves_profile_secret_source_scope(
        ...
        def _worker_sources(haishui_home):
            if Path(haishui_home).resolve() == worker_home.resolve():
    ...
    def test_mcp_list_expands_url_ref_from_profile_secret_scope(
        ...
        monkeypatch.setattr(
            ...
            lambda haishui_home: {"MCP_GH_URL": "http://worker/mcp"}
            if Path(haishui_home).resolve() == worker_home.resolve() else {},
        )
...
class TestProfileScopedModel:
    ...
    def _accept_any_model(self, monkeypatch):
        ...
        monkeypatch.setattr("haishui_cli.model_switch.switch_model", _switch)
    ...
    def test_model_options_uses_config_only_scope_for_selected_profile(
        ...
        monkeypatch.setattr(
            "haishui_cli.inventory.load_picker_context", lambda: object()
        )
        monkeypatch.setattr(
            "haishui_cli.inventory.build_model_options_payload",
...
class TestProfileScopedPostSetup:
    def test_post_setup_spawns_with_profile_flag(
        ...
        config / write per-profile state see the same HAISHUI_HOME the rest
        ...
        monkeypatch.setattr(
            "haishui_cli.tools_config.valid_post_setup_keys",
...
class TestProfileScopedGateway:
    ...
    def test_status_reads_requested_profile_home(
        ...
        def fake_get_running_pid(*args, **kwargs):
            ...
            seen_homes.append(str(get_haishui_home()))
            return None
        ...
        assert resp.json()["haishui_home"] == str(isolated_profiles["worker_beta"])
...
class TestProfileScopedChatPty:
    def test_chat_argv_scopes_haishui_home(self, isolated_profiles, monkeypatch):
        import haishui_cli.web_server as web_server

        monkeypatch.setattr(
            "haishui_cli.main_tui_launch._make_tui_argv",
            lambda root, tui_dev=False: (["cat"], None),
            raising=False,
        )
        argv, cwd, env = _web_server_chat._resolve_chat_argv(profile="worker_beta")
        assert env is not None
        assert env["HAISHUI_HOME"] == str(isolated_profiles["worker_beta"])
        # Scoped chat must NOT attach to the dashboard's in-memory gateway.
        assert "HAISHUI_TUI_GATEWAY_URL" not in env
...
class TestProfileScopedAudio:
    ...
    def test_transcribe_runs_inside_target_profile_home(
        ...
        def _fake_transcribe(path):
            ...
            seen["home"] = str(get_haishui_home())
