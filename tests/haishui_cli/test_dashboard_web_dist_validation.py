"""Regression tests: `haishui dashboard` validates HAISHUI_WEB_DIST before serving.

A custom HAISHUI_WEB_DIST without --skip-build previously skipped BOTH the
...
@pytest.fixture()
def main_mod():
    import haishui_cli.main as main
    return main
...
def _wire_common(main_mod, monkeypatch):
    monkeypatch.setattr(
        "haishui_cli.profiles.get_active_profile_name", lambda: "default"
    )
    ...
    monkeypatch.setattr(
        "haishui_cli.mcp_startup.start_background_mcp_discovery",
        lambda **_k: None,
    )


def test_env_dist_without_index_exits(main_mod, monkeypatch, tmp_path, capsys):
    """HAISHUI_WEB_DIST pointing at a dist with no index.html must exit 1,
    not start a server that 404s."""
    _wire_common(main_mod, monkeypatch)
    empty_dist = tmp_path / "empty_dist"
    empty_dist.mkdir()
    monkeypatch.setenv("HAISHUI_WEB_DIST", str(empty_dist))
    ...
    builds = []
    monkeypatch.setattr(
        "haishui_cli.main_web_build._build_web_ui", lambda *a, **k: builds.append(a) or True
    )
    ...
    assert "HAISHUI_WEB_DIST" in out and str(empty_dist) in out
...
def test_skip_build_missing_dist_attempts_one_recovery_build(
    ...
    _wire_common(main_mod, monkeypatch)
    monkeypatch.delenv("HAISHUI_WEB_DIST", raising=False)
    project_root = tmp_path / "proj"
    dist = project_root / "haishui_cli" / "web_dist"
    ...
    monkeypatch.setattr("haishui_cli.main_web_build._build_web_ui", fake_build)
