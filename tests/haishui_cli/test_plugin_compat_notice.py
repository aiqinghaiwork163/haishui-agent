"""haishui_cli.plugin_compat: detect plugins on old import paths, tell the user, disable after the date.

Kept with the compat layer (tests/haishui_cli/test_compat_manifest_targets.py); deleted with it.
"""
...
from haishui_cli import plugin_compat as pc
...
MANIFEST = {"tools.web_tools": {"prefers_gateway": "tools.tool_backend_helpers.prefers_gateway"},
            "haishui_cli.kanban_db": {"connect": "haishui_cli.kanban_db_connect.connect"}}
...
@pytest.mark.parametrize("src, expect", [
    ...
    ("from unittest.mock import patch\npatch('haishui_cli.kanban_db.connect')\n", ["haishui_cli.kanban_db.connect"]),
...
def test_scan_plugin_walks_dir_and_skips_tests(tmp_path):
    ...
    (tmp_path / "sub").mkdir(); (tmp_path / "sub" / "m.py").write_text("import haishui_cli.kanban_db as k\nk.connect()\n")
...
def test_disable_only_after_the_date_and_not_when_allowed(tmp_path, monkeypatch):
    ...
    assert reason and pc.COMPAT_REMOVAL in reason and "haishui plugins compat" in reason
...
def test_summary_lines_name_plugins_and_the_date():
    ...
    assert "haishui plugins compat" in tail
