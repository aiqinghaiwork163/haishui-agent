...
Haishui keeps its OWN copy of the Codex OAuth token (per profile + top-level),
separate from the Codex CLI's ``~/.codex/auth.json``. OAuth refresh_tokens are
single-use, so when the Codex CLI (or another Haishui process) rotates the shared
...
from haishui_cli.auth import AuthError, _refresh_codex_auth_tokens, resolve_codex_runtime_credentials
...
def test_self_heals_on_stale_refresh_token(monkeypatch):
    ...
    # the recovered token was persisted to the Haishui auth store
...
def test_self_heals_missing_singleton_access_token_from_codex_cli(tmp_path, monkeypatch):
    """Exact cron failure path: Haishui auth has refresh_token but missing access_token."""
    ...
    assert resolved["source"] == "haishui-auth-store"
    stored = json.loads((haishui_home / "auth.json").read_text())
...
def test_opt_out_never_adopts_codex_cli_login(tmp_path, monkeypatch):
    ...
    family the user did not hand to Haishui. Both automatic recovery paths must leave it (and Haishui' own
    auth.json) untouched and surface the real error instead."""
    haishui_home = tmp_path / "haishui"
    codex_home = tmp_path / "codex"
    haishui_home.mkdir()
    codex_home.mkdir()
    (haishui_home / "config.yaml").write_text("auth:\n  adopt_external_logins: false\n")
    haishui_auth = {"version": 1, "providers": {"openai-codex": {
        "tokens": {"refresh_token": "stale-refresh"}, "auth_mode": "chatgpt"}}}
    (haishui_home / "auth.json").write_text(json.dumps(haishui_auth))
    (codex_home / "auth.json").write_text(json.dumps({
    ...
    assert json.loads((haishui_home / "auth.json").read_text()) == haishui_auth
