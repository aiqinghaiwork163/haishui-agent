...
def _client():
    ...
    # Keep the state DB under the isolated HAISHUI_HOME for any handler that
    # touches it.
...
class TestMcpEndpoints:
    ...
    def _setup(self, _isolate_haishui_home):
    ...
    def test_http_bearer_auth_separates_secret_from_config(
        ...
        config_text = (haishui_home / "config.yaml").read_text()
        env_text = (haishui_home / ".env").read_text()
    ...
    def test_http_oauth_mode_is_persisted_for_existing_auth_flow(self):
        ...
        from haishui_cli.mcp_config import _get_mcp_servers
...
class TestCredentialPoolEndpoints:
    ...
    def test_env_seeded_delete_stays_deleted(self):
        ...
        load_pool() re-seeds from ~/.haishui/.env on every call, so removing
        just the pool row silently reverts on the next dashboard refresh.
        The endpoint must mirror `haishui auth remove`: clean up the backing
    ...
    def test_post_readd_lifts_suppression(self):
        ...
        Mirrors `haishui auth add`, which clears every suppression for the
...
class TestMemoryEndpoints:
    ...
    def _setup(self, _isolate_haishui_home):
        ...
        (get_haishui_home() / "memories").mkdir(parents=True, exist_ok=True)
...
class TestPairingEndpoints:
    ...
    def test_pairing_is_isolated_per_profile(self):
        ...
        (get_haishui_home() / "profiles" / "work").mkdir(parents=True, exist_ok=True)
        (get_haishui_home() / "profiles" / "work" / "config.yaml").write_text("{}\n")  # identity marker
...
class TestWebhookEndpoints:
    ...
    def test_enable_platform_starts_gateway_restart(self, monkeypatch):
        ...
        monkeypatch.setattr(_web_server_gateway, "_spawn_haishui_action", fake_spawn_action)
...
class TestSystemStatsEndpoint:
    ...
    def test_stats_shape(self):
        ...
        for key in ("os", "arch", "hostname", "python_version", "haishui_version"):
    ...
    def test_sources_lists_configured_hubs(self, monkeypatch):
        ...
        class _Src:
            ...
            def search(self, q, limit=10):
                return [_FakeMeta("haishui-index/featured-skill", "trusted")]

        def _fake_router():
            srcs = [_Src("official"), _Src("github")]
            # haishui-index source advertises availability + featured search.
            idx = _Src("haishui-index")
        ...
        assert {"official", "github", "haishui-index"} <= ids
...
class TestOfficialSkillsCatalogEndpoint:
    ...
    def test_lists_full_catalog_with_installed_flags(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "haishui_cli.web_routers.skills._installed_hub_identifiers",
...
class TestSkillsHubPreviewEndpoint:
    ...
    def test_preview_returns_skill_md_text(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "haishui_cli.skills_hub._resolve_source_meta_and_bundle",
...
class TestUpdateCheckEndpoint:
    """``GET /api/haishui/update/check`` reports availability without applying.

    Powers the dashboard's check-before-you-update flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/haishui/update`` runs ``haishui update``.
    """
    ...
    def test_git_install_reports_behind_count(self, monkeypatch):
        ...
        import haishui_cli.banner as banner

        monkeypatch.setattr(banner, "check_for_updates", lambda: 5)

        r = self.client.get("/api/haishui/update/check")
        assert r.status_code == 200
        body = r.json()
        assert {
            "install_method",
    ...
    def test_managed_runtime_dashboard_is_not_applyable(self, monkeypatch):
        ...
        body = self.client.get("/api/haishui/update/check").json()
...
class TestDebugShareEndpoint:
    ...
    def test_redact_false_is_honored(self, monkeypatch):
        import haishui_cli.debug as dbg
        ...
        monkeypatch.setattr("haishui_cli.dump.run_dump", lambda a: None)
...
class TestToolsConfigEndpoints:
    ...
    the dashboard surface that replicates the `haishui tools` configurator."""
...
# ---------------------------------------------------------------------------
# _spawn_haishui_action env scrubbing (#52470)
# ---------------------------------------------------------------------------

def test_spawn_haishui_action_scrubs_gateway_loop_guard_env(monkeypatch, tmp_path):
    """The dashboard runs inside the gateway, so os.environ has
    _HAISHUI_GATEWAY=1. Spawned actions (e.g. `gateway restart`) must NOT inherit
    ...
    # Isolate the module-global proc registry: _spawn_haishui_action stores
    ...
    _web_server_gateway._spawn_haishui_action(["gateway", "restart"], "gateway-restart")

    assert "_HAISHUI_GATEWAY" not in captured["env"]
    assert captured["env"]["HAISHUI_NONINTERACTIVE"] == "1"
...
def test_named_profile_action_isolates_parent_env_and_loads_target_env(monkeypatch, tmp_path):
    ...
    import haishui_cli.env_loader as env_loader
    import haishui_cli.web_server as ws

    user_home = tmp_path / "user"
    default_home = user_home / ".haishui"
    ...
    for key, value in {
        ...
        "HAISHUI_ACP_AUTH_METHOD": "default-acp",
    ...
    monkeypatch.setattr(ws.subprocess, "Popen", _fake_popen)
    _web_server_gateway._spawn_haishui_action(["-p", "verifier", "gateway", "restart"], "gateway-restart")

    child_env = captured["env"]
    assert captured["cmd"][-4:] == ["-p", "verifier", "gateway", "restart"]
    assert child_env["HAISHUI_HOME"] == str(target_home)
    assert child_env["HAISHUI_NONINTERACTIVE"] == "1"
    ...
    for leaked_key in (
        ...
        "A2A_AUTH_MINI", "EXTERNAL_PROFILE_AUTH", "HAISHUI_ACP_AUTH_METHOD",
    ):
    ...
    probe = subprocess.run(
        [
            ...
            "load_haishui_dotenv(haishui_home=os.environ['HAISHUI_HOME']); "
...
def test_desktop_lifespan_reaps_orphan_gateways_on_startup(
    monkeypatch, _isolate_haishui_home
):
    ...
    startup under HAISHUI_DESKTOP=1 so the stale orphan is cleared first.
    """
    import haishui_cli.web_server as ws
    ...
    # web_server imports the reaper lazily from haishui_cli.gateway, so patch it
    # on that module.
    import haishui_cli.gateway as g
