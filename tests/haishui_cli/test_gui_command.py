"""Tests for ``haishui gui`` desktop launcher wiring."""
...
from haishui_cli import main as cli_main
from haishui_cli import main_desktop
from haishui_cli import main_install_repair
from haishui_cli import main_web_build
...
def _isolate_xdg_data_home(tmp_path, monkeypatch):
    ...
    call live, and on a Linux dev box it wrote a ``haishui.desktop`` pointing
...
def _stable_keychain_detection(monkeypatch):
    ...
    monkeypatch.delenv("HAISHUI_DESKTOP_PASSWORD_STORE", raising=False)
...
def _make_packaged_executable(root: Path, monkeypatch) -> Path:
    ...
    if sys.platform == "darwin":
        exe = desktop_dir / "release" / "mac-arm64" / "Haishui.app" / "Contents" / "MacOS" / "Haishui"
    elif sys.platform == "win32":
        exe = desktop_dir / "release" / "win-unpacked" / "Haishui.exe"
    else:
        exe = desktop_dir / "release" / "linux-unpacked" / "haishui"
...
def _packaged_exe_rel() -> Path:
    ...
    if sys.platform == "darwin":
        return Path("mac-arm64") / "Haishui.app" / "Contents" / "MacOS" / "Haishui"
    if sys.platform == "win32":
        return Path("win-unpacked") / "Haishui.exe"
    return Path("linux-unpacked") / "haishui"
...
def test_gui_installs_packages_and_launches_desktop_app(tmp_path, monkeypatch):
    ...
    with patch("haishui_cli.main_install_repair._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("haishui_cli.main_web_build._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("haishui_cli.main_desktop._desktop_build_needed", return_value=True), \
         patch("haishui_cli.main_desktop._write_desktop_build_stamp"), \
         patch("haishui_cli.main_desktop._desktop_macos_relaunchable_fixup"), \
         patch("haishui_cli.main_desktop._desktop_linux_sandbox_fixup", return_value=True), \
         patch("haishui_cli.main_desktop._register_linux_desktop_entry"), \
         patch("haishui_cli.main.subprocess.run", side_effect=_pack_into_staging(root)) as mock_run, \
...
def test_gui_install_env_prepends_managed_node_on_bare_path(tmp_path, monkeypatch):
    ...
    the parent PATH is stripped, so the install env MUST carry the Haishui-managed
    Node ahead of that bare PATH or the install dies with ``node: not found``.
    """
    import os

    from haishui_constants import iter_haishui_node_dirs

    root = _make_desktop_tree(tmp_path)
    monkeypatch.setattr(cli_main, "PROJECT_ROOT", root)
    _make_packaged_executable(root, monkeypatch)

    # A managed Node tree on disk so with_haishui_node_path() actually prepends it.
    ...
    install_ok = subprocess.CompletedProcess(["npm", "ci"], 0)
    launch_ok = subprocess.CompletedProcess(["haishui"], 0)
    ...
    with patch("haishui_cli.main_install_repair._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         ...
         patch("haishui_cli.main_desktop._desktop_macos_relaunchable_fixup"), \
         patch("haishui_cli.main_desktop._desktop_linux_sandbox_fixup", return_value=True), \
         patch("haishui_cli.main.subprocess.run", return_value=launch_ok), \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns(skip_build=False))

    managed_dirs = [str(p) for p in iter_haishui_node_dirs() if p.is_dir()]
...
def test_gui_does_not_retry_after_packaged_executable_exists(tmp_path, monkeypatch, capsys):
    ...
    Haishui.app/Contents/MacOS/Haishui in place. Re-downloading Electron can't
    ...
    with patch("haishui_cli.main_install_repair._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("haishui_cli.main_web_build._run_npm_install_deterministic", return_value=install_ok), \
         patch("haishui_cli.main_desktop._desktop_macos_relaunchable_fixup"), \
         patch("haishui_cli.main_desktop._purge_electron_build_cache", return_value=[Path("/c/electron.zip")]) as mock_purge, \
         patch("haishui_cli.main_desktop._redownload_electron_dist", return_value=True) as mock_dl, \
         patch("haishui_cli.main.subprocess.run", side_effect=pack_fail) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def _make_signable_app(desktop_dir: Path) -> Path:
    """Build a fake packaged Haishui.app with the pieces the signer must find."""
    ...
    app = desktop_dir / "release" / "mac-arm64" / "Haishui.app"
    _write_info_plist(app, "com.nousresearch.haishui")
    (app / "Contents" / "MacOS").mkdir(parents=True)
    (app / "Contents" / "MacOS" / "Haishui").write_text("", encoding="utf-8")

    helper = app / "Contents" / "Frameworks" / "Haishui Helper.app"
    _write_info_plist(helper, "com.nousresearch.haishui.helper")
...
def test_desktop_macos_local_codesign_signs_native_binaries(tmp_path, monkeypatch):
    ...
    Haishui.app component, silently skipping every .node/.dylib/crashpad
...
def test_setup_tcc_identity_creates_cert_imports_trusts_and_configures(tmp_path, monkeypatch, capsys):
    ...
    identity = "Haishui Local Signing"
    calls = []
    state = {"trusted": False}
    ...
    monkeypatch.setattr("haishui_cli.config.set_config_value", lambda key, value: None)
    ...
    assert not list(tmp_path.glob("haishui-tcc-*"))
...
def test_setup_tcc_identity_fails_when_trust_step_fails(tmp_path, monkeypatch, capsys):
    ...
    assert main_desktop._desktop_macos_setup_tcc_identity("Haishui Local Signing") is False
...
def test_setup_tcc_identity_skips_generation_when_already_valid(tmp_path, monkeypatch, capsys):
    ...
    def fake_run(cmd, **kwargs):
        calls.append(list(cmd))
        if cmd[:4] == ["/usr/bin/security", "find-identity", "-v", "-p"]:
            return _fake_proc(cmd, stdout='  1) ABCD "Haishui Local Signing"\n     1 valid identities found')
        return _fake_proc(cmd)
    ...
    assert main_desktop._desktop_macos_setup_tcc_identity("Haishui Local Signing") is True
...
def test_cmd_gui_setup_tcc_identity_exits_before_build(tmp_path, monkeypatch):
    """`haishui desktop --setup-tcc-identity` calls the setup and exits 0/1
    without building or launching the app."""
    root = _make_desktop_tree(tmp_path)
    monkeypatch.setattr(cli_main, "PROJECT_ROOT", root)
    _make_packaged_executable(root, monkeypatch)

    with patch("haishui_cli.main_desktop._desktop_macos_setup_tcc_identity", return_value=True) as mock_setup, \
         patch("haishui_cli.main_web_build._run_npm_install_deterministic") as mock_install, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(setup_tcc_identity=True, identity="Haishui Local Signing"))

    assert exc.value.code == 0
    mock_setup.assert_called_once_with("Haishui Local Signing")
...
def test_gui_registers_linux_desktop_entry_before_launch(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("haishui_cli.linux_desktop_entry.is_supported", lambda: True)
    monkeypatch.setattr(
        "haishui_cli.linux_desktop_entry.install_desktop_entry",
        lambda project_root: registered.append(project_root) or (tmp_path / "haishui.desktop"),
    )

    launch_ok = subprocess.CompletedProcess([str(packaged_exe)], 0)

    with patch("haishui_cli.main_desktop._desktop_build_needed", return_value=False), \
         patch("haishui_cli.main_install_repair._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("haishui_cli.main_desktop._desktop_linux_sandbox_fixup", return_value=True), \
         patch("haishui_cli.main.subprocess.run", return_value=launch_ok), \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
...
def test_gui_shell_launch_defers_desktop_entry_until_window_reveal(tmp_path, monkeypatch):
    ...
    (#111906). Electron gets the pipe's write end via HAISHUI_DESKTOP_READY_FD."""
    root = _make_desktop_tree(tmp_path)
    monkeypatch.setattr(cli_main, "PROJECT_ROOT", root)
    monkeypatch.setenv("DESKTOP_STARTUP_ID", "gnome-shell/Haishui/1-0_TIME1")
    monkeypatch.setattr("haishui_cli.linux_desktop_entry.time.sleep", lambda _s: None)
    packaged_exe = _make_packaged_executable(root, monkeypatch)
    ...
    monkeypatch.setattr(
        "haishui_cli.linux_desktop_entry.install_desktop_entry",
        lambda project_root: events.append(f"install:{project_root}") or (tmp_path / "haishui.desktop"),
    )

    def fake_electron(cmd, **kwargs):
        events.append("spawn")
        fd = int(kwargs["env"]["HAISHUI_DESKTOP_READY_FD"])
    ...
    with patch("haishui_cli.main_desktop._desktop_build_needed", return_value=False), \
         patch("haishui_cli.main_install_repair._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("haishui_cli.main_desktop._desktop_linux_sandbox_fixup", return_value=True), \
         patch("haishui_cli.main.subprocess.run", side_effect=fake_electron), \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_launches_even_when_desktop_entry_install_fails(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("haishui_cli.linux_desktop_entry.is_supported", lambda: True)
    monkeypatch.setattr("haishui_cli.linux_desktop_entry.install_desktop_entry", boom)
    ...
    with patch("haishui_cli.main_desktop._desktop_build_needed", return_value=False), \
         patch("haishui_cli.main_install_repair._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("haishui_cli.main_desktop._desktop_linux_sandbox_fixup", return_value=True), \
         patch("haishui_cli.main.subprocess.run", return_value=launch_ok) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_skips_desktop_entry_off_linux(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("haishui_cli.linux_desktop_entry.is_supported", lambda: False)

    def fail(_project_root):
        raise AssertionError("must not install a desktop entry off Linux")

    monkeypatch.setattr("haishui_cli.linux_desktop_entry.install_desktop_entry", fail)
...
def test_desktop_launch_options_normalizes_password_store(raw, expected):
    ...
    with patch("haishui_cli.config.load_config", return_value=cfg):
...
def test_desktop_launch_options_ozone_hint_defaults_auto():
    with patch("haishui_cli.config.load_config", return_value={}):
...
def test_gui_bridges_ozone_hint_to_launch_env(tmp_path, monkeypatch):
    ...
    with patch("haishui_cli.main_install_repair._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("haishui_cli.main_web_build._run_npm_install_deterministic", return_value=ok), \
         patch("haishui_cli.main_desktop._desktop_build_needed", return_value=True), \
         patch("haishui_cli.main_desktop._write_desktop_build_stamp"), \
         patch("haishui_cli.main_desktop._desktop_macos_relaunchable_fixup"), \
         patch("haishui_cli.main_desktop._desktop_linux_sandbox_fixup", return_value=True), \
         patch("haishui_cli.config.load_config", return_value=cfg), \
         patch("haishui_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("haishui_cli.main.subprocess.run", side_effect=_pack_into_staging(root)) as mock_run, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
    ...
    with patch("haishui_cli.main_install_repair._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         ...
         patch("haishui_cli.config.load_config", return_value=cfg), \
         patch("haishui_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("haishui_cli.main.subprocess.run", side_effect=_pack_into_staging(root)) as mock_run2, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
...
def _clear_keychain_env(monkeypatch):
    for var in (
        ...
        "HAISHUI_DESKTOP_PASSWORD_STORE",
    ):
...
def test_detect_linux_password_store_via_dbus_secret_service(monkeypatch):
    ...
    with patch("haishui_cli.main.subprocess.run", return_value=ping_ok) as mock_run:
        assert main_desktop._detect_linux_password_store() == "gnome-libsecret"
...
def test_detect_linux_password_store_none_when_no_keychain(monkeypatch):
    ...
    with patch("haishui_cli.main.subprocess.run", return_value=ping_fail):
        assert main_desktop._detect_linux_password_store() is None
    with patch("haishui_cli.main.subprocess.run", side_effect=FileNotFoundError):
        assert main_desktop._detect_linux_password_store() is None
...
def test_gui_linux_packaged_launch_bridges_detected_password_store(tmp_path, monkeypatch):
    ...
    with patch("haishui_cli.main_install_repair._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         ...
         patch("haishui_cli.main_desktop._desktop_macos_relaunchable_fixup"), \
         patch("haishui_cli.main_desktop._desktop_linux_sandbox_fixup", return_value=True), \
         patch("haishui_cli.config.load_config", return_value={}), \
         patch("haishui_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("haishui_cli.main_desktop._detect_linux_password_store", return_value="gnome-libsecret"), \
         patch("haishui_cli.main.subprocess.run", side_effect=_pack_into_staging(root)) as mock_run, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())

    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert launch_env["HAISHUI_DESKTOP_PASSWORD_STORE"] == "gnome-libsecret"
...
def test_gui_linux_source_launch_bridges_detected_password_store(tmp_path, monkeypatch):
    ...
    with patch("haishui_cli.main_install_repair._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         ...
         patch("haishui_cli.config.load_config", return_value={}), \
         patch("haishui_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("haishui_cli.main_desktop._detect_linux_password_store", return_value="kwallet6"), \
         patch("haishui_cli.main.subprocess.run", side_effect=_pack_into_staging(root)) as mock_run, \
         pytest.raises(SystemExit):
    ...
    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert launch_env["HAISHUI_DESKTOP_PASSWORD_STORE"] == "kwallet6"
...
def test_gui_config_password_store_skips_detection(tmp_path, monkeypatch):
    ...
    with patch("haishui_cli.main_install_repair._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         ...
         patch("haishui_cli.config.load_config", return_value=cfg), \
         patch("haishui_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("haishui_cli.main_desktop._detect_linux_password_store") as mock_detect, \
         patch("haishui_cli.main.subprocess.run", side_effect=_pack_into_staging(root)) as mock_run, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
...
def test_gui_explicit_password_store_env_wins_over_config_and_detection(tmp_path, monkeypatch):
    _clear_keychain_env(monkeypatch)
    monkeypatch.setenv("HAISHUI_DESKTOP_PASSWORD_STORE", "basic")
    root = _make_desktop_tree(tmp_path)
    monkeypatch.setattr(cli_main, "PROJECT_ROOT", root)
    _make_packaged_executable(root, monkeypatch)
    ...
    mock_detect.assert_not_called()
    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert launch_env["HAISHUI_DESKTOP_PASSWORD_STORE"] == "basic"
...
def test_gui_password_store_bridge_is_linux_only(tmp_path, monkeypatch):
    ...
    mock_detect.assert_not_called()
    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert "HAISHUI_DESKTOP_PASSWORD_STORE" not in launch_env
...
def _gui_build_patches(root: Path, run_side_effect):
    return [
        ...
        patch("haishui_cli.main_desktop._desktop_exe_integrity_error", return_value=None),
        patch("haishui_cli.main_web_build._run_npm_install_deterministic",
              return_value=subprocess.CompletedProcess(["npm", "ci"], 0)),
        patch("haishui_cli.main_desktop._desktop_build_needed", return_value=True),
        patch("haishui_cli.main_desktop._write_desktop_build_stamp"),
        patch("haishui_cli.main_desktop._desktop_macos_relaunchable_fixup"),
        patch("haishui_cli.main_desktop._register_linux_desktop_entry"),
        patch("haishui_cli.main_desktop._stop_desktop_processes_locking_build", return_value=[]),
        patch("haishui_cli.main_desktop._purge_electron_build_cache", return_value=[]),
        patch("haishui_cli.main_desktop._redownload_electron_dist", return_value=False),
        patch("haishui_cli.main.subprocess.run", side_effect=run_side_effect),
    ]
