...
leaked shell variable, so ``scripts/run_tests.sh``'s ``env -i`` was no help:
  ...
  1. A test drives the ``voice.toggle`` RPC with ``action="tts"``. The
     ...
     ``os.environ["HAISHUI_VOICE_TTS"] = "1"``.
  ...
  3. Any later test that drives a turn to completion hits the TTS dispatch in
     ``prompt.submit``, which calls ``haishui_cli.voice.speak_text`` on a
...
Two independent defences in ``tests/conftest.py``, one test each below:
  ...
  * ``_HAISHUI_BEHAVIORAL_VARS`` now blanks ``HAISHUI_VOICE`` /
    ``HAISHUI_VOICE_TTS`` at every test setup, so step 2 cannot cross a test
    boundary.
...
def test_voice_toggle_still_leaks_the_env_var_but_speech_is_stubbed(monkeypatch):
    ...
    assert os.environ.get("HAISHUI_VOICE_TTS") == "1"
    ...
    # calls it (a late `from haishui_cli.voice import speak_text`), with the
...
def test_voice_env_does_not_leak_into_the_next_test():
    ...
    one above, in the same process, which left ``HAISHUI_VOICE_TTS=1`` set.
    Before ``HAISHUI_VOICE``/``HAISHUI_VOICE_TTS`` were added to
    ``_HAISHUI_BEHAVIORAL_VARS``, this assertion failed.
    """
    assert "HAISHUI_VOICE_TTS" not in os.environ
    assert "HAISHUI_VOICE" not in os.environ
...
def test_guard_can_be_opted_out_of_explicitly():
    ...
    import haishui_cli.voice as voice
