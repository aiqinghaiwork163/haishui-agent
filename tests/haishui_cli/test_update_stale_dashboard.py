"""Tests for the stale-dashboard handling run at the end of ``haishui update``.

``haishui update`` detects ``haishui dashboard`` processes left over from the
...
from haishui_cli.main_dashboard import _find_stale_dashboard_pids
from haishui_cli.dashboard_procs import _kill_stale_dashboard_processes
from haishui_cli import dashboard_procs
from haishui_cli import main_dashboard
from haishui_cli import update_cmd
from haishui_cli.update_cmd import _finish_dashboard_update_cleanup
from haishui_cli.main_dashboard import _restart_managed_dashboard_service
from haishui_cli.dashboard_procs import _kill_stale_dashboard_processes as _warn_stale_dashboard_processes
...
def _write_valid_ssh_backend_lock(tmp_path, monkeypatch) -> int:
    ...
    (lock_dir / "backend.lock.json").write_text(json.dumps({
        ...
        "port": 46369,
        "profile": "default",
        "haishuiPath": "/opt/haishui/bin/haishui",
        "haishuiHome": str(tmp_path),
    ...
    monkeypatch.delenv("HAISHUI_DESKTOP_CHILD_PID", raising=False)
    return pid
...
def test_update_cleanup_spares_backend_owned_by_valid_ssh_lock(tmp_path, monkeypatch):
    ...
    with patch(
        "haishui_cli.main_dashboard._find_stale_dashboard_pids",
    ...
    def test_self_pid_excluded(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(os.getpid(), "python3 -m haishui_cli.main dashboard"),
                    _ps_line(12345, "haishui dashboard --port 9119"),
                ]) + "\n",
                stderr="",
            )
...
class TestKillStaleDashboardPosix:
    ...
    def test_sigterm_graceful_exit(self, capsys):
        ...
        with patch("haishui_cli.main_dashboard._find_stale_dashboard_pids",
    ...
    def test_user_scope_restart_never_falls_back_to_system_or_sudo(self, capsys):
        ...
        def fake_run(args, *a, **kw):
            ...
            if args == ["systemctl", "--user", "list-unit-files", "haishui-dashboard.service", "--no-legend", "--no-pager"]:
                return MagicMock(returncode=0, stdout="haishui-dashboard.service enabled enabled\n", stderr="")
            if args == ["systemctl", "--user", "is-active", "haishui-dashboard.service"]:
                return MagicMock(returncode=0, stdout="active\n", stderr="")
            if args == ["systemctl", "--user", "is-enabled", "haishui-dashboard.service"]:
                return MagicMock(returncode=0, stdout="enabled\n", stderr="")
            if args == ["systemctl", "--user", "restart", "haishui-dashboard.service"]:
                return MagicMock(returncode=0, stdout="", stderr="")
            raise AssertionError(f"unexpected subprocess.run call: {args}")

        with patch("subprocess.run", side_effect=fake_run), \
             patch("haishui_cli.main_dashboard._find_stale_dashboard_pids", return_value=[]) as find_pids, \
             patch("os.kill") as kill:
            _kill_stale_dashboard_processes(restart_managed=True)

        assert calls == [
            ["systemctl", "--user", "list-unit-files", "haishui-dashboard.service", "--no-legend", "--no-pager"],
            ["systemctl", "--user", "is-active", "haishui-dashboard.service"],
            ["systemctl", "--user", "is-enabled", "haishui-dashboard.service"],
            ["systemctl", "--user", "restart", "haishui-dashboard.service"],
        ]
        ...
        assert "✓ restarted haishui-dashboard.service" in capsys.readouterr().out
...
class TestKillStaleDashboardWindows:
    ...
    def test_taskkill_invoked_for_each_pid(self, capsys):
        ...
        with patch("haishui_cli.main_dashboard._find_stale_dashboard_pids",
             ...
             patch("haishui_cli._subprocess_compat.pid_is_haishui", return_value=True), \
...
class TestDashboardUpdateCleanup:
    ...
    def test_all_failed_stops_do_not_claim_the_dashboard_was_stopped(self, capsys, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("HAISHUI_HOME", str(own_home))
        with patch(
            "haishui_cli.main._kill_stale_dashboard_processes",
...
class TestWindowsWmicEncoding:
    ...
    `haishui update` on non-UTF-8 system locales (e.g. cp936 on zh-CN).
    """
    ...
    def test_wmic_routed_through_bounded_probe_run_with_ignore_errors(self):
        ...
        with patch("sys.platform", "win32"), \
             patch("haishui_cli._subprocess_compat.bounded_probe_run") as mock_probe:
            mock_probe.return_value = subprocess.CompletedProcess(
                args=["wmic"],
                returncode=0,
                stdout=(
                    "CommandLine=python -m haishui_cli.main dashboard\n"
                    "ProcessId=12345\n"
                ),
                stderr="",
            )
    ...
    def test_probe_failure_fails_open_to_empty_list(self):
        ...
        with patch("sys.platform", "win32"), \
             patch("haishui_cli._subprocess_compat.bounded_probe_run",
                   return_value=None):
            assert _find_stale_dashboard_pids() == []
...
class TestSupervisedBackendRestart:
    ...
    def test_supervised_pid_restarts_owning_unit(self, capsys):
        ...
        with patch.object(main_dashboard, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch.object(main_dashboard, "_get_pid_cgroup_path",
                          return_value="/system.slice/haishui-serve.service"), \
             patch.object(main_dashboard, "_get_systemd_service_for_pid",
                          return_value="haishui-serve.service"), \
        ...
        restart.assert_called_once_with(
            "haishui-serve.service", "/system.slice/haishui-serve.service"
        )
        out = capsys.readouterr().out
        assert "✓ restarted systemd service haishui-serve.service" in out
    ...
    def test_already_restarted_unit_is_left_untouched(self):
        """Review on #83595: haishui update's systemd fleet-restart loop may
        ...
        with patch.object(main_dashboard, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch("time.sleep"):
            result = _kill_stale_dashboard_processes(
                restart_managed=True, already_restarted_units={"haishui-serve"}
            )
...
class TestManualBackendRespawn:
    ...
    def test_non_orphan_fixed_port_still_respawns(self, capsys):
        ...
        live = self._live()
        argv = ["haishui", "dashboard", "--port", "8300"]
        ...
        with patch.object(main_dashboard, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch("haishui_cli.dashboard_procs._haishui_home_for_pid", return_value=None), \
    ...
    def test_respawn_adds_no_open_to_dashboard_commands(self, tmp_path, monkeypatch):
        ...
        with patch.object(live.subprocess, "Popen", _FakePopen):
            failed = live._respawn_dashboard_processes([
                ["haishui", "dashboard", "--port", "8300"],
                ["haishui", "serve", "--host", "0.0.0.0"],
            ])

        assert failed == []
        assert spawned[0] == ["haishui", "dashboard", "--port", "8300", "--no-open"]
        assert spawned[1] == ["haishui", "serve", "--host", "0.0.0.0"]
    ...
    def test_respawn_failure_returned(self, tmp_path, monkeypatch, capsys):
        ...
        with patch.object(live.subprocess, "Popen", side_effect=OSError("no such file")):
            failed = live._respawn_dashboard_processes([["haishui", "serve"]])

        assert failed == [["haishui", "serve"]]
        out = capsys.readouterr().out
...
class TestFilterDashboardRespawnCandidates:
    ...
    def test_skips_serve_port_zero(self):
        from haishui_cli.dashboard_procs import _filter_dashboard_respawn_candidates
        ...
        assert _filter_dashboard_respawn_candidates([
            (42, argv, "/home/u/.haishui/profiles/mini-cat"),
        ]) == []
    ...
    def test_keeps_ppid1_fixed_port_for_repeat_update(self):
        ...
        argv = ["haishui", "dashboard", "--port", "9119"]
        assert _filter_dashboard_respawn_candidates([(10, argv, None)]) == [argv]

    def test_dedupes_identical_normalized_cmdlines(self):
        from haishui_cli.dashboard_procs import _filter_dashboard_respawn_candidates

        a = ["/usr/bin/python3", "-m", "haishui_cli.main", "dashboard", "--port", "8300"]
        b = ["/other/python", "-m", "haishui_cli.main", "dashboard", "--port", "8300"]
    ...
    def test_caps_one_per_profile(self):
        ...
        a = ["haishui", "--profile", "coder", "dashboard", "--port", "8300"]
        b = ["haishui", "--profile", "coder", "dashboard", "--port", "8301"]
        c = ["haishui", "--profile", "writer", "dashboard", "--port", "8302"]
        out = _filter_dashboard_respawn_candidates([
            (1, a, None),
            (2, b, None),
            (3, c, None),
    ...
    def test_caps_one_per_haishui_home(self):
        from haishui_cli.dashboard_procs import _filter_dashboard_respawn_candidates

        home = "/tmp/haishui-home-a"
        a = ["haishui", "dashboard", "--port", "8300"]
        b = ["haishui", "dashboard", "--port", "8301"]
        out = _filter_dashboard_respawn_candidates(
            [
                (1, a, home),
                (2, b, home),
    ...
    def test_profile_flag_and_profiles_home_share_cap(self):
        ...
        out = _filter_dashboard_respawn_candidates([
            (1, a, None),
            (2, b, "/home/u/.haishui/profiles/coder"),
        ])
        assert out == [a]
    ...
    def test_default_profile_same_root_home_caps(self):
        ...
        a = ["haishui", "--profile", "default", "dashboard", "--port", "8300"]
        b = ["haishui", "dashboard", "--port", "8301"]
        home = "/home/u/.haishui"
        out = _filter_dashboard_respawn_candidates(
            [
    ...
    def test_foreign_home_backend_is_not_replayed(self):
        """A backend from another HAISHUI_HOME is never respawned (#94030).
        ...
        out = _filter_dashboard_respawn_candidates(
            [
                (1, a, "/home/u/.haishui"),
                (2, b, "/work/project/.haishui"),
            ],
            own_home="/home/u/.haishui",
        )
        assert out == [a]
    ...
    def test_skips_sidecar_fixed_port_serve_on_foreign_home(self):
        ...
        argv = [
            "/Users/u/.haishui-sidecar/haishui-agent/venv/bin/python",
            "-m", "haishui_cli.main",
            "serve", "--host", "127.0.0.1", "--port", "9118", "--skip-build",
        ]
        out = _filter_dashboard_respawn_candidates(
            [(15364, argv, "/Users/u/.haishui-lifeos")],
            own_home="/Users/u/.haishui",
        )
        assert out == []

    def test_matching_haishui_home_is_kept(self):
        from haishui_cli.dashboard_procs import _filter_dashboard_respawn_candidates

        argv = ["haishui", "serve", "--host", "127.0.0.1", "--port", "9118"]
        out = _filter_dashboard_respawn_candidates(
            [(15364, argv, "/Users/u/.haishui")],
            own_home="/Users/u/.haishui",
        )
        assert out == [argv]

    def test_symlinked_haishui_home_compares_equal(self, tmp_path):
        from haishui_cli.dashboard_procs import _filter_dashboard_respawn_candidates
    ...
    def test_unknown_home_stays_eligible(self):
        """Unreadable HAISHUI_HOME (env probe failed) keeps pre-#94030 behaviour."""
        from haishui_cli.dashboard_procs import _filter_dashboard_respawn_candidates
    ...
    def test_own_home_defaults_to_get_haishui_home(self, monkeypatch):
        ...
        monkeypatch.setattr(
            haishui_constants, "get_haishui_home", lambda: Path("/home/u/.haishui")
        )
        argv = ["haishui", "serve", "--port", "9118"]
        foreign = _filter_dashboard_respawn_candidates([
            (1, argv, "/Users/u/.haishui-lifeos"),
        ])
        assert foreign == []
        own = _filter_dashboard_respawn_candidates([
            (1, argv, "/home/u/.haishui"),
        ])
        assert own == [argv]
    ...
    def test_keeps_fixed_port_serve(self):
        ...
        argv = ["haishui", "serve", "--host", "0.0.0.0", "--port", "9119"]
        assert _filter_dashboard_respawn_candidates([
            (9, argv, None),
        ]) == [argv]
...
class TestCmdlineCapture:
    ...
    def test_reads_proc_cmdline_when_available(self, tmp_path, monkeypatch):
        ...
        proc_file.write_bytes(b"/usr/bin/python3\x00-m\x00haishui_cli.main\x00serve\x00")
        ...
        assert argv == ["/usr/bin/python3", "-m", "haishui_cli.main", "serve"]
...
class TestPostUpdateDashboardCleanupIsolation:
    ...
    def test_cleanup_failure_is_isolated_and_recorded(self, capsys):
        ...
        import haishui_cli.update_receipt as ur
        from haishui_cli import update_cmd
        ...
        try:
            ...
            with patch(
                ...
                side_effect=AttributeError(
                    "module 'haishui_cli.main_dashboard' has no attribute '_loaded_launchd_backend_jobs'"
                ),
            ):
...
class TestLaunchdSupervisedBackends:
    ...
    ARGV = [
        "/opt/haishui/venv/bin/python", "-m", "haishui_cli.main",
    ...
    def _run(self, pid, jobs, *, restart_ok=True, restart_managed=True):
        ...
        with patch.object(main_dashboard, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch("haishui_cli.dashboard_procs._process_ancestors", return_value=[]), \
             patch("haishui_cli.dashboard_procs._haishui_home_for_pid", return_value=None), \
             patch.object(live, "_respawn_dashboard_processes", return_value=[]) as respawn, \
    ...
    def test_launchd_owned_backend_restarts_through_launchd_never_as_a_detached_respawn(self, capsys):
        ...
        job = ("system", "ai.haishui.dashboard", list(self.ARGV), None)
        result, restart, respawn = self._run(9102, [job])
        respawn.assert_not_called()
        restart.assert_called_once_with("system", "ai.haishui.dashboard", None)
        assert result["killed"] == [9102] and result["unrecovered"] == []
        out = capsys.readouterr().out
        assert "✓ restarted launchd job system/ai.haishui.dashboard" in out
        assert "when you're ready" not in out

        result, restart, respawn = self._run(9103, [("gui/501", "ai.haishui.dashboard", list(self.ARGV), 9103)],
                                             restart_ok=False)
        respawn.assert_not_called()
        assert result["unrecovered"] == [9103]
        assert "run: launchctl kickstart -k gui/501/ai.haishui.dashboard" in capsys.readouterr().out

        # A LaunchDaemon lives in the system domain: kickstart needs root, so a non-root hint says sudo.
        with patch("haishui_cli.dashboard_procs.os.geteuid", return_value=501, create=True):  # windows-footgun: ok — patch target string, create=True
            self._run(9105, [("system", "ai.haishui.serve", list(self.ARGV), None)], restart_ok=False)
        assert "run: sudo launchctl kickstart -k system/ai.haishui.serve" in capsys.readouterr().out

        other = ("gui/501", "ai.haishui.other", ["haishui", "dashboard", "--port", "8300"], 777)
    ...
    def test_loaded_backend_jobs_scans_plists_and_attributes_pids_host_independently(
        ...
        agents.mkdir()
        daemons.mkdir()
        backend_argv = ["/opt/haishui/venv/bin/python", "-m", "haishui_cli.main", "dashboard", "--port", "9119"]
        serve_argv = ["/opt/haishui/venv/bin/python", "-m", "haishui_cli.main", "serve", "--port", "8642"]
        gateway_argv = ["/opt/haishui/venv/bin/python", "-m", "haishui_cli.main", "gateway", "run"]
        for path, label, argv in (
            (agents / "ai.haishui.dashboard.plist", "ai.haishui.dashboard", backend_argv),
            (agents / "ai.haishui.gateway.plist", "ai.haishui.gateway", gateway_argv),
            (agents / "ai.haishui.serve-old.plist", "ai.haishui.serve-old", serve_argv),
            (daemons / "ai.haishui.serve.plist", "ai.haishui.serve", serve_argv),
        ):
        ...
        def fake_print(domain, label):
            ...
            if (domain, label) == (f"user/{uid}", "ai.haishui.dashboard"):
                return (True, 4242)
            if (domain, label) == ("system", "ai.haishui.serve"):
                return (True, None)
            return (False, None)
        ...
        with patch("haishui_cli.gateway._launchd_print_service_pid", side_effect=fake_print):
            jobs = _no_real_launchd_jobs([("agent", agents), ("daemon", daemons)])

        assert jobs == [
            (f"user/{uid}", "ai.haishui.dashboard", backend_argv, 4242),
            ("system", "ai.haishui.serve", serve_argv, None),
        ]
        assert not any(label == "ai.haishui.gateway" for _domain, label in probed)
        assert probed[:2] == [(f"gui/{uid}", "ai.haishui.dashboard"), (f"user/{uid}", "ai.haishui.dashboard")]
        assert not any(domain == "system" and label != "ai.haishui.serve" for domain, label in probed)

        owning = main_dashboard._launchd_job_owning_backend
        assert owning(4242, ["something", "else"], jobs) == (f"user/{uid}", "ai.haishui.dashboard", 4242)
        assert owning(9999, ["python"], jobs, ancestors=[4242, 1]) == (f"user/{uid}", "ai.haishui.dashboard", 4242)
        assert owning(9999, serve_argv, jobs) == ("system", "ai.haishui.serve", None)
        ...
        assert owning(9999, backend_argv + ["--no-open"], jobs) == (f"user/{uid}", "ai.haishui.dashboard", 4242)
