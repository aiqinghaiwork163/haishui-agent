...
from haishui_cli import resource_limits
...
def test_fresh_process_import_without_posix_resource_is_a_safe_noop():
    code = textwrap.dedent(
        ...
        spec = importlib.util.spec_from_file_location(
            "haishui_cli._resource_limits_without_posix_resource",
            module_path,
        )
...
def test_serve_startup_applies_limit_before_web_server(monkeypatch):
    ...
    import haishui_cli.plugins
    import haishui_cli.web_server

    # cmd_dashboard(headless_backend=True) exports HAISHUI_SERVE_HEADLESS=1 into
    ...
    # 404 path.
    monkeypatch.setenv("HAISHUI_SERVE_HEADLESS", "0")
...
def test_named_profile_reroute_defers_limit_to_final_process(monkeypatch, tmp_path):
    ...
    assert calls == []
    assert exec_call["argv"][1:5] == ["-m", "haishui_cli.main", "-p", "default"]
    assert exec_call["env"]["HAISHUI_HOME"] == str(tmp_path)
...
def test_dashboard_lifecycle_flags_skip_limit_adjustment(monkeypatch, lifecycle_flag):
    ...
    from haishui_cli import main as cli_main
    import haishui_cli.main_dashboard as haishui_cli_main_dashboard
    ...
    monkeypatch.setattr(haishui_cli_main_dashboard, "_find_stale_dashboard_pids", lambda **_: [])
