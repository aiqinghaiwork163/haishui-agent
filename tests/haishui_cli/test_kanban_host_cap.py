...
1. The standalone daemon path (``haishui kanban daemon --force`` /
   :func:`haishui_cli.kanban_db_dispatch.run_daemon`) never resolved
...
def test_run_daemon_resolves_and_passes_max_in_progress(
    ...
    ``haishui kanban dispatch`` paths both resolved the memory-derived
    default.
    """
    captured: dict = {}
