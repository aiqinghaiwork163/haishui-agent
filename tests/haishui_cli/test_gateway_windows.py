"""Tests for haishui_cli.gateway_windows."""
...
import haishui_cli.gateway as gateway
import haishui_cli.gateway_windows as gateway_windows
import haishui_cli.setup as setup


_BREAKAWAY_MARKER = "_HAISHUI_GATEWAY_BREAKAWAY"
...
def test_build_gateway_argv_keeps_venv_console_python_for_uv_venv(monkeypatch, tmp_path):
    ...
    haishui_home = tmp_path / "haishui-home"
    ...
    monkeypatch.setattr(gateway, "_profile_arg", lambda haishui_home: "")
    monkeypatch.setattr("haishui_cli.config.get_haishui_home", lambda: str(haishui_home))

    argv, cwd, env_overlay = gateway_windows._build_gateway_argv()

    assert argv[:3] == [str(venv_python), "-m", "haishui_cli.main"]
    assert cwd == str(haishui_home.resolve())
...
def test_spawn_detached_marks_primary_breakaway_success(monkeypatch, tmp_path, caplog):
    ...
    argv = ["python.exe", "-m", "haishui_cli.main", "gateway", "run"]
    cwd = str(tmp_path)
    calls = []
    ...
    monkeypatch.setattr(
        gateway_windows,
        "_build_gateway_argv",
        lambda home=None: (argv, cwd, {"HAISHUI_GATEWAY_DETACHED": "1"}),
    )
    monkeypatch.setattr("haishui_cli.config.get_haishui_home", lambda: tmp_path)
...
def test_spawn_detached_warns_and_marks_no_breakaway_fallback(
    ...
    monkeypatch.setattr(
        ...
        lambda home=None: (
            argv,
            cwd,
            {"HAISHUI_GATEWAY_DETACHED": "1", "SECRET_SENTINEL": "do-not-log"},
        ),
    )
    monkeypatch.setattr("haishui_cli.config.get_haishui_home", lambda: tmp_path)
    monkeypatch.setattr(gateway_windows.subprocess, "Popen", fake_popen)
...
class TestStableWindowsGatewayWorkingDir:
    def test_stable_gateway_working_dir_uses_haishui_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".haishui"
        home.mkdir()
        monkeypatch.setattr("haishui_cli.config.get_haishui_home", lambda: home)
    ...
    def test_stable_gateway_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        missing = tmp_path / "missing" / ".haishui"
        project = tmp_path / "checkout"
        monkeypatch.setattr("haishui_cli.config.get_haishui_home", lambda: missing)
...
def _arrange_startup_fallback(monkeypatch, tmp_path, running_pids):
    script_path = tmp_path / "Haishui_Gateway_alice.cmd"
    startup_entry = tmp_path / "Startup" / "Haishui_Gateway_alice.cmd"
    calls = []

    monkeypatch.setattr(gateway_windows, "_prompt_install_choices", lambda *args, **kwargs: (False, True))
    monkeypatch.setattr(gateway_windows, "_assert_windows", lambda: None)
    monkeypatch.setattr(gateway_windows, "get_task_name", lambda: "Haishui_Gateway_alice")
...
def test_elevated_gateway_command_uses_hidden_console_python(monkeypatch):
    ...
    monkeypatch.setattr(gateway_windows.sys, "executable", r"C:\Haishui\venv\Scripts\python.exe")
    ...
    assert verb == "runas"
    assert executable == r"C:\Haishui\venv\Scripts\python.exe"
...
def test_install_scheduled_task_recreates_instead_of_change(monkeypatch, tmp_path):
    ...
    ok, detail = gateway_windows._install_scheduled_task("Haishui_Gateway_alice", script_path)

    assert ok is True
    assert "/Change" not in [arg for call in calls for arg in call]
    assert calls[0][:4] == ("/Delete", "/F", "/TN", "Haishui_Gateway_alice")
    ...
    assert "Haishui_Gateway_alice.vbs" in xml_seen["text"]
...
def test_gateway_vbs_script_is_console_less(monkeypatch):
    ...
    assert "haishui_cli.main" in content
    assert "gateway run" in content
    assert ", 0, False" in content  # hidden window, detached/async
    for var in ("HAISHUI_HOME", "PYTHONIOENCODING", "HAISHUI_GATEWAY_DETACHED", "VIRTUAL_ENV", "PYTHONPATH"):
...
def test_atomic_write_leaves_no_staging_file_when_swap_fails(monkeypatch, tmp_path):
    ...
    startup.mkdir()
    entry, staging = startup / "Haishui_Gateway.vbs", startup / "Haishui_Gateway.tmp"
...
def test_uninstall_and_reinstall_sweep_stale_startup_staging_file(monkeypatch, tmp_path):
    ...
    startup = tmp_path / "Startup"
    startup.mkdir()
    entry, staging = startup / "Haishui_Gateway_alice.vbs", startup / "Haishui_Gateway_alice.tmp"
    script = tmp_path / "task" / "Haishui_Gateway_alice.cmd"
    ...
    monkeypatch.setattr(gateway_windows, "_legacy_startup_entry_path", lambda: startup / "Haishui_Gateway_alice.cmd")
...
<Task version="1.3" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  ...
  <Actions Context="Author">
    <Exec>
      ...
      <Arguments>"C:\\Users\\me\\.haishui\\gateway-service\\Haishui_Gateway.vbs"</Arguments>
    </Exec>
  </Actions>
</Task>
"""
...
def test_scheduled_task_drift_names_missing_hardening_leaves(monkeypatch):
    ...
    ``haishui gateway status`` prints together with the ``haishui gateway install`` repair hint."""
    launcher = Path(r"C:\Users\me\.haishui\gateway-service\Haishui_Gateway.vbs")
    template = gateway_windows._build_scheduled_task_xml("Haishui_Gateway", launcher, r"PC\me")
...
def test_reconcile_scheduled_task_reregisters_only_on_drift(monkeypatch, tmp_path):
    ...
    assert gateway_windows.reconcile_scheduled_task("Haishui_Gateway") is True
    ...
    calls.clear()
    assert gateway_windows.reconcile_scheduled_task("Haishui_Gateway") is False
...
def _arrange_uninstalled_start(monkeypatch):
    ...
    installs, spawns = [], []
    monkeypatch.delenv("HAISHUI_GATEWAY_INSTALL_START_ON_LOGIN", raising=False)
    monkeypatch.delenv("HAISHUI_NONINTERACTIVE", raising=False)
    monkeypatch.setattr(gateway_windows, "_assert_windows", lambda: None)
...
def test_stdin_interactive_only_when_isatty_and_a_console_answers_get_console_mode():
    """Windows CRT isatty() is True for the NUL device (`haishui gateway start < NUL`, stdin=DEVNULL), so
...
def test_start_with_nul_stdin_starts_the_gateway_but_never_installs_login_persistence(monkeypatch, capsys):
    ...
    assert "haishui gateway install" in capsys.readouterr().out


def test_start_without_tty_starts_the_gateway_but_never_installs_login_persistence(monkeypatch, capsys):
    """`haishui gateway start < /dev/null` must not answer the persistence question with a default Yes
    ...
    assert "haishui gateway install" in out and "did not complete" not in out
...
def test_start_on_tty_hands_both_answers_to_install_and_honours_the_env_opt_out(monkeypatch):
    ...
    again). HAISHUI_GATEWAY_INSTALL_START_ON_LOGIN=0 → no question, no install, a plain start."""
    installs, spawns = _arrange_uninstalled_start(monkeypatch)
    monkeypatch.setattr(setup, "is_interactive_stdin", lambda: True)
    ...
    installs.clear()
    monkeypatch.setenv("HAISHUI_GATEWAY_INSTALL_START_ON_LOGIN", "0")
...
# resume_pending=True) never fires when `haishui gateway stop` kills the
