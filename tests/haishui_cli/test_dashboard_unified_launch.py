...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "haishui_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # HAISHUI_HOME.  For a standard install (HAISHUI_HOME unset) that root is
        # the platform-native default (~/.haishui), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from haishui_constants import get_default_haishui_root
        assert env.get("HAISHUI_HOME") == str(get_default_haishui_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (HAISHUI_DESKTOP=1) must NOT
