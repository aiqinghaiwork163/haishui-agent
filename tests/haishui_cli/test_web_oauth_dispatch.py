"""Regression tests for the OAuth dispatcher in haishui_cli.web_server.
...
Haishui must not mint subscription OAuth tokens from an unattended HTTP
...
import haishui_cli.web_routers.oauth as _rt_oauth
import haishui_cli.web_server_oauth as _web_server_oauth

client = TestClient(app)
HEADERS = {"X-Haishui-Session-Token": _SESSION_TOKEN}
...
def test_minimax_login_does_not_launch_anthropic_flow():
    ...
    with patch(
        "haishui_cli.auth._minimax_request_user_code",
        return_value=fake_user_code_resp,
    ), patch(
        "haishui_cli.auth._minimax_pkce_pair",
        return_value=("verifier-stub", "challenge-stub", "stub-state"),
    ), patch(
        "haishui_cli.web_server_oauth._minimax_poller",
        return_value=None,
    ):
        resp = client.post(
...
def test_minimax_start_route_honors_poller_mock_on_owning_module(tmp_path, monkeypatch):
    ...
    with patch(
        ...
        "haishui_cli.auth._minimax_poll_token",
        fail_poll_token,
    ), patch(
        "haishui_cli.web_server_oauth._minimax_poller",
        fake_poller,
...
def test_oauth_provider_status_uses_profile_query(tmp_path, monkeypatch):
    from haishui_cli import web_server as ws
    ...
    def fake_status():
        observed_homes.append(get_haishui_home())
    ...
    fake_catalog = ({
        ...
        "name": "Fake OAuth",
        "flow": "pkce",
        "cli_command": "haishui auth add fake-oauth",
...
def test_codex_dashboard_start_rewords_device_authorization_error(monkeypatch):
    ...
    try:
        ...
        assert "haishui auth" not in detail
    finally:
...
def test_anthropic_dashboard_oauth_is_removed_and_external():
    ...
    assert providers["anthropic"]["cli_command"] == "haishui auth add anthropic"
...
def test_accounts_offers_every_oauth_provider_from_catalog():
    ...
    `haishui model` universe) must be offered by /api/providers/oauth. This keeps
    ...
    for d in provider_catalog():
        if d.tab == "accounts":
            assert d.slug in offered, (
                f"{d.slug} is an accounts-tab provider in `haishui model` but is "
...
def test_oauth_catalog_marks_external_providers_not_disconnectable():
    """External CLI credentials are visible in Accounts but cannot be removed by Haishui."""
    resp = client.get("/api/providers/oauth", headers=HEADERS)
    assert resp.status_code == 200, resp.text
    providers = {p["id"]: p for p in resp.json()["providers"]}
...
def test_xai_dashboard_poller_seeds_single_entry_and_clears_suppression(tmp_path, monkeypatch):
    ...
    ``device_code`` suppression left by a prior ``haishui auth remove
    xai-oauth``.
    """
    from haishui_cli import auth as auth_mod
    from haishui_cli import web_server as ws
    ...
    # Prior `haishui auth remove xai-oauth` left the source suppressed.
...
def test_status_falls_through_to_generic_dispatcher_for_catalog_only_provider():
    ...
    logged-out; now they dispatch to haishui_cli.auth.get_auth_status (the
    ...
    with patch("haishui_cli.auth.get_auth_status", return_value=fake_status):
