"""Shallow-checkout guard on the `haishui update` apply path (#53479).
...
def _run_count_block(*, shallow: bool, raw_count: str, api_count):
    ...
    with patch.object(update_cmd, "subprocess") as sub:
        ...
        with patch("haishui_cli.banner._github_compare_behind", return_value=api_count):
...
def test_shallow_zero_count_short_circuits_without_api():
    with patch("haishui_cli.banner._github_compare_behind") as api:
