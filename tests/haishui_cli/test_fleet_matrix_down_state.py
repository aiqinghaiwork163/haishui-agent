...
def _setup(monkeypatch, tmp_path, record: dict):
    ...
    monkeypatch.setattr("haishui_cli.profiles._get_default_haishui_home", lambda: home)
    monkeypatch.setattr(
...
def test_killed_never_replaced_gateway_is_a_down_row(monkeypatch, tmp_path):
    _setup(
        monkeypatch,
        tmp_path,
        {"pid": _DEAD_PID, "gateway_state": "running", "kind": "haishui-gateway",
         "code_version": "0.20.5"},
    )
...
def test_cleanly_stopped_gateway_is_not_down(monkeypatch, tmp_path):
    for benign_state in ("stopped", "startup_failed"):
        _setup(
            monkeypatch,
            tmp_path,
            {"pid": _DEAD_PID, "gateway_state": benign_state, "kind": "haishui-gateway"},
        )
