...
``haishui update`` / ``haishui dashboard --stop`` fall back to ``_kill_pids_posix`` for a
...
# Chat TUI): `haishui dashboard --stop` / `haishui update` must not SIGTERM their own process.
_STOPPER_CHILD = textwrap.dedent(
    """
    import os, pathlib, sys
    from haishui_cli import dashboard_procs as dp
