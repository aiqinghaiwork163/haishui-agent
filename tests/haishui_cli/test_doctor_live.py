"""Tests for ``haishui doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from haishui_cli import doctor_live
from haishui_cli.doctor_live import (
    ProbeResult,
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `haishui doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("haishui_cli.doctor.HAISHUI_HOME", tmp_path / "home")
        monkeypatch.setattr("haishui_cli.doctor.PROJECT_ROOT", tmp_path / "root")
