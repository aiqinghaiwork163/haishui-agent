...
from haishui_cli.subcommands.dashboard import build_dashboard_parser
...
from haishui_cli.subcommands.whatsapp import build_whatsapp_parser
...
def test_config_get_unset_subcommands_parse():
    """`haishui config get/unset` parse key args (and --json for get)."""
...
# ── deprecated `haishui login` fails gracefully, not with argparse error ────
#
# `haishui login` is a removed command; its handler (`login_command` in
# `haishui_cli/auth.py`) prints a deprecation notice pointing at `haishui auth` /
# `haishui model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `haishui --help`.
