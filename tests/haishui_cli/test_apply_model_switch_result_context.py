...
``HaishuiCLI._apply_model_switch_result()`` printed ``ModelInfo.context_window``
...
def _run_display(monkeypatch, result):
    ...
    # Avoid writing to ~/.haishui/config.yaml during the test.
    monkeypatch.setattr(cli_mod, "save_config_value", lambda *a, **k: None)
    cli_mod.HaishuiCLI._apply_model_switch_result(_StubCLI(), result, False)
    return captured
...
def test_global_switch_clears_context_pin_owned_by_previous_route(monkeypatch):
    ...
    with (
        ...
        patch("haishui_cli.config.read_user_config_raw", return_value=configured),
    ):
        cli_mod.HaishuiCLI._apply_model_switch_result(cli, result, True)
