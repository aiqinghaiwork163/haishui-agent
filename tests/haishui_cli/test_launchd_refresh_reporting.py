...
def _stale_plist(tmp_path, monkeypatch, *, registered: bool):
    plist_path = tmp_path / "com.haishui.plist"
    ...
    monkeypatch.setattr(gw, "get_launchd_label", lambda: "com.haishui.agent")
...
def test_install_repair_warns_instead_of_claiming_success(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("haishui_constants.display_haishui_home", lambda: "~/.haishui-work")
    ...
    assert "~/.haishui-work/logs/launchd-reload.log" in out
