...
#88251: ``haishui update`` treated a failed desktop pack as non-fatal, printed
...
import haishui_cli.update_cmd_maint as update_cmd_maint
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "haishui_constants.with_haishui_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "haishui_constants.display_haishui_home", lambda: str(tmp_path), raising=False
    )
...
def test_gateway_exit_code_file_tracks_desktop_rebuild(tmp_path, monkeypatch):
    monkeypatch.setattr(update_cmd, "get_haishui_home", lambda: tmp_path)
