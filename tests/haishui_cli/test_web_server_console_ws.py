"""Dashboard Haishui Console websocket tests."""
...
@pytest.fixture
def console_client(monkeypatch, _isolate_haishui_home):
...
def test_console_ws_cancel_returns_to_prompt(console_client, monkeypatch):
    from haishui_cli.console_engine import ConsoleResult, HaishuiConsoleEngine

    def slow_execute(self, line: str, *, confirmed: bool = False):
        time.sleep(0.2)
        return ConsoleResult("ok", output="late", command=line)

    monkeypatch.setattr(HaishuiConsoleEngine, "execute", slow_execute)
    ...
    with console_client.websocket_connect(_url()) as conn:
        ...
        assert complete["prompt"] == "haishui> "
...
def test_console_cancel_stops_forked_agent_request_before_reporting(console_client, monkeypatch, blocking_provider, stop):
    ...
    from haishui_cli.web_routers import chat_ws
    ...
    skill_dir = get_haishui_home() / "skills" / "console-cancel-probe"
