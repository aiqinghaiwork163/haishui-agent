...
- Bare `haishui setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `haishui setup --quick` runs the narrower "fill in missing items" flow.
- `haishui setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("haishui_cli.setup.ensure_haishui_home", {}),
        ("haishui_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("haishui_cli.config.is_managed", {"return_value": False}),
        ("haishui_cli.setup.load_config", {"return_value": {}}),
        ("haishui_cli.setup.save_config", {}),
        ("haishui_cli.setup.get_env_value", {"return_value": None}),
        ("haishui_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("haishui_cli.setup._print_setup_summary", {}),
        ("haishui_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("haishui_cli.setup.load_config", {"return_value": {}}),
        ("haishui_cli.setup.save_config", {}),
        ("haishui_cli.auth.get_active_provider", {"return_value": None}),
        ("haishui_cli.setup.get_env_value", {"return_value": None}),
        ("haishui_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `haishui setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="haishui_cli.setup.prompt_choice",
                quick="haishui_cli.setup._run_quick_setup",
                model="haishui_cli.setup.setup_model_provider",
                terminal="haishui_cli.setup.setup_terminal_backend",
                agent="haishui_cli.setup.setup_agent_settings",
                gateway="haishui_cli.setup.setup_gateway",
                tools="haishui_cli.setup.setup_tools",
            )
            from haishui_cli.setup import run_setup_wizard
...
class TestFreshInstall:
    ...
    def test_reconfigure_on_fresh_install_falls_through(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("haishui_cli.setup.prompt_choice", {"return_value": 0}),
                first="haishui_cli.setup_quick._run_first_time_quick_setup",
            )
            from haishui_cli.setup import run_setup_wizard
            from haishui_cli import setup as setup_mod
    ...
    def test_blank_slate_runs_inside_navigation_step(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("haishui_cli.setup.prompt_choice", {"return_value": 2}),
                blank="haishui_cli.setup_quick._run_blank_slate_setup",
            )
            from haishui_cli import setup as setup_mod
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "haishui_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["haishui", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
