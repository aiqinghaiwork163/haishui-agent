"""
Tests for haishui_cli.mcp_config — ``haishui mcp`` subcommands.
...
def _isolate_config(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "haishui_cli.config.get_haishui_home", lambda: tmp_path
    )
    config_path = tmp_path / "config.yaml"
    env_path = tmp_path / ".env"
    monkeypatch.setattr(
        "haishui_cli.config.get_config_path", lambda: config_path
    )
    monkeypatch.setattr(
        "haishui_cli.config.get_env_path", lambda: env_path
    )
    return tmp_path
...
class TestMcpList:
    def test_list_empty_config(self, tmp_path, capsys):
        from haishui_cli.mcp_config import cmd_mcp_list
...
class TestMcpRemove:
    def test_remove_existing_server(self, tmp_path, capsys, monkeypatch):
        ...
        from haishui_cli.mcp_config import cmd_mcp_remove
    ...
    def test_remove_cleans_oauth_tokens(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr("builtins.input", lambda _: "y")
        # Also patch get_haishui_home in the mcp_config module namespace
        monkeypatch.setattr(
            "haishui_cli.mcp_config.get_haishui_home", lambda: tmp_path
        )
...
class TestMcpAdd:
    ...
    def test_add_http_server_all_tools(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr(
            "haishui_cli.mcp_config._probe_single_server", mock_probe
        )
        # No auth, accept all tools
        inputs = iter(["n", ""])  # no auth needed, enable all
        monkeypatch.setattr("builtins.input", lambda _: next(inputs))

        from haishui_cli.mcp_config import cmd_mcp_add
    ...
    def test_add_preset_fills_transport(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr(
            "haishui_cli.mcp_config._MCP_PRESETS",
...
class TestMcpTest:
    ...
    def test_test_success(self, tmp_path, capsys, monkeypatch):
        ...
        from haishui_cli.mcp_config import cmd_mcp_test
    ...
    def test_exit_codes_distinguish_failure_from_unknown_server(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr("haishui_cli.mcp_config._probe_single_server", lambda name, cfg, **kw: [])
        ...
        monkeypatch.setattr("haishui_cli.mcp_config._probe_single_server", failing_probe)
    ...
    def test_cli_dispatcher_forwards_test_exit_code(self, tmp_path, monkeypatch):
        """``haishui mcp test`` reaches ``main()`` with the handler's code (the dispatcher used to drop it)."""
        _seed_config(tmp_path, {"ink": {"url": "https://mcp.ml.ink/mcp"}})
        from haishui_cli.main import cmd_mcp
    ...
    def test_probe_uses_configured_connect_timeout(self, monkeypatch):
        ...
        import asyncio
        from haishui_cli import mcp_config
...
class TestProbeEnvResolution:
    ...
    where `haishui mcp add --auth header` sent a literal
    ``Authorization: Bearer ${MCP_X_API_KEY}`` and got 401."""

    def test_resolve_interpolates_header(self, monkeypatch):
        from haishui_cli.mcp_config import _resolve_mcp_server_config
    ...
    def test_probe_resolves_before_connect(self, monkeypatch):
        ...
        import haishui_cli.mcp_config as mc
    ...
    def test_probe_propagates_explicit_connect_timeout_to_config(self, monkeypatch):
        """An explicit `connect_timeout=` override (e.g. `haishui mcp login`'s 315s, extended so a
...
class TestStripBearerPrefix:
    ...
    def test_bare_token_unchanged(self):
        from haishui_cli.mcp_config import _strip_bearer_prefix
...
class TestConfigHelpers:
    def test_save_and_load_mcp_server(self, tmp_path):
        from haishui_cli.mcp_config import _save_mcp_server, _get_mcp_servers
...
class TestDispatcher:
    def test_no_action_shows_list(self, tmp_path, capsys):
        from haishui_cli.mcp_config import mcp_command
...
class TestMcpLogin:
    def test_login_rejects_unknown_server(self, tmp_path, capsys):
        _seed_config(tmp_path, {})
        from haishui_cli.mcp_config import cmd_mcp_login
    ...
    def test_login_false_success_no_token(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr(
            "haishui_cli.mcp_config._probe_single_server",
...
class TestMcpReauth:
    def test_reauth_all_visits_only_oauth_servers_in_order(
        ...
        visited = []
        monkeypatch.setattr(
            "haishui_cli.mcp_config._reauth_oauth_server",
            lambda name, cfg: visited.append(name) or True,
        )
        from haishui_cli.mcp_config import cmd_mcp_reauth
...
def test_tool_filters_keeps_explicit_empty_include():
    ...
    from haishui_cli.mcp_config import _tool_filters
