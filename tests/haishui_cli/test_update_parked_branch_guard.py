"""Regression tests for the parked-branch guard in ``haishui update``.
...
memory-limit``, days behind main) left there by earlier tooling. ``haishui
...
def _no_config(monkeypatch):
    ...
    import haishui_cli.config as haishui_config

    monkeypatch.setattr(haishui_config, "load_config", lambda: {})
...
def test_skip_warning_names_branch_behind_count_and_commands(repo_pair, capsys):
    ...
    assert f"git -C {repo_pair} checkout main && haishui update" in out
...
def test_print_update_completion_carries_branch_and_sha(
    ...
    monkeypatch.setattr(haishui_main, "PROJECT_ROOT", repo_pair)
...
def _patch_update_flow(monkeypatch, repo, run_real_git=True):
    ...
    """
    monkeypatch.setattr(haishui_main, "PROJECT_ROOT", repo)
    ...
    monkeypatch.setattr(haishui_main, "_record_bytecode_fingerprint", lambda *a, **k: None)
    ...
    monkeypatch.setattr(haishui_main, "_pause_windows_gateways_for_update", lambda: None)
    monkeypatch.setattr(
        haishui_main, "_resume_windows_gateways_after_update", lambda *a, **k: None
    )
    monkeypatch.setattr(haishui_main, "_capture_active_lazy_features", lambda: [])
    monkeypatch.setattr(haishui_main, "_capture_active_tool_dependencies", lambda: [])
