"""Tests for --ignore-user-config and --ignore-rules flags on `haishui chat`.
...
files. In Haishui the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.haishui/config.yaml`` in ``load_cli_config()``
...
argparse → cmd_chat → cli.main() → HaishuiCLI → AIAgent call chain.
"""
...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("HAISHUI_IGNORE_USER_CONFIG", "HAISHUI_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``HAISHUI_IGNORE_USER_CONFIG=1``.

    When the env var is set, user config at ``<haishui_home>/config.yaml`` is
    ...
    def _write_user_config(self, tmp_path, model_default):
        ...
        # config. With HAISHUI_IGNORE_USER_CONFIG=1, load_cli_config() falls
    ...
    def _reload_cli(self, monkeypatch, tmp_path):
        """Point cli._haishui_home at tmp_path and return a fresh load_cli_config."""
        import cli
        monkeypatch.setattr(cli, "_haishui_home", tmp_path)
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "test-vendor/ignore-user-config-sentinel")
        monkeypatch.setenv("HAISHUI_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestIgnoreRulesEnvGate:
    """The constructor / env var must propagate to ``HaishuiCLI.ignore_rules``
    so ``AIAgent`` is built with ``skip_context_files=True`` and
    ``skip_memory=True``.
    """

    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting HAISHUI_IGNORE_RULES=1 flips HaishuiCLI.ignore_rules True."""
        monkeypatch.setenv("HAISHUI_IGNORE_RULES", "1")

        # Import HaishuiCLI lazily — cli.py has heavy module-init side effects
        # that we don't want to run at test collection time.
        import cli
        importlib.reload(cli)

        # Build only enough of HaishuiCLI to reach the ignore_rules assignment.
        ...
        obj = object.__new__(cli.HaishuiCLI)
        # Replicate the exact logic from cli.py HaishuiCLI.__init__:
        ignore_rules = False  # constructor default
        obj.ignore_rules = ignore_rules or os.environ.get("HAISHUI_IGNORE_RULES") == "1"

        assert obj.ignore_rules is True


class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``haishui_cli/main.py`` must set
    ...
    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("HAISHUI_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("HAISHUI_IGNORE_RULES", raising=False)
        ...
        assert os.environ.get("HAISHUI_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("HAISHUI_IGNORE_RULES") == "1"
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "HAISHUI_IGNORE_USER_CONFIG" not in os.environ
        assert "HAISHUI_IGNORE_RULES" not in os.environ
