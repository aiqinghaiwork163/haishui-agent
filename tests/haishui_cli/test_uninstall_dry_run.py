...
def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(uninstall, "get_haishui_home", lambda: haishui_home)
    monkeypatch.setattr(uninstall, "_is_default_haishui_home", lambda home: False)
    ...
    assert str(haishui_home) in output
    assert project_root.exists()
    assert haishui_home.exists()
