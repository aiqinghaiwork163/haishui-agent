...
import haishui_cli.backup as backup
...
def profiles(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "haishui_cli.profiles._get_default_haishui_home", lambda: default_home
    )
    monkeypatch.setattr(
        "haishui_cli.profiles._get_profiles_root", lambda: tmp_path / "home" / "profiles"
    )
    monkeypatch.setattr(
        "haishui_cli.profiles._PROFILE_ID_RE",
...
class TestSiblingEnumeration:
    ...
    def test_never_raises(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr("haishui_cli.profiles._get_default_haishui_home", _boom)
...
class TestAllProfileSnapshots:
    ...
    def test_one_failing_sibling_does_not_block_others(self, profiles, monkeypatch):
        ...
        def _flaky(label=None, haishui_home=None, keep=None, max_file_size=None):
            if haishui_home == profiles["work"]:
                raise OSError("disk full")
            return real(
                label=label, haishui_home=haishui_home, keep=keep,
