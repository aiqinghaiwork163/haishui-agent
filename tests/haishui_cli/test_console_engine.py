...
from haishui_cli.console_engine import HaishuiConsoleEngine, run_console_repl
...
MUTATING_CONFIRMATION_SMOKE_COMMANDS = [
    ...
    "mcp picker",
    "backup --quick -o /tmp/haishui-console-test.zip",
    "import /tmp/haishui-console-test.zip",
...
def test_sessions_list_and_stats_use_isolated_session_store(_isolate_haishui_home):
...
def test_sessions_export_rejects_oversized_single_before_touching_output(
    ...
    monkeypatch.setattr(haishui_state, "resolved_max_export_messages", lambda: 2)
    materialized = []
    ...
    result = HaishuiConsoleEngine().execute(
...
def test_cron_pause_resume_and_run_require_confirmation(_isolate_haishui_home):
...
def test_repl_runs_non_interactive_lines_without_prompts(_isolate_haishui_home):
    ...
    assert code == 0
    assert "Haishui Console" in stdout.getvalue()
    assert "haishui>" not in stdout.getvalue()
    assert stderr.getvalue() == ""


def test_capture_output_surfaces_string_exit_code_as_command_error():
    from haishui_cli.console_engine import ConsoleCommandError, _capture_output
...
def test_execute_handler_string_exit_returns_error_not_crash(_isolate_haishui_home):
    result = HaishuiConsoleEngine().execute(
...
def test_console_checkpoints_prune_does_not_reprompt_for_orphans(
    _isolate_haishui_home, monkeypatch
):
    ...
    result = HaishuiConsoleEngine().execute("checkpoints prune", confirmed=True)
