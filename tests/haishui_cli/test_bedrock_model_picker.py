...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (HAISHUI_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via HAISHUI_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in HAISHUI_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from haishui_cli.providers import HAISHUI_OVERLAYS
        assert "bedrock" in HAISHUI_OVERLAYS
