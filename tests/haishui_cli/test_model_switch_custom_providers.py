...
The terminal `haishui model` flow already exposes `custom_providers`, but the
...
import haishui_cli.providers as providers_mod
import pytest
import yaml
from haishui_cli.model_switch import list_authenticated_providers, switch_model
from haishui_cli.model_switch_providers import (
    _fetch_picker_live_models,
    _NativePickerModelList,
    _save_discovered_models_to_config,
)
from haishui_cli.providers import resolve_provider_full
...
def _disable_live_custom_provider_model_probe(monkeypatch):
    ...
    monkeypatch.setattr("haishui_cli.models.fetch_api_models", lambda *_a, **_kw: None)
    monkeypatch.setattr(
        "haishui_cli.models.cached_provider_model_ids", lambda *_a, **_kw: []
    )
    monkeypatch.setattr(
        "haishui_cli.models.provider_model_ids", lambda *_a, **_kw: []
    )
    monkeypatch.setattr(
        "haishui_cli.models_local.fetch_ollama_local_models", lambda *_a, **_kw: None
    )


def test_picker_native_probe_failure_falls_back_to_openai_catalog(monkeypatch):
    monkeypatch.setattr(
        "haishui_cli.models_local.should_use_ollama_native_catalog", lambda *a, **k: True
    )
    monkeypatch.setattr(
        "haishui_cli.models._get_ollama_native_headers", lambda *a, **k: {}
    )
    monkeypatch.setattr(
        "haishui_cli.models_local.fetch_ollama_local_models", lambda *a, **k: None
    )
    monkeypatch.setattr(
        "haishui_cli.models.fetch_api_models", lambda *a, **k: ["fallback-model"]
    )
...
def test_picker_native_catalog_is_admitted_to_the_shared_model_cache(monkeypatch):
    ...
    monkeypatch.setattr("haishui_cli.models._get_ollama_native_headers", lambda *a, **k: {})
    monkeypatch.setattr(
        "haishui_cli.models_local.fetch_ollama_local_models", lambda *a, **k: ["qwen3:8b"]
    )
...
def _native_picker_probe(monkeypatch, models_by_call):
    ...
    monkeypatch.setattr(
        "haishui_cli.models_local.fetch_ollama_local_models", lambda *a, **k: next(answers)
    )


def _age_cached_rows(seconds):
    from haishui_cli import models as models_mod
...
def test_picker_native_catalog_uses_the_short_native_ttl(monkeypatch):
    ...
    """
    from haishui_cli.models_local import _OLLAMA_LOCAL_MODELS_CACHE_TTL
    ...
    refreshes = []
    monkeypatch.setattr(
        "haishui_cli.models._spawn_swr_refresh", lambda key, fn=None: refreshes.append(key)
    )
...
def test_picker_empty_native_catalog_is_not_stale_served(monkeypatch):
    ...
    """
    from haishui_cli.models import _PROVIDER_MODELS_CACHE_TTL
...
def test_picker_generic_discovery_preserves_api_mode(monkeypatch):
    ...
    monkeypatch.setattr(
        "haishui_cli.models_local.should_use_ollama_native_catalog", lambda *a, **k: False
    )
    monkeypatch.setattr("haishui_cli.models.cached_fetch_api_models", cached)
...
def test_list_authenticated_providers_includes_custom_providers(monkeypatch):
    ...
    monkeypatch.setattr(providers_mod, "HAISHUI_OVERLAYS", {})
    monkeypatch.setattr("haishui_cli.models.fetch_api_models", lambda *a, **k: [])
...
def test_providers_singular_model_does_not_suppress_ollama_native_discovery(monkeypatch):
    ...
    monkeypatch.setattr(providers_mod, "HAISHUI_OVERLAYS", {})
    monkeypatch.setattr(
        "haishui_cli.models_local.fetch_ollama_local_models",
...
def test_list_authenticated_providers_can_skip_custom_provider_live_probe(monkeypatch):
    ...
    monkeypatch.setattr("haishui_cli.models.fetch_api_models", fetch)
...
def test_list_authenticated_providers_can_probe_active_bare_custom_endpoint(monkeypatch):
    ...
    monkeypatch.setattr(providers_mod, "HAISHUI_OVERLAYS", {})
    monkeypatch.setattr(
        "haishui_cli.models.fetch_api_models",
...
def test_switch_model_accepts_explicit_bare_custom_current_endpoint(monkeypatch):
    ...
    monkeypatch.setattr("haishui_cli.models_validate.validate_requested_model", lambda *a, **k: _MOCK_VALIDATION)
    monkeypatch.setattr("haishui_cli.model_switch.get_model_info", lambda *a, **k: None)
    monkeypatch.setattr("haishui_cli.model_switch.get_model_capabilities", lambda *a, **k: None)
...
def test_switch_model_does_not_send_ollama_headers_to_unrelated_custom_endpoint(monkeypatch):
    ...
    monkeypatch.setattr(
        "haishui_cli.models_local.should_use_ollama_native_catalog",
        fake_native_detection,
    )
    monkeypatch.setattr(
        "haishui_cli.models_local._get_ollama_request_headers",
        lambda: {"Authorization": "Bearer configured-ollama-secret"},
    )
    monkeypatch.setattr(
        "haishui_cli.models._get_provider_config_dict",
        lambda provider: (
    ...
    monkeypatch.setattr("haishui_cli.models_validate.validate_requested_model", fake_validation)
    monkeypatch.setattr("haishui_cli.model_switch.get_model_info", lambda *a, **k: None)
    monkeypatch.setattr("haishui_cli.model_switch.get_model_capabilities", lambda *a, **k: None)
...
def test_picker_selection_resolves_named_custom_provider_model_id(monkeypatch):
    ...
    monkeypatch.setattr(
        "haishui_cli.models_validate.validate_requested_model",
        lambda *a, **k: _MOCK_VALIDATION,
    )
    monkeypatch.setattr("haishui_cli.model_switch.get_model_info", lambda *a, **k: None)
    monkeypatch.setattr(
        "haishui_cli.model_switch.get_model_capabilities",
        lambda *a, **k: None,
    )
...
def test_custom_provider_no_key_singular_model_still_probes_live_models(monkeypatch):
    ...
    probed so /model matches the terminal ``haishui model`` flow. Ollama-native
    ...
    monkeypatch.setattr("haishui_cli.models.fetch_api_models", fake_fetch_api_models)
...
def test_list_enumerates_dict_format_models_alongside_default(monkeypatch):
    ...
    Regression: Haishui's own writer stores configured models as a dict
...
def test_picker_endpoint_authorization_overrides_inferred_bearer(monkeypatch):
    from haishui_cli.model_switch_providers import _fetch_picker_live_models
    ...
    monkeypatch.setattr("haishui_cli.models_local.should_use_ollama_native_catalog", lambda *a, **k: True)
    monkeypatch.setattr("haishui_cli.models_local.fetch_ollama_local_models", fake_native)
...
def test_lmstudio_picker_probes_active_config_base_url(monkeypatch):
    ...
    monkeypatch.setattr("haishui_cli.models_local.fetch_lmstudio_models", _fake_fetch)
...
def test_lmstudio_bare_providers_block_does_not_hide_live_catalog(monkeypatch):
    ...
    monkeypatch.setattr(
        "haishui_cli.models_local.fetch_lmstudio_models",
...
def test_lmstudio_providers_block_with_explicit_endpoint_still_uses_section3(monkeypatch):
    ...
    monkeypatch.setattr(
        "haishui_cli.model_switch_providers._discover_endpoint_models", _fake_discover
    )
...
def test_custom_providers_uses_live_models_for_multi_model_endpoint(monkeypatch):
    ...
    """
    monkeypatch.setattr("agent.models_dev.fetch_models_dev", lambda: {})
    monkeypatch.setattr("haishui_cli.providers.HAISHUI_OVERLAYS", {})
...
def test_resolve_custom_provider_passes_key_env():
    ...
    """
    from haishui_cli.providers import resolve_custom_provider
...
def test_discovered_models_auto_saved_to_cache(monkeypatch):
    ...
    monkeypatch.setattr("haishui_cli.models.fetch_api_models", fake_fetch_api_models)
    monkeypatch.setattr(
        "haishui_cli.model_switch_providers._save_discovered_models_to_config",
...
def test_save_discovered_models_preserves_dict_form(monkeypatch):
    ...
    from haishui_cli.model_switch_providers import _save_discovered_models_to_config
    ...
    monkeypatch.setattr("haishui_cli.config.save_config", fake_save)
    monkeypatch.setattr(
        "haishui_cli.config.load_config",
        lambda: {
            "custom_providers": [
                {
                    "name": "my-gateway",
...
def test_model_flow_named_custom_persists_discovered_models(monkeypatch):
    """The ``haishui model`` named-custom-provider flow persists the discovered
    ...
    ``haishui model`` collapsed to the single ``model:`` default everywhere but
    ...
    monkeypatch.setattr(
        "haishui_cli.curses_ui.curses_radiolist", lambda *a, **k: 0
    )
    # No-op downstream writes so the test never touches a real config.
    monkeypatch.setattr("haishui_cli.main_provider_setup._save_custom_provider", lambda *a, **k: None)
    monkeypatch.setattr("haishui_cli.auth._save_model_choice", lambda *a, **k: None)
    monkeypatch.setattr("haishui_cli.auth.deactivate_provider", lambda *a, **k: None)
    monkeypatch.setattr(
        "haishui_cli.config.load_config",
        lambda: {"model": {}, "providers": {}, "custom_providers": []},
    )
    monkeypatch.setattr("haishui_cli.config.save_config", lambda cfg: None)
    ...
    from haishui_cli.model_setup_flows_custom import _model_flow_named_custom
...
def test_custom_provider_context_length_models_dict_still_probes(monkeypatch):
    ...
    ``haishui model`` writes ``models: {default: {context_length: N}}`` for
...
def _seed_custom_model_cache(monkeypatch, models, *, age_seconds=10):
    ...
    import haishui_cli.models as models_mod
...
def test_keyless_endpoint_with_saved_catalog_still_reads_cache(monkeypatch):
    """A keyless local server must not be pinned by Haishui' own auto-save.
...
def test_keyless_endpoint_with_saved_catalog_is_still_not_probed(monkeypatch):
    ...
    The no-key + declared-catalog combination exists to keep Haishui from
...
def test_legacy_sentinel_catalog_still_resolves_and_migrates(tmp_path, monkeypatch):
    ...
    Pre-fix Haishui wrote ``__discovered_model_catalog__: true`` (and
...
def test_same_provider_switch_on_session_only_custom_endpoint_keeps_endpoint(monkeypatch):
    ...
    monkeypatch.setattr("haishui_cli.model_switch.load_config", lambda: {"model": {"provider": "openrouter", "default": "x"}}, raising=False)
    monkeypatch.setattr(
        "haishui_cli.models.probe_api_models",
