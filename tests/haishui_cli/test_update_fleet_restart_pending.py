...
A ``haishui update`` killed after git pull advanced HEAD but before the
...
- next ``haishui update`` with git already up to date still runs the
...
import haishui_cli.main_web_build as main_web_build
import haishui_cli.main_install_repair as main_install_repair
from haishui_cli import update_cmd
import haishui_cli.update_cmd_fleet as update_cmd_fleet
...
def _patch_update_deps(monkeypatch, tmp_path, run_side_effect):
    ...
    monkeypatch.setattr(haishui_main.subprocess, "run", run_side_effect)
    monkeypatch.setattr(haishui_main, "PROJECT_ROOT", tmp_path)
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(haishui_main, "_resolve_update_branch", lambda args: "main")
    monkeypatch.setattr(haishui_main, "_is_windows", lambda: False)
    ...
    monkeypatch.setattr(
        haishui_main, "_stash_local_changes_if_needed", lambda *a, **k: None
    )
    monkeypatch.setattr(haishui_main, "_clear_bytecode_cache", lambda *a, **k: 0)
    monkeypatch.setattr(
        haishui_main, "_record_bytecode_fingerprint", lambda *a, **k: None
    )
    monkeypatch.setattr(
        main_web_build, "_record_bytecode_fingerprint", lambda *a, **k: None
    )
    monkeypatch.setattr(haishui_main, "_run_pre_update_backup", lambda *a, **k: None)
    monkeypatch.setattr(
        haishui_main, "_pause_windows_gateways_for_update", lambda: None
    )
    monkeypatch.setattr(
        haishui_main, "_resume_windows_gateways_after_update", lambda *a, **k: None
    )
    monkeypatch.setattr(haishui_main, "_write_update_incomplete_marker", lambda: None)
    monkeypatch.setattr(haishui_main, "_clear_update_incomplete_marker", lambda: None)
    ...
    monkeypatch.setattr(haishui_main, "_build_web_ui", lambda *a, **k: None)
    ...
    import haishui_cli.gateway as haishui_gateway

    monkeypatch.setattr(
        haishui_gateway, "find_gateway_pids", lambda **_kwargs: []
    )
    monkeypatch.setattr(haishui_gateway, "supports_systemd_services", lambda: False)
    monkeypatch.setattr(
        haishui_gateway, "find_profile_gateway_processes", lambda *a, **k: []
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "haishui_cli.update_inventory.collect_runtime_inventory",
...
def test_marker_round_trip_under_haishui_home():
    path = update_cmd._fleet_restart_pending_marker_path()
    assert path.parent == get_haishui_home()
...
def test_pending_needed_when_unfinished_receipt_runtime_sha_skews(monkeypatch):
    ...
    receipt_dir = get_haishui_home() / "logs" / "update_receipts"
...
def test_run_pending_restart_true_when_no_gateways(monkeypatch, capsys):
    monkeypatch.setattr(
        "haishui_cli.gateway.find_gateway_pids", lambda **k: []
    )
    ...
    monkeypatch.setattr("haishui_cli.gateway_windows.is_installed", lambda: False)
...
def test_clean_update_escalates_surviving_serve_as_unaccounted(
    ...
    ``haishui-gateway.service``) and an unmanaged ``serve`` on the same
    ...
    import haishui_cli.update_inventory as ui
    ...
    import haishui_cli.process_identity as pi
    ...
    latest = get_haishui_home() / "logs" / "update_receipts" / "latest.json"
...
def test_clean_update_defers_desktop_owned_serve_and_clears_marker(
    ...
    haishui_main.cmd_update(args)  # no SystemExit(1)
...
def test_interrupt_between_pull_and_restart_leaves_marker(
    ...
    monkeypatch.setattr(haishui_main, "_clear_bytecode_cache", _interrupt)
...
def test_startup_warn_prints_when_marker_present(capsys):
    ...
    assert "haishui gateway restart" in err
...
#
# `systemctl --user restart haishui-gateway` never runs this module's clear path, and an update
...
def test_startup_warn_kept_without_positive_evidence(monkeypatch, capsys, disk_sha, fleet):
    ...
    _patch_marker_sha(monkeypatch, disk_sha)
    monkeypatch.setattr("haishui_cli.update_receipt.collect_fleet_versions", lambda **kwargs: fleet)
...
def test_startup_warn_silent_when_failed_receipt_already_restarted_fleet(monkeypatch, capsys):
    ...
    restart it performed; ``haishui update``'s catch-up still owes the fleet the checkout."""
        ...
        json.dumps(
            {
                ...
                "stop_reason": "AttributeError: module 'haishui_cli.main_dashboard' has no attribute 'x'",
                "pre_update": {"sha": pre}, "post_update": {"sha": pulled},
                "gateway_restart": {
                    "restarted_services": ["haishui-gateway"], "relaunched_profiles": [],
...
def test_startup_warn_silent_when_completed_update_fleet_restarted_onto_moved_checkout(monkeypatch, capsys):
    ...
    plus ``haishui gateway restart`` leaves every owed gateway on today's checkout — newer than the
