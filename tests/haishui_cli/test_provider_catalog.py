"""Tests for the unified provider catalog (haishui_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``haishui model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from haishui_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
