"""Tests for the haishui_cli.nous_billing HTTP client's response handling.
...
from haishui_cli import nous_billing as nb
...
def test_billing_token_cache_is_scoped_per_profile_home(monkeypatch, tmp_path):
    ...
    multiplex gateway, where the per-turn HAISHUI_HOME override selects the auth.json."""
    ...
    monkeypatch.setattr(
        ...
        lambda **kw: f"tok-{haishui_constants.get_haishui_home().name}")
    a, b = tmp_path / "a", tmp_path / "b"
    a.mkdir(); b.mkdir()
    seen = []
    for home in (a, b):
        tok = haishui_constants.set_haishui_home_override(str(home))
        try:
