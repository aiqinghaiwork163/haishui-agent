"""Tests for the direct Haishui-to-Relay shared-metrics runtime."""
...
from haishui_cli import lifecycle, plugins
from agent import relay_runtime
from haishui_cli.observability import relay_shared_metrics
from haishui_cli.plugins import PluginManager
...
def direct_runtime(tmp_path, monkeypatch):
    fake = _Relay()
    monkeypatch.setenv("HAISHUI_HOME", str(tmp_path / "haishui-home"))
    monkeypatch.setattr(relay_runtime, "_load_nemo_relay", lambda: fake)
    monkeypatch.setattr(
        "haishui_cli.config.read_raw_config_readonly",
...
def test_direct_runtime_records_without_enabling_a_plugin(direct_runtime, tmp_path):
    ...
    assert scope_starts[1][1] == "haishui.task_run"
    ...
    assert tool_starts[0][1] == "haishui.tool_call"
    ...
    active_marks = [
        ...
        if event[0] == "scope.event" and event[1] == "haishui.client.active"
    ]
    ...
    root = tmp_path / "haishui-home" / "telemetry" / "shared_metrics"
    ...
    assert set(metrics) == {
        "haishui.client.active",
        "haishui.model_route.count",
        "haishui.task_run.finished",
        "haishui.task_run.started",
        "haishui.tool_approval.count",
        "haishui.tool_call.count",
    }
    assert metrics["haishui.client.active"] == {
        "name": "haishui.client.active",
        "type": "counter",
        "dimensions": {},
        "value": 1,
    }
    assert metrics["haishui.model_route.count"]["dimensions"] == {
        "model": "claude-sonnet",
        "provider": "anthropic",
    }
    assert metrics["haishui.model_route.count"]["value"] == 1
    assert metrics["haishui.tool_call.count"] == {
        "name": "haishui.tool_call.count",
        "type": "counter",
        "dimensions": {
            "approval_outcome": "approved",
            "latency_bucket": "250ms_to_500ms",
    ...
    assert metrics["haishui.tool_approval.count"] == {
        "name": "haishui.tool_approval.count",
        "type": "counter",
        "dimensions": {
    ...
    assert metrics["haishui.task_run.started"] == {
        "name": "haishui.task_run.started",
        "type": "counter",
        "dimensions": {
            "entrypoint": "interactive",
            "execution_surface": "cli",
    ...
    terminal = metrics["haishui.task_run.finished"]["dimensions"]
...
def test_real_binding_drives_lifecycle_aggregation_export_and_snapshot(
    ...
    monkeypatch.setattr(
        "haishui_cli.observability.shared_metrics._utc_now",
        lambda: tomorrow,
    )
    ...
    assert by_metric["haishui.client.active"][0]["dimensions"] == {}
    assert by_metric["haishui.client.active"][0]["value"] == 1
    assert len(by_metric["haishui.task_run.started"]) == 1
    assert by_metric["haishui.task_run.started"][0]["value"] == 3
    assert len(by_metric["haishui.model_route.count"]) == 1
    model_counter = by_metric["haishui.model_route.count"][0]
    ...
    assert {
        ...
        for counter in by_metric["haishui.tool_call.count"]
    ...
    assert len(by_metric["haishui.tool_approval.count"]) == 1
    approval_counter = by_metric["haishui.tool_approval.count"][0]
    ...
    terminal_by_outcome = {
        ...
        for counter in by_metric["haishui.task_run.finished"]
    }
    ...
    for package in package_payloads:
        assert package["schema_version"] == "haishui.shared_metrics.v2"
...
def test_real_binding_correlates_plugin_approval_denial_to_tool_metric(
    ...
    tool_metrics = [
        counter
        for counter in snapshot
        if counter["metric_name"] == "haishui.tool_call.count"
    ]
    ...
    approval_metrics = [
        counter
        for counter in snapshot
        if counter["metric_name"] == "haishui.tool_approval.count"
    ]
...
def test_core_runtime_is_fail_open_without_a_published_binding(monkeypatch, caplog):
    ...
    assert "Haishui Relay runtime initialization failed" in caplog.text
    relay_runtime._reset_for_tests()
...
def test_managed_config_cannot_override_shared_metrics_consent(
    ...
    from haishui_cli import config, managed_scope
    ...
    monkeypatch.setenv("HAISHUI_MANAGED_DIR", str(managed))
...
def test_disabling_shared_metrics_stops_collection_and_shutdown_export(
    ...
    runtime.subscriber(
        SimpleNamespace(
            ...
            category="function",
            category_profile=None,
            name="haishui.task_run",
            scope_category="start",
            metadata={
                "haishui.metrics.schema_version": "haishui.metrics.event.v1",
...
def test_same_request_id_is_isolated_between_tasks(direct_runtime):
    ...
    task_ends = [
        event[2]["output"]
        for event in direct_runtime.events
        if event[0] == "scope.pop" and event[1][1] == "haishui.task_run"
    ]
...
def test_reused_task_id_starts_a_new_run_for_each_turn(direct_runtime):
    ...
    task_starts = [
        event
        for event in direct_runtime.events
        if event[0] == "scope.push" and event[1] == "haishui.task_run"
    ]
    task_ends = [
        event
        for event in direct_runtime.events
...
def test_approval_without_tool_context_is_counted_as_unattributed(direct_runtime):
    ...
    [approval] = [
        event
        for event in direct_runtime.events
        if event[0] == "scope.event" and event[1] == "haishui.tool_approval"
    ]
...
def test_failed_flush_keeps_daily_export_open_for_later_task(
    ...
    metrics = {metric["name"]: metric for metric in package["metrics"]}
    assert metrics["haishui.task_run.started"]["value"] == 2
    assert metrics["haishui.task_run.finished"]["value"] == 2
    assert flush_attempts == 2
    assert "Haishui shared-metrics task flush failed" in caplog.text
...
def test_skill_lifecycle_flows_through_relay_to_a_privacy_safe_package(
    ...
    outbox = tmp_path / "haishui-home" / "telemetry" / "shared_metrics" / "outbox"
    [package_path] = list(outbox.glob("*.json"))
    package = json.loads(package_path.read_text(encoding="utf-8"))
    skill_metrics = [
        metric
        for metric in package["metrics"]
        if metric["name"].startswith("haishui.skill.")
    ]
    assert {metric["name"] for metric in skill_metrics} == {
        "haishui.skill.lifecycle.count",
        "haishui.skill.load.count",
    }
...
def test_skill_lifecycle_with_only_task_id_uses_unique_task_scope(direct_runtime):
    ...
    [mark] = [
        event
        for event in direct_runtime.events
        if event[0] == "scope.event" and event[1] == "haishui.skill.lifecycle"
    ]
...
def test_late_skill_lifecycle_is_not_reemitted_at_the_root(direct_runtime):
    ...
    assert [
        event
        for event in direct_runtime.events
        if event[0] == "scope.event" and event[1] == "haishui.skill.load"
    ] == []
