"""A pooled Desktop backend (``haishui --profile X serve``, HAISHUI_HOME=<root>/profiles/X) answers its
REST without ``?profile=``. Those UNSCOPED reads/verbs are about X — a profile the live default
multiplexer serves — and must agree with the scoped ``?profile=X`` answer and with ``haishui -p X status``:
...
restart`` under X's HAISHUI_HOME that exited 78 while the UI reported success.
"""
...
def pooled_served_process(tmp_path, monkeypatch):
    """Process whose HAISHUI_HOME is a served named profile; the default home records a live multiplexer."""
    ...
    monkeypatch.setenv("HAISHUI_HOME", str(root / "profiles" / "alpha"))
...
def test_unscoped_lifecycle_verbs_in_a_served_profile_process_address_the_multiplexer(pooled_served_process):
    ...
    # The child must run under the DEFAULT home (the multiplexer's), not inherit alpha's HAISHUI_HOME.
    assert _profile_action_environment(restart)["HAISHUI_HOME"] == str(pooled_served_process)
