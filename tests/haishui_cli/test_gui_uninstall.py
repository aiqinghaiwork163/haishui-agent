"""Tests for haishui_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import haishui_cli.gui_uninstall as gu


def _make_agent(haishui_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = haishui_home / "haishui-agent"
    (agent_root / "haishui_cli").mkdir(parents=True)
    (agent_root / "haishui_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(haishui_home: Path) -> None:
    """Create the source-built GUI artifacts a `haishui desktop` run produces."""
    desktop = haishui_home / "haishui-agent" / "apps" / "desktop"
    ...
    (haishui_home / "haishui-agent" / "node_modules").mkdir(parents=True)
    (haishui_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(haishui_home: Path) -> None:
    (haishui_home / "config.yaml").write_text("x: 1\n")
    (haishui_home / ".env").write_text("KEY=secret\n")
    (haishui_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(haishui_home)
    _make_gui_build(haishui_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(haishui_home)
    ...
    assert summary["haishui_home"] == str(haishui_home)
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `haishui desktop` installs is removable."""
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    haishui_home = tmp_path / ".haishui"
    _make_agent(haishui_home)
    icon = lde.icon_path(haishui_home / "haishui-agent")
    icon.parent.mkdir(parents=True, exist_ok=True)
    icon.write_bytes(b"\x89PNG")
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    removed = gu.uninstall_gui(haishui_home)
    ...
    assert lde.icon_path(haishui_home / "haishui-agent").exists()
    # The agent itself survives a GUI uninstall.
    assert (haishui_home / "haishui-agent" / "haishui_cli").is_dir()
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(tmp_path / ".haishui")
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import haishui_cli.uninstall as uninstall
