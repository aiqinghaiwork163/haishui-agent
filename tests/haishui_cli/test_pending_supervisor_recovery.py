...
from haishui_cli import gateway, main, update_cmd_fleet as fleet, update_receipt
...
def test_pending_marker_requires_complete_systemd_recovery(monkeypatch, tmp_path, failure):
    ...
    def systemctl(cmd, **kw):
        if "list-units" in cmd:
            ...
            return SimpleNamespace(returncode=int(failure == "listing"), stdout=(
                "haishui-gateway-one.service loaded active running\n"
                + ("" if failure == "missing-owned" else "haishui-gateway-two.service loaded failed failed\n")), stderr="")
        bad = cmd[-1] == "haishui-gateway-two"
        if "restart" in cmd:
    ...
    monkeypatch.setattr(update_receipt, "collect_fleet_versions", lambda **kw: [
        {"profile": name.removeprefix("haishui-gateway-"), "state": "current", "code_sha": "pending"}
    ...
    if failure not in (None, "running"):
        ...
        assert set(recovered) == {"haishui-gateway-one", "haishui-gateway-two"}
...
def test_pending_launchd_requires_complete_supervision(monkeypatch, tmp_path, failure):
    ...
    current, sibling = "ai.haishui.gateway", "ai.haishui.gateway-two"
