...
A named profile (``HAISHUI_HOME`` under ``profiles/<name>``) resolves provider state and the
credential pool from ITS OWN ``auth.json`` only. The root ``~/.haishui/auth.json`` is never a
...
def profile_env(tmp_path, monkeypatch):
    """Global root + an active named profile under Path.home()/.haishui/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.haishui            (has its own auth.json fixture)
    * Profile     -> tmp_path/.haishui/profiles/coder   (active, HAISHUI_HOME points here)
    """
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    global_root = tmp_path / ".haishui"
    global_root.mkdir()
...
def test_named_profile_never_reads_the_root_store(profile_env):
    ...
    from haishui_cli.auth import get_provider_auth_state, read_credential_pool, resolve_codex_runtime_credentials
    from haishui_cli.auth_xai import _read_xai_oauth_tokens
    ...
    token = set_haishui_home_override(own)
    try:
...
def test_profile_refresh_never_writes_through_to_the_root_store(profile_env):
    ...
    from haishui_cli.auth_codex import _save_codex_tokens, clear_codex_pool_quota_cooldowns
    from haishui_cli.auth_xai import _save_xai_oauth_tokens
...
def test_auth_lock_reentrancy_is_scoped_after_profile_context_switch(profile_env):
    ...
    with auth._auth_store_lock():
        ...
        token = set_haishui_home_override(profile_b)
        try:
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (HAISHUI_HOME != ~/.haishui, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    haishui_home = tmp_path / "classic"
    haishui_home.mkdir()
