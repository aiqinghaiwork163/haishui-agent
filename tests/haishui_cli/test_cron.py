"""Tests for haishui_cli.cron command handling."""
...
from haishui_cli import cron as cron_cli
...
class TestUnverifiedDeliveryVisibility:
    ...
    state must be visible in ``haishui cron list`` and ``haishui cron doctor``,
...
class TestCronDoctor:
    def test_doctor_reports_cron_health_issues(self, tmp_cron_dir, capsys):
        ...
        assert "haishui cron edit" in out
...
class TestCronListStatusRendering:
    ...
    def test_delivery_failed_is_not_green_ok(self, tmp_cron_dir, capsys, monkeypatch):
        monkeypatch.setattr("haishui_cli.gateway.find_gateway_pids", lambda: [1])
        # capsys is not a tty, so force colors on to check the paint itself.
        monkeypatch.setattr("haishui_cli.colors.should_use_color", lambda: True)
        create_job(prompt="Daily digest", schedule="every 1h")
        jobs = load_jobs()
...
class TestExternalCronProviderStatus:
    ...
    def test_status_reports_provider_not_ticker_for_chronos(
        ...
        monkeypatch.setattr(
            "haishui_cli.cron._active_cron_provider_name", lambda: "chronos"
        )
...
class TestCronRunBackgroundDispatch:
    """`haishui cron run` must not report 'failed' when the run was dispatched
...
class TestStatusSurfacesDeadScheduler:
    ...
    def _dead_gateway(self, monkeypatch):
        monkeypatch.setattr("haishui_cli.gateway.find_gateway_pids", lambda: [])
        monkeypatch.setattr(
            "haishui_cli.gateway.named_profile_served_by_running_multiplexer", lambda: None
        )
