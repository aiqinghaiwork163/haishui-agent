...
subparser, placing the flag BEFORE the subcommand (e.g. 'haishui --yolo chat')
...
def _build_parser():
    """Build the haishui argument parser from the real code.
...
class TestChatVerboseArg:
    ...
    def test_cmd_chat_forwards_none_when_verbose_is_absent(self, monkeypatch):
        ...
        fake_banner = types.ModuleType("haishui_cli.banner")
        ...
        monkeypatch.setitem(sys.modules, "haishui_cli.banner", fake_banner)
...
class TestYoloEnvVar:
    """Verify --yolo sets HAISHUI_YOLO_MODE regardless of flag position.
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `haishui gateway run --accept-hooks` failed
    ...
    # One driver subprocess parses ALL argvs: haishui_cli.main is a very heavy
    # import (previously 11 separate `python -m haishui_cli.main` spawns with a
...
"""
    ...
    def test_accepted_at_every_position(self):
        """Every `haishui <argv>` must exit 0 (help) rather than failing
        with `unrecognized arguments`."""
        import json
        import subprocess
...
class TestChatSubparserInheritedValueFlags:
    ...
    Regression test for #28780: `haishui -t web chat` silently dropped the
    ...
    Uses the real `haishui_cli._parser.build_top_level_parser()` rather than
    ...
    def test_chat_subparser_inherited_value_flags_use_suppress(self):
        ...
        assert not offenders, (
            ...
            "top-level value when used as `haishui <flag> <value> chat`:\n  "
