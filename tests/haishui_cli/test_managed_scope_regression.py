...
def haishui_home(tmp_path, monkeypatch):
    ...
    # /etc/haishui on the dev/CI box can't influence the test.
    monkeypatch.setenv("HAISHUI_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(haishui_home, monkeypatch):
...
def test_env_expansion_in_user_config(haishui_home, monkeypatch):
    from haishui_cli.config import load_config, cfg_get
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_haishui_dotenv(haishui_home=str(home))
