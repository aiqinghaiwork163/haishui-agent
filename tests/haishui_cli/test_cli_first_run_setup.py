...
def test_credentials_ready_false_when_no_provider(monkeypatch):
    ...
    monkeypatch.setattr("haishui_cli.runtime_provider.resolve_runtime_provider", _raise)
...
def test_offer_first_run_setup_routes_into_shared_picker(monkeypatch):
    ...
    monkeypatch.setattr("haishui_cli.main.select_provider_and_model", _fake_picker)
    monkeypatch.setattr("builtins.input", lambda *a, **k: "y")
    # After the picker "runs", config has a provider and creds resolve.
    monkeypatch.setattr(
        "haishui_cli.config.load_config",
        lambda: {"model": {"provider": "nous", "default": "haishui-4-405b"}},
    )
    monkeypatch.setattr(
        "haishui_cli.runtime_provider.resolve_runtime_provider",
        lambda **kw: {
    ...
    assert shell.model == "haishui-4-405b"
...
def test_offer_first_run_setup_re_resolves_reasoning_for_picked_model(monkeypatch):
    ...
    monkeypatch.setitem(cli.CLI_CONFIG, "agent", {
        **cli.CLI_CONFIG.get("agent", {}), "reasoning_effort": "medium",
        "reasoning_overrides": {"haishui-4-405b": "high"}})
    shell = _make_shell(cli, monkeypatch)
    assert shell.reasoning_config["effort"] == "medium"
    monkeypatch.setattr("haishui_cli.main.select_provider_and_model", lambda: None)
    monkeypatch.setattr("builtins.input", lambda *a, **k: "y")
...
def test_offer_first_run_setup_declined(monkeypatch):
    ...
    monkeypatch.setattr("haishui_cli.main.select_provider_and_model", _fail_picker)
...
def test_offer_first_run_setup_picker_cancel_is_graceful(monkeypatch):
    ...
    monkeypatch.setattr("haishui_cli.main.select_provider_and_model", _cancel_picker)
...
def test_empty_key_error_names_actual_provider(monkeypatch, capsys):
    ...
    assert "haishui model" in out or "haishui setup" in out
...
def _forbid_wizard(monkeypatch, shell):
    monkeypatch.setattr("haishui_cli.main.select_provider_and_model",
...
def test_benched_credential_prints_cooldown_instead_of_wizard(monkeypatch, capsys):
    ...
    def _raise(**kwargs):
        raise AuthError("Haishui is not logged into Nous Portal.", provider="nous",
    ...
    assert "re-authenticate" not in out and "haishui model" not in out
...
def test_auth_json_only_login_explains_instead_of_wizard(monkeypatch, capsys, tmp_path):
    ...
    monkeypatch.setattr("haishui_cli.runtime_provider.resolve_runtime_provider", lambda **kw: (_ for _ in ()).throw(
        AuthError("Haishui is not connected to any AI provider yet.", code="no_provider_configured")))
    shell._maybe_offer_first_run_setup()
    assert offered == [True]
