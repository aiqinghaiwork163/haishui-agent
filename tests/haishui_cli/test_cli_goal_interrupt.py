...
minimal ``HaishuiCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def haishui_home(tmp_path, monkeypatch):
    """Isolated HAISHUI_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".haishui"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("HAISHUI_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves HAISHUI_HOME each test.
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal HaishuiCLI stub with an active goal wired in."""
    from cli import HaishuiCLI
    from haishui_cli.goals import GoalManager

    cli = HaishuiCLI.__new__(HaishuiCLI)
...
class TestInterruptAutoPause:
    ...
    def test_interrupted_turn_is_resumable(self, haishui_home):
        ...
        with patch("haishui_cli.goals.judge_goal"):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, haishui_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, haishui_home):
        ...
        src = inspect.getsource(HaishuiCLI.chat)
