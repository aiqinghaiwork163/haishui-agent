"""Tests for haishui_cli.profile_distribution — git-based profile installs.
...
from haishui_cli.profile_distribution import (
    DEFAULT_DIST_OWNED,
    DistributionError,
...
# ---------------------------------------------------------------------------
# Isolated profile env (matches tests/haishui_cli/test_profiles.py)
# ---------------------------------------------------------------------------
...
class TestVersionRequires:
    ...
    def test_check_matrix(self, spec, cur, ok):
        if ok:
            check_haishui_requires(spec, cur)
        else:
            with pytest.raises(DistributionError, match="requires Haishui"):
                check_haishui_requires(spec, cur)
...
class TestInstall:
    ...
    def test_update_respects_distribution_owned_allowlist(self, profile_env):
        ...
        from haishui_cli.profile_distribution import read_manifest as _read
    ...
    def test_install_enforces_haishui_requires(self, profile_env, monkeypatch):
        # Pin current Haishui version to something well below the requirement
        import haishui_cli
        monkeypatch.setattr(haishui_cli, "__version__", "0.1.0", raising=False)

        mf = DistributionManifest(
            name="future",
            version="1.0.0",
            haishui_requires=">=99.0.0",
        )
...
class TestDescribe:
    ...
    def test_describe_missing_profile_raises(self, profile_env):
        with pytest.raises(DistributionError, match="No profile named .*haishui profile list"):
...
class TestInstalledAtStamp:
    ...
    def test_update_refreshes_installed_at(self, profile_env, monkeypatch):
        ...
        monkeypatch.setattr(
            "haishui_cli.profile_distribution.datetime", _FakeDT, raising=True
        )

        from haishui_cli.profile_distribution import update_distribution
...
class TestProfileInfoDistribution:
    ...
    def test_malformed_manifest_does_not_break_list(self, profile_env):
        from haishui_cli.profiles import create_profile, list_profiles, get_profile_dir
