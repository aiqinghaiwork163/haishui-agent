...
Covers:
- Defense 1: gateway stop/restart refuse when _HAISHUI_GATEWAY=1
...
from haishui_cli.cron import cron_command
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `python*` carries no haishui/gateway token yet takes it down — on every platform.
        ...
        "pkill -9 python3",
        "pkill -f python",
        'pkill -f "python -m haishui_cli.main"',
        "pkill -f haishui_cli",
        "killall -9 python3.12",
        "sudo pkill -9 python3",
        "pgrep python | xargs kill -9",
        "kill $(pidof python3)",
        # Windows spellings of the haishui-gateway forms (salvaged from #94379).
        "haishui.exe gateway restart",
        "haishui.cmd gateway stop",
        r'"C:\Program Files\haishui.exe" gateway restart',
        "taskkill /F /IM haishui-gateway.exe",
        "Stop-Process -Name haishui-gateway -Force",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        # Unrelated scripts that merely contain "haishui" cannot match the gateway cmdline.
        "pkill -f 'haishui-polis/run.sh'",
        "pkill -f my_haishui_bot.py",
        "pgrep python",
        "haishui.exe gateway start",
        "my-haishui.exe gateway restart",
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.haishui.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.haishui/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l haishui-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.haishui.gateway.restart-once.plist",
        "launchctl bootout gui/501/ai.haishui.gateway",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.haishui.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.haishui/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_label_defined_before_verb_is_caught(self):
        ...
        cmd = (
            "uid=$(id -u); for item in 'ai.haishui.gateway-apollo:/a.plist' "
            "'ai.haishui.gateway-cronus:/c.plist' 'ai.haishui.gateway-plutus:/p.plist' "
            "'ai.haishui.gateway:/Users/botuser/Library/LaunchAgents/ai.haishui.gateway.plist'; "
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated haishui note on the next line"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `launchctl kick"start" ... ai.haishui.gateway` executes exactly as
        ...
        'launchctl kick"start" -k gui/501/ai.haishui.gateway',
        "launchctl kick'start' -k gui/501/ai.haishui.gateway",
        "launchctl kick\\start -k gui/501/ai.haishui.gateway",
        'launchctl "kickstart" -k gui/501/ai.haishui.gateway',
        # Splices on the newer/legacy unload spellings this PR added.
        'launchctl boot"out" gui/501/ai.haishui.gateway',
        "launchctl dis\\able gui/501/ai.haishui.gateway",
        # The gateway identifier itself can be spliced just as easily.
        'launchctl bootout gui/501/ai.haishui."gateway"',
        # Same class on the systemctl and haishui-CLI branches.
        'systemctl re"start" haishui-gateway',
        'haishui gateway re"start"',
    ])
    ...
    def test_spliced_verb_inside_shell_c_payload_is_blocked(self):
        ...
        command = 'sh -c \'launchctl kick"start" -k gui/501/ai.haishui.gateway\''
    ...
    @pytest.mark.parametrize("text", [
        ...
        'launchctl kick"start" -k gui/501/ai.haishui.update-checker',
        'systemctl re"start" haishui-meta.service',
    ...
    @pytest.mark.parametrize("text", [
        ...
        "haishui cron list",
        "haishui update",
        "haishui config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `haishui gateway start` is benign — starting a gateway from inside a
        ...
        "haishui gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway haishui
        # services must not be falsely blocked (the old `.*haishui` matched any
        # haishui token).
        "launchctl unload ai.haishui.update-checker.plist",
        "launchctl restart ai.haishui.daemon",
        ...
        "systemctl restart haishui-meta.service",
        "systemctl restart haishui-cron-helper",
        ...
        "echo after the haishui gateway restarted cleanly",
        "the haishui gateway stopped responding, please investigate",
        # #92372 Branch D: `p?kill` without a leading \b matched the "kill"
        # tail of "skill".
        "haishui skill view gateway-notes && echo haishui gateway docs",
        # #77173/#77536: a file path with embedded spaces containing the
        # lifecycle words must not match — `haishui` is a path component
        # there, not a command.
        "cat '/docs/haishui gateway restart-notes.md'",
        "less /home/user/notes/haishui gateway restart runbook.txt",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "haishui gateway restart; echo done",
        "haishui gateway stop && echo stopped",
        # #77173 command-position anchor must not weaken separator/subshell
        # forms either.
        "true;haishui gateway restart",
        "true && haishui gateway stop",
        "echo $(haishui gateway restart)",
        "echo `haishui gateway restart`",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'import subprocess\nsubprocess.run(["launchctl", "bootout", "gui/501/ai.haishui.gateway"])',
        'subprocess.run(["haishui", "gateway", "restart"])',
        'os.system("launchctl kickstart -k gui/501/ai.haishui.gateway")',
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'data = ["haishui", "notes"]  # unrelated list',
    ])
    ...
    def test_inert_heredoc_body_prose_not_blocked(self):
        ...
        text = (
            ...
            "If the box is wedged, a human can run: haishui gateway restart\n"
            "EOF"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
...
class TestProfileFlagGatewayLifecycle:
    """#78028: `haishui -p <profile> gateway restart|stop` bypasses Branch A's
    ...
    def _pin_profile_identity(self, monkeypatch):
        # The ambient test env may carry HAISHUI_HOME/HAISHUI_PROFILE; pin the
        # profile identity explicitly so every assertion is deterministic.
        monkeypatch.setenv("HAISHUI_PROFILE", "zeus")
        monkeypatch.delenv("HAISHUI_PROFILE_NAME", raising=False)

    @pytest.mark.parametrize("text", [
        "haishui -p zeus gateway stop",
        "haishui -p zeus gateway restart",
        "haishui --profile zeus gateway restart",
        "haishui --profile zeus gateway stop",
        "haishui --profile=zeus gateway restart",
        # Global flags before/after the selector must not hide the shape.
        "haishui -v -p zeus gateway restart",
        "haishui -p zeus -v gateway restart",
        "haishui --debug --profile zeus gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        "haishui -p venus gateway stop",
        "haishui -p venus gateway restart",
        "haishui --profile venus gateway restart",
        "haishui --profile=venus gateway stop",
        "haishui -p venus -v gateway restart",
    ])
    def test_sibling_allowed(self, text):
        assert not _contains_gateway_lifecycle_command(text), f"Should allow: {text!r}"

    @pytest.mark.parametrize("text", [
        "haishui -p zeus gateway start",
        "haishui -p zeus gateway start --all",
    ])
    ...
    def test_adjacent_form_still_blocked(self):
        ...
        assert _contains_gateway_lifecycle_command("haishui gateway restart")
        assert _contains_gateway_lifecycle_command("haishui gateway stop")

    def test_haishui_home_derived_profile(self, monkeypatch):
        # Without HAISHUI_PROFILE the guard falls back to the HAISHUI_HOME-
        # derived profile identity (get_active_profile_name) — the signal the
        # gateway process itself carries.
        monkeypatch.delenv("HAISHUI_PROFILE", raising=False)
        monkeypatch.delenv("HAISHUI_PROFILE_NAME", raising=False)
        import haishui_cli.profiles as profiles_mod

        monkeypatch.setattr(profiles_mod, "get_active_profile_name", lambda: "zeus")
        assert _contains_gateway_lifecycle_command("haishui -p zeus gateway restart")
        assert not _contains_gateway_lifecycle_command("haishui -p venus gateway restart")
    ...
    def test_no_profile_context_conservative_allow(self, monkeypatch):
        ...
        assert not _contains_gateway_lifecycle_command("haishui -p zeus gateway restart")
        assert _contains_gateway_lifecycle_command("haishui gateway restart")
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_haishui_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade haishui then run haishui gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_haishui_gateway_once.sh). The script must live under
        # HAISHUI_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("HAISHUI_HOME", str(tmp_path / ".haishui"))
        scripts_dir = tmp_path / ".haishui" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_HAISHUI_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _HAISHUI_GATEWAY=1.

    Issue #37453: systemctl --user restart haishui-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart haishui-gateway",
        "systemctl --user restart haishui-gateway",
        "systemctl stop haishui-gateway.service",
        "haishui gateway restart",
        "haishui gateway uninstall",
        "launchctl kickstart gui/501/ai.haishui.gateway",
        "launchctl bootout gui/501/ai.haishui.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.haishui.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.haishui/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.haishui.gateway.restart-once.plist",
        "pkill -f haishui.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart haishui-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.haishui.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-haishui label: label-independent detection is the point
        # (#62891 second reproduction used `ai.haishui.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.haishui.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_cli_agent_session_not_blocked_by_inherited_env(
        ...
        """#92560: CLI/TUI agent sessions inherit _HAISHUI_GATEWAY=1 from the
        ...
        # Simulate a CLI agent session: _HAISHUI_GATEWAY=1 is in the
        ...
        self._patch_env(monkeypatch, _FakeEnv(), inside_gateway=False)
        monkeypatch.setenv("_HAISHUI_GATEWAY", "1")
        monkeypatch.setattr(
            tt, "_check_all_guards", lambda cmd, env, **kwargs: {"approved": True}
        )

        result = json.loads(tt.terminal_tool(command="haishui gateway restart"))

        assert result["exit_code"] == 0
        assert calls == ["haishui gateway restart"]
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.haishui.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.haishui.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-haishui systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run haishui gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("haishui gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfehaishui gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under HAISHUI_HOME/scripts (the
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("haishui gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/haishui\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 haishui gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_haishui_home()` falls back to Path.home(), which raises
        RuntimeError when neither HAISHUI_HOME nor HOME resolves
...
class TestRelativePathDoesNotDisableDataExemption:
    ...
    @pytest.mark.parametrize("command", [
        "grep -r 'systemctl restart haishui-gateway' .",
        "grep -rn 'haishui gateway restart' ./logs",
        "rg 'haishui gateway restart' ../archive",
        "grep -c 'systemctl stop haishui-gateway' ./var/log/syslog",
        "sqlite3 ./stats.db \"SELECT restart_reason FROM haishui_gateway_restarts\"",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 ./db ".shell haishui gateway restart"',
        'sqlite3 ./db ".system systemctl restart haishui-gateway"',
        'psql ./x -c "\\! systemctl restart haishui-gateway"',
        "grep -r 'haishui gateway restart' . | sh",
        "grep -r 'haishui gateway restart' ./logs | bash",
        "grep -r 'haishui gateway restart' . | sudo sh",
        "grep -r 'x' . ; haishui gateway restart",
        "grep -r 'x' . && systemctl restart haishui-gateway",
        'grep -r "$(haishui gateway restart)" .',
        "rg 'x' ./logs | xargs systemctl restart haishui-gateway",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell haishui gateway restart"',
        'sqlite3 db ".system systemctl restart haishui-gateway"',
        'psql -c "\\! systemctl restart haishui-gateway"',
    ])
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    haishui_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run haishui gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HAISHUI_HOME", str(tmp_path / ".haishui"))
        (tmp_path / ".haishui").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run haishui gateway restart nightly",
        ))
...
class TestTerminalToolGatewayLifecycleGuardRemote:
    ...
    def test_unscannable_executed_script_names_the_reason(self, monkeypatch, tmp_path):
        ...
        import tools.terminal_tool as tt
        from haishui_cli.sqlite_safe_read import connect_tracked
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop haishui-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart haishui-gateway' /var/log/syslog",
        "rg 'haishui gateway restart' /home/user/.haishui/logs/",
        "journalctl -u haishui-gateway --grep 'systemctl restart haishui-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'haishui_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell haishui gateway restart"',
        'psql -c "\\! systemctl restart haishui-gateway"',
        "grep 'systemctl restart haishui-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart haishui-gateway",
        'grep "$(systemctl restart haishui-gateway)" f',
        "grep 'restart' log; systemctl restart haishui-gateway",
        'sqlite3 db "SELECT 1"; haishui gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "haishui gateway restart",
        "sudo systemctl stop haishui-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart haishui-gateway%'\""
        )
