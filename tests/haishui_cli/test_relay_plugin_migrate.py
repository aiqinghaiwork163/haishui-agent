"""``haishui update`` / ``haishui migrate relay``: legacy ``HAISHUI_NEMO_RELAY_ATIF_*``/``ATOF_*`` vars
...
from haishui_cli.relay_plugin_migrate import (
...
HAISHUI_NEMO_RELAY_ATOF_ENABLED=1
HAISHUI_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY={home}/telemetry/atof
HAISHUI_NEMO_RELAY_ATOF_FILENAME=haishui-atof.jsonl
HAISHUI_NEMO_RELAY_ATOF_MODE=append
HAISHUI_NEMO_RELAY_ATIF_ENABLED=1
HAISHUI_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY={home}/telemetry/atif
HAISHUI_NEMO_RELAY_ATIF_FILENAME_TEMPLATE=trajectory-{{session_id}}.json
HAISHUI_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
"""
...
def test_legacy_env_becomes_validated_toml_selected_from_env(profile_env):
    ...
    assert sink["type"] == "file" and sink["filename"] == "haishui-atof.jsonl"
    assert document["components"][0]["config"]["atif"]["filename_template"] == "trajectory-{session_id}.json"
    # Relay itself accepts the file Haishui will load at runtime.
    ...
    assert "# migrated to relay-plugins.toml: HAISHUI_NEMO_RELAY_ATOF_ENABLED=1" in (profile_env / ".env").read_text(encoding="utf-8")
