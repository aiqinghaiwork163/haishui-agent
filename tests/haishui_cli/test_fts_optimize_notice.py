...
def test_update_notice_offers_v1_trigram_tool_calls_rebuild(tmp_path, monkeypatch, capsys):
    ...
    class FakeSessionDB:
        ...
        _db_needs_fts_storage_upgrade = staticmethod(
            haishui_state.SessionDB._db_needs_fts_storage_upgrade
        )

    monkeypatch.setattr(haishui_constants, "get_haishui_home", lambda: tmp_path)
    monkeypatch.setattr(haishui_state, "SessionDB", FakeSessionDB)
    ...
    assert "haishui sessions optimize-storage" in capsys.readouterr().out
    conn.close()
