...
``get_haishui_home()`` printed ``[HAISHUI_HOME fallback] ... wrong profile`` on every ``haishui``
command for users of ``haishui profile use`` (#112319, also reported in #112839).
"""
...
def test_help_text_names_the_profile_config_once_the_process_is_re_homed(sticky_profile_home, monkeypatch):
    from haishui_cli._parser import _cfg_path

    monkeypatch.setenv("HAISHUI_HOME", str(sticky_profile_home / "profiles" / "coder"))

    assert _cfg_path() == "~/.haishui/profiles/coder/config.yaml"
