...
def _run(monkeypatch, capsys, argv_tail, db):
    ...
    monkeypatch.setattr(haishui_state, "SessionDB", lambda *args, **kwargs: db)
    monkeypatch.setattr(sys, "argv", ["haishui", "sessions", *argv_tail])
    try:
        main_mod.main()
        code = 0
...
def test_pin_multiple_ids_one_missing(monkeypatch, capsys):
    ...
    assert "No session 'nope'" in out and "haishui sessions list" in out
    assert code == 1
...
def test_pinned_empty_hint(monkeypatch, capsys):
    ...
    assert "haishui sessions pin" in out
