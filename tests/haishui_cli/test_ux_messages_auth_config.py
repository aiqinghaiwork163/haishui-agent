...
class TestConfigParseWarning:
    ...
    def test_warning_names_repair_commands_and_line(self, tmp_path, monkeypatch, capsys, fallback):
        ...
        assert "line " in err
        assert "`haishui config edit`" in err and "`haishui config check`" in err
...
class TestConfigGuardCopy:
    def test_require_parseable_user_config_says_what_happened_and_how_to_fix(self, tmp_path, monkeypatch):
        ...
        assert msg.startswith("Haishui stopped because your settings file")
        assert "`haishui config edit`" in msg and "--ignore-user-config" in msg
    ...
    def test_refuse_write_names_repair_commands_and_absolute_backup_dir(self, tmp_path, monkeypatch):
        ...
        assert "`haishui config edit`" in msg and "`haishui config check`" in msg
        assert f"{display_haishui_home()}/backups/config/" in msg
...
def test_no_provider_configured_points_at_model_login_and_auth_add(monkeypatch, tmp_path):
    from haishui_cli.auth import AuthError, resolve_provider
    from haishui_constants import display_haishui_home
    ...
    assert "`haishui model`" in msg and "`/login`" in msg and "`haishui auth add <provider>`" in msg
    assert "~/.haishui/.env" not in msg or display_haishui_home() == "~/.haishui"
    assert f"{display_haishui_home()}/.env" in msg
...
class TestSignInFailureCopy:
    @pytest.mark.parametrize("code, pointer", [
        ("expired_token", "`haishui portal`"),
        ("access_denied", "`haishui model`"),
    ])
    def test_poll_error_codes_map_to_plain_copy(self, code, pointer):
        from haishui_cli import auth_device_flow
        ...
        with pytest.raises(RuntimeError) as excinfo:
            auth_device_flow._poll_for_token(
                client=_Client(), portal_base_url="https://portal.nousresearch.com", client_id="haishui-cli",
                device_code="device", expires_in=5, poll_interval=1)

        msg = str(excinfo.value)
        assert not msg.startswith(code)
        assert "`haishui portal`" in msg and pointer in msg
    ...
    def test_network_failure_names_host_and_retry_command(self):
        ...
        assert "portal.nousresearch.com" in lines[0] and "`haishui portal`" in lines[0]
    ...
    def test_run_login_prints_plain_copy_not_raw_exception(self, capsys):
        from haishui_cli.model_setup_flows_common import _run_login
    ...
    def test_login_nous_prints_plain_copy(self, monkeypatch, capsys):
        from haishui_cli import auth_nous

        def _boom(**_k):
            raise httpx.ConnectError("[Errno -2] Name or service not known")

        monkeypatch.setattr(auth_nous, "_offer_shared_nous_import", lambda *_a, **_k: None, raising=False)
        monkeypatch.setattr("haishui_cli.auth._nous_device_code_login", _boom)
        ...
        out = capsys.readouterr().out
        assert "Could not sign in" in out and "`haishui portal`" in out
...
class TestAuthCommandsCopy:
    def test_unknown_provider_offers_did_you_mean_and_pointers(self, monkeypatch):
        ...
        assert "`haishui auth`" in msg and "`haishui model`" in msg
    ...
    def test_refresh_failure_names_relogin_command(self, monkeypatch):
        ...
        assert "`haishui auth add nous --type oauth`" in msg
...
def test_creds_for_target_provider_names_auth_add_and_model(monkeypatch):
    ...
    assert "`haishui auth add kimi-coding`" in msg and "/model" in msg
...
class TestProviderSetupFailureCopy:
    def test_setup_provider_step_reports_reason_and_that_nothing_changed(self, monkeypatch, capsys, tmp_path):
        from haishui_cli import setup as setup_mod
        ...
        monkeypatch.setattr("haishui_cli.main.select_provider_and_model", _boom)
        monkeypatch.setattr(setup_mod, "print_header", lambda *_a, **_k: None)
        monkeypatch.setattr(setup_mod, "_info", lambda *_a, **_k: None)
        monkeypatch.setattr("haishui_cli.config.load_config", lambda: {})
    ...
    def test_quick_setup_error_hook_uses_same_copy(self, capsys):
        ...
        lines = provider_setup_failure_lines(RuntimeError("keyring locked"), retry_command="haishui portal")
        assert lines[0].startswith("Could not finish connecting a provider")
        assert "haishui portal" in " ".join(lines)
...
class TestRunLoginProviderAwareCopy:
    def test_minimax_failure_names_minimax_retry_not_portal(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "haishui auth add minimax-oauth" in out
        assert "haishui portal" not in out and "portal.nousresearch.com" not in out
        assert "haishui login" not in out
    ...
    def test_nous_failure_keeps_portal_retry(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "haishui portal" in out and "portal.nousresearch.com" in out
    ...
    def test_silent_nonzero_system_exit_still_tells_user_how_to_retry(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "Sign-in did not complete" in out and "haishui auth add openai-codex" in out
    ...
    def test_system_exit_with_message_surfaces_the_message(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "token endpoint returned 400" in out and "haishui model" in out
