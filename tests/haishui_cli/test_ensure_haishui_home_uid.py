"""Regression tests for #34107 — Docker UID/GID handling in ensure_haishui_home.

When Haishui runs in Docker with ``HAISHUI_UID=1000`` / ``HAISHUI_GID=911``,
the entrypoint chowns the top-level ``HAISHUI_HOME`` once at startup. But
subdirectories created at runtime by ``ensure_haishui_home()`` — especially
...
The fix is a ``_chown_to_haishui_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_haishui_uid_gid
# ---------------------------------------------------------------------------


class TestResolveHaishuiUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("HAISHUI_UID", "1000")
        monkeypatch.setenv("HAISHUI_GID", "911")
        from haishui_cli.config import _resolve_haishui_uid_gid
...
# ---------------------------------------------------------------------------
# _chown_to_haishui_uid
# ---------------------------------------------------------------------------


class TestChownToHaishuiUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_haishui_uid(d)  # must not raise
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("HAISHUI_UID", raising=False)
        monkeypatch.delenv("HAISHUI_GID", raising=False)
        from haishui_cli import config as cfg
