...
from haishui_cli.auth import PROVIDER_REGISTRY, resolve_provider, resolve_api_key_provider_credentials
from haishui_cli.models import _PROVIDER_MODELS, _PROVIDER_LABELS, _PROVIDER_ALIASES, normalize_provider
...
class TestOllamaCloudModelCatalog:
    ...
    def test_provider_model_ids_returns_dynamic_models(self, tmp_path, monkeypatch):
        ...
        with patch("haishui_cli.models.fetch_api_models", return_value=["qwen3.5:397b"]), \
...
class TestOllamaCloudMergedDiscovery:
    def test_merges_live_and_models_dev(self, tmp_path, monkeypatch):
        ...
        with patch("haishui_cli.models.fetch_api_models", return_value=["qwen3.5:397b", "glm-5"]), \
             patch("agent.models_dev.fetch_models_dev", return_value=mock_mdev):
    ...
    def test_cache_only_serves_stale_cache_without_rewriting_disk(self, tmp_path, monkeypatch):
        ...
        with patch("agent.models_dev.fetch_models_dev", return_value=mock_mdev), \
             patch("haishui_cli.models.fetch_api_models", side_effect=AssertionError("network probe ran")):
        ...
        with patch("agent.models_dev.fetch_models_dev", return_value=mock_mdev), \
             patch("haishui_cli.models.fetch_api_models", return_value=["live-only", "shared", "new-live"]) as live:
...
class TestOllamaCloudProvidersNew:
    def test_overlay_exists(self):
        ...
        assert "ollama-cloud" in HAISHUI_OVERLAYS
        overlay = HAISHUI_OVERLAYS["ollama-cloud"]
        assert overlay.transport == "openai_chat"
        assert overlay.base_url_env_var == "OLLAMA_BASE_URL"

    def test_alias_resolves(self):
        from haishui_cli.providers import normalize_provider as np
...
class TestOllamaCloudSuffixStripping:
    ...
    def test_no_duplicate_when_live_clean_and_mdev_suffixed(self, tmp_path, monkeypatch):
        ...
        with patch("haishui_cli.models.fetch_api_models", return_value=["kimi-k2.6", "glm-5.1"]), \
             patch("agent.models_dev.fetch_models_dev", return_value=mock_mdev):
            result = fetch_ollama_cloud_models(force_refresh=True)
    ...
    def test_strip_suffix_helper(self):
        ...
        from haishui_cli.models_local import _strip_ollama_cloud_suffix
