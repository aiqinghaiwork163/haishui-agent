...
managers acquire the same path through ``haishui_state_registry`` from the REPL thread a
...
import haishui_cli.goals as goals
...
def test_cli_session_store_is_the_registry_handle_goals_reuse(monkeypatch):
    ...
    constructed = []
    real_open = haishui_state_registry._open_session_db

    def recording_open(path):
        constructed.append(path)
        return real_open(path)

    monkeypatch.setattr(haishui_state_registry, "_open_session_db", recording_open)

    cli = SimpleNamespace()
    try:
        HaishuiCLI._init_session_store(cli)
        ...
        haishui_state_registry.close_all()
