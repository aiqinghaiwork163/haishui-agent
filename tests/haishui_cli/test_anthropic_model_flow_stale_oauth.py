"""Tests for Bug #12905 fix — stale OAuth token detection in haishui model flow.

Bug 3: `haishui model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.haishui/.env but no valid
...
class TestStaleOAuthTokenDetection:
    ...
    def test_stale_oauth_token_triggers_reauth(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr("haishui_cli.secret_prompt.masked_secret_prompt", lambda _: "")

        from haishui_cli.model_setup_flows import _model_flow_anthropic
        cfg = {}
