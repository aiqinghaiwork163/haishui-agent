...
2. ``cli_main._refresh_windows_gateway_launchers`` — ``haishui
...
def test_restart_spec_normalizes_legacy_pythonw_argv(tmp_path):
    ...
    argv = [str(pythonw), "-m", "haishui_cli.main", "gateway", "run"]
    with mock.patch.object(
        gateway_windows, "_stable_gateway_working_dir", return_value=str(tmp_path)
    ), mock.patch("haishui_cli.config.get_haishui_home", return_value=str(tmp_path)):
...
# ---------------------------------------------------------------------------
# _refresh_windows_gateway_launchers: haishui update regenerates launchers
# ---------------------------------------------------------------------------


def test_update_launcher_refresh_reregisters_drifted_scheduled_task(monkeypatch):
    """``haishui update`` must not only rewrite the launcher scripts but also re-register a Scheduled
    ...
    assert reconciled == ["Haishui_Gateway"]
