"""Tests for the update check mechanism in haishui_cli.banner.
...
def git_repo(tmp_path, monkeypatch):
    ...
    repo_dir = tmp_path / "haishui-agent"
    repo_dir.mkdir()
    (repo_dir / ".git").mkdir()
    monkeypatch.setenv("HAISHUI_HOME", str(tmp_path))
    monkeypatch.delenv("HAISHUI_REVISION", raising=False)
    ...
    monkeypatch.setattr("haishui_cli.config.get_project_root", lambda: repo_dir)
    return repo_dir
...
def test_passive_check_uses_the_api_and_never_fetches(git_repo, monkeypatch):
    ...
    tip.assert_called_once_with("aiqinghaiwork163/haishui-agent", "main")
...
def test_cache_is_daily_but_invalidated_when_head_moves(git_repo, monkeypatch):
    """A fresh cache answers without any network; ``haishui update`` moving HEAD busts it at once;
