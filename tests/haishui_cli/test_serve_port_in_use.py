...
``haishui serve``, the gateway, anything) → ``haishui serve`` printed only
...
* conflict → single stdout sentinel ``BACKEND_PORT_IN_USE port=<port>`` +
  ...
  transient-condition convention) — and NO ``HAISHUI_BACKEND_READY``.
* free explicit port → boots and announces ``HAISHUI_BACKEND_READY`` exactly
...
def test_probe_detects_held_socket():
    from haishui_cli.web_server_lifecycle import _port_bind_conflict
...
def test_addr_in_use_error_classification():
    ...
    from haishui_cli.web_server_lifecycle import _is_addr_in_use_error
...
def test_exit_code_is_distinct_tempfail():
    from haishui_cli.web_server_lifecycle import PORT_IN_USE_EXIT_CODE
...
# ---------------------------------------------------------------------------
# E2E: real start_server in a subprocess (temp HAISHUI_HOME)
# ---------------------------------------------------------------------------
...
def _spawn_serve(port: int, tmp_path: Path, merge_stderr: bool = True) -> subprocess.Popen:
    ...
    env.update(
        ...
        HAISHUI_SERVE_HEADLESS="1",
        PYTHONUNBUFFERED="1",
    )
    # Ensure a stray desktop-parent env can't arm the watchdog/reaper paths.
    for k in ("HAISHUI_DESKTOP", "HAISHUI_PARENT_PID", "HAISHUI_PARENT_START_MARKER",
              "HAISHUI_PARENT_NONCE"):
        env.pop(k, None)
    code = (
...
def test_conflict_emits_sentinel_and_exit_75(tmp_path):
    ...
    assert "HAISHUI_BACKEND_READY" not in out  # never claimed ready
...
def test_free_port_boots_and_announces_ready(tmp_path):
    ...
    proc = _spawn_serve(port, tmp_path)
    try:
        ready, lines = _read_until(proc, "HAISHUI_BACKEND_READY")
        out = "".join(lines)
        assert ready, f"no READY sentinel; output:\n{out}"
        assert f"HAISHUI_BACKEND_READY port={port}" in out
...
def test_ephemeral_port_zero_unaffected(tmp_path):
    ...
    try:
        ...
        ready_line = next(l for l in lines if "HAISHUI_BACKEND_READY" in l)
