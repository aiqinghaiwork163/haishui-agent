...
import haishui_cli.web_routers.dashboard_ui as _rt_dashboard_ui
...
from haishui_cli import plugin_catalog
...
def _patch_minimal_hub_dependencies(monkeypatch, *, check_fn, discover_all_plugins=None):
    ...
    monkeypatch.setattr(_cfg_mod, "get_haishui_home", lambda: Path("/tmp/haishui-home"))
...
def test_plugins_hub_cold_cache_schedules_background_probe(monkeypatch):
    ...
    assert refreshed["plugins"][0]["auth_command"] == "haishui auth demo"
...
def test_plugins_hub_uses_cached_failed_check_fn_verdict(monkeypatch):
    ...
    assert payload["plugins"][0]["auth_command"] == "haishui auth demo"
...
def test_plugin_install_endpoint_invalidates_hub_cache(monkeypatch):
    ...
    from haishui_cli.web_models import _AgentPluginInstallBody
