"""Tests for IRC gateway configuration via `haishui setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Haishui install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Haishui install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#haishui")
            monkeypatch.setenv("IRC_NICKNAME", "haishui-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `haishui setup gateway` experience with IRC present."""
