...
subsystem with REAL module functions against a temp HAISHUI_HOME — it is not
...
1. RECEIPT ALWAYS FINALIZED — ``begin → steps → finalize`` writes a
   ...
   ``/api/haishui/update/receipt`` — #92780) can never interpret a crash
...
``haishui_cli.update_receipt`` / ``haishui_cli.update_inventory`` API.
"""
...
def receipt_home(tmp_path, monkeypatch):
    """Hermetic HAISHUI_HOME so receipts never touch the real profile."""
    home = tmp_path / ".haishui"
    home.mkdir()
    monkeypatch.setattr(
        "haishui_cli.config.get_haishui_home", lambda: home, raising=False
    )
    ur._current = None
    yield home
    ur._current = None
...
class TestSuccessImpliesAccounting:
    ...
    def test_full_accounting_permits_success_with_evidence(
        ...
        outcomes, incomplete, path = self._drive_decision(
            ...
            restarted_services=["haishui-serve-ops.service"],
    ...
    def test_failed_unit_also_refuses_success(self, receipt_home):
        ...
        outcomes = match_runtime_outcomes(
            ...
            externally_supervised_profiles=[],
            killed_pids=set(),
            failed_units=["haishui-gateway.service"],
        )
...
class TestRefusalIsNotFailure:
    ...
    def test_refusal_receipt_distinguishable_from_failure(
        ...
        ur.begin_update_receipt()
        ur.record_step("venv_preflight", False, "another haishui holds venv")
    ...
    def test_refused_receipt_survives_with_its_steps(self, receipt_home):
        ...
        ur.begin_update_receipt()
        ur.record_step("windows_preflight", False, "haishui.exe running")
