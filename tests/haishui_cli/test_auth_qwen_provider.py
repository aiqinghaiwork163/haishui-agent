"""Tests for Qwen OAuth provider authentication (haishui_cli/auth.py).
...
def qwen_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "haishui_cli.auth._qwen_cli_auth_path", lambda: creds_path
    )
    return tmp_path
...
def test_get_qwen_auth_status_refreshes_expired_token(qwen_env):
    ...
    with patch(
        "haishui_cli.auth._refresh_qwen_cli_tokens", return_value=refreshed
    ) as mock_refresh:
        status = get_qwen_auth_status()
...
def test_model_flow_qwen_oauth_stale_token_shows_reauth_guidance(qwen_env, monkeypatch, capsys):
    from haishui_cli.model_setup_flows import _model_flow_qwen_oauth
    ...
    monkeypatch.setattr(
        "haishui_cli.auth._refresh_qwen_cli_tokens",
