...
A long-lived multiplex process (the Desktop's ``haishui serve`` backend) routes records to every
...
def routed_profile(tmp_path, monkeypatch):
    ...
    haishui_logging.setup_logging(haishui_home=home, force=True)
    try:
        yield home, profile
    finally:
...
def _log_into(profile: Path, *, adopt: bool) -> None:
    ...
    ``setup_logging(haishui_home=<profile>)`` adoption path ``agent_init`` takes."""
    logger = logging.getLogger("agent.tests.profile-delete")
    token = set_haishui_home_override(profile)
    try:
        if adopt:
            haishui_logging.setup_logging(haishui_home=profile)
        else:
            assert haishui_logging.enable_profile_log_routing([Path(profile).parent.parent, profile]) is True
...
def _routers() -> list:
    return [h for h in haishui_logging._queued_file_handlers
            if isinstance(h, haishui_logging._ProfileRoutingFileHandler)]
