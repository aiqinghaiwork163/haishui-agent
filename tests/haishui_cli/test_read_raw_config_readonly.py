...
@pytest.fixture()
def isolated_haishui_home():
    """Per-test HAISHUI_HOME dir (already redirected by the autouse conftest
...
def test_freshness_after_config_edit(isolated_haishui_home):
    from haishui_cli.config import read_raw_config_readonly

    cfg = _write_config(isolated_haishui_home, {"display": {"ephemeral_system_ttl": 1}})
    first = read_raw_config_readonly()
    assert first["display"]["ephemeral_system_ttl"] == 1

    _write_config(isolated_haishui_home, {"display": {"ephemeral_system_ttl": 7}})
...
def test_missing_config_returns_empty(isolated_haishui_home):
    from haishui_cli.config import read_raw_config_readonly

    cfg = isolated_haishui_home / "config.yaml"
    if cfg.exists():
        cfg.unlink()
