...
from haishui_cli.tool_availability_notices import filter_to_enabled_toolsets, tool_availability_warning_lines
...
def test_terminal_line_names_backend_reason_and_fix_commands():
    ...
    assert "haishui doctor" in text and "haishui setup terminal" in text
...
def test_web_collapses_env_dump_into_one_provider_sentence():
    ...
    assert "Web search is off" in text and "haishui setup tools" in text
...
def test_generic_toolset_lists_vars_and_setup_hint():
    ...
    text = "\n".join(lines)
    assert "rl" in text and "TINKER_API_KEY" in text and "haishui setup" in text
...
def test_web_notice_cites_setup_tools_not_the_toggle_menu():
    # `haishui tools` with no subcommand is the enable/disable menu; providers live under `haishui setup tools`.
    lines = tool_availability_warning_lines([_WEB], terminal_reason=None, terminal_backend="local")
    text = "\n".join(lines)
    assert "haishui setup tools" in text
    assert "haishui tools[/]" not in text


class TestFilterToEnabledToolsets:
    _BUNDLE = {"haishui-cli": ["terminal", "process_manage", "web_search", "read_file"],
    ...
    def test_composite_bundle_keeps_terminal_notice_on_default_install(self):
        kept = filter_to_enabled_toolsets([_TERMINAL, _WEB, _RL], ["haishui-cli"], self._resolve)
    ...
    def test_resolver_error_is_not_fatal(self):
        ...
        assert filter_to_enabled_toolsets([_TERMINAL], ["haishui-cli"], boom) == []
