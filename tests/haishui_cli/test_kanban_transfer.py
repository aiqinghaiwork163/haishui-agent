"""Tests for kanban board export / import (``haishui_cli.kanban_transfer``).
...
from haishui_cli import kanban_transfer as kt
from haishui_cli.archive_safe import normalize_archive_parts, safe_extract_targz
...
def kanban_root(tmp_path, monkeypatch):
    ...
    def _use(name: str) -> Path:
        ...
        monkeypatch.setenv("HAISHUI_KANBAN_HOME", str(root))
        for var in ("HAISHUI_KANBAN_DB", "HAISHUI_KANBAN_WORKSPACES_ROOT",
                    "HAISHUI_KANBAN_ATTACHMENTS_ROOT", "HAISHUI_KANBAN_BOARD"):
...
def test_import_rejects_a_future_format_version(kanban_root, tmp_path):
    ...
    kanban_root("target")
    with pytest.raises(ValueError, match="newer than this Haishui"):
