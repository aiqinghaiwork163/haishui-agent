...
haishui_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `haishui config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_haishui_home = get_haishui_home()`` at import time (module
    singleton), so monkeypatching HAISHUI_HOME after import doesn't move it.
    ...
    this only matters for tests that swap HAISHUI_HOME.)
    """
    import cli

    cli._haishui_home = home
