...
Covers three additions to ``haishui update``:
...
def test_detect_venv_python_prefetches_only_cheap_process_fields(_winp, tmp_path):
    venv_py = str(tmp_path / "venv" / "Scripts" / "python.exe")
    holder = _proc(101, venv_py, "python.exe", [venv_py, "-m", "haishui_cli.main", "serve"])
...
def test_detect_venv_python_matches_uv_default_dotvenv(_winp, tmp_path):
    ...
    no ``haishui_cli.main`` in its cmdline, so only that arm can match it — the guard was blind to it."""
    venv_py = str(tmp_path / ".venv" / "Scripts" / "python.exe")
    holder = _proc(104, venv_py, "python.exe", [venv_py, str(tmp_path / "tools" / "haishui_kernel_runner.py")])
    me = MagicMock()
    me.parents.return_value = []
    fake_psutil = types.SimpleNamespace(
...
def test_detect_venv_python_keeps_external_interpreter_fallback(_winp, tmp_path):
    external = _proc(
        ...
        "python.exe",
        ["python.exe", "-m", "haishui_cli.main", "serve"],
        str(tmp_path),
    )
    me = MagicMock()
    me.parents.return_value = []
...
def _run_update_until_guard(args):
    ...
    with patch.object(cli_main, "_is_windows", return_value=True), patch.object(
        ...
        cli_main,
        "_detect_venv_python_processes",
        return_value=[(101, "python.exe", "python.exe -m haishui_cli.main serve")],
    ), patch.object(
