"""Tests for haishui_cli.update_receipt — Phase 1 of the fleet-update plan (#91277).
...
from haishui_cli import update_cmd, update_cmd_maint


@pytest.fixture()
def receipt_home(tmp_path, monkeypatch):
    """Isolated HAISHUI_HOME for receipt writes."""
    home = tmp_path / ".haishui"
    home.mkdir()
    # ``_receipt_dir`` resolves through ``haishui_constants.get_haishui_home`` (env var), not
    # ``haishui_cli.config`` — patch where production reads.
...
class TestReceiptLifecycle:
    def test_begin_record_finalize_roundtrip(self, receipt_home):
        ...
        ur.record_gateway_restart(
            restarted_services=["haishui-gateway"],
        ...
        assert gr["restarted_services"] == ["haishui-gateway"]
...
class TestCommandBoundaryFinalization:
    ...
    def test_pending_receipt_persisted_on_exit_2_refusal(self, receipt_home):
        ur.begin_update_receipt()
        ur.record_step("windows_preflight", False, "another haishui.exe running")
    ...
    def test_cmd_update_boundary_finalizes_on_early_exit(
        ...
        def _fake_impl(args, gateway_mode):
            ur.begin_update_receipt()
            ur.record_step("windows_preflight", False, "haishui.exe holds venv")
            sys.exit(2)

        monkeypatch.setattr(update_cmd, "_cmd_update_impl", _fake_impl)
        monkeypatch.setattr(
            haishui_main, "detect_install_method", lambda *a, **k: "git", raising=False
        )
        monkeypatch.setattr(
            haishui_main,
        ...
        monkeypatch.setattr(
            haishui_main, "_finalize_update_output", lambda state: None, raising=False
        )
        ...
        import haishui_cli.update_lock as update_lock_mod
...
class TestFleetClassification:
    def _fleet_with(self, monkeypatch, tmp_path, record, expected_sha="a" * 40):
        ...
        monkeypatch.setattr(
            "haishui_cli.profiles._get_default_haishui_home", lambda: home
        )
        monkeypatch.setattr(
            "haishui_cli.profiles._get_profiles_root",
    ...
    def test_identity_pending_row_gets_restart_aware_copy(self, capsys):
        ...
        assert ok is False
        out = capsys.readouterr().out
        assert "code identity not published yet" in out and "haishui gateway status" in out
...
class TestGatewayStatusStamping:
    ...
    def test_code_identity_failure_degrades_to_absent(self, monkeypatch):
        ...
        monkeypatch.setattr("haishui_cli.build_info.get_code_identity", _boom)
        record = gs._build_runtime_status_record()
