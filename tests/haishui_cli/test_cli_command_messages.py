"""User-facing message contracts for `haishui skills/plugins/mcp/cron/doctor` failure paths.

Every failure line must say what happened and name an existing command to type next; none may
lead with a raw exception, a status literal, or a hardcoded ``~/.haishui`` path (UX message audit,
...
def test_skill_fetch_failure_names_search_and_doctor():
    from haishui_cli.skills_hub import _print_fetch_failure
    c, buf = _console()
    _print_fetch_failure(c, sources=[], identifier="someone/typo-skill")
    out = buf.getvalue()
    assert "haishui skills search typo-skill" in out
    assert "haishui doctor" in out
    assert "from any source" not in out


def test_scan_block_message_explains_force_and_points_at_inspect():
    from haishui_cli.skills_hub import _scan_block_message
    ...
    assert "haishui skills inspect acme/evil-skill" in msg
...
def test_plugin_clone_failure_leads_with_next_steps_and_escapes_git_output():
    from haishui_cli.plugins_cmd import _clone_failure_message
    ...
    assert "haishui plugins search" in msg and "gh auth login" in msg
...
def test_unknown_plugin_message_points_at_list_and_full_key_rule():
    from haishui_cli.plugins_cmd import _unknown_plugin_message
    msg = _unknown_plugin_message("firecrawl")
    assert "No plugin named 'firecrawl'" in msg
    assert "haishui plugins list" in msg and "web/firecrawl" in msg and "haishui plugins install" in msg
...
def test_banner_failed_mcp_line_shows_reason_and_command():
    from haishui_cli.banner import _mcp_server_line
    ...
    assert "haishui mcp test gh" in line
    ...
    assert "haishui mcp login linear" in auth


def test_mcp_probe_failure_unwraps_group_and_points_at_login_for_401():
    from haishui_cli.mcp_config import _probe_failure_next_step, _probe_failure_reason
    ...
    assert "haishui mcp login linear" in _probe_failure_next_step("linear", group)

    missing = FileNotFoundError(2, "No such file or directory", "npx")
    assert "config.yaml" in _probe_failure_next_step("gh", missing)
    assert "~/.haishui" not in _probe_failure_next_step("gh", missing) or "profiles" in _probe_failure_next_step("gh", missing)


# ── cron ──────────────────────────────────────────────────────────────────────────────────────

def test_cron_delivery_failure_lines_have_no_status_literals_and_name_the_fix():
    from haishui_cli.cron import _cron_doctor_issues_for_job, _job_warnings, _last_run_display
    ...
    assert "haishui cron edit job42 --deliver" in joined
    assert "haishui gateway restart" in joined and "haishui cron run job42" in joined
    assert "gateway was unreachable" in joined


def test_cron_queued_status_reads_as_in_progress():
    from haishui_cli.cron import _last_run_display
    assert "in progress" in _last_run_display({"last_status": "delivery_queued"})


# ── doctor / advisories / paths ───────────────────────────────────────────────────────────────

def test_advisory_remediation_uses_active_haishui_home(monkeypatch, tmp_path):
    from haishui_cli import security_advisories as sa
    monkeypatch.setattr(sa, "display_haishui_home", lambda: "~/.haishui/profiles/work")
    hit = SimpleNamespace(advisory=sa.ADVISORIES[0], package="mistralai", installed_version="2.4.6")
    text = "\n".join(sa.full_remediation_text(hit))
    assert "~/.haishui/profiles/work/.env" in text
    assert "{haishui_home}" not in text
    assert not re.search(r"~/\.haishui/\.env", text)


def test_doctor_ack_failure_names_config_path_and_retry(monkeypatch, capsys):
    import haishui_cli.doctor as doctor
    monkeypatch.setattr("haishui_cli.security_advisories.ack_advisory", lambda _id: False)
    monkeypatch.setattr(doctor, "_DHH", "~/.haishui/profiles/work")
    from haishui_cli.security_advisories import ADVISORIES
    try:
        doctor._ack_advisory(ADVISORIES[0].id)
    except SystemExit as exc:
        assert exc.code == 1
    out = capsys.readouterr().out
    assert "~/.haishui/profiles/work/config.yaml" in out
    assert "haishui config path" in out and f"haishui doctor --ack {ADVISORIES[0].id}" in out
    assert "Failed to persist" not in out


def test_backup_recovery_hint_cites_the_slash_command_not_a_fake_subcommand():
    from haishui_cli.backup import _snapshot_recovery_hint
    hint = _snapshot_recovery_hint()
    assert "/snapshot list" in hint and "/snapshot restore" in hint
    assert "haishui snapshot" not in hint  # `haishui snapshot` is not a CLI subcommand
