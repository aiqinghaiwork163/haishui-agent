...
def _enable(haishui_home: Path, name: str) -> None:
    """Append ``name`` to ``plugins.enabled`` in ``<haishui_home>/config.yaml``."""
...
class TestCategoryNamespaceRecursion:
    def test_category_namespace_discovered(self, tmp_path, monkeypatch):
        ...
        import os
        haishui_home = Path(os.environ["HAISHUI_HOME"])  # set by hermetic conftest fixture
        user_plugins = haishui_home / "plugins"

        _write_plugin(user_plugins, ["image_gen", "openai"])
        _enable(haishui_home, "image_gen/openai")
...
class TestKindField:
    def test_default_kind_is_standalone(self, tmp_path, monkeypatch):
        import os
        haishui_home = Path(os.environ["HAISHUI_HOME"])  # set by hermetic conftest fixture
        _write_plugin(haishui_home / "plugins", ["p1"])
        _enable(haishui_home, "p1")
...
class TestBackendGate:
    ...
    def test_exclusive_kind_skipped(self, tmp_path, monkeypatch):
        ...
        _enable(haishui_home, "some-backend")
...
class TestRegisterImageGenProvider:
    def test_accepts_valid_provider(self, tmp_path, monkeypatch):
        ...
        _enable(haishui_home, "my-img-plugin")
    ...
    def test_rejects_non_provider(self, tmp_path, monkeypatch, caplog):
        ...
        _enable(haishui_home, "bad-img-plugin")
