"""Tests for haishui_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $HAISHUI_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Haishui-managed node dir.
"""
...
def _make_haishui_node(haishui_home: Path) -> Path:
    """Create a fake $HAISHUI_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = haishui_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    haishui_home = fake_home / ".haishui"
    _make_haishui_node(haishui_home)
    ...
    removed = uninstall.remove_node_symlinks(haishui_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    haishui_home = fake_home / ".haishui"
    node_bin = _make_haishui_node(haishui_home)
