...
``haishui gateway status`` / ``gateway list`` / ``profile list`` keyed liveness
...
def _fake_multiplexer(monkeypatch, tmp_path, *, multiplex: bool, pid_file: bool = True):
    ...
    (tmp_path / "gateway_state.json").write_text(json.dumps({
        "pid": os.getpid(), "kind": "haishui-gateway", "gateway_state": "running",
        "start_time": status._get_process_start_time(os.getpid()), "haishui_home": str(tmp_path),
    }))
    ...
    monkeypatch.setattr(
        status, "_read_process_cmdline", lambda pid: "python -m haishui_cli.main gateway run --replace"
    )
