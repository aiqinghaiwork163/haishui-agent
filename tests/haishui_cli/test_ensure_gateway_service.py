"""Tests for haishui_cli.gateway.ensure_gateway_service — the zero-prompt
service install/start path used by `haishui setup` and `haishui import`.
...
def _patch_host(monkeypatch, *, container=False, systemd=True, macos=False, windows=False):
    monkeypatch.setattr("haishui_constants.is_container", lambda: container)
