"""``haishui update`` refreshes the installed macOS ``Haishui.app`` from the rebuilt bundle (#52339).

``haishui desktop --build-only`` only packages into ``apps/desktop/release/``; Finder launches the
...
def _bundle(root: Path, asar: bytes) -> Path:
    app = root / "Haishui.app"
    (app / "Contents" / "MacOS").mkdir(parents=True)
    (app / "Contents" / "MacOS" / "Haishui").write_bytes(b"\xcf\xfa\xed\xfe")
...
def test_stale_bundle_is_replaced_current_and_running_are_left_alone(rebuilt, tmp_path):
    ...
    installed, problems = main_desktop._install_rebuilt_macos_bundles(
        rebuilt, [stale, current, running, tmp_path / "missing" / "Haishui.app"],
        running={running.resolve()})

    assert installed == [stale]
    assert _asar(stale) == b"rebuilt"
    assert not (stale.parent / "Haishui.app.haishui-update-old").exists()
    assert not (stale.parent / "Haishui.app.haishui-update-new").exists()
    assert current_marker.read_text() == "untouched"
    # A live app is reported, never swapped under.
    assert _asar(running) == b"older"
    assert len(problems) == 1 and str(running) in problems[0] and "quit Haishui Desktop" in problems[0]
...
def test_failed_swap_keeps_the_previous_bundle_launchable(rebuilt, tmp_path, monkeypatch):
    ...
    def fail_final_rename(self, target):
        if self.name.endswith(".haishui-update-new"):
