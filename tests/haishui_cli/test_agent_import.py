"""Tests for haishui_cli.agent_import — ``haishui import-agent``.

Covers: source detection, Claude Code and Codex parsing, mapping into the
real Haishui stores (memories/MEMORY.md, config.yaml command_allowlist /
...
Uses the profile_env fixture pattern from tests/haishui_cli/test_profiles.py:
Path.home() and HAISHUI_HOME are redirected to tmp_path so nothing touches
the real ~/.haishui.
"""
...
from haishui_cli.agent_import import (
    ENTRY_DELIMITER,
    AgentImporter,
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and HAISHUI_HOME (profile_env pattern)
# ---------------------------------------------------------------------------

@pytest.fixture()
def profile_env(tmp_path, monkeypatch):
    """Isolated environment: Path.home() -> tmp_path, HAISHUI_HOME -> tmp/.haishui."""
...
@pytest.fixture()
def haishui_home(profile_env):
    return profile_env / ".haishui"
...
def run_import(agent, source, haishui_home, execute, overwrite=False):
    return AgentImporter(
        agent=agent,
        source_root=source,
        target_root=haishui_home,
        execute=execute,
        overwrite=overwrite,
    ).run()
...
class TestDetection:
    ...
    def test_unsupported_agent_raises(self, haishui_home, tmp_path):
        with pytest.raises(ValueError):
            AgentImporter("cursor", tmp_path, haishui_home)
...
class TestDryRun:
    def test_claude_dry_run_writes_nothing(self, claude_tree, haishui_home):
        before = snapshot_tree(haishui_home)
        report = run_import("claude-code", claude_tree, haishui_home, execute=False)
        assert snapshot_tree(haishui_home) == before
        assert report["dry_run"] is True
        assert report["summary"]["imported"] > 0


    def test_dry_run_and_real_run_plan_same_items(self, claude_tree, haishui_home):
        preview = run_import("claude-code", claude_tree, haishui_home, execute=False)
        applied = run_import("claude-code", claude_tree, haishui_home, execute=True)
...
class TestClaudeCodeImport:
    @pytest.fixture()
    def report(self, claude_tree, haishui_home):
        return run_import("claude-code", claude_tree, haishui_home, execute=True)


    def test_allowlist_lands_in_config_yaml(self, report, haishui_home):
        config = yaml.safe_load((haishui_home / "config.yaml").read_text(encoding="utf-8"))
...
class TestCodexImport:
    @pytest.fixture()
    def report(self, codex_tree, haishui_home):
        return run_import("codex", codex_tree, haishui_home, execute=True)


    def test_mcp_servers_from_config_toml(self, report, haishui_home):
        config = yaml.safe_load((haishui_home / "config.yaml").read_text(encoding="utf-8"))
    ...
    def test_skill_copied(self, report, haishui_home):
        assert (haishui_home / "skills" / "codex-imports" / "db-migrate" / "SKILL.md").exists()
...
class TestSecretsNeverImported:
    def test_no_secret_values_anywhere_claude(self, claude_tree, haishui_home):
        run_import("claude-code", claude_tree, haishui_home, execute=True)
        blob = "".join(
            p.read_text(encoding="utf-8", errors="replace")
            for p in haishui_home.rglob("*") if p.is_file()
        )
        assert "ghp_SECRET123" not in blob
        assert "Bearer abc123" not in blob
        assert "sk-ant-SUPERSECRET" not in blob

    def test_no_secret_values_anywhere_codex(self, codex_tree, haishui_home):
        run_import("codex", codex_tree, haishui_home, execute=True)
        blob = "".join(
            p.read_text(encoding="utf-8", errors="replace")
            for p in haishui_home.rglob("*") if p.is_file()
        )
    ...
    def test_stripped_secrets_reported(self, claude_tree, haishui_home):
        report = run_import("claude-code", claude_tree, haishui_home, execute=True)
    ...
    def test_non_secret_header_kept(self, claude_tree, haishui_home):
        run_import("claude-code", claude_tree, haishui_home, execute=True)
        config = yaml.safe_load((haishui_home / "config.yaml").read_text(encoding="utf-8"))
...
class TestMalformedInputs:
    def test_bad_settings_json_reports_error(self, profile_env, haishui_home):
        root = profile_env / ".claude"
        root.mkdir()
        (root / "settings.json").write_text("{not json!!", encoding="utf-8")
        (root / "CLAUDE.md").write_text("- still importable\n", encoding="utf-8")
        report = run_import("claude-code", root, haishui_home, execute=True)
        ...
        assert "still importable" in (
            haishui_home / "memories" / "MEMORY.md").read_text(encoding="utf-8")


    def test_empty_tree_all_skipped(self, profile_env, haishui_home):
        root = profile_env / ".codex"
        root.mkdir()
        report = run_import("codex", root, haishui_home, execute=True)
        assert report["summary"]["imported"] == 0
...
class TestMergeSemantics:
    ...
    def test_existing_skill_conflicts_without_overwrite(
            self, claude_tree, haishui_home):
        dest = haishui_home / "skills" / "claude-code-imports" / "deploy-helper"
    ...
    def test_reimport_is_idempotent_for_memory(self, claude_tree, haishui_home):
        run_import("claude-code", claude_tree, haishui_home, execute=True)
        first = (haishui_home / "memories" / "MEMORY.md").read_text(encoding="utf-8")
        report = run_import("claude-code", claude_tree, haishui_home, execute=True)
        assert (haishui_home / "memories" / "MEMORY.md").read_text(encoding="utf-8") == first
...
class TestExistingMemoryStorePreserved:
    ...
    @pytest.fixture()
    def seeded_home(self, haishui_home):
        memory = haishui_home / "memories" / "MEMORY.md"
...
class TestExistingConfigPreserved:
    ...
    @pytest.fixture()
    def config_path(self, haishui_home):
        return haishui_home / "config.yaml"
    ...
    def test_malformed_config_is_left_byte_identical(
            self, claude_tree, haishui_home, config_path):
    ...
    def test_malformed_config_records_an_error_not_an_import(
        ...
        for kind in CONFIG_WRITING_KINDS:
            ...
            assert "haishui config edit" in reason
    ...
    def test_unreadable_config_is_left_byte_identical(
        ...
        try:
            report = run_import("claude-code", claude_tree, haishui_home,
                                execute=True)
            statuses = {
                i["status"]
                for kind in CONFIG_WRITING_KINDS
    ...
    def test_import_preserves_every_pre_existing_config_key(
        ...
        assert merged["model"] == "haishui-4-405b"
    ...
    def test_config_write_is_atomic_and_leaves_no_temp_files(
        ...
        leftovers = [p.name for p in haishui_home.glob(".tmp*")]
        assert leftovers == []

    def test_symlinked_config_stays_a_symlink(
            self, claude_tree, haishui_home, tmp_path, config_path):
    ...
    def test_failed_write_does_not_truncate_the_existing_config(
            self, haishui_home, config_path, monkeypatch):
        """An interrupted dump leaves the previous file complete."""
        from haishui_cli import agent_import
...
class TestCliWiring:
    ...
    def test_command_dry_run_via_cli_writes_nothing(
            self, claude_tree, haishui_home, capsys):
        """End-to-end through import_agent_command with --dry-run."""
        import types
        from haishui_cli.agent_import import import_agent_command
        ...
        assert not (haishui_home / "memories" / "MEMORY.md").exists()
        assert not (haishui_home / "skills" / "claude-code-imports").exists()
        config_text = (haishui_home / "config.yaml").read_text(encoding="utf-8") \
            if (haishui_home / "config.yaml").exists() else ""
...
class TestSyncManifest:
    ...
    def test_import_registers_source_and_unchanged_sync_is_noop(
            self, claude_tree, haishui_home, capsys):
        from haishui_cli.agent_import_sync import load_sync_manifest

        self._run_command("claude-code", claude_tree)
        entry = load_sync_manifest(haishui_home)["agents"]["claude-code"]
    ...
    def test_sync_reimports_changed_source_but_never_clobbers_user_skill(
        ...
        user_skill = haishui_home / "skills" / "claude-code-imports" / "hand-rolled"
        ...
        imports = haishui_home / "skills" / "claude-code-imports"
        ...
        # An imported skill the user then EDITED locally is no longer Haishui-owned: the next sync
    ...
    def test_sync_dry_run_previews_without_writing(self, claude_tree, haishui_home, capsys):
        from haishui_cli.agent_import_sync import load_sync_manifest

        self._run_command("claude-code", claude_tree)
        old_digest = load_sync_manifest(haishui_home)["agents"]["claude-code"]["digest"]
        (claude_tree / "CLAUDE.md").write_text(
            CLAUDE_MD + "\n- Dry sync entry\n", encoding="utf-8")
        before = snapshot_tree(haishui_home)
        capsys.readouterr()
        ...
        assert snapshot_tree(haishui_home) == before
        assert load_sync_manifest(haishui_home)["agents"]["claude-code"]["digest"] == old_digest
