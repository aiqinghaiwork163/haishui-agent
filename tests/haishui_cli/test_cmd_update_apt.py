"""APT-managed Haishui installs must never fall through to the git updater."""
...
def test_apt_stamp_is_detected_and_recommends_pkg_upgrade(tmp_path):
    from haishui_cli.config import detect_install_method, recommended_update_command_for_method

    (tmp_path / ".install_method").write_text("apt\n", encoding="utf-8")
    assert detect_install_method(project_root=tmp_path) == "apt"
    assert recommended_update_command_for_method("apt") == "pkg upgrade haishui-agent"


@patch("haishui_cli.config.is_managed", return_value=False)
@patch("haishui_cli.config.detect_install_method", return_value="apt")
...
def test_cmd_update_apt_prints_pkg_guidance_without_git(
    ...
    assert "pkg upgrade haishui-agent" in capsys.readouterr().out
