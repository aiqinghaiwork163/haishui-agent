...
def test_detect_returns_none_when_probe_subprocess_fails(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        haishui_cli_main_install_repair, "_resolve_install_target_python", lambda *a, **k: python
    )
    monkeypatch.setattr(
        m.subprocess,
        "run",
...
def test_capture_active_tool_dependencies_uses_tools_status_probes(monkeypatch):
    from haishui_cli import tools_config_post_setup
...
def test_restore_active_tool_dependencies_uses_static_allowlist(monkeypatch):
    ...
    monkeypatch.setattr(
        haishui_cli_main_install_repair,
        "_run_package_only_install",
        lambda cmd, *, env=None: calls.append((cmd, env)),
    )
...
def test_cmd_update_captures_and_propagates_pre_rebuild_snapshot(
    ...
    monkeypatch.setattr(haishui_cli_main_install_repair, "_is_windows", lambda: False)
    ...
    monkeypatch.setattr(m, "_get_origin_url", lambda *args: "https://github.com/aiqinghaiwork163/haishui-agent.git")
