...
The notice prints the most recent curator run summary on `haishui update`,
...
session start) so users normally never see the rename map. `haishui update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    from haishui_cli import update_cmd_maint
