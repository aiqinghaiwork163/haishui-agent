...
Regression: the old implementation wrote ``haishui_conversation_<ts>.json``
to the current working directory (CWD). Users who ran /save expected the
file to be discoverable via ``haishui sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.haishui/sessions/saved/`` and prints
...
def haishui_home(tmp_path, monkeypatch):
    ...
    # Clear any cached haishui_home computation
    import haishui_constants
    if hasattr(haishui_constants, "_haishui_home_cache"):
        haishui_constants._haishui_home_cache = None
    return home
...
def test_save_conversation_writes_under_haishui_home(haishui_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.haishui/sessions/saved/, not CWD."""
    # Change CWD to a different directory to prove the file does NOT go there.
    work = tmp_path / "somewhere-else"
    work.mkdir()
    monkeypatch.chdir(work)

    # Import fresh to pick up the HAISHUI_HOME fixture
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "haishui_constants"]:
    ...
    cli.HaishuiCLI.save_conversation(stub, "/save json")

    # File must NOT be in CWD
    cwd_leak = list(work.glob("haishui_conversation_*.json"))
    assert not cwd_leak, f"snapshot leaked to CWD: {cwd_leak}"

    # File MUST be under ~/.haishui/sessions/saved/
    saved_dir = haishui_home / "sessions" / "saved"
    assert saved_dir.is_dir(), "expected saved/ subdirectory to be created"
    files = list(saved_dir.glob("haishui_conversation_*.json"))
    ...
    assert "haishui --resume 20260101_120000_abc123" in out, out


def test_save_conversation_empty_history_does_nothing(haishui_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "haishui_constants"]:
        sys.modules.pop(mod, None)
    import cli
...
def test_save_conversation_bare_shows_usage(haishui_home, capsys):
    ...
    cli.HaishuiCLI.save_conversation(stub, "/save")
...
def test_save_conversation_bad_format_shows_usage(haishui_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "haishui_constants"]:
        sys.modules.pop(mod, None)
    import cli

    stub = _make_stub_cli([{"role": "user", "content": "hi"}])
    cli.HaishuiCLI.save_conversation(stub, "/save pdf")
