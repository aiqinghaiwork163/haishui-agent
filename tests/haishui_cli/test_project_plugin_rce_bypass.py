...
Execution via the ``HAISHUI_ENABLE_PROJECT_PLUGINS`` bypass in the web
server's dashboard plugin loader.

Two primitives combined into the original advisory chain:

1. ``haishui_cli.web_server_dashboard._discover_dashboard_plugins`` opted into
   the untrusted ``./.haishui/plugins/`` source via
   ``os.environ.get("HAISHUI_ENABLE_PROJECT_PLUGINS")`` — truthy for
...
2. ``haishui_cli.web_server_dashboard._mount_plugin_api_routes`` then imported
...
# ---------------------------------------------------------------------------
# Layer 1 — HAISHUI_ENABLE_PROJECT_PLUGINS env gate uses truthy semantics.
# ---------------------------------------------------------------------------
...
class TestProjectPluginsEnvGate:
    ...
    def project_plugin(self, tmp_path, monkeypatch):
        """Plant a project-source plugin under CWD's ``.haishui/plugins``
    ...
    def test_falsy_values_keep_project_plugins_disabled(
        ...
        if value == "":
            monkeypatch.delenv("HAISHUI_ENABLE_PROJECT_PLUGINS", raising=False)
        else:
            monkeypatch.setenv("HAISHUI_ENABLE_PROJECT_PLUGINS", value)

        plugins = web_server._get_dashboard_plugins(force_rescan=True)
        names = {p["name"] for p in plugins}
        assert "evil" not in names, (
            f"HAISHUI_ENABLE_PROJECT_PLUGINS={value!r} must NOT enable the "
...
class TestEndToEndPocBlocked:
    ...
    ``HAISHUI_ENABLE_PROJECT_PLUGINS=0`` (so the operator believed the
    ...
    def test_full_chain_blocked(self, tmp_path, monkeypatch):
        ...
        for call in spec.call_args_list:
            ...
            assert module_name != "haishui_dashboard_plugin_evil"
            assert target != payload_py
            assert "evil-repo" not in target.parts
        assert "haishui_dashboard_plugin_evil" not in sys.modules
