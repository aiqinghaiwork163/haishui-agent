...
from haishui_cli.active_sessions import session_already_owned_message
from haishui_cli.cli_chat_error_copy import agent_init_failure_message, chat_error_response
...
@pytest.mark.parametrize("exc, pointer, absent", [
    (_StatusError("HTTP 401: Invalid API key", 401), "haishui model", "HTTP 401"),
    ...
    (Exception("Unknown error"), "haishui doctor", "Unknown error"),
])
...
def test_chat_error_response_accepts_plain_string_summary():
    ...
    assert "haishui model" in text.splitlines()[0]
...
def test_agent_init_failure_message_says_message_not_sent_and_points_to_doctor():
    ...
    assert "haishui doctor" in text and "/model" in text
...
def test_owned_message_first_line_is_plain_and_details_follow():
    ...
    assert first == ("This chat is open in another Haishui window/terminal. "
...
# ── cli-30: haishui sessions with a bad id / unopenable DB ──────────────────

def test_sessions_not_found_points_to_list(capsys):
    from haishui_cli.sessions_cmd import _not_found
    assert _not_found("abc") == 1
    out = capsys.readouterr().out
    assert "No session 'abc'" in out
    assert "haishui sessions list" in out


def test_sessions_db_open_failure_points_to_repair(monkeypatch, capsys):
    import haishui_cli.sessions_cmd as sessions_cmd
    ...
    db_path = haishui_state._default_db_path()
    ...
    out = capsys.readouterr().out
    assert code == 1
    assert "haishui sessions repair" in out
