...
``set_haishui_home_override`` / ``reset_haishui_home_override``. Because
``_plugins_dir()`` keys off ``get_haishui_home()``, the list action must then
...
def test_plugins_manage_profile_reads_that_profiles_dir(tmp_path, monkeypatch):
    ...
    import haishui_cli.profiles as profiles
