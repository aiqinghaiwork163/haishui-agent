...
def _load_plugin_router():
    ...
    spec = importlib.util.spec_from_file_location(
        "haishui_dashboard_plugin_kanban_test", plugin_file,
    )
...
def test_ws_events_rejects_when_token_required(tmp_path, monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "haishui_cli.web_server_chat", stub)
    monkeypatch.setattr(haishui_cli, "web_server_chat", stub, raising=False)
...
def test_dashboard_board_project_binding_is_exposed_in_ui():
    ...
    assert "haishui-kanban-board-project-unbind" in bundle
...
# tests/haishui_cli/test_kanban_db.py. The OUTER fallback here is not, which
