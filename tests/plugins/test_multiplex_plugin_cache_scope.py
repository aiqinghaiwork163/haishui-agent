...
HAISHUI_HOME override (``haishui_constants.set_haishui_home_override``).
...
def homes(tmp_path, monkeypatch):
    """Profile A (launch home, ``HAISHUI_HOME``) and profile B with different config/.env values."""
...
def test_router_efforts_cache_and_base_url_follow_the_active_profile(homes, monkeypatch):
    ...
    profile's .env."""
    import haishui_cli.urllib_security as urllib_security
...
def test_memory_provider_skill_prune_only_touches_the_active_home(homes, monkeypatch):
    ...
    from haishui_cli.plugins import _reset_plugin_managers_for_tests, get_plugin_manager
    ...
    try:
        for home in (a, b):
            with scoped(home):
                ...
                provider.initialize(session_id=f"s-{home.name}", haishui_home=str(home))
