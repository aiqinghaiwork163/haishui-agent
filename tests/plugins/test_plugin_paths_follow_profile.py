"""Plugin data paths follow the active profile's HAISHUI_HOME, including the ContextVar override.

Several plugins carried a ``~/.haishui`` fallback (guarding an ImportError of ``haishui_constants``
...
def _openviking_log(home):
    ...
    return ov.get_haishui_home() / ov._OPENVIKING_SERVER_LOG_RELATIVE_PATH
