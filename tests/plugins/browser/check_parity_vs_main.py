...
Run from the PR worktree:
    ...
    cd ~/.haishui/haishui-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.haishui/haishui-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.haishui/haishui-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a haishui-agent checkout"
)
...
# Isolated HAISHUI_HOME for the config write.
