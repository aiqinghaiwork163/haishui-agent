...
from haishui_cli.memory_setup import _CANCELLED
...
def _provider_for_mode(tmp_path, monkeypatch, mode: str):
    ...
    monkeypatch.setattr(
        "plugins.memory.hindsight.get_haishui_home", lambda: tmp_path
    )

    provider = HindsightMemoryProvider()
    provider.initialize(session_id="test-session", haishui_home=str(tmp_path), platform="cli")
    return provider
...
def provider(tmp_path, monkeypatch):
    ...
    p.initialize(session_id="test-session", haishui_home=str(tmp_path), platform="cli")
...
def test_normalize_retain_tags_accepts_csv_and_dedupes():
    assert _normalize_retain_tags("agent:fakeassistantname, source_system:haishui-agent, agent:fakeassistantname") == [
        "agent:fakeassistantname",
        "source_system:haishui-agent",
    ]
...
class TestConfig:
    ...
    def test_default_values(self, provider):
        ...
        assert provider._retain_context == "conversation between Haishui Agent and the User"
    ...
    def test_custom_config_values(self, provider_with_config):
        ...
        assert p._retain_source == "haishui"
...
class TestPostSetup:
    def test_setup_cancel_at_mode_picker_writes_nothing(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("plugins.memory.hindsight.get_haishui_home", lambda: haishui_home)

        save_config = MagicMock()
        which = MagicMock(return_value="/usr/bin/uv")
        run = MagicMock()
        monkeypatch.setattr("haishui_cli.memory_setup._curses_select", lambda *args, **kwargs: _CANCELLED)
        ...
        monkeypatch.setattr("haishui_cli.config.save_config", save_config)

        provider = HindsightMemoryProvider()
        provider.post_setup(str(haishui_home), {"memory": {"provider": "builtin"}})

        save_config.assert_not_called()
        which.assert_not_called()
        run.assert_not_called()
        assert not (haishui_home / ".env").exists()
        assert not (haishui_home / "hindsight" / "config.json").exists()
        assert not (user_home / ".hindsight" / "profiles" / "haishui.env").exists()
    ...
    def test_local_embedded_setup_materializes_profile_env(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("haishui_cli.memory_setup._curses_select", lambda *args, **kwargs: next(selections))
        monkeypatch.setattr("shutil.which", lambda name: None)
        monkeypatch.setattr("builtins.input", lambda prompt="": "")
        monkeypatch.setattr("sys.stdin.isatty", lambda: True)
        monkeypatch.setattr("getpass.getpass", lambda prompt="": "sk-local-test")
        saved_configs = []
        monkeypatch.setattr("haishui_cli.config.save_config", lambda cfg: saved_configs.append(cfg.copy()))

        provider = HindsightMemoryProvider()
        provider.post_setup(str(haishui_home), {"memory": {}})
        ...
        profile_env = user_home / ".hindsight" / "profiles" / "haishui.env"
...
class TestToolHandlers:
    ...
    def test_retain_defaults_item_timestamp_when_no_occurred_at(self, provider, monkeypatch):
        ...
        monkeypatch.setattr("plugins.memory.hindsight._haishui_now", lambda: event_time)
        result = json.loads(provider.handle_tool_call(
            "hindsight_retain", {"content": "user likes dark mode"}
        ))
        assert result["result"] == "Memory stored successfully."
...
class TestSyncTurn:
    def test_sync_turn_retains_metadata_rich_turn(self, provider_with_config, monkeypatch):
        ...
        item = call_kwargs["items"][0]
        assert item["context"] == "conversation between Haishui Agent and the User"
        ...
        assert item["metadata"]["source"] == "haishui"
    ...
    def test_resume_creates_new_document(self, tmp_path, monkeypatch):
        ...
        config_path.parent.mkdir(parents=True, exist_ok=True)
        config_path.write_text(json.dumps(config))
        monkeypatch.setattr("plugins.memory.hindsight.get_haishui_home", lambda: tmp_path)

        p1 = HindsightMemoryProvider()
        p1.initialize(session_id="resumed-session", haishui_home=str(tmp_path), platform="cli")
        ...
        p2.initialize(session_id="resumed-session", haishui_home=str(tmp_path), platform="cli")
...
class TestRetainIndicator:
    ...
    def test_status_callback_wired_from_initialize(self, tmp_path, monkeypatch):
        ...
        p.initialize(session_id="s", haishui_home=str(tmp_path), status_callback=cb)
...
class TestBankIdTemplate:
    def test_sanitize_bank_segment_passthrough(self):
        assert _sanitize_bank_segment("haishui") == "haishui"
    ...
    def test_resolve_empty_template_uses_fallback(self):
        result = _resolve_bank_id_template(
            "", fallback="haishui", profile="coder"
        )
        assert result == "haishui"


    def test_resolve_sanitizes_placeholder_values(self):
        result = _resolve_bank_id_template(
            "user-{user}", fallback="haishui",
    ...
    def test_provider_uses_bank_id_template_from_config(self, tmp_path, monkeypatch):
        config = {
            ...
            "bank_id_template": "haishui-{profile}",
        }
        config_path = tmp_path / "hindsight" / "config.json"
        config_path.parent.mkdir(parents=True, exist_ok=True)
        config_path.write_text(json.dumps(config))
        ...
        p.initialize(
            ...
            agent_workspace="haishui",
        )
        assert p._bank_id == "haishui-coder"
        assert p._bank_id_template == "haishui-{profile}"
...
class TestAvailability:
    def test_available_with_api_key(self, tmp_path, monkeypatch):
        monkeypatch.setattr(
            "plugins.memory.hindsight.get_haishui_home",
...
def test_load_config_corrupt_profile_file_falls_through_to_env(tmp_path, monkeypatch):
    """A corrupt $HAISHUI_HOME/hindsight/config.json is not the config: the loader falls through
    ...
    assert cfg["banks"]["haishui"]["bankId"] == "from-env"
...
class TestPostSetupEnvEncoding:
    def _run_cloud_post_setup(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("haishui_cli.memory_setup._curses_select",
                            lambda *a, **kw: 0)  # cloud mode
        monkeypatch.setattr("haishui_cli.config.save_config", lambda c: None)
...
class TestClientAutoUpgradeRoutesThroughLazyDeps:
    ...
    def _init_with_outdated_client(self, tmp_path, monkeypatch, outcome):
        ...
        provider = HindsightMemoryProvider()
        provider.initialize(session_id="s", haishui_home=str(tmp_path), platform="cli")
        return calls
...
class TestMultiplexBackgroundScope:
    ...
    def scoped_embedded(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("plugins.memory.hindsight.get_haishui_home", lambda: home)
        home_tok = set_haishui_home_override(str(home))
        scope_tok = set_secret_scope(build_profile_secret_scope(home))
        yield created, home
        set_multiplex_active(False)
        reset_secret_scope(scope_tok)
        reset_haishui_home_override(home_tok)

    def test_writer_thread_resolves_profile_secret(self, scoped_embedded):
        created, home = scoped_embedded
        p = HindsightMemoryProvider()
        p._mode = "local_embedded"
        p._config = {"profile": "haishui", "llm_provider": "openai", "llm_model": "m"}
        p._ensure_writer()
    ...
    def test_daemon_start_thread_resolves_profile_secret(self, scoped_embedded):
        created, home = scoped_embedded
        p = HindsightMemoryProvider()
        p.initialize(session_id="s1", haishui_home=str(home), platform="cli")
