...
_CATALOG = {
    "templates": [
        {"id": "conversation", "name": "Conversation", "integrations": ["litellm", "haishui"],
        ...
        {"id": "haishui-gateway-bot", "name": "Gateway Bot", "integrations": ["haishui"],
         "manifest_file": "templates/haishui-gateway-bot.json"},
    ]
}


def test_fetch_haishui_templates_filters_to_haishui(monkeypatch):
    monkeypatch.setattr(tpl, "_get_json", lambda url: _CATALOG)
    entries = tpl.fetch_haishui_templates("https://example/templates.json")
    ids = [e["id"] for e in entries]
    assert ids == ["conversation", "haishui-gateway-bot"]  # coding-agent excluded
...
def test_fetch_manifest_resolves_relative_url(monkeypatch):
    ...
    assert seen["url"] == "https://raw.example/data/templates/haishui-gateway-bot.json"
...
def test_apply_template_posts_to_import_endpoint(monkeypatch):
    ...
    tpl.apply_template("https://api.hindsight.vectorize.io/", "haishui", "hsk_abc", {"version": "1"})

    assert captured["url"] == "https://api.hindsight.vectorize.io/v1/default/banks/haishui/import"
...
def test_apply_template_omits_auth_when_no_key(monkeypatch):
    ...
    monkeypatch.setattr(tpl, "open_credentialed_url", _fake_open)
    tpl.apply_template("http://localhost:8888", "haishui", None, {"version": "1"})
...
def test_run_template_step_applies_selected(monkeypatch):
    monkeypatch.setattr(tpl, "fetch_haishui_templates", lambda url=None: [
        {"id": "haishui-gateway-bot", "name": "Gateway Bot", "manifest_file": "templates/x.json"},
    ])
    ...
    result = tpl.run_template_step(
        api_url="https://api", bank_id="haishui", api_key="k",
        select=_select_returning(0), cancelled=-1, log=lambda *_: None,
    )
    assert result == "haishui-gateway-bot"
    assert applied["bank"] == "haishui"


def test_run_template_step_blank_selection_skips(monkeypatch):
    entries = [{"id": "haishui-gateway-bot", "name": "Gateway Bot", "manifest_file": "templates/x.json"}]
    monkeypatch.setattr(tpl, "fetch_haishui_templates", lambda url=None: entries)
...
def test_run_template_step_swallows_fetch_errors(monkeypatch):
    ...
    monkeypatch.setattr(tpl, "fetch_haishui_templates", _boom)
    # must not raise
...
def test_run_template_step_swallows_apply_errors(monkeypatch):
    ...
    logs = []
    result = tpl.run_template_step(
        api_url="https://api", bank_id="haishui", api_key=None,
...
def test_probe_existing_customization_true_when_bank_has_config(monkeypatch):
    ...
    assert tpl.probe_existing_customization("https://api", "haishui", "k") is True


def test_probe_existing_customization_false_when_empty(monkeypatch):
    monkeypatch.setattr(tpl, "open_credentialed_url",
                        _fake_open({"version": "1"}))
    assert tpl.probe_existing_customization("https://api", "haishui", "k") is False
