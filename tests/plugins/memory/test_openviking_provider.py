...
_EXPECTED_USER_AGENT = f"openviking-memory-haishui/{_HAISHUI_VERSION}"
...
def test_openviking_provider_config_loader_uses_readonly_config(monkeypatch):
    ...
    config = openviking_module._load_haishui_openviking_config()
...
""",
    ...
    settings = openviking_module._resolve_connection_settings(
        openviking_module._load_haishui_openviking_config()
    )
...
def test_post_setup_existing_profile_picker_validates_and_links_saved_profile(
    ...
    from haishui_cli import memory_setup
    ...
    OpenVikingMemoryProvider().post_setup(str(haishui_home), config)
...
def test_start_local_openviking_server_strips_pythonpath_from_child_env(monkeypatch):
    """The spawned server must not inherit Haishui's PYTHONPATH (#78153).

    Inheriting it makes openviking-server import packages from the Haishui
    venv instead of its own, and on Windows locks Haishui venv DLLs so the
    venv cannot be rebuilt during `haishui update`.
    """
    popen_calls = []
    ...
    monkeypatch.setattr(openviking_module.shutil, "which", lambda name: "/usr/local/bin/openviking-server")
    monkeypatch.setattr(openviking_module.subprocess, "Popen", fake_popen)
    monkeypatch.setenv("PYTHONPATH", "/opt/haishui/.venv/Lib/site-packages")
    monkeypatch.setenv("HAISHUI_PROFILE", "test-profile")
    ...
    assert child_env.get("HAISHUI_PROFILE") == "test-profile"
...
def test_https_local_endpoint_is_not_runtime_autostart_eligible(monkeypatch):
    ...
    assert warnings == [
        ...
        "OpenViking memory is temporarily unavailable; Haishui will retry on a later access or when "
        "the config changes. "
...
def test_runtime_does_not_autostart_when_local_server_reports_unhealthy(monkeypatch):
    ...
    assert warnings == [
        ...
        "OpenViking memory is temporarily unavailable; Haishui will retry on a later access "
...
def test_tool_add_resource_rejects_haishui_credential_file_upload(tmp_path, monkeypatch):
    import agent.file_safety as fs

    haishui_home = tmp_path / "haishui_home"
    haishui_home.mkdir()
    auth_json = haishui_home / "auth.json"
    auth_json.write_text('{"OPENROUTER_API_KEY":"sk-test-secret"}', encoding="utf-8")
    monkeypatch.setattr(fs, "_haishui_home_path", lambda: haishui_home)
...
def test_viking_client_delete_uses_identity_headers(monkeypatch):
    ...
    assert captured["kwargs"]["headers"]["X-OpenViking-Actor-Peer"] == "haishui"
...
# ---------------------------------------------------------------------------
# on_session_switch — flush + commit + rotate behavior (haishui-agent#28296)
# ---------------------------------------------------------------------------
...
def test_sync_turn_captures_session_id_before_worker_runs():
    ...
    assert captured_payloads == [{
        "messages": [
            ...
            {"role": "assistant", "parts": [{"type": "text", "text": "a"}], "peer_id": "haishui"},
        ]
    }]
...
def test_session_needs_commit_guard_wins_over_stale_turn_count():
    """Regression for haishui-agent#28296 review (M3): once a session is marked
...
def test_concurrent_providers_claim_unlocked_pending_owner_once(
    ...
    for provider in providers:
        ...
        provider._haishui_home = str(tmp_path)
...
def test_is_available_false_without_any_endpoint(monkeypatch):
    _clear_openviking_env(monkeypatch)
    monkeypatch.setattr(
        openviking_module, "_load_haishui_openviking_config", lambda: {}
    )
...
class TestOpenVikingEnvWriter:
    ...
    def test_bom_prefixed_env_updates_in_place(self, tmp_path):
        ...
        # The stale value must be gone, not left as a duplicate. Haishui and
