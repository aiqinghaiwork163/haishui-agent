"""Shared fixtures for the haishui-agent test suite.
...
2. **Isolated HAISHUI_HOME.** HAISHUI_HOME points to a per-test tempdir so
   code reading ``~/.haishui/*`` via ``get_haishui_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".haishui"`` instead of the canonical
   ``get_haishui_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No HAISHUI_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox HAISHUI_HOME before ANY test module is imported ──────────────────
# `haishui_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_haishui_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.haishui/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes HAISHUI_HOME, but fixtures run
...
# the REAL Haishui root — capture it BEFORE the sandbox rewires HAISHUI_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.haishui (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("HAISHUI_KANBAN_HOME", "").strip()
_PRE_SANDBOX_HAISHUI_HOME = os.environ.get("HAISHUI_HOME", "")


def _haishui_home_points_at_production(value: str) -> bool:
    """True when a pre-set HAISHUI_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    HAISHUI_HOME=~/.haishui``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``haishui_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.haishui`` — the escape vector that landed pytest
    ...
    HAISHUI_HOME is honored now.
    """
    if not value:
        return True
    try:
        # The platform-default root, not a hardcoded ``~/.haishui``: Windows installs live under
        # ``%LOCALAPPDATA%\haishui``, and a dev shell exporting that path used to be honored as
        # "custom", pinning import-time paths (``tui_gateway.server._haishui_home``) to the live
        # install so the state.db guard tripped on every store-touching test (#112692).
        from haishui_state_guard import _real_platform_state_root

        resolved = Path(value).expanduser().resolve()
        real_root = _real_platform_state_root() or (Path.home() / ".haishui").resolve()
    except Exception:
        return True
...
if _haishui_home_points_at_production(os.environ.get("HAISHUI_HOME", "")):
    _SESSION_HAISHUI_HOME = tempfile.mkdtemp(prefix="haishui-test-home-")
    os.environ["HAISHUI_HOME"] = _SESSION_HAISHUI_HOME
    atexit.register(shutil.rmtree, _SESSION_HAISHUI_HOME, True)
...
# like a real CLI") — which used to disarm haishui_state's live-DB guard in
# the child at the same moment the child lost the HAISHUI_HOME redirect.
# HAISHUI_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# haishui_state_guard._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export HAISHUI_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["HAISHUI_TEST_ISOLATION"] = os.environ.get("HAISHUI_HOME", "") or "1"

#: HAISHUI_HOME as it stood when conftest was imported - i.e. before any test
...
HAISHUI_HOME_AT_CONFTEST_IMPORT = os.environ.get("HAISHUI_HOME", "")
...
# HAISHUI_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_HAISHUI_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``haishui_cli.voice.speak_text`` — real synthesis,
    ...
    # any test run launched FROM a Haishui agent session inherits them and
    # haishui_constants home-resolution helpers prefer them over monkeypatched
    # HOME (test_subprocess_home_isolation red locally, green on CI).
    "HAISHUI_REAL_HOME",
    "TERMINAL_HOME_MODE",
    "HAISHUI_INTERACTIVE",
    "HAISHUI_QUIET",
    "HAISHUI_TOOL_PROGRESS",
    "HAISHUI_TOOL_PROGRESS_MODE",
    "HAISHUI_MAX_ITERATIONS",
    "HAISHUI_SESSION_PLATFORM",
    "HAISHUI_SESSION_CHAT_ID",
    "HAISHUI_SESSION_CHAT_NAME",
    "HAISHUI_SESSION_CHAT_TYPE",
    "HAISHUI_SESSION_THREAD_ID",
    "HAISHUI_SESSION_SOURCE",
    "HAISHUI_SESSION_KEY",
    "HAISHUI_GATEWAY_SESSION",
    "HAISHUI_CRON_SESSION",
    "_HAISHUI_GATEWAY",
    "HAISHUI_PLATFORM",
    "HAISHUI_MODEL",
    "HAISHUI_INFERENCE_MODEL",
    "HAISHUI_INFERENCE_PROVIDER",
    "HAISHUI_TUI_PROVIDER",
    "HAISHUI_MANAGED",
    "HAISHUI_MANAGED_DIR",
    "HAISHUI_DEV",
    "HAISHUI_CONTAINER",
    "HAISHUI_EPHEMERAL_SYSTEM_PROMPT",
    "HAISHUI_TIMEZONE",
    "HAISHUI_REDACT_SECRETS",
    "HAISHUI_BACKGROUND_NOTIFICATIONS",
    "HAISHUI_EXEC_ASK",
    "HAISHUI_HOME_MODE",
    "HAISHUI_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.haishui/kanban.db instead of the per-test HAISHUI_HOME.
    "HAISHUI_KANBAN_DB",
    "HAISHUI_KANBAN_BOARD",
    "HAISHUI_KANBAN_HOME",
    "HAISHUI_KANBAN_WORKSPACES_ROOT",
    "HAISHUI_KANBAN_LOGS_ROOT",
    "HAISHUI_KANBAN_TASK",
    "HAISHUI_KANBAN_WORKSPACE",
    "HAISHUI_KANBAN_RUN_ID",
    "HAISHUI_KANBAN_CLAIM_LOCK",
    "HAISHUI_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    "HAISHUI_DELEGATED_CHILD_CONTEXT",
    "HAISHUI_TENANT",
    ...
    # otherwise-unrelated config tests away from the default "haishui" host.
    "HAISHUI_HONCHO_HOST",
    ...
    "HAISHUI_DASHBOARD_OAUTH_CLIENT_ID",
    "HAISHUI_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and HAISHUI_HOME to per-test tempdirs so code that
    reads ``~/.haishui/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral HAISHUI_* vars that could change test semantics.
    for name in _HAISHUI_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("HAISHUI_HONCHO_HOST", "haishui")

    # 3. Redirect HAISHUI_HOME to a per-test tempdir. Code that reads
    #    ``~/.haishui/*`` via ``get_haishui_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.haishui/*`` via
    #    ``Path.home() / ".haishui"`` instead of ``get_haishui_home()``
    #    is a bug to fix at the callsite.
    fake_haishui_home = tmp_path / "haishui_test"
    fake_haishui_home.mkdir()
    (fake_haishui_home / "sessions").mkdir()
    (fake_haishui_home / "cron").mkdir()
    (fake_haishui_home / "memories").mkdir()
    (fake_haishui_home / "skills").mkdir()
    monkeypatch.setenv("HAISHUI_HOME", str(fake_haishui_home))
    ...
    # haishui_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("HAISHUI_TEST_ISOLATION", str(fake_haishui_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("HAISHUI_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. haishui_state computes ``DEFAULT_DB_PATH = get_haishui_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from haishui_state import ...``) that
    ...
    haishui_state_mod = sys.modules.get("haishui_state")
    if haishui_state_mod is not None and hasattr(haishui_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            haishui_state_mod, "DEFAULT_DB_PATH", fake_haishui_home / "state.db"
        )
    ...
    monkeypatch.setenv("HAISHUI_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.haishui/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import haishui_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path HAISHUI_HOME can't leak forward. Paths
...
@pytest.fixture(autouse=True)
def _isolate_haishui_home(_hermetic_environment):
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from haishui_cli import kanban_db_dispatch as _kbd_mod
    except Exception:
        return
...
def _neutralize_git_safe_directory_read(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_safe_directory"):
        return
    try:
        from haishui_cli import _subprocess_compat
    except Exception:
        return
...
def _close_leaked_session_dbs():
    ...
    Root cause of OOM incident 20260816: ~40 files under tests/haishui_cli/
    ...
    ``pytest tests/haishui_cli/`` accumulated 16-25 GB RSS and had to be
    OOM-killed three times in one day.

    Rather than editing every test file, ``SessionDB.__init__`` registers each
    instance in ``haishui_state_guard._test_instance_registry`` (a WeakSet,
    populated only when the ``HAISHUI_TEST_ISOLATION`` marker is set — i.e.
    ...
    Instances opened through ``haishui_state_registry.acquire()`` are skipped:
    ...
    yield
    try:
        from haishui_state_guard import _test_instance_registry as registry
    except Exception:
        return
    if not registry:
        return
...
# invocation), kanban writes silently pollute the real ~/.haishui. This autouse
...
# allow-list because test-level fixtures legitimately move HAISHUI_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session HAISHUI_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``HAISHUI_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) Haishui root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_HAISHUI_HOME and not _haishui_home_points_at_production(
        _PRE_SANDBOX_HAISHUI_HOME
    ):
        # HAISHUI_HOME was genuinely set to a CUSTOM root before the sandbox
        ...
        return get_default_haishui_root().resolve()
    # No pre-existing HAISHUI_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".haishui").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.haishui`` captured at import time. Hermetic tests that legitimately
    move HAISHUI_HOME to sibling tempdirs are unaffected.

    Only patches when ``haishui_cli.kanban_db_connect`` is *already imported*
    ...
    """
    _kdb = sys.modules.get("haishui_cli.kanban_db")
    _kdbc = sys.modules.get("haishui_cli.kanban_db_connect")
    if _kdb is None or _kdbc is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import haishui_cli.kanban_db`
    ...
    def _guarded_connect(db_path=None, *args, **kwargs):
        ...
        raise RuntimeError(
            ...
            f"to the real ~/.haishui. See #69283."
        )
...
# ``haishui_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL Haishui root. This fixture wires the
# test-side knobs:
...
#     installs where HAISHUI_HOME is not ~/.haishui) into the guard's
...
# so subprocess children that import haishui_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("haishui_state")
    ...
    if _PRE_SANDBOX_HAISHUI_HOME and not _haishui_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_HAISHUI_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Haishui home override redirects every later
    # ``get_haishui_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from haishui_constants import get_haishui_home_override, set_haishui_home_override

        if get_haishui_home_override() is not None:
            set_haishui_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal haishui config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``haishui-gateway`` process
...
# correlated with a ``tests/haishui_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> haishui-gateway``
...
def _wal_is_usable() -> bool:
    """True when Haishui will actually put a database into WAL mode here.

    Haishui refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Haishui can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Haishui managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``haishui_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_haishui_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_haishui_home`` fixture
    redirects ``HAISHUI_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.haishui/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``haishui_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["HAISHUI_VOICE_TTS"] = "1"``. The
...
#      ``haishui_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``haishui_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``haishui_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``haishui_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# Haishui runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.haishui``.
#
...
def _relocate_basetemp_outside_operator_home(config) -> None:
    """Move pytest's basetemp out of the operator's platform-native Haishui home.

    Every per-test sandbox is ``<basetemp>/.../haishui_test``. ``get_default_haishui_root()``
    prefers the platform-native home whenever ``HAISHUI_HOME`` sits *under* it, so a basetemp
    inside ``~/.haishui`` (or ``%LOCALAPPDATA%\\haishui``, where ``TEMP`` commonly lives on
    ...
    """
    from haishui_constants import _get_platform_default_haishui_home

    native = _get_platform_default_haishui_home().resolve()
    ...
    # Haishui home). The repo is no escape either: the default install checks it
    # out *inside* the home (~/.haishui/haishui-agent). A sibling of the native
    ...
    safe = Path(tempfile.mkdtemp(prefix="haishui-pytest-basetemp-", dir=safe_root))
    assert not safe.resolve().is_relative_to(native), (
        f"pytest basetemp {safe} still resolves inside the operator's Haishui home {native}; "
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Haishui falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Haishui uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart haishui-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting haishui/python patterns are also blocked.
    """
    ...
    def _matches_haishui_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _HAISHUI_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_haishui_gateway(cmd_str):
            return False
        try:
            tokens = _shlex.split(cmd_str)
        except ValueError:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch haishui-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m haishui_cli.main".
                if (
                    "haishui" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live haishui-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting haishui/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `haishui update` (or the
        # equivalent `python -m haishui_cli.main update`).  These commands
        ...
        if "update" in low and (
            # haishui update / haishui update --gateway / setsid bash -c ... haishui update
            ("haishui" in low and "update" in low.split())
            or
            # python -m haishui_cli.main update --gateway
            ("haishui_cli" in low and "update" in low.split())
            or
            # venv/bin/haishui update  (absolute path variant used in tests)
            (".venv/bin/haishui" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`haishui update` against the real checkout, fetching "
        ...
        # Block spawning a REAL gateway runtime (``python -m haishui_cli.main
        # gateway run|start|restart``). ``_spawn_haishui_action`` launches it
        # with start_new_session=True, so it outlives the pytest worker; the
        # child inherits the pytest-tmp HAISHUI_HOME, resolves the DEVELOPER's
        # ``haishui-gateway`` systemd unit (a tmp home hashes to no profile
        ...
        # A gateway launched INSIDE a container (`docker exec … haishui gateway start`) cannot
        ...
        if (
            ...
            raise RuntimeError(
                ...
                "haishui gateway runtime that outlives the test (it is "
                "detached), restarts the developer's live gateway, and "
                "holds the webhook port. Patch the spawn seam where "
                "production reads it (haishui_cli.web_server_gateway."
                "_spawn_haishui_action), or mark with "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_HAISHUI_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import haishui_cli.voice as _voice
    except Exception:
