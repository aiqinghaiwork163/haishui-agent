"""pytest's basetemp must never sit inside the operator's platform-native Haishui home.

Every per-test sandbox is ``<basetemp>/.../haishui_test`` and ``get_default_haishui_root()``
...
def test_basetemp_inside_the_native_home_is_relocated_outside_it(tmp_path, monkeypatch):
    ...
    native.mkdir()
    monkeypatch.setattr(haishui_constants, "_get_platform_default_haishui_home", lambda: native)
    ...
    monkeypatch.setenv("HAISHUI_HOME", str(relocated / "t0" / "haishui_test"))
    assert haishui_constants.get_default_haishui_root() == relocated / "t0" / "haishui_test"
...
def test_fallback_root_escapes_a_repo_checked_out_inside_the_native_home(tmp_path, monkeypatch):
    # Default install: repo at ~/.haishui/haishui-agent and TEMP under the home (Windows).
    native = tmp_path / "native-home"
    (native / "haishui-agent").mkdir(parents=True)
    monkeypatch.setattr(haishui_constants, "_get_platform_default_haishui_home", lambda: native)
    monkeypatch.setattr(suite_conftest, "PROJECT_ROOT", native / "haishui-agent")
