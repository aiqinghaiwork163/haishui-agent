"""Regression tests for aiqinghaiwork163/haishui-agent#2765.
...
service that didn't inherit ``~/.haishui/.env``) used to be dropped silently.
...
def test_warns_once_and_dedupes(caplog):
    ...
    assert "haishui memory status" in msg
