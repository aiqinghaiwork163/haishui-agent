...
def test_default_when_nothing_set(monkeypatch):
    ...
    monkeypatch.delenv("HAISHUI_LANGUAGE", raising=False)
...
def test_language_is_per_profile_under_multiplex(monkeypatch, tmp_path):
    """HAISHUI_LANGUAGE in the DEFAULT profile's environ must not leak into a secondary profile's
    ...
    monkeypatch.setenv("HAISHUI_LANGUAGE", "zh")  # default profile's .env, bridged into environ
    i18n.reset_language_cache()
...
def test_locales_dir_env_override_ignored_when_missing(tmp_path, monkeypatch):
    """A bogus HAISHUI_BUNDLED_LOCALES falls through to source/wheel resolution
    instead of returning a path that doesn't exist."""
    monkeypatch.setenv("HAISHUI_BUNDLED_LOCALES", str(tmp_path / "does-not-exist"))
