...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated HAISHUI_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="haishui_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".haishui"))
    prev_home = os.environ.get("HAISHUI_HOME")
    os.environ["HAISHUI_HOME"] = os.path.join(test_home, ".haishui")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("haishui_"):
            del sys.modules[mod]
    ...
    try:
        ...
        # references into haishui_cli.*/tools.* and their monkeypatches would otherwise
        ...
        if prev_home is None:
            ...
            os.environ["HAISHUI_HOME"] = prev_home
