"""``auth.adopt_external_logins: false`` keeps Haishui off the Claude Code login (#113023).

Claude Code's OAuth refresh token is single-use: once Haishui borrows and refreshes it, Haishui and
Claude Code hold one token family and whichever refreshes first logs the other out. With the opt-out
set, Haishui must neither read nor refresh ``~/.claude/.credentials.json``, must drop the pool row an
earlier adopting process persisted, and must say so in ``haishui auth list``.
"""
...
def test_opt_out_never_reads_or_refreshes_claude_code_login(tmp_path, monkeypatch):
    ...
    _write_config(haishui_home, adopt=None)
    ...
    _write_config(haishui_home, adopt=False)
    ...
    _write_config(haishui_home, adopt=True)
