...
Haishui preserves the callable end-to-end so the SDK refreshes tokens
...
class TestTruncateTokenCallable:
    def test_callable_returns_placeholder(self):
        ...
        ``"<function ...>"``."""
        from haishui_cli.web_server_oauth import _truncate_token
...
class TestCliEnsureRuntimeCredentialsCallable:
    ...
    ``HaishuiCLI`` instance) — the predicate change is the load-bearing
    ...
    def test_callable_predicate_present_in_cli_runtime_validation(self):
        ...
        src = (Path(__file__).resolve().parent.parent.parent
               / "haishui_cli" / "cli_agent_setup_mixin.py").read_text()
...
class TestInlinedDisplayMasks:
    ...
    def test_cli_show_config_handles_callable(self):
        """``cli.HaishuiCLI.show_config`` previously did
        ...
        assert "is_token_provider(display_key)" in src, (
            "cli.HaishuiCLI.show_config must guard the displayed key via "
            "is_token_provider so callable Entra ID providers don't "
            "crash /config."
        )
        assert '"Microsoft Entra ID"' in src, (
            "cli.HaishuiCLI.show_config must print the static "
