...
def _set_registry(monkeypatch, entries: List[Dict[str, Any]]) -> None:
    ...
    monkeypatch.setattr(
        "haishui_cli.plugins.get_plugin_auxiliary_tasks", lambda: list(entries)
    )


def _set_builtins(monkeypatch, keys: List[str]) -> None:
    monkeypatch.setattr(
        "haishui_cli.main_provider_setup._AUX_TASKS", [(k, k.title(), "") for k in keys]
    )
...
class TestOwnershipIntegration:
    ...
    def test_owner_stored_as_canonical_id(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "haishui_cli.plugins._ensure_plugins_discovered", lambda: manager
        )
        _set_builtins(monkeypatch, ["vision"])
...
""",
        ...
        monkeypatch.setattr("haishui_cli.plugins._ensure_plugins_discovered", lambda: manager)
        _set_builtins(monkeypatch, [])
