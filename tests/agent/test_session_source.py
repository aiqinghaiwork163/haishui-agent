...
def test_oneshot_run_gets_distinct_source(monkeypatch, inherited):
    """A finite `haishui chat -q` / `haishui -z` run is tagged `oneshot`, whether launched from a plain shell
    or spawned inside a TUI/Desktop session (whose transport label it inherits but is not) (#112550)."""
    monkeypatch.setenv("HAISHUI_SESSION_SOURCE", inherited)
...
def test_oneshot_keeps_explicit_source_flag(monkeypatch, explicit):
    """`haishui chat -q --source tui` is a documented flag, not an inherited transport label: main.py
    marks it HAISHUI_SESSION_SOURCE_EXPLICIT=1 and the one-shot drop must not override it."""
    monkeypatch.setenv("HAISHUI_SESSION_SOURCE", explicit)
    monkeypatch.setenv("HAISHUI_SESSION_SOURCE_EXPLICIT", "1")
    monkeypatch.setenv("HAISHUI_SINGLE_QUERY_SESSION", "1")
