...
def test_aiagent_reuses_existing_errors_log_handler():
    ...
    error_log_path = (run_agent._haishui_home / "logs" / "errors.log").resolve()
...
class TestInit:
    ...
    def test_prompt_caching_cache_ttl_defaults_without_config(self):
        ...
        with (
            ...
            patch("model_tools.check_toolset_requirements", return_value={}),
            patch("agent.process_bootstrap.OpenAI"),
            patch("haishui_cli.config.load_config", return_value={}), patch("haishui_cli.config.load_config_readonly", return_value={}),
        ):
            a = AIAgent(
                api_key="test-k...7890",
                model="anthropic/claude-sonnet-4-20250514",
...
class TestBuildApiKwargs:
    ...
    def test_core_responses_preserves_supported_xhigh(self, agent, monkeypatch):
        ...
        monkeypatch.setattr(
            "haishui_cli.models.github_model_reasoning_efforts",
...
class TestExecuteToolCalls:
    ...
    def test_keyboard_interrupt_emits_cancelled_post_tool_hook(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("haishui_cli.lifecycle.invoke_hook", _capture_hook)
        monkeypatch.setattr("haishui_cli.lifecycle.has_hook", lambda name: True)
    ...
    def test_interrupt_skips_remaining(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("haishui_cli.lifecycle.has_hook", lambda name: True)
        monkeypatch.setattr(
            "haishui_cli.lifecycle.invoke_hook",
...
class TestConcurrentToolExecution:
    ...
    def test_sequential_registry_tool_forwards_request_middleware_trace(
        ...
        from haishui_cli.middleware import RequestMiddlewareResult
        ...
        mock_msg = _mock_assistant_msg(content="", tool_calls=[tool_call])
        monkeypatch.setattr(
            "haishui_cli.middleware.apply_tool_request_middleware",
        ...
        monkeypatch.setattr(
            "haishui_cli.middleware.run_tool_execution_middleware",
            lambda _name, args, callback, **_kwargs: callback(args),
        )
        monkeypatch.setattr(
            "haishui_cli.plugins._dispatch_pre_tool_call_hooks",
    ...
    def test_managed_tool_pipeline_rejects_second_dispatch(self, agent, monkeypatch):
        ...
        assert duplicate_errors == [
            "Haishui tool execution callback invoked more than once"
        ]
    ...
    def test_managed_tool_pipeline_allows_one_concurrent_dispatch(
        ...
        assert outcome.result == "ok"
        assert dispatched == [{"command": "true"}]
        assert errors == ["Haishui tool execution callback invoked more than once"]
        assert outcome.blocked is False
...
class TestHandleMaxIterations:
    ...
    def test_api_sanitizer_repairs_tool_call_with_empty_function_name(self, agent):
        ...
        call, because haishui' dispatch loop keeps empty-name calls paired with
...
class TestRunConversation:
    ...
    def test_task_start_failure_closes_relay_turn_and_lease(self, agent):
        ...
        with (
            ...
            patch(
                "haishui_cli.observability.relay_shared_metrics.start_task_run",
                side_effect=start_error,
            ),
            patch(
                "haishui_cli.observability.relay_shared_metrics.finish_task_run"
            ) as finish_task_run,
    ...
    def test_ollama_small_runtime_context_fails_before_api_call(self, agent, caplog):
        ...
        assert "Ollama runtime context too small for Haishui tool use" in caplog.text
    ...
    def test_request_scoped_api_hooks_fire_for_each_api_call(self, agent):
        ...
        with (
            ...
            patch(
                "haishui_cli.lifecycle.has_hook",
                side_effect=lambda name: name in {"pre_api_request", "post_api_request"},
            ),
            patch("haishui_cli.lifecycle.invoke_hook", side_effect=_record_hook),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_api_request_error_hook_skips_payload_work_without_listener(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("haishui_cli.lifecycle.has_hook", lambda name: False)
        monkeypatch.setattr("haishui_cli.lifecycle.invoke_hook", _invoke_hook)
    ...
    def test_request_scoped_api_hooks_skip_payload_work_without_listeners(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("haishui_cli.lifecycle.has_hook", _has_hook)
        ...
        with (
            patch("haishui_cli.lifecycle.invoke_hook", return_value=[]),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_engine_preflight_fires_below_threshold(self, agent):
        ...
        should_compress_preflight() hook so engines like haishui-lcm can
    ...
    def test_stub_stall_mid_tool_call_recovers_within_3_retries(self, agent):
        ...
        from haishui_constants import PARTIAL_STREAM_STUB_ID
    ...
    def test_kanban_block_called_on_iteration_exhaustion(self, agent, monkeypatch):
        ...
        monkeypatch.setenv("HAISHUI_KANBAN_TASK", "t_test_task_123")
        ...
        with (
            ...
            patch("haishui_cli.kanban_db_dispatch._record_task_failure",
                  mock_record_failure),
            patch("haishui_cli.kanban_db_connect.connect", mock_connect),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_no_kanban_block_when_not_in_kanban_mode(self, agent, monkeypatch):
        """The exhaustion bridge must NOT fire when HAISHUI_KANBAN_TASK
...
class TestNousCredentialRefresh:
    ...
    def test_try_refresh_nous_client_credentials_rebuilds_client(
        ...
        monkeypatch.setattr(
            "haishui_cli.auth.resolve_nous_runtime_credentials", _fake_resolve
        )
...
class TestSystemPromptStability:
    ...
    def test_fresh_build_when_no_history(self, agent):
        ...
        assert "Haishui Agent" in agent._cached_system_prompt
