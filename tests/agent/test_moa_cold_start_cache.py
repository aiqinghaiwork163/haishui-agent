...
def test_preset_resolution_is_cached_across_create_calls(monkeypatch, tmp_path):
    ...
    calls = {"n": 0}
    import haishui_cli.moa_config as moa_cfg_mod
...
def test_slot_runtime_is_cached_across_create_calls(monkeypatch, tmp_path):
    ...
    import haishui_cli.runtime_provider as rt_mod
...
def test_slot_runtime_cache_expires_after_ttl(monkeypatch):
    ...
    key = (haishui_home_key(), "openai", "gpt-5")
...
def test_slot_runtime_cache_is_scoped_per_profile_home(monkeypatch, tmp_path):
    ...
    accounts; a cached api_key/base_url must never cross the per-turn HAISHUI_HOME override."""
    import agent.moa_loop as moa
    ...
    monkeypatch.setattr(
        ...
        lambda **kw: {"provider": "openai", "model": "gpt-5",
                      "api_key": f"key-{haishui_constants.get_haishui_home().name}",
                      "base_url": "https://x", "api_mode": None})

    slot = {"provider": "openai", "model": "gpt-5"}
    tok = haishui_constants.set_haishui_home_override(str(a))
    try:
        assert moa._slot_runtime(slot)["api_key"] == "key-a"
    finally:
        haishui_constants.reset_haishui_home_override(tok)
    tok = haishui_constants.set_haishui_home_override(str(b))
    try:
