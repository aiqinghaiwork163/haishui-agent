...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate HAISHUI_HOME under tmp_path.

    ``HAISHUI_ALLOW_PRIVATE_URLS`` opts the loopback test server into private-IP
    ...
    monkeypatch.setenv("HAISHUI_ALLOW_PRIVATE_URLS", "1")
    from tools import url_safety
    url_safety._reset_allow_private_cache()
    (tmp_path / ".haishui").mkdir()

    # Force the constants/image cache helpers to re-read HAISHUI_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("haishui_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_haishui_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under HAISHUI_HOME — gateway cleanup
