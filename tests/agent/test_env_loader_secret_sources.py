"""Tests for the secret-source tracking in ``haishui_cli.env_loader``.

These cover the small public surface that lets `haishui model` / `haishui setup`
...
from haishui_cli import env_loader  # noqa: E402
...
def test_single_profile_scoped_load_keeps_override_behavior(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("HAISHUI_TEST_SHARED_ADAPTER_CONFIG", raising=False)
    other_home = tmp_path / "other"
    other_home.mkdir()
    (other_home / ".env").write_text("HAISHUI_TEST_SHARED_ADAPTER_CONFIG=second\n")

    was_active = secret_scope.is_multiplex_active()
    secret_scope.set_multiplex_active(False)
    home_token = set_haishui_home_override(other_home)
    try:
        loaded = env_loader.load_haishui_dotenv(haishui_home=other_home)
    finally:
        secret_scope.set_multiplex_active(was_active)
        reset_haishui_home_override(home_token)

    try:
        assert os.environ.get("HAISHUI_TEST_SHARED_ADAPTER_CONFIG") == "second"
        assert (other_home / ".env") in loaded
    finally:
        os.environ.pop("HAISHUI_TEST_SHARED_ADAPTER_CONFIG", None)
...
def test_multiplex_dotenv_load_hydrates_sources_without_global_env(
    ...
    was_active = secret_scope.is_multiplex_active()
    home_token = set_haishui_home_override(tmp_path)
    secret_scope.set_multiplex_active(True)
    try:
        assert env_loader.load_haishui_dotenv(haishui_home=tmp_path) == []
    finally:
        secret_scope.set_multiplex_active(was_active)
        reset_haishui_home_override(home_token)
...
def test_cold_profile_hydration_seeds_op_env_bootstrap(tmp_path, monkeypatch):
    ...
    load_haishui_dotenv() reads <home>/.op.env for OP_SERVICE_ACCOUNT_TOKEN
...
def test_apply_external_secret_sources_dedupes_within_process(tmp_path, monkeypatch):
    """``load_haishui_dotenv()`` is called at module-import time from several
    hot modules (cli.py, haishui_cli/main.py, run_agent.py, ...).  The
    ...
    runs exactly once per HAISHUI_HOME per process.
    """
    ...
    # cli.py, haishui_cli/main.py, run_agent.py, trajectory_compressor.py,
...
def test_env_shadowed_reapply_keeps_home_snapshot(tmp_path, monkeypatch, _fresh_registry):
    ...
    env_loader.load_haishui_dotenv(haishui_home=home)
