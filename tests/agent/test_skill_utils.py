...
class TestParseConfigStringList:
    """#86661: `haishui config set` and JSON-mode editor saves store lists as
    ...
    def test_python_literal_array_string_parses(self):
        # `haishui config set` can persist single-quoted Python-literal forms.
...
def test_skill_config_home_vars_use_subprocess_home(tmp_path, monkeypatch):
    ...
    haishui_home = tmp_path / "da\\ta"
    subprocess_home = haishui_home / "home"
    subprocess_home.mkdir(parents=True)
    (haishui_home / "config.yaml").write_text("", encoding="utf-8")
    monkeypatch.setenv("HAISHUI_HOME", str(haishui_home))
    monkeypatch.setenv("HOME", str(haishui_home))
