...
def test_kanban_guidance_requires_worker_task_at_agent_init(monkeypatch, task_id, expected):
    ...
    if task_id is None:
        ...
        monkeypatch.setenv("HAISHUI_KANBAN_TASK", task_id)
    monkeypatch.setattr("haishui_cli.plugins.discover_plugins", lambda: None)
    monkeypatch.setattr(
        model_tools,
...
def test_kanban_guidance_fallback_requires_owned_worker_task(monkeypatch, task_id, owner, expected):
    ...
    is not identity, and an inherited HAISHUI_KANBAN_TASK is not ownership (#112486)."""
...
class TestNamedProfileHintIntegration:
    ...
    ``TestNamedProfileHint`` mocks ``get_haishui_home``,
    ``get_default_haishui_root`` and ``_resolve_active_profile_name``, so it
    ...
    ``HAISHUI_HOME`` and no resolver mocks.
    """

    def test_real_haishui_home_under_profiles_renders_correct_paths(
        ...
        assert get_default_haishui_root() == root

        agent = _make_agent(valid_tool_names=["read_file"])
        with patch("agent.coding_context._coding_mode", return_value="off"):
            prompt = "\n\n".join(_prompt_parts(agent).values())

        assert "Active Haishui profile: coder." in prompt
    ...
    def test_real_default_home_renders_default_branch(self, tmp_path, monkeypatch):
        """HAISHUI_HOME at the root resolves to the default profile, unchanged."""
        root = tmp_path / ".haishui"
        ...
        assert "Active Haishui profile: default." in prompt
...
def test_coding_prompt_orders_shared_context_before_workspace(monkeypatch):
    ...
    monkeypatch.setattr(system_prompt, "HAISHUI_AGENT_HELP_GUIDANCE", "HELP")
    monkeypatch.setattr(system_prompt, "HAISHUI_AGENT_HELP_GUIDANCE_NO_SKILLS", "HELP")
    monkeypatch.setattr(system_prompt, "STEER_CHANNEL_NOTE", "STEER")
    monkeypatch.setattr(system_prompt, "get_haishui_home", lambda: Path("/haishui"))

    # Production renders this as str(get_haishui_home()) + "/profiles/<name>/",
    # and str(Path("/haishui")) is platform-dependent (backslash on Windows) —
    # build the expectation the same way instead of hardcoding "/haishui".
    _home_str = str(Path("/haishui"))
    expected_profile = (
    ...
    with (
        ...
        patch("haishui_time.now", return_value=datetime(2026, 1, 2)),
    ):
...
class TestTelegramRichMessagesHint:
    ...
    def test_base_hint_without_rich_messages(self, monkeypatch):
        ...
        with patch("haishui_cli.config.load_config_readonly") as mock_cfg:
    ...
    def test_gateway_rich_messages_integration_via_real_config(self, tmp_path, monkeypatch):
        ...
        under HAISHUI_HOME with ``gateway.platforms.telegram.extra.rich_messages``
        ...
        from haishui_cli import config as _cfgmod
...
def test_conversation_start_uses_session_start_not_build_time(monkeypatch):
    ...
    with (
        ...
        patch("haishui_time.now", return_value=datetime(2026, 1, 2, 9, 0)),
    ):
        prompt = build_system_prompt(agent, system_message="SYSTEM_MESSAGE")
...
class TestConversationStartedTwoLine:
    ...
    def test_same_day_session_keeps_single_line(self):
        from haishui_time import now as haishui_now
        sid = haishui_now().strftime("%Y%m%d_%H%M%S_fresh")
