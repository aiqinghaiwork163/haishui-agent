...
binary download are exercised manually by `haishui secrets bitwarden
...
def haishui_home(tmp_path, monkeypatch):
    """Point Haishui at an isolated home directory."""
    home = tmp_path / ".haishui"
    home.mkdir()
    monkeypatch.setenv("HAISHUI_HOME", str(home))
    # Some modules cache get_haishui_home; clear if needed.
    import haishui_constants
    if hasattr(haishui_constants, "_HAISHUI_HOME_CACHE"):
        haishui_constants._HAISHUI_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(haishui_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from haishui_cli.env_loader import _apply_external_secret_sources
