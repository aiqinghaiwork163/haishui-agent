...
def isolated_home(monkeypatch):
    """Temp HAISHUI_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="haishui_test_31179_")
    haishui_home = os.path.join(test_home, ".haishui")
...
def _fresh_modules():
    """Drop cached haishui modules so each test reloads against current env."""
    _drop_reload_targets()
