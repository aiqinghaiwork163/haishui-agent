...
def _ensure_redaction_enabled(monkeypatch):
    """Ensure HAISHUI_REDACT_SECRETS is not disabled by prior test imports."""
...
class TestTerminalOutputRedaction:
    ...
    def test_command_reads_secret_file_detection(self):
        ...
        # With paths
        assert _command_reads_secret_file("cat ~/.haishui/.env")
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("cat ~/.haishui/config.yaml", "api_key: haishuiConfigSecret123", "haishuiConfigSecret123"),
            (
                "head ~/.haishui/profiles/work/config.yaml",
            ...
            (
                "grep 'foo|bar' ~/.haishui/config.yaml",
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "cat ~/.haishui/config.example.yaml",
            "cat ~/.haishui/config.template.yaml",
            "cat ~/.bashrc.example",
...
class TestHaishuiHomePathClassification:
    """``_is_secret_file_arg`` must see the RESOLVED Haishui home: a managed Windows home
    (``%LOCALAPPDATA%\\haishui``) has no ``.haishui`` segment and a resolved path never spells
    ``$HAISHUI_HOME``, so the literal test alone classified its ``config.yaml`` as ordinary YAML."""
    ...
    def test_resolved_home_config_is_secret_bearing_but_project_config_is_not(self, tmp_path, monkeypatch):
        ...
        home = tmp_path / "haishui"  # no ".haishui" segment
