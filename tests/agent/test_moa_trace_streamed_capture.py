...
path via ``haishui chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp HAISHUI_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point HAISHUI_HOME at a temp dir and turn moa.save_traces on."""
    haishui_home = tmp_path / ".haishui"
    haishui_home.mkdir()
    monkeypatch.setenv("HAISHUI_HOME", str(haishui_home))

    # save_moa_turn reads config via haishui_cli.config.load_config; stub it to
    ...
    return haishui_home / "moa-traces"
