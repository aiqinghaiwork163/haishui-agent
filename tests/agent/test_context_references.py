...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "Haishui Tests")
...
def test_binary_reference_block_maps_host_attachment_to_container_path(tmp_path: Path, monkeypatch):
    ...
    attachments = haishui_home / "attachments"
    ...
    assert result.expanded
    # Default container base for the docker backend is /root/.haishui.
    assert "/root/.haishui/attachments/archive.zip" in result.message
...
def test_oversized_text_reference_maps_host_attachment_to_container_path(
    ...
    assert "/root/.haishui/attachments/large.txt" in attached_context
    assert "too large to inline safely" in result.message
...
def test_binary_reference_block_keeps_host_path_on_local_backend(tmp_path: Path, monkeypatch):
    ...
    assert result.expanded
    assert str(payload) in result.message
    assert "/root/.haishui/attachments/" not in result.message
...
async def test_blocks_canonical_read_denylist_credential_stores(tmp_path: Path, monkeypatch):
    ...
    a chat peer could otherwise attach `@file:~/.haishui/auth.json` and read the
    ...
    oauth = haishui_home / ".anthropic_oauth.json"
    oauth.write_text('{"access_token": "OAUTH-SECRET"}\n', encoding="utf-8")

    mcp_token = haishui_home / "mcp-tokens" / "github.json"
    ...
    result = await preprocess_context_references_async(
        "inspect @file:.haishui/auth.json and @file:.haishui/.anthropic_oauth.json "
        "and @file:.haishui/mcp-tokens/github.json and @file:project/.env",
        cwd=tmp_path,
...
async def test_canonical_guard_fails_closed_when_lookup_raises(tmp_path: Path, monkeypatch):
    ...
    result = await preprocess_context_references_async(
        "inspect @file:.haishui/auth.json",
        cwd=tmp_path,
        allowed_root=tmp_path,
        context_length=100_000,
    )
...
@pytest.mark.parametrize(
    ...
    [
        "/tmp/plain.png",
        "/Users/me/Library/Application Support/Haishui/composer-images/a.png",
...
async def test_side_thread_expansion_guards_the_served_profile_home(tmp_path: Path, monkeypatch):
    ...
    token = set_haishui_home_override(served_home)
    try:
        result = preprocess_context_references(
    ...
    assert any("internal Haishui path" in w for w in result.warnings)
