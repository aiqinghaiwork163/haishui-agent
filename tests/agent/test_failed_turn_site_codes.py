...
def test_persistence_failure_default_copy_is_actionable_and_profile_aware(monkeypatch):
    monkeypatch.setenv("HAISHUI_HOME", "/srv/haishui-profile")
    text = TurnExplainersMixin._format_turn_completion_explanation("session_persistence_failed", "replaced")
    assert "haishui gateway stop" in text and "haishui doctor" in text
    assert "~/.haishui" not in text and "/srv/haishui-profile" in text
    assert "manifest" not in text  # the runbook stays in logger.error at haishui_state
