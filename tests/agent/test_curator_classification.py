...
def test_reconcile_absorbed_into_beats_everything_else(curator_env):
    ...
    out = curator_env._reconcile_classification(
        ...
        destinations={"haishui-agent-dev"},
        absorbed_declarations={
            "pr-review-format": {"into": "haishui-agent-dev", "declared": True},
        },
    )
    ...
    assert e["into"] == "haishui-agent-dev"
...
# `haishui curator status`). The full data has always been in REPORT.md on
...
# ---------------------------------------------------------------------------
# Pin hint — surfaces `haishui curator pin <umbrella>` in the rename block so
