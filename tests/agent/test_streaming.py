...
class TestStreamingAccumulator:
    ...
    def test_endpoint_rejecting_stream_options_is_retried_without_it(self, mock_close, mock_create, monkeypatch):
        ...
        the transient-retry budget: with HAISHUI_STREAM_RETRIES=0 it still happens."""
        from openai import APIStatusError
        from run_agent import AIAgent
        monkeypatch.setenv("HAISHUI_STREAM_RETRIES", "0")
...
class TestStreamingFallback:
    ...
    def test_sse_connection_lost_retried_as_transient(self, mock_close, mock_create):
        ...
        # Should retry 3 times (default HAISHUI_STREAM_RETRIES=2 → 3 attempts)
...
class TestAnthropicStreamCallbacks:
    ...
    def test_anthropic_stream_parser_valueerror_retries_before_delivery(
        ...
        agent.api_mode = "anthropic_messages"
        agent._interrupt_requested = False
        monkeypatch.setenv("HAISHUI_STREAM_RETRIES", "1")
...
class TestPartialToolCallWarning:
    ...
    def test_partial_tool_call_surfaces_warning(self, mock_close, mock_create):
        ...
        import os as _os
        _prev = _os.environ.get("HAISHUI_STREAM_RETRIES")
        _os.environ["HAISHUI_STREAM_RETRIES"] = "0"
        try:
            response = agent._interruptible_streaming_api_call({})
        finally:
            if _prev is None:
                _os.environ.pop("HAISHUI_STREAM_RETRIES", None)
            else:
                _os.environ["HAISHUI_STREAM_RETRIES"] = _prev
    ...
    def test_zero_char_partial_stream_retries_instead_of_empty_stub(self, mock_close, mock_create):
        ...
        with patch.dict("os.environ", {"HAISHUI_STREAM_RETRIES": "1"}):
            response = agent._interruptible_streaming_api_call({})
    ...
    def test_zero_char_partial_stream_exhausted_raises_to_main_loop(self, mock_close, mock_create):
        ...
        with patch.dict("os.environ", {"HAISHUI_STREAM_RETRIES": "0"}), pytest.raises(httpx.RemoteProtocolError):
            agent._interruptible_streaming_api_call({})
        assert calls["n"] == 1
...
class TestSilentRetryMidToolCall:
    ...
    def test_silent_retry_recovers_tool_call(
        ...
        import os as _os
        _prev = _os.environ.get("HAISHUI_STREAM_RETRIES")
        _os.environ["HAISHUI_STREAM_RETRIES"] = "2"
        try:
            response = agent._interruptible_streaming_api_call({})
        finally:
            if _prev is None:
    ...
    def test_silent_retry_exhausted_falls_back_to_stub(
        ...
        import os as _os
        _prev = _os.environ.get("HAISHUI_STREAM_RETRIES")
        _os.environ["HAISHUI_STREAM_RETRIES"] = "1"
        try:
            response = agent._interruptible_streaming_api_call({})
        finally:
            if _prev is None:
...
class TestBedrockStreamLivenessWatchdog:
    ...
    def test_stalled_stream_bumps_streak_and_aborts(self, monkeypatch):
        ...
        monkeypatch.setenv("HAISHUI_STREAM_STALE_TIMEOUT", "0.5")
        monkeypatch.setenv("HAISHUI_STREAM_STALE_GIVEUP", "5")
    ...
    def test_successful_stream_resets_streak(self, monkeypatch):
        ...
        monkeypatch.setenv("HAISHUI_STREAM_STALE_TIMEOUT", "60")
