...
normal ``haishui chat -q`` CLI agent (its default toolset includes ``cronjob``)
running with ``HAISHUI_KANBAN_TASK`` legitimately set in its own environment,
...
``$HAISHUI_KANBAN_TASK`` — letting an unrelated cron job close the worker's task
...
def worker_env(monkeypatch):
    ...
    monkeypatch.setenv("HAISHUI_KANBAN_TASK", "t_worker_real_task")
    monkeypatch.setenv("HAISHUI_KANBAN_WORKSPACE", "/tmp/ws")
    monkeypatch.setenv("HAISHUI_KANBAN_RUN_ID", "42")
    monkeypatch.setenv("HAISHUI_KANBAN_CLAIM_LOCK", "lock-abc")
    monkeypatch.setenv("HAISHUI_KANBAN_BOARD", "team-alpha")
...
class TestRunJobKanbanIsolation:
        ...
        class FakeAgent:
            def __init__(self, **kwargs):
                ...
                observed["kanban_env_during_init"] = {
                    ...
                    if k.startswith("HAISHUI_KANBAN_")
                }
    ...
    def test_environment_is_left_untouched(self, monkeypatch, worker_env):
        ...
        before = {
            k: v for k, v in os.environ.items() if k.startswith("HAISHUI_KANBAN_")
        }
    ...
    def test_context_reset_even_when_job_raises(self, monkeypatch, worker_env):
        ...
        assert os.environ.get("HAISHUI_KANBAN_BOARD") == "team-alpha"
...
def test_dispatcher_grants_only_the_assigned_worker_scope(tmp_path, monkeypatch):
    ...
    from haishui_cli.kanban_db_dispatch import _default_spawn

    monkeypatch.setenv("HOME", str(tmp_path))
    db = tmp_path / "board.db"
    monkeypatch.setenv("HAISHUI_KANBAN_DB", str(db))
    conn = connect(db)
    ...
    worker.chmod(0o700)
    monkeypatch.setenv("HAISHUI_BIN", str(worker))
    # Building a new worker under an existing task must replace, not inherit, its scope.
    monkeypatch.setenv("HAISHUI_KANBAN_TASK", "prior-task")
    ...
    monkeypatch.setenv("HAISHUI_DELEGATED_CHILD_CONTEXT", str(tmp_path))
    ...
    assert os.environ["HAISHUI_KANBAN_TASK"] == "prior-task"
    conn.close()
