...
def test_cli_keeps_discovered_home_when_launch_selection_changes(tmp_path, monkeypatch, profile):
    ...
    token = set_haishui_home_override(str(root))
    ...
    def run(argv, env, report_path, timeout):
        ...
        code = ('import json,sys; sys.argv=["haishui"]+json.loads(sys.argv[1]); '
                'import haishui_cli.main; from haishui_constants import get_haishui_home; '
                'print(json.dumps(str(get_haishui_home())))')
        # Everything after the launcher (binary or ``python -m haishui_cli.main``) is the CLI argv.
        cli_argv = argv[3:] if argv[1:3] == ["-m", "haishui_cli.main"] else argv[1:]
    ...
    try:
        ...
        assert seen == [home]
        assert not (tmp_path / ".haishui").exists()
    finally:
