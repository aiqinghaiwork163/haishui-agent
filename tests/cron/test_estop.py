"""Global emergency stop (`haishui pause` / `haishui resume`) — agent/estop.py.
...
already in flight is never touched. Removing the sentinel (`haishui resume`)
...
def haishui_home(tmp_path, monkeypatch):
    """Point HAISHUI_HOME at a temp dir and reset estop module log state."""
...
def test_engage_creates_sentinel_and_is_engaged(haishui_home):
    assert estop.is_engaged() is False
    estop.engage()
    assert (haishui_home / "ESTOP").exists()
    assert estop.is_engaged() is True


def test_disengage_removes_sentinel(haishui_home):
    estop.engage()
    assert estop.disengage() is True
    assert not (haishui_home / "ESTOP").exists()
    assert estop.is_engaged() is False
    # Disengaging when not engaged is a no-op that reports False.
    assert estop.disengage() is False


def test_reason_and_timestamp_stored(haishui_home):
    ...
    raw = json.loads((haishui_home / "ESTOP").read_text(encoding="utf-8"))
    assert raw["reason"] == "runaway cron fan-out"


def test_get_state_none_when_disengaged(haishui_home):
    assert estop.get_state() is None


def test_corrupt_sentinel_still_engages(haishui_home):
    """A hand-touched/corrupt ESTOP file must still pause (fail safe)."""
    (haishui_home / "ESTOP").write_text("not json", encoding="utf-8")
    assert estop.is_engaged() is True
    state = estop.get_state()
    assert state is not None
...
def test_paused_reply_none_when_disengaged(haishui_home):
    assert estop.paused_reply() is None


def test_paused_reply_surfaces_reason_and_resume_hint(haishui_home):
    ...
    assert "haishui resume" in notice


def test_paused_reply_without_reason(haishui_home):
    estop.engage()
    notice = estop.paused_reply()
    assert notice is not None
    assert "paused" in notice.lower()
...
def test_check_paused_logs_once_per_engagement(haishui_home, caplog):
...
def test_cron_tick_skips_dispatch_when_engaged(haishui_home, monkeypatch):
...
def test_cron_tick_resumes_after_disengage(haishui_home, monkeypatch):
    from cron import scheduler
...
def test_kanban_dispatch_blocked_when_engaged(haishui_home):
...
@pytest.mark.asyncio
async def test_gateway_new_turn_gets_paused_reply(haishui_home):
...
@pytest.mark.asyncio
async def test_gateway_internal_events_bypass_estop(haishui_home):
...
# ── CLI: haishui pause / haishui resume ───────────────────────────────────────


def test_cli_pause_engages_with_reason(haishui_home, capsys):
    from haishui_cli.subcommands.pause import cmd_pause
...
def test_cli_pause_idempotent(haishui_home, capsys):
    from haishui_cli.subcommands.pause import cmd_pause
...
def test_cli_resume_disengages(haishui_home, capsys):
    from haishui_cli.subcommands.pause import cmd_pause, cmd_resume
...
def test_cli_resume_when_not_paused(haishui_home, capsys):
    from haishui_cli.subcommands.pause import cmd_resume
...
def test_builtin_subcommands_include_pause_resume():
    from haishui_cli.main import _BUILTIN_SUBCOMMANDS

    assert "pause" in _BUILTIN_SUBCOMMANDS
    assert "resume" in _BUILTIN_SUBCOMMANDS


# ── haishui status surfacing ─────────────────────────────────────────────────


def test_status_line_when_paused(haishui_home):
    from haishui_cli.status import _estop_status_line
...
def test_is_engaged_fails_safe_on_stat_error(haishui_home, monkeypatch):
    """A stat failure must report ENGAGED (fail safe) — the pause has to
    hold even when HAISHUI_HOME is misbehaving, matching the module's
...
@pytest.mark.asyncio
async def test_gateway_slash_commands_bypass_estop(haishui_home):
    ...
    try:
        ...
        return
    assert reply is None or "haishui is paused" not in (reply or "").lower()
...
@pytest.mark.asyncio
async def test_gateway_pause_command_engages_and_resumes(haishui_home):
    from gateway.run import GatewayRunner
...
def test_pause_command_registered_for_gateway():
    from haishui_cli.commands import GATEWAY_KNOWN_COMMANDS, resolve_command
...
def test_profile_gateway_honors_canonical_root_estop(tmp_path, monkeypatch):
    """fleet-analyst-class: HAISHUI_HOME is a profile dir; pause lives at root.

    A process launched with HAISHUI_HOME=~/.haishui/profiles/fleet-analyst must
    still treat ~/.haishui/ESTOP as engaged. Otherwise `haishui pause` is not
