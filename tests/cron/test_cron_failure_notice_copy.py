"""User-facing cron failure notices: plain words, the real output path, and the exact `haishui cron`
...
def test_generic_failure_names_runs_and_pause_commands_and_the_real_output_dir():
    ...
    for cmd in ("haishui cron runs ab12cd34", "haishui cron run ab12cd34", "haishui cron pause ab12cd34"):
        assert f"`{cmd}`" in msg
    assert f"{display_haishui_home()}/cron/output/ab12cd34/" in msg
...
def test_auth_failure_points_at_login_and_a_retry_command(monkeypatch):
    ...
    assert "/login" in msg and "`haishui auth add <provider>`" in msg
    assert "haishui login" not in msg  # that command was removed
    assert "`haishui cron run ab12cd34`" in msg
...
def test_auth_failure_names_the_pinned_provider_and_the_failing_profile(monkeypatch, tmp_path):
    ...
    _no_chain(monkeypatch)
    profile_home = tmp_path / ".haishui" / "profiles" / "ops"
    ...
    assert "`haishui -p ops auth add openai-codex --type oauth`" in msg, msg
    assert "<provider>" not in msg
    unpinned = _summarize_cron_failure_for_delivery(JOB, "Error code: 401 - Unauthorized")
    assert "`haishui -p ops auth add <provider>`" in unpinned, unpinned
...
def test_rate_and_usage_limit_phrases_still_yield_a_provider_notice(monkeypatch):
    ...
    for text in (
        ...
        assert "`haishui cron run ab12cd34`" in msg or "`haishui cron edit ab12cd34" in msg, msg
...
def test_cron_cause_gloss_is_the_shared_table():
    ...
    for reason in FAILURE_CAUSE_GLOSS:
        ...
        assert notice is not None and "`haishui cron" in notice, reason
...
def test_transient_provider_failures_never_lead_with_jargon(monkeypatch):
    ...
    for err in ("Request timed out.", "HTTP 429: Too Many Requests"):
        ...
        assert "`haishui cron run ab12cd34`" in msg
        assert "`haishui cron runs ab12cd34`" in msg
...
def test_blocked_config_notice_says_it_did_not_run_and_will_self_heal():
    ...
    assert "`haishui cron doctor`" in text
