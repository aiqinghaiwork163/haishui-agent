...
def cron_store(tmp_path, monkeypatch):
    ...
    (haishui_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("HAISHUI_HOME", str(haishui_home))
    monkeypatch.setattr(J, "HAISHUI_DIR", haishui_home)
    monkeypatch.setattr(J, "CRON_DIR", haishui_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", haishui_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", haishui_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_haishui_now", lambda: now):
            due = J.get_due_jobs()
