...
def multiplex_homes(tmp_path, monkeypatch):
    ...
    ``get_default_haishui_root`` is patched so the primary config, the profiles
    ...
    monkeypatch.setattr(
        "haishui_constants.get_default_haishui_root", lambda: root
    )
    token = set_haishui_home_override(str(grant_home))
