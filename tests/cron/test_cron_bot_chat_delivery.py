...
from haishui_cli.quiet_single_query import TURN_REPORT_FILE_ENV, write_turn_report
...
def test_named_profile_resolves_when_exists():
    with mock.patch("haishui_cli.profiles.profile_exists", return_value=True):
...
def test_unknown_profile_resolves_to_none():
    with mock.patch("haishui_cli.profiles.profile_exists", return_value=False):
...
def test_deliver_runs_canonical_bot_chat_lane():
    ...
    with mock.patch.object(sched_delivery, "_run_bot_chat_turn", side_effect=fake_run), \
         mock.patch.object(sched_delivery.shutil, "which", return_value="/usr/bin/haishui"):
        err = _deliver_to_bot_chat({"id": "j1", "name": "Daily digest"}, "the output", "")

    assert err is None
    argv = calls["argv"]
    # The running install's interpreter, not whatever `haishui` PATH names (same order as /update).
    assert argv[:3] == [sys.executable, "-m", "haishui_cli.main"]
...
def test_deliver_timeout_returns_error_string():
    with mock.patch.object(
        sched_delivery, "_run_bot_chat_turn",
        side_effect=subprocess.TimeoutExpired(cmd="haishui", timeout=600),
    ), mock.patch.object(sched_delivery.shutil, "which", return_value="/usr/bin/haishui"):
        err = _deliver_to_bot_chat({"id": "j1", "name": "n"}, "out", "")
    assert err is not None
...
def _child_env() -> dict:
    """The stand-in child imports ``haishui_cli`` from this checkout, like the real ``-m haishui_cli.main``."""
...
def test_delivery_targets_include_local_profiles():
    with mock.patch("haishui_cli.profiles.list_profile_names",
