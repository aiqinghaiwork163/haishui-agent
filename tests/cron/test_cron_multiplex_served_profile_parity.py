...
builds must resolve exactly as it would under a standalone ``haishui -p <name> gateway run``.
"""
...
def two_homes(tmp_path, monkeypatch):
    ...
    (root / ".env").write_text(
        "HAISHUI_CRON_TIMEOUT=111\nHAISHUI_MODEL=default-model\nTERMINAL_ENV=docker\n"
        "TERMINAL_DOCKER_IMAGE=default-only-image\nHAISHUI_LANGUAGE=en\n")
    (alpha / ".env").write_text("HAISHUI_CRON_TIMEOUT=222\nHAISHUI_LANGUAGE=zh\n")
    ...
    for key, val in (("HAISHUI_CRON_TIMEOUT", "111"), ("HAISHUI_MODEL", "default-model"),
                     ("TERMINAL_ENV", "docker"), ("TERMINAL_DOCKER_IMAGE", "default-only-image"),
                     ("HAISHUI_LANGUAGE", "en")):
        monkeypatch.setenv(key, val)
    set_multiplex_active(True)
    home_token = set_haishui_home_override(str(alpha))
    try:
        yield root, alpha
    finally:
...
def test_cron_env_settings_resolve_from_the_served_profile(two_homes):
    """``HAISHUI_CRON_TIMEOUT`` / ``HAISHUI_MODEL`` are alpha's (or absent) both with the fire-time
    ...
    try:
        ...
        try:
            ...
            assert captured["target_model"] == ""  # alpha has no HAISHUI_MODEL; default's must not leak
...
def test_child_env_for_served_profile_drops_launch_profile_settings(two_homes):
    ...
    try:
        ...
        reset_secret_scope(token)
    for key in ("HAISHUI_MODEL", "TERMINAL_ENV", "TERMINAL_DOCKER_IMAGE", "HAISHUI_LANGUAGE", "HAISHUI_CRON_TIMEOUT"):
        assert key not in env, key
    assert env["HAISHUI_HOME"] == str(alpha)
    assert "PATH" in env

    # No-op when the scope IS the launch profile (standalone / default profile's own children).
    reset_haishui_home_override(set_haishui_home_override(None))
    home_token = set_haishui_home_override(str(root))
    try:
        env = strip_launch_profile_env(build_subprocess_env(scrub_secrets=True, inherit_profile_home=True))
    finally:
        reset_haishui_home_override(home_token)
    assert env["TERMINAL_ENV"] == "docker"
    assert env["HAISHUI_MODEL"] == "default-model"
