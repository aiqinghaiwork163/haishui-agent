...
profile's HAISHUI_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.haishui/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active HAISHUI_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever HAISHUI_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    HAISHUI_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import haishui_constants

    monkeypatch.setattr(
        haishui_constants, "_get_platform_default_haishui_home", lambda: root
    )
    monkeypatch.setenv("HAISHUI_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile HAISHUI_HOME (<root>/profiles/<name>), the cron store
    ...
    assert haishui_constants.get_haishui_home().resolve() == profile_home.resolve()
    assert haishui_constants.get_default_haishui_root().resolve() == root.resolve()

    # cron/jobs.py computes HAISHUI_DIR from get_haishui_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.HAISHUI_DIR.resolve() == profile_home.resolve()
        assert (
