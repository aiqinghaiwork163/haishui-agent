...
def test_parentless_seed_survives_a_restart_and_hides_its_runbook(monkeypatch, tmp_path):
    ...
    try:
        result = _create({
            "cols": 96, "source": "desktop", "title": "Welcome to Haishui",
            "messages": [
                {"role": "user", "content": "Private setup runbook", "display_kind": "hidden"},
                {"role": "assistant", "content": "Welcome to Haishui"},
        ...
        assert db.get_session(key)["title"] == "Welcome to Haishui"
        rows = db.get_messages_as_conversation(key)
        assert [r["content"] for r in rows] == ["Private setup runbook", "Welcome to Haishui", "Second question"]
