...
def local_route(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("haishui_cli.banner.get_update_result", lambda **kw: None)
    monkeypatch.setattr("haishui_cli.banner.get_available_skills", lambda: {})
...
def test_session_info_recovers_identity_from_the_owning_profile(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(server, "_haishui_home", launch)
    monkeypatch.setattr(server, "_probe_credentials", lambda agent: None)
    monkeypatch.setattr("haishui_cli.banner.get_update_result", lambda **kw: None)
    monkeypatch.setattr("haishui_cli.banner.get_available_skills", lambda: {})
    ...
    assert get_haishui_home() == launch
    ...
    with server._profile_build_scope(secondary):
        ...
        assert get_haishui_home() == secondary
    assert get_haishui_home() == launch
