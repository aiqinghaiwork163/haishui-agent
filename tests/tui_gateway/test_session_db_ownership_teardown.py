...
class _RecordingDB:
    """Stand-in for ``haishui_state.SessionDB`` that counts ``close()`` calls."""
...
def test_lazy_recall_open_is_owned_by_the_agent(monkeypatch):
    ...
    monkeypatch.setattr("haishui_state_registry.acquire", _factory)
...
def build_env(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(server, "set_haishui_home_override", lambda _home: None)
    monkeypatch.setattr(server, "reset_haishui_home_override", lambda _tok: None)
