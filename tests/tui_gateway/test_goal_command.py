...
def haishui_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves HAISHUI_HOME.
...
@pytest.fixture()
def server(haishui_home, monkeypatch):
    ...
    # Pin config resolution to the isolated HAISHUI_HOME. Sibling test
    ...
    # module-level ``_haishui_home = get_haishui_home()`` snapshot freezes
    ...
    monkeypatch.setattr(mod, "_haishui_home", haishui_home)
...
def test_moa_bare_returns_usage(server, session, haishui_home):
    _write_moa_config(haishui_home, """
moa:
...
def test_goal_draft_uses_session_profile_without_blocking_rpc_reader(
    ...
    def draft(objective):
        observed.append(get_haishui_home())
        started.set()
