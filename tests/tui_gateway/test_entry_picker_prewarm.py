...
guard is covered in ``tests/haishui_cli/test_picker_prewarm.py``):

- ``main()`` invokes ``haishui_cli.model_switch_providers.prewarm_picker_cache_async``
...
import haishui_cli.model_switch_providers
...
def _run_main(monkeypatch, events, *, prewarm=None):
    ...
    # entry.main() imports the helper lazily from haishui_cli.model_switch,
