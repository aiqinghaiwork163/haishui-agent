...
the installed haishui ``utils`` module and crashes the worker on ``import cli``
...
``acp_adapter/entry.py`` (via the shared ``haishui_bootstrap.harden_import_path``
...
def test_slash_worker_imports_from_cwd_with_colliding_utils(tmp_path):
    ...
    env = {k: v for k, v in os.environ.items() if k != "HAISHUI_PYTHON_SRC_ROOT"}
...
def test_sys_path_guard_runs_before_cli_import():
    ...
    ``haishui_bootstrap.harden_import_path()`` call precedes ``import cli``."""
    ...
    assert harden_call_line is not None, (
        "slash_worker.py must call haishui_bootstrap.harden_import_path()"
    )
...
def test_guard_delegates_to_shared_helper_not_inline():
    ...
    assert '{"", "."}' not in src and "{'', '.'}" not in src, (
        "slash_worker.py should delegate to haishui_bootstrap.harden_import_path, "
        "not re-implement the guard inline"
    )
    assert "haishui_bootstrap.harden_import_path()" in src, (
        "slash_worker.py must call the shared haishui_bootstrap.harden_import_path guard"
    )
