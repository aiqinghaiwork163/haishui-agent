...
``HAISHUI_HOME`` with named profile dirs — no mocks of the config/mcp layer — and
...
@pytest.fixture
def haishui_root(tmp_path, monkeypatch):
    """A temp HAISHUI_HOME root with two named profiles: 'work' and 'other'.

    Pointing HAISHUI_HOME at a dir outside ~/.haishui makes it the profile ROOT
    (get_default_haishui_root's Docker/custom branch), so named profiles live at
    ``<root>/profiles/<name>/`` and the launch/default profile is ``<root>``.
    """
    root = tmp_path / "haishui_home"
...
def test_add_lands_in_named_profile_only(haishui_root):
    root = haishui_root
    resp = _call(
        "mcp.servers.add",
        {
...
def test_list_reflects_the_scoped_profile(haishui_root):
    _result(
        _call(
            "mcp.servers.add",
...
def test_status_is_profile_scoped_and_credential_safe(haishui_root):
    _result(
        _call(
            "mcp.servers.add",
            {"profile": "work", "name": "svc-a", "config": {"command": "svc-a-bin"}},
...
def test_status_does_not_mix_launch_runtime_into_another_profile(haishui_root):
...
def test_status_includes_named_profile_runtime_in_multiplex(haishui_root):
    ...
    work_token = set_haishui_home_override(haishui_root / "profiles" / "work")
    try:
        work_scope = haishui_home_key()
    finally:
        reset_haishui_home_override(work_token)
...
def test_set_api_key_writes_env_and_header_to_right_profile(haishui_root):
    root = haishui_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_set_api_key_stdio_references_env_block(haishui_root):
    root = haishui_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_remove_scoped_to_profile(haishui_root):
    root = haishui_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_add_duplicate_and_missing_errors(haishui_root):
    _result(
        _call(
            "mcp.servers.add",
...
def test_add_requires_transport(haishui_root):
    resp = _call("mcp.servers.add", {"profile": "work", "name": "empty", "config": {}})
    assert "error" in resp
    assert resp["error"]["code"] == 4063


def test_default_profile_add_when_profile_omitted(haishui_root):
    root = haishui_root
    _result(
        _call(
            "mcp.servers.add",
    ...
    # Omitted profile → launch/default profile == HAISHUI_HOME root config.yaml.
    default_cfg = _read_yaml(root / "config.yaml")
...
def test_test_resolves_env_refs_from_requested_profile_secret_scope(haishui_root, monkeypatch):
    ...
    work = haishui_root / "profiles" / "work"
