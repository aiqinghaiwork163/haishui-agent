...
Haishui's own top-level modules (``utils``, ``proxy``, ``ui``).  entry.py must
neutralize this before any non-stdlib import is resolved, by delegating to the
shared ``haishui_bootstrap.harden_import_path`` guard.
...
def test_entry_calls_shared_harden_guard_before_heavy_imports():
    """entry.py must call haishui_bootstrap.harden_import_path() before it
    ...
    assert harden_call_line is not None, (
        "entry.py must call haishui_bootstrap.harden_import_path()"
    )
...
def test_guard_handles_absolute_cwd_path():
    ...
    path, ahead of the Haishui root.  harden_import_path must relocate the
    Haishui root to the front so ``from utils import ...`` resolves to Haishui."""
    import sys

    original = sys.path[:]
    try:
        sys.path[:] = ["/home/user/tg-ws-proxy", "/opt/haishui", "/usr/lib"]
        haishui_bootstrap.harden_import_path(src_root="/opt/haishui")
        assert sys.path[0] == "/opt/haishui"
        assert sys.path.index("/opt/haishui") < sys.path.index(
