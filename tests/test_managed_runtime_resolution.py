"""Guard: Haishui-owned subprocesses must not resolve managed runtimes by bare PATH.

Haishui installs runtimes for itself — ``uv`` at ``$HAISHUI_HOME/bin/uv``, Node at
``$HAISHUI_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in Haishui's own
...
* when a system copy also exists, the one Haishui does not own wins — which is
...
``iter_haishui_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes Haishui provisions into HAISHUI_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not Haishui-owned subprocess code: plugins ship their own
...
_ALLOWED: dict[tuple[str, str], str] = {
    ...
    ("haishui_cli/update_cmd_deps.py", "uv"): (
    ...
    ("haishui_cli/update_cmd_deps.py", "npm"): (
...
def _source_files() -> list[Path]:
    ...
    # (haishui_agent-<version>/) gets created and deleted concurrently, and
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a Haishui-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$HAISHUI_HOME/bin (uv) and $HAISHUI_HOME/node are not on an "
        ...
        "  node/npm -> haishui_constants.find_node_executable()\n"
        "  PATH env -> haishui_constants.iter_haishui_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(haishui_constants, helper))


def test_managed_uv_helpers_exist():
    from haishui_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
