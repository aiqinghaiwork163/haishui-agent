...
downstream consumers (CI / `haishui update`) rely on:
...
Haishui-side adapter behaves correctly against the documented response
shape.
"""
...
class TestDoctorExitCodes:
    ...
    def test_spawn_denied_exits_2_with_diagnosis(self, capsys):
        ...
        assert "Access is denied" in err and "HAISHUI_CUA_DRIVER_CMD" in err
...
class TestJsonOutput:
    def test_json_output_is_parseable_round_trip(self):
        ...
        # health_report keys are preserved; Haishui adds haishui_identity.
        parsed = json.loads(out.getvalue())
        report = _ok_report()
        for key, value in report.items():
            assert parsed[key] == value
        assert "haishui_identity" in parsed
        assert parsed["haishui_identity"]["resolved_binary"]


# ── HAISHUI_CUA_DRIVER_CMD resolution ───────────────────────────────────────
...
class TestDriverCmdResolution:
    ...
    def test_env_var_used_when_no_arg_given(self, monkeypatch):
        ...
        monkeypatch.setenv("HAISHUI_CUA_DRIVER_CMD", "/env/path/cua-driver")
        proc = _fake_proc_with_responses(
            {"jsonrpc": "2.0", "id": 1, "result": {}},
            {"jsonrpc": "2.0", "id": 2, "result": {"structuredContent": _ok_report()}},
        )
        with patch("shutil.which", return_value="/env/path/cua-driver") as which_mock, \
             patch("subprocess.Popen", return_value=proc), \
             patch("sys.stdout", new_callable=StringIO), \
             patch("haishui_cli.tools_config._cua_driver_cmd", side_effect=Exception("force env")):
    ...
    def test_user_local_driver_is_found_when_path_omits_it(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("HAISHUI_CUA_DRIVER_CMD", raising=False)
...
class TestDoctorVersionIdentity:
    ...
    def test_matching_versions_no_mismatch_flag(self):
        ...
        assert payload["haishui_identity"]["version_mismatch"] is False
