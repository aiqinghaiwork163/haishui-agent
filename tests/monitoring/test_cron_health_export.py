...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("haishui.gateway.up"), _M("haishui.gateway.active_agents"),
        _M("haishui.gateway.busy"), _M("haishui.gateway.drainable"),
        _M("haishui.gateway.restart_requested"),
        _M("haishui.platform.up"), _M("haishui.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("haishui.cron.scheduler.heartbeat_age_seconds"),
        _M("haishui.cron.scheduler.last_success_age_seconds"),
        _M("haishui.cron.scheduler.catch_up_occurrences"),
        _M("haishui.cron.jobs.enabled"), _M("haishui.cron.jobs.running"),
        _M("haishui.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "Haishui Agent-owned Relay transport health" in text
