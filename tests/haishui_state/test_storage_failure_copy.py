...
@pytest.mark.parametrize(
    ...
    [
        ...
        (sqlite3.OperationalError("attempt to write a readonly database"), "storage_readonly", "haishui doctor --fix"),
        (sqlite3.DatabaseError("database disk image is malformed"), "storage_corrupt", "haishui doctor --fix"),
        (OSError(28, "No space left on device"), "disk_full", "Free some disk space"),
        (None, "storage_unavailable", "haishui doctor --fix"),
    ],
)
...
def test_action_command_is_pinned_to_the_failing_profile(monkeypatch, tmp_path):
    ...
    whose state.db is not the process default; a bare ``haishui`` follows active_profile)."""
    ...
    for exc in (sqlite3.OperationalError("database is locked"), sqlite3.DatabaseError("malformed"), None):
        ...
        assert f"`haishui {selector}" in action and "`haishui doctor" not in action and "`haishui gateway" not in action
