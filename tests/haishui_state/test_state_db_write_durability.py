...
The defect: haishui_state already knows macOS `fsync()` does not guarantee
...
from haishui_state import repair_state_db_schema
from haishui_state_repair import _connect_repair_durable
...
def test_repair_path_has_no_bare_connects() -> None:
    ...
    """
    # The repair/probe helpers live in haishui_state_repair; haishui_state only
    # re-imports them.
    import haishui_state_repair

    source = Path(haishui_state_repair.__file__).read_text(encoding="utf-8")
    tree = ast.parse(source, filename=str(haishui_state_repair.__file__))
