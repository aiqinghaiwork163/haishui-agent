...
- files inside the Haishui home tree are repaired with ``chmod u+rw``
...
from haishui_state import SessionDB, preflight_db_writability
...
def haishui_home(tmp_path, monkeypatch):
    """Isolated HAISHUI_HOME so the repair scope covers tmp DBs."""
...
class TestRepairScope:
    def test_repairs_readonly_db_inside_home(self, haishui_home):
        db = haishui_home / "state.db"
        _make_db(db)
        os.chmod(db, 0o444)
    ...
    def test_repairs_readonly_sidecars(self, haishui_home):
        db = haishui_home / "state.db"
        _make_wal_db(db)
    ...
    def test_repairs_readonly_parent_directory(self, haishui_home):
        sub = haishui_home / "kanban"
        sub.mkdir()
        db = sub / "kanban.db"
        _make_db(db)
        os.chmod(sub, 0o555)
...
class TestRefusalOutsideScope:
    def test_actionable_error_names_file_and_chmod(self, haishui_home, tmp_path):
    ...
    def test_wal_error_warns_against_deletion(self, haishui_home, tmp_path):
        outside = tmp_path / "elsewhere"
        outside.mkdir()
        db = outside / "custom.db"
        _make_wal_db(db)
...
class TestSkips:
    ...
    def test_healthy_db_untouched(self, haishui_home):
        db = haishui_home / "state.db"
        _make_db(db)
...
class TestSessionDBIntegration:
    def test_sessiondb_selfheals_readonly_db_in_home(self, haishui_home):
        db_path = haishui_home / "state.db"
