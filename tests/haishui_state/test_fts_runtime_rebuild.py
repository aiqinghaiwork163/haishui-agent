...
import haishui_state_schema
from haishui_state import SessionDB
from haishui_state_common import FTS_REBUILD_DEFERRAL_KEY, FTS_STALE_KEY, LEGACY_FTS_SQL, LEGACY_FTS_TRIGRAM_SQL, SCHEMA_SQL, _FTS_TRIGGERS
from haishui_state_dbfile import _concrete_state_db_holder_pids, _is_inactive_orphan_desktop_holder
...
class TestRuntimeFtsRebuild:
    def test_reap_candidates_exclude_uninspectable_holder_suspicions(
        ...
        assert _concrete_state_db_holder_pids(
            db_path,
            [
                (222, "uninspectable holder: python -m haishui_cli.main serve --port 0"),
    ...
    @pytest.mark.parametrize(
        "argv",
        (
            ("journalctl", "-u", "haishui-agent.service"),
            ("grep", "haishui-agent", "/var/log/syslog"),
            (
                "/usr/sbin/tailscaled",
                "be-child",
                "ssh",
                "--cmd=python -m haishui_cli.main gateway",
            ),
            ("tmux", "new-session", "/opt/haishui-agent/.venv/bin/haishui gateway"),
            ("python3", "/opt/haishui-agent/tools/check_state.py"),
            ("haishui-monitor", "gateway"),
            ("haishuictl", "serve"),
            ("python3", "worker.py", "haishui_cli.main"),
            ("python3", "-m", "other.module", "haishui_cli.main"),
            ("python3", "-c", "haishui_cli.main"),
            ("python3", "-Icprint('haishui_cli.main')", "haishui_cli/main.py"),
        ),
    )
    def test_uninspectable_non_haishui_process_is_not_a_holder(self, argv):
        assert not haishui_state_holders._looks_like_haishui(argv)

    @pytest.mark.parametrize(
        "argv",
        (
            ("/usr/local/bin/haishui", "gateway"),
            ("/usr/local/bin/haishui-agent", "serve"),
            ("/usr/local/bin/haishui-acp", "--stdio"),
            ("/usr/bin/python3", "-m", "haishui_cli.main", "gateway"),
            ("/usr/bin/python3", "-m", "acp_adapter"),
            ("/usr/bin/python3", "-Im", "haishui_cli.main", "gateway"),
            ("/usr/bin/python3", "-mhaishui_cli.main", "gateway"),
            ("/usr/bin/python3", "-W", "ignore", "-m", "haishui_cli.main"),
            ("/usr/bin/python3", "-Xdev", "-m", "haishui_cli.main"),
            (
                "/opt/haishui-agent/.venv/bin/python",
                "/opt/haishui-agent/haishui_cli/main.py",
                "gateway",
            ),
            ("python.exe", "--", "haishui_cli/main.py", "gateway"),
            ("python3", "/opt/haishui-agent/run_agent.py", "--query", "hello"),
        ),
    )
    def test_uninspectable_haishui_process_remains_a_holder(self, argv):
        assert haishui_state_holders._looks_like_haishui(argv)
    ...
    def test_foreign_holder_detection_proc_readlink_deleted_wal(
        ...
        monkeypatch.setattr(haishui_state_holders.os, "stat", _stat)
    ...
    def test_foreign_holder_ignores_same_path_with_different_file_identity(
        ...
        assert haishui_state_holders.foreign_state_db_holders(db_path) == []
    ...
    def test_foreign_holder_uninspectable_process_cmdline_fallback(
        ...
        flagged when /proc/<pid>/cmdline identifies it as a Haishui process."""
        db_path = tmp_path / "state.db"
        ...
        # PID 222's cmdline is world-readable and looks like Haishui
        cmdline_path = proc_root / "222" / "cmdline"
        cmdline_path.write_bytes(
            b"python3\x00-m\x00haishui_cli.main\x00chat\x00"
        )
        ...
        def _fake_argv(pid):
            ...
            try:
                ...
                return None
        monkeypatch.setattr(haishui_state_holders, "_read_proc_argv", _fake_argv)

        holders = haishui_state_holders.foreign_state_db_holders(db_path)
        # Should include PID 222 with the cmdline info
        assert len(holders) == 1
        assert holders[0][0] == 222
        assert "haishui_cli.main" in holders[0][1]
    ...
    def test_structural_corruption_propagates_without_live_fts_mutation(
        ...
        from haishui_state import StateDbCorruptError
    ...
    def test_repeated_deferrals_reap_inactive_orphan_then_rebuild(
        ...
        monkeypatch.setattr(haishui_state_schema.time, "time", lambda: 120.0)
    ...
    def test_same_holder_set_across_futile_window_names_holder_and_gateway_safe_remedy(
        ...
        monkeypatch.setattr(
            haishui_state_schema, "_read_proc_argv", lambda pid: ["python", "-m", "haishui_cli.main", "serve"], raising=False,
        )
        clock = [1000.0]
        monkeypatch.setattr(haishui_state_schema.time, "time", lambda: clock[0])

        from haishui_cli.doctor_state import _render_state_db_stats
        from haishui_state_dbfile import collect_state_db_stats
        ...
        futile_attempts = getattr(haishui_state_schema, "_FTS_HOLDER_FUTILE_ATTEMPTS", 10)
        futile_seconds = getattr(haishui_state_schema, "_FTS_HOLDER_FUTILE_SECONDS", 1800.0)
        reopened = SessionDB(db_path=db_path)
        try:
            ...
            assert "pid 4242: python -m haishui_cli.main serve" in msg
    ...
    def test_retry_backoff_resets_when_the_blocking_holder_set_changes(
        ...
        try:
            ...
            reopened._fts_stale_retry_after = time.monotonic() + haishui_state_schema._FTS_STALE_RETRY_MAX_SECONDS
            reopened._fts_stale_retry_interval = haishui_state_schema._FTS_STALE_RETRY_MAX_SECONDS
...
class TestPhysicalCorruptionAcceptance:
    ...
    def test_canonical_btree_corruption_fails_closed(
        ...
        try:
            caplog.clear()
            with caplog.at_level("WARNING", logger="haishui_state"):
                with pytest.raises(sqlite3.DatabaseError) as caught:
