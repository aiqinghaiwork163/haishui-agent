"""The conftest WAL gate must agree with haishui_state, and must not import it.

``tests/conftest.py::_wal_is_usable`` duplicates the SQLite WAL-reset version
predicate instead of importing ``haishui_state``. That is deliberate: importing
``haishui_state`` during collection caches ``DEFAULT_DB_PATH`` from the real
``~/.haishui`` before the per-test ``HAISHUI_HOME`` redirect, which makes tests
...
def test_conftest_gate_agrees_with_haishui_state(version_info, monkeypatch):
    ...
    assert _wal_is_usable() is not is_sqlite_wal_reset_vulnerable(version_info), (
        f"conftest gate and haishui_state disagree for SQLite {version_info}"
    )


def test_conftest_does_not_import_haishui_state_at_collection():
    """The gate must stay import-free of haishui_state.

    Importing it during collection caches DEFAULT_DB_PATH from the real
    ~/.haishui, so tests read live production sessions instead of a tempdir.
    Reading the source is not an option here (banned), so assert on behavior:
    the gate must work with ``haishui_state`` absent from ``sys.modules`` and
    ...
    def guard(name, *args, **kwargs):
        if name == "haishui_state" or name.startswith("haishui_state."):
            blocked.append(name)
            raise AssertionError(
                "conftest._wal_is_usable imported haishui_state — this caches "
                "DEFAULT_DB_PATH from the real ~/.haishui during collection"
            )
        return real_import(name, *args, **kwargs)

    saved = sys.modules.pop("haishui_state", None)
    ...
    try:
        ...
        if saved is not None:
            sys.modules["haishui_state"] = saved
    assert not blocked
