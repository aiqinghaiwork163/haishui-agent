...
work on a file several Haishui processes share (gateway service, the Desktop
app's `haishui serve` backend, CLI sessions, the TUI slash worker):

* `haishui_state_common.fts_rebuild_admission` — full structural FTS rebuilds
* `haishui_state_repair._cross_process_repair_lock`   — writable_schema surgery / VACUUM
