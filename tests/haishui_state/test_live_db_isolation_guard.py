...
developer's REAL ~/.haishui/state.db, and a pytest-spawned process flipped
...
# Must match the root the guard itself computes.  Hardcoding ``~/.haishui``
# silently disarmed every assertion below on Windows, where the real root is
# ``%LOCALAPPDATA%\haishui``: the paths under test were then *correctly*
...
REAL_ROOT = haishui_state._real_platform_state_root()
...
class TestProductionPathRefused:
    def test_explicit_production_db_path_raises(self):
        """SessionDB pointed at the real ~/.haishui/state.db must fail hard."""
    ...
    def test_default_resolution_to_production_raises(self, monkeypatch):
        ...
        Simulates the escape vector: HAISHUI_HOME leaked/reset to the real
        home (subprocess child, stale worktree, gateway-launched shell) so
        ``_default_db_path()`` resolves the production DB.
        """
        monkeypatch.setenv("HAISHUI_HOME", str(REAL_ROOT))
        ...
        monkeypatch.setattr(
            haishui_state, "DEFAULT_DB_PATH", haishui_state._IMPORT_DEFAULT_DB_PATH
        )
        with pytest.raises(RuntimeError, match="live-system guard"):
            SessionDB()
...
class TestHermeticPathsAllowed:
    ...
    def test_tmp_haishui_home_default_resolution_works(self, tmp_path, monkeypatch):
        """Argless SessionDB() under a hermetic HAISHUI_HOME must succeed."""
        monkeypatch.setenv("HAISHUI_HOME", str(tmp_path / "hermetic-home"))
        monkeypatch.setattr(
            haishui_state, "DEFAULT_DB_PATH", haishui_state._IMPORT_DEFAULT_DB_PATH
        )
        db = SessionDB()
...
class TestBypassMarker:
    ...
    def test_bypass_marker_disables_state_db_guard(self):
        ...
        """
        haishui_state._ensure_test_isolation(REAL_ROOT / "state.db")
...
class TestSessionStoreLoudFailure:
    def test_guard_error_is_not_swallowed_into_jsonl_fallback(
        ...
        monkeypatch.setattr(haishui_state, "SessionDB", _boom)
        with pytest.raises(RuntimeError, match="live-system guard"):
...
class TestSubprocessChildCovered:
    def test_child_without_haishui_home_is_refused(self, tmp_path):
        """A subprocess child of a test (no HAISHUI_HOME) must be blocked.
        ...
        env = {
            ...
            if k not in ("HAISHUI_HOME", "PYTEST_PLUGINS", "PYTHONPATH")
        }
    ...
    def test_child_with_tmp_haishui_home_succeeds(self, tmp_path):
        """Same child, hermetic HAISHUI_HOME: must work — no false positive."""
        env = {
            k: v
            for k, v in os.environ.items()
        ...
        env["PYTEST_CURRENT_TEST"] = "tests/fake.py::test_child (call)"
        env["HAISHUI_HOME"] = str(tmp_path / "child-home")
        env["PYTHONPATH"] = str(Path(__file__).resolve().parents[2])
        code = (
            "from haishui_state import SessionDB\n"
            "db = SessionDB()\n"
