...
The ``haishui update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m haishui_cli.main gateway run`` — image name
    ``pythonw``, not ``haishui.exe`` — so the ``taskkill /IM haishui.exe`` guard
    ...
    # The haishui.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM haishui.exe' in text
