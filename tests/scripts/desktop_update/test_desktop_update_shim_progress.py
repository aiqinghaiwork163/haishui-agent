...
def progress(tmp_path):
    ...
    status = tmp_path / "haishui-update-status"
...
def test_every_state_carries_a_clock(progress):
    ...
    for state, message in [
        ...
        ("done", ""),
        ("manual", "Reopen Haishui to finish."),
        ("error", "Update failed."),
    ]:
...
# Stands in for `haishui update`, and reports the stage that was on screen
...
n="$(cat "$HAISHUI_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$HAISHUI_TEST_CALLS"
for f in "$TMPDIR"/haishui-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$HAISHUI_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$HAISHUI_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    install_root = tmp_path / "haishui-agent"
    (install_root / "venv" / "bin").mkdir(parents=True)
    haishui = install_root / "venv" / "bin" / "haishui"
    haishui.write_text(FAKE_HAISHUI)
    haishui.chmod(0o755)
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "HAISHUI_TEST_CAPTURE": str(capture),
        "HAISHUI_TEST_CALLS": str(calls),
        "HAISHUI_TEST_EXITS": str(tmp_path / "exits"),
    }
    ...
    result = tmp_path / ".haishui-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`haishui update` is the longest wait in the hand-off and the one the
