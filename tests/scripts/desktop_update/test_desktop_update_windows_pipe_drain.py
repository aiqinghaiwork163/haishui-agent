...
``Invoke-HaishuiStep``, which starts the step with ``RedirectStandardOutput`` /
...
last of them to close it. ``haishui update`` deliberately runs build steps with
stdout inherited (the tee-stderr runner in ``haishui_cli/main.py``), so the
...
closes and ``Invoke-HaishuiStep`` blocks for the life of the gateway.

Everything the hand-off owes the Desktop is downstream of that call:
``.haishui-update-result.json`` is never written, ``.haishui-update-in-progress``
is never cleared, and the Desktop is never relaunched -- so the app sits on
"Updating Haishui" until the user kills the gateway by hand, and the stale marker
...
step writing to both pipes took 18.3s vs 0.29s. ``haishui update`` is exactly this
...
class TestIdleWatchdogCountsUpdateLogGrowth:
    ...
    Real updates are stdout-silent for 40+ minutes: ``haishui update`` captures
    the (very loud) Electron/vite build into ``logs/update.log`` — NOT the
    child's stdout (``haishui_cli/update_cmd.py``, the update-log tee) — so the
    ...
    the ``HAISHUI_UPDATE_STEP_IDLE_SECONDS`` override each fails a test here.
    """
...
def test_update_step_survives_pipe_leak_flood_and_live_child_stall(
    ...
    *leak* -- a step that spawns a grandchild with ``UseShellExecute = $false``
    ...
    asserts the grandchild was **still alive** when ``Invoke-HaishuiStep``
    ...
    *logstall* -- a step that is silent on its pipes but appends to the
    ...
    This is the shape of every real ``haishui update`` build: output streams to
    ...
    env = {
        **os.environ,
        ...
        "HAISHUI_UPDATE_PIPE_DRAIN_SECONDS": "3",
        "HAISHUI_UPDATE_STEP_IDLE_SECONDS": "3",
        "HAISHUI_SELFTEST_HOLD_SECONDS": "45",
    }
    ...
    assert "PIPE-DRAIN SELF-TEST: PASS" in result.stdout, (
        ...
        "Haishui' forever) or metered a chatty step (backpressure on the running "
