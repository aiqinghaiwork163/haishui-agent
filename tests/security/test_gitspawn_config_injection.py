...
``[diff "x"] command=/textconv=`` driver. Haishui gathers workspace context by
...
context-gathering git path Haishui runs neutralizes every sink. They use a real
...
def test_web_git_diff_is_safe(malicious_repo):
    from haishui_cli import web_git
    repo, marker = malicious_repo
