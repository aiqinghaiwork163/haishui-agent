...
Haishui state stays profile-scoped through HAISHUI_HOME. Host subprocesses should
...
See: https://github.com/aiqinghaiwork163/haishui-agent/issues/25114
See: https://github.com/aiqinghaiwork163/haishui-agent/issues/36144
See: https://github.com/aiqinghaiwork163/haishui-agent/issues/29015
"""
...
class TestGetSubprocessHome:
    """Unit tests for haishui_constants.get_subprocess_home()."""

    def _host_mode(self, monkeypatch):
        monkeypatch.setattr(haishui_constants, "is_container", lambda: False)
        monkeypatch.delenv("TERMINAL_HOME_MODE", raising=False)
        monkeypatch.delenv("HAISHUI_REAL_HOME", raising=False)
    ...
    def test_host_auto_keeps_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        haishui_home = real_home / ".haishui" / "profiles" / "coder"
        profile_home = haishui_home / "home"
        profile_home.mkdir(parents=True)
        monkeypatch.setenv("HOME", str(real_home))
        monkeypatch.setenv("HAISHUI_HOME", str(haishui_home))
        from haishui_constants import get_subprocess_home
    ...
    def test_real_mode_repairs_parent_home_already_pointing_at_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HAISHUI_REAL_HOME", str(real_home))

        from haishui_constants import get_subprocess_home, get_real_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        self._container_mode(monkeypatch)
        base = tmp_path / ".haishui" / "profiles"
        ...
        monkeypatch.setenv("HAISHUI_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("HAISHUI_HOME", str(base / "beta"))
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_host_auto_preserves_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HAISHUI_REAL_HOME"] == str(real_home)
    ...
    def test_profile_mode_injects_profile_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(haishui_home / "home")
        assert result["HAISHUI_REAL_HOME"] == str(real_home)
    ...
    def test_no_injection_when_haishui_home_unset(self, monkeypatch):
...
class TestProfileBootstrap:
    ...
    def test_create_profile_bootstraps_home_dir(self, tmp_path, monkeypatch):
        ...
        from haishui_cli.profiles import create_profile
