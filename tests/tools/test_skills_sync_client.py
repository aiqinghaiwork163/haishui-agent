...
def synced_env(tmp_path, monkeypatch):
    """A HAISHUI_HOME with two opted-in skills + a token-carrying identity."""
    import haishui_constants
    home = tmp_path / "haishui"
    skills = home / "skills"
    skills.mkdir(parents=True)
    monkeypatch.setattr(haishui_constants, "get_haishui_home", lambda: home)
...
# ---------------------------------------------------------------------------
# Env-var configuration (Haishui Cloud "on by default" via environment)
# ---------------------------------------------------------------------------

class TestEnvConfig:
    def test_base_url_env_wins(self, monkeypatch):
        monkeypatch.setenv("HAISHUI_SYNC_BASE_URL", "https://plane.example/")
    ...
    def test_base_url_defaults_to_production(self, monkeypatch):
        ...
        monkeypatch.delenv("HAISHUI_SYNC_BASE_URL", raising=False)
        monkeypatch.setattr("haishui_cli.config.load_config", lambda: {}, raising=False)
    ...
    def test_feature_enabled_env(self, monkeypatch):
        # Default off.
        monkeypatch.delenv("HAISHUI_SYNC_ENABLED", raising=False)
        monkeypatch.setattr("haishui_cli.config.load_config", lambda: {}, raising=False)
        assert ssc.sync_feature_enabled() is False
        for truthy in ("1", "true", "YES", "on"):
            monkeypatch.setenv("HAISHUI_SYNC_ENABLED", truthy)
            assert ssc.sync_feature_enabled() is True
        for falsy in ("0", "false", "off"):
            monkeypatch.setenv("HAISHUI_SYNC_ENABLED", falsy)
            assert ssc.sync_feature_enabled() is False

    def test_default_opt_in_env(self, monkeypatch):
        monkeypatch.delenv("HAISHUI_SYNC_DEFAULT_OPT_IN", raising=False)
        monkeypatch.setattr("haishui_cli.config.load_config", lambda: {}, raising=False)
        assert ssc.sync_default_opt_in() is False
        monkeypatch.setenv("HAISHUI_SYNC_DEFAULT_OPT_IN", "true")
    ...
    def test_env_overrides_config_yaml(self, monkeypatch):
        ...
        monkeypatch.setenv("HAISHUI_SYNC_ENABLED", "false")
        monkeypatch.setattr(
            "haishui_cli.config.load_config",
            lambda: {"sync": {"enabled": True}},
            raising=False,
...
class TestDeviceName:
    def test_default_is_hostname_seeded(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("HAISHUI_SYNC_DEVICE_NAME", raising=False)
        monkeypatch.setattr(
    ...
    def test_existing_file_wins_over_default_and_env(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HAISHUI_SYNC_DEVICE_NAME", "cloud-seed")
        assert ssc.stable_device_id() == "Explicit Name"

    def test_env_seeds_first_use(self, tmp_path, monkeypatch):
        # Haishui Cloud path: HAISHUI_SYNC_DEVICE_NAME seeds the first-use label.
        monkeypatch.setattr(ssc, "_skills_dir", lambda: tmp_path)
        monkeypatch.setenv("HAISHUI_SYNC_DEVICE_NAME", "haishui-cloud-ben-1")
        assert ssc.stable_device_id() == "haishui-cloud-ben-1"
        # persisted so it stays stable even if the env later changes
        assert (tmp_path / ".sync_device_id").read_text() == "haishui-cloud-ben-1"
        monkeypatch.setenv("HAISHUI_SYNC_DEVICE_NAME", "changed")
        assert ssc.stable_device_id() == "haishui-cloud-ben-1"
