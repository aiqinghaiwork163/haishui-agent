...
Kept alongside the compat layer (tests/haishui_cli/test_compat_manifest_targets.py); both are deleted with it.
"""
import importlib
import json
import warnings
...
def test_compat_resolution_warns_once_per_name_with_the_new_location():
    from haishui_cli.plugin_compat import HaishuiPluginCompatWarning, _seen
    ...
    with warnings.catch_warnings(record=True) as rec:
        ...
        getattr(mod, name)
        getattr(mod, name)
    ours = [w for w in rec if issubclass(w.category, HaishuiPluginCompatWarning)]
...
def test_importing_the_facade_itself_does_not_warn():
    ...
    from haishui_cli.plugin_compat import HaishuiPluginCompatWarning
    import sys
    sys.modules.pop("tools.web_tools", None)
    with warnings.catch_warnings(record=True) as rec:
        warnings.simplefilter("always")
        importlib.import_module("tools.web_tools")
    assert not [w for w in rec if issubclass(w.category, HaishuiPluginCompatWarning)]
