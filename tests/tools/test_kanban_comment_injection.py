...
def worker_home(tmp_path, monkeypatch):
    ...
    for var in ("HAISHUI_KANBAN_DB", "HAISHUI_KANBAN_WORKSPACES_ROOT", "HAISHUI_KANBAN_HOME", "HAISHUI_KANBAN_BOARD"):
...
def test_seed_then_inject_new_comment(worker_home, monkeypatch):
    ...
    monkeypatch.setenv("HAISHUI_PROFILE", "worker-bot")
    agent = FakeAgent()
...
def test_delegated_child_in_worker_process_neither_receives_nor_consumes_notes(worker_home, monkeypatch):
    """A delegate_task child inherits the worker's ``HAISHUI_KANBAN_TASK``; operator notes
