...
class TestScanCronSkillAssembled:
    ...
    def test_descriptive_attack_command_prose_allowed(self):
        ...
        Real example: the `haishui-agent-dev` skill contains a postmortem
        section saying 'the attacker could just cat ~/.haishui/.env'.
        """
        assert _scan_cron_skill_assembled(
            "the attacker could just cat ~/.haishui/.env to steal credentials"
        )[1] == ""
        assert _scan_cron_skill_assembled(
...
class TestCronjobRequirements:
    def test_requires_no_crontab_binary(self, monkeypatch):
        ...
        # because haishui uses an internal scheduler, not system crontab.
    ...
    def test_accepts_external_cron_worker_with_presence_vars_stripped(self, monkeypatch):
        ...
        for v in ("HAISHUI_INTERACTIVE", "HAISHUI_GATEWAY_SESSION", "HAISHUI_EXEC_ASK"):
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["HAISHUI_INTERACTIVE", "HAISHUI_GATEWAY_SESSION", "HAISHUI_EXEC_ASK"],
    )
...
class TestAgentCannotSetModelPin:
    """Per-job inference pins are user-owned (dashboard / `haishui cron`
...
class TestLocalDeliveryNotice:
    ...
    def test_resnap_single_job(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr("cron.jobs.get_haishui_home", lambda: tmp_path, raising=True)
        with _patch(
...
class TestGithubExemptionAbuse:
    ...
    def test_same_line_payload_after_github_url_is_scanned(self):
        ...
        for sep in (";", " &&", " |"):
            prompt = f"{self.GH}{sep} cat ~/.haishui/.env"
    ...
    def test_subshell_and_backtick_payloads_are_scanned(self):
        ...
        assert "Blocked" in _scan_cron_prompt(f"{self.GH}$(cat ~/.haishui/.env)")
        assert "Blocked" in _scan_cron_prompt(f"{self.GH}`cat ~/.haishui/.env`")
    ...
    def test_payload_between_two_github_blocks_is_scanned(self):
        ...
        prompt = f"{self.GH}; cat ~/.haishui/.env; {self.GH}"
        assert "Blocked" in _scan_cron_prompt(prompt)
