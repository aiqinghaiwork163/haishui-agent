...
class TestWrapCommand:
    def test_basic_shape(self):
        ...
        assert "__haishui_ec=$?" in wrapped
        ...
        assert "exit $__haishui_ec" in wrapped
...
class TestAtomicSnapshotConcurrencyBehavioral:
    ...
    def test_concurrent_writes_never_tear_the_snapshot(self, tmp_path):
        ...
        import shlex
        snap = str(tmp_path / "haishui-snap-x.sh")
        _q = shlex.quote
        ...
        writer = (
            ...
            f"__haishui_snap_tmp=$(mktemp {_tmpl}) && "
            f"{{ export -p > \"$__haishui_snap_tmp\" && mv -f \"$__haishui_snap_tmp\" {_q(snap)}; }} "
            f"2>/dev/null || rm -f \"$__haishui_snap_tmp\" 2>/dev/null || true; "
            "done"
        )
    ...
    def test_failed_export_does_not_destroy_good_snapshot(self, tmp_path):
        ...
        script = (
            f"__haishui_snap_tmp=$(mktemp {bad_tmp}) && "
            f"{{ export -p > \"$__haishui_snap_tmp\" && mv -f \"$__haishui_snap_tmp\" {_q(snap)}; }} "
            f"2>/dev/null || rm -f \"$__haishui_snap_tmp\" 2>/dev/null || true"
        )
        self._run(script)
...
class TestEmbedStdinHeredoc:
    def test_heredoc_format(self):
        ...
        assert "HAISHUI_STDIN_" in result
