    ...
    return [
        {
            ...
            "connection_label": "Haishui Cloud",
            "title": "Moxie",
...
def test_roster_roundtrip_and_validation(root):
    rows = _rows() + [
        ...
        "not-a-dict",
        {"profile": "default", "handle": "haishui", "connection_id": "cloud-1"},  # dupe
    ]
...
def test_resolve_remote_target_forms(root):
    ...
    assert bot_relay.resolve_remote_target("@haishui", roster)["profile"] == "default"
    ...
    assert bot_relay.resolve_remote_target("haishui@cloud-1", roster)["profile"] == "default"
    ...
    assert bot_relay.resolve_remote_target("haishui@nope", roster) is None
...
def test_resolve_ambiguous_handle_across_connections(root):
    ...
    assert "haishui" in forms  # unique handle stays bare
...
def test_relay_route_queues_envelope_and_spawns_waiter(tmp_path, monkeypatch):
    ...
    bot_relay.write_remote_roster(home, [
        {"profile": "default", "handle": "haishui", "connection_id": "cloud-1",
         "connection_label": "Haishui Cloud", "title": "Moxie"},
    ])
    ...
    agent = _FakeAgent(home)
    out = json.loads(message_agent_tool(target="haishui", message="ping", agent=agent))
    assert out.get("status") == "queued"
    assert "Haishui Cloud" in spawned["label"]
    ...
    assert pending[0]["message"].startswith("Message from 🤖 haishui (@haishui): ping")
...
def test_protocol_section_lists_remote_teammates(tmp_path):
    ...
    assert "`@haishui` — on Haishui Cloud — Moxie" in section
...
def test_cleanup_bot_relay_artifacts_sweeps_stale_plaintext(tmp_path, monkeypatch):
    ...
    stale_env = bot_relay.enqueue_envelope(
        ...
        sender_profile="default", sender_handle="haishui",
    )
...
def test_cleanup_bot_relay_artifacts_missing_dir_is_zero(tmp_path, monkeypatch):
    monkeypatch.setenv("HAISHUI_HOME", str(tmp_path / "nope"))
...
def test_ttl_config_read_is_lazy_and_defensive(monkeypatch):
    ...
    def _boom(name, *a, **k):
        if name.startswith("haishui_cli"):
...
def test_message_agent_surfaces_runtime_offline_refusal(tmp_path, monkeypatch):
    ...
    bot_relay.write_remote_roster(home, [
        {"profile": "default", "handle": "haishui", "connection_id": "cloud-1",
         "connection_label": "Haishui Cloud", "online": False},
    ])
    monkeypatch.setattr(
        "tools.bot_mode_dm._spawn_delivery",
        lambda *a, **k: json.dumps({"status": "queued"}),
...
# ── delivery turn author (HAISHUI_TURN_AUTHOR on the recipient turn) ──────────
...
def test_delivery_env_carries_only_the_given_author(monkeypatch):
    """The dispatcher's own HAISHUI_TURN_AUTHOR never reaches the child: dropped without an author, replaced with one."""
    from agent.turn_author import TURN_AUTHOR_ENV

    monkeypatch.setenv("HAISHUI_RELAY_TEST_MARKER", "kept")
    monkeypatch.setenv(TURN_AUTHOR_ENV, json.dumps({"id": "bot:previous", "name": "previous", "is_bot": True}))
    monkeypatch.setenv("HAISHUI_SESSION_KEY", "session-A")
    monkeypatch.setenv("HAISHUI_UI_SESSION_ID", "ui-A")
    monkeypatch.setenv("HAISHUI_SESSION_ID", "session-A")
    monkeypatch.setenv("HAISHUI_SESSION_PROFILE", "profile-A")
    # A session-* knob, not identity: stripping it would break the child's watcher tuning.
    monkeypatch.setenv("HAISHUI_SESSION_STALL_TIMEOUT", "97")
    ...
    assert env["HAISHUI_RELAY_TEST_MARKER"] == "kept"
    assert "HAISHUI_SESSION_KEY" not in env
    assert "HAISHUI_UI_SESSION_ID" not in env
    assert "HAISHUI_SESSION_ID" not in env
    assert "HAISHUI_SESSION_PROFILE" not in env
    assert env["HAISHUI_SESSION_STALL_TIMEOUT"] == "97"
