...
class TestTerminalIntegration:
    ...
    def test_blocklisted_var_blocked_by_default(self):
        ...
        blocked_var = next(iter(_HAISHUI_PROVIDER_ENV_BLOCKLIST))
    ...
    def test_passthrough_cannot_override_provider_blocklist(self):
        ...
        Haishui provider credentials — that was the bypass where a skill
    ...
    def test_passthrough_cannot_override_internal_dynamic_secret(self):
        """A skill must NOT be able to register dynamically-named Haishui
    ...
    def test_non_haishui_api_key_still_registerable(self):
        """Third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
        Haishui provider credentials and must still pass through — skills
    ...
    def test_provider_blocklist_import_failure_fails_closed(self, monkeypatch):
        ...
        otherwise a skill could tunnel a Haishui credential into the
        execute_code child (regression for #37950 / GHSA-rhgp-j443-p4rf).

        Verifies the full path: _is_haishui_provider_credential returns True,
        register_env_passthrough refuses the var, and _scrub_child_env keeps
        it out of the child env. A non-Haishui key is also rejected here (the
        ...
        assert _ep_mod._is_haishui_provider_credential("OPENAI_API_KEY")
        assert _ep_mod._is_haishui_provider_credential("ANTHROPIC_API_KEY")
        assert _ep_mod._is_haishui_provider_credential("GH_TOKEN")
