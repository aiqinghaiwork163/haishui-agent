...
def _restore_tool_and_agent_modules():
    ...
    original_modules = {
        ...
        if name in {"tools", "agent", "haishui_cli"}
    ...
    try:
        ...
        _reset_modules(("tools", "agent", "haishui_cli"))
...
def _install_fake_tools_package(*, credential_mounts=None):
    ...
    haishui_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["haishui_cli"] = haishui_cli
    sys.modules["haishui_cli.config"] = types.SimpleNamespace(
        get_haishui_home=lambda: Path(tempfile.gettempdir()) / "haishui-home",
    )
...
def test_managed_modal_rejects_host_credential_passthrough():
    _install_fake_tools_package(
        credential_mounts=[{
            ...
            "container_path": "/root/.haishui/token.json",
        }]
    )
    managed_modal = _load_tool_module("tools.environments.managed_modal", "environments/managed_modal.py")
