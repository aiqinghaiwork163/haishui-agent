"""Cross-session HAISHUI_SESSION_ID leak via the shared bash snapshot.
...
``HAISHUI_SESSION_ID`` into the snapshot, so every LATER session ``source``d that
stale value and its ``echo $HAISHUI_SESSION_ID`` reported a FOREIGN session's id
— overriding the correct per-command Popen env injected by
``_inject_session_context_env``.

The fix strips the per-session bridged vars (HAISHUI_SESSION_* / UI /
...
run stamps per command (HAISHUI_DELEGATED_CHILD_CONTEXT, HAISHUI_CRON_SESSION), or
...
def test_export_snippet_shape():
    snippet = _export_dump_excluding_session_vars('"$__haishui_snap_tmp"')
    ...
    assert "${!HAISHUI_SESSION_*}" in snippet
    assert "${!HAISHUI_CRON_AUTO_DELIVER_*}" in snippet
    assert "${!HAISHUI_BROWSER_CONTROL_*}" in snippet
    assert "HAISHUI_UI_SESSION_ID" in snippet
    assert "grep -vE" not in snippet
    assert '"$__haishui_snap_tmp"' in snippet
    ...
    assert snippet.rstrip().endswith('> "$__haishui_snap_tmp"')
    ...
    try:
        def run_as(sid):
            ...
            def worker():
                ...
                out["r"] = env.execute('echo "[$HAISHUI_SESSION_ID]"')
        ...
        if os.path.exists(snap):
            with open(snap) as f:
                assert "HAISHUI_SESSION_ID" not in f.read()
    finally:
        env.cleanup()
...
def test_export_dump_drops_every_bridged_var_and_the_delegation_marker():
    ...
    exports = "; ".join([f'export {n}="x"' for n in scoped] + ['export HAISHUI_HOME="/h"', 'export MYVAR="keep"'])
    out = subprocess.run(
    ...
    assert 'declare -x HAISHUI_HOME="/h"' in out
