"""Tests for tools.wake_word — the "Hey Haishui" hotword detector.
...
def test_config_defaults_and_clamping():
    ...
    assert ww.wake_phrase({"phrase": "hey haishui"}) == "hey haishui"
    assert ww.wake_phrase({}) == "hey haishui"
...
def test_looks_like_path():
    ...
    assert _looks_like_path("models/hey_haishui.onnx")
...
def test_load_wake_word_config_guards_non_dict(monkeypatch):
    monkeypatch.setattr(
        "haishui_cli.config.load_config", lambda: {"wake_word": "oops"}
    )
...
def test_requirements_openwakeword_available(monkeypatch):
    ...
    r = ww.check_wake_word_requirements(
        {"provider": "openwakeword", "phrase": "hey haishui"}
    )
    assert r["available"] is True
    assert r["provider"] == "openwakeword"
    assert r["phrase"] == "hey haishui"
...
def _install_fake_openwakeword(monkeypatch):
    ...
    class _FakeModel:
        def __init__(self, wakeword_models, inference_framework="onnx"):
            ...
            self.models = {"hey_haishui": object()}

        def predict(self, frame):
            return {"hey_haishui": 0.0}
...
def test_openwakeword_ensures_base_models_for_custom_path(monkeypatch):
    ...
    eng = ww._OpenWakeWordEngine(
        {"provider": "openwakeword", "openwakeword": {"model": "/models/hey_haishui.onnx"}}
    )
    assert calls["download"] == [["/models/hey_haishui.onnx"]]
    assert eng._labels == ["hey_haishui"]


def test_bundled_hey_haishui_model_ships_on_disk():
    # The "hey haishui" wake word works out of the box only if the model is
