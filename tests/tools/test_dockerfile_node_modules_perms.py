...
Older images made /opt/haishui/ui-tui and /opt/haishui/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV HAISHUI_TUI_DIR=/opt/haishui/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R haishui:haishui /opt/haishui/ui-tui" not in text
    assert "chown -R haishui:haishui /opt/haishui/node_modules" not in text
