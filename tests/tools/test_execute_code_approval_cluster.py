...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_env._scrub_child_env — broad HAISHUI_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: HAISHUI_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_haishui_allowlist_and_secret_blocks():
    from tools.code_execution_env import _scrub_child_env

    env = {
        # operational allowlist → kept
        "HAISHUI_HOME": "/h", "HAISHUI_PROFILE": "p",
        "HAISHUI_CONFIG": "/c.yaml", "HAISHUI_ENV": "/e",
        "HAISHUI_DELEGATED_CHILD_CONTEXT": "1",
        # other HAISHUI_* → dropped (broad prefix removed)
        "HAISHUI_BASE_URL": "https://x", "HAISHUI_INTERACTIVE": "1",
    ...
    for kept in (
        ...
        "HAISHUI_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "HAISHUI_BASE_URL", "HAISHUI_INTERACTIVE", "HAISHUI_KANBAN_TASK",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped HAISHUI_* vars."""
    import logging
