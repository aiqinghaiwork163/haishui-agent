...
was gated on ``[ -f "$HAISHUI_HOME/.env" ]`` while the first-boot seed that
...
def _run_keygen(
    ...
    script = (
        ...
        f"{env_setup}"
        f'HAISHUI_HOME="{home}"\n'
        # In tests we run unprivileged; as_haishui is a passthrough then.
        'as_haishui() { "$@"; }\n'
...
def test_keygen_skips_when_container_env_provides_key(
    ...
    Haishui loads $HAISHUI_HOME/.env with override=True, so a key generated
...
def test_dockerignore_keeps_env_example_template() -> None:
    """The first-boot seed copies /opt/haishui/.env.example -> $HAISHUI_HOME/.env.
    ...
    assert verdict == "included", (
        ...
        "seeds $HAISHUI_HOME/.env from it on first boot"
    )
