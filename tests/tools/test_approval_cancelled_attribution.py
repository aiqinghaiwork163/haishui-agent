...
def gateway_session(monkeypatch):
    ...
    for k in ("HAISHUI_CRON_SESSION", "HAISHUI_YOLO_MODE", "HAISHUI_INTERACTIVE"):
        monkeypatch.delenv(k, raising=False)
    monkeypatch.setenv("HAISHUI_GATEWAY_SESSION", "1")
    monkeypatch.setenv("HAISHUI_SESSION_KEY", SESSION_KEY)
    # ``--yolo`` is frozen at import from HAISHUI_YOLO_MODE; a host shell running yolo must not
...
def cli_session(monkeypatch):
    mod._session_approved.clear()
    for k in ("HAISHUI_CRON_SESSION", "HAISHUI_YOLO_MODE", "HAISHUI_GATEWAY_SESSION", "HAISHUI_EXEC_ASK"):
        monkeypatch.delenv(k, raising=False)
