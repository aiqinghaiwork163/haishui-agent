...
class TestSendMessageTool:
    ...
    def test_ntfy_topic_target_bypasses_channel_directory(self):
        ...
        ntfy_cfg = SimpleNamespace(enabled=True, token=None, extra={"topic": "haishui-in"})
...
class TestSendToPlatformChunking:
    ...
    def test_signal_long_message_is_chunked(self, monkeypatch):
        ...
        The standalone path (haishui send / cron / MCP) speaks raw JSON-RPC via
...
class TestSendToPlatformWhatsapp:
    def test_whatsapp_routes_via_local_bridge_sender(self):
        ...
        assert _call.args[2] == "hello from haishui"
...
class TestParseTargetRef:
    ...
    def test_explicit_targets_round_trip_chat_and_thread(self):
        cases = [
            ...
            ("matrix", "@haishui:matrix.org", "@haishui:matrix.org", None),
...
class TestSendViaAdapterStandaloneFallback:
    ...
    When the gateway runner is not in this process (e.g. ``haishui cron``
    runs separately from ``haishui gateway``), ``_send_via_adapter`` should
...
def test_not_configured_error_names_resolved_home_and_consulted_sources(tmp_path, monkeypatch):
    ...
    hardcoded ``~/.haishui``) and what each source held, so a Windows/profile home user can fix the right file."""
...
def test_not_configured_error_names_default_root_gateway_and_secret_sources(tmp_path, monkeypatch):
    """Under ``HAISHUI_HOME=<root>/profiles/<p>`` the error says a live gateway from the default root has the
