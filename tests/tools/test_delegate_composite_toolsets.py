...
class TestExpandParentToolsets(unittest.TestCase):
    ...
    def test_composite_haishui_cli_expands_web(self):
        """haishui-cli includes web_search/web_extract → 'web' should be in expansion."""
        expanded = _expand_parent_toolsets({"haishui-cli"})
        self.assertIn("web", expanded)
        self.assertIn("terminal", expanded)
        self.assertIn("browser", expanded)
        # Original composite is preserved
        self.assertIn("haishui-cli", expanded)


    def test_intersection_with_expanded_composite(self):
        """End-to-end: requesting ['web'] from parent with ['haishui-cli'] yields ['web']."""
        parent_toolsets = {"haishui-cli"}
    ...
    def test_included_toolsets_of_composite_parent_are_grantable(self):
        ...
        self.assertNotIn("haishui-cli", expanded)

    def test_composites_with_allowed_included_tools_are_not_stripped(self):
        toolsets = ["safe", "haishui-gateway", "haishui-cli", "delegation", "kanban"]

        self.assertEqual(
            _strip_blocked_tools(toolsets),
            ["safe", "haishui-gateway", "haishui-cli"],
        )
