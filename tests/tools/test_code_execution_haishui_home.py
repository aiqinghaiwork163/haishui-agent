"""execute_code child env honors the multiplexed per-turn HAISHUI_HOME override (#110303).

Under a multiplexed Desktop/Dashboard connection one server process serves several
profiles, binding a context-local HAISHUI_HOME override per turn. ``_build_child_env``
scrubs the server process's ``os.environ`` — which carries the machine-default
HAISHUI_HOME — so without the rewrite below, skill scripts run via ``execute_code``
...
def home_override():
    ...
    def _set(path):
        tokens.append(set_haishui_home_override(path))
        return str(path)
...
def _child_env():
    return _build_child_env(
        rpc_endpoint="sock",
        rpc_token="tok",
        tmpdir="/tmp/haishui-test",
        child_python=sys.executable,
    )


class TestMultiplexedHaishuiHome:
    def test_override_rewrites_stale_server_default_per_turn(self, monkeypatch, home_override, tmp_path):
        ...
        monkeypatch.setenv("HAISHUI_HOME", "/machine/default/.haishui")
        alpha = home_override(tmp_path / "profiles" / "alpha")
        assert _child_env()["HAISHUI_HOME"] == alpha
        beta = home_override(tmp_path / "profiles" / "beta")
        assert _child_env()["HAISHUI_HOME"] == beta
    ...
    def test_no_override_leaves_inherited_value_untouched(self, monkeypatch):
        ...
        assert _child_env()["HAISHUI_HOME"] == "/machine/default/.haishui"
