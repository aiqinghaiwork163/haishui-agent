...
class TestHandleFunctionCall:
    ...
    def test_post_tool_call_receives_non_negative_integer_duration_ms(self):
        ...
        with (
            ...
            patch("haishui_cli.plugins.has_hook", return_value=True),
            patch("haishui_cli.plugins.invoke_hook") as mock_invoke_hook,
        ):
    ...
    def test_no_listener_skips_post_and_transform_emit(self):
        ...
        with (
            patch("model_tools.registry.dispatch", return_value='{"ok":true}'),
            patch("haishui_cli.plugins.has_hook", return_value=False),
            patch("haishui_cli.plugins.invoke_hook") as mock_invoke_hook,
        ):
    ...
    def test_tool_request_and_execution_middleware_wrap_registry_dispatch(self, monkeypatch):
        ...
        monkeypatch.setattr("haishui_cli.plugins.invoke_middleware", fake_invoke_middleware)
        ...
        monkeypatch.setattr("haishui_cli.plugins.has_hook", lambda name: True)
    ...
    def test_registry_exception_emits_terminal_tool_hook(self, monkeypatch):
        from haishui_cli import lifecycle

        hook_calls = []
        monkeypatch.setattr("haishui_cli.plugins.invoke_hook", lambda *_args, **_kwargs: [])
...
class TestPreToolCallBlocking:
    ...
    def test_blocked_tool_returns_error_and_skips_dispatch(self, monkeypatch):
        ...
        monkeypatch.setattr("haishui_cli.plugins.invoke_hook", fake_invoke_hook)
        monkeypatch.setattr("haishui_cli.plugins.has_hook", lambda name: True)
        monkeypatch.setattr("model_tools.registry.dispatch", fake_dispatch)
...
class TestDisabledToolsetsPlatformBundle:
    ...
    def test_disabling_platform_bundle_preserves_core_tools(self):
        ...
        assert names_telegram == names_no_yuanbao, (
            f"Tools lost after disabling haishui-yuanbao: "
            f"{names_telegram - names_no_yuanbao}"
        )

    def test_disabling_platform_bundle_removes_own_tools(self):
        """Disabling haishui-discord should remove discord-specific tools."""
        from model_tools import get_tool_definitions

        tools = get_tool_definitions(
            enabled_toolsets=["haishui-discord"],
            disabled_toolsets=["haishui-discord"],
            quiet_mode=True,
        )
    ...
    def test_bundle_non_core_tools_unknown_falls_back(self):
        ...
        assert bundle_non_core_tools("haishui-does-not-exist") == set()
...
class TestDisabledToolsetsPostureToolset:
    ...
    not own -- same non-core-delta subtraction as haishui-* bundles (#33924) --
...
def test_tool_defs_cache_key_sees_config_replacement_with_pinned_mtime(tmp_path):
    ...
    with patch("haishui_cli.config.get_config_path", return_value=cfg):
