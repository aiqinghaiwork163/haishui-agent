...
def _restore_tool_modules():
    original_haishui_home = os.environ.get("HAISHUI_HOME")
    original_modules = {
        name: module
        ...
        or name == "haishui_cli"
        or name.startswith("haishui_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_haishui_home is None:
            os.environ.pop("HAISHUI_HOME", None)
        else:
            os.environ["HAISHUI_HOME"] = original_haishui_home
        _reset_modules(("tools", "haishui_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    haishui_cli = types.ModuleType("haishui_cli")
    haishui_cli.__path__ = [str(REPO_ROOT / "haishui_cli")]  # type: ignore[attr-defined]
    sys.modules["haishui_cli"] = haishui_cli
    haishui_home = tmp_path / "haishui-home"
    os.environ["HAISHUI_HOME"] = str(haishui_home)
    sys.modules["haishui_cli.config"] = types.SimpleNamespace(
        get_haishui_home=lambda: haishui_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="haishui-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": haishui_home / "modal_snapshots.json",
