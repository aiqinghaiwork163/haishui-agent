...
def test_output_path_rejects_haishui_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    haishui_home = tmp_path / "haishui-home"
    haishui_home.mkdir()
    monkeypatch.setattr(file_safety, "_haishui_home_path", lambda: haishui_home)
    monkeypatch.setattr(file_safety, "_haishui_root_path", lambda: haishui_home)

    target = haishui_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    haishui_home = tmp_path / "haishui-home"
    token_dir = haishui_home / "mcp-tokens"
