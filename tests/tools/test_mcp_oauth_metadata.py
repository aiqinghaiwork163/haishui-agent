...
Covers:
- :class:`HaishuiTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.HaishuiMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _HAISHUI_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# HaishuiTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = HaishuiTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("HAISHUI_HOME", str(tmp_path))
        storage = HaishuiTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (HaishuiMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: HaishuiTokenStorage, **context_attrs):
    ...
    provider = _HAISHUI_PROVIDER_CLS.__new__(_HAISHUI_PROVIDER_CLS)
    provider._haishui_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HAISHUI_HOME", str(tmp_path))
        storage = HaishuiTokenStorage("mgr-srv")
        ...
        with patch.object(
            _HAISHUI_PROVIDER_CLS.__bases__[-1], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HAISHUI_HOME", str(tmp_path))
        storage = HaishuiTokenStorage("flow-srv")
        ...
        with patch.object(
            _HAISHUI_PROVIDER_CLS.__bases__[-1],
            "async_auth_flow",
