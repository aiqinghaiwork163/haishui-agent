...
def _make_get_files(tmp_files, remote_base="/root/.haishui"):
...
def _make_manager(tmp_files, remote_base="/root/.haishui", upload=None, delete=None):
...
class TestEdgeCases:
    ...
    def test_file_disappears_between_list_and_upload(self, tmp_path):
        ...
        upload = MagicMock()
        mgr = FileSyncManager(
            get_files_fn=lambda: [(str(f), "/root/.haishui/ephemeral.txt")],
            upload_fn=upload,
            delete_fn=MagicMock(),
        )
...
class TestConcurrency:
    def test_sync_back_waits_for_active_sync_transaction(self, tmp_path):
        ...
        def get_files():
            return [
                (str(path), f"/root/.haishui/cache/images/{path.name}")
                for path in sorted(tmp_path.glob("*.png"))
            ]

        def upload(_host_path, remote_path):
            if remote_path == f"/root/.haishui/cache/images/{new_file.name}":
                upload_started.set()
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
            ...
            sync_back_future = executor.submit(mgr.sync_back, haishui_home=tmp_path)
    ...
    def test_sync_back_does_not_overwrite_uploaded_credential_files(self, tmp_path, monkeypatch):
            ...
            lambda: [
                {
                    ...
                    "container_path": "/root/.haishui/credentials/token.json",
                }
            ],
        )
        monkeypatch.setattr(
            "tools.credential_files.iter_skills_files",
            lambda container_base="/root/.haishui": [
                {
                    "host_path": str(skill),
        ...
        def bulk_download(dest: Path) -> None:
            with tarfile.open(dest, "w") as tar:
                for name, data in {
                    "root/.haishui/credentials/token.json": b"remote-token",
                    "root/.haishui/skills/skill.py": b"remote-skill",
                }.items():
        ...
        mgr = FileSyncManager(
            get_files_fn=lambda: iter_sync_files("/root/.haishui"),
            upload_fn=MagicMock(),
            delete_fn=MagicMock(),
            bulk_download_fn=bulk_download,
        )
        ...
        mgr.sync(force=True)
        mgr.sync_back(haishui_home=tmp_path)
