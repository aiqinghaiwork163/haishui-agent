...
Uses a throwaway HAISHUI_HOME so it never touches ~/.haishui. No real names.
"""
...
def _fresh_home():
    """Point HAISHUI_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_haishui_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["HAISHUI_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER HAISHUI_HOME is set.
...
# a different get_haishui_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"HAISHUI_HOME (throwaway): {HOME}")
