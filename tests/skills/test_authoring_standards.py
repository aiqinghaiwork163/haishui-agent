...
def test_required_frontmatter_fields(p):
    ...
    haishui = (fm.get("metadata") or {}).get("haishui") or {}
    if not (haishui.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.haishui.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    haishui = (fm.get("metadata") or {}).get("haishui") or {}
    dangling = [
        rs for rs in (haishui.get("related_skills") or []) if rs not in _all_names()
    ]
