...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.haishui/haishui-agent/utils.py)
...
2. ``haishui update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``haishui_cli/main.py`` (the ``haishui update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``haishui_constants`` after the
