...
def test_two_queries_never_consume_previous_query_caches(tmp_path, monkeypatch):
    ...
    assert HonchoClientConfig.from_global_config(host="haishui", config_path=path).recall_sync
    config_provider.save_config({"hosts": {"haishui": {"recallSync": False}}}, str(tmp_path))
    assert not HonchoClientConfig.from_global_config(host="haishui", config_path=path).recall_sync
...
def test_missing_peer_notice_surfaces_once():
    ...
    assert "haishui honcho peer --user" in first
