...
Under a gateway (Telegram, Discord, Slack, ...) Haishui passes the
...
class TestPinPeerNameConfigParsing:
    ...
    def test_root_level_true(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HAISHUI_HOME", str(tmp_path / "isolated"))
...
class TestCrossPlatformMemoryUnification:
    """The same physical user talking to Haishui via Telegram AND Discord
...
class TestPinUserPeerAlias:
    ...
    def test_pinPeerName_still_works_unchanged(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "***",
            "peerName": "eri",
            "hosts": {"haishui": {"pinPeerName": True}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.pin_peer_name is True
...
class TestPinTransition:
    ...
    def test_identity_signature_reflects_both_session_prefixes(self, tmp_path, monkeypatch):
        ...
        cfg_path = tmp_path / "honcho.json"
        monkeypatch.setenv("HAISHUI_HOME", str(tmp_path))
        base = {"apiKey": "k", "peerName": "Igor", "aiPeer": "haishui"}
    ...
    def test_identity_signature_reflects_a_repointed_host_workspace(self, tmp_path, monkeypatch):
        """``haishui honcho peers map`` can repoint a host block's workspace; the cached agent's manager
        ...
        cfg_path.write_text(json.dumps({**base, "hosts": {"haishui": {"workspace": "old"}}}))
        sig_old = provider.identity_signature()["workspace"]
        cfg_path.write_text(json.dumps({**base, "hosts": {"haishui": {"workspace": "new"}}}))
...
class TestProfilePeerUniqueness:
    """Each Haishui profile can pin to its own unique peerName.

    Profile cloning copies host blocks, but operators routinely diverge them
    afterwards (e.g. `haishui -p partner` pinned to a different person's peer).
