...
Override the image with ``HAISHUI_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("HAISHUI_TEST_IMAGE", "haishui-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``HAISHUI_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("HAISHUI_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged haishui user
# ---------------------------------------------------------------------------
#
# Background: every Haishui runtime path inside the container drops to UID
# 10000 (the ``haishui`` user) via ``s6-setuidgid haishui``. ``docker exec``
...
# fails (PermissionError swallowed) for haishui, so a test that ran as root
...
# helpers below run every probe under ``-u haishui`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: haishui).
