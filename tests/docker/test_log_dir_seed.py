"""Runtime smoke test for Docker $HAISHUI_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the haishui user after container boot.
...
registered later runs its log service as the dropped haishui user and
...
def test_logs_gateways_seeded_and_haishui_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by haishui after boot."""
    ...
    assert "logs=haishui" in r.stdout, (
        f"logs/ not owned by haishui: {r.stdout}"
    )
    assert "gateways=haishui" in r.stdout, (
        f"logs/gateways/ not owned by haishui: {r.stdout}"
    )
