...
Build the real image and verify the actual runtime behavior:
  ...
  3. --user <haishui-uid> (10000) is allowed (supported non-root start)
"""
...
def test_arbitrary_user_uid_rejected(
    ...
    assert "HAISHUI_UID" in combined or "PUID" in combined, (
...
def test_user_pinned_to_haishui_uid_works(
    built_image: str,
) -> None:
    """docker run --user 10000:10000 (the haishui UID) must be allowed.
    ...
    assert r.returncode == 0, (
        f"--user 10000:10000 (haishui UID) was rejected: {r.stderr[-500:]}"
    )
    assert "OK" in r.stdout