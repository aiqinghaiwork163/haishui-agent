...
def test_tini_compat_shim_exists(built_image: str) -> None:
    ...
    Hostinger's 'Haishui WebUI' catalog, NAS compose projects that keep
...
def test_entrypoint_is_dispatcher_not_tini(built_image: str) -> None:
    ...
    r2 = subprocess.run(
        ["docker", "run", "--rm", "--entrypoint", "sh", built_image, "-c",
         "grep -q 'exec /init /opt/haishui/docker/main-wrapper.sh' "
         "/opt/haishui/docker/entrypoint-dispatch.sh"],
        capture_output=True, text=True, timeout=60,
    )
