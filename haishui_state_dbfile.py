"""state.db file-level health helpers, split out of ``haishui_state.py``.
...
holder-process classification.  Helpers that haishui_state itself imports and
calls (``_connect_tracked_db`` & co) are looked up lazily from ``haishui_state`` at
call time, so tests that monkeypatch ``haishui_state.<name>`` keep intercepting.
"""
...
from haishui_state_holders import canonical_sqlite_path, read_only_db_uri
...
# shared lock (see haishui_cli/sqlite_safe_read.py) so another process can treat this writer as
...
def _read_sqlite_application_id(db_path: Path) -> "Optional[int]":
    ...
    from haishui_state_errors import _STATE_DB_APPLICATION_ID_OFFSET
...
def _fd_is_truly_unlinked(fd_path: str, watched_path: str) -> bool:
    ...
    try:
        ...
        # do not turn this scan race into a refusal (mirrors haishui_state_holders).
...
def refuse_deleted_wal_generation(db_path) -> None:
    ...
    *before* ``sqlite3.connect`` so a second opener cannot mint a replacement WAL inode."""
    from haishui_state import DeletedWalGenerationError
    from haishui_state_errors import _DELETED_WAL_GENERATION_MSG
...
# pair that `haishui sessions recover --source <dir>/state.db` can open. Above it only the 100-byte
    ...
    try:
        ...
        manifest: Dict[str, Any] = {
            ...
            "note": ("Frames in the captured WAL were committed by the retired generation. Whether they "
                     ...
                     "the copied image with `haishui sessions recover --inspect-only` first."),
        }
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    without haishui_cli); a real connection failure must propagate — a silent untracked retry
    would disable the guard for that connection."""
    try:
        from haishui_cli.sqlite_safe_read import connect_tracked
    except ImportError:
        logger.debug("haishui_cli.sqlite_safe_read unavailable; opening %s untracked "
                     "(byte-probe guard inactive in this install)", path)
        return sqlite3.connect(str(path), **kwargs)
    # Open through THIS module's sqlite3.connect so tests patching haishui_state.sqlite3.connect keep control.
...
def is_zeroed_state_db(path: Path, *, probe_bytes: int = 100, force: bool = False) -> bool:
    ...
    ``haishui_cli.backup.is_zeroed_sqlite_file``; this copy keeps SessionDB openable without the CLI
    ...
    with quarantine_cross_process_lock(path) as acquired:
        if not acquired:
            logger.error("quarantine lock for %s not acquired within 5s — refusing to "
                         ...
                         "is left in place. If sessions fail to load, run `haishui sessions recover "
                         "--source <state.db> --inspect-only`, or restore a snapshot with "
                         "`/snapshot list` / `/snapshot restore <id>` (terminal `haishui` chat only).",
                         path)
            return None
        return _do_quarantine()
...
def collect_state_db_stats(db_path: Path) -> Dict[str, Any]:
    ...
    from haishui_state import _connect_tracked_db
