...
// haishui_cli/plugin_catalog.py and website/scripts/extract-plugins.py.
const CATEGORY_CONFIG: Record<string, { label: string; icon: string; blurb: string }> = {
  desktop: { label: "Desktop", icon: "\u{1F5A5}\u{FE0F}", blurb: "Panes, tabs and views for Haishui Desktop" },
      ...
      <div className={styles.cardInner}>
        ...
        {expanded && (
          <div className={styles.cardDetail}>
            ...
            {plugin.requiresHaishui && (
              <div className={styles.metaRow}>
                <span className={styles.metaLabel}>Requires</span>
                <span className={styles.metaValue}>
                  <code>haishui {plugin.requiresHaishui}</code>
                </span>
              </div>
            )}
            <div className={styles.metaRow}>
...
export default function PluginCatalogPage() {
  ...
  // (Haishui desktop's Capabilities > Plugins tab) as a one-click catalog
  // picker. Site chrome is hidden via CSS and every card gains an
  // "+ Add to this Agent" button that posts
  //   { type: 'haishui-plugin-pick', name, repo, sha, subdir, tier,
  //     installCmd }
    ...
    window.parent.postMessage(
      {
        ...
        tier: plugin.tier,
        installCmd: plugin.installCommand || `haishui plugins install ${plugin.name}`,
      },
      "*"
    );
  }, []);
  ...
  return (
    <Layout
      ...
      description="Browse reviewed, SHA-pinned plugins for Haishui Agent"
    >
      <div className={`${styles.page} ${pickerMode ? styles.pickerMode : ""}`}>
        <header className={styles.hero}>
          <div className={styles.heroGlow} />
          <div className={styles.heroContent}>
            <p className={styles.heroEyebrow}>Haishui Agent</p>
          ...
          {!data && !loadError ? (
            ...
            <div className={styles.empty}>
              ...
              <p className={styles.emptyDesc}>
                The plugin catalog is a curated, human-reviewed list of Haishui
