...
| | |
|---|---|
| 来源 | 可选 — 通过 `haishui skills install official/mcp/fastmcp` 安装 |
...
| 相关 skill | [`haishui-agent`](/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-haishui-agent), [`mcporter`](/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- 除工具外还需暴露资源或 prompt
- 在接入 Haishui 或其他客户端之前，使用 FastMCP CLI 对服务器进行冒烟测试
- 将服务器安装到 Claude Code、Claude Desktop、Cursor 或类似的 MCP 客户端
- 为 HTTP 部署准备 FastMCP 服务器仓库

若服务器已存在且只需连接到 Haishui，请使用 `native-mcp`。若目标是对现有 MCP 服务器进行临时 CLI 访问而非构建新服务器，请使用 `mcporter`。
...
- 使用 `native-mcp` skill，在 `~/.haishui/config.yaml` 中配置服务器，或
...
服务器构建部分可能正确，但 Haishui 配置有误。加载 `native-mcp` skill 并在 `~/.haishui/config.yaml` 中配置服务器，然后重启 Haishui。
