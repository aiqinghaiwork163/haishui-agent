...
sidebar_position: 1
title: "消息网关"
description: "通过 Telegram、Discord、Slack、WhatsApp、Signal、SMS、Email、Home Assistant、Mattermost、Matrix、DingTalk、Yuanbao、Microsoft Teams、LINE、Webhooks 或任何兼容 OpenAI 的前端与 Haishui 对话 — 架构与配置概览"
---

# 消息网关

通过 Telegram、Discord、Slack、WhatsApp、Signal、SMS、Email、Home Assistant、Mattermost、Matrix、DingTalk、Feishu/Lark、WeCom、Weixin、BlueBubbles（iMessage）、QQ、Yuanbao、Microsoft Teams、LINE、ntfy 或浏览器与 Haishui 对话。网关是一个单一后台进程，连接所有已配置的平台，管理会话，运行 cron 任务，并传递语音消息。

完整的语音功能集——包括 CLI 麦克风模式、消息中的语音回复以及 Discord 语音频道对话——请参阅 [Voice Mode](../features/voice-mode.md) 和 [Use Voice Mode with Haishui](../../guides/use-voice-mode-with-haishui.md)。
...
在群聊、hooks 和自动化流程中，Haishui 支持显式的静默令牌。若 agent 的最终回复恰好只是一个受支持的令牌，网关会抑制外发投递，不向聊天发送任何内容。
...
静默只是投递层面的决定：Haishui 仍会把这条静默回复保存在会话记录中，因此对话依旧正常交替。失败的回合仍会以错误形式呈现；不会因为文本形似静默令牌而隐藏失败。
...
```bash
haishui gateway setup        # 交互式配置所有消息平台
```
...
```bash
haishui gateway              # 在前台运行
haishui gateway setup        # 交互式配置消息平台
haishui gateway install      # 安装为用户服务（Linux）/ launchd 服务（macOS）
sudo haishui gateway install --system   # 仅 Linux：安装开机启动的系统服务
haishui gateway start        # 启动默认服务
haishui gateway stop         # 停止默认服务
haishui gateway status       # 检查默认服务状态
haishui gateway status --system         # 仅 Linux：显式检查系统服务
```
...
| `/update` | 将 Haishui Agent 更新至最新版本 |
| `/help` | 显示可用命令 |
...
# 其他配对命令：
haishui pairing list          # 查看待审核和已批准的用户
haishui pairing revoke telegram 123456789  # 撤销访问权限
```
...
第一次在任意平台向繁忙的 agent 发送消息时，Haishui 会在繁忙确认中附加一行提示，说明该配置项（`"💡 First-time tip — …"`）。该提示每次安装只触发一次——由 `onboarding.seen.busy_input_prompt` 下的标志锁定。删除该键可再次看到提示。

如果你觉得繁忙确认消息过多——尤其是使用语音输入或快速连续发送消息时——可设置 `display.busy_ack_enabled: false`。你的输入仍会正常排队/引导/中断，只是聊天回复被静默。

## 工具进度通知

在 `~/.haishui/config.yaml` 中控制显示多少工具活动信息：
...
当运行后台会话的 agent 使用 `terminal(background=true)` 启动长时间运行的进程（服务器、构建等）时，网关可以向你的聊天推送状态更新。通过 `~/.haishui/config.yaml` 中的 `display.background_process_notifications` 控制：
...
haishui gateway start                 # 启动服务
haishui gateway stop                  # 停止服务
haishui gateway status                # 检查状态
journalctl --user -u haishui-gateway -f  # 查看日志
...
除非你确实有此需要，否则避免同时安装用户和系统网关单元。Haishui 检测到两者同时存在时会发出警告，因为 start/stop/status 行为会变得不明确。

:::info 多个安装
如果你在同一台机器上运行多个 Haishui 安装（使用不同的 `HAISHUI_HOME` 目录），每个安装都有自己的 systemd 服务名称。默认的 `~/.haishui` 使用 `haishui-gateway`；其他安装使用 `haishui-gateway-<hash>`。`haishui gateway` 命令会自动针对当前 `HAISHUI_HOME` 对应的正确服务。
:::

### macOS（launchd）

```bash
haishui gateway install               # 安装为 launchd agent
haishui gateway start                 # 启动服务
haishui gateway stop                  # 停止服务
haishui gateway status                # 检查状态
tail -f ~/.haishui/logs/gateway.log   # 查看日志
```

生成的 plist 文件位于 `~/Library/LaunchAgents/ai.haishui.gateway.plist`。它包含三个环境变量：
...
- **HAISHUI_HOME** — 将网关限定到你的 Haishui 安装。

:::tip 安装后 PATH 变更
launchd plist 是静态的——如果你在配置网关后安装了新工具（例如通过 nvm 安装新版 Node.js，或通过 Homebrew 安装 ffmpeg），请重新运行 `haishui gateway install` 以捕获更新后的 PATH。网关会检测到过时的 plist 并自动重新加载。
:::

:::info 多个安装
与 Linux systemd 服务类似，每个 `HAISHUI_HOME` 目录都有自己的 launchd 标签。默认的 `~/.haishui` 使用 `ai.haishui.gateway`；其他安装使用 `ai.haishui.gateway-<suffix>`。
:::
...
| 平台 | 工具集 | 功能 |
|----------|---------|--------------|
| CLI | `haishui-cli` | 完全访问 |
| Telegram | `haishui-telegram` | 完整工具，包括终端 |
| Discord | `haishui-discord` | 完整工具，包括终端 |
| WhatsApp | `haishui-whatsapp` | 完整工具，包括终端 |
| Slack | `haishui-slack` | 完整工具，包括终端 |
| Google Chat | `haishui-google_chat` | 完整工具，包括终端 |
| Signal | `haishui-signal` | 完整工具，包括终端 |
| SMS | `haishui-sms` | 完整工具，包括终端 |
| Email | `haishui-email` | 完整工具，包括终端 |
| Home Assistant | `haishui-homeassistant` | 完整工具 + HA 设备控制（ha_list_entities、ha_get_state、ha_call_service、ha_list_services） |
| Mattermost | `haishui-mattermost` | 完整工具，包括终端 |
| Matrix | `haishui-matrix` | 完整工具，包括终端 |
| DingTalk | `haishui-dingtalk` | 完整工具，包括终端 |
| Feishu/Lark | `haishui-feishu` | 完整工具，包括终端 |
| WeCom | `haishui-wecom` | 完整工具，包括终端 |
| WeCom Callback | `haishui-wecom-callback` | 完整工具，包括终端 |
| Weixin | `haishui-weixin` | 完整工具，包括终端 |
| BlueBubbles | `haishui-bluebubbles` | 完整工具，包括终端 |
| QQBot | `haishui-qqbot` | 完整工具，包括终端 |
| Yuanbao | `haishui-yuanbao` | 完整工具，包括终端 |
| Microsoft Teams | `haishui-teams` | 完整工具，包括终端 |
| API Server | `haishui-api-server` | 完整工具（去除 `clarify`、`send_message`、`text_to_speech`——程序化访问没有交互用户） |
| Webhooks | `haishui-webhook` | 完整工具，包括终端 |
...
1. **网关日志**（`~/.haishui/logs/gateway.log` 或 systemd / launchd 单元日志）。搜索平台名称以及 `circuit breaker`、`paused` 或 `disabled`。触发事件包含失败次数和最后一个错误。
