...
* **宿主持有凭据。** OAuth token、刷新流程、凭据池、每任务辅助覆盖——Haishui 已有的所有凭据概念均适用。Plugin 永远看不到 token；宿主通过 `result.audit` 将调用归因回溯。
...
第三个完整示例（包含图像输入）位于
[`haishui-example-plugins`](https://github.com/NousResearch/haishui-example-plugins/tree/main/plugin-llm-example)
仓库（参考 plugin 的配套仓库——不随 haishui-agent 本体打包）。关于异步接口（`acomplete()` / `acomplete_structured()` 与 `asyncio.gather()` 配合使用），请参见同一仓库中的
[`plugin-llm-async-example`](https://github.com/NousResearch/haishui-example-plugins/tree/main/plugin-llm-async-example)。
...
result = ctx.llm.complete(
    ...
    provider=None,         # 可选，受门控——Haishui provider id（如 "openrouter"）
...
Plugin 可以为自己注册的任务提供 provider/模型注册默认值。运营人员在 `auxiliary.<task>` 中的配置会覆盖这些默认值，并控制部署选择。Plugin 只能使用自己注册的任务；未知或属于其他 plugin 的任务名会在调用 provider 前失败。`allow_task_override: true` 是运营人员显式授予的权限，用于使用 Haishui 内置辅助任务；它不允许使用其他 plugin 的任务。省略 `task=`（或使用 `"auto"`）会继续使用当前主 provider/模型。
...
plugins:
  entries:
    my-plugin:
      llm:
        # 允许此 plugin 选择不同的 Haishui provider
        # （必须是 Haishui 已知的 provider——与
        # `haishui model` 和 config.yaml model.provider 中的名称相同）
...
* **认证。** 从 `~/.haishui/auth.json` / 环境变量中提取 API 密钥、OAuth token 或刷新 token，包括配置了凭据池时的处理。Plugin 永远看不到这些内容。
* **视觉路由。** 当提供图像输入而用户当前激活的文本模型仅支持文本时，宿主自动回退到已配置的视觉模型。
* **回退链。** 若用户主 provider 返回 5xx 或 429，请求在向 plugin 返回错误前会经过 Haishui 常规的聚合器感知回退流程。
...
现有 `ctx.*` 方法各自扩展一个已有的 Haishui 子系统：
...
* 实现：[`agent/plugin_llm.py`](https://github.com/aiqinghaiwork163/haishui-agent/blob/main/agent/plugin_llm.py)
* 测试：[`tests/agent/test_plugin_llm.py`](https://github.com/aiqinghaiwork163/haishui-agent/blob/main/tests/agent/test_plugin_llm.py)
* 参考 plugin（配套仓库）：
  * [`plugin-llm-example`](https://github.com/NousResearch/haishui-example-plugins/tree/main/plugin-llm-example) — 带图像输入的同步结构化提取
  * [`plugin-llm-async-example`](https://github.com/NousResearch/haishui-example-plugins/tree/main/plugin-llm-async-example) — 使用 `asyncio.gather()` 的异步示例
* 辅助客户端（底层引擎）：参见
