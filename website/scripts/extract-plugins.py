...
- ``plugin-catalog.json`` — ``{"entries": [raw YAML mappings], "removed": [...]}`` in the loader's own
  schema; installed Haishui clients fetch this for live catalog refresh
  (``haishui_cli.plugin_catalog.LIVE_CATALOG_URL``) so new entries and removals reach them without
...
def load_catalog_entries(catalog_dir: Path, stars: dict[str, int] | None = None) -> list[dict]:
    ...
    for path in sorted(catalog_dir.glob("*.yaml")):
        ...
        entries.append({
            ...
            "requiresHaishui": str(raw.get("requires_haishui") or "").strip(),
            ...
            "installCommand": f"haishui plugins install {name}",
