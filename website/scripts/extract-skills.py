...
2. The unified Haishui Skills Index at ``website/static/api/skills-index.json``,
    ...
    for base_dir, source_label in LOCAL_SKILL_DIRS:
        ...
        for root, _dirs, files in os.walk(base_path):
            ...
            if isinstance(metadata, dict):
                haishui_meta = metadata.get("haishui", {})
                if isinstance(haishui_meta, dict):
                    tags = haishui_meta.get("tags", [])
            if not tags:
                tags = fm.get("tags", [])
