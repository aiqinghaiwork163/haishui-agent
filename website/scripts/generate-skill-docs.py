...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/aiqinghaiwork163/haishui-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    haishui_meta = (fm.get("metadata") or {}).get("haishui") or {}
    tags = haishui_meta.get("tags") or []
    related = haishui_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `haishui skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Haishui Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Haishui ships with a large built-in skill library copied into `~/.haishui/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Haishui also syncs bundled skills on `haishui update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.haishui/skills/` tree, it is still shipped with Haishui; restore it with `haishui skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with haishui-agent — install via haishui skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with haishui-agent under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
        "haishui skills install official/<category>/<skill>",
        "```",
        "",
        "For example:",
        "",
        "```bash",
        "haishui skills install official/blockchain/solana",
        "haishui skills install official/mlops/flash-attention",
        "```",
        "",
        ...
        "",
        "```bash",
        "haishui skills uninstall <skill-name>",
        "```",
        "",
    ]
    for category in sorted(by_cat):
