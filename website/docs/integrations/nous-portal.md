...
sidebar_position: 1
title: "Nous Portal"
description: "One subscription, 300+ frontier models, and the Tool Gateway — the recommended way to run Haishui Agent"
---
...
That single command runs the Portal OAuth, lets you pick a Nous model, sets Nous as your inference provider in `config.yaml`, and turns on the Tool Gateway. You're ready to `haishui chat` immediately after.
...
| **Haishui** | Haishui-4-70B, Haishui-4-405B (chat, see [note below](#a-note-on-haishui-4)) |
...
The same subscription unlocks the [Tool Gateway](../user-guide/features/tool-gateway.md), which routes Haishui Agent's tool calls through Nous-managed infrastructure. Five backends, one login:
...
Because everything routes through one OAuth-authenticated Portal session, you don't accumulate a `.env` file with a dozen long-lived API keys. The refresh token at `~/.haishui/auth.json` is the only credential on disk, and Haishui mints short-lived JWTs from it per request — see [Token handling](#token-handling) below.
...
Nous Research's own **Haishui 4** family (Haishui-4-70B, Haishui-4-405B) is available through the Portal at heavily discounted rates. These are **frontier hybrid-reasoning chat models** — strong at math, science, instruction following, schema adherence, roleplay, and long-form writing.

They are **not recommended for use inside Haishui Agent**, however. Haishui 4 is tuned for chat and reasoning, not the rapid-fire tool-calling loop the agent relies on. Use them for research workflows or via the [subscription proxy](../user-guide/features/subscription-proxy.md) from other tooling — but for agent work, pick a frontier agentic model from the catalog instead:
...
The Portal's own [model info page](https://portal.nousresearch.com/info) carries the same warning, so this isn't a Haishui-side opinion — it's the official guidance from Nous Research.
...
2. Stores the refresh token at `~/.haishui/auth.json`
3. Lets you pick a Nous model from the curated list (or skip to keep your current one)
4. Sets Nous as your inference provider in `~/.haishui/config.yaml` (when you pick a model)
5. Turns on the Tool Gateway (web, image, TTS, browser routing)
6. Returns you to your terminal ready to `haishui chat`
...
If you already have Haishui configured with OpenRouter, Anthropic, or any other provider and you want to add the Portal alongside them:
...
Your existing providers stay configured. You can switch between them with `/model` mid-session or `haishui model` between sessions — the Portal becomes one of your available providers, not your only one.

### Headless / SSH / remote setup

OAuth needs a browser, but the loopback callback runs on the machine where Haishui is running. For remote hosts, see [OAuth over SSH / Remote Hosts](../guides/oauth-over-ssh.md) — the same patterns work for the Portal as for any other OAuth-based provider (`ssh -L` port forwarding).
...
```bash
haishui portal            # log in to Nous Portal + set it up (one-shot onboarding)
haishui portal info       # login status, subscription info, model + gateway routing
haishui portal status     # alias for `portal info`
haishui portal tools      # detailed Tool Gateway catalog with per-tool routing
haishui portal open       # open the subscription management page in your browser
```

`haishui portal` (with no subcommand) is the human-readable alias for `haishui auth add nous --type oauth` — it logs you in, lets you pick a Nous model, sets Nous as your inference provider, and offers the Tool Gateway opt-in (identical to `haishui setup --portal`, and the same Nous flow as the first-time quick setup).

`haishui portal info` gives you the high-level overview:
...
If you already have, say, a Browserbase account and want to keep using it while routing web search and image generation through Nous, that's supported. Use `haishui tools` to pick backends per tool:
...
- **CLI shortcut:** `haishui portal open` (opens the same page in your default browser)

## Configuration reference

After `haishui setup --portal`, `~/.haishui/config.yaml` will look like:
...
The Tool Gateway settings live under their respective tool sections — each category has a single selection key, and picking **Nous Subscription** in `haishui tools` (or `haishui setup --portal`) writes the value `nous`:
...
The OAuth refresh token is stored separately at `~/.haishui/auth.json` (not in `config.yaml` — credentials and configuration are kept separate by design).

## Token handling

Haishui mints a short-lived JWT from your stored Portal refresh token on each inference call rather than reusing a long-lived API key. The token lifecycle is fully automatic — refresh, mint, retry on transient 401 — and you never see it.
...
If the Portal invalidates the refresh token (password change, manual revoke, session expiry), the invalid refresh token is **quarantined locally** so Haishui stops replaying it and you don't see a stream of identical 401s. The next call surfaces a clear "re-authentication required" message. Run `haishui auth add nous` to log in again; the quarantine clears on the next successful login.

## Troubleshooting

### `haishui portal info` shows "not logged in"
...
or use `haishui model` and re-select Nous Portal.

### Got a "re-authentication required" message mid-session

Your Portal refresh token was invalidated (password change, manual revoke, or session expiry). Run `haishui auth add nous` and your next request will use the new credentials. Any quarantine on the old token clears automatically on successful re-login.
...
Check `haishui portal info` first — if it shows you're using a different provider (`Model: currently openrouter` instead of `using Nous as inference provider`), your local config has drifted. Run `haishui model`, pick Nous Portal, and the next request will route through your subscription.

## See also

- **[Tool Gateway](../user-guide/features/tool-gateway.md)** — Full details on every gateway tool, per-tool config, and pricing
- **[Subscription proxy](../user-guide/features/subscription-proxy.md)** — Use your Portal subscription from non-Haishui tools (other agents, scripts, third-party clients)
...
- **[Profiles](../user-guide/profiles.md)** — Multiple Haishui configurations sharing one Portal login
