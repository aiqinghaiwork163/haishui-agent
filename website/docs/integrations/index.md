...
Haishui Agent connects to external systems for AI inference, tool servers, IDE workflows, programmatic access, and more. These integrations extend what Haishui can do and where it can run.
...
Haishui supports multiple AI inference providers out of the box. Use `haishui model` to configure interactively, or set them in `config.yaml`.

- **[AI Providers](./providers.md)** — OpenRouter, Anthropic, OpenAI, Google, and any OpenAI-compatible endpoint. Haishui auto-detects capabilities like vision, streaming, and tool use per provider.
...
- **[MCP Servers](../user-guide/features/mcp.md)** — Connect Haishui to external tool servers via Model Context Protocol. Access tools from GitHub, databases, file systems, browser stacks, internal APIs, and more without writing native Haishui tools. Supports both stdio and SSE transports, per-server tool filtering, and capability-aware resource/prompt registration.

## Web Search Backends

The `web_search` and `web_extract` tools support eight backend providers, configured via `config.yaml` or `haishui tools`:
...
Haishui includes full browser automation with multiple backend options for navigating websites, filling forms, and extracting information:
...
- **[IDE Integration (ACP)](../user-guide/features/acp.md)** — Use Haishui Agent inside ACP-compatible editors such as VS Code, Zed, and JetBrains. Haishui runs as an ACP server, rendering chat messages, tool activity, file diffs, and terminal commands inside your editor.

## Programmatic Access

- **[API Server](../user-guide/features/api-server.md)** — Expose Haishui as an OpenAI-compatible HTTP endpoint. Any frontend that speaks the OpenAI format — Open WebUI, LobeChat, LibreChat, NextChat, ChatBox — can connect and use Haishui as a backend with its full toolset.
...
Haishui runs as a gateway bot on 27+ messaging platforms, all configured through the same `gateway` subsystem:
...
| **Slack** | [api.slack.com/apps?new_app=1](https://api.slack.com/apps?new_app=1) | The **Create New App** dialog — pick *From an app manifest* and paste the manifest `haishui slack manifest --agent-view` generates |
...
- **[Buzz](./buzz.md)** — Block's Nostr-based human+agent workspace. Three integration paths: Buzz Desktop spawns Haishui as a managed ACP runtime, the `buzz-acp` relay bridge hosts a Haishui identity server-side, or the native gateway platform joins Buzz channels with full Haishui memory/skills/approvals/cron. The overview page compares all three.
...
- **[Plugin System](../user-guide/features/plugins.md)** — Extend Haishui with custom tools, lifecycle hooks, and CLI commands without modifying core code. Plugins are discovered from `~/.haishui/plugins/`, project-local `.haishui/plugins/`, and pip-installed entry points.
- **[Build a Plugin](../developer-guide/plugins/index.md)** — Step-by-step guide for creating Haishui plugins with tools, hooks, and CLI commands.
