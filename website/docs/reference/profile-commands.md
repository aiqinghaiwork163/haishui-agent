...
This page covers all commands related to [Haishui profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `haishui profile` without a subcommand shows help.
...
```bash
haishui profile use <name>
```

Sets `<name>` as the active profile. All subsequent `haishui` commands (without `-p`) will use this profile.
...
haishui profile use default
```

## `haishui profile create`

```bash
haishui profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `haishui profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `haishui update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.haishui`), use `haishui skills opt-out` / `haishui skills opt-in`. |
...
haishui profile create work2 --clone-from work

# Clone everything from a specific profile
haishui profile create work2-backup --clone-from work --clone-all
```

## `haishui profile describe`

```bash
haishui profile describe [<name>] [options]
```
...
haishui profile describe researcher

# Set it explicitly
haishui profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
haishui profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
haishui profile describe --all --auto
```

## `haishui profile delete`

```bash
haishui profile delete <name> [options]
```
...
```bash
haishui profile delete mybot
haishui profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.haishui`) cannot be deleted — use `haishui uninstall` to remove everything.
:::

## `haishui profile show`

```bash
haishui profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

The skills count here (and in `haishui profile list`) is counted on the spot. The Desktop and dashboard profile lists are polled every few seconds, so they show the last known count instead and refresh it in the background — a freshly started backend may briefly show `0` skills for a profile until the first background count lands, and a skill you just installed appears in those lists within about a minute.

This shows the profile's Haishui home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ haishui profile show work
Profile: work
Path:    ~/.haishui/profiles/work
...
## `haishui profile alias`

```bash
haishui profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Haishui installation.
...
```bash
haishui profile alias work
# Creates/updates ~/.local/bin/work

haishui profile alias work --name mywork
# Creates ~/.local/bin/mywork

haishui profile alias work --remove
# Removes the wrapper script
```

## `haishui profile rename`

```bash
haishui profile rename <old-name> <new-name>
```
...
```bash
haishui profile rename mybot assistant
# ~/.haishui/profiles/mybot → ~/.haishui/profiles/assistant
...
after the rename; `haishui profile migrate-identity` retries that step too if it was reported as
failed.

## `haishui profile migrate-identity`

```bash
haishui profile migrate-identity <old-name> <new-name>
```

Retries the identity migration of a rename that already completed. Run it if `haishui profile
...
#     haishui profile migrate-identity mybot assistant

haishui profile migrate-identity mybot assistant
# ✓ Session/routing identity migrated: mybot → assistant
```

## `haishui profile purge-identity`

```bash
haishui profile purge-identity <name>
```

Retries the identity purge of a delete that already completed. Run it if `haishui profile delete`
...
is decided by `haishui profile delete`, which removes the profile's own `profiles/<name>/`, its
...
#   Restart the gateway, then run:
#     haishui profile purge-identity mybot

haishui profile purge-identity mybot
# ✓ Session/routing identity purged: mybot
```

## `haishui profile export`

```bash
haishui profile export <name> [options]
```
...
```bash
haishui profile export work
# Creates work.tar.gz in the current directory

haishui profile export work -o ./work-2026-03-29.tar.gz
```
...
## `haishui profile import`

```bash
haishui profile import <archive> [options]
```
...
```bash
haishui profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

haishui profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `haishui -p telemetry`). |
...
haishui profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
haishui profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
haishui profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
haishui profile install ./telemetry/
```

### `haishui profile update`

```bash
haishui profile update <name> [--force-config] [--yes]
```
...
### `haishui profile info`

```bash
haishui profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Haishui version, author, env var requirements, the source URL/path, and
...
`haishui profile list` also shows the distribution name and version in a
`Distribution` column, and `haishui profile show <name>` / `delete <name>`
...
haishui profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
haishui profile install https://github.com/your-org/internal-assistant.git
```
...
`haishui_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Haishui version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Haishui can clone from.
3. Tell recipients to run `haishui profile install <your-repo-url>`.
...
## `haishui -p` / `haishui --profile`

```bash
haishui -p <name> <command> [options]
haishui --profile <name> <command> [options]
```

Global flag to run any Haishui command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
haishui -p work chat -q "Check the server status"
haishui --profile dev gateway start
haishui -p personal skills list
haishui -p work config edit
```

## `haishui completion`

```bash
haishui completion <shell>
```
...
- `haishui profile <TAB>` — subcommands (list, use, create, etc.)
- `haishui profile use <TAB>` — profile names
- `haishui -p <TAB>` — profile names
