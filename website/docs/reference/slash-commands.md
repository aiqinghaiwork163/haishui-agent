...
Haishui has two slash-command surfaces, both driven by a central `COMMAND_REGISTRY` in `haishui_cli/commands.py`:
...
| `/goal <text>` | Set a standing goal Haishui works toward across turns — our take on the Ralph loop. After each turn an auxiliary judge model decides whether the goal is done; if not, Haishui auto-continues. Subcommands: `/goal status`, `/goal pause`, `/goal resume`, `/goal clear`. Budget defaults to 20 turns (`goals.max_turns`); any real user message preempts the continuation loop, and state survives `/resume`. See [Persistent Goals](../user-guide/features/goals.md) for the full walkthrough. |
...
| `/export [profile] [-o out.tar.gz]` | **CLI only.** Pack a profile into a shareable `.tar.gz` — skills, memory, persona, crons, plugins, settings, and (from the desktop) themes and layout. Credentials (`auth.json`, `.env`) are stripped. Defaults to the active profile and `<name>.tar.gz` in the current directory. Same archive as `haishui profile export`; for a versioned, updatable share use a [profile distribution](../user-guide/profile-distributions.md) instead. |
...
| `/busy [queue\|steer\|interrupt\|status]` | Control what happens when you message while Haishui is working — queue the new message, steer mid-turn, or interrupt immediately. Works in the CLI and messaging gateway. |
...
| `/wake [on\|off\|status]` | CLI-only: toggle the "Hey Haishui" wake word listener. |
...
| `/bundles` | List configured skill bundles — `/<name>` slash aliases that preload several skills at once. Configure under `bundles:` in `~/.haishui/config.yaml`. See [Skill Bundles](../user-guide/features/skills.md#skill-bundles). |
...
| `/plan [task]` | Write a markdown implementation plan to `.haishui/plans/` in the active workspace — planning only, no execution. Empty argument infers the task from the conversation. (Formerly the bundled `plan` skill; now built-in so it survives the Telegram/Discord command-menu caps.) |
...
| `/reload-skills` (alias: `/reload_skills`) | Re-scan `~/.haishui/skills/` for newly installed or removed skills |
...
| `/version` | Show Haishui Agent version, build, and environment info. |
...
User-defined quick commands map a short slash command to either a shell command or another slash command. Configure them in `~/.haishui/config.yaml`:
...
Define your own short names for models you use often, then reach them with `/model <alias>` in a running session, `haishui chat --model <alias>` at startup, or any messaging platform. Aliases work identically in these paths, on session-only (default) and `--global` switches.

Two config formats are supported:

**Full form** — pin an exact model, provider, and optionally a base URL. Put this in `~/.haishui/config.yaml`:
...
> Slack itself blocks native slash commands inside message threads ("/queue is not supported in threads. Sorry!") and never delivers them to Haishui. Inside a Slack thread, use the `!` prefix instead — `!stop`, `!new`, `!status` — and the gateway dispatches it exactly like the slash form. `@Haishui !stop` and `@Haishui /stop` work in threads too. Only the first token is checked against the known command list, so messages like `!nice work` pass through to the agent unchanged. See [Using commands inside threads](../user-guide/messaging/slack.md#using-commands-inside-threads-the-cmd-prefix) for details.
...
| `/login` | Sign in with a Nous account. **Paired direct messages only** — in a group, channel, or broadcast-shaped platform Haishui refuses. On Slack use `/haishui login`. |
| `/whoami` | Show your slash command access level (admin / user). |
...
| `/diff [staged\|all\|session] [--stat]` | Show git changes in the working directory (fenced and truncated to platform message limits). `session` shows the cumulative diff of everything Haishui changed; `--stat` shows just the summary. |
...
| `/goal <text>` | Set a standing goal Haishui works toward across turns — our take on the Ralph loop. A judge model checks after each turn; if not done, Haishui auto-continues until it is, you pause/clear it, or the turn budget (default 20) is hit. Subcommands: `/goal status`, `/goal pause`, `/goal resume`, `/goal clear`. Safe to run mid-agent for status/pause/clear; setting a new goal requires `/stop` first. See [Persistent Goals](../user-guide/features/goals.md). |
| `/subgoal <text>` | Append criteria to the active `/goal` mid-loop (`/subgoal`, `/subgoal remove <N>`, `/subgoal clear`). |
| `/heartbeat every <interval> <prompt>` (alias: `/hb`) | Set a recurring prompt that re-enters this session when idle. Subcommands: `status`, `pause`, `resume`, `clear`. On Slack use `/haishui heartbeat …`. |
...
| `/refine [focus]` | Run the memory/skill self-improvement review now, optionally with focus instructions. On Slack use `/haishui refine …`. |
| `/review [instructions]` | Spawn an independent reviewer subagent for the work just discussed (PR, code, docs); its review re-enters this chat when done. On Slack use `/haishui review …`. |
...
| `/plan [task]` | Write a markdown implementation plan to `.haishui/plans/`; no execution. |
...
| `/skills [pending\|approve\|reject\|diff\|approval]` | Review pending **skill** writes staged by the write-approval gate (`skills.write_approval`). Shows a one-line gist per staged write; `/skills diff <id>` is truncated for chat — read the full diff on the CLI or in `~/.haishui/pending/skills/<id>.json`. Only appears when the gate is on (or staged writes remain); search/install stay CLI-only. |
...
- In the TUI, `/sessions` shows live sessions in the current TUI process. Use `/resume [name]` or `haishui --tui --resume <id-or-title>` for saved or closed transcripts.
...
**Inline skip:** append `now`, `--yes`, or `-y` to bypass the modal for a single invocation — e.g. `/reset now`, `/new --yes my-session`, `/clear -y`, `/undo -y`. Useful when the modal doesn't render correctly on your terminal (see [issue #30768](https://github.com/aiqinghaiwork163/haishui-agent/issues/30768) for native Windows PowerShell) or when scripting against the CLI.

Set `approvals.destructive_slash_confirm: false` in `~/.haishui/config.yaml` to disable the prompts globally; set it back to `true` to re-enable. See [Security — Destructive slash command confirmation](../user-guide/security.md#dangerous-command-approval) for context.
