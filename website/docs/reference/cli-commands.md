...
description: "Authoritative reference for Haishui terminal commands and command families"
---
...
```bash
haishui [global-options] <command> [subcommand/options]
```
...
| `--profile <name>`, `-p <name>` | Select which Haishui profile to use for this invocation. Overrides the sticky default set by `haishui profile use`. |
...
| `--ignore-user-config` | Ignore `~/.haishui/config.yaml` and fall back to built-in defaults. Credentials in `.env` are still loaded. |
| `--ignore-rules` | Skip auto-injection of `AGENTS.md`, `SOUL.md`, `.cursorrules`, memory, and preloaded skills. |
| `--tui` | Launch the [TUI](../user-guide/tui.md) instead of the classic CLI. Equivalent to `HAISHUI_TUI=1`. Always wins over `display.interface`. |
...
| Command | Purpose |
|---------|---------|
| `haishui chat` | Interactive or one-shot chat with the agent. |
| `haishui model` | Interactively choose the default provider and model. |
| `haishui moa` | Configure named Mixture of Agents presets selectable from the model picker. |
| `haishui fallback` | Manage fallback providers tried when the primary model errors. |
| `haishui gateway` | Run or manage the messaging gateway service. |
| `haishui proxy` | Local OpenAI-compatible proxy that attaches OAuth provider credentials. See [Subscription Proxy](../user-guide/features/subscription-proxy.md). |
| `haishui egress` | Outbound credential-injection firewall for remote terminal sandboxes (iron-proxy). Disabled by default. See [Egress proxy](../user-guide/egress/iron-proxy.md). |
| `haishui lsp` | Manage Language Server Protocol integration (semantic diagnostics for write_file/patch). |
| `haishui setup` | Interactive setup wizard for all or part of the configuration. |
| `haishui whatsapp` | Configure and pair the WhatsApp bridge. |
| `haishui whatsapp-cloud` | Configure the official Meta WhatsApp Business Cloud API adapter (Business account + public webhook required). Distinct from `haishui whatsapp` (Baileys personal-account bridge). |
| `haishui slack` | Slack helpers (currently: generate the app manifest with every command as a native slash). |
| `haishui auth` | Manage credentials — add, list, remove, reset, status, logout. Handles OAuth flows for Codex/Nous/Anthropic. |
| `haishui login` / `logout` | **Deprecated** — use `haishui auth` instead. |
| `haishui send` | Send a one-shot message to a configured messaging platform (Telegram, Discord, Slack, Signal, SMS, …). Useful from shell scripts, cron jobs, CI hooks, and monitoring daemons — no agent loop, no LLM. |
| `haishui peer` | Register peer Haishui gateways on other machines and DM their agents' canonical Bot Chats (`haishui peer dm <peer>[/<agent>] "…"`). The transport behind cross-machine bot-to-bot messaging. |
| `haishui secrets` | Manage external secret sources (currently Bitwarden Secrets Manager) for pulling API keys at process startup instead of from `~/.haishui/.env`. |
| `haishui migrate` | Diagnose and (optionally) rewrite `config.yaml` to replace references to retired models or deprecated settings (e.g. `migrate xai`). |
| `haishui status` | Show agent, auth, and platform status. |
| `haishui cron` | Inspect and tick the cron scheduler. |
| `haishui pause` / `haishui resume` | Global emergency stop: no new cron fires (built-in ticker, managed-cron webhook, misfire catch-up), kanban dispatch or gateway turns start until resumed; in-flight work is never killed. |
| `haishui kanban` | Multi-profile collaboration board (tasks, links, dispatcher). |
| `haishui project` | Manage named, multi-folder workspaces (projects). Anchors desktop session grouping and, when bound to a kanban board, gives tasks a deterministic worktree + branch convention. State is per-profile. |
| `haishui webhook` | Manage dynamic webhook subscriptions for event-driven activation. |
| `haishui hooks` | Inspect, approve, or remove shell-script hooks declared in `config.yaml`. |
| `haishui doctor` | Diagnose config and dependency issues. |
| `haishui security audit` | On-demand supply-chain audit (OSV.dev) for the venv, plugin requirements, and pinned MCP servers. |
| `haishui approvals` | Approval-prompt tools — mine approval history into allowlist proposals. |
| `haishui dump` | Copy-pasteable setup summary for support/debugging. |
| `haishui prompt-size` | Show a byte breakdown of the system prompt + tool schemas (skills index, memory, profile). Runs offline. |
| `haishui debug` | Debug tools — upload logs and system info for support. |
| `haishui backup` | Back up Haishui home directory to a zip file. |
| `haishui checkpoints` | Inspect / prune / clear `~/.haishui/checkpoints/` (the shadow store used by `/rollback`). Run with no args for a status overview. |
| `haishui import` | Restore a Haishui backup from a zip file. |
| `haishui logs` | View, tail, and filter agent/gateway/error log files. |
| `haishui config` | Show, edit, migrate, and query configuration files. |
| `haishui skin` | List, switch, and tweak display skins. |
| `haishui console` | Open the safe Haishui command console. |
| `haishui pairing` | Approve or revoke messaging pairing codes. |
| `haishui skills` | Browse, install, publish, audit, and configure skills. |
| `haishui bundles` | Group several skills under a single `/<name>` slash command. See [Skill Bundles](../user-guide/features/skills.md#skill-bundles). |
| `haishui curator` | Background skill maintenance — status, run, pause, pin. See [Curator](../user-guide/features/curator.md). |
| `haishui journey` (aliases `learning`, `memory-graph`) | Timeline of learned skills + memories over time. |
| `haishui memory` | Configure external memory provider. Plugin-specific subcommands (e.g. `haishui honcho`) register automatically when their provider is active. |
| `haishui acp` | Run Haishui as an ACP server for editor integration. |
| `haishui mcp` | Manage MCP server configurations and run Haishui as an MCP server. |
| `haishui plugins` | Manage Haishui Agent plugins (install, enable, disable, remove). |
| `haishui portal` | Nous Portal status, subscription link, and Tool Gateway routing. See [Tool Gateway](../user-guide/features/tool-gateway.md). |
| `haishui tools` | Configure enabled tools per platform. |
| `haishui computer-use` | Install or check the Computer Use (cua-driver) backend (macOS/Windows/Linux). |
| `haishui pets` | Browse, install, and select [petdex](../user-guide/features/pets.md) animated pets shown across the CLI, TUI, and desktop app. Subcommands: `list`, `install`, `select`, `show`, `off`, `scale`, `remove`, `doctor`. |
| `haishui sessions` | Browse, export, prune, rename, and delete sessions. |
| `haishui insights` | Show token/cost/activity analytics. |
| `haishui claw` | OpenClaw migration helpers. |
| `haishui import-agent` | Import a Claude Code (`~/.claude`) or Codex CLI (`~/.codex`) setup. |
| `haishui dashboard` | Launch the web dashboard for managing config, API keys, and sessions. |
| `haishui serve` | Start the Haishui backend server (headless; powers the desktop app and remote backends). |
| `haishui desktop` (alias `gui`) | Build and launch the native Electron desktop app. |
| `haishui profile` | Manage profiles — multiple isolated Haishui instances. |
| `haishui completion` | Print shell completion scripts (bash/zsh/fish). |
| `haishui --version` | Show version information. |
| `haishui update` | Pull latest code and reinstall dependencies. `--check` previews without installing; `--backup` takes a pre-pull `HAISHUI_HOME` snapshot. |
| `haishui uninstall` | Remove Haishui from the system. |

## `haishui chat`

```bash
haishui chat [options]
```
...
| `--ignore-user-config` | Ignore `~/.haishui/config.yaml` and use built-in defaults. Credentials in `.env` are still loaded. Useful for isolated CI runs, reproducible bug reports, and third-party integrations. |
| `--ignore-rules` | Skip auto-injection of `AGENTS.md`, `SOUL.md`, `.cursorrules`, persistent memory, and preloaded skills. Combine with `--ignore-user-config` for a fully isolated run. |
| `--safe-mode` | Troubleshooting mode: disable ALL customizations — user config, rules/memory injection, plugins, shell hooks, and MCP servers (implies `--ignore-user-config` and `--ignore-rules`). Use to isolate whether a problem comes from your setup or from Haishui itself. |
...
```bash
haishui
haishui chat -q "Summarize the latest PRs"          # seeds an interactive session
haishui chat --oneshot -q "Summarize the latest PRs"  # answer and exit
haishui chat --provider openrouter --model anthropic/claude-sonnet-4.6
haishui chat --toolsets web,terminal,skills
haishui chat --quiet -q "Return only JSON"
haishui chat -q "Inspect this repository" --format stream-json
haishui chat --worktree -q "Review this repo and open a PR"
haishui chat --ignore-user-config --ignore-rules -q "Repro without my personal setup"
haishui chat --safe-mode -q "Is this bug mine or Haishui'?"
```
...
```bash
haishui chat -q "Summarize this repository" --format stream-json
```
...
(`HAISHUI_KANBAN_TASK` set) whose turn failed only because the provider was
...
### `haishui -z <prompt>` — scripted one-shot

For programmatic callers (shell scripts, CI, cron, parent processes piping in a prompt), `haishui -z` is the purest one-shot entry point: **single prompt in, final response text out, nothing else on stdout or stderr.** No banner, no spinner, no tool previews, no `Session:` line — just the agent's final reply as plain text.

```bash
haishui -z "What's the capital of France?"
# → Paris.

# Parent scripts can cleanly capture the response:
answer=$(haishui -z "summarize this" < /path/to/file.txt)
```

Per-run overrides (no mutation to `~/.haishui/config.yaml`):

| Flag | Equivalent env var | Purpose |
|---|---|---|
| `-m` / `--model <model>` | `HAISHUI_INFERENCE_MODEL` | Override the model for this run |
...
```bash
haishui -z "…" --provider openrouter --model openai/gpt-5.5
# or:
HAISHUI_INFERENCE_MODEL=anthropic/claude-sonnet-4.6 haishui -z "…"
```

Same agent, same tools, same skills — just strips every interactive / cosmetic layer. If you need tool output in the transcript too, use `haishui chat --oneshot -q` instead; `-z` is explicitly for "I only want the final answer".
...
`haishui -z "…" --usage-file /path/report.json` writes a machine-readable usage report after the run: `estimated_cost_usd`, `input_tokens` / `output_tokens` / `cache_read_tokens` / `cache_write_tokens` / `reasoning_tokens` / `total_tokens`, `api_calls`, `model`, `provider`, `session_id`, `service_tier`, the `completed` / `failed` / `partial` / `interrupted` flags and `turn_exit_reason` (why `completed` is false, e.g. `max_iterations_reached(3/3)`). Those top-level counters cover the **main agent loop** only. Auxiliary LLM calls made on the same run (title generation, vision, context compression, `web_extract`, background review, …) are reported separately under `auxiliary` — the same totals plus a per-task `by_task` map — and `total_including_auxiliary` (`estimated_cost_usd`, `total_tokens`, `api_calls`) is the grand total to bill on. The report is written **even when the run fails**, so batch pipelines can always account for spend. It has no effect outside `-z`/`--oneshot`, and a broken usage write never masks the run's own outcome.

```bash
haishui -z "summarize this repo" --usage-file /tmp/usage.json
...
Interactive provider + model selector. **This is the command for adding new providers, setting up API keys, and running OAuth flows.** Run it from your terminal — not from inside an active Haishui chat session.
...
:::warning haishui model vs /model — know the difference
**`haishui model`** (run from your terminal, outside any Haishui session) is the **full provider setup wizard**. It can add new providers, run OAuth flows, prompt for API keys, and configure endpoints.

**`/model`** (typed inside an active Haishui chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys.

**If you need to add a new provider:** Exit your Haishui session first (`Ctrl+C` or `/quit`), then run `haishui model` from your terminal prompt.
:::
...
If you've only configured OpenRouter, `/model` will only show OpenRouter models. To add another provider (Anthropic, DeepSeek, Copilot, etc.), exit your session and run `haishui model` from the terminal.
:::
...
| `migrate-legacy` | Remove legacy `haishui.service` units left over from pre-rename installs. Profile units (`haishui-gateway-<profile>.service`) and unrelated services are never touched. Flags: `--dry-run`, `-y`/`--yes`. |
| `enroll` | Experimental: enroll this gateway with a relay connector and save relay credentials for connector-backed platforms. See [Haishui Relay](../user-guide/messaging/relay.md). |

Options:

| Option | Description |
|--------|-------------|
| `--all` | On `start` / `restart` / `stop`: act on **every profile's** gateway, not just the active `HAISHUI_HOME`. Useful if you run multiple profiles side-by-side and want to restart them all after `haishui update`. |
| `--no-supervise` | On `run`: inside the s6-overlay Docker image, opt out of auto-supervision and use pre-s6 foreground semantics — gateway runs as the container's main process with no auto-restart. No-op outside the s6 image. Equivalent to setting `HAISHUI_GATEWAY_NO_SUPERVISE=1`. |
...
`haishui gateway restart`, or service-restart update exits with status `75`
...
`haishui gateway enroll` accepts `--token`, `--connector-url`, `--gateway-id`, and `--wake-url`. It exchanges the enrollment token with the connector and writes the resulting `GATEWAY_RELAY_ID`, `GATEWAY_RELAY_SECRET`, `GATEWAY_RELAY_DELIVERY_KEY`, optional `GATEWAY_RELAY_URL`, and (when `--wake-url` is given) `GATEWAY_RELAY_WAKE_URL` values to the active profile's `.env`.

:::tip WSL users
Use `haishui gateway run` instead of `haishui gateway start` — WSL's systemd support is unreliable. Wrap it in tmux for persistence: `tmux new -s haishui 'haishui gateway run'`. See [WSL FAQ](./faq.md#wsl-gateway-keeps-disconnecting-or-haishui-gateway-start-fails) for details.
:::

## `haishui lsp`

```bash
haishui lsp <subcommand>
```
...
```bash
haishui setup [model|tts|terminal|gateway|tools|agent] [--non-interactive] [--reset] [--quick] [--reconfigure] [--portal]
```

**Easiest path:** `haishui setup --portal` — OAuth into Nous Portal and opt into the [Tool Gateway](../user-guide/features/tool-gateway.md) in one shot.
...
| `--reconfigure` | Backwards-compat alias — bare `haishui setup` on an existing install now does this by default. |
...
```bash
haishui portal [status|open|tools]
```
...
For configuration of the gateway itself, see [Tool Gateway](../user-guide/features/tool-gateway.md). For the one-shot setup path, see `haishui setup --portal` above.
...
```bash
haishui slack manifest              # print manifest to stdout
haishui slack manifest --write      # write to ~/.haishui/slack-manifest.json
haishui slack manifest --long-description-file AGENTS.md --write
haishui slack manifest --slashes-only  # just the features.slash_commands array
```
...
|------|---------|---------|
| `--write [PATH]` | stdout | Write to a file instead of stdout. Bare `--write` writes `$HAISHUI_HOME/slack-manifest.json`. |
| `--name NAME` | `Haishui` | Bot display name in Slack. |
...
Run `haishui slack manifest --write` again after `haishui update` to pick
up any new commands.
...
```bash
haishui send --to <target> "message text"
haishui send --to <target> --file <path>
echo "message" | haishui send --to <target>
haishui send --list [platform]
```

Send a one-shot message to a configured messaging platform without spinning up an agent or gateway loop. Reuses the gateway's already-configured credentials (`~/.haishui/.env` + `~/.haishui/config.yaml`) so ops scripts, cron jobs, CI hooks, and monitoring daemons can post status updates without reimplementing each platform's REST client.

For bot-token platforms (Telegram, Discord, Slack, Signal, SMS, WhatsApp-CloudAPI) no running gateway is required — `haishui send` talks directly to the platform's REST endpoint. Plugin platforms that need a persistent adapter still require a live gateway.
...
If neither a positional `message` argument nor `--file` is provided, `haishui send` reads from stdin when it is not a TTY. Exit codes: `0` on success, `1` on delivery/backend failure, `2` on usage errors.
...
```bash
haishui send --to telegram "MEDIA:/tmp/screenshot.png"
haishui send --to telegram "Build chart for today MEDIA:/tmp/chart.png"   # with caption
haishui send --to discord:#ops "MEDIA:/tmp/report.pdf"
```
...
```bash
haishui send --to telegram "[[as_document]] MEDIA:/tmp/screenshot.png"
```

Examples:

```bash
haishui send --to telegram "deploy finished"
echo "RAM 92%" | haishui send --to telegram:-1001234567890
haishui send --to discord:#ops --file /tmp/report.md
haishui send --to slack:#eng --subject "[CI]" --file build.log
haishui send --list                  # all platforms
haishui send --list telegram         # filter by platform
```
...
```bash
haishui peer add <name> --url http://host:port --key <API_SERVER_KEY>
haishui peer list
haishui peer dm <peer>[/<agent>] "message"
haishui peer run <peer>[/<agent>] --idempotency-key <key> "message"
haishui peer status <peer>[/<agent>] <run_id>
haishui peer stop <peer>[/<agent>] <run_id>
haishui peer remove <name>
```

Bot-to-bot DMs across machines. Register another Haishui gateway (any machine
running the `api_server` platform) as a *peer*, then message its agents:
`haishui peer dm` resolves the remote agent's canonical **Bot Chat** session
over the peer's API server, runs one agent turn there, and prints the reply
on stdout — the cross-machine twin of the local
`haishui -p <bot> chat --in ~ -c "Bot Chat" …` bot-messaging command.
...
| Subcommand | Description |
|--------|-------------|
| `add <name> --url <URL> [--key <KEY>] [--note TEXT]` | Register or update a peer. The URL goes to `config.yaml` (`bot_peers`); the key is stored as `HAISHUI_PEER_<NAME>_KEY` in `~/.haishui/.env`. |
...
includes the peer roster and the `haishui peer dm` pattern, so agents discover
...
```bash
haishui secrets bitwarden <subcommand>
haishui secrets bw <subcommand>          # short alias
```

Pull API keys from an external secret manager at process startup instead of storing them in `~/.haishui/.env`. Currently supports **Bitwarden Secrets Manager**. See the full guide: [Bitwarden integration](../user-guide/secrets/bitwarden.md).
...
> Not to be confused with `haishui claw migrate` (one-shot import of OpenClaw configuration into Haishui) — `haishui migrate` is the top-level config-rewrite command.


## `haishui proxy`

```bash
haishui proxy <subcommand>
```
...
```bash
haishui security <subcommand>
```

On-demand vulnerability scan against [OSV.dev](https://osv.dev). Covers the Haishui venv (installed PyPI distributions), Python dependencies declared by plugins under `~/.haishui/plugins/`, and pinned `npx`/`uvx` MCP servers in `config.yaml`. Does NOT scan globally-installed packages or editor/browser extensions.
...
| `--skip-venv` | off | Skip scanning the Haishui Python venv. |
| `--skip-plugins` | off | Skip scanning plugin requirements files. |
| `--skip-mcp` | off | Skip scanning pinned MCP servers in `config.yaml`. |


## `haishui login` / `haishui logout` *(Deprecated)*

:::caution
`haishui login` has been removed. Use `haishui auth` to manage OAuth credentials, `haishui model` to select a provider, or `haishui setup` for full interactive setup.
:::
...
```bash
haishui auth                                              # Interactive wizard
haishui auth list                                         # Show all pools
haishui auth list openrouter                              # Show specific provider
haishui auth add openrouter --api-key sk-or-v1-xxx        # Add API key
haishui auth add openrouter --type oauth                  # Browser login (OpenRouter PKCE) mints a key for you
haishui auth add anthropic --type oauth                   # Add OAuth credential
haishui auth add openai-codex --type oauth --priority 0   # Add an account and try it first
haishui auth remove openrouter 2                          # Remove by index
haishui auth priority openrouter backup-key 0             # Move a credential to the front of fill_first order
haishui auth reset openrouter                             # Clear cooldowns
haishui auth reset openrouter 2                           # Clear the cooldown on one credential
haishui auth refresh openai-codex work                    # Refresh one OAuth credential and clear its cooldown
haishui auth status anthropic                             # Show auth status for a provider
haishui auth logout anthropic                             # Log out and clear stored auth state
haishui auth spotify                                      # Authenticate Haishui with Spotify via PKCE
```
...
```bash
haishui cron <list|create|edit|pause|resume|run|remove|status|runs|incidents|doctor|tick>
```
...
`$HAISHUI_HOME/plugins/<name>/`. An unknown or unavailable provider falls back to
...
```bash
haishui kanban [--board <slug>] <action> [options]
```

Multi-profile, multi-project collaboration board. Each install can host many boards (one per project, repo, or domain); each board is a standalone queue with its own SQLite DB and dispatcher scope. New installs start with one board called `default`, whose DB is `~/.haishui/kanban.db` for back-compat; additional boards live at `~/.haishui/kanban/boards/<slug>/kanban.db`. The gateway-embedded dispatcher sweeps every board per tick.

**Global flags (apply to every action below):**

| Flag | Purpose |
|------|---------|
| `--board <slug>` | Operate on a specific board. Defaults to the current board (set via `haishui kanban boards switch`, the `HAISHUI_KANBAN_BOARD` env var, or `default`). |

**This is the human / scripting surface.** Agent workers spawned by the dispatcher drive the board through a dedicated `kanban_*` [toolset](../user-guide/features/kanban.md#how-workers-interact-with-the-board) (`kanban_show`, `kanban_complete`, `kanban_request_review`, `kanban_request_changes`, `kanban_block`, `kanban_create`, `kanban_link`, `kanban_comment`, `kanban_heartbeat`; orchestrator profiles also get `kanban_list` and `kanban_unblock`) instead of shelling to `haishui kanban`. Workers have `HAISHUI_KANBAN_BOARD` pinned in their env so they physically cannot see other boards.
...
| `boards switch <slug>` / `boards use` | Persist `<slug>` as the active board (writes `~/.haishui/kanban/current`). |
...
haishui kanban boards create atm10-server --name "ATM10 Server" --icon 🎮
haishui kanban --board atm10-server create "Restart server" --assignee ops

# Switch the active board for subsequent calls.
haishui kanban boards switch atm10-server
haishui kanban list                  # shows atm10-server tasks

# Archive a board (recoverable) or hard-delete it.
haishui kanban boards rm atm10-server
haishui kanban boards rm atm10-server --delete
```

Board resolution order (highest precedence first): `--board <slug>` flag → `HAISHUI_KANBAN_BOARD` env var → `~/.haishui/kanban/current` file → `default`.
...
```bash
haishui egress install                  # download the pinned iron-proxy binary
haishui egress install --force          # re-download even if already installed

haishui egress setup                    # interactive wizard: CA, mappings, config
haishui egress setup --tunnel-port N    # override the tunnel listener port (default 9090)
haishui egress setup --from-bitwarden   # use Bitwarden Secrets Manager as credential source
haishui egress setup --no-bitwarden     # explicitly switch back to env-based credentials
haishui egress setup --rotate-tokens    # mint fresh proxy tokens (default preserves existing)

haishui egress start                    # spawn the managed proxy daemon
haishui egress stop                     # SIGTERM (then SIGKILL after 5s grace)
haishui egress restart                  # stop (if running) then start — needed for secret changes
haishui egress reload                   # hot-reload the ruleset in-place (no restart, no dropped
                                       #   connections) via the loopback management API

haishui egress status                   # binary + config + pid + listening + mappings
haishui egress status --show-tokens     # print proxy tokens in full (default: redacted)

haishui egress disable                  # flip proxy.enabled = false (does not stop a running proxy)
haishui egress config                   # print the path to proxy.yaml for inspection
```
...
haishui egress setup && haishui egress start
haishui config set terminal.backend docker   # if not already

# Switching credential source after the fact
haishui egress setup --from-bitwarden       # env → bitwarden
haishui egress setup --no-bitwarden         # bitwarden → env
# (just `setup` without either flag preserves the existing mode)

# Rotating all tokens (e.g. after a suspected token leak)
haishui egress setup --rotate-tokens    # setup offers to restart the running daemon for you
# (running sandboxes still hold old tokens; restart them too)

# Adding a new upstream
# Edit ~/.haishui/config.yaml proxy.extra_allowed_hosts: [api.example.com]
haishui egress setup
haishui egress restart                  # one-command apply (stop + start)
```

### Diagnostic shortcuts

```bash
haishui egress status                     # current state in one view
cat ~/.haishui/proxy/proxy.yaml           # the rendered iron-proxy config
tail -20 ~/.haishui/proxy/iron-proxy.log  # daemon-level diagnostics
tail -f ~/.haishui/proxy/iron-proxy.log | jq  # daemon + per-request log (line-delimited JSON; v0.39 combines both streams)
```
...
```bash
haishui project <create|list|show|add-folder|remove-folder|rename|set-primary|use|archive|restore|bind-board>
```
...
```bash
haishui webhook <subscribe|list|remove|test>
```
...
### `haishui webhook subscribe`

```bash
haishui webhook subscribe <name> [options]
```
...
| `--script` | Filter/transform script under `~/.haishui/scripts/`. The webhook payload is passed as JSON on stdin; JSON stdout replaces the payload, and empty stdout, `[SILENT]`, or a nonzero exit code ignores the webhook. See [Script Filters and Transforms](../user-guide/messaging/webhooks.md#script-filters-and-transforms). |
...
Subscriptions persist to `~/.haishui/webhook_subscriptions.json` and are hot-reloaded by the webhook adapter without a gateway restart. Re-running `subscribe` for an existing name keeps its secret and profile binding unless you pass `--secret` / `--route-profile`.
...
```bash
haishui dump [--show-keys]
```

Outputs a compact, plain-text summary of your entire Haishui setup. Designed to be copy-pasted into Discord, GitHub issues, or Telegram when asking for support — no ANSI colors, no special formatting, just data.
...
| Section | Details |
|---------|---------|
| **Header** | Haishui version, release date, git commit hash |
| **Environment** | OS, Python version, OpenAI SDK version |
| **Identity** | Active profile name, HAISHUI_HOME path |
...
:::tip
`haishui dump` is specifically designed for sharing. For interactive diagnostics, use `haishui doctor`. For a visual overview, use `haishui status`.
:::

## `haishui debug`

```bash
haishui debug share [options]
```
...
The report includes system info (OS, Python version, Haishui version), recent agent, gateway, GUI/dashboard, and desktop logs (512 KB limit per file), and redacted API key status. By default, uploads are redacted so secrets are not included.
...
```bash
haishui debug share              # Upload debug report, print URL
haishui debug share --lines 500  # Include more log lines
haishui debug share --expire 30  # Keep paste for 30 days
haishui debug share --nous       # Upload a private diagnostics bundle for Nous support
haishui debug share --local      # Print report to terminal (no upload)
```

## `haishui backup`

```bash
haishui backup [options]
```

Create a zip archive of your Haishui configuration, skills, sessions, and data. The backup excludes the haishui-agent codebase itself, and it does not nest earlier backup artifacts (`backups/`, `state-snapshots/`) — each of those already contains its own copy of `state.db`.

| Option | Description |
|--------|-------------|
| `-o`, `--output <path>` | Output path for the zip file (default: `~/haishui-backup-<timestamp>.zip`). |
| `-q`, `--quick` | Quick snapshot: only critical state files (config.yaml, state.db, .env, auth, cron jobs). Much faster than a full backup. |
| `-l`, `--label <name>` | Label for the snapshot (only used with `--quick`). |
| `-k`, `--keep <N>` | After a full backup, delete older `haishui-backup-*.zip` files in the output directory beyond the newest N (default 3; `0` keeps everything). Custom-named zips are never touched. |

The backup uses SQLite's `backup()` API for safe copying, so it works correctly even when Haishui is running (WAL-mode safe).
...
- `models/`, `runtimes/`, `node/` at the root of `~/.haishui` (and of each `profiles/<name>/`) — regenerable runtime downloads, often tens of GB. Deeper directories with the same names (a skill's `models/`) are kept.
...
- The `haishui-agent` code itself (this is a user-data backup, not a repo snapshot).

### Examples

```bash
haishui backup                           # Full backup to ~/haishui-backup-*.zip
haishui backup -o /tmp/haishui.zip        # Full backup to specific path
haishui backup --quick                   # Quick state-only snapshot
haishui backup --quick --label "pre-upgrade"  # Quick snapshot with label
```

## `haishui checkpoints`

```bash
haishui checkpoints [COMMAND]
```

Inspect and manage the shadow git store at `~/.haishui/checkpoints/` — the storage layer behind the in-session `/rollback` command. Safe to run any time; does not require the agent to be running.

| Subcommand | Description |
|------------|-------------|
| `status` (default) | Show total size, project count, and per-project breakdown. Bare `haishui checkpoints` is equivalent. |
...
```bash
haishui checkpoints                                  # status overview
haishui checkpoints prune --retention-days 3         # aggressive cleanup
haishui checkpoints prune --max-size-mb 200          # tighten size cap once
haishui checkpoints clear-legacy -f                  # drop v1 archive dirs
haishui checkpoints clear -f                         # wipe everything
```
...
```bash
haishui import <zipfile> [options]
```

Restore a previously created Haishui backup into your Haishui home directory. All files in the archive overwrite existing files in your Haishui home; `--force` only skips the confirmation prompt that fires when the target already has a Haishui installation.
...
```
  ⚠ Session data replaced by older backup contents:
    ...
    Recover from a newer backup or snapshot: haishui snapshot list
```

### Examples
```bash
haishui import ~/haishui-backup-20260423.zip           # Prompts before overwriting existing config
haishui import ~/haishui-backup-20260423.zip --force   # Overwrite without prompting
```

## `haishui logs`

```bash
haishui logs [log_name] [options]
```

View, tail, and filter Haishui log files. All logs are stored in `~/.haishui/logs/` (or `<profile>/logs/` for non-default profiles).
...
haishui logs gateway -n 100

# Show only warnings and errors from the last hour
haishui logs --level WARNING --since 1h

# Filter by a specific session
haishui logs --session abc123

# Follow errors.log, starting from 30 minutes ago
haishui logs errors --since 30m -f
...
haishui logs --level WARNING --since 2h --session tg-12345
```
...
Haishui uses Python's `RotatingFileHandler`. Old logs are rotated automatically — look for `agent.log.1`, `agent.log.2`, etc. The `haishui logs list` subcommand shows all log files including rotated ones.


## `haishui prompt-size`

```bash
haishui prompt-size [--platform <name>] [--json]
```
...
- **Prompt tiers** — stable / context / volatile, matching how Haishui layers
...
haishui prompt-size --platform telegram

# Machine-readable output for scripts
haishui prompt-size --json
```

:::tip
The skills index and tool schemas scale with how many skills and tools you have
enabled. To shrink the prompt, disable unused toolsets (`haishui tools`) or
uninstall skills you don't need (`haishui skills`). Context files (AGENTS.md,
...
`haishui config set/get/unset` use `.` as the nesting separator, but many real
...
- **Existing keys just work.** When navigating an existing mapping, an
  ...
  splitting. `haishui config set providers.p.models.grok-4.6.supports_vision true`
...
- **Creating a new dotted key requires escaping.** Escape literal dots with a
  backslash: `haishui config set 'providers.p.models.grok-4\.7.context_length' 128000`
...
```bash
haishui pairing <list|approve|revoke|clear-pending>
```
...
```bash
haishui skills browse
haishui skills browse --source official
haishui skills search react --source skills-sh
haishui skills search https://mintlify.com/docs --source well-known
haishui skills inspect official/security/1password
haishui skills inspect skills-sh/vercel-labs/json-render/json-render-react
haishui skills install official/migration/openclaw-migration
haishui skills install skills-sh/anthropics/skills/pdf --force
haishui skills install https://sharethis.chat/SKILL.md                     # Direct URL (+ referenced support files)
haishui skills install https://example.com/SKILL.md --name my-skill        # Override name when frontmatter has none
haishui skills check
haishui skills update
haishui skills config
haishui skills reset google-workspace
haishui skills reset google-workspace --restore --yes
haishui skills opt-out                  # stop future bundled-skill seeding (nothing deleted)
haishui skills opt-out --remove --yes   # also delete UNMODIFIED bundled skills
haishui skills opt-in --sync            # undo: remove marker and re-seed now
```
...
- `--source well-known` lets you point Haishui at a site exposing `/.well-known/skills/index.json`.
...
```bash
haishui bundles <subcommand>
```

Skill bundles group several skills under one `/<bundle-name>` slash command. Invoking the bundle loads every referenced skill into a single combined user message. Storage: `~/.haishui/skill-bundles/<slug>.yaml`. See [Skill Bundles](../user-guide/features/skills.md#skill-bundles) for the YAML schema and behavior.
...
| `reload` | Re-scan `~/.haishui/skill-bundles/` and report added/removed bundles |

Examples:

```bash
haishui bundles create backend-dev \
  --skill github-code-review \
  --skill test-driven-development \
  --skill github-pr-workflow \
  -d "Backend feature work"

haishui bundles list
haishui bundles show backend-dev
haishui bundles delete backend-dev
```
...
| `backup` | Take a manual tar.gz snapshot of `~/.haishui/skills/` (curator also snapshots automatically before every real run) |
| `rollback` | Restore `~/.haishui/skills/` from a snapshot (defaults to newest) |
...
On a fresh install the first scheduled pass is deferred by one full `interval_hours` (7 days by default) — the gateway will not curate immediately on the first tick after `haishui update`. Use `haishui curator run --dry-run` to preview before that happens.
...
```bash
haishui moa list
haishui moa configure [name]
haishui moa delete <name>
```

`haishui moa configure` reuses Haishui' provider → model picker for each reference model and the aggregator. A preset is an execution-mode configuration, not a primary model or provider.

## `haishui fallback`

```bash
haishui fallback <subcommand>
```
...
| `add` | Pick a provider + model (same picker as `haishui model`) and append to the chain |
...
```bash
haishui hooks <subcommand>
```

Inspect shell-script hooks declared in `~/.haishui/config.yaml`, test them against synthetic payloads, and manage the first-use consent allowlist at `~/.haishui/shell-hooks-allowlist.json`.
...
When an external memory provider is active, it may register its own top-level `haishui <provider>` command for provider-specific management (e.g. `haishui honcho` when Honcho is active). Inactive providers do not expose their subcommands. Run `haishui --help` to see what's currently wired in.
:::
...
Starts Haishui as an ACP (Agent Client Protocol) stdio server for editor integration.
...
```bash
cd ~/.haishui/haishui-agent && uv pip install -e '.[acp]'
```
...
```bash
haishui mcp <subcommand>
```

Manage MCP (Model Context Protocol) server configurations and run Haishui as an MCP server.
...
| `install <name>` | Install a catalog entry (e.g. `haishui mcp install n8n`). |
| `serve [-v\|--verbose]` | Run Haishui as an MCP server — expose conversations to other agents. |
...
See [MCP Config Reference](./mcp-config-reference.md), [Use MCP with Haishui](../guides/use-mcp-with-haishui.md), and [MCP Server Mode](../user-guide/features/mcp.md#running-haishui-as-an-mcp-server).

## `haishui plugins`

```bash
haishui plugins [subcommand]
```

Unified plugin management — general plugins, memory providers, and context engines in one place. Running `haishui plugins` with no subcommand opens a composite interactive screen with two sections:
...
| `install <identifier> [--force] [--ref COMMIT_SHA] [--allow-removed]` | Install a plugin from the Haishui plugin catalog (bare entry name), a Git URL, or `owner/repo` shorthand. Catalog names resolve to the entry's repo at its pinned 40-hex commit SHA, show the declared capability summary, and record catalog provenance in a `.haishui-catalog.json` sidecar. Raw URLs are flagged as custom (unreviewed) sources; `--ref` (full 40-character commit SHA) pins them. `--allow-removed` (DANGEROUS) bypasses the removed-plugin blocklist. |
| `search [term] [--json]` | Search the Haishui plugin catalog (matches entry names, descriptions, and declared tools; omit `term` to list everything). The catalog is curated in-repo (`plugin-catalog/`), refreshed from the live repo with a 6-hour cache, and falls back to the in-tree copy offline. Cataloged ≠ audited — admission reviews the entry, not the code. |
...
See [Plugins](../user-guide/features/plugins.md) and [Build a Haishui Plugin](../developer-guide/plugins/index.md).

## `haishui tools`

```bash
haishui tools [--summary]
```
...
## `haishui computer-use`

```bash
haishui computer-use <subcommand>
```
...
`haishui computer-use install` is the stable entry point for installing the
[cua-driver](https://github.com/trycua/cua) binary used by the
`computer_use` toolset. It runs the same upstream installer that
`haishui tools` invokes when you first enable Computer Use, so it's safe
to use for re-running the install if the toolset toggle didn't trigger
it (for example, on returning-user setups).

If cua-driver is already present, Haishui checks its version and runtime
...
installer. Haishui never replaces a custom binary selected through
`HAISHUI_CUA_DRIVER_CMD`; update that binary directly or remove the override.
`haishui computer-use status` reports when repair is required.

The built-in `computer_use` toolset is the recommended Haishui integration.
Registering raw Cua MCP tools is an alternative when you need Cua's low-level
tool vocabulary. `cua-driver skills install` detects Haishui and links Cua's
skill pack into the Haishui skills directory automatically.

Permission mode and capability-manifest approval
belong to runtime launch. In bounded mode Haishui passes Cua's canonical
...
`haishui update` automatically re-runs the upstream installer at the end
of the update if cua-driver is on PATH, so most users will not need to
call `--upgrade` manually. Use it when upstream ships a fix you want
right now without waiting for the next Haishui update.

## `haishui pets`

```bash
haishui pets <list|install|select|show|off|scale|remove|doctor>
```

[Petdex](https://github.com/crafter-station/petdex) is a public gallery of animated sprite pets for coding agents. Install one and Haishui shows it reacting to agent activity across the CLI, TUI, and desktop app.
...
```bash
haishui insights [--days N] [--source platform]
```
...
```bash
haishui claw migrate [options]
```

Migrate your OpenClaw setup to Haishui. Reads from `~/.openclaw` (or a custom path) and writes to `~/.haishui`. Automatically detects legacy directory names (`~/.clawdbot`, `~/.moltbot`) and config filenames (`clawdbot.json`, `moltbot.json`).

| Option | Description |
|--------|-------------|
| `--dry-run` | Preview what would be migrated without writing anything. |
| `--preset <name>` | Migration preset: `full` (all compatible settings) or `user-data` (excludes infrastructure config). Neither preset imports secrets — pass `--migrate-secrets` explicitly. |
| `--overwrite` | Overwrite existing Haishui files on conflicts (default: refuse to apply when the plan has conflicts). |
| `--migrate-secrets` | Include API keys in migration. Required even under `--preset full`. |
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.haishui/` (by default a single restore-point archive is written to `~/.haishui/backups/pre-migration-*.zip` before apply; restorable with `haishui import`). |
...
The migration covers 30+ categories across persona, memory, skills, model providers, messaging platforms, agent behavior, session policies, MCP servers, TTS, and more. Items are either **directly imported** into Haishui equivalents or **archived** for manual review.
...
haishui claw migrate --dry-run

# Full migration (all compatible settings, no secrets)
haishui claw migrate --preset full

# Full migration including API keys
haishui claw migrate --preset full --migrate-secrets

# Migrate user data only (no secrets), overwrite conflicts
haishui claw migrate --preset user-data --overwrite

# Migrate from a custom OpenClaw path
haishui claw migrate --source /home/user/old-openclaw
```

## `haishui import-agent`

```bash
haishui import-agent [claude-code|codex] [options]
```

Import a **Claude Code** (`~/.claude`) or **OpenAI Codex CLI** (`~/.codex`) setup into Haishui. Maps `CLAUDE.md`/`AGENTS.md` instructions to memory entries, `Bash(...)` permission allow/deny rules to `command_allowlist`/`approvals.deny`, MCP servers to `mcp_servers` in `config.yaml`, and skill directories into `~/.haishui/skills/`. Always previews before applying; API keys and credentials are never imported.
...
Every successful import registers its source in `~/.haishui/import-sync.json`; `haishui import-agent --sync` then re-imports any registered source whose files changed (a cron-friendly way to keep an imported Claude Code / Codex setup current). See the **[import guide](../user-guide/import-from-other-agents.md)** for the full mapping tables.

## `haishui serve`

```bash
haishui serve [options]
```

Start the Haishui **backend server** — the JSON-RPC/WebSocket gateway the [desktop app](../user-guide/desktop.md) and remote clients connect to. It is the same server `haishui dashboard` runs, but **headless**: it never opens a browser UI. The desktop app launches its own `haishui serve` backend; use this command directly when you want a headless backend on a remote host. Accepts the same `--host` / `--port` / `--insecure` / `--skip-build` / `--stop` / `--status` options as `haishui dashboard` below (a non-loopback bind engages the same auth gate). Requires the `[web]` extra; the embedded Chat socket additionally needs `[pty]` on a POSIX host.

**Port conflicts:** if the requested port (default `9119`) is already held by another process (e.g. a second `haishui serve` or the gateway), the command prints a machine-readable sentinel line `BACKEND_PORT_IN_USE port=<port>` to stdout, a human hint naming the likely holder, and exits with code **75** (`EX_TEMPFAIL`) instead of a generic error — so scripts and the desktop app can tell "port occupied" apart from "backend broken". Pass `--port 0` to bind a free ephemeral port (the successful boot announces the chosen port via `HAISHUI_BACKEND_READY port=<port>`).

## `haishui dashboard`

```bash
haishui dashboard [options]
```

Launch the web dashboard — a browser-based UI for managing configuration, API keys, and monitoring sessions. (For a headless backend with no browser UI — e.g. what the desktop app spawns — use [`haishui serve`](#haishui-serve) above.) Requires `cd ~/.haishui/haishui-agent && uv pip install -e ".[web]"` (FastAPI + Uvicorn). The embedded browser Chat tab is always available and additionally needs the `pty` extra (`cd ~/.haishui/haishui-agent && uv pip install -e ".[web,pty]"`) plus a POSIX PTY environment such as Linux, macOS, or WSL2. See [Web Dashboard](../user-guide/features/web-dashboard.md) for full documentation.
...
| `--stop` | — | Stop the running `haishui dashboard` / `haishui serve` backend **of this Haishui home** and exit (`-p <profile>` / `HAISHUI_HOME` select which; backends of other profiles, other installs on the machine, and the shell you typed the command into are never touched — a backend whose owner cannot be read is left alone). SIGTERM, a 10s grace, then SIGKILL; a hosted Chat TUI that outlives its backend is stopped too (it would otherwise keep the deleted `state.db-wal` open and block the next start). Messaging-gateway bots started from the dashboard are not touched. |
| `--status` | — | List running `haishui dashboard` processes and exit. |

### `haishui dashboard register`

Register this install as a self-hosted dashboard with your Nous Portal account. Creates an OAuth client, writes `HAISHUI_DASHBOARD_OAUTH_CLIENT_ID` into `~/.haishui/.env`, and prints how to engage the login gate. Requires being logged in (`haishui setup`).

| Option | Description |
|--------|-------------|
| `--name` | Human-readable label for the dashboard (default: auto-generated). |
| `--redirect-uri` | Public HTTPS OAuth redirect URI (e.g. `https://haishui.example.com/auth/callback`). Omit for localhost-only use. |
| `--portal-url` | Override the Nous Portal base URL for registration (default: the portal you logged into). Also settable via `HAISHUI_DASHBOARD_PORTAL_URL`. |
...
haishui dashboard --port 8080 --no-open
...
```bash
haishui profile <subcommand>
```

Manage profiles — multiple isolated Haishui instances, each with its own config, sessions, skills, and home directory.
...
```bash
haishui profile list
haishui profile create work --clone
haishui profile create work --clone --sync-imports   # also carry over the import-agent sync manifest
haishui profile use work
haishui profile alias work --name h-work
haishui profile export work -o work-backup.tar.gz
haishui profile import work-backup.tar.gz --name restored
haishui profile install github.com/user/my-distro --alias
haishui profile update work
haishui -p work chat -q "Hello from work profile"
```
...
Print a shell completion script to stdout. Source the output in your shell profile for tab-completion of Haishui commands, subcommands, and profile names.

Examples:

```bash
# Bash
haishui completion bash >> ~/.bashrc

# Zsh
haishui completion zsh >> ~/.zshrc

# Fish
haishui completion fish > ~/.config/fish/completions/haishui.fish
```

## `haishui update`

```bash
haishui update [--gateway] [--check] [--plan] [--no-backup] [--backup] [--yes]
```

Pulls the latest `haishui-agent` code and reinstalls dependencies in the managed venv, then re-runs the post-install hooks (MCP servers, skills sync, completion install). Safe to run on a live install. Use `--check` to see whether your checkout is behind `origin/main` without installing.

`haishui update` pulls the configured update branch (default: `main`). If your checkout is on another branch, Haishui may check out the update branch before pulling. Commit branch work before updating when you want to keep it outside the update autostash flow.
...
| `--backup` | Force a **full** pre-update backup for this run: the quick state snapshot plus a complete zip of `HAISHUI_HOME` (config, auth, sessions, skills, pairing data). The default mode is `quick` — a lightweight state snapshot only. Set the permanent mode via `updates.pre_update_backup: quick | full | off` in `config.yaml`. |
| `--yes`, `-y` | Assume yes for interactive prompts such as config migration and stash restore. API-key entry is skipped; run `haishui config migrate` separately for those. |

Additional behavior:

- **Gateway restart.** After a successful update, Haishui attempts to restart all running gateway profiles automatically so they pick up the new code. Use `haishui gateway restart` when you want to restart a gateway without applying an update.
...
- **Update receipts + fleet version check.** Every run writes a machine-readable receipt to `~/.haishui/logs/update_receipts/` (pre-update fleet plan, steps, skips with reasons, restart outcome; `latest.json` points at the newest). After the restart phase the updater verifies each live gateway's running code against the updated checkout and prints a per-profile version matrix; a gateway still on pre-update code fails the update (exit 1) with the exact restart command.
...
- **npm lockfile churn.** Before stashing or switching branches, Haishui makes a best-effort cleanup of tracked `package-lock.json` diffs produced by npm install/build steps. Commit or manually stash intentional lockfile edits before running `haishui update`.
- **Pairing data snapshot.** Even when `--backup` is off, `haishui update` takes a lightweight snapshot of `~/.haishui/pairing/` and the Feishu comment rules before `git pull`. You can roll it back with `haishui backup restore --state pre-update` if a pull rewrites a file you were editing.
- **Legacy `haishui.service` warning.** If Haishui detects a pre-rename `haishui.service` systemd unit (instead of the current `haishui-gateway.service`), it prints a one-time migration hint so you can avoid flap-loop issues.
...
| Command | Description |
|---------|-------------|
| `haishui --version` | Print version information. |
| `haishui update` | Pull latest changes and reinstall dependencies. |

| `haishui uninstall [--full] [--gui] [--dry-run] [--yes]` | Remove Haishui, optionally deleting all config/data. `--gui` removes only the desktop Chat GUI, leaving the agent intact; `--full` also deletes config/data; `--dry-run` prints what would be removed without changing anything; `--yes` skips prompts. |
