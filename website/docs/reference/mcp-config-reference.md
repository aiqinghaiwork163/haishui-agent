...
description: "Reference for Haishui Agent MCP configuration keys, filtering semantics, and utility-tool policy"
---
...
Values resolve from the active profile's secret scope (falling back to the process environment), so put the secret in `~/.haishui/.env`. An unset variable keeps its literal placeholder.
...
Haishui may register these utility wrappers per MCP server:
...
Even when `resources: true` or `prompts: true`, Haishui only registers those utility tools if the MCP session actually exposes the corresponding capability.
...
If filtering removes all server-native tools and no utility tools are registered, Haishui does not create an empty MCP runtime toolset for that server.
...
Behavior:
- Haishui uses the MCP SDK's OAuth 2.1 PKCE flow (metadata discovery, client identification, token exchange, and refresh)
- On first connect, a browser window opens for authorization
- Tokens are persisted to `~/.haishui/mcp-tokens/<server>.json` (a named profile uses `~/.haishui/profiles/<name>/mcp-tokens/`) and reused across sessions
...
device authorization from a terminal on the machine running Haishui:

```bash
haishui mcp login protected_api --flow device
```

Open the printed verification URL on any device and enter the displayed user code.
Haishui polls for approval, respects `authorization_pending` and `slow_down`, and stops
...
Set `oauth.flow: device` on the server to make `haishui mcp login` and `haishui mcp reauth`
...
The document declares a fixed set of loopback redirect URIs, and the spec requires the redirect URI in an authorization request to be an *exact string match* against one of them — so a CIMD flow cannot use the random high port Haishui normally picks. Haishui therefore pins the callback to one of ports `27890`–`27894`.

That pin has to be chosen before the server's capabilities are known, because the redirect URI is fixed at the start of the flow while the server's metadata only arrives partway through. So Haishui pins the port for any flow that *could* end up using CIMD, and reverts to a random port for the rest:

- A server Haishui has connected to before, whose cached metadata does not advertise CIMD, keeps the random port it has always used.
- A server Haishui has never reached gets a pinned port on that first login, since guessing is the only way CIMD can ever be used.
...
If a server fetches the document and refuses it at the *token* endpoint (`invalid_client`), Haishui logs the rejection, records it under `~/.haishui/mcp-tokens/<server>.cimd-off`, and uses DCR for that server from then on.

A server that cannot fetch or validate the document at all aborts at the *authorization* endpoint instead, before any redirect happens. There is no signal Haishui can observe there, so the browser shows an invalid-client error and the login times out after five minutes. The timeout message names the document and points at `cimd: false`. Running `haishui mcp login <server>` clears the recorded rejection, so a corrected document gets another chance.
...
OAuth discovery and dynamic-client-registration requests (the `/.well-known/...` metadata documents and the `registration_endpoint` POST) always carry `User-Agent: Haishui-Agent/<version>`. The MCP SDK builds those requests without any client default headers, and WAF-fronted authorization servers answer a header-less request with `403` — the metadata document then looks unreadable, registration falls back to a guessed `/register` on the MCP host, and the login fails with `Registration failed: 404`. When every metadata fetch does fail, the error now leads with those statuses (`Could not read authorization-server metadata (403 from https://…/.well-known/oauth-authorization-server; …)`) before the registration fallback's own error.

## Add to Haishui link

MCP vendors and docs can offer a one-click **"Add to Haishui"** button that opens the Haishui desktop app with a pre-filled server config, mirroring Cursor's `cursor://anysphere.cursor-deeplink/mcp/install` scheme:

```text
haishui://mcp/install?name=NAME&config=BASE64
```
...
const link = `haishui://mcp/install?name=example&config=${btoa(JSON.stringify(config))
