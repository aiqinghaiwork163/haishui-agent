...
Plugins can launch and supervise fresh Haishui child sessions without importing
...
active agent turn fails closed with `No active Haishui parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Haishui can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Haishui's existing unsafe-tool block remains enforced.
