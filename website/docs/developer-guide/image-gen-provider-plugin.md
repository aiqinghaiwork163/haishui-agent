...
description: "How to build an image-generation backend plugin for Haishui Agent"
---
...
:::tip
Image-gen is one of several **backend plugins** Haishui supports. The others (with more specialized ABCs) are [Memory Provider Plugins](./memory-provider-plugin.md), [Context Engine Plugins](./context-engine-plugin.md), and [Model Provider Plugins](./model-provider-plugin.md). General tool/hook/CLI plugins live in [Build a Haishui Plugin](./plugins/index.md).
:::

## How discovery works

Haishui scans for image-gen backends in three places:

1. **Bundled** — `<repo>/plugins/image_gen/<name>/` (auto-loaded with `kind: backend`, always available)
2. **User** — `~/.haishui/plugins/image_gen/<name>/` (opt-in via `plugins.enabled`)
3. **Pip** — packages declaring a `haishui_agent.plugins` entry point

Each plugin's `register(ctx)` function calls `ctx.register_image_gen_provider(...)` — that puts it into the registry in `agent/image_gen_registry.py`. The active provider is picked by `image_gen.provider` in `config.yaml`; `haishui tools` walks users through selection.

The `image_generate` tool wrapper asks the registry for the active provider and dispatches there. If no provider is registered, the tool surfaces a helpful error pointing at `haishui tools`.
...
A bundled plugin is complete at this point. User plugins at `~/.haishui/plugins/image_gen/<name>/` need to be added to `plugins.enabled` in `config.yaml` (or run `haishui plugins enable <name>`).
...
class MyBackendImageGenProvider(ImageGenProvider):
    ...
    def display_name(self) -> str:
        # Human label shown in `haishui tools`. Defaults to name.title() if omitted.
        return "My Backend"
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        # Catalog shown in `haishui tools` model picker.
        return [
            {
                "id": "my-model-fast",
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        # Metadata for the `haishui tools` picker — keys to prompt for at setup.
        return {
            "name": "My Backend",
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            #   - base64 data: save under $HAISHUI_HOME/cache/images/ via save_b64_image()
...
`kind: backend` is what routes the plugin to the image-gen registration path. `requires_env` is prompted during `haishui plugins install`.
...
| `display_name` | — | `name.title()` | Label shown in `haishui tools` |
| `is_available()` | — | `True` | Gate for missing creds/deps |
| `list_models()` | — | `[]` | Catalog for `haishui tools` model picker |
...
Some backends return image URLs (fal, Replicate); others return base64 payloads (OpenAI gpt-image-2). For the base64 case, use `save_b64_image()` — it writes to `$HAISHUI_HOME/cache/images/<prefix>_<timestamp>_<uuid>.<ext>` and returns the absolute `Path`. Pass that path (as `str`) as `image=` in `success_response()`. Gateway delivery (Telegram photo bubble, Discord attachment) recognizes both URLs and absolute paths.

## User overrides

Drop a user plugin at `~/.haishui/plugins/image_gen/<name>/` with the same `name` property as a bundled one and enable it via `haishui plugins enable <name>` — the registry is last-writer-wins, so your version replaces the built-in. Useful for pointing an `openai` plugin at a private proxy, or swapping in a custom model catalog.

## Testing

```bash
export HAISHUI_HOME=/tmp/haishui-imggen-test
mkdir -p $HAISHUI_HOME/plugins/image_gen/my-backend
# …copy __init__.py + plugin.yaml into that dir…

export MY_BACKEND_API_KEY=your-test-key
haishui plugins enable my-backend

# Pick it as the active provider
echo "image_gen:" >> $HAISHUI_HOME/config.yaml
echo "  provider: my-backend" >> $HAISHUI_HOME/config.yaml

# Exercise it
haishui -z "Generate an image of a corgi in a spacesuit"
```

Or interactively: `haishui tools` → "Image Generation" → select `my-backend` → enter API key if prompted.
...
- [Build a Haishui Plugin](./plugins/index.md) — general tools/hooks/slash commands guide
