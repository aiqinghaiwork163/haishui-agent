...
# Haishui Observer Hooks

Haishui observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Haishui' planner, model providers, memory, tool registry,
...
Haishui also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "haishui.observer.v1"
```

Hook callbacks are fail-open. Haishui catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Haishui only builds
...
The native NeMo Relay SDK integration maps Haishui session, turn, LLM, and tool
