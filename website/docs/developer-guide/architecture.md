...
sidebar_position: 1
title: "Architecture"
description: "Haishui Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Haishui Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ haishui_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # HaishuiCLI facade — mixins in haishui_cli/cli_*_mixin.py
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── haishui_state.py           # SQLite session/state database facade (+ haishui_state_*.py siblings)
├── haishui_constants.py       # HAISHUI_HOME, profile-aware paths
...
│
├── haishui_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — `haishui` subcommands (parsers in subcommands/, main_*.py)
...
│   ├── skills_config.py      # haishui skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # haishui tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # haishui gateway start/stop
│
...
```text
User input → HaishuiCLI.process_input()
...
Three discovery sources: `~/.haishui/plugins/` (user), `.haishui/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `haishui plugins` or `config.yaml`.
...
Exposes Haishui as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`haishui -p <name>`) gets its own HAISHUI_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
