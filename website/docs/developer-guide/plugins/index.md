...
title: "Build a Haishui Plugin"
description: "Step-by-step guide to building a complete Haishui plugin with tools, hooks, data files, and skills"
---

# Build a Haishui Plugin

This guide walks through building a complete Haishui plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
Haishui has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| An **STT backend** (custom whisper / ASR CLI) | [Voice Message Transcription](../../user-guide/features/tts.md#voice-message-transcription-stt) — set `HAISHUI_LOCAL_STT_COMMAND` to an argv-tokenized template |
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](../../user-guide/features/hooks.md#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.haishui/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](../../user-guide/features/skills.md) — `haishui skills tap add <repo>` · [Publishing a tap](../../user-guide/features/skills.md#publishing-a-custom-skill-tap) |
...
```bash
haishui plugins install owner/repository --no-enable
haishui plugins list
haishui plugins enable <plugin-name>
```
...
Haishui validates `plugin.json`, Agent Skills frontmatter, fixed component
...
profile-scoped writable directory managed by Haishui.
...
Portable `streamable-http` entries are routed through Haishui' existing native
...
same full-trust posture as other installed Haishui plugins.
...
records it as Published. Haishui keys behavior on the canonical v1.0.0 schema
...
not by one global plugin API number. Haishui does not expose a
...
continue to work after a normal Haishui upgrade.
...
- **Hook payloads are keyword payloads.** New hook data is added as keyword
  ...
  Haishui inspects callback signatures: a legacy callback receives the fields it
...
- **Manifests are open to additions.** Unknown `plugin.yaml` fields are ignored.
  Older Haishui releases can therefore load a plugin whose manifest contains
...
Haishui enforces this contract with frozen external-plugin fixtures discovered
from an isolated `HAISHUI_HOME`. Those tests load and invoke the plugin through
...
Haishui's internals were split into `<stem>_<topic>` sibling modules in Sep 2026 (PR #102117). **Internal
...
- **Check your plugin:** `haishui plugins compat /path/to/your/plugin` lists every `file:line` with the
  old path and the new one, and exits 1 while any remain. `COMPAT_MANIFEST.md` in the repo is the full map.
- **What users see:** a notice under the CLI banner, in `haishui doctor` and after `haishui update`, and a
  one-time Desktop dialog naming the plugin. Each resolution through an old path also emits a
  `HaishuiPluginCompatWarning` once per process.
- **From 2026-09-14:** plugins that still import old paths are **not loaded** (the reason shows in
  `haishui plugins list`). Users can force-load with `plugins.allow_deprecated_imports: true` until the
...
```bash
mkdir -p ~/.haishui/plugins/calculator
cd ~/.haishui/plugins/calculator
```

### Validate with Plugin Doctor

`haishui plugins doctor [path-or-id]` runs the same directory discovery,
manifest parser, namespaced import, `register(ctx)`, hook registry, and tool
registry used by Haishui itself. It reports invalid hook names, callbacks that do
...
```bash
haishui plugins doctor . --ci
```

Doctor uses a temporary `HAISHUI_HOME`, restores plugin registration state after
...
This tells Haishui: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
`llm.task_override` (see `haishui_cli/plugin_capabilities.py` for the
...
`haishui_agent.plugin_capabilities` entry-point group. Each declaration is
named `<plugin-id>.<capability-id>` and points at the same object as your
`haishui_agent.plugins` entry point:

```toml
[project.entry-points."haishui_agent.plugins"]
calculator = "my_pkg:register"

[project.entry-points."haishui_agent.plugin_capabilities"]
"calculator.tools.override" = "my_pkg:register"
```

Haishui reads these from installed metadata without importing your code, so
`haishui plugins capabilities` and the consent flow stay accurate for pip
installs.
...
this Haishui understands still loads with a warning.
...
| `python_dependencies` | list of str | PEP 508 requirements (e.g. `"requests>=2.0,<3"`). Installed into Haishui' venv on `haishui plugins install` / `enable` and **re-applied after every `haishui update`** (see [Python dependencies](#python-dependencies)). A `pyproject.toml` beside `plugin.yaml` with `[project].dependencies` is the equivalent, preferred form. |
| `python_runtime` | str | `external` — the plugin manages its own interpreter/venv (sidecar pattern); Haishui installs nothing and leaves any `pyproject.toml` alone. |
...
requires_plugins:
  - id: other-plugin
    version_range: ">=1.0,<2"
python_dependencies:
  - "somepkg>=1.0,<2"     # installed on install/enable, re-applied after haishui update
config_schema:
...
When both exist the `pyproject.toml` wins. What Haishui does with them:

- **Install / enable** — the declared packages are installed into Haishui' venv with
  `uv pip install` (pip fallback) under a **constraints file built from Haishui' own pinned
  dependencies**, so a plugin can never move a core package (httpx, pydantic, …) off the version
  Haishui was tested with. Environment markers (`; sys_platform == "win32"`) are honoured.
...
- **`haishui update` re-applies them** — the update's `uv sync` rebuilds the venv from Haishui' lock
  and strips anything else. Afterwards Haishui walks every profile's enabled plugins and reinstalls
  ...
  message** naming it, and memory providers are kept over everything else, because a Haishui that
  boots without memory looks like data loss.
- **`haishui plugins update`** re-runs the install for whatever the new revision declares.
- **`--no-deps`** on `haishui plugins install` skips all of this for one plugin (no conflict gate,
...
- **Opt out with `python_runtime: external`** — plugins that keep a heavy runtime (torch, native
  ...
  `plugin.yaml`; Haishui then installs nothing and the plugin never joins the shared resolution.
- **Nothing to load is an error** — `haishui plugins validate` (and the catalog CI) fail a
...
`HAISHUI_HOME/plugins/` survives `haishui update` and Desktop updates: the updater only rebuilds the
...
4. **Accept `**kwargs`:** Haishui may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `haishui my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but Haishui continues fine
...
Use plugin-relative config keys for user-visible behavior. Haishui resolves them
...
`<HAISHUI_HOME>/plugin-data/`. Neither API exposes another plugin's namespace.
...
If your plugin doesn't show up — or shows up but isn't loading — set `HAISHUI_PLUGINS_DEBUG=1` to get verbose discovery logs on stderr:

```bash
HAISHUI_PLUGINS_DEBUG=1 haishui plugins list
```
...
The same logs are always written to `~/.haishui/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:

```bash
haishui logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `haishui plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout:** Native packages use `~/.haishui/plugins/<plugin-name>/plugin.yaml` (flat) or one category level. Portable packages use root `plugin.json` in the same locations. Anything deeper is ignored.
...
tree, and `haishui plugins update` / `remove` git-pull or delete it — your
...
# <haishui home>/plugin-data/<name>/ — created on first use
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.haishui/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.haishui/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `haishui plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing Haishui into installing arbitrary packages — only specs Haishui itself ships are eligible |
...
auditable in `~/.haishui/logs/agent.log`. Plugins load after built-in
...
does not ship with Haishui core (user, project, or pip source),
...
plugins:
  entries:
    my-plugin:                    # the plugin's registry key from `haishui plugins list`
...
loader, the plugin is disabled and Haishui continues. The gate exists
...
You normally never edit this key by hand. `haishui plugins enable <name>`
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), Haishui injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.

#### Return format

```python
# Dict with context key
return {"context": "Recalled memories:\n- User prefers dark mode\n- Last project: haishui-agent"}
...
Per-hook context is capped at `10,000` characters by default. Anything above the cap is written to `$HAISHUI_HOME/hook_outputs/<session_id>/<uuid>.txt` and replaced with a head/tail preview plus the saved path. The model can read the full content via `read_file` or `terminal` if it genuinely needs it. This keeps a runaway plugin from inflating every subsequent turn's prompt and blowing out the prompt cache prefix. Tune in `config.yaml`:
...
hooks:
  output_spill:
    ...
    # directory: null      # default: $HAISHUI_HOME/hook_outputs
```
...
- **The system prompt is Haishui's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
The canonical list of kinds is `VALID_MIDDLEWARE` in `haishui_cli/middleware.py`:
...
| `llm_request` | `request`, `original_request`, context kwargs | Return `{"request": {...}}` to replace the effective provider kwargs before Haishui sends them. |
...
- Middleware payloads carry `middleware_schema_version` (`haishui.middleware.v1`) alongside the observer telemetry fields.
- Unknown kinds register with a warning instead of failing, so a plugin written against a newer Haishui still loads on an older one.

### Register CLI commands

Plugins can add their own `haishui <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for haishui my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: haishui my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for haishui my-plugin."""
...
After registration, users can run `haishui my-plugin status`, `haishui my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `haishui name` in a terminal |
...
`ctx._cli_ref` is only populated in an **interactive CLI** session. It is `None` in the gateway, in non-interactive `haishui chat -q` runs, and in **kanban-spawned worker sessions** — so any plugin logic that reaches through `_cli_ref` silently no-ops in exactly those contexts. Two stable, session-agnostic APIs cover what hooks actually need:

- **`ctx.profile_name`** — the active profile name (e.g. `"default"`, or the assignee profile in a kanban worker). Derived from `HAISHUI_HOME`, so it works everywhere with no `_cli_ref` dependency.
...
For running a full `haishui <subcommand>` (e.g. `haishui kanban show`), shell out with the `terminal` tool via `ctx.dispatch_tool("terminal", {"command": "haishui kanban show ..."})` — there is no in-process slash-command bridge for headless worker sessions, and tools are the supported way to drive Haishui from a hook.
...
Haishui has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.haishui/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `haishui gateway status` without SDK instantiation.
...
Haishui also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](../../user-guide/features/plugins.md#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into Haishui without any Python plugin. Declare them in `~/.haishui/config.yaml`:
...
Haishui connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](../../user-guide/features/mcp.md).

### Gateway event hooks — fire on lifecycle events

Drop a manifest + handler into `~/.haishui/hooks/<name>/`:

```yaml
# ~/.haishui/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.haishui/hooks/long-task-alert/handler.py
...
haishui skills search my-workflow --source myorg/skills-repo
haishui skills install myorg/skills-repo/my-workflow
```
...
```bash
pip install haishui-plugin-calculator
# Plugin auto-discovered on next haishui startup
```
...
```python
# Wrong — will break if Haishui passes extra context
def handler(args):
    ...
