...
has to be thrown away or summarized. Haishui has always done this in one batch:
...
Haishui folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Haishui's normal session flush is
...
With `micro_compact` unset or `false` Haishui behaves exactly as it always has:
...
Defaults to `$HAISHUI_HOME/logs/agent.log`. It reports passes, outcome mix, net
