...
Haishui Agent uses a SQLite database (`~/.haishui/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source files: `haishui_state.py` (facade) plus the `haishui_state_*.py` siblings (schema, fts, search, compression, portability, gateway, ...)

## Haishui home and profile isolation

`get_haishui_home()` is the authoritative filesystem resolver for state and
configuration. It uses a context-local override first, then the `HAISHUI_HOME`
environment variable, and finally the platform default (`~/.haishui` on macOS
and Linux; `%LOCALAPPDATA%/haishui` on Windows). Consequently, the default
database is always `get_haishui_home() / "state.db"`, not a path that callers
should hard-code as `~/.haishui/state.db`.

Named profiles are isolated directories: a profile named `coder`, for example,
uses `<default Haishui root>/profiles/coder/` and therefore has its own
...
profile must retain or pass that profile's `HAISHUI_HOME`; falling back to the
...
of Haishui. An explicit `--profile`/`-p` resolves that profile and writes the
resolved directory to `HAISHUI_HOME`. Without an explicit selector, a
profile-specific `HAISHUI_HOME` is preserved; otherwise the bootstrap can use
the default root's active-profile selection. `HOME` only determines the
platform default used when no context override or `HAISHUI_HOME` is available.
Changing `HOME` is not a safe way to select a named profile. In particular, a
subprocess that drops `HAISHUI_HOME` can fall back to the default profile even
when another profile is active, so subprocess spawners should pass
`HAISHUI_HOME` explicitly.

Use `display_haishui_home()` only for user-facing text. It formats the resolved
home relative to the user's home directory when possible (for example,
`~/.haishui/profiles/coder`); it does not provide a separate resolution rule.

### Test isolation guard

Tests must use a temporary `HAISHUI_HOME` or an explicit temporary database
path. The live-system guard raises before a test-context process opens a
production `state.db` under the real default Haishui root or a real named
profile, preventing fixture data or SQLite side effects from reaching a live
installation.

`HAISHUI_STATE_DB_GUARD_BYPASS=1` is a test-only escape hatch for a spawned
...
either bypass in normal Haishui commands, development shells, or application
...
Each named profile stores its transcript in its own `$HAISHUI_HOME/state.db`,
including when one `haishui serve` process serves several profiles. In-session
...
splice boundary, Haishui excludes only that leading item when it exactly matches
...
```
~/.haishui/state.db (SQLite, WAL mode)
...
`haishui sessions recover` copies the row-bearing tables above into the
...
Abridged — see `SCHEMA_SQL` in `haishui_state_common.py` (applied by `haishui_state_schema.py`) for the full current column list
...
an authenticated `haishui serve` (OAuth or the basic username/password provider)
...
indexed columns — see `SCHEMA_SQL` in `haishui_state_common.py` for the exact SQL.
...
Multiple haishui processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.haishui/state.db
...
Default path: `get_haishui_home() / "state.db"` — `~/.haishui/state.db` for the
default profile, `~/.haishui/profiles/<name>/state.db` for a named profile, or
wherever `HAISHUI_HOME` points (see [Haishui home and profile isolation](#haishui-home-and-profile-isolation)).
