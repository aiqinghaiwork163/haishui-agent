...
description: "How Haishui stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `haishui_cli/cron.py` | CLI `haishui cron` subcommands |
...
Jobs are stored in `~/.haishui/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
renderer (`haishui cron list`/`doctor`, the `cronjob` tool, the web dashboard
...
The same store fields drive every topology: a standalone `haishui -p X gateway
...
In CLI mode, cron jobs only fire when `haishui cron` commands are run or during active CLI sessions.
...
```python
# ~/.haishui/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.haishui/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `haishui cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `haishui cron` CLI provides direct job management:

```bash
haishui cron list                    # Show all jobs
haishui cron create                  # Interactive job creation (alias: add)
haishui cron edit <job_id>           # Edit job configuration
haishui cron pause <job_id>          # Pause a running job
haishui cron resume <job_id>         # Resume a paused job
haishui cron run <job_id>            # Trigger immediate execution
haishui cron remove <job_id>         # Delete a job
```
