...
The ACP adapter wraps Haishui' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
haishui acp / haishui-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.haishui/.env
  -> configure stderr logging
  -> construct HaishuiACPAgent
...
- `allow_once` -> Haishui `once`
- `allow_always` -> Haishui `always`
- reject options -> Haishui `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Haishui tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["haishui-acp"])
...
retries, interrupt before any reply) is closed by the core loop with a Haishui-authored
...
Instead it reuses Haishui' runtime resolver:

- `acp_adapter/auth.py`
- `haishui_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Haishui provider/credentials. It also always advertises a terminal setup auth method (`haishui-setup`, args `--setup`) so first-run ACP clients can open Haishui' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.haishui/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `haishui-acp` toolset definition
- `haishui_cli/main.py` — `haishui acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `haishui-acp` script
