...
sidebar_position: 15
title: "CLI Internals"
description: "How haishui_cli is shaped: slash dispatch, config loaders, the skin engine, the transactional update pipeline, and process-identity rules"
---

# CLI Internals

Companion to `haishui_cli/AGENTS.md` (the rules) — this page holds the longer explanations.

## Update pipeline

The stage-by-stage contract (`plan → snapshot → apply → restart-per-kind → verify → report`) and the
field failure each stage guards are documented in `haishui_cli/AGENTS.md`; user-facing behaviour
...
- Use the canonical matchers: `gateway.status.looks_like_gateway_command_line` (gateway run),
  `haishui_cli.update_cmd._haishui_holder_subcommand` (top-level subcommand of any Haishui argv). Never
...
Built-in skins (`_BUILTIN_SKINS` in `haishui_cli/skin_engine.py`): `default` (classic gold/kawaii),
...
User skins are `~/.haishui/skins/<name>.yaml` with the same keys, activated with `/skin <name>` or
`display.skin: <name>`; the full YAML template is in the
[Skins & Themes](../user-guide/features/skins.md) user guide.

## Profiles: multi-instance support

Haishui supports profiles — fully isolated instances, each with its own `HAISHUI_HOME` (config, API
keys, memory, sessions, skills, gateway). For single-profile commands (`haishui -p x <cmd>`),
`_apply_profile_override()` in `haishui_cli/main.py` sets `HAISHUI_HOME` before any module imports, so
every `get_haishui_home()` reference scopes to the active profile. The multiplex gateway and the
Desktop/dashboard `serve` backend serve several profiles from one process instead: the active
profile is a contextvar override bound per activity, `os.environ["HAISHUI_HOME"]` stays the launch
...
`Path.home() / ".haishui" / "profiles"`, not `get_haishui_home() / "profiles"`) so
`haishui -p coder profile list` sees all profiles regardless of which one is active — intentional.
