...
Chronos lets a hosted Haishui gateway **scale to zero** while idle and still
...
|---|---|---|---|
| 1 | agent → NAS (`provision`/`cancel`/`list`) | the agent's existing **Nous Portal access token** (Bearer) — for a hosted agent this is the **bootstrap-session token** NAS planted in `auth.json` (client `haishui-cli-vps`), NOT an `agent:*` client token | NAS (its normal agent-token path) |
...
> minted under the bootstrap-only client `haishui-cli-vps` and seeded into the
...
1. **Dashboard app** (`haishui_cli/web_server.py`) — the agent's only public
...
- **Multi-replica agents:** the store CAS makes the fire at-most-once across N
  gateway replicas sharing one `HAISHUI_HOME` — exactly one replica runs each
  fire.
...
`invalid_client` — the token in `auth.json` is a plain `haishui-cli` user login
rather than the `haishui-cli-vps` bootstrap session or an `agent:*` client — the
rejection is deterministic for the life of that credential: every arm, re-arm
and `list` would fail the same way, and a `haishui auth` re-login makes it
