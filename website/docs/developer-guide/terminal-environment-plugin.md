...
Haishui runs shell commands through a pluggable set of **terminal backends**.
...
installed under `~/.haishui/plugins/`, registering a backend the user selects
...
| `haishui setup` backend picker | `display_name`, `description`, `setup_instructions()`, `post_setup()` |
| Dashboard terminal-backend picker (probe status) | `probe()` |
| `haishui status` / `haishui doctor` | `doctor_checks()` |
...
```python title="~/.haishui/plugins/acmebox/__init__.py"
...
class AcmeBoxProvider(TerminalEnvironmentProvider):
    ...
    def cache_path_base(self):
        return "~/.haishui"    # where synced cache files land, or None
...
```yaml title="~/.haishui/plugins/acmebox/plugin.yaml"
name: acmebox
version: 0.1.0
...
```bash
haishui plugins enable acmebox
haishui config set terminal.backend acmebox
```
