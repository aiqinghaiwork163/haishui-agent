...
by `haishui_cli/gateway_multiplex_mode.py::resolve_multiplex_mode`, which runs
the `haishui gateway migrate` preflight and keeps the gateway standalone when a
...
- `get_secret(name)` resolves: global allowlist → active scope → fallback.
  ...
  - multiplexing **on**, no scope installed: **raises `UnscopedSecretError`**
    ...
    value.
- A small allowlist (`HAISHUI_HOME`, `HAISHUI_PROFILE`, proxy settings,
...
reload helper: `haishui_cli.env_loader.load_haishui_dotenv` skips the
...
## The HAISHUI_HOME override

`haishui_constants.py` holds a context-local override consulted by
`get_haishui_home()` before the `HAISHUI_HOME` env var. Everything that resolves
...
automatically. `get_process_haishui_home()` exists for the few machine-level
assets that must not follow the override. `haishui_home_key()` gives
...
`profiles_to_serve(multiplex, profile_allowlist)` in `haishui_cli/profiles.py`
...
| Built-in tool registry | Built-ins are process-global; plugin-registered tools are overlaid per profile via `haishui_home_key()`. |
...
- `agent/secret_scope.py`, `haishui_constants.py`, `gateway/profile_routing.py`,
  `gateway/run.py` (`_profile_runtime_scope`), `haishui_cli/profiles.py`
