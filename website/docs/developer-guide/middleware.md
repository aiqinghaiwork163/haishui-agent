...
Haishui middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Haishui'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Haishui calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Haishui retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Haishui guardrails,
...
- `telemetry_schema_version`: currently `haishui.observer.v1`
- `middleware_schema_version`: currently `haishui.middleware.v1`
...
Haishui stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Haishui runs
them as a nested chain in registration order. Middleware failures are fail-open:
Haishui logs a warning and continues with the next middleware or the base
...
For each provider request, Haishui applies middleware in this order:
...
For each tool call, Haishui applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Haishui pre-execution path against the effective arguments:
...
For isolated local testing, use one `HAISHUI_HOME` for plugin enablement and the
agent run:

```bash
export HAISHUI_HOME=/tmp/haishui-middleware-test
mkdir -p "$HAISHUI_HOME"
haishui plugins enable <plugin-name>
haishui chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run haishui plugins enable <plugin-name>
uv run haishui chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["haishui_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/haishui-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Haishui expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Haishui treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Haishui preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Haishui does not convert downstream
