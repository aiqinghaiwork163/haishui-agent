...
title: "Desktop Plugin SDK (@haishui/plugin-sdk)"
description: "Extend the native Haishui Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Haishui Desktop](../user-guide/desktop.md) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `HaishuiPlugin`.
It imports one module — `@haishui/plugin-sdk` — and gets everything: the app's
...
`$HAISHUI_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across Haishui. This page is the **native
desktop app** (`haishui desktop`) SDK — the `@haishui/plugin-sdk` module and
`$HAISHUI_HOME/desktop-plugins/`. The **web dashboard** (`haishui dashboard`) has
its own, unrelated plugin system on `window.__HAISHUI_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](../user-guide/features/extending-the-dashboard.md). Python
CLI/gateway plugins are documented at [Build a Haishui Plugin](./plugins/index.md).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$HAISHUI_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Unified package** | `$HAISHUI_HOME/plugins/<id>/desktop/plugin.js` | plugins that also ship agent-side code | none — same disk pipeline |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

All three take the same `HaishuiPlugin` contract, appear in **Capabilities → Plugins**,
...
[`haishui-example-plugins`](https://github.com/NousResearch/haishui-example-plugins)
repo.

## Quick start — your first plugin

Create `$HAISHUI_HOME/desktop-plugins/hello/plugin.js` (that's `~/.haishui/...`
...
```javascript
// ~/.haishui/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@haishui/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Haishui' }),
      jsx('div', {
...
The only importable specifiers are `@haishui/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `HaishuiPlugin`:

```ts
interface HaishuiPlugin {
...
interface PluginContext {
  ...
  os: PluginOs
  /** Plugin-scoped JSON persistence (keys live under `haishui.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@haishui/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@haishui/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@haishui/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@haishui/plugin-sdk'
...
```javascript
import { Button, useTheme } from '@haishui/plugin-sdk'
...
```javascript
import { host, requestTheme } from '@haishui/plugin-sdk'
...
```javascript
import { TRANSCRIPT_DIRECTIVE_AREA } from '@haishui/plugin-sdk'
...
`window.haishui.send('get-price eth')` (or a declarative
`<button data-haishui-send="get-price eth">` — no script needed) hands that
...
```javascript
import { Contribute, WORKSPACE_PAGE_HEADER_AREA } from '@haishui/plugin-sdk'
...
for requests; `targetProfile` is the backend Haishui profile served by that route.
...
approval/turn alerts use. It fires only while the user is away from Haishui
...
ctx.os.notify({
  ...
  // Body click → focus Haishui + navigate. Same vocabulary as OS deep links:
  activate: 'haishui://index-network/intent/1',
  ...
  actions: [
    { id: 'open', label: 'Open', activate: 'haishui://index-network/intent/1' },
    { id: 'dismiss', label: 'Dismiss', onAction: () => dismiss('1') },
  ],
})
```

`activate` is deeplink-compatible: `haishui://index-network/intent/1` and the
hash path `/index-network/intent/1` resolve to the same in-app route (and the
same `haishui://…` URL works as an OS deep link). Action buttons only render on
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@haishui/plugin-sdk'
...
```javascript
import { queryClient } from '@haishui/plugin-sdk'
...
copies that half into `$HAISHUI_HOME/desktop-plugins/<id>/` beside a
`.haishui-package.json` marker, and the renderer loads it through the exact same
pipeline as the standalone disk door (hot reload included):

```
~/.haishui/plugins/<id>/           # ONE installable folder
...
is refreshed when the source `plugin.js` changes (`haishui plugins update`, or
...
a package into `~/.haishui/plugins` is inert on every surface until the user
...
backend, the remote box's `~/.haishui/plugins` is not reachable as a filesystem —
...
the `haishui://` scheme — a plain anchor on your website or README:
...
`haishui-dev://`. Full link reference:
...
`dashboard/` subfolder of a regular Haishui plugin and declare it in a
`manifest.json`:
...
haishui-agent codebase directly (`haishui_state`, `haishui_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.haishui/`)
...
(`haishui.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `HaishuiPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@haishui/plugin-sdk` alias.
2. It's still lint-fenced to `@haishui/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@haishui/plugin-sdk`, `react`,
...
| Plugin contract | `HaishuiPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginOs`, `PluginRestOptions`, `PluginNativeNotificationInput`, `PluginNotificationAction`, `HaishuiOpenTarget`, `Contribution` |
...
### Agents: the `haishui-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`haishui-desktop-plugins`** skill — it carries the same contract as this page in
...
`$HAISHUI_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `haishui logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@haishui/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.haishui/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.haishui/logs/errors.log` for
