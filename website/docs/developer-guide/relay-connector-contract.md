...
description: "Wire contract between the Haishui gateway relay adapter and external connectors (experimental)"
---
...
This document is the formal interface between the **Haishui gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `haishui gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`haishui gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `haishui` CLI.
