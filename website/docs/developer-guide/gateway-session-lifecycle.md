...
| `rewind_session(session_id, n=1)` | Back up `n` user turns via soft-delete (keeps audit trail); thin wrapper over `SessionDB.rewind_user_turn` (`haishui_state_rewind.py`), the one rewind shared with CLI `/undo`/`/retry` and the TUI. Returns `{rewound_count, turns_undone, target_text}`. |
...
The canonical transcript store is SQLite via `SessionDB` (from `haishui_state`). The
...
Counts consecutive restarts via a JSON file (`{HAISHUI_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `haishui update`, `haishui gateway restart`,
or `/restart`.
