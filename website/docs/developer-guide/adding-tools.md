...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to Haishui Agent — schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in Haishui tool** to the repository itself.
If you want a personal, project-local, or otherwise custom tool without
modifying Haishui core, use the plugin route instead:

- [Plugins](../user-guide/features/plugins.md)
- [Build a Haishui Plugin](./plugins/index.md)
...
2. **`toolsets.py`** — add tool name to `_HAISHUI_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `haishui_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `haishui_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `haishui chat -q "Use the weather tool for London"`
