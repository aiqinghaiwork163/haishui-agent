...
description: "Run Haishui Agent directly on an Android phone with Termux"
---

# Haishui on Android with Termux
...
Haishui Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Haishui from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
APT-managed Haishui installs are marked with install method `apt`. Haishui therefore does not run its Git self-updater against package-owned files; use the package manager instead:

```bash
pkg update
pkg upgrade haishui-agent
```

Packaging/repository/signing problems for this option should be reported to the community packaging repositories above. Haishui runtime bugs can still be reported here, keeping in mind that Android/Termux support is best-effort.
...
- links `haishui` into `$PREFIX/bin` so it stays on your Termux PATH
...
Haishui requires **Python >=3.11,&lt;3.14**. Current Termux ships `python`
...
### 5. Put `haishui` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/haishui" "$PREFIX/bin/haishui"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `haishui` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.haishui/.env`.
...
### `haishui doctor` says ripgrep or Node is missing
