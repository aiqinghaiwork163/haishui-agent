...
description: "A diagnostic checklist for when Haishui seems less capable than before or forgets things mid-session — model switches, context pressure, wrong context detection, and the frozen memory snapshot"
---

# Troubleshooting: "My Agent Feels Dumber"

Sometimes Haishui seems less sharp than it was yesterday, or forgets something you told it twenty minutes ago. This is almost never mysterious — there's usually one specific, checkable cause. Work through this checklist in order: the steps are sorted by how often each one turns out to be the answer.
...
**What it means:** Haishui may have auto-detected the wrong context length for your model. Set it explicitly:
...
Ollama users: if you set a custom `num_ctx`, set the matching context length in Haishui — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured. On a running gateway, edits to `model.context_length` or any `compression.*` key take effect on the next message — no restart needed.
...
**Symptom:** You asked Haishui to remember something during this session, it confirmed the save, but later in the *same* session it doesn't seem to know it.
...
**Symptom:** Haishui doesn't recall a detail from a session last week, even though you discussed it at length.

**Check:** Memory capacity and contents. The system prompt memory header shows usage (e.g., `[67% — 1,474/2,200 chars]`), and `haishui journey list` shows every saved memory entry and skill.
...
**Symptom:** Haishui used to handle a specific workflow expertly and now approaches it naively, or says it can't do something it did before.

**Check:**

- `/skills` — browse installed skills (a skill the agent relied on may have been removed).
- `/reload-skills` — re-scan `~/.haishui/skills/` for newly installed or removed skills.
...
- With `in_place: false` (legacy behavior), each compaction rotates to a **new session linked to the old one** — a titled session becomes `"my project" → "my project #2" → "my project #3"`. If you resume by title, `haishui -c "my project"` automatically picks the most recent variant.
