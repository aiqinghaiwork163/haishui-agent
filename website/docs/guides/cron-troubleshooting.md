...
description: "Diagnose and fix common Haishui cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`haishui gateway` for foreground, or `haishui gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `haishui cron tick`.
...
```bash
date
haishui cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.haishui/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.haishui/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.haishui/.env` |
...
| `local` | Write access to `~/.haishui/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `haishui cron list` for updated `last_error` field (if available).
...
### Check 5: Relay-fronted platforms (Haishui Cloud / Team Gateway)
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `haishui skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `haishui skills list`.
...
2. `~/.haishui/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `haishui cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Haishui config directory). Verify:
```bash
ls ~/.haishui/scripts/your-script.py   # Must exist
haishui cron edit <job_id> --script ~/.haishui/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `haishui skills install <skill-name>`.
...
Jobs are stored in `~/.haishui/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.haishui/cron/jobs.json
chmod 600 ~/.haishui/cron/jobs.json   # Your user should own it
```
...
```bash
haishui cron list                    # Show all jobs, states, next_run times
haishui cron run <job_id>            # Schedule for next tick (for testing)
haishui cron edit <job_id>           # Fix configuration issues
haishui logs                         # View recent Haishui logs
haishui skills list                  # Verify installed skills
```
