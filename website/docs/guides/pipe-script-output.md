...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `haishui send`."
---

# Pipe Script Output to Messaging Platforms

`haishui send` is a small, scriptable CLI that pushes a message to any
messaging platform Haishui is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | haishui send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `haishui
...
haishui send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
haishui send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
haishui send --list

# Filter by platform
haishui send --list telegram
```
...
Any platform Haishui ships adapters for works as a target:
...
`haishui send` resolves the message body in this order:

1. **Positional argument** — `haishui send --to telegram "hi"`
2. **`--file PATH`** — `haishui send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | haishui send --to telegram`

When stdin is a TTY (no pipe), Haishui does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  haishui send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `haishui send` reuses your Haishui config, the same script works on
any host where Haishui is installed — no need to export bot tokens into
...
`haishui send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  haishui send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | haishui send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.haishui/bin/haishui send \
...
```bash
./train.py --epochs 200 && \
  haishui send --to telegram "training done" || \
  haishui send --to telegram "training failed (exit $?)"
```
...
haishui send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(haishui send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `haishui send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `haishui send` calls
the platform's REST endpoint directly using credentials from
`~/.haishui/.env` and `~/.haishui/config.yaml` (or the equivalent files under
your resolved Haishui home — `%LOCALAPPDATA%\haishui` on Windows, or the profile
directory when `HAISHUI_HOME` / `-p` is set). It's a standalone subprocess
...
`Looked in: C:\Users\me\AppData\Local\haishui\.env (no DISCORD_BOT_TOKEN),
C:\Users\me\AppData\Local\haishui\config.yaml (no platforms.discord block),
...
`.env` so `haishui send` can use it. When your shell is scoped to a profile home
(`HAISHUI_HOME=<root>/profiles/<name>`) but the connected gateway runs from the
default root, the error says so — the gateway never read the profile's `.env`,
and `haishui send --list` points at the root's `channel_directory.json`.
...
pointing at the gateway; start it with `haishui gateway start` and retry.
...
# Machine-readable
haishui send --list --json
```

The listing is built from `~/.haishui/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`haishui gateway
...
| Approach | Multi-platform | Reuses Haishui creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `haishui send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`haishui send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `haishui send`.
...
- [Gateway Internals](../developer-guide/gateway-internals.md) —
  the delivery router that `haishui send` shares with cron delivery.
