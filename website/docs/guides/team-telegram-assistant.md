...
This tutorial walks you through setting up a Telegram bot powered by Haishui Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Haishui Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](../getting-started/installation.md) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.haishui/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Haishui itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Haishui Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_haishui_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Haishui Agent. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.haishui/.env`:
...
```
[Gateway] Starting Haishui Gateway...
...
# View live logs
journalctl --user -u haishui-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `haishui gateway install` to update the plist.
:::
...
```bash
haishui gateway stop && haishui gateway start
```
...
haishui pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.haishui/.env`:
...
Control how much detail the bot shows when using tools. In `~/.haishui/config.yaml`:
...
Customize how the bot communicates by editing `~/.haishui/SOUL.md`:

For a full guide, see [Use SOUL.md with Haishui](./use-soul-with-haishui.md).
...
```bash
# From the CLI
haishui cron list          # View all scheduled jobs
haishui cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u haishui-gateway` (Linux) or `~/.haishui/logs/gateway.log` (macOS) |
| Cron job output | `~/.haishui/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.haishui/cron/jobs.json` |
| Pairing data | `~/.haishui/pairing/` |
| Session history | `~/.haishui/sessions/` |
