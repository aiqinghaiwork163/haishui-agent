---
sidebar_position: 6
title: "Use MCP with Haishui"
description: "A practical guide to connecting MCP servers to Haishui Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Haishui

This guide shows how to actually use MCP with Haishui Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Haishui tool
- you want Haishui to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Haishui to internal APIs, databases, or company systems without modifying Haishui core

Do not use MCP when:
- a built-in Haishui tool already solves the job well
...
- Haishui remains the agent
- MCP servers contribute tools
- Haishui discovers those tools at startup or reload time
...
If you installed Haishui with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Haishui banner/status should show MCP integration when configured
- ask Haishui what tools it has available
- use `/reload-mcp` after config changes
- check logs if the server failed to connect
- run `haishui mcp test <server>` from a shell — it connects, lists the
...
## WSL2: bridge Haishui in WSL to Windows Chrome

This is the practical setup when:

- Haishui runs inside WSL2
...
In this setup, Haishui does **not** connect to Chrome directly. Instead:

- Haishui runs in WSL
- Haishui starts a local stdio MCP server
...
```text
Haishui (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Haishui stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Haishui core browser transport
...
```bash
haishui mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
haishui mcp test chrome-devtools-win
```

Then start a fresh Haishui session or run:
...
Once loaded, Haishui can use the MCP-prefixed browser tools directly. For example:
...
If Haishui runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Haishui from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Haishui from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Haishui:
...
2. Haishui-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Haishui will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Haishui to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want Haishui to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. Haishui remains the agent; Open Scaffold remains the repo-local record.

Add the server for one scaffolded repository:

```bash
haishui mcp add open_scaffold --command npx --args -y open-scaffold@latest mcp serve --repo /absolute/path/to/repo
haishui mcp test open_scaffold
```

Then keep the exposed surface read-oriented. Choose `select` in the `haishui mcp add` prompt, or edit `config.yaml` afterward:
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want Haishui to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize Haishui to merge, publish, deploy, or spawn runtimes.
...
Now Haishui can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Haishui core.
...
Because Haishui now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
