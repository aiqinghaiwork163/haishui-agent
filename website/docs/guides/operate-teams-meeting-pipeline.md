...
```bash
haishui teams-pipeline validate
```
...
```bash
haishui teams-pipeline token-health
haishui teams-pipeline token-health --force-refresh
```
...
```bash
haishui teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
haishui teams-pipeline maintain-subscriptions
haishui teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Haishui cron (recommended if you already run the Haishui gateway)

Haishui ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.haishui/scripts/`. First create the script:

```bash
mkdir -p ~/.haishui/scripts
cat > ~/.haishui/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec haishui teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.haishui/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
haishui cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/haishui-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Haishui Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=haishui
EnvironmentFile=/etc/haishui/env
ExecStart=/usr/local/bin/haishui teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/haishui-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Haishui Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now haishui-teams-pipeline-maintain.timer
systemctl list-timers haishui-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/haishui teams-pipeline maintain-subscriptions >> /var/log/haishui/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.haishui/.env` at the top of a wrapper script that crontab calls.
...
```bash
haishui teams-pipeline subscriptions   # should show expirationDateTime advanced
haishui teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
haishui teams-pipeline list
haishui teams-pipeline list --status failed
haishui teams-pipeline show <job-id>
```

### Replay a stored job

```bash
haishui teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
haishui teams-pipeline fetch --meeting-id <meeting-id>
haishui teams-pipeline fetch --join-web-url "<join-url>"
haishui teams-pipeline fetch --join-web-url "<join-url>" --organizer-user-id <entra-user-id>
```
...
- run `haishui teams-pipeline maintain-subscriptions --dry-run`
- inspect `haishui teams-pipeline list --status failed`
...
- run `haishui teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `haishui teams-pipeline run`
...
- [ ] `haishui teams-pipeline validate` returns an OK snapshot
- [ ] `haishui teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Haishui cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
