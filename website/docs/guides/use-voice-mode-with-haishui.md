---
sidebar_position: 8
title: "Use Voice Mode with Haishui"
description: "A practical guide to setting up and using Haishui voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Haishui
...
- you want Haishui sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Haishui.
...
## Step 1: make sure normal Haishui works first
...
Haishui supports both local and cloud speech stacks.
...
### If you use `haishui setup`

If you choose NeuTTS in the setup wizard, Haishui checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Haishui transcribes and responds
...
- asking Haishui to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Haishui loop.

## Tuning CLI behavior

### Silence threshold

If Haishui starts/stops too aggressively, tune:
...
Haishui stays a normal chat bot, but can speak replies.
...
- you want Haishui to function like a portable research or ops assistant
...
Haishui joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Haishui detects speech boundaries
- transcripts are posted in the associated text channel
- Haishui responds in text and audio
...
1. get text Haishui working
2. run `haishui setup tts` to enable voice support
