...
3. Sets `model.provider: nous` in `~/.haishui/config.yaml`
...
OAuth needs a browser, but the loopback callback runs on the machine where Haishui is running. Two options:

```bash
# Option A: SSH port forwarding (preferred)
ssh -N -L 8642:127.0.0.1:8642 user@remote-host    # in a local terminal
haishui setup --portal                              # on the remote, open the printed URL in your local browser

# Option B: device-code login (works from Cloud Shell, Codespaces, EC2 Instance Connect)
haishui auth add nous --type oauth
# Then re-run `haishui setup --portal` to wire the provider + gateway
```
...
```
Hey, search the web for "Haishui Agent release notes" and summarize the top 3 hits.
```

You should see Haishui call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.

## 5. Pick the model you actually want

`haishui setup --portal` lets you pick a model during setup, but the whole point of the subscription is access to the full catalog — switch any time with `/model` mid-session:
...
haishui config set model.default anthropic/claude-sonnet-4.6
```

### Don't pick Haishui-4 for agent work

Haishui-4-70B and Haishui-4-405B are available on the Portal at deep discounts, but they're **chat/reasoning models**, not tool-call-tuned. They will struggle with multi-step agent loops. Use them for conversation/research work through the [subscription proxy](../user-guide/features/subscription-proxy.md) from non-agent tools. For Haishui Agent itself, stick to the frontier agentic models above.

The Portal's own [info page](https://portal.nousresearch.com/info) carries this warning too — it's the official Nous guidance, not just a Haishui-side opinion.
...
These rows appear in `haishui tools` even before you've logged into Nous Portal — if you pick "Nous Subscription" without an active session, Haishui runs the Portal login inline (without changing your inference provider or your other tools).

Verify your mix with:

```bash
haishui portal tools
```
...
Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and Haishui will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
If you use [Haishui profiles](../user-guide/profiles.md) (e.g. a separate config per project), each profile is an independent credential island: a profile that has never signed in to the Portal fails closed instead of adopting another profile's session. Sign in once per profile with `haishui -p <name> portal` — when a shared Portal session already exists on the machine it offers to import it without a browser round-trip, and from then on the shared token store keeps that profile's token current. See [Profile setup](../integrations/nous-portal.md#profile-setup).

For team setups where multiple humans share a machine, each human has their own Portal account → each home directory holds its own `~/.haishui/auth.json` → no token sharing across users. This is the right boundary.

## Troubleshooting

### `haishui portal info` shows "not logged in" after `haishui setup --portal`
...
```bash
haishui config set model.provider nous
```
...
Re-verify with `haishui portal info`.
...
Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so Haishui doesn't replay it endlessly. Just log in again:

```bash
haishui auth add nous
```
...
If a model is genuinely unavailable, [open an issue](https://github.com/aiqinghaiwork163/haishui-agent/issues) — most gaps are routing config we can update.

### Billing not appearing on my Portal account

`haishui portal info` will tell you whether you're actually routing through the Portal or some other provider. Common causes:
...
- Multiple Haishui profiles where you're using the wrong one (check `haishui profile list`)

### Want to revoke and start clean

```bash
haishui auth logout nous       # wipes the local refresh token
...
- **[Subscription proxy](../user-guide/features/subscription-proxy.md)** — Use your Portal subscription from non-Haishui tools
...
- **[Profiles](../user-guide/profiles.md)** — Share one Portal login across multiple Haishui configurations
