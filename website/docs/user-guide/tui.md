...
sidebar_position: 2
title: "TUI"
description: "Launch the modern terminal UI for Haishui — mouse-friendly, rich overlays, and non-blocking input."
---

# TUI

The TUI is the modern front-end for Haishui — a terminal UI backed by the same Python runtime as the [Classic CLI](cli.md). Same agent, same sessions, same slash commands; a cleaner, more responsive surface for interacting with them.

It's the recommended way to run Haishui interactively.
...
haishui --tui -c
haishui --tui --continue
haishui --tui --resume latest

# Resume a specific session by ID or title
haishui --tui -r 20260409_000000_aa11bb
haishui --tui --resume "my t0p session"
...
```bash
export HAISHUI_TUI=1
haishui          # now uses the TUI
haishui chat     # same
```

Or make it the persistent default in `~/.haishui/config.yaml`:
...
With `display.interface: tui`, a bare `haishui` (and `haishui chat`) launches the TUI. Explicit flags always win — run `haishui --cli` to drop back to the classic REPL for a single invocation, or `haishui --tui` / `HAISHUI_TUI=1` to force the TUI when the config default is `cli`.
...
- **Instant first frame** — the banner paints before the app finishes loading, so the terminal never feels frozen while Haishui is starting.
...
- **Node.js** ≥ 20 — the TUI runs as a subprocess launched from the Python CLI. `haishui doctor` verifies this.
- **TTY** — like the classic CLI, piping stdin or running in non-interactive environments falls back to single-query mode.

On first launch Haishui installs the TUI's Node dependencies into `ui-tui/node_modules` (one-time, a few seconds). Subsequent launches are fast. If you pull a new Haishui version, the TUI bundle is rebuilt automatically when sources are newer than the dist.

:::tip Working across git worktrees?
Contributors who run `haishui --tui --dev` from many worktrees can share one `node_modules` instead of installing per checkout — see [TUI & Desktop from Worktrees](../developer-guide/worktree-ui-dev.md).
:::

### External prebuild

Distributions that ship a prebuilt bundle (Nix, system packages) can point Haishui at it:

```bash
export HAISHUI_TUI_DIR=/path/to/prebuilt/ui-tui
haishui --tui
```
...
| `/reload` | Re-reads `~/.haishui/.env` into the running TUI process so newly added API keys take effect without a restart |
...
Use the live session switcher when you want one terminal to act as a dispatcher for several TUI sessions. It lists only sessions that are currently live in this TUI process; closed sessions remain saved transcripts and can still be reopened with `/resume` or `haishui --tui --resume <id-or-title>`.
...
<img alt="Haishui TUI Session Orchestrator with one live session and a +new row" src="/docs/img/docs/tui-session-orchestrator/session-orchestrator.png" />

<video controls muted loop playsInline src="/docs/img/docs/tui-session-orchestrator/session-orchestrator-demo.mp4" title="Haishui TUI Session Orchestrator demo" style={{maxWidth: '100%'}}></video>
...
1. `HAISHUI_TUI_THEME` env var — highest priority. Values: `light`, `dark`, or a raw 6-char background hex (e.g. `ffffff`, `1a1a2e`).
...
```bash
export HAISHUI_TUI_THEME=light
```

## Busy indicator styles

The status-bar busy indicator is pluggable — the default rotates Haishui' kawaii face palette every 2.5 seconds during agent work. Pick a different style via config or the `/indicator` slash command:
...
By default, `haishui --tui` starts a fresh session each launch. To re-attach to the most recent TUI session automatically (useful when your terminal or SSH connection drops unexpectedly), opt in:

```bash
export HAISHUI_TUI_RESUME=1          # most-recent TUI session
# or:
export HAISHUI_TUI_RESUME=<session-id>   # specific session
```
...
- **Working directory with git branch** — `~/projects/haishui-agent (docs/two-week-gap-sweep)`. The branch suffix updates when you `git checkout` in a side terminal (mtime-cached) so the TUI reflects your actual active branch, not whatever it was at launch.
...
- **`⚠ YOLO`** — visible warning whenever YOLO mode is on (`haishui --yolo`, `/yolo`, or `HAISHUI_YOLO_MODE=1`). The same badge also appears in the startup banner so you cannot launch an auto-approving session without noticing.

## Configuration

The TUI respects all standard Haishui config: `~/.haishui/config.yaml`, profiles, personalities, skins, quick commands, credential pools, memory providers, tool/skill enablement. No TUI-specific config file exists.
...
Sessions are shared between the TUI and the classic CLI — both write to the same `~/.haishui/state.db`. You can start a session in one, resume in the other. The session picker surfaces sessions from both sources, with a source tag.
...
You may see a `HAISHUI_TUI_GATEWAY_URL` env var referenced in the codebase or logs. This is an **internal wiring detail of the web dashboard**, not a user-facing remote-attach knob. When you open the dashboard's "Chat" tab (`haishui dashboard` → `/chat`), the dashboard's web server spawns an embedded TUI child process and injects `HAISHUI_TUI_GATEWAY_URL` so that child attaches to the dashboard's own in-process `tui_gateway` over a loopback WebSocket (`/api/ws`). The `/api/ws` endpoint exists only inside the dashboard server (`haishui_cli/web_server.py`) and is bound to that process's lifetime and auth.

There is no general "point any TUI at any standalone gateway port" mode. In particular, the OpenAI-compatible API server (`haishui gateway` / the `api_server` platform) does **not** serve `/api/ws` — it's the model-backend surface (`/v1/chat/completions`, `/v1/models`, …) and deliberately does not expose the TUI's JSON-RPC control channel. Setting `HAISHUI_TUI_GATEWAY_URL` to that port will 404.

### If the connection drops

- **Spawned gateway (default):** when the gateway process dies mid-session the TUI says *Haishui stopped unexpectedly — restarting and reopening your chat*, respawns it (bounded to a few attempts per minute) and reopens the same saved session. The reply that was in flight is lost with the process.
- **Attached gateway (dashboard chat):** when only the WebSocket drops the TUI says *Connection to Haishui lost — reconnecting and reopening your chat…*, reconnects with growing backoff and reattaches to the same session — including a reply that is still streaming on the backend. Nothing is resubmitted.

If you want multiple surfaces to share one set of sessions, use the shared `~/.haishui/state.db` (see [Sessions](sessions.md)) or the web dashboard's embedded chat (see [Web Dashboard](features/web-dashboard.md#chat)) — not a hand-set gateway URL.

## Reverting to the classic CLI

Launching `haishui` (without `--tui`) stays on the classic CLI by default. To make a machine prefer the TUI, set `display.interface: tui` in `~/.haishui/config.yaml` (persistent) or `HAISHUI_TUI=1` in your shell profile (per-shell). To go back, set `interface: cli` / unset the env var, or pass `haishui --cli` for a one-off.

If the TUI fails to launch (no Node, missing bundle, TTY issue), Haishui prints a diagnostic and falls back — rather than leaving you stuck.
