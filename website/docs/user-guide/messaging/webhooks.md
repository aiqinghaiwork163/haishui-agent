...
<div style={{position: 'relative', width: '100%', aspectRatio: '16 / 9', marginBottom: '1.5rem'}}>
  <iframe
    ...
    title="Haishui Agent — Webhooks Tutorial"
...
1. Enable via `haishui gateway setup` or environment variables
2. Define routes in `config.yaml` **or** create them dynamically with `haishui webhook subscribe`
...
| `script` | No | Filter/transform script under `~/.haishui/scripts/`. The webhook payload is passed as JSON on stdin. JSON object stdout replaces the payload before templating; text stdout is exposed as `script_output`; empty stdout, `[SILENT]`, or a nonzero exit code ignores the webhook. |
| `skills` | No | List of skill names to load for the agent run. |
| `toolsets` | No | List of toolset keys (e.g. `["terminal", "file", "web"]`) that **replaces** the platform-level webhook toolset for runs triggered by this route only. Manual config edit only — not settable via `haishui webhook subscribe`, so agent-created subscriptions cannot self-grant elevated tools. Names are validated the same way as `platform_toolsets` entries (unknown or platform-restricted names are dropped). See [Per-route toolsets](#per-route-toolsets). |
          ...
          filters:
            ...
            - any:
                ...
                - field: "payload.project_id"
                  in_file: "~/.haishui/data/todoist/watchlist.json"
...
- Pending groups are flushed (dispatched immediately) when the adapter disconnects — a gateway reconnect or `haishui gateway stop` — not dropped. Buffered events live in memory, so a hard process kill loses at most the current window's buffered burst.
...
Use `script` when declarative filters are not enough. Scripts must live under `~/.haishui/scripts/` for the active profile; relative paths resolve there, and path traversal outside that directory is blocked. `.sh` and `.bash` scripts run with bash, and all other extensions run with the current Python interpreter.

The route payload is sent to stdin as JSON:

```python
# ~/.haishui/scripts/todoist-haishui-label.py
import json
import sys

payload = json.load(sys.stdin)
labels = payload.get("payload", {}).get("labels", [])
if "haishui" not in labels:
    print("[SILENT]")
    raise SystemExit(0)
...
- Empty stdout, exact `[SILENT]`, `{"__haishui_ignore__": true}`, timeout, missing script, or nonzero exit code returns HTTP 200 with `{"status":"ignored","reason":"script"}`.
...
Add the `github-pr` route to your `~/.haishui/config.yaml` as shown in the example above.
...
Open a pull request on the repository. The webhook fires, Haishui processes the event, and posts a review comment on the PR.
...
```bash
haishui webhook subscribe antenna-matches \
  --deliver telegram \
...
```bash
haishui webhook subscribe pr-feedback \
  --events "pull_request_review" \
...
In addition to static routes in `config.yaml`, you can create webhook subscriptions dynamically using the `haishui webhook` CLI command. This is especially useful when the agent itself needs to set up event-driven triggers.

### Create a subscription

```bash
haishui webhook subscribe github-issues \
  --events "issues" \
...
```bash
haishui webhook list
```

### Remove a subscription

```bash
haishui webhook remove github-issues
```

### Test a subscription

```bash
haishui webhook test github-issues
haishui webhook test github-issues --payload '{"issue": {"number": 42, "title": "Test"}}'
```

### How dynamic subscriptions work

- Subscriptions are stored in `~/.haishui/webhook_subscriptions.json`
...
The agent can create subscriptions via the terminal tool when guided by the `webhook-subscriptions` skill. Ask the agent to "set up a webhook for GitHub issues" and it will run the appropriate `haishui webhook subscribe` command.
...
For dynamic subscriptions, add the `toolsets` key by editing `~/.haishui/webhook_subscriptions.json` directly:
...
- `haishui webhook subscribe` deliberately does **not** accept a toolsets flag. Granting elevated tools is a manual config-file edit, so an agent creating its own subscription at runtime cannot self-grant `terminal`.
...
This is the same trust model that applies to everything the agent reads: web pages, files, and tool output are all untrusted input. Haishui does not — and cannot reliably — sanitize untrusted text with a blocklist; phrasing, encoding, and translation make that trivially bypassable. **The trust boundary is the agent's capability surface, not the input channel.** Harden there:
...
- Run the gateway in foreground to see logs: `haishui gateway run`
