...
sidebar_position: 30
title: "Haishui Relay"
description: "Connect Haishui to messaging platforms through a relay connector that owns the platform credentials — enrollment, capabilities, config, and troubleshooting"
---

# Haishui Relay (Connector)
...
Haishui Relay is not a chat platform itself — it is a **connector system** that
...
1. Resolves a fresh Nous Portal access token from your existing login
   (`~/.haishui/auth.json`) — this proves which Nous org (tenant) you own. If
...
3. The connector verifies the token (signature, single-use, tenant match),
   ...
   once.
4. Persists the credentials into `~/.haishui/.env`:
...
`haishui gateway enroll` refuses to run in managed/hosted installs — there the
...
`haishui gateway enroll` remains available while runtime relay is disabled.

| Setting | Where | Meaning |
|---------|-------|---------|
| `GATEWAY_RELAY_URL` | env (`~/.haishui/.env`) | Connector relay WebSocket URL. Enables relay unless explicitly disabled in platform configuration. |
...
token. Re-login with `haishui auth add nous` (or `haishui setup`) and retry.
...
startup. Restart it (`haishui gateway restart`).
