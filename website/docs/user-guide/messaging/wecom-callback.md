...
Connect Haishui to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Haishui supports two WeCom integration modes:
...
> Run `haishui gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Haishui decrypts the message, queues it for the agent
...
(Use `haishui gateway start` only after `haishui gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Haishui and the token the
...
in `~/.haishui/.env` values around `=` will also break signature checks. After
fixing, restart `haishui gateway run`.
...
Check `haishui gateway run` logs for the bound host/port. If the adapter bound to
