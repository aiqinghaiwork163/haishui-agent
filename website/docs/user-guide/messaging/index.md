...
description: "Chat with Haishui from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---
...
For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](../features/voice-mode.md) and [Use Voice Mode with Haishui](../../guides/use-voice-mode-with-haishui.md).
...
saved by `haishui gateway setup` can enable a credential-based platform without a
...
```mermaid
flowchart TB
    subgraph Gateway["Haishui Gateway"]
...
For group chats, hooks, and automation flows, Haishui supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Haishui keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Haishui does not hide failures just because the text resembles a silence token.
...
```bash
haishui gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
haishui gateway              # Run in foreground
haishui gateway setup        # Configure messaging platforms interactively
haishui gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo haishui gateway install --system   # Linux only: install a boot-time system service
haishui gateway start        # Start the default service
haishui gateway stop         # Stop the default service
haishui gateway status       # Check default service status
haishui gateway status --system         # Linux only: inspect the system service explicitly
```

### Stack dump on demand (`SIGUSR2`)

On Linux and macOS, `kill -USR2 <gateway pid>` appends a dump of every thread's
stack to `~/.haishui/logs/gateway_faulthandler.log` and the gateway keeps
...
supervisor restarts the process. `haishui gateway status` renders that record as
...
stamp is more than 120 s old, `haishui gateway status` prints
...
```yaml title="~/.haishui/config.yaml"
gateway:
...
```bash
haishui gateway install --force
```

A positive value makes the generated unit use `Type=notify`,
`NotifyAccess=main`, and the matching `WatchdogSec`. Haishui sends heartbeats
...
| `/update` | Update Haishui Agent to the latest version |
...
Different channels can run different models and personas from a **single gateway** — e.g. a cheap fast model in `#daily` and a frontier model with a specialist prompt in `#dev`. Configure `channel_overrides` under the platform in `~/.haishui/config.yaml`:
...
haishui pairing approve telegram XKGH5N7P

# Other pairing commands:
haishui pairing list          # View pending + approved users
haishui pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Haishui appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.haishui/config.yaml`:
...
Setting `display.tool_progress: log` sends **no** progress bubbles to chat. Instead, each tool call is appended as a line to `~/.haishui/logs/tool_calls.log` — a rotating audit file (5 MB × 3 backups) run through the same secret-redacting formatter as regular logs, so credentials never land on disk. Use it when you want a full tool-call trail without any chat noise.

### Configurable status phrases

Long-running gateway status lines ("still working…"-style heartbeats) draw from a phrase catalog. Built-in defaults ship in `gateway/assets/status_phrases.yaml`; you can add your own with profile-portable files under `HAISHUI_HOME`:

- `~/.haishui/status_phrases.yaml` or any `*.yaml` in `~/.haishui/status_phrases/` (conventional paths, auto-loaded), or
- point config at a relative path:

```yaml
display:
  status_phrases:
    path: status_phrases/whatsapp.yaml  # relative to HAISHUI_HOME
...
Off by default. When enabled, Haishui prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.haishui/config.yaml`:
...
```bash
HAISHUI_BACKGROUND_NOTIFICATIONS=result
```
...
haishui gateway start                 # Start the service
haishui gateway stop                  # Stop the service
haishui gateway status                # Check status
journalctl --user -u haishui-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo haishui gateway install --system
sudo haishui gateway start --system
sudo haishui gateway status --system
journalctl -u haishui-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit Haishui installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit haishui-gateway` (or `sudo systemctl edit haishui-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `haishui update`. When `haishui update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart haishui-gateway` command (it never blocks on an interactive password prompt).
...
```bash
haishui gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `haishui update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo haishui update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/haishui-gateway`:

```
haishui ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed haishui-gateway*, /usr/bin/systemctl --no-ask-password start haishui-gateway*, /usr/bin/systemctl --no-ask-password restart haishui-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Haishui will warn if it detects both because start/stop/status behavior gets ambiguous.

:::note Inside a container, only the system scope is offered
`haishui gateway install` (and the `haishui gateway setup` wizard) refuse to install a **user** service when Haishui detects it is running inside a container. A user unit lands in `~/.config/systemd/user`, and when that home is bind-mounted from the host (podman/distrobox), the host's own `systemd --user` enables and starts the same unit — a second gateway polling the same bot token. Run the gateway as the container's main process (`haishui gateway run`, with a container restart policy), or in a systemd container (systemd as PID 1) install the isolated system scope: `sudo haishui gateway install --system --run-as-user <user>`.
:::

:::info Multiple installations
If you run multiple Haishui installations on the same machine (with different `HAISHUI_HOME` directories), each gets its own systemd service name. The default `~/.haishui` uses `haishui-gateway`; other installations use `haishui-gateway-<hash>`. The `haishui gateway` commands automatically target the correct service for your current `HAISHUI_HOME`.
:::

### macOS (launchd)

```bash
haishui gateway install               # Install as launchd agent
haishui gateway start                 # Start the service
haishui gateway stop                  # Stop the service
haishui gateway status                # Check status
tail -f ~/.haishui/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.haishui.gateway.plist`. It includes three environment variables:
...
- **HAISHUI_HOME** — scopes the gateway to your Haishui installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `haishui gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `HAISHUI_HOME` directory gets its own launchd label. The default `~/.haishui` uses `ai.haishui.gateway`; other installations use `ai.haishui.gateway-<suffix>`.
:::

### Windows (Task Scheduler)

```powershell
haishui gateway install               # Register the Haishui_Gateway Scheduled Task (runs at logon)
haishui gateway start                 # Start the gateway hidden, without a console window
haishui gateway stop                  # Drain and stop the service
haishui gateway status                # Check status, including registration drift
```

The Scheduled Task runs `wscript.exe` on a generated `.vbs` launcher under `%USERPROFILE%\.haishui\gateway-service\`. The launcher starts `python.exe -m haishui_cli.main gateway run` with a hidden window and **exits immediately** — by design: `wscript.exe` has no console, so at logon it never receives the `CTRL_CLOSE_EVENT` that kills a `cmd.exe`-hosted gateway, and the gateway inherits one hidden console instead of every subprocess flashing its own (see `haishui_cli/gateway_windows.py::_build_gateway_vbs_script`).

:::warning RestartOnFailure covers the launcher, not the gateway
Because the launcher returns as soon as the gateway is spawned, Task Scheduler only ever sees the launcher's exit code. The `<RestartOnFailure>` policy in the registered task therefore fires only when `wscript.exe` itself fails to start the gateway — it does **not** restart a gateway that crashes or is killed later. Gateway auto-restart on Windows relies on the gateway's own in-process restart path (`/restart`, updates, and the `haishui gateway restart` command); a gateway killed from outside stays down until `haishui gateway start` or `schtasks /Run /TN <task>`.
:::

`haishui gateway install` writes the task from the current template; a task registered by an older build would otherwise keep its old settings (no `RestartOnFailure`, no logon `Delay`, an older launcher command line) indefinitely. `haishui gateway status` compares the registered task with the current template and warns when it predates it:

```
⚠ Scheduled Task registration predates the current template (missing: RestartOnFailure, LogonTrigger Delay; version 1.3 vs 1.4)
  Repair: haishui gateway start  (or: haishui gateway install)
```

`haishui gateway start` and `haishui update` run the same comparison and re-register a drifted task from the current template automatically (like the systemd unit refresh on Linux); when `schtasks` refuses without elevation, re-run `haishui gateway install`, which can request administrator approval. The check is silent when the task cannot be queried, and it only inspects a few settings Haishui owns (task version, `RestartOnFailure`, the logon trigger delay and the launcher arguments), so deliberate local edits elsewhere in the task are not flagged.
...
|----------|---------|--------------|
| CLI | `haishui-cli` | Full access |
| Telegram | `haishui-telegram` | Full tools including terminal |
| Discord | `haishui-discord` | Full tools including terminal |
| WhatsApp | `haishui-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `haishui-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `haishui-slack` | Full tools including terminal |
| Google Chat | `haishui-google_chat` | Full tools including terminal |
| Signal | `haishui-signal` | Full tools including terminal |
| SMS | `haishui-sms` | Full tools including terminal |
| Email | `haishui-email` | Full tools including terminal |
| Home Assistant | `haishui-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `haishui-mattermost` | Full tools including terminal |
| Matrix | `haishui-matrix` | Full tools including terminal |
| DingTalk | `haishui-dingtalk` | Full tools including terminal |
| Feishu/Lark | `haishui-feishu` | Full tools including terminal |
| WeCom | `haishui-wecom` | Full tools including terminal |
| WeCom Callback | `haishui-wecom-callback` | Full tools including terminal |
| Weixin | `haishui-weixin` | Full tools including terminal |
| BlueBubbles | `haishui-bluebubbles` | Full tools including terminal |
| QQBot | `haishui-qqbot` | Full tools including terminal |
| Yuanbao | `haishui-yuanbao` | Full tools including terminal |
| Microsoft Teams | `haishui-teams` | Full tools including terminal |
| API Server | `haishui-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `haishui-webhook` | Full tools including terminal |
| Raft | `haishui-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
`platforms.<name>.enabled: false` in `~/.haishui/config.yaml` is authoritative.
...
1. **Gateway log** (`~/.haishui/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
