...
`~/.haishui/config.yaml`, `~/.haishui/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/haishui`:

```text
/etc/haishui/
├── config.yaml     # managed config layer (wins over ~/.haishui/config.yaml)
└── .env            # managed env layer (wins over ~/.haishui/.env + shell)
```
...
The location can be relocated with the `HAISHUI_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `HAISHUI_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Haishui.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export HAISHUI_MANAGED_DIR=/opt/org/haishui-policy
```

:::warning
A user who can set `HAISHUI_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `haishui doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/haishui/config.yaml` (managed) | `/etc/haishui/.env` (managed) |
| 2 | `~/.haishui/config.yaml` (user) | `~/.haishui/.env` (user) |
...
```bash
haishui config        # shows a header naming the managed source + the pinned keys
haishui doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Haishui refuses and names the source:

```bash
$ haishui config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/haishui/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `haishui config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/haishui

# Pin some config values for every user on this machine
sudo tee /etc/haishui/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/haishui/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/haishui
sudo chmod 0644 /etc/haishui/config.yaml /etc/haishui/.env
```

Changes take effect on the next Haishui start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`haishui doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Haishui as `root`), managed scope is advisory.
