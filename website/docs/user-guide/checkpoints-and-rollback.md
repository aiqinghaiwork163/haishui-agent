...
Haishui Agent can automatically snapshot your project before **destructive operations** and restore it with a single command. Checkpoints are **opt-in** as of v2 — most users never use `/rollback`, and the shadow-store storage is non-trivial over time, so the default is off.

Enable checkpoints per-session with `--checkpoints`:

```bash
haishui chat --checkpoints
```

Or enable globally in `~/.haishui/config.yaml`:
...
This safety net is powered by an internal **Checkpoint Manager** that keeps a single shared shadow git repository under `~/.haishui/checkpoints/store/` — your real project `.git` is never touched. Every project the agent works in shares the same store, so git's content-addressable object DB deduplicates across projects and across turns.
...
| Command | Description |
|---------|-------------|
| `haishui checkpoints` | Show total size, project count, per-project breakdown |
| `haishui checkpoints status` | Same as bare `checkpoints` |
| `haishui checkpoints list` | Alias for `status` |
| `haishui checkpoints prune` | Force a sweep: delete orphans/stale, GC, enforce size cap |
| `haishui checkpoints clear` | Nuke the entire checkpoint base (asks first) |
| `haishui checkpoints clear-legacy` | Delete only the `legacy-*` archives from v1 migration |

## How Checkpoints Work

At a high level:

- Haishui detects when tools are about to **modify files** in your working tree.
- Once per conversation turn (per directory), it:
  - Resolves a reasonable project root for the file.
  - Initialises or reuses the **single shared shadow store** at `~/.haishui/checkpoints/store/`.
  - Stages into a per-project index, builds a tree, and commits to a per-project ref (`refs/haishui/<project-hash>`).
...
checkpoints:
  ...
  # Auto-maintenance (on by default): sweep ~/.haishui/checkpoints/ in the
  ...
  # cleanup is only ever done via the explicit `haishui checkpoints prune`
...
When `enabled: false`, the Checkpoint Manager is a no-op and never attempts git operations. When `auto_prune: false`, the store grows until you run `haishui checkpoints prune` manually.
...
Haishui responds with a formatted list showing change statistics:
...
```text
Checkpoint base: /home/you/.haishui/checkpoints
Total size:      142.3 MB
  store/         138.1 MB
  legacy-*       4.2 MB
Projects:        12
  ...
  /home/you/code/haishui-agent                                        20       2h ago  live
...
Clear with: haishui checkpoints clear-legacy
```

Force a full sweep (ignores the 24h idempotency marker):

```bash
haishui checkpoints prune --retention-days 3 --max-size-mb 200
```
...
`/rollback <N>` restores only the files Haishui itself changed. Every successful
...
Haishui last wrote (you edited it afterwards, or Haishui never touched it) is
...
If the ledger is empty (a store created before this feature, or Haishui hasn't
...
~/.haishui/checkpoints/
  ...
  │   ├── refs/haishui/<hash> # per-project branch tip
...
Each `<hash>` is derived from the absolute path of the working directory. You normally never need to touch these manually — use `haishui checkpoints status` / `prune` / `clear` instead.

### Migration from v1

Before the v2 rewrite, each working directory got its own complete shadow git repo directly under `~/.haishui/checkpoints/<hash>/`. That layout couldn't dedup objects across projects and had a documented no-op pruner — the store would grow without bound.

On first v2 run, any pre-v2 shadow repos are moved into `~/.haishui/checkpoints/legacy-<timestamp>/` so the new single-store layout starts clean. Old `/rollback` history is still reachable by manually inspecting the legacy archive with `git`; once you're confident you don't need it, run:

```bash
haishui checkpoints clear-legacy
```
...
- **Enable checkpoints only when you need them** — `haishui chat --checkpoints` or per-profile `enabled: true`.
...
- **Check `haishui checkpoints status` occasionally** if you use checkpoints regularly — shows which projects are active and what the store costs you.
- **Combine with Git worktrees** for maximum safety — keep each Haishui session in its own worktree/branch, with checkpoints as an extra layer.
