...
Haishui Agent is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, Haishui checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.haishui/config.yaml`:
...
| `timeout` | `300` | Seconds Haishui waits for an approval reply before timing out. |
...
| `destructive_slash_confirm` | `true` | When true, destructive session slash commands (`/clear`, `/new`, `/reset`, `/undo`) prompt before discarding conversation state. Three-option dialog (Approve Once / Always Approve / Cancel) routed through native yes/no buttons on Telegram, Discord, and Slack; text fallback elsewhere. Users who click **Always Approve** flip this key to `false`. The TUI also honors this setting for its `/clear`, `/new`, and `/reset` modal; `HAISHUI_TUI_NO_CONFIRM=1` force-skips that modal regardless of the configured value. |
...
1. **CLI flag**: Start a session with `haishui --yolo` or `haishui chat --yolo`
2. **Slash command**: Type `/yolo` during a session to toggle it on/off
3. **Environment variable**: Set `HAISHUI_YOLO_MODE=1`
...
YOLO mode is available in both CLI and gateway sessions. Internally, it sets the `HAISHUI_YOLO_MODE` environment variable which is checked before every command execution.

When YOLO is active, Haishui shows two persistent visual reminders so it's hard to forget that approval prompts are bypassed:
...
`HAISHUI_SUPERVISED_CHILD` marker they export.
...
the job uses KeepAlive or controls Haishui.
...
Some commands are so catastrophic — irreversible filesystem wipes, fork bombs, direct block-device writes — that Haishui refuses to run them **regardless** of:
...
Configure the timeout in `~/.haishui/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.haishui/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.haishui/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` haishui/gateway | Self-termination prevention |
...
The `HAISHUI_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.haishui/config.yaml`:
...
unattended ones: a cron job, `haishui chat -q` run or webhook session under
...
re-save it with `haishui config edit`. Other malformed values are ignored with
a warning; they never become per-character approvals. Loading does not rewrite
your configuration file.

:::tip
Use `haishui config edit` to review or remove patterns from your permanent allowlist.
:::

:::caution
The list is read when Haishui starts. A pattern you remove while a session is
already running stays approved in that session until it next writes the file
(the next time you answer `always` to a prompt) or you restart Haishui. If you
removed it for safety reasons, restart.
:::

### Mining Approval History (`haishui approvals suggest`)
...
```bash
haishui approvals suggest            # dry run — prints a numbered proposal
haishui approvals suggest --apply 1,3  # merge picks into command_allowlist
haishui approvals suggest --json     # machine-readable output
```

The command scans the session database (`~/.haishui/state.db`) for
...
Before `write_file` or `patch` touches disk, Haishui checks the target path against a denylist and an optional sandbox. Blocked writes return an error to the agent immediately — **there is no approval prompt** and no way to override from the chat UI. The model may still claim the edit succeeded; when `display.file_mutation_verifier` is on (default), trust the [file-mutation verifier footer](./configuration.md#file-mutation-verifier) over the assistant's closing summary.

### Protected paths (always blocked)

These categories are always denied, even when `HAISHUI_WRITE_SAFE_ROOT` is unset:

| Category | Examples |
|----------|----------|
| OS credential stores | `~/.ssh/` (keys, `authorized_keys`), `~/.aws/`, `~/.kube/`, `/etc/sudoers`, `~/.netrc` |
| Haishui secret stores | `.env`, `.anthropic_oauth.json`, `auth/google_oauth.json`, Bitwarden cache (`cache/bws_cache.json`, `cache/bws_cache.enc.json`), `vault/`, `browser-profile/`, `mcp-tokens/`, `pairing/` under HAISHUI_HOME (active profile and global root). Control files (`auth.json`, `config.yaml`, `webhook_subscriptions.json`) are read-denied but stay writable. |
...
Sensitive paths inside the safe root are still blocked — pointing `HAISHUI_WRITE_SAFE_ROOT` at `$HOME` does not allow writing `~/.ssh/id_rsa`.

The `~` in the OS-credential rows means *every* home a write can land in, not just the process `HOME`: the OS user's real home, the profile home (`{HAISHUI_HOME}/home` under `TERMINAL_HOME_MODE=profile`, containers and spawned workers, where the process `HOME` is pinned), and named accounts (`~root/.ssh/authorized_keys`). An absolute path to the real home's `~/.aws/credentials` is denied even when the agent process runs with `HOME` pointed elsewhere.

Safe-root violations return `Write denied: '…' is outside HAISHUI_WRITE_SAFE_ROOT (…)`. Credential-path blocks use `Write denied: '…' is a protected system/credential file.`
...
### HAISHUI_WRITE_SAFE_ROOT (optional sandbox)
...
To allow both a workspace and Haishui home:

```bash
export HAISHUI_WRITE_SAFE_ROOT=/path/to/project:/home/you/.haishui
```

Unset the variable to restore unrestricted writes (subject to the protected-path denylist). Full reference: [HAISHUI_WRITE_SAFE_ROOT](../reference/environment-variables.md#haishui_write_safe_root).

### Cron and other Haishui state

Do not ask the agent to `patch` `~/.haishui/cron/jobs.json` directly. Use the `cronjob` tool, [`haishui cron`](./features/cron.md), or `/cron` — they update the job store through the supported API. The same applies to other Haishui control files when write safety blocks direct edits.
...
When running the messaging gateway, Haishui controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.haishui/.env`:
...
Set GATEWAY_ALLOW_ALL_USERS=true in ~/.haishui/.env to allow open access,
...
For more flexible authorization, Haishui includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `haishui pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.haishui/config.yaml`:
...
haishui pairing list

# Approve a pairing code
haishui pairing approve telegram ABC12DEF

# Revoke a user's access
haishui pairing revoke telegram 123456789

# Clear all pending codes
haishui pairing clear-pending
```

:::tip Docker users: run pairing commands as the `haishui` user
The official Docker image runs the gateway as the unprivileged `haishui` user
...
Always pass `-u haishui`:

```bash
docker exec -u haishui haishui-agent haishui pairing approve telegram ABC12DEF
```
...
When using the `docker` terminal backend, Haishui applies strict security hardening to every container.
...
Container resources are configurable in `~/.haishui/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.haishui/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `HAISHUI_HOME`. Skills declare these in frontmatter:
...
When loaded, Haishui checks if these files exist in the active profile's `HAISHUI_HOME` and registers them for mounting:
...
With the switch off Haishui never reads or refreshes those files: the `claude_code` credential-pool row disappears, `haishui auth list` prints one line saying so, and the log carries one INFO line per process. Only automatic adoption is affected — `haishui auth add openai-codex` still asks before importing an existing Codex CLI login. Add your own logins with `haishui auth add anthropic` / `haishui auth add openai-codex`.
...
| **terminal** (local) | Blocks explicit Haishui infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- Haishui infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
Haishui integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
Tirith ships prebuilt binaries for Linux (x86_64 / aarch64) and macOS (x86_64 / arm64). On platforms with no prebuilt binary (Windows, etc.), tirith is silently skipped — pattern-matching guards still run, and the CLI does not surface an "unavailable" banner. To use tirith on Windows, run Haishui under WSL.
...
Your own `SOUL.md` in `HAISHUI_HOME` is treated differently: it is a file you wrote (file-tool writes to it
need your approval, and project checkouts never supply it), so a scanner hit there **does not block the
file**. Haishui logs a warning naming the matched pattern, loads the file as usual, and `/context` lists it as
...
the file. The exception does not extend to a `SOUL.md` shipped by a profile distribution: `haishui profile
install <git-url>` and `haishui profile update` copy a third party's `SOUL.md` into the profile home without
...
4. **Store secrets securely** — keep API keys in `~/.haishui/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.haishui/logs/` for unauthorized access attempts
10. **Keep updated** — run `haishui update` regularly for security patches
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.haishui/.env`:
...
TERMINAL_SSH_USER=haishui
TERMINAL_SSH_KEY=~/.ssh/haishui_agent_key
```
...
Haishui ships with a built-in advisory scanner that flags Python packages in the active venv that match a curated catalog of known-compromised versions (supply-chain worms like the May 2026 `mistralai 2.4.6` poisoning). Implementation lives in `haishui_cli/security_advisories.py`.

How it runs:

- **CLI startup banner.** A one-line warning is printed if any advisory matches, with a pointer to `haishui doctor` for the full remediation.
- **`haishui doctor`.** Surfaces every active advisory with version specifics and 2-4 step remediation instructions.
...
```bash
haishui doctor --ack <advisory-id>
```
...
Many features (Mistral TTS, ElevenLabs, Honcho memory, Bedrock, Slack, Matrix, …) depend on Python packages that not every user needs. Haishui installs these **lazily** on first use rather than eagerly under `haishui-agent[all]`. The implementation lives in `tools/lazy_deps.py`.
...
3. If the install fails or the user has disabled lazy installs, the call raises `FeatureUnavailable` with the actual pip stderr and a pointer at `haishui tools`.
...
When disabled, backends that need optional deps will tell the user to run the install manually (`pip install …`) or pick a different backend via `haishui tools`.
