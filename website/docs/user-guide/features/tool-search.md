...
**Tool Search** is Haishui' opt-in progressive-disclosure layer for that
...
:::info Built-in Haishui tools never defer
The tools that make up Haishui' core capability set (`terminal`,
`read_file`, `write_file`, `patch`, `search_files`, `todo`, `memory`,
`browser_*`, `web_search`, `web_extract`, `clarify`, `execute_code`,
`delegate_task`, `session_search`, and the rest of
`_HAISHUI_CORE_TOOLS`) are *always* loaded directly. Only MCP tools and
...
When the model invokes `tool_call`, Haishui **unwraps the bridge** and
...
- **No provider-native validation for deferred schemas.** `tool_describe`
  ...
  only the generic `tool_call.arguments` object. Haishui therefore coerces and
  validates the underlying arguments locally before dispatch; the concrete
  tool or MCP server remains responsible for schemas Haishui cannot safely
...
- **No JS sandbox.** Haishui uses the simpler "structured tools" mode
