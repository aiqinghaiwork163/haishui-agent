...
Inspired by Prime-Agent's `/heartbeat`. The Haishui adaptation keeps the strict message-flow invariants: the heartbeat is injected only between turns (never mid-run), as a plain user-role message.

## Heartbeat vs cron: which one do I want?

They look similar but serve different jobs:

| | `/heartbeat` | [`haishui cron`](./cron) |
|---|---|---|
...
`/hb` is an alias. Works on the CLI, the TUI / Desktop app, and gateway platforms (on Slack, use `/haishui heartbeat …`).
...
Haishui: [Heartbeat — recurring instruction, fires every 15m]
