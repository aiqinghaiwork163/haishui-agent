...
When Haishui Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.haishui/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.haishui/config.yaml` (switchable per session
via `/personality`).
...
If you use Haishui' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.haishui/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Haishui Agent's deliverable
