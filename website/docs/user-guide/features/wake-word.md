...
sidebar_position: 11
title: "Wake Word"
description: "Hands-free 'Hey Haishui' wake word — start a voice session by speaking, the 'Hey Siri' way"
---

# Wake Word ("Hey Haishui")

The wake word turns Haishui into a hands-free assistant across the CLI, TUI, and
desktop app: with one setting on, Haishui listens in the background for a spoken
trigger phrase. Say it, and Haishui starts a fresh session, opens the microphone,
...
When the desktop app connects to a **remote** Haishui backend (for example a
...
Haishui supports **client capture** for that case:
...
|--------|------|---------|-------|
| **openWakeWord** (default) | Free | None | Local ONNX models. Ships a bundled **"hey haishui"** model (default); also supports `hey_jarvis`, `alexa`, `hey_mycroft`, … and custom models |
...
By default the phrase is **"hey haishui"** — a model for it ships with Haishui, so
...
```bash
cd ~/.haishui/haishui-agent && uv pip install -e ".[wake]"
```

## Quick start

```bash
# In an interactive `haishui` session:
...
desktop ear button — also writes `wake_word.enabled` to `~/.haishui/config.yaml`,
...
wake_word:
  ...
  phrase: "hey haishui"        # cosmetic label only — detection is keyed by the model/keyword below
  ...
  openwakeword:
    model: hey_haishui         # bundled default; OR a built-in name OR a path to a custom .onnx/.tflite
...
- **`confirmation_frames`** (default `3`, openWakeWord only) — how many
  *consecutive* over-threshold frames are required before the wake fires. A real
  "hey haishui" holds a high score across several frames; an ambient blip spikes
...
- **`sensitivity`** (default `0.6`) — the detection threshold, `0.0`–`1.0`.
  ...
  **all** engines — for openWakeWord it's the raw per-frame score threshold, for
  ...
  if real "hey haishui" utterances are missed.
...
default) to let Haishui choose per platform: **tflite on Apple Silicon**, onnx
...
fire. The tflite backend needs `ai-edge-litert` on macOS, which Haishui installs
on demand alongside the other wake-word deps.

### Surfaces (CLI, TUI, GUI)

The wake word works in all three Haishui surfaces, and `surface` picks which one
...
| `cli` | Only the classic `haishui` CLI. |
| `tui` | Only `haishui --tui`. |
| `gui` | Only the desktop app. |

The detector is on-device and single-mic, so only one surface listens at a time,
including when Haishui surfaces run in separate processes. Ownership is sticky:
the first eligible claimant keeps the listener until it stops, disconnects, or
its process exits. Haishui does not silently fail over to another open surface.
...
"Hey Haishui" works out of the box — the bundled openWakeWord model
(`model: hey_haishui`) is the default. To wake on something else, the easiest
...
command (`haishui -p <profile>`) instead of routing.
...
wake_word:
  ...
  phrase: "computer"
  openwakeword:
    model: ~/.haishui/wakewords/computer.onnx   # or a built-in name like hey_jarvis
```
...
syllables with an uncommon word ("haishui" qualifies) beat common words like
"hello" or "stop".
:::

### Option C — Porcupine (custom keyword in seconds)

Create a "Hey Haishui" keyword in the [Picovoice Console](https://console.picovoice.ai/),
...
wake_word:
  ...
  phrase: "hey haishui"
  porcupine:
    keyword: ~/.haishui/wakewords/hey_haishui.ppn
```

Set your access key in `~/.haishui/.env`:
...
- A TTS provider for speaking the reply (the default `edge-tts` works with no
  ...
  both STT and TTS are ready — `haishui tools` (Voice section) sets them up.
- The wake engine deps (auto-installed, or `haishui-agent[wake]`).
...
but the phrase never fires. Haishui detects this (`/wake status` shows
"mic delivers only silence"; the desktop's folded voice menu carries the same
hint on its trigger).
Fix: System Settings → Privacy & Security → Microphone → enable the Haishui
backend (it may appear as your terminal, `python`, or Haishui), then toggle the
...
```bash
haishui config set wake_word.input_device "Microphone Array"
```

Use `null` to return to the process default:

```bash
haishui config set wake_word.input_device null
```
