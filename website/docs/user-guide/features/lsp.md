...
Haishui runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Haishui
...
1. Haishui captures a baseline of current diagnostics for the file.
...
…). Haishui auto-detects the binary on PATH or in
`<HAISHUI_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Haishui at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<HAISHUI_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`haishui lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Haishui installs both packages together when you
run `haishui lsp install typescript` or auto-install fires on first
use.

## CLI

```
haishui lsp status          # service state + per-server install status
haishui lsp list            # registry, optionally --installed-only
haishui lsp install <id>    # eagerly install one server
haishui lsp install-all     # try every server with a known recipe
haishui lsp restart         # tear down running clients
haishui lsp which <id>      # print resolved binary path
```

`haishui lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <HAISHUI_HOME>/lsp/bin
...
When `install_strategy: auto`, Haishui installs binaries into
`<HAISHUI_HOME>/lsp/bin/`. NPM packages land in
`<HAISHUI_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Haishui-owned and is
...
**single process** per Haishui process: the first Python project spawns
...
**`haishui lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<HAISHUI_HOME>/lsp/bin/`. Run
`haishui lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `haishui lsp status`**
...
When `haishui lsp status` shows a `Backend warnings` section, install
...
`~/.haishui/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.haishui/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `haishui lsp restart` to clear the set;
