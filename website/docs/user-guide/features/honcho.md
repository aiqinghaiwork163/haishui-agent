...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Haishui's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Haishui instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
haishui memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `haishui` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Automatically generated Haishui titles (`derived` or `llm`), including lineage titles for Desktop branches, are display metadata and do not override `sessionStrategy`. An explicit user title remains an intentional session-name override for non-gateway, non-`per-session` sessions without a manual mapping.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Haishui syncs them back at session init.
...
The `haishui honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `haishui memory setup honcho` (or run `haishui memory setup` and pick it from the list); the `haishui honcho` subcommand then appears on the next invocation.

```bash
haishui memory setup honcho    # Configure Honcho directly (works before activation)
haishui honcho status          # Connection status, config, and key settings
haishui honcho setup           # Redirects to `haishui memory setup` (post-activation alias)
haishui honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
haishui honcho peer            # Show or update peer names + dialectic reasoning level
haishui honcho mode            # Show or set recall mode (hybrid/context/tools)
haishui honcho tokens          # Show or set token budget for context and dialectic
haishui honcho identity        # Seed or show the AI peer's Honcho identity
haishui honcho sync            # Sync Honcho config to all existing profiles
haishui honcho peers           # Show peer identities across all profiles
haishui honcho sessions        # List known Honcho session mappings
haishui honcho map             # Map current directory to a Honcho session name
haishui honcho enable          # Enable Honcho for the active profile
haishui honcho disable         # Disable Honcho for the active profile
haishui honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `haishui honcho`

If you previously used the standalone `haishui honcho setup`:
...
No re-login or re-setup needed. Run `haishui memory setup` and select "honcho" — the wizard detects your existing config.
