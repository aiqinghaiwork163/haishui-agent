...
Schedule tasks to run automatically with natural language or cron expressions. Haishui exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
All of this is available to Haishui itself through the `cronjob` tool, so you can create, pause, edit, and remove jobs by asking in plain language — no CLI required.

:::tip
**Which model does a cron job run on?** Resolution at fire time is: per-job pin → `cron.model` in `config.yaml` → the global default from `haishui model`.

- **Per-job pin** — set by *you* via the dashboard, `haishui cron create/edit --model … --provider …`, or by editing `~/.haishui/cron/jobs.json`. Once set, it sticks until you change it. The agent's `cronjob` tool cannot set or change per-job models — inference pins are user-owned.
- **`cron.model` / `cron.model_provider`** — a cron-fleet default: every unpinned job runs on this model, independent of your chat model. Set it once (`haishui config set cron.model <name>`) and switching your chat model with `haishui model` or `/model` never touches your cron fleet.
- **Global default** — only when neither of the above is set does a job follow `haishui model`. Haishui **snapshots** the provider and model at creation, and that snapshot is the job's effective pin: if you later switch the global default (`haishui model`, `/model`, `haishui config set model.default …`), the job **keeps running on the model and provider it was created under** and logs one INFO line per run noting the difference. A global model change never stops a scheduled job, and an unattended job never silently inherits a switch to a paid provider/model (#44585). To move a job to the new default, **resnap** it (`haishui cron resnap <job_id>`, or `--all` for every unpinned job) so it adopts the current default while staying unpinned, pin it (`haishui cron edit <job_id> --provider <provider> --model <model>`), or set `cron.model` to move the whole fleet at once. Jobs created before snapshots existed keep following the live global default.
...
`haishui setup --portal` is the lowest-friction option for unattended runs since OAuth refresh is automatic. See [Nous Portal](../../integrations/nous-portal.md).
:::

:::tip
**Per-job reasoning effort.** A job can pin its own thinking level, independent of the model pin: one of `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`, `ultra`. When set, it overrides both the global `agent.reasoning_effort` and per-model `agent.reasoning_overrides` for that job's runs (`none` disables thinking). Set it via `haishui cron create/edit --reasoning-effort high`; pass an empty string on edit to clear the pin and follow config again. (It is deliberately not exposed on the agent's `cronjob` tool — model configuration stays a user decision.) Levels a model doesn't support are clamped or omitted by the provider at request time — pinning `xhigh` on a model that caps at `high` runs at `high`. The pin has no effect on `no_agent` jobs (there is no LLM call to tune). Use it to run heavy scheduled analyses at `high` while cheap recurring jobs run at `minimal`, without touching your global default.
:::

:::warning
Cron-run sessions cannot recursively create more cron jobs. Haishui disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
haishui cron create "every 2h" "Check server status"
haishui cron create "every 1h" "Summarize new feed items" --skill blogwatcher
haishui cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
Ask Haishui normally:

```text
Every morning at 9am, check Hacker News for AI news and send me a summary on Telegram.
```

Haishui will use the unified `cronjob` tool internally.
...
Or: `haishui config set cron.preflight false`
...
```bash
haishui cron edit <job_id> --provider <provider> --model <model>   # one job
haishui config set cron.model <model>                               # every unpinned job
```

`haishui config set model.default …` and the Desktop model picker list the unpinned jobs that will
...
```bash
haishui cron resnap <job_id>   # one job
haishui cron resnap --all      # every unpinned agent job
```
...
A cron job can load one or more skills before it runs the prompt. Each skill loads exactly as it does from `/skill-name` in a chat session, including the `[Skill config ...]` block with its resolved `metadata.haishui.config` values from `config.yaml`.
...
haishui cron create "every 1d at 09:00" \
...
```bash
haishui cron edit <job_id> --schedule "every 4h"
haishui cron edit <job_id> --prompt "Use the revised task"
haishui cron edit <job_id> --skill blogwatcher --skill maps
haishui cron edit <job_id> --add-skill maps
haishui cron edit <job_id> --remove-skill blogwatcher
haishui cron edit <job_id> --clear-skills
```
...
```bash
haishui cron list
haishui cron pause <job_id_or_name>
haishui cron resume <job_id_or_name>
haishui cron run <job_id_or_name>
haishui cron remove <job_id_or_name>
haishui cron edit <job_id_or_name> [...flags]
haishui cron status
haishui cron tick
```
...
### Pausing everything: `haishui pause`

`haishui pause [--reason ...]` is the global emergency stop (`haishui resume` lifts it). While it is engaged no scheduled cron fire starts, whichever door it arrives through: the built-in ticker skips its dispatch, the managed-cron (hosted scheduler) fire webhook answers `503` with `Retry-After: 60` so the scheduler redelivers the fire after you resume, and the [misfire catch-up](#misfire-catch-up) sweep stays idle instead of force-firing everything that was held back. Runs already in flight are never killed, and nothing is lost: due work catches up on the first tick or sweep after `haishui resume`. Explicit manual runs (`haishui cron run`, the dashboard's Trigger button) are an operator override and still execute while paused.
...
```bash
haishui cron create "every 1h" "Post the digest" --paused --paused-reason "Awaiting review"
haishui cron resume <job_id>
```
...
```bash
haishui gateway install     # Install as a user service
sudo haishui gateway install --system   # Linux: boot-time system service for servers
haishui gateway             # Or run in foreground

haishui cron list
haishui cron status
```

`haishui cron status` reports whether the scheduler is alive (gateway process, ticker heartbeat, last successful tick) and the soonest scheduled run across your active jobs, ordered by actual instant even when jobs store different UTC offsets. A `next_run_at` that is already more than 15 minutes in the past is never shown as an upcoming "Next run": `cron status` prints `⚠ Next run <time> is OVERDUE — passed 7h ago but the job has not fired`, `cron list` and the in-chat `/cron list` label the row `Overdue:`, the dashboard and the Desktop cron panel (including a Bot's Routines card) show `Overdue since`, and when the scheduler has stopped ticking `status` (with the gateway down) and the dashboard Cron page also say when it last ticked. That is the signature of a scheduler that stopped ticking — restart the gateway (`haishui gateway restart`) so the next tick picks the overdue job up, or run it right away with `haishui cron run <id>`.

For a named profile served by the default-profile multiplexer, `haishui cron status` names that scheduler host and reports the named profile's own heartbeat health. Missing or stale heartbeats point to `haishui --profile default gateway restart`. `cron list` and `cron create` also warn when that heartbeat is missing or stale; `cron status` additionally checks the last successful tick and reports tick errors.

### Gateway scheduler behavior

On each tick Haishui:

1. loads jobs from `~/.haishui/cron/jobs.json`
...
A file lock at `~/.haishui/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
The worker is the gateway's own interpreter running `python -m cron.scheduler`, with the gateway's checkout pinned on its `PYTHONPATH` (plus any entries the gateway itself was started with), so it imports the same Haishui tree the gateway runs — regardless of the venv's editable-install mapping, the unit's `WorkingDirectory`, or `PYTHONSAFEPATH` on the host. A worker that dies before acknowledging the handoff records its own stderr tail in the job's last error and in the execution ledger, so the failing import (or whatever killed it) is named instead of a bare exit code.

### Execution history

Haishui records each claimed cron attempt in the profile-local
`~/.haishui/cron/executions.db` before executor or provider dispatch. Attempts
...
`haishui cron run` / `/cron run`, so a one-shot invocation with no scheduler
running heals the ledger too — Haishui marks an abandoned attempt `unknown` only
...
Inspect recent attempts with `haishui cron runs [job-id] --limit 20` (alias:
...
that the retained ledger records as completed, Haishui skips that replay and
...
in a row and suggesting you fix, pause (`haishui cron pause <job>`), or remove
it. Any successful run resets the streak, and `haishui cron list` shows the
...
(the run is still recorded — `haishui cron runs` and the failure streak see it,
...
```bash
haishui cron incidents                 # list incidents (newest activity first)
haishui cron incidents --state alerted # filter: detected | alerted | resolved | closed
haishui cron incidents ack <id>        # acknowledge — silence this signature for good
```
...
### Fleet health check: `haishui cron doctor`

`haishui cron doctor` is a read-only health check over every active job. It prints grouped, per-job issues and exits `1` while any finding stands, including historical late or catch-up dispatches (`0` when no findings remain).

A successful catch-up does not clear the lateness warning; the next on-time dispatch does. A watchdog such as `haishui cron doctor || alert` can therefore keep alerting for a full schedule interval after the host wakes, even if the catch-up succeeds.
...
- script missing, not a file, or resolving outside `HAISHUI_HOME/scripts`,
...
`haishui cron incidents` (durable failure records) and `haishui cron runs`
...
| `"local"` | Save to local files only (`~/.haishui/cron/output/`) | Default on CLI |
...
`last_delivery_error`. `haishui cron list` shows it in yellow as
`delivery_failed: <reason>`, `haishui cron doctor` reports it as a delivery
...
- `bot-chat:<profile>` targets another profile **on the same machine**. Names are validated against `haishui profile list` when the job is created; profiles on other gateways or machines can never be targeted, so same-named profiles across machines are unambiguous.
...
in `haishui cron list`:

```
⚠ Delivery UNVERIFIED: adapter acked slack:C0123456 without message_id/raw_response
```

and in `haishui cron doctor` as `last delivery unverified (...)`. The marker is
...
If the agent's final response contains `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.haishui/cron/output/`), but no message is sent to the delivery target.
...
The run is then recorded as failed (`last_status`, failure streak, `haishui cron runs` and `haishui cron incidents`
all reflect it) and the failure notice is delivered like any other failed run. The full response is still saved
under `~/.haishui/cron/output/` for triage. The marker is strict: mentioning or quoting `[CRON_FAILURE]` anywhere
...
Or set the `HAISHUI_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 3600s default.
...
Or set the `HAISHUI_CRON_MEDIA_SEND_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 300s default. A timed-out attachment is recorded in the job's run status as a partial delivery failure (the text still delivers).
...
`.sh` / `.bash` files run under `bash` from `PATH` when available, otherwise `/bin/bash` (important on Windows Git Bash). Anything else runs under the current Python interpreter (`sys.executable`). Scripts must resolve inside `$HAISHUI_HOME/scripts/` — relative names, absolute paths, and `~`-prefixed paths are accepted when the resolved target stays in that directory; paths that escape it are rejected. Subprocess env is sanitized (`_sanitize_subprocess_env`): provider API credentials and other Haishui-managed secrets are **not** inherited by cron scripts.
...
cronjob(
    action="create",
    prompt="Fetch the top 10 AI/ML stories from Hacker News. Save them to ~/.haishui/data/briefs/raw.md in markdown format with title, URL, and score.",
    schedule="0 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.haishui/data/briefs/raw.md. Score each story 1–10 for engagement potential and novelty. Output the top 5 to ~/.haishui/data/briefs/ranked.md.",
    schedule="30 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.haishui/data/briefs/ranked.md. Write 3 tweet drafts (hook + body + hashtags). Deliver to telegram:7976161601.",
    schedule="0 8 * * *",
...
- When Job 2 fires, Haishui reads Job 1's most recent output from `~/.haishui/cron/output/{job1_id}/*.md`
...
From the CLI: `haishui cron create "every 6h" "Scan for news" --continuity`, and `haishui cron edit <job_id> --continuity` / `--no-continuity` to toggle it on an existing job. The same toggle appears in the dashboard's cron editor and the desktop Bot Mode routine dialog.
...
Haishui home. Its `## Error` section includes the chained traceback, with credential patterns
...
- `haishui cron list`: a red missed-fire warning under the job
- `haishui cron doctor`: a per-job missed-fire finding that makes the command exit `1`
- The dashboard job view

The stamp always reflects **current** auto-fire health: it is overwritten by newer misses and cleared automatically by the next successful run. If you see it, the job and its schedule are fine — the gateway side of the fire path needs attention (most commonly, restart the gateway through its supervisor so it loads the full profile environment: `haishui gateway restart`).
...
`haishui cron list` as `⚠ late` / `⚠ catch-up after missed fire`.
...
Or run `haishui config set cron.catch_up_missed false`. With this opt-out, a recurring
...
Runtimes that can't receive detached results (one-shot `haishui -z`, `haishui
...
Cron runs each job in a fresh agent session with no chat platform attached. By default the cron agent gets **the toolset you configured for the `cron` platform in `haishui tools`** — not the CLI default, not everything under the sun.
...
When `enabled_toolsets` is set on a job it wins; otherwise the `haishui tools` cron-platform config wins; otherwise Haishui falls back to the built-in defaults. If the cron-platform toolset config cannot be read at all (for example a malformed `platform_toolsets` block in `config.yaml`), the run fails with a recorded error instead of quietly running with every tool — check `haishui cron list` / `haishui cron doctor`. This matters for cost control: carrying `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.
...
If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether Haishui should even invoke the agent. Emit a final stdout line of the form:
...
```bash
#!/bin/bash
# ~/.haishui/scripts/feed-changed.sh
FEED="$HOME/data/feed.json"
STATE="$HOME/.haishui/scripts/.feed-changed.last"
...
:::tip
Haishui's own `~/.haishui/state.db` is an internal schema that changes between releases. Don't query it from a pre-run gate — point at your own database or feed instead.
:::

Credit: this recipe set was prompted by @iankar8's exploration in [#2654](https://github.com/NousResearch/haishui-agent/pull/2654), which proposed adding sql/file/command triggers as a parallel mechanism. The `script` + `wakeAgent` gate already covers all three cases at $0, so the work landed as documentation instead.
...
Jobs are stored in `~/.haishui/cron/jobs.json`. Output from job runs is saved to `~/.haishui/cron/output/{job_id}/{timestamp}.md`.

Job definitions are plain JSON on disk: they survive `haishui update`, gateway restarts, and machine reboots. A job that was mid-run during a restart is marked `unknown` in the execution ledger — it is not automatically retried, but the job's next scheduled tick fires normally. See [Execution history](#execution-history) for details.

:::tip
Ask the agent to manage jobs through the `cronjob` tool, `haishui cron edit`, or `/cron` — not by patching `jobs.json` directly. Direct edits can fail silently when [file write safety](../security.md#file-write-safety) blocks the path (for example when `HAISHUI_WRITE_SAFE_ROOT` is set), and the [file-mutation verifier](../configuration.md#file-mutation-verifier) footer is the authoritative signal that nothing was saved.
:::

Jobs may store `model` and `provider` as `null`. When those fields are omitted, Haishui resolves them at execution time from the global configuration. They only appear in the job record when a per-job override is set.
