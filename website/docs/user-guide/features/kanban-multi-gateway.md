...
Haishui supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.haishui/config.yaml`:
...
Or set the env var: `HAISHUI_KANBAN_DISPATCH_IN_GATEWAY=false`
