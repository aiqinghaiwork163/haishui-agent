...
fills the password through Haishui. The tool result it sees is
...
- **Authenticator key saved with the login** (the "setup key" or `otpauth://`
  ...
  hold a TOTP seed count too): Haishui generates the current code and enters
  it. Nobody is asked. Add the key in **Settings → Passwords & Logins → Add**
  or `haishui vault add`; the item shows a *2FA auto* badge.
- **Code sent to your phone or email**: a small prompt appears in your
  surface ("Verification code for github.com"), you type the code, Haishui
...
in, Haishui picks it up automatically and its website logins become fillable
...
per session, 30 minutes idle). Haishui hands the master password to the manager's
...
Prefer not to use a detected manager? `haishui vault sources --disable bitwarden`,
...
Passwords & Logins → Add**, or `haishui vault add`), bound to the checkout site,
...
- **CLI**: `haishui vault list`, `haishui vault add`, `haishui vault rm <handle>`,
  `haishui vault sources`.

Items live encrypted under `~/.haishui/vault/` (Fernet key + vault file, both
...
Cron jobs, webhooks, the API server and `haishui chat -q` have nobody to answer a
...
**Does:** the password never enters the model's context through Haishui: not in
