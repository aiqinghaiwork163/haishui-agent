...
description: "Durable SQLite-backed task board for coordinating multiple Haishui profiles"
---
...
Haishui Kanban is a durable task board, shared across all your Haishui profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.haishui/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.
...
The board has two front doors, both backed by the same `~/.haishui/kanban.db`:

- **Agents drive the board through a dedicated `kanban_*` toolset** — `kanban_show`, `kanban_list`, `kanban_complete`, `kanban_request_review`, `kanban_request_changes`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_attach`, `kanban_attach_url`, `kanban_attachments`, `kanban_create`, `kanban_link`, `kanban_unblock`. The dispatcher spawns each worker with these tools already in its schema; orchestrator profiles can also enable the `kanban` toolset explicitly. The model reads and routes tasks by calling tools directly, *not* by shelling out to `haishui kanban`. See [How workers interact with the board](#how-workers-interact-with-the-board) below.
- **You (and scripts, and cron) drive the board through `haishui kanban …`** on the CLI, `/kanban …` as a slash command, or the dashboard. These are for humans and automation — the places without a tool-calling model behind them.
...
- **Dispatcher** — a long-lived loop that, every N seconds (default 60): reclaims stale claims, reclaims crashed workers (PID gone but TTL not yet expired), reaps workers that outlived their finished run (a worker still alive after its own `kanban_complete`/`kanban_block` is terminated once its run has been closed for two minutes, leaving it time to finish its final turn — matched by PID *and* spawn-time fingerprint, so a recycled PID is never signalled; recorded as a `terminal_worker_reaped` event), promotes ready tasks, atomically claims, spawns assigned profiles. Runs **inside the gateway** by default (`kanban.dispatch_in_gateway: true`). One dispatcher sweeps all boards per tick; workers are spawned with `HAISHUI_KANBAN_BOARD` pinned so they can't see other boards. After `kanban.failure_limit` consecutive spawn failures on the same task (default: 2) the dispatcher auto-blocks it with the last error as the reason — prevents thrashing on tasks whose profile doesn't exist, workspace can't mount, etc.
...
haishui kanban boards list

# Create a new board.
haishui kanban boards create atm10-server \
    --name "ATM10 Server" \
...
haishui kanban --board atm10-server list
haishui kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
haishui kanban boards switch atm10-server
haishui kanban boards show             # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
haishui kanban boards rename atm10-server "ATM10 (Prod)"
...
2. `HAISHUI_KANBAN_BOARD` env var (set by the dispatcher when spawning a
   worker, so workers can't see other boards).
3. `~/.haishui/kanban/current` — the slug persisted by `haishui kanban
   boards switch`.
4. `default`.
...
`haishui dashboard` → Kanban tab shows a board switcher at the top as soon
...
- **Project** (in both modals) — binds the board to a Haishui Project
...
- **Storage** — files land under
  `<haishui-home>/kanban/attachments/<task_id>/` for the default board, or
  `<haishui-home>/kanban/boards/<slug>/attachments/<task_id>/` for a named
  board. Set `HAISHUI_KANBAN_ATTACHMENTS_ROOT` to pin a custom location.
...
- **Download / remove** — the drawer lists each attachment with a download
  ...
  last reference is removed). From the CLI, `haishui kanban attach-rm
...
haishui gateway start

# 3. Create a task (you — or an orchestrator agent via kanban_create)
haishui kanban create "research AI funding landscape" --assignee researcher

# 4. Watch activity live (you)
haishui kanban watch

# 5. See the board (you)
haishui kanban list
haishui kanban stats
```

When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `haishui kanban show t_abcd`.
...
Override the config flag at runtime via `HAISHUI_KANBAN_DISPATCH_IN_GATEWAY=0`
for debugging. Standard gateway supervision applies: run `haishui gateway
...
until one comes up — `haishui kanban create` warns about this at creation
time.
...
its own transient scope (`systemd-run --user --scope --unit haishui-worker-kanban-<task>-run-<run>`)
...
- **`haishui kanban dispatch` from your own unit** (a `Type=oneshot` timer, a
...
Running `haishui kanban daemon` as a separate process is **deprecated**;
...
Mounting one `kanban.db` in several Haishui homes (containers, fleet hosts) shares
...
haishui kanban create "nightly ops review" \
    --assignee ops \
...
```bash
haishui kanban complete t_abc t_def t_hij --result "batch wrap"
haishui kanban archive  t_abc t_def t_hij
haishui kanban unblock  t_abc t_def
haishui kanban block    t_abc "need input" --ids t_def t_hij
```
...
```bash
haishui -p planner tools enable kanban                      # CLI / TUI / Desktop chats
haishui -p planner tools enable kanban --platform telegram  # a gateway platform
```

Each platform has its own selection under `platform_toolsets.<platform>` in
`config.yaml`; the toolset is also a checkbox in `haishui tools` and the dashboard.
...
**Workers do not shell out to `haishui kanban`.** When the dispatcher spawns a worker it sets `HAISHUI_KANBAN_TASK=t_abcd` in the child's env, and that env var flips on a dedicated **kanban toolset** in the model's schema. The same toolset is also available to orchestrator profiles that enable `kanban` in their toolsets config. These tools read and mutate the board directly via the Python `kanban_db` layer, same as the CLI does. A running worker calls these like any other tool; it never sees or needs the `haishui kanban` CLI.
...
kanban_show()                                     # no args — uses HAISHUI_KANBAN_TASK
...
### Why tools instead of shelling to `haishui kanban`

Three reasons:

1. **Backend portability.** Workers whose terminal tool points at a remote backend (Docker / Modal / Singularity / SSH) would run `haishui kanban complete` *inside* the container, where `haishui` isn't installed and `~/.haishui/kanban.db` isn't mounted. The kanban tools run in the agent's own Python process and always reach `~/.haishui/kanban.db` regardless of terminal backend.
...
**Zero schema footprint on normal sessions.** A regular `haishui chat` session has zero `kanban_*` tools in its schema unless the active profile explicitly enables the `kanban` toolset for orchestrator work. Dispatcher-spawned task workers get task-scoped tools because `HAISHUI_KANBAN_TASK` is set; orchestrator profiles get the broader routing surface through config. No tool bloat for users who never touch kanban.

The auto-injected kanban guidance teaches the model which tool to call when and in what order. It is injected only for the dispatcher-spawned worker that owns the task: an interactive session that merely has the `kanban` toolset enabled keeps the tools but is not told it "has been assigned ONE task", and a `delegate_task` child or a cron run fired from inside a worker — which inherit the worker's `HAISHUI_KANBAN_TASK` — neither receives the worker protocol nor records a terminal outcome against the worker's card when its own turn budget runs out.
...
```json
{
  ...
  "verification": ["pytest tests/haishui_cli/test_kanban_db.py -q"],
...
2. `cd $HAISHUI_KANBAN_WORKSPACE` (via the terminal tool) and do the work there.
...
revoked key parks the card exactly like an implementer. `haishui kanban show`
surfaces it as *Provider rejected this profile's credential or model — blocked
after one attempt*; fix the assignee profile's provider (`haishui -p <profile>
auth` / `setup`), then `haishui kanban unblock <id>`. The worker also writes its exit code as the
last line of its own log (`[kanban-worker-exit] rc=<code>`), so a per-tick
`haishui kanban dispatch` process — which never reaped the worker and cannot
...
**Agent-side prevention:** Before the worker exits, Haishui injects up to two
...
guard is active only for the dispatcher-spawned worker itself (`HAISHUI_KANBAN_TASK` is
...
and can be disabled with `HAISHUI_KANBAN_STOP_NUDGE=0`.
...
breaker block) until `haishui kanban unblock <id>`, which also grants a fresh
...
**Worker session names.** A worker's session is titled after its card (`Fix the swap modal`, or `Kanban task <id>` when the board row can't be read) at spawn, so `haishui sessions` and session search show the card, not a model's guess. Workers never make the auxiliary `title_generation` model call that names interactive sessions; a manual `/title` in a worker session still wins.
...
```bash
haishui kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

haishui kanban create "audit auth flow" \
    --assignee reviewer \
...
The dispatcher emits one `--skills <name>` flag per skill listed, so the worker spawns with all of them loaded on top of the auto-injected kanban guidance. The skill names must match skills that are actually installed on the assignee's profile (run `haishui skills list` to see what's available); there's no runtime install.
...
```bash
# At creation
haishui kanban create "hard refactor" --assignee coder \
    --model claude-opus-4.6 --provider anthropic

# Or later — takes effect on the next dispatch
haishui kanban set-model t_abcd claude-opus-4.6 --provider anthropic
haishui kanban set-model t_abcd none    # clear the override
```
...
Kanban's per-profile configs make the planner/worker cost split natural. Decomposing a project into well-scoped cards takes frontier-level judgment; executing a card that already carries a clear goal, context, and handoff evidence usually doesn't — and the workers are where the vast majority of tokens are spent, so the worker model is where the cost lives. Run your orchestrator/dispatcher profile on a frontier model and point worker profiles at inexpensive models. Each profile has its own `config.yaml` under `~/.haishui/profiles/<name>/`, and the dispatcher injects the profile-scoped `HAISHUI_HOME` when it spawns `haishui -p <assignee>`, so each worker reads its own profile's model settings:

```yaml
# ~/.haishui/config.yaml (orchestrator / dispatcher profile)
model:
  default: "your-frontier-model"

# ~/.haishui/profiles/coder/config.yaml (worker profile)
model:
  default: "your-inexpensive-model"

# ~/.haishui/profiles/researcher/config.yaml (another worker profile)
model:
  default: "your-inexpensive-model"
```

For the occasional quality-sensitive card, pin just that task back to a stronger model with the [per-task model override](#per-task-model-override) (`--model`/`--provider` at create time, `haishui kanban set-model` later, or the dashboard's model dropdown) — no profile edits needed.
...
```bash
haishui kanban create "Translate the docs site to French" \
...
A goal-mode worker's **Worker log** (dashboard drawer, `haishui kanban log <id>`) carries the same live tool feed as any other worker's, plus one `kanban goal loop: turn N/M verdict=…` line per judged turn, so you can follow what the loop is doing while the card is running.
...
**Manual** — `kanban.auto_decompose: false`. Triage tasks stay in triage until you act. Click the **⚗ Decompose** button on a card, run `haishui kanban decompose <id>` (or `--all`), or use `/kanban decompose <id>` from a chat. This matches the pre-decomposer behavior of the board, useful when you want full control over what runs when.
...
Flip between the two modes from the **Orchestration: Auto/Manual** pill at the top of the kanban page (emerald = Auto, muted gray = Manual), or by editing `config.yaml` directly. Both modes coexist with `haishui kanban specify` — that's still available as a single-task spec rewrite when you don't want fan-out.

The decomposer's routing decisions depend on profile descriptions, which is a per-profile labeling primitive you set with `haishui profile create --description "..."`, `haishui profile describe <name> --text "..."`, `haishui profile describe <name> --auto` (LLM-generates from the profile's installed skills + model), or the dashboard's per-profile editor in the expanded **Orchestration settings** panel. Profiles without a description still appear in the roster — they're routable by name, just less precisely. The decomposer NEVER lands a child task with `assignee=None`: when the LLM picks an unknown profile, the child gets routed to `kanban.default_assignee`, else the root task's assignee (if it names an existing profile), else the active default profile.
...
Config knobs (all under `kanban:` in `~/.haishui/config.yaml`):
...
| `auxiliary.profile_describer` | Model that auto-generates profile descriptions (called by `haishui profile describe --auto`). |
...
└──────────┬─────────────┘                                    │
           ...
           ▼                                                  │
┌────────────────────────┐                                    │
│  ~/.haishui/kanban.db   │ ───── append task_events ──────────┘
│  (WAL, shared)         │
└────────────────────────┘
```
...
The plugin uses the standard Haishui dashboard plugin contract — see [Extending the Dashboard](./extending-the-dashboard) for the full manifest reference, shell slots, page-scoped slots, and the Plugin SDK. Extra columns, custom card chrome, tenant-filtered layouts, or full `tab.override` replacements are all expressible without forking this plugin.
...
```
haishui kanban init                                     # create kanban.db + print daemon hint
haishui kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
haishui kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived]
        ...
        [--json]
haishui kanban show <id> [--json]
haishui kanban assign <id> <profile>                    # or 'none' to unassign
haishui kanban reassign <id>... <profile>               # bulk re-assign tasks to a profile
haishui kanban edit <id> [--title ...] [--body ...]     # edit task title / body / priority in place
        [--priority N]
haishui kanban promote <id>...                          # move todo/blocked tasks to ready (recovery)
haishui kanban schedule <id> --at <ISO8601>             # set/clear a task's scheduled_at start time
haishui kanban diagnostics [--json]                     # board health snapshot (alias: diag)
haishui kanban link <parent_id> <child_id>
haishui kanban unlink <parent_id> <child_id>
haishui kanban claim <id> [--ttl SECONDS]
haishui kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
haishui kanban complete <id>... [--result "..."] [--force]
haishui kanban block <id> "<reason>" [--ids <id>...]
haishui kanban unblock <id>...
haishui kanban archive <id>...

haishui kanban request-review <id> [--summary "..."] [--metadata JSON] [--reviewer PROFILE]
haishui kanban request-changes <id> "<required changes>"               # active reviewer -> implementer
haishui kanban reopen-review  <id>... [--reason "..."]                 # changes requested: 'review' -> ready/todo

haishui kanban tail <id>                                # follow a single task's event stream
haishui kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
haishui kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
haishui kanban runs <id> [--json]                       # attempt history (one row per run)
haishui kanban assignees [--json]                       # profiles on disk + per-assignee task counts
haishui kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
haishui kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `haishui gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
haishui kanban stats [--json]                           # per-status + per-assignee counts
haishui kanban log <id> [--tail BYTES]                  # worker log from ~/.haishui/kanban/logs/
haishui kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
        [--chat-type dm|group|channel|thread] [--delivery-mode notify|notify+wake|wake]
haishui kanban notify-list [<id>] [--json]
haishui kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
haishui kanban context <id>                             # what a worker sees
haishui kanban specify [<id> | --all] [--tenant T]      # flesh out a triage-column idea
        [--author NAME] [--json]                       #   into a full spec and promote to todo
haishui kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
Every `haishui kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `haishui chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `haishui_cli.kanban.run_slash()` entry point that reuses the `haishui kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `haishui kanban`. You don't have to leave the chat to drive the board.
...
The gateway normally queues slash commands and user messages while an agent is still thinking — that's what stops you from accidentally starting a second turn while the first is in flight. **`/kanban` is explicitly exempted from this guard.** The board lives in `~/.haishui/kanban.db`, not in the running agent's state, so reads (`list`, `show`, `context`, `tail`, `watch`, `stats`, `runs`) and writes (`comment`, `unblock`, `block`, `assign`, `archive`, `create`, `link`, …) all go through immediately, even mid-turn.
...
Dispatcher workers creating tasks through `kanban_create` or `haishui kanban create`
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`haishui kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
| **P9 Triage specifier** | rough idea → `triage` → `haishui kanban specify` expands body → `todo` | "turn this one-liner into a spec'd task" |
...
_metadata_: `{"changed_files": ["haishui_cli/retry.py", "tests/test_retry.py"], "decisions": ["capped backoff at 60s", "jitter = full"]}`
```
...
haishui kanban create "Fix CI failure from t_impl: test_retry flakes on 3.11" \
    --assignee coder \
    --parent t_impl \
    --body "$(cat <<'EOF'
...
```
hotspot: haishui_cli/kanban_db.py — third conflicting edit to the dispatch loop this wave
```
...
```bash
haishui kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7 \
    --chat-type group --delivery-mode notify+wake
haishui kanban notify-list
haishui kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
```bash
haishui kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
haishui kanban runs t_abcd
...
**Bulk close caveat.** `haishui kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.

**Dependency refusal on complete names the parents.** `kanban_complete` / `haishui kanban complete` / the dashboard's "mark done" and "request review" actions (single and bulk) on a card whose direct parent is not `done`/`archived` (a parent reopened mid-run, or an edge that predates the running-child refusal) reports `unsatisfied parent dependencies: t_… (todo)` instead of the generic "unknown id, stale run, or already terminal" text; the card stays in-flight. `kanban_show` lists the same parents under `unsatisfied_parents`, and `haishui kanban show` / `haishui kanban diagnostics` / the dashboard raise a `running_with_open_parents` warning on a running card in that state. Finish the parent or `haishui kanban unlink <parent> <child>`; there is no force path through the dependency gate.

**Live-claim guard on complete.** A `running` task whose worker holds a live claim is only completed by that worker (`kanban_complete` from inside the run) or by an explicit operator override: `haishui kanban complete <id> --force` and the dashboard's "mark done" action. A claim-less `haishui kanban complete <id>` or an orchestrator session's `kanban_complete` is refused with a pointer to `--force` / `haishui kanban reclaim`, so a second session can no longer close a live worker's run underneath it. Completing `ready`, `blocked` or `review` cards without a claim is unchanged.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`haishui kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `haishui kanban heartbeat $TASK` to signal liveness during long operations. |
...
`haishui kanban tail <id>` shows these for a single task. `haishui kanban watch` streams them board-wide.

## Out of scope

Kanban is deliberately single-host. `~/.haishui/kanban.db` is a local SQLite file and the dispatcher spawns workers on the same machine. Running a shared board across two hosts is not supported — there's no coordination primitive for "worker X on host A, worker Y on host B," and the crash-detection path assumes PIDs are host-local. If you need multi-host, run an independent board per host and use `delegate_task` / a message queue to bridge them.
