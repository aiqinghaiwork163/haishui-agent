...
title: "Plugins"
description: "Extend Haishui with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

Haishui has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
[Adding Tools](../../developer-guide/adding-tools.md) page is for built-in Haishui
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a Haishui Plugin](../../developer-guide/plugins/index.md)** — step-by-step guide with a complete working example.

## Quick overview

Drop a directory into `~/.haishui/plugins/` with a `plugin.yaml` and Python code:

```
~/.haishui/plugins/my-plugin/
...
Start Haishui — your tools appear alongside built-in tools. The model can call them immediately.

### Minimal working example

Here is a complete plugin that adds a `hello_world` tool and logs every tool call via a hook.

**`~/.haishui/plugins/hello-world/plugin.yaml`**
...
**`~/.haishui/plugins/hello-world/__init__.py`**

```python
"""Minimal Haishui plugin — registers a tool and a hook."""
...
Drop both files into `~/.haishui/plugins/hello-world/`, restart Haishui, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

The model-facing tool description belongs in `schema["description"]`. The optional `ctx.register_tool(description=...)` value is separate `ToolEntry` registry metadata: when omitted, it defaults to the schema description, but Haishui does not copy it back into a schema that lacks `description`. Prefer defining the text once in the schema. If you provide both values, keep them synchronized; the model sees the schema value.

Project-local plugins under `./.haishui/plugins/` are disabled by default. Enable them only for trusted repositories by setting `HAISHUI_ENABLE_PROJECT_PLUGINS=true` before starting Haishui.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `haishui <plugin> <subcommand>` |
...
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `haishui plugins install` |
| Distribute via pip | `[project.entry-points."haishui_agent.plugins"]` |
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with Haishui — see [Built-in Plugins](./built-in-plugins.md) |
| User | `~/.haishui/plugins/` | Personal plugins |
| Project | `.haishui/plugins/` | Project-specific plugins (requires `HAISHUI_ENABLE_PROJECT_PLUGINS=true`) |
| pip | `haishui_agent.plugins` entry_points | Distributed packages |
| Nix | `services.haishui-agent.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](../../getting-started/nix-setup.md#plugins) |

Later sources override earlier ones on name collision, so a user plugin with the same name as a bundled plugin replaces it.

### Plugin sub-categories

Within each source, Haishui also recognizes sub-category directories that route plugins to specialized discovery systems:
...
User plugins at `~/.haishui/plugins/model-providers/<name>/` override bundled model providers of the same name (last-writer-wins in `register_provider()`), so you can replace a built-in provider profile without any repo edits. Memory providers resolve the other way round: for `~/.haishui/plugins/memory/<name>/` the **bundled** provider wins on a name collision (bundled, then user, then project, then entry points; first seen wins), so a user memory provider needs its own unique name.

## Plugins are opt-in (with a few exceptions)

**General plugins and user-installed backends are disabled by default** — discovery finds them (so they show up in `haishui plugins` and `/plugins`), but nothing with hooks or tools loads until you add the plugin's name to `plugins.enabled` in `~/.haishui/config.yaml`. This stops third-party code from running without your explicit consent.
...
```bash
haishui plugins                    # interactive toggle (space to check/uncheck)
haishui plugins enable <name>      # add to allow-list
haishui plugins disable <name>     # remove from allow-list + add to disabled
```

After `haishui plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.

For a reproducible install, pin a full immutable commit (tags, branches, and
abbreviated SHAs are not accepted):

```bash
haishui plugins install owner/repo --ref 0123456789abcdef0123456789abcdef01234567
```

Haishui checks out the commit detached, verifies that `HEAD` exactly matches the
requested SHA, and records the canonical source, installed revision, and pin
status in the current profile. `haishui plugins update` refuses to move a pinned
plugin; choose a new exact commit explicitly with
`haishui plugins install <source> --force --ref <new-commit>`. The
profile-local install metadata contains no config values, environment values,
secrets, or capability grants.

The same pin is available in Haishui Desktop: **Skills → Plugins → Install from
...
can confirm everyone is running the same commit. `haishui plugins list` prints
...
`haishui plugins install` clones non-interactively (it never prompts for a
username or password), so a private repo needs a credential Haishui can find on
its own. Every clone, pinned `--ref` fetch and `haishui plugins update` pull is
...
refuses anonymous access does Haishui look for a credential. For an `https://`
...
before. The same resolution applies to `haishui plugins update`, catalog MCP
installs from git, and profile distributions fetched from a git URL.

### What the allow-list does NOT gate

Several categories of plugin bypass `plugins.enabled` — they're part of Haishui' built-in surface and would break basic functionality if gated off by default:
...
| **User-installed platforms** (under `~/.haishui/plugins/platforms/`) | Opt-in via `plugins.enabled` — third-party gateway adapters need explicit consent. |

In short: **bundled "always-works" infrastructure loads automatically; third-party general plugins are opt-in.** The `plugins.enabled` allow-list is the gate specifically for arbitrary code a user drops into `~/.haishui/plugins/`.

### Approval transports

An approval transport changes **where a human sees and answers** an existing
Haishui tool-approval request. It does not decide whether a command needs
...
`present` may be synchronous or async. Haishui runs it on a bounded worker and
...
`transport_fallback: builtin`. Without that exact opt-in, Haishui never
materializes the prompt on another surface.

Haishui still owns hardline blocks, sudo-stdin protection, user deny rules,
...
When you upgrade to a version of Haishui that has opt-in plugins (config schema v21+), any user plugins already installed under `~/.haishui/plugins/` that weren't already in `plugins.disabled` are **automatically grandfathered** into `plugins.enabled`. Your existing setup keeps working. Bundled standalone plugins are NOT grandfathered — even existing users have to opt in explicitly. (Bundled platform/backend plugins never needed grandfathering because they were never gated.)

## Available hooks

Plugins can register the 27 lifecycle events currently accepted by `haishui_cli.plugins.VALID_HOOKS`. The **[Event Hooks catalog](./hooks.md#shipped-plugin-hook-catalog)** is canonical for exact timing, return handling, payload fields, and privacy notes.
...
Haishui has four kinds of plugins:

| Type | What it does | Selection | Location |
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.haishui/plugins/` |
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and Haishui also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:

| Want to add… | How | Authoring guide |
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Haishui Plugin](../../developer-guide/plugins/index.md) · [Adding Tools](../../developer-guide/adding-tools.md) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](./hooks.md) · [Build a Haishui Plugin](../../developer-guide/plugins/index.md) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Haishui Plugin](../../developer-guide/plugins/index.md) · [Extending the CLI](../../developer-guide/extending-the-cli.md) |
| A **subcommand** for `haishui <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](../../developer-guide/extending-the-cli.md) |
...
| An **STT backend** (any CLI — whisper.cpp, custom whisper binary, local ASR CLI) | Config-driven (recommended) — declare under `stt.providers.<name>` with `type: command` in `config.yaml`, or set `HAISHUI_LOCAL_STT_COMMAND` for the legacy single-command escape hatch. OR Python backend plugin — `ctx.register_transcription_provider()` for Python-SDK engines (OpenRouter, SenseAudio, Gemini-STT, etc.). | [STT Setup](./tts.md#stt-custom-command-providers) · [Python plugin guide](./tts.md#python-plugin-providers-stt) |
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. Haishui auto-discovers the server's tools and registers them alongside built-ins. | [MCP](./mcp.md) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `haishui skills tap add <repo>` | [Skills Hub](./skills.md#skills-hub) · [Publishing a custom tap](./skills.md#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.haishui/hooks/<name>/` | [Event Hooks](./hooks.md#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `haishui plugins install` needed. See the **[Nix Setup guide](../../getting-started/nix-setup.md#plugins)** for full details.
...
```bash
haishui plugins                               # unified interactive UI
haishui plugins list                          # table: enabled / disabled / not enabled
haishui plugins search <term>                 # search the Haishui plugin catalog
haishui plugins install <name>                # install a catalog entry (repo @ reviewed pinned SHA)
haishui plugins install user/repo             # install from Git, then prompt Enable? [y/N]
haishui plugins install user/repo --enable    # install AND enable (no prompt)
haishui plugins install user/repo --no-enable # install but leave disabled (no prompt)
haishui plugins update my-plugin              # pull latest (local edits are autostashed and re-applied)
haishui plugins remove my-plugin              # uninstall
haishui plugins enable my-plugin              # add to allow-list
haishui plugins disable my-plugin             # remove from allow-list + add to disabled
haishui plugins capabilities [my-plugin]      # declared vs granted capabilities
```

### One-click install links (Desktop)

Haishui Desktop registers the `haishui://` URL scheme, so a website, README, or
chat message can link straight to a plugin install:

```
haishui://plugin/install?repo=owner/repo            # main install link
haishui://plugin/install?repo=owner/repo&enable=1   # enable the agent plugin after install
haishui://plugin/install?repo=owner/repo&force=1    # replace an existing install
```

Clicking one opens Haishui and shows a **confirmation dialog** — the repo id,
...
Plugins → Install from Git**. Legacy `haishui://plugin-agent/…` and
`haishui://plugin-desktop/…` URLs route into the same dialog. In dev builds
(`npm run dev`) the scheme is `haishui-dev://`.

Websites need no SDK — a normal anchor works:

```html
<a href="haishui://plugin/install?repo=owner/repo&enable=1">Install in Haishui</a>
```

MCP servers have the equivalent link form — see
[Add to Haishui link](../../reference/mcp-config-reference.md#add-to-haishui-link).
...
When a plugin declares capabilities, `haishui plugins install` (and
`haishui plugins enable`) shows the list with one-line risk descriptions and
...
granted, `haishui plugins update` surfaces the additions and asks again. New
...
`haishui plugins enable <id>` interactively to grant them later.

Inspect the state at any time:

```bash
haishui plugins capabilities             # all plugins with declared/granted capabilities
haishui plugins capabilities my-plugin   # one plugin, declared vs granted
```
...
not a code audit, and Haishui has not reviewed the plugin's code. Only install
...
### Discovering plugins — the Haishui plugin catalog

`haishui plugins search <term>` searches the **Haishui plugin catalog** — the
curated, SHA-pinned catalog maintained in the haishui-agent repository
(`plugin-catalog/`). Matching covers entry names, descriptions, and declared
tools:

```bash
haishui plugins search telegram    # search the catalog
haishui plugins browse             # browse every entry
haishui plugins info <name>        # full details for one entry
```
...
recorded so `haishui plugins update` can re-pin when the catalog moves:

```bash
haishui plugins install <catalog-name>
```
...
A **plugin pack** is a declarative, shareable YAML file (`haishui-pack.yaml`)
...
version: 1.0.0
plugins:
  - name: haishui-telegram-business       # bare plugin-catalog name…
...
config:                                  # optional, non-secret seeds only
  haishui-media-studio:
    default_model: flux-3
skills: []                               # declared list only (not auto-installed yet)
```

```bash
haishui plugins pack show ./haishui-pack.yaml     # dry-run review
haishui plugins pack install ./haishui-pack.yaml  # review → confirm → install
haishui plugins pack export > haishui-pack.yaml   # snapshot the current install
haishui plugins pack export --enabled-only       # only plugins.enabled
```
...
same pinned install path as `haishui plugins install --ref <sha>` and record
...
capability-consent prompt — identical to a single `haishui plugins install`.
...
provenance (installed via `haishui plugins install`). Local-only plugins are
...
auto-installed — install those manually for now (`haishui skills`). Wiring
skill-hub ids into pack install is a documented follow-up seam.

### Install-time security scanning

Every `haishui plugins install` and `haishui plugins update` runs a static
...
On `haishui plugins update`, a dangerous verdict on the updated tree
...
Running `haishui plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `haishui plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.
...
- Haishui reuses that session's stored platform, chat, thread, profile, and conversation history. Plugins cannot supply a new chat route through this API.
- Haishui rechecks the stored route against the gateway's current authorisation rules before dispatch.
- Routes that relied only on an adapter-time or upstream authorisation decision are rejected unless Haishui can revalidate them from current core allowlists, pairing, or explicit allow-all configuration.
...
- The route and conversation are pinned while dispatch is pending. Haishui drops the request if topic recovery changes the route or the session rotates before handling starts.
...
:::warning
Only grant gateway injection to plugins you trust. Haishui checks this host API permission and restricts it to existing session routes, but Python plugins run in-process and this setting is not a sandbox.
:::
...
`ctx.call_mcp()` lets a plugin call a tool on one of the user's configured MCP servers — synchronously, from any hook or tool handler — routing through Haishui' existing native MCP client (same connections, trust-tier gates, circuit breaker, and reconnect logic as model-invoked MCP tools; never a parallel client).
