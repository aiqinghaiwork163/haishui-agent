...
Haishui Agent can drive your desktop — clicking, typing, scrolling,
...
The built-in `computer_use` toolset is the recommended Haishui integration. It
...
**Fresh installs already have the driver.** The Haishui installer
...
- **`haishui tools`** → pick `🖱️  Computer Use` — installs the driver
...
```
haishui computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `haishui computer-use
status` to verify the install.

Already have cua-driver? Haishui reuses it when it supports the 0.20 runtime
contract. During setup, toolset enablement, `haishui update`, and the first
`computer_use` call of a session, Haishui checks the local version and
...
selected with `HAISHUI_CUA_DRIVER_CMD` stays
under your control, so Haishui reports the incompatibility and leaves it
unchanged.

If you install Cua Driver first, `cua-driver skills install` installs Cua's
skill pack under `~/.cua-driver/skills/cua-driver`. Haishui autodetection is a
planned cua-driver follow-up, so currently point Haishui at that directory or
...
interface. The built-in toolset provides Haishui actions, configuration,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording**. Grant the identity named by `haishui computer-use doctor`. Standard mode uses CuaDriver.app; bounded and unrestricted modes use the Haishui host identity. |
...
```
haishui -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.haishui/config.yaml`.

## Permission modes and logged-in browser profiles

Haishui maps its existing approval UX onto cua-driver's immutable runtime
modes. Permission mode and capability manifest approval are launch settings.
They cannot change after the runtime starts:

| Haishui session | cua-driver mode | Human intervention |
|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Haishui approvals; Cua stops at its protected boundary |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Haishui risk acceptance; no runtime Cua prompts |
...
computer_use:
  ...
  capability_manifest: ~/.haishui/cua-manifest.yaml
```
...
for the format). Haishui launches a private runtime with
...
identity instead of resetting with every Haishui build), and Haishui verifies
...
resetting or closing the Haishui session, cancellation cleanup, or process exit
closes that transport session. Haishui also stops private runtimes that it
launched for bounded or unrestricted access. One Haishui
conversation cannot change another runtime's mode or grants. Bounded and
unrestricted modes use a private
embedded service under the Haishui host identity.
...
## `haishui computer-use doctor` — your first triage stop

`haishui computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ haishui computer-use doctor
...
Haishui run declares a public cua-driver **session name** (something like
`haishui-3a7b9c14d2e8`). The name labels cursor identity and related state, so
...
without it. Haishui disables it automatically where it is a known failure
...
Haishui keeps its wrapper skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the Haishui-side `computer_use` workflow and action vocabulary. For
...
The command links the pack into `~/.haishui/skills/cua-driver` (Haishui is one
...
These are **platform deep dives, not duplicates of the Haishui skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Haishui
...
image capture also saves a bounded, shareable copy under Haishui' image cache
...
Haishui applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.haishui/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Haishui applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Haishui agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Haishui agent itself at High integrity (launch from an
  elevated terminal); otherwise target non-elevated windows.
- **Windows: `haishui computer-use doctor` fails with "Access is denied"
  while the tool works.** A cua-driver installed under
  `C:\Program Files\WindowsApps` cannot be executed by the Haishui venv
  ...
  `HAISHUI_CUA_DRIVER_CMD` to a copy outside `WindowsApps`.
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Haishui warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Haishui from inside the RDP / console
...
On Linux, native Wayland support remains an explicit opt-in. Haishui passes the
...
```
HAISHUI_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
HAISHUI_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Haishui disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Haishui sets
...
When it's on, `haishui computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Haishui at a binary you built from source instead
of the published release. Haishui resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`HAISHUI_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Haishui testing (see notes).
...
### Option B — point Haishui straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`HAISHUI_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
HAISHUI_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
HAISHUI_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Haishui is using your build

- `haishui computer-use status` prints the resolved binary path and
  version.
- `haishui computer-use doctor` confirms the binary is reachable and
...
- **Haishui spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  Haishui YOLO, Haishui instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Haishui
  change needed either way.
- **Local builds skip the version check.** Haishui warns when the
...
**First action when anything's off: run `haishui computer-use doctor`.**
...
Run `haishui computer-use install` to fetch the cua-driver binary, or
run `haishui tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `haishui computer-use doctor` will
...
- **Haishui-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  Haishui `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  `LINUX.md` / `RECORDING.md` / `WEB_APPS.md`. Haishui autodetection is a
  planned follow-up; currently point Haishui at the installed pack directory
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Haishui among them)
