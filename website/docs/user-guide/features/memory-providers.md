...
Haishui Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
haishui memory setup      # interactive picker + configuration
haishui memory status     # check what's active
haishui memory off        # disable external provider
```

You can also select the active memory provider via `haishui plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.haishui/config.yaml`:
...
When a memory provider is active, Haishui automatically:
...
**Setup Wizard:**
```bash
haishui memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `haishui honcho setup` command still works (it now redirects to `haishui memory setup`), but is only registered after Honcho is selected as the active memory provider.
...
:::tip Migrating from `haishui honcho`
If you previously used `haishui honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Haishui profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Haishui profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Haishui profile. Host key `haishui` → default; `haishui.<profile>` for others. |
...
```bash
haishui profile create coder --clone
```

`--clone` creates a `haishui.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Haishui profile, creates host blocks for any profile without one, inherits settings from the default `haishui` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `haishui memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
haishui memory setup    # select "openviking"
# Or manually:
haishui config set memory.provider openviking
```

`haishui memory setup` can reuse or copy connection values from
`~/.openviking/ovcli.conf`. Manual setup uses the active profile's `.env` file;
for the default profile that is `~/.haishui/.env`, and for named profiles use
`~/.haishui/profiles/<profile>/.env`.
...
Peer identity is optional. By default, Haishui sends no peer ID and writes
...
Set `memory.openviking.agent: haishui` to restore the old peer-scoped writes.
...
Haishui sends `User-Agent: openviking-memory-haishui/<version>` on OpenViking
...
**Setup (Platform):**
```bash
haishui memory setup    # select "mem0" → "Platform"
# Or manually:
haishui config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.haishui/.env
```

**Setup (OSS):**
```bash
haishui memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
haishui memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
haishui memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Setup (Self-Hosted Dashboard):** connect to a Mem0 server you run via Docker (the dashboard's REST API):

```bash
haishui memory setup    # select "mem0" → "Self-hosted server"
# Or via flags:
haishui memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
```

Or configure manually — either as env vars:

```bash
echo "MEM0_HOST=http://localhost:8888" >> ~/.haishui/.env
echo "MEM0_API_KEY=your-admin-api-key" >> ~/.haishui/.env
```
...
**Config:** `$HAISHUI_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.haishui/.env`.
...
| `user_id` | `haishui-user` | User identifier |
| `agent_id` | `haishui` | Agent identifier |
...
**Switching modes:** Re-run `haishui memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
haishui memory setup    # select "hindsight"
# Or manually:
haishui config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.haishui/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p haishui ui start`

**Config:** `$HAISHUI_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `haishui` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Haishui Agent and the User` | Context label for retained memories |
...
**Setup:**
```bash
haishui memory setup    # select "holographic"
# Or manually:
haishui config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.haishui-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$HAISHUI_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
haishui memory setup    # select "retaindb"
# Or manually:
haishui config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.haishui/.env
```
...
# Then configure Haishui
haishui memory setup    # select "byterover"
# Or manually:
haishui config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$HAISHUI_HOME/byterover/` (profile-scoped)
...
| **Requires** | `pip install supermemory` + [cloud API key](http://app.supermemory.ai/integrations?connect=haishui), or a [self-hosted server](https://supermemory.ai/docs/self-hosting/overview) |
...
**Setup:**
```bash
haishui memory setup    # select "supermemory"
# Or manually:
haishui config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.haishui/.env
```
...
Before running `haishui memory setup`, set `base_url` in
`$HAISHUI_HOME/supermemory.json`:
...
Then run `haishui memory setup` and enter the API key printed by the local
server. Configuring the endpoint first ensures the setup connection probe also
stays local.

**Config:** `$HAISHUI_HOME/supermemory.json`
...
| `container_tag` | `haishui` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `haishui-{identity}` → `haishui-coder`) to isolate memories per Haishui profile
...
| **Requires** | `pip install haishui-memori` + `haishui-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install haishui-memori
haishui-memori install
haishui config set memory.provider memori
haishui memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `haishui-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](../profiles.md):

- **Local storage providers** (Holographic, ByteRover) use `$HAISHUI_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$HAISHUI_HOME/` so each profile has its own credentials
- **Cloud providers** (RetainDB) auto-derive profile-scoped project names
- **Env var providers** (OpenViking) are configured via each profile's `.env` file

## Providers Moving to the Plugin Catalog

Memory providers are moving out of the Haishui tree into their maintainers' own repositories,
...
When a provider you have configured stops shipping with Haishui, `haishui update` installs its
...
`false`, in which case it prints the `haishui plugins install <name>` one-liner instead).
