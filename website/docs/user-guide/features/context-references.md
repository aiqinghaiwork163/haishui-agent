...
Type `@` followed by a reference to inject content directly into your message. Haishui expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Haishui env: `$HAISHUI_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$HAISHUI_HOME/skills/.hub/`
