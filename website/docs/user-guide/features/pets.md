...
Haishui can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<HAISHUI_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Haishui renders the sprite at full fidelity when your
...
haishui pets list
haishui pets list cat

# Install a pet and make it active in one step
haishui pets install boba --select
...
## `haishui pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `haishui pets list [query] [--limit N]` |
| List installed pets | `haishui pets list --installed` |
| Install a pet | `haishui pets install <slug> [--select] [--force]` |
...
| Preview/animate | `haishui pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `haishui pets off` |
| Remove an installed pet | `haishui pets remove <slug>` |
| Diagnose setup | `haishui pets doctor` |

`haishui pets show` flags:
...
Beyond installing pre-made pets from the gallery, Haishui can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `haishui pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `haishui tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `HAISHUI_PET_IMAGE_PROVIDER` env var (e.g. `HAISHUI_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Haishui is
...
Run `haishui pets doctor` — it reports:
...
- The [`haishui-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-haishui-agent.md)
