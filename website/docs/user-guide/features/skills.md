...
All skills live in **`~/.haishui/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point Haishui at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
By default every profile is seeded with the bundled skill catalog, and each `haishui update` adds any newly bundled skills. If you want a profile with **no bundled skills** — and that stays empty across updates — you have two paths:

**At install time** (applies to the default `~/.haishui` profile):

```bash
curl -fsSL https://aiqinghaiwork163.github.io/haishui-agent/install.sh | bash -s -- --no-skills
```

**At profile-create time** (named profiles):

```bash
haishui profile create research --no-skills
```

**On an already-installed profile** (default or named), toggle it at runtime:

```bash
haishui skills opt-out            # stop future seeding — nothing on disk is touched
haishui skills opt-out --remove   # also delete UNMODIFIED bundled skills (confirms first)
haishui skills opt-in --sync      # undo: remove the marker and re-seed now
```

All three paths write a `.no-bundled-skills` marker into the profile directory. While the marker is present, the installer, `haishui update`, and any skill sync all skip bundled-skill seeding for that profile. Delete the marker (or run `haishui skills opt-in`) to re-enable.

:::note Safe by default
`haishui skills opt-out` only stops *future* seeding — it never deletes anything already on disk. The optional `--remove` flag deletes bundled skills **only** when they are unmodified (byte-identical to the version Haishui installed). Skills you have edited, skills installed from the hub, and skills you wrote yourself are always kept.
:::
...
(Plan mode works the same way but is a built-in command now: `/plan [request]` tells Haishui to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.haishui/plans/` relative to the active workspace/backend working directory.)

You can also interact with skills through natural conversation:

```bash
haishui chat --toolsets skills -q "What skills do you have?"
haishui chat --toolsets skills -q "Show me the axolotl skill"
```
...
description, the standard section order, Haishui-tool framing, no invented
commands).
...
/home/user/.haishui/cache/chart-q4-2025.png
...
When a missing value is encountered, Haishui asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `haishui setup` or `~/.haishui/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `haishui config migrate` prompts for unconfigured settings, and `haishui config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.haishui/skills/                  # Single source of truth
...
and `examples/`. Unreferenced repository files are not copied. Haishui scans the
...
policy above), Haishui can run [NVIDIA SkillEvaluator](https://github.com/NVIDIA/SkillEvaluator)
...
If you maintain skills outside of Haishui — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Haishui to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.haishui/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.haishui/skills/` (or `skills.create_dir` when configured — see below). Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch` (targeted or full rewrite), `write_file`, `remove_file`, or `delete`.
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the Haishui process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Haishui ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.haishui/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
By default the agent writes new skills to the profile-local `~/.haishui/skills/`. If you want agent-created skills to land somewhere else — a shared "brain" directory, a git-tracked repo, or a fleet-wide skills volume — set `create_dir` under the `skills` section:
...
Paths support `~` expansion and `${VAR}` substitution; relative paths resolve against your Haishui home. Setting `create_dir` to the local skills dir is the same as leaving it unset.


## Project-Local Skills

Repos can carry their own skills, active only for sessions started inside that project — the same pattern other agent harnesses use for repo-local configuration. When you launch Haishui inside a git checkout, it looks for skills in:

```text
<project-root>/.haishui/skills/    # Haishui-native location
...
Skills are procedure documents the agent follows, so Haishui does **not** auto-load them from arbitrary cloned repos. The first time you run Haishui in a repo with project skills, the banner shows a notice:

```text
◆ 3 project skill(s) found in /home/you/myproject but not loaded — run `haishui skills trust` to enable them.
```

Trust the repo once (from inside it, or by passing the path):

```bash
haishui skills trust             # trust the current repo
haishui skills trust ~/myproject # or explicitly
haishui skills untrust           # revoke
```

Trusted roots are stored in `skills.trusted_project_dirs` in `~/.haishui/config.yaml`. Set `skills.project_discovery: false` to turn the feature off entirely (no scanning, no notices).

### Precedence

Project skills are the **highest-precedence tier**: `project → local (~/.haishui/skills/) → external_dirs`. A project skill named `deploy` overrides a same-named profile or bundled skill for sessions inside that repo — that's the point: vendored repo skills win on their home turf, without touching your global profile. Project skills are tagged `[project]` in the agent's skill index so provenance stays visible.

Like external dirs, project skill directories are treated as repo-owned: autonomous skill maintenance (the curator) never modifies them, and new agent-created skills always go to `~/.haishui/skills/`.

### Scan-time quarantine

Trust is a repo-level decision, but a repo's skill content changes with every `git pull`. To close that gap, every project skill is scanned with the same security scanner used for Skills Hub installs before it enters the index. A skill whose scan verdict is **dangerous** (prompt-injection directives, credential-exfiltration commands, hidden-text tricks) is quarantined: it does not appear in the skill index, `skills_list`, slash commands, and refuses to load by name with an explanatory error. Scans are content-hash cached under `~/.haishui/cache/project_skill_scans/` (never inside your repo) and re-run automatically when the skill's content changes.
...
In the TUI and Desktop the project root follows each **session's workspace** (the directory shown in the sidebar / set with the workspace picker), so starting `haishui --tui` inside a trusted repo registers its project skills as slash commands even when `terminal.cwd` is left at the default placeholder `.`; two sessions open in two repos each see their own.
...
Bundles live in **`~/.haishui/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `haishui bundles list`.
...
haishui bundles create research

# Overwrite an existing bundle
haishui bundles create backend-dev --skill ... --force

# Delete a bundle
haishui bundles delete backend-dev

# Re-scan ~/.haishui/skill-bundles/ and report changes
haishui bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.haishui/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.haishui/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
Staged writes survive restarts under `~/.haishui/pending/skills/` and are
...
Unfiltered searches (CLI, TUI, and the dashboard) are answered from a cached centralized index that covers the external registries. That index is rebuilt periodically, so when it has no match for your query Haishui also asks `skills.sh`, ClawHub, LobeHub and well-known endpoints directly — a skill published minutes ago still shows up. That extra pass gets at most 8 seconds of the search budget, so a slow registry cannot turn a miss into a long wait. Custom GitHub taps are not part of that fallback (search them with `--source github`, or via the index once it catches up), and provider filters such as `--source nvidia` do not trigger it (those registries carry no provider data).

### Common commands

```bash
haishui skills browse                              # Browse all hub skills (official first)
haishui skills browse --source official            # Browse only official optional skills
haishui skills search kubernetes                   # Search all sources
haishui skills search react --source skills-sh     # Search the skills.sh directory
haishui skills search https://mintlify.com/docs --source well-known
haishui skills inspect openai/skills/k8s           # Preview before installing
haishui skills install openai/skills/k8s           # Install with security scan
haishui skills install official/security/1password
haishui skills install skills-sh/vercel-labs/json-render/json-render-react --force
haishui skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
haishui skills list --source hub                   # List hub-installed skills
haishui skills check                               # Check installed hub skills for upstream updates
haishui skills update                              # Reinstall hub skills with upstream changes when needed
haishui skills audit                               # Re-scan all hub skills for security
haishui skills uninstall k8s                       # Remove a hub skill
haishui skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
haishui skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
haishui skills publish skills/my-skill --to github --repo owner/repo
haishui skills snapshot export setup.json          # Export skill config
haishui skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Haishui. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `haishui skills search <query> --source skills-sh`. Haishui resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Haishui currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Haishui repository itself and install with built-in trust.
...
This is Vercel's public skills directory. Haishui can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
haishui skills search https://mintlify.com/docs --source well-known
haishui skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
haishui skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

Haishui can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
haishui skills install openai/skills/k8s
haishui skills tap add myorg/skills-repo
```
...
[Skills Hub](https://aiqinghaiwork163.github.io/haishui-agent/docs) page — instead of a
tag-derived guess. This is generic: any tap that ships the file gets real
categorization, no Haishui-side changes required.
...
- Haishui source id: `clawhub`

#### 6. LobeHub (`lobehub`)

Haishui can search and convert agent entries from LobeHub's public catalog into installable Haishui skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Haishui source id: `lobehub`

#### 7. browse.sh (`browse-sh`)

Haishui integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with Haishui' browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- Haishui source id: `browse-sh`
- Trust level: `community`

```bash
haishui skills search airbnb --source browse-sh
haishui skills inspect browse-sh/airbnb.com/search-listings-ddgioa
haishui skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Haishui also fetches explicitly referenced files under `references/`, `templates/`, `scripts/`, `assets/`, and `examples/`, then scans and installs the complete bundle.

- Haishui source id: `url`
- Identifier: the URL itself (no prefix needed)
- Scope: `SKILL.md` plus exact referenced support files in the allowlisted directories. Haishui does not enumerate or copy unrelated files from the host.

```bash
haishui skills install https://sharethis.chat/SKILL.md
haishui skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
haishui skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `haishui skills update` re-fetches from the same URL automatically when you want to refresh.
...
`haishui skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Haishui | Always trusted |
...
```bash
haishui skills check          # Report which installed hub skills changed upstream
haishui skills update         # Reinstall only the skills with updates available
haishui skills update react   # Update one specific installed hub skill
haishui skills update react --force   # Overwrite a skill you've edited locally
```

This uses the stored source identifier plus the current upstream bundle content hash to detect drift.

Checks skip network requests for missing or non-directory installs (`orphaned`) and unsafe or unresolvable recorded paths (`invalid_install`). Missing-directory entries can be removed with `haishui skills uninstall <name>`; invalid paths require inspecting and repairing the active profile's `skills/.hub/lock.json` before retrying. No entries are removed automatically.
...
Skills you have edited locally (the on-disk content no longer matches the hash recorded at install time) are **skipped** by `haishui skills update` so your changes are never silently overwritten. Pass `--force` to replace them with the upstream version anyway.
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Haishui users add with `haishui skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.haishui.tags`, `version`, `author`, `platforms`, `metadata.haishui.config`).
...
Haishui discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.

#### Minimal tap example

```
my-org/haishui-skills
└── skills/
    └── deploy-runbook/
        └── SKILL.md
```
...
After pushing that to GitHub, any Haishui user can subscribe and install:

```bash
haishui skills tap add my-org/haishui-skills
haishui skills search deploy
haishui skills install my-org/haishui-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.haishui/skills/.hub/taps.json`:
...
The `haishui skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `haishui skills tap list` shows the effective path per tap.
...
```bash
haishui skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.haishui/skills/.hub/taps.json` (created on demand).

## Bundled skill updates (`haishui skills reset`)

Haishui ships with a set of bundled skills in `skills/` inside the repo. On install and on every `haishui update`, a sync pass copies those into `~/.haishui/skills/` and records a manifest at `~/.haishui/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, Haishui recomputes the hash of your local copy and compares it to the origin hash:
...
Generated runtime caches inside a skill (`__pycache__/`, `.pytest_cache/`, `.mypy_cache/`, `.ruff_cache/`, and a `.pyc` sitting next to its `.py`) are not part of the hash, so running a skill's helper script never marks it user-modified or hides it from `haishui skills list-modified` / `diff`.

The protection is good, but it has one sharp edge. If you edit a bundled skill and then later want to abandon your changes and go back to the bundled version by just copy-pasting from `~/.haishui/haishui-agent/skills/`, the manifest still holds the *old* origin hash from whenever the last successful sync ran. Your fresh copy-paste contents (current bundled hash) won't match that stale origin hash, so sync keeps flagging it as user-modified.

`haishui skills reset` is the escape hatch:
...
haishui skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `HAISHUI_HOME`, so `haishui -p coder skills reset <name>` only affects that profile.
:::
