...
chat completions — use your Haishui-managed provider subscription as their
...
| Use case | "Use Haishui as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Haishui stores
the refresh token in `~/.haishui/auth.json` — the same place all Haishui
...
Model:      Haishui-4-70B    # or Haishui-4.3-36B, Haishui-4-405B
```
...
interface in `haishui_cli/proxy/adapters/`.
...
If you see `not logged in`, run `haishui portal`. If you see
...
`haishui_cli/proxy/adapters/<provider>.py` and registering it in
