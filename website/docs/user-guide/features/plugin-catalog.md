...
description: "Browse and install reviewed, SHA-pinned Haishui plugins from the curated catalog"
---

# Plugin Catalog

The plugin catalog is a curated, human-reviewed directory of Haishui plugins you
can install by name with a single command:

```bash
haishui plugins install <name>
```
...
| Field | Meaning |
|---|---|
| `name` | The catalog key you pass to `haishui plugins install` |
...
| `requires_haishui` | Minimum Haishui version, e.g. `>=0.19` (optional) |
...
- **Scanned at admission, trusted at install.** Admission CI runs the same
  security scanner the installer runs (`haishui plugins validate` includes a
...
The catalog name and the plugin's own manifest name can differ; `haishui
...
```bash
haishui plugins install touchdesigner
haishui plugins enable td
```
...
skill, so one install gives Haishui code, dependency, container and IaC scanning
plus the workflow for using it; the catalog name and manifest name match:

```bash
haishui plugins install snyk
haishui plugins enable snyk
```

### Updating a catalog install

`haishui plugins update <name>` never runs `git pull` for catalog installs —
...
enabled/disabled state is preserved. `haishui plugins list` shows catalog
...
`haishui plugins install <git-url>` still works for any repository, but it
...
`haishui plugins list --json` reports `update_available`, the Desktop Plugins
tab shows an **Update to 1.4.0** button, and `haishui plugins update <name>`
...
- [Plugins](plugins.md) — the plugin system itself: manifest format, enabling,
  configuration
- [Built-in Plugins](built-in-plugins.md) — plugins that ship with Haishui
- [Build a Haishui Plugin](../../developer-guide/plugins/index.md) — write your own
