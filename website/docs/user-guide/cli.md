...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Haishui Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Haishui Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `haishui setup --portal` — and you're ready to `haishui chat`. See [Nous Portal](../integrations/nous-portal.md).
:::

:::tip
Haishui also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `haishui --tui` — see the [TUI](tui.md) guide.
:::
...
haishui chat --query-file prompt.txt
haishui chat --query-file - < prompt.txt

# With a specific model
haishui chat --model "anthropic/claude-sonnet-4"

# With a specific provider
haishui chat --provider nous        # Use Nous Portal
haishui chat --provider openrouter  # Force OpenRouter
...
haishui chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
haishui --continue             # Resume the most recent CLI session (-c)
haishui --resume <session_id>  # Resume a specific session by ID (-r)
haishui --resume latest        # Resume the most recent session (same as -c)
haishui --resume latest --in ./dir  # Resume ./dir's latest session, staying in ./dir

# Verbose mode (debug output)
haishui chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
haishui -w                         # Interactive mode in worktree
haishui -w -z "Fix issue #123"     # Single query in worktree
```

### Worktree cleanup

`haishui -w` sessions create disposable worktrees under `<repo>/.worktrees/`.
...
```bash
haishui worktree list              # audit: age, size, verdict, reason per tree
haishui worktree list --json       # machine-readable audit (trees, external trees, branches)
haishui worktree prune             # remove safe trees + delete merged branches
haishui worktree prune --dry-run   # show the plan without changing anything
haishui worktree prune --older-than 7   # only reap trees idle for 7+ days
haishui worktree prune --trees-only     # leave local branches alone
haishui worktree prune --branches-only  # leave worktrees alone
```
...
- Trees **in use by a running haishui session** are never touched.
- **Untracked-only scratch** (PR body drafts, notes) is archived to
  `~/.haishui/archive/worktree-prune/` before its tree is removed — never
  destroyed.
...
launches `haishui -w` for days — no longer accumulate merged scratch trees
...
The `haishui plugins` commands manage native Haishui plugins and portable Agent
...
haishui plugins disable <plugin-name>
haishui plugins update <plugin-name>
haishui plugins remove <plugin-name>
```

Portable packages remain disabled until explicitly enabled. Haishui currently
...
<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Haishui CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Haishui CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `HAISHUI_YOLO_MODE` is on (either `haishui --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Haishui to background (Unix only). Run `fg` in the shell to resume. |
...
- **Runs on your machine, in the session working directory.** With the default local terminal backend `!pwd` matches what the agent would see. A remote or sandboxed `terminal.backend` (`ssh`, `docker`, …) is **not** used for `!` commands — they always run on the host where Haishui itself runs, so `!hostname` names your machine while the agent's `terminal` tool names the backend. Ask the agent (or open a shell on the target) to run something *inside* the backend. Path completion in the composer, by contrast, does follow the configured backend and lists the target's filesystem.
- **Approvals still apply.** A dangerous command (`rm -rf`, writes to `~/.haishui/config.yaml`, etc.) goes through the same approval prompt the agent's `terminal` tool uses. `!` is a cost/latency shortcut, not a security bypass.
...
Type `/` to see the autocomplete dropdown. Haishui supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Haishui replies |
...
```bash
haishui -s haishui-agent-dev,github-auth
haishui chat -s github-pr-workflow -s github-auth
```

Haishui loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.
...
`--ignore-rules` (equivalently `HAISHUI_IGNORE_RULES=1`) skips auto-load together with AGENTS.md, SOUL.md, `.cursorrules` and memory injection; explicit `-s` skills still load. The setting is profile-scoped: each profile's `config.yaml` controls its own list.

## Skill Slash Commands

Every installed skill in `~/.haishui/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.haishui/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Haishui..."
  # Add your own!
```
...
`Ctrl+J` and backslash continuation are enabled by default, matching Claude Code / Codex / OpenCode multiline shortcuts. On supported terminals such as iTerm2, Haishui also requests extended key reporting so `Shift+Enter` arrives as a distinct newline key. If your terminal sends LF for plain `Enter` and you need the legacy `Ctrl+J`-as-submit fallback, opt out:
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work by default. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Haishui — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The first time you press Enter while Haishui is working, Haishui prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Haishui to the background — just like any terminal process. The shell prints a confirmation:

```
Haishui Agent has been suspended. Run `fg` to bring Haishui Agent back.
```
...
Resume this session with:
  haishui --resume 20260225_143052_a1b2c3
...
```bash
haishui --continue                          # Resume the most recent CLI session
haishui -c                                  # Short form
haishui -c "my project"                     # Resume a named session (latest in lineage)
haishui --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
haishui --resume "refactoring auth"         # Resume by title
haishui --resume latest                     # Resume the most recent session (same as -c)
haishui --resume latest --in ./my-project   # Latest session for ./my-project's workspace
haishui -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `haishui sessions rename <id> <title>` from the command line. Use `haishui sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Haishui's SQLite state database under `~/.haishui/state.db`. The database keeps:
...
Haishui immediately confirms the task and gives you back the prompt:
