...
Haishui Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.haishui/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Haishui stores session history so it can resume conversations, but it does not
...
Haishui explicitly injects for that turn.
...
For example, if a user sends an image and asks Haishui to make a meme from it,
Haishui may inspect that image once with vision and run an image-processing
...
`haishui sessions prune` only when you want to delete old ended sessions from
storage. If `state.db` has simply grown large, start with the non-destructive
option first: `haishui sessions optimize` merges FTS5 index segments and
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`haishui` or `haishui chat`) |
| `oneshot` | Finite non-interactive runs: `haishui chat --oneshot -q`, `-Q`, `haishui -z`, and `-q` on non-TTY stdio. Hidden from the TUI, Desktop and dashboard session pickers (like `kanban` and `tool`), even when launched from inside a TUI or Desktop session — the run inherits that transport's environment but is not that conversation. Still counts as CLI history: `haishui -c` / `--resume latest` continue the last one-shot, and `haishui sessions list` shows it. An explicit `--source <tag>` always wins (`haishui chat -q --source tui` is stored as `tui`). |
...
haishui -c "my project"

# If there are lineage variants (my project, my project #2, my project #3),
# this automatically resumes the most recent one
haishui -c "my project"   # → resumes "my project #3"
```
...
haishui --resume 20250305_091523_a1b2c3d4
haishui -r 20250305_091523_a1b2c3d4

# Resume by title
haishui --resume "refactoring auth"

# Resume the most recent session — same lookup as -c
haishui --resume latest

# Or with the chat subcommand
haishui chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `haishui sessions list`.
...
haishui --resume latest --in ./my-project

# Works with the TUI too
haishui --tui --resume latest --in ./my-project
```
...
```bash
haishui --resume 20250305_091523_a1b2c3 --no-restore-cwd
```
...
`haishui sessions list` accepts `--workspace <needle>` to show only sessions whose workspace key (git repo root, else cwd) matches — by path substring or exact directory basename:

```bash
haishui sessions list --workspace my-project
haishui sessions list --workspace ~/code/haishui-agent
```

### Conversation Recap on Resume

When you resume a session, Haishui displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Haishui session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.haishui/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `haishui -r "<title>"` from the shell) and pick up where the platform left off.
...
Haishui automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `haishui sessions list` or `haishui sessions browse`.
...
```bash
haishui sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Haishui creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`haishui -c "my project"`), it automatically picks the most recent session in the lineage.
...
Haishui provides a full set of session management commands via `haishui sessions`:

### List Sessions

```bash
# List recent sessions (default: last 20)
haishui sessions list

# Filter by platform
haishui sessions list --source telegram

# Show more sessions
haishui sessions list --limit 50
```
...
`haishui sessions export` is one surface for every export format, selected with `--format`:
...
haishui sessions export backup.jsonl

# Export sessions from a specific platform
haishui sessions export telegram-history.jsonl --source telegram

# Export a single session
haishui sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4

# Point at a directory (existing, or ending in /) and the file is named for you:
# ~/exports/haishui_session_20250305_091523_a1b2c3d4.jsonl
haishui sessions export ~/exports/ --session-id 20250305_091523_a1b2c3d4

# Redact API keys/tokens/credentials from the exported content
haishui sessions export backup.jsonl --redact
```

Exported files contain one JSON object per line with full session metadata and all messages.

Each record also carries a `timings` block derived from the message timestamps, so a reader of an export attached to a bug report can tell a single long model gap from many small tool round-trips without reconstructing it by hand. It holds only ids, roles, counts and durations — `wall_clock_ms`, `largest_gap_ms`, `role_counts`, `tool_calls_emitted` and per-message `intervals` — never prompt text, tool arguments or results, so it survives `--redact` unchanged. Haishui does not persist a model/tool stopwatch, so `complete` is always `false`; when a session has no timestamped messages, `available` is `false` and `unavailable_reason` says why. The block is rebuilt on every export and ignored (and not counted toward size limits) on import.
...
haishui sessions export --format html --session-id 20250305_091523_a1b2c3d4 transcript.html

# All Telegram sessions from the last week in one file, secrets redacted
haishui sessions export --format html --newer-than 1w --source telegram --redact archive.html
```
...
haishui sessions export prompts.jsonl --session-id 20250305_091523_a1b2c3d4 --only user-prompts

# Markdown, straight to stdout
haishui sessions export - --session-id 20250305_091523_a1b2c3d4 --only user-prompts --format md
```
...
`--format trace` emits Claude Code JSONL — the transcript shape the Hugging Face Hub auto-detects for its [Agent Trace Viewer](https://huggingface.co/docs/hub/agent-traces). Write it locally, or add `--upload` to push it to your own private `haishui-traces` dataset (reads `HF_TOKEN`):

```bash
# Trace of the most recent session, to stdout
haishui sessions export --format trace

# One session to a local trace file
haishui sessions export --format trace --session-id 20250305_091523_a1b2c3d4 trace.jsonl

# Upload straight to your private HF traces dataset
haishui sessions export --format trace --session-id 20250305_091523_a1b2c3d4 --upload
```
...
Pass `--format md` or `--format qmd` when you want a readable, file-based archive before hiding or deleting old sessions. Markdown/QMD exports write one file per session into a directory (default: `~/.haishui/session-exports`).

```bash
# Export one session to Markdown
haishui sessions export --format md --session-id 20250305_091523_a1b2c3d4

# Export a compression lineage as one logical document
haishui sessions export --format md --session-id 20250305_091523_a1b2c3d4 --lineage logical

# Preview ended sessions older than 90 days without writing files
haishui sessions export --format md --older-than 90 --dry-run

# Export ended Telegram sessions older than 2 weeks to QMD files
haishui sessions export --format qmd --older-than 2w --source telegram

# Export long Claude sessions, secrets redacted
haishui sessions export --format md --model sonnet --min-messages 50 --redact

# Only after verification, export and delete one explicitly named session
haishui sessions export --format md --session-id 20250305_091523_a1b2c3d4 --delete-after-verified --yes
```
...
haishui sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
haishui sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
haishui sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
haishui sessions pin 20250305_091523_a1b2c3d4
haishui sessions pin 20250305 20250306

# Remove the pin
haishui sessions unpin 20250305_091523_a1b2c3d4

# List pinned sessions
haishui sessions pinned

# Machine-readable output, e.g. for a nightly backup of your pin set
haishui sessions pinned --json > pinned-sessions.json
```
...
haishui sessions prune

# Custom age threshold — bare numbers are days
haishui sessions prune --older-than 30

# Durations work too: 5h, 30m, 2d, 1w
haishui sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
haishui sessions prune --newer-than 5h

# Explicit window with absolute timestamps
haishui sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
haishui sessions prune --source telegram
haishui sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
haishui sessions prune --newer-than 5h --title "smoke test"   # title substring
haishui sessions prune --older-than 30 --max-messages 3        # tiny sessions
haishui sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
haishui sessions prune --model gpt-5 --older-than 1w           # by model (substring)
haishui sessions prune --provider openrouter --older-than 60   # by billing provider
haishui sessions prune --branch feature/old-experiment         # by git branch
haishui sessions prune --user 12345678 --chat-type group       # by messaging origin
haishui sessions prune --max-tokens 500 --older-than 7         # by token usage
haishui sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
haishui sessions prune --newer-than 5h --dry-run

# Skip confirmation
haishui sessions prune --older-than 30 --yes
```
...
the implicit 90-day default, so `haishui sessions prune --source cron` or
`--model gpt-4o` matches all ages — add a time flag to narrow it. Only a
completely bare `haishui sessions prune` keeps the 90-day cutoff. Every
...
`haishui sessions archive` takes the same filters as `prune` but soft-hides
...
haishui sessions archive --newer-than 5h

# Archive by title substring, preview first
haishui sessions archive --title "dry run" --dry-run
haishui sessions archive --title "dry run" --yes
```

At least one filter is required — a bare `haishui sessions archive` refuses to
archive your entire history. Archived sessions are hidden from
`haishui sessions list` and `/resume` but remain in the database and can be
unarchived from the Desktop/Dashboard session list.

### Session Statistics

```bash
haishui sessions stats
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`haishui insights`](../reference/cli-commands.md#haishui-insights).
...
`haishui sessions repair-routing` finds message-bearing session rows with no
...
haishui sessions repair-routing
...
haishui sessions repair-routing --apply

# Widen/narrow the contiguity window (default 900 seconds)
haishui sessions repair-routing --max-gap-seconds 300
```
...
(`cp ~/.haishui/state.db ~/.haishui/state.db.bak`).
...
profile. Current versions put new state in the right place; `haishui sessions
repair-profiles` settles what is already crossed.

```bash
# Report only — every store is scanned, nothing is written
haishui sessions repair-profiles

# Perform the repairs (stop the gateway first; a snapshot of every store is taken)
haishui sessions repair-profiles --apply

# Machine-readable report
haishui sessions repair-profiles --json
```
...
`haishui profile migrate-identity <old> <new>`), and `agent:main:…` rows inside
...
Started a conversation in another agent CLI? You can pull it into Haishui and
continue it here. Haishui reads Claude Code's session logs
...
haishui sessions import

# Limit to one tool, or point at a specific file
haishui sessions import --from claude
haishui sessions import --from codex ~/.codex/sessions/2026/08/15/rollout-....jsonl

# Import-and-resume in one step
haishui --resume @claude
haishui --resume @codex
```

`haishui sessions import` creates a new Haishui session titled
`Imported from Claude Code: <first user message>` (or Codex CLI) and prints
the id plus a ready-to-paste `haishui --resume <id>` command.
`--resume @claude` / `--resume @codex` show the same picker and drop you
straight into the imported conversation.

**Haishui Desktop** has the same importer in the command palette (**Import
session**). It lists the logs on the machine the
connected backend runs on — not the computer running the app — shows a
read-only preview, and **Continue in Haishui** copies the conversation into the
...
When Haishui cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Haishui uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Haishui in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.haishui/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.haishui/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `gateway_routing` table in `~/.haishui/state.db` | Maps session keys to active session IDs (origin metadata, expiry flags) |
| Legacy routing mirror | `~/.haishui/sessions/sessions.json` | Backward-compat mirror of the routing index, written when `gateway.write_sessions_json: true` (the default) |
...
`state.db`; `~/.haishui/sessions/sessions.json` is a **legacy mirror** of it,
...
`haishui sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.haishui/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `haishui sessions list`, the cause is
`state.db` not receiving them — run `haishui sessions repair` and watch for a
...
`*.jsonl` files in `~/.haishui/sessions/`. They are no longer written or
read by Haishui. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Haishui process in the same `HAISHUI_HOME`

Without pruning, `state.db` grows without bound — multi-GB files within weeks were reported on gateway + cron installs. If you would rather keep every ended session forever (the pre-#54189 behavior), turn it off in `~/.haishui/config.yaml`:
...
`haishui sessions recover` synthesizes for orphaned messages) whose last
...
`in its tip segment`). `haishui sessions export` still works for such sessions.
...
haishui sessions delete <session_id>
...
:::tip
Auto-prune is **on by default**: ended sessions that have been inactive for `sessions.retention_days` (default 90) are removed at startup, and active sessions are never touched (see [Automatic Cleanup](#automatic-cleanup) above). Session history powers `session_search` recall across past conversations, so if you want to keep every ended session forever, set `sessions.auto_prune: false` in `config.yaml`, or raise `retention_days`. With auto-prune off, `haishui sessions prune` remains available for one-off cleanup (observed failure mode without any pruning: a 384 MB `state.db` with ~1000 sessions slowing down FTS5 inserts and `/resume` listing).
:::
