...
# Connecting Desktop to Many Haishui Instances

Register every Haishui backend you own — the local runtime, remote gateways on
your LAN or VPS, SSH hosts, and Haishui Cloud instances — in one desktop app,
...
- **The sidebar profile rail** — the plug button at the right end of the rail
  (tooltip: **"Connect another Haishui gateway…"**) deep-links straight to
...
list of Haishui gateways. Its intro says it plainly: *"Manage this device and
every Haishui gateway it can reach through remote, SSH, or Cloud connections."*
Each entry is a *connection*:

| Kind | What it is | Auth |
|---|---|---|
| **Local** | "The Haishui runtime managed by this app." | automatic |
| **Remote gateway** | "A Haishui gateway reachable over HTTP(S) — LAN, Tailscale, or the internet." | session token or OAuth |
| **SSH** | "A Haishui install reached over SSH." The app opens the tunnel and starts the dashboard for you | SSH key + adopted token |
| **Haishui Cloud** | "A hosted instance discovered through your Haishui Cloud account." | portal sign-in |
...
- **Duplicates are rejected when you save**: there is only ever one **local**
  ...
  **SSH** entries are deduplicated on the normalized `user@host:port` plus
  remote profile.
- Cloud entries normally come from the Haishui Cloud sign-in/discovery flow at
  the top of the Gateways page — the **Haishui Cloud** kind in the add-connection
...
The Haishui Cloud panel also lists **Saved Cloud gateways** when portal discovery
...
3. Pick the kind: **Local**, **Haishui Cloud**, **Remote gateway**, or **SSH**.
   (**Local** is disabled while the app-managed local entry exists — which is
   almost always; **Haishui Cloud** directs you to the cloud sign-in/discovery
   flow above.)
4. Fill the fields:
   - **Name** — required, unique; the "device name" shown everywhere this
     instance appears (placeholder: `Homelab`). Max 64 characters.
   - *Remote gateway only:*
     - **Gateway URL** — the base URL of a running `haishui serve` backend,
   ...
   - *SSH only:*
     ...
     - **Haishui path (optional)** — full path to the `haishui` executable on
       the remote (for example `/opt/haishui/bin/haishui`). Leave blank to
       auto-detect. Set it when the remote's non-interactive shell does not
       have `haishui` on its `PATH` and **Test** reports *"Haishui is not
...
Gateways are machines or hosted backends; profiles are isolated Haishui agents
...
connection is registered) dispatches `haishui update` to every eligible
...
- **Haishui Cloud** instances are skipped with a *"Managed by Haishui Cloud"*
...
panel, ⌘K **Update Haishui**, the update-ready toast) run the same fan-out
...
- **"Connection test failed"** — the backend isn't reachable at that URL from
  this machine. Check that `haishui serve` is running on the remote host, the
...
- **"Update Haishui Desktop to chat with agents on other connections"** — the
