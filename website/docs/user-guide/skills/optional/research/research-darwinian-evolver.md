...
| | |
|---|---|
| Source | Optional — install with `haishui skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research\darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Haishui Agent |
| License | MIT |
...
import upstream classes into Haishui itself.
...
```bash
mkdir -p ~/.haishui/cache/darwinian-evolver && cd ~/.haishui/cache/darwinian-evolver
...
```bash
cd ~/.haishui/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.haishui/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.haishui/skills/research/darwinian-evolver
DE_DIR=~/.haishui/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Haishui core.
   Custom driver scripts under `~/.haishui/skills/...` are user-side and fine.
