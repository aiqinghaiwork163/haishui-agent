...
| | |
|---|---|
| Source | Optional — install with `haishui skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Haishui MCP catalog. The
...
Two halves, in this order: the editor side must be up before Haishui connects.
...
### One-time, Haishui side

    haishui mcp install unreal-engine
...
`~/.haishui/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Haishui — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Haishui connects
...
2. Start the Haishui session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Haishui
they appear as:

| Haishui tool | Purpose |
|---|---|
...
then happens at `haishui mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Haishui runs batched calls
...
- **Start order matters.** Editor + server up first, then the Haishui
...
- **Timeouts on long operations.** Haishui' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.haishui/config.yaml` for
