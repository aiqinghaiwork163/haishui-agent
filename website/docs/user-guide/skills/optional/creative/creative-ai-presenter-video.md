...
| | |
|---|---|
| Source | Optional — install with `haishui skills install official/creative/ai-presenter-video` |
| Path | `optional-skills/creative/ai-presenter-video` |
| Version | `1.0.0` |
| Author | cclank (https://github.com/cclank/lanshu-create-ai-presenter-video), ported by Haishui Agent |
| License | MIT |
...
> kept substantively verbatim in `references/`; Haishui adaptations live in
> this hub file. Scripts are deterministic (no network, no credentials).

## Haishui adaptations (read first)

- **Skill dir resolution** — upstream hardcoded its own agent's skills path.
  In Haishui the loader expands `${HAISHUI_SKILL_DIR}` to this skill's installed
  directory, so every command below uses that token directly:

  ```bash
  SKILL_DIR="${HAISHUI_SKILL_DIR}"
  ```
