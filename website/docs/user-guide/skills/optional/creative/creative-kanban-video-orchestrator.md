...
| | |
|---|---|
| Source | Optional — install with `haishui skills install official/creative/kanban-video-orchestrator` |
...
short to a music video to an ASCII loop — in a Haishui Kanban pipeline that
...
3. **Generates** a setup script that creates Haishui profiles, project workspace, and the initial kanban task
...
For mapping role → which Haishui skills + toolsets it loads, see
...
3. Creates each Haishui profile via `haishui profile create --clone`
...
7. Fires the initial `haishui kanban create` task assigned to the director

Use `scripts/bootstrap_pipeline.py` to generate setup.sh from a brief +
team-design JSON. See **[references/kanban-setup.md](https://github.com/aiqinghaiwork163/haishui-agent/blob/main/optional-skills/creative\kanban-video-orchestrator/references/kanban-setup.md)**
...
```bash
haishui kanban watch --tenant <project-tenant>     # live events
haishui kanban list  --tenant <project-tenant>     # board snapshot
haishui dashboard                                   # visual board UI
```
...
playbook, see **[references/monitoring.md](https://github.com/aiqinghaiwork163/haishui-agent/blob/main/optional-skills/creative\kanban-video-orchestrator/references/monitoring.md)**.
...
task graphs. See **[references/examples.md](https://github.com/aiqinghaiwork163/haishui-agent/blob/main/optional-skills/creative\kanban-video-orchestrator/references/examples.md)**.
...
6. **The director never executes.** Even with the full `kanban + terminal +
   ...
   a `haishui kanban create` call to a specialist profile. The kanban
...
8. **Verify API keys BEFORE firing.** External APIs (TTS, image-gen,
   image-to-video) need keys in `${HAISHUI_HOME:-~/.haishui}/.env` or the user's secret store.
