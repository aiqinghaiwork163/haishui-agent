...
| | |
|---|---|
| Source | Optional — install with `haishui skills install official/creative/pixel-art` |
...
import sys
import os
sys.path.insert(0, os.path.expanduser("~/.haishui/skills/creative/pixel-art/scripts"))
...
```bash
cd ~/.haishui/skills/creative/pixel-art/scripts
...
- ffmpeg on PATH (only needed for video — Haishui installs package this)
