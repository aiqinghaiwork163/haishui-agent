...
- smoke-test a server with the FastMCP CLI before wiring it into Haishui or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Haishui. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.haishui/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.haishui/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Haishui integration, either:

- configure the server in `~/.haishui/config.yaml` using the `native-mcp` skill, or
...
### Haishui cannot see the deployed server

The server-building part may be correct while the Haishui config is not. Load the `native-mcp` skill and configure the server in `~/.haishui/config.yaml`, then restart Haishui.
