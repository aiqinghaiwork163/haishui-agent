...
| | |
|---|---|
| Source | Optional — install with `haishui skills install official/payments/stripe-projects` |
...
Wraps the [Stripe Projects](https://projects.dev) CLI plugin so Haishui can provision SaaS services (Neon, Twilio, Vercel, etc.), generate and sync credentials into the user's `.env`, and manage billing across providers from one place.
