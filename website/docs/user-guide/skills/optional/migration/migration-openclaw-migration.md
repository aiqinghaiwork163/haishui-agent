---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into Haishui"
sidebar_label: "Openclaw Migration"
description: "Import an OpenClaw setup (memories, skills) into Haishui"
---
...
Import an OpenClaw setup (memories, skills) into Haishui.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `haishui skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration\openclaw-migration` |
| Version | `1.0.0` |
| Author | Haishui Agent (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Haishui`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> Haishui Migration

Use this skill when a user wants to move their OpenClaw setup into Haishui Agent with minimal manual cleanup.
...
```bash
haishui claw migrate              # Full interactive migration
...
haishui claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `haishui setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_haishui.py` to:

- import `SOUL.md` into the Haishui home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Haishui memory entries
- merge OpenClaw command approval patterns into Haishui `command_allowlist`
- migrate Haishui-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Haishui working-directory configuration
- copy OpenClaw skills into `~/.haishui/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Haishui workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.haishui/tts/`
- archive non-secret docs that do not have a direct Haishui destination
...
- `scripts/openclaw_to_haishui.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.haishui/skills/migration/openclaw-migration/scripts/openclaw_to_haishui.py`

Do not guess a shorter path like `~/.haishui/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.haishui/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Haishui-compatible secrets, currently:
...
Haishui CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Haishui. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Haishui workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Haishui skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python ~/.haishui/skills/migration/openclaw-migration/scripts/openclaw_to_haishui.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python /home/USER/.haishui/skills/migration/openclaw-migration/scripts/openclaw_to_haishui.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python ~/.haishui/skills/migration/openclaw-migration/scripts/openclaw_to_haishui.py --preset user-data
```

Execute a user-data migration:

```bash
python ~/.haishui/skills/migration/openclaw-migration/scripts/openclaw_to_haishui.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python ~/.haishui/skills/migration/openclaw-migration/scripts/openclaw_to_haishui.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python ~/.haishui/skills/migration/openclaw-migration/scripts/openclaw_to_haishui.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Haishui unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Haishui targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Haishui destination. Unsupported auth blobs must still be reported as skipped.
...
- Haishui persona state imported
- Haishui memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.haishui/skills/openclaw-imports/`
