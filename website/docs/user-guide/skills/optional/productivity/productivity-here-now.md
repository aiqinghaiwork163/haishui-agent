...
| | |
|---|---|
| Source | Optional — install with `haishui skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${HAISHUI_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${HAISHUI_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${HAISHUI_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client haishui
```
...
```bash
PUBLISH="${HAISHUI_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client haishui
```
...
```bash
DRIVE="${HAISHUI_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `haishui`)    |
