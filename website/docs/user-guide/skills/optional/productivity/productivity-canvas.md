...
| | |
|---|---|
| Source | Optional — install with `haishui skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Haishui Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${HAISHUI_HOME:-~/.haishui}/.env`:
...
```bash
CANVAS="python $HAISHUI_HOME/skills/productivity/canvas/scripts/canvas_api.py"
