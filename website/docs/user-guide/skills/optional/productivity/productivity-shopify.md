...
| | |
|---|---|
| Source | Optional — install with `haishui skills install official/productivity/shopify` |
...
4. Save to `${HAISHUI_HOME:-~/.haishui}/.env`:
   ```
...
}' '{"input":{"title":"Test Hoodie","status":"DRAFT","vendor":"Haishui","productType":"Apparel","tags":["test"]}}'
```
