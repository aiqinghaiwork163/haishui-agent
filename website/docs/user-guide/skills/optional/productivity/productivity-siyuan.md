...
| | |
|---|---|
| Source | Optional — install with `haishui skills install official/productivity/siyuan` |
...
3. Store it in `${HAISHUI_HOME:-~/.haishui}/.env`:
   ```
...
```yaml
# In ~/.haishui/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
