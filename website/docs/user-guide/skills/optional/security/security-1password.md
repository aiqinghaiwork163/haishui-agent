...
- `tmux` available for stable authenticated sessions during Haishui terminal calls (desktop app flow only)
...
### Service Account (recommended for Haishui)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${HAISHUI_HOME:-~/.haishui}/.env` (the skill will prompt for this on first load).
...
## Haishui Execution Pattern (desktop app flow)

Haishui terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/haishui-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/haishui-op.sock"
