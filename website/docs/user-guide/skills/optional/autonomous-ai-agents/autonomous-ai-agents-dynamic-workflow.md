...
| | |
|---|---|
| Source | Optional — install with `haishui skills install official/autonomous-ai-agents/dynamic-workflow` |
| Path | `optional-skills/autonomous-ai-agents/dynamic-workflow` |
| Version | `2.0.0` |
| Author | Teknium + Haishui Agent |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `orchestration`, `fan-out`, `subagents`, `delegation`, `verification`, `migration`, `audit`, `research`, `campaign` |
| Related skills | [`haishui-agent`](../../bundled/autonomous-ai-agents/autonomous-ai-agents-haishui-agent.md), [`simplify-code`](../../bundled/software-development/software-development-simplify-code.md) |
...
for serial chains. For a refactor or fix campaign on haishui-agent itself, load
`haishui-agent` (the dev workflow) alongside; this skill owns the fan-out shape.
...
10. Live QA is its own wave: one squad per surface, isolated `HAISHUI_HOME`,
