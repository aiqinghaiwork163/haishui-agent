---
title: "Honcho — Configure and troubleshoot Honcho memory for Haishui"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for Haishui"
---
...
Configure and troubleshoot Honcho memory for Haishui.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `haishui skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Haishui

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Haishui profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/haishui#running-honcho-locally-with-haishui

### Verify

```bash
haishui honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Haishui profile's AI peer
...
Honcho models conversations as interactions between **peers**. Haishui creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Haishui instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Haishui run |
| `global` | Single session across all directories |

Manual override: `haishui honcho map my-project-name`
...
Each Haishui profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block haishui.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `haishui.coder` host block in `honcho.json`
...
```bash
haishui honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Haishui when Honcho memory is active.
...
Config file: `$HAISHUI_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `haishui honcho setup`. Ensure `memory.provider: honcho` is in `~/.haishui/config.yaml`.

### Memory not persisting across sessions
Check `haishui honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `haishui profile create <name> --clone`. For existing profiles: `haishui honcho sync`.
...
| Command | Description |
|---------|-------------|
| `haishui honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `haishui honcho status` | Show resolved config, connection test, peer info for active profile |
| `haishui honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `haishui honcho disable` | Disable Honcho for the active profile |
| `haishui honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `haishui honcho peers` | Show peer identities across all profiles |
| `haishui honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `haishui honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `haishui honcho sessions` | List known directory-to-session-name mappings |
| `haishui honcho map <name>` | Map current working directory to a Honcho session name |
| `haishui honcho identity` | Seed AI peer identity or show both peer representations |
| `haishui honcho sync` | Create host blocks for all Haishui profiles that don't have one yet |
| `haishui honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Haishui + Honcho |
| `haishui memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `haishui memory status` | Show active memory provider and config |
| `haishui memory off` | Disable external memory provider |
