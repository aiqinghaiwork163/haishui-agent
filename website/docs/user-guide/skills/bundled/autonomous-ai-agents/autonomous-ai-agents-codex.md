...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Haishui terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Haishui itself, `model.provider: openai-codex` uses Haishui-managed Codex
OAuth from `~/.haishui/auth.json` after `haishui auth add openai-codex`. For the
...
## Haishui Gateway Caveat

When invoking the Codex CLI from a Haishui gateway/service context (for example,
