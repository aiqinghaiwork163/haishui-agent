...
This wrapper skill teaches the Haishui `computer_use` workflow and action
...
skill installed by `cua-driver skills install`. Haishui autodetection is a
planned cua-driver follow-up, so currently point Haishui at the resulting
...
Haishui autodetection is a planned follow-up in trycua/cua. For now, the command
installs the pack under `~/.cua-driver/skills/cua-driver`; point Haishui at that
