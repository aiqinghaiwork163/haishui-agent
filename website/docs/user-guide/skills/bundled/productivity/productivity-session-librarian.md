...
| List/filter by metadata (age, source, cost, tokens, workspace) | `haishui sessions list` / `stats` via terminal |
| Rename | `haishui sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `haishui sessions archive <filters>` |
| Delete (destructive) | `haishui sessions delete` / `haishui sessions prune <filters>` |
| Export before deleting anything valuable | `haishui sessions export --session-id <id> --format md` |
...
`haishui sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `haishui sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `haishui sessions list`
