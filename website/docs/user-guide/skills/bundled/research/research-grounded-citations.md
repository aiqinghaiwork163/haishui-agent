...
Ledger location: `$HAISHUI_HOME/cache/citations/ledger.json` (profile-aware).
Override per task with `--ledger <path>` or `HAISHUI_CITATION_LEDGER`.

## How to Run

```bash
S=~/.haishui/skills/research/grounded-citations/scripts/sources.py
...
`haishui skills install official/social-media/reddit-reading` or
`haishui skills install official/research/rss-feeds` before using them.
...
- **Parallel subagents.** Each subagent has its own working directory; point
  them all at one ledger with `--ledger` (or `HAISHUI_CITATION_LEDGER`) if their
