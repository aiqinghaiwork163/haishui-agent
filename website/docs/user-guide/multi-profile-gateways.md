...
If you only run one Haishui agent, you don't need this page — see
...
[Connecting Desktop to Many Haishui Instances](./multi-connection-desktop.md).

## When to use this

You want this setup when you have two or more Haishui agents that should all
...
(`ai.haishui.gateway-<name>.plist`), a systemd user service
(`haishui-gateway-<name>.service`), a systemd **system** service when installed with
`sudo haishui gateway install --system` (runs as the invoking user via `User=`), a
Windows Scheduled Task, or an s6/Docker service — and the Desktop app spawns its own
per-profile `haishui serve` backend. This guide adds the patterns for managing them
collectively.
...
haishui profile create coder
haishui profile create personal-bot
haishui profile create research
...
[`haishui gateway migrate --multiplex`](#migrating-from-per-profile-gateways) and
...
`haishui gateway migrate --multiplex` one-liner. Nothing is changed on disk.
...
Other processes (`haishui -p <name> gateway start`, the dashboard, `haishui gateway
...
```bash
haishui config set gateway.multiplex_profiles true    # or false
haishui gateway restart
```

Equivalently, in the default profile's `~/.haishui/config.yaml`:
...
You do **not** run `haishui gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `haishui gateway run`, `start`,
...
loop. `haishui -p coder gateway stop` refuses the same way (exit 78) when coder has no
gateway of its own — there is nothing to stop but the multiplexer, which
`haishui gateway stop` on the default profile takes down for every served profile.
...
The setup flows follow the same rule: `haishui -p coder setup gateway`, `haishui -p coder setup`,
`haishui -p coder gateway setup` and `haishui -p coder import` configure the profile's bots but
...
- **WhatsApp (bridge) and Relay are shared ingress owned by the default profile.**
  ...
  is not being served; `haishui gateway status --profile work` shows
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.haishui/profiles/coder/.env`; the default listener key is rejected. Under
...
- Webhook routes without `profile` remain default-profile routes and are not
  ...
  way: `haishui webhook subscribe <name> --route-profile coder` writes
  `profile: coder` into the default gateway's `webhook_subscriptions.json` and
  prints the `/p/coder/webhooks/<name>` URL (`haishui webhook ls` shows the
...
A standalone `haishui -p coder gateway run` binds coder's Twilio, LINE, Teams,
...
```
$ haishui -p coder gateway status
✓ Gateway is running via the default-profile multiplexer
  Manage it from the default profile: haishui gateway status
...
`haishui gateway status` and `haishui status` on the default profile list the same
...
There is a single process-level PID and lock (the multiplexer, under the default home). `haishui status` on the default profile reports the multiplexer and lists the profiles it serves (`Serves: coder, research`). `haishui -p coder status` and `haishui -p coder gateway status` report "running via the default-profile multiplexer" instead of "stopped". The dashboard's `/api/status?profile=coder` / Channels page report the multiplexer as coder's running gateway, with coder's own adapters as its platforms. The single `gateway_state.json` lives under the default home: secondary adapters appear there as `<profile>:<platform>` entries beside `served_profiles`; no per-profile gateway status file is written.

`haishui -p coder cron status` prints `Scheduler host: default-profile multiplexer`, then checks coder's own ticker heartbeat and last successful tick. A missing or stale heartbeat produces a warning rather than an unconditional running verdict; the restart hint targets `haishui --profile default gateway restart`. `cron list` and `cron create` also warn when a served profile has no fresh heartbeat. `cron status` adds tick-failure details that those lightweight checks do not read.
...
`HINDSIGHT_BANK_ID`, `HAISHUI_HONCHO_HOST`) are read from the routed profile's
...
`HAISHUI_LANGUAGE` and `display.language`, and `hooks.outbound[].secret_env` are
...
`HAISHUI_HOME`, without the default profile's secrets in their environment, and
...
| Provider logins and the credential pool (`auth.json`: OAuth tokens, `haishui auth add` keys) | The profile's own `auth.json` | "Not connected to any AI provider" with the `haishui -p <name> model` hint — never the default profile's login, and a refresh never writes to the root store |
...
| Dashboard actions (`haishui -p <name> …` spawned by the Desktop/dashboard) | A scrubbed child env pinned to that profile's `HAISHUI_HOME` | The child loads its own `.env`; the dashboard profile's tokens and ports are not inherited |
...
archived or deleted instead — `haishui profile delete <name>`, or move the
...
multiplexer started as `haishui -p <name> gateway run` always ticks its own
profile's cron store as well.

The served set is **live**. A profile created while the multiplexer is running
(`haishui profile create`, the dashboard, Desktop or the TUI) is served at once:
...
default profile's `gateway_state.json` is updated, and `haishui -p <name> gateway
...
adapters the same way, and `haishui profile rename` unroutes the old name before
...
`~/.local/bin/haishui-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: haishui-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    haishui gateway "$action"
  else
    haishui -p "$profile" gateway "$action"
  fi
}
...
```bash
haishui-gateways start      # start every configured profile
haishui-gateways stop       # stop every configured profile
haishui-gateways restart    # restart all
haishui-gateways status     # status across all
haishui-gateways list       # delegates to `haishui gateway list`
```

:::tip
The `default` profile is targeted with `haishui gateway <action>` (no `-p`),
not `haishui -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `haishui -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.haishui.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/haishui-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.haishui.gateway.plist` /
`haishui-gateway.service`.
...
```bash
# Default profile
tail -f ~/.haishui/logs/gateway.log
tail -f ~/.haishui/logs/gateway.error.log

# Named profile
tail -f ~/.haishui/profiles/<name>/logs/gateway.log
tail -f ~/.haishui/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.haishui/logs/gateway.log ~/.haishui/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
haishui logs -f                  # follow default profile
haishui -p coder logs -f         # follow one profile
haishui logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
haishui profile list             # profiles + model + gateway state
haishui-gateways status          # full status across every profile
launchctl list | grep haishui    # macOS — PIDs and labels
systemctl --user list-units 'haishui-gateway-*'   # Linux — units
```
...
```
~/.haishui/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.haishui/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
haishui config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.haishui/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=haishui --why="gateways running" \
  sleep infinity &
...
`haishui-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.haishui/.env ~/.haishui/profiles/*/.env
```
...
```bash
haishui gateway migrate --multiplex --dry-run   # print the plan and any blockers; changes nothing
haishui gateway migrate --multiplex             # apply (asks for confirmation on a TTY; -y skips)
haishui gateway migrate --standalone            # roll back to per-profile gateways
```

### What `haishui update` does
...
service) and `gateway.multiplex_profiles` is off, `haishui update` runs the same
preflight:

- **Nothing blocks it** → the migration runs automatically (the same code path
  as `haishui gateway migrate --multiplex --yes`) and prints what it did. This
...
install that is already multiplexing is left alone. `haishui update` also does
nothing when no secondary profile runs its own gateway — it never flips modes
on an install where nothing was running.

### Boundaries `haishui update` never crosses on its own

The unattended hook only folds profiles that share **one UNIX user, one service
domain and one `profiles/` tree** — the shape `haishui profile create` produces.
...
| `HAISHUI_HOME` outside `<default home>/profiles/` | a unit pinning `HAISHUI_HOME=/opt/haishui/profiles/emma` |

In that case `haishui update` prints the boundary it found plus
`haishui gateway migrate --multiplex`, and changes nothing — no unit is removed
...
findings appear as **notices** in `haishui gateway migrate --multiplex --dry-run`
...
```bash
haishui config set gateway.auto_multiplex_migration false
```

`haishui update` then leaves per-profile gateways exactly as they are, with no
...
other setting, so a value pinned in the managed scope (`/etc/haishui/config.yaml`)
wins over the profile's own file. It governs the **automatic** path only:
`haishui gateway migrate --multiplex` is an explicit request and still migrates
(and is the supported way to opt back in). Absent or `true` keeps the default
behaviour described above.

The explicit command is different: `haishui gateway migrate --multiplex` with
...
`haishui profile create --clone` leaves the source's bot tokens and allowlists
...
Older clones that still carry them are flagged by `haishui profile list`.
:::
...
1. Stops each secondary profile's standalone gateway and uninstalls its
   ...
   recorded in `~/.haishui/gateway_migration.json` for rollback.
...
above). `haishui profile create` confirms this when the live multiplexer picked the
profile up; it prints the `haishui gateway restart` reminder only when it could not
reach the multiplexer (for example, a gateway started from an older build).

### Rollback

```bash
haishui gateway migrate --standalone
```
...
window, the next `haishui gateway migrate --multiplex` sees the flag on, the
...
with `haishui config set gateway.multiplex_profiles false && haishui gateway restart`
...
flag, stop the per-profile tasks, `haishui gateway restart`). The dashboard's
System page offers the same migration as a button when the preflight finds an
eligible install.

## Updating the code

`haishui update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `haishui gateway start` after a previous `haishui gateway stop`. The
...
```bash
ps -ef | grep "haishui_cli.*-p <profile>"
cat ~/.haishui/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.haishui.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.haishui.gateway-<profile>.plist

# Linux
systemctl --user restart haishui-gateway-<profile>.service
```

### Health check

```bash
haishui doctor                  # default profile
haishui -p <profile> doctor     # one profile
```
