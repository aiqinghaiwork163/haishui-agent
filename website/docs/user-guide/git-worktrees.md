...
title: "Git Worktrees"
description: "Run multiple Haishui agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Haishui Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Haishui so each session has a clean, isolated working directory.

## Why Use Worktrees with Haishui?

Haishui treats the **current working directory** as the project root:

- CLI: the directory where you run `haishui` or `haishui chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.haishui/config.yaml`
...
Haishui creates `.worktrees/my-experiment/` inside the repo (branch
`haishui/my-experiment`, based on the freshly-fetched remote tip unless
`worktree_sync: false`), and retargets the session's terminal and file tools
into it — no restart needed. Omit the name to get a random `haishui-<id>`
...
git worktree add ../repo-feature feature/haishui-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/haishui-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Haishui there:

```bash
cd ../repo-feature

# Start Haishui in the worktree
haishui
```
...
git worktree add ../repo-experiment-a feature/haishui-a
git worktree add ../repo-experiment-b feature/haishui-b
```
...
Each Haishui process:

- Works on its own branch (`feature/haishui-a` vs `feature/haishui-b`).
...
- Haishui checkpoint data under `~/.haishui/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Haishui experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/haishui-checkpoints-docs`, `feature/haishui-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Haishui from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `haishui -w` (Automatic Worktree Mode)

Haishui has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `haishui/haishui-<hash>`).
...
```bash
haishui -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `haishui -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Haishui session its own clean checkout.
