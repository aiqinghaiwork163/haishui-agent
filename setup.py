...
Haishui Agent (see website/docs/getting-started/platform-support.md). The
...
build sandbox. ``nix/python.nix`` sets ``HAISHUI_NIX_BUILD=1`` on the
Haishui package derivation, so only that build may create an artifact.
...
_IN_NIX_BUILD = os.environ.get("HAISHUI_NIX_BUILD") == "1"
...
_BLOCK_MESSAGE = (
    ...
    "the Haishui Nix derivation sets HAISHUI_NIX_BUILD=1. If it does, file a bug."
)
...
# Root single-file modules (``run_agent``, ``haishui_state``, ``toolsets``...)
