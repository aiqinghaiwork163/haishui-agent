...
This plugin ships bundled with Haishui but is **opt-in** — it only loads when
...
haishui tools  # → Langfuse Observability
...
Set these in `~/.haishui/.env` (or via `haishui tools`):
...
haishui chat -q "hello"              # then check Langfuse for a "Haishui turn" trace
```

Generation observations include the Haishui system prompt when the provider
uses a separate `system` param (Anthropic Messages API). Open an **LLM call**
child span to inspect `role: system` (truncated via `HAISHUI_LANGFUSE_MAX_CHARS`).

## Optional tuning

```bash
HAISHUI_LANGFUSE_ENV=production       # environment tag
HAISHUI_LANGFUSE_RELEASE=v1.0.0       # release tag
HAISHUI_LANGFUSE_SAMPLE_RATE=0.5      # sample 50% of traces
HAISHUI_LANGFUSE_MAX_CHARS=12000      # max chars per field (default: 12000)
HAISHUI_LANGFUSE_CAPTURE=sanitized    # content capture mode (see below)
HAISHUI_LANGFUSE_DEBUG=true           # verbose plugin logging
```

## Capture modes

`HAISHUI_LANGFUSE_CAPTURE` controls how much *content* (prompts, responses,
...
```bash
haishui plugins disable observability/langfuse
```
