...
class ChronosCronScheduler(CronScheduler):
    ...
    def _note_identity_rejected(self) -> None:
        ...
        ``agent:*`` client or the hosted bootstrap session, and a plain ``haishui auth`` login is
        ...
        logger.warning(
            ...
            "or the hosted bootstrap session), so no job can be armed. A normal `haishui auth` re-login "
        ...
        if self._stop_event is None:
            return  # start() never ran (e.g. a CLI `haishui cron add`); nothing to tick here
