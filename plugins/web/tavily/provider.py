...
keyless ring — keyless access is opt-in by selecting Tavily in ``haishui tools``.
"""
...
def _missing_key_error(action: str) -> str:
    return f"TAVILY_API_KEY is not set. Get a key at https://app.tavily.com/home or select Tavily in `haishui tools` for opt-in keyless {action}."
