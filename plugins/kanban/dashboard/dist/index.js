/**
 * Haishui Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.haishui/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__HAISHUI_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__HAISHUI_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "haishui-kanban-dot-triage",
    todo: "haishui-kanban-dot-todo",
    ready: "haishui-kanban-dot-ready",
    running: "haishui-kanban-dot-running",
    blocked: "haishui-kanban-dot-blocked",
    review: "haishui-kanban-dot-review",
    done: "haishui-kanban-dot-done",
    archived: "haishui-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-haishui-task";
  ...
  const DOCS_URL = "https://aiqinghaiwork163.github.io/haishui-agent/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://aiqinghaiwork163.github.io/haishui-agent/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "haishui.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="haishui-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])haishui-kanban-md-code\1/i.test(attrs)) {
      return ' class="haishui-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "haishui-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "haishui-kanban-md",
  ...
  // standard `drop` event and our `haishui-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("haishui-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("haishui-kanban-column--drop");
          if (target) target.classList.add("haishui-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("haishui-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("haishui-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("haishui-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__HAISHUI_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "haishui-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "haishui-kanban-attention",
        "haishui-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "haishui-kanban-attention-bar" },
        h("span", { className: "haishui-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "haishui-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "haishui-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "haishui-kanban-attention-row",
                  "haishui-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "haishui-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "haishui-kanban-attention-row-id" }, task.id),
                h("span", { className: "haishui-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "haishui-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "haishui-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "haishui-kanban-diag-action-btn",
      action.suggested ? "haishui-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " haishui-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".haishui-kanban-drawer-comment-row input, .haishui-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "haishui-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("haishui-kanban-diag", sevClass) },
      h("div", { className: "haishui-kanban-diag-header" },
        h("span", { className: "haishui-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "haishui-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "haishui-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "haishui-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "haishui-kanban-diag-data-row" },
                  h("span", { className: "haishui-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "haishui-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "haishui-kanban-diag-data-row" },
                h("span", { className: "haishui-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "haishui-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "haishui-kanban-diag-reassign-row" },
            h("span", { className: "haishui-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "haishui-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "haishui-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "haishui-kanban-diag-msg",
              msg.ok ? "haishui-kanban-diag-msg--ok" : "haishui-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "haishui-kanban-section" },
      h("div", { className: "haishui-kanban-section-head-row" },
        h("span", { className: "haishui-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "haishui-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "haishui-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "haishui-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "haishui-kanban-docs-link",
      title: "Open Haishui Kanban docs in a new tab",
      "aria-label": "Haishui Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `haishui kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "haishui-kanban-msg-ok" : "haishui-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardProjectBadge(props) {
    ...
    return h(Badge, {
      variant: "outline",
      className: "haishui-kanban-board-project text-xs font-normal gap-1",
      ...
      h("button", {
        type: "button",
        className: "haishui-kanban-board-project-unbind",
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "haishui-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "haishui-kanban-boardswitcher" },
      h("div", { className: "haishui-kanban-boardswitcher-inner" },
  ...
  // Live (non-archived) Haishui projects available for board scoping,
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "haishui-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "haishui-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "haishui-kanban-dialog-title" },
        ...
        h("div", { className: "haishui-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Haishui profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "haishui-kanban-bulk" },
      h("span", { className: "haishui-kanban-bulk-count" },
      ...
      h("div", { className: "haishui-kanban-bulk-priority",
      ...
      h("div", { className: "haishui-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Haishui profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "haishui-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("haishui-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("haishui-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "haishui-kanban-trash",
        dragOver ? "haishui-kanban-trash--drop" : "",
        props.draggingTaskId ? "haishui-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "haishui-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "haishui-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".haishui-kanban-card")) return true;
      if (target.closest && target.closest(".haishui-kanban-column-add")) return true;
      if (target.closest && target.closest(".haishui-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("haishui-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("haishui-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".haishui-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "haishui-kanban-columns",
        isScrollable ? "haishui-kanban-columns--scrollable" : "",
        isPanning ? "haishui-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("haishui-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("haishui-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "haishui-kanban-column",
        dragOver ? "haishui-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "haishui-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "haishui-kanban-col-check",
        ...
        h("span", { className: cn("haishui-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "haishui-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "haishui-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "haishui-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "haishui-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "haishui-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "haishui-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "haishui-kanban-lane" },
                  h("div", { className: "haishui-kanban-lane-head" },
                    h("span", { className: "haishui-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "haishui-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "haishui-kanban-card--stale-red";
    if (age >= tier.amber) return "haishui-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".haishui-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "haishui-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "haishui-kanban-card",
        props.selected ? "haishui-kanban-card--selected" : "",
        props.failed ? "haishui-kanban-card--failed" : "",
        props.draggingSource ? "haishui-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "haishui-kanban-card-content" },
          h("div", { className: "haishui-kanban-card-row" },
            h("label", {
              className: "haishui-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "haishui-kanban-card-check",
            ...
            h("span", { className: "haishui-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or haishui kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "haishui-kanban-warning-badge",
                    "haishui-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "haishui-kanban-needs-assignee",
          ...
          h("div", { className: "haishui-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "haishui-kanban-card-row haishui-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "haishui-kanban-assignee",
                            title: `Assigned to Haishui profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "haishui-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "haishui-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "haishui-kanban-ago",
  ...
  // centered modal reusing the haishui-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "haishui-kanban-dialog haishui-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "Haishui profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "Haishui profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "haishui-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "haishui-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "haishui-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "haishui-kanban-drawer-close",
        ...
        data ? h("div", { className: "haishui-kanban-drawer-comment-foot" },
          h("div", {
            className: "haishui-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "haishui-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "haishui-kanban-section" },
      h("div", { className: "haishui-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "haishui-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "haishui-kanban-drawer-body" },
      h("div", { className: "haishui-kanban-drawer-title" },
        h("span", { className: cn("haishui-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "haishui-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "haishui-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "haishui-kanban-section" },
            h("div", { className: "haishui-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "haishui-kanban-section" },
            h("div", { className: "haishui-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "haishui-kanban-done-no-result haishui-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "haishui-kanban-section" },
            h("div", { className: "haishui-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "haishui-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "haishui-kanban-section" },
        h("div", { className: "haishui-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "haishui-kanban-comment" },
            h("div", { className: "haishui-kanban-comment-head" },
              h("span", { className: "haishui-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "haishui-kanban-diag-action-btn",
      ...
      h("div", { className: "haishui-kanban-section" },
        h("div", { className: "haishui-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "haishui-kanban-comment" },
                h("div", { className: "haishui-kanban-comment-head" },
                  h("span", { className: "haishui-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "haishui-kanban-comment-ago" },
      ...
      h("div", { className: "haishui-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            key: e.id,
            className: cn(
              "haishui-kanban-event",
              isDiag ? "haishui-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "haishui-kanban-event-header" },
                  h("span", { className: "haishui-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "haishui-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "haishui-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "haishui-kanban-event-header-plain" },
                  h("span", { className: "haishui-kanban-event-kind" }, e.kind),
                  h("span", { className: "haishui-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "haishui-kanban-event-phantom-row" },
                  h("span", { className: "haishui-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "haishui-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "haishui-kanban-event-payload" },
    ...
    return h("div", { className: "haishui-kanban-section" },
      h("div", { className: "haishui-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "haishui-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `haishui-kanban-run--${r.outcome || r.status || "ended"}`
          : "haishui-kanban-run--active";
        return h("div", { key: r.id, className: cn("haishui-kanban-run", outcomeClass) },
          h("div", { className: "haishui-kanban-run-head" },
            h("span", { className: "haishui-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "haishui-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "haishui-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "haishui-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "haishui-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "haishui-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "haishui-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "haishui-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "haishui-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "haishui-kanban-pre haishui-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "haishui-kanban-meta-row" },
      h("span", { className: "haishui-kanban-meta-label" }, props.label),
      h("span", { className: "haishui-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "haishui-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "haishui-kanban-meta-row" },
        h("span", { className: "haishui-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "haishui-kanban-meta-value haishui-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "haishui-kanban-meta-row" },
        h("span", { className: "haishui-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "haishui-kanban-meta-value haishui-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "haishui-kanban-meta-row" },
        h("span", { className: "haishui-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "haishui-kanban-meta-value haishui-kanban-editable",
    ...
    return h("div", { className: "haishui-kanban-meta-row" },
      h("span", { className: "haishui-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "haishui-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "haishui-kanban-section" },
      h("div", { className: "haishui-kanban-section-head-row" },
        h("span", { className: "haishui-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "haishui-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "haishui-kanban-section" },
      h("div", { className: "haishui-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "haishui-kanban-deps-row" },
        h("span", { className: "haishui-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "haishui-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "haishui-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "haishui-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "haishui-kanban-dep-chip-x",
      ...
      h("div", { className: "haishui-kanban-deps-row" },
        h("span", { className: "haishui-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "haishui-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "haishui-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "haishui-kanban-msg-ok"
          : "haishui-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "haishui-kanban-section" },
      ...
      h("div", { className: "haishui-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "haishui-kanban-home-sub haishui-kanban-home-sub--on"
              : "haishui-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__HAISHUI_PLUGINS__ && typeof window.__HAISHUI_PLUGINS__.register === "function") {
    window.__HAISHUI_PLUGINS__.register("kanban", KanbanPage);
  }
})();
