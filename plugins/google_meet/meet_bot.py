...
Standalone subprocess spawned by ``process_manager.py``; configured via ``HAISHUI_MEET_*`` env,
status + transcript written under ``$HAISHUI_MEET_OUT_DIR`` (filesystem is the only IPC).
...
Debug: ``HAISHUI_MEET_URL=... HAISHUI_MEET_OUT_DIR=/tmp/x HAISHUI_MEET_HEADED=1 \\
...
# collects {speaker, text}; ``window.__haishuiMeetDrain()`` pulls new entries.
_CAPTION_OBSERVER_JS = r"""
(() => {
  if (window.__haishuiMeetInstalled) return;
  window.__haishuiMeetInstalled = true;
  window.__haishuiMeetQueue = [];
  ...
  function pushEntry(speaker, text) {
    ...
    window.__haishuiMeetQueue.push({
      ts: Date.now(),
  ...
  window.__haishuiMeetDrain = () => {
    const out = window.__haishuiMeetQueue.slice();
    window.__haishuiMeetQueue = [];
    return out;
  };
})();
...
_ADMISSION_PROBE_JS = r"""
    (() => {
      ...
      if (window.__haishuiMeetInstalled && document.querySelector(
...
def _start_pcm_pump(rt: dict, bridge_info: dict, pcm_path: Path, state: "_BotState") -> None:
    ...
    if platform_tag == "linux":
        cmd = ["paplay", "--raw", "--rate=24000", "--format=s16le", "--channels=1",
               f"--device={target or 'haishui_meet_sink'}", str(pcm_path)]
...
def _setup_realtime(rt: dict, api_key: str, state: _BotState) -> None:
    ...
    if not api_key:
        state.set(error="realtime mode requested but no API key in HAISHUI_MEET_REALTIME_KEY/OPENAI_API_KEY — falling back to transcribe")
        rt["enabled"] = False
        return
    try:
...
_BotConfig = SimpleNamespace  # everything the bot reads from ``HAISHUI_MEET_*`` env vars


def _config_from_env() -> _BotConfig:
    env = os.environ.get
    out_raw = env("HAISHUI_MEET_OUT_DIR", "").strip()
    return _BotConfig(
        url=env("HAISHUI_MEET_URL", "").strip(),
        out_dir=Path(out_raw) if out_raw else None,
        headed=env("HAISHUI_MEET_HEADED", "").lower() in {"1", "true", "yes"},
        auth_state=env("HAISHUI_MEET_AUTH_STATE", "").strip(),
        guest_name=env("HAISHUI_MEET_GUEST_NAME", "Haishui Agent"),
        duration_s=_parse_duration(env("HAISHUI_MEET_DURATION", "")),
        realtime=env("HAISHUI_MEET_MODE", "transcribe").strip().lower() == "realtime",
        # HAISHUI_MEET_REALTIME_KEY is resolved by process_manager.start() via the parent's
        # profile secret scope; OPENAI_API_KEY only serves standalone `python -m` runs.
        realtime_api_key=env("HAISHUI_MEET_REALTIME_KEY") or env("OPENAI_API_KEY", ""),
        realtime_model=env("HAISHUI_MEET_REALTIME_MODEL", "gpt-realtime"),
        realtime_voice=env("HAISHUI_MEET_REALTIME_VOICE", "alloy"),
        realtime_instructions=env("HAISHUI_MEET_REALTIME_INSTRUCTIONS", ""),
        lobby_timeout=float(env("HAISHUI_MEET_LOBBY_TIMEOUT", "300")))
    ...
    while not stop_flag["stop"]:
        ...
        if not state.in_call and (now - last_admission_check) > 3.0:
            ...
            if _probe(page, _ADMISSION_PROBE_JS):
                ...
                return
        try:
            queued = page.evaluate("window.__haishuiMeetDrain && window.__haishuiMeetDrain()")
...
def run_bot() -> int:
    ...
    if not _is_safe_meet_url(cfg.url):
        sys.stderr.write("google_meet bot: refusing to launch — HAISHUI_MEET_URL must be a "
                         "meet.google.com URL. got: %r\n" % cfg.url)
        return 2
    if cfg.out_dir is None:
        sys.stderr.write("google_meet bot: HAISHUI_MEET_OUT_DIR is required\n")
        return 2
