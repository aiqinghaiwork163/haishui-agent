...
One active meeting at a time, recorded in ``$HAISHUI_HOME/workspace/meetings/.active.json``
...
def _root() -> Path:
    return Path(get_haishui_home()) / "workspace" / "meetings"
...
def start(url: str, *, out_dir: Optional[Path] = None, headed: bool = False,
          auth_state: Optional[str] = None, guest_name: str = "Haishui Agent", duration: Optional[str] = None,
    ...
    for name in ("transcript.txt", "status.json"):
        with contextlib.suppress(OSError):
            (out / name).unlink()
    env = {**os.environ, "HAISHUI_MEET_URL": url, "HAISHUI_MEET_OUT_DIR": str(out),
           "HAISHUI_MEET_GUEST_NAME": guest_name}
    for value, var in (
        (headed and "1", "HAISHUI_MEET_HEADED"),
        (auth_state, "HAISHUI_MEET_AUTH_STATE"),
        (duration, "HAISHUI_MEET_DURATION"),
        (mode, "HAISHUI_MEET_MODE"),  # bot defaults to transcribe when unset (v1 behavior)
        (realtime_model, "HAISHUI_MEET_REALTIME_MODEL"),
        (realtime_voice, "HAISHUI_MEET_REALTIME_VOICE"),
        (realtime_instructions, "HAISHUI_MEET_REALTIME_INSTRUCTIONS")):
        if value:
            env[var] = value
    ...
    if not realtime_api_key:
        ...
        realtime_api_key = get_secret("HAISHUI_MEET_REALTIME_KEY") or get_secret("OPENAI_API_KEY")
    if realtime_api_key:
        env["HAISHUI_MEET_REALTIME_KEY"] = realtime_api_key
