...
Let the haishui agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where haishui runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `haishui meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `haishui meet` CLI |
...
| `cli.py` | `haishui meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$HAISHUI_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `haishui meet node {run,list,approve,remove,status,ping}` |
...
haishui meet install                                      # pip + Chromium
haishui meet setup                                        # preflight
haishui meet auth                                         # optional
haishui meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
haishui meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.haishui/.env
haishui meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
haishui meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
haishui meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.haishui/.env
haishui meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, haishui will **not** switch your system audio input automatically — the
...
haishui plugins enable google_meet
haishui meet node run --display-name my-mac --host 0.0.0.0 --port 18789
