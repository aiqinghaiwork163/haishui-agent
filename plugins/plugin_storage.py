"""Per-plugin persistent storage: ``<haishui home>/plugin-data/<name>/``.

Plugins must NOT park state in ``<haishui home>/plugins/<name>/`` (the install dir, deleted by
...
# Mirrors the plugin-name shape `haishui plugins install` accepts (no separators/traversal).
...
def plugin_data_dir(name: str) -> Path:
    """Return (and create) ``<haishui home>/plugin-data/<name>/``; resolves ``get_haishui_home()`` on
    ...
    root = get_haishui_home() / "plugin-data" / _validate_name(name)
