...
An out-of-tree ACP provider (``~/.haishui/plugins/model-providers/`` or a pip entry point)
...
class CopilotACPProfile(ProviderProfile):
    ...
    def fetch_models(
        ...
        The CLI may keep its login in an OS credential store with no token Haishui can reuse, so
        ...
        """
        from haishui_cli.auth import resolve_external_process_provider_credentials
...
copilot_acp = CopilotACPProfile(
    ...
    # haishui_cli/auth.py), so existing setups keep working.
    process_command="copilot",
    process_args=("--acp", "--stdio"),
    process_command_env_vars=("HAISHUI_COPILOT_ACP_COMMAND", "COPILOT_CLI_PATH"),
    process_args_env_var="HAISHUI_COPILOT_ACP_ARGS",
)
