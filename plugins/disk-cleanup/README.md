...
Auto-tracks and cleans up ephemeral files created during Haishui Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$HAISHUI_HOME` and `/tmp/haishui-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `HAISHUI_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `HAISHUI_HOME` or `/tmp/haishui-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$HAISHUI_HOME/disk-cleanup/` is itself excluded
- `$HAISHUI_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
