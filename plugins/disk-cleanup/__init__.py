"""disk-cleanup plugin — auto-cleanup of ephemeral Haishui session files.
...
def _extract_paths_from_terminal(args: Dict[str, Any], result: str) -> Set[str]:
    ...
    if isinstance(cmd, str) and cmd:
        with contextlib.suppress(ValueError):  # tokenise — catches `touch /tmp/haishui-x/test_foo.py`
...
All operations are scoped to HAISHUI_HOME and /tmp/haishui-*.
...
def _cmd_track(argv: List[str]) -> str:
    ...
    return f"Not tracked (already present, missing, or outside HAISHUI_HOME): {path_arg}"
...
def register(ctx) -> None:
    ...
    ctx.register_command("disk-cleanup", handler=_handle_slash,
                         description="Track and clean up ephemeral Haishui session files.")
