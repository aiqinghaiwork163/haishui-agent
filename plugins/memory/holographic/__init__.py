"""haishui-memory-store — holographic memory plugin (MemoryProvider): structured fact storage with entity
resolution, trust scoring, and HRR-based compositional retrieval. Original plugin by dusterbloom (PR #2351).
Config in $HAISHUI_HOME/config.yaml under plugins.haishui-memory-store: db_path ($HAISHUI_HOME/memory_store.db),
...
def _load_plugin_config() -> dict:
    try:
        from haishui_cli.config import load_config_readonly  # canonical: managed-scope overlay + ${VAR} expansion
        return cfg_get(load_config_readonly(), "plugins", "haishui-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, haishui_home):
        """Write config to config.yaml under plugins.haishui-memory-store."""
        # The canonical writer: config lock, managed-mode refusal, default stripping, atomic replace.
        # ``merge_existing`` keeps every other section; *haishui_home* is the active profile already.
        from haishui_cli.config import save_config
        save_config({"plugins": {"haishui-memory-store": dict(values)}}, merge_existing=True)

    def get_config_schema(self):
        from haishui_constants import display_haishui_home
        return [
            {"key": "db_path", "description": "SQLite database path", "default": f"{display_haishui_home()}/memory_store.db"},
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _haishui_home = str(get_haishui_home())
        db_path = self._config.get("db_path", _haishui_home + "/memory_store.db")
        if isinstance(db_path, str):  # expand $HAISHUI_HOME so paths resolve to the active profile
            db_path = db_path.replace("$HAISHUI_HOME", _haishui_home).replace("${HAISHUI_HOME}", _haishui_home)
