...
- OpenViking server running and reachable from Haishui

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
Haishui can identify older servers that expose the legacy status-only health
...
connection values into Haishui, or create a minimal `ovcli.conf` when one does
not exist.
...
default profile that is `~/.haishui/.env`; for a named profile use
...
OpenViking's server config is separate from Haishui:
...
Haishui-side provider config is read from environment variables in the active
profile's `.env`:
...
When `OPENVIKING_API_KEY` is set, Haishui lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Haishui sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
Haishui also sends `User-Agent: openviking-memory-haishui/<version>` on
OpenViking requests. This standard harness identifier contains the Haishui
...
does not ask for a peer ID. Without a configured peer, Haishui sends neither
...
order remains environment, linked OpenViking config, then Haishui YAML. To use
...
on the old implicit `haishui` peer now use user memory for new writes. Without
...
Set `agent: haishui` to restore peer-scoped writes. Memories written at user
...
`viking_remember` creates a one-shot `haishui-remember-<random>` OpenViking
...
not advertised or used. The one-shot session is separate from the live Haishui
...
Use the same OpenViking profile and credentials as Haishui for manual recovery.
...
Haishui built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Haishui action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under user memory, or the configured peer memory directory |

Built-in `replace` and `remove` operations are not mirrored because Haishui
...
file URIs, such as
`viking://user/default/peers/haishui/memories/preferences/mem_abc123.md` (any
