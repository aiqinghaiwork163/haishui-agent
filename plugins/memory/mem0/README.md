...
```bash
haishui memory setup    # select "mem0"
```
...
Behavioral settings live in `$HAISHUI_HOME/mem0.json` (set them via `haishui memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.haishui/.env`.
...
| `user_id` | `haishui-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Haishui session and call `mem0_search` — it connects to your server.
...
```bash
haishui memory setup mem0 --mode oss \
...
```bash
haishui memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$HAISHUI_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.haishui/mem0_qdrant"}}
  }
}
```
...
```bash
haishui memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$HAISHUI_HOME/mem0.json`).
