...
- Hosted: API key from [app.supermemory.ai/integrations?connect=haishui](http://app.supermemory.ai/integrations?connect=haishui)
...
Before running `haishui memory setup`, add the local endpoint to
...
Then run `haishui memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$HAISHUI_HOME/supermemory.json`
...
| `container_tag` | `haishui` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `haishui-{identity}` → `haishui-coder`). |
...
`https://api.supermemory.ai`. Haishui resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: haishui`, and document writes stamp
`metadata.sm_source: haishui`. This is a **functional routing key, not telemetry**:
it groups Haishui-written memories into a dedicated "Haishui" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Haishui profile:

```json
{
  "container_tag": "haishui-{identity}"
}
```

For a profile named `coder`, this resolves to `haishui-coder`. The default profile resolves to `haishui-default`. Without `{identity}`, all profiles share the same container.
