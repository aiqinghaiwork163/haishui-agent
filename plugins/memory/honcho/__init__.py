...
Config chain: $HAISHUI_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
"""
...
class HonchoMemoryProvider(DialecticMixin, MemoryProvider):
    ...
    def save_config(self, values, haishui_home):
        """Merge ``values`` into $HAISHUI_HOME/honcho.json (Honcho SDK native format); a file that does not parse raises.
        ...
        config_path = Path(haishui_home) / "honcho.json"
    ...
    def _resolve_session_key(self, cfg, session_id: str, **kwargs) -> str:
        ...
        ) or session_id or "haishui-default"
    ...
    def _run_session_init(self, label: str) -> bool:
        ...
        try:
            self._do_session_init(self._config, self._lazy_init_session_id or "haishui-default", **dict(init_kwargs))
        except Exception as e:
            self._manager = None
            self._session_initialized = False
            detail: object = e
    ...
    def _do_session_init(self, cfg, session_id: str, **kwargs) -> None:
        ...
        if cfg.session_strategy == "per-session":
            ...
            try:
                ...
                self._manager.migrate_memory_files(self._session_key, str(get_haishui_home() / "memories"))
    ...
    def _pop_auth_notice(self) -> str:
        ...
        return ("[Honcho memory status] Authentication with the Honcho memory backend has expired and automatic "
                ...
                "Tell the user (once) that Honcho memory is paused and that running 'haishui honcho setup' "
    ...
    def _peer_failure_text(self) -> str:
        ...
        if self._init_peer_platform in _LOCAL_PLATFORMS:
            return f"{text} Set one with 'haishui honcho peer --user <name>'."
