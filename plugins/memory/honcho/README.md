...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/haishui>
...
```bash
haishui memory setup honcho   # configure Honcho directly (works on a fresh install)
haishui memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
haishui config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.haishui/.env
```

> `haishui honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `haishui memory setup honcho`.
...
Set `"recallSync": true` in `$HAISHUI_HOME/honcho.json` (at the root or under
`hosts.haishui`), or enable **Current-query recall** in the memory settings or
`haishui honcho setup`. An explicit host-level `false` overrides root-level `true`.
...
cycle (not per pass). Select a fast, inexpensive model under `haishui model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$HAISHUI_HOME/honcho.json` | Profile-local (isolated Haishui instances) |
| 2 | `~/.haishui/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Haishui profile: `haishui` (default) or `haishui_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `haishui honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 2 | `per-session` strategy | Haishui session ID (`20260415_a3f2b1`) |
| 3 | Manual map (`sessions` config) | `"myproject-main"` |
| 4 | Explicit `/title` command (non-automatic title) | `"refactor-auth"` |
| 5 | `per-repo` strategy | Git root directory name (`haishui-agent`) |
...
Automatically generated Haishui titles (`derived` or `llm`) are display metadata and do not override `sessionStrategy`. An explicit user title remains an intentional session-name override for non-gateway, non-`per-session` sessions.
...
If `sessionPeerPrefix` is `true`, the user peer name is prepended: `alice-haishui-agent`.
...
- **`per-session`** — Haishui session ID (timestamp + hex). Every `haishui` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Haishui profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`haishui`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Haishui profile: `haishui` (default) or `haishui_<profile>` (e.g. `haishui -p coder` -> host key `haishui_coder`). Older `haishui.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `HAISHUI_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `haishui-agent`) |
...
|---------|-------------|
| `haishui memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `haishui honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `haishui memory setup`) |
| `haishui honcho status` | Show resolved config for active profile |
| `haishui honcho enable` / `disable` | Toggle Honcho for active profile |
| `haishui honcho mode <mode>` | Change recall or observation mode |
| `haishui honcho peer --user <name>` | Update user peer name |
| `haishui honcho peer --ai <name>` | Update AI peer name |
| `haishui honcho tokens --context <N>` | Set context token budget |
| `haishui honcho tokens --dialectic <N>` | Set dialectic max chars |
| `haishui honcho map <name>` | Map current directory to a session name |
| `haishui honcho sync` | Create host blocks for all Haishui profiles |
