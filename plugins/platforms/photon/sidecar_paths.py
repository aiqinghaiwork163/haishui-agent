...
``$HAISHUI_HOME/photon/sidecar``. The mirror is refreshed by content compare; ``node_modules``
...
# Written by `haishui photon install-sidecar` on npm failure so check_requirements() can
...
def dir_writable(path: Path) -> bool:
    ...
    probe = path / ".haishui-write-probe"
    try:
        probe.touch()
        probe.unlink()
    except OSError:
...
def resolve_sidecar_dir(source_dir: Optional[Path] = None) -> Path:
    ...
    mirror = get_haishui_home() / "photon" / "sidecar"
    try:
