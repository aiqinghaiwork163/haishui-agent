...
// `message.content.text` before Haishui can see it. We rewrite the two inbound
...
const MARKER = "Haishui patch: Preserve mixed text + attachment iMessage payloads";
...
export function patchSpectrumTs(root = scriptDir()) {
  ...
  for (const file of files) {
    ...
    // Haishui to patch; keep the legacy v8 path below for older pinned installs.
    if (
