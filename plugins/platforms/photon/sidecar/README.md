...
Small Node helper that bridges Haishui Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Haishui is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Haishui-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Haishui can receive inbound messages
...
The Haishui plugin's `haishui photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Haishui is Python, so replies go through this sidecar
