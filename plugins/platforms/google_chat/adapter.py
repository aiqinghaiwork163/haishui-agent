...
def ensure_google_chat_deps() -> bool:
    ...
    ``HAISHUI_LAZY_INSTALL_TARGET`` instead of the read-only venv. Resets the
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # The profile scope this adapter was connected under (multiplex: HAISHUI_HOME override + secret
        ...
        from haishui_constants import get_haishui_home as _get_haishui_home
        self._thread_count_store = _ThreadCountStore(_get_haishui_home() / "google_chat_thread_counts.json")
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        from haishui_constants import get_haishui_home as _get_haishui_home
        return _get_haishui_home() / "google_chat_bot_id.json"
    ...
    async def _build_message_event(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> Optional[MessageEvent]:
        """Parse a Chat API message into a haishui MessageEvent."""
    ...
    async def send_clarify(
        ...
        def _button(text: str, choice: str) -> Dict[str, Any]:
            return {"text": text, "action": "haishui_clarify", "parameters": {"clarify_id": clarify_id, "choice": choice}}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Haishui is thinking…' marker (Chat has no typing API); ``send()``
        ...
        body = _thread_body(getattr(self.config, "typing_status_text", None) or "Haishui is thinking…", thread_id)
...
Walkthrough:
  ...
  3. Create a Pub/Sub topic (e.g. haishui-chat-events) and a Pull subscription.
...
def interactive_setup() -> None:
    """``haishui setup`` wizard: print GCP instructions, prompt for env vars, persist to ``~/.haishui/.env``."""
    from haishui_cli.cli_output import print_info, print_success, print_warning, prompt, prompt_yes_no
    ...
    print()
    print_success("Google Chat configuration saved to ~/.haishui/.env")
    print_info("Restart the gateway: haishui gateway restart")
...
async def _standalone_send(
    ...
    gateway runner is not in-process, e.g. ``haishui cron``). Needs SA credentials and a
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `haishui setup` to install Google Chat support.",
        ...
        platform_hint=(
            ...
            "confirmations instead. While you are generating a response, a 'Haishui is thinking…' marker message "
