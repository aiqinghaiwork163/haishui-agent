...
def _make_activity_event(*, hook_event_name: str, session_id: Any, status: str = "ok", tool_name: Any = None,
    ...
    event: Dict[str, Any] = {"schema": ACTIVITY_EVENT_SCHEMA, "eventId": f"haishui-{uuid.uuid4()}",
...
class RaftAdapter(BasePlatformAdapter):
    ...
    async def _handle_wake(self, request: "web.Request") -> "web.Response":
        ...
        # No payload["schema"] gate: the bridge owns schema evolution; Haishui only checks content-free.
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        """Accept Raft wake hints without interrupting an active Haishui turn."""
        if event.internal:
...
def interactive_setup() -> None:
    """``haishui gateway setup`` flow: persists ``RAFT_PROFILE`` to the Haishui env file.
    CLI helpers are lazy-imported so the plugin stays importable in gateway runtime and tests."""
    from haishui_cli.cli_output import print_header, print_info, print_success, print_warning, prompt
    ...
    if declines_reconfigure("Raft", "Reconfigure Raft?", "RAFT_PROFILE"):
        ...
        return
    for line in ("Connect Haishui to Raft as an external agent.", "Create the External Agent in Raft first, then run:",
