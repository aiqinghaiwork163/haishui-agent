...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        path is rechecked against allowed roots (tempdir, ``/tmp``→``/private/tmp`` on macOS, HAISHUI_HOME).
        ...
        try:
            ...
            haishui_home = Path(get_haishui_home()).resolve()
        except Exception:
            haishui_home = Path.home().joinpath(".haishui").resolve()
        resolved = path.resolve()
        if not any(resolved.is_relative_to(r) for r in (Path(tempfile.gettempdir()).resolve(), Path("/tmp").resolve(), haishui_home)):
...
def is_connected(config) -> bool:
    """Surface in ``haishui status`` even before the adapter is instantiated."""
    return validate_config(config)


def _env_enablement() -> Optional[Dict[str, Any]]:
    """``env_enablement_fn``: seed ``PlatformConfig.extra`` from env-only setups so ``haishui status`` sees them."""
    if not _env_credentials_present():
        return None
...
def interactive_setup() -> None:
    """``haishui setup line`` wizard (writes ``~/.haishui/.env``); CLI helpers are lazy-imported."""
    from haishui_cli.config import get_env_value, save_env_value
    from haishui_cli.cli_output import print_header, print_info, prompt
