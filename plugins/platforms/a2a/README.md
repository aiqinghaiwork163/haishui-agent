# A2A — Agent-to-Agent protocol for Haishui
...
peer (another Haishui, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
haishui gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Haishui serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.haishui/a2a_audit.jsonl`.
- Conversations persist to `~/.haishui/a2a_conversations/` — they survive context
