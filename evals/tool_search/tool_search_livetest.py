#!/usr/bin/env python3
"""Live test harness for Haishui Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any haishui imports.
ORIGINAL_HOME = os.environ.get("HAISHUI_HOME")
ORIGINAL_AUTH = Path.home() / ".haishui" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.haishui/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.haishui/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="haishui_ts_live_"))
    haishui_home = home_dir / ".haishui"
    haishui_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, haishui_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".haishui" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, haishui_home / ".env")
        ...
        load_haishui_dotenv(haishui_home=str(Path.home() / ".haishui"))
    ...
    (haishui_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return haishui_home
...
def reset_module_state():
    """Drop cached modules so the new HAISHUI_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "haishui_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def main():
    ...
    # Restore original HAISHUI_HOME
    if ORIGINAL_HOME is not None:
        os.environ["HAISHUI_HOME"] = ORIGINAL_HOME
    else:
