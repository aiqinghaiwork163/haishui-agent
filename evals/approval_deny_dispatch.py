...
with tempfile.TemporaryDirectory(prefix="haishui-deny-dispatch-") as directory:
    home = Path(directory)
    os.environ.clear()
    # The terminal's own NOPASSWD probe must also resolve to our inert fixture.
    os.environ.update(PATH=directory + ":/usr/bin:/bin", HOME=directory, HAISHUI_HOME=directory,
                      HAISHUI_INTERACTIVE="1", TERMINAL_ENV="local")
