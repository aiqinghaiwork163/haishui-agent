...
os.environ.clear()
os.environ.update(
    HOME=home, HAISHUI_HOME=home + "/.haishui", PATH="/usr/bin:/bin", NO_PROXY="*"
)
os.chdir(home)
...
assert Path(os.environ["HAISHUI_HOME"]).is_relative_to(Path(os.environ["HOME"]))
...
Path(os.environ["HAISHUI_HOME"]).mkdir(parents=True, exist_ok=True)
# JSON is valid YAML, allowing the real config loader to read the seeded file.
(Path(os.environ["HAISHUI_HOME"]) / "config.yaml").write_text(json.dumps(config))
from run_agent import AIAgent
from providers import get_provider_profile
from haishui_cli.config import get_compatible_custom_providers, load_config
from haishui_cli.config_providers import get_custom_provider_context_length
    ...
    json.dumps(
        {
            ...
            "haishui_home": os.environ["HAISHUI_HOME"],
            "captures": captures,
            "rows": rows,
