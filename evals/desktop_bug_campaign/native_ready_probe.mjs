...
const home = mkdtempSync(join(tmpdir(), 'haishui-native-ready-'))
const token = randomUUID()
const env = {
  PATH: process.env.PATH, HOME: home, HAISHUI_HOME: home,
  LANG: 'C.UTF-8', PYTHONUNBUFFERED: '1', HAISHUI_NONINTERACTIVE: '1',
  HAISHUI_DASHBOARD_SESSION_TOKEN: token, HAISHUI_SERVE_HEADLESS: '1',
  HAISHUI_PARENT_PID: String(process.pid),
}
const child = spawn(process.argv[2] || join(repo, '.venv/bin/python'),
  ['-m', 'haishui_cli.main', 'serve', '--host', '127.0.0.1', '--port', '0', '--isolated'],
...
try {
  ...
  for (let i = 0; i < 20; i++) {
    for (const path of ['/api/status', '/api/profiles']) {
      const response = await fetch(`http://127.0.0.1:${port}${path}`, { headers: { 'X-Haishui-Session-Token': token } })
