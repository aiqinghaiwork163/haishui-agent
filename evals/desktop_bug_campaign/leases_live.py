...
def backend(port):
    from haishui_cli.web_server import app, start_server
...
''')
    ...
    (home / 'user').mkdir()
    env.update(HAISHUI_HOME=str(home), HAISHUI_DASHBOARD_SESSION_TOKEN='lease-probe',
    ...
    def control(port, action, sid):
        req = urllib.request.Request(f'http://127.0.0.1:{port}/lease-probe/{action}/{sid}', data=b'', method='POST', headers={'X-Haishui-Session-Token': 'lease-probe'})
