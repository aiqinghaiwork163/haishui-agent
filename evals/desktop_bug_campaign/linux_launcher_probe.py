...
def main():
    ...
    env = {"PATH": "/usr/bin:/bin", "HOME": str(root / "home"),
           "HAISHUI_HOME": str(root / "home/.haishui"),
           "XDG_DATA_HOME": str(root / "xdg"), "LANG": "C.UTF-8",
           "HAISHUI_NONINTERACTIVE": "1"}
    Path(env["HAISHUI_HOME"]).mkdir(parents=True)
    venv_dir = root / "venv"
    ...
    install = (
        ...
        "from haishui_cli.linux_desktop_entry import install_desktop_entry; "
        f"sys.argv[0]={str(repo / 'haishui')!r}; "
    ...
    rows["venv_import"] = run([str(python), "-I", "-c", "import yaml,haishui_cli.main; print(yaml.__version__)"], env, "/")
    rows["base_import_negative"] = run([str(python.resolve()), "-I", "-c", "import yaml,haishui_cli.main"], env, "/")
    ...
    custom = original.replace(b"Name=Haishui\n", b"Name=Haishui custom\n").replace(b"Terminal=false", b"Terminal=true")
    entry.write_bytes(custom)
    config = Path(env["HAISHUI_HOME"]) / "config.yaml"
