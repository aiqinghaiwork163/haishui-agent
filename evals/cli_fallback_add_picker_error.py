"""Drive the real ``haishui fallback add`` under a Linux PTY into an ordinary picker error.
...
def _persisted_model(root: Path, env: dict) -> dict:
    ...
    out = subprocess.run(
        [sys.executable, "-c", "import json; from haishui_cli.config import load_config; "
...
def run(root: Path, output: Path) -> dict:
    with tempfile.TemporaryDirectory(prefix="haishui_test_fallback_") as home:
        hh = Path(home) / ".haishui"
        hh.mkdir()
        ...
        env = {"PATH": os.environ["PATH"], "HOME": home, "HAISHUI_HOME": str(hh),
        ...
        proc = subprocess.Popen([sys.executable, "-m", "haishui_cli.main", "fallback", "add"],
