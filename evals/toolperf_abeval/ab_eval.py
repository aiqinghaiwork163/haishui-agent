...
measured in the production session DB) through `haishui chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    HAISHUI_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured Haishui home with credentials for the
...
(tracker: aiqinghaiwork163/haishui-agent#77056).
"""
import json
import os
import re
...
""".strip(),
            ...
            env.update({
                ...
                "HAISHUI_NEMO_RELAY_PLUGINS_TOML": str(relay_config),
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "haishui_cli.main", "chat", "--query", q,
