...
test.beforeAll(async () => {
  ...
  env = buildAppEnv(sandbox, { HOME: sandbox.root, HAISHUI_DESKTOP_PYTHON: python,
    ...
    PYTHONPATH: repo, HAISHUI_SINGLE_QUERY_LINGER_SECONDS: '30' })
  for (const name of ['alpha', 'beta', 'gamma']) {
    const h = path.join(sandbox.haishuiHome, 'profiles', name)
    execFileSync(python, ['-c', 'import sys; from pathlib import Path; from haishui_state import SessionDB; d=SessionDB(db_path=Path(sys.argv[1])/"state.db"); d.create_session("matrix-"+sys.argv[2],"cli",cwd=sys.argv[3]); d.set_session_title("matrix-"+sys.argv[2],"Bot Chat"); d.close()', h, name, sandbox.root], { env, cwd: repo })
  }
...
async function openBot(name: string) {
  ...
  const row = page.getByRole('button', { name: new RegExp(`^${name === 'default' ? 'haishui' : name}\\b`, 'i') }).filter({ visible: true }).first()
...
function dbMessages(name: string) {
  const home = name === 'default' ? fixture.sandbox.haishuiHome : path.join(fixture.sandbox.haishuiHome, 'profiles', name)
...
test('incoming live sender card is rendered before any reload', async () => {
  ...
  const child = spawn(python, ['-m', 'haishui_cli.main', '-p', 'beta', 'chat', '--in', '~', '-c', 'Bot Chat', '--create-if-missing', '-Q', '-q', 'E2E_DM(alpha)[matrix-live-sender-card]'], { cwd: repo, env, stdio: ['ignore', output, output] })
  try {
