...
def main():
    ...
    home = Path(tempfile.mkdtemp(prefix='haishui-acp-empty-'))
    haishui = home / '.haishui'
    haishui.mkdir()
    (haishui / 'config.yaml').write_text(
    ...
    env.update(HOME=str(home), HAISHUI_HOME=str(haishui), PYTHONPATH=os.pathsep.join([str(repo), os.environ.get('PYTHONPATH', '')]),
               HAISHUI_ACP_SKIP_CONFIGURED_MCP='1', OPENAI_API_KEY='local-fixture-key',
    ...
    def rows():
        with sqlite3.connect(haishui / 'state.db') as db:
