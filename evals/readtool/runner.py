"""Run the read-tool eval through the REAL Haishui AIAgent.

For each task: fresh temp HAISHUI_HOME, fresh fixture workspace, real
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    haishui_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".haishui"
    ...
    try:
        ...
        shutil.rmtree(haishui_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.haishui/.env; set +a  — then relaunch."
        )
