...
def run_probe(repo, receipt):
    ...
    owner = Path(os.environ["HAISHUI_HOME"])
    ...
    with serving(Provider()) as provider, serving(CallbackServer(
        ...
        reloaded = asyncio.run(HaishuiTokenStorage("positive", haishui_home=owner).get_tokens())
        ...
        override = set_haishui_home_override(other)
        try:
...
def main():
    ...
    if args.cold_probe:
        sys.path.insert(0, str(repo))
        logging.disable(logging.CRITICAL)
        from haishui_cli.mcp_config import _get_mcp_servers, _probe_single_server
    ...
    if not args.isolated_worker:
        with tempfile.TemporaryDirectory(prefix="haishui-oauth-http-") as temp:
            home = Path(temp)
            (home / ".haishui").mkdir()
            env = {"PATH": os.environ.get("PATH", ""), "HOME": temp,
                   "HAISHUI_HOME": str(home / ".haishui"), "LANG": "C.UTF-8", "TZ": "UTC",
                   "PYTHONNOUSERSITE": "1"}
