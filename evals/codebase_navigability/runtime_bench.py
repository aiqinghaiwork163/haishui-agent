...
Each probe runs in a FRESH subprocess with an isolated HAISHUI_HOME so nothing is cached across reps.
...
ENV = {**os.environ, "HAISHUI_HOME": HOME, "PYTHONPATH": TREE, "PYTHONDONTWRITEBYTECODE": "0", "OPENROUTER_API_KEY": "sk-bench-placeholder",
       "HAISHUI_SKIP_UPDATE_CHECK": "1", "NO_COLOR": "1", "TERM": "dumb", "COLUMNS": "120"}
for k in list(ENV):
    if k.startswith(("HAISHUI_SESSION", "HAISHUI_PROFILE")): ENV.pop(k)
...
IMPORT_TARGETS = ["run_agent", "cli", "haishui_cli.main", "gateway.run", "tools.registry", "haishui_state", "tui_gateway.server", "haishui_cli.web_server", "model_tools", "agent.prompt_builder"]
probe = r'''
...
tree=os.getcwd(); foreign=[m for m,mod in list(sys.modules.items()) if getattr(mod,"__file__",None) and "haishui-agent" in mod.__file__ and not mod.__file__.startswith(tree) and "site-packages" not in mod.__file__]
...
# 2. CLI end-to-end startup: `haishui --version`, `haishui --help`, `haishui doctor --help`, `haishui config get model` (no network)
CLI = {"version": ["haishui_cli/main.py", "--version"], "help": ["haishui_cli/main.py", "--help"], "config_get": ["haishui_cli/main.py", "config", "get", "model"], "tools_list": ["haishui_cli/main.py", "tools", "--help"], "skills_help": ["haishui_cli/main.py", "skills", "--help"]}
cli = {}
...
try:
    ...
    T("resolve_toolset_haishui_default_ms", lambda: resolve_toolset("haishui-default"), 200)
...
try:
    import haishui_state, tempfile, uuid
    from pathlib import Path
    db=haishui_state.SessionDB(Path(tempfile.mkdtemp())/"s.db") if hasattr(haishui_state,"SessionDB") else None
    if db:
        sid=str(uuid.uuid4())
