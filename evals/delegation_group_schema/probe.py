...
with tempfile.TemporaryDirectory(prefix="delegate-schema-") as home:
    os.environ.clear()
    os.environ.update(HOME=home, HAISHUI_HOME=home, PATH="/usr/bin:/bin", HAISHUI_PLATFORM="cli")
    ...
    definitions = get_tool_definitions(enabled_toolsets=["haishui-cli"], quiet_mode=True)
    serialized = json.dumps(definitions, separators=(",", ":"))
    repeated = get_tool_definitions(enabled_toolsets=["haishui-cli"], quiet_mode=True)
