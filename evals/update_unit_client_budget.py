...
os.environ.clear()
os.environ.update(allowed, HOME=str(home), HAISHUI_HOME=str(home / "haishui"))
...
def scoped_systemctl(argv, *, timeout):
    if "list-units" in argv:
        ...
        argv = [arg for arg in argv if arg not in ("haishui-gateway*", "haishui-serve*")] + [unit + ".service"]
...
unit = f"haishui-serve-audit-089c35aa-{tag}"
cmd = ["systemctl", "--user"]
def run(args):
