...
Run with the Haishui venv Python and --repo CHECKOUT --output RECEIPT_DIR.
No user environment/config is inherited by Haishui children. Unix PTY required.
"""
import argparse
import http.server
import json
...
def cli_pty(repo, env):
    ...
    proc = subprocess.Popen([sys.executable, "-m", "haishui_cli.main", "model"],
    ...
    try:
        for schema in ("providers", "custom_providers"):
            for surface in ("cli", "rows"):
                ...
                with tempfile.TemporaryDirectory(prefix="haishui-keycmd-live-") as tmp:
                    home = Path(tmp)
                    state = home / ".haishui"
                    state.mkdir()
                    ...
                    env = {"HOME": str(home), "HAISHUI_HOME": str(state),
