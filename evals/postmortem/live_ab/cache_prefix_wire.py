...
message prefix between call N and N+1. If Haishui strips prior-turn thinking, call N+1's messages[:k]
...
#   python cache_prefix_wire.py <repo_root> <A|B> [--haishui-home DIR]   (default HAISHUI_HOME: the real one, for credentials)
sys.path.insert(0, sys.argv[1])
os.environ.setdefault("HAISHUI_HOME", os.path.expanduser("~/.haishui"))
if "--haishui-home" in sys.argv:
    os.environ["HAISHUI_HOME"] = sys.argv[sys.argv.index("--haishui-home") + 1]
...
time.sleep(1)
log = subprocess.run(f"grep -h '\\[{sid}\\]' ~/.haishui/logs/agent.log | grep 'API call #'", shell=True, capture_output=True).stdout.decode("utf-8", "replace")
