...
- **`forensics/`** reads a *copy* of a Haishui `state.db` (plus rotated `agent.log*` and git) and
...
- A Haishui checkout with its venv (`.venv/bin/python`). NeMo Relay is not required for the forensics;
  it is what the run itself used for the wire captures in `live_ab/cache_prefix_wire.py`.
- For forensics: a **copy** of `~/.haishui/state.db` (never point at the live file; `sqlite3 state.db ".backup copy.db"`
  or `cp` while Haishui is idle) and, optionally, the rotated `~/.haishui/logs/agent.log*`.
- For `--live` probes: real credentials in `HAISHUI_HOME` (they spend cents per run).
...
$P -m evals.postmortem.forensics.logcalls   --db state_copy.db --logs "$HOME/.haishui/logs/agent.log*"
...
Haishui recorded (an estimator, not an invoice).
...
```bash
# offline probes (fake providers, temp HAISHUI_HOME), main vs a branch or integration checkout:
...
Forensic lanes: five parallel Haishui subagents (2026-09-04), rewritten here to take `--db`/`--root`
