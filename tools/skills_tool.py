...
HAISHUI_HOME = get_haishui_home()  # all skills live in ~/.haishui/skills/ (seeded from bundled)
...
def _skills_dir() -> Path:
    ...
    it, else live profile-scoped HAISHUI_HOME (long-lived runtimes may import before profile set)."""
    configured = Path(SKILLS_DIR)
    return configured if configured != _SKILLS_DIR_AT_IMPORT else get_haishui_home() / "skills"
...
def load_env() -> Dict[str, str]:
    """Snapshot of HAISHUI_HOME/.env for the post-skill secret-capture diff (same tokenizer that
    installs the profile scope, so a captured value never differs from the served one)."""
    from agent.secret_scope import load_env_file

    return load_env_file(get_haishui_home() / ".env")
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    """``~/.haishui/skills/mlops/axolotl/SKILL.md`` -> ``"mlops"``; active profile dir first
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    """Disabled in config? Platform precedence: explicit arg, ``HAISHUI_PLATFORM``, session
    ``HAISHUI_SESSION_PLATFORM``. A globally-disabled skill stays disabled on every platform
    ...
    try:
        ...
        resolved_platform = platform or os.getenv("HAISHUI_PLATFORM")
        if not resolved_platform:
            with suppress(Exception):
                from gateway.session_context import get_session_env
                resolved_platform = get_session_env("HAISHUI_SESSION_PLATFORM") or ""
...
def _org_provenance_header(skill_dir: Path, active_skills_dir: Path):
    ...
    header = (
        ...
        "> `haishui sync propose` (or automatically, if your org enables it).\n\n")
    ...
    if skill_md is not None and project_dirs:
        ...
        if _under_any(skill_md, project_dirs) and is_quarantined_project_skill(skill_md):
            return _fail(
                ...
                "`haishui skills untrust`."), None, None
...
def _log_security_warnings(name: str, skill_md: Path, content: str, all_dirs, active_skills_dir):
    ...
    if not _under_any(skill_md, trusted_dirs):
        warnings.append(f"skill file is outside the trusted skills directory (~/.haishui/skills/): {skill_md}")
...
def skill_view(
    ...
    try:
        ...
        if _is_skill_disabled(resolved_name):
            return _fail(f"Skill '{resolved_name}' is disabled. Enable it with `haishui skills` or inspect the files directly on disk.")
        if file_path and skill_dir:
            return _serve_skill_file(
                skill_dir, file_path, name, list_available=True, mark_read=True,
                hint="Use a relative path within the skill directory")
        # tags/related_skills: metadata.haishui.* (agentskills.io) first, then top-level.
        metadata = frontmatter.get("metadata")
        haishui_meta = (metadata.get("haishui", {}) or {}) if isinstance(metadata, dict) else {}
        tags, related_skills = (
            _parse_tags(haishui_meta.get(k) or frontmatter.get(k, "")) for k in ("tags", "related_skills"))
