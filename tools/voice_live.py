"""GPT-Live voice chat mode: the full-duplex voice frontend that delegates to Haishui.
...
delegates every real request to Haishui as its *client-delegation* backend. Haishui stays the
...
* the renderer turns each ``session.delegation.created`` into a normal ``prompt.submit`` on the
  ...
  ``session.commentary.append`` — Haishui' answer is what the voice speaks.
...
# backend (Haishui) carries the real instructions, tools and memory.
LIVE_PERSONA = (
    "You are Haishui, a calm and friendly voice assistant. Speak naturally at an unhurried pace. "
    ...
    "Delegation policy:\n"
    "Backend tools:\n"
    "- Haishui agent: a full AI agent with tools — it can run commands, read and edit files, "
