...
_HAISHUI_CONFIG_FILES = r'\.haishui/(?:config\.yaml|SOUL\.md)'
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.haishui/\.env|\~/\.haishui/\.env',
     "haishui_env_access", "critical", "exfiltration", "directly references Haishui secrets file"),
    ...
    # invocation: any client plugin that relays Haishui' secure prompts has to name them, and a bare
    ...
    # quarantine only acts on "dangerous") but high for Haishui/other config (setup docs routinely say
    ...
    (_prose_modify_re(_HAISHUI_CONFIG_FILES),
     "haishui_config_mod", "high", "persistence", "modification language aimed at Haishui configuration files (verify intent)"),
    (_shell_write_re(_HAISHUI_CONFIG_FILES),
     "haishui_config_mod_shell", "critical", "persistence", "shell write (redirect/sed -i/tee/cp/mv) targeting Haishui configuration files"),
    (r'\.haishui/config\.yaml|\.haishui/SOUL\.md',
     "haishui_config_ref", "low", "persistence", "references Haishui configuration files (informational; only modification intent is scored)"),
...
# `.skillignore` is Haishui-native; `.clawhubignore` is honored for skills published through ClawHub.
