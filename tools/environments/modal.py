...
def _snapshot_store() -> Path:
    ...
    return get_haishui_home() / "modal_snapshots.json"
...
class ModalEnvironment(BaseEnvironment):
    ...
    def __init__(self, image: str, cwd: str = "/root", timeout: int = 60,
        ...
        def _create(image_spec: Any) -> None:
            async def _create_sandbox():
                app = await _modal.App.lookup.aio("haishui-agent", create_if_missing=True)
    ...
    def _modal_bulk_download(self, dest: Path) -> None:
        """Download remote .haishui/ as a tar archive (sandboxes run as root, so /root/.haishui)."""
        # --exclude: live sockets cannot be archived ("socket ignored") and must not fail the download.
        data = self._exec("tar cf - --exclude='*.sock' -C / root/.haishui", timeout=120, fail_label="bulk download", capture=True)
