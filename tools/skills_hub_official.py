"""Skills Hub official sources: repo-shipped optional skills and the centralized Haishui index."""
...
from tools.skills_hub_models import (
    SkillBundle, SkillMeta, SkillSource, _haishui_tags, _matches_query, _memo_json, _parse_frontmatter, hub,
)
...
class OptionalSkillSource(SkillSource):
    ...
    activated by default — absent from the system prompt and not copied to ~/.haishui/skills/ at
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        # (metadata.haishui.upstream); install pulls the live content from there.
        try:
    ...
    def _fetch_from_live_repo(self, rel: str) -> Optional[SkillBundle]:
        ...
        ``main``; rather than demanding ``haishui update`` first, resolve against the live repo.
    ...
    def _upstream_pointer_from_content(self, content: Union[str, bytes]) -> Optional[Dict[str, str]]:
        """Parse ``metadata.haishui.upstream: {repo: owner/name, path: ...}`` out of SKILL.md content
        ...
        haishui_meta = meta_block.get("haishui") if isinstance(meta_block, dict) else None
        upstream = haishui_meta.get("upstream") if isinstance(haishui_meta, dict) else None
...
class HaishuiIndexSource(SkillSource):
    """Skill source backed by the centralized Haishui Skills Index: a JSON catalog on the docs site,
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            from tools.skills_hub_search import _load_haishui_index
            self._index, self._loaded = _load_haishui_index(), True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        for github_id in filter(None, candidates):
            ...
            if bundle:
                bundle.source = entry.get("source", "haishui-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            ...
            source=entry.get("source", "haishui-index"), identifier=entry.get("identifier", ""),
