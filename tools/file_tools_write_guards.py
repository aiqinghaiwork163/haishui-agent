...
# resolve per call in production (below) because ``get_haishui_home()`` /
...
_haishui_config_resolved: str | None = None
_haishui_config_resolved_loaded = False
_real_haishui_home_cached: str | None = None
_real_haishui_home_loaded = False
...
def _haishui_home_real() -> str:
    from haishui_constants import get_haishui_home
    return os.path.realpath(str(get_haishui_home()))


def _get_haishui_config_resolved() -> str | None:
    """Resolved absolute path of the Haishui config file for the ACTIVE profile.

    Resolved per call so it tracks the per-turn ``HAISHUI_HOME`` scope (#107327);
    a test may pin it via ``_haishui_config_resolved`` + ``_haishui_config_resolved_loaded``."""
    if _haishui_config_resolved_loaded:
        return _haishui_config_resolved
    try:
        ...
        # home rather than the active multiplex ``HAISHUI_HOME`` — comparing
        ...
        # ``get_haishui_home()`` key the happy path uses, and substitute no
        # unrelated home if even that is gone (#107327 follow-up; PR #107335).
        try:
            from haishui_constants import get_haishui_home
            return str((Path(str(get_haishui_home())) / "config.yaml").resolve())
        except Exception:
            return None


def _get_real_haishui_home() -> str | None:
    """Realpath of the authoritative Haishui home for the ACTIVE profile.

    Resolved per call so it tracks the per-turn ``HAISHUI_HOME`` scope (#107327);
    a test may pin it via ``_real_haishui_home_cached`` + ``_real_haishui_home_loaded``.
    Consumers exempting a whole TREE want ``_haishui_exempt_homes()``: under a named
    profile this home is ``<root>/profiles/<name>`` and the root is exempt too."""
    if _real_haishui_home_loaded:
        return _real_haishui_home_cached
    try:
        return _haishui_home_real()
    except Exception:
        # Same active-profile binding on the exception path (see
        # ``_get_haishui_config_resolved``): re-derive from ``get_haishui_home()``
        # rather than ``_expand_tilde("~/.haishui")`` so the protected-instruction
        ...
        # the consumer: the ``~/.haishui`` exemption is skipped, so the gate runs.
        try:
            from haishui_constants import get_haishui_home
            return os.path.realpath(str(get_haishui_home()))
        except Exception:
...
def _haishui_exempt_homes() -> tuple[str, ...]:
    """Realpaths of the Haishui home tree(s) the protected-instruction gate must stay out of:
    the ACTIVE profile's home, plus the Haishui ROOT when that home is a named profile
    (``<root>/profiles/<name>``). Exempting only the profile dir left the root's DIRECT files
    (LEDGER.md / MEMORY.md / SOUL.md / AGENTS.md ...) to the ``.haishui`` component rule, which
    gated them like a project-local ``<repo>/.haishui/config.yaml`` — fail-closed headless
    (#110630). They are the agent's own store, governed by their own guards, exactly like
    ``~/.haishui`` under the default profile. The root is added only when the shape really is a
    named profile (``named_profile_home``), so a coincidental ``profiles/`` dir elsewhere never
    exempts its parent; the home comes from the ACTIVE scope, never ``HAISHUI_HOME`` alone."""
    home = _get_real_haishui_home()
    if not home:
        return ()
    try:
        from haishui_constants import named_profile_home
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    haishui_config = _get_haishui_config_resolved()
    if haishui_config and haishui_config in candidates:
        return (
            f"Refusing to write to Haishui config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.haishui/config.yaml directly or use 'haishui config' instead.")
    return None
...
# vector (AGENTS.md / CLAUDE.md / SOUL.md / .cursorrules / project .haishui tree).
...
def _protected_instruction_config() -> tuple[bool, list[str]]:
    ...
    try:
        from haishui_cli.config import load_config, cfg_get
        cfg = load_config()
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    # ~/.haishui itself is governed by its own guards (config.yaml hard-block,
    # mirror guard, write_approval); this gate targets PROJECT-LOCAL files only.
    # Must run before the ``.haishui`` component rule, which would match the home.
    # ``_haishui_exempt_homes`` also covers the ROOT when the active home is a named
    # profile, so ~/.haishui/<file> cannot read as project-local ``.haishui`` config.
    for real_home in _haishui_exempt_homes():
    ...
    for candidate in (normalized, resolved):
        ...
        if base_lower in _PROTECTED_INSTRUCTION_BASENAMES or any(
            ...
            return base
        # Project-local .haishui config dirs (<repo>/.haishui/config.yaml) steer
        # behavior too. Only the IMMEDIATE parent counts — matching any ancestor
        # would gate every write inside a checkout living under ~/.haishui.
        parts = candidate.replace("\\", "/").rstrip("/").split("/")
        if len(parts) >= 2 and parts[-2] == ".haishui":
            return candidate
    return None
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Haishui mirror prefix for persistent Docker file tools."""
    try:
        ...
        if env is not None:
            ...
            return "/root/.haishui" if persistent_docker else None
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    Haishui state (a write the host never reads). Not profile isolation — that guard was
