...
is ALWAYS host-side ``$HAISHUI_HOME/cache/spillover/{id}.txt`` (works for sessions that never
...
STORAGE_DIR = "/tmp/haishui-results"
...
def get_spillover_dir():
    """Return $HAISHUI_HOME/cache/spillover as a Path (not created)."""
    from haishui_constants import get_haishui_home
    return get_haishui_home() / SPILLOVER_SUBDIR
...
def _write_to_spillover(content: str, filename: str):
    """Write host-side to $HAISHUI_HOME/cache/spillover; returns path str or None.
...
def _resolve_storage_dir(env) -> str:
    ...
    return f"{temp_dir.rstrip('/') or '/'}/haishui-results" if temp_dir else STORAGE_DIR
...
HEREDOC_MARKER = "HAISHUI_PERSIST_EOF"
# ---- END PLUGIN-COMPAT ----
