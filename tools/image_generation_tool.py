#!/usr/bin/env python3
"""Image generation via FAL.ai (model picked in ``haishui tools``, persisted to ``image_gen.model``).
...
def _resolve_managed_fal_gateway():
    """Managed gateway config for the stored `haishui tools` selection, or ``None`` for direct FAL.
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            raise ValueError(
                ...
                f"  • Pick a different model via `haishui tools` → Image Generation."
...
_CACHE_BASE_BY_BACKEND = {"docker": "/root/.haishui", "singularity": "/root/.haishui",
                          "modal": "/root/.haishui", "ssh": "~/.haishui"}
...
def _agent_cache_base_for_env(env: Any) -> str | None:
    if env is not None:
        ...
        if remote_home:
            return f"{str(remote_home).rstrip('/')}/.haishui"
...
def _prepare_fal_request(model_id, meta, prompt, aspect_ratio, seed, overrides, source_images):
    ...
    if source_images and not edit_endpoint:
        raise ValueError(
            ...
            f"via `haishui tools` → Image Generation.")
...
def _build_no_backend_setup_message() -> str:
    ...
    if managed:
        lines.append("  2. Sign in to a Nous account that has the managed FAL gateway enabled "
                     "(`haishui setup`)")
    lines.append("  3. Configure a different image_gen provider via `haishui tools` → Image Generation "
                 "(run `haishui plugins list` to see installed backends)")
    return "\n".join(lines)
...
def _dispatch_to_plugin_provider(
    ...
    if provider is None:
        return _provider_error(
            ...
            f"Run `haishui plugins list` to see available image gen backends.", "provider_not_registered")
    ...
    try:
        ...
        if is_type_error and ("image_url" in kwargs or "reference_image_urls" in kwargs):
            ...
            return _provider_error(
                ...
                f"text-to-image, or pick a backend that supports editing via `haishui tools` → "
...
def _maybe_route_managed_model(
    ...
    if provider is None:
        ...
        return _provider_error(
            ...
            f"available. Pick another model via `haishui tools` → Image Generation.", "provider_not_registered")
