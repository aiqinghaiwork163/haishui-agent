...
# match haishui_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def _runtime_provider_credentials(v: dict, explicit_request_overrides) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'haishui auth'."
        )
...
def _load_config() -> dict:
    ...
    (follows HAISHUI_HOME/profile; no deepcopy, since this runs on every get_definitions() rebuild) over the legacy
    ``cli.CLI_CONFIG``, which can hide user-set keys — except that ``HAISHUI_IGNORE_USER_CONFIG=1`` is only honored
    by the legacy loader, so it stays authoritative when that flag is set."""
    if os.environ.get("HAISHUI_IGNORE_USER_CONFIG") != "1":
        try:
...
def _resolve_child_runtime(
    ...
    # would pin a child Haishui/Qwen subagent onto the parent's Claude Messages wire (or the reverse).
