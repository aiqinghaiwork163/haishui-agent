...
When any profile carries ``ui_meta['haishui-bots']`` in profile.yaml (Bot-Mode-managed),
...
def _default_home() -> str:
    """Ambient process HAISHUI_HOME (env, else the platform default) as a string."""
...
def _haishui_root(home: Path) -> Path:
    """Root ~/.haishui for both the default profile and named profiles."""
...
def _handle(name: str) -> str:
    # The mention middleware aliases the default profile as @haishui.
    return "haishui" if name == "default" else name
...
def _roster(root: Path) -> list[tuple[str, Path]]:
    ...
    teammates (#99392)."""
    from haishui_constants import named_profile_is_live
...
def _bots_meta(data: dict | None) -> dict | None:
    """The ``ui_meta['haishui-bots']`` block of a parsed profile.yaml, if a dict."""
    ui_meta = data.get("ui_meta") if data else None
    bots = ui_meta.get("haishui-bots") if isinstance(ui_meta, dict) else None
    return bots if isinstance(bots, dict) else None


def _is_bot_managed(profile_dir: Path) -> bool:
    return _bots_meta(_read_yaml_dict(profile_dir / "profile.yaml", "haishui-bots")) is not None
...
def is_bot_mode_managed(home: str | os.PathLike | None = None) -> bool:
    ...
    return _swallow(lambda: _any_managed(_haishui_root(_resolve_home(home))), False)
...
def _profile_role(profile_dir: Path) -> str:
    ...
    WHO to message for a job. A friendly ``display_name`` (``haishui profile rename``) that
...
def _display_name(name: str, profile_dir: Path) -> str:
    ...
    then profile.yaml ``display_name`` (``haishui profile rename``), else the @handle — the
    renamed primary signs as ``Maia (@haishui)``, not ``haishui (@haishui)`` (#89720)."""
    return next((n for n in _friendly_names(profile_dir) if n), None) or _handle(name)


# Tokens the Desktop mention parser reserves; a bot titled "Haishui" never hijacks @haishui.
_RESERVED_ALIASES = frozenset({"all", "everyone", "user", "default", "haishui"})
...
def _peers(root: Path) -> list[str]:
    """Registered peer gateway names (``haishui peer``) from config.yaml, read
...
def _peer_paragraph(root: Path) -> str:
    ...
    return (
        ...
        "for the peer's main agent). Run `haishui peer list` for the live "
        "peer list."
    )


def _build_section(home: Path) -> str:
    root = _haishui_root(home)
    ...
    return (
        f"{_PROTOCOL_HEADING}\n"
        "This install runs Bot Mode: each Haishui profile is an agent teammate with "
...
def get_bot_mode_protocol_section(home: str | os.PathLike | None = None, *, force_refresh: bool = False) -> str:
    ...
    the AGENT'S OWN resolved home (session-db derived), not ambient HAISHUI_HOME — build threads
...
def capability_fingerprint(home: str | os.PathLike | None = None) -> str:
    ...
    try:
        ...
        token = set_haishui_home_override(str(resolved))
        try:
