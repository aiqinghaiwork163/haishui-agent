...
processes survive cleanup, idle reaping, or Haishui exit.
...
# display_haishui_home imported lazily at call site (stale-module safety during haishui update)
...
def _current_session_key() -> str:
    ...
    return get_session_env("HAISHUI_SESSION_KEY", "")


def _current_session_profile() -> str:
    """Active session's Haishui profile name, or "" (same lookup discipline as
    :func:`_current_session_key`)."""
    from gateway.session_context import get_session_env

    return get_session_env("HAISHUI_SESSION_PROFILE", "")
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps Exited containers labeled ``haishui-agent=1`` for the current
    profile — leftovers of Haishui processes that died without firing
    ...
    Haishui processes sharing a profile) and the once-per-interpreter flag so
...
def _ensure_terminal_env_bridged() -> None:
    ...
    at startup; processes that skip those paths (``haishui serve``, Desktop
    ...
    env values (possibly stale from ``haishui setup``); env values for omitted keys
    ...
    context-local ``HAISHUI_HOME`` override (multiplexed dashboard / gateway
    ...
    with _quiet("terminal config → env fallback bridge failed"):
        from haishui_cli.config import apply_terminal_config_to_env, read_raw_config
...
def _resolve_config_cwd(env_type: str, mount_docker_cwd: bool) -> tuple:
    ...
    from haishui_cli.config import _is_ssh_remote_tilde_cwd
...
def terminal_tool(
    ...
    ``_host_local`` forces the local backend for Haishui-owned control-plane
