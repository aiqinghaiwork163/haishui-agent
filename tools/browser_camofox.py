...
``~/.haishui/.env`` (Docker: see ``CAMOFOX_REWRITE_LOOPBACK_URLS`` below).
"""
...
from haishui_cli.config import cfg_get, load_config, read_raw_config
...
def _config_cdp_url() -> str:
    ...
    try:
        from haishui_cli.config import read_raw_config  # late-bound: tests patch the source module
...
def _camofox_identity_override(task_id: Optional[str], camofox_cfg: Dict[str, Any]) -> Optional[Dict[str, str]]:
    ...
    share a user ID so Haishui uses the same profile), or None."""
...
def _get_session(task_id: Optional[str]) -> Dict[str, Any]:
    ...
    with _sessions_lock:
        ...
        if identity is None:
            identity = {"user_id": f"haishui_{uuid.uuid4().hex[:10]}", "session_key": f"task_{task_id[:16]}"}
...
def _save_screenshot(content: bytes) -> str:
    """Write PNG bytes under ``$HAISHUI_HOME/browser_screenshots`` and return the path."""
    from haishui_constants import get_haishui_home
    screenshots_dir = get_haishui_home() / "browser_screenshots"
