...
``metadata.haishui.blueprint`` (``schedule`` required; optional ``deliver`` [default "origin"],
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.haishui.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.haishui.blueprint`` is a mapping with a
    ...
    haishui = meta.get("haishui") if isinstance(meta, dict) else None
    blueprint = haishui.get("blueprint") if isinstance(haishui, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.haishui.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    ``metadata.haishui.blueprint`` block) ready for ``haishui skills publish``.
    ...
    frontmatter = {
        ...
        "metadata": {"haishui": {"tags": ["blueprint", "automation"], "blueprint": block}},
    }
