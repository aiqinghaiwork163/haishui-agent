...
~/.haishui/skills/, tracking each synced skill's origin hash in .bundled_manifest (v2 "name:hash"
...
from haishui_constants import get_bundled_skills_dir, get_haishui_home, get_optional_skills_dir
...
# retarget HAISHUI_HOME after import, and frozen constants would resolve (and for
...
# once under the launch HAISHUI_HOME and later scope requests to a different profile via
# set_haishui_home_override(). See #65828.
_HAISHUI_HOME_AT_IMPORT = HAISHUI_HOME
...
def _haishui_home() -> Path:
    return _live(HAISHUI_HOME, _HAISHUI_HOME_AT_IMPORT, get_haishui_home)


def _skills_dir() -> Path:
    return _live(SKILLS_DIR, _SKILLS_DIR_AT_IMPORT, lambda: _haishui_home() / "skills")
...
# Written by `haishui profile create --no-skills` / installer `--no-skills`: sync seeds only
# essential skills. Mirrors haishui_cli.profiles.NO_BUNDLED_SKILLS_MARKER (no CLI import here).
NO_BUNDLED_SKILLS_MARKER = ".no-bundled-skills"


def _get_bundled_dir() -> Path:  # HAISHUI_BUNDLED_SKILLS env first, then repo-relative
...
def _write_manifest(entries: Dict[str, str]):
    ...
    mkdir_under_haishui_home(_manifest_file().parent)
    try:
    ...
    for candidate in st.active_index.get(skill_name, []):
        ...
        if not _matches_origin_hash(candidate, origin_hash):  # moving a customized copy would edit user work
            st.say(
                ...
                f"Run `haishui skills reset {skill_name} --restore` to move to the new location.")
            continue
        try:
        ...
        if dest.exists():
            ...
            if _dir_hash(dest) == bundled_hash:
                ...
                st.say(
                    ...
                    f"by this name — yours was kept. Run `haishui skills reset {skill_name}` to "
...
def sync_skills(quiet: bool = False) -> dict:
    """Sync bundled skills into ~/.haishui/skills/ using the manifest; returns the per-category
    result dict. Opted-out profiles seed ONLY ESSENTIAL_SKILLS (the system prompt always
    points at ``haishui-agent``)."""
    essential_only = (_haishui_home() / NO_BUNDLED_SKILLS_MARKER).exists()
...
def _rmtree_writable(path: Path) -> None:
    ...
    child of the active skills root (bad join / missing HAISHUI_HOME / malicious manifest entry).
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.haishui/skills/ ...")
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (_haishui_home() / NO_BUNDLED_SKILLS_MARKER).exists()
