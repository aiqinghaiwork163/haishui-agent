...
# prefix, the exact-name HAISHUI_ allowlist, or (Windows) an OS-essential name.
# The broad "HAISHUI_" prefix is deliberately NOT safe — it leaked config vars
# without a secret substring (HAISHUI_BASE_URL, HAISHUI_KANBAN_DB, *_WEBHOOK).
# HAISHUI_RPC_SOCKET / HAISHUI_RPC_DIR / TZ / HOME are injected after scrubbing.
...
# imports may read at import time. HAISHUI_DELEGATED_CHILD_CONTEXT must ride
# along or a child that imports Haishui code loses the Kanban mutation guard
# while still inheriting HAISHUI_HOME.
_HAISHUI_CHILD_ALLOWED = frozenset({
    "HAISHUI_HOME", "HAISHUI_PROFILE", "HAISHUI_CONFIG", "HAISHUI_ENV", "HAISHUI_DELEGATED_CHILD_CONTEXT",
})
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    (4) operational HAISHUI_* pass by exact name; (5) on Windows the
    ...
    scrubbed = {}
    # Non-secret HAISHUI_* vars no allowlist admits are dropped on purpose; a script importing a
    ...
    for k, v in source_env.items():
        ...
        if (any(k.startswith(p) for p in _SAFE_ENV_PREFIXES)
                or k in _HAISHUI_CHILD_ALLOWED
                or (is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS)):
            scrubbed[k] = v
        elif k.startswith("HAISHUI_"):
            _dropped_haishui.append(k)
    ...
    if _dropped_haishui:
        logger.debug(
            "execute_code: dropped %d non-allowlisted HAISHUI_* var(s) from the "
            ...
            len(_dropped_haishui), ", ".join(sorted(_dropped_haishui)),
        )
    ...
    # non-allowlisted HAISHUI_* values in otherwise ordinary execution environments.
    if scoped.get(DELEGATED_CHILD_ENV_MARKER):
        for key in (DELEGATED_CHILD_ENV_MARKER, "HAISHUI_KANBAN_DB", "HAISHUI_KANBAN_BOARD"):
            if key in scoped:
...
def _build_child_env(*, rpc_endpoint: str, rpc_token: str, tmpdir: str,
    ...
    child_env["HAISHUI_RPC_SOCKET"] = rpc_endpoint
    child_env["HAISHUI_RPC_TOKEN"] = rpc_token
    ...
    # Only TZ reaches the child; HAISHUI_TIMEZONE is an internal setting (and under the multiplexed
    # gateway holds only the default profile's value — haishui_time resolves the routed profile's).
    from haishui_time import get_timezone_name
    ...
    child_env.pop("HAISHUI_TIMEZONE", None)
    apply_subprocess_home_env(child_env)
    # Multiplexed gateway/Desktop (#110303): the server process env carries the machine-default
    # HAISHUI_HOME, but this turn runs under a per-profile override (ContextVar bound per turn).
    ...
    _home_override = get_haishui_home_override()
    if _home_override:
        child_env["HAISHUI_HOME"] = _home_override
    # PYTHONPATH: the staging dir (haishui_tools.py) must always be importable even when project
    # mode changes CWD. Haishui's root is added ONLY when the child runs in Haishui's Python env —
    # exposing Haishui's site-packages to an external interpreter can mix incompatible compiled
    # extensions (3.12 NumPy under a 3.9 venv). Inherited Haishui-owned entries are stripped first.
    # Before re-injecting PYTHONPATH, strip Haishui-owned entries that leaked through _scrub_child_env
    # (PYTHONPATH is in _SAFE_ENV_PREFIXES so it passes the scrub). They are redundant for same-Haishui-
    ...
    _strip_haishui_owned_pythonpath(child_env)
    _existing_pp = child_env.get("PYTHONPATH", "")
    _pp_parts = [tmpdir]
    if _uses_haishui_python_environment(child_python):
        _pp_parts.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    elif child_python not in _external_env_logged:
        # Surface once per interpreter so "import haishui_constants fails" is diagnosable.
        _external_env_logged.add(child_python)
        logger.info("execute_code: child interpreter %s is outside the Haishui "
                    "environment; haishui root omitted from PYTHONPATH", child_python)
    if _existing_pp:
...
# Interpreter paths already reported as outside the Haishui environment.
...
def _uses_haishui_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to Haishui's active Python environment. Short-circuits when
    it IS the running interpreter (by path or realpath — covers ``uv run`` venvs) so no probe
    runs on the default strict path and a flaky probe can never drop the haishui root."""
