...
One append-only log per child under ``<haishui_home>/cache/delegation/live/
...
def live_transcript_root() -> Path:
    """Root directory for live transcripts (profile-safe, never ~/.haishui)."""
    from haishui_constants import get_haishui_dir
    return get_haishui_dir("cache/delegation", "delegation_cache") / "live"
...
class LiveTranscriptWriter:
    ...
    def __init__(self, delegation_id: str, task_index: int, goal: str,
        ...
        with _best_effort(f"init ({delegation_id} task {task_index})"):
            ...
            path.write_text(
                "=== Haishui subagent live transcript ===\n"
