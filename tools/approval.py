...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("HAISHUI_YOLO_MODE", ""))
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode derives from Haishui YOLO. Lazy import so approval-only
...
def _permanent_set() -> set:
    ...
    profile (HAISHUI_HOME override) → its own set, lazily loaded from ITS ``command_allowlist``: the
...
def _unattended_contexts() -> list[_Unattended]:
    """Active unattended contexts in evaluation order: single-query first (``haishui chat -q``
    exports HAISHUI_INTERACTIVE=1 but nobody answers); cron beats a platform marker because
...
def _presence(approval_callback=None) -> tuple:
    ...
    Single-query ``-q`` and cron clear the presence trio: ``haishui chat -q`` exports
    HAISHUI_INTERACTIVE=1 for sudo prompts, and a gateway sets HAISHUI_EXEC_ASK=1 at startup and
    ...
    is_ask = env_var_enabled("HAISHUI_EXEC_ASK")
        ...
        for ctx in _unattended_contexts():
            ...
            if fail_closed_when_no_human:
                logger.warning("%s (pattern: %s): %s — no interactive user/gateway present; "
                               "BLOCKED (fail-closed). Set HAISHUI_INTERACTIVE or "
                               "HAISHUI_GATEWAY_SESSION to answer the prompt.", *log_args)
                return _blocked(no_human_block_message or (
                    ...
                    pattern_key=pattern_key, description=description)
        logger.warning("%s (pattern: %s): %s — set HAISHUI_INTERACTIVE or "
                       "HAISHUI_GATEWAY_SESSION to require approval.", *log_args)
        return _approved()
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'reset_haishui_interactive_context': ('tools.approval_context', 'reset_haishui_interactive_context'),
    ...
    'set_haishui_interactive_context': ('tools.approval_context', 'set_haishui_interactive_context'),
}
