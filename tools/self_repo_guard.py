...
def _block_message(operation: str, root: Path) -> str:
    ...
    scratch = (Path(haishui_home).expanduser() if haishui_home else Path.home() / ".haishui") / "scratch"
    return (
        f"Blocked: `{operation}` would rewrite Haishui's live source checkout "
        ...
        "is pushed. To change this checkout, stop Haishui, run the command externally, then restart "
        "Haishui.")
