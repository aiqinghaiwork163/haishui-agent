#!/usr/bin/env python3
"""One parent-death supervisor per Haishui process, shared by all stdio MCP servers.

Why this exists
---------------
When Haishui dies without running its cleanup path (SIGKILL, OOM killer, a hard
crash), stdio MCP servers it spawned are reparented to init and keep running
forever.  macOS has no ``PR_SET_PDEATHSIG``, so something has to outlive Haishui
and reap them.

This module is deliberately standard-library-only and must not import anything
from ``tools/``: it runs after Haishui may already be dead, and pulling in
...
with a single supervisor per Haishui process:

* **Death detection is a blocking read on a pipe.**  Haishui holds the only write
  end.  When Haishui dies -- by any means, including SIGKILL -- the write end
...
Haishui never unregistered *is* the orphan set, so a clean Haishui shutdown --
...
an unrelated process owned by the same user.  If Haishui then dies ungracefully
...
Two things narrow the window.  Haishui prunes registrations whose group has no
...
exposure already exists in Haishui's own killpg-based orphan cleanup, which this
...
def _is_safe_target(pgid: int, *, own_pgid: int, parent_pgid: int) -> bool:
    ...
    Defensive only -- Haishui already filters non-MCP children before it
    ...
    Haishui process tree or the session.
    """
    if pgid <= 1:
        return False
...
def _serve(stream, *, own_pgid: int, parent_pgid: int) -> set[int]:
    ...
    stopped. Nothing in Haishui can produce that today, but this process is the
...
def main(argv=None) -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Process group of the spawning Haishui process; never signalled.",
    )
