...
_FOREGROUND_GUIDANCE = (
    (
        ...
        "notify_on_complete=true) so Haishui tracks the process, then run readiness "
...
def gateway_lifecycle_block(
    *,
    ...
    ``systemctl``/``launchctl``/``haishui gateway restart|stop|uninstall``
    targeting haishui-gateway would SIGTERM the gateway — and this very
    ...
    SUPERVISED-gateway probe, not the raw ``_HAISHUI_GATEWAY`` marker: that
    marker leaks into every process that merely imports gateway.run (haishui
    serve, CLI, web server), which must still be able to restart the gateway;
    an unsupervised foreground ``haishui gateway run`` has no KeepAlive to turn
    ...
    if lifecycle_scan_root_within_budget(command) and contains_launchctl_submit_command(command):
        return _blocked_json(
            ...
            "or determine whether it is independent of Haishui. Perform authorized "
    ...
    if unsafe:
        ...
        return _blocked_json(
            ...
            "to child processes). Run `haishui gateway restart` from a "
