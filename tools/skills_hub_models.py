...
def _haishui_tags(fm: dict) -> Any:
    """``metadata.haishui.tags`` from parsed frontmatter, or ``[]`` (unvalidated type)."""
    metadata = fm.get("metadata", {})
    haishui_meta = metadata.get("haishui", {}) if isinstance(metadata, dict) else None
    return haishui_meta.get("tags", []) if isinstance(haishui_meta, dict) else []
