...
def _check_frontmatter(frontmatter: Dict[str, Any], skill_dir: Optional[Path]) -> Iterator[LintFinding]:
    ...
    haishui_meta = meta.get("haishui") if isinstance(meta, dict) else None
    if not isinstance(haishui_meta, dict):
        yield _warn("missing-metadata", "frontmatter is missing metadata.haishui.{tags, related_skills}.")
    elif "tags" not in haishui_meta:
        yield _warn("missing-metadata", "metadata.haishui.tags is missing.")
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("haishui", "agent", "haishui agent") and (
        author != "Haishui Agent"):
        yield _warn("author-caps", f"author '{author}' should be 'Haishui Agent' (proper caps) "
