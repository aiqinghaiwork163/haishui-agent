...
def _truncate_to_char_budget(content: str, max_chars: int) -> tuple[str, int, bool]:
    ...
    Ported in spirit from nearai/ironclaw#5029 (dual line/byte cap on ``read_file``). Where haishui
...
def read_file_tool(path: str, offset: int = 1, limit: int = DEFAULT_READ_LIMIT, task_id: str = "default") -> str:
    ...
    → document extraction → binary-extension guard → Haishui internal denylist
    ...
    try:
        ...
        # Haishui internal denylist (prompt injection via catalog metadata,
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with haishui_tools.write_file() for very "
            "large files."
        )
