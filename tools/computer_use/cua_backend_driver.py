...
# var would only LOOK like it pinned. Point HAISHUI_CUA_DRIVER_CMD at a specific binary instead.
_CUA_DRIVER_CMD_ENV = "HAISHUI_CUA_DRIVER_CMD"
...
def _wsl_windows_path_to_posix(path: str) -> str:
    """Translate a Windows absolute manifest command to its DrvFS ``/mnt/<drive>/...`` form when Haishui runs in WSL
    (a Windows cua-driver manifest can report ``C:\\...`` while Haishui spawns via POSIX). Non-Windows paths and
...
def _candidate_cua_driver_commands(override: Optional[str] = None) -> List[str]:
    """Candidate commands in resolution order. ``override`` / a non-empty ``HAISHUI_CUA_DRIVER_CMD`` is authoritative
...
def cua_driver_install_hint() -> str:
    ...
    return ("cua-driver is not installed. Install with one of:\n  haishui computer-use install\n"
            ...
            "Or run `haishui tools` and enable the Computer Use toolset to install it automatically.")
...
def _manifest_contract_reason(manifest: Optional[Dict[str, Any]]) -> str:
    ...
    if tuple(int(part) for part in match.groups()) < _CUA_DRIVER_RUNTIME_CONTRACT_MIN:
        return "Haishui computer use requires cua-driver 0.20.0 or newer"
...
def cua_driver_runtime_contract_status(binary: Optional[str] = None) -> Dict[str, Any]:
    """Report whether a local driver can host Haishui' 0.20 integration."""
...
def cua_driver_update_nudge() -> Optional[str]:
    ...
    return (f"cua-driver {state.get('latest_version') or '?'} is available "
            f"(you have {state.get('current_version') or '?'}); update with `haishui computer-use install --upgrade`.")
