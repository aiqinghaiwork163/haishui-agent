 ...
 * dev:mock dev flow.
 *
 * Implements just enough of the /v1/* surface for `haishui serve` to resolve a
...
/**
 * Drives a real code edit followed by two finish attempts. Haishui should add
...
export const APPROVAL_COMMAND = 'rm -rf /tmp/haishui-e2e-approval-probe'
 ...
 * `E2E_SAY(code-farmer)[{at}haishui Reply with B.] E2E_SAY(haishui)[B]`.
...
export function createBackgroundReleaseHandle(): BackgroundReleaseHandle {
  const path = nodePath.join(
    os.tmpdir(),
    `haishui-e2e-bg-release-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
  )
...
/** Create an isolated HAISHUI_HOME + Electron user-data dir in the OS temp dir. */
function createDevSandbox(): DevSandbox {
  const root = fs.mkdtempSync(nodePath.join(os.tmpdir(), `haishui-dev-mock-${Date.now()}`))
  const haishuiHome = nodePath.join(root, 'haishui-home')
...
function writeMockConfig(haishuiHome: string, mockUrl: string): void {
  fs.writeFileSync(
    nodePath.join(haishuiHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(nodePath.join(haishuiHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function runDevLaunch(): Promise<void> {
  ...
  writeMockConfig(sandbox.haishuiHome, mock.url)
  console.log(`  HAISHUI_HOME: ${sandbox.haishuiHome}`)
  ...
  const env: Record<string, string> = {
    ...
    HAISHUI_DESKTOP_HAISHUI_ROOT: repoRoot,
    HAISHUI_DESKTOP_APP_NAME: `HaishuiDevMock-${Date.now()}`,
  }
