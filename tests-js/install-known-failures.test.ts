...
const lockedLog = [
  'error: failed to remove file `C:/install/venv/Lib/site-packages/../../Scripts/haishui.exe`: Access is denied. (os error 5)',
  'File "C:/install/venv/Scripts/haishui.exe/__main__.py", line 10, in <module>',
...
const base = {
  ...
  installMethod: 'installer-script', updateMethod: 'haishui-update',
  error: 'E2E ASSERTION FAILED: haishui update exited 1 (expected 0)', logs: { update: lockedLog },
}
...
describe('known install failures', () => {
  it('recognizes the released launcher self-lock, not generic access denied', () => {
    ...
    expect(matchKnownFailure({ ...base, logs: { update: lockedLog.replaceAll('haishui.exe', 'other.exe') } })).toBeNull()
  ...
  it('matches manual-only app updates only for the three proven July script cases', () => {
    const sample = {
      ...
      updateMethod: 'haishui-desktop-app-update',
      error: 'E2E ASSERTION FAILED: app driven via captured haishui desktop spec; update completed',
      logs: { desktop: '[haishui] [updates] no staged updater; surfacing manual `haishui update` for CLI install at C:/install\n[haishui] [updates] manual: haishui update\n' },
    }
    ...
    expect(matchKnownFailure({ ...sample, logs: { desktop: '[updates] manual: haishui update' } })).toBeNull()
  })
...
it('renders known receipts as footnotes, without suppressing a red job', async () => {
  ...
  const name = 'windows: installer-script -> haishui-update (v2026.3.12 -> HEAD)'
