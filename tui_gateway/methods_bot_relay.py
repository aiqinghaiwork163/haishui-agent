...
def _relay_root() -> Path:
    ...
    every HAISHUI_HOME, including non-``profiles/`` subdirs of ``~/.haishui``."""
    from tools.bot_mode_probe import _default_home, _haishui_root
    return _haishui_root(Path(_default_home()))
...
def _(rid, params: dict, _root=_relay_root, _run=_run_delivery,
    ...
    GATEWAY via the one-turn ``haishui -p <profile> chat -c "Bot Chat"`` transport local DMs use →
    ...
    try:
        ...
        resolved = "default" if profile.lower() == "haishui" else profile
        ...
        owner_home = live_home if live_home is not None else Path(_haishui_home)
        ...
        fd, tmp = tempfile.mkstemp(prefix="haishui-relay-dm-", suffix=".txt", text=True)
        try:
