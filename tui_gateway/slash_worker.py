"""Persistent slash-command worker — one HaishuiCLI per TUI session.

Protocol: reads JSON lines from stdin {id, command}, writes {id, ok, output|error} to stdout.
"""

# Stop a ``utils/`` (or ``proxy/``, ``ui/``) package in the launch directory from shadowing Haishui's own
...
# retry loop. ``haishui_bootstrap`` lives at the repo root (no collision risk), so importing it first is safe.
# ``haishui_bootstrap`` lives at the repo root, so importing it is safe before the guard runs (its name won't
...
_WATCHDOG_POLL_S = max(0.05, env_float("HAISHUI_SLASH_WATCHDOG_POLL_S", 2.0))
_ORPHAN_GRACE_S = max(0.0, env_float("HAISHUI_SLASH_WATCHDOG_GRACE_S", 5.0))
...
def _prepare_slash_worker_runtime() -> None:
    """Start bounded MCP discovery before HaishuiCLI snapshots tools: each slash_worker child is its
    own process — the parent ``haishui serve`` discovery thread does not populate this registry.

    See #61891.
    """
    from haishui_cli.mcp_startup import start_background_mcp_discovery, wait_for_mcp_discovery
...
def _run(cli: HaishuiCLI, command: str) -> str:
...
def main():
    ...
    args = p.parse_args()
    os.environ["HAISHUI_SESSION_KEY"] = args.session_key
    os.environ["HAISHUI_INTERACTIVE"] = "1"
    # Start before the (hundreds-of-ms) HaishuiCLI build — that window is itself an orphan risk if the
    # gateway dies mid-spawn.
    _start_parent_death_watchdog(os.getppid())
    _prepare_slash_worker_runtime()
    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = HaishuiCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
