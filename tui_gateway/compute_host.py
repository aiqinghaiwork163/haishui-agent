...
class ComputeHost:
    ...
    def __init__(
        ...
        self._heartbeat_secs = (
            ...
            else float(os.environ.get("HAISHUI_COMPUTE_HOST_HEARTBEAT_SECS") or "15"))
    ...
    def _run_real_turn(self, frame: dict[str, Any]) -> None:
        ...
        try:
            ...
            with contextlib.suppress(Exception):
                import haishui_undo
                haishui_undo.on_user_message_appended(session["session_key"])
            with contextlib.suppress(Exception):
    ...
    def _build_server_session(self, server: Any, frame: dict[str, Any], sid: str) -> dict:
        ...
        try:
            if profile_home:
                from haishui_constants import set_haishui_home_override
            ...
            if home_token is not None:
                with contextlib.suppress(Exception):
                    from haishui_constants import reset_haishui_home_override
...
def _default_workers() -> int:
    try:
        return max(2, int(os.environ.get("HAISHUI_TUI_RPC_POOL_WORKERS") or "8"))
    except (TypeError, ValueError):
        return 8


def run_host(stdin: Any = None, stdout: Any = None) -> None:
    os.environ["HAISHUI_COMPUTE_HOST_CHILD"] = "1"
    ...
    host.emit({
        ...
        "haishui_home": os.environ.get("HAISHUI_HOME", "")})
