...
def _handle_admitted_request(req: dict) -> dict | None:
    ...
    if not (fn := _methods.get(method)):
        return _err(rid, -32601, f"unknown method: {method} — the client and the Haishui backend are out of sync "
                    "(different versions); run `haishui update` and restart both")
