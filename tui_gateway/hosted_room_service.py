...
def _hosted_room_turn_timeout_seconds() -> float:
    try:
        agent_timeout = float(os.getenv("HAISHUI_AGENT_TIMEOUT", "1800"))
...
class HostedRoomService:
    ...
    def local_profiles(self) -> tuple[str, ...]:
        from haishui_constants import named_profile_has_identity, named_profile_is_deleted

        profiles, profiles_dir = {"default"}, self.root / "profiles"
        if profiles_dir.is_dir():
            # ``profiles/.deleted/`` is the tombstone dir `haishui profile delete` leaves behind, not a
