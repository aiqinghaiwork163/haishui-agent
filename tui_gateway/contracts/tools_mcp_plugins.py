...
method("reload.env", params=ReloadEnvParams, result=ReloadEnvResult,
       doc="Re-read ~/.haishui/.env (CLI /reload parity); built agents keep their pool until /new.")
...
class SkillBrowseItem(OpenModel):
    """``haishui_cli.skills_hub.browse_skills`` row."""
...
class SkillInspectInfo(OpenModel):
    """``haishui_cli.skills_hub.inspect_skill``; ``{}`` when the identifier resolves nowhere."""
...
class PluginsManageResult(Result):
    ...
    ``install`` → ``haishui_cli.plugins_cmd.dashboard_install_plugin``'s ok payload; ``update`` →
    ``ok``/``unchanged``/``sha``."""
