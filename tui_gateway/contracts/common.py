...
class OpenModel(Result):
    ...
    (the closed set is owned elsewhere: haishui_state rows, provider inventories)."""
...
class StoredSessionRow(OpenModel):
    """One ``sessions`` row as ``haishui_state`` lists it (``session.list`` / ``session.info`` rows /
    ``sessions.changed``)."""
...
class MessageReaction(OpenModel):
    """One persisted reaction row (``haishui_state_messages.set_message_reaction``); ``seen`` is
