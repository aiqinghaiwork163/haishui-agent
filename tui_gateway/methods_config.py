...
from haishui_constants import display_haishui_home as _display_haishui_home
...
def _cfg_get_provider(params):
    from haishui_cli.models import list_available_providers, normalize_provider
...
def _cfg_get_mtime(params):
    cfg_path = _haishui_home / "config.yaml"
    try:
...
_CONFIG_GETTERS = {
    ...
    "profile": lambda params: {"home": str(_haishui_home), "display": _display_haishui_home()},
...
def _readiness_check(rid, params, probe):
    ...
    optional ``profile`` param's HAISHUI_HOME + ``.env`` secret scope (ContextVars: concurrent checks
    ...
    # (``HAISHUI_CODEX_BASE_URL`` for openai-codex) fails closed and the UI shows onboarding.
...
def _(rid, params: dict) -> dict:
    ...
    added after boot — the Models page, a picker key, ``haishui setup`` from a shell — flips it
    ...
    try:
        ...
        from haishui_cli.free_tier_bootstrap import wait_for_record
    ...
    try:
        ...
        def probe(profile, scoped):
            ...
            if (not provider_configured and provider == "bedrock"
                ...
                return fail("No Haishui provider is configured.", source)
            ...
            from haishui_cli.anon_auth import route_is_welcome_host
...
def _(rid, params: dict) -> dict:
    ...
    force-redaction pipeline as ``haishui debug share --nous``; redaction is NOT client-controllable
    ...
    try:
        from haishui_cli.debug import _redact_log_text, build_nous_bundle, collect_share_bundle
