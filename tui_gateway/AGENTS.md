...
activate with `haishui --tui` or `HAISHUI_TUI=1`. `tui_gateway` is ALSO the backend the Desktop app
...
(`4000` + key path) and, under `HAISHUI_TEST_ISOLATION=1`, raises `ContractViolation` when a handler's
...
the launch home, never `~/.haishui`. The first non-launch home hosted flips
...
**A method that sets only `get_haishui_home_override()` is half-bound**: config paths resolve to the
...
(`haishui -p X` workers, `key_cmd` helpers, browser drivers), never `dict(os.environ)`. Grep for
...
themselves come from `haishui_cli/commands.py` (`haishui_cli/AGENTS.md`).
...
npm run dev       # watch mode (rebuilds haishui-ink + tsx --watch)
npm start         # production
npm run build     # full build (haishui-ink + tsc)
