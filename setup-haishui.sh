...
# 5. Symlinks the 'haishui' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}☤ Haishui Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/haishui-uv-install.$$.log")"
...
if is_termux; then
    # Haishui currently declares requires-python >=3.11,<3.14. Termux can expose
    ...
    if [ -z "${PYTHON_PATH:-}" ]; then
        if command -v python >/dev/null 2>&1; then
            ...
            echo -e "${RED}✗${NC} Termux Python $PYTHON_FOUND_VERSION is not supported; Haishui requires Python >=3.11,<3.14"
...
# ============================================================================
# PATH setup — symlink haishui into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up haishui command..."

HAISHUI_BIN="$SCRIPT_DIR/venv/bin/haishui"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$HAISHUI_BIN" "$COMMAND_LINK_DIR/haishui"
echo -e "${GREEN}✓${NC} Symlinked haishui → $COMMAND_LINK_DISPLAY_DIR/haishui"
...
# ============================================================================
# Seed bundled skills into ~/.haishui/skills/
# ============================================================================

HAISHUI_SKILLS_DIR="${HAISHUI_HOME:-$HOME/.haishui}/skills"
mkdir -p "$HAISHUI_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.haishui/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$HAISHUI_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  haishui status        # Check configuration"
if is_termux; then
    echo "  haishui gateway       # Run gateway in foreground"
else
    echo "  haishui gateway install # Install gateway service (messaging + cron)"
fi
echo "  haishui cron list     # View scheduled jobs"
echo "  haishui doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m haishui_cli.main setup
fi
