"""Timezone-aware clock for Haishui.

``now()`` returns a tz-aware datetime in the user's configured IANA timezone. Resolution order:
``HAISHUI_TIMEZONE`` env var, then ``timezone`` in ``~/.haishui/config.yaml``, else server-local
...
# HAISHUI_HOME, so one unkeyed global would leak the first profile's timezone into later
...
def _env_timezone() -> str:
    """``HAISHUI_TIMEZONE`` when it may speak for the active profile. Under the multiplexed
    ...
    if is_multiplex_active():
        return ""
    return os.getenv("HAISHUI_TIMEZONE", "").strip()
...
def _resolve_timezone_name() -> str:
    ...
    try:
        ...
        # parse is the stdlib-safe fallback for bootstrap consumers without haishui_cli importable.
        try:
