#!/bin/sh
# shellcheck shell=sh
# /opt/haishui/bin/haishui — `docker exec` privilege-drop shim.
#
# Background
# ----------
# The s6 image runs the supervised gateway/main process as the unprivileged
# `haishui` user (UID 10000). When an operator runs `docker exec <c> haishui ...`
# the default UID is root (0), and any file the command writes under
# $HAISHUI_HOME — auth.json, .env, config.yaml — ends up root-owned and
# unreadable to the supervised gateway. The most common manifestation: the
# user runs `docker exec <c> haishui login`, this writes
# /opt/data/auth.json as root:root mode 0600, and from then on the gateway
# returns "Provider authentication failed: Haishui is not logged into Nous
# Portal" on every incoming message — even though `docker exec <c> haishui
...
# Fix
# ---
# This shim sits at /opt/haishui/bin/haishui and is placed earliest on PATH.
# When invoked as root, it drops to the haishui user (via s6-setuidgid)
# before exec'ing the real venv binary, so anything that writes under
# $HAISHUI_HOME is uid-aligned with the supervised processes. When invoked
# as any non-root UID — including the supervised processes themselves,
# `docker exec --user haishui`, kanban subagents, etc. — it short-circuits
...
# (/opt/haishui/.venv/bin/haishui), so the second hop cannot re-enter this
# shim regardless of PATH state. No sentinel env var needed.
#
# Opt-out: set HAISHUI_DOCKER_EXEC_AS_ROOT=1 (1/true/yes, case-insensitive)
...
# state via the haishui CLI. Default is to drop.

set -e

REAL=/opt/haishui/.venv/bin/haishui

# Defensive: if the venv binary is missing (corrupted image, partial
# install), fail loudly rather than silently masking it.
if [ ! -x "$REAL" ]; then
    echo "haishui-shim: $REAL not found or not executable" >&2
    exit 127
fi

# Already non-root? Just exec the real binary. This is the hot path for
# supervised processes (uid 10000) and for `docker exec --user haishui`.
if [ "$(id -u)" != "0" ]; then
    exec "$REAL" "$@"
fi

# Root, with opt-out set? Honor it.
case "${HAISHUI_DOCKER_EXEC_AS_ROOT:-}" in
    1|true|TRUE|True|yes|YES|Yes)
        exec "$REAL" "$@"
        ;;
esac

# Root, no opt-out. Drop to the haishui user.
#
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "haishui-shim: $S6_SUID not found; refusing to silently run as root." >&2
    echo "haishui-shim: re-run with --user haishui or set HAISHUI_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi

# Reset HOME to the haishui user's home before dropping privileges. Without
