...
#
# Wired into the image as /etc/cont-init.d/01-haishui-setup by the
...
HAISHUI_HOME="${HAISHUI_HOME:-/opt/data}"
INSTALL_DIR="/opt/haishui"

# Drop to haishui via s6-setuidgid, but skip it when already non-root.
as_haishui() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid haishui "$@"; }
...
# starts non-root. The baked install tree under /opt/haishui is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $HAISHUI_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass HAISHUI_UID/HAISHUI_GID — or the PUID/PGID aliases — which the
...
# `id -u haishui` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# haishui UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u haishui)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-haishui UID).
...
and the baked /opt/haishui install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e HAISHUI_UID=\$(id -u) -e HAISHUI_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the haishui user to that UID/GID at boot and chowns the data
...
# --- Bootstrap HAISHUI_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid haishui mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom HAISHUI_HOME paths whose parents only
# root can create (e.g. `HAISHUI_HOME=/home/haishui/.haishui` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for HAISHUI_UID/HAISHUI_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  HAISHUI_UID/
# HAISHUI_GID still win when both are set.  See #15290, salvages #25872.
HAISHUI_UID="${HAISHUI_UID:-${PUID:-}}"
HAISHUI_GID="${HAISHUI_GID:-${PGID:-}}"

if [ -n "${HAISHUI_UID:-}" ] && validate_uid_gid "$HAISHUI_UID" && [ "$HAISHUI_UID" != "$(id -u haishui)" ]; then
    echo "[stage2] Changing haishui UID to $HAISHUI_UID"
    usermod -u "$HAISHUI_UID" haishui
fi
if [ -n "${HAISHUI_GID:-}" ] && validate_uid_gid "$HAISHUI_GID" && [ "$HAISHUI_GID" != "$(id -g haishui)" ]; then
    echo "[stage2] Changing haishui GID to $HAISHUI_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$HAISHUI_GID" haishui 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised haishui user (UID 10000)
...
# haishui process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes haishui. Idempotent across container
...
#   - haishui is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G haishui 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] haishui already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" haishui 2>/dev/null; then
        echo "[stage2] Added haishui to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group haishui failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When HAISHUI_UID is remapped or the top-level $HAISHUI_HOME isn't owned by
# the runtime haishui UID, restore ownership to haishui — but ONLY for the
# directories haishui actually writes to. The full $HAISHUI_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of haishui-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_haishui_uid=$(id -u haishui)
...
chown_haishui_tree() {
    ...
    chown -R haishui:haishui "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

tree_has_non_haishui_owner() {
    target="$1"
    find "$target" \( ! -user haishui -o ! -group haishui \) -print -quit 2>/dev/null | grep -q .
}

needs_chown=false
if [ "$(stat -c %u "$HAISHUI_HOME" 2>/dev/null)" != "$actual_haishui_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $HAISHUI_HOME (targeted) to haishui ($actual_haishui_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $HAISHUI_HOME: chown the directory itself (not its contents)
    # so haishui can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$HAISHUI_HOME"; then
        :
    else
        chown haishui:haishui "$HAISHUI_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $HAISHUI_HOME failed (rootless container?) — continuing"
    fi
    # Haishui-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by haishui (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$HAISHUI_HOME/$sub" ] && tree_has_non_haishui_owner "$HAISHUI_HOME/$sub"; then
            chown_haishui_tree "$HAISHUI_HOME/$sub"
        fi
    done
fi
...
# haishui user. Hosted/container instances keep mutable state under
# $HAISHUI_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# HAISHUI_DISABLE_LAZY_INSTALLS=1. Keeping /opt/haishui root-owned and
...
# $HAISHUI_HOME/lazy-packages dir on the data volume (Dockerfile sets
# HAISHUI_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to haishui
...
# Always reset ownership of $HAISHUI_HOME/profiles to haishui on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> haishui …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as haishui and walks
...
if [ -d "$HAISHUI_HOME/profiles" ] && tree_has_non_haishui_owner "$HAISHUI_HOME/profiles"; then
    chown_haishui_tree "$HAISHUI_HOME/profiles"
fi

# Always reset ownership of $HAISHUI_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged haishui runtime even
...
# gate as profiles/).
if [ -d "$HAISHUI_HOME/cron" ] && tree_has_non_haishui_owner "$HAISHUI_HOME/cron"; then
    chown_haishui_tree "$HAISHUI_HOME/cron"
fi

# Always ensure logs/gateways is haishui-owned (#45258). Formerly healed by
...
# top-level $HAISHUI_HOME is mis-owned, so a warm volume with haishui-owned
# HAISHUI_HOME but root-owned logs/gateways would otherwise leave
# s6-setuidgid haishui mkdir failing with Permission denied. Non-recursive:
# profile leaf dirs are each created/owned by their own log/run as haishui.
if [ -d "$HAISHUI_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$HAISHUI_HOME/logs/gateways"; then
        :
    else
        chown haishui:haishui "$HAISHUI_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# haishui pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged haishui gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $HAISHUI_HOME is
...
# and cron/.
if [ -d "$HAISHUI_HOME/platforms/pairing" ] && tree_has_non_haishui_owner "$HAISHUI_HOME/platforms/pairing"; then
    chown_haishui_tree "$HAISHUI_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$HAISHUI_HOME/pairing" ] && tree_has_non_haishui_owner "$HAISHUI_HOME/pairing"; then
    chown_haishui_tree "$HAISHUI_HOME/pairing"
fi

# Reset ownership of haishui-owned top-level state files on every boot.
# The targeted data-volume chown above only covers haishui-owned
# *subdirectories*; loose state files living directly under $HAISHUI_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> haishui …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged haishui runtime then hits
...
# sweep so host-owned files in a bind-mounted $HAISHUI_HOME are never
...
# entries of haishui_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$HAISHUI_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$HAISHUI_HOME/$f"; then
            :
        else
            chown haishui:haishui "$HAISHUI_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the haishui runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$HAISHUI_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$HAISHUI_HOME/config.yaml"; then
        :
    else
        chown haishui:haishui "$HAISHUI_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$HAISHUI_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as haishui user ---
# Run as haishui via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $HAISHUI_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_haishui mkdir -p \
    "$HAISHUI_HOME/backups" \
    "$HAISHUI_HOME/cron" \
    "$HAISHUI_HOME/sessions" \
    "$HAISHUI_HOME/logs" \
    "$HAISHUI_HOME/logs/gateways" \
    "$HAISHUI_HOME/hooks" \
    "$HAISHUI_HOME/memories" \
    "$HAISHUI_HOME/skills" \
    "$HAISHUI_HOME/skins" \
    "$HAISHUI_HOME/plans" \
    "$HAISHUI_HOME/workspace" \
    "$HAISHUI_HOME/home" \
    "$HAISHUI_HOME/pairing" \
    "$HAISHUI_HOME/platforms/pairing" \
    "$HAISHUI_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/haishui/.install_method (see Dockerfile), NOT written here into
# $HAISHUI_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $HAISHUI_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.haishui:/opt/data) and sometimes shared with a
...
# 'haishui update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $HAISHUI_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/haishui stamp, so deleting the data-dir copy is safe).
if [ -f "$HAISHUI_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$HAISHUI_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$HAISHUI_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$HAISHUI_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$HAISHUI_HOME/$dest"; then
            :
        else
            as_haishui cp "$INSTALL_DIR/$src" "$HAISHUI_HOME/$dest"
        fi
    fi
}
...
# the first-boot seed above depends on /opt/haishui/.env.example being present
...
# not generate one. Haishui loads $HAISHUI_HOME/.env with override=True, so
...
if [ -n "${API_SERVER_KEY:-}" ]; then
    if [ -f "$HAISHUI_HOME/.env" ] && grep -q '^API_SERVER_KEY=..*' "$HAISHUI_HOME/.env" 2>/dev/null; then
        echo "[stage2] Warning: API_SERVER_KEY is set in both the container environment and $HAISHUI_HOME/.env — the .env value wins at runtime (loaded with override=True)"
    else
        ...
        if [ -f "$HAISHUI_HOME/.env" ] && ! refuse_symlinked_path "clean" "$HAISHUI_HOME/.env"; then
            sed -i '/^API_SERVER_KEY=$/d' "$HAISHUI_HOME/.env" 2>/dev/null || true
        fi
        echo "[stage2] API_SERVER_KEY provided via container environment — skipping generation"
    fi
elif ! grep -q '^API_SERVER_KEY=..*' "$HAISHUI_HOME/.env" 2>/dev/null; then
    if refuse_symlinked_path "append" "$HAISHUI_HOME/.env"; then
        :
    else
        if [ ! -f "$HAISHUI_HOME/.env" ]; then
            ...
            (umask 077 && as_haishui touch "$HAISHUI_HOME/.env") 2>/dev/null || true
        fi
        if [ -f "$HAISHUI_HOME/.env" ]; then
            ...
            if [ -n "$_gen_key" ]; then
                ...
                sed -i '/^API_SERVER_KEY=$/d' "$HAISHUI_HOME/.env" 2>/dev/null || true
                if printf 'API_SERVER_KEY=%s\n' "$_gen_key" >> "$HAISHUI_HOME/.env" 2>/dev/null; then
                    echo "[stage2] Generated API_SERVER_KEY for the loopback gateway api_server"
                else
                    echo "[stage2] Warning: could not write API_SERVER_KEY to $HAISHUI_HOME/.env (read-only volume?) — gateway api_server (cron fires) will be unavailable"
                fi
            fi
            unset _gen_key
        else
            echo "[stage2] Warning: could not create $HAISHUI_HOME/.env — gateway api_server (cron fires) will be unavailable"
        fi
    fi
fi

# --- Sync deploy-injected Nous routing overrides into every profile .env ---
# Under multiplex, haishui_cli.auth_nous reads HAISHUI_PORTAL_BASE_URL (or its
...
# carries the value into $HAISHUI_HOME/.env and every profiles/*/.env: the
...
# (/etc/haishui/.env) composition reverted by #111600 is restored.
...
sync_routing_overrides() {
    ...
    for _name in HAISHUI_PORTAL_BASE_URL NOUS_PORTAL_BASE_URL NOUS_INFERENCE_BASE_URL; do
        eval "_value=\${$_name:-}"
        ...
        if [ ! -f "$_file" ] && ! (umask 077 && as_haishui touch "$_file") 2>/dev/null; then
...
sync_routing_overrides "$HAISHUI_HOME/.env"
for _profile_dir in "$HAISHUI_HOME"/profiles/*/; do
...
if [ -f "$HAISHUI_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$HAISHUI_HOME/.env"; then
        :
    else
        chown haishui:haishui "$HAISHUI_HOME/.env" 2>/dev/null || true
        chmod 600 "$HAISHUI_HOME/.env" 2>/dev/null || true
    fi
fi
...
# as the unprivileged `haishui` user — without this it gets EACCES on every
...
# haishui group (any group member could e.g. stop/suspend this machine), not
...
if [ -S /.fly/api ]; then
    if refuse_symlinked_path "chgrp/chmod" /.fly/api; then
        :
    elif chgrp haishui /.fly/api 2>/dev/null && chmod g+w /.fly/api 2>/dev/null; then
        echo "[stage2] Granted haishui group access to the Fly Machines API socket"
    else
...
# $HAISHUI_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `haishui update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set HAISHUI_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$HAISHUI_HOME/config.yaml" ]; then
    s6-setuidgid haishui "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$HAISHUI_HOME/auth.json" ] && [ -n "${HAISHUI_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$HAISHUI_HOME/auth.json"; then
        :
    else
        printf '%s' "$HAISHUI_AUTH_JSON_BOOTSTRAP" > "$HAISHUI_HOME/auth.json"
        chown haishui:haishui "$HAISHUI_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$HAISHUI_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via HAISHUI_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$HAISHUI_HOME/auth.json" ] && [ -n "${HAISHUI_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$HAISHUI_HOME/auth.json"; then
        :
    else
        s6-setuidgid haishui "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_nous_session.py" \
            "$HAISHUI_HOME/auth.json" \
...
# can set HAISHUI_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# HAISHUI_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$HAISHUI_HOME/gateway_state.json" ] && \
        [ "${HAISHUI_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$HAISHUI_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$HAISHUI_HOME/gateway_state.json"
        chown haishui:haishui "$HAISHUI_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$HAISHUI_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_haishui "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/haishui/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Haishui spawns for the
...
# main-wrapper.sh propagates it into the supervised ``haishui`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-haishui, dashboard, etc.).
