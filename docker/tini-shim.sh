...
# Haishui WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: HAISHUI_TINI_SHIM_TARGET overrides the `/init` path (and
# HAISHUI_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${HAISHUI_TINI_SHIM_TARGET:-/init}"
WRAPPER="${HAISHUI_TINI_SHIM_WRAPPER:-/opt/haishui/docker/main-wrapper.sh}"
