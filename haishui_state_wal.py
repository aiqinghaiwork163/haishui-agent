"""SQLite journal-mode and PRAGMA policy for state.db (split from haishui_state).

Patchable helpers are looked up as module globals at call time, so tests patch ``haishui_state_wal.<name>``.
"""
...
from haishui_cli.sqlite_runtime import is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable
...
# line would repeat per connection). Tests clear these via ``haishui_state_wal.<name>``.
...
def apply_wal_with_fallback(conn: sqlite3.Connection, *, db_label: str = "state.db", require_wal: bool = False) -> str:
    ...
    and ``haishui_cli.kanban_db_connect.connect``. WAL-incompatible filesystems either raise ``OperationalError`` ("locking protocol" / "disk I/O error") or —
...
def _wal_reset_repair_hint() -> str:
    """Repair hint matching what ``haishui update`` can actually do for this install type.

    See #75153.
    """
    try:
        from haishui_cli.config import detect_install_method, get_project_root, recommended_update_command_for_method
        method = detect_install_method(get_project_root())
        cmd = recommended_update_command_for_method(method)
        if method in {"git", "unknown"}:
            return f"Haishui-managed installs can repair the embedded runtime with `{cmd}`"
        return f"update the container image with `{cmd}`" if method == "docker" else cmd  # else nix/nixos
    except Exception:
        return "install a Python build bundled with SQLite 3.51.3+ (or backports 3.50.7 / 3.44.6) and restart Haishui"
...
_ONCE_LOGS = {
    "wal_reset_bug": (_wal_reset_bug_warned_lock, "_wal_reset_bug_warned_paths", logging.WARNING,
        ...
        "%s. See `haishui doctor`. This warning fires once per process per database."),
    ...
    "cross_vm_fs_existing_wal": (_cross_vm_existing_wal_warned_lock, "_cross_vm_existing_wal_warned_paths", logging.ERROR,
        ...
        "concurrent writers can silently corrupt the database. Haishui does not live-downgrade an on-disk WAL database. "
        "Fix one of two ways: stop every Haishui process using this database and run a one-time offline "
...
def apply_database_pragmas(conn: sqlite3.Connection, *, db_label: str = "state.db") -> None:
    ...
    try:
        from haishui_cli.config import cfg_get, load_config_readonly  # local: avoids a circular import
