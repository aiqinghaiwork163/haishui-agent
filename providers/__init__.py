...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with haishui-agent)
2. User plugins: ``$HAISHUI_HOME/plugins/model-providers/<name>/``
3. Pip-installed plugins: distributions exposing a ``haishui_agent.plugins``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$HAISHUI_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$HAISHUI_HOME/plugins/model-providers/`` if it exists."""
    try:
        from haishui_constants import get_haishui_home

        d = get_haishui_home() / "plugins" / "model-providers"
...
def _installed_plugins_dir() -> Path | None:
    """Return ``$HAISHUI_HOME/plugins/`` if it exists.

    This is where ``haishui plugins install`` clones a plugin — flat, one
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple HAISHUI_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_haishui_user_provider_{safe_name}"
...
def _discover_entry_point_providers() -> None:
    """Import pip-installed provider plugins via the ``haishui_agent.plugins``
    ...
    * **Provider targets only.** The ``haishui_agent.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$HAISHUI_HOME``) keep their
    ...
    try:
        from haishui_cli.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$HAISHUI_HOME/plugins/model-providers/<name>/``
      2b. Plugins installed by ``haishui plugins install`` at
          ``$HAISHUI_HOME/plugins/<name>/`` that declare ``kind: model-provider``
    ...
    # 0. Pip-installed plugins — entry points in the ``haishui_agent.plugins``
    ...
    #    filesystem steps means a bundled or ``$HAISHUI_HOME`` profile of the
    ...
    # 1. Bundled plugins — shipped with haishui-agent.
    ...
    # 2. User plugins — under $HAISHUI_HOME/plugins/model-providers/<name>/.
    ...
    # 2b. Plugins installed by ``haishui plugins install`` / the plugin index.
    #     Those clone into $HAISHUI_HOME/plugins/<name>/ — flat, NOT under
