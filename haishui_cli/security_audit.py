"""On-demand supply-chain audit for Haishui Agent installs.
...
def _discover_plugins(haishui_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.haishui/plugins``. Plugins typically don't install
...
def _discover_components(
    *, skip_venv: bool = False, skip_plugins: bool = False, skip_mcp: bool = False, haishui_home: Optional[Path] = None
) -> list[Component]:
    """Discover all scannable components across the enabled sources."""
    home = haishui_home or Path(get_haishui_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `haishui security audit`."""
    ...
    components = _discover_components(haishui_home=home, **skips)
    ...
    try:
        findings = run_audit(haishui_home=home, components=components)
