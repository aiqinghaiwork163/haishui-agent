...
# ``haishui update`` is a single-threaded CLI command; a module singleton lets the 7k-line updater
...
class UpdateReceipt:
    """Collects the observable facts of one ``haishui update`` run."""
    ...
    def gateway_restart_result(
        ...
        if fresh_recovery is not None:
            ...
            # ``haishui serve`` hosts tui_gateway and is not a gateway profile, so neither the
...
def _receipt_dir() -> Path:
    # ``haishui_constants`` (stdlib-only), never ``haishui_cli.config``: the receipt must be
    ...
    return get_haishui_home() / "logs" / "update_receipts"
...
def finalize_pending_update_receipt(exit_code: Optional[int] = None, stop_reason: str = "") -> Optional[Path]:
    ...
    ``haishui update`` has many early ``sys.exit`` paths (preflight refusals, venv-holder refusal,
...
def _profile_homes() -> list[tuple[str, Path]]:
    ...
    from haishui_cli.profiles import _get_default_haishui_home, _get_profiles_root, _PROFILE_ID_RE
...
_FLEET_ROW_IDENTITY_PENDING = (
    ...
    " — re-check with `haishui gateway status`"
)
...
def print_fleet_version_matrix(fleet: list[dict[str, Any]]) -> bool:
    ...
    if stale_or_down:
        ...
        print("  Run `haishui gateway restart` (or `haishui -p <profile> gateway restart` for a named")
        print("  profile), then `haishui gateway status` to confirm.")
