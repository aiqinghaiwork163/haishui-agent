"""Per-profile view of ``haishui_cli.models``' module-level catalog slots.
...
files. In a multiplexed gateway every turn runs under a HAISHUI_HOME override, so a single module slot
...
def profile_slot_get(module: Any, attr: str, default: Any = None) -> Any:
    ...
    return _SLOTS_BY_HOME.get((haishui_home_key(), attr), default)
...
def profile_slot_set(module: Any, attr: str, value: Any) -> None:
    if get_haishui_home_override() is None:
        ...
        _SLOTS_BY_HOME[(haishui_home_key(), attr)] = value
