"""Cross-process mutual exclusion for in-flight Haishui updates.
...
MARKER_NAME = ".haishui-update-in-progress"

# Set by an orchestrating updater (Tauri `haishui-setup --update`) to its own pid before
# spawning `haishui update` as a child stage; the parent holds the marker for its whole run,
...
HANDOFF_PID_ENV = "HAISHUI_UPDATE_HANDOFF_PID"
...
# Tauri updater (UPDATE_EXIT_CONCURRENT in update.rs) to show "Haishui is still running".
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Haishui home (never the context-local profile override): the Rust
    updater resolves ``$HAISHUI_HOME`` or the platform default and the desktop pins that same
    ...
    return get_process_haishui_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``haishui update`` as a (grand)child, so a live marker
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Haishui update is already running (started {elapsed} ago, "
        f"process {holder.pid}).\n"
        "\n"
        "  Running two at once would corrupt the install. Wait for it to finish\n"
        "  (watch `haishui logs`), or close the Desktop/dashboard window that\n"
        "  started it, then run `haishui update` again."
    )
