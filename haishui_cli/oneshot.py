...
Toolsets = explicit --toolsets, else the user's "cli" toolsets from `haishui tools`. Rules /
memory / AGENTS.md / preloaded skills = same as a normal chat turn. Approvals are auto-bypassed
(HAISHUI_YOLO_MODE=1). Model/provider mirror `haishui chat`: both optional; only --model → auto-detect
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"haishui -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in _ALL_TOOLSETS for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "haishui -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"haishui -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "haishui -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )
    if not valid:
        return None, "haishui -z: --toolsets did not contain any valid toolsets.\n"
    return valid, None
...
def run_oneshot(
    ...
    Model/provider fall back to ``HAISHUI_INFERENCE_MODEL`` and config.yaml. ``usage_file`` gets a
    ...
    env_model_early = os.getenv("HAISHUI_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "haishui -z: --provider requires --model (or HAISHUI_INFERENCE_MODEL). "
    ...
    # Same finite-chat marker as `haishui chat -q` (cli.py): the session-source resolver uses it to drop an
    ...
    if failure is not None:
        ...
        real_stderr.write(f"haishui -z: agent failed: {failure}\n")
        real_stderr.flush()
        return 1
    ...
    if exit_code == 1:
        real_stderr.write("haishui -z: no final response was produced; treating the run as failed.\n")
        real_stderr.flush()
    return exit_code


def _create_session_db_for_oneshot():
    """Best-effort SessionDB — oneshot bypasses ``HaishuiCLI._init_agent()``, so it must wire the
...
def _resolve_model_and_provider(cfg: dict, model: Optional[str], provider: Optional[str]) -> _ModelChoice:
    ...
    env_model = os.getenv("HAISHUI_INFERENCE_MODEL", "").strip()
    ...
    from haishui_cli import model_switch as _ms
    try:
    ...
    if direct is None:
        ...
        current_provider = cfg_provider or os.getenv("HAISHUI_INFERENCE_PROVIDER", "").strip().lower() or "auto"
        # Same owner as HaishuiCLI startup: a provider-qualified string (``custom:<name>:<model>``,
...
def _load_resume_target(session_db, resume: Optional[str]) -> tuple[Optional[str], list, Optional[dict]]:
    ...
    nothing but is recorded under the requested session — ``haishui -z "hello" -c <title>
...
def _apply_stored_session_runtime(
    ...
    if explicit_model:
        return choice
    from haishui_cli.cli_model_switch_mixin import stored_session_route
...
def _run_agent(
    ...
    from haishui_cli.mcp_startup import ensure_mcp_discovery_before_agent_build
    ...
    try:
        agent = AIAgent(
            ...
            # HAISHUI_INTERACTIVE (never set), hook approval via HAISHUI_ACCEPT_HOOKS=1, dangerous
            # commands via HAISHUI_YOLO_MODE=1, skill secret capture degrades gracefully.
