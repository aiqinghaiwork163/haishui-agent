"""``haishui portal`` — the human-readable entry point for Nous Portal."""
...
DOCS_URL = "https://aiqinghaiwork163.github.io/haishui-agent/docs/user-guide/features/tool-gateway"
...
def _cmd_status(args) -> int:
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `haishui model`)")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `haishui portal`.", Colors.YELLOW))
        print()
...
def _cmd_login(args) -> int:
    ...
    Reuses the exact wiring behind ``haishui setup --portal`` so the commands stay in lockstep.
    """
    from haishui_cli.setup import _run_portal_one_shot
...
# Default (None/"") is the one-shot onboarding (alias for `haishui auth add nous --type oauth` /
# `haishui setup --portal`). `status` kept as a back-compat alias for `info`.
_SUBCOMMANDS = {
    None: _cmd_login,
    "": _cmd_login,
    "login": _cmd_login,
...
def portal_command(args) -> int:
    """Top-level dispatch for `haishui portal <subcommand>`."""
    sub = getattr(args, "portal_command", None)
    handler = _SUBCOMMANDS.get(sub)
    if handler is not None:
        return handler(args)
    print(f"Unknown portal subcommand: {sub}", file=sys.stderr)
    print("Run `haishui portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `haishui portal` on the given argparse subparsers object."""
    portal_parser = subparsers.add_parser(
        "portal",
        help="Set up Nous Portal (login, model pick, Tool Gateway); see also `portal info`",
        description=(
            "Run `haishui portal` with no subcommand to log in to Nous Portal "
            "and set it up — pick a model, set Nous as your provider, and offer "
            "the Tool Gateway (the human-readable alias for `haishui auth add "
            "nous --type oauth`, identical to `haishui setup --portal`). "
            "Subcommands: login (default), info, open, tools."
        ),
    )
    portal_sub = portal_parser.add_subparsers(dest="portal_command")

    # `status` is a hidden (no help) back-compat alias; registration order = `haishui portal -h` order.
