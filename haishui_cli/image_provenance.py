"""Image-authored deployment provenance for immutable Haishui runtimes.

The image bakes ``/etc/haishui/image-provenance.json`` outside ``$HAISHUI_HOME`` and the checkout, so a
...
IMAGE_PROVENANCE_PATH = Path("/etc/haishui/image-provenance.json")
