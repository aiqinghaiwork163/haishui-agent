"""``haishui plugins validate`` — admission checks for a plugin directory.
...
scratch child process (with a throwaway ``HAISHUI_HOME``), so a crashing or
...
_PROBE_TIMEOUT = 30
_PROBE_SENTINEL = "HAISHUI_VALIDATE_JSON:"
...
def _requires_haishui_spec_valid(spec: str) -> bool:
    """Strictly validate a ``requires_haishui`` spec.

    Unlike :func:`haishui_cli.plugins_manifest.version_satisfies` (permissive at load
    ...
    """
    from haishui_cli.plugins_manifest import _VERSION_COMPARATOR_RE, _version_tuple
...
def _check_requires_haishui(report: ValidationReport, manifest: dict) -> None:
    spec = str(manifest.get("requires_haishui") or "").strip()
    if not spec:
        report.add("requires_haishui", True, "not declared")
        return
    if _requires_haishui_spec_valid(spec):
        report.add("requires_haishui", True, f"spec {spec!r} parses")
    else:
        report.add(
            "requires_haishui",
            False,
            f"requires_haishui spec {spec!r} does not parse "
...
# from haishui so a hostile plugin only sees a bare interpreter — the one
...
class RecordingContext:
    ...
    plugin_id = "haishui_validate_probe_plugin"
...
def _run_capability_probe(plugin_dir: Path, manifest: dict) -> Tuple[Optional[dict], str]:
    ...
    """
    with tempfile.TemporaryDirectory(prefix="haishui-validate-") as scratch:
        env = dict(os.environ)
        env["HAISHUI_HOME"] = scratch
        try:
...
def validate_plugin_dir(plugin_dir: Path) -> ValidationReport:
    ...
    _check_requires_haishui(report, manifest)
...
def _check_loadable(report: ValidationReport, plugin_dir: Path) -> None:
    """A plugin.yaml with nothing beside it that Haishui can load (no ``register()`` module, no
...
def _check_python_dependencies(report: ValidationReport, plugin_dir: Path) -> None:
    ...
    from haishui_cli.plugin_python_deps import read_declaration
    ...
    if decl.external:
        ...
        return
    from haishui_cli.plugin_python_deps import applicable_specs, unsupported_specs
