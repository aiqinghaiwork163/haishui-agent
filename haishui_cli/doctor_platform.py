"""Host-platform checks for haishui doctor: interpreter, SQLite, certificates, macOS TCC, gateway supervision, command install.
Split out of ``haishui_cli/doctor.py``, which re-exports every name so ``haishui_cli.doctor.<name>`` keeps resolving (and monkeypatching)."""
...
from haishui_cli.config import is_nix_install_method, recommended_update_command_for_method
from haishui_cli.doctor_report import (
    Finding, _fail_and_issue, _section, check_bool, check_fail, check_info, check_ok, check_warn, doctor_check,
    warn_on_error,
)
from haishui_constants import is_termux as _is_termux
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    from haishui_cli.doctor import PROJECT_ROOT
    from haishui_cli.config import detect_install_method
    ...
    action = cmd if is_nix_install_method(method) else {  # nix: prose guidance, not a shell command
        "docker": f"run `{cmd}`, then recreate all Haishui containers", "apt": f"run `{cmd}`"}.get(method, "run `haishui update`")
    return f"({action}; fixed versions: 3.51.3+ / 3.50.7 / 3.44.6 — see https://sqlite.org/wal.html#walresetbug)"


def _haishui_database_paths(haishui_home: Path) -> list[tuple[str, Path]]:
    """(display name, path) pairs for Haishui-managed SQLite databases: backup.py's per-profile store list + per-board kanban.db."""
    from haishui_cli.backup import _QUICK_STATE_FILES
    entries = [(name, haishui_home / name) for name in _QUICK_STATE_FILES if name.endswith(".db")]
    for board_db in sorted((haishui_home / "kanban" / "boards").glob("*/kanban.db")):
        entries.append((str(board_db.relative_to(haishui_home)), board_db))
    return entries
...
def _read_journal_mode(db_path: Path) -> tuple[str | None, str | None]:
    ...
    process's POSIX advisory locks (see ``haishui_cli.sqlite_safe_read``), and the dashboard console runs
    ``run_doctor`` in-process with live ``SessionDB`` connections — the helper refuses then (unreadable).
    """
    from haishui_cli.sqlite_safe_read import has_live_connection, read_header_bytes_preopen
...
def _report_database_holders(name: str, db_path: Path) -> None:
    ...
    from haishui_state_holders import _read_proc_argv, foreign_state_db_holders, psutil
...
def _report_database_journal_modes(haishui_home: Path | None = None, version_info: tuple[int, ...] | None = None) -> None:
    ...
    from haishui_cli.doctor import HAISHUI_HOME
    ...
    try:
        databases = _haishui_database_paths(haishui_home if haishui_home is not None else HAISHUI_HOME)
    except Exception as exc:
        check_warn(f"Could not list Haishui databases: {exc}")
        return
    exposed = []
    for name, path in databases:
        ...
        if error is None and mode == "wal" and configured == "delete":
            ...
            check_warn(f"{name} is in WAL mode ({size}) despite database.journal_mode=delete",
                       ...
                       + ". Stop every Haishui process for this profile, then run a one-time offline "
            ...
            check_warn(f"{name} is in WAL mode on a cross-VM filesystem (virtiofs/9p, {size})",
                       "(WAL can silently corrupt across the VM boundary; stop every Haishui process and run a one-time "
...
def _check_version_consistency(issues: list[str]) -> None:
    """Detect pyproject.toml vs haishui_cli.__version__ drift (a conflict resolution can revert one but not the
    other). Silent no-op for installed wheels (no pyproject)."""
    try:
        from haishui_cli import __version__ as init_version
    except Exception:
        return
    ...
    _fail_and_issue("Version mismatch between source files", f"(pyproject.toml {pyproject_version} != haishui_cli/__init__.py {init_version})",
                    "Re-sync version files (e.g. run 'haishui update', or set haishui_cli/__init__.py __version__ to match pyproject.toml)", issues)
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    try:
        from haishui_cli.service_manager import S6ServiceManager, detect_service_manager
    except Exception:
        return
    ...
    for static in ("main-haishui", "dashboard"):  # s6-rc symlinks under /run/service/, same s6-svstat probe
    ...
    if not profiles:
        return check_info("No per-profile gateways registered yet — create one with `haishui profile create <name>`")
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        issues.append(f"Repair the CA bundle: run `haishui doctor --fix`, or `{pip_cmd}`")
        return
...
def _check_gateway_service_linger(issues: list[str]) -> None:
    ...
    try:
        from haishui_cli.gateway import get_systemd_linger_status, get_systemd_unit_path, is_linux
...
_TCC_CDHASH_DETAIL = (
    ...
    "all permission grants. Run `haishui update` to get the stable identifier-pinned signing identity, "
    "then re-grant permissions once.")
_TCC_STABLE_DETAIL = {
    True: "(certificate-anchored DR; grants survive rebuilds)",
    False: "(identifier-pinned DR; grants survive rebuilds — for the strongest anchor, see `haishui desktop --setup-tcc-identity`)",
}
...
def check_macos_tcc_grants() -> None:
    ...
    check_info("If macOS still re-prompts for permissions (toggle shows ON): the stored grant is stale — run "
               "`tccutil reset ScreenCapture com.nousresearch.haishui` (repeat per affected service), toggle it ON in "
               "System Settings, then fully quit & relaunch Haishui once.")


def _desktop_app_bundle() -> Path | None:
    """Locate the locally-built desktop bundle (``apps/desktop/release/mac-<arch>/Haishui.app``), newest first.

    The only layout whose ad-hoc re-signed bundle can invalidate TCC grants. ``/Applications/Haishui.app`` is
    deliberately not probed: it is the separately-signed, certificate-anchored Haishui-Setup launcher.
    """
    release_dir = Path(__file__).resolve().parents[1] / "apps" / "desktop" / "release"
    candidates = [p for p in release_dir.glob("mac*/Haishui.app") if p.is_dir()]
...
def check_macos_tcc_anchor(should_fix: bool = False) -> None:
    ...
    with warn_on_error("macOS TCC anchor check failed"):
        from haishui_cli import macos_tcc_anchor as tcc
...
def check_macos_full_disk_access() -> None:
    ...
    try:
        ...
        check_info("One switch silences all macOS folder prompts: grant your terminal app Full Disk Access and Haishui "
                   ...
                   "    then enable your terminal (and Haishui.app if you use Desktop), and restart them once. "
                   "With Haishui' stable signing identities the grant survives every update.")
    except OSError:
...
def _check_security_advisories(should_fix: bool, f: Finding) -> None:
    ...
    from haishui_cli.security_advisories import detect_compromised, filter_unacked, full_remediation_text, get_acked_ids
    ...
    for hit in fresh_hits:
        ...
        _fail_and_issue(f"{hit.advisory.title}", f"({hit.package}=={hit.installed_version})",
                        ...
                        f"and rotate credentials, then run `haishui doctor --ack {hit.advisory.id}`.", f.manual_issues)
...
def _check_python_environment(should_fix: bool, f: Finding) -> None:
    ...
    with warn_on_error("SQLite version probe failed: {e}", ""):
        import sqlite3
        from haishui_state_wal import is_sqlite_wal_reset_vulnerable, sqlite_source_id
        src = sqlite_source_id()
        # Warn-only: Haishui already refuses WAL on fresh DBs and runtime repair is best-effort.
...
def _check_command_installation(should_fix: bool, f: Finding) -> None:
    ...
    venv_bin = next((c for c in (PROJECT_ROOT / n / "bin" / "haishui" for n in ("venv", ".venv")) if c.exists()), None)
    if venv_bin is None:
        check_warn("Venv entry point not found", "(haishui not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')")
    ...
    link = link_dir / "haishui"
    if link.is_symlink():
        target, expected = link.resolve(), venv_bin.resolve()
        if target == expected:
            return check_ok(f"{display}/haishui → correct target")
        check_warn(f"{display}/haishui points to wrong target", f"(→ {target}, expected → {expected})")
        if not should_fix:
            return f.issues.append(f"Broken symlink at {display}/haishui — run 'haishui doctor --fix'")
        link.unlink()
        verb = "Fixed"
    elif link.exists():  # regular file (wrapper script), not a symlink
        return check_ok(f"{display}/haishui exists (non-symlink)")
    else:
        check_fail(f"{display}/haishui not found", "(haishui command may not work outside the venv)")
        if not should_fix:
            return f.issues.append(f"Missing {display}/haishui symlink — run 'haishui doctor --fix'")
        link_dir.mkdir(parents=True, exist_ok=True)
        verb = "Created"
    link.symlink_to(venv_bin)
    check_ok(f"{verb} symlink: {display}/haishui → {venv_bin}")
    f.fixed += 1
