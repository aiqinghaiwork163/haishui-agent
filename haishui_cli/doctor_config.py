"""Configuration-file checks for haishui doctor: .env, config.yaml validation, drift, deprecations.
Split out of ``haishui_cli/doctor.py``."""
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.haishui/.env contains provider auth/base URL settings."""
    from haishui_cli.doctor import _PROVIDER_ENV_HINTS
...
# TERMINAL_CWD do not false-positive). HAISHUI_TOOL_PROGRESS is silently ignored since the v12 config floor
# removed its only consumer; HAISHUI_TOOL_PROGRESS_MODE is still read by the gateway as a back-compat fallback.
_DEPRECATED_ENV_VARS: tuple[tuple[str, str], ...] = (
    ("HAISHUI_TOOL_PROGRESS", "display.tool_progress in config.yaml — ignored/unsupported since config floor v12"),
    ("HAISHUI_TOOL_PROGRESS_MODE", "display.tool_progress in config.yaml"),
...
def collect_relay_plugin_cutover_findings(raw_config: dict | None, env_map: dict | None) -> list[tuple[str, str]]:
    """Return actionable findings for the removed Haishui Relay plugin."""
    from haishui_cli.relay_plugin_cutover import (LEGACY_RELAY_EXPORT_ENV_VARS, RELAY_PLUGINS_CONFIG_ENV,
    ...
    if not str(effective_env.get(RELAY_PLUGINS_CONFIG_ENV, "")).strip():
        findings += [(name, f"run `haishui migrate relay` to generate relay-plugins.toml and set {RELAY_PLUGINS_CONFIG_ENV}; "
...
def managed_scope_check() -> None:
    """Report the active managed scope (resolved dir + pinned key counts); silent when none. A HAISHUI_MANAGED_DIR
    ...
    if os.environ.get("HAISHUI_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via HAISHUI_MANAGED_DIR={managed_dir}")
...
def _check_env_file(should_fix: bool, f: Finding) -> None:
    """Managed scope plus ~/.haishui/.env presence and provider credentials."""
    from haishui_cli.doctor import HAISHUI_HOME, PROJECT_ROOT, _DHH
    managed_scope_check()
    env_path = HAISHUI_HOME / '.env'
    if env_path.exists():
        ...
        if not check_bool(_has_provider_env_config(content), "API key or custom endpoint configured", f"No API key found in {_DHH}/.env"):
            f.issues.append("Run 'haishui setup' to configure API keys")
        ...
        if should_fix:
            ...
            check_info("Run 'haishui setup' to configure API keys")
            f.fixed += 1
        else:
            check_info("Run 'haishui setup' to create one")
            f.issues.append("Run 'haishui setup' to create .env")
...
def _known_provider_ids(cfg: dict) -> tuple[set, list, object, object, object]:
    ...
    with warn_on_error(""):
        from haishui_cli.auth import PROVIDER_REGISTRY, resolve_provider as resolve_auth
    ...
    with warn_on_error(""):
        ...
        from haishui_cli.providers import custom_provider_aliases as aliases, normalize_provider as normalize, resolve_provider_full as resolve_full
        with warn_on_error(""):
...
def _validate_model_config(config_path, issues: list) -> None:
    ...
    if provider and provider != "auto" and (catalog_provider is None or (known_providers and not (accept & valid_provider_ids))):
        ...
        _fail_and_issue(f"model.provider '{provider_raw}' is not a recognised provider", f"(known: {known_list})",
                        ...
                        f"Fix: run 'haishui config set model.provider <valid_provider>'", issues)
    ...
    if runtime_provider and runtime_provider not in ("auto", "custom"):
        from haishui_cli.doctor import _DHH
        with warn_on_error(""):
            if not _provider_has_credentials(runtime_provider):
                _fail_and_issue(f"model.provider '{runtime_provider}' is set but no API key is configured",
                                f"(add it to {_DHH}/.env or run 'haishui setup')",
                                f"No credentials found for provider '{runtime_provider}'. Run 'haishui setup' or set the provider's "
                                f"API key in {_DHH}/.env, or switch providers with 'haishui config set model.provider <name>'", issues)
...
def _check_config_file(should_fix: bool, f: Finding) -> None:
    ...
    from haishui_cli.doctor import HAISHUI_HOME, PROJECT_ROOT, _DHH
    config_path = HAISHUI_HOME / 'config.yaml'
    if config_path.exists():
        ...
        if example_config.exists():
            ...
            from haishui_cli.config import DEFAULT_CONFIG, save_config
...
def _drift_config_version(f: Finding, should_fix: bool, config_path) -> None:
    ...
    if check_bool(current_ver >= latest_ver, f"Config version up to date (v{current_ver})", outdated):
        return
    if not should_fix:
        f.issues.append("Run 'haishui doctor --fix' or 'haishui setup' to migrate config")
        return
    try:
        ...
        f.issues.append("Run 'haishui setup' to migrate config")
...
def _drift_stale_root_keys(f: Finding, should_fix: bool, config_path) -> None:
    ...
    from haishui_cli.config import atomic_config_write, read_user_config_raw
    ...
    if not should_fix:
        f.issues.append("Stale root-level provider/base_url in config.yaml — run 'haishui doctor --fix'")
        return
...
def _drift_max_iterations_ghost(f: Finding, should_fix: bool, config_path) -> None:
    """A stale HAISHUI_MAX_ITERATIONS in .env shadows agent.max_turns in config.yaml.

    The setup wizard used to dual-write the budget. The gateway bridge derives HAISHUI_MAX_ITERATIONS from
    ...
    """
    from haishui_cli.doctor import _DHH
    # Detect stale HAISHUI_MAX_ITERATIONS ghost in .env shadowing agent.max_turns in config.yaml (issue
    # #17534). The setup wizard used to dual-write the iteration budget to both stores; users who later edit
    # only config.yaml are left with a .env ghost. The gateway bridge normally derives HAISHUI_MAX_ITERATIONS
    ...
    from haishui_cli.config import load_env, read_user_config_raw, remove_env_value
    raw_config = read_user_config_raw(config_path)
    ...
    if cfg_max_turns is None or env_ghost is None or str(cfg_max_turns).strip() == str(env_ghost).strip():
        return
    check_warn(f"HAISHUI_MAX_ITERATIONS={env_ghost} in .env shadows agent.max_turns={cfg_max_turns} in config.yaml",
               "(stale ghost from an earlier `haishui setup` run)")
    if not should_fix:
        f.issues.append("Stale HAISHUI_MAX_ITERATIONS in .env shadows config.yaml — run 'haishui doctor --fix'")
    elif remove_env_value("HAISHUI_MAX_ITERATIONS"):
        check_ok(f"Removed stale HAISHUI_MAX_ITERATIONS from .env (config.yaml agent.max_turns={cfg_max_turns} is now authoritative)")
        f.fixed += 1
    else:
        check_warn("Could not remove HAISHUI_MAX_ITERATIONS from .env")
        f.manual_issues.append(f"Manually delete the HAISHUI_MAX_ITERATIONS line from {_DHH}/.env — config.yaml agent.max_turns is authoritative.")
...
def _drift_deprecations(f: Finding, should_fix: bool, config_path) -> None:
    ...
    from haishui_cli.config import load_env, read_user_config_raw
...
def _check_config_drift(should_fix: bool, f: Finding) -> None:
    """Config version, stale root keys, HAISHUI_MAX_ITERATIONS ghost, deprecations, structure.
...
def _check_xai_retirement(should_fix: bool, f: Finding) -> None:
    from haishui_cli.config import load_config
    from haishui_cli.xai_retirement import MIGRATION_GUIDE_URL, find_retired_xai_refs, format_issue
...
def _check_plugin_compat(should_fix: bool, f: Finding) -> None:
    from haishui_cli.plugin_compat import ALLOW_KEY, COMPAT_REMOVAL, compat_report, removal_in_effect
    ...
    check_info("Details: haishui plugins compat")
    f.manual_issues.append(
        f"Update {len(report)} plugin(s) still importing pre-decomposition paths (haishui plugins compat) — "
