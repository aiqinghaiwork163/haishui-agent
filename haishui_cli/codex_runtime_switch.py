...
_ARG_SYNONYMS = {
    ...
    "off": "auto", "default": "auto", "disable": "auto", "haishui": "auto"}

_HAISHUI_TOOLS_CALLBACK_NOTE = (
    "Haishui tool callback registered: codex can now use "
    "web_search, web_extract, browser_*, vision_analyze, "
    "image_generate, skill_view, skills_list, text_to_speech, "
    "kanban_* (worker + orchestrator) via MCP.",
    "  (delegate_task, memory, session_search, todo run "
    "only on the default Haishui runtime — they need the "
    "agent loop context.)")
...
def _migration_lines(config: dict) -> list[str]:
    ...
    lines: list[str] = []
    try:
        from haishui_cli.codex_runtime_plugin_migration import HAISHUI_TOOLS_MCP_SERVER_NAME, migrate
        mig_report = migrate(config)
        # The haishui-tools callback is internal plumbing — surfaced separately below.
        user_servers = [s for s in mig_report.migrated if s != HAISHUI_TOOLS_MCP_SERVER_NAME]
        if user_servers:
        ...
        if HAISHUI_TOOLS_MCP_SERVER_NAME in mig_report.migrated:
            lines.extend(_HAISHUI_TOOLS_CALLBACK_NOTE)
...
def apply(
    ...
    # correct but the world state (managed block in ~/.codex/config.toml, haishui-tools MCP
    ...
    if new_value == "codex_app_server":
        ...
        # Migrate Haishui' MCP servers + Codex's curated plugins into ~/.codex/config.toml so the
        # spawned codex subprocess sees the same tool surface AND can call back into Haishui.
        ...
        msg_lines.append(
            ...
            "Haishui tools available via MCP callback).")
        msg_lines.append(
            "Effective on next session — current cached agent keeps "
            "the prior runtime to preserve prompt cache.")
    else:
        msg_lines.append("OpenAI/Codex turns will use the default Haishui runtime.")
