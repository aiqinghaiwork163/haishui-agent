"""ZIP-download fallback for ``haishui update`` (Windows with broken git): two-phase stage/commit swap, dirty-tree guard.

Split out of ``update_cmd.py``; every name is re-imported there so ``haishui_cli.update_cmd.<name>`` keeps
...
_ZIP_STAGING_ARTIFACT_SUFFIXES = ".haishui-update-staging", ".haishui-update-old"
...
_STASH_HINT = "  Stash or commit your changes, then rerun `haishui update`."
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    Thin alias over the two-phase helpers; retained for the ``haishui_cli.main`` re-export surface.
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    staging = f"{dst}.haishui-update-staging"
    backup = f"{dst}.haishui-update-old"
...
def _zip_overlay_block_reason(root: Path, *, ignore_staging_artifacts: bool = False) -> Optional[str]:
    ...
    is for the pre-swap re-check: phase 1 leaves our own ``*.haishui-update-staging`` siblings that git
...
def _extracted_root(tmp_dir: str, branch: str) -> str:
    """GitHub ZIPs extract to ``haishui-agent-<branch>/``; fall back to the first non-``__MACOSX`` dir."""
    extracted = os.path.join(tmp_dir, f"haishui-agent-{branch}")
...
def _require_staging_space(extracted: str, entries: list[str], project_root: str) -> None:
    ...
    need = sum(
        ...
        # root holds 20 first-party modules (run_agent.py, cli.py, haishui_constants.py, ...). Check up front
...
def _stage_entries(extracted: str, entries: list[str], project_root: str) -> list[tuple[str, str]]:
    ...
    try:
        for item in entries:
            ...
            # desktop app, win-unpacked/ Haishui.exe) exists only in the LIVE tree. Graft the live release
...
def _download_and_swap_zip(branch: str, zip_url: str) -> None:
    ...
    tmp_dir = tempfile.mkdtemp(prefix="haishui-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"haishui-agent-{branch}.zip")
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False, _windows_gateway_resume=None) -> bool:
    ...
    from haishui_cli.update_cmd import _hand_off_post_swap, _m, _read_project_version, _resolve_update_options, _sweep_bytecode_after_update
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `haishui update "
            f"--branch {branch}`, or update against main with `haishui update`."
        )
        _m().sys.exit(1)
...
def _finish_zip_update(
    *, active_tool_dependencies, pre_update_version, had_desktop_app_before_update: bool,
    ...
    if not import_ok:
        ...
        print("  Re-run `haishui update` to complete it.")
        _m().sys.exit(1)
