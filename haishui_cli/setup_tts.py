...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    API-key prompt for users who logged in via ``haishui model`` -> xAI Grok OAuth."""
    try:
...
def _xai_api_key_path():
    ...
    return ("No xAI API key provided for TTS. Configure XAI_API_KEY via haishui setup model "
    ...
    if selected == "nous-openai":
        ...
        if _setup.get_env_value("VOICE_TOOLS_OPENAI_KEY") or _setup.get_env_value("OPENAI_API_KEY"):
            _setup.print_warning("Direct OpenAI credentials are still configured and may take precedence "
                                 "until removed from ~/.haishui/.env.")
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'haishui setup tts')."""
