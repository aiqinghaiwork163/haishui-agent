...
DEFAULT_CODEX_MODELS: List[str] = [
    ...
    # not in the public API, so it stays out of the "openai" catalog in haishui_cli/models.py.
...
# OpenAI API, so it intentionally stays out of the "openai" provider catalog in haishui_cli/models.py — only
...
_FORWARD_COMPAT_TEMPLATE_MODELS: List[tuple[str, tuple[str, ...]]] = [
    ...
    # Spark surfaces whenever a compatible template is present; the backend (not Haishui)
...
def _add_context_variants(model_ids: List[str]) -> List[str]:
    ...
    The suffix is Haishui-side only — stripped before the id hits the wire (agent/transports/codex.py,
