...
DeepInfra pricing adapters. Split out of ``haishui_cli.models``; helpers still defined there are
looked up on ``haishui_cli.models`` at call time so ``patch("haishui_cli.models.<name>")`` mocks keep
intercepting.
"""
...
from haishui_cli.models_reasoning_caps import _seed_reasoning_caps
...
def _get_json(url: str, headers: dict[str, str], timeout: float, opener=None) -> Optional[dict]:
    ...
    from haishui_cli.models import _urlopen_model_catalog_request
...
def _pricing_entry(pricing: dict, prompt_key: str = "prompt", completion_key: str = "completion") -> dict[str, Any]:
    ...
    ``pricing`` block whose cache fields already use the haishui names."""
    entry: dict[str, Any] = {
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    from haishui_cli.models import _HAISHUI_USER_AGENT
    ...
    headers = {"Accept": "application/json", "User-Agent": _HAISHUI_USER_AGENT}
    if api_key:
...
def nous_policy_allowed_ids(*, force_refresh: bool = False) -> Optional[set[str]]:
    ...
    try:
        from haishui_cli.nous_account import nous_policy_present
...
def _remember_provider_cache_key(provider: str, cache_key: str) -> None:
    from haishui_cli.models import _pricing_profile_key
...
def pricing_cache_scope(provider: str, *, current_provider: str = "", current_base_url: str = "") -> str:
    ...
    from haishui_cli.models import _deepinfra_catalog_url, _pricing_profile_key, normalize_provider
...
def _cached_only_pricing(normalized: str) -> dict[str, dict[str, str]]:
    ...
    from haishui_cli.models import _deepinfra_catalog_cache, _deepinfra_catalog_url, _pricing_profile_key
    if normalized == "deepinfra":
...
def _fetch_novita_pricing(timeout: float = 8.0, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    ...
    headers = {"Authorization": f"Bearer {api_key}", "Accept": "application/json", "User-Agent": _HAISHUI_USER_AGENT}
