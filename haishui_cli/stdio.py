...
def configure_windows_stdio() -> bool:
    ...
    ``HAISHUI_DISABLE_WINDOWS_UTF8=1`` to opt out (forces the old cp1252 path for diagnosing
    ...
    if _CONFIGURED:
        return False
    if not is_windows() or os.environ.get("HAISHUI_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
...
def _augment_path_with_known_tools() -> None:
    r"""Prepend Haishui-managed tool directories to ``PATH`` (no-op on POSIX / missing dirs).

    install.ps1 adds entries like ``%LOCALAPPDATA%\haishui\git\bin`` to the User PATH via
    ``SetEnvironmentVariable``, but already-running shells never see that broadcast, so a haishui
    ...
    # dir hosts haishui.exe + pip console scripts; WinGet\Links is where ``winget install`` drops
    # CLI shims (ripgrep lands there as rg.exe).
    candidate_dirs = [
        os.path.join(local_appdata, "haishui", "git", "cmd"),
        os.path.join(local_appdata, "haishui", "git", "bin"),
        os.path.join(local_appdata, "haishui", "git", "usr", "bin"),
        os.path.join(local_appdata, "haishui", "haishui-agent", "venv", "Scripts"),
