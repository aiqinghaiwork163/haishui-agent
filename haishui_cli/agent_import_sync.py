"""``haishui import-agent --sync`` — keep previously imported Claude Code / Codex setups current.

Every successful ``haishui import-agent`` run records its source in ``HAISHUI_HOME/import-sync.json``
...
def _iter_sync_files(agent: str, source_root: Path) -> Iterator[Path]:
    ...
    :class:`~haishui_cli.agent_import.AgentImporter` reads for ``agent``."""
...
def sync_imported_agents(args) -> None:
    """Handle ``haishui import-agent --sync``: re-import every registered source whose digest
    changed. Prompt-free (cron-friendly); ``--dry-run`` previews without touching the manifest."""
    from haishui_cli.agent_import import AgentImporter, print_import_report
    from haishui_cli.setup import print_error, print_header, print_info, print_success
    from haishui_constants import get_haishui_home

    dry_run = bool(getattr(args, "dry_run", False))
    haishui_home = get_haishui_home().resolve()
    agents: Dict[str, Any] = load_sync_manifest(haishui_home).get("agents", {})
    if not agents:
        print()
        print_info("No import sources registered yet.")
        print_info("Run 'haishui import-agent' first — successful imports are "
    ...
    for agent_name in sorted(agents):
        ...
        try:
            report = AgentImporter(agent_name, source_dir, haishui_home, execute=not dry_run,
        ...
        if not dry_run:
            try:
                ...
                update_sync_manifest(agent_name, source_dir, haishui_home, overwrite, report,
