...
Split out of :mod:`haishui_cli.plugins`; validation warns and never fails a load.
"""
...
from haishui_cli.plugin_capabilities import parse_declared_capabilities as _parse_declared_capabilities
...
# and continue loading. ``capabilities``/``emits``/``listens``/``haishui``/``depends`` are reserved.
...
_KNOWN_MANIFEST_FIELDS: Set[str] = {
    ...
    "license", "homepage", "tags", "capabilities", "emits", "listens", "haishui", "depends",
    "requires_haishui", "python_runtime",
}

# Highest manifest schema version this Haishui understands.
...
def _plugins_debug() -> bool:
    from haishui_cli import plugins as _origin
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    if mv > SUPPORTED_MANIFEST_VERSION:
        logger.warning(
            "Plugin %s: manifest_version %d is newer than this Haishui "
        ...
        for dep in manifests[k].requires_plugins:
            ...
            if resolved is None:
                logger.warning(
                    ...
                    "via ctx.has_plugin). Run `haishui plugins enable %s` if it is installed.",
                    k, dep_id, dep_id,
                )
            elif resolved == k:
...
class PluginManifest:
    ...
    kind: str = "standalone"
    # Path-derived registry key used by plugins.enabled/disabled and `haishui plugins list`: ``disk-cleanup``
    # for a flat plugin, ``image_gen/openai`` for a category plugin. Empty -> name.
    key: str = ""
    # Haishui version requirement (``">=0.19"``, comma-separated clauses allowed). Unsatisfied plugins are
    # recorded with an error and skipped before import — see ``requires_haishui_error``.
    requires_haishui: str = ""
    portable: bool = False
    skill_namespace: str = ""
    ...
    # VALIDATED AND SURFACED ONLY — Haishui never auto-installs these (isolation design for the install seam
...
# ── requires_haishui version gate ─────────────────────────────────────────────
_VERSION_COMPARATOR_RE = re.compile(r"^\s*(>=|<=|==|!=|>|<)\s*(.+?)\s*$")


def running_haishui_version() -> str:
    """Installed ``haishui-agent`` distribution version, else ``haishui_cli.__version__`` (source checkout)."""
    try:
        return importlib.metadata.version("haishui-agent")
    except Exception:
...
def requires_haishui_error(manifest: "PluginManifest") -> Optional[str]:
    """Load-blocking reason when the manifest's ``requires_haishui`` rejects the running version."""
    if not manifest.requires_haishui:
        return None
    current = running_haishui_version()
    if version_satisfies(manifest.requires_haishui, current):
        return None
    return f"requires haishui {manifest.requires_haishui}, running {current}"
...
def parse_manifest_file(
    ...
    try:
        ...
        return PluginManifest(
            ...
            kind=kind, key=key, requires_haishui=str(data.get("requires_haishui") or "").strip(),
