"""haishui fallback — manage the fallback provider chain (tried in order when the primary fails).

Subcommands: ``list`` (default), ``add`` (same picker as `haishui model`), ``remove``, ``clear``.
"""
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if chain is None:
        print("  Add one with:  haishui fallback add\n")
        return
    print()
    ...
    print("  Docs: https://aiqinghaiwork163.github.io/haishui-agent/docs/user-guide/features/fallback-providers\n")


def cmd_fallback_add(args) -> None:
    """Launch the same picker as `haishui model`, then append the selection to the chain."""
    from haishui_cli.main import _require_tty, select_provider_and_model
    from haishui_cli.config import load_config, save_config
    ...
    print("\n  Adding a fallback provider.  The picker below is the same one used by\n"
          "  `haishui model` — select the provider + model you want as a fallback.\n")
    ...
    print("  Run `haishui fallback list` to view, or `haishui fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``haishui fallback [subcommand]``."""
