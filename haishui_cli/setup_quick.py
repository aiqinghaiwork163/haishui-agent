"""Streamlined setup flows: the Nous Portal one-shot (`haishui portal`), first-time quick setup,
Blank Slate setup and the `--quick` missing-items pass. Names from setup.py are imported lazily
per function so test patches on ``haishui_cli.setup`` take effect."""
...
logger = logging.getLogger("haishui_cli.setup")
...
def _blank_slate_done(config: dict, haishui_home, tools_line: str, *extra: str, intro: str | None = None) -> None:
    """Shared Blank Slate epilogue: success banner, the "enable later" hints, then the summary."""
    from haishui_cli.setup import _info, _print_setup_summary, print_success
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    _info(*([intro] if intro else []), tools_line, "  Seed skills:         haishui skills opt-in --sync",
          "  Add MCP servers:     haishui mcp add", *extra, "  Tune agent settings: haishui setup agent", None)
...
def _reload_config_into(config: dict, *, dict_only: bool = False) -> None:
    ...
    from haishui_cli.setup import load_config
...
def _run_nous_flow(config: dict, *, context: str, cancel_exc: tuple, cancel_lines: tuple, print_error) -> bool:
    ...
    single source of truth shared with ``haishui model``. False when cancelled or failed (the
    message is already printed)."""
    from haishui_cli.setup import _info
    try:
        from haishui_cli.model_setup_flows import _model_flow_nous
...
def _run_portal_one_shot(config: dict) -> None:
    """One-shot Nous Portal setup (``haishui setup --portal`` / ``haishui portal``)."""
    from haishui_cli.setup import _info, _print_banner, print_error, print_info, print_success
    _print_banner("│     ☤ Haishui Setup — Nous Portal (one-shot)             │")
    ...
    def _on_error(exc: Exception) -> None:
        ...
        print()
        lead, *rest = provider_setup_failure_lines(exc, retry_command="haishui portal")
        print_error(f"  {lead}")
        for line in rest:
            print_info(f"  {line}")

    if not _run_nous_flow(config, context="`haishui portal`", cancel_exc=(KeyboardInterrupt, EOFError, SystemExit),
                          cancel_lines=(None, "  Setup cancelled.", "  You can retry later with `haishui portal`."),
    ...
    _info("  Run `haishui portal info` to inspect routing.", "  Run `haishui` to start chatting.")


def _run_first_time_quick_setup(config: dict, haishui_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice("Connect a messaging platform? (Telegram, Discord, etc.)", [
        "Set up messaging now (recommended)", "Skip — set up later with 'haishui setup gateway'",
    ], 0)
    if gateway_choice == 0:
        ...
        # platforms come alive as soon as tokens are added later (e.g. via `haishui import`).
        from haishui_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
    print()
    print_success("Setup complete! You're ready to go.")
    _info(None, "  Configure all settings:    haishui setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  haishui setup gateway")
...
def _print_macos_fda_tip() -> None:
    ...
    _info(None, "  macOS tip: silence ALL folder permission prompts with one switch —",
          ...
          "  your terminal (and Haishui.app if you use Desktop), or run:",
          "    open \"x-apple.systempreferences:com.apple.preference.security?Privacy_AllFiles\"",
          "  The grant is permanent — it survives every Haishui update.")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    ``vision`` (``read_file`` can't read images) and ``skills`` (the seeded ``haishui-agent`` skill
    ...
    try:
        ...
        from haishui_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
        all_keys = {k for k, _, _ in CONFIGURABLE_TOOLSETS}
        all_keys.update(_get_plugin_toolset_keys())
        # Plain TOOLSETS entries catch recovered toolsets like ``kanban``. Skip "haishui-*" platform
        ...
        for k, tdef in TOOLSETS.items():
            if k.startswith("haishui-") or (isinstance(tdef, dict) and (tdef.get("includes") or tdef.get("posture"))):
                continue
...
def _blank_slate_minimize_config(config: dict):
    """Turn OFF every optional config feature; all opt back in via ``haishui setup agent``."""
...
def _run_blank_slate_setup(config: dict, haishui_home, is_existing: bool):
    ...
    _info("Everything starts OFF. First we force-enable only what's required",
          ...
          "essential `haishui-agent` skill is always kept so the agent",
          "can help you drive and configure Haishui itself.", None)
    ...
    if path != 0:
        _blank_slate_walkthrough(config, haishui_home)
        return
    save_config(config)
    # Blank Slate means no bundled skills; record the opt-out so future `haishui update` runs
    # don't re-inject them.
    _set_bundled_skills_opt_out(True, "skill opt-out")
    _blank_slate_done(config, haishui_home, "  Enable tools:        haishui tools", "  Enable plugins:      haishui plugins",
                      intro="Enable anything later, on demand:")


def _blank_slate_walkthrough(config: dict, haishui_home):
    ...
    def _opted_out(_result) -> None:
        _info("No skills seeded (except the essential `haishui-agent`",
              "skill). A .no-bundled-skills marker keeps future",
              "`haishui update` runs from re-injecting them. Opt back in any",
              "time with `haishui skills opt-in --sync`.")
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `haishui tools`.")
    ...
    for header, question, yes_msg, no_msg in (
        ("Plugins", "Review and enable built-in plugins now?",
         "Manage plugins with `haishui plugins list` / `haishui plugins install`.",
         "No plugins enabled. Add later with `haishui plugins`."),
        ("MCP Servers", "Add an MCP server now?",
         "Add servers with `haishui mcp add <name> --url ... | --command ...`.",
         "No MCP servers configured. Add later with `haishui mcp add`."),
    ):
    ...
    if prompt_yes_no("Connect a messaging platform (Telegram, Discord, …)?", default=False):
        setup_gateway(config)
    save_config(config)
    _blank_slate_done(config, haishui_home, "  Enable more tools:   haishui tools")


def _run_quick_setup(config: dict, haishui_home):
    ...
    from haishui_cli.config import (get_missing_env_vars, get_missing_config_fields, check_config_version)
    ...
    if not (missing_required or missing_optional or missing_config or current_ver < latest_ver):
        ...
        _info(None, "Run 'haishui setup' and choose 'Full Setup' to reconfigure,",
    ...
    if missing_messaging:  # checklist, then prompt for each selected platform's vars
        ...
        _info("Connect Haishui to messaging apps to chat from anywhere.",
              "You can configure these later with 'haishui setup gateway'.")
