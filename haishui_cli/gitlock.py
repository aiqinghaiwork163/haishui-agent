...
class _ShallowLock:
    ...
    def __enter__(self) -> "_ShallowLock":
        ...
        try:
            os.write(fd, b"haishui shallow maintenance\n")
        finally:
            os.close(fd)
        return self
...
def prune_stale_shallow_grafts(repo_root: Path) -> int:
    ...
    break ``merge-base`` and push ``haishui update`` into the orphan-divergence reset path
