...
from haishui_cli.local_runtime.gguf import SPLIT_PART_RE, model_id_from_stem

logger = logging.getLogger(__name__)

_SUPERVISOR = None  # process-wide singleton; one router per Haishui process
...
def _detect_gpu_vendor() -> str | None:
    ...
    from haishui_cli.local_runtime.hardware import _nvidia_smi_path
...
def models_dir() -> Path:
    ...
    return get_default_haishui_root() / "models"
...
def _presets_stale() -> bool:
    ...
    with suppress(Exception):
        from haishui_cli.local_runtime.presets import read_preset_decisions
...
def _stop_state_server(state: dict) -> None:
    ...
    server."""
    from haishui_cli.local_runtime.endpoint import _pid_alive
...
def _generate_presets(mdir: Path, preset_path: Path) -> Path | None:
    ...
    """
    from haishui_cli.local_runtime.hardware import probe_budget
    from haishui_cli.local_runtime.presets import generate_presets
...
def ensure_local_runtime(config: dict, force: bool = False) -> "object | None":
    ...
    # Another Haishui process may already be supervising — reuse via state, but ONLY while its
