...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or haishui -c.",
    ...
    "Ctrl+Z suspends Haishui to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "haishui -c resumes your most recent CLI session. haishui -c \"project name\" resumes by title.",
    "haishui -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "haishui -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "haishui chat -t web,terminal enables only specific toolsets for a focused session.",
    "haishui chat -s github-pr-workflow preloads a skill at launch.",
    "haishui chat -q \"query\" runs a single non-interactive query and exits.",
    "haishui chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "haishui chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "haishui --yolo bypasses all dangerous command approval prompts for the entire session.",
    "haishui chat --source telegram tags the session for filtering in haishui sessions list.",
    "haishui -p work chat runs under a specific profile without changing your default.",
    # --- CLI Subcommands ---
    "haishui doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "haishui dump outputs a compact setup summary — great for bug reports.",
    "haishui config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "haishui config edit opens config.yaml in your default editor.",
    "haishui config check scans for missing or stale configuration options.",
    "haishui sessions browse opens an interactive session picker with search.",
    "haishui sessions stats shows session counts by platform and database size.",
    "haishui sessions prune --older-than 30 cleans up old sessions.",
    "haishui skills search react --source skills-sh searches the skills.sh public directory.",
    "haishui skills check scans installed hub skills for upstream updates.",
    "haishui skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "haishui skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "haishui mcp add github --command npx adds MCP servers from the command line.",
    "haishui mcp serve runs Haishui itself as an MCP server for other agents.",
    "haishui auth add lets you add multiple API keys for credential pool rotation.",
    "haishui completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "haishui logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "haishui backup creates a zip backup of your entire Haishui home directory.",
    "haishui profile create coder creates an isolated profile that becomes its own command.",
    "haishui profile create work --clone copies your current config and keys to a new profile.",
    "haishui update syncs new bundled skills to ALL profiles automatically.",
    "haishui gateway install sets up Haishui as a system service (systemd/launchd).",
    "haishui memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "haishui webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: haishui tools disables unused tools, haishui skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "haishui models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    # --- Configuration ---
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as haishui -w).",
    ...
    "HAISHUI_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Haishui tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'haishui profile create coder' creates the 'coder' command.",
    "haishui profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Haishui prints a resume command with session ID and stats.",
    "haishui sessions export backup.jsonl exports all sessions for backup or analysis.",
    "haishui -r SESSION_ID resumes any specific past session by its ID.",
    # --- Memory ---
    ...
    "haishui skills install official/security/1password installs optional skills from the repo.",
    ...
    "/plan writes a markdown implementation plan to .haishui/plans/ without executing anything.",
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: haishui cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Haishui runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "haishui gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Haishui via a local macOS server.",
    ...
    "Context files (.haishui.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Haishui loads project context from .haishui.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "haishui mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "haishui mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "haishui mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — haishui tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.haishui/checkpoints/ — your project's .git is never touched.",
    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "haishui chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "haishui plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "haishui acp runs Haishui as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "HAISHUI_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "haishui auth add nous or haishui auth add openai-codex sets up OAuth-based providers.",
    ...
    "haishui status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.haishui/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Haishui your own.",
    ...
    "Before a gateway session reset, Haishui auto-flushes important facts to memory in the background.",
    ...
    "execute_code helpers require explicit imports: from haishui_tools import json_parse, shell_quote, retry.",
    ...
    "The skills audit log at ~/.haishui/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope Haishui state via HAISHUI_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "HAISHUI_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in HAISHUI_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.haishui/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'haishui profile create ls' would be rejected.",
    "haishui profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "haishui claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    '/goal <text> sets a standing Ralph-loop objective — Haishui auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Haishui config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Haishui is working.',
    ...
    '/reload reloads ~/.haishui/.env into the running session — pick up new API keys without restarting.',
    ...
    "HAISHUI_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.haishui/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.haishui/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',
    # --- Curator ---
    'haishui curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "haishui curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'haishui curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',
    # --- Credential Pools & Routing ---
    'haishui auth reset <provider> [target] clears cooldowns and exhaustion flags on a whole credential pool, or on one credential by index, id, or label.',
    ...
    # --- TUI & Dashboard ---
    'HAISHUI_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "HAISHUI_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'haishui dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'haishui dashboard embeds the full Haishui TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.haishui/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.haishui/dashboard-plugins/ — no npm build required.',
    ...
    'HAISHUI_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs concise/all/error/off).',
    'HAISHUI_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'HAISHUI_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'HAISHUI_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.haishui/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.haishui/skills/ so drop-in skills appear without restarting the session.',
    ...
    'haishui -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'haishui chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'haishui chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'haishui chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "haishui chat --source tool tags programmatic chats so they don't clutter haishui sessions list.",
    'haishui dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'haishui sessions rename <ID> "new title" renames any past session; haishui sessions delete <ID> removes one.',
    'haishui import restores a full Haishui backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'haishui fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'haishui pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'haishui setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'haishui status --deep runs the full health sweep across every component; plain haishui status is the quick view.',
    # --- Agent Behavior Env Vars ---
    'HAISHUI_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'HAISHUI_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.haishui/plugins/ — trust-gated by design.',
    "HAISHUI_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'HAISHUI_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'HAISHUI_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'HAISHUI_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'HAISHUI_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'HAISHUI_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'HAISHUI_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',
    # --- Gateway Behavior Env Vars ---
    'HAISHUI_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'HAISHUI_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'HAISHUI_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'HAISHUI_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Haishui block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from haishui sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.haishui/dashboard-plugins/.',
]
...
# Task-oriented example prompts for the empty composer. Kept generic — Haishui is
