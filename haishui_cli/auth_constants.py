...
Pure leaf: imports nothing from ``haishui_cli.auth`` so the per-provider modules
...
# httpx is imported lazily (~30ms) because haishui_cli.auth is on the interactive-CLI startup path
...
if TYPE_CHECKING:
    ...
    class _LazyHttpx:
        ...
        # set/del forward to the real module so monkeypatch.setattr("haishui_cli.auth.httpx.Client")
...
DEFAULT_NOUS_CLIENT_ID = "haishui-cli"
...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from haishui_cli import __version__ as _HAISHUI_CLI_VERSION
except Exception:  # pragma: no cover - version import should always succeed
    _HAISHUI_CLI_VERSION = "unknown"
CODEX_OAUTH_USER_AGENT = f"haishui-cli/{_HAISHUI_CLI_VERSION}"
