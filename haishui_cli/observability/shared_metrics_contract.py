"""Bounded product contract for the first Haishui shared-metrics slice."""
...
SCHEMA_KEY = "haishui.metrics.schema_version"
SCHEMA_VERSION = "haishui.metrics.event.v2"
MODEL_CALL_SCOPE = "haishui.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "haishui.task_run"
TOOL_CALL_SCOPE = "haishui.tool_call"
CLIENT_ACTIVE_MARK = "haishui.client.active"
TOOL_APPROVAL_MARK = "haishui.tool_approval"
SKILL_LIFECYCLE_MARK = "haishui.skill.lifecycle"
SKILL_LOAD_MARK = "haishui.skill.load"
SUBSCRIBER_NAME = "haishui.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "haishui.client.active"
LEGACY_MODEL_CALL_METRIC = "haishui.model_call.count"
MODEL_ROUTE_METRIC = "haishui.model_route.count"
TASK_STARTED_METRIC = "haishui.task_run.started"
TASK_FINISHED_METRIC = "haishui.task_run.finished"
TOOL_CALL_METRIC = "haishui.tool_call.count"
TOOL_APPROVAL_METRIC = "haishui.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "haishui.skill.lifecycle.count"
SKILL_LOAD_METRIC = "haishui.skill.load.count"
...
CLIENT_RESOURCE_KEYS = frozenset({"architecture", "haishui_version", "install_method", "os_family"})
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Haishui installation method."""
    normalized = _norm(value)
    return _allowlisted("nixos" if normalized == "nix" else normalized, CLIENT_INSTALL_METHODS)


def client_resource(
    haishui_version: Any, *, os_name: Any, architecture: Any, install_method: Any
) -> dict[str, str]:
    """Build the bounded client resource attached to aggregate packages."""
    version = str(haishui_version or "").strip()
    return {
        "architecture": client_architecture(architecture),
        "haishui_version": version if 0 < len(version) <= 64 else "unknown",
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("haishui_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Haishui logical scope."""
    metadata = _relay_metadata(
        event, RUNTIME_SCHEMA_KEY, RUNTIME_SCHEMA_VERSION, "haishui.call_role"
    )
    call_role = (metadata or {}).get("haishui.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from haishui_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Haishui terminal state to bounded (outcome, end_reason, termination)."""
...
_TOOL_CATEGORY_PREFIXES = (
    ...
    (("discord", "email", "feishu", "haishui-yuanbao", "slack", "sms"), "communication"),
)


def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Haishui registry toolset metadata to a low-cardinality category."""
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Haishui tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Haishui."""
