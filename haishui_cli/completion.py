"""Shell completion script generation for haishui CLI. Walks the live argparse parser tree, so
    ...
    for cmd, info in subcommands:
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_haishui_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;")
            ...
            cases.append(
                ...
                f"            ;;")
    cases_str = "\n".join(cases)
    return f"""# Haishui Agent bash completion
# Add to ~/.bashrc:
#   eval "$(haishui completion bash)"

_haishui_profiles() {{
    local profiles_dir="$HOME/.haishui/profiles"
...
_haishui_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_haishui_profiles)" -- "$cur"))
        return
    fi
...
complete -F _haishui_completion haishui
"""
    ...
    for cmd, info in subcommands:
        ...
        if cmd == "profile":
            ...
            sub_cases.append(
                ...
                f"                    ;;")
    sub_cases_str = "\n".join(sub_cases)
    return f"""#compdef haishui
# Haishui Agent zsh completion
# Add to ~/.zshrc:
...
_haishui_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.haishui/profiles" ]]; then
        profiles+=($HOME/.haishui/profiles/*(N/:t))
    fi
...
_haishui() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_haishui_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'haishui command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Haishui Agent fish completion",
        "# Add to your config:",
        "#   haishui completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __haishui_profiles",
        "    echo default",
        "    if test -d $HOME/.haishui/profiles",
        "        for d in $HOME/.haishui/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c haishui -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c haishui -f -s p -l profile"
        " -d 'Profile name' -xa '(__haishui_profiles)'",
        "",
        "# Top-level subcommands"]
    for cmd, info in subcommands:
        lines.append(
            f"complete -c haishui -f "
        ...
        if cmd == "profile":  # profile names for the actions that take one
            for action in sorted(_PROFILE_NAME_ACTIONS):
                lines.append(
                    ...
                    f"-a '(__haishui_profiles)' -d 'Profile name'")
    lines.append("")
    return "\n".join(lines)
