"""Desktop (Electron) app: build/stamp, stage-and-swap pack, exe integrity gate, macOS signing/TCC, Linux sandbox, launch (haishui gui/desktop).
...
from haishui_cli.desktop_console import desktop_console_output, desktop_launch_notice
from haishui_cli.main_tui_launch import _npm_lifecycle_env
...
def _desktop_stamp_path() -> Path:
    """Path of the desktop build stamp under $HAISHUI_HOME."""
    from haishui_constants import get_haishui_home
    return get_haishui_home() / "desktop-build-stamp.json"
...
def _renderer_bundle_dir(desktop_dir: Path, *, source_mode: bool) -> Optional[Path]:
    ...
    # macOS: …/Haishui.app/Contents/MacOS/Haishui → …/Contents/Resources
    resources = (
...
def _desktop_packaged_executable_in(release_dir: Path) -> Optional[Path]:
    ...
    *release_dir* is electron-builder's ``directories.output`` — the live ``apps/desktop/release`` or a
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Haishui.app/Contents/MacOS/Haishui"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / d / "Haishui.exe" for d in ("win-unpacked", "win-ia32-unpacked", "win-arm64-unpacked")
        ]
    else:
        candidates = [
            release_dir / d / n for d in ("linux-unpacked", "linux-arm64-unpacked") for n in ("haishui", "Haishui")
        ]
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # mtime can hand a wrong-architecture Haishui.exe to the launcher. Prefer
        ...
        # wrong-architecture Haishui.exe to the launcher, which Windows rejects with "This app can't run on
        # your computer" (#69179).
...
# PLACE: before-pack.mjs wipes ``release/<platform>- unpacked`` (or the mac ``Haishui.app``) and the Electron
# unpack + asar + rename then rebuild it. Any failure after that wipe — corrupt cached zip, blocked
# download, missing dep, disk full — leaves the user with NO app, and ``haishui update`` used to report
...
# (Desktop → haishui-setup --update → `haishui update` → `haishui desktop --build-only` → relaunch) rebuilds
# Haishui.exe on the end user's machine and used to verify only that the file EXISTS before declaring
...
# Haishui.exe is caught BEFORE the updater replaces the working app, and the previous build can be restored
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    emulated process arch) and rejected a correctly-built ARM64 ``Haishui.exe``. Binding
...
def _windows_native_machine() -> str:
    ...
    chain runs an x64 haishui-setup.exe (and thus x64 Python) on Windows-on-ARM devices, where
...
def _ensure_desktop_exe_launchable(desktop_dir: Path, packaged_executable: Optional[Path]) -> tuple:
    ...
    print(f"✗ The built Haishui.exe failed its integrity check: {error}\n    at: {packaged_executable}")
    ...
    if restored is not None:
        print("  ↩ Update aborted — restored the previous working Haishui.exe from backup.")
        print("    Your existing version was kept and still works. Run `haishui desktop`")
        print("    (or the in-app update) again to retry with a fresh Electron download.")
        return restored, True

    print("  ✗ No usable backup was found to restore.")
    print("    Run `haishui desktop --force-build` to rebuild, or re-run the Haishui")
...
def _redownload_electron_dist(project_root: Path, env: dict, *, mirror: Optional[str] = None) -> bool:
    ...
    if not installer.is_file():
        return False
    from haishui_constants import find_node_executable, with_haishui_node_path
    ...
    dl_env = with_haishui_node_path(env)
    if mirror:
...
def _desktop_macos_local_codesign(app: Path, *, desktop_dir: Path, identity: str = "-") -> bool:
    ...
    #    the outer Haishui.app component.
...
def _desktop_macos_relaunchable_fixup(
    ...
    "Haishui is damaged" and loses every grant. Clear quarantine xattrs, then sign
    ...
    if exe is None:
        return True
    # exe = .../Haishui.app/Contents/MacOS/Haishui  ->  app bundle = .../Haishui.app
    app = exe.parents[2]
...
def _macos_create_signing_identity(
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="haishui-tcc-"))
    try:
        ...
        def _export_p12(extra_args: list) -> None:
            subprocess.run(
                [
                    ...
                    "-out", str(p12), "-passout", "pass:haishuilocal",
                ],
                capture_output=True, check=True)
...
def _desktop_macos_setup_tcc_identity(identity: str = "Haishui Local Signing") -> bool:
    ...
    print(
        ...
        "stuck, reset it with:  tccutil reset All com.nousresearch.haishui"
    )
    return True
...
def _running_macos_app_bundles() -> set[Path]:
    """``.app`` bundles of every live Haishui Desktop process. A running bundle is never swapped
    ...
    for proc in psutil.process_iter(["exe"]):
        ...
        if exe.endswith("/Contents/MacOS/Haishui"):
...
def _install_rebuilt_desktop_app(desktop_dir: Path) -> tuple[list[Path], list[str]]:
    """Copy the rebuilt macOS bundle over every stale installed ``Haishui.app`` (#52339).

    ``haishui desktop --build-only`` (what ``haishui update`` runs) packages into
    ...
    if rebuilt_exe is None:
        return [], []
    from haishui_cli.gui_uninstall import packaged_gui_app_paths  # noqa: PLC0415
    # .../Haishui.app/Contents/MacOS/Haishui -> .../Haishui.app
...
def _install_rebuilt_macos_bundles(
    ...
    for app in candidates:
        ...
        if app.resolve() in running:
            problems.append(
                f"{app} is running and was not refreshed; quit Haishui Desktop and run "
                "`haishui update` again (or update from inside the app)")
            continue
        tmp = app.parent / f"{app.name}.haishui-update-new"
        old = app.parent / f"{app.name}.haishui-update-old"
        shutil.rmtree(tmp, ignore_errors=True)
        shutil.rmtree(old, ignore_errors=True)
        try:
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Haishui Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Haishui Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _register_linux_desktop_entry(defer: bool = False):
    """Install the XDG desktop entry for Haishui Desktop (Linux only, best-effort).

    ``Exec`` and ``Icon`` are absolute so the entry works outside a login shell.
    ``haishui uninstall --gui`` removes it.
    ...
    from haishui_cli.main import PROJECT_ROOT
    try:
        from haishui_cli.linux_desktop_entry import DeferredDesktopEntryInstall, install_desktop_entry, is_supported
        if not is_supported():
            return None
        if defer:
...
def _install_desktop_workspace_deps(npm: str, env: dict) -> None:
    ...
    from haishui_cli.main import PROJECT_ROOT
    from haishui_cli.main_web_build import _run_npm_install_deterministic
    ...
    nixos_env = with_haishui_node_path(_nixos_build_env())
...
def _run_desktop_pack_with_recovery(
    ...
    if build_result.returncode != 0 and staging_dir is not None and _staged_exe() is None:
        ...
        if restored:
            ...
            for p in purged:
                print(f"    - {p}")
            # The purge can't remove a win-unpacked tree whose Haishui.exe is
...
def _promote_staged_desktop_app(desktop_dir: Path, staging_dir: Path) -> Path:
    ...
    # Haishui.exe Windows cannot load. Verified on the STAGED exe, so a failure
...
def _build_desktop_app(desktop_dir: Path, *, source_mode: bool, npm: str, env: dict) -> Optional[Path]:
    ...
    if not source_mode:
        ...
        # A running desktop instance holds Haishui.exe locked on Windows, so the
    ...
    if build_result.returncode != 0:
        ...
        if sys.platform == "win32":
            print("  If this says \"Access is denied\" on Haishui.exe, close any")
            print("  running Haishui desktop window and retry.")
        print("  If the log shows Electron download retries, rebuild via a mirror:")
        print("    ELECTRON_MIRROR=<mirror-base-url> haishui desktop --force-build")
...
def _prefer_wsl_d3d12(env: dict) -> None:
    ...
    from haishui_constants import is_wsl
...
def _desktop_launch_env(args: argparse.Namespace) -> tuple[dict, list[str]]:
    ...
    from haishui_constants import with_haishui_node_path
    # with_haishui_node_path() copies os.environ when called with no arg.
    env = with_haishui_node_path()
    _prefer_wsl_d3d12(env)
    for attr, key in (
        ("fake_boot", "HAISHUI_DESKTOP_BOOT_FAKE"), ("ignore_existing", "HAISHUI_DESKTOP_IGNORE_EXISTING")):
        if getattr(args, attr, False):
            env[key] = "1"
    if getattr(args, "haishui_root", None):
        env["HAISHUI_DESKTOP_HAISHUI_ROOT"] = str(Path(args.haishui_root).expanduser().resolve())
    cwd = getattr(args, "cwd", None)
    env["HAISHUI_DESKTOP_CWD"] = str(Path(cwd).expanduser().resolve()) if cwd else os.getcwd()

    config_electron_flags, config_disable_gpu, config_password_store, config_ozone_hint = (
        _desktop_launch_options())
    if config_disable_gpu != "auto" and "HAISHUI_DESKTOP_DISABLE_GPU" not in os.environ:
        env["HAISHUI_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    if sys.platform == "linux" and "HAISHUI_DESKTOP_PASSWORD_STORE" not in os.environ:
        password_store = (
            config_password_store if config_password_store != "auto" else _detect_linux_password_store()
        )
        if password_store:
            env["HAISHUI_DESKTOP_PASSWORD_STORE"] = password_store
...
def cmd_gui(args: argparse.Namespace):
    ...
    from haishui_cli.main import PROJECT_ROOT
    from haishui_cli.main_install_repair import _resolve_node_runtime_npm
    desktop_dir = PROJECT_ROOT / "apps" / "desktop"
    ...
    if getattr(args, "setup_tcc_identity", False):
        identity = getattr(args, "identity", None) or "Haishui Local Signing"
    ...
    if source_mode or needs_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  haishui gui")
            sys.exit(1)
    ...
    from haishui_cli.linux_desktop_entry import launched_from_shell
    ...
    if source_mode:
        print("→ Launching Haishui Desktop from source build...")
    ...
    if not source_mode:
        desktop_launch_notice(f"→ Launching packaged Haishui Desktop: {' '.join(launch_command)}")
    pass_fds: tuple[int, ...] = ()
