"""Git plumbing for ``haishui update``: fork/upstream sync, trampoline-git detection, lockfile/EOL churn cleanup, orphan rescue refs, parked-branch assessment, fetch-failure classification.
...
def _git_stdout(git_cmd, args, cwd, **kw) -> Optional[str]:
    ...
    from haishui_cli.update_cmd import _git_run
...
def _prune_orphan_rescue_refs(
    ...
    """Expire old orphan rescue refs (``refs/haishui-update-backups/orphan-<branch>-<ts>-<sha>``).
    ...
    with suppress(OSError):
        prefix = f"refs/haishui-update-backups/orphan-{branch}-"
...
def _print_parked_branch_skip_warning(git_cmd: list[str], cwd: Path, current_branch: str, target_branch: str, reason: str) -> None:
    ...
    print(
        ...
        f"    git -C {cwd} status\n"
        f"    git -C {cwd} checkout {target_branch} && haishui update\n"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_haishui_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    with suppress(Exception):
        from haishui_constants import get_haishui_home
        (get_haishui_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path, *, assume_yes: bool = False, input_fn=None) -> bool:
    ...
    See #97052.
    """
    from haishui_cli.update_cmd import _count_commits_between, _has_upstream_remote, _no_prompt_git_kwargs, _should_skip_upstream_prompt
...
def _portable_git_candidates() -> list:
    ...
    profile-scoped HAISHUI_HOME), then profile home as a fallback for custom layouts.

    The Haishui-managed PortableGit tree lives under the SHARED root (``<root>/git/...``), not the
    profile-scoped HAISHUI_HOME (``<root>/profiles/<name>``), so a profile-scoped ``haishui update`` must look
    there (monerostar review, #87876).
    """
    from haishui_cli.update_cmd import get_default_haishui_root, get_haishui_home
    candidates = []
    with suppress(Exception):
        candidates += [root / "git" / "mingw64" / "libexec" / "git-core" / "git.exe" for root in (get_default_haishui_root(), Path(get_haishui_home()))]
    return candidates
...
def _ensure_non_trampoline_git(git_cmd: list) -> list:
    ...
    from haishui_cli.update_cmd import _locate_real_git
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    on update; install.ps1 pins ``false`` but older checkouts never got it and only ``haishui update`` can
    ...
    reuses its build-pinned ``install.ps1`` forever — so ``haishui update``, which ships with the checkout
