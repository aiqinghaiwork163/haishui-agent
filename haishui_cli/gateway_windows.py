...
_TASK_NAME_DEFAULT = "Haishui_Gateway"
_TASK_DESCRIPTION = "Haishui Agent Gateway - Messaging Platform Integration"
_TASK_LOGON_DELAY = "PT30S"
_TASK_RESTART_INTERVAL = "PT1M"
_TASK_RESTART_COUNT = 999

_GATEWAY_ENV = (("PYTHONIOENCODING", "utf-8"), ("HAISHUI_GATEWAY_DETACHED", "1"), ("HAISHUI_SUPERVISED_CHILD", "1"))
...
def _haishui_home() -> Path:
    ...
    return Path(get_haishui_home())


def _preserve_haishui_home_path(path: str | Path) -> str:
    r"""Render Haishui-owned paths under the configured HAISHUI_HOME spelling.

    ``%LOCALAPPDATA%\haishui`` may be a symlink/junction to another drive; launcher files must not
    bake in the resolved target for paths under HAISHUI_HOME.
    """
    candidate = Path(path)
    try:
        home = _haishui_home()
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Haishui profile."""
    from haishui_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "haishui_cli.main", *_current_profile_cli_args(), "gateway", command, *(extra_args or [])]
...
def _launch_elevated_install(force: bool = False, *, start_now: bool | None = None, start_on_login: bool | None = None) -> bool:
    ...
    overrides = {"HAISHUI_GATEWAY_ELEVATED_HANDOFF": "1"}
    extra_args = ["--elevated-handoff"]
    if force:
        extra_args.append("--force")
    for choice, env_key, flag in (
        (start_now, "HAISHUI_GATEWAY_INSTALL_START_NOW", "start-now"),
        (start_on_login, "HAISHUI_GATEWAY_INSTALL_START_ON_LOGIN", "start-on-login"),
    ):
...
def get_task_name() -> str:
    ...
    _assert_windows()
    from haishui_cli.gateway import _profile_suffix  # local: avoids circular init during boot
...
def get_task_script_path() -> Path:
    """The generated ``gateway.cmd`` wrapper under ``<HAISHUI_HOME>/gateway-service/`` (per-profile
    installs stay self-contained); the VBS launcher lives beside it."""
    _assert_windows()
    script_dir = _haishui_home() / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    """Stable cwd for detached/startup runs: anchor at HAISHUI_HOME when it exists (mirrors the POSIX
...
def _gateway_run_argv(python_exe: str, profile_arg: str) -> list[str]:
    """``python -m haishui_cli.main [--profile X] gateway run`` — shared by every launcher renderer."""
    argv = [python_exe, "-m", "haishui_cli.main"]
    if profile_arg:
...
def _launcher_settings(home: Path | None = None) -> tuple[str, str, str, str]:
    """Return (python_path, working_dir, haishui_home, profile_arg) for generated launchers.
    ``home`` targets another profile's HAISHUI_HOME (per-profile cold-start, #110959)."""
    from haishui_cli.gateway import PROJECT_ROOT, _profile_arg, get_python_path  # avoid circular init

    haishui_home = str(home if home is not None else _haishui_home())
    return (
        _preserve_haishui_home_path(get_python_path()),
        _stable_gateway_working_dir(PROJECT_ROOT),
        haishui_home,
        _profile_arg(haishui_home),
    )


def _launcher_pythonpath_entries(extra_pythonpath: list[str]) -> list[str]:
    return [
        _preserve_haishui_home_path(Path(__file__).resolve().parent.parent),
        *[_preserve_haishui_home_path(entry) for entry in extra_pythonpath],
    ]


def _build_gateway_cmd_script(python_path: str, working_dir: str, haishui_home: str, profile_arg: str) -> str:
    ...
    lines = [
        ...
        f'set "HAISHUI_HOME={haishui_home}"',
        *[f'set "{k}={v}"' for k, v in _GATEWAY_ENV],
        # VIRTUAL_ENV lets the gateway's own python detection find the venv.
        f'set "VIRTUAL_ENV={_preserve_haishui_home_path(venv_dir)}"',
...
def _build_gateway_vbs_script(python_path: str, working_dir: str, haishui_home: str, profile_arg: str) -> str:
    ...
    lines = [
        ...
        f"env.Item({q('HAISHUI_HOME')}) = {q(haishui_home)}",
        *[f"env.Item({q(k)}) = {q(v)}" for k, v in _GATEWAY_ENV],
        f"env.Item({q('VIRTUAL_ENV')}) = {q(_preserve_haishui_home_path(venv_dir))}",
...
def _atomic_write(path: Path, content: str, tmp: Path) -> None:
    ...
    ``Haishui_Gateway.tmp`` pops up in Notepad after every sign-in (#114093).
    """
    try:
...
def _build_gateway_argv(home: Path | None = None) -> tuple[list[str], str, dict[str, str]]:
    ...
    _assert_windows()
    from haishui_cli.gateway import PROJECT_ROOT

    python_path, working_dir, haishui_home, profile_arg = _launcher_settings(home)
    python_exe, venv_dir, extra_pythonpath = _resolve_detached_python(python_path)
    env_overlay = {"HAISHUI_HOME": haishui_home, **dict(_GATEWAY_ENV), "VIRTUAL_ENV": _preserve_haishui_home_path(venv_dir)}
    _prepend_pythonpath(env_overlay, [_preserve_haishui_home_path(p) for p in (PROJECT_ROOT, *extra_pythonpath)])
...
def windowless_gateway_restart_spec(run_argv: list[str]) -> tuple[list[str], str, dict[str, str]]:
    ...
    interpreter via ``_resolve_detached_python`` and supplies the stable cwd + env overlay (HAISHUI_HOME,
    ...
    try:
        haishui_home = str(_haishui_home().resolve())
    except Exception:
        haishui_home = ""
    env_overlay: dict[str, str] = {"PYTHONIOENCODING": "utf-8", "HAISHUI_GATEWAY_DETACHED": "1", "VIRTUAL_ENV": str(venv_dir)}
    if haishui_home:
        env_overlay["HAISHUI_HOME"] = haishui_home
...
def _stdin_is_interactive(*, isatty: bool, console_mode_ok: bool | None) -> bool:
    ...
    every character device — the NUL device included (`haishui gateway start < NUL`, stdin=DEVNULL) — so
...
def _prompt_install_choices(start_now: bool | None = None, start_on_login: bool | None = None) -> tuple[bool, bool]:
    ...
    if start_now is None:
        start_now = _install_choice_from_env("HAISHUI_GATEWAY_INSTALL_START_NOW")
    if start_on_login is None:
        start_on_login = _install_choice_from_env("HAISHUI_GATEWAY_INSTALL_START_ON_LOGIN")
    if start_now is not None and start_on_login is not None:
        return start_now, start_on_login

    from haishui_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    if running_pids or start_now:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"haishui {profile_arg} gateway start" if profile_arg else "haishui gateway start"
...
def _offer_elevated_install(headline: str, force: bool, start_now: bool, start_on_login: bool) -> bool:
    ...
    if prompt_yes_no("  Open the UAC prompt now?", False):
        if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
            print("✓ Launched elevated Haishui gateway install prompt.")
            if start_now:
                print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
            else:
                print("  Approve the Windows UAC prompt, then run: haishui gateway status")
            return True
...
def install(
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run in the foreground later with: haishui gateway run")
        return

    task_name = get_task_name()
    script_path = _write_task_script()
    # A pre-fix install that failed its Startup-folder swap left `Haishui_Gateway.tmp` there, and the
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: haishui gateway start")
        _print_next_steps()
        return
...
def _start_attestation_path(home: Path | None = None) -> Path:
    ...
    return (home if home is not None else _haishui_home()).joinpath(*_START_ATTESTATION_RELATIVE)
...
def _write_start_attestation(pids: list[int], via: str, home: Path | None = None) -> None:
    ...
    newer one written by a concurrent ``haishui gateway start`` (#110020 review)."""
    try:
        path = _start_attestation_path(home)
        path.parent.mkdir(parents=True, exist_ok=True)
        from haishui_cli.process_identity import _process_create_time
...
# meant to bridge the seconds between a ✓ and the next ``haishui gateway status``/``update``; a
...
def _attested_pid_exited_cleanly(pid: int, create_time: float | None = None, home: Path | None = None) -> bool:
    ...
    try:
        ...
        sentinel = get_lifecycle_sentinel_path(home if home is not None else _haishui_home())
...
def attested_death_generation(current_pids: list[int], home: Path | None = None) -> str | None:
    ...
    one-shot marker — ``haishui update`` consults it to decide whether a Desktop-owned install
...
def _format_attestation_warning(attested: list[int], data: dict) -> str:
    ...
    lines = [
        ...
        "shutdown record.",
        "  This usually means the shell that ran `haishui gateway start` was inside "
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        print(f"  Check the log for startup errors:\n    type {_haishui_home()}\\logs\\gateway.log\n    type {_haishui_home()}\\logs\\gateway-stdio.log")
...
def _print_next_steps() -> None:
    ...
    print(f"  type {_haishui_home()}\\logs\\gateway.log       # View logs")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_gateway_command("uninstall"):
                    print("✓ Launched elevated Haishui gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: haishui gateway status")
                    return
...
def _print_scheduled_task_drift(task_name: str) -> None:
    ...
    repair runs from ``start()`` / ``haishui update`` via ``reconcile_scheduled_task``)."""
    drift = scheduled_task_drift(task_name)
    if drift:
        print(f"⚠ Scheduled Task registration predates the current template ({'; '.join(drift)})")
        print("  Repair: haishui gateway start  (or: haishui gateway install)")
...
def reconcile_scheduled_task(task_name: str) -> bool:
    ...
    ``schtasks`` refused (typically Access Denied — the elevating ``haishui gateway install`` is the fallback)."""
    drift = scheduled_task_drift(task_name)
    if not drift:
        return False
    ...
    if not ok:
        print("  Repair manually: haishui gateway install")
    return ok
...
def start() -> None:
    ...
    if not is_task_registered() and not is_startup_entry_installed():
        ...
        # answer — the HAISHUI_GATEWAY_INSTALL_START_ON_LOGIN override or a real TTY prompt — never on a
        # non-TTY default (#113977). Declining still starts the gateway; the command is ``start``.
        start_on_login = _install_choice_from_env("HAISHUI_GATEWAY_INSTALL_START_ON_LOGIN")
        if start_on_login is None:
            from haishui_cli.setup import is_interactive_stdin, is_noninteractive, prompt_yes_no
        ...
        if start_on_login:
            ...
            return
        print("ℹ Login auto-start not installed; add it later with: haishui gateway install")
...
def restart() -> None:
    ...
    from haishui_cli.gateway import _wait_for_api_server_port_free  # avoid circular init
    ...
    if not _wait_for_gateway_ready(timeout_s=15.0):
        raise RuntimeError(
            ...
            "Check logs/gateway.log and run `haishui gateway status`."
        )
