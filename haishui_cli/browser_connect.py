"""Shared helpers for attaching Haishui to a local Chromium-family CDP port.
...
# the real profile into ``~/.haishui/browser-profile/`` and launch the user's real binary on
...
def real_profile_copy_dir(browser: str) -> str:
    """Return the haishui-owned snapshot dir for ``browser``'s real profile."""
    return str(get_haishui_home() / "browser-profile" / browser)
...
def _secure_snapshot(path: str, *, contents: bool = False) -> None:
    ...
    NixOS group-share carve-out, HAISHUI_UID/GID) as every Haishui secret dir — via ``_secure_dir``/
    ...
    ``HAISHUI_HOME_MODE`` hatch. Best-effort; never blocks a launch."""
    try:
        from haishui_cli.config import _secure_dir, _secure_file
        if not contents:
            _secure_dir(path)
            return
...
def _unavailable_auth_dbs_error(browser: str, failed: dict[str, str]) -> str:
    ...
    if all(reason == _AUTH_DB_LOCKED for reason in failed.values()):
        return (f"{browser} is running and holds the profile's {names} with a write lock, so their "
                "SQLite backup made no progress within five seconds. Haishui does not fall back to a "
...
_SNAPSHOT_DONE_MARKER = ".haishui-snapshot-complete"
...
def _locked_profile_error(browser: str) -> str:
    ...
    if _real_profile_autoclose():
        msg = (
            ...
            "yet. Haishui can close it for you (this quits the browser — you'll lose unsaved "
        ...
        msg = (
            ...
            "browser.use_real_profile off. (Enable browser.real_profile_autoclose to let Haishui "
...
def snapshot_real_profile(browser: str, src: str | None = None) -> tuple[str | None, str | None]:
    """Snapshot ``browser``'s real ACTIVE profile into the haishui copy dir; returns ``(dst, err)``.
...
def cleanup_real_profile_snapshots() -> None:
    ...
    root = str(get_haishui_home() / "browser-profile")
...
def chrome_debug_data_dir() -> str:
    return str(get_haishui_home() / "chrome-debug")
