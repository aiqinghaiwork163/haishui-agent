"""Top-level argparse construction for the haishui CLI.

Only the top-level parser and the ``chat`` subparser live here. Every other subparser (model,
gateway, sessions, …) is built by ``haishui_cli/subcommands/<group>.py`` and wired in
...
# (it sets ``HAISHUI_HOME`` and strips itself from ``sys.argv``), so it isn't on the parser.
...
# live parser fails. The derived path is authoritative; tests/haishui_cli/
...
def _cfg_path() -> str:
    """``~/.haishui/config.yaml`` spelled for the active profile, for help text.

    ``main._apply_profile_override`` builds this parser (via ``top_level_value_flag_sets``) BEFORE
    it re-homes the process to the sticky ``active_profile``; ``get_haishui_home()`` would emit the
    "[HAISHUI_HOME fallback] ... wrong profile" warning on every ``haishui`` command for that
    throwaway help string. Read the process home directly: after the override it IS the profile home.
    """
    from haishui_constants import display_haishui_home, get_process_haishui_home

    return f"{display_haishui_home(get_process_haishui_home())}/config.yaml"
...
def top_level_value_flag_sets() -> tuple[frozenset[str], frozenset[str]]:
    ...
    argparse surface — the drift that made ``haishui --reasoning high chat …`` misread ``high`` as
...
def _inherited_flag(parser, *args, **kwargs):
    """``parser.add_argument`` + tag the Action ``inherit_on_relaunch`` for ``haishui_cli.relaunch``."""
...
_EPILOGUE = """
Examples:
    haishui                        Start interactive chat
    haishui chat -q "Hello"        Single query mode
    haishui --tui                  Launch the modern TUI (or set display.interface: tui)
    haishui --cli                  Force the classic REPL (overrides display.interface: tui)
    haishui -c                     Resume the most recent session
    haishui -c "my project"        Resume a session by name (latest in lineage)
    haishui --resume <session_id>  Resume a specific session by ID
    haishui --resume latest        Resume the most recent session (same as -c)
    haishui --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    haishui setup                  Run setup wizard
    haishui logout                 Clear stored authentication
    haishui auth add <provider>    Add a pooled credential
    haishui auth list              List pooled credentials
    haishui auth remove <p> <t>    Remove pooled credential by index, id, or label
    haishui auth reset <p> [t]     Clear exhaustion status for a provider, or one credential
    haishui auth priority <p> <t> <n>  Move a pooled credential to priority n (0 = tried first)
    haishui auth refresh <p> [t]   Refresh a pooled OAuth credential and clear its cooldown
    haishui model                  Select default model
    haishui fallback [list]        Show fallback provider chain
    haishui fallback add           Add a fallback provider (same picker as `haishui model`)
    haishui fallback remove        Remove a fallback provider from the chain
    haishui config                 View configuration
    haishui config edit            Edit config in $EDITOR
    haishui config set model gpt-4 Set a config value
    haishui gateway                Run messaging gateway
    haishui gateway install        Install gateway background service
    haishui gateway start          Start the installed gateway service
    haishui gateway stop           Stop the gateway service
    haishui gateway status         Show gateway status
    haishui -p <profile> <cmd>     Run any command against a named profile's
                                  home (also --profile) — e.g. haishui -p coder gateway stop
    haishui -s haishui-agent-dev,github-auth
    haishui -w                     Start in isolated git worktree
    haishui sessions list          List past sessions
    haishui sessions browse        Interactive session picker
    haishui sessions rename ID T   Rename/title a session
    haishui logs                   View agent.log (last 50 lines)
    haishui logs -f                Follow agent.log in real time
    haishui logs errors            View errors.log
    haishui logs --since 1h        Lines from the last hour
    haishui debug share             Upload debug report for support
    haishui console                Open the safe Haishui command console
    haishui update                 Update to latest version
    haishui dashboard              Start web UI dashboard (port 9119)
    haishui dashboard --stop       Stop running dashboard processes
    haishui dashboard --status     List running dashboard processes

For more help on a command:
    haishui <command> --help
"""
...
def _add_top_level_flags(parser: argparse.ArgumentParser) -> None:
    ...
    inherited(parser, "-m", "--model", default=None, help=(
        ...
        "Applies to -z/--oneshot and --tui. Also settable via HAISHUI_INFERENCE_MODEL env var."))
    inherited(parser, "--provider", default=None, help=(
        "Provider override for this invocation (e.g. openrouter, anthropic). "
        "Applies to -z/--oneshot and --tui. The persistent provider lives in config.yaml "
        "under model.provider — use `haishui setup` or edit the file to change it."))
    ...
    inherited(parser, "--accept-hooks", action="store_true", default=False, help=(
        ...
        "without a TTY prompt.  Equivalent to HAISHUI_ACCEPT_HOOKS=1 or "
...
def _build_chat_parser(subparsers) -> argparse.ArgumentParser:
    ...
    ``haishui -m foo chat`` argparse first sets ``args.model`` from the top-level parser, then
    ...
    unless the flag is actually passed after the subcommand (tests/haishui_cli/
    test_argparse_flag_propagation.py).
    """
    chat_parser = subparsers.add_parser(
        "chat", help="Interactive chat with the agent",
        description="Start an interactive chat session with Haishui Agent")
    ...
    inherited(chat_parser, "--accept-hooks", action="store_true", default=SUPPRESS, help=(
        "Auto-approve any unseen shell hooks declared in config.yaml "
        "without a TTY prompt (see also HAISHUI_ACCEPT_HOOKS env var and "
    ...
    inherited(chat_parser, "--safe-mode", action="store_true", default=SUPPRESS,
              help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Haishui itself.")
...
class HaishuiArgumentParser(argparse.ArgumentParser):
    ...
    def _check_value(self, action, value):
        if isinstance(action, argparse._SubParsersAction) and value not in action.choices:
            # ``self.prog`` is "haishui" at the top level and "haishui gateway" for a nested group
...
def build_top_level_parser():
    ...
    """
    parser = HaishuiArgumentParser(
        prog="haishui", description="Haishui Agent - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter, epilog=_EPILOGUE)
    _add_top_level_flags(parser)
    # metavar keeps the usage line to ``haishui [...] <command>`` instead of the brace list of
    # every subcommand name; ``haishui --help`` still lists each command with its help row.
