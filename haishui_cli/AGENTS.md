# haishui_cli/ + cli.py — CLI, slash commands, config, skins, updater, profiles

Applies on top of the root `AGENTS.md`. Long-form: `website/docs/developer-guide/cli-internals.md`.

## CLI architecture

`cli.py` holds `HaishuiCLI` (REPL loop, config, slash dispatch); behaviour lives in mixins
`haishui_cli/cli_commands_mixin.py`, `cli_stream_mixin.py`, `cli_status_bar_mixin.py`,
...
`HaishuiCLI._SLASH_DISPATCH` (`canonical -> (method name, pass_arg)`), falling back to a
`_handle_<name>_command` method by naming convention. **There is no `elif` ladder — do not add one.**
Skill slash commands (`agent/skill_commands.py`) scan `~/.haishui/skills/` and inject as a **user
message**, never into the system prompt (prompt caching).

Rules: all interactive menu-pickers use curses (`haishui_cli/curses_ui.py`; example
`haishui_cli/tools_config.py`). Never emit `\033[K` (ANSI erase-to-EOL) in spinner/display code —
...
## Slash command registry (`haishui_cli/commands.py`)
...
`haishui_cli/goal_command.py::dispatch_goal_command` owns `/goal` parsing and manager
...
## Config system (`haishui_cli/config.py`)
...
- **.env = SECRETS ONLY** (keys, tokens, passwords): add to `OPTIONAL_ENV_VARS` with
  ...
  in `.env` is deprecated — the loader warns; canonical is `terminal.cwd`. `haishui config
...
- **Three loaders — know which you're in:** `load_cli_config()` (CLI, `cli.py`); `load_config()`
  (`haishui tools/setup`, most subcommands, `haishui_cli/config.py`, merges `DEFAULT_CONFIG`);
  `haishui_cli/config_effective.py::load_user_config_effective()` (gateway runtime via
  `gateway/run.py::_load_gateway_config`, TUI gateway `_load_cfg`, cron, `haishui send`, doctor,
  `haishui_time`/`haishui_logging`: user file + managed overlay + `${VAR}` expansion + model-key
...
- **Every `DEFAULT_CONFIG` key has a runtime reader, and every reader a registry entry.** Both drift
  ...
  key: `rg -n '"<key>"' haishui_cli/config_defaults.py` AND `rg -n '<key>' --glob '!tests' .` both hit,
...
## Skin engine (`haishui_cli/skin_engine.py`)
...
`load_skin(name)` (user `~/.haishui/skins/*.yaml` → built-ins → default; missing values inherit
...
## Update pipeline (`haishui update`) — transactional; every stage guards a real field failure
...
- **Plan** (`update_inventory.py`, `haishui update --plan`): read-only inventory — install kind, all
...
- **Restart-per-kind**: systemd and launchd restarts are FLEET-WIDE (every `haishui-gateway*` unit /
  `ai.haishui.gateway*` LaunchAgent), drain-first (SIGUSR1), with per-unit/per-label failure
...
- **Report**: every run writes a machine-readable receipt to `~/.haishui/logs/update_receipts/`
...
- **Nothing runs pulled code in the pre-pull interpreter** (`update_handoff.py`). The process that
  started `haishui update` imported the PRE-pull tree; once git (or the ZIP swap) has replaced the
  ...
  `haishui update <same flags> --post-swap <file>` under the venv interpreter, which imports only the
...
`_apply_profile_override()` in `haishui_cli/main.py` sets `HAISHUI_HOME` before any module import for
single-profile commands (`haishui -p x <cmd>`), so there `get_haishui_home()` scopes to the active
profile. The multiplex gateway and the Desktop/dashboard `serve` backend instead bind the active
profile per activity via a contextvar override while `os.environ["HAISHUI_HOME"]` keeps the launch
...
carries an identity marker (`haishui_constants.named_profile_has_identity`: `config.yaml`/`.env`/`SOUL.md`/
...
cron all go through `mkdir_under_haishui_home` / `_ensure_cron_dir`, which refuse a deleted or
missing named profile, #94590). Process-global per-profile slots (MCP discovery in `mcp_startup.py`,
tool registry overlays) key on `haishui_constants.haishui_home_key()`, never a single flag.
...
Migration from per-profile gateways: `haishui_cli/gateway_migrate.py` (`haishui gateway migrate
...
other service domain / UNIX user / HAISHUI_HOME outside `profiles/` — notices for the explicit command,
...
`haishui gateway restart` for a gateway Haishui did not install (custom launchd agent / unit running
...
profile's `HAISHUI_HOME` (and `HOME` for the service user) explicitly — a supervisor starts with an
...
## Nous free tier (`haishui_cli/anon_auth.py`)
...
which never contains a raw exception, a URL or a `haishui` verb) and `.copy_terminal`, so no caller
...
contextvars. `upgrade_guest` (`haishui auth upgrade`), the CLI `/login` handler and the desktop
