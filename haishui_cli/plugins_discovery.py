...
Split out of :mod:`haishui_cli.plugins`. Names that tests patch on the origin
...
from haishui_cli.plugin_capabilities import VALID_CAPABILITY_IDS
from haishui_cli.plugin_capabilities import parse_declared_capabilities as _parse_declared_capabilities
from haishui_cli.plugins_manifest import (
...
ENTRY_POINTS_GROUP = "haishui_agent.plugins"
ENTRY_POINT_CAPABILITIES_GROUP = "haishui_agent.plugin_capabilities"
...
def discover_entrypoint_manifests() -> List["PluginManifest"]:
    ...
    ``haishui_agent.plugin_capabilities`` group (``<plugin-id>.<capability-id>`` entries pointing at the same
...
def collect_directory_manifests() -> List[PluginManifest]:
    ...
    from haishui_cli import plugins as _origin  # patched names resolve through the origin
    manifests: List[PluginManifest] = []
    ...
    user_dir = get_haishui_home() / "plugins"
    logger.debug("Scanning user plugins: %s", user_dir)
    _scan("user", user_dir, "user")
    if _origin._env_enabled("HAISHUI_ENABLE_PROJECT_PLUGINS"):
        project_dir = Path.cwd() / ".haishui" / "plugins"
        logger.debug("Scanning project plugins: %s", project_dir)
        _scan("project", project_dir, "project")
    else:
        logger.debug("Project plugins disabled (set HAISHUI_ENABLE_PROJECT_PLUGINS=1 to enable)")
    return manifests
...
def gate_manifest(
    ...
    if names & LEGACY_RELAY_PLUGIN_KEYS:
        error = (
            "removed — Relay lifecycle is owned by Haishui core; configure "
            f"{RELAY_PLUGINS_CONFIG_ENV} instead"
        )
        return _placeholder(error, logging.WARNING, "Refusing to load removed Haishui Relay plugin '%s'; %s", error)
    if names & disabled:
    ...
    if manifest.source == "bundled":
        ...
        # Bundled platforms register LAZILY: eagerly importing ~20 heavy SDKs added seconds to every `haishui`
    ...
    if enabled is None or not names & enabled:
        return _placeholder(
            f"not enabled in config (run `haishui plugins enable {lookup_key}` to activate)", logging.DEBUG,
