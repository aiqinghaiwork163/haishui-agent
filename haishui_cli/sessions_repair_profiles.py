"""``haishui sessions repair-profiles`` — settle crossed-profile durable state across every store.
...
def enumerate_stores() -> List[Store]:
    ...
    root = _get_default_haishui_home()
...
class RepairPlan:
        ...
        if key_profile not in self.claimed:
            self._add(Finding(
                ...
                reason=f"no live profile named {key_profile!r}; create it, then rerun, or "
                       f"`haishui profile migrate-identity {key_profile} <target>`"))
            return
        moving.add(sid)
        self._add(Finding(
    ...
    def _scan_routing(self, session: _Session, store: Store) -> None:
        from haishui_state_profile_repair import session_key_profile
...
def default_snapshot(store: Store) -> Optional[str]:
    ...
    try:
        return create_quick_snapshot(label="repair-profiles", haishui_home=store.home)
