"""haishui webhook — manage dynamic webhook subscriptions from the CLI."""
...
def _subscriptions_path() -> Path:
    ...
    return get_haishui_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: haishui gateway run
"""


def webhook_command(args):
    """Entry point for 'haishui webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)
    if not sub:
        print("Usage: haishui webhook {subscribe|list|remove|test}")
        print("Run 'haishui webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    if profile_arg is None:
        ...
        from haishui_cli.profiles import normalize_profile_name, profile_exists, validate_profile_name
        try:
    ...
    if cron_job:
        ...
        try:
            ...
            return
        if job is None:
            print(f"Error: no cron job matches '{cron_job}'. List jobs with: haishui cron list")
            return
    ...
    print("  The gateway must be running to receive events (haishui gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: haishui webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from haishui webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (haishui gateway run)")
