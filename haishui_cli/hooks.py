"""haishui hooks — inspect and manage shell-script hooks."""
...
def hooks_command(args) -> None:
    """Entry point for ``haishui hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)
    if not sub:
        print("Usage: haishui hooks {list|test|revoke|doctor}")
        print("Run 'haishui hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs and not outbound:
        print("No shell hooks or outbound webhooks configured in ~/.haishui/config.yaml.")
        print("See `haishui hooks --help` or")
        print("    website/docs/user-guide/features/hooks.md")
        print("for the config schema and worked examples.")
        return

    if not specs:
        print("No shell hooks configured in ~/.haishui/config.yaml.")
    else:
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if drift is True:
                        print(
                            ...
                            f"run `haishui hooks doctor` to re-validate"
                        )
            print()
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Haishui wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if drift is True:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `haishui hooks revoke` + re-approve to refresh")
        elif drift is False:
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `haishui hooks doctor`.")
