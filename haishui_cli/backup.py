"""Backup and import commands for haishui CLI."""
...
from haishui_constants import (
    LOCAL_RUNTIME_ROOT_DIRS, _get_platform_default_haishui_home, get_default_haishui_root, get_haishui_home,
    display_haishui_home,
)
from haishui_state_dbfile import RETIRED_GENERATION_DIR_SUFFIX
from utils import (
...
# Where ``haishui backup --quick`` / ``/snapshot`` / the pre-update safety net write state
...
def _snapshot_recovery_hint() -> str:
    """How to restore a state snapshot. There is no `haishui snapshot` subcommand — only the /snapshot
    slash command inside a `haishui` session (haishui_cli/commands.py)."""
    return ("To restore a newer snapshot, start `haishui` in a terminal and run `/snapshot list`, then "
            "`/snapshot restore <id>` (CLI only).")

# Directory names to skip (matched against each path component). ``haishui-agent`` only matches at
# the root (``_should_exclude``) so skill dirs like ``skills/.../haishui-agent/`` survive. The
# dependency/cache entries matter: one plugin venv or pip/uv cache under HAISHUI_HOME walked
...
_EXCLUDED_DIRS = {
    "haishui-agent",     # the codebase repo — re-clone instead
...
# Haishui-managed runtime downloads (see ``LOCAL_RUNTIME_ROOT_DIRS``). Matched ONLY at the root of
# HAISHUI_HOME and at ``profiles/<name>/`` — a deeper dir of the same name (a skill's ``models/``)
# is user data.
...
# Files ``haishui import`` must never overwrite, matched by basename so root and named profiles are
...
# Reserved archive subtree for memory-provider state OUTSIDE HAISHUI_HOME (e.g. ~/.honcho, via
...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the Haishui backup slot."""
...
@contextmanager
def _backup_operation_lock(haishui_home: Path, timeout_seconds: float = 0.25):
    """Acquire one cross-process backup slot for full and quick snapshots."""
    lock_path = haishui_home / ".backup.lock"
        ...
        while not acquired:
            try:
                ...
                if time.monotonic() >= deadline:
                    raise BackupInProgressError("another Haishui backup is already running")
                time.sleep(0.05)
        yield
    finally:
        if acquired:
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to haishui root) should be skipped."""
    parts = rel_path.parts
    if _in_excluded_root_dir(rel_path):
        return True
    # ``haishui-agent`` only matches at the root level; nested same-named dirs are preserved.
    if any(p in _EXCLUDED_DIRS and (p != "haishui-agent" or p == parts[0]) for p in parts):
        return True
    name = rel_path.name
    return name in _EXCLUDED_NAMES or name.startswith(_EXCLUDED_PREFIXES) or name.endswith(_EXCLUDED_SUFFIXES)


def _iter_backup_files(haishui_root: Path, out_path: Path, skipped_dirs: Optional[set] = None):
    ...
    excluded tree, the root-only ``haishui-agent`` carve-out, root runtime trees, per-file rules),
    shared by ``haishui backup`` and the pre-update / pre-migration path so they can never drift.
    """
    for dirpath, dirnames, filenames in os.walk(haishui_root, followlinks=False):
        rel_dir = Path(dirpath).relative_to(haishui_root)
        is_root = rel_dir == Path(".")
        kept = [
            d for d in dirnames
            if (d not in _EXCLUDED_DIRS or (d == "haishui-agent" and not is_root))
        ...
        for fname in filenames:
            rel = rel_dir / fname
            fpath = haishui_root / rel
            # zipfile.write() follows file symlinks, so skip links before any archive write can
            # copy data from outside HAISHUI_HOME; never archive the output zip into itself.
...
# 30 GB state.db, reading as a hung ``haishui update``) is replaced by the O(1) header+schema probe.
...
def _unlink_move_restore_db(src: Path, dst: Path) -> bool:
    ...
    try:
        ...
        logger.error("Refusing unlink+move restore of %s: %s Close the in-process "
                     "database handles (or restart Haishui) and retry.", dst, exc2)
        return False
...
_RUN_BACKUP_PREFIX = "haishui-backup-"
...
def run_backup(args) -> bool:
    """Create a zip backup of the Haishui home directory.
    ...
    """
    haishui_root = get_default_haishui_root()

    if not haishui_root.is_dir():
        print(f"Error: Haishui home directory not found at {haishui_root}")
        sys.exit(1)

    try:
        with _backup_operation_lock(haishui_root):
            return _run_backup_locked(args, haishui_root)
    except BackupInProgressError as exc:
        print(f"Error: {exc}")
        raise SystemExit(2) from exc


def _run_backup_locked(args, haishui_root: Path) -> bool:
    ...
    print(f"Scanning {display_haishui_home()} ...")
    skipped_dirs: set = set()
    files_to_add: list[tuple[Path, Path]] = list(_iter_backup_files(haishui_root, out_path, skipped_dirs))
    ...
    print(f"\nBackup {'incomplete' if errors else 'complete'}: {out_path}\n"
          ...
          f"  Time:        {elapsed:.1f}s")
    if external_to_add:
        print(f"\n  Included {len(external_to_add)} memory-provider file(s) stored outside {display_haishui_home()}.")
    if skipped_external:
    ...
    if errors:
        ...
        print(f"\nRestore with: haishui import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Haishui backup."""
    names = zf.namelist()
    if not names:
        return False, "zip archive is empty"
    # Telltale files a haishui home has — at the root or one level deep (zipped directory).
    if not any(Path(n).name in {"config.yaml", ".env", "state.db"} for n in names):
        return False, "zip does not appear to be a Haishui backup (no config.yaml, .env, or state databases found)"
    return True, ""
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    if len(first_parts) == 1 and first_parts <= {".haishui", "haishui"}:
...
def _count_session_rows(path: Path) -> Optional[Tuple[int, int]]:
    ...
    ``None`` means "unknown" (missing, not a Haishui session store, unreadable) — never "zero":
...
def _confirm_import_overwrite(haishui_root: Path) -> bool:
    """Prompt before importing over an existing installation; True when import may proceed."""
    if not any((haishui_root / m).exists() for m in ("config.yaml", ".env")):
        return True
    print("\nWarning: Target directory already has Haishui configuration.\n"
...
def _import_members(
    zf: zipfile.ZipFile, members: List[str], prefix: str, haishui_root: Path, file_count: int
    ...
    for member in members:
        ...
        # NOT under HAISHUI_HOME; provider configs commonly hold credentials, so tighten to 0600.
        ...
        if external:
            ...
            if rel.endswith(_SQLITE_SIDECAR_SUFFIXES):
                skipped_runtime.append(rel)
                continue
            target = haishui_root / rel
            root = haishui_root.resolve()
...
def run_import(args) -> None:
    """Restore a Haishui backup from a zip file."""
    ...
    # ``get_default_haishui_root()`` would silently retarget a profile restore at the live root.
    haishui_root = get_haishui_home()
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Backup contains {file_count} files\nTarget: {display_haishui_home()}")
        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")
        if not args.force and not _confirm_import_overwrite(haishui_root):
            return
        ...
        restored, restored_external, errors, skipped_runtime, db_shrunk = _import_members(
            zf, members, prefix, haishui_root, file_count)
        elapsed = time.monotonic() - t0
        ...
        if restored_external:
            print(f"\n  Restored {restored_external} memory-provider file(s) to "
                  f"their original location(s) outside {display_haishui_home()}.")
        if errors:
        ...
        restored_profiles = _restore_profile_wrappers(haishui_root)
        print()
        if not (haishui_root / "haishui-agent").is_dir():
            print("Note: The haishui-agent codebase was not included in the backup.\n"
                  "  If this is a fresh install, run: haishui update")
        if restored_profiles:
            print("\nTo re-enable gateway services for profiles:")
            for pname in restored_profiles:
                print(f"  haishui -p {pname} gateway install")
        _revive_gateway_after_import(haishui_root)
        print("Done. Your Haishui configuration has been restored.")


def _restore_profile_wrappers(haishui_root: Path) -> List[str]:
    """Re-create shell wrapper scripts for restored named profiles; return the profile names seen."""
    profiles_dir = haishui_root / "profiles"
    ...
    except ImportError:  # haishui_cli.profiles unavailable (fresh install)
        if any(profiles_dir.iterdir()):
            print("\n  Profiles detected but aliases could not be created.\n"
                  "  Run: haishui profile list  (after installing haishui)")
    return [n for n, _ in restored_profiles]


def _revive_gateway_after_import(haishui_root: Path) -> None:
    ...
    """
    native_default = _get_platform_default_haishui_home()
    if haishui_root != native_default and any(
        ...
        print("\nRestored into a non-default home; leaving the gateway service alone to avoid clashing "
              ...
              "To start a gateway for this home, run:  haishui gateway install")
        return
    try:
        from haishui_cli.gateway import ensure_gateway_service, _is_service_running
...
# --- Quick state snapshots (used by /snapshot slash command and haishui backup --quick) ---

# Critical state files (relative to HAISHUI_HOME) for quick snapshots; everything else is
...
# outside state.db, so it is listed explicitly — ``haishui update`` snapshots this set (#15733).
_QUICK_STATE_FILES = (
    ...
    # schema-init re-creates an empty one (#52889). Skipped when outside HAISHUI_HOME.
...
def _quick_snapshot_root(haishui_home: Optional[Path] = None) -> Path:
    home = haishui_home or get_haishui_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None, haishui_home: Optional[Path] = None, keep: Optional[int] = None,
...
def _create_quick_snapshot_locked(
    ...
    multi-GB ``state.db`` never stalls ``haishui update`` while the small files are always captured.
    """
...
def list_quick_snapshots(limit: int = 20, haishui_home: Optional[Path] = None) -> List[Dict[str, Any]]:
    """List existing quick state snapshots, most recent first."""
    results = []
    for d in _snapshot_dirs(_quick_snapshot_root(haishui_home)):
...
def restore_quick_snapshot(snapshot_id: str, haishui_home: Optional[Path] = None) -> bool:
...
def restore_cron_jobs_if_emptied(snapshot_id: str, haishui_home: Optional[Path] = None) -> Optional[Dict[str, Any]]:
    """Safety net for silent cron-job loss across ``haishui update``.
...
def create_pre_update_snapshots_all_profiles(
    ...
    home = invoking_home or get_haishui_home()
    for name, profile_home in _sibling_profile_homes(home):
        try:
            snap_id = create_quick_snapshot(
                label="pre-update", haishui_home=profile_home, keep=keep, max_file_size=max_file_size)
            if snap_id:
...
def restore_config_model_settings_if_rewritten(
    snapshot_id: str, haishui_home: Optional[Path] = None) -> Optional[Dict[str, Any]]:
    """Safety net for silent config.yaml model/MoA loss across ``haishui update``.
...
def _restore_all_sibling_profiles(
    ...
    """Run ``restore_fn(snap_id, haishui_home=...)`` for every sibling against ITS OWN
    ...
    for name, snap_id in profile_snapshots.items():
        ...
        if profile_home is None:
            continue
        try:
            result = restore_fn(snap_id, haishui_home=profile_home)
        except Exception as exc:
...
def prune_quick_snapshots(keep: int = _QUICK_DEFAULT_KEEP, haishui_home: Optional[Path] = None) -> int:
    """Remove oldest quick snapshots beyond the keep limit. Returns count deleted."""
    return _prune_oldest(_snapshot_dirs(_quick_snapshot_root(haishui_home)), keep, shutil.rmtree, "snapshot")


def run_quick_backup(args) -> None:
    """CLI entry point for haishui backup --quick."""
    snap_id = create_quick_snapshot(label=getattr(args, "label", None))
    if snap_id:
        print(f"State snapshot created: {snap_id}\n"
              f"  {len(list_quick_snapshots())} snapshot(s) stored in {display_haishui_home()}/state-snapshots/\n"
...
def _write_full_zip_backup(out_path: Path, haishui_root: Path) -> Optional[Path]:
    """Full zip snapshot of ``haishui_root`` to ``out_path`` under the backup slot (same rules as
    :func:`run_backup`); None when nothing to back up, another backup running, or write error."""
    try:
        with _backup_operation_lock(haishui_root):
            return _write_full_zip_backup_locked(out_path, haishui_root)
    except BackupInProgressError as exc:
        logger.warning("Full-zip backup skipped: %s", exc)
        return None


def _write_full_zip_backup_locked(out_path: Path, haishui_root: Path) -> Optional[Path]:
    scan_started = time.monotonic()
    logger.info("automatic backup phase=scan status=started")
    try:
        files_to_add = list(_iter_backup_files(haishui_root, out_path))
    except OSError as exc:
...
def _create_prefixed_full_backup(
    haishui_home: Optional[Path], prefix: str, keep: int, what: str, prune_what: str) -> Optional[Path]:
    """Write ``<HAISHUI_HOME>/backups/<prefix><timestamp>.zip`` and prune older same-prefix zips.
    Returns the path, or ``None`` if nothing to back up or the write failed. Never raises."""
    haishui_root = haishui_home or get_default_haishui_root()
    if not haishui_root.is_dir():
        return None
    backup_dir = haishui_root / _PRE_UPDATE_BACKUPS_DIR
    try:
    ...
    if _write_full_zip_backup(out_path, haishui_root) is None:
        return None
...
def create_pre_update_backup(
    haishui_home: Optional[Path] = None, keep: int = _PRE_UPDATE_DEFAULT_KEEP) -> Optional[Path]:
    """Full zip backup to ``backups/pre-update-<timestamp>.zip``, auto-pruned; ``None`` if nothing
    was found or the backup failed. Never raises — ``haishui update`` continues anyway."""
    return _create_prefixed_full_backup(haishui_home, _PRE_UPDATE_PREFIX, max(keep, 1), "pre-update", "backup")


def create_pre_migration_backup(
    haishui_home: Optional[Path] = None, keep: int = _PRE_MIGRATION_DEFAULT_KEEP) -> Optional[Path]:
    """Full zip backup to ``backups/pre-migration-<timestamp>.zip`` before ``haishui claw migrate``
    (same dir as update backups so listings/``haishui import`` find it); ``None`` if nothing was
    found or the write failed. Never raises."""
    return _create_prefixed_full_backup(
        haishui_home, _PRE_MIGRATION_PREFIX, max(keep, 0), "pre-migration", "pre-migration backup")
