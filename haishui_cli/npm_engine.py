...
Scope is deliberately narrow: Haishui only upgrades an npm inside its **own** managed Node tree
(``$HAISHUI_HOME/node``), installing in place with ``--prefix`` so ``bin/npm`` keeps resolving to
...
from haishui_constants import (
    bootstrap_haishui_managed_node,
    get_haishui_home,
...
def managed_npm_prefix(npm: str | os.PathLike[str] | None) -> Path | None:
    """Return the Haishui-managed Node root *npm* lives in, else ``None``.

    Symlinks are resolved first: ``~/.local/bin/npm`` → ``$HAISHUI_HOME/node/bin/npm`` →
...
def upgrade_managed_npm(npm: str, npm_range: str, *, prefix: Path, quiet: bool = False) -> bool:
    ...
    into ``$HAISHUI_HOME/node/etc/npmrc`` (so global installs land on PATH), and without the override
    the "upgrade" would install a second npm elsewhere while the managed one stayed stale.
    """
    if not quiet:
        print(f"→ Upgrading Haishui-managed npm to satisfy {npm_range}…", flush=True)
    ...
    if managed_node_tree_in_use():
        if not quiet:
            print(
                "  ⚠ deferred: the Haishui-managed Node.js tree is in use by a "
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="haishui-npm-upgrade-") as tmp:
...
def _probe_version(npm: str) -> str | None:
    try:
        result = subprocess.run([npm, "--version"], timeout=30, env=with_haishui_node_path(), **_RUN_KW)
    except (OSError, subprocess.SubprocessError):
        return None
...
def _print_manual_fix(npm: str, npm_range: str, actual: str | None) -> None:
    ...
    print(
        ...
        f"  Resolved npm: {npm}\n"
        "  Haishui could not provision its own Node.js runtime and never\n"
...
def _provision_managed_npm(npm_range: str | None, *, quiet: bool = False) -> str | None:
    """Provision (or reuse) the managed tree under ``$HAISHUI_HOME/node`` and return a satisfying npm.
    ...
    if not quiet:
        print(
            "→ Provisioning a Haishui-managed Node.js runtime "
            "(the resolved npm belongs to your system and is left alone)…",
            flush=True,
        )
    managed_npm = bootstrap_haishui_managed_node()
    if not managed_npm:
        if not quiet:
...
def maybe_repair_npm_engine(npm: str | None, output: str, *, quiet: bool = False) -> str | None:
    ...
    if prefix is not None:
        # Haishui owns this npm — upgrade in place. Only an npm-range failure is fixable this way.
