#!/usr/bin/env python3
"""Skills Hub CLI — Unified interface for the Haishui Skills Hub."""
...
# serves everything from "haishui-index", so that entry MUST exceed the whole catalog or browse
...
_BROWSE_LIMITS = {
    "haishui-index": 1000000, "official": 200, "skills-sh": 200, "well-known": 50,
    "github": 200, "clawhub": 500, "lobehub": 500, "browse-sh": 500}
# Programmatic (TUI gateway) browse keeps its own, lower caps.
_BROWSE_API_LIMITS = {
    "haishui-index": 5000, "official": 100, "skills-sh": 100, "well-known": 25,
...
def _ident_col(style: str) -> tuple:
    ...
    # copy into `haishui skills install`.
...
def _existing_categories() -> List[str]:
    """Sorted category buckets under ``~/.haishui/skills/`` (children without their own SKILL.md)."""
...
def _prompt_for_category(c: Console, existing: List[str]) -> str:
    ...
    if existing:
        ...
        c.print("[bold]Category[/] "
                f"[dim](optional — press Enter to install flat at {display_haishui_home()}/skills/<name>/)[/]")
...
def do_search(query: str, source: str = "all", limit: int = 10, console: Optional[Console] = None,
    ...
    c.print(table)
    c.print("[dim]Use: haishui skills inspect <identifier> to preview, "
            "haishui skills install <identifier> to install "
            "(--json for scripting)[/]\n")
...
def _render_browse_page(c: Console, deduped, page_items, page: int, total_pages: int,
    ...
    c.print("[dim]Tip: 'haishui skills inspect <identifier>' to preview, "
            "'haishui skills install <identifier>' to install, "
            "'haishui skills search <query>' to search deeper[/]\n")
...
def browse_skills(page: int = 1, page_size: int = 20, source: str = "all") -> dict:
    ...
    # haishui-index AND the external APIs at once would double-count every skill.
...
def do_inspect(identifier: str, console: Optional[Console] = None) -> None:
    ...
    if preview is not None:
        c.print(Panel(preview, title="SKILL.md Preview", subtitle="haishui skills install <id> to install"))
    c.print()
...
def _scan_block_message(result, identifier: str) -> str:
    ...
    can help, and the read-only next step (``haishui skills inspect``). The hard-block rule mirrors
    ...
    policy = ("Haishui never installs unverified skills with high-risk findings, even with --force."
    ...
    return (f"the security scan found {findings} in '{identifier}' (listed above). "
            ...
            f"installing, run `haishui skills inspect {identifier}`.")
...
def _resolve_url_bundle_name(c: Console, bundle, meta, identifier: str,
    ...
    if name_override and _is_valid_installed_skill_name(name_override):
        ...
        c.print(f"[bold red]Cannot install from URL:[/] {url}\n"
                ...
                f"  [bold]haishui skills install {url} --name <your-name>[/]\n\n"
...
def _announce_blueprint(c: Console, skill_name: str) -> None:
    """Offer an installed skill's ``metadata.haishui.blueprint`` via /suggestions — never
    ...
    try:
        ...
        if register_blueprint_suggestion(spec) is not None:
            ...
            c.print("[dim]You can still schedule it any time by asking the agent "
                    "or via[/] [bold]haishui cron add[/][dim].[/]\n")
...
def _print_fetch_failure(c: Console, sources, identifier: str, meta=None, source=None) -> None:
    ...
    if meta is not None and not rate_limited:
        ...
        c.print("[dim]Stale index entry: the skill was likely renamed or removed by "
                "its author. Try `haishui skills search` for an alternative.[/]\n")
        return
    ...
    if rate_limited:
        ...
        c.print(f"Check the name with [bold]haishui skills search {identifier.rsplit('/', 1)[-1]}[/] "
                "and check your internet connection. If it keeps failing, run [bold]haishui doctor[/].\n")
...
_INSTALL_PANELS = {
    "official": (
        ...
        "It ships with haishui-agent but is not activated by default.\n"
...
def _confirm_install(c: Console, bundle, category: str) -> bool:
    ...
    files_at = f"Files will be at: [cyan]{display_haishui_home()}/skills/{category + '/' if category else ''}{bundle.name}/[/]"
...
def do_list(source_filter: str = "all", enabled_only: bool = False,
    ...
    profile's config — ``-p`` swaps HAISHUI_HOME at process start, so no profile flag here."""
    from tools.skills_hub import HubLockFile, ensure_hub_dirs
...
def do_check(name: Optional[str] = None, console: Optional[Console] = None) -> None:
    ...
    if orphaned:
        c.print(f"[yellow]Orphaned:[/] {', '.join(orphaned)} — lock-file entries whose local "
                ...
                "remove the stale entry with: haishui skills uninstall <name>\n")
...
def do_update(name: Optional[str] = None, console: Optional[Console] = None,
    ...
    Mirrors the user-modified protection bundled skills already get from ``haishui update`` (ported from
    ...
    if skipped_local:
        ...
        c.print("[dim]Overwrite with: haishui skills update <name> --force[/]\n")
...
def do_list_modified(console: Optional[Console] = None, as_json: bool = False) -> None:
    """List bundled skills the user has edited (which `haishui update` keeps)."""
    ...
    c.print(f"\n[bold]{len(modified)} user-modified bundled skill(s)[/] "
            "[dim](kept as-is by `haishui update`):[/]")
    for entry in modified:
        c.print(f"  [yellow]~[/] {entry['name']}")
    c.print()
    c.print("[dim]See changes:   haishui skills diff <name>[/]")
    c.print("[dim]Resume updates: haishui skills reset <name>          (keep your copy, re-baseline)[/]")
    c.print("[dim]Revert to stock: haishui skills reset <name> --restore[/]\n")
...
def do_diff(name: str, console: Optional[Console] = None) -> None:
    ...
    c.print()
    c.print(f"[dim]Revert with: haishui skills reset {name} --restore[/]\n")
...
def do_tap(action: str, repo: str = "", console: Optional[Console] = None) -> None:
    ...
    if action == "list":
        ...
        if not repo:
            _print_error(c, f"Repo required. Usage: haishui skills tap {action} owner/repo")
            return
    ...
    if target == "github":
        if not repo:
            _print_error(c, "--repo required for GitHub publish.\n"
                            "Usage: haishui skills publish <path> --to github --repo owner/repo")
            return
        auth = GitHubAuth()
        if not auth.is_authenticated():
            _print_error(c, "GitHub authentication required.\n"
                            f"Set GITHUB_TOKEN in {display_haishui_home()}/.env "
...
def _github_publish(skill_path: Path, skill_name: str, target_repo: str, auth) -> tuple:
    ...
    try:
        resp = call("post", f"{target_repo}/pulls", json={
            ...
            "body": f"Submitting the `{skill_name}` skill via Haishui Skills Hub.\n\n"
                    f"This skill was scanned by the Haishui Skills Guard before submission.",
...
def _snapshot_cli(args) -> None:
    ...
    if snap_action == "export":
        ...
        _console.print("Usage: haishui skills snapshot [export|import]\n")


def _tap_cli(args) -> None:
    tap_action = getattr(args, "tap_action", None)
    if not tap_action:
        _console.print("Usage: haishui skills tap [list|add|remove]\n")
        return
    do_tap(tap_action, repo=getattr(args, "repo", "") or getattr(args, "name", ""))


# `haishui skills <action>` -> handler(args). Lambdas late-bind the do_* names so
# tests that patch("haishui_cli.skills_hub.do_install") still intercept.
_CLI_ACTIONS = {
...
def skills_command(args) -> None:
    """Router for `haishui skills <subcommand>` — called from haishui_cli/main.py."""
    handler = _CLI_ACTIONS.get(getattr(args, "skills_action", None))
    if handler is None:
        _console.print("Usage: haishui skills [browse|search|install|inspect|list|list-modified|diff|check|update|audit|uninstall|reset|opt-out|opt-in|publish|snapshot|tap]\n")
        _console.print("Run 'haishui skills <command> --help' for details.\n")
        return
    handler(args)
