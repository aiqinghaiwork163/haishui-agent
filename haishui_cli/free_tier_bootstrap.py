...
Every Haishui process that may need the free tier runs this once at boot (``haishui serve`` on a
...
(:func:`haishui_cli.anon_auth.guest_enabled`), resolves which provider carries inference, records
...
def reconcile_record() -> Optional[SetupRecord]:
    ...
    writes this process never saw (``haishui setup`` / ``haishui model`` from a shell, a hand edit).
    ...
    if record is None or record.provider_configured:
        return record
    from haishui_constants import get_process_haishui_home, haishui_home_key
    if haishui_home_key() != haishui_home_key(get_process_haishui_home()) or _inventory_stamp == _config_stamp():
        return record
...
def start_background_bootstrap() -> threading.Thread:
    """``haishui serve`` entry: run on a daemon thread so a slow portal never delays the socket."""
