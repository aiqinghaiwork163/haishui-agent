...
The provider list shown by ``haishui model`` (CLI/TUI) and the desktop Settings → Providers tabs
...
come from :data:`haishui_cli.auth.PROVIDER_REGISTRY` (credential truth); ``display_name`` /
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `haishui model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """One descriptor per provider in the ``haishui model`` universe (:data:`CANONICAL_PROVIDERS`,
    ...
    PROVIDER_REGISTRY = _safe_import("haishui_cli.auth", "PROVIDER_REGISTRY", {})
    OPTIONAL_ENV_VARS = _safe_import("haishui_cli.config", "OPTIONAL_ENV_VARS", {})
    ...
    HAISHUI_OVERLAYS = _safe_import("haishui_cli.providers", "HAISHUI_OVERLAYS", {})
    try:
    ...
    for order, entry in enumerate(CANONICAL_PROVIDERS):
        ...
        overlay = HAISHUI_OVERLAYS.get(slug)
