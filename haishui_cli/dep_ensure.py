...
from haishui_constants import agent_browser_runnable, find_node_executable
from tools.environments.local import haishui_subprocess_env

_IS_WINDOWS = platform.system() == "Windows"

_DEP_CHECKS = {
    # find_node_executable() rather than a bare which(): $HAISHUI_HOME/node is not on PATH, so
    ...
    "browser": lambda: (agent_browser_runnable(shutil.which("agent-browser")) or _has_system_browser()
                        or _has_haishui_agent_browser() or _has_npx_agent_browser()),
...
def _has_haishui_agent_browser() -> bool:
    from haishui_constants import get_haishui_home  # late import: tests patch haishui_constants.get_haishui_home
    ...
    # install.sh installs into $HAISHUI_HOME/node/bin/ via npm -g --prefix; legacy git clones used node_modules/.bin/.
...
def ensure_dependency(dep: str, interactive: bool = True) -> bool:
    ...
    if shell == "powershell":
        ...
        if not ps_bin:
            ...
            return False
        cmd = [ps_bin, "-ExecutionPolicy", "Bypass", "-File", str(script), "-Ensure", dep, "-HaishuiHome", str(get_haishui_home())]
    else:
        cmd = ["bash", str(script), "--ensure", dep]
    run_env = {**haishui_subprocess_env(inherit_credentials=False), "IS_INTERACTIVE": "false"}
