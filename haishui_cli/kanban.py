"""``haishui kanban …`` — dispatch (``kanban_command``), task-verb handlers, ``run_slash`` for ``/kanban``.
...
from haishui_cli import kanban_swarm as ks
from haishui_cli.kanban_output import (
...
from haishui_cli.kanban_boards import _dispatch_boards
from haishui_cli.kanban_ops import (
    _cmd_daemon, _kanban_config, _cmd_dispatch, _cmd_gc, _cmd_repair, _cmd_tail, _cmd_watch,
)
from haishui_cli.kanban_parser import build_parser  # noqa: F401  (re-exported: haishui_cli.main, run_slash)
...
def _check_dispatcher_presence(haishui_home: Optional[Path] = None) -> tuple[bool, str]:
    """``(running, message)`` for the "will anything dispatch this?" warning: True when a gateway is
    alive for this HAISHUI_HOME with ``kanban.dispatch_in_gateway`` on, else False + human guidance.
    Fails OPEN (probe/config errors -> ``(True, "")``) — a missed warning beats crying wolf.
    ``haishui_home`` scopes the probe to a profile dir (dashboard backend); CLI callers pass None.

    The dashboard plugin API passes it because the dashboard backend process can be running under a
    different HAISHUI_HOME than the profile the request targets, which otherwise produced a "no gateway is
    ...
    try:
        ...
        liveness = resolve_gateway_liveness(profile_dir=haishui_home, use_cache=False)
    except Exception:
    ...
    if pid:
        return (False, "Gateway is running but kanban.dispatch_in_gateway=false in "
                ...
                "standalone daemon (`haishui kanban daemon --force`).")
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from ``haishui kanban …``; returns a shell-style exit code."""
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print("usage: haishui kanban <action> [options]\n"
                  "Run 'haishui kanban --help' for the full list of actions.", file=sys.stderr)
        return 0
    ...
    # `--board <slug>` pins HAISHUI_KANBAN_BOARD for the duration of this call so it inherits the
    ...
    if board_override:
        ...
        if normed != kb.DEFAULT_BOARD and not kb.board_exists(normed):
            return _err(f"kanban: board {normed!r} does not exist. "
                        f"Create it with `haishui kanban boards create {normed}`.")
    ...
    with board_scope:
        ...
        # "no such table: tasks" on first use from a fresh HAISHUI_HOME.
        try:
            kb.init_db()
...
def _profile_author() -> str:
    ...
    for env in ("HAISHUI_PROFILE_NAME", "HAISHUI_PROFILE"):
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    if profiles:
        ...
        print("No profiles found under ~/.haishui/profiles/.\n"
              "Create one with `haishui -p <name> setup` before assigning tasks.")
    print(
...
def _cmd_assignees(args: argparse.Namespace) -> int:
    ...
    if _json_out(args, data):
        return 0
    if not data:
        print("(no assignees — create a profile with `haishui -p <name> setup`)")
        return 0
...
def _cmd_create(args: argparse.Namespace) -> int:
    ...
    with kbc.connect_closing() as conn:
        task_id = kb.create_task(
            ...
            creator_task_id=(os.environ.get("HAISHUI_KANBAN_TASK")
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    from haishui_cli import kanban_diagnostics as kd
...
def _cmd_link(args: argparse.Namespace) -> int:
    ...
    expected_child_run_id = (
        ...
        if args.child_id == os.environ.get("HAISHUI_KANBAN_TASK") else None)
    with kbc.connect_closing() as conn:
    ...
    if gated:
        print(
            ...
            f"completes; use `haishui kanban unlink {args.parent_id} {args.child_id}` "
            f"to run it now."
        )
    return 0
...
def _worker_run_id_for(task_id: str) -> Optional[int]:
    ...
    raw = os.environ.get("HAISHUI_KANBAN_RUN_ID")
    if os.environ.get("HAISHUI_KANBAN_TASK") != task_id or not raw:
        return None
    try:
        return int(raw)
    except ValueError:
        ...
        def op(tid):
            ...
            try:
                ...
                fail_msg[tid] = (f"cannot complete {tid}: a live worker is running it. Wait for the "
                                 f"worker, `haishui kanban reclaim {tid}` to release it, or re-run with "
            ...
            if not done:
                ...
                if blockers:
                    ...
                    fail_msg[tid] = (f"cannot complete {tid}: unsatisfied parent dependencies: {detail}; "
                                     f"complete the parents first, or `haishui kanban unlink <parent> {tid}`.")
            return done
...
def _cmd_specify(args: argparse.Namespace) -> int:
    ...
    from haishui_cli import kanban_specify as spec
...
def _cmd_decompose(args: argparse.Namespace) -> int:
    ...
    from haishui_cli import kanban_decompose as decomp
