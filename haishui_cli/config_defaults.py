"""Default configuration data for Haishui Agent: DEFAULT_CONFIG and OPTIONAL_ENV_VARS.

Pure-data leaf module — must not import from haishui_cli.config. Comments are the user-facing
docs of config.yaml.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["haishui-cli"],
    # journal_mode: SQLite journal mode for every Haishui DB. "wal" default; use "delete" on
    ...
    "session": {
        # Per-terminal `haishui -c`: each CLI session writes a breadcrumb under
        # $HAISHUI_HOME/terminal-sessions/<terminal-id>, so bare -c/--continue resumes THIS
    ...
    "agent": {
        ...
        # Default 30 min is a safety valve for wedged agents, not a target latency — an interactive `haishui
        ...
        # Haishui-level retry attempts for API errors (connection drops, timeouts, 5xx) wrapping the
        ...
        # wrapping Haishui (sandbox runner, managed platform) can describe proxy/credential/ mount
        # layout without editing SOUL.md. Env HAISHUI_ENVIRONMENT_HINT overrides it.
        "environment_hint": "",
        ...
        # share or `haishui doctor` (status channel only; prompt cache untouched). 0 = no escalation
        ...
        # gateway status (`haishui status` / fleet monitoring). Retries never stop — a signal, not a
        ...
        # HAISHUI_LOCAL_STREAM_STALE_TIMEOUT overrides.
    ...
    "terminal": {
        ...
        # sandboxes). Empty = TMPDIR/TMP/TEMP if set, else HAISHUI_HOME/cache/terminal (auto-pruned
        ...
        # (bytes); raise it for a ~/.haishui tree that legitimately exceeds 2 GiB.
        "sync_back_max_bytes": 2 * 1024 * 1024 * 1024,
        # HOME for host tool subprocesses: "auto" = host keeps the real OS-user HOME, containers use
        # HAISHUI_HOME/home; "real" = force real HOME; "profile" = force HAISHUI_HOME/home when it
        ...
        # ["/home/user/.haishui/cache/documents:/output"]. For gateway MEDIA delivery, write to
        ...
        # default for images whose entrypoints must start as root (e.g. the bundled Haishui image,
        # which drops to `haishui` via s6-setuidgid). When on, SETUID/SETGID caps are omitted.
    ...
    "web": {
        ...
        # parallel, firecrawl, keenable; tavily is opt-in keyless via `haishui tools`, not a ring
        # member). Set by the `haishui tools` picker. "free" = always anonymous endpoint even with a
    ...
    "browser": {
        ...
        # Consent to browse with the user's REAL logins locally: runs on a Haishui-managed SNAPSHOT
        # of the ACTIVE default-Chromium profile (Local State -> profile.last_used; cookies, logins,
        # prefs copied and re-synced per fresh session) driven by Haishui' packaged Chromium. The
        # snapshot dir sidesteps Chrome 136+'s default-profile debugging block and never contends
        # with the running browser. Turning off deletes ~/.haishui/browser-profile/ so credentials
        ...
        # runs `haishui browser close-profile` (kills that profile's browser tree, unsaved tabs lost)
    ...
    # write_file/patch call); restore with /rollback. Opt-in via `haishui chat --checkpoints` or
    ...
    "checkpoints": {
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ~/.haishui/checkpoints/ size (MB); the oldest checkpoint per project
        ...
        # must not guess. Orphans: `haishui checkpoints prune` (`--keep-orphans` to skip).
        "auto_prune": True,
        "retention_days": 7,
        "min_interval_hours": 24,
    },
    # Hard cap (chars) for one auto-loaded context file (SOUL.md, AGENTS.md, CLAUDE.md, .haishui.md,
    ...
    # Same bound for single-query mode (``haishui -q/-z``). With only ONE turn there is no
    ...
    "compression": {
        ...
        # Haishui' summarizer cannot shrink it. native = codex decides; haishui = Haishui' threshold
    ...
    "auxiliary": {
        ...
        # of skills); route cheaper via `haishui model` → auxiliary → Curator.
        "curator": _aux(600),
    ...
    "display": {
        ...
        # Interface bare `haishui`/`haishui chat` launches: "cli" (prompt_toolkit REPL) | "tui" (Ink).
        # Flags win: `--cli` forces the REPL, `--tui` / HAISHUI_TUI=1 forces the TUI.
        "interface": "cli",
        # `haishui --tui` auto-resumes the most recent human-facing session (like `haishui -c`).
        # HAISHUI_TUI_RESUME=<id> always wins.
        ...
        # `path`/`paths` = HAISHUI_HOME-relative YAML files/dirs (or conventional status_phrases.yaml
        ...
        # Gateway runtime footer on the FINAL message, e.g. `model · 68% · ~/projects/haishui`.
        ...
        # CLI/TUI/desktop, managed with `haishui pets`. No effect on prompt caching.
        "pet": {
            "enabled": False,
            "slug": "",   # active pet slug in get_haishui_home()/pets/; empty → first installed
    ...
    "dashboard": {
        ...
        # client disconnects. 0 = park forever. Env: HAISHUI_TUI_WS_ORPHAN_REAP_GRACE_S.
        ...
        # newest message older than HAISHUI_TUI_SESSION_TTL_S, default 6h) with
        ...
        # plugin. Env HAISHUI_DASHBOARD_OAUTH_CLIENT_ID / HAISHUI_DASHBOARD_PORTAL_URL win when
        ...
        # no-op. Env HAISHUI_DASHBOARD_BASIC_AUTH_USERNAME / _PASSWORD_HASH / _PASSWORD / _SECRET /
        ...
        # HAISHUI_DASHBOARD_DRAIN_SECRET; no-op unless >=256-bit, weak secrets rejected
        # (fail-closed). scope = capability label; min_secret_chars in url-safe-b64 chars.
        "drain_auth": {"scope": "drain", "min_secret_chars": 43},
        # Public URL (env HAISHUI_DASHBOARD_PUBLIC_URL): full authority (scheme + host + optional
        # prefix, e.g. https://example.com/haishui) for the OAuth redirect_uri; its hostname is
    ...
    "tts": {
        ...
        "piper": {
            ...
            # (~/.haishui/cache/piper-voices/), use_cuda, length_scale (2.0 = twice as slow),
    ...
    "voice": {
        ...
        #   chained  — STT → Haishui turn → TTS (the stt.* / tts.* providers below)
        #   gpt-live — one full-duplex voice model (OpenAI GPT-Live) owns the mic and speaker and
        #              DELEGATES every real request to Haishui (any model / provider you have
    ...
    # "Hey Haishui" hands-free wake word: always-on, on-device hotword detection that starts a fresh
    ...
    "wake_word": {
        ...
        "openwakeword": {
            # "hey_haishui" | built-in openWakeWord name ("hey_jarvis", "alexa", ...) | path to a
    ...
    # plugins/context_engine/<name>/ or ~/.haishui/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # ~/.haishui/cache/delegation/ with a head+tail window + read_file offset footer, nothing
    ...
    "moa": {
        ...
        # <haishui_home>/moa-traces/<session_id>.jsonl (or trace_dir) for auditing.
        "save_traces": False,
        "trace_dir": "",
    ...
    # and resolved; read-only — creation goes to ~/.haishui/skills/ unless create_dir redirects it.
    "skills": {
        ...
        # HAISHUI_HOME, scanned alongside the local dir.
        "create_dir": "",
        # In a git checkout, <root>/.haishui/skills/ and <root>/.agents/skills/ load as the
        # highest-precedence tier — ONLY if the root is in trusted_project_dirs. false = no scan, no
        # untrusted-skills notice.
        "project_discovery": True,
        # Trusted project roots; managed by `haishui skills trust` / `untrust`.
        ...
        # skip; HAISHUI_IGNORE_RULES suppresses the list like the other auto-injected context.
        "auto_load": [],
        # Substitute ${HAISHUI_SKILL_DIR} / ${HAISHUI_SESSION_ID} in SKILL.md content.
        "template_vars": True,
        ...
        # Advisory NVIDIA SkillEvaluator Tier 1 scan on `haishui skills install` (alongside the
        ...
        "write_approval": False,
        # Audit ledger: every skill mutation appends to ~/.haishui/skills/.curator_ledger.jsonl with
        # before/after hashes (blobs under ~/.haishui/.curator_backups/blobs/); powers `haishui
    ...
    # daemon. `haishui curator status` shows the last run.
    "curator": {
        "enabled": True,
        ...
        # aux-model cost. `haishui curator run --consolidate` overrides once.
        "consolidate": False,
        # Also prune bundled built-ins (a suppression list stops `haishui update` restoring them);
        ...
        "prune_builtins": True,
        # TTL purge of skills/.archive/: 0 = never; > 0 lets the explicit `haishui curator purge`
        # delete older archived skills (never automatic; logged in the ledger).
        "archive_ttl_days": 0,
        # Before every real (non-dry-run) pass, snapshot ~/.haishui/skills/ to
        # ~/.haishui/skills/.curator_backups/<utc-iso>/skills.tar.gz (`haishui curator rollback`).
        "backup": {
            "enabled": True,
    ...
    # peerName, sessions, enabled); haishui-specific overrides only here.
    "honcho": {},
    ...
    "whatsapp": {
        # reply_prefix: None = built-in "☤ *Haishui Agent*" header; "" disables; \n allowed.
    },
    ...
    #   cron job, a -q session (HAISHUI_INTERACTIVE=1 but nobody to answer), or an unattended
    ...
    "approvals": {
        ...
        # export HAISHUI_INTERACTIVE=1 (for interactive sudo prompts) but have NO user waiting to answer
        ...
        # Approve" → false. HAISHUI_TUI_NO_CONFIRM=1 skips the TUI modal.
    ...
    # are given.
    "platform_hints": {},
    # Plugin system. `enabled`/`disabled` lists are written by `haishui plugins enable|disable` and
    ...
    "plugins": {
        ...
        # 2026-09-14 removal date (see COMPAT_MANIFEST.md, `haishui plugins compat`). Stopgap only: the
    ...
    # approvals persist in ~/.haishui/shell-hooks-allowlist.json. Schema + examples:
    # website/docs/user-guide/features/hooks.md.
    "hooks": {},
    # Auto-accept shell-hook registrations without a TTY prompt (also --accept-hooks or
    # HAISHUI_ACCEPT_HOOKS=1). Gateway/cron/non-interactive runs need one of these to pick up
    # newly-added hooks.
    ...
    "auth": {  # Login policy (credentials themselves live in auth.json / .env).
        ...
        # logins automatically when Haishui has no usable login of its own. Their refresh tokens are single-use
        # and rotate, so two programs on one login can log each other out; set false to make Haishui use only
        # its own logins (`haishui auth add <provider>`). `haishui auth add openai-codex` still offers the import
        # interactively.
    ...
    "security": {  # Security: pre-exec scanning via tirith plus related guards.
        ...
        # .haishui config) always need human approval, even under yolo. Extra patterns are fnmatch
        ...
        # banner. Add via `haishui doctor --ack <id>`; remove by editing the list. Catalog:
        # haishui_cli/security_advisories.py.
        "acked_advisories": [],
    ...
    "cron": {
        ...
        # an installed provider (plugins/cron_providers/<name>/ or $HAISHUI_HOME/plugins/ <name>/),
        ...
        # Env override: HAISHUI_CRON_MAX_PARALLEL.
        ...
        "output_retention": 50,
        # Timeout (seconds) for a no-agent cron script. Env: HAISHUI_CRON_SCRIPT_TIMEOUT. Keep in
        ...
        # job's dispatch guard forever. Env: HAISHUI_CRON_SESSION_DB_TIMEOUT. 0 = unlimited.
        "session_db_timeout_seconds": 10,
        # Timeout (seconds) per media attachment send during gateway delivery; large attachments
        # (long TTS audio, big exports) need more than 30s. Env: HAISHUI_CRON_MEDIA_SEND_TIMEOUT.
        ...
        # before one reminder ping (the run is still recorded; `haishui cron incidents` shows it).
        # A green run or a different error alerts again immediately; `haishui cron incidents ack`
    ...
    # promotes dependency-satisfied todos to ready, and fires `haishui -p <assignee> chat -q ...` per
    ...
    "kanban": {
        ...
        # Per-home claim allowlist for boards shared across Haishui homes (#110995): profile names
        ...
        # Auto-run the decomposer on Triage tasks every tick. False = manual via `haishui kanban
    ...
    "code_execution": {  # execute_code settings (programmatic tool calls).
        ...
        # relative paths resolve. strict = isolated temp dir with haishui-agent's own python
    ...
    # by tool_search / tool_describe / tool_call bridges and surfaced on demand. Core Haishui tools
    ...
    "logging": {  # File logging to ~/.haishui/logs/: agent.log captures INFO+, errors.log WARNING+.
    ...
    "model_catalog": {
        ...
        # CLI refetches on the next /model or `haishui model` once the cache is older. Network
    ...
    # into "text-only" and hide vision_analyze / reasoning controls (#112649). Provider keys: Haishui
    ...
    "monitoring": {
        ...
        "gateway_health_export": {  # Gateway health & diagnostics export.
            ...
            "resource_attributes": {
                "service.name": "haishui-gateway", "deployment.environment.name": "production"
            },
        },
    ...
    "gateway": {  # Gateway settings (messaging platforms: Telegram, Discord, Slack, ...).
        ...
        # to HAISHUI_GATEWAY_PLATFORM_CONNECT_TIMEOUT, which wins if set explicitly.
        ...
        # run_gateway() bridges these to HAISHUI_STARTUP_WATCHDOG / HAISHUI_STARTUP_WATCHDOG_TIMEOUT_S
        # and re-arms the live handle; explicit env wins.
        "startup_watchdog": True,
        "startup_watchdog_timeout_seconds": 300,
        # Keep writing the legacy ~/.haishui/sessions/sessions.json mirror of the routing index
        ...
        # `false` keeps per-profile gateways for good. `haishui gateway migrate --multiplex` folds a
        ...
        # adapter is parked; `haishui profile create --clone` therefore leaves messaging channels
        # behind unless --clone-channels is passed.
        "multiplex_profiles": True,
        # May `haishui update` fold this install onto a multiplexed default gateway by itself?
        ...
        # `haishui gateway migrate --multiplex` is an explicit request and always proceeds.
        ...
        # (HAISHUI_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent AND a wakeUrl is
        ...
        # HAISHUI_GATEWAY_MAX_STARTS / HAISHUI_GATEWAY_START_WINDOW_S.
        ...
        # credential/system denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.haishui/.env, auth.json). True:
        # files must be under the Haishui cache, media_delivery_allow_dirs, or fresher than
        # trust_recent_files_seconds — recommended for public-facing gateways so prompt injection
        # can't exfiltrate host secrets. Bridged to HAISHUI_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra roots (project/scratch dirs, mounted shares) from which bare file paths may be
        # uploaded; the Haishui cache is always trusted. List of absolute paths or one
        # os.pathsep-separated string; tildes expanded. Bridged to HAISHUI_MEDIA_ALLOW_DIRS. Honored
        # in both modes.
        ...
        # pure-allowlist mode. Bridged to HAISHUI_MEDIA_TRUST_RECENT_FILES. Only consulted when
        # strict is true.
        "trust_recent_files": True,
        # Recency window in seconds; 600 covers a multi-tool turn. Bridged to
        # HAISHUI_MEDIA_TRUST_RECENT_SECONDS. Only consulted when strict is true.
    ...
    # Automatic cleanup of ~/.haishui/state.db, which otherwise grows without bound and slows FTS5
    ...
    "sessions": {
        ...
        "auto_prune": True,
        # Inactive days of ended-session history to keep (= `haishui sessions prune`).
        ...
        # stay until `haishui sessions optimize-storage` runs, since the rebuild is disk-heavy on
        # large DBs. advise = `haishui update` prints a one-line notice with reclaimable size when a
        ...
        # (native/fts5_cjk/build.sh → ~/.haishui/lib/libfts5_cjk.so), 1-2 char CJK terms get exact
        # index matches instead of LIKE scans. True = use when present (inert otherwise); False =
        # never load/serve it. Bridged to HAISHUI_CJK_FTS.
        "cjk_fts": True,
        ...
        # Max active messages per session for in-memory export (`haishui sessions export`); checked
    ...
    "doctor": {
        # Per-probe timeout (seconds) for `haishui doctor --live` real-call probes.
        "live_probe_timeout": 10,
    },

    "updates": {
        # Passive version/banner checks only; explicit `haishui update --check` remains enabled.
        "check": True,
        # Pre-update backup. quick = snapshot small critical state (pairing JSONs, cron jobs,
        # config.yaml, .env, auth.json, profile DBs) into <HAISHUI_HOME>/state-snapshots/, skipping
        # files >1 GiB; restore via ``/snapshot``. full = quick PLUS a ``haishui backup`` zip in
        # <HAISHUI_HOME>/backups/ (``haishui import`` restores; slow on large homes; ``--backup``
        ...
        # update cleanly. `haishui update --switch-branch` overrides to switch for one run.
        "parked_branch_strategy": "switch",
        # Refresh an installed cua-driver during `haishui update` (best-effort, macOS only). Turn off
    ...
    "lsp": {
        ...
        "wait_mode": "document",
        "wait_timeout": 5.0,
        # Missing server binaries: auto = install via npm/go/pip into <HAISHUI_HOME>/lsp/bin/ on
    ...
    # (SuperGrok OAuth or XAI_API_KEY) AND the toolset is enabled in `haishui tools`.
    "x_search": {
    ...
    # encrypted vault (`haishui vault add`, Desktop → Settings → Credential Vault) is always on.
    ...
    "secrets": {
        ...
        "bitwarden": {
            ...
            # ~/.haishui/.env (or the shell), never in config.yaml.
            ...
            # Last-good fallback for NETWORK/TIMEOUT outages: AES-GCM cache under ~/.haishui/cache/,
            ...
            # Auto-download bws into ~/.haishui/bin/ on first use; False = bws must be on PATH.
            "auto_install": True,
    ...
    "computer_use": {
        # cua-driver's upstream PostHog telemetry defaults ON; Haishui sets
        ...
        # #47032; Linux/WSL2 idle spin #28152). cua-driver ≥ 0.6.x supports --no-overlay; Haishui also calls
    ...
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned binary into ~/.haishui/bin/; False = iron-proxy on PATH.
        "auto_install": True,
    ...
    "desktop": {  # Haishui Desktop (Electron) launch options; only affect `haishui desktop`.
        ...
        # Bridged to HAISHUI_DESKTOP_DISABLE_GPU: auto = disable GPU only on remote displays
        ...
        # to HAISHUI_DESKTOP_PASSWORD_STORE; ignored off-Linux.
        "password_store": "auto",
    ...
    "nous": {
        ...
        "anthropic_wire": "chat",
        # Nous free tier: with no other provider configured, Haishui sets up a free Nous identity on
        # first use (inference on nous/welcome + connectors) and offers `/login` (terminal:
        # `haishui auth upgrade`) to sign in. false turns the free tier off entirely: nothing is set
    ...
    "local_runtime": {
        # Off = detection-only (Haishui still finds an external llama-server you run).
        "enabled": False,
        # Pinned llama.cpp release tag; bumped by Haishui releases after validation.
        "tag": "b10964",
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": _prov(
        ...
        "OAuth2, not a static API key — this points at the credentials Haishui mints short-lived "
    ...
    "AZURE_FOUNDRY_BASE_URL": _prov(
        "Azure Foundry base URL (set via 'haishui model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": _tool(
        ...
        "read from the Haishui auth store)", "Tool-gateway user token", None, advanced=True),
    ...
    "PORCUPINE_ACCESS_KEY": _tool(
        "Picovoice access key for the Porcupine 'Hey Haishui' wake word engine (optional; "
    ...
    "HAISHUI_LANGFUSE_PUBLIC_KEY": _tool("Langfuse project public key (pk-lf-...)",
        "Langfuse public key", "https://cloud.langfuse.com", password=False),
    "HAISHUI_LANGFUSE_SECRET_KEY": _tool("Langfuse project secret key (sk-lf-...)",
        "Langfuse secret key", "https://cloud.langfuse.com"),
    "HAISHUI_LANGFUSE_BASE_URL": _tool("Langfuse server URL (default: https://cloud.langfuse.com)",
    ...
    "SLACK_ALLOWED_USERS": _msg(
        "Comma-separated Slack member IDs allowed to use Haishui, e.g. U01ABC2DEF3. Without "
    ...
    "MATRIX_USER_ID": _msg("Matrix user ID (e.g. @haishui:example.org)",
    ...
    "MATRIX_DEVICE_ID": _msg(
        "Stable Matrix device ID for E2EE persistence across restarts (e.g. HAISHUI_BOT)",
    ...
    "IRC_CHANNEL": _msg("IRC channel to join (e.g. #haishui)", "IRC channel", None),
    "IRC_NICKNAME": _msg("Bot nickname on IRC (default: haishui-bot)", "IRC nickname", None),
    ...
    "API_SERVER_MODEL_NAME": _msg(
        "Model name advertised on /v1/models. Defaults to the profile name (or 'haishui-agent' "
    ...
    "GATEWAY_PROXY_URL": _msg(
        "URL of a remote Haishui API server to forward messages to (proxy mode). When set, the "
        ...
        "Remote Haishui API server URL (e.g. http://192.168.1.100:8642)", None, advanced=True),
    "GATEWAY_PROXY_KEY": _msg(
        "Bearer token for authenticating with the remote Haishui API server (proxy mode). Must "
    ...
    # HAISHUI_TOOL_PROGRESS_MODE (deprecated; use display.tool_progress) is intentionally NOT listed:
    # this dict feeds user-facing surfaces (dashboard keys page, setup checklists), so deprecated
    # knobs stay in config._EXTRA_ENV_KEYS only. HAISHUI_TOOL_PROGRESS is unsupported.
    "HAISHUI_PREFILL_MESSAGES_FILE": _setting(
        "Path to JSON file with ephemeral prefill messages for few-shot priming",
        "Prefill messages file path", None),
    "HAISHUI_EPHEMERAL_SYSTEM_PROMPT": _setting(
