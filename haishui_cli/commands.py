"""Slash command registry for the Haishui CLI and gateway.
...
from haishui_constants import INDICATOR_STYLES
...
class CommandDef:
    ...
    # Key in ``haishui_cli.slash_exec.EXECUTORS`` (a string, not a callable: keeps this module
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("snapshot", "Create or restore state snapshots of Haishui config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Haishui works on across turns until achieved", "Session",
    ...
    CommandDef("plan", "Write a markdown implementation plan to .haishui/plans/ without executing anything", "Session",
               args_hint="[task]"),
    ...
    CommandDef("wake", "Toggle the 'Hey Haishui' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]", subcommands=("on", "off", "status")),
    CommandDef("busy", "Control how messages behave while Haishui is working", "Configuration",
    ...
    CommandDef("reload-skills", "Re-scan ~/.haishui/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Haishui Agent to the latest version", "Info",
               busy_policy="dispatch", desktop="terminal"),
    CommandDef("version", "Show Haishui Agent version", "Info", aliases=("v",),
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``haishui skills list`` and the agent's ``/skill-name`` dispatch but
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'discord_skill_commands_by_category': ('haishui_cli.commands_platforms', 'discord_skill_commands_by_category'),
    'slack_app_manifest': ('haishui_cli.commands_platforms', 'slack_app_manifest'),
    'slack_native_slashes': ('haishui_cli.commands_platforms', 'slack_native_slashes'),
    'slack_subcommand_map': ('haishui_cli.commands_platforms', 'slack_subcommand_map'),
    'telegram_bot_commands': ('haishui_cli.commands_platforms', 'telegram_bot_commands'),
    'telegram_menu_commands': ('haishui_cli.commands_platforms', 'telegram_menu_commands'),
    'telegram_menu_max_commands': ('haishui_cli.commands_platforms', 'telegram_menu_max_commands'),
}
