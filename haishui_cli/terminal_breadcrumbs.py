"""Per-terminal session breadcrumbs for ``haishui -c`` / ``--continue``. Strictly best-effort: no
...
def _breadcrumbs_dir() -> Path:
    ...
    return get_haishui_home() / "terminal-sessions"
...
def write_breadcrumb(session_id: str, cwd: Optional[str] = None) -> None:
    ...
    try:
        ...
        mkdir_under_haishui_home(directory)
        now = time.time()
