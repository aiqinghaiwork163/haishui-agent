...
from haishui_cli.web_routers._common import http_failure, scoped_to_thread
...
from haishui_cli.config import DEFAULT_CONFIG, OPTIONAL_ENV_VARS, read_raw_config, custom_endpoint_key_env, coerce_provider_id, find_provider_entry, get_compatible_custom_providers, redact_key, _deep_merge
from haishui_cli.config_providers import _custom_provider_entry_to_provider_config
from haishui_cli.web_models import ConfigUpdate, EnvVarUpdate, EnvVarDelete, EnvVarReveal, CustomEndpointUpdate
...
_channel_managed_env_keys = late("_channel_managed_env_keys", "haishui_cli.web_server_messaging")
...
async def update_config(
    ...
    def _run():
        ...
        # different HAISHUI_HOME than this process's gateway sessions.
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (the ``haishui model`` universe), so the desktop Keys tab renders a card even
...
def _get_env_vars_sync(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # by the SAME provider identity the CLI `haishui model` picker uses.
...
def list_custom_endpoints(profile: Optional[str] = None):
    ...
    rather than the process-level HAISHUI_HOME (mirrors ``/api/config``).
    """
