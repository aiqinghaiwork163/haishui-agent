...
from haishui_cli.web_server_files import (
...
# points the managed root at HAISHUI_HOME. Mirrors the two canonical guards
...
# trees are blocked wherever they sit under the root, no HAISHUI_HOME resolution.
...
def _decode_data_url(data_url: str) -> tuple[bytes, str]:
    from haishui_cli.web_server import _MANAGED_FILE_MAX_BYTES
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    the returned gateway-visible path under ``HAISHUI_HOME/images/`` (the same
    ...
    def _run():
        ...
        with _profile_scope(profile) as scoped_home:
            img_dir = Path(scoped_home or get_haishui_home()) / "images"
...
async def stream_upload_to_path(
    ...
    *,
    ...
    """
    from haishui_cli.web_server import _MANAGED_FILE_MAX_BYTES, _UPLOAD_CHUNK_BYTES
...
async def fs_write_text(payload: FsWriteText):
    ...
    Mirrors the Electron ``haishui:fs:writeText`` hardening: path validated by
    ...
    tmp = target.with_name(f".{target.name}.haishui-tmp-{os.getpid()}")
    try:
...
async def _fs_download_path(path: str, profile: Optional[str], session_id: Optional[str]) -> Path:
    if session_id is not None:
        from haishui_cli.web_routers.sessions import get_session_detail
    ...
    if profile is not None:
        from haishui_cli.web_server_cron import _cron_profile_home
...
async def fs_read_data_url(
    ...
    from haishui_cli.web_server import _FS_DATA_URL_MAX_BYTES
