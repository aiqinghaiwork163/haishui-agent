...
from haishui_cli.web_routers._common import (
    _CONFIG_MUTATION_LOCK, _profile_cli_args, _profile_scope, _spawn_haishui_action,
...
def _terminal_backend_rows() -> List[Dict[str, str]]:
    ...
    from haishui_cli.web_server_profiles import _TERMINAL_BACKENDS
...
def _probe_docker_backend(_cfg) -> tuple:
    ...
    ``find_docker()`` honors ``HAISHUI_DOCKER_BINARY``, then ``docker`` / ``podman``
...
def _toolset_model_catalog(ts_key: str, plugin_name: str, config: dict):
    ...
    from haishui_cli.tools_config import IMAGEGEN_BACKENDS, _plugin_image_gen_catalog, _plugin_video_gen_catalog
...
def _category_providers(ts_key: str, config: dict) -> list:
    ...
    from haishui_cli.tools_config import TOOL_CATEGORIES, _visible_providers
...
async def get_toolsets(profile: Optional[str] = None):
    from haishui_cli.tools_config import (
        ...
        gui_toolset_label)
    from haishui_cli.platforms import platform_label
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    # install `haishui tools` runs — otherwise the toggle "saves" but the tool
...
async def select_toolset_provider(
    ...
    ``haishui tools``, so both write identical keys).
    ...
    """
    from haishui_cli.tools_config import apply_provider_selection, web_provider_capabilities
...
async def run_toolset_post_setup(
    ...
    """Spawn ``haishui tools post-setup <key>`` (long-running installs) as a
    background action tailed via ``GET /api/actions/tools-post-setup/status``;
    ``profile`` is threaded so hooks see the drawer's HAISHUI_HOME."""
    from haishui_cli.tools_config import valid_post_setup_keys
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``haishui computer-use permissions grant`` (macOS-only: launches
...
_PLUGIN_COMPAT_LAZY = {
    'LateState': ('haishui_cli.web_deps', 'LateState'),
}
