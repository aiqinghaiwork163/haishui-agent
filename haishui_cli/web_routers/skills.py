...
from haishui_cli.web_server_profiles import _hub_action_name, _installed_hub_identifiers
...
# Labels per hub source id (matches `haishui skills search` provenance); keep in
# sync with create_source_router()'s source list.
_SKILL_HUB_SOURCE_LABELS = {
    "official": "Official (Nous)",
    "haishui-index": "Haishui Index",
    "skills-sh": "skills.sh",
...
def _resolve_hub_skill(ident: str, profile: Optional[str]):
    ...
    from haishui_cli.skills_hub import _resolve_source_meta_and_bundle
    from tools.skills_hub_search import create_source_router
...
# Sources subsumed by an available haishui-index (progressive per-source fan-out
