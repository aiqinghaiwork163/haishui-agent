"""Post-swap hand-off: finish ``haishui update`` in an interpreter born on the pulled code.

``haishui update`` starts in an interpreter that imported the PRE-pull tree. Once ``git merge``
...
into a hand-off file and re-executes ``haishui update --post-swap <file>`` under the venv
...
POST_SWAP_ENV = "HAISHUI_UPDATE_POST_SWAP"
...
def write_handoff(payload: dict[str, Any]) -> Path:
    """Persist the post-swap payload under HAISHUI_HOME; returns its path."""
    from haishui_constants import get_haishui_home

    directory = get_haishui_home() / "logs" / "update_receipts"
...
def _running_from_windows_shim() -> bool:
    from haishui_cli.main_install_repair import _windows_shim_in_process_chain
...
def post_swap_command(handoff_path: Path, argv_tail: list[str]) -> list[str]:
    """``python -m haishui_cli.main update <original flags> --post-swap <file>``."""
    return [str(post_swap_python()), "-m", "haishui_cli.main", "update", *argv_tail, "--post-swap", str(handoff_path)]


def post_swap_child_env() -> dict[str, str]:
    """Environment for the child. ``HAISHUI_UPDATE_REEXEC`` marks it as already off the Windows
    ...
    from haishui_cli.main_install_repair import _UPDATE_REEXEC_ENV
    from haishui_cli.update_lock import HANDOFF_PID_ENV
...
def continue_update_in_fresh_interpreter(payload: dict[str, Any], *, argv_tail: list[str]) -> int | None:
    ...
    ``haishui.exe``, the child cannot be awaited: the shim is one of the files the dependency
    ...
    if _running_from_windows_shim():
        try:
            ...
            return None
        print("→ Windows: haishui.exe cannot replace itself while it runs; the update")
