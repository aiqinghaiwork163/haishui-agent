"""CLI subcommand: ``haishui send`` — pipe text from shell scripts to any configured messaging platform
...
def _read_message_body(positional: Optional[str], file_path: Optional[str]) -> Optional[str]:
    ...
    if file_path:
        ...
        try:
            ...
            _fail(
                f"haishui send: {file_path} is not a text file. --file reads the "
                ...
                f'  haishui send --to telegram "MEDIA:{file_path}"\n'
                f'  haishui send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  haishui send --to telegram "[[as_document]] MEDIA:{file_path}"',
                _USAGE_EXIT)
        except OSError as exc:
            _fail(f"haishui send: cannot read {file_path}: {exc}", _USAGE_EXIT)
...
def _emit_result(result_json: str, *, json_mode: bool, quiet: bool) -> int:
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"haishui send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        return _fail(f"haishui send: failed to load channel directory: {exc}")
    try:
        raw = load_directory()
    except Exception as exc:
        return _fail(f"haishui send: failed to read channel directory: {exc}")
    ...
    if platform_filter:
        ...
        if not filtered:
            return _fail(
                f"haishui send: no targets found for platform '{platform_filter}'. "
    ...
    if not platforms:
        ...
        print("Set one up with `haishui gateway setup`, or run the gateway once so")
        from haishui_constants import get_default_haishui_root, get_haishui_home, haishui_home_key
        home, root = get_haishui_home(), get_default_haishui_root()
        ...
        if haishui_home_key(root) != haishui_home_key(home) and (root / "channel_directory.json").exists():
...
def _load_haishui_env() -> None:
    """Populate the credential environment from ``<HAISHUI_HOME>/.env`` AND bridge top-level ``config.yaml``
    ...
    if isinstance(scope, dict):
        ...
        if env_path.exists():
            try:
                from haishui_cli.env_loader import _load_dotenv_with_fallback
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    _load_haishui_env()  # the downstream gateway config loader reads credentials from os.environ
    if getattr(args, "list_targets", False):  # --list short-circuits everything else
        # `haishui send --list telegram` lands "telegram" in the `message` positional.
        exit_code = _list_targets(getattr(args, "message", None), json_mode=getattr(args, "json", False))
        sys.exit(exit_code)
    target = (getattr(args, "to", None) or "").strip()
    if not target:
        _fail(
            "haishui send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  haishui send --to telegram \"hello\"\n"
            "  haishui send --to discord:#ops --file report.md\n"
            "  haishui send --list      # list available targets",
            _USAGE_EXIT)
    mentions = list(getattr(args, "mentions", None) or [])
    if mentions and target.split(":", 1)[0].strip().lower() != "whatsapp":
        _fail("haishui send: --mention is only supported for WhatsApp targets.", _USAGE_EXIT)
    invalid_mentions = _invalid_whatsapp_mentions(mentions)
    if invalid_mentions:
        _fail(
            "haishui send: invalid --mention value(s): "
    ...
    if message is None or not message.strip():
        _fail(
            "haishui send: no message provided. Pass text as a positional "
    ...
    # Lazy import keeps `haishui send --help` fast (no tool registry / gateway config stack).
...
_SEND_ARGUMENTS = (
    ...
    (("-l", "--list"), dict(dest="list_targets", action="store_true", default=False,
                            help="List available targets. Optional positional filter: `haishui send --list telegram`.")),
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Haishui "
            "is already configured for. Reuses the gateway's platform "
            f"credentials ({haishui_home / '.env'} + "
            f"{haishui_home / 'config.yaml'}) — no LLM, "
        ...
        epilog=(
            ...
            "  haishui send --to whatsapp:GROUP@g.us --mention 15551234567 \"@15551234567 hello\"\n"
            "  haishui send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
