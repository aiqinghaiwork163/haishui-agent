...
# Profiles must never receive a copy; each profile signs in on its own (``haishui -p <name> auth add``).
...
def strip_cloned_single_use_oauth_grants(profile_dir: Path) -> Dict[str, Any]:
    ...
    Called after any path that copies credential files between profiles (``haishui profile create
    ...
    """
    from haishui_cli.auth import _same_path, _save_auth_store
    ...
    try:
        ...
        root_auth_path = get_default_haishui_root() / "auth.json"
