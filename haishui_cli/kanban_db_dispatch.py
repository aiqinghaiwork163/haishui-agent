...
Split out of ``haishui_cli.kanban_db``; origin-resident helpers are reached
...
from haishui_cli.quiet_single_query import KANBAN_WORKER_EXIT_TRAILER

if TYPE_CHECKING:
    from haishui_cli.kanban_db import Task
...
# ``HAISHUI_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``.
...
class DispatchResult:
    ...
    Code terminal like ``orion-cc``), not a Haishui profile. Expected steady-state
...
def describe_suppression(results: Iterable[Optional["DispatchResult"]]) -> str:
    ...
    bare zero-spawn count while ``haishui kanban tail`` is the only place the
...
# children): a per-tick ``haishui kanban dispatch`` process finds it empty, so
...
def _worker_log_exit_code(task_id: str, board: Optional[str] = None) -> Optional[int]:
    ...
    (``haishui_cli.quiet_single_query.exit_single_query``), so it is there whether
...
_LOG_CHROME = re.compile(r"[─━═╭╮╰╯│┃┌┐└┘]+|☤\s*Haishui")
    ...
    if sweep.exited_hook_payloads and _kb._kanban_observer_consumed("on_kanban_worker_exited"):
        ...
        for hook_fields in sweep.exited_hook_payloads:
            ...
            _kb._fire_kanban_lifecycle_hook(
                ...
                # fire in the DISPATCHER process (gateway-embedded dispatcher or ``haishui kanban
...
def _profile_exists_fn() -> Optional[Callable[[str], bool]]:
    """``haishui_cli.profiles.profile_exists``, or ``None`` when it cannot be
    ...
    """
    try:
        from haishui_cli.profiles import normalize_profile_name, profile_exists
    except Exception:
        return None
...
def _dispatch_profile_allowlist(normalize_profile_name) -> Optional[frozenset]:
    ...
    On a shared board (one ``kanban.db`` mounted across several Haishui homes),
...
def dispatch_profile_allowlist_summary() -> str:
    ...
    Surfaced by ``haishui kanban diagnostics`` so an operator on a shared board
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    """True iff a ready+assigned+unclaimed task maps to a real Haishui profile.
...
def review_dispatch_enabled() -> bool:
    """Whether review tasks dispatch automatically. Default true (Haishui ships
...
def count_running_tasks_other_boards(board: Optional[str] = None) -> int:
    ...
    Boards are matched by resolved DB path, so ``HAISHUI_KANBAN_DB`` (pins every
...
def _dispatch_lane_task(
    ...
    *,
    ...
    # would fail ``haishui -p <assignee>`` at startup and loop ready→crash→ready
    ...
    if guard_reason is not None:
        ...
        # Event so ``haishui kanban tail`` shows why the task looks stuck.
...
def _module_haishui_argv() -> list[str]:
    """Interpreter-bound Haishui CLI invocation (``haishui_cli.main`` is the
    console-script target — there is no top-level ``haishui`` package)."""
    return [sys.executable, "-m", "haishui_cli.main"]


def _absolute_haishui_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Haishui shim."""
...
def _haishui_path_argv(path: str) -> list[str]:
    """argv for a resolved Haishui executable path. Windows batch shims
    ...
    if _kb._IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_haishui_argv()
    return [_absolute_haishui_path(path)]


def _resolve_haishui_argv() -> list[str]:
    """Resolve the ``haishui`` invocation as argv for ``Popen``: ``$HAISHUI_BIN``
    ...
    haishui_cli.main`` (exactly this install; also covers shim-less cron,
    systemd ``User=``, launchd), then ``which("haishui")`` (Windows: safe PATH
    search, batch shims fall back to the module form) only when ``haishui_cli``
    is not importable. The module argv must win over PATH: a PATH-first lookup
    lets an attacker-planted ``haishui`` shadow the running install (#111569).
    Mirrors ``gateway.run._resolve_haishui_bin``; local because ``haishui_cli``
    sits below ``gateway`` in the dependency order.
    """
    import importlib.util
    import shutil

    env_bin = os.environ.get("HAISHUI_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _haishui_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _haishui_path_argv(resolved_env_bin)
        return _module_haishui_argv()
    ...
    haishui_bin = _safe_which_no_cwd("haishui") if _kb._IS_WINDOWS else shutil.which("haishui")
    if haishui_bin:
        return _haishui_path_argv(haishui_bin)
    return _module_haishui_argv()
...
def _resolve_worker_cli_toolsets(haishui_home: Optional[str]) -> Optional[list[str]]:
    ...
    tools when ``HAISHUI_KANBAN_TASK`` is set.
    """
    if not haishui_home:
        return None
    try:
        ...
        token = set_haishui_home_override(haishui_home)
        ...
        secret_token = (
            set_secret_scope(build_profile_secret_scope(Path(haishui_home)))
        ...
        _kb._log.debug(
            "kanban worker: could not resolve CLI toolsets for HAISHUI_HOME=%r (%s)",
            haishui_home,
            exc,
        )
        return None
...
def _worker_argv(task: Task, profile_arg: str, haishui_home: Optional[str]) -> list[str]:
    """Build the ``haishui -p <profile> --cli ... chat -q ...`` worker command."""
    cmd = [
        *_resolve_haishui_argv(),
        "-p", profile_arg,
        ...
        "--cli",
        # Workers run under a profile-scoped HAISHUI_HOME and so see that
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(haishui_home)
    if worker_toolsets:
...
def _open_worker_log(task: Task, board: Optional[str]):
    ...
    `haishui kanban log` reads its own file and boards sharing task ids don't
    collide."""
...
def _default_spawn(task: Task, workspace: str, *, board: Optional[str] = None) -> Optional[int]:
    """Fire-and-forget ``haishui -p <profile> chat -q ...`` subprocess.
    ...
    ``HAISHUI_KANBAN_DB`` / ``HAISHUI_KANBAN_BOARD`` / workspaces_root to the
    board the task was claimed from, so workers cannot see other boards.
    """
    if not task.assignee:
        raise ValueError(f"task {task.id} has no assignee")

    from haishui_cli.profiles import normalize_profile_name, resolve_profile_env
    ...
    try:
        ...
        # HAISHUI_PROFILE (set below) instead.
        profile_home = None
    ...
    # Inject HAISHUI_HOME so the worker reads the profile-scoped config.yaml:
    # without it the child's get_haishui_home() falls back to the DEFAULT
    # profile root because `haishui -p` applies its override before
    # haishui_constants is imported.
    if profile_home:
        env["HAISHUI_HOME"] = profile_home
    ...
    if task.tenant:
        env["HAISHUI_TENANT"] = task.tenant
    env["HAISHUI_KANBAN_TASK"] = task.id
    env["HAISHUI_KANBAN_WORKSPACE"] = workspace
    ...
    env["HAISHUI_SESSION_SOURCE"] = "kanban"
    ...
    if task.branch_name:
        env["HAISHUI_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["HAISHUI_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["HAISHUI_KANBAN_CLAIM_LOCK"] = task.claim_lock
    ...
    if task.goal_mode:
        env["HAISHUI_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["HAISHUI_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # match after `haishui -p` rewrites HAISHUI_HOME (symlink / Docker layouts).
    env["HAISHUI_KANBAN_DB"] = str(_kb.kanban_db_path(board=board))
    env["HAISHUI_KANBAN_WORKSPACES_ROOT"] = str(_kb.workspaces_root(board=board))
    _retag_legacy_worker_sessions(env["HAISHUI_KANBAN_WORKSPACES_ROOT"])
    # Board slug — defense-in-depth pin if a path is resolved without the
    # DB / workspaces env vars.
    env["HAISHUI_KANBAN_BOARD"] = _kb._normalize_board_slug(board) or _kb.get_current_board()
    # kanban_comment reads HAISHUI_PROFILE for its default author; `-p` alone
    # doesn't set the env var.
    env["HAISHUI_PROFILE"] = profile_arg
    ...
    # `--cli` is the highest-precedence TUI override; dropping HAISHUI_TUI covers
    # older haishui builds on PATH that predate the flag's precedence.
    env.pop("HAISHUI_TUI", None)

    cmd = _worker_argv(task, profile_arg, env.get("HAISHUI_HOME"))
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`haishui` executable not found on PATH. "
            "Install Haishui Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    the gateway dispatcher and ``haishui kanban dispatch`` — the standalone
...
from haishui_cli import kanban_db as _kb  # noqa: E402
from haishui_cli import kanban_db_connect as _kbc  # noqa: E402
from haishui_cli import kanban_db_workspace as _kbw  # noqa: E402
