...
All HttpOnly, ``SameSite=Lax`` unless noted, Path = proxy prefix or /: ``haishui_session_at``
(access token; Max-Age = token TTL), ``haishui_session_rt`` (rotating refresh token; written only
when the provider returned one, always cleared on logout/expiry), ``haishui_session_provider``
(non-secret routing hint so an RT is not handed to the wrong provider), ``haishui_session_pkce``
...
cookies set on such a 302, crbug 40508226), ``haishui_sso_attempt`` (auto-SSO loop guard, 60 s).
...
SESSION_AT_COOKIE = "haishui_session_at"
SESSION_RT_COOKIE = "haishui_session_rt"
SESSION_PROVIDER_COOKIE = "haishui_session_provider"
PKCE_COOKIE = "haishui_session_pkce"
SSO_ATTEMPT_COOKIE = "haishui_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    """``Path=/haishui`` under a proxy prefix (no leak to sibling apps), else ``/``."""
...
def set_session_cookies(
    ...
    Nous Portal issues a 24h rotating refresh token (haishui #37247); a provider that omits it returns
