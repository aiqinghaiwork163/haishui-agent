"""Plugin catalog — curated, Nous-approved Haishui plugins shipped with the repo.
...
an installed Haishui sees new entries and removals without updating. Any fetch failure falls back to the
...
LIVE_CATALOG_URL = "https://aiqinghaiwork163.github.io/haishui-agent/docs/api/plugin-catalog.json"
...
class PluginCatalogEntry:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "requires_haishui": self.requires_haishui,
...
def entry_from_mapping(data: Any, label: str) -> Optional[PluginCatalogEntry]:
    ...
    return PluginCatalogEntry(
        ...
        requires_haishui=str(data.get("requires_haishui") or "").strip(),
...
def _live_cache_path() -> Path:
    ...
    return get_haishui_home() / "cache" / "plugin-catalog.json"
...
def fetch_live_catalog(*, force: bool = False) -> Optional[Dict[str, Any]]:
    ...
    ``HAISHUI_HOME/cache`` for :data:`LIVE_CATALOG_TTL_SECONDS`. ``None`` on ANY failure — callers fall
    ...
    try:
        ...
        mkdir_under_haishui_home(cache.parent)
...
def entry_capability_summary(entry: PluginCatalogEntry) -> str:
    ...
    if entry.requires_haishui:
        bits.append(f"Requires Haishui {entry.requires_haishui}.")
    return " ".join(bits)
