...
def _strip_haishui_path_lines(content: str) -> str:
    """Drop the ``# Haishui Agent`` marker (+ its PATH line) and any haishui PATH line; squash blank runs."""
    new_lines = []
    skip_next = False
    for line in content.split('\n'):
        if '# Haishui Agent' in line or '# haishui-agent' in line:
            skip_next = True
            continue
        if skip_next and ('haishui' in line.lower() and 'PATH' in line):
            skip_next = False
            continue
        skip_next = False
        if 'haishui' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
            continue
...
def remove_path_from_shell_configs():
    """Remove Haishui PATH entries from shell configuration files."""
    removed_from = []
    for config_path in (c for c in (Path.home() / n for n in _SHELL_RC_NAMES) if c.exists()):
        try:
            content = config_path.read_text(encoding="utf-8")
            new_content = _strip_haishui_path_lines(content)
...
def remove_wrapper_script():
    """Remove the haishui wrapper script if it exists."""
    def _unlink_ours(wrapper: Path) -> bool:
        # Only our wrapper (contains a haishui_cli / haishui-agent reference).
        content = wrapper.read_text(encoding="utf-8")
        return _unlink_if('haishui_cli' in content or 'haishui-agent' in content, wrapper)

    candidates = (
        bin_dir / name
        for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin"))
        for name in ("haishui", "haishui-acp", "haishui-agent"))
...
def remove_node_symlinks(haishui_home: Path) -> list:
    ...
    node_dir = (haishui_home / "node").resolve()
...
def uninstall_gateway_service():
    ...
    try:
        from haishui_cli.gateway import kill_gateway_processes, find_gateway_pids
...
def _remove_launchd_gateway() -> bool:
    ...
    from haishui_cli.gateway import get_launchd_plist_path
...
def _remove_windows_gateway() -> bool:
    ...
    from haishui_cli import gateway_windows as gw
...
# HAISHUI_HOME / HAISHUI_GIT_BASH_PATH (HKCU\Environment), User-scope PATH entries
# (%LOCALAPPDATA%\haishui\git\{cmd,bin,usr\bin}, ...\haishui\node), PortableGit + Node copies
...
def _haishui_path_markers(haishui_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Prefixes identifying Haishui-owned User-PATH entries (prefix match sweeps git\cmd, git\bin,
    ...
    root = str(haishui_home).rstrip("\\/")
    subs = ("haishui-agent", "git", "node", "venv") + (("bin",) if include_managed_bin else ())
    return [f"{root}\\{sub}" for sub in subs]


def remove_path_from_windows_registry(haishui_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Strip Haishui-owned entries from User-scope PATH in the registry (see ``_haishui_path_markers``)."""
    markers = tuple(m.lower() for m in _haishui_path_markers(haishui_home, include_managed_bin=include_managed_bin))
...
def remove_haishui_env_vars_windows() -> list[str]:
    """Delete HAISHUI_HOME and HAISHUI_GIT_BASH_PATH from User-scope env vars."""
    def edit(winreg, key, removed):
        for name in ("HAISHUI_HOME", "HAISHUI_GIT_BASH_PATH"):
            try:
...
def remove_portable_tooling_windows(haishui_home: Path) -> list[Path]:
    """Delete the PortableGit / Node / gateway-service dirs the Windows installer created under
    ``haishui_home`` (isolated from any system Git / Node, so nothing else breaks)."""
    targets = (haishui_home / sub for sub in ("git", "node", "gateway-service"))
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    """Delete the ``haishui`` launchers install.ps1 staged in the managed ``bin`` dir: every mode
    ...
    try:
        ...
        from haishui_cli._install_repair import _WINDOWS_BIN_LAUNCHERS
        from haishui_constants import get_default_haishui_root
        bin_dir = get_default_haishui_root() / "bin"
    except Exception as e:
...
def _is_default_haishui_home(haishui_home: Path) -> bool:
    """Return True when ``haishui_home`` points at the default (non-profile) root."""
    try:
        from haishui_constants import get_default_haishui_root
        return haishui_home.resolve() == get_default_haishui_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    out to ``haishui -p <name> gateway stop|uninstall`` because service names / unit paths derive
    from the current HAISHUI_HOME and can't be switched in-process."""
    name = profile.name
    log_info(f"Uninstalling profile '{name}'...")

    # 1. Gateway service, via `python -m haishui_cli.main` (the `haishui` wrapper may be half-gone).
    haishui_invocation = [sys.executable, "-m", "haishui_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                haishui_invocation + ["gateway", subcmd], capture_output=True, text=True,
    ...
    # 3. Wipe the profile's HAISHUI_HOME directory.
    _rmtree_step(profile.path, indent="  ", fully=False)


def run_gui_uninstall(args):
    """``haishui uninstall --gui``: remove the desktop app's built artifacts, packaged bundle
    (best-effort) and Electron userData — never config/sessions/.env, the agent or its venv."""
    from haishui_cli.gui_uninstall import agent_is_installed, gui_install_summary, uninstall_gui
    haishui_home = get_haishui_home()
    summary = gui_install_summary(haishui_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    _print_box("│         ☤ Haishui Chat GUI Uninstaller                  │", Colors.MAGENTA)
    print()

    if not summary["gui_installed"]:
        print("No Haishui Chat GUI installation was found.")
        print(f"  Checked: {haishui_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Haishui agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(haishui_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Haishui agent at {haishui_home / 'haishui-agent'}")
        print(f"  • Your config, sessions, and secrets under {haishui_home}")
        print()
    ...
    print()
    uninstall_gui(haishui_home)

    print()
    _print_box("│            ✓ Chat GUI Uninstalled!                      │", Colors.GREEN)
    print()
    print("The Haishui agent is still installed. Run 'haishui' to use the CLI,")
    print("or 'haishui uninstall' to remove the agent too.")
    print()


def run_uninstall(args):
    """Interactive/``--yes`` uninstall: full removes code and ``~/.haishui/``; keep-data removes only
    ...
    if bool(getattr(args, "dry_run", False)):
        _print_uninstall_dry_run(
            project_root=project_root, haishui_home=haishui_home, full_uninstall=full_flag)
        return

    # Named profiles (only when uninstalling from the default root) are offered for cleanup too,
    # instead of leaving zombie HAISHUI_HOMEs and systemd units behind.
    is_default_profile = _is_default_haishui_home(haishui_home)
    ...
    print()
    _print_box("│            ☤ Haishui Agent Uninstaller                  │", Colors.MAGENTA)
    print()

    # Show what will be affected
    print(color("Current Installation:", Colors.CYAN, Colors.BOLD))
    print(f"  Code:    {project_root}")
    print(f"  Config:  {haishui_home / 'config.yaml'}")
    print(f"  Secrets: {haishui_home / '.env'}")
    print(f"  Data:    {haishui_home / 'cron/'}, {haishui_home / 'sessions/'}, {haishui_home / 'logs/'}")
    print()
    ...
    # services, alias wrappers, HAISHUI_HOME dirs) — otherwise they leave zombie services behind.
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Haishui data!", Colors.RED, Colors.BOLD))
        print(color("   Including: configs, API keys, sessions, scheduled jobs, logs", Colors.RED))
        if remove_profiles:
            print(color(f"   Plus {n_profiles} profile(s): {profile_names}", Colors.RED))
    else:
        print("This will remove the Haishui code but keep your configuration and data.")
    ...
    _perform_uninstall(
        project_root=project_root, haishui_home=haishui_home, full_uninstall=full_uninstall,
        remove_profiles=remove_profiles, named_profiles=named_profiles)


def _print_uninstall_dry_run(*, project_root: Path, haishui_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Haishui PATH entries from shell configs / Windows User PATH")
    print("  • Haishui wrapper scripts and Haishui-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if not full_uninstall:
        print(f"  • Keep Haishui config/data: {haishui_home}")
    else:
        print(f"  • Haishui config/data: {haishui_home}")
        profiles = _discover_named_profiles() if _is_default_haishui_home(haishui_home) else []
        if profiles:
...
def _perform_uninstall(
    *,
    ...
    the checkout -> (Windows) PortableGit/Node -> optionally ``$HAISHUI_HOME`` and named profiles."""
    print()
    ...
    # 2-3b. PATH entries, wrapper, Windows launchers, node symlinks. Windows: haishui_home is
    #    %VAR%-expanded because install.ps1 writes literal C:\Users\<u>\...; haishui\bin (launchers +
    ...
    windows = _is_windows()
    sweep_managed_bin = windows and full_uninstall and _is_default_haishui_home(haishui_home)
    for on_this_platform, label, remove, success_fmt, none_msg in (
        ...
        (windows, "Removing PATH entries from Windows User environment...",
         lambda: remove_path_from_windows_registry(
             Path(os.path.expandvars(str(haishui_home))), include_managed_bin=sweep_managed_bin),
         "Removed from User PATH: {}", "No Haishui-owned PATH entries in User environment"),
        (windows, "Removing HAISHUI_HOME / HAISHUI_GIT_BASH_PATH User env vars...",
         remove_haishui_env_vars_windows, "Removed User env var: {}", "No Haishui-set User env vars to remove"),
        (True, "Removing haishui command...", remove_wrapper_script, "Removed {}", "No wrapper script found"),
        (windows, "Removing Windows haishui launchers...",
         remove_windows_bin_launchers, "Removed {}", "No Windows haishui launchers found"),
        (True, "Removing Haishui-managed node/npm/npx symlinks...",
         lambda: remove_node_symlinks(haishui_home), "Removed {}", "No Haishui-managed node/npm/npx symlinks found"),
    ):
        if on_this_platform:
    ...
    #     .env (safe in keep-data mode); the packaged app + Electron userData live OUTSIDE HAISHUI_HOME.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from haishui_cli.gui_uninstall import uninstall_gui
        if not uninstall_gui(haishui_home):
    ...
    if windows:
        _remove_step(
            ...
            lambda: remove_portable_tooling_windows(haishui_home), "Removed {}",
            "No Windows installer artifacts to remove")

    # 5. Optionally remove ~/.haishui/ data directory (and named profiles)
    if full_uninstall:
        ...
        _rmtree_step(haishui_home)
    else:
        log_info(f"Keeping configuration and data in {haishui_home}")
    ...
    print()
    print("Thank you for using Haishui Agent! ☤")
    print()
...
def main(argv=None) -> int:
    """``python -m haishui_cli.uninstall --mode <gui|lite|full>``. Imports only stdlib +
    ``haishui_constants`` + ``haishui_cli.colors``, so it runs under a bare system Python (no venv)."""
    import argparse
    parser = argparse.ArgumentParser(prog="python -m haishui_cli.uninstall")
    parser.add_argument(
