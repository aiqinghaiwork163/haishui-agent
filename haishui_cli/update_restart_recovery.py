...
- **gateway profiles**, relaunched through the existing per-profile
  ``haishui_cli.main -p <profile> gateway restart`` command; and
- **``haishui-serve*`` systemd units**, restarted directly through
  ``systemctl``.  ``haishui serve`` is not a gateway profile and has no
  ...
  inventory, so a manually launched or Desktop-owned ``haishui serve`` — which
  has no relaunch authority — can never enter this path.

Serve-unit identity is always ``<scope>/<unit>`` (``user/haishui-serve``,
``system/haishui-serve``).  The two managers can each own a unit of the same
...
_RECOVERY_ENV = "HAISHUI_UPDATE_RESTART_RECOVERY"
_GATEWAY_MARKERS = ("_HAISHUI_GATEWAY", "HAISHUI_GATEWAY", "HAISHUI_GATEWAY_MODE")
_PROFILE_RESTART_TIMEOUT = 90
_VERIFY_TIMEOUT = 15
_PROFILE_ID_RE = re.compile(r"^[a-z0-9][a-z0-9_-]{0,63}$")
_SUPERVISOR_RE = re.compile(r"^[a-z0-9][a-z0-9_-]{0,31}$")
_UNIT_RE = re.compile(r"^haishui-serve(-[a-z0-9][a-z0-9_-]{0,63})?\.service$")
_SERVE_UNIT_PATTERN = "haishui-serve*"
...
def _launch_profile() -> str:
    """Profile this recovery process was launched as: ``<root>/profiles/<name>`` in ``HAISHUI_HOME`` names it,
    anything else is the default profile. Mirrors ``haishui_cli.profiles.profile_root_for_env_home`` without
    importing it — this module stays stdlib-only at import time."""
    home = os.environ.get("HAISHUI_HOME", "").strip()
    if home:
...
def _run_profile_restart(profile: str, *, run: Callable[..., Any]) -> bool:
    ...
    argv = [sys.executable, "-m", "haishui_cli.main", "-p", profile, "gateway", "restart"]
...
def _systemd_unit_candidates(profile: str) -> tuple[str, ...]:
    ...
    if profile == "default":
        return ("haishui-gateway.service", "gateway.service", "gateway-default.service")
    return (f"haishui-gateway-{profile}.service", f"gateway-{profile}.service")
...
def _systemctl_scopes() -> list[tuple[str, list[str]]]:
    ...
    ``systemctl`` comes from ``shutil.which`` so this module never imports a Haishui platform helper —
...
def _listed_serve_units(scope: list[str], *, run: Callable[..., Any]) -> list[str]:
    ...
    if result is None:
        return []
    # The glob is a systemd pattern, not a name gate (`haishui-serve*` also matches
    # `haishui-server.service`): require the exact base unit or the profile family.
    units: list[str] = []
...
def _normalized_skips(skip_units: Iterable[Any]) -> tuple[set[tuple[str, str]], set[str]]:
    """Split already-settled units into scope-qualified and legacy entries (a bare ``"haishui-serve"``
...
def restart_serve_units(
    *, skip_units: Iterable[Any] = (), run: Callable[..., Any] = subprocess.run, sleep: Callable[[float], Any] = time.sleep
    ...
    """Restart every active ``haishui-serve*`` systemd unit from this process.

    Units are enumerated from systemd, never from the update inventory, so a manually launched or
    Desktop-owned ``haishui serve`` (no unit) structurally cannot be touched here.
    """
...
def _parse_payload(stream) -> tuple[list[str], dict[str, str], bool, list[str]]:
    ...
    if raw_serve is not None:
        ...
        for entry in raw_skip:
            ...
            # scope (`haishui-serve.service` can exist in BOTH managers; settling one says nothing about
