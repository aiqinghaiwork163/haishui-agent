"""Configuration management for Haishui Agent: config.yaml / .env loading, saving,
validation, migration, and the ``haishui config`` command."""
...
# dies with ImportError before any later heal point is reached. See haishui_cli.stale_modules.
from haishui_cli.stale_modules import drop_stale_root_modules
...
from haishui_cli import managed_scope
from haishui_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
from haishui_cli.secret_prompt import masked_secret_prompt
# Re-export from haishui_constants — canonical definition lives there.
from haishui_constants import get_haishui_home, get_process_haishui_home  # noqa: F401
...
_PARSE_FAILURE_FALLBACK_MSG = {
    "last-known-good": "Haishui is running on the settings it loaded before the edit until it is fixed, so recent changes are not applied.",
    "last-known-good-backup": "Haishui is running on your last good settings until it is fixed, so recent changes are not applied.",
    "refuse-write": "Nothing was written, so the existing file is preserved."}
_PARSE_FAILURE_DEFAULTS_MSG = (
    "Haishui is running on default settings until it is fixed, so none of your saved settings are applied.")
_PARSE_FAILURE_REPAIR_MSG = "Open it with `haishui config edit`, fix {where}, then run `haishui config check`."
...
def format_config_parse_failure(config_path: Path, exc: Exception, *, fallback: str = "defaults") -> str:
    """User copy for an unparseable config.yaml: what happened, what Haishui is doing, how to fix.
...
def _warn_config_parse_failure(
    ...
    from haishui_cli.config_backups import backup_config
    ...
    try:
        sys.stderr.write(f"⚠️  haishui config: {msg}\n    Details: {_yaml_error_details(exc)}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
# interpreter init (PYTHON*, NODE_*: Haishui restarts through them), PATH (fix tool lookup
...
# and Haishui runtime-location / security-policy flags (config.yaml is the supported surface).
#
# ``HAISHUI_*`` overall is NOT blocked — many integration credentials use that prefix
# (HAISHUI_LANGFUSE_PUBLIC_KEY, HAISHUI_SPOTIFY_CLIENT_ID, ...). The denylist is name-by-name so
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Haishui runtime location
    "HAISHUI_HOME", "HAISHUI_PROFILE", "HAISHUI_CONFIG", "HAISHUI_ENV",
    "HAISHUI_CONFIG_PATH", "HAISHUI_ENV_PATH",
    # MCP catalog trust root; package-manager wrappers may still set it in the process env.
    "HAISHUI_OPTIONAL_MCPS",
    # Local ACP subprocess selection (executable/argv authority).
    "HAISHUI_COPILOT_ACP_COMMAND", "HAISHUI_COPILOT_ACP_ARGS",
    # Security policy / approval-routing context — set via their dedicated controls only.
    "HAISHUI_YOLO_MODE", "HAISHUI_ACCEPT_HOOKS", "HAISHUI_REDACT_SECRETS",
    "HAISHUI_INTERACTIVE", "HAISHUI_EXEC_ASK", "HAISHUI_GATEWAY_SESSION",
    "HAISHUI_CRON_SESSION", "HAISHUI_SINGLE_QUERY_SESSION",
    "HAISHUI_SESSION_KEY", "HAISHUI_SESSION_PLATFORM"})
...
def validate_env_var_name_for_write(key: str) -> None:
    ...
    if _env_var_policy_name(key) in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "or Haishui runtime location and security policy (HAISHUI_HOME, HAISHUI_YOLO_MODE, ...) "
            "cannot be persisted via the env writer. If you really need this, edit ~/.haishui/.env "
            "directly.")
...
# acquires it again (haishui_cli/plugins.py: `with ..., config_mod._CONFIG_LOCK:` then
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # back-compat fallback. The boolean HAISHUI_TOOL_PROGRESS variant is unsupported (its only
    # consumer, the v3->4 migration, is below the v12 support floor); doctor flags it as ignored.
    "HAISHUI_TOOL_PROGRESS_MODE",
    ...
    "HAISHUI_LANGFUSE_ENV", "HAISHUI_LANGFUSE_RELEASE", "HAISHUI_LANGFUSE_SAMPLE_RATE",
    "HAISHUI_LANGFUSE_MAX_CHARS", "HAISHUI_LANGFUSE_CAPTURE", "HAISHUI_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY", "LANGFUSE_SECRET_KEY", "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so profiles can use different backends.
    "HAISHUI_ACP_AUTH_METHOD", "HAISHUI_ACP_AUTO_APPROVE", "HAISHUI_COPILOT_ACP_COMMAND",
    "HAISHUI_COPILOT_ACP_ARGS", "COPILOT_CLI_PATH", "COPILOT_ACP_BASE_URL"})
...
# HAISHUI_MANAGED). Module-level so tests can patch it without touching /nix/store.
...
# Explicit opt-out (``HAISHUI_MANAGED=false``): without this a bool-shaped value became a package
# manager literally named "false" and is_managed() blocked `haishui update` (#12864).
_MANAGED_FALSE_VALUES = frozenset({"false", "0", "no", "off"})


def get_managed_system() -> Optional[str]:
    """Return the package manager owning this install, if any.
    Signals: HAISHUI_MANAGED env var (systemd service) or a ``.managed`` marker file in
    HAISHUI_HOME (NixOS activation script — interactive shells don't see the service env)."""
    marker = os.getenv("HAISHUI_MANAGED", "").strip().lower() or None
    managed_marker = get_haishui_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Haishui is running in package-manager-managed mode."""
...
_NIX_UPDATE_MSG = (
    "Update Haishui through the Nix source that installed it "
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Haishui was installed: apt/docker/nix/nixos/home-manager/git/unknown.
    Order: code-scoped ``<install tree>/.install_method`` stamp (authoritative) -> legacy
    ``$HAISHUI_HOME/.install_method`` -> managed marker -> /nix/store path -> .git dir -> unknown.
    The stamp lives next to the code because HAISHUI_HOME is shared data: a container and a host
    install can bind-mount the same home, so a home-scoped ``docker`` stamp would make the host
    ``haishui update`` refuse to run. A legacy ``docker`` value is therefore ignored unless we are
    ...
    the code; - the published ``aiqinghaiwork163/haishui-agent`` image bakes a ``docker`` stamp into
    ``/opt/haishui`` at build time. An unsupported manual install dropped into a container (no stamp) falls
    through to the ``.git`` checks and behaves like any off-path install. See issue #34397.
    """
    # The stamp is a property of the running code tree (parent of haishui_cli/), NOT of $HAISHUI_HOME,
    ...
    method = _install_method_stamp(get_haishui_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Import-safe wrapper around ``haishui_constants.is_container``."""
    try:
...
_UPDATE_COMMAND_BY_METHOD = {
    "docker": "docker pull aiqinghaiwork163/haishui-agent:latest",
    "apt": "pkg upgrade haishui-agent",  # "apt" == Termux APT by contract; uses Termux's `pkg`.
}
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    return _UPDATE_COMMAND_BY_METHOD.get(method, "haishui update")
...
# Shared by ``cmd_update`` and ``_cmd_update_check`` (haishui_cli/main.py) so the wording never
# forks. The published image excludes ``.git``, so the git update path can never succeed there
# and the generic "reinstall via install.sh" fallback would install a NEW host-side Haishui.
_DOCKER_UPDATE_MESSAGE = """\
✗ ``haishui update`` doesn't apply inside the Docker container.

Haishui Agent runs as a published image (aiqinghaiwork163/haishui-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull aiqinghaiwork163/haishui-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate haishui-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm aiqinghaiwork163/haishui-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/aiqinghaiwork163/haishui-agent/tags
  • Your config and session history live under ``$HAISHUI_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``haishui update`` inside Docker."""
    return _DOCKER_UPDATE_MESSAGE


def format_managed_message(action: str = "modify this Haishui installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    return (
        f"Cannot {action}: this Haishui installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Haishui.")
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from HAISHUI_HOME/.container-mode.
    ...
    already inside the container, or when HAISHUI_DEV=1 is set. Only FileNotFoundError is
    swallowed; other errors (permissions, malformed data) propagate."""
    if os.environ.get("HAISHUI_DEV") == "1":
        return None
    ...
    try:
        info = {}
        with open(get_haishui_home() / ".container-mode", "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
    ...
    return {
        ...
        "container_name": info.get("container_name", "haishui-agent"),
        "exec_user": info.get("exec_user", "haishui"),
        "haishui_bin": info.get("haishui_bin", "/data/current-package/bin/haishui")}


# ---- Config paths / HAISHUI_HOME skeleton ----

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_haishui_home() / "config.yaml"
...
def require_parseable_user_config(*, ignore_user_config: bool = False) -> None:
    ...
    states; ``--ignore-user-config`` / HAISHUI_IGNORE_USER_CONFIG=1 remain authoritative."""
    if ignore_user_config or os.environ.get("HAISHUI_IGNORE_USER_CONFIG") == "1":
        return
    ...
    message = (
        f"Haishui stopped because your settings file ({config_path}) has a formatting error"
        f"{f' at {where}' if where else ''}. Fix it with `haishui config edit` and check with "
        "`haishui config check`, or add --ignore-user-config to run once with default settings.")
    if backup_path is not None:
...
def _resolve_haishui_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read HAISHUI_UID / HAISHUI_GID (set by Docker deployments); (None, None) if unset/invalid/Windows.
    The entrypoint chowns HAISHUI_HOME once, but subdirs created at runtime (``profiles/<name>/``)
    need the same chown or they land root:root and block later uid-mapped workers.

    Docker containers running Haishui commonly set these to map the in-container user to a host user so
    ...
    return _env_int("HAISHUI_UID"), _env_int("HAISHUI_GID")


def _chown_to_haishui_uid(path) -> None:
    """Chown ``path`` to ``HAISHUI_UID:HAISHUI_GID`` when set; EPERM/ENOENT are non-fatal (the
    ...
    :func:`ensure_haishui_home` on Docker deployments. See #34107.
    """
    uid, gid = _resolve_haishui_uid_gid()
...
def _secure_dir(path):
    """chmod a directory owner-only (0700) and apply HAISHUI_UID/GID ownership. No-op when managed;
    in a container only an explicit HAISHUI_HOME_MODE is applied. HAISHUI_HOME_MODE (e.g. 0701)
    overrides the mode so a web server can traverse HAISHUI_HOME to a served subdirectory without
    directory listings.

    Also applies ``HAISHUI_UID``/``HAISHUI_GID``-based ownership when those env vars are set (#34107 — Docker
    ...
    if is_managed():
        return
    explicit_mode = os.environ.get("HAISHUI_HOME_MODE", "").strip()
    ...
    # start (#10757). An explicit HAISHUI_HOME_MODE is the operator's choice and is still applied.
    if _is_container() and not explicit_mode:
        _chown_to_haishui_uid(path)
        return
    try:
...
def _is_container() -> bool:
    """Detect Docker/Podman/LXC (or HAISHUI_CONTAINER / HAISHUI_SKIP_CHMOD opt-out).
    ...
    if (os.environ.get("HAISHUI_CONTAINER") or os.environ.get("HAISHUI_SKIP_CHMOD")
...
_HAISHUI_HOME_ENSURED: set = set()
_HAISHUI_HOME_SUBDIRS = (
    "cron", "sessions", "logs", "logs/curator", "memories",
    "pairing", "hooks", "image_cache", "audio_cache", "skills")


def ensure_haishui_home():
    """Ensure the ~/.haishui directory skeleton exists with secure permissions.
    ...
    from haishui_constants import assert_named_profile_home_live
    assert_named_profile_home_live(home)
    if key in _HAISHUI_HOME_ENSURED and home.is_dir():
        return
    from haishui_cli.config_home import initialize_home
    initialize_home(home, _HAISHUI_HOME_SUBDIRS, _HAISHUI_HOME_ENSURED)


# ---- Config loading/saving ----

from haishui_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: E402,F401
from haishui_cli.config_providers import (  # noqa: E402,F401  (re-exported; callers/tests use haishui_cli.config.<name>)
...
# Back-compat re-exports — :mod:`haishui_cli.personality` owns personality/overlay semantics.
from haishui_cli.personality import (  # noqa: E402,F401
...
def _split_key_path(key: str) -> list[str]:
    ...
    ``haishui config set`` uses ``.`` as the nesting separator, so a key that itself contains a literal dot
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `haishui config set` routes this credential-shaped key to .env through the
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # A malformed SKILL.md must never break `haishui update`; this prompting is a nicety.
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (haishui_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard
    "platform_toolsets",     # written by the setup wizard
    "known_plugin_toolsets", # haishui_cli/tools_config.py toolset-save flow
...
def _validate_timezone(config: Dict[str, Any], issues: List[ConfigIssue]) -> None:
    ...
    ``haishui_time._get_zoneinfo()`` swallows an invalid name behind a single WARNING in the
    ...
    hint = ("Use an IANA zone name such as America/New_York or Asia/Tokyo (see "
            ...
            "schedules silently fall back to server-local time. HAISHUI_TIMEZONE overrides "
            "this key when set.")
    ...
    for _key in ("backend", "search_backend", "extract_backend"):
        ...
        if note:
            _issue(issues, "warning",
                   ...
                   "Run 'haishui tools' and pick a different Web Search & Extract provider")
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            from haishui_cli.config_home import config_load_issue
    ...
    if cp and not config.get("model"):
        _issue(issues, "warning",
               "custom_providers defined but no 'model' section — Haishui won't know which provider to use",
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'haishui doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars() -> None:
    ...
    if lines:
        ...
        hint_path = display_haishui_home()
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from haishui_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ haishui config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
...
def _disable_suspicious_mcp_servers(results: Dict[str, Any], quiet: bool) -> None:
    ...
    if not isinstance(raw_mcp_servers, dict):
        return
    try:
        from haishui_cli.mcp_security import validate_mcp_server_entry
    except Exception:
        return
    mcp_touched = False
...
def _warn_invalid_platform_toolsets(results: Dict[str, Any], quiet: bool) -> None:
    ...
    try:
        ...
        from haishui_cli.toolset_validation import validate_platform_toolsets
        from haishui_cli.toolset_scope import toolset_allowed_for_platform
...
def _offer_list(heading: str, items: List[str], question: str) -> bool:
    ...
    if not _ask_yes_no(question):
        print("  Set later with: haishui config set <key> <value>")
        return False
    print()
    return True
...
def _env_expand_match(m: re.Match) -> str:
    ...
    if inner.startswith("env:"):
        logger.warning(
            "Config ref %r: %s is not set (check ~/.haishui/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    made against a different environment (load before ``load_haishui_dotenv()``, in-process
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    LiteLLM users reach for, and ``haishui config set`` blindly accepts any dotted key — so
    ...
    from OpenAI-compatible backends) — while display paths (``haishui status``/``dump``) read ``name`` and
...
# the HAISHUI_MAX_ITERATIONS env bridge, works in every ``<``/``>=``/``max - used`` comparison in
...
def _backups_dir_display() -> str:
    from haishui_constants import display_haishui_home
    return f"{display_haishui_home()}/backups/config/"


_FIX_PERMS = "Fix the file permissions or move it aside first."
_FIX_YAML = (
    "Fix it with `haishui config edit` and check with `haishui config check`, or copy the newest good "
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    """Whether the remote SSH shell must expand *cwd* itself: ``~`` expanded on the Haishui host
...
def _last_known_good_fallback(config_path: Path, path_key: str, cache_sig, exc: Exception) -> Optional[Dict[str, Any]]:
    ...
    if lkg is None:
        # Fresh process (CLI restart, `haishui config get`): nothing loaded yet in this process, so
        ...
        from haishui_cli.config_backups import load_newest_good_backup
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        if cached is not None and cache_sig is not None and cached[:8] == cache_sig:
            ...
            # load_haishui_dotenv() pins unexpanded literals for the process lifetime.
            # Without this, a load_config() that ran before load_haishui_dotenv() pins unexpanded literals
...
#   openai-codex (OAuth — haishui auth) — OpenAI Codex
#   nous         (OAuth — haishui auth) — Nous Portal
...
def save_config(
    ...
    """Save configuration to ~/.haishui/config.yaml.
...
def load_env() -> Dict[str, str]:
    """Load ~/.haishui/.env as a dict. Memoised inside ``load_env_file`` (``get_env_value()`` runs
...
def sanitize_env_file() -> int:
    """Rewrite ~/.haishui/.env with normalized line formatting; returns the number of changed lines."""
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.haishui/.env (also matching ``export KEY=`` lines, so a save
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    so two endpoints on one host get separate slots. The fixed ``HAISHUI_CUSTOM_`` prefix keeps the
    ...
    return f"HAISHUI_CUSTOM_{slug}_API_KEY" if slug else "HAISHUI_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.haishui/.env and os.environ; True if it was found and removed."""
...
def save_env_value_secure(key: str, value: str) -> Dict[str, Any]:
    ...
    from haishui_cli.credential_lifecycle import save_provider_env_credential
...
def reload_env() -> int:
    """Re-read ~/.haishui/.env into os.environ; returns count of vars changed.
    Removes deleted vars only when known to Haishui (OPTIONAL_ENV_VARS and _EXTRA_ENV_KEYS) so
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` (scope-aware) or ``~/.haishui/.env``.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a Haishui-managed credential preferring ``~/.haishui/.env`` over ``os.environ``, so a
...
def _show_model_section(config: Dict[str, Any]) -> None:
    ...
    # Read the .env FILE directly so a stale HAISHUI_MAX_ITERATIONS ghost is caught even when the
    # gateway bridge already overrode os.environ.
    try:
        env_ghost = load_env().get("HAISHUI_MAX_ITERATIONS")
    except Exception:
        env_ghost = None
    if env_ghost is not None and str(env_ghost).strip() != str(cfg_max_turns).strip():
        print(color(f"                ⚠ .env has stale HAISHUI_MAX_ITERATIONS={env_ghost} "
                    f"(run 'haishui doctor --fix' to remove)", Colors.YELLOW))
...
def _show_display_section(config: Dict[str, Any]) -> None:
    ...
    try:
        from haishui_cli.personality import active_personality_name
...
def show_config():
    ...
    print(color("│              ☤ Haishui Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from haishui_cli.auth import get_anthropic_key
    ...
    print(color("  haishui config edit     # Edit config file", Colors.DIM))
    print(color("  haishui config set <key> <value>", Colors.DIM))
    print(color("  haishui setup           # Run setup wizard", Colors.DIM))
    print()
...
def resolve_cron_model_drift_defaults(
    ...
    Mirrors the scheduler's precedence: a truthy configured model wins over ``HAISHUI_MODEL``; the
    ...
    return provider, configured_model or _model_assignment_text(env.get("HAISHUI_MODEL", ""))
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "`haishui cron edit <job_id> --provider <provider> --model <model>` or set a fleet default "
        "with `haishui config set cron.model <model>`.")
...
def _coerce_config_set_value(key: str, value: str) -> Any:
    """Auto-coerce a ``haishui config set`` string to bool/None/int/float/list/dict.
...
def _refuse_container_type_mismatch(key: str, value: Any, user_config: Dict[str, Any], force: bool) -> Any:
    ...
    _exit_invalid(
        ...
        f"  Pass a YAML/JSON literal, e.g.:\n    haishui config set {key} '{literal}'\n"
...
def _redirect_platform_display_key(key: str) -> tuple[str, Optional[str]]:
    ...
    Before #71047 a write such as ``haishui config set platforms.telegram.streaming false`` landed on a key
...
def _guard_section_overwrite(key: str, value: Any, user_config: Dict[str, Any], force: bool) -> str:
    ...
    err += [
        ...
        f"    haishui config set {key}.<sub-key> <value>",
        "  Or use --force to replace the entire section:",
        f"    haishui config set --force {key} {value!r}"]
...
def _touch_skin_file(key: str, value: Any) -> None:
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_haishui_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
            pass
...
def _print_unknown_key_notice(key: str, suggestion: Optional[str]) -> None:
    print(color(
        ...
        "but Haishui may not read it.", Colors.YELLOW))
    if suggestion:
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    if _is_env_config_key(key):
        ...
        return
    from haishui_cli.config_env_routing import is_env_setting_key, save_env_setting
    ...
    if is_env_setting_key(key):
        ...
        # here, --force included (#111848). The env writer's denylist (HAISHUI_YOLO_MODE, PATH, ...)
    ...
    if key == "model.provider" and _old_provider.lower() != str(value).strip().lower():
        from haishui_cli.route_identity import drop_stale_model_route
        ...
        if _popped:
            ...
            _route_notice = color(
                ...
                "`haishui config unset model.base_url` (and model.api_mode).", Colors.YELLOW)
    # api_base -> base_url alias at set-time too (mirrors _normalize_root_model_keys).
    if key.strip().lower() in ("model.api_base", "api_base"):
        # Normalize the api_base → base_url alias at set-time too (issue #8919), so a fresh `haishui config
...
def get_config_value(key: str, *, as_json: bool = False, raw: bool = False):
    ...
    from haishui_cli.config_env_routing import is_env_setting_key, read_env_setting
    ...
    if _split_key_path(key)[0] in _known_top_level_keys():
        ...
        if not is_known:
            print(color(
                f"⚠ '{key}' is not a recognized config key — Haishui may not read it; the value "
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # See #51071.
        from haishui_cli.credential_lifecycle import remove_provider_env_credential

        if not remove_provider_env_credential(key.upper()).get("found"):
            _exit_invalid(f"Config key not set: {key}")
        print(f"✓ Unset {key} from {get_env_path()}")
        return
    from haishui_cli.config_env_routing import is_env_setting_key, remove_env_setting
...
_USAGE_GET = ("Usage: haishui config get <key> [--json] [--raw]", [
    "haishui config get model", "haishui config get terminal.backend",
    "haishui config get skills.config --json"], None)
_USAGE_SET = ("Usage: haishui config set [--force] <key> <value>", [
    "haishui config set model anthropic/claude-sonnet-4", "haishui config set terminal.backend docker",
    "haishui config set OPENROUTER_API_KEY sk-or-..."], [
    "", "  --force: skip the unknown-key notice for unrecognized keys,",
    "           and allow a scalar to replace a whole mapping section"])
_USAGE_UNSET = ("Usage: haishui config unset <key>", [
    "haishui config unset model", "haishui config unset terminal.backend",
    "haishui config unset OPENROUTER_API_KEY"], None)
...
def _cmd_config_check(args):
    ...
    if missing_config:
        ...
        print("    Run 'haishui config migrate' to add them")
...
_CONFIG_USAGE = """Available commands:
  haishui config           Show current configuration
  haishui config edit      Open config in editor
  haishui config get <key>          Print a resolved config value
  haishui config set <key> <value>   Set a config value
  haishui config unset <key>        Remove a config value
  haishui config check     Check for missing/outdated config
  haishui config migrate   Update config with new options
  haishui config path      Show config file path
  haishui config env-path  Show .env file path"""
...
def _inject_platform_plugin_env_vars() -> None:
    ...
    Chat etc. are configurable in ``haishui config`` UI without the core knowing they exist.
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``haishui_cli/`` — i.e. the git checkout for source
    installs, ``/opt/haishui`` inside the published image. It is a property of
    the running interpreter, NOT of ``$HAISHUI_HOME``, which is why a
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$HAISHUI_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/haishui`` in the published image, which instead bakes the stamp at
...
_PLUGIN_COMPAT_LAZY = {
    'normalize_route_base_url': ('haishui_cli.route_identity', 'normalize_route_base_url'),
}
