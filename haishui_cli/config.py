...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    the code; - the published ``nousresearch/haishui-agent`` image bakes a ``docker`` stamp into
...
_UPDATE_COMMAND_BY_METHOD = {
    "docker": "docker pull nousresearch/haishui-agent:latest",
...
Haishui Agent runs as a published image (nousresearch/haishui-agent), not a
...
Verify the new version after restart:
  docker run --rm nousresearch/haishui-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/haishui-agent/tags
