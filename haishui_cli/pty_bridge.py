"""PTY bridge for `haishui dashboard` chat tab.
...
``haishui_cli.web_server``. POSIX-only: depends on ``fcntl``, ``termios`` and ``ptyprocess`` (native
...
# ui-tui/packages/haishui-ink/src/ink/termio/host.ts — keep the two in sync.
PTY_HOST_ENV = "HAISHUI_PTY_HOST"
        ...
        if not _PTY_AVAILABLE:
            if sys.platform.startswith("win"):
                raise PtyUnavailableError("Pseudo-terminals are unavailable on this platform. "
                                          "Haishui Agent supports Windows only via WSL.")
