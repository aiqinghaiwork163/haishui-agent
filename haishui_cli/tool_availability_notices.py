...
_MULTI_PROVIDER_NOTICES: dict[str, str] = {
    "web": ("[yellow]⚠ Web search is off[/] — no search provider is set up yet (any one of Nous subscription, Exa, "
            "Tavily, Firecrawl, Brave, or free DuckDuckGo works). Run [bold]haishui setup tools[/] and set one up under "
            "\"Web Search & Scraping\"."),
}

_GENERIC_FOOTER = "[dim]   Run 'haishui setup tools' to configure[/]"
...
def filter_to_enabled_toolsets(unavailable: list[dict], enabled: Iterable[str],
    ...
    composite bundle such as ``["haishui-cli"]``, never the individual names ``check_tool_availability``
...
def _terminal_line(backend: str, reason: Optional[str]) -> str:
    ...
    return (f"[yellow]⚠ Terminal tool disabled:[/] the '{backend}' backend is not usable{detail}. "
            "Run [bold]haishui doctor[/] for details, or [bold]haishui setup terminal[/] to pick another backend.")
