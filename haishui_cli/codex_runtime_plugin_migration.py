"""Migrate Haishui MCP server config and Codex's installed curated plugins into ~/.codex/config.toml.
"""
...
from agent.transports.haishui_tools_mcp_server import HAISHUI_TOOLS_MCP_SERVER_NAME
...
MIGRATION_END_MARKER = "# end haishui-agent managed section"
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Haishui config.")
...
# Haishui MCP keys codex understands (transport stdio/http, timeouts, general). Any other key is
# dropped with a warning: ``sampling`` has no codex equivalent; the rest are unknown Haishui keys.
_KNOWN_HAISHUI_KEYS = {
...
# (haishui key, codex key, skip note) — timeouts are emitted as floats or skipped when non-numeric.
_TIMEOUT_KEYS = (
...
def _translate_one_server(name: str, haishui_cfg: dict) -> tuple[Optional[dict], list[str]]:
    """Translate one Haishui MCP server config to codex's inline-table dict.

    Returns ``(codex_entry, skipped_keys)``; ``codex_entry`` is None when the config is unusable.
    stdio (``command``) wins over ``url`` when both are set. Haishui' ``transport: sse`` hint is
    informational only — codex auto-negotiates. ``enabled`` is emitted only when explicitly false
    (codex defaults to true).
    """
    if not isinstance(haishui_cfg, dict):
        return None, []
    skipped: list[str] = []
    out: dict[str, Any] = {}
    if haishui_cfg.get("command"):
        if haishui_cfg.get("url"):
            skipped.append("url (both command and url set; preferring stdio)")
        out["command"] = str(haishui_cfg["command"])
        if haishui_cfg.get("args"):
            out["args"] = [str(a) for a in haishui_cfg["args"]]
        if haishui_cfg.get("env"):
            out["env"] = _str_map(haishui_cfg["env"])
        if haishui_cfg.get("cwd"):
            out["cwd"] = str(haishui_cfg["cwd"])
    elif haishui_cfg.get("url"):
        out["url"] = str(haishui_cfg["url"])
        if haishui_cfg.get("headers"):
            out["http_headers"] = _str_map(haishui_cfg["headers"])
        if haishui_cfg.get("transport") == "sse":
            skipped.append("transport=sse (codex auto-negotiates)")
    else:
        return None, ["no command or url field"]
    for haishui_key, codex_key, note in _TIMEOUT_KEYS:
        if haishui_key in haishui_cfg:
            try:
                out[codex_key] = float(haishui_cfg[haishui_key])
            except (TypeError, ValueError):
                skipped.append(note)
    if haishui_cfg.get("enabled") is False:
        out["enabled"] = False
    for key in haishui_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            skipped.append(f"{key} (no codex equivalent)")
        elif key not in _KNOWN_HAISHUI_KEYS:
            skipped.append(f"{key} (unknown Haishui key)")
    return out, skipped
...
# env-var passthrough (HAISHUI_HOME, PYTHONPATH) could carry one in pathological cases.
_TOML_ESCAPES = (
    ("\\", "\\\\"), ('"', '\\"'), ("\b", "\\b"), ("\t", "\\t"),
    ("\n", "\\n"), ("\f", "\\f"), ("\r", "\\r"))
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out += ["# (no MCP servers, plugins, or permissions configured by Haishui)", MIGRATION_END_MARKER]
        return "\n".join(out) + "\n"
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(codex_home=str(codex_home) if codex_home else None) as client:
            client.initialize(client_name="haishui-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    Such dirs are reaped between sessions; a HAISHUI_HOME pointing there burned into
...
def _build_haishui_tools_mcp_entry() -> dict:
    """Codex stdio entry launching Haishui' own tool surface as an MCP server (browser/web/
    delegate_task/vision/memory/skills call-backs).

    HAISHUI_HOME passes through only IF SET, read from os.environ (not get_haishui_home()): when
    unset the codex subprocess must inherit its launcher's runtime HAISHUI_HOME (systemd, gateway,
    kanban), not a migrate-time default burned into config.toml that pins the wrong profile. The
    pytest-tempdir guard keeps a sibling test's monkeypatched HAISHUI_HOME out of the user's real
    config. PYTHONPATH passes through so a worktree-launched haishui finds the branch's modules.
    """
    import sys
    env: dict[str, str] = {}
    # HAISHUI_HOME passes through IF SET so the MCP subprocess sees the same config / auth / sessions DB as
    # the parent CLI. Read from os.environ (not get_haishui_home()) on purpose: when the env var is unset we
    # want codex's subprocess to inherit whatever HAISHUI_HOME its launcher sets at runtime (systemd unit,
    # gateway, kanban dispatcher, custom shell), rather than burning the migrate-time resolved default into
    # config.toml — that would override the launcher's HAISHUI_HOME and pin the subprocess to the wrong
    # profile. The pytest-tempdir guard below catches the issue #26250 Bug C scenario: a sibling test's
    # monkeypatch.setenv("HAISHUI_HOME", tmp_path) would otherwise leak a transient pytest tempdir into the
    # user's real ~/.codex/config.toml and silently brick codex once the tempdir is GC'd.
    haishui_home = os.environ.get("HAISHUI_HOME") or ""
    if haishui_home and not _looks_like_test_tempdir(haishui_home):
        env["HAISHUI_HOME"] = haishui_home
    if os.environ.get("PYTHONPATH"):
        env["PYTHONPATH"] = os.environ["PYTHONPATH"]
    # Quiet mode + redaction defaults so the MCP wire stays clean.
    env["HAISHUI_QUIET"] = "1"
    env["HAISHUI_REDACT_SECRETS"] = env.get("HAISHUI_REDACT_SECRETS", "true")
    return {
...
def migrate(
    haishui_config: dict, *, codex_home: Optional[Path] = None, dry_run: bool = False,
    discover_plugins: bool = True, default_permission_profile: Optional[str] = ":workspace",
    expose_haishui_tools: bool = True) -> MigrationReport:
    """Translate Haishui mcp_servers config + Codex curated plugins into ~/.codex/config.toml.
    ...
    approval prompt on every write. ``expose_haishui_tools`` registers Haishui' own tool surface
    (agent/transports/haishui_tools_mcp_server.py, launched on demand by codex over stdio) as an MCP
    ...
    haishui_servers = (haishui_config or {}).get("mcp_servers") or {}
    if not isinstance(haishui_servers, dict):
        report.errors.append("mcp_servers in Haishui config is not a dict; cannot migrate.")
        return report
    translated: dict[str, dict] = {}
    for raw_name, cfg in haishui_servers.items():
        name = str(raw_name)
    ...
    if expose_haishui_tools:
        translated[HAISHUI_TOOLS_MCP_SERVER_NAME] = _build_haishui_tools_mcp_entry()
        if HAISHUI_TOOLS_MCP_SERVER_NAME not in report.migrated:
            report.migrated.append(HAISHUI_TOOLS_MCP_SERVER_NAME)
