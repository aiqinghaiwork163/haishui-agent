...
_REASON_COPY: dict[str, str] = {
    "auth": "Your {provider} key was rejected. Run `haishui model` to re-enter it.",
    "auth_permanent": "Your {provider} key was rejected. Run `haishui model` to re-enter it.",
...
_UNKNOWN_COPY = "The model request failed. Run /model to switch or `haishui doctor` to check the setup."
...
def agent_init_failure_message(error: BaseException) -> str:
    ...
    return (
        f"Haishui couldn't start the model connection: {_short(str(error)) or type(error).__name__}. "
        "Your message was not sent. Run `haishui doctor` to check the setup, "
