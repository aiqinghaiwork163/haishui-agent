...
def _find_stale_dashboard_pids(*, exclude_pids: set[int] | None = None,
    ...
    *scope_home*: keep only backends whose resolved Haishui home (see
    ``_haishui_home_for_pid``) is this home; unreadable ownership is spared, never guessed.
    ...
    """
    from haishui_cli.dashboard_procs import (
        _caller_ancestor_pids,
        _is_caller_wrapper_shell,
        _pids_owned_by_haishui_home,
    ...
    # 'haishui dashboard --stop'``); killing it takes down the invoking terminal.
    ancestors = _caller_ancestor_pids()
    pids = [pid for pid in pids if not _is_caller_wrapper_shell(pid, ancestors)]
    return _pids_owned_by_haishui_home(pids, scope_home) if scope_home else pids
...
def _parse_dashboard_runtime(command: str) -> tuple[str, str, int] | None:
    ...
    for candidate in ("dashboard", "serve"):
        patterns = (f"haishui {candidate}", f"haishui_cli.main {candidate}", f"haishui_cli/main.py {candidate}")
...
_DASHBOARD_SYSTEMD_UNIT = "haishui-dashboard.service"
...
def _restart_managed_dashboard_service(reason: str, unit: str = _DASHBOARD_SYSTEMD_UNIT) -> bool:
    ...
    # User manager first (Haishui installs Linux services in the user scope by
...
def _get_systemd_service_for_pid(pid: int) -> str | None:
    """The systemd service unit name *pid* belongs to (``haishui-serve.service``), or None.
...
# launchd plist directories that can supervise a ``haishui dashboard`` / ``haishui serve`` backend on
...
def _loaded_launchd_backend_jobs(
    ...
    ``ProgramArguments`` is a ``haishui dashboard`` / ``haishui serve`` backend. macOS only (empty
    ...
    from haishui_cli.gateway import _launchd_print_service_pid
...
def _restart_launchd_job(domain: str, label: str, old_pid: int | None, *, timeout: float = 15.0) -> bool:
    ...
    from haishui_cli.gateway import _wait_for_launchd_service_pid
    try:
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    """Respawn manually-started dashboards after ``haishui update``, detached, logging to
    ...
    log_path = get_haishui_home() / "logs" / "dashboard-restart.log"
...
class _UpdateOutputStream:
    """stdout/stderr wrapper for ``haishui update``: mirrors to ``logs/update.log`` and, once the
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    try:
        ...
        # haishui_cli.config.get_haishui_home to simulate setup failure.
        from haishui_cli.config import get_haishui_home as _get_haishui_home
        logs_dir = _get_haishui_home() / "logs"
        logs_dir.mkdir(parents=True, exist_ok=True)
        log_file = open(logs_dir / "update.log", "a", buffering=1, encoding="utf-8")

        import datetime as _dt

        stage = "continued on the pulled code" if os.environ.get("HAISHUI_UPDATE_POST_SWAP") == "1" else "started"
        log_file.write(f"\n=== haishui update {stage} {_dt.datetime.now().isoformat(timespec='seconds')} ===\n")
...
def _report_dashboard_status() -> int:
    ...
    """
    from haishui_cli.dashboard_procs import _scan_dashboard_processes
    ...
    if not live:
        print("No haishui dashboard or serve processes running.")
        return 0

    print(f"{len(live)} haishui dashboard/serve process(es) running:")
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    try:
        from haishui_cli.web_server import should_require_dashboard_auth
    ...
    print("    [2] OAuth via Nous Portal (run `haishui dashboard register`)\n    [3] Cancel\n")
    ...
    try:
        ...
        from haishui_cli.plugins_cmd import ensure_basic_auth_plugin_enabled_in_config
        cfg = load_config()
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    if sys.platform == "win32":
        from haishui_cli.windows_ssh_runtime import read_token
    ...
    # The Desktop client writes the token under the account's $HOME/.haishui/
    # desktop-ssh, independent of HAISHUI_HOME and the active profile. Anchor
    # validation there, NOT get_haishui_home(): a non-default profile or a Docker
    # /opt/data root re-homes get_haishui_home() and would reject every token.
    # See #69551.
    token_root = Path.home() / ".haishui" / "desktop-ssh"
    try:
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    A standalone ``haishui dashboard`` inheriting that ``HAISHUI_WEB_DIST`` would
...
def _route_named_profile_dashboard(
    ...
    (HAISHUI_DESKTOP=1) stay per-profile. Returns normally when no routing applies.
    """
    try:
    ...
    if (
        ...
        or os.environ.get("HAISHUI_DESKTOP") == "1"
    ):
        return
    ...
    reexec_argv = [
        sys.executable, "-m", "haishui_cli.main",
        "-p", "default",
    ...
    # HAISHUI_HOME is pinned to the machine root below — the factory must not
    # re-inject a profile home.
    env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=False)
    # Pin the child to the machine ROOT, resolved explicitly rather than by
    # dropping HAISHUI_HOME: in the Docker layout the root is /opt/data, and an
    # unset HAISHUI_HOME would fall back to $HOME/.haishui = /opt/data/.haishui — an
    # empty auto-seeded home with only the default profile and no install stamp.
    # get_default_haishui_root() strips a trailing profiles/<name> for both layouts.
    try:
        from haishui_constants import get_default_haishui_root
        env["HAISHUI_HOME"] = str(get_default_haishui_root())
    except Exception:
        env.pop("HAISHUI_HOME", None)  # prior behaviour rather than blocking the reroute
...
def _resolve_dashboard_web_dist(args, _headless_backend: bool) -> None:
    ...
    ``serve`` sets HAISHUI_SERVE_HEADLESS so mount_spa() stays off. Otherwise build
    unless HAISHUI_WEB_DIST / --skip-build promise a dist — then verify index.html
    (else the server serves 404s). --skip-build on the default location gets ONE
    recovery build; a caller-managed HAISHUI_WEB_DIST can't be populated.
    """
    from haishui_cli.main import PROJECT_ROOT
    from haishui_cli.main_web_build import _build_web_ui
    skip_build = getattr(args, "skip_build", False)
    if _headless_backend:
        os.environ["HAISHUI_SERVE_HEADLESS"] = "1"  # set before web_server import
    elif "HAISHUI_WEB_DIST" not in os.environ and not skip_build:
        ...
        _dist_root = (
            ...
            Path(os.environ["HAISHUI_WEB_DIST"])
            if "HAISHUI_WEB_DIST" in os.environ
            else PROJECT_ROOT / "haishui_cli" / "web_dist"
        )
        if not (_dist_root / "index.html").exists():
            # Only the default dist location is recoverable (desktop launches with
            # --build-mode skip after a wipe of web_dist); a custom HAISHUI_WEB_DIST
            ...
            _recoverable = "HAISHUI_WEB_DIST" not in os.environ
            if _recoverable:
        ...
        # HAISHUI_WEB_DIST without --skip-build: the env var points at a
        # caller-managed dist, so validate it like the --skip-build branch.
        # HAISHUI_WEB_DIST is set without --skip-build: the build is skipped (the env var points at a
        ...
        # path).
        _dist_root = Path(os.environ["HAISHUI_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ HAISHUI_WEB_DIST is set but no web dist found at: {_dist_root}")
            print(_PRE_BUILD_HINT)
            print("  Or unset HAISHUI_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # web_server reads HAISHUI_WEB_DIST raw at import (no expanduser), so a
        # validated "~/dist" would otherwise pass here and still 404 there.
        os.environ["HAISHUI_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from HAISHUI_WEB_DIST: {_dist_root}")
