"""Dashboard process-hygiene helpers — extracted from ``haishui_cli/main.py``.

Helpers defined in ``haishui_cli.main_dashboard`` / ``haishui_cli.main_install_repair`` are imported at
...
_DASHBOARD_PATTERNS = tuple(
    ...
    for launcher in ("haishui", "haishui_cli.main", "haishui_cli/main.py"))
...
def _iter_process_table() -> list[tuple[int, str]]:
    ...
    if sys.platform == "win32":
        ...
        return rows
    # ps, not `pgrep -f "haishui.*dashboard"` (greedy regex; consistent with gateway pid scan).
...
def _scan_dashboard_processes(*, exclude_pids: set[int] | None = None) -> list[tuple[int, str]]:
    ...
    ``haishui update`` (every API call 401s). *exclude_pids* (Desktop's HAISHUI_DESKTOP_CHILD_PID
    backends) are never returned.

    *exclude_pids* is an optional set of PIDs that must never be returned. This is used by the Haishui
    Desktop Electron app to protect its own backend child process: when the desktop spawns ``haishui serve``
    as a backend and triggers an auto-update, the update must not kill the backend that the desktop itself
    manages. The desktop sets the environment variable ``HAISHUI_DESKTOP_CHILD_PID`` on the spawned backend
    ...
    try:
        ...
        return []
    # Spawn-ledger augmentation: substring patterns miss profiled launches (`haishui --profile p
    ...
    with contextlib.suppress(Exception):
        ...
        from haishui_cli.process_identity import ledger_entries
...
def _haishui_home_for_pid(pid: int) -> str | None:
    """The Haishui home *pid* runs on, tri-state: ``None`` ONLY when its environment is unreadable
    ...
    exec-time env + argv (``haishui -p X serve`` rewrites ``HAISHUI_HOME`` in ``os.environ`` AFTER
    startup, which ``/proc/<pid>/environ`` never reflects): a profile-shaped ``HAISHUI_HOME``
    without a flag is the home; otherwise the root is ``HAISHUI_HOME`` (its grandparent when
    ...
    if env is None:
        return None
    from haishui_cli.main_dashboard import _dashboard_cmdline_for_pid
    from haishui_cli.profiles import get_active_profile, normalize_profile_name, profile_root_for_env_home
    argv = _dashboard_cmdline_for_pid(pid) or []
    env_home = env.get("HAISHUI_HOME", "").strip()
    profile = _profile_flag_value(argv)
    if profile is None and env_home and (
        Path(env_home).parent.name == "profiles" or env.get("HAISHUI_UPDATE_POST_SWAP") == "1"
    ):
        return env_home
    if sys.platform == "win32":
        ...
        default_home = base / "haishui"
    else:
        default_home = Path(env.get("HOME") or Path.home()) / ".haishui"
    root = profile_root_for_env_home(env_home, default_home)
    fixed_identity = any(env.get(k) for k in ("HAISHUI_SUPERVISED_CHILD", "HAISHUI_S6_SUPERVISED_CHILD",
                                               "HAISHUI_GATEWAY_EXTERNAL_SUPERVISOR")) or "--ssh-session-token-file" in argv
...
def _is_ephemeral_port_zero_backend(argv: list[str]) -> bool:
    ...
    ``haishui update`` multiplies listening backends because ``--port 0`` binds a fresh port.
...
def _pids_owned_by_haishui_home(pids: list[int], home: str) -> list[int]:
    """Return only *pids* whose resolved Haishui home (``_haishui_home_for_pid``) is ``home``.

    Dashboard argv is discovery-only: it is not an ownership proof because
    several Haishui installs and profiles can run the same command on one
    ...
    return [
        pid for pid in pids
        if (pid_home := _haishui_home_for_pid(pid))
        and _normalized_home_for_compare(pid_home) == target
    ]


def _profile_key_for_respawn(argv: list[str], haishui_home: str | None = None) -> str:
    """Stable owner key: ``HAISHUI_HOME`` when known, else ``--profile`` / ``-p``.
    ...
    """
    if haishui_home:
        parts = _resolved_home(haishui_home).parts
        if len(parts) >= 2 and parts[-2] == "profiles" and parts[-1]:
            return f"profile:{parts[-1]}"
        return f"home:{_normalized_home_for_compare(haishui_home)}"
...
def _filter_dashboard_respawn_candidates(
    ...
    """Select which killed manual backends ``(pid, argv, haishui_home)`` to respawn after update.

    Rules: never resurrect Desktop ``--port 0`` backends; never replay a backend from a
    **foreign** ``HAISHUI_HOME`` (the argv-only respawn would come back on this install's home
    ...
    (``HAISHUI_DESKTOP_CHILD_PID``) owns their lifecycle. These are also the PPID-1 orphans that previously
    ...
    Intentionally does **not** blanket-skip every PPID-1 process: a prior ``haishui update`` respawn detaches
    ...
    for _pid, argv, haishui_home in candidates:
        if not argv or _is_ephemeral_port_zero_backend(argv):
            continue
        if own_key and haishui_home and _normalized_home_for_compare(haishui_home) != own_key:
            continue
        norm = _normalize_dashboard_cmdline(argv)
        profile_key = _profile_key_for_respawn(argv, haishui_home)
...
def _exclude_pids_from_env() -> set[int]:
    """PIDs Desktop marks as live backends (``HAISHUI_DESKTOP_CHILD_PID``, comma-separated)."""
    out: set[int] = set()
    for part in os.environ.get("HAISHUI_DESKTOP_CHILD_PID", "").split(","):
        with contextlib.suppress(ValueError):
            out.add(int(part))
    return out


#: Executables that only *carry* a haishui command line. A process headed by one of these
#: never serves traffic itself; when its argv matches the dashboard patterns it is a
#: wrapper around the command (``bash -c 'haishui dashboard --stop'``), not a backend.
...
def _is_caller_wrapper_shell(pid: int, ancestors: set[int]) -> bool:
    ...
    ``bash -c 'haishui dashboard --stop'`` wrapper) matches on its own argv. Ancestor alone
...
def _kill_pids_windows(pids: list[int], killed: list[int], failed: list[tuple[int, str]]) -> None:
    ...
    from haishui_cli._subprocess_compat import pid_is_haishui, windows_hide_flags
    ...
    for pid in pids:
        try:
            ...
            elif not pid_is_haishui(pid, expected_start_time=expected_start_time):
                failed.append((pid, "not haishui-owned or process identity changed"))
            else:
...
# haishui_cli/web_server.py::_lifespan: stop_hosted_room_service(timeout=5.0) + the startup-thread
...
# and keep the deleted state.db-wal inode open — the next haishui start refuses with a FATAL
...
def _is_detached_session_leader(pid: int, tty: str) -> bool:
    ...
    ``haishui --tui`` child is a session leader too (``pty.fork``) but owns the pts whose master the
...
def _posix_descendants(roots: list[int]) -> dict[int, tuple[int, int | None]]:
    ...
    subtrees. So is the calling process with its subtree and its ancestor chain: ``haishui dashboard
    --stop`` / ``haishui update`` run from a shell escape inside the hosted Chat TUI are same-session
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``haishui dashboard`` / ``haishui serve`` processes (update end, ``--stop``).
    ...
    ``HAISHUI_HOME`` are candidates; unknown ownership fails closed. This is
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the caller already restarted directly —
    e.g. ``haishui update``'s systemd fleet-restart loop, which restarts ``haishui-serve*`` units before this
    ...
    """
    from haishui_cli import main_dashboard as _dash

    if restart_managed and _dash._restart_managed_dashboard_service(reason):
        # The dashboard unit is handled but other backends (e.g. haishui-serve.service) are not:
        # mark the unit handled so the filter below drops its PIDs, and keep going.
        _dash_unit = getattr(_dash, "_DASHBOARD_SYSTEMD_UNIT", "haishui-dashboard.service")
    ...
    if restart_managed and sys.platform != "win32":
        for pid in pids:
            ...
            if launchd_jobs and (job := _launchd_owner(pid, cmdline)):
                ...
                # Manual process: exact argv + HAISHUI_HOME for the respawn and its profile cap.
                # Manually-started process: preserve its exact argv so we can respawn it after the update
                # (#40449, #68934). Snapshot HAISHUI_HOME before the kill so per-profile caps still work
                # after the process is gone (#78821).
                pid_cmdline[pid] = cmdline
                pid_home[pid] = _haishui_home_for_pid(pid)
...
def _restart_killed_backends(
    ...
    # Two categories: Without this, a remote backend (haishui serve) under Restart=on-failure never comes
...
def _detect_concurrent_haishui_instances(
    ...
    Windows blocks DELETE/REPLACE on a running .exe, so a Desktop-spawned ``haishui.EXE`` makes
    ...
    *shim* ancestor (the setuptools launcher is a separate native process from its
    ...
    """
    from haishui_cli.main_install_repair import _haishui_exe_shims, _is_windows
    ...
    try:
        ...
        return []
    shim_paths = {_norm_exe(shim) for shim in _haishui_exe_shims(scripts_dir)}
    if not shim_paths:
        return []
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    """True for the Desktop-local shape ``haishui serve [--isolated] --host 127.0.0.1 --port 0``.
    ...
    """
    from haishui_cli.update_cmd_windows import _haishui_holder_subcommand
    # Canonical token matcher, never argv substrings: ``kanban --preserve-cache`` contains "serve" and
    # ``vim notes about haishui serve`` contains both markers — this predicate decides a kill.
    if _haishui_holder_subcommand(command) != "serve":
        return False
...
# (apps/desktop/electron/remote-lifecycle.ts) for every ``haishui serve`` it spawns. Such a backend
...
def _haishui_home_dir() -> Path:
    """The process's Haishui home: remote-backend locks are a process-level asset, so a request scoped
    to another profile must still see the same lock dir."""
    from haishui_constants import get_process_haishui_home
...
def _valid_lockfile_payload(parsed: object, ownership_id: str) -> bool:
    ...
    if any(not isinstance(parsed.get(f), str) or len(parsed[f]) > 1024
           for f in ("profile", "haishuiPath", "haishuiHome", "logPath", "startedAt")):
        return False
    # Suffix-only check of logPath so a relocated HAISHUI_HOME can't reject a legitimate backend.
...
def _remote_lock_roots(base_dir: Path | None) -> list[Path]:
    ...
    The Desktop writes SSH locks beneath the ROOT home (``~/.haishui/desktop-ssh``), but a profile
    backend (``haishui --profile X serve``) runs with ``HAISHUI_HOME=<root>/profiles/X`` — scanning only
    ...
    roots: list[Path] = []
    for home in (_haishui_home_dir(), get_default_haishui_root()):
...
def _lock_owned_serve_pids(base_dir: Path | None = None) -> set[int]:
    """PIDs claimed by valid ``{haishui_home}/desktop-ssh/<ownershipId>/backend.lock.json`` records
...
def _reap_orphaned_desktop_local_serves(
    *, reason: str = "orphaned desktop-local haishui serve", signal_term=None, signal_kill=None,
    sleep_fn=None, lock_owned_pids_fn=None, process_age_seconds_fn=None) -> dict[str, list]:
    """Kill leftover Desktop-local ``haishui serve`` backends with no parent. Never raises.
    ...
    0/1; not self / parent / HAISHUI_DESKTOP_CHILD_PID; not claimed by a valid
    ...
    after HAISHUI_BACKEND_READY, so a live sibling mid-startup is briefly unowned).
    """
    import signal as _signal
    import time as _time
