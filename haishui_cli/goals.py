"""Persistent session goals — the Ralph loop for Haishui.
...
# (schema DDL, FTS tables, first haishui_cli.config import) measures ~300ms warm and more on slow
...
def _get_session_db() -> Optional[Any]:
    """Cached SessionDB per HAISHUI_HOME (profile switches pick the right DB); None on any failure.
    ...
    if cached is not None and _registry_tore_down(cached):
        # ``haishui profile delete`` force-closes every handle under the profile home
        # (``haishui_state_registry.close_all_under``) before rmtree; a same-name recreate in this
