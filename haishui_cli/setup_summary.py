...
resolved through the module object so test patches on ``haishui_cli.setup.<name>`` take effect."""

import logging
from tools import tool_backend_helpers
from haishui_cli import nous_subscription
...
_TTS_SUMMARY_ROWS = {
    ...
    "neutts": ("NeuTTS", "neutts", "run 'haishui setup tts'"),
    "kittentts": ("KittenTTS", "kittentts", "run 'haishui setup tts'")}
...
_STT_SUMMARY_DEFAULT = ("Local Whisper", "faster_whisper", "run 'haishui tools' → Speech-to-Text")
...
_EDIT_WIZARD_ROWS = (
    ("haishui setup", "          Re-run the full wizard"), ("haishui setup model", "    Change model/provider"),
    ("haishui setup terminal", " Change terminal backend"), ("haishui setup gateway", "  Configure messaging"),
    ("haishui setup tools", "    Configure tool providers"))
_EDIT_CONFIG_ROWS = (
    ("haishui config", "         View current settings"), ("haishui config edit", "    Open config in your editor"),
    ("haishui config set <key> <value>", ""))
_READY_ROWS = (
    ("haishui", "              Start chatting"), ("haishui gateway", "      Start messaging gateway"),
    ("haishui doctor", "       Check for issues"))
...
def _vision_row(config, feats):
    ...
    try:
        ...
        ok = False
    return ("Vision (image analysis)", ok, None if ok else "run 'haishui setup' to configure")
...
def _video_gen_row(config, feats):
    # Opt-in via `haishui tools` → Video Generation. Only show the row when a plugin reports
...
def _modal_row(config, feats):
    ...
    if _setup.cfg_get(config, "terminal", "backend") == "modal":
        ...
        return ("Modal Execution", False, "run 'haishui setup terminal'")
...
def _spotify_row(config, feats):
    # OAuth via haishui auth spotify — check auth.json, not env vars
    try:
...
def _print_setup_summary(config: dict, haishui_home):
    ...
    try:
        ...
        print()
        _setup.print_warning("No inference provider is configured — Haishui cannot chat yet.")
        _setup._info("  Finish this one step with either of:",
              "    haishui model            (pick any provider/model)",
              "    haishui setup --portal   (Nous Portal OAuth, no API key)")
    ...
    if available_count < len(tool_status):
        _setup.print_warning("Some tools are disabled. Run 'haishui setup tools' to configure them,")
    ...
    for label, value in (("Settings:", f"  {_setup.get_config_path()}"), ("API Keys:", f"  {_setup.get_env_path()}"),
                         ("Data:", f"      {haishui_home}/cron/, sessions/, logs/")):
...
import haishui_cli.setup as _setup  # noqa: E402  (bottom: haishui_cli.setup imports this module)
