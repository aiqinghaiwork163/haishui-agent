"""Windows gateway lifecycle for ``haishui update``: pause/resume/cold-start the service, sweep venv holders, reap orphaned backends.

Split out of ``update_cmd.py``; names are re-imported there so ``haishui_cli.update_cmd.<name>`` still resolves/monkeypatches.
...
logger = logging.getLogger("haishui_cli.update_cmd")  # log-record parity with the origin module
...
def _self_and_non_gateway_ancestor_pids(psutil) -> set[int]:
    ...
    with suppress(Exception):
        # Never return ourselves or our own ancestry: a CLI ``haishui update`` runs from the venv python and
...
def _detect_venv_python_processes(*, exclude_pids: set[int] | None = None) -> list[tuple[int, str, str]]:
    ...
    The haishui.exe shim guard misses the Desktop backend and anything off ``venv\\Scripts\\python(w).exe``;
    ...
    if not _m()._is_windows() or psutil is None:
        return []
    from haishui_constants import project_venv_dir
    ...
    for proc in proc_iter:
        ...
        name_low = name.lower()
        if not (in_venv or name_low.startswith(("python", "pypy")) or name_low in {"uv.exe", "uvx.exe", "haishui.exe"}):
            continue
        ...
        # its .pyd files — match cmdline (venv path, or `-m haishui_cli.main` + root/cwd).
        if in_venv or venv_prefix in cmdline_low or (
            "haishui_cli.main" in cmdline_low and (root_prefix in cmdline_low or _cwd_prefix(proc).startswith(root_prefix))
        ):
...
def _haishui_holder_subcommand(cmdline: str) -> str | None:
    """The actual Haishui SUBCOMMAND a venv-holder argv runs, or None (callers must NOT guess a label).

    Token-based, never substring (``kanban --preserve-cache`` contains "serve"): find the ``haishui_cli.main`` /
    ``haishui(.exe)`` entry token, return the first following token that isn't a flag or a flag's value.
    ...
    def _is_entry(i: int, token: str) -> bool:
        ...
        return (low.endswith("haishui_cli.main") and i > 0 and tokens[i - 1] == "-m") or (
            low.rsplit("\\", 1)[-1].rsplit("/", 1)[-1] in ("haishui", "haishui.exe"))
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    Labels come from the parsed SUBCOMMAND, never substring: a standalone ``haishui dashboard`` must not be
    called the Desktop backend, ``--preserve-cache`` must not match "serve". Unknown argv gets no hint.

    See #90778.
    """
    hint_by_subcommand = {
        "serve": "  ← Haishui backend (if the Desktop app is open, close it)",
        "dashboard": "  ← haishui dashboard (stop it: haishui dashboard stop, or close that terminal)",
        "gateway": "  ← gateway",
    }
    lines = ["✗ Other Haishui processes are running from this install's venv:"]
    for pid, name, cmdline in matches[:6]:
        hint = hint_by_subcommand.get(_haishui_holder_subcommand(cmdline) or "", "")
    ...
    lines.append(
        ...
        "  Close the Haishui desktop app / other Haishui terminals, then re-run:\n    haishui update\n"
        "  (or use `haishui update --force-venv` to proceed anyway at your own risk)"
    )
    return "\n".join(lines)
...
def _leftover_pausable_gateway_pids(matches: list[tuple[int, str, str]]) -> list[int] | None:
    ...
    from haishui_cli._scan_venv_blockers import _is_pausable_gateway
    psutil = _psutil()
    pids: list[int] = []
...
def _refuse_gateway_ancestor_tree_kill(pids: list[int], *, gateway_mode: bool) -> bool:
    """Refuse a plain Windows update that would tree-kill its own ancestry (a chat agent's ``haishui update`` is
    ...
    if gateway_mode or not pids:
        return False
    def _ancestors():
        from haishui_cli.gateway import _is_pid_ancestor_of_current_process
    ...
    print(
        ...
        "  From a chat platform, use `/update` instead.\n  Otherwise, run `haishui update` from a separate terminal."
    )
    return True
...
def _ledger_manual_serve_holders(matches: list[tuple[int, str, str]]) -> list[dict]:
    ...
    try:
        from haishui_cli.process_identity import ledger_entries, spawner_is_dead
    except Exception:
        return []
...
def _serve_relaunch_commands(entries: list[dict]) -> list[list[str]]:
    ...
    with suppress(Exception):
        ...
        if scripts_dir is not None:
            haishui = next((str(scripts_dir / n) for n in ("haishui.exe", "haishui") if (scripts_dir / n).is_file()), haishui)
    ...
    for entry in entries:
        ...
        commands.append(
            [haishui] + (["--profile", profile] if profile and profile != "default" else [])
...
def _relaunch_stopped_serves(token: dict) -> None:
    ...
    from haishui_cli.update_cmd import _m, _record_update_step
    ...
    if skipped or failed:
        print("  ⚠ Some stopped backends could not be relaunched automatically; restart them manually (haishui serve --host <ip> --port <port>).")
    _record_update_step(
...
def _is_backend_argv(argv_low: str) -> bool:
    ...
    Same predicate as ``_looks_like_desktop_control_plane``: ``-m haishui_cli.main`` entry shape (the
    Desktop's only spawn shape, ``apps/desktop/electron/main.ts``) AND the canonical holder classifier says
    ``serve``/``dashboard``. A user-launched ``haishui.exe serve`` / ``haishui dashboard`` is NOT the
...
def _orphaned_desktop_backend_pids(matches: list[tuple[int, str, str]]) -> list[tuple[int, int]] | None:
    ...
    would dead-end the update with "Haishui is still running" and zero open windows. Qualifies only if cmdline
    is a Haishui backend AND the parent is demonstrably gone (PID missing or reused). Tree-aware: holders inside
    ...
    it tree-kills its backends and waits for the venv shim before spawning haishui-setup, and the
    ...
    dead-ends the update with "Haishui is still running" while the user stares at zero open windows (ryanc's
    ...
    for pid, _name, cmdline in matches:
        ...
        try:
            ...
            # gateway.status.get_process_start_time so pid_is_haishui round-trips at kill time.
...
def _ledger_reapable_backend_pids(matches: list[tuple[int, str, str]]) -> list[int]:
    ...
    try:
        from haishui_cli.process_identity import REAPABLE_PURPOSES, ledger_entries, spawner_is_dead
...
def _handoff_reapable_backend_pids(matches: list[tuple[int, str, str]]) -> list[int] | None:
    ...
    hand-off. Inside the hand-off gate (marker + ``--gateway`` + no live ``haishui.exe`` shim) nothing legitimate
...
def _stop_process_trees(pids: list[int] | list[tuple[int, int]]) -> None:
    ...
    ``taskkill /T /F``: stopping only the parent can leave a ``.haishui-runtime`` child holding the install open.
    ...
    for entry in pids:
        ...
        try:
            if expected_start_time is None:
                ...
                continue
            if not pid_is_haishui(pid, expected_start_time=expected_start_time):
                logger.debug("Skipping taskkill of non-Haishui or changed PID %s", pid)
                continue
            subprocess.run(
...
def _looks_like_desktop_control_plane(cmdline: str) -> bool:
    """True for this-install ``haishui serve`` / ``haishui dashboard`` argv (Desktop control plane).
    ...
    """
    return "haishui_cli.main" in (cmdline or "").lower() and _haishui_holder_subcommand(cmdline) in _BACKEND_PURPOSES
...
def _restore_windows_gateway_service(name: str, *, timeout: float = 60.0) -> None:
    ...
    from haishui_cli.update_cmd import _start_windows_gateway_service
    service = _win_service(name)[1]
...
def _windows_cold_start_plan() -> dict | None:
    ...
    *generation* that authorized it on the token: the marker is a mutable one-shot that any concurrent
    ``haishui gateway status``/``start`` consumes, so execution authorizes the spawn from the token and
    consumes only that generation (#110020 review)."""
    from haishui_cli.update_cmd import _desktop_owns_gateway_lifecycle
...
def _pause_windows_gateway_services(service_gateways, token: dict, profiles: dict, unmapped: list) -> dict:
    ...
    from haishui_cli.update_cmd import _restore_windows_gateway_service, _stop_windows_gateway_service
    paused_services = []
...
def _discover_windows_gateways():
    ...
    from haishui_cli.gateway import find_gateway_pids, find_profile_gateway_processes, find_windows_gateway_services
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    Scheduled/startup gateways run via pythonw.exe, invisible to the haishui.exe instance guard, yet keep files
    ...
    with _abort_on_error("Could not prepare Windows gateway pause for update"):
        ...
        from haishui_cli.gateway import _capture_gateway_argv
    ...
    print("→ Stopping Windows gateway process(es) before updating Haishui...")
    ...
    if unmapped_pids:
        ...
        if any(not u.get("argv") for u in unmapped):  # no recoverable cmdline (psutil missing, denied, gone)
            print("    Restart manually after update: haishui gateway run")
...
def _record_attested_cold_start_profiles(token: dict, running_profiles: set) -> None:
    ...
    with _best_effort("Could not evaluate per-profile attested cold-starts before update: %s"):
        if not _desktop_owns_gateway_lifecycle():
            return
        from haishui_cli.profiles import get_active_profile_name, profiles_to_serve
...
def _cold_start_attested_profiles(token: dict) -> None:
    """Spawn each ``cold_start_profiles`` entry under its own HAISHUI_HOME and consume exactly the
...
def _cold_start_windows_gateway_after_update(token: dict | None = None) -> bool:
    ...
    ``haishui gateway status``/``start`` consumes, which would otherwise skip this spawn and clear the
    token (#110020 review). Only that generation is consumed afterwards — never a newer marker.
    """
    from haishui_cli.update_cmd import _desktop_owns_gateway_lifecycle, _m
    if not _m()._is_windows():
        return True
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    artifacts written once at install time — ``haishui update`` never touched them, so installs created
...
def _refresh_bootstrap_cache_scripts(branch: str = "main") -> None:
    """Overwrite ``$HAISHUI_HOME/bootstrap-cache/install-<ref>.{ps1,sh}`` for *branch* from the fresh checkout.

    Old ``haishui-setup.exe`` builds NEVER re-download a cached branch-ref script, so a stale one runs
    ...
    so the poisoned cache outlives every ``haishui update``.
    ...
    with _best_effort('Could not refresh bootstrap-cache scripts after update: %s'):
        cache_dir = Path(_m().get_haishui_home()) / "bootstrap-cache"
...
def _relaunch_paused_gateways(token: dict, profiles: dict, unmapped: list) -> tuple[list[str], int]:
    ...
    with _abort_on_error("Could not load Windows gateway restart helper"):
        from haishui_cli.gateway import launch_detached_gateway_restart_by_cmdline, launch_detached_profile_gateway_restart
...
def _verify_relaunched_gateways_alive(token: dict, profiles: dict, unmapped: list) -> None:
    ...
    if not ready_pids:
        ...
        print(
            ...
            "    Recover with: haishui gateway restart"
        )
...
def _in_handoff_without_live_shim(args) -> bool:
    """GUI hand-off gate: ``--gateway`` + update-incomplete marker AND no live ``haishui.exe`` shim.
    ...
    try:
        if not (bool(getattr(args, "gateway", False)) and _m()._update_marker_path().exists()):
            return False
        scripts_dir = _m()._venv_scripts_dir()
        return scripts_dir is not None and not _m()._detect_concurrent_haishui_instances(scripts_dir)
    except Exception:
        return False
...
def _clear_windows_venv_holders_or_exit(args, gateway_mode: bool, _windows_gateway_resume):
    ...
    from haishui_cli.update_cmd import _m, _record_update_step, _refuse_gateway_ancestor_tree_kill
    ...
    for classifier, message in (
        (_m()._ledger_reapable_backend_pids, "ledger-identified orphaned Haishui backend process(es) hold the venv"),
    ...
    # Manual serve/dashboard rung (e.g. `haishui serve --host <ip>` for a REMOTE Desktop): ledger identity
    ...
    if holders and _in_handoff_without_live_shim(args) and (handoff_backends := _m()._handoff_reapable_backend_pids(holders)):
        holders = _reap_and_rescan(
            f"  ⚠ {len(handoff_backends)} Haishui backend process(es) "
