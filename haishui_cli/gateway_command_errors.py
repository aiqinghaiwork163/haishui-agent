"""User-facing copy for ``haishui gateway start/stop/restart`` failures on systemd hosts.

``haishui_cli/gateway.py`` is a facade; this sibling owns the small exception -> guidance table so
...
_JOURNAL_HINT = 'journalctl --user -u haishui-gateway --since "5 min ago"'

_SYSTEMCTL_FAILED_LINES = (
    "Could not {verb} the gateway service; systemd reported an error.",
    "See why with `haishui gateway status --deep` or `{journal}`.",
    "To reinstall the service run `haishui gateway install --force`.",
)

_NO_SYSTEMCTL_LINES = (
    "This system has no systemd, so Haishui cannot install a background service here.",
    "Run the gateway directly with `haishui gateway run` (keep it alive with tmux or screen).",
)
