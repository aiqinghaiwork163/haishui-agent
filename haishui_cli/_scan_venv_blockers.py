"""Standalone venv-process scan for JSON consumption (``python -m haishui_cli._scan_venv_blockers``).
...
def _is_updater_owned_backend(pid: int, cmdline: str) -> bool:
    """True when *pid* is a Haishui backend the CLI updater can stop (positive ledger identity).

    The gateway exemption above keeps ``gateway run`` holders out of the blocker list because the updater's
    own pause machinery stops and resumes them. ``haishui serve`` / ``haishui dashboard`` backends had no such
...
def _updater_owned_backend_entry(pid: int, cmdline: str) -> dict | None:
    ...
    """
    try:
        from haishui_cli.update_cmd import _haishui_holder_subcommand  # noqa: PLC0415

        purpose = _haishui_holder_subcommand(cmdline)
    except Exception:
        return None
    if purpose not in _UPDATER_STOPPABLE_PURPOSES:
        return None
    try:
        from haishui_cli.process_identity import ledger_entries, spawner_is_dead  # noqa: PLC0415
