...
_WINDOWS_GATEWAY_BREAKAWAY_ENV = "_HAISHUI_GATEWAY_BREAKAWAY"
...
def noninteractive_git_env(base: "Mapping[str, str] | None" = None) -> dict[str, str]:
    ...
    Haishui's plumbing calls. ``core.sshCommand`` is pinned to ``ssh -o BatchMode=yes`` so the ssh
    ...
    Haishui shells out to git from many non-interactive contexts — MCP catalog installs, plugin
...
def _text_names_haishui(text: str) -> bool:
    r"""True when *text* names Haishui at a path-segment / token boundary.

    A bare ``"haishui" in text`` substring test would also match unrelated processes whose paths
    merely contain the letters (``...\shaishuia\...``) — the false-positive class this prevents.
    """
    return any(token.startswith(("haishui", ".haishui"))
               for token in re.split(r"[\\/\s=,;\"']+", text.lower()))


def _process_command_is_haishui(pid: int) -> bool:
    """Best-effort check that *pid* currently runs Haishui code."""
    try:
        import psutil
        ...
        return _text_names_haishui(f"{command} {executable}")
    except Exception:
        return False


def pid_is_haishui(pid: int, *, expected_start_time: int | None = None) -> bool:
    """Whether it is safe to use ``taskkill`` for *pid*.

    The PID must be valid, currently exist, and identify a Haishui process. When the caller captured
    ...
    if expected_start_time is not None and current_start_time != expected_start_time:
        return False
    try:
        return _process_command_is_haishui(pid)
    except Exception:
        return False
...
def _legacy_kill_process_tree(proc: "subprocess.Popen") -> None:
    ...
    if IS_WINDOWS:
        ...
        # be recycled while we hold it. The fail-closed ``pid_is_haishui`` guard is for BARE pids.
        try:
...
def bounded_probe_run(
    ...
    ``Get-CimInstance Win32_Process`` gateway scan hit exactly this during ``haishui update`` on slow-WMI
