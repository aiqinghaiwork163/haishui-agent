"""``haishui debug`` subcommand parser."""
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Haishui Agent. Use 'haishui debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    haishui debug share              Upload debug report (asks for confirmation)
    haishui debug share --yes        Skip confirmation (for scripts/CI)
    haishui debug share --lines 500  Include more log lines
    haishui debug share --expire 30  Keep dpaste.com fallback pastes for 30 days
    haishui debug share --local      Print report locally (no upload)
    haishui debug share --no-redact  Disable upload-time secret redaction
    haishui debug share --nous       Upload to Nous-internal storage (private)
    haishui debug delete <url>       Delete a previously uploaded paste
""")
    ...
    delete_parser = debug_sub.add_parser(
        "delete", help="Delete a paste uploaded by 'haishui debug share'")
