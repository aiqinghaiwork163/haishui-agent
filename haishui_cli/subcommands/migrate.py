"""``haishui migrate`` subcommand parser."""
...
def build_migrate_parser(subparsers) -> None:
    ...
    from haishui_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    migrate_relay = migrate_subparsers.add_parser(
        "relay", help="Convert legacy HAISHUI_NEMO_RELAY_ATIF_*/ATOF_* exporter vars into relay-plugins.toml",
        description="The NeMo Relay cutover stopped reading the legacy exporter variables; a .env that still "
            "carries them (and no HAISHUI_NEMO_RELAY_PLUGINS_TOML) exports nothing. Generate "
            "<haishui home>/relay-plugins.toml from them, point HAISHUI_NEMO_RELAY_PLUGINS_TOML at it, "
            "and comment the legacy lines out. `haishui update` runs this for every profile automatically.")
    migrate_relay.add_argument(
        "--all-profiles", action="store_true",
        help="Migrate the default home and every named profile (what `haishui update` does)")
    migrate_relay.add_argument(
...
def _cmd_migrate_relay(args) -> None:
    from haishui_cli.relay_plugin_migrate import cmd_migrate_relay
