"""``haishui login`` subcommand parser."""
...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``invalid choice``. Registered WITHOUT ``help=`` so it is omitted from ``haishui --help``
    ...
    login_parser = subparsers.add_parser(
        "login",
        description="Deprecated. Use `haishui auth` to manage credentials, "
    ...
    login_parser.add_argument(
        "--provider", default=None, help="(deprecated) Provider name; ignored — see `haishui model`")
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: haishui-cli)")
