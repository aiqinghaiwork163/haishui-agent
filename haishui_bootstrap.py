"""Process bootstrap for Haishui entry points: Windows UTF-8 stdio, import-path
...
point (``haishui``, ``haishui-agent``, ``haishui-acp``, ``gateway.run``, ``batch_runner``,
...
here may pull in a Haishui package that a project-local directory could shadow.
"""
...
def _patch_urllib3_create_connection(module) -> None:
    ...
    if getattr(module.create_connection, "_haishui_happy_eyeballs", False):
        return
    ...
    _urllib3_racer._haishui_happy_eyeballs = True  # type: ignore[attr-defined]
...
class _Urllib3ConnectionPatcher(importlib.abc.MetaPathFinder, importlib.abc.Loader):
    ...
    Importing urllib3 eagerly costs ~50 ms on every CLI start, and ``haishui`` / the TUI
...
def install_happy_eyeballs_socket_connect() -> None:
    ...
    """
    if getattr(socket.create_connection, "_haishui_happy_eyeballs", False):
        return
    ...
    _socket_racer._haishui_happy_eyeballs = True  # type: ignore[attr-defined]
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``haishui_cli._subprocess_compat.suppress_platform_ver_console`` for callers
    that never import ``haishui_cli.main``; double application is harmless.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Haishui modules.

    Haishui ships top-level modules with common names (``utils``, ``proxy``, ``ui``); a
    ...
    dropped and the Haishui root is *relocated* to the front, not merely inserted when
    ...
    """
    root = src_root or os.environ.get("HAISHUI_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    """Put the durable lazy-install dir (``HAISHUI_LAZY_INSTALL_TARGET``) on ``sys.path``.
    ...
    """
    if not os.environ.get("HAISHUI_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points only need ``import haishui_bootstrap`` first.
