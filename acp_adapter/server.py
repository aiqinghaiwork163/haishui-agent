"""ACP agent server — exposes Haishui Agent via the Agent Client Protocol."""
...
from acp_adapter.commands import HAISHUI_VERSION, SlashCommandsMixin, _estimate_tokens
...
from tools.approval_context import reset_haishui_interactive_context, set_haishui_interactive_context
...
def _history_summary_meta(message: dict[str, Any], text: str) -> dict[str, Any] | None:
    ...
    if kind is None and message.get(COMPRESSED_SUMMARY_METADATA_KEY):
        ...
        kind = "standalone"
    if kind == "standalone":
        return {"haishui": {"compactionSummary": True}}
    if kind == "merged":
        return {"haishui": {"containsCompactionSummary": True}}
    return None
...
class HaishuiACPAgent(SlashCommandsMixin, acp.Agent):
    """ACP Agent implementation wrapping Haishui AIAgent."""
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        """Authenticated providers + models, from the shared Haishui inventory (same substrate
        as ``haishui model``/TUI/dashboard) so the selector isn't just the current curated list."""
    ...
    def _switch_model(
        ...
        Resolution goes through ``haishui_cli.model_switch.switch_model`` seeded with the live
        ...
        from haishui_cli.config import get_compatible_custom_providers, load_config
    ...
    def _provenance_meta(
        self, acp_session_id: str, current_haishui_session_id: str, previous_haishui_session_id: Optional[str] = None
    ) -> Optional[dict]:
        """Best-effort ``_meta.haishui.sessionProvenance`` for an ACP session."""
        try:
            return session_provenance_meta(
                self.session_manager._get_db(), acp_session_id, current_haishui_session_id,
                previous_haishui_session_id=previous_haishui_session_id,
            )
        except Exception:
    ...
    async def _send_session_info_update(
        self, session_id: str, *,
        current_haishui_session_id: Optional[str] = None, previous_haishui_session_id: Optional[str] = None,
    ) -> None:
        """Session metadata update; pass ``previous_haishui_session_id`` when the internal head
        ...
        update = SessionInfoUpdate(
            ...
            field_meta=self._provenance_meta(
                session_id, current_haishui_session_id or session_id, previous_haishui_session_id
            ),
        )
        await self._send(
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            agent.enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(agent, "enabled_toolsets", None) or ["haishui-acp"],
    ...
    def _schedule_mcp_late_refresh(self, state: SessionState) -> None:
        ...
        try:
            from haishui_cli.mcp_startup import mcp_discovery_in_flight
        except Exception:
            return
        ...
        def _wait_then_refresh() -> None:
            try:
                from haishui_cli.mcp_startup import join_mcp_discovery
    ...
    async def initialize(
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="haishui-agent", version=HAISHUI_VERSION),
    ...
    def _run_agent_turn(
        ...
        Interactive routing is a ``tools.approval`` contextvar, not ``HAISHUI_INTERACTIVE`` in
        ...
        with contextlib.ExitStack() as stack:
            # HAISHUI_SESSION_KEY scopes per-session caches (interactive sudo password) to this
            # session, not the reused thread. ``cwd`` pins what the system prompt reports as the
            # working directory — otherwise it advertises the Haishui workspace while tools are
            # rooted at the client's project and edits land outside it. ``cron_session=""`` masks
            # any leaked process-global HAISHUI_CRON_SESSION.
            ...
            stack.callback(reset_haishui_interactive_context, set_haishui_interactive_context(True))
            # Tools tag side-effects with the ACP session (``kanban_create``); save/restore it.
            stack.callback(_restore_env, "HAISHUI_SESSION_ID", os.environ.get("HAISHUI_SESSION_ID"))
    ...
    async def prompt(self, prompt: list[PromptBlock], session_id: str, **kwargs: Any) -> PromptResponse:
        """Run Haishui on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        try:
            ...
            pre_turn_haishui_id = getattr(state.agent, "session_id", None)
        ...
        return await self._finish_turn(state, session_id, conn, result, pre_turn_haishui_id, cbs.streamed)
    ...
    async def _finish_turn(
        self, state: SessionState, session_id: str, conn: Any, result: dict, pre_turn_haishui_id: Any,
        ...
        post_turn_haishui_id = getattr(state.agent, "session_id", None)
        if conn and post_turn_haishui_id and pre_turn_haishui_id and post_turn_haishui_id != pre_turn_haishui_id:
            try:
                await self._send_session_info_update(
                    session_id, current_haishui_session_id=post_turn_haishui_id,
                    previous_haishui_session_id=pre_turn_haishui_id,
                )
            except Exception:
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Haishui has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
