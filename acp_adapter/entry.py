"""CLI entry point for the haishui-agent ACP adapter.

Loads ``~/.haishui/.env``, routes logging to stderr (stdout is reserved for ACP
JSON-RPC), and starts the ACP agent server.

Usage::

    python -m acp_adapter.entry   # or: haishui acp / haishui-acp
"""

# IMPORTANT: haishui_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See haishui_bootstrap.py for full rationale.
try:
    import haishui_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Partial ``haishui update`` (git-reset landed, ``uv pip install -e .`` did not):
    # UTF-8 stdio setup is skipped on Windows; POSIX is unaffected.
    pass
else:
    # Stop a ``utils/``/``proxy/``/``ui/`` package in the launch cwd from shadowing Haishui modules.
    haishui_bootstrap.harden_import_path()
...
def _load_env() -> None:
    """Load .env from HAISHUI_HOME (default ``~/.haishui``)."""
    from haishui_cli.env_loader import load_haishui_dotenv

    haishui_home = get_haishui_home()
    loaded = load_haishui_dotenv(haishui_home=haishui_home)
    log = logging.getLogger(__name__)
    for env_file in loaded or ():
        log.info("Loaded env from %s", env_file)
    if not loaded:
        log.info("No .env found at %s, using system env", haishui_home / ".env")


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(prog="haishui-acp", description="Run Haishui Agent as an ACP stdio server.")
    parser.add_argument("--version", action="store_true", help="Print Haishui version and exit")
    parser.add_argument("--check", action="store_true", help="Verify ACP dependencies and adapter imports, then exit")
    parser.add_argument("--setup", action="store_true",
                        help="Run interactive Haishui provider/model setup for ACP terminal auth")
    parser.add_argument("--setup-browser", action="store_true",
                        help="Install agent-browser + Playwright Chromium into ~/.haishui/node/ "
...
def _print_version() -> None:
    from haishui_cli import __version__ as haishui_version

    print(haishui_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import HaishuiACPAgent  # noqa: F401

    print("Haishui ACP check OK")


def _run_setup() -> None:
    from haishui_cli.main import main as haishui_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "haishui", "model"]
        haishui_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting haishui-agent ACP adapter")
    ...
    import acp
    from .server import HaishuiACPAgent
    ...
    if os.environ.get("HAISHUI_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
