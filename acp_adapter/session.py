"""ACP session manager — maps ACP sessions to Haishui AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.haishui/state.db``) so they
...
from haishui_constants import get_haishui_home, translate_cwd_for_wsl_backend, windows_path_to_wsl
...
def _translate_acp_cwd(cwd: str) -> str:
    ...
    when Haishui runs in WSL so agents, tools, and persisted sessions agree; no-op elsewhere."""
...
def _expand_acp_enabled_toolsets(toolsets: List[str] | None = None,
    ...
    names = [n for n in (toolsets or ["haishui-acp"]) if n]
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Haishui agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Haishui AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        the runtime provider config. ``db``: SessionDB; default lazily opens ``~/.haishui/state.db``."""
    ...
    def _get_db(self):
        ...
        error in a minimal test env). ``HAISHUI_HOME`` is resolved here, not via the import-time
        ...
        if self._db_instance is None:
            try:
                ...
                self._db_instance = acquire(get_haishui_home() / "state.db")
            except Exception:
    ...
    def _make_agent(self, *, session_id: str, cwd: str, model: str | None = None,
        ...
        kwargs = {
            ...
            "enabled_toolsets": (list(enabled_toolsets) if enabled_toolsets is not None
                                 else _expand_acp_enabled_toolsets(["haishui-acp"], mcp_server_names=configured_mcp_servers)),
        ...
        # picked up by HaishuiACPAgent._schedule_mcp_late_refresh.
        try:
