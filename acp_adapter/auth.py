"""ACP auth helpers — detect and advertise Haishui authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "haishui-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Haishui runtime provider, or None if unavailable.
...
def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Haishui.

    The ACP registry requires at least one usable auth method in the initial
    handshake. A fresh Zed install may have no Haishui credentials yet, so the
    ...
    if provider:
        methods.append(AuthMethodAgent(
            ...
            description=f"Authenticate Haishui using the currently configured {provider} runtime credentials.",
        ))
    methods.append(TerminalAuthMethod(
        id=TERMINAL_SETUP_AUTH_METHOD_ID, name="Configure Haishui provider", type="terminal", args=["--setup"],
        description=("Open Haishui' interactive model/provider setup in a terminal. "
                     "Use this when Haishui has not been configured on this machine yet."),
    ))
    return methods
...
def has_provider() -> bool:
    """Return True if Haishui can resolve any runtime provider credentials."""
