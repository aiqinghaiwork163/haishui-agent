...
Edit with `haishui config edit` or `haishui config set section.key value`.
Full reference: https://aiqinghaiwork163.github.io/haishui-agent/docs/user-guide/configuration
...
`haishui config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `haishui tools` (interactive) or `haishui tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_HAISHUI_CORE_TOOLS` is the default bundle most platforms inherit).
