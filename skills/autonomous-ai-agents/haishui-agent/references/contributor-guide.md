...
Config: `~/.haishui/config.yaml` (settings), `~/.haishui/.env` (API keys) — both under `$HAISHUI_HOME` when it is set.
...
`_HAISHUI_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_haishui_home()` for paths,
never hardcode `~/.haishui`. For custom/local-only tools, write a plugin in
`~/.haishui/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `haishui_cli/commands.py`
...
- Tests auto-redirect `HAISHUI_HOME` to temp dirs — never touch real `~/.haishui/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/haishui_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_haishui_home()` from `haishui_constants` for all paths (profile-safe)
