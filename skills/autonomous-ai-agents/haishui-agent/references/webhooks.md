...
Create dynamic webhook subscriptions so external services (GitHub, GitLab, Stripe, CI/CD, IoT sensors, monitoring tools) can trigger Haishui agent runs by POSTing events to a URL.
...
Add to `${HAISHUI_HOME:-~/.haishui}/.env`:
```bash
...
All management is via the `haishui webhook` CLI command:

### Create a subscription
```bash
haishui webhook subscribe <name> \
...
- **Route scripts** (`--script` on subscribe, or `script:` on a config route): a script under `~/.haishui/scripts/` receives the payload as JSON on stdin. JSON stdout replaces the payload before prompt templating; empty stdout, `[SILENT]`, or a nonzero exit ignores the webhook. `.sh`/`.bash` run with bash, everything else with Python. Scripts cannot live outside `~/.haishui/scripts/` (path traversal is blocked).

```bash
haishui webhook subscribe todoist-haishui \
  --prompt "Task changed: {payload.content}" \
  --script "todoist-haishui-label.py" \
...
```bash
haishui webhook remove <name>
```

### Test a subscription
```bash
haishui webhook test <name>
haishui webhook test <name> --payload '{"key": "value"}'
```
...
### GitHub: PR reviews
```bash
haishui webhook subscribe github-prs \
  --events "pull_request" \
...
```bash
haishui webhook subscribe ci-builds \
  --events "pipeline" \
...
```bash
haishui webhook subscribe alerts \
...
- Subscriptions persist to `~/.haishui/webhook_subscriptions.json`

## How It Works

1. `haishui webhook subscribe` writes to `~/.haishui/webhook_subscriptions.json`
...
1. **Is the gateway running?** Check with `systemctl --user status haishui-gateway` or `ps aux | grep gateway`
2. **Is the webhook server listening?** `curl http://localhost:8644/health` should return `{"status": "ok"}`
3. **Check gateway logs:** `grep webhook ~/.haishui/logs/gateway.log | tail -20`
4. **Signature mismatch?** Verify the secret in your service matches the one from `haishui webhook list`. GitHub sends `X-Hub-Signature-256`, GitLab sends `X-Gitlab-Token`.
5. **Firewall/NAT?** The webhook URL must be reachable from the service. For local development, use a tunnel (ngrok, cloudflared).
6. **Wrong event type?** Check `--events` filter matches what the service sends. Use `haishui webhook test <name>` to verify the route works.
