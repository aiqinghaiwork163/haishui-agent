# Themes / Skins — Author a Haishui Color Theme

Author a Haishui **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`haishui_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.haishui/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Haishui itself.
...
- Write access to the Haishui home dir — `~/.haishui` by default, or `$HAISHUI_HOME`
  / the active profile's dir. Skins live in `<haishui-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `haishui config set`).
...
3. `write_file` it to `<haishui-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `haishui config set display.skin default` (or they can `/skin default`).
...
```
haishui skin set <key> <hex>      # e.g. haishui skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `haishui skin set background "#08201f"` changes
only the background; `haishui skin use <name>` / `haishui skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.haishui`** when a profile is active — resolve the real home
  from `$HAISHUI_HOME` first, falling back to `~/.haishui`.
...
- **Never hand-edit `config.yaml` to activate.** Use `haishui config set
...
- **You apply it, not the user.** `haishui config set display.skin <name>` is
...
- `read_file` the written `<haishui-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `haishui config get display.skin` and confirm it reports `<name>`.
