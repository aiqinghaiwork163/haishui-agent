...
| nous | OAuth device code | `haishui auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `haishui auth add openai-codex` |
| qwen-oauth | OAuth | `haishui auth add qwen-oauth` |
| minimax-oauth | OAuth | `haishui auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`haishui auth`).
Fallback chain when the primary fails: `haishui fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`haishui_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   haishui config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
