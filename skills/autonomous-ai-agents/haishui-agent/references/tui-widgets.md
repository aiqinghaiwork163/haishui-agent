...
Author widget apps for the Haishui TUI (`haishui --tui`): glanceable ambient
...
- The TUI must be in use (`haishui --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.haishui/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.haishui/logs/tui_gateway_crash.log` if a widget never appears.
