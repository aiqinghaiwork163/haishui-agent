...
Write plugins for the Haishui desktop app: statusbar items, layout panes,
...
system (`~/.haishui/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$HAISHUI_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$HAISHUI_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
  ...
  `.haishui-package.json` marker) — the ONLY root the renderer loads from — so
...
- The Haishui desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$HAISHUI_HOME/desktop-plugins/` (usually
  `~/.haishui/desktop-plugins/`).

## How to Run

1. Create `$HAISHUI_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.haishui/...` by default, or `~/.haishui/profiles/<profile>/...` under a
...
The ONLY import surface is `@haishui/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Haishui
  ...
  (`haishui://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.haishui/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@haishui/plugin-sdk`, `react`, and
