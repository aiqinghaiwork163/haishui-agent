...
Haishui runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
`haishui config edit` writes correctly.

### `execute_code` / Sandbox

**WinError 10106** from the sandbox child process — it can't create an
`AF_INET` socket. Root cause is usually Haishui's env scrubber dropping
...
Haishui-installed `venv/Scripts/` has no pip/pytest (stripped for size).
...
every Haishui tool and most Windows APIs. Prefer forward slashes in code
