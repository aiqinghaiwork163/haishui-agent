...
## Layer 1 — Is this thing a Haishui plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Haishui process**. Its LLM calls go through Haishui's already-configured provider. | Already uses Portal if user's Haishui is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Haishui. That LLM call is already authenticated
via Haishui's provider config. The plugin's own server auth (e.g.
...
What Haishui Agent has that *feels* like OAuth — `haishui login --provider nous`
opening a browser, user signs in, token lands in `~/.haishui/auth.json` — is a
**Haishui-specific browser flow**. Under the hood it produces a credential
Haishui uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Haishui's existing Portal credential out of `~/.haishui/auth.json`.
...
rides on the credential Haishui already holds.

Where this could live in Haishui:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Haishui itself.
...
1. First decide: Haishui plugin (runs inside Haishui) or separate app? If plugin,
   it already uses Portal via Haishui's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Haishui agent itself") is fully supported. Haishui treats agent
...
pitch in the Tool Gateway announcement is specifically about Haishui Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Haishui is configured to use Portal as its provider. It is
