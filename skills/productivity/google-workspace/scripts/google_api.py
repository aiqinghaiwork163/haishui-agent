#!/usr/bin/env python3
"""Google Workspace API CLI for Haishui Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Haishui-facing JSON contract and falls back to the Python client
...
from _haishui_home import get_haishui_home
...
def _gws_binary() -> str | None:
    override = os.getenv("HAISHUI_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Haishui Agent")
