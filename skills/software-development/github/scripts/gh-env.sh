#!/usr/bin/env bash
# GitHub environment detection helper for Haishui Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _haishui_env="${HAISHUI_HOME:-$HOME/.haishui}/.env"; [ -f "$_haishui_env" ] && grep -q "^GITHUB_TOKEN=" "$_haishui_env" 2>/dev/null; then
