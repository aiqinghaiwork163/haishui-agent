...
Ledger path resolution (first wins):
  --ledger PATH
  $HAISHUI_CITATION_LEDGER
  $HAISHUI_HOME/cache/citations/ledger.json
"""
...
from _haishui_home import get_haishui_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("HAISHUI_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_haishui_home() / "cache" / "citations" / "ledger.json"
