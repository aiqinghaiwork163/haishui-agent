...
```bash
haishui              # CLI interactiva — inicia una conversación
haishui model        # Elige tu proveedor y modelo LLM
haishui tools        # Configura qué herramientas están habilitadas
haishui config set   # Establece valores de configuración individuales
haishui gateway      # Inicia el gateway de mensajería (Telegram, Discord, etc.)
haishui setup        # Ejecuta el asistente de configuración completo
haishui claw migrate # Migra desde OpenClaw (si vienes de OpenClaw)
haishui update       # Actualiza a la última versión
...
Haishui funciona con cualquier proveedor que quieras — eso no cambiará. Pero si prefieres no recopilar cinco claves API separadas para el modelo, búsqueda web, generación de imágenes, TTS y un navegador en la nube, **[Nous Portal](https://portal.nousresearch.com)** las cubre todas bajo una sola suscripción:
...
Haishui tiene dos puntos de entrada: inicia la interfaz de terminal con `haishui`, o ejecuta el gateway y habla con él desde Telegram, Discord, Slack, WhatsApp, Signal o Email. Una vez en una conversación, muchos comandos de barra son compartidos entre ambas interfaces.

| Acción                              | CLI                                           | Plataformas de mensajería                                                         |
| ----------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------- |
| Empezar a chatear                   | `haishui`                                      | Ejecuta `haishui gateway setup` + `haishui gateway start`, luego envía un mensaje al bot |
...
Si vienes de OpenClaw, Haishui puede importar automáticamente tu configuración, memorias, habilidades y claves API.

**Durante la configuración inicial:** El asistente de configuración (`haishui setup`) detecta automáticamente `~/.openclaw` y ofrece migrar antes de que comience la configuración.

**En cualquier momento después de instalar:**

```bash
haishui claw migrate              # Migración interactiva (preset completo)
haishui claw migrate --dry-run    # Vista previa de qué se migraría
haishui claw migrate --preset user-data   # Migrar sin secretos
haishui claw migrate --overwrite  # Sobreescribir conflictos existentes
```
...
- **Habilidades** — habilidades creadas por el usuario → `~/.haishui/skills/openclaw-imports/`
...
Consulta `haishui claw migrate --help` para todas las opciones, o usa la habilidad `openclaw-migration` para una migración guiada interactiva por el agente con vistas previas de dry-run.
