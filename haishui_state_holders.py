...
an unlinked SQLite DB/WAL/SHM generation.  ``haishui_state`` supplies only the
...
_HAISHUI_EXECUTABLES = frozenset({"haishui", "haishui-agent", "haishui-acp"})
_HAISHUI_PYTHON_MODULES = frozenset({"acp_adapter", "haishui_cli.main"})
_HAISHUI_PYTHON_SCRIPTS = frozenset({"haishui_cli/main.py", "run_agent.py"})
...
def _looks_like_haishui(argv: Sequence[str]) -> bool:
    """Return whether argv identifies a supported Haishui execution target."""
    if not argv:
        return False
    program = os.path.basename(argv[0]).lower().removesuffix(".exe")
    if program in _HAISHUI_EXECUTABLES:
        return True
    ...
    if kind == "module":
        return normalized in _HAISHUI_PYTHON_MODULES
    return any(
        normalized == script or normalized.endswith(f"/{script}")
        for script in _HAISHUI_PYTHON_SCRIPTS
    )
...
def _argv_scoped_to_other_home(argv: Sequence[str], db_path: Path) -> bool:
    ...
    ``state.db`` lives at the HAISHUI_HOME root, so an absolute-path token
    containing a ``/.haishui`` segment (or naming a ``state.db``/WAL/SHM under
    some other parent) identifies that token's own Haishui home.  When at least
    ...
    of ours (issue #92401: a second gateway under /home/demo/.haishui deferred
    ...
    for token in argv:
        ...
        if path_token is not None:
            ...
            if normalized in ours or normalized.startswith(this_home + os.sep):
                return False
            if "/.haishui" in normalized or normalized.endswith("/.haishui"):
...
def foreign_state_db_holders(db_path: Path) -> List[Tuple[int, str]]:
    ...
    # HAISHUI_HOME would otherwise make every holder invisible and let maintenance proceed.
