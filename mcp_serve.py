"""
Haishui MCP Server — expose messaging conversations as MCP tools (`haishui mcp serve`).
...
Matches OpenClaw's 9-tool channel bridge surface plus the Haishui-specific
channels_list. Client config: {"mcpServers": {"haishui": {"command": "haishui", "args": ["mcp", "serve"]}}}
"""
...
logger = logging.getLogger("haishui.mcp_serve")
...
def _haishui_home() -> Path:
    try:
        ...
        return Path(os.environ.get("HAISHUI_HOME", Path.home() / ".haishui"))


def _get_sessions_dir() -> Path:
    return _haishui_home() / "sessions"


def _read_state_db_mtime() -> float:
    try:
        return (_haishui_home() / "state.db").stat().st_mtime
...
def _load_channel_directory() -> dict:
    ...
    return _read_json(_haishui_home() / "channel_directory.json")
...
class EventBridge:
    ...
    event queue with waiter support (the Haishui analogue of OpenClaw's WebSocket
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the Haishui MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(f"MCP server requires the 'mcp' package. Install with: {sys.executable} -m pip install 'mcp'")
    mcp = MCPServer("haishui", instructions=(
        "Haishui Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Haishui MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
