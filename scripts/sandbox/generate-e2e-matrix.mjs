 ...
 *   The artifact published on the website right now -- Haishui-Setup.exe has
 ...
 * @typedef {InstallMethod | 'haishui-update' | 'open-app-update' | 'haishui-desktop-app-update'} UpdateMethod
 ...
 *   desktop-installer install; haishui-desktop-app-update starts the app
 *   via `haishui desktop`, which every install method provides on every
...
export const SPEC = {
  windows: {
    install: [
      ...
      // Website Haishui-Setup.exe, clicked through the GUI.
    ...
    update: [
      ...
      // Same button, app launched via `haishui desktop`.
      { method: 'haishui-desktop-app-update' },
    ],
  },
  macos: {
    install: [
      { method: 'installer-script' },
      { method: 'installer-script+desktop' },
      // The published Haishui-Setup.dmg from the website, mounted and run.
      { method: 'desktop-installer', versions: ['latest'] },
    ],
    update: [
      { method: 'installer-script' },
  ...
  linux: {
    ...
    update: [
      ...
      // linux, so there is no open-app-update; `haishui desktop` is always
...
export function methodNeedsDesktop(m) {
  return m.startsWith('desktop-installer') || m === 'installer-script+desktop' ||
    m === 'open-app-update' || m === 'haishui-desktop-app-update';
}
