...
VERSION_FILE = REPO_ROOT / "haishui_cli" / "__init__.py"
...
LEGACY_AUTHOR_MAP = {
    ...
    "pi@haishui.local": "Elektrofussel",  # PR #61675 salvage (honcho: defaultHost + private-range local URL detection)
    ...
    "kavi@local.haishui": "kavioavio",  # Issue #46544 / PR #47705 evolution (smart DENY exact-operation owner override)
    ...
    "sunsky.lau@gmail.com": "liuhao1024",  # PR #56993 salvage (gateway: process-level HAISHUI_HOME for pid/lock/status identity files; #56986)
    ...
    "wyuebei@gmail.com": "wyuebei-cloud",  # PR #56640 salvage (haishui journey: replace GNU-only %-d strftime with dt.day for Windows)
    ...
    "haishui.wanderer@yahoo.com": "trismegistus-wanderer",  # PR #31856 salvage (gateway: defer idle-TTL agent-cache eviction until the session store says the session actually expired, so the expiry watcher can still fire MemoryProvider.on_session_end with the live transcript; #11205)
    ...
    "me@keslerm.com": "keslerm",  # PR #3459 salvage (gateway: 'log' tool_progress mode — silent in chat, tool calls appended to ~/.haishui/logs/tool_calls.log via rotating handler; duplicate of #3458 by @dlkakbs who submitted 4 min earlier — both credited)
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows haishui-shim resolution + prefer --update on recovery; #52378)
    ...
    "haishui@marian.local": "Schrotti77",
    ...
    "274096618+haishui-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "haishui@nousresearch.com": "NousResearch",
    ...
    "sberan@gmail.com": "sberan",  # PR #54494 salvage (--connect-timeout flag on haishui mcp add)
    ...
    "haishui@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "teknium@haishui-agent": "teknium1",
    ...
    "github@nadyahaishui.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (HAISHUI_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (HAISHUI_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: haishui-eval community link)
    ...
    "haishuiagent26@gmail.com": "haishuiagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+haishuiagent26@users.noreply.github.com": "haishuiagent26",
    ...
    "vigo@haishui": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.haishui@agentmail.to": "oseftg",
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Haishui runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level HAISHUI_HOME state files)
    ...
    "zhchl@haishui-agent.local": "8294",  # PR #50572 salvage (honor config context_length on banner)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Haishui
    ...
    # Keep the bootstrap installer (Haishui-Setup.dmg CFBundleShortVersionString)
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "haishui@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/aiqinghaiwork163/haishui-agent",
    ...
    lines.append(f"# Haishui Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Haishui Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Haishui Agent Release Tool")
    ...
    print(f"{'='*60}")
    print("  Haishui Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Haishui Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Haishui Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Haishui Agent v{new_version} ({calver_date})' "
