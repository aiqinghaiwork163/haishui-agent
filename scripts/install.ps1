# ============================================================================
# Haishui Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$HaishuiHome = $(if ($env:HAISHUI_HOME) { $env:HAISHUI_HOME } else { "$env:LOCALAPPDATA\haishui" }),
    [string]$InstallDir = $(if ($env:HAISHUI_HOME) { "$env:HAISHUI_HOME\haishui-agent" } else { "$env:LOCALAPPDATA\haishui\haishui-agent" }),
    ...
    # builds apps/desktop into a launchable Haishui.exe.
    #
    # Why opt-in:
    #   * Haishui-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Haishui.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Haishui.exe
    ...
    #     `haishui desktop` already builds on demand.
...
# HAISHUI_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[haishui] $Message")
}
...
function ConvertTo-LongPath {
    ...
    try {
        if (-not ([System.Management.Automation.PSTypeName]'HaishuiInstall.LongPath').Type) {
            Add-Type -Namespace 'HaishuiInstall' -Name 'LongPath' -MemberDefinition @'
...
'@
        ...
        $length = [HaishuiInstall.LongPath]::GetLongPathNameW($Path, $buffer, $buffer.Capacity)
...
# long. An explicitly passed -HaishuiHome / -InstallDir is normalized in place
...
if ($PSBoundParameters.ContainsKey('HaishuiHome')) {
    $HaishuiHome = ConvertTo-LongPath $HaishuiHome
} else {
    $HaishuiHome = ConvertTo-LongPath $(
        if ($env:HAISHUI_HOME) { $env:HAISHUI_HOME } else { "$env:LOCALAPPDATA\haishui" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:HAISHUI_HOME) { "$env:HAISHUI_HOME\haishui-agent" } else { "$env:LOCALAPPDATA\haishui\haishui-agent" }
    )
}
if ($script:NormalizedProfilePaths) {
    ...
    Write-PathDiag "resolved install paths: HaishuiHome=$HaishuiHome InstallDir=$InstallDir"
}
...
function Write-Banner {
    ...
    Write-Host "|             * Haishui Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $HaishuiHome)) {
        New-Item -ItemType Directory -Force -Path $HaishuiHome | Out-Null
    }
    $envFile = Join-Path $HaishuiHome ".env"
...
function Install-AgentBrowser {
    ...
    # which every consumer (tools/browser_tool.py, `haishui update`'s npx
    ...
    $prefixDir = Join-Path $HaishuiHome "node"
...
function Install-Uv {
    # Haishui owns its own uv at $HaishuiHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (haishui_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `haishui update` stay in sync.
    $managedUv = Join-Path $HaishuiHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $HaishuiHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $HaishuiHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $HaishuiHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $HaishuiHome "bin"
        ...
        # (haishui_cli/managed_uv.py looks only at $HaishuiHome\bin\uv.exe).
...
# Put the Haishui-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone haishui-setup.exe run, a user typing `npm`) silently
...
# Upgrade the Haishui-managed Node tree's bundled npm into $NpmRange when
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "Haishui-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }

    Write-Info "Upgrading bundled npm to satisfy $range ..."

    $tmpCwd = Join-Path $env:TEMP ("haishui-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and HAISHUI_HOME wasn't propagated).
...
function Initialize-ManagedPythonEnvironment {
    # Python used by Haishui belongs to the checkout, never to another
    # application or a user-level uv configuration. Keep this aligned with
    # haishui_cli.managed_uv.managed_python_env(), which owns the update path.
    foreach ($name in @(
    ...
    $managedRoot = Join-Path $InstallDir ".haishui-runtime\python"
...
function Resolve-AvailablePythonVersion {
    # Return the path and minor version of the first Haishui-managed interpreter
    ...
    #
    # Under Haishui-Setup.exe each stage runs in a fresh powershell.exe. The
    ...
    foreach ($ver in $candidates) {
        ...
        try {
            ...
            throw "Failed to resolve Haishui-managed Python $ver`: $_"
        } finally {
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Haishui setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Haishui uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\haishui\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Haishui needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Haishui copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\haishui\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``HAISHUI_GIT_BASH_PATH`` (User scope) so Haishui can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a Haishui-managed PortableGit install instead..."
    }

    # Download PortableGit into $HaishuiHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $HaishuiHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Haishui features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Haishui needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``HAISHUI_GIT_BASH_PATH`` (User env scope) so Haishui can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $HaishuiHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $HaishuiHome\git\usr\bin\bash.exe
    $candidates += "$HaishuiHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$HaishuiHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("HAISHUI_GIT_BASH_PATH", $candidate, "User")
            $env:HAISHUI_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set HAISHUI_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Haishui may not find Git Bash."
    Write-Info "If needed, set HAISHUI_GIT_BASH_PATH manually to your bash.exe path."
}
...
# range used to provision the Haishui-managed tree. Keeping this probe separate
...
function Test-SystemNodeReady {
    ...
    if (Test-NodeVersionOk $version) {
        ...
        Write-Warn "Node.js $version is unsupported (Haishui requires Node 22.22+, 24.11+, or 26+)"
        return $false
    }
    ...
    if ($npmVersion) {
        Write-Warn "Node.js $version uses npm $npmVersion, which does not satisfy Haishui requirement $npmRange"
    } else {
...
function Test-Node {
    ...
    Write-Info "Using a Haishui-managed Node.js installation instead..."

    # Prefer a Haishui-managed Node from a previous run over a too-old system one.
    $managedNode = "$HaishuiHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$HaishuiHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$HaishuiHome\node"
        Write-Success "Node.js $version found (Haishui-managed)"
        ...
        Update-ManagedNpm "$HaishuiHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Haishui-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $HaishuiHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $HaishuiHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\haishui-node-extract"
            ...
            if ($extractedDir) {
                ...
                Get-ChildItem "$HaishuiHome" -Directory -Filter "node.old-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                Get-ChildItem "$HaishuiHome" -Directory -Filter "node.new-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                ...
                if (Test-Path "$HaishuiHome\node") {
                    try {
                        Rename-Item "$HaishuiHome\node" $backup -ErrorAction Stop
                    } catch {
                        Write-Warn "Haishui-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
                ...
                $version = & "$HaishuiHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $HaishuiHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\haishui-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `haishui
                    # update` path (#4735).
                    ...
                    $stashName = "haishui-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. haishui-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``haishui update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if Haishui behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running haishui processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/aiqinghaiwork163/haishui-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/aiqinghaiwork163/haishui-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/aiqinghaiwork163/haishui-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\haishui-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\haishui-agent-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `haishui update` (see the notes at the shared clone-path
    ...
    # next `haishui update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Haishui-Setup.exe
    ...
    if (-not $resolvedPython) {
        throw "Haishui-managed Python is unavailable. Run install.ps1 -Stage python first."
    }
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running haishui process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running haishui processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Haishui_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (haishui.exe) plus its child tree.
            & taskkill /F /T /IM haishui.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM haishui.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m haishui_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not haishui.exe.
            ...
            # Haishui children re-exec through .haishui-runtime, so killing only
            ...
            # user (see haishui_cli/gateway_windows.py), so the installer always
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-Haishui " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `haishui update` / the
...
function Restore-VenvBackup {
    ...
    # venv so Haishui (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `haishui.exe` exists
...
try:
    ...
    out = []
    for s in specs:
        m = re.search(r'haishui-agent\[([\w-]+)\]', s)
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install haishui-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and haishui.exe broken with
    ...
    # here, not 30 seconds later when the user runs `haishui`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close Haishui processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close Haishui processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves Haishui and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        # uv on Windows can register haishui.exe in dist-info/RECORD but fail to
        ...
        # `haishui` command while haishui-agent.exe / haishui-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m haishui_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `haishui dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\haishui_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `haishui dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `haishui dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: haishui_cli/web_server.py"
        }
    }
...
function Install-HaishuiCommandLaunchers {
    param(
        [Parameter(Mandatory=$true)] [string]$Root,
        [Parameter(Mandatory=$true)] [string]$Destination
    )

    # Expose ONLY the haishui launchers on PATH -- never the whole
    # venv\Scripts directory, which contains python.exe / pip.exe and
    # silently hijacks the `python` command in every terminal (#83797).
    # Requiring haishui.exe before creating the destination keeps the PATH
    # stage from reporting success with an unusable command (PR #92092).
    $scriptsDir = Join-Path $Root "venv\Scripts"
    $requiredSource = Join-Path $scriptsDir "haishui.exe"
    if (-not (Test-Path -LiteralPath $requiredSource -PathType Leaf)) {
        throw "Cannot set up the haishui command: required launcher not found: $requiredSource"
    }

    New-Item -ItemType Directory -Force -Path $Destination | Out-Null

    # Launcher form depends on the venv (keep in lockstep with
    # haishui_cli/_install_repair.py): a normal venv's exe trampoline
    ...
    foreach ($launcher in @("haishui", "haishui-acp")) {
    ...
    $requiredExe = Join-Path $Destination "haishui.exe"
    $requiredCmd = Join-Path $Destination "haishui.cmd"
    if (-not ((Test-Path -LiteralPath $requiredExe -PathType Leaf) -or
              (Test-Path -LiteralPath $requiredCmd -PathType Leaf))) {
        throw "Cannot set up the haishui command: launcher was not installed: $requiredExe"
    }
    return $Destination
}

function Set-PathVariable {
    Write-Info "Setting up haishui command..."
    
    if ($NoVenv) {
        $haishuiBin = "$InstallDir"
    } else {
        # $HaishuiHome\bin is the managed binary dir (shared with the managed
        # uv), OUTSIDE the git checkout: `haishui update`'s autostash
        ...
        # installer staged under haishui-agent\bin. No git operation can ever
        # touch this dir. Staging and verification live in
        # Install-HaishuiCommandLaunchers, which throws BEFORE any PATH
        # mutation when the launchers cannot be staged.
        $haishuiBin = "$HaishuiHome\bin"
        Install-HaishuiCommandLaunchers -Root $InstallDir -Destination $haishuiBin | Out-Null
    }
    ...
    #   haishui-agent\bin -- lived inside the git checkout, where the update
    #                       autostash could sweep the launchers off disk
    # The haishui-agent\bin FILES are left in place on purpose: editor/ACP
    ...
    if (-not $NoVenv) {
        ...
        if ($cleaned.Count -ne $items.Count) {
            ...
            Write-Info "Removed legacy launcher entries from user PATH (kept haishui via $haishuiBin)"
        }
    }
    
    if ($currentPath -notlike "*$haishuiBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$haishuiBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $haishuiBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set HAISHUI_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\haishui instead
    # of the Unix default ~/.haishui
    $currentHaishuiHome = [Environment]::GetEnvironmentVariable("HAISHUI_HOME", "User")
    if (-not $currentHaishuiHome -or $currentHaishuiHome -ne $HaishuiHome) {
        [Environment]::SetEnvironmentVariable("HAISHUI_HOME", $HaishuiHome, "User")
        Write-Success "Set HAISHUI_HOME=$HaishuiHome"
    }
    $env:HAISHUI_HOME = $HaishuiHome
    
    # Update current session
    $env:Path = "$haishuiBin;$env:Path"
    
    Write-Success "haishui command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.haishui-bootstrap-complete which tells the Haishui
    ...
    # Haishui-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `haishui update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".haishui-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the HAISHUI_HOME directory structure ($HaishuiHome, default %LOCALAPPDATA%\haishui)
    New-Item -ItemType Directory -Force -Path "$HaishuiHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HaishuiHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HaishuiHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HaishuiHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HaishuiHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HaishuiHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HaishuiHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HaishuiHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HaishuiHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$HaishuiHome\.env"
    ...
    # Create config.yaml
    $configPath = "$HaishuiHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Haishui's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$HaishuiHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in haishui_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $HaishuiHome"
    
    # Seed bundled skills into $HaishuiHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $HaishuiHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $HaishuiHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$HaishuiHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $HaishuiHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Haishui-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'haishui setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\haishui-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\haishui-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\haishui-npm-tui-$(Get-Random).log"
...
# failure is non-fatal (browser_exec can still run via uvx, and `haishui tools`
...
function Install-BrowserUseCli {
    ...
    $managedBin = Join-Path $HaishuiHome "bin"
    $managedBu = Join-Path $managedBin "browser-use.exe"
    # MANAGED-FIRST: only Haishui' managed copy short-circuits. A browser-use
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside Haishui' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'haishui tools')"
        }
    } catch {
...
# Provision it at install time so enabling the tool later -- via `haishui
...
# paths still lazy-install via install_cua_driver() (haishui_cli/tools_config)
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `haishui computer-use install` runs. Bounded
        ...
        # Haishui' _CUA_INSTALLER_TIMEOUT (660s).
        ...
        if (Wait-Job $job -Timeout 660) {
            ...
            if ($installedCuaDriver -and (Test-CuaDriverRuntimeContract -DriverPath $installedCuaDriver.Source)) {
                Write-Success "Computer Use driver installed (enable via 'haishui tools' -> Computer Use)"
            } else {
                Write-Warn "Computer Use driver install did not produce a compatible runtime -- repair it before enabling the tool."
                Write-Info "Install later with: haishui computer-use install"
            }
        } else {
...
# the final `electron` -> `Haishui` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Haishui.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_haishui_desktop command
    # resolves apps/desktop/release/win-unpacked/Haishui.exe directly,
    ...
    # enforces the supported Node lines and prepends the Haishui-managed Node to
    ...
    # The --dir mode produces an unpacked Haishui.exe in
    ...
    # are NOT trying to work around). The Haishui icon + product name are
    # stamped onto Haishui.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\haishui-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Haishui.exe",
        "$desktopDir\release\win-arm64-unpacked\Haishui.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Haishui.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Haishui icon + identity are stamped onto Haishui.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, haishui-agent#38216).
    ...
    #    Haishui.exe. We deliberately do NOT point them at `haishui desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Haishui.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Haishui.lnk')
        )
        ...
        # with the Haishui icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.haishui/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'haishui setup'."
        return
    }
    ...
    # Run haishui setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m haishui_cli.main setup
    } else {
        python -m haishui_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $haishuiCmd = "$InstallDir\venv\Scripts\haishui.exe"
    if (-not (Test-Path $haishuiCmd)) {
        $haishuiCmd = "haishui"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$HaishuiHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'haishui whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: haishui gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$HaishuiHome\logs\gateway.log"
            Start-Process -FilePath $haishuiCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$HaishuiHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: haishui gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: haishui gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$HaishuiHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$HaishuiHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$HaishuiHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$HaishuiHome\haishui-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   haishui              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   haishui setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   haishui config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   haishui config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   haishui gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   haishui update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Haishui repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    # Managed Python lives under $InstallDir\.haishui-runtime, so the checkout
...
if ($IncludeDesktop) {
    ...
    # (Haishui-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Haishui to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://aiqinghaiwork163.github.io/haishui-agent/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
