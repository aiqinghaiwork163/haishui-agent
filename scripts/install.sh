...
# Python-driven tool session (e.g. Haishui terminal tool). A pre-set PYTHONPATH
...
HAISHUI_HOME="${HAISHUI_HOME:-$HOME/.haishui}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${HAISHUI_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$HAISHUI_INSTALL_DIR"
...
#   code at /usr/local/lib/haishui-agent, command at /usr/local/bin/haishui,
#   data still at /root/.haishui (HAISHUI_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$HAISHUI_HOME/.no-bundled-skills so future"
            echo "                   'haishui update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Haishui.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.haishui/haishui-agent"
            echo "                   default (root, Linux): /usr/local/lib/haishui-agent"
            echo "  --haishui-home PATH  Data directory (default: ~/.haishui, or \$HAISHUI_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Haishui installs the code under"
            echo "  /usr/local/lib/haishui-agent and links the command into"
            echo "  /usr/local/bin/haishui (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$HAISHUI_HOME"
            echo "  (default /root/.haishui).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$HAISHUI_HOME/haishui-agent are preserved in-place."
...
# intentional, but they leave the checkout dirty — which forces `haishui update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Haishui-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Haishui Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install haishui command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `haishui` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $HAISHUI_HOME/haishui-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $HAISHUI_HOME/haishui-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/haishui-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $HAISHUI_HOME/haishui-agent — then preserve it)
#
# Always no-op when the user set --dir or $HAISHUI_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in HAISHUI_HOME.
    if is_termux; then
        INSTALL_DIR="$HAISHUI_HOME/haishui-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$HAISHUI_HOME/haishui-agent/.git" ]; then
            INSTALL_DIR="$HAISHUI_HOME/haishui-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/haishui-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/haishui-agent"
        ...
        # /usr/local/bin/haishui wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/haishui"
        log_info "  Data:    $HAISHUI_HOME (unchanged)"
...
# Point a Haishui-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $HAISHUI_HOME/node/bin — which is NOT on
...
# Haishui's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Haishui-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$HAISHUI_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$HAISHUI_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$HAISHUI_HOME/node/etc/npmrc"
}

get_haishui_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/haishui" ]; then
        echo "$link_dir/haishui"
    else
        echo "haishui"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://aiqinghaiwork163.github.io/haishui-agent/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Haishui owns its own uv at $HAISHUI_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (haishui_cli/managed_uv.py) looks in the same
    # place, so install.sh and `haishui update` stay in sync.
    local _managed_uv="$HAISHUI_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $HAISHUI_HOME/bin ..."
    mkdir -p "$HAISHUI_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/haishui-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/haishui-uv-installer.$$.sh")"
    ...
    # directly into $HAISHUI_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$HAISHUI_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
check_python() {
    if [ "$DISTRO" = "termux" ]; then
        ...
        # Haishui currently declares requires-python >=3.11,<3.14.  Termux can
        ...
        log_error "Termux Python $PYTHON_FOUND_VERSION is not supported; Haishui requires Python >=3.11,<3.14"
...
# replaced with the Haishui-managed Node $NODE_VERSION.
...
check_node() {
    ...
    # Repair pre-existing Haishui-managed installs where `npm install -g` lands
    ...
    if command -v node &> /dev/null && command -v npm &> /dev/null \
        ...
        log_warn "min-release-age-exclude) — installing Haishui-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi

    # Prefer a Haishui-managed Node from a previous run over a too-old system one.
    if [ -x "$HAISHUI_HOME/node/bin/node" ] && [ -x "$HAISHUI_HOME/node/bin/npm" ] \
        && node_satisfies_build "$("$HAISHUI_HOME/node/bin/node" --version)"; then
        export PATH="$HAISHUI_HOME/node/bin:$PATH"
        log_success "Node.js $("$HAISHUI_HOME/node/bin/node" --version) found (Haishui-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        log_warn "node found but npm is not on PATH (stray node symlink?) — installing Haishui-managed Node $NODE_VERSION LTS..."
    elif command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is unsupported (Haishui requires Node 22.22+, 24.11+, or 26+) — installing Haishui-managed Node $NODE_VERSION..."
...
# Download and adopt one Node release line (e.g. 26) into ~/.haishui/node/.
#
...
install_node_line() {
    ...
    log_info "Extracting to ~/.haishui/node/..."
    ...
    # Place into ~/.haishui/node/ and symlink binaries into the same bin dir
    # the haishui command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$HAISHUI_HOME/node"
    mkdir -p "$HAISHUI_HOME"
    mv "$extracted_dir" "$HAISHUI_HOME/node"
    rm -rf "$tmp_dir"
    ...
    ln -sf "$HAISHUI_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$HAISHUI_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$HAISHUI_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    if ! installed_ver=$("$HAISHUI_HOME/node/bin/node" --version 2>&1); then
    ...
    log_success "Node.js $installed_ver installed to ~/.haishui/node/"
    HAS_NODE=true
    return 0
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Haishui install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Haishui Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `haishui update` path
                # (#4735).
                ...
                local stash_name
                stash_name="haishui-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``haishui update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if Haishui behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up haishui command..."

    if [ "$USE_VENV" = true ]; then
        HAISHUI_BIN="$INSTALL_DIR/venv/bin/python"
        HAISHUI_ENTRYPOINT="$INSTALL_DIR/haishui"
    else
        HAISHUI_BIN="$(which haishui 2>/dev/null || echo "")"
        if [ -z "$HAISHUI_BIN" ]; then
            log_warn "haishui not found on PATH after install"
            return 0
        fi
    fi

    # Verify the interpreter and the checked-in entrypoint needed by the launcher.
    if [ ! -x "$HAISHUI_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$HAISHUI_ENTRYPOINT" ]; }; then
        log_warn "Haishui launcher prerequisites not found"
    ...
    # Create a user-facing shim for the haishui command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $HAISHUI_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$HAISHUI_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/haishui"
    if [ "$USE_VENV" = true ]; then
        ...
        cat > "$command_link_dir/haishui" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$HAISHUI_BIN" "$HAISHUI_ENTRYPOINT" "\$@"
EOF
    else
        cat > "$command_link_dir/haishui" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/haishui"
    log_success "Installed haishui launcher → $command_link_display_dir/haishui"

    # Also expose `haishui-agent`. The `haishui-agent` console script declared in
    ...
    rm -f "$command_link_dir/haishui-agent"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/haishui-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$HAISHUI_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/haishui-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$HAISHUI_BIN" run_agent.py "\$@"
EOF
    fi
    chmod +x "$command_link_dir/haishui-agent"
    log_success "Installed haishui-agent launcher → $command_link_display_dir/haishui-agent"

    # Also expose `haishui-acp`. ACP hosts (Zed, JetBrains, Buzz) resolve the
    # agent by command name on the login-shell PATH, and the `haishui-acp`
    # console script lives inside the venv, which is not on that PATH. Without
    # this launcher those hosts report Haishui as not installed. (#21454 applies
    # here too: clear the path first so `cat >` cannot follow an old symlink
    # into the venv and overwrite the console script.)
    rm -f "$command_link_dir/haishui-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/haishui-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$HAISHUI_BIN" "$HAISHUI_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/haishui-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$HAISHUI_BIN" acp "\$@"
EOF
    fi
    chmod +x "$command_link_dir/haishui-acp"
    log_success "Installed haishui-acp launcher → $command_link_display_dir/haishui-acp"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "haishui command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v haishui' \
                >/dev/null 2>&1; then
        ...
        log_info "haishui not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Haishui Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Haishui Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Haishui Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so haishui works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.haishui directory structure (config at top level, code in subdir)
    mkdir -p "$HAISHUI_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.haishui/.env (top level, easy to find)
    if [ ! -f "$HAISHUI_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$HAISHUI_HOME/.env"
            log_success "Created ~/.haishui/.env from template"
        else
            touch "$HAISHUI_HOME/.env"
            log_success "Created ~/.haishui/.env"
        fi
    else
        log_info "~/.haishui/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$HAISHUI_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.haishui/config.yaml (top level, easy to find)
    if [ ! -f "$HAISHUI_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$HAISHUI_HOME/config.yaml"
            log_success "Created ~/.haishui/config.yaml from template"
        fi
    else
        log_info "~/.haishui/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in haishui_cli/default_soul.py — the
    ...
    if [ ! -f "$HAISHUI_HOME/SOUL.md" ]; then
        cat > "$HAISHUI_HOME/SOUL.md" << 'SOUL_EOF'
...
SOUL_EOF
        log_success "Created ~/.haishui/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.haishui/"

    # Seed bundled skills into ~/.haishui/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `haishui update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'haishui update'." \
            > "$HAISHUI_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $HAISHUI_HOME/.no-bundled-skills"
        log_info "  Future 'haishui update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.haishui/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.haishui/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$HAISHUI_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$HAISHUI_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.haishui/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local env_file="$HAISHUI_HOME/.env"
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Haishui Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Haishui will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Haishui Agent browser tools — explicit browser override."
...
# install — the same closure `haishui update` installs
# (haishui_cli/main.py::_update_node_dependencies). Prebuilt/partial checkouts
...
install_node_deps() {
    ...
    # Keep the checkout clean so `haishui update` doesn't autostash every run.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
install_browser_use_cli() {
    ...
    # `haishui tools` can install it later.
    if [ "$SKIP_BROWSER" = true ]; then
    ...
    if [ -x "$HAISHUI_HOME/bin/browser-use" ]; then
        log_success "Browser Use CLI already installed"
        return 0
    fi

    log_info "Installing Browser Use CLI (default browser backend)..."
    # UV_TOOL_BIN_DIR keeps the binary inside Haishui' managed bin dir, where
    # the browser tool resolves it — no reliance on the user's PATH.
    if run_with_timeout 600 env UV_NO_CONFIG=1 UV_TOOL_BIN_DIR="$HAISHUI_HOME/bin" \
        ...
        log_info "Install later with: $UV_CMD tool install browser-use  (or via 'haishui tools')"
    fi
}
...
install_computer_use_driver() {
    ...
    # `haishui tools`, the dashboard, or the desktop app — is a config flip,
    # not a surprise multi-minute binary fetch (the confusion this fixes:
    # users had to discover `haishui computer-use install` on their own).
    ...
    # Same upstream installer `haishui computer-use install` runs; time-boxed
    # so a stalled GitHub download can't hang the Haishui install. The
    # upstream installer serializes with its own lock (600s stale window),
    # so give it a ceiling above that — matching Haishui'
    ...
    if run_with_timeout 660 /bin/bash -c \
        ...
        >"$cua_log" 2>&1; then
        log_success "Computer Use driver installed (enable via 'haishui tools' → Computer Use)"
    else
        log_warn "Computer Use driver install failed — it will install on demand when you enable the tool."
        log_info "Install later with: haishui computer-use install"
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'haishui setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m haishui_cli.main setup < /dev/tty
    else
        python -m haishui_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$HAISHUI_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Haishui to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$HAISHUI_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'haishui whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                HAISHUI_CMD="$(get_haishui_command_path)"
                $HAISHUI_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'haishui whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'haishui gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $HAISHUI_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $HAISHUI_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: haishui gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: haishui gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $HAISHUI_CMD gateway > "$HAISHUI_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.haishui/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: haishui gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: haishui gateway"
    fi
}

write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.haishui-bootstrap-complete, which tells the Haishui
    ...
    local marker_path="$INSTALL_DIR/.haishui-bootstrap-complete"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $HAISHUI_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $HAISHUI_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $HAISHUI_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}haishui${NC}              Start chatting"
    echo -e "   ${GREEN}haishui setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}haishui config${NC}       View/edit configuration"
    echo -e "   ${GREEN}haishui config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}haishui gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}haishui update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'haishui' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'haishui' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'haishui' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$HAISHUI_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$HAISHUI_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$HAISHUI_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$HAISHUI_HOME/node" --silent --ignore-scripts \
...
# extract a tree MISSING the electron binary, so the `electron`->`Haishui` rename
...
# _purge_electron_build_cache() used by `haishui desktop`; install.sh was the only
...
install_desktop() {
    ...
    # supported Node lines and prepends the Haishui-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `haishui desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Haishui" ]; then
            app="$desktop_dir/release/linux-unpacked/Haishui"
        elif [ -x "$desktop_dir/release/linux-unpacked/haishui" ]; then
            app="$desktop_dir/release/linux-unpacked/haishui"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Haishui.app" \
            "$desktop_dir/release/mac/Haishui.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # `haishui desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading HAISHUI_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if HAISHUI_HOME="$HAISHUI_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
from pathlib import Path
from haishui_cli.main import _desktop_macos_relaunchable_fixup
...
PYEOF
    ...
    # checkout stays clean for the next `haishui update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Haishui-managed Node
            # provisioned during prerequisites/node-deps (at $HAISHUI_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $HAISHUI_HOME.
    # $HAISHUI_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'haishui update' on this host install.
