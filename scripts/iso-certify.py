...
What it does
------------
1. Spawns a SCRATCH dashboard (``haishui dashboard``) bound to loopback on a
   free port, with an ISOLATED ``HAISHUI_HOME`` (temp dir, minimal seeded state).
   It NEVER touches the live :9119 dashboard / ai.haishui.dashboard / live
   state.db. Loopback bind ⇒ no auth gate (web_server.should_require_auth).
2. Arms the synthetic GIL-heavy turn seam (``HAISHUI_ISO_CERTIFY_SYNTH_TURN=1``,
...
the scratch HAISHUI_HOME; ``--isolation on|off`` sets it. Run BOTH:
...
_READY_RE = re.compile(r"HAISHUI_(?:DASHBOARD|BACKEND)_READY port=(\d+)")
...
class ScratchDashboard:
    ...
    def __enter__(self) -> "ScratchDashboard":
        ...
        env["HAISHUI_HOME"] = str(self.home)
        env["HOME"] = str(self.home.parent) if str(self.home.parent) else env.get("HOME", "")
        env["HAISHUI_HOME"] = str(self.home)
        env["PYTHONPATH"] = str(REPO_ROOT) + os.pathsep + env.get("PYTHONPATH", "")
        env["HAISHUI_ISO_CERTIFY_SYNTH_TURN"] = "1"
        cmd = [
            python, "-m", "haishui_cli.main", "dashboard",
...
def run_certify(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        with ScratchDashboard(
            ...
            env_extra={"HAISHUI_DASHBOARD_SESSION_TOKEN": token},
        ) as dash:
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--isolation", choices=["on", "off"], default="on",
                   help="set dashboard.turn_isolation in the scratch HAISHUI_HOME")
    ...
    p.add_argument("--keep-home", action="store_true", help="do not delete the scratch HAISHUI_HOME on exit")
