...
# (%HAISHUI_HOME%\haishui-setup.exe). That binary has no self-update path --
# copy_self_to_haishui_home deliberately no-ops during --update -- so every
...
#
# This script lives in the repo checkout, so EVERY `haishui update` refreshes
...
#     -InstallRoot <path>   repo checkout (HAISHUI_HOME\haishui-agent)
#     -Branch <ref>         branch to update against
#     -DesktopPid <pid>     the Electron main process to wait out
#     [-RelaunchExe <path>] Haishui.exe to start when done (omit = no relaunch)
#     [-NoUi]               headless (tests); default shows a progress window
#     [-NoMarkerCleanup]    leave .haishui-update-in-progress in place (tests)
#
...
# .haishui-update-result.json for the relaunched Desktop to surface, and
# relaunches the Desktop so the user is never left stranded.
#
# Marker: we claim HAISHUI_HOME\.haishui-update-in-progress with OUR pid as
# step 0 (the wrapper cmd.exe pid the Desktop saw is useless -- it exits
# immediately), retaining HAISHUI_UPDATE_STARTED_AT from the Desktop hand-off.
# haishui_cli/update_lock.py's ancestry rule lets our
# `haishui update` child adopt the claim; electron/update-marker.ts parks a
...
# pass our foreground right on to the new Haishui.exe pid.
try {
    Add-Type -Namespace HaishuiHandoff -Name Win32 -MemberDefinition @'
...
$HaishuiHome = if ($InstallRoot) { Split-Path -Parent $InstallRoot } else { $TempDir }
$MarkerPath = Join-Path $HaishuiHome ".haishui-update-in-progress"
$LogDir = Join-Path $HaishuiHome "logs"
$LogPath = Join-Path $LogDir "desktop-update-handoff.log"
$ResultPath = Join-Path $HaishuiHome ".haishui-update-result.json"
$script:Ui = $null
$script:UiStage = "Haishui will open once done."   # until the first gate; matches ui.html
...
function Stop-UiServer([switch]$LeaveWindow) {
    ...
    # profile plus any stale haishui-update-ui-* leftovers from interrupted
    ...
    # update marker (.haishui-update-in-progress) serialises hand-offs, so no
    # other run's profile can be in active use here.
    try {
        $profileDirs = @()
        if ($script:UiServer.Profile) { $profileDirs += $script:UiServer.Profile }
        Get-ChildItem -LiteralPath $TempDir -Directory -Filter "haishui-update-ui-*" -ErrorAction SilentlyContinue |
...
function Publish-UiProgress([string]$Message) {
    ...
    # stderr remain asynchronously drained in Invoke-HaishuiStep and are never
...
function Show-ProgressWindow {
    ...
    if ($htmlPath -and $browser) {
        ...
        if ($server) {
            try {
                ...
                $browserProfile = Join-Path $TempDir ("haishui-update-ui-{0}" -f $PID)
                $browserArgs = @(
    ...
    try {
        ...
        try {
            $form.Activate()
            if ($script:Win32) { [HaishuiHandoff.Win32]::SetForegroundWindow($form.Handle) | Out-Null }
        } catch {}
        [System.Windows.Forms.Application]::DoEvents()
...
function Show-ErrorFinale([string]$Message) {
    ...
    # log tail -- `haishui debug share` uploads the real evidence and the
    ...
    try {
        ...
        $ui.Sub.Text = "Run `"haishui debug share`" in a terminal to send a report."
        ...
        try {
            $ui.Form.Activate()
            if ($script:Win32) { [HaishuiHandoff.Win32]::SetForegroundWindow($ui.Form.Handle) | Out-Null }
        } catch {}
...
function Show-ManualFinale([string]$Message) {
    ...
    # they reopen Haishui themselves.
    if ($script:UiServer) {
...
function Start-DesktopRelaunch {
    ...
    # update it can remove the old Haishui.exe before writing the replacement,
    ...
    # DO NOT spawn Haishui.exe as our child: Electron/Chromium calls
    ...
    try {
        ...
        if ($r -and $r.ReturnValue -eq 0) {
            ...
            try {
                if ($script:Win32) {
                    [HaishuiHandoff.Win32]::AllowSetForegroundWindow([int]$r.ProcessId) | Out-Null
                    ...
                    while ((Get-Date) -lt $deadline) {
                        ...
                        if ($hwnd -ne [System.IntPtr]::Zero) {
                            [HaishuiHandoff.Win32]::ShowWindow($hwnd, 9) | Out-Null  # SW_RESTORE
                            [HaishuiHandoff.Win32]::SetForegroundWindow($hwnd) | Out-Null
    ...
    if (-not $spawned) {
        ...
        # verify by watching for a fresh Haishui process.
        try {
            ...
            while ((Get-Date) -lt $explorerDeadline) {
                ...
                if ($fresh.Count -gt 0) {
                    ...
                    try {
                        if ($script:Win32) {
                            [HaishuiHandoff.Win32]::AllowSetForegroundWindow([int]$fresh[0].Id) | Out-Null
...
# closes it. `haishui update` deliberately runs its build steps with stdout
# inherited (haishui_cli/main.py, the tee-stderr runner), so the tree under a
...
if ($env:HAISHUI_UPDATE_PIPE_DRAIN_SECONDS) {
    $parsedGrace = 0
    if ([int]::TryParse($env:HAISHUI_UPDATE_PIPE_DRAIN_SECONDS, [ref]$parsedGrace) -and $parsedGrace -ge 0) {
...
if ($env:HAISHUI_UPDATE_STEP_IDLE_SECONDS) {
    $parsedIdle = 0
    if ([int]::TryParse($env:HAISHUI_UPDATE_STEP_IDLE_SECONDS, [ref]$parsedIdle) -and $parsedIdle -gt 0) {
        $script:StepIdleTimeoutSeconds = $parsedIdle
    }
}

# Silence on the pipes is NOT silence in the update. `haishui update` captures
# the (very loud) Electron/vite build into logs/update.log instead of its own
# stdout (haishui_cli/update_cmd.py, the update-log tee), so a real update is
...
if ($env:HAISHUI_UPDATE_PROGRESS_LOG) {
    $script:StepProgressLogPath = $env:HAISHUI_UPDATE_PROGRESS_LOG
}
...
if (-not ("HaishuiUpdateJob" -as [type])) {
...
public static class HaishuiUpdateJob {
...
function Invoke-HaishuiStep([string]$Exe, [string[]]$HaishuiArgs, [string]$Tag) {
    ...
    # afterwards, where `haishui debug share` picks it up.
    #
    ...
    # .haishui-update-result.json, clearing .haishui-update-in-progress,
    ...
    # Haishui" until the user killed something by hand. Losing the tail of a
    ...
    $arguments = ($HaishuiArgs | ForEach-Object { '"{0}"' -f ($_ -replace '"', '\"') }) -join ' '
    ...
    try {
        ...
        $started = [HaishuiUpdateJob]::StartAssigned($Exe, $arguments)
    } finally {
    ...
    while ($true) {
        ...
        if ($proc.HasExited) {
            ...
            # Quiet pipes are how a healthy `haishui update` looks for 40+
            ...
            if ($currentLogStamp -ne $progressLogStamp) {
                ...
                $stalled = [HaishuiUpdateJob]::TerminateAndWait($job, 124, 10000)
                if (-not $stalled) {
...
function Resolve-HaishuiVenvDir([string]$Root) {
    # Match haishui_constants.project_venv_dir(): installer-created venv wins,
...
# the page can be QA'd in any browser; HAISHUI_SELFTEST_FAIL=1 exercises the
# error state, HAISHUI_SELFTEST_HOLD_SECONDS delays the terminal event.
if ($SelfTestUi) {
    ...
    $hold = 6
    if ($env:HAISHUI_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:HAISHUI_SELFTEST_HOLD_SECONDS }
    Publish-UiProgress "Testing quiet update"
    Start-Sleep -Seconds $hold
    if ($env:HAISHUI_SELFTEST_FAIL) {
...
# -SelfTestPipeDrain: prove Invoke-HaishuiStep survives a leaked pipe ------
# The #90455 deadlock needs no update, no checkout and no Haishui install to
...
#            update log, the shape of every real `haishui update` build (output
...
if ($SelfTestPipeDrain) {
    ...
    if ($env:HAISHUI_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:HAISHUI_SELFTEST_HOLD_SECONDS }
    $floodKb = 8192
    if ($env:HAISHUI_SELFTEST_FLOOD_KB) { $floodKb = [int]$env:HAISHUI_SELFTEST_FLOOD_KB }
    # $PSHOME is this interpreter's own directory -- no hardcoded system path.
    $powershell = Join-Path $PSHOME "powershell.exe"
    $stamp = [Guid]::NewGuid().ToString("N")
    $childPs1 = Join-Path $TempDir "haishui-pipe-drain-$stamp.ps1"
    $floodPs1 = Join-Path $TempDir "haishui-pipe-flood-$stamp.ps1"
    $pidFile = Join-Path $TempDir "haishui-pipe-drain-$stamp.pid"
    $stallPs1 = Join-Path $TempDir "haishui-step-stall-$stamp.ps1"
    $stallPidFile = Join-Path $TempDir "haishui-step-stall-$stamp.pid"
    $stallGrandchildPidFile = Join-Path $TempDir "haishui-step-stall-grandchild-$stamp.pid"
    $logStallPs1 = Join-Path $TempDir "haishui-step-logstall-$stamp.ps1"
    $logStallProgress = Join-Path $TempDir "haishui-step-logstall-$stamp.update.log"
...
'@
    ...
    # merely loud. `haishui update` is this shape -- the Electron/vite build
...
'@
    ...
    $res = Invoke-HaishuiStep $powershell @(
    ...
    $flood = Invoke-HaishuiStep $powershell @(
    ...
    $stall = Invoke-HaishuiStep $powershell @(
    ...
    try {
        $logstall = Invoke-HaishuiStep $powershell @(
    ...
    if ($stallAlive) { $problems += "stalled child pid $stallPid remained alive after Invoke-HaishuiStep returned" }
    if ($stallGrandchildAlive) { $problems += "stalled descendant pid $stallGrandchildPid remained alive after Invoke-HaishuiStep returned" }
...
try {
    ...
    try {
        ...
        $startedAt = 0L
        $hasStartedAt = [int64]::TryParse($env:HAISHUI_UPDATE_STARTED_AT, [ref]$startedAt)
    ...
    # Desktop launches us from HAISHUI_HOME; pin the process directory to the
    ...
    $VenvDir = Resolve-HaishuiVenvDir $InstallRoot
    ...
    if ($SelfTestWorkingDirectory) {
        ...
        $probe = Invoke-HaishuiStep $probeExe @("-NoProfile", "-Command", "[Environment]::CurrentDirectory") "cwd"
    ...
    Publish-UiProgress "Waiting for Haishui to close"
    if ($DesktopPid -gt 0) {
        ...
        if (Get-Process -Id $DesktopPid -ErrorAction SilentlyContinue) {
            ...
            $finalCode = 4
            $finalMsg = "Update aborted: the Haishui window (pid $DesktopPid) did not exit within 30s. Nothing was changed. Close Haishui fully and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    Publish-UiProgress "Preparing Haishui files"
    $shim = Join-Path $VenvDir "Scripts\haishui.exe"
    if (Test-Path -LiteralPath $shim) {
        ...
        if (-not $unlocked) {
            ...
            $finalCode = 5
            $finalMsg = "Update aborted: another process is still holding the Haishui install open ($shim locked after 20s). Nothing was changed. Close other Haishui windows/terminals and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    # --force skips only the haishui.exe shim guard, which step 2 just PROVED
    ...
    #
    # DRIVE THE UPDATE THROUGH venv\Scripts\python.exe, NOT venv\Scripts\haishui.exe.
    # `uv pip install -e .` has to replace the console-script shims, so
    # _quarantine_running_haishui_exe must first rename the running haishui.exe
    # out of the way. On Windows that rename fails whenever ANY child process
    # spawned from that haishui.exe is still alive: a child inherits a handle on
    # the parent image, and the resulting sharing violation is indistinguishable
    # from a user leaving a second Haishui window open. It is the inherited
    ...
    # install whose Start Menu shortcut points at a Haishui.exe that no longer
    ...
    #
    # Running the same code as `python.exe -m haishui_cli.main update` puts the
    ...
    if (-not (Test-Path -LiteralPath $pythonExe)) {
        $finalCode = 3
        $finalMsg = "Update aborted: $pythonExe is missing. The install needs repair (run the Haishui installer or `haishui doctor`)."
        Write-HandoffLog $finalMsg
        exit $finalCode
    }
    $updateArgs = @("-m", "haishui_cli.main", "update", "--yes", "--gateway", "--force", "--branch", $Branch)
    ...
    # collides with the "close all Haishui windows" sentinel.
    try {
        $updateHelp = & $pythonExe -m haishui_cli.main update --help 2>$null | Out-String
        if ($updateHelp -match "--keep-stash") {
            $updateArgs += "--keep-stash"
        } else {
            Write-HandoffLog "installed haishui predates --keep-stash; running without it"
        }
    } catch {
    ...
    $res = Invoke-HaishuiStep $pythonExe $updateArgs "update"
    Write-HandoffLog "haishui update exit code: $($res.Code)"

    $retryPolicyPath = Join-Path $PSScriptRoot "retry-policy.ps1"
    if (Test-Path -LiteralPath $retryPolicyPath) {
        . $retryPolicyPath
        $shouldRetry = Test-HaishuiUpdateShouldRetry -ExitCode $res.Code -InstallRoot $InstallRoot
    } else {
    ...
    # `haishui update` treats a Desktop GUI build failure as NON-fatal (prints
    ...
    if ($res.Code -eq 0 -and $res.Output -match "Desktop build failed") {
        Write-HandoffLog "haishui update reported a desktop build failure (non-fatal there, fatal here); retrying build"
        Publish-UiProgress "Rebuilding Desktop"
        $rebuild = Invoke-HaishuiStep $pythonExe @("-m", "haishui_cli.main", "desktop", "--force-build", "--build-only") "rebuild"
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        $verifyCode = "import haishui_cli.main; from haishui_cli.desktop_update_verify import verify_windows_desktop_update; verify_windows_desktop_update()"
        $verify = Invoke-HaishuiStep $pythonExe @("-c", $verifyCode) "verify"
        if ($verify.Code -ne 0) {
            $finalCode = 8
            $finalMsg = "The updated Haishui runtime or Desktop build failed verification. Repair the installation and review antivirus quarantine before retrying."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    }
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        ...
        $finalCode = 6
        $finalMsg = "Code and dependencies updated, but the Desktop app REBUILD FAILED - you are running the previous build. Run `haishui desktop --force-build` from a terminal to retry."
    } else {
        $finalCode = $res.Code
        $finalMsg = "Update failed (exit $($res.Code)). Run `haishui debug share` in a terminal to send a report."
    }
    exit $finalCode
} finally {
    ...
    #   3. only then the terminal UI state — done means "Haishui is back",
    ...
    if (-not $script:TreeSafeToFinalize) {
        ...
        $finalCode = 7
        $finalMsg = "Update recovery could not stop every updater process. Haishui was not restarted to avoid overlapping the active install. Wait for it to finish or restart Windows, then reopen Haishui."
        ...
        if ($finalCode -ne 0) {
            ...
            Publish-UiProgress "Opening Haishui"
            ...
            if (-not $cameBack -and $RelaunchExe) {
                ...
                $finalMsg = "Update complete. Reopen Haishui to finish (it could not restart itself)."
