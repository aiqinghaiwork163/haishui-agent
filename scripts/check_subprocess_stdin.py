...
When Haishui runs in TUI mode, the gateway child process communicates with
...
# ``get_haishui_home() / "plugins"`` (user) and ``./.haishui/plugins/`` (project,
# gated behind ``HAISHUI_ENABLE_PROJECT_PLUGINS``) — see
# ``haishui_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``haishui_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import haishui_constants (this script
    ...
    # plugins in get_haishui_home()/plugins/ and project plugins in
    # ./.haishui/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_haishui_home() / "plugins"]
    if os.environ.get("HAISHUI_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".haishui" / "plugins")
