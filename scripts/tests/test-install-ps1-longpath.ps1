...
function Invoke-Normalization {
    ...
    try {
        ...
        # its diagnostics are all `[haishui] `-prefixed, so the two separate
        ...
        $stderr = ($raw | Where-Object { $_ -like '`[haishui`]*' }) -join "`n"
        $stdout = ($raw | Where-Object { $_ -notlike '`[haishui`]*' }) -join "`n"
    } finally {
    ...
    return @{
        ...
        HaishuiHome = $(if ($paths) { $paths.haishui_home } else { $null })
...
$expectedInstallDir = "$($longRoot)${sep}AppData${sep}Local" + '\haishui\haishui-agent'
...
$result = Invoke-Normalization -Environment @{ TEMP = $shortTemp; TMP = $shortTemp } `
    -ExtraArgs @('-InstallDir', (Join-Path $shortProfile 'custom-haishui'))
Assert-Equal -Expected (Join-Path $longRoot 'custom-haishui') -Actual $result.InstallDir -Label "explicit -InstallDir keeps the caller's directory, on the long root"
