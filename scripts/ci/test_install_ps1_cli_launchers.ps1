# Behavioral test for install.ps1's haishui launcher staging (PR #92092,
...
#
# The test lifts the real Install-HaishuiCommandLaunchers function from the
...
# the caller (Set-PathVariable passes $HaishuiHome\bin -- the managed binary
...
$fn = $ast.Find({
    ...
    $n.Name -eq 'Install-HaishuiCommandLaunchers'
}, $true)

if (-not $fn) {
    throw "Install-HaishuiCommandLaunchers not found in $installPs1"
}
...
$caseRoot = [System.IO.Path]::GetFullPath((Join-Path $tempBase (
    'haishui-cli-launcher-test-' + [guid]::NewGuid().ToString('N')
)))
...
try {
    $installRoot = Join-Path $caseRoot 'haishui-agent'
    ...
    $missingThrew = $false
    try {
        Install-HaishuiCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    } catch {
        $missingThrew = $_.Exception.Message -like '*required launcher not found*'
    }
    Assert-True $missingThrew 'missing haishui.exe fails the launcher stage'
    ...
    $haishuiV1 = [byte[]](77, 90, 1)
    $haishuiV2 = [byte[]](77, 90, 2)
    $acp = [byte[]](77, 90, 3)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'haishui.exe'), $haishuiV1)
    Set-Content -Path (Join-Path $installRoot 'venv\pyvenv.cfg') `
        -Value "home = X" -Encoding Ascii

    $staged = Install-HaishuiCommandLaunchers -Root $installRoot -Destination $binDir
    Assert-True ($staged -eq $binDir) 'returns the destination it staged into'
    Assert-BytesEqual $haishuiV1 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'haishui.exe'))) `
        'normal venv: exe copy lands in the destination'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'haishui-acp.exe'))) `
        'optional ACP launcher may be absent'

    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'haishui.exe'), $haishuiV2)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'haishui-acp.exe'), $acp)
    Install-HaishuiCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-BytesEqual $haishuiV2 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'haishui.exe'))) `
        'installer refreshes an existing Haishui launcher'
    Assert-BytesEqual $acp `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'haishui-acp.exe'))) `
    ...
    Install-HaishuiCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-True (Test-Path -LiteralPath (Join-Path $binDir 'haishui.cmd')) `
        'relocatable venv: .cmd delegator staged'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'haishui.exe'))) `
        'relocatable venv: stale exe copy removed'
    $cmdBody = [System.IO.File]::ReadAllText((Join-Path $binDir 'haishui.cmd'))
    Assert-True ($cmdBody.Contains((Join-Path $scriptsDir 'haishui.exe')) -and $cmdBody.Contains('%*')) `
