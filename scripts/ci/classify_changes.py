...
# tree generates llms.txt — the index every LLM (Haishui included, via the
# haishui-agent skill) reads to learn what Haishui can do — and
...
_PY_RELEVANT_CONTRACT_FILES = {
    ...
    # tests/haishui_cli/test_desktop_slash_registry.py
...
_MCP_CATALOG_FILES = {"haishui_cli/mcp_catalog.py"}
...
# Rust crates — currently just the Tauri bootstrap installer (Haishui-Setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``haishui serve`` backend, the Docker image)
