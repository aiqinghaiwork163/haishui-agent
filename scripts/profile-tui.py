#!/usr/bin/env python3
"""Drive the Haishui TUI under HAISHUI_DEV_PERF and summarize the pipeline.
...
~30 Hz (matching xterm key-repeat), summarizes ~/.haishui/perf.log on exit.

The --tui build must exist (run `npm run build` in ui-tui first). This script
launches `node dist/entry.js` directly with HAISHUI_TUI_RESUME set so it
bypasses the haishui_cli wrapper — we want repeatable timing, not the CLI's
session-picker flow.

Environment overrides:
  HAISHUI_PERF_LOG     (default ~/.haishui/perf.log)
  HAISHUI_PERF_NODE    (default node from $PATH)
  HAISHUI_TUI_DIR      (default: <repo>/ui-tui relative to this script)

Exit code is 0 if the harness ran and parsed results, 2 if the TUI crashed
or produced no perf data (suggests HAISHUI_DEV_PERF wiring is broken).
"""
...
try:
    ...
    def get_haishui_home() -> Path:  # type: ignore[misc]
        val = (os.environ.get("HAISHUI_HOME") or "").strip()
        return Path(val) if val else Path.home() / ".haishui"

DEFAULT_TUI_DIR = Path(
    os.environ.get("HAISHUI_TUI_DIR")
    or str(Path(__file__).resolve().parent.parent / "ui-tui")
)
DEFAULT_LOG = Path(os.environ.get("HAISHUI_PERF_LOG", str(get_haishui_home() / "perf.log")))
DEFAULT_STATE_DB = get_haishui_home() / "state.db"
...
def format_report(data: dict[str, Any]) -> str:
    ...
    if not react:
        out.append("  (no react events — HAISHUI_DEV_PERF wired? threshold too high?)")
    else:
...
def run_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    env = os.environ.copy()
    env["HAISHUI_DEV_PERF"] = "1"
    env["HAISHUI_DEV_PERF_MS"] = str(args.threshold_ms)
    env["HAISHUI_DEV_PERF_LOG"] = str(log)
    env["HAISHUI_TUI_RESUME"] = sid
    ...
    node = os.environ.get("HAISHUI_PERF_NODE", "node")
...
def main() -> int:
    ...
    p.add_argument("--threshold-ms", type=float, default=0.0, help="HAISHUI_DEV_PERF_MS (0 = capture all)")
...
def loop_mode(args: argparse.Namespace) -> int:
    ...
    pkg_root = tui_dir / "packages" / "haishui-ink" / "src"
