...
Run after changing any ``desktop=`` value or alias in ``haishui_cli/commands.py``;
``tests/haishui_cli/test_desktop_slash_registry.py`` fails until the committed
...
def render() -> str:
    ...
    from haishui_cli.commands import desktop_surface_registry
