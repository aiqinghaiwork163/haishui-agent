"""Run a real Haishui CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_haishui_executable(haishui_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "haishui",
        Path(".venv") / "Scripts" / "haishui.exe",
    ):
        candidate = haishui_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("haishui")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "Haishui executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Haishui source checkout containing .venv/bin/haishui",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated HAISHUI_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["haishui.client.active"] != [
        {
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['haishui.client.active']}"
        )
    [model] = by_name["haishui.model_route.count"]
    expected_model = {
        "name": "haishui.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['haishui.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["haishui.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['haishui.task_run.started']}"
        )
    [terminal] = by_name["haishui.task_run.finished"]
    ...
    [tool] = by_name["haishui.tool_call.count"]
    ...
    lifecycle = by_name["haishui.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["haishui.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The Haishui development environment requires jsonschema"
        ) from exc
    ...
    if metrics["haishui.client.active"] != [
        {
            "name": "haishui.client.active",
            "type": "counter",
            "dimensions": {},
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['haishui.client.active']}"
        )
    [model] = metrics["haishui.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['haishui.model_route.count']}"
        )
    [terminal] = metrics["haishui.task_run.finished"]
    ...
    [tool] = metrics["haishui.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["haishui.skill.lifecycle.count"]
    ...
    loads = metrics["haishui.skill.load.count"]
    if {
        (
...
def main() -> int:
    args = _arguments()
    haishui_repo = args.haishui_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    haishui = _resolve_haishui_executable(haishui_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="haishui-relay-shared-metrics-"))
    home = root / "haishui-home"
    ...
    try:
        ...
        env = os.environ.copy()
        env["HAISHUI_HOME"] = str(home)
        python_paths = [str(haishui_repo)]
    ...
    (root / "haishui.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "haishui.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"Haishui exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("Haishui model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("Haishui did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("Haishui did not print the mock model response")
    ...
    package_paths, packages = _validate_packages(
        ...
        / "observability"
        / "schemas"
        / "haishui.shared_metrics.v2.schema.json",
    )

    print("Haishui -> NeMo Relay shared-metrics smoke test passed")
