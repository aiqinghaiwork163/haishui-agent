...
timeout. Config is env-only (``HAISHUI_STARTUP_WATCHDOG=0``,
``HAISHUI_STARTUP_WATCHDOG_TIMEOUT_S``) because config.yaml parsing is itself in scope.
...
ENV_STARTUP_WATCHDOG = "HAISHUI_STARTUP_WATCHDOG"
ENV_STARTUP_WATCHDOG_TIMEOUT_S = "HAISHUI_STARTUP_WATCHDOG_TIMEOUT_S"
...
# Module singleton: the arm sites (haishui_cli.main / haishui_cli.gateway /
...
def _process_haishui_home() -> Path:
    """HAISHUI_HOME for diagnostic files — stdlib-only replica of the haishui_constants default."""
...
def get_startup_watchdog_dump_path(home: Optional[Path] = None) -> Path:
    """Return ``<HAISHUI_HOME>/logs/gateway-startup-watchdog.log``."""
    base = home if home is not None else _process_haishui_home()
    return base.joinpath(*_DUMP_RELATIVE)


def startup_watchdog_disabled() -> bool:
    """True when ``HAISHUI_STARTUP_WATCHDOG`` opts out explicitly."""
...
def arm_startup_watchdog(
    ...
    *,
    ...
    ``HAISHUI_STARTUP_WATCHDOG=0`` or when the thread could not be started.
    """
    global _handle
    try:
