# nix/web.nix — Haishui Web Dashboard (Vite/React) frontend build
{ haishuiNpmLib, ... }:
haishuiNpmLib.buildNpmPackage {
  dirs = [
    "web"

    # @haishui/shared ships as a file: workspace dep of web, so its source
  ...
  buildPhase = ''
    ...
    # outDir in vite.config.ts points to ../haishui_cli/web_dist for the
