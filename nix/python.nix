...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # Haishui derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            haishui-agent = prev.haishui-agent.overrideAttrs (_old: {
              HAISHUI_NIX_BUILD = "1";
            });
          })
        ]
  ...
  # $HAISHUI_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$HAISHUI_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        haishui-agent = prev.haishui-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "haishui-agent-env" {
    haishui-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "haishui-agent-editable-env" {
    haishui-agent = dependency-groups;
  };
}
