# nix/packages.nix — Haishui Agent package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    {
    ...
    let
      ...
      minimal = pkgs.callPackage ./haishui-agent.nix {
    ...
    in
    {
      packages = {
        ...
        tui = full.haishuiTui;
        web = full.haishuiWeb;
        desktop = full.haishuiDesktop;

        update-npm-lockfile = full.haishuiNpmLib.updateNpmLockfile;
      };
    };
}
