...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      haishui-agent = self'.packages.default;
      haishuiVenv = haishui-agent.haishuiVenv;
      ...
      evalHomeModule =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              home = {
                username = "haishui-check";
                homeDirectory = "/home/haishui-check";
                stateVersion = "24.11";
              };
            }
            { services.haishui-agent = settings; }
      ...
      evalHomeSplit =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              programs.haishui-agent = programs;
              services.haishui-agent = services;
            }
          ];
        };

      # The option names that each module defines under
      # services.haishui-agent. The internal names that the module system adds
      # are not in the list.
      moduleOptionNames =
        eval: lib.attrNames (lib.filterAttrs (n: _: !lib.hasPrefix "_" n) eval.options.services.haishui-agent);
      ...
      configKeys = pkgs.runCommand "haishui-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${haishuiVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "haishui-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "haishui-build-package" { } ''
          echo "PASS: package built at ${haishui-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "haishui-build-devshell" { } ''
          ...
          let
            enabled = evalHomeSplit {
              ...
              services = {
                ...
                environment.HAISHUI_TEST = "1";
                environmentFiles = [ "/run/secrets/haishui-env" ];
                haishuiHomeFiles."SOUL.md" = "test soul";
            ...
            processes =
              if pkgs.stdenv.hostPlatform.isDarwin then
                ...
                }) (lib.filterAttrs (n: _: lib.hasPrefix "haishui" n) cfg.launchd.agents)
              else
                lib.mapAttrs (_: unit: {
                  argv = [ unit.Service.ExecStart ];
                  env = unit.Service.Environment;
                }) (lib.filterAttrs (n: _: lib.hasPrefix "haishui" n) cfg.systemd.user.services);
            ...
            activation = cfg.home.activation.haishuiAgentSetup.data;

            failures =
              lib.optional (names != [
                "haishui-agent"
                "haishui-backend"
              ]) "expected haishui-agent + haishui-backend processes, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/haishui gateway" (argvOf "haishui-agent")
              ) "gateway process does not run `haishui gateway`: ${argvOf "haishui-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/haishui serve" (argvOf "haishui-backend")
              ) "backend process does not run `haishui serve`: ${argvOf "haishui-backend"}"
              ++ lib.optional (
                !lib.hasInfix "--no-open" (argvOf "haishui-backend")
              ) "backend must pass --no-open so a service never opens a browser"
              ++ lib.optional (
                lib.any (n: !lib.hasInfix "/home/haishui-check/.haishui" (envOf n)) names
              ) "gateway and backend must share one HAISHUI_HOME"
              ++ lib.optional (
                cfg.home.sessionVariables.HAISHUI_HOME or null != "/home/haishui-check/.haishui"
              ) "programs.haishui-agent.enable must export HAISHUI_HOME for interactive shells"
              ++ lib.optional (
                !lib.hasInfix "haishui-config-merge" activation
              ) "activation must deep-merge config.yaml, not overwrite it"
              ++ lib.optional (
                !lib.hasInfix "/home/haishui-check/.haishui/SOUL.md" activation
              ) "haishuiHomeFiles must install into HAISHUI_HOME"
              ++ lib.optional (
                !lib.hasInfix "/home/test-user/workspace/AGENTS.md" activation
              ) "documents must install into workingDirectory"
              # The CLI reads HAISHUI_MANAGED to name the rebuild command when
              # it refuses to write the configuration. A Home Manager install
              # has no nixos-rebuild command. Thus it must not report NixOS.
              ++ lib.optional (
                !lib.any (n: lib.hasInfix "HAISHUI_MANAGED=home-manager" (envOf n)) names
              ) "processes must report HAISHUI_MANAGED=home-manager"
              ++ lib.optional (
                !lib.hasInfix "haishui-managed" activation
              ) "activation must write a .managed marker naming the managing system";
          in
          pkgs.runCommand "haishui-home-manager-module" { } (
            if failures != [ ] then
        ...
        workspace-files-need-a-directory =
          let
            ...
            # the priority.
            sameAsDefault = "/home/haishui-check";
            ...
            cases = [
              ...
              {
                name = "haishuiHomeFiles needs no directory";
                ok = accepts { haishuiHomeFiles."SOUL.md" = "x"; };
              }
              {
          ...
          in
          pkgs.runCommand "haishui-workspace-files-need-a-directory" { } (
            if failed != [ ] then
        ...
        # ── The desktop application shares one HAISHUI_HOME ───────────────
        # `programs.enable` exports HAISHUI_HOME with home.sessionVariables,
        ...
        # no shell profile. Thus the desktop application would open ~/.haishui
        # while the services use the HAISHUI_HOME of the module, and the user
        ...
        home-manager-desktop =
          let
            tokenFile = "/run/secrets/haishui-desktop-token";
            ...
            enabled = evalHomeSplit {
              ...
              services = {
                enable = true;
                haishuiHome = "/home/haishui-check/.haishui-work";
            ...
            desktopPackages = builtins.filter (p: (p.pname or "") == "haishui-desktop") cfg.home.packages;
            ...
            # second Haishui runtime beside the services is the fault that
            # `programs.enable` plus a plain desktop package would give.
            agentPackages = builtins.filter (p: (p.pname or "") == "haishui-agent") cfg.home.packages;
            ...
            backendScript =
              let
                argv =
                  if pkgs.stdenv.hostPlatform.isDarwin then
                    cfg.launchd.agents.haishui-backend.config.ProgramArguments
                  else
                    [ cfg.systemd.user.services.haishui-backend.Service.ExecStart ];
            ...
            failures =
              ...
              ) "programs.desktop.enable must install exactly one haishui-desktop package, got ${toString (lib.length desktopPackages)}"
              ++ lib.optional (
                setValue "HAISHUI_HOME" != "/home/haishui-check/.haishui-work"
              ) "the launcher must carry HAISHUI_HOME: a GUI launcher reads no shell profile, so home.sessionVariables never reaches it (got: ${toString (setValue "HAISHUI_HOME")})"
              ++ lib.optional (
                setValue "HAISHUI_MANAGED" != "home-manager"
              ) "the launcher must report HAISHUI_MANAGED=home-manager while the services own the configuration (got: ${toString (setValue "HAISHUI_MANAGED")})"
              ++ lib.optional (
                lib.length agentPackages == 1
                && setValue "HAISHUI_DESKTOP_HAISHUI" != "${lib.head agentPackages}/bin/haishui"
              ) "the launcher must pin the agent package that programs.enable installs, and not a second runtime: ${toString (setValue "HAISHUI_DESKTOP_HAISHUI")}"

              # ── The application reaches the backend of the service ──────
              ++ lib.optional (
                setValue "HAISHUI_DESKTOP_REMOTE_URL" != "http://127.0.0.1:9231"
              ) "the launcher must name the backend of the service, or the application starts a second one (got: ${toString (setValue "HAISHUI_DESKTOP_REMOTE_URL")})"
              ++ lib.optional (
                !lib.hasInfix "HAISHUI_DESKTOP_REMOTE_TOKEN" wrapper
              ) "the launcher must give a token with the URL: the desktop resolver throws when the URL is set alone"
              ++ lib.optional (
                !lib.hasInfix "HAISHUI_DASHBOARD_SESSION_TOKEN" backendScript
              ...
              ++ lib.optional (
                setValue "HAISHUI_DESKTOP_REMOTE_TOKEN" != null
              ) "the token must never be a --set value: makeWrapper writes it into the world-readable Nix store";
          in
          pkgs.runCommand "haishui-home-manager-desktop" { } (
            if failures != [ ] then
              throw "Home Manager desktop check failed:\n${lib.concatMapStringsSep "\n" (f: "  - ${f}") failures}"
            else
              ''
                echo "PASS: the desktop launcher shares HAISHUI_HOME, the runtime and the backend of the service"
                mkdir -p $out
                echo "ok" > $out/result
              ''
          );
        ...
        home-manager-desktop-standalone =
          let
            ...
            failures =
              ...
              ++ lib.optional (
                !lib.hasInfix "--set HAISHUI_HOME" wrapper
              ) "the launcher must carry HAISHUI_HOME even with no services"
              ++ lib.optional (
                lib.hasInfix "HAISHUI_MANAGED" wrapper
              ) "the launcher must not claim a managed install when no activation writes one"
              ++ lib.optional (
                lib.hasInfix "HAISHUI_DESKTOP_REMOTE_URL" wrapper
              ) "the launcher must not name a backend when the services run none"
              ++ lib.optional (
                cfg.systemd.user.services ? haishui-backend || cfg.launchd.agents ? haishui-backend
              ) "programs.enable alone must start no service";
          in
          pkgs.runCommand "haishui-home-manager-desktop-standalone" { } (
            if failures != [ ] then
        ...
        # command line. A silent removal thus leaves them with no `haishui`
        ...
        home-manager-install-package-removed =
          let
            ...
            cases = [
              {
                value = true;
                expect = "programs.haishui-agent.enable = true;";
              }
              {
                value = false;
                expect = "programs.haishui-agent.enable = false;";
              }
            ];
          ...
          in
          pkgs.runCommand "haishui-home-manager-install-package-removed" { } (
            if failures != [ ] then
        ...
        module-option-parity =
          ...
          in
          pkgs.runCommand "haishui-module-option-parity" { } (
            if failures != [ ] then
        ...
        nixos-module =
          let
            ...
            units = lib.filterAttrs (n: _: lib.hasPrefix "haishui" n) cfg.systemd.services;
            names = lib.attrNames units;
            execOf = name: units.${name}.serviceConfig.ExecStart;
            activation = cfg.system.activationScripts."haishui-agent-setup".text;

            failures =
              lib.optional (names != [
                "haishui-agent"
                "haishui-backend"
              ]) "expected haishui-agent + haishui-backend units, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/haishui gateway" (execOf "haishui-agent")
              ) "gateway unit does not run `haishui gateway`: ${execOf "haishui-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/haishui dashboard" (execOf "haishui-backend")
              ) "backend unit does not run `haishui dashboard`: ${execOf "haishui-backend"}"
              ++ lib.optional (
                units.haishui-agent.environment.HAISHUI_HOME != units.haishui-backend.environment.HAISHUI_HOME
              ) "gateway and backend must share one HAISHUI_HOME"
              ++ lib.optional (
                !lib.hasInfix "/var/lib/haishui/.haishui/SOUL.md" activation
              ) "haishuiHomeFiles must install into HAISHUI_HOME";
          ...
          in
          pkgs.runCommand "haishui-nixos-module" { } (
            if failures != [ ] then
        ...
        # polls and then execs haishui, and the assertions reject a
        # configuration that cannot work.
        backend-bind-wait =
          let
            execOf =
              settings:
              (evalNixosModule ({ enable = true; } // settings)).config.systemd.services.haishui-backend.serviceConfig.ExecStart;
            ...
            failures =
              ...
              lib.optional (!lib.hasInfix "bin/haishui serve --host 127.0.0.1" direct)
                "without waitFor the backend must exec haishui directly, got: ${direct}"
              ++ lib.optional (lib.hasInfix "haishui-backend-launch" direct)
                "without waitFor the backend must not use the launcher"

              # The hostname mode polls the resolver, then binds the name.
              ++ lib.optional (!lib.hasInfix "haishui-backend-launch" hostnameWait)
              ...
              ++ lib.optional (!lib.hasInfix "exec " hostnameScript)
                "the launcher must exec haishui, so that it keeps the MainPID"
              ...
              ++ lib.optional (!lib.hasInfix "bin/haishui dashboard" interfaceScript)
          ...
          in
          pkgs.runCommand "haishui-backend-bind-wait" { } (
            if failures != [ ] then
        ...
        cron-worker-user-scope =
          let
            ...
            managed = configOf { };
            gateway = managed.systemd.services.haishui-agent;
            ...
            unmanaged = (configOf { createUser = false; }).systemd.services.haishui-agent;
          ...
          in
          pkgs.runCommand "haishui-cron-worker-user-scope" { } (
            if failures != [ ] then
        ...
        # $HAISHUI_HOME/.env. The important property is that a second run
        ...
        env-file-assembly =
          let
            envScript = (import ./moduleCommon.nix { inherit lib; }).mkEnvScript {
              inherit pkgs;
              environment = {
                HAISHUI_PUBLIC = "visible";
              };
            };
          in
          pkgs.runCommand "haishui-env-file-assembly" { } ''
            set -e
            workdir=$(mktemp -d)
            ...
            grep -qx 'HAISHUI_PUBLIC=visible' "$workdir/.env" || \
        ...
        service-argv =
          let
            ...
            sentinel = "--haishui-nix-argv-probe";
            probe = argv: lib.escapeShellArgs (argv ++ [ sentinel ]);
          in
          pkgs.runCommand "haishui-service-argv" { } ''
            set -e
        ...
        package-contents = pkgs.runCommand "haishui-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${haishui-agent}/bin/haishui || (echo "FAIL: haishui binary missing"; exit 1)
          test -x ${haishui-agent}/bin/haishui-agent || (echo "FAIL: haishui-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${haishui-agent}/bin/haishui --version 2>&1 | grep -qi "haishui" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "haishui-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in haishui haishui-agent haishui-acp; do
            test -x ${haishui-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "haishui-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking haishui --help ==="
          ${haishui-agent}/bin/haishui --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${haishui-agent}/bin/haishui --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "haishui-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${haishui-agent}/share/haishui-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${haishui-agent}/share/haishui-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "HAISHUI_BUNDLED_SKILLS" ${haishui-agent}/bin/haishui || \
            (echo "FAIL: HAISHUI_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: HAISHUI_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${haishui-agent}/share/haishui-agent/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${haishui-agent}/share/haishui-agent/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "HAISHUI_OPTIONAL_SKILLS" ${haishui-agent}/bin/haishui || \
            (echo "FAIL: HAISHUI_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, HAISHUI_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "haishui-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${haishui-agent}/share/haishui-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${haishui-agent}/share/haishui-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "HAISHUI_BUNDLED_PLUGINS" ${haishui-agent}/bin/haishui || \
            (echo "FAIL: HAISHUI_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: HAISHUI_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "haishui-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${haishui-agent}/share/haishui-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${haishui-agent}/share/haishui-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${haishui-agent}/share/haishui-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "HAISHUI_BUNDLED_LOCALES" ${haishui-agent}/bin/haishui || \
            (echo "FAIL: HAISHUI_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: HAISHUI_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + HAISHUI_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && HAISHUI_BUNDLED_LOCALES=${haishui-agent}/share/haishui-agent/locales \
            ${haishuiVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with HAISHUI_BUNDLED_LOCALES set"; exit 1)
        ...
        # HAISHUI_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "haishui-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${haishui-agent}/share/haishui-agent/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${haishui-agent}/share/haishui-agent/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "HAISHUI_OPTIONAL_MCPS" ${haishui-agent}/bin/haishui || \
            (echo "FAIL: HAISHUI_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: HAISHUI_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${haishui-agent}/bin/haishui mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: haishui mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "haishui-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${haishui-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${haishui-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "HAISHUI_TUI_DIR" ${haishui-agent}/bin/haishui || \
            (echo "FAIL: HAISHUI_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: HAISHUI_TUI_DIR set in wrapper"
        ...
        # Verify HAISHUI_NODE is set in wrapper and points to Node 26+
        # (Haishui pins its toolchain to Node 26 everywhere)
        haishui-node = pkgs.runCommand "haishui-node-version" { } ''
          set -e
          echo "=== Checking HAISHUI_NODE in wrapper ==="
          grep -q "HAISHUI_NODE" ${haishui-agent}/bin/haishui || \
            (echo "FAIL: HAISHUI_NODE not set in wrapper"; exit 1)
          echo "PASS: HAISHUI_NODE present in wrapper"

          HAISHUI_NODE=$(sed -n "s/^export HAISHUI_NODE='\(.*\)'/\1/p" ${haishui-agent}/bin/haishui)
          test -x "$HAISHUI_NODE" || (echo "FAIL: HAISHUI_NODE=$HAISHUI_NODE not executable"; exit 1)
          echo "PASS: HAISHUI_NODE executable at $HAISHUI_NODE"

          NODE_MAJOR=$("$HAISHUI_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, Haishui requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All HAISHUI_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify HAISHUI_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "haishui-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(HAISHUI_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking HAISHUI_MANAGED guards ==="
          check_blocked "config set" ${haishui-agent}/bin/haishui config set model foo
          check_blocked "config edit" ${haishui-agent}/bin/haishui config edit
        ...
        extra-python-packages = let
          ...
          haishuiWithExtra = haishui-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "haishui-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${haishuiWithExtra}/bin/haishui || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${haishuiWithExtra}/bin/haishui || \
          ...
          if grep -q "PYTHONPATH" ${haishui-agent}/bin/haishui; then
        ...
        extra-dependency-groups = let
          haishuiWithGroups = haishui-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "haishui-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${haishuiWithGroups}"
          echo "venv: ${haishuiWithGroups.haishuiVenv}"
        ...
        messaging-variant = pkgs.runCommand "haishui-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.haishuiVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "haishui-config-roundtrip" {
          ...
          merge_and_load() {
            local haishui_home="$1"
            export HAISHUI_HOME="$haishui_home"
            ${configMergeScript} ${nixSettings} "$haishui_home/config.yaml"
            ${haishuiVenv}/bin/python3 -c '
import json, sys
