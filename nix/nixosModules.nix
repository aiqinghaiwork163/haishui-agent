# nix/nixosModules.nix — the NixOS module for haishui-agent
#
...
#
# Container mode: haishui runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $HAISHUI_HOME/.env
# and read by haishui at startup — no container recreation needed for env changes.
#
# Tool resolution: the haishui wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }:
{
  flake.nixosModules.default =
    ...
    let
      ...
      haishuiHome = "${cfg.stateDir}/.haishui";
      ...
      # HAISHUI_HOME via addToSystemPackages, so they can save settings through the
      ...
      # haishui user + sudo on first boot (writable layer persists), then
      # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
      containerEntrypoint = pkgs.writeShellScript "haishui-container-entrypoint" ''
        set -eu

        HAISHUI_UID="''${HAISHUI_UID:?HAISHUI_UID must be set}"
        HAISHUI_GID="''${HAISHUI_GID:?HAISHUI_GID must be set}"

        # ── Group: ensure a group with GID=$HAISHUI_GID exists ──
        # Check by GID (not name) to avoid collisions with pre-existing groups
        # (e.g. GID 100 = "users" on Ubuntu)
        EXISTING_GROUP=$(getent group "$HAISHUI_GID" 2>/dev/null | cut -d: -f1 || true)
        if [ -n "$EXISTING_GROUP" ]; then
          GROUP_NAME="$EXISTING_GROUP"
        else
          GROUP_NAME="haishui"
          if command -v groupadd >/dev/null 2>&1; then
            groupadd -g "$HAISHUI_GID" "$GROUP_NAME"
          elif command -v addgroup >/dev/null 2>&1; then
            addgroup -g "$HAISHUI_GID" "$GROUP_NAME" 2>/dev/null || true
          fi
        fi

        # ── User: ensure a user with UID=$HAISHUI_UID exists ──
        PASSWD_ENTRY=$(getent passwd "$HAISHUI_UID" 2>/dev/null || true)
        if [ -n "$PASSWD_ENTRY" ]; then
          TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
          TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
        else
          TARGET_USER="haishui"
          TARGET_HOME="/home/haishui"
          if command -v useradd >/dev/null 2>&1; then
            useradd -u "$HAISHUI_UID" -g "$HAISHUI_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
          elif command -v adduser >/dev/null 2>&1; then
            adduser -u "$HAISHUI_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
          fi
        fi
        mkdir -p "$TARGET_HOME"
        chown "$HAISHUI_UID:$HAISHUI_GID" "$TARGET_HOME"
        chmod 0750 "$TARGET_HOME"

        # Ensure HAISHUI_HOME is owned by the target user.
        ...
        if [ -n "''${HAISHUI_HOME:-}" ] && [ -d "$HAISHUI_HOME" ]; then
          find "$HAISHUI_HOME" \! -user "$HAISHUI_UID" -exec chown "$HAISHUI_UID:$HAISHUI_GID" {} +
        fi
        ...
        if [ ! -f /var/lib/haishui-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
          ...
          touch /var/lib/haishui-tools-provisioned
        fi

        if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/haishui ]; then
          mkdir -p /etc/sudoers.d
          echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/haishui
          chmod 0440 /etc/sudoers.d/haishui
        fi
        ...
        if command -v setpriv >/dev/null 2>&1; then
          exec setpriv --reuid="$HAISHUI_UID" --regid="$HAISHUI_GID" --init-groups "$@"
      ...
      # so they can update without recreation. Env vars go through $HAISHUI_HOME/.env.
      ...
      containerModeFile = pkgs.writeText "haishui-container-mode" ''
        # Written by the NixOS activation script. Do not edit manually.
        backend=${cfg.container.backend}
        container_name=${containerName}
        exec_user=${cfg.user}
        haishui_bin=${containerDataDir}/current-package/bin/haishui
      '';

      # Default: /var/lib/haishui/workspace → /data/workspace.
      ...
      commonServiceConfig = {
        ...
        # so interactive users in the haishui group can read/write them.
        UMask = "0007";
      ...
      // common.processEnvironment { inherit haishuiHome; };
    ...
    in
    {
      options.services.haishui-agent =
        ...
        // (
          with lib;
          {
            ...
            stateDir = mkOption {
              type = types.str;
              default = "/var/lib/haishui";
              description = "State directory. Contains .haishui/ subdir (HAISHUI_HOME).";
            };
            ...
            addToSystemPackages = mkOption {
              ...
              default = false;
              description = ''
                Add the haishui CLI to environment.systemPackages and export
                HAISHUI_HOME system-wide (via environment.variables) so interactive
            ...
            container = {
              ...
              hostUsers = mkOption {
                ...
                default = [ ];
                description = ''
                  Interactive users who get a ~/.haishui symlink to the service
                  stateDir. These users are automatically added to the haishui group.
                '';
                example = [ "sidbin" ];
              };
            };
      ...
      config = lib.mkIf cfg.enable (
        lib.mkMerge [
          ...
          # ── Host CLI ──────────────────────────────────────────────────────
          # Add the haishui CLI to system PATH and export HAISHUI_HOME system-wide
          # so interactive shells share state (sessions, skills, cron) with the
          # gateway service instead of creating a separate ~/.haishui/.
          (lib.mkIf cfg.addToSystemPackages {
            environment.systemPackages = [ effectivePackage ];
            environment.variables.HAISHUI_HOME = haishuiHome;
          })
              ...
              ++ [
                {
                  ...
                  message = "services.haishui-agent: backend.mode is not supported together with container.enable — the container runs the gateway only.";
                }
              ];
          }

          # ── Per-user profile for extraPackages ───────────────────────────
          # Wire extraPackages into the haishui user's per-user profile so the
          ...
          (lib.mkIf
            (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != [ ])
            {
              warnings = [
                ''
                  services.haishui-agent: container.enable is true and container.hostUsers
                  is set, but addToSystemPackages is false. Without a host-installed haishui
                  binary, container routing will not work for interactive users.
                  Set addToSystemPackages = true or ensure haishui is on PATH.
                ''
              ];
            }
          )
          ...
          # ── Directories ───────────────────────────────────────────────────
          {
            systemd.tmpfiles.rules = [
              ...
              "d ${haishuiHome}                  2770 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.stateDir}/home           0750 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.workingDirectory}        2770 ${cfg.user} ${cfg.group} - -"
            ]
            ++ map (d: "d ${haishuiHome}/${d} 2770 ${cfg.user} ${cfg.group} - -") common.stateSubdirs;
          }

          # ── Activation: link config + auth + documents ────────────────────
          {
            system.activationScripts."haishui-agent-setup" =
              lib.stringAfter
                (
                ...
                ''
                  ...
                  chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${haishuiHome} ${cfg.stateDir}/home ${cfg.workingDirectory}
                  chmod 2770 ${cfg.stateDir} ${haishuiHome} ${cfg.workingDirectory}
                  ...
                  find ${haishuiHome} -maxdepth 1 \
                    \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
                    -exec chmod g+rw {} + 2>/dev/null || true
                  for _subdir in ${lib.concatStringsSep " " common.stateSubdirs}; do
                    mkdir -p "${haishuiHome}/$_subdir"
                    chown ${cfg.user}:${cfg.group} "${haishuiHome}/$_subdir"
                    chmod 2770 "${haishuiHome}/$_subdir"
                    find "${haishuiHome}/$_subdir" -type f \
                      -exec chmod g+rw {} + 2>/dev/null || true
                  done

                  ${common.mkStateScript {
                    inherit pkgs cfg haishuiHome;
                    workingDirectory = cfg.workingDirectory;
                  ...
                  chown -h ${cfg.user}:${cfg.group} ${haishuiHome}/plugins/nix-managed-* 2>/dev/null || true
                  ...
                  ${
                    if cfg.container.enable then
                      ''
                        install -o ${cfg.user} -g ${cfg.group} -m 0644 ${containerModeFile} ${haishuiHome}/.container-mode
                      ''
                    else
                      ''
                        rm -f ${haishuiHome}/.container-mode
                        ...
                        ${lib.concatStringsSep "\n" (
                          map (
                            ...
                            let
                              ...
                              symlinkPath = "${userHome}/.haishui";
                            in
                            ''
                              if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${haishuiHome}" ]; then
                                rm -f "${symlinkPath}"
                                echo "haishui-agent: removed symlink ${symlinkPath}"
                              fi
                            ''
                  ...
                  # Create ~/.haishui -> stateDir/.haishui for each hostUser so the
          ...
          (lib.mkIf (!cfg.container.enable) {
            systemd.services.haishui-agent = {
              ...
              # $HAISHUI_HOME/.env by the activation script. load_haishui_dotenv()
              ...
              preStart = lib.mkIf lingerEnabled ''
                ...
                if [ ! -S "/run/user/$(id -u)/bus" ]; then
                  echo "haishui-agent: no user bus at /run/user/$(id -u)/bus after 10s;" \
          ...
          (lib.mkIf (!cfg.container.enable && cfg.backend.mode != "none") {
            systemd.services.haishui-backend = {
          ...
          (lib.mkIf cfg.container.enable {
            ...
            systemd.services.haishui-agent = {
              description = "Haishui Agent Gateway (container)";
              wantedBy = [ "multi-user.target" ];
              after = [
                "network-online.target"
              ]
              ...
              preStart = ''
                ...
                if [ "$NEED_CREATE" = "true" ]; then
                  ...
                  HAISHUI_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
                  HAISHUI_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
                  ...
                  ${containerBin} create \
                    ...
                    --env HAISHUI_UID="$HAISHUI_UID" \
                    --env HAISHUI_GID="$HAISHUI_GID" \
                    --env HAISHUI_HOME=${containerDataDir}/.haishui \
                    --env HAISHUI_MANAGED=true \
                    --env HOME=${containerHomeDir} \
                    ${lib.concatStringsSep " " cfg.container.extraOptions} \
                    ${cfg.container.image} \
                    ${containerDataDir}/current-package/bin/haishui gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
