# nix/desktop.nix — Haishui Desktop (Electron) app build + wrapper
#
# `haishuiAgent` is the fully-built `.#default` package — it ships the
# `haishui` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `HAISHUI_DESKTOP_HAISHUI` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Haishui CLI").
# No reimplementation of the agent resolution in this wrapper.
{
  ...
  # HAISHUI_HOME and HAISHUI_MANAGED here, which gives the app the same state
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "haishui-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "haishui-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = haishuiNpmLib.buildNpmPackage {
    dirs = [
      "apps/desktop"
      "apps/shared"
    ];
    pname = "haishui-desktop-renderer";
    ...
    buildPhase = ''
      ...
      pushd apps/desktop
        ...
        ${lib.getExe haishuiNpmLib.node-gyp} rebuild \
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/haishui-desktop $out/bin
    cp -r ${renderer}/* $out/share/haishui-desktop/
    ...
    substituteInPlace $out/share/haishui-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/haishui-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # HAISHUI_DESKTOP_HAISHUI to the absolute path of the nix-built `haishui`
    # binary so the desktop's resolver step 4 ("existing Haishui CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/haishui-desktop \
      --add-flags "$out/share/haishui-desktop" \
      --set HAISHUI_DESKTOP_HAISHUI "${lib.getExe haishuiAgent}" \
    ...
    install -m 0644 ${../apps/desktop/assets/icon.png} \
      $out/share/icons/hicolor/1024x1024/apps/haishui.png
    export PYTHONPATH=$(mktemp -d)
    cp ${../haishui_cli/linux_desktop_entry.py} "$PYTHONPATH/linux_desktop_entry.py"
    export DESKTOP_EXEC="$out/bin/haishui-desktop"
    export DESKTOP_ICON="$out/share/icons/hicolor/1024x1024/apps/haishui.png"
    python3 -c 'import os; from linux_desktop_entry import render_desktop_entry; print(render_desktop_entry(os.environ["DESKTOP_EXEC"], os.environ["DESKTOP_ICON"]))' > $out/share/applications/haishui.desktop
    runHook postInstall
  '';
