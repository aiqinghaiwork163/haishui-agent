...
#
# `services.haishui-agent` is the same option set on both modules. Both modules
...
#   homeManagerModules.nix  haishuiHome, programs.haishui-agent (the CLI and
...
let
  ...
  deepConfigType = types.mkOptionType {
    name = "haishui-config-attrs";
    description = "Haishui YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    options = {
      ...
      auth = mkOption {
        ...
        description = ''
          ...
          (remote MCP servers). Tokens are stored in $HAISHUI_HOME/mcp-tokens/.
        '';
      };
  ...
  sharedOptions =
    ...
    {
      enable = lib.mkEnableOption "Haishui Agent";
      ...
      package = mkOption {
        ...
        description = "The haishui-agent package to use.";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          The Haishui configuration, as an attribute set. The module joins the
          ...
          which includes the keys that `haishui config set` and the settings
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          $HAISHUI_HOME/.env. Haishui reads that file at each start, with
          load_haishui_dotenv().

          Each activation writes .env again from the start. Thus a secret
          file cannot go into .env two times.
        '';
        example = literalExpression ''[ config.sops.secrets."haishui/env".path ]'';
      };
      ...
      authFile = mkOption {
        ...
        description = ''
          ...
          does not exist. Thus a token that Haishui refreshes at runtime stays
          after an activation.
        '';
      };
      ...
      documents = mkOption {
        ...
        description = ''
          ...
          Haishui reads SOUL.md and memories/ from HAISHUI_HOME, so put those
          files in `haishuiHomeFiles`.
      ...
      haishuiHomeFiles = mkOption {
        type = documentsType;
        default = { };
        description = ''
          Files that the module installs into HAISHUI_HOME. Each key is a path
          relative to that directory, and the module makes the necessary
          subdirectories. Each value is a string or a path.

          Haishui reads SOUL.md and the memory files from HAISHUI_HOME and not
          from the working directory. Declare those files here, or Haishui
          does not load them.
        '';
        example = literalExpression ''
          {
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the haishui plugins
          directory. Each package must contain a plugin.yaml and __init__.py
          at its root. Haishui discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          haishui_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as haishui (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in haishui-agent's
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `haishui gateway`.";
      };
      ...
      # ── The backend: `haishui serve` or `haishui dashboard` ──────────────
      # `haishui serve` and `haishui dashboard` are the same entry point,
      # haishui_cli.main:cmd_dashboard, with one flag of difference. serve runs
      # without a user interface. dashboard also serves the web application.
      # Both give the /api/ws and /api/pty sockets that Haishui Desktop
      ...
      # controls an external gateway, with `haishui gateway restart`. It does
      ...
      backend = {
        mode = mkOption {
          ...
          description = ''
            ...
            - "serve"     — the backend without a user interface. It gives
                            the /api/ws and /api/pty sockets that Haishui
                            Desktop connects to.
        ...
        sessionTokenFile = mkOption {
          ...
          description = ''
            ...
            HAISHUI_DASHBOARD_SESSION_TOKEN. That token authorizes the /api
            routes and the /api/ws socket. Haishui Desktop presents the same
          ...
          '';
          example = literalExpression ''config.sops.secrets."haishui/desktop-token".path'';
        };
      };
    };
  ...
  # removal leaves them with no `haishui` on the PATH and no message. The
  ...
  installPackageRemovedMessage =
    value:
    ''
      services.haishui-agent.installPackage was removed. Haishui now
      separates the installation from the services, which is the
      Home Manager convention:

        programs.haishui-agent.enable = ${lib.boolToString (value != false)};  # the haishui CLI, and HAISHUI_HOME for your shells
        programs.haishui-agent.desktop.enable = true;  # the desktop application

      `services.haishui-agent` keeps the state, the configuration and
  ...
  mkConfigFiles =
    ...
    let
      generated = pkgs.writeText "haishui-config.yaml" (
  ...
  mkDocumentTree =
    { pkgs, documents }:
    pkgs.runCommand "haishui-documents" { } (
      ''
        mkdir -p $out
      ''
  ...
  mkEnvScript =
    { pkgs, environment }:
    let
      base = pkgs.writeText "haishui-env-base" (
        lib.concatStringsSep "\n" (lib.mapAttrsToList (k: v: "${k}=${v}") environment)
        + lib.optionalString (environment != { }) "\n"
      );
    in
    pkgs.writeShellScript "haishui-env-merge" ''
      set -eu
      ...
      for file in "$@"; do
        if [ -r "$file" ]; then
          ...
          echo "haishui-agent: WARNING cannot read environmentFile $file" >&2
        fi
      done
    '';
  ...
  mkStateScript =
    {
      ...
      # HAISHUI_MANAGED variable of the service. The value tells the shell
    ...
    let
      ...
      homeDocumentTree = mkDocumentTree {
        inherit pkgs;
        documents = cfg.haishuiHomeFiles;
      };
    ...
    in
    ''
      # Directories. The service units and Haishui make most of these
      ...
      ${run}mkdir -p ${
        lib.escapeShellArgs (
          ...
          ++ map (d: "${haishuiHome}/${d}") stateDirs
        )
      }

      # config.yaml: merge the Nix settings into the file on disk. Haishui
      ...
      ${
        if cfg.configFile != null then
          "${inst} -m ${modes.config} -D ${configFiles.effective} ${haishuiHome}/config.yaml"
        else
          ''
            ${run}${configFiles.mergeScript} ${configFiles.generated} ${haishuiHome}/config.yaml
            ${run}chmod ${modes.config} ${haishuiHome}/config.yaml
          ''
      }

      # The managed-mode marker. It makes an interactive shell also refuse to
      # change the configuration that Nix owns.
      ${inst} -m ${modes.managed} ${pkgs.writeText "haishui-managed" managedSystem} ${haishuiHome}/.managed

      ${lib.optionalString (cfg.environment != { } || cfg.environmentFiles != [ ]) ''
        ${run}${envScript} ${haishuiHome}/.env ${modes.env} ${lib.escapeShellArgs cfg.environmentFiles}
        ${lib.optionalString (owner != null) "${run}chown ${owner} ${haishuiHome}/.env"}
      ''}

      ${lib.optionalString (cfg.authFile != null) (
        if cfg.authFileForceOverwrite then
          "${inst} -m ${modes.auth} ${cfg.authFile} ${haishuiHome}/auth.json"
        else
          ''
            if [ ! -e ${haishuiHome}/auth.json ]; then
              ${inst} -m ${modes.auth} ${cfg.authFile} ${haishuiHome}/auth.json
            fi
          ''
      )}
      ...
      ${installDocuments homeDocumentTree haishuiHome cfg.haishuiHomeFiles}
      ...
      ${run}find ${haishuiHome}/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
      ${lib.concatMapStringsSep "\n" (plugin: ''
        if [ ! -f ${plugin}/plugin.yaml ]; then
          echo "haishui-agent: ERROR extraPlugins entry '${plugin}' has no plugin.yaml" >&2
          exit 1
        fi
        ${run}ln -sfn ${plugin} ${haishuiHome}/plugins/nix-managed-${lib.getName plugin}
      '') cfg.extraPlugins}
    '';
  ...
  gatewayArgv =
    cfg:
    [
      "${effectivePackage cfg}/bin/haishui"
      "gateway"
    ]
    ++ cfg.extraArgs;
  ...
  #
  # `exec` on the last line keeps haishui as the MainPID of the unit. No shell
  ...
  backendLauncher =
    ...
    pkgs.writeShellScript "haishui-backend-launch" (
      builtins.replaceStrings [ "@HOST@" ] [ ''"$_target"'' ] ''
        ...
        ${lib.optionalString (cfg.backend.sessionTokenFile != null) ''
          ...
          if [ ! -r "$_token_file" ]; then
            echo "haishui-backend: cannot read the session token file '$_token_file'. The unit stops." >&2
            echo "haishui-backend: backend.sessionTokenFile must name a runtime path that this user can read." >&2
            exit 1
          fi

          HAISHUI_DASHBOARD_SESSION_TOKEN="$(${pkgs.coreutils}/bin/tr -d '\r\n' < "$_token_file")"
          export HAISHUI_DASHBOARD_SESSION_TOKEN

          if [ -z "$HAISHUI_DASHBOARD_SESSION_TOKEN" ]; then
            echo "haishui-backend: the session token file '$_token_file' is empty. The unit stops." >&2
            exit 1
          fi
        ''}
        ${
          if cfg.backend.waitFor == null then
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "haishui-backend: '$_target' did not resolve after ''${_timeout}s. The unit stops." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "haishui-backend: waits for '$_target' to resolve..." >&2
                fi
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "haishui-backend: interface '$_iface' had no IPv4 address after ''${_timeout}s. The unit stops." >&2
                  echo "haishui-backend: a fallback address can expose the backend more widely than you intend." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "haishui-backend: waits for an IPv4 address on '$_iface'..." >&2
                fi
                ${pkgs.coreutils}/bin/sleep 2
                _waited=$(( _waited + 2 ))
              done
        ...
        echo "haishui-backend: binds to $_target:${toString cfg.backend.port} (from $_how)" >&2
  ...
  backendDescription =
    ...
    if cfg.backend.mode == "dashboard" then
      "Haishui Agent web dashboard and desktop backend"
    else
      "Haishui Agent backend for Haishui Desktop";

  # The environment that each Haishui process needs, from either module.
  #
  # managedSystem gives the value of HAISHUI_MANAGED. The CLI reads that
    ...
    [
      {
        ...
        message = ''
          ...
          To give Haishui an identity and a memory, use
          ${optionPath}.haishuiHomeFiles instead. Those files go to
          HAISHUI_HOME. Haishui reads SOUL.md and memories/ only from there.
        '';
      }
    ];
  ...
  # The subdirectories of HAISHUI_HOME that both modules make.
  stateSubdirs = [
    "cron"
    "sessions"
    "logs"
