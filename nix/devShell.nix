...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      haishuiNpmLib = self'.packages.default.passthru.haishuiNpmLib;
      ...
      haishuiAgentDevShellHook = self'.packages.default.passthru.devShellHook;
    in
    {
      devShells.default = pkgs.mkShell {
        packages = with pkgs; [
          (pkgs.runCommand "haishui" { } ''
            mkdir -p $out/bin
            install -Dm755 ${../haishui} $out/bin/haishui
          '')
        ...
        shellHook = ''
          ${haishuiAgentDevShellHook}
          ${haishuiNpmLib.mkNpmDevShellHook npmPackageJsonPaths}
          ...
          export HAISHUI_PYTHON_SRC_ROOT=$(git rev-parse --show-toplevel)
          ...
          echo "Haishui Agent dev shell in $HAISHUI_PYTHON_SRC_ROOT"
          echo "Ready. Run 'haishui' or 'sandbox haishui' to start."
        '';
      };
    };
}
