...
### 1. Start order: editor first, Haishui second

Haishui probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Haishui session.
...
restart the editor, restart the Haishui session.
...
server fails to bind (Output Log) or Haishui' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.haishui/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Haishui

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Haishui' connection
lives in `~/.haishui/config.yaml` via `haishui mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Haishui — just don't
mistake it for the Haishui setup step.
...
explicitly warns against overlapping calls. Haishui executes same-turn tool
...
### 9. Timeouts: Haishui gives up before Unreal does

Haishui' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Haishui runs elsewhere (SSH backend,
