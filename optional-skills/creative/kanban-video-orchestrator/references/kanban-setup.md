...
the actual `setup.sh` that creates the project workspace, configures Haishui
...
2. **Create profiles** — `haishui profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.haishui/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `haishui kanban create` assigned to the director
...
```bash
haishui profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.haishui/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.haishui/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Haishui
...
Each profile gets a `SOUL.md` at `~/.haishui/profiles/<name>/SOUL.md` that
...
```bash
haishui kanban create "Direct production of <video title>" \
    --assignee director \
...
the Haishui `.env` (`${HAISHUI_HOME:-$HOME/.haishui}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _haishui_env="${HAISHUI_HOME:-$HOME/.haishui}/.env"
    if grep -q "^${var}=" "$_haishui_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_haishui_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_haishui_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY haishui ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY haishui OPENROUTER_API_KEY || exit 1
# ...
```
