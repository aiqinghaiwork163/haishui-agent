...
haishui kanban watch --tenant <project-slug>

# Snapshot of the board
haishui kanban list --tenant <project-slug>
haishui kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
haishui kanban stats --tenant <project-slug>

# Visual dashboard (browser)
haishui dashboard

# Inspect a specific task (includes comments + events)
haishui kanban show <task-id>

# Follow a single task's event stream
haishui kanban tail <task-id>
```

Verify available subcommands with `haishui kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `haishui kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `haishui kanban block <id>` then `haishui kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `haishui kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `haishui kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `haishui profile list` (profile exists?), `haishui status` (gateway/dispatcher up?), and verify tenant. |
...
haishui kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
haishui kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(haishui kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
haishui kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
haishui kanban block <task-id>
haishui kanban archive <task-id>

# Diagnose what happened
haishui kanban show <task-id>      # task body, comments, recent events
haishui kanban tail <task-id>      # follow the live event stream
haishui kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `haishui kanban create` for the director
...
while true; do
    clear
    haishui kanban list --tenant <slug>
    echo "---"
    haishui kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `haishui kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `haishui profile list` and start
