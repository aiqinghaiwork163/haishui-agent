...
Maps each role archetype to the Haishui skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public haishui-agent
...
## Haishui skills relevant to video production

### Visual / rendering skills (`haishui-agent/skills/creative/`)
...
| `touchdesigner-mcp` (via the `touchdesigner` plugin: `haishui plugins install touchdesigner`) | Control a running TouchDesigner instance — real-time visuals, audio-reactive installation art, VJ | Renderer for real-time/audio-reactive content; installation art; live performance |
...
### Design / pre-production skills (`haishui-agent/skills/creative/`)
...
### Audio / media skills (`haishui-agent/skills/creative/` + `skills/media/`)
...
These are **not** Haishui skills but external CLIs / APIs that profiles invoke.
...
## Built-in Haishui tools for media review

These are native Haishui tools — not invoked via terminal but through their own
...
key is present in `${HAISHUI_HOME:-~/.haishui}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Haishui profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${HAISHUI_HOME:-~/.haishui}/.env` → environment variable.
...
When a new Haishui-public video skill ships:
