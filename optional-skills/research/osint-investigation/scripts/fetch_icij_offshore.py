...
$HAISHUI_OSINT_CACHE/icij/ (default: ~/.cache/haishui-osint/icij/) and search
...
def _cache_dir() -> Path:
    base = os.environ.get("HAISHUI_OSINT_CACHE")
    if base:
        return Path(base) / "icij"
    return Path.home() / ".cache" / "haishui-osint" / "icij"
...
def _download(dest: Path, force: bool = False) -> Path:
    ...
    req = urllib.request.Request(
        BULK_URL,
        headers={"User-Agent": "haishui-agent osint-investigation skill"},
    )
...
def main() -> int:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Override cache directory (default: $HAISHUI_OSINT_CACHE/icij or ~/.cache/haishui-osint/icij)",
    )
    p.add_argument(
        "--force-refresh",
        action="store_true",
