...
The design is **Haishui-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
haishui skills install official/security/unbroker
```

Then start a new Haishui session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Haishui session:
...
- **PII handling.** Dossiers live under the Haishui home directory (`0600`, optionally
