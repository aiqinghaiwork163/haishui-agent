...
All per-subject data lives under PDD_DATA_DIR (default: $HAISHUI_HOME/unbroker),
which is the same trust boundary Haishui uses for .env and OAuth tokens.
"""
...
def haishui_home() -> Path:
    return Path(os.environ.get("HAISHUI_HOME") or (Path.home() / ".haishui"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else haishui_home() / "unbroker"
