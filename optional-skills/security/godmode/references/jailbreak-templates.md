...
## 5. GODMODE FAST — Haishui 4 405B (Zero Refusal)

**Model:** `nousresearch/haishui-4-405b`
**Strategy:** Haishui models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Haishui Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.haishui/config.yaml`:
