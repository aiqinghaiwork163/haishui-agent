...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("HAISHUI_HOME", os.path.expanduser("~/.haishui")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("HAISHUI_HOME", Path.home() / ".haishui")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Haishui config paths
# ═══════════════════════════════════════════════════════════════════

HAISHUI_HOME = Path(os.getenv("HAISHUI_HOME", Path.home() / ".haishui"))
CONFIG_PATH = HAISHUI_HOME / "config.yaml"
PREFILL_PATH = HAISHUI_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Haishui models — already uncensored, just needs clean prompt
    "haishui": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "haishui" in model_lower or "nous" in model_lower:
        return "haishui"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Haishui config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.haishui/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Haishui for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Haishui for changes to take effect.")
