...
is typically only in `~/.haishui/.env`, not a shell profile, so the env form
...
```
OpenCode DOES use the `provider/model` slash form on the CLI (unlike Haishui,
...
`haishui-custom-providers` skill, pitfall 2). Haishui hits this because it forwards
