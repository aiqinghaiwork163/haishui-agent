...
Usage (via cron with --no-agent):
    ...
    haishui cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$HAISHUI_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Haishui-Watcher/1.0"})
