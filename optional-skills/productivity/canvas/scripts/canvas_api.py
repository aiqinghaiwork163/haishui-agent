#!/usr/bin/env python3
"""Canvas LMS API CLI for Haishui Agent.
...
def _check_config():
    ...
    if missing:
        haishui_env = os.path.join(
            os.environ.get("HAISHUI_HOME", os.path.expanduser("~/.haishui")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {haishui_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Haishui Agent"
    )
