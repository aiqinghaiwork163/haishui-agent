...
Usage:
  ...
  <server_name> matches the files in $HAISHUI_HOME/mcp-tokens/<server>.json etc.
...
def _haishui_home():
    # Prefer Haishui' own resolver (profile-safe); fall back to env then ~/.haishui.
    try:
        from haishui_constants import get_haishui_home
        return str(get_haishui_home())
    except Exception:
        return os.environ.get("HAISHUI_HOME") or os.path.expanduser("~/.haishui")


def _tokens_dir():
    return os.path.join(_haishui_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "haishui-diag", "version": "1.0"}},
        },
        headers={
