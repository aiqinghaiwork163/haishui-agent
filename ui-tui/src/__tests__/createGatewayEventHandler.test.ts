...
describe('createGatewayEventHandler', () => {
  ...
  it('surfaces self-improvement review summaries as a persistent system line', () => {
    ...
    onEvent({
      payload: { text: "💾 Self-improvement review: Skill 'haishui-release' patched" },
      type: 'review.summary'
    } as any)

    expect(ctx.system.sys).toHaveBeenCalledWith("💾 Self-improvement review: Skill 'haishui-release' patched")
  })
  ...
  it('annotates gateway.start_timeout with stderr tail lines so users can diagnose without /logs', () => {
    ...
    onEvent({
      payload: {
        ...
        stderr_tail:
          '[startup] timed out\nModuleNotFoundError: No module named openai\nFileNotFoundError: ~/.haishui/config.yaml'
      },
  ...
  it('prefers raw text over Rich-rendered ANSI on message.complete (#16391)', () => {
    ...
    const rendered = '\u001b[33mHaishui here.\u001b[0m\n\n\u001b[2mLine two.\u001b[0m'
  ...
  it('shows setup panel for missing provider startup error', () => {
    ...
    onEvent({
      payload: {
        message:
          'agent init failed: No LLM provider configured. Run `haishui model` to select a provider, or run `haishui setup` for first-time configuration.'
      },
      type: 'error'
    } as any)
  ...
  it('picks the polarity-matching paired palette from gateway.ready skins', async () => {
    ...
    vi.stubEnv('HAISHUI_TUI_BACKGROUND', '')
    ...
    vi.stubEnv('HAISHUI_TUI_BACKGROUND', '#ffffff')
    createGatewayEventHandler(buildCtx(appended))({ payload: skin, type: 'skin.changed' } as any)
  ...
  it('opens a fresh session before starting voice after wake detection', async () => {
    ...
    createGatewayEventHandler(ctx)({
      payload: { phrase: 'hey haishui', start_new_session: true },
      type: 'wake.detected'
    } as any)
  ...
  it('keeps the current session when wake detection disables session creation', async () => {
    ...
    createGatewayEventHandler(ctx)({
      payload: { phrase: 'hey haishui', start_new_session: false },
      type: 'wake.detected'
    } as any)
  ...
  it('keeps gateway noise informational and approval out of Activity', async () => {
    ...
    onEvent({ payload: { line: 'INFO haishui.mcp: 3 servers discovered' }, type: 'gateway.stderr' } as any)
