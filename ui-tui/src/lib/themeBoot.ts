 ...
 * `haishui-boot-background` / `haishui-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports HAISHUI_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.haishui matches get_haishui_home()'s default.
const bootFilePath = () => join(process.env.HAISHUI_HOME ?? join(homedir(), '.haishui'), 'tui-theme-boot.json')

// Never touch the user's real ~/.haishui from test runs (the TS suite has no
// HAISHUI_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into HAISHUI_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.HAISHUI_TUI_THEME || env.HAISHUI_TUI_LIGHT) {
    return result
  }
  ...
  if (boot.mode) {
    env.HAISHUI_TUI_THEME = boot.mode
  ...
  if (
    ...
    !env.HAISHUI_TUI_BACKGROUND
  ) {
    env.HAISHUI_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.HAISHUI_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.HAISHUI_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into HAISHUI_TUI_THEME.
