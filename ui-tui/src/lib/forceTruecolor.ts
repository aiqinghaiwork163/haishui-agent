...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.HAISHUI_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Haishui on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via HAISHUI_TUI_TRUECOLOR=1.
