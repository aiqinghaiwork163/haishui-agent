...
const logDir = join(process.env.HAISHUI_HOME?.trim() || join(homedir(), '.haishui'), 'logs')
...
// a real ~/.haishui (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('haishui-tui: parent lifecycle log unavailable\n')
    }
  }
}
