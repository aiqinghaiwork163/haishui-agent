import { Box, Text, useInput, useStdout } from '@haishui/ink'
import { fuzzyRank } from '@haishui/shared/fuzzy'
import type { ModelOptionProvider, ModelOptionsResult } from '@haishui/shared/gateway-events'
import { modelSearchText } from '@haishui/shared/model-search-text'
import { REASONING_EFFORTS } from '@haishui/shared/reasoning-effort'
...
export function ModelPicker({
  ...
  useInput((ch, key) => {
                ...
                prev.map(p =>
                  p.slug === provider.slug
                    ? {
                        ...
                        models: [],
                        total_models: 0,
                        warning: p.key_env ? `paste ${p.key_env} to activate` : 'run `haishui model` to configure'
                      }
                    : p
                )
              )
    ...
    if (key.return) {
      if (stage === 'provider') {
        ...
        if (provider.authenticated === false) {
          ...
          // Other auth types: no-op (warning shown tells them to run haishui model)
          return
        }
    ...
    // filter. With Ctrl held, @haishui/ink reports `ch` as the key name ('g'),
    ...
    return (
      <Box flexDirection="column" width={width}>
        ...
        <Text color={t.color.muted} wrap="truncate-end">
          Paste your API key below (saved to ~/.haishui/.env)
        </Text>
