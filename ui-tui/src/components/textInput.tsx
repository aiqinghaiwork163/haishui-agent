import type { InputEvent, Key } from '@haishui/ink'
import * as Ink from '@haishui/ink'
 ...
 * but that accepts `key.meta` on macOS — and haishui-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.HAISHUI_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
  ...
  useInput(
    (inp: string, k: Key, event: InputEvent) => {
      ...
      if (actionHome) {
        ...
        // ESC d, which haishui-ink decodes as meta+'d'; without this branch it
