...
import { Box, Text } from '@haishui/ink'
...
/**
 * User widget apps — Haishui authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$HAISHUI_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.haishui/plugins/`: files under HAISHUI_HOME execute
...
const widgetsDir = () => join(process.env.HAISHUI_HOME?.trim() || join(homedir(), '.haishui'), 'tui-widgets')
 ...
 *  every change, so a widget Haishui writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Haishui) ever writes must hot-load too — a 10s poll here read
