import type { RunExternalProcess } from '@haishui/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`haishui ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching haishui: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`haishui ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
