...
import { terminalBackgroundHex } from '@haishui/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $HAISHUI_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see HAISHUI_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['HAISHUI_TUI_BACKGROUND', process.env.HAISHUI_TUI_BACKGROUND ?? '(unset)'],
            ['HAISHUI_TUI_THEME', process.env.HAISHUI_TUI_THEME ?? '(unset)'],
