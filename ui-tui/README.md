...
React + Ink terminal UI for Haishui. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `HAISHUI_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `haishui setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.haishui/.haishui_history` or under `HAISHUI_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `haishui setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/haishui-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external haishui setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.haishui/tui-parent.log
    ...
    types/
      haishui-ink.d.ts            type declarations for @haishui/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent HaishuiCLI subprocess for slash commands
```
