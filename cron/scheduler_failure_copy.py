...
Every notice says WHAT happened and WHAT TO DO, and names the exact ``haishui cron`` command plus
...
def cron_output_dir_display(job_id: str) -> str:
    ...
    return f"{display_haishui_home()}/cron/output/{job_id}/"
...
_PROVIDER_FAILURE_ACTION: dict[str, str] = {
    "billing": (
        ...
        "`haishui cron edit {job_id} --provider <name>`."
    ),
    "auth": (
        "Sign in again with /login (or `{relogin}` in a terminal), or pin a "
        "working provider with `haishui cron edit {job_id} --provider <name>`, then "
        "`haishui cron run {job_id}` to retry."
    ),
    "model_not_found": "Pick another model with `haishui cron edit {job_id} --model <name>`.",
    "context_overflow": "Shorten the job's prompt with `haishui cron edit {job_id} --prompt <text>`.",
}
...
_PROVIDER_FAILURE_ACTION["content_policy_blocked"] = (
    "Reword the job's prompt with `haishui cron edit {job_id} --prompt <text>`, or pick another "
    "model with `haishui cron edit {job_id} --model <name>`."
)
_DEFAULT_FAILURE_ACTION = "Run it again with `haishui cron run {job_id}`, or edit it with `haishui cron edit {job_id}`."
...
def provider_failure_notice(
    ...
    if reason in _TRANSIENT_REASONS:
        action = (
            ...
            f"`haishui cron run {job_id}` tries now."
        )
    else:
    ...
    return (
        ...
        f"Run log: `haishui cron runs {job_id}`."
    )
...
def generic_failure_notice(job_name: str, job_id: str, cleaned_error: str) -> str:
    ...
    return (
        ...
        f"See the full run with `haishui cron runs {job_id}` (output saved under "
        f"{cron_output_dir_display(job_id)}); run it again with `haishui cron run {job_id}`, "
        f"edit it with `haishui cron edit {job_id}`, or pause it with `haishui cron pause {job_id}`."
    )
...
def script_timeout_notice(job_name: str, job_id: str) -> str:
    return (
        ...
        f"Check the script's output under {cron_output_dir_display(job_id)} or `haishui cron runs {job_id}`, "
        f"then run it again with `haishui cron run {job_id}`."
    )
...
def inactivity_notice(job_name: str, job_id: str) -> str:
    return (
        ...
        f"{cron_output_dir_display(job_id)} (`haishui cron runs {job_id}`), then run it again with "
        f"`haishui cron run {job_id}`."
    )
...
def blocked_config_notice(job_name: str, reason: str) -> str:
    ...
    if reason and reason[-1] not in ".!?":
        reason += "."
    return (
        f"⛔ Cron '{job_name}' did not run: {reason} Nothing was charged. Haishui will try again at "
