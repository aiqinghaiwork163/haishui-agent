"""Cron job storage: ~/.haishui/cron/jobs.json; output in
~/.haishui/cron/output/{job_id}/{timestamp}.md"""
...
from haishui_time import get_timezone
...
# Cron is per-profile by design: anchor at get_haishui_home() (active profile home), NOT
# get_default_haishui_root() — the shared root would funnel every profile's jobs into one jobs.json
# and run them under the ticker's HAISHUI_HOME, leaking config/credentials/skills across profiles.
# Each profile owns its own cron store under its own HAISHUI_HOME, and a profile-scoped gateway runs that
# profile's jobs under that same HAISHUI_HOME — so a job authored in profile `coder` lives in
# `~/.haishui/profiles/coder/cron/jobs.json` and executes with `coder`'s `.env`, `config.yaml`, and skills.
# Do NOT change this to the default root: that re-breaks per-profile isolation. See also the dynamic
# `_get_haishui_home()` / `_get_lock_paths()` resolution in cron/scheduler.py. See #4707.
HAISHUI_DIR = get_haishui_home().resolve()
...
CRON_DIR = HAISHUI_DIR / "cron"
JOBS_FILE = CRON_DIR / "jobs.json"
# Heartbeat: touched every ticker loop so `haishui cron status` can tell the ticker THREAD is alive,
# not just the gateway PROCESS; success = last tick that completed WITHOUT raising.
# The gateway process and the (separate) ``haishui cron status`` process share it so status can tell whether
...
# threshold in `haishui cron status` (haishui_cli/cron.py), so they never drift apart.
...
def _current_cron_store() -> _CronStorePaths:
    ...
    get_haishui_home(), so re-pointing HAISHUI_HOME after import uses ITS OWN store rather than the
...
# Fallback stale-recovery window for a one-shot's running-claim when HAISHUI_CRON_TIMEOUT=0
...
def _oneshot_run_claim_ttl_seconds() -> float:
    """One-shot running-claim TTL from ``HAISHUI_CRON_TIMEOUT``: unset/invalid → 600s → 1800s;
...
def _secure_dir(path: Path):
    ...
    container/HAISHUI_HOME_MODE rules as the rest of HAISHUI_HOME (#10757)."""
    from haishui_cli.config import _secure_dir as _shared_secure_dir
    _shared_secure_dir(path)


def _secure_file(path: Path):
    """Owner-only (0600) via the shared helper (managed/container skip included)."""
    from haishui_cli.config import _secure_file as _shared_secure_file
...
def _is_named_profile_path(path: Path) -> bool:
    """True if *path* is under ``<haishui_home>/profiles/<name>/`` (default/custom homes are not).
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    if 'T' in schedule or re.match(r'^\d{4}-\d{2}-\d{2}', schedule):
        try:
            ...
            # Naive timestamps become aware in the CONFIGURED Haishui timezone (not server-local):
            # the due-check compares against haishui_time.now().
            ...
            if dt.tzinfo is None:
                dt = dt.replace(tzinfo=_haishui_now().tzinfo)
            return {
                "kind": "once",
                "run_at": dt.isoformat(),
...
def _ensure_aware(dt: datetime) -> datetime:
    """Aware datetime in the configured Haishui timezone. Legacy naive values are read as
...
def _record_persisted_error_recovery(job: Dict[str, Any], previous_next_run: str) -> None:
    ...
    entry = {
        ...
        "rearmed_at": _haishui_now().isoformat(),
    }
...
def _record_timezone_migration_catchup(
    ...
    entry = {
        ...
        "fired_at": _haishui_now().isoformat(),
    }
    ...
    if kind == "cron":
        ...
        if not _ensure_croniter():
            logger.warning(
                ...
                "reinstall haishui-agent or run 'pip install croniter' in your runtime env.",
                expr)
            return None
...
# --- Ticker heartbeat (liveness signal for `haishui cron status`) ---
...
def record_ticker_heartbeat(success: bool = False) -> None:
    ...
    tick* marker. We track two distinct signals so `haishui cron status` can tell a thread that is merely
...
def get_ticker_heartbeat_age() -> Optional[float]:
    ...
    critical under multiplex_profiles where ``haishui cron status`` must report per-profile liveness
    (#69377).
    """
...
def _stage_jobs_payload(jobs_file: Path, jobs: List[Dict[str, Any]]) -> str:
    ...
    try:
        with os.fdopen(fd, "w", encoding="utf-8") as f:
            json.dump(
                {"jobs": jobs, "updated_at": _haishui_now().isoformat()},
    ...
    *, provider: Any, model: Any, base_url: Any, no_agent: Any,
    ...
    if normalized_provider is None:
        with contextlib.suppress(Exception):
            ...
            # Delegate all rate-limit / 5xx retry to haishui's outer conversation loop, which honors
...
def pause_job(job_id: str, reason: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    return update_job(job["id"], {
        "enabled": False,
        "state": "paused",
        "paused_at": _haishui_now().isoformat(),
...
def trigger_job(job_id: str, extra_prompt: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    if is_terminal_job(job):
        ...
        raise ValueError(
            ...
            f"Create a new occurrence with 'haishui cron resume {name} "
            "--run-now' or '--at <ISO-8601>'.")
    manual_run_at = _haishui_now().isoformat()
    return update_job(job["id"], {
        "enabled": True,
        "state": "scheduled",
        "paused_at": None,
...
def _claim_owner_is_dead(claim: Dict[str, Any]) -> bool:
    ...
    ``_machine_id()`` stamps ``host:pid[:token]``; a foreign host, an explicit HAISHUI_MACHINE_ID,
...
def _claim_is_live(claim: Any, now: datetime, ttl_seconds: float) -> bool:
    ...
    after the TTL (a killed ``haishui cron run`` otherwise blocks the next manual run for the
...
def note_fire_forward_failure(job_id: str, detail: str) -> bool:
    ...
    def apply(jobs, _i, job):
        job["last_fire_error"] = {
            "at": _haishui_now().isoformat(), "detail": str(detail or "")[:500]}
        save_jobs(jobs)
        return True
...
def _write_wedged_oneshot_diagnostic(job: Dict[str, Any]) -> None:
    ...
    written = _write_oneshot_diagnostic(
        ...
        f"- removed at: {_haishui_now().isoformat()}\n\n"
...
def _refresh_claim(jobs: List[Dict[str, Any]], claim: Any, expected_owner: str) -> bool:
    ...
    if not isinstance(claim, dict) or claim.get("by") != expected_owner:
        return False
    claim["at"] = _haishui_now().isoformat()
    save_jobs(jobs)
    return True
...
def _machine_id() -> str:
    ...
    fresh-claim check): ``HAISHUI_MACHINE_ID`` if set, else hostname:pid."""
    explicit = os.getenv("HAISHUI_MACHINE_ID", "").strip()
    if explicit:
        return explicit
    try:
        import socket
...
def _oneshot_dispatch_limit_reached(job: Dict[str, Any], scan: _DueScan) -> bool:
    ...
    if job.get("last_run_at") is not None:
        ...
        logger.warning(
            ...
            "'haishui cron resume <job> --run-now' (#93524).",
...
def _evaluate_due_job(job: Dict[str, Any], scan: _DueScan, run_claim_ttl: float) -> bool:
    ...
    if kind == "once":
        ...
        # same HAISHUI_HOME must not re-dispatch it while in flight, and advancing next_run_at by a
...
def _get_due_jobs_locked() -> List[Dict[str, Any]]:
    ...
    raw_jobs = load_jobs()
    scan = _DueScan(raw_jobs, _haishui_now())
...
# the disk.
# Unlike the quick-snapshot store (`haishui_cli.backup`, capped at 20) it had no retention, so a
...
def save_job_output(job_id: str, output: str):
    ...
    output_file = job_output_dir / f"{_haishui_now().strftime('%Y-%m-%d_%H-%M-%S')}.md"
