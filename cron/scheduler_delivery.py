...
def _open_continuable_cron_thread(job: dict, adapter, chat_id: str, loop) -> Optional[str]:
    ...
    if not callable(create_thread) or loop is None:
        return None
    thread_name = f"Haishui — {_cron_display_name(job)}"
    try:
...
def cron_delivery_targets() -> list[dict]:
    ...
    try:
        from haishui_cli.profiles import list_profile_names
...
def _run_bot_chat_turn(argv: list, env: dict, report_path: str, timeout: float) -> subprocess.CompletedProcess:
    """Run one ``haishui chat -Q`` delivery child; the cap bounds the TURN, not the process.

    The child records its turn outcome at *report_path* (``haishui_cli.quiet_single_query``)
    ...
    """
    from haishui_cli.quiet_single_query import read_turn_report
...
def _deliver_to_bot_chat(job: dict, content: str, profile: str, *, deferred: Optional[dict] = None,
    ...
    try:
        source_home = get_haishui_home().resolve()
    ...
    # The running install first (same trust order as gateway.run._resolve_haishui_bin): the
    # scheduler lives in the long-running gateway, so a PATH-first lookup would hand delivery
    # to whatever `haishui` PATH names — another install, or a planted one — instead of this one.
    try:
        import importlib.util as _ilu
        found = _ilu.find_spec("haishui_cli") is not None
    except Exception:
        found = False
    if found:
        argv = [sys.executable, "-m", "haishui_cli.main"]
    else:
        haishui_bin = shutil.which("haishui")
        if not haishui_bin:
            return ("Haishui could not deliver this result to Bot Chat: the `haishui` command was not found. "
                    "The result is saved; run `haishui cron runs` to see it, or `haishui doctor` if this keeps happening")
        argv = [haishui_bin]
    ...
    try:
        with tempfile.NamedTemporaryFile(
            "w", encoding="utf-8", suffix=".txt", prefix="haishui-cron-botchat-", delete=False,
        ) as fh:
            fh.write(message)
            query_file = fh.name
        ...
        from haishui_cli.quiet_single_query import TURN_REPORT_FILE_ENV
        ...
        if result.returncode != 0:
            ...
            return (
                f"Haishui could not deliver this result to Bot Chat (profile '{profile_label}'). "
                "The result is saved; run `haishui cron runs` to see it, or `haishui doctor` if this keeps happening"
                f". Details: {tail}")
...
def _deliver_result(
    ...
    # job's own attempt: a worker's script may dispatch another job in-process (`haishui cron run`),
    # and that nested delivery must not be keyed under the outer execution id.
    external_execution = os.environ.get("_HAISHUI_CRON_EXTERNAL_WORKER", "")
    ...
    # persisted as ``last_delivery_unverified`` so `haishui cron list` shows it.
    ...
    # at boot; standalone runs (`haishui cron run`) did not, silently dropping files. Idempotent.
