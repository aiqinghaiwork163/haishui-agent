...
``cron.scheduler``, not ``haishui_cli.main``, so nothing bootstraps the gateway's checkout
...
The shared subprocess sanitizer strips Haishui-owned PYTHONPATH entries because user
children must not see our tree. This child IS Haishui, so the pin is applied *after* the
...
def pin_haishui_tree_on_pythonpath(worker_env: dict, repo_root: Path) -> dict:
