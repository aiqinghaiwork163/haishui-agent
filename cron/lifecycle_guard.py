...
A cron job that restarts/stops the gateway from inside the gateway (``haishui gateway restart``,
``launchctl kickstart ai.haishui.gateway``, ``systemctl restart haishui-gateway``) kills the process,
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: destructive `haishui gateway` ops. `start` is excluded: starting from inside a
    # gateway is benign and a job may legitimately start a sibling profile. The lookbehind keeps
    # `haishui` from being a path component or word tail (`/docs/haishui gateway restart-notes.md`)
    # while every real command position (text start, whitespace, `;`/`&`/`|`, `$(`, backtick,
    # U+FFFD) still matches.
    # See #77173.
    # Windows spells the CLI with a launcher suffix (`haishui.exe`, npm-style `haishui.cmd`/`.ps1`);
    # same command, so the suffix is optional here.
    r"(?:(?<![/\w.\-])haishui(?:\.(?:exe|cmd|bat|com|ps1))?\s+gateway\s+(?:restart|stop|uninstall)\b)"
    # Branch B: launchctl ops anchored on a haishui-gateway label so unrelated haishui services stay
    ...
    # -l ai.haishui.gateway-<suffix> -- <helper-script>` (or `launchctl bootstrap gui/<uid> <plist>`) creates
    ...
    # Branch C: systemctl ops on a haishui-gateway unit.
...
# gateway is literally `python.exe` / `python3.12` (`python -m haishui_cli.main gateway run`), so
# `taskkill /F /IM python.exe`, `pkill -9 python3` or `killall python` carry no haishui/gateway token
...
# (`python -m haishui_cli.main gateway run` / `haishui gateway run`) only through its own tokens;
# an unrelated script that merely contains "haishui" (`haishui-polis/run.sh`, `my_haishui_bot.py`)
# cannot match it. Same haishui+gateway pairing as Branch D, plus the module path.
_GATEWAY_CMDLINE_TOKEN_RE = re.compile(r"(?i)haishui_cli|\bhaishui\b[^\n]*\bgateway\b|\bgateway\b[^\n]*\bhaishui\b")
...
HOST_INTERPRETER_KILL_REJECTION = (
    ...
    "for a background job Haishui started, or kill/taskkill by its explicit PID."
)
...
def _pattern_reaches_host_interpreter(pattern: str, *, full_cmdline: bool, exact: bool) -> bool:
    ...
    command line). `python -m haishui_cli.main …` is the gateway's own cmdline, so a `-f` pattern
    that names the interpreter and then only wildcards or a `haishui` token reaches it, while
    ...
    return not rest.strip() or bool(_ERE_WILDCARD_ONLY.match(rest)) or "haishui" in rest.lower()
...
# split across continuation lines (e.g. `launchctl submit \` / `  -l ai.haishui.gateway-... \` / `  -- ...`,
...
# Branch A2: `haishui -p <profile> gateway restart|stop` (also `--profile <name>` /
...
# `haishui -p venus gateway restart` operates on a sibling profile's gateway and is a legitimate fleet
...
# `$label` next to the verb. These verbs act on an EXISTING job, so the haishui-gateway label anchor
...
_HAISHUI_GATEWAY_LABEL_RE = re.compile(r"(?i)\bhaishui[.\-]?gateway\b")
...
def _current_profile_name() -> Optional[str]:
    """Profile running the guard: ``HAISHUI_PROFILE_NAME``/``HAISHUI_PROFILE`` env first, then
    ``haishui_cli.profiles.get_active_profile_name`` (from ``HAISHUI_HOME``); ``None`` if neither."""
    for env_name in ("HAISHUI_PROFILE_NAME", "HAISHUI_PROFILE"):
...
def _contains_launchctl_gateway_lifecycle(normalized_text: str) -> bool:
    ...
    return bool(_LAUNCHCTL_LIFECYCLE_VERBS_RE.search(normalized_text)) and bool(
        _HAISHUI_GATEWAY_LABEL_RE.search(normalized_text)
    )
...
def contains_gateway_lifecycle_command(text: str) -> bool:
    ...
    ``haishui``/``gateway`` token) — this function is the single choke point
    ...
    # <<'EOF'`) are masked before scanning (#88336): a runbook line "a human can run: haishui gateway
    ...
    # Profile-flag form (#78028): `haishui -p <profile> gateway restart|stop` bypasses Branch A because the
    # selector sits between `haishui` and `gateway`. It is only the same foot-gun when the named profile IS
    ...
    # Branch E (#113667): killers aimed at the interpreter image itself carry no haishui/gateway token.
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    ``<HAISHUI_HOME>/scripts/``) so the guard scans the file that will actually run."""
    ...
    if raw.is_absolute():
        return raw
    try:
        return get_haishui_home() / "scripts" / raw
    except (RuntimeError, OSError):
        # get_haishui_home() falls back to Path.home(), which raises when neither HAISHUI_HOME nor
...
def check_gateway_lifecycle(prompt: Optional[str], script: Optional[str] = None) -> None:
    ...
    if script:
        ...
        if resolved_script is not None and _on_cloud_path(resolved_script):
            raise GatewayLifecycleBlocked(
                ...
                "(e.g. ~/.haishui/scripts/) and recreate the job."
            )
    ...
    if refusal:
        ...
        # The direct command regex below still scans the full text, so a literal `haishui gateway restart`
    ...
    if unsafe:
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `haishui gateway restart` from a shell outside "
